...
[![npm](https://img.shields.io/npm/v/logan-spine-mcp?style=flat&color=blue)](https://www.npmjs.com/package/logan-spine-mcp)
...
[![License](https://img.shields.io/badge/license-MIT-green)](https://github.com/DeusData/logan-spine-mcp/blob/main/LICENSE)
...
```bash
npm install -g logan-spine-mcp
```

The runtime set for your platform is downloaded automatically at install time. There is one composition per platform and the graph UI is always included — the former `LSM_VARIANT=ui` opt-in is obsolete.
...
```bash
logan-spine-mcp install          # configure all detected coding agents
logan-spine-mcp --version
logan-spine-mcp --help
logan-spine-mcp update           # update to latest release
logan-spine-mcp uninstall        # remove agent configs
```
...
```bash
logan-spine-mcp cli index_repository '{"repo_path": "/path/to/repo"}'
logan-spine-mcp cli search_graph '{"name_pattern": ".*Handler.*", "label": "Function"}'
logan-spine-mcp cli trace_call_path '{"function_name": "main", "direction": "both"}'
logan-spine-mcp cli get_architecture '{}'
```
...
See [github.com/DeusData/logan-spine-mcp](https://github.com/DeusData/logan-spine-mcp) for the full README including all MCP tools, configuration options, graph data model, and language support details.
