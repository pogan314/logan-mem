...
func TestWindowsUsesOneDirectBinary(t *testing.T) {
	...
	if got := binaryNameForOS("windows"); got != "logan-spine-mcp.exe" {
...
func TestMutationSnapshotEnvironmentIgnoresExternalAssetOverrides(t *testing.T) {
	t.Setenv("LSM_ASSETS_DIR", filepath.Join(t.TempDir(), "integrations"))
	t.Setenv("LSM_UI_ASSETS_DIR", filepath.Join(t.TempDir(), "ui"))
	for _, entry := range mutationSnapshotEnvironment() {
		name := entry
		if separator := strings.IndexByte(entry, '='); separator >= 0 {
			name = entry[:separator]
		}
		if strings.EqualFold(name, "LSM_ASSETS_DIR") ||
			strings.EqualFold(name, "LSM_UI_ASSETS_DIR") {
...
func TestMutationSnapshotRejectsExplicitTargetOverlappingCache(t *testing.T) {
	directory := t.TempDir()
	executable := filepath.Join(directory, "logan-spine-mcp")
...
func TestMutationSnapshotCleanupFailurePreservesNativeResult(t *testing.T) {
	const exitHelper = "LSM_GO_MUTATION_EXIT_HELPER"
...
func TestKilledRuntimePublisherIsReconciledByLockedReadiness(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			command.Env = append(
				os.Environ(),
				"LSM_TEST_RUNTIME_CRASH_HELPER=1",
				"LSM_TEST_RUNTIME_CRASH_SOURCE="+source,
				"LSM_TEST_RUNTIME_CRASH_DESTINATION="+destination,
				"LSM_TEST_RUNTIME_CRASH_MARKER="+marker,
				"LSM_TEST_RUNTIME_CRASH_PHASE="+testCase.crashPhase,
			)
...
func TestRuntimePublicationCrashHelper(t *testing.T) {
	if os.Getenv("LSM_TEST_RUNTIME_CRASH_HELPER") != "1" {
		return
	}
	source := os.Getenv("LSM_TEST_RUNTIME_CRASH_SOURCE")
	destination := os.Getenv("LSM_TEST_RUNTIME_CRASH_DESTINATION")
	marker := os.Getenv("LSM_TEST_RUNTIME_CRASH_MARKER")
	crashPhase := os.Getenv("LSM_TEST_RUNTIME_CRASH_PHASE")
	binary := "logan-spine-mcp"
...
func TestRuntimeSetLockSerializesProcesses(t *testing.T) {
	const helperEnv = "LSM_GO_RUNTIME_LOCK_HELPER"
	if os.Getenv(helperEnv) == "1" {
		destination := os.Getenv("LSM_GO_RUNTIME_LOCK_DIRECTORY")
		waitingPath := os.Getenv("LSM_GO_RUNTIME_LOCK_WAITING")
		acquiredPath := os.Getenv("LSM_GO_RUNTIME_LOCK_ACQUIRED")
		var waitOnce sync.Once
		runtimeSetLockWaitObserver = func() {
			waitOnce.Do(func() {
	...
	cmd.Env = append(
		os.Environ(),
		helperEnv+"=1",
		"LSM_GO_RUNTIME_LOCK_DIRECTORY="+destination,
		"LSM_GO_RUNTIME_LOCK_WAITING="+waitingPath,
		"LSM_GO_RUNTIME_LOCK_ACQUIRED="+acquiredPath,
	)
