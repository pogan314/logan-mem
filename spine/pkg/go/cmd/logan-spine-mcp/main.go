// logan-spine-mcp — Go installer wrapper.
//
...
// Install:
//
//	go install github.com/DeusData/logan-spine-mcp/pkg/go/cmd/logan-spine-mcp@latest
package main
...
const (
	...
	version           = "0.8.1"
	windowsBinaryName = "logan-spine-mcp.exe"
	...
	runtimeSetLockName               = ".logan-spine-mcp-runtime.lock"
	...
	runtimeBackupPrefix              = ".lsm-runtime-backup-"
...
func main() {
	...
	if mutation == "update" {
		fmt.Fprintln(
			os.Stderr,
			"This Go wrapper is maintained by Go. Update it with \"go install github.com/DeusData/logan-spine-mcp/pkg/go/cmd/logan-spine-mcp@latest\".",
		)
		os.Exit(2)
	}
	executable, err := ensureBinary()
	if err != nil {
		fmt.Fprintf(os.Stderr, "logan-spine-mcp: %v\n", err)
		os.Exit(1)
	}
...
func printPortableMutationGuidance(args []string) {
	...
	packageCommand := "Remove-Item (Get-Command logan-spine-mcp).Source"
	fmt.Fprintf(
		os.Stderr,
...
func defaultManagedInstallDir() string {
	if runtime.GOOS == "windows" {
		...
		return filepath.Join(base, "Programs", "logan-spine-mcp")
	}
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".local", "bin")
	}
	return filepath.Join(os.TempDir(), "logan-spine-mcp-managed")
}
...
func mutationSnapshotEnvironment() []string {
	blocked := []string{"LSM_ASSETS_DIR", "LSM_UI_ASSETS_DIR"}
...
func cacheDir() string {
	if d := os.Getenv("LSM_CACHE_DIR"); d != "" {
		return d
	}
	switch runtime.GOOS {
	case "windows":
		if d := os.Getenv("LOCALAPPDATA"); d != "" {
			return filepath.Join(d, "logan-spine-mcp")
		}
	case "darwin":
		if home, err := os.UserHomeDir(); err == nil {
			return filepath.Join(home, "Library", "Caches", "logan-spine-mcp")
		}
	}
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cache", "logan-spine-mcp")
	}
	return filepath.Join(os.TempDir(), "logan-spine-mcp")
}
...
func download(dest string) error {
	...
	fmt.Fprintf(os.Stderr, "logan-spine-mcp: downloading v%s for %s/%s...\n", version, platform, arch)

	tmp, err := os.MkdirTemp("", "lsm-install-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmp)

	archivePath := filepath.Join(tmp, "lsm."+ext)
...
func execBinaryWithRuntimeLockAndRunner(
	...
	defer func() {
		...
		if cleanupErr := runtimeMutationSnapshotCleanup(snapshotDirectory); cleanupErr != nil {
			fmt.Fprintf(
				os.Stderr,
				"logan-spine-mcp: warning: private mutation snapshot cleanup failed: %v\n",
				cleanupErr,
			)
		}
	}()
