"""Downloads logan-spine-mcp on first run, then runs its native entry point."""
...
_WINDOWS_BINARY_NAME = "logan-spine-mcp.exe"
_UNIX_ARCHIVE_NAMES = (
    "logan-spine-mcp",
    "LICENSE",
    "install.sh",
...
_RUNTIME_LOCK_NAME = ".logan-spine-mcp-runtime.lock"
...
_RUNTIME_BACKUP_PREFIX = ".lsm-runtime-backup-"
_RUNTIME_BACKUP_RE = re.compile(r"\.lsm-runtime-backup-[0-9a-f]{32}\Z")
...
def _validate_url_scheme(url: str) -> None:
    ...
    if (
        ...
        sys.exit(
            f"logan-spine-mcp: refusing to fetch invalid, credentialed, or "
            f"non-https URL: {url}"
        )
...
def _download_https(url: str, dest: str, max_bytes: int = 0) -> None:
    ...
    for redirect_count in range(_MAX_REDIRECTS + 1):
        ...
        request = urllib.request.Request(
            current_url, headers={"User-Agent": "logan-spine-mcp-installer"}
        )
        try:
...
def _validate_archive_names(names, archive_names, casefold: bool = False):
    ...
    for name in names:
        ...
        if key in seen:
            sys.exit(
                f"logan-spine-mcp: refusing unsafe archive entry "
                f"(duplicate or case conflict: {name!r})"
            )
        seen.add(key)
        if name not in required_set:
            sys.exit(
                f"logan-spine-mcp: archive must contain only the exact "
                f"root files: {', '.join(required)}"
            )
        found.add(name)
    if found != required_set or len(seen) != len(required):
        sys.exit(
            f"logan-spine-mcp: archive must contain exactly one of each "
...
def _safe_extract_tar(
    ...
    for member in members:
        if not member.isfile():
            sys.exit(
                f"logan-spine-mcp: refusing unsafe tar entry "
    ...
    for name in runtime_names:
        ...
        if source is None:
            sys.exit(f"logan-spine-mcp: could not read archive member: {name}")
...
def _safe_extract_zip(
    ...
    for info in zf.infolist():
        ...
        if (
            ...
            sys.exit(
                f"logan-spine-mcp: refusing unsafe zip entry "
...
def _verify_checksum(archive_path: str, archive_name: str, version: str) -> None:
    ...
    try:
        ...
        with open(tmp_path, encoding="utf-8") as f:
            for line in f:
                ...
                if len(digest) != 64 or any(
                    ...
                    sys.exit(
                        f"logan-spine-mcp: invalid SHA256 checksum for "
                        f"{archive_name}"
                    )
                if expected is not None and expected != digest:
                    sys.exit(
                        f"logan-spine-mcp: conflicting SHA256 checksums for "
                        f"{archive_name}"
                    )
                expected = digest
        if expected is None:
            sys.exit(
                f"logan-spine-mcp: no checksum for {archive_name} in checksums.txt"
            )
        h = hashlib.sha256()
        ...
        if expected != actual:
            sys.exit(
                f"logan-spine-mcp: CHECKSUM MISMATCH for {archive_name}\n"
                f"  expected: {expected}\n"
                f"  actual:   {actual}"
            )
        print("logan-spine-mcp: checksum verified.", file=sys.stderr)
    except SystemExit:
        raise
    except Exception as exc:
        sys.exit(f"logan-spine-mcp: checksum verification failed: {exc}")
    finally:
...
def _version() -> str:
    try:
        ...
        return version("logan-spine-mcp")
    except Exception:
        return "0.8.1"
...
def _os_name() -> str:
    ...
    if p == "win32":
        return "windows"
    sys.exit(f"logan-spine-mcp: unsupported platform: {p}")
...
def _arch() -> str:
    ...
    if m in ("x86_64", "amd64"):
        return "amd64"
    sys.exit(f"logan-spine-mcp: unsupported architecture: {m}")
...
def _copy_runtime_backup_file(
    ...
    descriptor, staged_name = tempfile.mkstemp(
        dir=str(target.parent), prefix=".lsm-runtime-recovery-"
    )
...
def _download(version: str) -> Path:
    ...
    archive = f"logan-spine-mcp-{os_name}-{arch}{portable}.{ext}"
    ...
    print(
        f"logan-spine-mcp: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )

    with tempfile.TemporaryDirectory() as tmp:
        tmp_archive = os.path.join(tmp, f"lsm.{ext}")
        try:
            _download_https(url, tmp_archive)
        except (OSError, RuntimeError, urllib.error.URLError) as e:
            sys.exit(
                f"logan-spine-mcp: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = (
            _WINDOWS_BINARY_NAME if os_name == "windows" else "logan-spine-mcp"
        )
        ...
        for name in runtime_names:
            ...
            if not _regular_file(extracted_path):
                sys.exit(
                    f"logan-spine-mcp: required runtime member not found after "
                    f"extraction: {name}"
                )
            if name == bin_name:
        ...
        if _runtime_set_names(Path(tmp), bin_name) is None:
            sys.exit(
                "logan-spine-mcp: extracted runtime set failed content "
                "verification"
            )
        try:
            _verify_candidate(extracted_paths[bin_name])
        except (OSError, RuntimeError) as exc:
            sys.exit(f"logan-spine-mcp: {exc}")
    ...
    if mutation in ("install", "uninstall"):
        ...
        if sys.platform == "win32" and result != 0 and mutation == "uninstall":
            print(
                ...
                '"python -m pip uninstall logan-spine-mcp" for package maintenance. '
...
def _default_managed_install_dir() -> Path:
    ...
    return local_app_data / "Programs" / "logan-spine-mcp"
...
def _run_cache_sensitive_mutation(version: str, executable: Path, args) -> int:
    ...
    try:
        ...
        thread = threading.Thread(
            name="lsm-cache-lock-heartbeat", target=heartbeat, daemon=True
        )
        thread.start()
