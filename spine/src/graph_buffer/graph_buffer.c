 ...
 * The buffer frees everything in lsm_gbuf_free().
 */
...
#define LSM_EDGE_CONF_ABSENT (-1.0)
...
#define EDGE_KEY_BUF LSM_SZ_256

/* Per-type or per-key edge list stored in hash tables as values */
typedef LSM_DYN_ARRAY(const lsm_gbuf_edge_t *) edge_ptr_array_t;

/* Per-label or per-name node list */
typedef LSM_DYN_ARRAY(const lsm_gbuf_node_t *) node_ptr_array_t;
...
struct lsm_gbuf {
    ...
    LSM_DYN_ARRAY(lsm_gbuf_node_t *) nodes;

    /* Primary index: QN → lsm_gbuf_node_t* */
    LSMHashTable *node_by_qn;
    /* Primary index: "id" string → lsm_gbuf_node_t* */
    ...
    lsm_gbuf_node_t **by_id;
    int64_t by_id_cap;

    /* Secondary node indexes */
    LSMHashTable *nodes_by_label; /* key: label, value: (node_ptr_array_t*) */
    LSMHashTable *nodes_by_name;  /* key: name, value: (node_ptr_array_t*) */

    /* Edge storage: array of pointers to individually heap-allocated edges */
    LSM_DYN_ARRAY(lsm_gbuf_edge_t *) edges;

    /* Edge dedup index: "srcID:tgtID:type" → lsm_gbuf_edge_t* */
    LSMHashTable *edge_by_key;

    /* Edge secondary indexes: composite keys → edge_ptr_array_t */
    LSMHashTable *edges_by_source_type; /* "srcID:type" → edge_ptr_array_t* */
    LSMHashTable *edges_by_target_type; /* "tgtID:type" → edge_ptr_array_t* */
    LSMHashTable *edges_by_type;        /* "type" → edge_ptr_array_t* */
     ...
     * freed by free_node_strings/free_edge_strings — only once in lsm_gbuf_free. */
    LSMHashTable *intern_pool;

    /* Vector storage for semantic embeddings (filled by pass_semantic_edges,
     * consumed by lsm_write_db during dump). */
    LSMDumpVector *dump_vectors;
    int dump_vector_count;
    int dump_vector_cap;

    /* Token vector storage for enriched RI vectors (query-time lookup). */
    LSMDumpTokenVec *dump_token_vecs;
...
static const char *gb_intern(lsm_gbuf_t *gb, const char *s) {
    const char *key = s ? s : "{}";
    const char *found = lsm_ht_get(gb->intern_pool, key);
    if (found) {
        return found;
    }
    char *copy = strdup(key);
    if (copy) {
        lsm_ht_set(gb->intern_pool, copy, copy); /* key == value == owned copy */
    }
    return copy;
}
...
static node_ptr_array_t *get_or_create_node_array(LSMHashTable *ht, const char *key) {
    node_ptr_array_t *arr = lsm_ht_get(ht, key);
    if (!arr) {
        arr = calloc(LSM_ALLOC_ONE, sizeof(node_ptr_array_t));
        lsm_ht_set(ht, strdup(key), arr);
    }
    return arr;
}

/* Get or create an edge_ptr_array_t in a hash table */
static edge_ptr_array_t *get_or_create_edge_array(LSMHashTable *ht, const char *key) {
    edge_ptr_array_t *arr = lsm_ht_get(ht, key);
    if (!arr) {
        arr = calloc(LSM_ALLOC_ONE, sizeof(edge_ptr_array_t));
        lsm_ht_set(ht, strdup(key), arr);
    }
    return arr;
}
 ...
 * (pool-owned) — NOT freed here; the pool frees them once in lsm_gbuf_free. */
static void free_node_strings(lsm_gbuf_node_t *n) {
    free(n->name);
 ...
 * freed here; the pool frees it once in lsm_gbuf_free. */
static void free_edge_strings(lsm_gbuf_edge_t *e) {
    free(e->properties_json);
}

/* Allocate the next buffer-local or shared-atomic ID. */
static int64_t alloc_next_id(lsm_gbuf_t *gb) {
    if (gb->shared_ids) {
...
static void unindex_edge(lsm_gbuf_t *gb, const lsm_gbuf_edge_t *e) {
    char key[EDGE_KEY_BUF];

    make_edge_key(key, sizeof(key), e->source_id, e->target_id, e->type, e->properties_json);
    const char *ekey = lsm_ht_get_key(gb->edge_by_key, key);
    lsm_ht_delete(gb->edge_by_key, key);
    free((void *)ekey);

    make_src_type_key(key, sizeof(key), e->source_id, e->type);
    remove_edge_from_ptr_array(lsm_ht_get(gb->edges_by_source_type, key), e->id);

    make_src_type_key(key, sizeof(key), e->target_id, e->type);
    remove_edge_from_ptr_array(lsm_ht_get(gb->edges_by_target_type, key), e->id);

    remove_edge_from_ptr_array(lsm_ht_get(gb->edges_by_type, e->type), e->id);
}

/* Cascade-delete all edges touching nodes in deleted_set. */
static void cascade_delete_edges(lsm_gbuf_t *gb, LSMHashTable *deleted_set) {
    int write_idx = 0;
    for (int i = 0; i < gb->edges.count; i++) {
        lsm_gbuf_edge_t *e = gb->edges.items[i];
        char src_id[LSM_SZ_32];
        char tgt_id[LSM_SZ_32];
        make_id_key(src_id, sizeof(src_id), e->source_id);
        make_id_key(tgt_id, sizeof(tgt_id), e->target_id);

        if (lsm_ht_get(deleted_set, src_id) || lsm_ht_get(deleted_set, tgt_id)) {
            unindex_edge(gb, e);
            free_edge_strings(e);
            free(e);
        } else {
...
static void register_node_in_indexes(lsm_gbuf_t *gb, lsm_gbuf_node_t *node) {
    lsm_ht_set(gb->node_by_qn, node->qualified_name, node);

    if (node->id >= gb->by_id_cap) {
        int64_t nc = gb->by_id_cap > 0 ? gb->by_id_cap : LSM_SZ_1K;
        while (nc <= node->id) {
            nc *= 2;
        }
        lsm_gbuf_node_t **grown = realloc(gb->by_id, (size_t)nc * sizeof(*grown));
        if (grown) {
    ...
    lsm_da_push(by_label, (const lsm_gbuf_node_t *)node);

    node_ptr_array_t *by_name =
        get_or_create_node_array(gb->nodes_by_name, node->name ? node->name : "");
    lsm_da_push(by_name, (const lsm_gbuf_node_t *)node);
}

/* Push an edge pointer into a dynamic array (wraps macro to reduce CC contribution). */
static void edge_array_push(edge_ptr_array_t *arr, const lsm_gbuf_edge_t *edge) {
    lsm_da_push(arr, edge);
}

/* Index an edge by one key into a hash table bucket. */
static void index_edge_by_key(LSMHashTable *ht, const char *key, lsm_gbuf_edge_t *edge) {
    edge_ptr_array_t *arr = get_or_create_edge_array(ht, key);
    edge_array_push(arr, (const lsm_gbuf_edge_t *)edge);
}

/* Register an edge in secondary indexes (source_type, target_type, type). */
static void register_edge_in_indexes(lsm_gbuf_t *gb, lsm_gbuf_edge_t *edge) {
    char key[EDGE_KEY_BUF];
...
static void rebuild_edge_secondary_indexes(lsm_gbuf_t *gb) {
    lsm_ht_foreach(gb->edges_by_source_type, free_edge_array, NULL);
    lsm_ht_free(gb->edges_by_source_type);
    lsm_ht_foreach(gb->edges_by_target_type, free_edge_array, NULL);
    lsm_ht_free(gb->edges_by_target_type);
    lsm_ht_foreach(gb->edges_by_type, free_edge_array, NULL);
    lsm_ht_free(gb->edges_by_type);

    gb->edges_by_source_type = lsm_ht_create(LSM_SZ_256);
    gb->edges_by_target_type = lsm_ht_create(LSM_SZ_256);
    gb->edges_by_type = lsm_ht_create(LSM_SZ_32);
...
static void release_gbuf_indexes(lsm_gbuf_t *gb) {
    lsm_ht_free(gb->node_by_qn);
    gb->node_by_qn = NULL;
    free(gb->by_id);
    gb->by_id = NULL;
    gb->by_id_cap = 0;
    lsm_ht_foreach(gb->nodes_by_label, free_node_array, NULL);
    lsm_ht_free(gb->nodes_by_label);
    gb->nodes_by_label = NULL;
    lsm_ht_foreach(gb->nodes_by_name, free_node_array, NULL);
    lsm_ht_free(gb->nodes_by_name);
    gb->nodes_by_name = NULL;
    lsm_ht_foreach(gb->edge_by_key, free_key_only, NULL);
    lsm_ht_free(gb->edge_by_key);
    gb->edge_by_key = NULL;
    lsm_ht_foreach(gb->edges_by_source_type, free_edge_array, NULL);
    lsm_ht_free(gb->edges_by_source_type);
...
lsm_gbuf_t *lsm_gbuf_new(const char *project, const char *root_path) {
    lsm_gbuf_t *gb = calloc(LSM_ALLOC_ONE, sizeof(lsm_gbuf_t));
    if (!gb) {
        return NULL;
    }
    ...
    gb->node_by_qn = lsm_ht_create(LSM_SZ_256);
    gb->by_id = NULL;
    gb->by_id_cap = 0;
    gb->nodes_by_label = lsm_ht_create(LSM_SZ_32);
    gb->nodes_by_name = lsm_ht_create(LSM_SZ_256);

    gb->edge_by_key = lsm_ht_create(LSM_SZ_512);
    gb->edges_by_source_type = lsm_ht_create(LSM_SZ_256);
    gb->edges_by_target_type = lsm_ht_create(LSM_SZ_256);
    gb->edges_by_type = lsm_ht_create(LSM_SZ_32);

    gb->intern_pool = lsm_ht_create(LSM_SZ_1K);

    return gb;
}

lsm_gbuf_t *lsm_gbuf_new_shared_ids(const char *project, const char *root_path,
                                    _Atomic int64_t *id_source) {
    lsm_gbuf_t *gb = lsm_gbuf_new(project, root_path);
    if (gb && id_source) {
...
void lsm_gbuf_free(lsm_gbuf_t *gb) {
    if (!gb) {
        return;
    }
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        lsm_gbuf_node_t *n = gb->nodes.items[i];
        free_node_strings(n);
        free(n);
    }
    lsm_da_free(&gb->nodes);
    ...
    if (gb->intern_pool) {
        lsm_ht_foreach(gb->intern_pool, free_key_only, NULL);
        lsm_ht_free(gb->intern_pool);
    }
...
int lsm_gbuf_store_vector(lsm_gbuf_t *gb, int64_t node_id, const uint8_t *vector, int vector_len) {
    ...
    if (gb->dump_vector_count >= gb->dump_vector_cap) {
        ...
        LSMDumpVector *grown = realloc(gb->dump_vectors, (size_t)new_cap * sizeof(LSMDumpVector));
        if (!grown) {
            return GB_ERR;
        }
    ...
    gb->dump_vectors[gb->dump_vector_count++] = (LSMDumpVector){
        .node_id = node_id,
...
int lsm_gbuf_store_token_vector(lsm_gbuf_t *gb, const char *token, const uint8_t *vector,
    ...
    if (gb->dump_token_vec_count >= gb->dump_token_vec_cap) {
        ...
        LSMDumpTokenVec *grown =
            realloc(gb->dump_token_vecs, (size_t)new_cap * sizeof(LSMDumpTokenVec));
        if (!grown) {
            return GB_ERR;
        }
    ...
    gb->dump_token_vecs[idx] = (LSMDumpTokenVec){
...
int64_t lsm_gbuf_next_id(const lsm_gbuf_t *gb) {
    if (!gb) {
        return SKIP_ONE;
    }
    if (gb->shared_ids) {
...
void lsm_gbuf_set_next_id(lsm_gbuf_t *gb, int64_t next_id) {
    if (!gb) {
        return;
    }
    gb->next_id = next_id;
...
int64_t lsm_gbuf_upsert_node(lsm_gbuf_t *gb, const char *label, const char *name,
    ...
    lsm_gbuf_node_t *existing = lsm_ht_get(gb->node_by_qn, qualified_name);
    if (existing) {
         ...
         * label/name — lsm_gbuf_find_by_label/name then missed or mis-listed it,
        ...
        if (label_changed) {
            remove_node_from_ptr_array(
                lsm_ht_get(gb->nodes_by_label, existing->label ? existing->label : ""),
                existing->id);
        }
        if (name_changed) {
            remove_node_from_ptr_array(
                lsm_ht_get(gb->nodes_by_name, existing->name ? existing->name : ""), existing->id);
        }
        ...
        if (label_changed) {
            ...
            lsm_da_push(by_label, (const lsm_gbuf_node_t *)existing);
        }
        if (name_changed) {
            node_ptr_array_t *by_name =
                get_or_create_node_array(gb->nodes_by_name, existing->name ? existing->name : "");
            lsm_da_push(by_name, (const lsm_gbuf_node_t *)existing);
        }
        return existing->id;
    }

    /* Heap-allocate a new node (pointer stays stable across array growth) */
    lsm_gbuf_node_t *node = calloc(LSM_ALLOC_ONE, sizeof(lsm_gbuf_node_t));
    if (!node) {
        return 0;
    }
    ...
    lsm_da_push(&gb->nodes, node);
    register_node_in_indexes(gb, node);

    return id;
}

const lsm_gbuf_node_t *lsm_gbuf_find_by_qn(const lsm_gbuf_t *gb, const char *qn) {
    if (!gb || !qn) {
        return NULL;
    }
    return lsm_ht_get(gb->node_by_qn, qn);
}

const lsm_gbuf_node_t *lsm_gbuf_find_by_id(const lsm_gbuf_t *gb, int64_t id) {
...
int lsm_gbuf_find_by_label(const lsm_gbuf_t *gb, const char *label, const lsm_gbuf_node_t ***out,
                           int *count) {
    if (!gb || !out || !count) {
        return LSM_NOT_FOUND;
    }
    node_ptr_array_t *arr = lsm_ht_get(gb->nodes_by_label, label ? label : "");
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int lsm_gbuf_find_by_name(const lsm_gbuf_t *gb, const char *name, const lsm_gbuf_node_t ***out,
                          int *count) {
    if (!gb || !out || !count) {
        return LSM_NOT_FOUND;
    }
    node_ptr_array_t *arr = lsm_ht_get(gb->nodes_by_name, name ? name : "");
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int lsm_gbuf_node_count(const lsm_gbuf_t *gb) {
    /* Use QN hash table count since it's authoritative (handles deletes) */
    return gb ? (int)lsm_ht_count(gb->node_by_qn) : 0;
}

int lsm_gbuf_delete_by_label(lsm_gbuf_t *gb, const char *label) {
    if (!gb || !label) {
        return LSM_NOT_FOUND;
    }

    node_ptr_array_t *arr = lsm_ht_get(gb->nodes_by_label, label);
    if (!arr || arr->count == 0) {
        return 0;
    }

    /* Build hash set of deleted node IDs for O(1) lookup */
    LSMHashTable *deleted_set = lsm_ht_create(arr->count);
    for (int i = 0; i < arr->count; i++) {
        const lsm_gbuf_node_t *n = arr->items[i];

        char id_buf[LSM_SZ_32];
        make_id_key(id_buf, sizeof(id_buf), n->id);
        lsm_ht_set(deleted_set, strdup(id_buf), intptr_to_ptr(SKIP_ONE));

        /* Remove from primary indexes */
        lsm_ht_delete(gb->node_by_qn, n->qualified_name);
    ...
    lsm_ht_foreach(deleted_set, free_key_only, NULL);
    lsm_ht_free(deleted_set);
    return 0;
}

int lsm_gbuf_delete_by_file(lsm_gbuf_t *gb, const char *file_path) {
    if (!gb || !file_path) {
        return LSM_NOT_FOUND;
    }

    /* Collect IDs of nodes in this file */
    LSMHashTable *deleted_set = lsm_ht_create(LSM_SZ_64);
    int deleted_count = 0;
    int scanned = 0;
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        ...
        if (!n->qualified_name || !lsm_ht_get(gb->node_by_qn, n->qualified_name)) {
            continue;
        }
        ...
        remove_node_from_ptr_array(lsm_ht_get(gb->nodes_by_label, n->label), n->id);
        remove_node_from_ptr_array(lsm_ht_get(gb->nodes_by_name, n->name), n->id);
        ...
        /* NULL out QN so dump's liveness check (lsm_ht_get by QN) fails
    ...
    lsm_ht_free(deleted_set);
    {
        char s_buf[LSM_SZ_16];
        char d_buf[LSM_SZ_16];
        snprintf(s_buf, sizeof(s_buf), "%d", scanned);
        snprintf(d_buf, sizeof(d_buf), "%d", deleted_count);
        lsm_log_info("gbuf.delete_by_file", "file", file_path, "scanned", s_buf, "deleted", d_buf);
    }
    return deleted_count;
}

int lsm_gbuf_load_from_db(lsm_gbuf_t *gb, const char *db_path, const char *project) {
    ...
    if (sqlite3_prepare_v2(db, "SELECT MAX(id) FROM nodes WHERE project = ?", LSM_NOT_FOUND, &stmt,
                           NULL) != SQLITE_OK) {
        lsm_store_close(store);
        return LSM_NOT_FOUND;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        int64_t new_id = lsm_gbuf_upsert_node(gb, label, name, qn, fp, sl, el, props);
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (new_src > 0 && new_tgt > 0) {
            lsm_gbuf_insert_edge(gb, new_src, new_tgt, type, props);
        }
    }
    sqlite3_finalize(stmt);
...
void lsm_gbuf_foreach_node(const lsm_gbuf_t *gb, lsm_gbuf_node_visitor_fn fn, void *userdata) {
    if (!gb || !fn) {
        return;
    }
    for (int i = 0; i < gb->nodes.count; i++) {
        const lsm_gbuf_node_t *n = gb->nodes.items[i];
        if (n->qualified_name && lsm_ht_get(gb->node_by_qn, n->qualified_name)) {
            fn(n, userdata);
        }
    }
...
void lsm_gbuf_foreach_edge(const lsm_gbuf_t *gb, lsm_gbuf_edge_visitor_fn fn, void *userdata) {
    if (!gb || !fn) {
        return;
    }
    for (int i = 0; i < gb->edges.count; i++) {
...
int64_t lsm_gbuf_insert_edge(lsm_gbuf_t *gb, int64_t source_id, int64_t target_id, const char *type,
                             const char *properties_json) {
    if (!gb || !type) {
        return 0;
    }
    ...
    lsm_gbuf_edge_t *existing = lsm_ht_get(gb->edge_by_key, key);
    if (existing) {
    ...
    lsm_gbuf_edge_t *edge = calloc(LSM_ALLOC_ONE, sizeof(lsm_gbuf_edge_t));
    if (!edge) {
        return 0;
    }
    ...
    lsm_da_push(&gb->edges, edge);

    /* Dedup index */
    lsm_ht_set(gb->edge_by_key, strdup(key), edge);
...
int lsm_gbuf_find_edges_by_source_type(const lsm_gbuf_t *gb, int64_t source_id, const char *type,
                                       const lsm_gbuf_edge_t ***out, int *count) {
    if (!gb || !out || !count) {
        return LSM_NOT_FOUND;
    }
    char key[EDGE_KEY_BUF];
    make_src_type_key(key, sizeof(key), source_id, type);
    edge_ptr_array_t *arr = lsm_ht_get(gb->edges_by_source_type, key);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int lsm_gbuf_find_edges_by_target_type(const lsm_gbuf_t *gb, int64_t target_id, const char *type,
                                       const lsm_gbuf_edge_t ***out, int *count) {
    if (!gb || !out || !count) {
        return LSM_NOT_FOUND;
    }
    ...
    edge_ptr_array_t *arr = lsm_ht_get(gb->edges_by_target_type, key);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int lsm_gbuf_find_edges_by_type(const lsm_gbuf_t *gb, const char *type,
                                const lsm_gbuf_edge_t ***out, int *count) {
    if (!gb || !out || !count) {
        return LSM_NOT_FOUND;
    }
    edge_ptr_array_t *arr = lsm_ht_get(gb->edges_by_type, type);
    if (arr && arr->count > 0) {
        *out = arr->items;
        *count = arr->count;
    } else {
...
int lsm_gbuf_edge_count(const lsm_gbuf_t *gb) {
    return gb ? gb->edges.count : 0;
}

int lsm_gbuf_edge_count_by_type(const lsm_gbuf_t *gb, const char *type) {
    if (!gb || !type) {
        return 0;
    }
    edge_ptr_array_t *arr = lsm_ht_get(gb->edges_by_type, type);
...
int lsm_gbuf_delete_edges_by_type(lsm_gbuf_t *gb, const char *type) {
    if (!gb || !type) {
        return LSM_NOT_FOUND;
    }
...
static void merge_update_existing(lsm_gbuf_t *dst, lsm_gbuf_node_t *existing,
                                  const lsm_gbuf_node_t *sn, LSMHashTable **remap) {
    /* Same guard as lsm_gbuf_upsert_node: a per-file "Module" def coming from a
    ...
    if (!module_on_container) {
         ...
         * lsm_gbuf_upsert_node exactly. Distinct source entities can share a
        ...
        if (sn_wins) {
            ...
            if (label_changed) {
                remove_node_from_ptr_array(
                    lsm_ht_get(dst->nodes_by_label, existing->label ? existing->label : ""),
                    existing->id);
            }
            if (name_changed) {
                remove_node_from_ptr_array(
                    lsm_ht_get(dst->nodes_by_name, existing->name ? existing->name : ""),
                    existing->id);
            }
    ...
    if (sn->id != existing->id) {
        if (!*remap) {
            *remap = lsm_ht_create(LSM_SZ_32);
        }
        char key[LSM_SZ_32];
        make_id_key(key, sizeof(key), sn->id);
        int64_t *val = malloc(sizeof(int64_t));
        *val = existing->id;
        lsm_ht_set(*remap, strdup(key), val);
    }
}

/* Copy a non-colliding src node into dst with its original ID. */
static void merge_copy_new_node(lsm_gbuf_t *dst, const lsm_gbuf_node_t *sn) {
    lsm_gbuf_node_t *node = calloc(LSM_ALLOC_ONE, sizeof(lsm_gbuf_node_t));
    if (!node) {
        return;
    }
...
static void merge_remap_edges(lsm_gbuf_t *dst, lsm_gbuf_t *src, LSMHashTable *remap) {
    for (int i = 0; i < src->edges.count; i++) {
        lsm_gbuf_edge_t *se = src->edges.items[i];
        ...
        if (remap) {
            ...
            int64_t *remapped = lsm_ht_get(remap, key);
            if (remapped) {
                new_src = *remapped;
            }

            make_id_key(key, sizeof(key), se->target_id);
            remapped = lsm_ht_get(remap, key);
            if (remapped) {
                new_tgt = *remapped;
            }
        }

        lsm_gbuf_insert_edge(dst, new_src, new_tgt, se->type, se->properties_json);
    }
}

int lsm_gbuf_merge(lsm_gbuf_t *dst, lsm_gbuf_t *src) {
    if (!dst || !src) {
        return LSM_NOT_FOUND;
    }
    ...
    LSMHashTable *remap = NULL;

    for (int i = 0; i < src->nodes.count; i++) {
        lsm_gbuf_node_t *sn = src->nodes.items[i];
        if (!sn->qualified_name) {
            continue;
        }

        /* Skip nodes deleted from QN index */
        if (!lsm_ht_get(src->node_by_qn, sn->qualified_name)) {
            continue;
        }

        lsm_gbuf_node_t *existing = lsm_ht_get(dst->node_by_qn, sn->qualified_name);
        if (existing) {
    ...
    if (remap) {
        lsm_ht_foreach(remap, free_remap_entry, NULL);
        lsm_ht_free(remap);
    }
...
static int cmp_dump_vectors_by_id(const void *a, const void *b) {
    int64_t da = ((const LSMDumpVector *)a)->node_id;
    int64_t db = ((const LSMDumpVector *)b)->node_id;
    return (da > db) - (da < db);
}

static LSMDumpNode *build_dump_nodes(lsm_gbuf_t *gb, int live_count, int64_t *temp_to_final,
                                     int64_t max_temp_id, int *out_count,
                                     lsm_gbuf_node_t ***src_out) {
    size_t cap = (size_t)(live_count > 0 ? live_count : SKIP_ONE);
    LSMDumpNode *dump_nodes = malloc(cap * sizeof(LSMDumpNode));
    ...
    lsm_gbuf_node_t **src = malloc(cap * sizeof(lsm_gbuf_node_t *));
    int idx = 0;
...
static LSMDumpEdge *build_dump_edges(lsm_gbuf_t *gb, const int64_t *temp_to_final,
    ...
    LSMDumpEdge *dump_edges =
        malloc((size_t)(valid_edges > 0 ? valid_edges : SKIP_ONE) * sizeof(LSMDumpEdge));
...
static void remap_sort_dedup_vectors(lsm_gbuf_t *gb, const int64_t *temp_to_final,
                                     int64_t max_temp_id) {
    int remapped = 0;
    int dropped = 0;
    for (int i = 0; i < gb->dump_vector_count; i++) {
    ...
    if (dropped > 0) {
        char r_buf[LSM_SZ_16];
        char d_buf[LSM_SZ_16];
        snprintf(r_buf, sizeof(r_buf), "%d", remapped);
        snprintf(d_buf, sizeof(d_buf), "%d", dropped);
        lsm_log_info("dump.vectors.remap", "remapped", r_buf, "dropped", d_buf);
    }
    gb->dump_vector_count = remapped;

    if (gb->dump_vector_count >= GB_MIN_FOR_DEDUP) {
        qsort(gb->dump_vectors, (size_t)gb->dump_vector_count, sizeof(LSMDumpVector),
...
static void log_dump_summary(int node_count, int edge_count) {
    ...
    lsm_log_info("gbuf.dump", "nodes", b1, "edges", b2);
}

static void free_dump_resources(char **url_paths, char **local_names, int edge_count,
                                LSMDumpEdge *dump_edges, LSMDumpNode *dump_nodes,
...
static int count_live_nodes(lsm_gbuf_t *gb) {
    int count = 0;
    for (int i = 0; i < gb->nodes.count; i++) {
        lsm_gbuf_node_t *n = gb->nodes.items[i];
        if (n->qualified_name && lsm_ht_get(gb->node_by_qn, n->qualified_name)) {
...
static void generate_iso_timestamp(char *buf, size_t buf_size) {
    ...
    struct tm tm_buf;
    struct tm *tm_val = lsm_gmtime_r(&now, &tm_buf);
...
static void release_and_remap_vectors(lsm_gbuf_t *gb, const int64_t *temp_to_final,
                                      int64_t max_temp_id) {
    LSM_PROF_START(t_vec_remap);
    remap_sort_dedup_vectors(gb, temp_to_final, max_temp_id);
    LSM_PROF_END_N("dump", "5_vector_remap_sort", t_vec_remap, gb->dump_vector_count);
}

int lsm_gbuf_dump_to_sqlite(lsm_gbuf_t *gb, const char *path) {
    if (!gb || !path) {
        return LSM_NOT_FOUND;
    }

    LSM_PROF_START(t_count);
    int live_count = count_live_nodes(gb);
    LSM_PROF_END_N("dump", "1_count_live_nodes", t_count, live_count);

    LSM_PROF_START(t_build_nodes);
    ...
    int node_idx = 0;
    lsm_gbuf_node_t **src_nodes = NULL;
    LSMDumpNode *dump_nodes =
        build_dump_nodes(gb, live_count, temp_to_final, max_temp_id, &node_idx, &src_nodes);
    LSM_PROF_END_N("dump", "2_build_dump_nodes", t_build_nodes, node_idx);
    ...
    LSM_PROF_START(t_release_idx);
    release_gbuf_indexes(gb);
    LSM_PROF_END("dump", "4_release_gbuf_indexes", t_release_idx);

    char indexed_at[LSM_SZ_64];
    ...
    for (int off = 0; off < node_idx; off += DUMP_PARTITION_NODES) {
        ...
        rc = lsm_writer_append_nodes(w, &dump_nodes[off], chunk);
        if (rc != 0) {
            break;
        }
        free_heavy = free_heavy || (lsm_mem_budget() > 0 && lsm_mem_over_budget());
    ...
    LSM_PROF_END_N("dump", "2b_stream_append_nodes", t_append, node_idx);

    int edge_idx = 0;
    char **url_paths = NULL;
    char **local_names = NULL;
    LSMDumpEdge *dump_edges = NULL;
    if (rc == 0) {
        LSM_PROF_START(t_build_edges);
        dump_edges =
            build_dump_edges(gb, temp_to_final, max_temp_id, &edge_idx, &url_paths, &local_names);
        LSM_PROF_END_N("dump", "3_build_dump_edges", t_build_edges, edge_idx);
    ...
    LSM_PROF_START(t_finalize);
    int frc = lsm_writer_finalize(w, gb->project, gb->root_path, indexed_at, dump_nodes, node_idx,
                                  dump_edges, edge_idx, gb->dump_vectors, gb->dump_vector_count,
                                  gb->dump_token_vecs, gb->dump_token_vec_count);
    LSM_PROF_END_N("dump", "6_write_db_finalize", t_finalize, node_idx + edge_idx);
    if (rc == 0) {
        rc = frc;
    }
...
int lsm_gbuf_flush_to_store(lsm_gbuf_t *gb, lsm_store_t *store) {
    if (!gb || !store) {
        return LSM_NOT_FOUND;
    }

    /* Upsert project */
    lsm_store_upsert_project(store, gb->project, gb->root_path);

    /* Begin bulk mode */
    lsm_store_begin_bulk(store);
    lsm_store_drop_indexes(store);
    lsm_store_begin(store);

    /* Delete existing project data */
    lsm_store_delete_edges_by_project(store, gb->project);
    lsm_store_delete_nodes_by_project(store, gb->project);
    ...
    for (int i = 0; i < gb->nodes.count; i++) {
        ...
        int64_t real_id = lsm_store_upsert_node(store, &sn);
    ...
    for (int i = 0; i < gb->edges.count; i++) {
        ...
        lsm_store_insert_edge(store, &se);
    }

    lsm_store_commit(store);
    lsm_store_create_indexes(store);
    lsm_store_end_bulk(store);

    free(temp_to_real);
    return 0;
}

int lsm_gbuf_merge_into_store(lsm_gbuf_t *gb, lsm_store_t *store) {
    if (!gb || !store) {
        return LSM_NOT_FOUND;
    }
