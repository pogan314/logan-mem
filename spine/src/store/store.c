 ...
 *
 * Implements the opaque lsm_store_t handle with prepared statement caching,
...
enum {
     ...
     * LSM_SZ_1K; percent-encoding can triple it, plus the "file://" prefix,
...
static void store_set_error(lsm_store_t *s, const char *msg) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s", msg);
}

static void store_set_error_sqlite(lsm_store_t *s, const char *prefix) {
    snprintf(s->errbuf, sizeof(s->errbuf), "%s: %s", prefix, sqlite3_errmsg(s->db));
}

static int exec_sql(lsm_store_t *s, const char *sql) {
    if (!s || !s->db) {
...
static sqlite3_stmt *prepare_cached(lsm_store_t *s, sqlite3_stmt **slot, const char *sql) {
    if (!s || !s->db) {
        return NULL;
    }
    if (*slot) {
    ...
    int rc = sqlite3_prepare_v2(s->db, sql, LSM_NOT_FOUND, slot, NULL);
    if (rc != SQLITE_OK) {
...
static int init_schema(lsm_store_t *s) {
    const char *ddl =
         ...
         * built sqlite_autoindex_edges_1 (internal/lsm/sqlite_writer.c) —
     ...
     * the column — the widened upsert in lsm_store_insert_edge can neither
     ...
     * reindex). Read-only query opens skip init_schema and keep working. */
    {
        ...
        if (sqlite3_prepare_v2(s->db, "SELECT local_name_gen FROM edges LIMIT 0;", LSM_NOT_FOUND,
                               &probe, NULL) != SQLITE_OK) {
            lsm_log_warn("store.schema", "result", "incompatible", "missing",
     ...
     * because we feed it `lsm_camel_split(name)` at insert time but want
...
static int create_user_indexes(lsm_store_t *s) {
    const char *sql =
...
int64_t lsm_store_resolve_mmap_size(void) {
    ...
    char buf[ST_BUF_64];
    if (lsm_safe_getenv("LSM_SQLITE_MMAP_SIZE", buf, sizeof(buf), NULL) == NULL) {
...
static int configure_pragmas(lsm_store_t *s, bool in_memory, bool read_only) {
    int rc;
    ...
    if (in_memory) {
        ...
        snprintf(mmap_sql, sizeof(mmap_sql), "PRAGMA mmap_size = %lld;",
                 (long long)lsm_store_resolve_mmap_size());
         ...
         * lsm_store_checkpoint's SIGBUS note), so without a size limit the -wal
...
#define LSM_STORE_DENOM_EPS_D 1e-10
#define LSM_STORE_DENOM_EPS_F 1e-10F
#define LSM_STORE_INT8_MAX 127.0F
#define LSM_STORE_UNIT_POS_F 1.0F
#define LSM_STORE_UNIT_POS_D 1.0
...
static sqlite3_destructor_type lsm_sqlite_transient_destructor(void) {
    static const volatile intptr_t raw = -1;
    sqlite3_destructor_type dtor = NULL;
    memcpy(&dtor, (const void *)&raw, sizeof(dtor));
    return dtor;
}
#define LSM_SQLITE_TRANSIENT (lsm_sqlite_transient_destructor())
...
static void sqlite_camel_split(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!input || !input[0]) {
        sqlite3_result_text(ctx, input ? input : "", SQLITE_AUTO_LEN, LSM_SQLITE_TRANSIENT);
        return;
    }
    ...
    if (len < 0 || len >= (int)sizeof(buf)) {
        ...
        sqlite3_result_text(ctx, input, SQLITE_AUTO_LEN, LSM_SQLITE_TRANSIENT);
        return;
    }
    ...
    buf[len] = '\0';
    sqlite3_result_text(ctx, buf, len, LSM_SQLITE_TRANSIENT);
}
 ...
 * value), so lsm_regcomp is called exactly once per statement instead of once per row. */
static void sqlite_regexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    lsm_regex_t *re = (lsm_regex_t *)sqlite3_get_auxdata(ctx, 0);
    if (!re) {
        re = malloc(sizeof(lsm_regex_t));
        if (!re) {
            sqlite3_result_error_nomem(ctx);
            return;
        }
        if (lsm_regcomp(re, pattern, LSM_REG_EXTENDED | LSM_REG_NOSUB) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", LSM_NOT_FOUND);
            return;
        }
        sqlite3_set_auxdata(ctx, 0, re, regex_free_cb);
    }

    sqlite3_result_int(ctx, lsm_regexec(re, text, 0, NULL, 0) == 0 ? SKIP_ONE : 0);
}
...
static void sqlite_iregexp(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    if (!re) {
        ...
        if (lsm_regcomp(re, pattern, LSM_REG_EXTENDED | LSM_REG_NOSUB | LSM_REG_ICASE) != 0) {
            free(re);
            sqlite3_result_error(ctx, "invalid regex", LSM_NOT_FOUND);
            return;
        }
...
static void sqlite_cosine_i8(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
    ...
    sqlite3_result_double(ctx, denom > LSM_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0);
}
...
static lsm_store_t *store_open_internal(const char *path, bool in_memory, bool create) {
    lsm_store_t *s = calloc(LSM_ALLOC_ONE, sizeof(lsm_store_t));
    if (!s) {
        return NULL;
    }
    ...
    if (path && !in_memory) {
        if (!lsm_path_for_file_api(path, open_path, sizeof(open_path))) {
            free(s);
            return NULL;
        }
    ...
    sqlite3_create_function(s->db, "lsm_cosine_i8", ST_COL_2, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_cosine_i8, NULL, NULL);
    /* camelCase splitter for FTS5 BM25 indexing */
    sqlite3_create_function(s->db, "lsm_camel_split", SKIP_ONE, SQLITE_UTF8 | SQLITE_DETERMINISTIC,
                            NULL, sqlite_camel_split, NULL, NULL);

    if (configure_pragmas(s, in_memory, false) != LSM_STORE_OK || init_schema(s) != LSM_STORE_OK ||
        create_user_indexes(s) != LSM_STORE_OK) {
        sqlite3_close(s->db);
...
lsm_store_t *lsm_store_open_path_query(const char *db_path) {
    ...
    char open_path[4096];
    if (!lsm_path_for_file_api(db_path, open_path, sizeof(open_path))) {
        free(s);
        return NULL;
    }
    ...
    if (configure_pragmas(s, false, true) != LSM_STORE_OK) {
        sqlite3_close(s->db);
        safe_str_free(&s->db_path);
        free(s);
        return NULL;
...
bool lsm_store_check_integrity_deep(lsm_store_t *s) {
    if (!lsm_store_check_integrity(s)) {
        return false;
    }
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, "PRAGMA quick_check(1);", LSM_NOT_FOUND, &stmt, NULL) !=
        SQLITE_OK) {
        return false;
    }
    bool ok = false;
 ...
 * lsm_store_check_integrity() treats any prepare failure as "corrupt". This
...
lsm_integrity_verdict_t lsm_store_check_integrity_verdict(lsm_store_t *s) {
    if (!s || !s->db) {
        ...
        return LSM_INTEGRITY_TRANSIENT;
    }

    /* ── Shallow check (projects table shape + root_path sanity) ── */
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, "SELECT count(*) FROM projects;", LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        ...
        return st_rc_is_transient(rc) ? LSM_INTEGRITY_TRANSIENT : LSM_INTEGRITY_CORRUPT;
    }
    lsm_integrity_verdict_t verdict = LSM_INTEGRITY_OK;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        int row_count = sqlite3_column_int(stmt, 0);
        if (row_count > ST_MAX_ROW_CHECK) {
            (void)fprintf(stderr, "ERROR store.corrupt table=projects rows=%d (expected 1)\n",
                          row_count);
            verdict = LSM_INTEGRITY_CORRUPT;
        }
    } else {
        ...
        verdict = st_rc_is_transient(step_rc) ? LSM_INTEGRITY_TRANSIENT : LSM_INTEGRITY_CORRUPT;
    }
    sqlite3_finalize(stmt);
    if (verdict != LSM_INTEGRITY_OK) {
        return verdict;
    }

    /* root_path sanity — same logic as lsm_store_check_integrity, but classify
    ...
    rc = sqlite3_prepare_v2(s->db, "PRAGMA quick_check(1);", LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return st_rc_is_transient(rc) ? LSM_INTEGRITY_TRANSIENT : LSM_INTEGRITY_CORRUPT;
    }
    if (sqlite3_step(stmt) == SQLITE_ROW) {
...
sqlite3 *lsm_store_get_db(lsm_store_t *s) {
    return s ? s->db : NULL;
}
...
int lsm_store_begin_bulk(lsm_store_t *s) {
    ...
    return exec_sql(s, "PRAGMA cache_size = -65536;"); /* LSM_SZ_64 MB */
}
...
int lsm_store_checkpoint(lsm_store_t *s) {
    ...
    if (wal_frames >= ST_WAL_STARVE_WARN_FRAMES && checkpointed < wal_frames) {
        ...
        lsm_log_warn("store.wal.starving", "wal_frames", frames_buf, "checkpointed", ckpt_buf,
                     "hint",
...
static int prepare_sqlite_for_publish(sqlite3 *db) {
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(db, "PRAGMA journal_mode=DELETE;", LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        return LSM_STORE_ERR;
    }
    ...
    sqlite3_finalize(stmt);
    return delete_mode ? LSM_STORE_OK : LSM_STORE_ERR;
}
...
int lsm_store_prepare_path_for_replace(const char *path) {
    ...
    if (!lsm_path_for_file_api(path, prepare_path, sizeof(prepare_path))) {
        return LSM_STORE_ERR;
    }
    sqlite3 *db = NULL;
...
int lsm_store_backup_path(const char *source_path, const char *staging_path) {
    ...
    if (lsm_remove_db_sidecars(staging_path) != 0) {
        return LSM_STORE_ERR;
    }

    char source_api[4096];
    char staging_api[4096];
    if (!lsm_path_for_file_api(source_path, source_api, sizeof(source_api)) ||
        !lsm_path_for_file_api(staging_path, staging_api, sizeof(staging_api))) {
        return LSM_STORE_ERR;
    }
    sqlite3 *source = NULL;
    sqlite3 *dest = NULL;
    ...
    int result = LSM_STORE_ERR;
    if (backup) {
        int step_rc = sqlite3_backup_step(backup, LSM_NOT_FOUND);
...
int lsm_store_seal_for_atomic_publish(lsm_store_t *s) {
    ...
    if (exec_sql(s, "PRAGMA synchronous = FULL;") != LSM_STORE_OK) {
        return LSM_STORE_ERR;
    }
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(s->db, "PRAGMA journal_mode = DELETE;", LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
...
int lsm_store_seal_existing_path_for_replace(const char *db_path) {
     ...
     * (lsm_store_open_internal). SQLite's win VFS needs the wide-round-tripped
    ...
    char seal_path[4096];
    if (!lsm_path_for_file_api(db_path, seal_path, sizeof(seal_path))) {
        return LSM_STORE_ERR;
    }
    lsm_store_t maintenance = {0};
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(maintenance.db, "PRAGMA quick_check(1);", LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
        int primary = rc & 0xff;
        sqlite3_close(maintenance.db);
    ...
    if (!lsm_store_check_integrity(&maintenance)) {
        sqlite3_close(maintenance.db);
        return LSM_STORE_NOT_FOUND;
    }

    int seal_rc = lsm_store_seal_for_atomic_publish(&maintenance);
    sqlite3_close(maintenance.db);
    return seal_rc;
}
...
int lsm_store_dump_to_file(lsm_store_t *s, const char *dest_path) {
    ...
    char tmp_path[LSM_SZ_1K];
    snprintf(tmp_path, sizeof(tmp_path), "%s.tmp", dest_path);
    (void)unlink(tmp_path);

    char tmp_api[4096];
    if (!lsm_path_for_file_api(tmp_path, tmp_api, sizeof(tmp_api))) {
    ...
    rc = sqlite3_backup_step(bk, LSM_NOT_FOUND); /* copy ALL pages in one shot */
    ...
    lsm_remove_db_sidecars(dest_path);
    if (lsm_rename_replace(tmp_path, dest_path) != 0) {
...
int lsm_store_generation(lsm_store_t *s, char *buf, size_t bufsz) {
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
        return LSM_STORE_OK; /* pre-migration DB: stable "legacy" generation */
    }
    if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int lsm_store_get_project(lsm_store_t *s, const char *name, lsm_project_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->root_path = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
...
int lsm_store_list_projects(lsm_store_t *s, lsm_project_t **out, int *count) {
    ...
    int n = 0;
    lsm_project_t *arr = malloc(cap * sizeof(lsm_project_t));

    int scan_rc1;
    while ((scan_rc1 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_project_t));
        }
        ...
        arr[n].root_path = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
        n++;
    }
    if (scan_rc1 != SQLITE_DONE) { /* SCANCHK:1:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        lsm_store_free_projects(arr, n);
        *out = NULL;
        *count = 0;
        return LSM_STORE_ERR;
    }
...
int lsm_store_delete_project(lsm_store_t *s, const char *name) {
    ...
    if (exec_sql(s, "BEGIN;") != LSM_STORE_OK) {
        return LSM_STORE_ERR;
    }
    ...
    for (size_t i = 0; i < sizeof(cleanup_sql) / sizeof(cleanup_sql[0]); i++) {
        ...
        if (sqlite3_prepare_v2(s->db, cleanup_sql[i], LSM_NOT_FOUND, &cleanup, NULL) != SQLITE_OK) {
...
static void scan_node(sqlite3_stmt *stmt, lsm_node_t *n) {
    n->id = sqlite3_column_int64(stmt, 0);
    n->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    n->label = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
    n->name = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_3));
    n->qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_4));
    n->file_path = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_5));
    n->start_line = sqlite3_column_int(stmt, LSM_SZ_6);
    n->end_line = sqlite3_column_int(stmt, LSM_SZ_7);
...
int lsm_store_find_node_by_qn(lsm_store_t *s, const char *project, const char *qn,
                              lsm_node_t *out) {
    if (!s || !s->db) {
        return LSM_STORE_ERR;
    }
...
int lsm_store_find_nodes_by_name_any(lsm_store_t *s, const char *name, lsm_node_t **out,
    ...
    int n = 0;
    lsm_node_t *arr = malloc(cap * sizeof(lsm_node_t));
    int scan_rc2;
    while ((scan_rc2 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_node_t));
        }
...
int lsm_store_find_node_ids_by_qns(lsm_store_t *s, const char *project, const char **qns,
    ...
    for (int i = 0; i < qn_count; i++) {
        ...
        int rc = lsm_store_find_node_by_qn(s, project, qns[i], &node);
        if (rc == LSM_STORE_OK) {
            out_ids[i] = node.id;
            found++;
            lsm_node_free_fields(&node);
...
static int find_nodes_generic(lsm_store_t *s, sqlite3_stmt **slot, const char *sql,
                              const char *project, const char *val, lsm_node_t **out, int *count) {
    if (!s || !s->db) {
        *out = NULL;
        *count = 0;
        return LSM_STORE_ERR;
...
int lsm_store_upsert_node_batch(lsm_store_t *s, const lsm_node_t *nodes, int count,
    ...
    for (int i = 0; i < count; i++) {
        int64_t id = lsm_store_upsert_node(s, &nodes[i]);
        if (id == LSM_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return LSM_STORE_ERR;
        }
...
int64_t lsm_store_insert_edge(lsm_store_t *s, const lsm_edge_t *e) {
    ...
    sqlite3_bind_int64(stmt, LSM_SZ_3, e->target_id);
...
static void scan_edge(sqlite3_stmt *stmt, lsm_edge_t *e) {
    e->id = sqlite3_column_int64(stmt, 0);
    e->project = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
    e->source_id = sqlite3_column_int64(stmt, LSM_SZ_2);
    e->target_id = sqlite3_column_int64(stmt, LSM_SZ_3);
    e->type = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_4));
    e->properties_json = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_5));
}

/* Generic: find multiple edges by a filter. */
static int find_edges_generic(lsm_store_t *s, sqlite3_stmt **slot, const char *sql,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_edge_t *arr = malloc(cap * sizeof(lsm_edge_t));

    int scan_rc4;
    while ((scan_rc4 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_edge_t));
        }
...
int lsm_store_fetch_call_edges(lsm_store_t *s, const char *project, int max_edges,
    ...
    bind_text(stmt, SKIP_ONE, project);
    sqlite3_bind_int(stmt, 2, max_edges > 0 ? max_edges + SKIP_ONE : LSM_NOT_FOUND);
...
int lsm_store_find_edges_by_source(lsm_store_t *s, int64_t source_id, lsm_edge_t **out,
                                   int *count) {
...
int lsm_store_find_edges_by_target(lsm_store_t *s, int64_t target_id, lsm_edge_t **out,
                                   int *count) {
...
int lsm_store_insert_edge_batch(lsm_store_t *s, const lsm_edge_t *edges, int count) {
    ...
    exec_sql(s, "BEGIN IMMEDIATE;");
    for (int i = 0; i < count; i++) {
        int64_t id = lsm_store_insert_edge(s, &edges[i]);
        if (id == LSM_STORE_ERR) {
            exec_sql(s, "ROLLBACK;");
            return LSM_STORE_ERR;
        }
...
int lsm_store_upsert_file_hash(lsm_store_t *s, const char *project, const char *rel_path,
    ...
    sqlite3_bind_int64(stmt, LSM_SZ_4, mtime_ns);
    sqlite3_bind_int64(stmt, LSM_SZ_5, size);
...
int lsm_store_get_file_hashes(lsm_store_t *s, const char *project, lsm_file_hash_t **out,
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_file_hash_t *arr = malloc(cap * sizeof(lsm_file_hash_t));

    int scan_rc5;
    while ((scan_rc5 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_file_hash_t));
        }
        ...
        arr[n].sha256 = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
        arr[n].mtime_ns = sqlite3_column_int64(stmt, LSM_SZ_3);
        arr[n].size = sqlite3_column_int64(stmt, LSM_SZ_4);
        n++;
    }
    if (scan_rc5 != SQLITE_DONE) { /* SCANCHK:5:stmt */
        store_set_error_sqlite(s, "row scan aborted");
        lsm_store_free_file_hashes(arr, n);
        *out = NULL;
        *count = 0;
        return LSM_STORE_ERR;
    }
...
int lsm_store_get_file_hash(lsm_store_t *s, const char *project, const char *rel_path,
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->size = sqlite3_column_int64(stmt, LSM_SZ_4);
        sqlite3_finalize(stmt);
        if (!out->project || !out->rel_path || !out->sha256) {
            lsm_store_clear_file_hash(out);
            return LSM_STORE_ERR;
        }
        return LSM_STORE_OK;
    }
...
static void cov_failure_fingerprint(const lsm_coverage_row_t *rows, int count,
    ...
    for (int i = 0; i < count; i++) {
        ...
        lsm_sha256_update(&sha, rel, strlen(rel) + 1);
        lsm_sha256_update(&sha, kind, strlen(kind) + 1);
        lsm_sha256_update(&sha, detail, strlen(detail) + 1);
    }
...
static void cov_dir_ids_free(LSMHashTable *ht) {
    if (!ht) {
        return;
    }
    lsm_ht_foreach(ht, cov_dir_ids_free_entry, NULL);
    lsm_ht_free(ht);
}

static int cov_rebuild_shadow_graph(lsm_store_t *s, const char *project) {
    ...
    lsm_coverage_row_t *fp_rows = NULL;
    int fp_count = 0;
    if (lsm_store_coverage_get(s, project, &fp_rows, &fp_count) != LSM_STORE_OK) {
        lsm_store_free_coverage(fp_rows, fp_count);
        return LSM_STORE_ERR;
    }
    char fp[LSM_SHA256_HEX_LEN + 1];
    cov_failure_fingerprint(fp_rows, fp_count, fp);
    lsm_store_free_coverage(fp_rows, fp_count);
    char fp_key[LSM_SZ_512];
    snprintf(fp_key, sizeof(fp_key), "coverage_shadow_fp:%s", project);
    char prev_fp[LSM_SHA256_HEX_LEN + 1] = {0};
    {
        sqlite3_stmt *get = NULL;
        if (sqlite3_prepare_v2(s->db, "SELECT v FROM store_meta WHERE k = ?1;", LSM_NOT_FOUND, &get,
                               NULL) == SQLITE_OK) {
    ...
    for (int w = 0; w < 2; w++) {
        ...
        if (sqlite3_prepare_v2(s->db, wipes[w], LSM_NOT_FOUND, &del, NULL) != SQLITE_OK) {
    ...
    int count = 0;
    if (lsm_store_coverage_get(s, project, &rows, &count) != LSM_STORE_OK) {
    ...
    if (lsm_store_upsert_project(s, covproj, "") != LSM_STORE_OK) {
        lsm_store_free_coverage(rows, count);
        return LSM_STORE_ERR;
    }

    lsm_node_t root = {.project = covproj,
                       .label = "Project",
                       .name = project,
                       .qualified_name = covproj,
                       .properties_json = "{}"};
    int64_t root_id = lsm_store_upsert_node(s, &root);
    if (root_id <= 0) {
        lsm_store_free_coverage(rows, count);
        return LSM_STORE_ERR;
    }
    ...
    LSMHashTable *dir_ids = lsm_ht_create(LSM_SZ_256);
    for (int i = 0; i < count; i++) {
        ...
        for (char *p = pathbuf; *p; p++) {
            ...
            *p = '\0';
            int64_t *cached = dir_ids ? (int64_t *)lsm_ht_get(dir_ids, pathbuf) : NULL;
            if (cached) {
                parent = *cached;
                *p = '/';
                continue;
            ...
            lsm_node_t folder = {.project = covproj,
                                 .label = "Folder",
                                 .name = seg ? seg + 1 : pathbuf,
                                 .qualified_name = pathbuf,
                                 .file_path = pathbuf,
                                 .properties_json = "{}"};
            int64_t fid = lsm_store_upsert_node(s, &folder);
            if (fid <= 0) {
                *p = '/';
            ...
            lsm_edge_t e = {.project = covproj,
                            .source_id = parent,
                            .target_id = fid,
                            .type = "CONTAINS_FOLDER",
                            .properties_json = "{}"};
            if (lsm_store_insert_edge(s, &e) <= 0) {
                *p = '/';
                cov_dir_ids_free(dir_ids);
                lsm_store_free_coverage(rows, count);
                return LSM_STORE_ERR;
        ...
        lsm_node_t file = {.project = covproj,
                           .label = "File",
                           .name = base ? base + 1 : rel,
                           .qualified_name = rel,
                           .file_path = rel,
                           .properties_json = props};
        int64_t file_id = lsm_store_upsert_node(s, &file);
        if (file_id <= 0) {
            cov_dir_ids_free(dir_ids);
            lsm_store_free_coverage(rows, count);
            return LSM_STORE_ERR;
    ...
    lsm_store_free_coverage(rows, count);
    {
        ...
        if (sqlite3_prepare_v2(s->db, "INSERT OR REPLACE INTO store_meta (k, v) VALUES (?1, ?2);",
                               LSM_NOT_FOUND, &set, NULL) == SQLITE_OK) {
...
int lsm_store_coverage_replace_ex(lsm_store_t *s, const char *project,
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &cov_t0);
    sqlite3_stmt *del = NULL;
    if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage WHERE project = ?1;", LSM_NOT_FOUND,
                           &del, NULL) != SQLITE_OK) {
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &cov_t1);
    cov_ms[0] =
    ...
    if (sqlite3_prepare_v2(
            ...
            "VALUES (?1, ?2, ?3, ?4);",
            LSM_NOT_FOUND, &ins, NULL) != SQLITE_OK) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        bind_text(ins, LSM_SZ_4, rows[i].detail ? rows[i].detail : "");
    ...
    if (sqlite3_prepare_v2(s->db,
                           ...
                           LSM_NOT_FOUND, &prune, NULL) != SQLITE_OK) {
    ...
    if (meta) {
        ...
        if (sqlite3_prepare_v2(
                ...
                LSM_NOT_FOUND, &up_meta, NULL) != SQLITE_OK) {
        ...
        bind_text(up_meta, LSM_SZ_4, recorded_at);
        bind_text(up_meta, LSM_SZ_5, recording_status);
        ...
        if (sqlite3_prepare_v2(s->db, "DELETE FROM index_coverage_meta WHERE project = ?1;",
                               LSM_NOT_FOUND, &del_meta, NULL) != SQLITE_OK) {
    ...
    if (cov_rebuild_shadow_graph(s, project) != LSM_STORE_OK) {
        (void)exec_sql(s, "ROLLBACK;");
        return LSM_STORE_ERR;
    }
    lsm_clock_gettime(CLOCK_MONOTONIC, &cov_t1);
        ...
        (cov_t1.tv_sec - cov_t0.tv_sec) * 1000 + (cov_t1.tv_nsec - cov_t0.tv_nsec) / 1000000;
    {
        ...
        lsm_log_info("publish.timing.coverage", "del", b0, "rows", b1, "prune", b2, "meta", b3,
...
int lsm_store_coverage_replace(lsm_store_t *s, const char *project, const lsm_coverage_row_t *rows,
                               int count) {
    return lsm_store_coverage_replace_ex(s, project, rows, count, NULL);
}

static int coverage_query_rows(lsm_store_t *s, const char *project, const char *selector,
                               const char *sql, lsm_coverage_row_t **out, int *count) {
    if (!out || !count) {
        return LSM_STORE_ERR;
    }
    *out = NULL;
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sql, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_coverage_row_t *arr = malloc((size_t)cap * sizeof(*arr));
    if (!arr) {
        sqlite3_finalize(stmt);
        return LSM_STORE_ERR;
    }
    ...
    while ((scan_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (!arr[n].rel_path || !arr[n].kind || !arr[n].detail) {
            sqlite3_finalize(stmt);
            lsm_store_free_coverage(arr, n + 1);
            return LSM_STORE_ERR;
        }
        n++;
    }
...
int lsm_store_coverage_meta_get(lsm_store_t *s, const char *project, lsm_coverage_meta_t *out) {
    ...
    if (rc == SQLITE_ROW) {
        ...
        out->recording_status = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_4));
        out->ignored_files_stored = sqlite3_column_int(stmt, LSM_SZ_5);
        ...
        if (!out->project || !out->generation || !out->index_mode || !out->recorded_at ||
            ...
            lsm_store_coverage_meta_clear(out);
            return LSM_STORE_ERR;
        }
        return LSM_STORE_OK;
    }
...
int lsm_store_coverage_get(lsm_store_t *s, const char *project, lsm_coverage_row_t **out,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_coverage_row_t *arr = malloc(cap * sizeof(lsm_coverage_row_t));
    int scan_rc6;
    while ((scan_rc6 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_coverage_row_t));
        }
        arr[n].rel_path = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].kind = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].detail = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
        n++;
    }
...
int lsm_store_find_nodes_by_file_overlap(lsm_store_t *s, const char *project, const char *file_path,
    ...
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    int rc = sqlite3_prepare_v2(s->db, sql, LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_node_t *nodes = malloc(cap * sizeof(lsm_node_t));
    int scan_rc7;
    while ((scan_rc7 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            nodes = safe_realloc(nodes, cap * sizeof(lsm_node_t));
        }
        memset(&nodes[n], 0, sizeof(lsm_node_t));
...
int lsm_store_find_nodes_by_qn_suffix(lsm_store_t *s, const char *project, const char *suffix,
    ...
    *count = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    int rc =
        sqlite3_prepare_v2(s->db, project ? sql_with_project : sql_any, LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
...
void lsm_store_node_degree(lsm_store_t *s, int64_t node_id, int *in_deg, int *out_deg) {
    ...
    *out_deg = 0;
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, in_sql, LSM_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
    ...
    if (sqlite3_prepare_v2(s->db, out_sql, LSM_NOT_FOUND, &stmt, NULL) == SQLITE_OK) {
        sqlite3_bind_int64(stmt, SKIP_ONE, node_id);
        if (sqlite3_step(stmt) == SQLITE_ROW) {
...
int lsm_store_list_files(lsm_store_t *s, const char *project, char ***out, int *count) {
    ...
    *count = 0;
    ...
    sqlite3_bind_text(stmt, SKIP_ONE, project, LSM_NOT_FOUND, SQLITE_STATIC);
...
static int query_neighbor_names(sqlite3 *db, const char *sql, int64_t node_id, int limit,
    ...
    *out_count = 0;
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db, sql, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
...
static int count_degrees_direction(lsm_store_t *s, const int64_t *node_ids, int id_count,
...
int lsm_store_upsert_file_hash_batch(lsm_store_t *s, const lsm_file_hash_t *hashes, int count) {
    ...
    int rc = lsm_store_begin(s);
    if (rc != LSM_STORE_OK) {
        return rc;
    }

    for (int i = 0; i < count; i++) {
        rc = lsm_store_upsert_file_hash(s, hashes[i].project, hashes[i].rel_path, hashes[i].sha256,
                                        hashes[i].mtime_ns, hashes[i].size);
        if (rc != LSM_STORE_OK) {
            lsm_store_rollback(s);
            return rc;
        }
    }

    return lsm_store_commit(s);
}

/* ── LSP surface rows (closure-repair incremental) ─────────────── */

static int upsert_lsp_surface_row(lsm_store_t *s, const lsm_lsp_surface_row_t *row) {
    const char *sql =
...
int lsm_store_get_lsp_surfaces(lsm_store_t *s, const char *project, lsm_lsp_surface_row_t **out,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_lsp_surface_row_t *rows = calloc((size_t)cap, sizeof(*rows));
    if (!rows) {
        sqlite3_finalize(stmt);
        return LSM_STORE_ERR;
    }
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            lsm_lsp_surface_row_t *grown = realloc(rows, (size_t)cap * sizeof(*rows));
            if (!grown) {
                oom = true;
                break;
            }
...
static int dependent_files_chunk(lsm_store_t *s, const char *project,
                                 const char *const *target_files, int chunk_count,
                                 LSMHashTable *seen, char ***out, int *out_count, int *out_cap) {
    char sql[LSM_SZ_4K];
    ...
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        ...
        if (!path || !path[0] || lsm_ht_get(seen, path)) {
            continue;
        }
        ...
        (*out)[(*out_count)++] = copy;
        lsm_ht_set(seen, copy, copy);
    }
    sqlite3_finalize(stmt);
    return step_rc == SQLITE_DONE ? LSM_STORE_OK : LSM_STORE_ERR;
}
...
int lsm_store_get_dependent_files(lsm_store_t *s, const char *project,
    ...
    *out_count = 0;
    if (!s || !project || target_count <= 0) {
        return target_count == 0 ? LSM_STORE_OK : LSM_STORE_ERR;
    }
    ...
    LSMHashTable *seen = lsm_ht_create((size_t)target_count * PAIR_LEN);
    if (!seen) {
        return LSM_STORE_ERR;
    }
    for (int i = 0; i < target_count; i++) {
        lsm_ht_set(seen, target_files[i], (void *)target_files[i]);
    }
    char **files = NULL;
    int count = 0;
    int cap = 0;
    int rc = LSM_STORE_OK;
    for (int off = 0; off < target_count && rc == LSM_STORE_OK; off += DEPFILE_CHUNK) {
    ...
    lsm_ht_free(seen);
    if (rc != LSM_STORE_OK) {
        lsm_store_free_dependent_files(files, count);
        return rc;
    }
    *out = files;
    *out_count = count;
...
int lsm_store_find_edges_by_url_path(lsm_store_t *s, const char *project, const char *keyword,
    ...
    *count = 0;
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_edge_t *edges = malloc(cap * sizeof(lsm_edge_t));
    int scan_rc12;
    while ((scan_rc12 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            edges = safe_realloc(edges, cap * sizeof(lsm_edge_t));
        }
        memset(&edges[n], 0, sizeof(lsm_edge_t));
...
int lsm_store_restore_from(lsm_store_t *dst, lsm_store_t *src) {
    ...
    int rc = sqlite3_backup_step(bk, LSM_NOT_FOUND); /* copy all pages */
    sqlite3_backup_finish(bk);
...
static void search_apply_degree_filter(char *sql, size_t sql_sz, const lsm_search_params_t *p) {
    bool has_degree_filter = (p->min_degree >= 0 || p->max_degree >= 0);
    if (!has_degree_filter) {
        return;
    }
    char inner_sql[LSM_SZ_4K];
 ...
 * lsm_extract_like_hints bails on alternation, so no false negatives. */
static void where_add_like_hints(const char *column, const char *pattern, char *where, int where_sz,
    ...
    int nhints = lsm_extract_like_hints(pattern, hints, ST_LIKE_HINT_MAX);
    char bind_buf[LSM_SZ_64];
...
static int search_where_basic(const lsm_search_params_t *params, char *where, int where_sz,
                              int *wlen, int *nparams, search_bind_t *binds, int *bind_idx,
                              search_like_pool_t *pool) {
    char bind_buf[LSM_SZ_64];
    ...
    if (params->file_pattern) {
        char *lp = lsm_glob_to_like(params->file_pattern);
...
static void search_where_advanced(const lsm_search_params_t *params, char *where, int where_sz,
                                  int *wlen, int *nparams, search_bind_t *binds, int *bind_idx) {
    if (params->relationship) {
        char rel_clause[LSM_SZ_256];
...
static int search_build_where(const lsm_search_params_t *params, char *where, int where_sz,
...
int lsm_store_search(lsm_store_t *s, const lsm_search_params_t *params, lsm_search_output_t *out) {
    ...
    int rc = sqlite3_prepare_v2(s->db, count_sql, LSM_NOT_FOUND, &cnt_stmt, NULL);
    if (rc == SQLITE_OK) {
    ...
    rc = sqlite3_prepare_v2(s->db, sql, LSM_NOT_FOUND, &main_stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_search_result_t *results = malloc(cap * sizeof(lsm_search_result_t));

    int scan_rc13;
    while ((scan_rc13 = sqlite3_step(main_stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            results = safe_realloc(results, cap * sizeof(lsm_search_result_t));
        }
        memset(&results[n], 0, sizeof(lsm_search_result_t));
        scan_node(main_stmt, &results[n].node);
        results[n].in_degree = sqlite3_column_int(main_stmt, ST_COL_9);
        results[n].out_degree = sqlite3_column_int(main_stmt, LSM_DECIMAL_BASE);
        n++;
    }
...
void lsm_store_search_free(lsm_search_output_t *out) {
    ...
    for (int i = 0; i < out->count; i++) {
        lsm_search_result_t *r = &out->results[i];
...
static int bfs_collect_edges(lsm_store_t *s, int64_t start_id, const lsm_node_hop_t *visited,
                             int visited_count, const char *types_clause, const char **edge_types,
                             int edge_type_count, lsm_edge_info_t **out_edges,
                             int *out_edge_count) {
    *out_edges = NULL;
    *out_edge_count = 0;
    ...
    if (sqlite3_exec(s->db,
        ...
        return LSM_STORE_OK; /* best-effort: nodes without edges beat an error */
    }
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_ids(id) VALUES (?1)", LSM_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
        return LSM_STORE_OK;
    }
    ...
    int rc = sqlite3_prepare_v2(s->db, edge_sql, LSM_NOT_FOUND, &estmt, NULL);
    if (rc != SQLITE_OK) {
        *out_edges = NULL;
        *out_edge_count = 0;
        return LSM_STORE_OK;
    ...
    int en = 0;
    lsm_edge_info_t *edges = malloc(ecap * sizeof(lsm_edge_info_t));

    int scan_rc14;
    while ((scan_rc14 = sqlite3_step(estmt)) == SQLITE_ROW) {
        if (en >= ecap) {
            ecap *= ST_GROWTH;
            edges = safe_realloc(edges, ecap * sizeof(lsm_edge_info_t));
        }
        ...
        edges[en].type = heap_strdup((const char *)sqlite3_column_text(estmt, LSM_SZ_2));
        ...
        edges[en].properties_json = heap_strdup((const char *)sqlite3_column_text(estmt, LSM_SZ_5));
        en++;
    }
...
int lsm_store_bfs(lsm_store_t *s, int64_t start_id, const char *direction, const char **edge_types,
    ...
    lsm_node_t root = {0};
    int rc = lsm_store_find_node_by_id(s, start_id, &root);
    if (rc != LSM_STORE_OK) {
        return rc;
    }
    out->root = root;

    char types_clause[LSM_SZ_512];
    ...
    sqlite3_stmt *stmt = NULL;
    rc = sqlite3_prepare_v2(s->db, sql, LSM_NOT_FOUND, &stmt, NULL);
    if (rc != SQLITE_OK) {
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_node_hop_t *visited = malloc(cap * sizeof(lsm_node_hop_t));

    int scan_rc15;
    while ((scan_rc15 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            visited = safe_realloc(visited, cap * sizeof(lsm_node_hop_t));
        }
...
int lsm_store_bfs_multi(lsm_store_t *s, const int64_t *seed_ids, int seed_count,
    ...
    sqlite3_stmt *ins = NULL;
    if (sqlite3_prepare_v2(s->db, "INSERT OR IGNORE INTO bfs_seeds(id) VALUES (?1)", LSM_NOT_FOUND,
                           &ins, NULL) != SQLITE_OK) {
    ...
    if (n > max_results) {
        ...
        n = max_results;
        lsm_node_free_fields(&visited[n].node);
        if (truncated) {
            *truncated = true;
        }
    }
...
void lsm_store_traverse_free(lsm_traverse_result_t *out) {
    ...
    for (int i = 0; i < out->visited_count; i++) {
        lsm_node_hop_t *h = &out->visited[i];
...
lsm_risk_level_t lsm_hop_to_risk(int hop) {
    switch (hop) {
    case SKIP_ONE:
        return LSM_RISK_CRITICAL;
    case ST_COL_2:
        return LSM_RISK_HIGH;
    case ST_COL_3:
        return LSM_RISK_MEDIUM;
    default:
        return LSM_RISK_LOW;
    }
}
...
const char *lsm_risk_label(lsm_risk_level_t level) {
    switch (level) {
    case LSM_RISK_CRITICAL:
        return "CRITICAL";
    case LSM_RISK_HIGH:
        return "HIGH";
    case LSM_RISK_MEDIUM:
        return "MEDIUM";
    case LSM_RISK_LOW:
    default:
        return "LOW";
...
lsm_impact_summary_t lsm_build_impact_summary(const lsm_node_hop_t *hops, int hop_count,
    ...
    lsm_impact_summary_t s = {0};
    for (int i = 0; i < hop_count; i++) {
        switch (lsm_hop_to_risk(hops[i].hop)) {
        case LSM_RISK_CRITICAL:
            s.critical++;
            break;
        case LSM_RISK_HIGH:
...
int lsm_deduplicate_hops(const lsm_node_hop_t *hops, int hop_count, lsm_node_hop_t **out,
    ...
    *out_count = 0;
    ...
    lsm_node_hop_t *result = malloc(hop_count * sizeof(lsm_node_hop_t));
    int n = 0;
    ...
    *out = safe_realloc(result, n * sizeof(lsm_node_hop_t));
    *out_count = n;
    return LSM_STORE_OK;
}
...
static void schema_discover_props(sqlite3 *db, const char *sql, const char *project,
    ...
    if (sqlite3_prepare_v2(db, sql, LSM_NOT_FOUND, &pstmt, NULL) == SQLITE_OK) {
...
bool lsm_store_arch_path_scoped(const char *path) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512 + 4];
...
int lsm_store_count_edges_scoped(lsm_store_t *s, const char *project, const char *path) {
    ...
    char like[LSM_SZ_512];
    if (!arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like))) {
        return lsm_store_count_edges(s, project);
    }
    const char *sql =
...
static int get_schema_impl(lsm_store_t *s, const char *project, lsm_schema_info_t *out,
                           bool with_props) {
    memset(out, 0, sizeof(*out));
    if (!s || !s->db) {
        return LSM_NOT_FOUND;
    ...
    /* Node labels */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        lsm_label_count_t *arr = malloc(cap * sizeof(lsm_label_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            return LSM_NOT_FOUND;
        }
        ...
        while ((scan_rc16 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                ...
                void *tmp = realloc(arr, new_cap * sizeof(lsm_label_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
    ...
    /* Edge types */
    {
        ...
        int cap = ST_INIT_CAP_8;
        int n = 0;
        lsm_type_count_t *arr = malloc(cap * sizeof(lsm_type_count_t));
        if (!arr) {
            sqlite3_finalize(stmt);
            lsm_store_schema_free(out);
            return LSM_NOT_FOUND;
        ...
        while ((scan_rc17 = sqlite3_step(stmt)) == SQLITE_ROW) {
            if (n >= cap) {
                int new_cap = cap * ST_GROWTH;
                void *tmp = realloc(arr, new_cap * sizeof(lsm_type_count_t));
                if (!tmp) {
                    for (int i = 0; i < n; i++) {
...
int lsm_store_get_schema_counts_scoped(lsm_store_t *s, const char *project, const char *path,
    ...
    char sqlbuf[ST_SQL_BUF];
    {
        ...
        snprintf(sqlbuf, sizeof(sqlbuf), "%s%s GROUP BY label ORDER BY COUNT(*) DESC;", base,
                 arch_path_scope_sql());
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, sqlbuf, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            return LSM_NOT_FOUND;
...

    {
        ...
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(s->db, esql, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK || !stmt) {
            if (stmt) {
                sqlite3_finalize(stmt);
            }
            lsm_store_schema_free(out);
...
const char *lsm_qn_to_package(const char *qn) {
    ...
    static LSM_TLS char buf[LSM_SZ_256];
...
static const char *file_ext(const char *path) {
    ...
    static LSM_TLS char buf[LSM_SZ_16];
    int len = (int)strlen(dot);
    if (len >= (int)sizeof(buf)) {
        return NULL;
    }
    for (int i = 0; i < len; i++) {
        buf[i] = (char)((dot[i] >= 'A' && dot[i] <= 'Z') ? dot[i] + LSM_SZ_32 : dot[i]);
    }
    buf[len] = '\0';
    return buf;
}
...
static int arch_languages(lsm_store_t *s, const char *project, const char *path,
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, sqlbuf, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    const char *lang_names[LSM_SZ_64];
    int lang_counts[LSM_SZ_64];
    int nlang = 0;
    ...
    if (nlang > LSM_DECIMAL_BASE) {
        nlang = ST_MAX_LANG;
    }

    out->languages = (nlang > 0) ? calloc(nlang, sizeof(lsm_language_count_t)) : NULL;
...
static int arch_entry_points(lsm_store_t *s, const char *project, const char *path,
                             lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_entry_point_t *arr = calloc(cap, sizeof(lsm_entry_point_t));
    int scan_rc21;
    while ((scan_rc21 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_entry_point_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].file = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
        n++;
    }
...
static char *extract_json_string_prop(const char *json, const char *key, int key_len) {
    ...
    if (!end || end - m >= LSM_SZ_256) {
        return NULL;
    }
    char vbuf[LSM_SZ_256];
    memcpy(vbuf, m, end - m);
    vbuf[end - m] = '\0';
    return heap_strdup(vbuf);
}

static int arch_routes(lsm_store_t *s, const char *project, const char *path,
                       lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_route_info_t *arr = calloc(cap, sizeof(lsm_route_info_t));
    int scan_rc22;
    while ((scan_rc22 = sqlite3_step(stmt)) == SQLITE_ROW) {
        const char *name = (const char *)sqlite3_column_text(stmt, 0);
        const char *props = (const char *)sqlite3_column_text(stmt, SKIP_ONE);
        const char *fp = (const char *)sqlite3_column_text(stmt, LSM_SZ_2);
        if (lsm_is_test_file_path(fp)) {
            continue;
        }
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_route_info_t));
        }
...
static int arch_hotspots(lsm_store_t *s, const char *project, const char *path,
                         lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_8;
    int n = 0;
    lsm_hotspot_t *arr = calloc(cap, sizeof(lsm_hotspot_t));
    int scan_rc23;
    while ((scan_rc23 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_hotspot_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
        arr[n].qualified_name = heap_strdup((const char *)sqlite3_column_text(stmt, SKIP_ONE));
        arr[n].fan_in = sqlite3_column_int(stmt, LSM_SZ_2);
        n++;
    }
...
static int arch_boundaries(lsm_store_t *s, const char *project, const char *path,
                           lsm_cross_pkg_boundary_t **out_arr, int *out_count) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    const char *nbase =
        ...
        "(" LSM_SQL_CALLABLE_OR_TYPE_LABELS ")";
    if (scoped) {
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, LSM_NOT_FOUND, &nstmt, NULL) != SQLITE_OK) {
    ...
    while ((scan_rc24 = sqlite3_step(nstmt)) == SQLITE_ROW) {
        ...
        npkgs[nn] = heap_strdup(lsm_qn_to_package(qn));
        nn++;
    }
    ...
    sqlite3_stmt *estmt = NULL;
    if (sqlite3_prepare_v2(s->db, esql, LSM_NOT_FOUND, &estmt, NULL) != SQLITE_OK) {
        for (int i = 0; i < nn; i++) {
            free(npkgs[i]);
        }
        free(nids);
    ...
    lsm_cross_pkg_boundary_t *result =
        (bn > 0) ? calloc(bn, sizeof(lsm_cross_pkg_boundary_t)) : NULL;
    for (int i = 0; i < bn; i++) {
...
static int arch_packages_from_qn(lsm_store_t *s, const char *project, const char *path,
                                 lsm_package_summary_t **out_arr, int *out_count) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(s->db, qsqlbuf, LSM_NOT_FOUND, &stmt, NULL) != SQLITE_OK) {
    ...
    lsm_package_summary_t *arr = (np > 0) ? calloc(np, sizeof(lsm_package_summary_t)) : NULL;
    for (int i = 0; i < np; i++) {
        arr[i].name = pnames[i];
...
static int arch_packages(lsm_store_t *s, const char *project, const char *path,
                         lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    int cap = ST_INIT_CAP_16;
    int n = 0;
    lsm_package_summary_t *arr = calloc(cap, sizeof(lsm_package_summary_t));
    int scan_rc27;
    while ((scan_rc27 = sqlite3_step(stmt)) == SQLITE_ROW) {
        if (n >= cap) {
            cap *= ST_GROWTH;
            arr = safe_realloc(arr, cap * sizeof(lsm_package_summary_t));
        }
        arr[n].name = heap_strdup((const char *)sqlite3_column_text(stmt, 0));
...
static void classify_layer(const char *pkg, int in, int out_deg, bool has_routes,
    ...
    static LSM_TLS char reason_buf[LSM_SZ_128];
...
static int collect_pkg_names(lsm_store_t *s, const char *sql, const char *project, const char *path,
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && count < max_pkgs) {
        const char *qn = (const char *)sqlite3_column_text(stmt, 0);
        pkgs[count++] = heap_strdup(lsm_qn_to_package(qn));
    }
    sqlite3_finalize(stmt);
    return count;
}

static int arch_layers(lsm_store_t *s, const char *project, const char *path,
                       lsm_architecture_info_t *out) {
    /* Get boundaries for fan analysis */
    lsm_cross_pkg_boundary_t *boundaries = NULL;
    int bcount = 0;
    ...
    int nepkgs = collect_pkg_names(s,
                                   ...
                                   project, path, entry_pkgs, LSM_SZ_32);

    /* Compute fan-in/out per package */
    char *all_pkgs[LSM_SZ_64];
    int fan_in[LSM_SZ_64];
    int fan_out[LSM_SZ_64];
    int npkgs = 0;
    ...
    out->layers = (npkgs > 0) ? calloc(npkgs, sizeof(lsm_package_layer_t)) : NULL;
    ...
    for (int i = SKIP_ONE; i < npkgs; i++) {
        ...
        while (j > 0 && strcmp(out->layers[j].name, out->layers[j - SKIP_ONE].name) < 0) {
            lsm_package_layer_t tmp = out->layers[j];
...
static lsm_file_tree_entry_t make_tree_entry(const char *path, char **files, int fn,
                                             char **dir_paths, const int *dir_child_counts,
                                             int dn) {
    lsm_file_tree_entry_t e = {0};
...
static void push_tree_entry(lsm_file_tree_entry_t **entries, int *en, int *ecap,
                            lsm_file_tree_entry_t e) {
    if (*en >= *ecap) {
        *ecap *= ST_GROWTH;
        *entries = safe_realloc(*entries, *ecap * sizeof(lsm_file_tree_entry_t));
    }
    (*entries)[(*en)++] = e;
}
...
static void arch_collect_entries(char **dir_paths, int *dir_child_counts, char ***dir_children,
                                 int dn, char **files, int fn, lsm_file_tree_entry_t **entries_out,
                                 int *en_out) {
    int ecap = LSM_SZ_64;
    int en = 0;
    lsm_file_tree_entry_t *entries = calloc(ecap, sizeof(lsm_file_tree_entry_t));
    ...
    for (int i = SKIP_ONE; i < en; i++) {
        ...
        while (j > 0 && strcmp(entries[j].path, entries[j - SKIP_ONE].path) < 0) {
            lsm_file_tree_entry_t tmp = entries[j];
...
static int arch_file_tree(lsm_store_t *s, const char *project, const char *path,
                          lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
...
static void louvain_build_weights(const int64_t *nodes, int n, const lsm_louvain_edge_t *edges,
...
static void lg_free(lsm_lg_t *g) {
    free(g->off);
    free(g->nbr);
    free(g->w);
    free(g->k);
 ...
 * comm[] in place. */
static void leiden_move(const lsm_lg_t *g, int *comm, double gamma, double twom) {
    int n = g->n;
...
static int leiden_refine(const lsm_lg_t *g, const int *comm, double gamma, double twom,
                         int *refined) {
    int n = g->n;
    double *stot = calloc((size_t)n, sizeof(double));
    double *acc = calloc((size_t)n, sizeof(double));
...
static int leiden_aggregate(const lsm_lg_t *g, const int *refined, int r_count, const int *comm,
                            lsm_lg_t *out, int *seed) {
    int n = g->n;
    ...
    for (int r = 0; r < r_count; r++) {
        seed[r] = LSM_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
        int r = refined[i];
        k2[r] += g->k[i];
        gcount[r]++;
        if (seed[r] == LSM_NOT_FOUND) {
            seed[r] = comm[i];
        }
    }
    gstart[0] = 0;
...
int lsm_leiden(const int64_t *nodes, int node_count, const lsm_louvain_edge_t *edges,
    ...
    lsm_louvain_result_t *result = malloc((size_t)n * sizeof(*result));
    if (!result) {
        return LSM_NOT_FOUND;
    }
    for (int i = 0; i < n; i++) {
    ...
    lsm_lg_t g;
    int built = (wn > 0) ? lg_build(n, wsi, wdi, ww, wn, &g) : LSM_NOT_FOUND;
    free(wsi);
    free(wdi);
    free(ww);
    if (built != LSM_STORE_OK) {
    ...
    for (int level = 0; level < LEIDEN_MAX_LEVELS; level++) {
        ...
        if (!seed || leiden_aggregate(&g, refined, r_count, comm, &g2, seed) != LSM_STORE_OK) {
            free(seed);
            free(refined);
            break;
        }
...
int lsm_louvain(const int64_t *nodes, int node_count, const lsm_louvain_edge_t *edges,
    ...
    return lsm_leiden(nodes, node_count, edges, edge_count, 1.0, out, out_count);
}

/* ── Architecture: community clusters via Leiden ───────────────── */

enum {
    LSM_CLUSTER_TOP_N = 12,       /* report at most this many clusters */
    LSM_CLUSTER_MAX_TOPNODES = 5, /* representative node names per cluster */
    LSM_CLUSTER_MAX_PKGS = 5,     /* packages listed per cluster */
    LSM_CLUSTER_MIN_MEMBERS = 2,  /* skip singletons */
    LSM_CLUSTER_NODE_CAP = 8000   /* bound the work for very large graphs */
};
...
/* Index of node id within the id-sorted ids[], or LSM_NOT_FOUND. */
static int cluster_id_index(const int64_t *ids, int n, int64_t id) {
    ...
    return hit ? (int)(hit - ids) : LSM_NOT_FOUND;
}
...
static void cluster_build_one(lsm_cluster_info_t *ci, int c, int n, const int *comm,
    ...
    /* Top nodes by degree. */
    int top_idx[LSM_CLUSTER_MAX_TOPNODES];
    int top_deg[LSM_CLUSTER_MAX_TOPNODES];
    int tn = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] != c) {
            continue;
        ...
        if (pos < LSM_CLUSTER_MAX_TOPNODES) {
            int last = (tn < LSM_CLUSTER_MAX_TOPNODES) ? tn : LSM_CLUSTER_MAX_TOPNODES - 1;
            ...
            top_idx[pos] = i;
            top_deg[pos] = d;
            if (tn < LSM_CLUSTER_MAX_TOPNODES) {
                tn++;
            }
        }
    }
    ...
    const char *pkgs[LSM_CLUSTER_MAX_PKGS];
    int pkg_counts[LSM_CLUSTER_MAX_PKGS];
    int pc = 0;
    for (int i = 0; i < n; i++) {
        if (comm[i] == c) {
            cluster_add_pkg(pkgs, pkg_counts, &pc, LSM_CLUSTER_MAX_PKGS,
                            lsm_qn_to_top_package(qns[i]));
        }
    }
    if (pc > 0) {
...
static int arch_clusters(lsm_store_t *s, const char *project, const char *path,
                         lsm_architecture_info_t *out) {
    char norm[LSM_SZ_512];
    char like[LSM_SZ_512];
    bool scoped = arch_path_prepare(path, norm, sizeof(norm), like, sizeof(like));
    ...
    const char *nbase = "SELECT id, name, qualified_name, file_path FROM nodes "
                        "WHERE project=?1 AND label IN (" LSM_SQL_CALLABLE_OR_TYPE_LABELS ")";
    if (scoped) {
    ...
    if (sqlite3_prepare_v2(s->db, nsqlbuf, LSM_NOT_FOUND, &st, NULL) != SQLITE_OK) {
        return LSM_STORE_OK; /* clusters are best-effort */
    }
    bind_text(st, SKIP_ONE, project);
    if (scoped) {
        arch_bind_path_scope(st, ST_COL_2, ST_COL_3, norm, like);
        sqlite3_bind_int(st, ST_COL_4, LSM_CLUSTER_NODE_CAP);
    } else {
        sqlite3_bind_int(st, LSM_SZ_2, LSM_CLUSTER_NODE_CAP);
    }
    int cap = ST_INIT_CAP_8;
    int n = 0;
    ...
    while ((scan_rc29 = sqlite3_step(st)) == SQLITE_ROW) {
        ...
        qns[n] = heap_strdup((const char *)sqlite3_column_text(st, LSM_SZ_2));
        n++;
    }
    ...
    lsm_louvain_edge_t *edges = malloc((size_t)n * sizeof(lsm_louvain_edge_t));
    int *esrc = malloc((size_t)n * sizeof(int));
    int *edst = malloc((size_t)n * sizeof(int));
    int *degree = calloc((size_t)n, sizeof(int));
    int ne = 0;
    const char *esql = "SELECT source_id, target_id FROM edges WHERE project=?1 AND type='CALLS'";
    if (sqlite3_prepare_v2(s->db, esql, LSM_NOT_FOUND, &st, NULL) == SQLITE_OK) {
        int ecap = n;
        bind_text(st, SKIP_ONE, project);
        int scan_rc30;
        while ((scan_rc30 = sqlite3_step(st)) == SQLITE_ROW) {
            ...
            if (ne >= ecap) {
                ecap *= ST_GROWTH;
                edges = safe_realloc(edges, (size_t)ecap * sizeof(lsm_louvain_edge_t));
    ...
    lsm_louvain_result_t *res = NULL;
    int rn = 0;
    int *comm = NULL;
    int C = 0;
    if (lsm_leiden(ids, n, edges, ne, 1.0, &res, &rn) == LSM_STORE_OK && res && rn == n) {
    ...
    if (comm && C > 0) {
        ...
        lsm_cluster_info_t *clusters =
            malloc((size_t)LSM_CLUSTER_TOP_N * sizeof(lsm_cluster_info_t));
        int cc = 0;
        for (int r = 0; r < C && cc < LSM_CLUSTER_TOP_N; r++) {
            int c = rank[r].comm;
            if (members[c] < LSM_CLUSTER_MIN_MEMBERS) {
...
static bool want_aspect(const char **aspects, int aspect_count, const char *name) {
    ...
    for (int i = 0; i < aspect_count; i++) {
        ...
        if (strcmp(aspects[i], "overview") == 0 && lsm_store_arch_aspect_in_overview(name)) {
            return true;
        }
...
int lsm_store_get_architecture(lsm_store_t *s, const char *project, const char *path,
    ...
    if (want_aspect(aspects, aspect_count, "boundaries")) {
        lsm_cross_pkg_boundary_t *barr = NULL;
        int bcount = 0;
...
static void adr_save_section(lsm_adr_sections_t *result, char *section_name, char *buf,
                             int buf_len) {
...
int lsm_adr_validate_content(const char *content, char *errbuf, int errbuf_size) {
    lsm_adr_sections_t sections = lsm_adr_parse_sections(content);
    char missing[LSM_SZ_256] = "";
    int mlen = 0;
    int nmissing = 0;
    ...
    lsm_adr_sections_free(&sections);
...
int lsm_store_adr_store(lsm_store_t *s, const char *project, const char *content) {
    ...
    int rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    return (rc == SQLITE_DONE) ? LSM_STORE_OK : LSM_STORE_ERR;
}
...
int lsm_store_adr_get(lsm_store_t *s, const char *project, lsm_adr_t *out) {
    ...
    out->created_at = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_2));
    out->updated_at = heap_strdup((const char *)sqlite3_column_text(stmt, LSM_SZ_3));
    rc = sqlite3_finalize(stmt);
    if (rc != SQLITE_OK) {
        lsm_store_adr_free(out);
...
int lsm_store_adr_update_sections(lsm_store_t *s, const char *project, const char **keys,
    ...
    /* Get existing ADR */
    lsm_adr_t existing;
    int rc = lsm_store_adr_get(s, project, &existing);
    if (rc != LSM_STORE_OK) {
    ...
    lsm_adr_sections_t sections = lsm_adr_parse_sections(existing.content);
    ...
    /* Render merged */
    char *merged = lsm_adr_render(&sections);
    lsm_adr_sections_free(&sections);

    /* Check length */
    if ((int)strlen(merged) > LSM_ADR_MAX_LENGTH) {
        char msg[LSM_SZ_128];
        snprintf(msg, sizeof(msg), "merged ADR exceeds %d chars (%d chars)", LSM_ADR_MAX_LENGTH,
                 (int)strlen(merged));
    ...
    /* Store merged */
    rc = lsm_store_adr_store(s, project, merged);
    free(merged);
    if (rc != LSM_STORE_OK) {
        return rc;
    }

    return lsm_store_adr_get(s, project, out);
}
...
void lsm_store_free_projects(lsm_project_t *projects, int count) {
    ...
    for (int i = 0; i < count; i++) {
        lsm_project_free_fields(&projects[i]);
    }
    free(projects);
}
...
static bool vs_load_enriched_vector(lsm_store_t *s, const char *project, const char *token,
                                    float *out) {
    ...
    if (sqlite3_step(tv_stmt) == SQLITE_ROW) {
        ...
        if (vec && vec_len == VS_VEC_DIM) {
            for (int d = 0; d < VS_VEC_DIM; d++) {
                out[d] = (float)vec[d] / LSM_STORE_INT8_MAX;
            }
            found = true;
        }
    }
...
static void vs_fill_sparse_random(const char *token, float *out) {
    ...
    for (int i = 0; i < VS_SPARSE_NNZE; i++) {
        ...
        float sign = (h & SKIP_ONE) ? LSM_STORE_UNIT_POS_F : -LSM_STORE_UNIT_POS_F;
        out[pos] += sign;
    }
}
...
static int8_t vs_clamp_int8(float v) {
    if (v > LSM_STORE_INT8_MAX) {
        return (int8_t)LSM_STORE_INT8_MAX;
    }
    if (v < -LSM_STORE_INT8_MAX) {
        return (int8_t)-LSM_STORE_INT8_MAX;
    }
...
static bool vs_normalize_and_quantize(const float *src, int8_t *dst) {
    ...
    mag = sqrtf(mag);
    if (mag < LSM_STORE_DENOM_EPS_F) {
        return false;
    }
    float inv = LSM_STORE_UNIT_POS_F / mag;
    for (int d = 0; d < VS_VEC_DIM; d++) {
        dst[d] = vs_clamp_int8(src[d] * inv * LSM_STORE_INT8_MAX);
    }
    return true;
}
...
static int vs_build_keyword_vectors(lsm_store_t *s, const char *project, const char **keywords,
...
static double vs_min_cosine_score(const int8_t *node_vec, int node_vec_len,
    ...
    double min_score = LSM_STORE_UNIT_POS_D;
    for (int k = 0; k < actual_kw; k++) {
        ...
        double cos_k = denom > LSM_STORE_DENOM_EPS_D ? (double)dot / denom : 0.0;
        if (cos_k < min_score) {
            min_score = cos_k;
        }
    }
...
static lsm_vector_result_t *vs_append_result(lsm_vector_result_t *results, int *count, int *cap,
                                             sqlite3_stmt *stmt,
                                             const int8_t (*kw_vecs)[VS_VEC_DIM], int actual_kw) {
    if (*count >= *cap) {
        int nc = *cap < LSM_SZ_16 ? LSM_SZ_16 : *cap * ST_COL_2;
        lsm_vector_result_t *grown = realloc(results, (size_t)nc * sizeof(lsm_vector_result_t));
        if (!grown) {
            return NULL;
        }
        results = grown;
...
int lsm_store_vector_search(lsm_store_t *s, const char *project, const char **keywords,
    ...
    *out_count = 0;
    ...
    const char *sql = "SELECT n.id, n.name, n.qualified_name, n.file_path, n.label,"
                      "       lsm_cosine_i8(v.vector, ?1) as score, v.vector"
                      " FROM node_vectors v"
    ...
    int fetch_limit = (limit > 0 ? limit : LSM_SZ_16) * ST_COL_5;
...

    {
        ...
        lsm_log_info("vector_search.exec", "kw_count", kw_buf, "fetch_limit", fl_buf, "project",
                     project);
    }

    lsm_vector_result_t *results = NULL;
    int count = 0;
    int cap = 0;
    int step_rc = 0;
    while ((step_rc = sqlite3_step(stmt)) == SQLITE_ROW) {
        lsm_vector_result_t *grown =
    ...
    if (step_rc != SQLITE_DONE) {
        ...
        lsm_log_warn("vector_search.step_error", "rc", rc_buf, "msg", sqlite3_errmsg(s->db));
    }
    {
        char cnt_buf[VS_STR_BUF];
        snprintf(cnt_buf, sizeof(cnt_buf), "%d", count);
        lsm_log_info("vector_search.done", "candidates", cnt_buf);
    }
    sqlite3_finalize(stmt);
    ...
    for (int i = 0; i < count - SKIP_ONE; i++) {
        for (int j = i + SKIP_ONE; j < count; j++) {
            if (results[j].score > results[i].score) {
                lsm_vector_result_t tmp = results[i];
                results[i] = results[j];
                results[j] = tmp;
            }
        }
    ...
    int final_limit = limit > 0 ? limit : LSM_SZ_16;
