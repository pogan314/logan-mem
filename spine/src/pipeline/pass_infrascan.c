...
bool lsm_is_k8s_manifest(const char *name, const char *content) {
    if (!name || !content || lsm_is_kustomize_file(name)) {
        return false;
    }
...
bool lsm_is_secret_binding(const char *key, const char *value) {
    ...
    if (value && lsm_is_secret_value(value)) {
        return true;
    }
    return false;
}
...
static void df_parse_from(const char *line, lsm_dockerfile_result_t *r) {
...
static void df_parse_expose(const char *line, lsm_dockerfile_result_t *r) {
...
static void df_parse_env(const char *line, lsm_dockerfile_result_t *r) {
    ...
    /* Filter secrets */
    if (lsm_is_secret_binding(key, value)) {
        return;
    }
...
static void df_parse_arg(const char *line, lsm_dockerfile_result_t *r) {
    ...
    if (r->build_arg_count >= LSM_SZ_32) {
        return;
    }
...
static void df_parse_json_cmd(const char *line, int prefix_len, char *out, size_t out_sz) {
    ...
    rtrim(raw);
    lsm_clean_json_brackets(raw, out, out_sz);
}

static void df_parse_directives(const char *line, lsm_dockerfile_result_t *r) {
...
static void strip_surrounding_quotes(char *value) {
    ...
    if (vlen >= LSM_QUOTE_PAIR && ((value[0] == '"' && value[vlen - SKIP_ONE] == '"') ||
                                   (value[0] == '\'' && value[vlen - SKIP_ONE] == '\''))) {
        memmove(value, value + LSM_QUOTE_OFFSET, vlen - LSM_QUOTE_PAIR);
        value[vlen - IS_QUOTE_TRIM] = '\0';
    }
}

/* Parse a single dotenv line: KEY=VALUE. Returns 1 if env var added. */
static int parse_dotenv_line(const char *trimmed, lsm_dotenv_result_t *out) {
    char key[LSM_SZ_128];
    int klen = 0;
    ...
    if (lsm_is_secret_binding(key, value) || out->env_count >= LSM_SZ_64) {
        return 0;
    }
...
static void shell_parse_export(const char *line, lsm_shell_result_t *r) {
    /* export VAR=VALUE */
...
static void shell_parse_plain_var(const char *line, lsm_shell_result_t *r) {
...
static void shell_parse_source(const char *line, lsm_shell_result_t *r) {
    /* source <file> or . <file> */
    const char *p = NULL;
    ...
    if (plen >= LSM_QUOTE_PAIR && ((path[0] == '"' && path[plen - SKIP_ONE] == '"') ||
                                   (path[0] == '\'' && path[plen - SKIP_ONE] == '\''))) {
        memmove(path, path + LSM_QUOTE_OFFSET, plen - LSM_QUOTE_PAIR);
...
static void shell_parse_docker(const char *line, lsm_shell_result_t *r) {
...
static bool shell_try_shebang(const char *trimmed, lsm_shell_result_t *out, bool *shebang_checked) {
    if (*shebang_checked) {
        return false;
    }
...
static void shell_dispatch_line(const char *trimmed, lsm_shell_result_t *out) {
...
static bool tf_detect_block_header(const char *trimmed, lsm_terraform_result_t *out,
                                   tf_block_kind_t *cur_block, int *cur_var_idx, int *cur_mod_idx) {
    char ident1[LSM_SZ_128];

    /* resource "type" "name" */
    {
        int rc = tf_extract_ident(trimmed, "resource", ident1, sizeof(ident1));
        if (rc > 0 && out->resource_count < LSM_SZ_32) {
            char ident2[LSM_SZ_128];
    ...
    if (consumed > 0 && out->variable_count < LSM_SZ_32) {
...
static void tf_parse_block_attrs(const char *trimmed, lsm_terraform_result_t *out,
                                 tf_block_kind_t cur_block, int cur_var_idx, int cur_mod_idx) {
    switch (cur_block) {
    ...
    case TF_BLOCK_TERRAFORM: {
        ...
        char backend_name[LSM_SZ_128] = {0};
...
static bool tf_result_has_content(const lsm_terraform_result_t *out) {
...
char *lsm_infra_qn(const char *project_name, const char *rel_path, const char *infra_type,
    ...
    char *base = lsm_pipeline_fqn_compute(project_name, rel_path, "");
    if (!base) {
        return NULL;
    }

    char result[LSM_SZ_1K];
        ...
        if (content < eol && *content != '#') {
            if (indent == 0) {
                ...
                if (!in_deps && nv && out->chart_name[0] == '\0') {
                    helm_copy_scalar(nv, eol, out->chart_name, LSM_HELM_NAME_MAX);
                }
            } else if (in_deps) {
                ...
                if (nv && out->dep_count < LSM_HELM_MAX_DEPS) {
                    char buf[LSM_HELM_NAME_MAX];
                    helm_copy_scalar(nv, eol, buf, LSM_HELM_NAME_MAX);
                    if (buf[0]) {
                        memcpy(out->deps[out->dep_count], buf, LSM_HELM_NAME_MAX);
                        out->dep_count++;
                    }
                }
            }
