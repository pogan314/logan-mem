...
#include "pipeline/pipeline_internal.h" // lsm_route_canon_path
...
static LSM_TLS lsm_cross_repo_after_insert_test_hook_t cr_after_insert_test_hook = NULL;
static LSM_TLS void *cr_after_insert_test_context = NULL;
...
static LSM_TLS char cr_ibuf[LSM_SZ_32];
...
static bool cr_store_has_exact_project(lsm_store_t *store, const char *project) {
    lsm_project_t *projects = NULL;
    int count = 0;
    if (!store || !lsm_store_check_integrity(store) ||
        lsm_store_list_projects(store, &projects, &count) != LSM_STORE_OK) {
...
static bool cr_project_exists(const char *project) {
    ...
    lsm_store_t *store = lsm_store_open_path_query(path);
    bool exists = cr_store_has_exact_project(store, project);
    lsm_store_close(store);
    return exists;
}

static lsm_store_t *cr_open_existing_project(const char *project) {
    char path[CR_PATH_BUF];
    if (!cr_db_path(project, path, sizeof(path))) {
        return NULL;
    }
    lsm_store_t *store = lsm_store_open_path_existing(path);
...
static cr_run_status_t delete_cross_edges(lsm_store_t *store, const char *project,
    ...
    for (size_t i = 0; i < sizeof(edge_types) / sizeof(edge_types[0]); i++) {
        ...
        if (lsm_store_delete_edges_by_type(store, project, edge_types[i]) != LSM_STORE_OK) {
            return CR_RUN_FAILED;
        }
 ...
 * table is UNIQUE(source_id, target_id, type) and lsm_store_insert_edge
 * upserts on conflict, so a pair reached from both match directions or on a
 * repeat run never duplicates a row. (#523) */
static bool insert_cross_edge(lsm_store_t *store, const char *project, int64_t from_id,
    ...
    bool inserted = lsm_store_insert_edge(store, &edge) > 0;
    if (inserted) {
        ctx->mutated = true;
 ...
 * lsm_route_canon_path only canonicalizes placeholder syntax, never strips
...
static bool lookup_node_info(struct sqlite3 *db, int64_t node_id, char *name_out, size_t name_sz,
    ...
    if (sqlite3_prepare_v2(db, "SELECT name, file_path FROM nodes WHERE id = ?1", LSM_NOT_FOUND,
                           &st, NULL) != SQLITE_OK) {
        return false;
    }
...
static int64_t find_route_handler(lsm_store_t *target_store, const char *route_qn,
                                  char *handler_name, size_t name_sz, char *handler_file,
                                  size_t file_sz, bool *failed) {
    *failed = false;
    handler_name[0] = '\0';
    handler_file[0] = '\0';
    struct sqlite3 *db = lsm_store_get_db(target_store);
    if (!db) {
        *failed = true;
        return 0;
    }
    ...
    if (sqlite3_prepare_v2(
            ...
            LSM_NOT_FOUND, &s, NULL) != SQLITE_OK) {
        *failed = true;
        return 0;
    }
    if (sqlite3_bind_text(s, SKIP_ONE, route_qn, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
        *failed = true;
        return 0;
    }
...
static int64_t find_route_handler_fuzzy(lsm_store_t *target_store, const char *concrete_path,
...
static bool emit_cross_route_bidirectional(
    lsm_store_t *src_store, const char *src_project, struct sqlite3 *src_db, int64_t caller_id,
    int64_t local_route_id, lsm_store_t *tgt_store, const char *tgt_project, int64_t handler_id,
    ...
    struct sqlite3 *tgt_db = lsm_store_get_db(tgt_store);
    if (!tgt_db) {
        return false;
    }
    sqlite3_stmt *rq = NULL;
    if (sqlite3_prepare_v2(tgt_db, "SELECT id FROM nodes WHERE qualified_name = ?1 LIMIT 1",
                           LSM_NOT_FOUND, &rq, NULL) != SQLITE_OK) {
        return false;
    }
    if (sqlite3_bind_text(rq, SKIP_ONE, route_qn, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(rq);
        return false;
    }
    ...
    char caller_name[LSM_SZ_256] = {0};
    char caller_file[LSM_SZ_512] = {0};
...
static cr_match_result_t match_http_routes(lsm_store_t *src_store, const char *src_project,
                                           lsm_store_t *tgt_store, const char *tgt_project,
                                           cr_run_context_t *ctx) {
    struct sqlite3 *src_db = lsm_store_get_db(src_store);
    if (!src_db) {
    ...
    if (sqlite3_bind_text(s, SKIP_ONE, src_project, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
        return cr_match_finish(ctx, 0, true);
    }
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char url_path[LSM_SZ_256] = {0};
        char method[LSM_SZ_32] = {0};
        ...
        const char *curl = lsm_route_canon_path(cr_url_path(url_path), cpath, sizeof(cpath));
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method[0] ? method : "ANY", curl);

        char handler_name[LSM_SZ_256] = {0};
        char handler_file[LSM_SZ_512] = {0};
...
static cr_match_result_t match_async_routes(lsm_store_t *src_store, const char *src_project,
                                            lsm_store_t *tgt_store, const char *tgt_project,
                                            cr_run_context_t *ctx) {
    struct sqlite3 *src_db = lsm_store_get_db(src_store);
    if (!src_db) {
 ...
 * for a complete bidirectional pair, 0 for no match, and LSM_STORE_ERR when a
 * query or write fails. */
static int try_match_channel_listener(lsm_store_t *src_store, const char *src_project,
                                      lsm_store_t *tgt_store, const char *tgt_project,
    ...
    if (sqlite3_prepare_v2(tgt_db,
                           ...
                           LSM_NOT_FOUND, &tq, NULL) != SQLITE_OK) {
        return LSM_STORE_ERR;
    }
    if (sqlite3_bind_text(tq, SKIP_ONE, tgt_project, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK ||
        sqlite3_bind_text(tq, PAIR_LEN, channel_name, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(tq);
        return LSM_STORE_ERR;
    }
    ...
    char listener_name[LSM_SZ_256] = {0};
    char listener_file[LSM_SZ_512] = {0};
    ...
    char caller_name[LSM_SZ_256] = {0};
    char caller_file[LSM_SZ_512] = {0};
    if (!lookup_node_info(lsm_store_get_db(src_store), emitter_id, caller_name, sizeof(caller_name),
    ...
    if (!insert_cross_edge(src_store, src_project, emitter_id, channel_id, "CROSS_CHANNEL", fwd,
                           ctx)) {
        return cr_cancel_requested(ctx) ? LSM_STORE_NOT_FOUND : LSM_STORE_ERR;
    }
    if (cr_cancel_requested(ctx)) {
        return LSM_STORE_NOT_FOUND;
    }
    ...
    return insert_cross_edge(tgt_store, tgt_project, listener_id, tgt_channel_id, "CROSS_CHANNEL",
                             rev, ctx)
               ? 1
               : (cr_cancel_requested(ctx) ? LSM_STORE_NOT_FOUND : LSM_STORE_ERR);
}

static cr_match_result_t match_channels(lsm_store_t *src_store, const char *src_project,
                                        lsm_store_t *tgt_store, const char *tgt_project,
                                        cr_run_context_t *ctx) {
    struct sqlite3 *src_db = lsm_store_get_db(src_store);
    if (!src_db) {
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char *channel_name_copy = lsm_strdup(channel_name);
        if (!channel_name_copy) {
            failed = true;
            break;
        }
        char transport[LSM_SZ_64] = {0};
        ...
        if (matched == LSM_STORE_ERR) {
            failed = true;
            break;
        }
        if (matched == LSM_STORE_NOT_FOUND && cr_cancel_requested(ctx)) {
            break;
        }
        if (matched > 0) {
            count++;
...
static bool lookup_node_qn(struct sqlite3 *db, int64_t node_id, char *out, size_t out_sz,
    ...
    *failed = false;
    ...
    sqlite3_stmt *st = NULL;
    if (sqlite3_prepare_v2(db, "SELECT qualified_name FROM nodes WHERE id = ?1", LSM_NOT_FOUND, &st,
                           NULL) != SQLITE_OK) {
        *failed = true;
        return false;
    }
...
static cr_match_result_t match_typed_routes(lsm_store_t *src_store, const char *src_project,
                                            lsm_store_t *tgt_store, const char *tgt_project,
    ...
    sqlite3_stmt *s = NULL;
    if (sqlite3_prepare_v2(src_db, sql, LSM_NOT_FOUND, &s, NULL) != SQLITE_OK) {
        return cr_match_finish(ctx, 0, true);
    }
    if (sqlite3_bind_text(s, SKIP_ONE, src_project, LSM_NOT_FOUND, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(s);
    ...
    while (scanned < CR_MAX_EDGES && !cr_cancel_requested(ctx) &&
        ...
        char svc_val[LSM_SZ_256] = {0};
        char meth_val[LSM_SZ_256] = {0};
...
static int collect_all_projects(char ***out, cr_run_context_t *ctx) {
    *out = NULL;
    ...
    while ((ent = lsm_readdir(d)) != NULL) {
        ...
        char project[LSM_DIRENT_NAME_MAX];
        ...
        if (!lsm_validate_project_name(project) || !cr_project_exists(project)) {
            continue;
        }
        if (count >= cap) {
            cap *= PAIR_LEN;
        ...
        projects[count] = lsm_strdup(project);
        if (!projects[count]) {
            failed = true;
            break;
        }
...
static int collect_named_projects(const char **targets, int target_count, char ***out,
    ...
    *out = NULL;
    ...
    for (int i = 0; i < target_count; i++) {
        ...
        if (!targets[i] || strcmp(targets[i], "*") == 0 || !lsm_validate_project_name(targets[i])) {
            free_project_list(projects, count);
            return -1;
        }
        projects[count] = lsm_strdup(targets[i]);
        if (!projects[count]) {
            free_project_list(projects, count);
            return -1;
        }
...
lsm_cross_repo_result_t lsm_cross_repo_match_cancellable(const char *project,
                                                         const char **target_projects,
                                                         int target_count,
                                                         const atomic_int *cancelled) {
    lsm_cross_repo_result_t result = {0};
    cr_run_context_t run = {.cancelled = cancelled};
    struct timespec t0;
    clock_gettime(CLOCK_MONOTONIC, &t0);

    if (!project || !lsm_validate_project_name(project) || !target_projects || target_count <= 0 ||
    ...
    lsm_store_t *src_store = cr_open_existing_project(project);
    if (!src_store) {
        result.failed = true;
        free_project_list(resolved, resolved_count);
        return result;
    ...
    for (int i = 0; i < resolved_count; i++) {
        ...
        lsm_store_t *tgt_store = cr_open_existing_project(tgt);
        if (!tgt_store) {
            result.failed = true;
            break;
        }
    ...
    if (result.cancelled) {
        lsm_log_info("cross_repo.cancelled", "project", project, "partial_results",
        ...
        lsm_log_info("cross_repo.done", "project", project, "total", cr_itoa(total));
    }

    return result;
}

lsm_cross_repo_result_t lsm_cross_repo_match(const char *project, const char **target_projects,
                                             int target_count) {
    return lsm_cross_repo_match_cancellable(project, target_projects, target_count, NULL);
}
