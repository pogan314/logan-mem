...
static bool try_include_flag(lsm_compile_flags_t *f, const char **args, int argc, int *i,
...
static bool try_define_flag(lsm_compile_flags_t *f, const char **args, int argc, int *i) {
    const char *arg = args[*i];
...
lsm_compile_flags_t *lsm_extract_flags(const char **args, int argc, const char *directory) {
    lsm_compile_flags_t *f = calloc(LSM_ALLOC_ONE, sizeof(*f));
    if (!f) {
        return NULL;
    }
...
static int extract_flag_args(yyjson_val *args_val, yyjson_val *cmd_val, const char **flag_args,
    ...
    if (cmd_val && yyjson_is_str(cmd_val)) {
        int n = lsm_split_command(yyjson_get_str(cmd_val), split_args, max_args);
        for (int j = 0; j < n; j++) {
...
static int process_compile_entry(yyjson_val *entry, const char *repo_path, char **out_path,
    ...
    char *split_args[LSM_SZ_256] = {NULL};
    const char *flag_args[LSM_SZ_256];
    int flag_argc = extract_flag_args(args_val, cmd_val, flag_args, split_args, LSM_SZ_256);

    if (flag_argc == 0) {
        return 0;
    }

    lsm_compile_flags_t *f = lsm_extract_flags(flag_args, flag_argc, directory);
...
int lsm_parse_compile_commands(const char *json_data, const char *repo_path, char ***out_paths,
    ...
    *out_flags = NULL;
    ...
    lsm_compile_flags_t **flags = calloc(arr_len, sizeof(lsm_compile_flags_t *));
    int count = 0;
