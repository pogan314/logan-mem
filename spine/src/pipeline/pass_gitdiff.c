...
void lsm_parse_range(const char *s, int *out_start, int *out_count) {
    ...
    *out_count = SKIP_ONE;
    ...
    if (!comma) {
        *out_start = (int)strtol(s, NULL, LSM_DECIMAL_BASE);
        return;
    }
    ...
    memcpy(buf, s, len);
    buf[len] = '\0';
    *out_start = (int)strtol(buf, NULL, LSM_DECIMAL_BASE);

    /* Parse count */
    *out_count = (int)strtol(comma + SKIP_ONE, NULL, LSM_DECIMAL_BASE);
}

#define HUNK_LINE_BUF 1536

/* Parse a single name-status line into a lsm_changed_file_t.
 * Returns true if a valid entry was produced. */
static bool parse_one_name_status(const char *line, size_t line_len, lsm_changed_file_t *out_f) {
    ...
    *tab1 = '\0';
    ...
    return lsm_is_trackable_file(out_f->path);
}
...
static bool parse_hunk_line(const char *line, size_t line_len, const char *current_file,
    ...
    int start;
    int cnt;
    lsm_parse_range(range_str, &start, &cnt);

    if (start <= 0 || !lsm_is_trackable_file(current_file)) {
        return false;
    }
...
int lsm_parse_hunks(const char *output, lsm_changed_hunk_t *out, int max_out) {
    ...
    int count = 0;
    char current_file[LSM_SZ_512] = {0};
    const char *line = output;
