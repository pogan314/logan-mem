...
static bool is_dep_section(const char *s) {
    ...
    for (int i = 0; secs[i]; i++) {
        if (lsm_strcasestr(s, secs[i]) != NULL) {
            return true;
        }
    }
    return false;
...
static void log_candidate_truncation(const char *side, int cap) {
    ...
    lsm_log_info("configlinker.truncated", "side", side, "cap", cap_buf);
}
...
static const lsm_gbuf_node_t **canonical_node_copy(const lsm_gbuf_node_t *const *nodes, int count) {
    if (!nodes || count <= 0) {
        return NULL;
    }
    const lsm_gbuf_node_t **sorted = malloc((size_t)count * sizeof(*sorted));
    if (!sorted) {
        return NULL;
    }
...
typedef struct {
    int64_t node_id;
    char normalized[LSM_SZ_256];
    char name[LSM_SZ_256];
} config_entry_t;

/* Collect config Variable nodes with ≥2 tokens, each ≥3 chars. */
static int collect_config_entries(const lsm_gbuf_node_t *const *vars, int var_count,
                                  config_entry_t *out, int max_out) {
    int n = 0;
    const lsm_gbuf_node_t **sorted = canonical_node_copy(vars, var_count);
    if (sorted) {
        vars = sorted;
    }
    for (int i = 0; i < var_count && n < max_out; i++) {
        if (!lsm_has_config_extension(vars[i]->file_path)) {
            continue;
        }

        char norm[LSM_SZ_256];
        int tokens = lsm_normalize_config_key(vars[i]->name, norm, sizeof(norm));
        if (tokens < PAIR_LEN) {
            continue;
        }
...
static int collect_code_entries(lsm_gbuf_t *gb, code_entry_t *out, int max_out) {
    int n = 0;
    ...
    for (int li = 0; labels[li] && n < max_out; li++) {
        ...
        const lsm_gbuf_node_t **sorted = canonical_node_copy(nodes, count);
        const lsm_gbuf_node_t *const *scan = sorted ? sorted : nodes;

        for (int i = 0; i < count && n < max_out; i++) {
            if (lsm_has_config_extension(scan[i]->file_path)) {
                continue;
            }

            char norm[LSM_SZ_256];
            int tokens = lsm_normalize_config_key(scan[i]->name, norm, sizeof(norm));
...
static int strategy_key_symbols(lsm_gbuf_t *gb) {
    /* Get all Variable nodes */
    const lsm_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (lsm_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    }

    config_entry_t config_entries[LSM_SZ_4K];
    int config_count = collect_config_entries(vars, var_count, config_entries, LSM_SZ_4K);

    if (config_count == 0) {
        return 0;
    }

    code_entry_t code_entries[LSM_SZ_8K];
    int code_count = collect_code_entries(gb, code_entries, LSM_SZ_8K);
    ...
    for (int ci = 0; ci < config_count; ci++) {
        for (int co = 0; co < code_count; co++) {
            ...
            if (confidence > 0.0) {
                ...
                lsm_gbuf_insert_edge(gb, code_entries[co].node_id, config_entries[ci].node_id,
                                     "CONFIGURES", props);
                edge_count++;
            }
        }
...
static int collect_manifest_deps(const lsm_gbuf_node_t *const *vars, int var_count,
...
static double match_dep_to_import(const lsm_gbuf_node_t *target, const char *dep_lower) {
    char target_lower[LSM_SZ_256];
...
static int strategy_dep_imports(lsm_gbuf_t *gb) {
    const lsm_gbuf_node_t **vars = NULL;
    int var_count = 0;
    if (lsm_gbuf_find_by_label(gb, "Variable", &vars, &var_count) != 0) {
        return 0;
    ...
    dep_entry_t deps[LSM_SZ_2K];
    int dep_count = collect_manifest_deps(vars, var_count, deps, LSM_SZ_2K);
    ...
    const lsm_gbuf_edge_t **imports = NULL;
    int import_count = 0;
    if (lsm_gbuf_find_edges_by_type(gb, "IMPORTS", &imports, &import_count) != 0) {
        return 0;
    }
    ...
    for (int di = 0; di < dep_count; di++) {
        char dep_lower[LSM_SZ_256];
        lowercase_into(dep_lower, sizeof(dep_lower), deps[di].name);

        for (int ii = 0; ii < import_count; ii++) {
            const lsm_gbuf_node_t *target = lsm_gbuf_find_by_id(gb, imports[ii]->target_id);
            if (!target) {
                continue;
            }

            const lsm_gbuf_node_t *source = lsm_gbuf_find_by_id(gb, imports[ii]->source_id);
            if (!source) {
                continue;
            }
            ...
            if (confidence > 0.0) {
                ...
                lsm_gbuf_insert_edge(gb, source->id, deps[di].node_id, "CONFIGURES", props);
                edge_count++;
            }
        }
    }
...
int lsm_pipeline_pass_configlink(lsm_pipeline_ctx_t *ctx) {
    ...
    const lsm_gbuf_node_t **vars_check = NULL;
    int var_check_count = 0;
    if (!has_config && lsm_gbuf_find_by_label(gb, "Variable", &vars_check, &var_check_count) == 0) {
        for (int i = 0; i < var_check_count; i++) {
            if (lsm_has_config_extension(vars_check[i]->file_path)) {
                has_config = true;
                break;
            }
        }
    ...
    if (!has_config) {
        const lsm_gbuf_node_t **mods_check = NULL;
        int mod_check_count = 0;
        if (lsm_gbuf_find_by_label(gb, "Module", &mods_check, &mod_check_count) == 0) {
            for (int i = 0; i < mod_check_count; i++) {
                if (lsm_has_config_extension(mods_check[i]->file_path)) {
                    has_config = true;
                    break;
                }
            }
    ...
    if (!has_config) {
        lsm_log_info("configlinker.skip", "reason", "no_config_files");
        return 0;
    }
    ...
    lsm_log_info("configlinker.strategy", "name", "key_symbol", "edges", buf1);

    int dep_edges = strategy_dep_imports(gb);
    snprintf(buf2, sizeof(buf2), "%d", dep_edges);
    lsm_log_info("configlinker.strategy", "name", "dep_import", "edges", buf2);
    ...
    lsm_log_info("configlinker.strategy", "name", "file_ref", "edges", buf3);

    snprintf(buf4, sizeof(buf4), "%d", key_edges + dep_edges + ref_edges);
    lsm_log_info("configlinker.done", "total", buf4);
