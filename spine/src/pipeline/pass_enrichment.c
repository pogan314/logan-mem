...
int lsm_split_camel_case(const char *s, char **out, int max_out) {
    ...
    for (size_t i = SKIP_ONE; i < len; i++) {
        if (s[i] >= 'A' && s[i] <= 'Z' && s[i - SKIP_ONE] >= 'a' && s[i - SKIP_ONE] <= 'z') {
            if (count < max_out) {
                out[count] = lsm_strndup(s + start, i - start);
                count++;
            }
            start = i;
        }
    ...
    /* Emit remaining */
    if (count < max_out) {
        out[count] = lsm_strndup(s + start, len - start);
        count++;
    }
    return count;
}
...
int lsm_tokenize_decorator(const char *dec, char **out, int max_out) {
    ...
    while (part && count < max_out) {
        ...
        int camel_count = lsm_split_camel_case(part, camel_parts, ENRICH_MAX_CAMEL);
...
static int extract_decorator_words(const char *json, char ***out_words) {
    ...
    char *all_words[LSM_SZ_256];
    int total = 0;
    LSMHashTable *seen = lsm_ht_create(LSM_SZ_32);

    for (int i = 0; decorators[i]; i++) {
        char *tokens[LSM_SZ_32];
        int tc = lsm_tokenize_decorator(decorators[i], tokens, LSM_SZ_32);
        for (int j = 0; j < tc; j++) {
            if (!lsm_ht_get(seen, tokens[j]) && total < LSM_SZ_256) {
                lsm_ht_set(seen, tokens[j], intptr_to_ptr(SKIP_ONE));
...
static int collect_decorated_nodes(lsm_gbuf_t *gbuf, tagged_node_t **out_nodes,
                                   LSMHashTable *word_counts) {
    ...
    for (int l = 0; l < nlabels; l++) {
        ...
        int fc = 0;
        lsm_gbuf_find_by_label(gbuf, labels[l], &found, &fc);
        if (!found || fc <= 0) {
            continue;
        }
        ...
        for (int i = 0; i < fc; i++) {
            ...
            if (node_count >= node_cap) {
                node_cap = node_cap ? node_cap * PAIR_LEN : LSM_SZ_64;
            ...
            tn->word_count = wc;
            for (int w = 0; w < wc; w++) {
                intptr_t cnt = (intptr_t)lsm_ht_get(word_counts, words[w]);
                lsm_ht_set(word_counts, words[w], intptr_to_ptr(cnt + SKIP_ONE));
            }
        }
    }
...
static int apply_decorator_tags(lsm_gbuf_t *gbuf, tagged_node_t *nodes, int node_count,
                                LSMHashTable *candidates) {
    int tagged = 0;
    for (int n = 0; n < node_count; n++) {
        char *tag_words[LSM_SZ_256];
        int tag_count = 0;
        for (int w = 0; w < nodes[n].word_count; w++) {
            if (lsm_ht_get(candidates, nodes[n].words[w]) && tag_count < LSM_SZ_256) {
        ...
        const lsm_gbuf_node_t *gn = lsm_gbuf_find_by_qn(gbuf, nodes[n].qualified_name);
        if (!gn) {
            continue;
        }
        ...
        if (new_props) {
            lsm_gbuf_upsert_node(gbuf, gn->label, gn->name, gn->qualified_name, gn->file_path,
...
int lsm_pipeline_pass_decorator_tags(lsm_gbuf_t *gbuf, const char *project) {
    ...
    LSMHashTable *word_counts = lsm_ht_create(LSM_SZ_128);
    tagged_node_t *nodes = NULL;
    int node_count = collect_decorated_nodes(gbuf, &nodes, word_counts);
    if (node_count == 0) {
        lsm_ht_free(word_counts);
        free(nodes);
        return 0;
    }

    /* Phase 2: Determine candidates (words on 2+ nodes) */
    LSMHashTable *candidates = lsm_ht_create(LSM_SZ_64);
    int candidate_count = 0;
    for (int n = 0; n < node_count; n++) {
        for (int w = 0; w < nodes[n].word_count; w++) {
            const char *word = nodes[n].words[w];
            intptr_t cnt = (intptr_t)lsm_ht_get(word_counts, word);
            if (cnt >= PAIR_LEN && !lsm_ht_get(candidates, word)) {
                lsm_ht_set(candidates, word, intptr_to_ptr(SKIP_ONE));
                candidate_count++;
            }
        }
    }
    ...
    lsm_ht_free(word_counts);
    lsm_ht_free(candidates);

    lsm_log_info("pass.decorator_tags", "candidates", candidate_count > 0 ? "yes" : "0", "tagged",
                 tagged > 0 ? "yes" : "0");
    return tagged;
}
