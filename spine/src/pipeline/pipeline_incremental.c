...
#define LSM_NS_PER_MS 1000000.0
...
static atomic_int g_incr_test_last_route = LSM_INCREMENTAL_ROUTE_NONE;
...
lsm_incremental_route_t lsm_pipeline_incremental_test_last_route(void) {
    return (lsm_incremental_route_t)atomic_load(&g_incr_test_last_route);
}
...
void lsm_pipeline_incremental_test_reset_faults(void) {
    ...
    atomic_store(&g_incr_test_last_route, LSM_INCREMENTAL_ROUTE_NONE);
    lsm_pipeline_persist_test_reset_faults();
}
...
static void incr_test_set_last_route(lsm_incremental_route_t route) {
...
static double elapsed_ms(struct timespec start) {
    ...
    return (s * LSM_MS_PER_SEC) + (ns / LSM_NS_PER_MS);
}
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * LSM_NS_PER_SEC) + (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * LSM_NS_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * LSM_NS_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}
...
static int semantic_manifest_hash_file(const char *abs_path, char out[LSM_SHA256_HEX_LEN + 1],
                                       int64_t *mtime_ns, int64_t *size) {
    for (int attempt = 0; attempt < 2; attempt++) {
        lsm_path_info_t before;
        if (lsm_path_info_utf8(abs_path, &before) != 0 || !before.is_regular || before.is_symlink) {
            return LSM_NOT_FOUND;
        }
        FILE *f = lsm_fopen(abs_path, "rb");
        if (!f) {
            return LSM_NOT_FOUND;
        }
        lsm_sha256_ctx sha;
        lsm_sha256_init(&sha);
        unsigned char buf[LSM_SZ_64K];
        size_t n;
        while ((n = fread(buf, 1, sizeof(buf), f)) > 0) {
            lsm_sha256_update(&sha, buf, n);
        }
        bool read_ok = !ferror(f);
        if (fclose(f) != 0) {
            read_ok = false;
        }
        lsm_path_info_t after;
        if (!read_ok || lsm_path_info_utf8(abs_path, &after) != 0 || !after.is_regular ||
            after.is_symlink) {
            return LSM_NOT_FOUND;
        }
    ...
    lsm_log_warn("semantic_manifest.raced", "path", abs_path);
    return LSM_NOT_FOUND;
}
...
static int semantic_manifest_add_digest(semantic_manifest_builder_t *builder, const char *project,
    ...
    if (!builder || !project || !rel_path || !rel_path[0] || !sha ||
        strlen(sha) != LSM_SHA256_HEX_LEN || lsm_ht_has(builder->seen_paths, rel_path)) {
        return LSM_NOT_FOUND;
    }
    for (int i = 0; i < LSM_SHA256_HEX_LEN; i++) {
        if (!((sha[i] >= '0' && sha[i] <= '9') || (sha[i] >= 'a' && sha[i] <= 'f'))) {
            return LSM_NOT_FOUND;
        }
    }
    if (builder->count == builder->cap) {
        int new_cap = builder->cap > 0 ? builder->cap * 2 : LSM_SZ_64;
        lsm_file_hash_t *grown = realloc(builder->items, (size_t)new_cap * sizeof(*builder->items));
        if (!grown) {
            return LSM_NOT_FOUND;
        }
        builder->items = grown;
    ...
    uint32_t before_count = lsm_ht_count(builder->seen_paths);
    lsm_ht_set(builder->seen_paths, path_copy, path_copy);
    if (lsm_ht_count(builder->seen_paths) != before_count + 1U) {
        free(project_copy);
        free(path_copy);
        free(sha_copy);
        return LSM_NOT_FOUND;
    }
    builder->items[builder->count++] = (lsm_file_hash_t){
...
static int semantic_manifest_add(semantic_manifest_builder_t *builder, const char *project,
    ...
    if (lsm_ht_has(builder->seen_paths, rel_path)) {
        return 0;
    }
    ...
    if (semantic_manifest_hash_file(abs_path, sha, &mtime_ns, &size) != 0) {
        lsm_log_error("semantic_manifest.err", "path", abs_path);
        return LSM_NOT_FOUND;
    }
    return semantic_manifest_add_digest(builder, project, rel_path, sha, mtime_ns, size);
}

static void semantic_manifest_git_digest(const lsm_git_context_t *git_ctx,
                                         char out[LSM_SHA256_HEX_LEN + 1]) {
    static const char domain[] = "lsm-semantic-git-context-v1";
    lsm_sha256_ctx sha;
    lsm_sha256_init(&sha);
...
#define HASH_GIT_BOOL(field)                                      \
    do {                                                          \
        ...
        lsm_sha256_update(&sha, #field, sizeof(#field));          \
        lsm_sha256_update(&sha, &value, sizeof(value));           \
    } while (0)
#define HASH_GIT_STRING(field)                                               \
    do {                                                                     \
        const char *value = git_ctx && git_ctx->field ? git_ctx->field : ""; \
        lsm_sha256_update(&sha, #field, sizeof(#field));                     \
        lsm_sha256_update(&sha, value, strlen(value) + 1);                   \
    } while (0)
...
static int semantic_manifest_walk_controls(semantic_manifest_builder_t *builder,
    ...
    int rc = 0;
    lsm_dirent_t *entry;
    while (rc == 0 && (entry = lsm_readdir(dir)) != NULL) {
        const char *name = entry->name;
        if (strcmp(name, ".") == 0 || strcmp(name, "..") == 0) {
            continue;
        }
        char abs_path[LSM_SZ_4K];
        char rel_path[LSM_SZ_4K];
        ...
        lsm_path_info_t path_info;
        if (lsm_path_info_utf8(abs_path, &path_info) != 0) {
            rc = LSM_NOT_FOUND;
            break;
        }
        ...
        bool root_control =
            (!rel_dir || !rel_dir[0]) &&
            (strcmp(name, ".lsmignore") == 0 || strcmp(name, ".logan-spine.json") == 0);
...
typedef struct {
    ...
    int worker_id;
    int worker_count;
    char (*shas)[LSM_SHA256_HEX_LEN + 1];
    int64_t *mtimes;
    int64_t *sizes;
    int8_t *rcs;
...
int lsm_pipeline_build_semantic_manifest(const char *project, const char *repo_path,
    ...
    *out_count = 0;
    ...
    uint64_t reserve = (uint64_t)(file_count > 0 ? file_count : 0) * 2U + LSM_SZ_64;
    if (reserve > UINT32_MAX) {
        reserve = UINT32_MAX;
    }
    builder.seen_paths = lsm_ht_create((uint32_t)reserve);
    if (!builder.seen_paths) {
        return LSM_NOT_FOUND;
    }
    int rc = 0;
    char git_digest[LSM_SHA256_HEX_LEN + 1];
    char absent_config_digest[LSM_SHA256_HEX_LEN + 1];
    semantic_manifest_git_digest(git_ctx, git_digest);
    lsm_sha256_hex("lsm-userconfig-unavailable-v1", strlen("lsm-userconfig-unavailable-v1"),
    ...
    rc = semantic_manifest_add_digest(&builder, project, LSM_SEMANTIC_INPUT_GIT_CONTEXT, git_digest,
                                      0, 0);
    if (rc == 0) {
        rc = semantic_manifest_add_digest(&builder, project, LSM_SEMANTIC_INPUT_GLOBAL_CONFIG,
                                          global_config_digest, 0, 0);
    }
    if (rc == 0) {
        rc = semantic_manifest_add_digest(&builder, project, LSM_SEMANTIC_INPUT_PROJECT_CONFIG,
                                          project_config_digest, 0, 0);
    }
    struct timespec t_hash;
    lsm_clock_gettime(CLOCK_MONOTONIC, &t_hash);
    int hash_workers = lsm_default_worker_count(true);
    if (rc == 0 && file_count >= MANIFEST_PARALLEL_MIN_FILES && hash_workers > SKIP_ONE) {
        char (*shas)[LSM_SHA256_HEX_LEN + 1] = malloc((size_t)file_count * sizeof(*shas));
        ...
        lsm_thread_t *threads = calloc((size_t)hash_workers, sizeof(lsm_thread_t));
        ...
        if (shas && mtimes && sizes && rcs && threads && workers) {
            ...
            for (int w = 0; w < hash_workers; w++) {
                workers[w] = (manifest_hash_worker_t){.files = files,
                                                      ...
                                                      .sizes = sizes,
                                                      .rcs = rcs};
                if (lsm_thread_create(&threads[w], 0, manifest_hash_worker, &workers[w]) != 0) {
                    break;
                }
                spawned++;
            }
            ...
            for (int i = 0; rc == 0 && i < file_count; i++) {
                ...
                if (rcs[i] != 0) {
                    lsm_log_error("semantic_manifest.err", "path", files[i].path);
                    rc = LSM_NOT_FOUND;
                    break;
                }
                if (lsm_ht_has(builder.seen_paths, files[i].rel_path)) {
                    continue;
                }
    ...
    lsm_log_info("pass.timing", "pass", "semantic_manifest_hash", "files", itoa_buf(file_count),
    ...
    if (rc == 0) {
        lsm_path_alias_collection_t *aliases =
            lsm_load_path_aliases_excluded(repo_path, excluded_dirs, excluded_count);
        if (aliases) {
            ...
            lsm_path_alias_collection_free(aliases);
        }
    }
    if (rc != 0) {
        lsm_ht_free(builder.seen_paths);
        lsm_pipeline_free_semantic_manifest(builder.items, builder.count);
        return rc;
    }
    lsm_ht_free(builder.seen_paths);
...
bool lsm_pipeline_semantic_manifests_equal(const lsm_file_hash_t *left, int left_count,
    ...
    uint64_t reserve = (uint64_t)(right_count > 0 ? right_count : 0) * 2U + LSM_SZ_64;
    if (reserve > UINT32_MAX) {
        reserve = UINT32_MAX;
    }
    LSMHashTable *by_path = lsm_ht_create((uint32_t)reserve);
    if (!by_path) {
        return false;
    }
    bool equal = true;
    for (int i = 0; equal && i < right_count; i++) {
        if (!right[i].rel_path || !right[i].sha256 || lsm_ht_has(by_path, right[i].rel_path)) {
            equal = false;
            break;
        }
        uint32_t before = lsm_ht_count(by_path);
        lsm_ht_set(by_path, right[i].rel_path, (void *)right[i].sha256);
        equal = lsm_ht_count(by_path) == before + 1U;
    }
    for (int i = 0; equal && i < left_count; i++) {
        ...
        const char *sha = lsm_ht_get(by_path, left[i].rel_path);
...
int lsm_pipeline_build_fresh_semantic_manifest(const char *project, const char *repo_path, int mode,
    ...
    *out_count = 0;
    lsm_discover_opts_t opts = {
        .mode = (lsm_index_mode_t)mode,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    lsm_file_info_t *fresh_files = NULL;
    int fresh_file_count = 0;
    char **fresh_excluded = NULL;
    int fresh_excluded_count = 0;
    lsm_ignored_file_t *fresh_ignored = NULL;
    int fresh_ignored_count = 0;
    int fresh_ignored_total = 0;
    lsm_userconfig_t *fresh_userconfig = lsm_userconfig_load(repo_path);
    lsm_git_context_t fresh_git_ctx = {0};
    const lsm_userconfig_t *previous_userconfig = lsm_get_user_lang_config();
    int rc = fresh_userconfig ? lsm_git_context_resolve(repo_path, &fresh_git_ctx) : LSM_NOT_FOUND;
    if (rc == 0) {
        lsm_set_user_lang_config(fresh_userconfig);
        rc = lsm_discover_ex2(repo_path, &opts, &fresh_files, &fresh_file_count, &fresh_excluded,
                              &fresh_excluded_count, &fresh_ignored, &fresh_ignored_count,
                              &fresh_ignored_total);
    }
    if (rc == 0) {
        rc = lsm_pipeline_build_semantic_manifest(project, repo_path, fresh_files, fresh_file_count,
                                                  fresh_excluded, fresh_excluded_count,
                                                  &fresh_git_ctx, fresh_userconfig, out, out_count);
    }
    lsm_set_user_lang_config(previous_userconfig);
    lsm_git_context_free(&fresh_git_ctx);
    lsm_userconfig_free(fresh_userconfig);
    lsm_discover_free(fresh_files, fresh_file_count);
    lsm_discover_free_excluded(fresh_excluded, fresh_excluded_count);
    lsm_discover_free_ignored(fresh_ignored, fresh_ignored_count);
    return rc;
}
...
static bool *classify_files(lsm_file_info_t *files, int file_count, lsm_file_hash_t *stored,
    ...
    LSMHashTable *ht =
        lsm_ht_create(stored_count > 0 ? (size_t)stored_count * PAIR_LEN : LSM_SZ_64);
    for (int i = 0; i < stored_count; i++) {
        lsm_ht_set(ht, stored[i].rel_path, &stored[i]);
    }

    for (int i = 0; i < file_count; i++) {
        lsm_file_hash_t *h = lsm_ht_get(ht, files[i].rel_path);
        if (!h) {
            /* New file */
            changed[i] = true;
            n_changed++;
 ...
 * claude-connectors/logan-spine-index-repository-is-destructive-...
 ...
 *   - snprintf truncation (combined path ≥ LSM_SZ_4K) → preserve. We can't
...
static int find_deleted_files(const char *repo_path, lsm_file_info_t *files, int file_count,
                              lsm_file_hash_t *stored, int stored_count, char ***out_deleted,
                              lsm_file_hash_t **out_mode_skipped, int *out_mode_skipped_count) {
    *out_deleted = NULL;
    ...
    *out_mode_skipped_count = 0;
    ...
    if (!repo_path) {
        ...
        lsm_log_error("incremental.err", "msg", "find_deleted_files_null_repo_path");
        return 0;
    }

    LSMHashTable *current = lsm_ht_create((size_t)file_count * PAIR_LEN);
    for (int i = 0; i < file_count; i++) {
        lsm_ht_set(current, files[i].rel_path, &files[i]);
    }

    int del_count = 0;
    int del_cap = LSM_SZ_64;
    char **deleted = malloc((size_t)del_cap * sizeof(char *));
    if (!deleted) {
        lsm_log_error("incremental.err", "msg", "find_deleted_files_oom");
        lsm_ht_free(current);
        return 0;
    }

    int ms_count = 0;
    int ms_cap = LSM_SZ_64;
    lsm_file_hash_t *mode_skipped = malloc((size_t)ms_cap * sizeof(lsm_file_hash_t));
    if (!mode_skipped) {
        lsm_log_error("incremental.err", "msg", "find_deleted_files_oom_ms");
        free(deleted);
        lsm_ht_free(current);
        return 0;
    }
    ...
    for (int i = 0; i < stored_count; i++) {
        ...
        if (lsm_ht_get(current, stored[i].rel_path)) {
        ...
        if (n < 0 || n >= (int)sizeof(abs_path)) {
            ...
            lsm_log_warn("incremental.path_truncated", "rel_path", stored[i].rel_path);
            preserve = true;
        } else {
            struct stat st;
            if (stat(abs_path, &st) == 0) {
                ...
                lsm_log_warn("incremental.stat_uncertain", "rel_path", stored[i].rel_path, "errno",
                             itoa_buf(errno));
                preserve = true;
            }
        }
        ...
        if (preserve) {
            ...
            if (ms_count >= ms_cap) {
                ms_cap *= PAIR_LEN;
                lsm_file_hash_t *tmp = realloc(mode_skipped, (size_t)ms_cap * sizeof(*tmp));
                if (!tmp) {
                    lsm_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom_ms");
                    break;
                }
                mode_skipped = tmp;
            }
            ...
            if (!rp || (stored[i].sha256 && !sh)) {
                 ...
                 * orphaned-node bug. */
                lsm_log_error("incremental.err", "msg", "find_deleted_files_strdup_oom", "rel_path",
                              stored[i].rel_path);
                free(rp);
                free(sh);
                break;
        ...
        if (del_count >= del_cap) {
            ...
            if (!tmp) {
                lsm_log_error("incremental.err", "msg", "find_deleted_files_realloc_oom");
                break;
            }
            deleted = tmp;
        }
...
static void free_mode_skipped(lsm_file_hash_t *ms, int count) {
    if (!ms) {
        return;
    }
    for (int i = 0; i < count; i++) {
 ...
 *
 * The purge step (lsm_gbuf_delete_by_file) removes a changed file's nodes,
 ...
 *   - lsm_gbuf_insert_edge dedups, so re-linking an edge the resolver already
...
typedef struct {
    lsm_gbuf_t *gbuf;
    LSMHashTable *changed_paths; /* rel_path -> non-NULL sentinel (membership set) */
    lsm_saved_edge_t *items;
    int count;
    int cap;
} lsm_edge_capture_t;
...
/* lsm_gbuf_foreach_edge visitor: snapshot inbound cross-file edges into
 * changed files so they survive the purge and can be re-linked afterward. */
static void incr_capture_inbound_edge(const lsm_gbuf_edge_t *edge, void *userdata) {
    lsm_edge_capture_t *cap = (lsm_edge_capture_t *)userdata;
    if (incr_edge_type_is_recomputed(edge->type)) {
        return;
    }
    const lsm_gbuf_node_t *src = lsm_gbuf_find_by_id(cap->gbuf, edge->source_id);
    const lsm_gbuf_node_t *tgt = lsm_gbuf_find_by_id(cap->gbuf, edge->target_id);
    ...
    if (!lsm_ht_get(cap->changed_paths, tgt->file_path) ||
        lsm_ht_get(cap->changed_paths, src->file_path)) {
        return;
    }
    if (cap->count >= cap->cap) {
        int ncap = (cap->cap > 0) ? cap->cap * PAIR_LEN : LSM_SZ_64;
        lsm_saved_edge_t *tmp = realloc(cap->items, (size_t)ncap * sizeof(*tmp));
        if (!tmp) {
            lsm_log_warn("incremental.edge_snapshot_oom", "captured", itoa_buf(cap->count));
            return; /* best-effort: stop capturing, keep what we have */
        }
        cap->items = tmp;
        cap->cap = ncap;
    }
    lsm_saved_edge_t *s = &cap->items[cap->count];
...
static int incr_restore_inbound_edges(lsm_gbuf_t *gbuf, lsm_edge_capture_t *cap) {
    int restored = 0;
    for (int i = 0; i < cap->count; i++) {
        lsm_saved_edge_t *s = &cap->items[i];
        const lsm_gbuf_node_t *src = lsm_gbuf_find_by_qn(gbuf, s->source_qn);
        const lsm_gbuf_node_t *tgt = lsm_gbuf_find_by_qn(gbuf, s->target_qn);
        if (src && tgt) {
            lsm_gbuf_insert_edge(gbuf, src->id, tgt->id, s->type, s->props);
            restored++;
        }
    }
    return restored;
...
static void incr_free_edge_capture(lsm_edge_capture_t *cap) {
    for (int i = 0; i < cap->count; i++) {
...
static bool incr_label_is_registry_symbol(const char *label) {
    ...
    return label && (strcmp(label, "Function") == 0 || strcmp(label, "Method") == 0 ||
                     lsm_label_is_type_like(label) || strcmp(label, "Variable") == 0 ||
                     strcmp(label, "Field") == 0);
}

/* Callback for lsm_gbuf_foreach_node: seed the registry with the existing
...
static void registry_visitor(const lsm_gbuf_node_t *node, void *userdata) {
    lsm_registry_t *r = (lsm_registry_t *)userdata;
    if (!incr_label_is_registry_symbol(node->label)) {
        return;
    }
    lsm_registry_add(r, node->name, node->qualified_name, node->label);
}

static void free_incremental_result_cache(LSMFileResult **cache, int count) {
    if (!cache) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
typedef struct {
    ...
    lsm_lsp_surface_row_t *fresh_rows;
    int fresh_count;
    char **def_modules;
    ...
    LSMArena *rehydrate_arenas;
    int rehydrate_arena_count;
} closure_resolve_t;

typedef struct {
    lsm_file_info_t *files; /* heap array; entries borrow strings from the
                             * caller's discovery array */
    int count;
    int n_changed;
    int n_dependents;
    lsm_lsp_surface_row_t *stored_rows; /* whole previous generation */
    int stored_count;
} closure_plan_t;

static void closure_plan_free(closure_plan_t *plan) {
    free(plan->files);
    lsm_store_free_lsp_surfaces(plan->stored_rows, plan->stored_count);
...
static LSMHashTable *surface_name_set(const char *defs_json) {
    ...
    LSMHashTable *set = lsm_ht_create(LSM_SZ_64);
    if (!set) {
        yyjson_doc_free(doc);
        return NULL;
    }
    ...
    for (int a = 0; a < 2; a++) {
        ...
        yyjson_arr_foreach(arrs[a], idx, max, item) {
            ...
            if (name && name[0] && !lsm_ht_get(set, name)) {
                char *copy = strdup(name);
                if (copy) {
                    lsm_ht_set(set, copy, copy);
                }
            }
        }
    }
...
static void surface_name_set_free(LSMHashTable *set) {
    if (!set) {
        return;
    }
    lsm_ht_foreach(set, surface_name_free_visitor, NULL);
    lsm_ht_free(set);
}

typedef struct {
    LSMHashTable *stored_set;
    bool added;
...
static void surface_added_visitor(const char *key, void *value, void *userdata) {
    ...
    if (!check->added && !lsm_ht_get(check->stored_set, key)) {
        check->added = true;
    }
}
...
static int surface_added_names(const char *stored_json, const char *fresh_json, bool *out_added) {
    *out_added = true; /* fail closed */
    LSMHashTable *stored_set = surface_name_set(stored_json);
    LSMHashTable *fresh_set = surface_name_set(fresh_json);
    int rc = (stored_set && fresh_set) ? 0 : -1;
    if (rc == 0) {
        surface_added_check_t check = {.stored_set = stored_set, .added = false};
        lsm_ht_foreach(fresh_set, surface_added_visitor, &check);
...
static int run_extract_resolve(lsm_pipeline_ctx_t *ctx, lsm_file_info_t *changed_files, int ci,
...
#define MIN_FILES_FOR_PARALLEL_INCR 50
    int worker_count = lsm_default_worker_count(true);
    bool use_parallel =
        closure != NULL || (worker_count > SKIP_ONE && ci > MIN_FILES_FOR_PARALLEL_INCR);

    if (use_parallel) {
        lsm_log_info("incremental.mode", "mode", "parallel", "workers", itoa_buf(worker_count),
                     "changed", itoa_buf(ci));

        _Atomic int64_t shared_ids;
        atomic_init(&shared_ids, lsm_gbuf_next_id(ctx->gbuf));
...
#endif
        {
            cache = (LSMFileResult **)calloc((size_t)ci, sizeof(LSMFileResult *));
        }
        if (!cache) {
            lsm_log_error("incremental.err", "phase", "result_cache_alloc");
            return LSM_NOT_FOUND;
        }

        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        int rc = lsm_parallel_extract(ctx, changed_files, ci, cache, &shared_ids, worker_count);
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
        lsm_log_info("pass.timing", "pass", "incr_extract", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc != 0) {
            free_incremental_result_cache(cache, ci);
            return rc;
        }
        lsm_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));

        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = lsm_build_registry_from_cache(ctx, changed_files, ci, cache);
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
        lsm_log_info("pass.timing", "pass", "incr_registry", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc != 0) {
            free_incremental_result_cache(cache, ci);
            return rc;
        ...
        int64_t registry_next_id = lsm_gbuf_next_id(ctx->gbuf);
        ...
        LSMModuleDefIndex *module_def_index = NULL;
        LSMCrossLspRegistries cross_registries = {0};
        LSMCrossLspRegistries *registries_arg = NULL;
        if (closure) {
            ...
            int fresh_count = 0;
            LSMLSPDef *fresh_defs =
                def_modules && def_starts
                    ? lsm_pxc_collect_all_defs(cache, changed_files, ci, ctx->project_name,
                                               def_modules, &fresh_count, def_starts)
                    : NULL;
            if ((fresh_defs || fresh_count == 0) && def_starts &&
                lsm_lsp_surface_build_rows(ctx->project_name, cache, changed_files, ci, fresh_defs,
            ...
            if (all_def_count > 0) {
                all_defs = (LSMLSPDef *)lsm_arena_alloc(&closure->arena,
                                                        (size_t)all_def_count * sizeof(LSMLSPDef));
            }
            if (all_defs) {
                if (closure->base_def_count > 0) {
                    memcpy(all_defs, closure->base_defs,
                           (size_t)closure->base_def_count * sizeof(LSMLSPDef));
                }
                if (fresh_count > 0) {
                    memcpy(all_defs + closure->base_def_count, fresh_defs,
                           (size_t)fresh_count * sizeof(LSMLSPDef));
                }
                module_def_index = lsm_pxc_build_module_def_index(all_defs, all_def_count);
                ...
                if (ci > CLOSURE_BUDGET_FLOOR_FILES) {
                    LSMArena *xa = &closure->arena;
                    cross_registries.go = lsm_go_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.python =
                        lsm_py_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.c = lsm_c_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.cs = lsm_cs_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.ts = lsm_ts_build_cross_registry(xa, all_defs, all_def_count);
                    cross_registries.java =
                        lsm_java_build_cross_registry(xa, all_defs, all_def_count);
            ...
            def_modules = NULL;
            lsm_log_info("incremental.closure_registries", "base",
                         itoa_buf(closure->base_def_count), "fresh", itoa_buf(fresh_count),
                         "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
        }
        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = lsm_parallel_resolve(ctx, changed_files, ci, cache, &shared_ids, worker_count,
        ...
        lsm_log_info("pass.timing", "pass", "incr_resolve", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        lsm_gbuf_set_next_id(ctx->gbuf, atomic_load(&shared_ids));
        free_incremental_result_cache(cache, ci);
        return rc;
    } else {
        lsm_log_info("incremental.mode", "mode", "sequential", "changed", itoa_buf(ci));
        ...
        LSMFileResult **prior_cache = ctx->result_cache;
        LSMFileResult **cache = prior_cache;
        ...
        int rc = lsm_pipeline_pass_definitions(ctx, changed_files, ci);
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = lsm_pipeline_pass_calls(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = lsm_pipeline_pass_usages(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
        if (rc == 0) {
            rc = lsm_pipeline_pass_semantic(ctx, changed_files, ci);
        }
        if (rc == 0) {
            rc = lsm_pipeline_check_cancel(ctx);
        }
...
static int run_postpasses(lsm_pipeline_ctx_t *ctx, lsm_file_info_t *changed_files, int ci,
                          const char *project) {
    struct timespec t;

    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    int rc = lsm_pipeline_pass_tests(ctx, changed_files, ci);
    lsm_log_info("pass.timing", "pass", "incr_tests", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    if (rc != 0 || lsm_pipeline_check_cancel(ctx)) {
        return rc != 0 ? rc : LSM_NOT_FOUND;
    }

    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    rc = lsm_pipeline_pass_decorator_tags(ctx->gbuf, project);
    lsm_log_info("pass.timing", "pass", "incr_decorator_tags", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc < 0 || lsm_pipeline_check_cancel(ctx)) {
        return rc < 0 ? rc : LSM_NOT_FOUND;
    }
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    rc = lsm_pipeline_pass_configlink(ctx);
    lsm_log_info("pass.timing", "pass", "incr_configlink", "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc < 0 || lsm_pipeline_check_cancel(ctx)) {
        return rc < 0 ? rc : LSM_NOT_FOUND;
    }
    ...
    if (ctx->mode <= LSM_MODE_MODERATE) {
        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = lsm_pipeline_pass_similarity(ctx);
        lsm_log_info("pass.timing", "pass", "incr_similarity", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc < 0 || lsm_pipeline_check_cancel(ctx)) {
            return rc < 0 ? rc : LSM_NOT_FOUND;
        }
        ...
        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        rc = lsm_pipeline_pass_semantic_edges(ctx);
        lsm_log_info("pass.timing", "pass", "incr_semantic_edges", "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        if (rc < 0 || lsm_pipeline_check_cancel(ctx)) {
            return rc < 0 ? rc : LSM_NOT_FOUND;
        }
...
static int dump_and_persist(lsm_gbuf_t *gbuf, const char *db_path, const char *project,
                            atomic_int *cancelled, const lsm_file_hash_t *manifest,
                            int manifest_count, const char *adr_content,
                            const lsm_coverage_row_t *cov, int cov_count,
                            const lsm_coverage_meta_t *meta_template,
                            const lsm_lsp_surface_row_t *surface_rows, int surface_row_count) {
    struct timespec t;
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    lsm_pipeline_generation_t generation = {
        ...
        .coverage_meta = meta_template ? *meta_template : (lsm_coverage_meta_t){0},
        .surface_rows = surface_rows,
        .surface_row_count = surface_row_count,
    };
    int rc = lsm_pipeline_publish_generation(&generation);
    lsm_log_info("incremental.dump", "rc", itoa_buf(rc), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    if (rc != 0) {
        return rc;
    }
...
typedef struct {
    const lsm_lsp_surface_row_t *const *rows;
    int row_count;
    int worker_id;
    int worker_count;
    LSMArena *arena;      /* this worker's arena (owned by cr) */
    LSMLSPDef **row_defs; /* per-row result pointers */
...
static void *rehydrate_worker(void *arg) {
    ...
    for (int i = w->worker_id; i < w->row_count; i += w->worker_count) {
        LSMLSPDef *defs = NULL;
        int count = lsm_lsp_surface_defs_from_json(w->arena, w->rows[i]->defs_json, &defs);
        w->row_defs[i] = defs;
...
static int closure_probe_surfaces(lsm_pipeline_t *p, const char *project,
                                  lsm_file_info_t *probe_files, int probe_count,
                                  lsm_lsp_surface_row_t **out_rows, int *out_count) {
    *out_rows = NULL;
    *out_count = 0;
    lsm_gbuf_t *probe_gbuf = lsm_gbuf_new(project, lsm_pipeline_repo_path(p));
    if (!probe_gbuf) {
        return -1;
    }
    ...
    lsm_path_alias_collection_t *aliases =
        lsm_load_path_aliases_excluded(lsm_pipeline_repo_path(p), excluded_dirs, excluded_count);
    lsm_pipeline_ctx_t probe_ctx = {
        .project_name = project,
        .repo_path = lsm_pipeline_repo_path(p),
        .gbuf = probe_gbuf,
        .registry = NULL,
        .cancelled = lsm_pipeline_cancelled_ptr(p),
        .pipeline = NULL, /* probe must not record file errors twice */
        .mode = lsm_pipeline_get_mode(p),
        .path_aliases = aliases,
        .excluded_dirs = excluded_dirs,
        .excluded_count = excluded_count,
    };
    LSMFileResult **cache = (LSMFileResult **)calloc((size_t)probe_count, sizeof(LSMFileResult *));
    int rc = cache ? 0 : -1;
    if (rc == 0) {
        _Atomic int64_t probe_ids;
        atomic_init(&probe_ids, lsm_gbuf_next_id(probe_gbuf));
        rc = lsm_parallel_extract(&probe_ctx, probe_files, probe_count, cache, &probe_ids,
                                  lsm_default_worker_count(true));
    }
    if (rc == 0) {
        ...
        if (def_modules && def_starts) {
            defs = lsm_pxc_collect_all_defs(cache, probe_files, probe_count, project, def_modules,
                                            &def_count, def_starts);
            rc = lsm_lsp_surface_build_rows(project, cache, probe_files, probe_count, defs,
...
static bool closure_is_alias_config(const lsm_path_alias_collection_t *aliases,
                                    const char *rel_path) {
    if (!rel_path) {
        return false;
    }
...
static void closure_seed_governed(LSMHashTable *closure_set, const lsm_file_info_t *files,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!lsm_ht_get(closure_set, files[i].rel_path)) {
            lsm_ht_set(closure_set, files[i].rel_path, (void *)files[i].rel_path);
        }
    }
}
...
static int closure_try_plan(lsm_pipeline_t *p, lsm_store_t *store, const char *project,
                            lsm_file_info_t *files, int file_count, const lsm_file_hash_t *stored,
                            int stored_count, const lsm_file_hash_t *fresh, int fresh_count,
                            closure_plan_t *plan) {
    memset(plan, 0, sizeof(*plan));
    struct timespec t;
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    ...
    LSMHashTable *fresh_by_path = lsm_ht_create((size_t)fresh_count * PAIR_LEN + LSM_SZ_64);
    LSMHashTable *files_by_path = lsm_ht_create((size_t)file_count * PAIR_LEN + LSM_SZ_64);
    LSMHashTable *stored_by_path = lsm_ht_create((size_t)stored_count * PAIR_LEN + LSM_SZ_64);
    LSMHashTable *closure_set = lsm_ht_create(LSM_SZ_64);
    char **changed_paths = NULL;
    char **dependents = NULL;
    int dependent_count = 0;
    lsm_lsp_surface_row_t *probe_rows = NULL;
    int probe_count = 0;
    lsm_file_info_t *probe_files = NULL;
    lsm_path_alias_collection_t *plan_aliases = NULL;
    int n_changed = 0;
    int n_deleted = 0;
    ...
    for (int i = 0; i < file_count; i++) {
        lsm_ht_set(files_by_path, files[i].rel_path, &files[i]);
    }
    for (int i = 0; i < fresh_count; i++) {
        lsm_ht_set(fresh_by_path, fresh[i].rel_path, (void *)&fresh[i]);
    }
    for (int i = 0; i < stored_count; i++) {
        lsm_ht_set(stored_by_path, stored[i].rel_path, (void *)&stored[i]);
    }
    ...
    plan_aliases = lsm_load_path_aliases_excluded(lsm_pipeline_repo_path(p), NULL, 0);
    bool governed_seeded = false;
    for (int i = 0; i < fresh_count; i++) {
        const lsm_file_hash_t *old_row = lsm_ht_get(stored_by_path, fresh[i].rel_path);
        bool synthetic = semantic_manifest_is_virtual_path(fresh[i].rel_path);
        bool discovered = lsm_ht_get(files_by_path, fresh[i].rel_path) != NULL;
    ...
    for (int i = 0; i < stored_count; i++) {
        if (!lsm_ht_get(fresh_by_path, stored[i].rel_path)) {
            if (!lsm_ht_get(files_by_path, stored[i].rel_path) &&
    ...
    if (lsm_store_get_lsp_surfaces(store, project, &plan->stored_rows, &plan->stored_count) !=
            LSM_STORE_OK ||
        plan->stored_count == 0) {
        decline = "no_surface_rows";
        goto done;
    }
    LSMHashTable *rows_by_path = lsm_ht_create((size_t)plan->stored_count * PAIR_LEN);
    if (!rows_by_path) {
        decline = "alloc";
        goto done;
    }
    for (int i = 0; i < plan->stored_count; i++) {
        lsm_ht_set(rows_by_path, plan->stored_rows[i].rel_path, &plan->stored_rows[i]);
    }

    probe_files =
        (lsm_file_info_t *)calloc((size_t)(n_changed ? n_changed : 1), sizeof(lsm_file_info_t));
    if (!probe_files) {
        lsm_ht_free(rows_by_path);
        decline = "alloc";
        goto done;
    }
    for (int i = 0; i < n_changed; i++) {
        lsm_file_info_t *info = lsm_ht_get(files_by_path, changed_paths[i]);
        if (!info) {
            lsm_ht_free(rows_by_path);
    ...
    for (int i = 0; i < n_changed; i++) {
        const lsm_lsp_surface_row_t *stored_row = lsm_ht_get(rows_by_path, changed_paths[i]);
        const lsm_lsp_surface_row_t *fresh_row = NULL;
    ...
    if (dep_target_count > 0 &&
        lsm_store_get_dependent_files(store, project, dep_targets, dep_target_count, &dependents,
                                      &dependent_count) != LSM_STORE_OK) {
        free(dep_targets);
    ...
    for (int i = 0; i < n_changed; i++) {
        if (!lsm_ht_get(closure_set, changed_paths[i])) {
            lsm_ht_set(closure_set, changed_paths[i], changed_paths[i]);
        }
    }
    for (int i = 0; i < dependent_count; i++) {
        if (!lsm_ht_get(files_by_path, dependents[i])) {
            decline = "dependent_not_discovered";
            goto done;
        }
        if (!lsm_ht_get(closure_set, dependents[i])) {
            lsm_ht_set(closure_set, dependents[i], dependents[i]);
        }
    }
     ...
     * the executor. */
    int closure_count = (int)lsm_ht_count(closure_set);
    ...
    plan->files = (lsm_file_info_t *)calloc((size_t)(closure_count ? closure_count : 1),
                                            sizeof(lsm_file_info_t));
    if (!plan->files) {
        decline = "alloc";
        goto done;
    }
    ...
    lsm_log_info("incremental.closure_plan", "changed", itoa_buf(n_changed), "surface_changed",
                 itoa_buf(n_surface_changed), "deleted", itoa_buf(n_deleted), "dependents",
                 itoa_buf(dependent_count));
    lsm_log_info("incremental.closure_plan_done", "closure", itoa_buf(plan->count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

done:
    lsm_path_alias_collection_free(plan_aliases);
    free(probe_files);
    lsm_store_free_lsp_surfaces(probe_rows, probe_count);
    lsm_store_free_dependent_files(dependents, dependent_count);
    free(changed_paths);
    lsm_ht_free(fresh_by_path);
    lsm_ht_free(files_by_path);
    lsm_ht_free(stored_by_path);
    lsm_ht_free(closure_set);
    if (decline) {
        lsm_log_info("incremental.closure_decline", "reason", decline, "elapsed_ms",
                     itoa_buf((int)elapsed_ms(t)));
        lsm_store_free_lsp_surfaces(plan->stored_rows, plan->stored_count);
        memset(plan, 0, sizeof(*plan));
        return 0;
 ...
 * LSM_PIPELINE_FORCE_FULL_REINDEX: the live database is never touched, so
...
static int run_closure_delta(lsm_pipeline_t *p, const char *db_path, const char *project,
                             const lsm_file_hash_t *baseline_manifest, int baseline_count,
                             closure_plan_t *plan, lsm_file_info_t *changed_files, int ci,
                             char **deleted, int deleted_count, lsm_file_hash_t *mode_skipped,
                             int mode_skipped_count, lsm_coverage_row_t *old_cov, int old_cov_count,
                             lsm_store_t *route_store, struct timespec t0) {
    struct timespec t;
    lsm_store_close(route_store);

    int result = LSM_PIPELINE_FORCE_FULL_REINDEX;
    char *stage = NULL;
    lsm_store_t *staging = NULL;
    lsm_delta_saved_edge_t *snapshot = NULL;
    int snapshot_count = 0;
    lsm_gbuf_t *gbuf = NULL;
    lsm_registry_t *registry = NULL;
    lsm_path_alias_collection_t *path_aliases = NULL;
    LSMHashTable *stale_surface_set = NULL;
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    if (lsm_delta_stage_clone(db_path, &stage) != 0) {
        goto out;
    }
    lsm_log_info("delta.clone", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    staging = lsm_store_open_path(stage);
    if (!staging) {
        goto out;
    }
    lsm_log_info("delta.staging_open", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    if (lsm_delta_snapshot_inbound(staging, project, purge_paths, purge_count, &snapshot,
                                   &snapshot_count) != 0) {
        goto out;
    }
    if (lsm_delta_purge(staging, project, purge_paths, purge_count) != 0) {
        goto out;
    }
    lsm_log_info("delta.snapshot_purge", "captured", itoa_buf(snapshot_count), "purged_files",
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    gbuf = lsm_gbuf_new(project, lsm_pipeline_repo_path(p));
    if (!gbuf) {
        goto out;
    }
    int64_t max_db_id = lsm_delta_preseed(staging, project, gbuf);
    if (max_db_id < 0) {
        goto out;
    }
    registry = lsm_registry_new();
    if (!registry) {
        goto out;
    }
    lsm_gbuf_foreach_node(gbuf, registry_visitor, registry);
    lsm_log_info("delta.preseed_done", "registry", itoa_buf(lsm_registry_size(registry)),
                 "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    stale_surface_set = lsm_ht_create((size_t)(purge_count)*PAIR_LEN + LSM_SZ_64);
    ...
    for (int i = 0; i < purge_count; i++) {
        ...
        lsm_ht_set(stale_surface_set, key, key);
    }
    lsm_arena_init(&cr.arena);
    cr_arena_live = true;
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    {
        /* Eligible rows first (skip everything being re-parsed or deleted),
         * then fan the JSON decode out across workers. */
        const lsm_lsp_surface_row_t **elig = (const lsm_lsp_surface_row_t **)calloc(
        ...
        int elig_count = 0;
        for (int i = 0; i < plan->stored_count; i++) {
            const lsm_lsp_surface_row_t *row = &plan->stored_rows[i];
            if (!lsm_ht_get(stale_surface_set, row->rel_path)) {
                elig[elig_count++] = row;
            }
        }
        int workers = lsm_default_worker_count(true);
        if (workers < 1) {
            workers = 1;
        }
        ...
        LSMLSPDef **row_defs =
            (LSMLSPDef **)calloc((size_t)(elig_count ? elig_count : 1), sizeof(*row_defs));
        int *row_counts = (int *)calloc((size_t)(elig_count ? elig_count : 1), sizeof(int));
        cr.rehydrate_arenas = (LSMArena *)calloc((size_t)workers, sizeof(LSMArena));
        lsm_thread_t *threads = (lsm_thread_t *)calloc((size_t)workers, sizeof(lsm_thread_t));
        ...
        for (int w = 0; w < workers; w++) {
            lsm_arena_init(&cr.rehydrate_arenas[w]);
        }
        int spawned = 0;
        for (int w = 0; w < workers; w++) {
            ...
            if (lsm_thread_create(&threads[w], 0, rehydrate_worker, &wargs[w]) != 0) {
                break;
            }
            spawned++;
        }
        ...
        for (int i = 0; i < elig_count; i++) {
            if (row_counts[i] < 0) {
                lsm_log_error("delta.rehydrate_failed", "path", elig[i]->rel_path);
                decode_ok = false;
                break;
            }
            total += row_counts[i];
        ...
        if (decode_ok && total > 0) {
            cr.base_defs =
                (LSMLSPDef *)lsm_arena_alloc(&cr.arena, (size_t)total * sizeof(LSMLSPDef));
            if (!cr.base_defs) {
                decode_ok = false;
            } else {
                for (int i = 0; i < elig_count; i++) {
                    if (row_counts[i] > 0) {
                        memcpy(cr.base_defs + cr.base_def_count, row_defs[i],
                               (size_t)row_counts[i] * sizeof(LSMLSPDef));
    ...
    lsm_log_info("delta.rehydrate", "defs", itoa_buf(cr.base_def_count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));
    ...
    for (int i = 0; i < ci; i++) {
        char *file_qn = lsm_pipeline_fqn_compute(project, changed_files[i].rel_path, "__file__");
        if (file_qn) {
            ...
            lsm_gbuf_upsert_node(gbuf, "File", basename, file_qn, rel, 0, 0, props);
            free(file_qn);
        }
    }
    ...
    if (phase_rc == 0) {
        phase_rc = lsm_pipeline_pass_k8s(&ctx, changed_files, ci);
    }
    if (phase_rc == 0) {
        phase_rc = lsm_pipeline_check_cancel(&ctx);
    }
    if (phase_rc == 0) {
    ...
    lsm_pipeline_set_pkgmap(NULL);
    if (phase_rc != 0) {
        lsm_log_error("delta.err", "phase", "extract_resolve", "rc", itoa_buf(phase_rc));
        goto out;
    }
    lsm_log_info("delta.repair", "files", itoa_buf(ci), "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    if (lsm_delta_patch(staging, project, gbuf, max_db_id, snapshot, snapshot_count) != 0) {
        goto out;
    }
    lsm_log_info("delta.patch_done", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    /* Coverage: previous failure rows for files not re-extracted + this
     * run's fresh entries — same merge the legacy tail performs. */
    lsm_file_error_t *run_errs = NULL;
    int run_err_count = 0;
    lsm_pipeline_get_file_errors(p, &run_errs, &run_err_count);
    char **run_excluded = NULL;
    int run_excluded_count = 0;
    lsm_pipeline_get_excluded(p, &run_excluded, &run_excluded_count);
    lsm_ignored_file_t *run_ignored = NULL;
    int run_ignored_count = 0;
    int run_ignored_total = 0;
    lsm_pipeline_get_ignored(p, &run_ignored, &run_ignored_count, &run_ignored_total);
    int cov_cap = old_cov_count + run_err_count + run_excluded_count + run_ignored_count;
    bool coverage_rows_available = cov_cap == 0;
    if (cov_cap > 0) {
        cov = (lsm_coverage_row_t *)malloc((size_t)cov_cap * sizeof(*cov));
    ...
    if (cov) {
        for (int i = 0; i < old_cov_count; i++) {
            ...
            if (!by_design && old_cov[i].rel_path &&
                !lsm_ht_get(stale_surface_set, old_cov[i].rel_path)) {
                cov[cov_n++] = old_cov[i];
            }
        }
...
#endif
    if (lsm_pipeline_build_fresh_semantic_manifest(project, lsm_pipeline_repo_path(p),
                                                   lsm_pipeline_get_mode(p), &manifest,
                                                   &manifest_count) != 0 ||
        !lsm_pipeline_semantic_manifests_equal(baseline_manifest, baseline_count, manifest,
                                               manifest_count)) {
        lsm_log_warn("delta.abort", "reason", "semantic_inputs_changed");
        result = LSM_PIPELINE_ABORT_PRESERVE_DB;
        goto out;
    }
    ...
    {
        sqlite3 *sdb = lsm_store_get_db(staging);
        sqlite3_stmt *del = NULL;
        bool surf_ok = sdb != NULL;
        if (surf_ok && sqlite3_prepare_v2(sdb,
                                          "DELETE FROM lsp_surface WHERE project = ?1"
                                          " AND rel_path = ?2",
                                          LSM_NOT_FOUND, &del, NULL) == SQLITE_OK) {
            for (int i = 0; surf_ok && i < ci + deleted_count; i++) {
                sqlite3_reset(del);
                sqlite3_bind_text(del, 1, project, LSM_NOT_FOUND, SQLITE_TRANSIENT);
                sqlite3_bind_text(del, 2, purge_paths[i], LSM_NOT_FOUND, SQLITE_TRANSIENT);
        ...
        if (!surf_ok || lsm_store_upsert_lsp_surface_batch(staging, cr.fresh_rows,
                                                           cr.fresh_count) != LSM_STORE_OK) {
            goto out;
        }
    }
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    lsm_pipeline_set_committed_counts(p, lsm_store_count_nodes(staging, project),
                                      lsm_store_count_edges(staging, project));
    lsm_log_info("delta.committed_counts", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    {
        int index_mode = lsm_pipeline_get_mode(p);
        lsm_pipeline_generation_t generation = {
            .gbuf = NULL,
            .final_db_path = db_path,
            .project = project,
        ...
        staging = NULL;
        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        result = lsm_pipeline_publish_staged(stage, &generation, false, true);
        lsm_log_info("delta.publish_total", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
        stage = NULL; /* publish_staged owns and frees it on every path */
    }
    lsm_log_info("incremental.done_delta", "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
...
out:
    ...
    if (stage) {
        lsm_pipeline_discard_stage(stage);
        free(stage);
    }
    if (result == LSM_PIPELINE_FORCE_FULL_REINDEX) {
        lsm_log_warn("delta.fallback", "reason", "delta_failed_full_rebuild");
#if defined(LSM_INCREMENTAL_TEST_API) && LSM_INCREMENTAL_TEST_API
        incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_FORCED_FULL);
#endif
    }
    lsm_delta_free_snapshot(snapshot, snapshot_count);
    surface_name_set_free(stale_surface_set);
    if (cr_arena_live) {
        lsm_store_free_lsp_surfaces(cr.fresh_rows, cr.fresh_count);
        for (int i = 0; i < cr.def_module_count; i++) {
            free(cr.def_modules[i]);
        }
        free(cr.def_modules);
        for (int i = 0; i < cr.rehydrate_arena_count; i++) {
            lsm_arena_destroy(&cr.rehydrate_arenas[i]);
        }
        free(cr.rehydrate_arenas);
        lsm_arena_destroy(&cr.arena);
    }
    lsm_pipeline_free_semantic_manifest(manifest, manifest_count);
    free(cov);
    free(purge_paths);
    if (registry) {
        lsm_registry_free(registry);
    }
    ...
    lsm_store_free_coverage(old_cov, old_cov_count);
    return result;
}
...
int lsm_pipeline_run_incremental(lsm_pipeline_t *p, const char *db_path, lsm_file_info_t *files,
    ...
    incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_NONE);
#endif
...
#if defined(LSM_INCREMENTAL_TEST_API) && LSM_INCREMENTAL_TEST_API
    lsm_store_t *store =
        force_legacy_partial ? lsm_store_open_path(db_path) : lsm_store_open_path_query(db_path);
#else
    lsm_store_t *store = lsm_store_open_path_query(db_path);
#endif
    if (!store) {
        lsm_log_error("incremental.err", "msg", "open_db_failed", "path", db_path);
        return LSM_NOT_FOUND;
    }

    /* Load stored file hashes */
    lsm_file_hash_t *stored = NULL;
    int stored_count = 0;
    if (lsm_store_get_file_hashes(store, project, &stored, &stored_count) != LSM_STORE_OK) {
        lsm_log_error("incremental.err", "msg", "manifest_read_failed", "project", project);
    ...
    if (!force_legacy_partial) {
        ...
        int meta_rc = lsm_store_coverage_meta_get(store, project, &meta);
        const char *mode_name = incr_mode_name(lsm_pipeline_get_mode(p));
        bool metadata_current = meta_rc == LSM_STORE_OK &&
        ...
        bool exact = metadata_current &&
                     lsm_pipeline_semantic_manifests_equal(stored, stored_count, baseline_manifest,
                                                           baseline_count);
        lsm_store_coverage_meta_clear(&meta);
        if (exact) {
            lsm_store_free_file_hashes(stored, stored_count);
            lsm_store_close(store);
#if defined(LSM_INCREMENTAL_TEST_API) && LSM_INCREMENTAL_TEST_API
            incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_NOOP);
#endif
            lsm_log_info("incremental.noop", "reason", "semantic_manifest_equal");
            return 0;
        }
        if (force_full_on_mismatch) {
            ...
            incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_FORCED_FULL);
#endif
            lsm_log_info("incremental.force_full", "reason", "mode_downgrade_changed");
        ...
        if (metadata_current &&
            ...
            closure_active = true;
#if defined(LSM_INCREMENTAL_TEST_API) && LSM_INCREMENTAL_TEST_API
            incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_CLOSURE_REPAIR);
#endif
            lsm_log_info("incremental.route", "route", "closure_repair", "files",
            ...
            incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_FORCED_FULL);
#endif
            lsm_log_info("incremental.force_full", "reason", "semantic_manifest_changed");
            return LSM_PIPELINE_FORCE_FULL_REINDEX;
        }
    }
    ...
    char **deleted = NULL;
    lsm_file_hash_t *mode_skipped = NULL;
    int mode_skipped_count = 0;
    int deleted_count =
        find_deleted_files(lsm_pipeline_repo_path(p), files, file_count, stored, stored_count,
                           &deleted, &mode_skipped, &mode_skipped_count);

    lsm_log_info("incremental.classify", "changed", itoa_buf(n_changed), "unchanged",
    ...
    if (!closure_active && n_changed == 0 && deleted_count == 0) {
        ...
        incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_NOOP);
#endif
        lsm_log_info("incremental.noop", "reason", "no_changes");
        free(is_changed);
        free(deleted);
        free_mode_skipped(mode_skipped, mode_skipped_count);
        lsm_store_free_file_hashes(stored, stored_count);
...
#if defined(LSM_INCREMENTAL_TEST_API) && LSM_INCREMENTAL_TEST_API
    if (!closure_active) {
        incr_test_set_last_route(LSM_INCREMENTAL_ROUTE_LEGACY_PARTIAL);
    }
#endif
    ...
    lsm_coverage_row_t *old_cov = NULL;
    int old_cov_count = 0;
    if (lsm_store_coverage_get(store, project, &old_cov, &old_cov_count) != LSM_STORE_OK) {
        lsm_log_error("incremental.err", "msg", "coverage_read_failed", "project", project);
        lsm_store_free_coverage(old_cov, old_cov_count);
        free(is_changed);
        for (int i = 0; i < deleted_count; i++) {
            free(deleted[i]);
    ...
    lsm_file_info_t *changed_files =
        (n_changed > 0) ? malloc((size_t)n_changed * sizeof(lsm_file_info_t)) : NULL;
    int ci = 0;
    if (closure_active) {
    ...
    lsm_log_info("incremental.reparse", "files", itoa_buf(ci));
    ...
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    lsm_gbuf_t *existing = lsm_gbuf_new(project, lsm_pipeline_repo_path(p));
    int load_rc = lsm_gbuf_load_from_db(existing, db_path, project);
    lsm_log_info("incremental.load_db", "rc", itoa_buf(load_rc), "nodes",
                 itoa_buf(lsm_gbuf_node_count(existing)), "edges",
                 itoa_buf(lsm_gbuf_edge_count(existing)), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    if (load_rc != 0) {
        lsm_log_error("incremental.err", "msg", "load_db_failed");
        lsm_gbuf_free(existing);
        free(changed_files);
        for (int i = 0; i < deleted_count; i++) {
            free(deleted[i]);
        }
    ...
    char *saved_adr = NULL;
    lsm_adr_t existing_adr = {0};
    int adr_rc = lsm_store_adr_get(store, project, &existing_adr);
    if (adr_rc == LSM_STORE_OK) {
        bool had_adr_content = existing_adr.content != NULL;
        if (had_adr_content) {
            saved_adr = strdup(existing_adr.content);
        }
        lsm_store_adr_free(&existing_adr);
    ...
    } else if (adr_rc != LSM_STORE_NOT_FOUND) {
        lsm_store_adr_free(&existing_adr);
        lsm_gbuf_free(existing);
        free(changed_files);
        for (int i = 0; i < deleted_count; i++) {
    ...
    lsm_edge_capture_t edge_cap = {0};
    edge_cap.gbuf = existing;
    {
        LSMHashTable *changed_paths = lsm_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : LSM_SZ_64);
        for (int i = 0; i < ci; i++) {
            lsm_ht_set(changed_paths, changed_files[i].rel_path, &changed_files[i]);
        }
        edge_cap.changed_paths = changed_paths;
        lsm_clock_gettime(CLOCK_MONOTONIC, &t);
        lsm_gbuf_foreach_edge(existing, incr_capture_inbound_edge, &edge_cap);
        edge_cap.changed_paths = NULL;
        lsm_ht_free(changed_paths); /* keys borrowed from changed_files; not freed here */
    }
    lsm_log_info("incremental.edge_snapshot", "captured", itoa_buf(edge_cap.count), "elapsed_ms",
                 itoa_buf((int)elapsed_ms(t)));

    /* Step 2: Purge stale nodes */
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    for (int i = 0; i < ci; i++) {
        lsm_gbuf_delete_by_file(existing, changed_files[i].rel_path);
    }
    for (int i = 0; i < deleted_count; i++) {
        lsm_gbuf_delete_by_file(existing, deleted[i]);
        free(deleted[i]);
    }
    free(deleted);
    lsm_log_info("incremental.purge", "elapsed_ms", itoa_buf((int)elapsed_ms(t)));

    /* Step 3-5: Registry + extract + resolve */
    lsm_registry_t *registry = lsm_registry_new();
    lsm_clock_gettime(CLOCK_MONOTONIC, &t);
    lsm_gbuf_foreach_node(existing, registry_visitor, registry);
    lsm_log_info("incremental.registry_seed", "symbols", itoa_buf(lsm_registry_size(registry)),
                 "elapsed_ms", itoa_buf((int)elapsed_ms(t)));
    ...
    lsm_path_alias_collection_t *path_aliases =
        lsm_load_path_aliases_excluded(lsm_pipeline_repo_path(p), excluded_dirs, excluded_count);
    ...
    for (int i = 0; i < ci; i++) {
        ...
        if (file_qn) {
            ...
            const char *ext = strrchr(basename, '.');
            snprintf(props, sizeof(props), "{\"extension\":\"%s\"}", ext ? ext : "");
            lsm_gbuf_upsert_node(existing, "File", basename, file_qn, rel, 0, 0, props);
            free(file_qn);
        }
    }
    ...
    if (phase_rc != 0) {
        lsm_log_error("incremental.err", "phase", "extract_resolve", "rc", itoa_buf(phase_rc));
        free(changed_files);
        lsm_registry_free(registry);
        lsm_path_alias_collection_free(path_aliases);
    ...
    if (cov) {
        LSMHashTable *changed_set = lsm_ht_create(ci > 0 ? (size_t)ci * PAIR_LEN : LSM_SZ_64);
        for (int i = 0; i < ci; i++) {
            lsm_ht_set(changed_set, changed_files[i].rel_path, &changed_files[i]);
        }
        for (int i = 0; i < old_cov_count; i++) {
            bool by_design = old_cov[i].kind && strncmp(old_cov[i].kind, "not_indexed", 11) == 0;
            if (!by_design && old_cov[i].rel_path &&
                !lsm_ht_get(changed_set, old_cov[i].rel_path)) {
                cov[cov_n++] = old_cov[i];
            }
        }
        lsm_ht_free(changed_set);
        for (int i = 0; i < run_err_count; i++) {
            cov[cov_n].rel_path = run_errs[i].path;
            cov[cov_n].kind = run_errs[i].phase;
            cov[cov_n].detail = run_errs[i].reason;
    ...
    lsm_log_info("incremental.edge_relink", "relinked", itoa_buf(relinked), "captured",
    ...
    lsm_pipeline_persist_test_run_before_final_manifest();
#endif
    int manifest_rc = lsm_pipeline_build_fresh_semantic_manifest(
        project, lsm_pipeline_repo_path(p), lsm_pipeline_get_mode(p), &manifest, &manifest_count);
    if (manifest_rc != 0 || !lsm_pipeline_semantic_manifests_equal(
                                baseline_manifest, baseline_count, manifest, manifest_count)) {
        lsm_log_warn("incremental.abort", "reason", "semantic_inputs_changed");
        lsm_pipeline_free_semantic_manifest(manifest, manifest_count);
        free(cov);
        lsm_store_free_coverage(old_cov, old_cov_count);
        free_mode_skipped(mode_skipped, mode_skipped_count);
    ...
    lsm_pipeline_set_committed_counts(p, lsm_gbuf_node_count(existing),
                                      lsm_gbuf_edge_count(existing));
    int index_mode = lsm_pipeline_get_mode(p);
    ...
    int persist_rc =
        dump_and_persist(existing, db_path, project, lsm_pipeline_cancelled_ptr(p), manifest,
    ...
    lsm_log_info("incremental.done", "elapsed_ms", itoa_buf((int)elapsed_ms(t0)));
    return persist_rc;
}
