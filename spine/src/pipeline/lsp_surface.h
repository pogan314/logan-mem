 ...
 * it: the LSMLSPDefs pass_lsp_cross registers into the cross registries,
 ...
 */
#ifndef LSM_PIPELINE_LSP_SURFACE_H
#define LSM_PIPELINE_LSP_SURFACE_H
 ...
 * is the (file_count + 1)-entry prefix array from lsm_pxc_collect_all_defs:
 * all_defs[def_starts[i] .. def_starts[i+1]) are file i's defs. Rows carry
 * heap strings; release with lsm_store_free_lsp_surfaces. Files with an
...
/* Decode one row's defs_json back into the LSMLSPDef array pass_lsp_cross
...
#endif /* LSM_PIPELINE_LSP_SURFACE_H */
