...
static void deferred_buf_push(deferred_edge_buf_t *buf, int64_t src, int64_t tgt, float score,
    ...
    if (buf->count >= buf->cap) {
        int nc = buf->cap < LSM_SZ_256 ? LSM_SZ_256 : buf->cap * GROW;
...
static int inject_calls_pattern_tokens(const lsm_gbuf_node_t *n, const lsm_gbuf_t *gbuf,
    ...
    int ec = 0;
    if (lsm_gbuf_find_edges_by_source_type(gbuf, n->id, "CALLS", &edges, &ec) != 0) {
        return count;
    }
    for (int e = 0; e < ec && count < max_tokens; e++) {
        const lsm_gbuf_node_t *t = lsm_gbuf_find_by_id(gbuf, edges[e]->target_id);
        if (!t || !t->name) {
            continue;
        }
...
static int inject_pattern_tokens(const lsm_gbuf_node_t *n, const lsm_gbuf_t *gbuf, char **tokens,
                                 int count, int max_tokens) {
    if (!n || count >= max_tokens) {
        return count;
    }

    char bt_buf[LSM_SZ_512];
    const char *bt = n->properties_json
                         ? json_str_value(n->properties_json, "bt", bt_buf, sizeof(bt_buf))
                         : NULL;
    char dec_buf[LSM_SZ_256];
...
static int tokenize_json_string_field(const char *json, const char *key, char **tokens, int count,
    ...
    count += lsm_sem_tokenize(buf, tokens + count, max_tokens - count);
    return count;
}
...
static int tokenize_json_array_field(const char *json, const char *key, char **tokens, int count,
    ...
    char *arr[LSM_SZ_16];
    int n = json_str_array(json, key, arr, LSM_SZ_16);
    for (int p = 0; p < n; p++) {
        if (count < max_tokens) {
            count += lsm_sem_tokenize(arr[p], tokens + count, max_tokens - count);
        }
        free(arr[p]);
    }
    return count;
...
static const char **collect_sorted_call_neighbors(const lsm_gbuf_t *gbuf, int64_t node_id,
                                                  bool outbound, int *out_n) {
    *out_n = 0;
    const lsm_gbuf_edge_t **edges = NULL;
    int ec = 0;
    int rc = outbound ? lsm_gbuf_find_edges_by_source_type(gbuf, node_id, "CALLS", &edges, &ec)
                      : lsm_gbuf_find_edges_by_target_type(gbuf, node_id, "CALLS", &edges, &ec);
    if (rc != 0 || ec <= 0) {
        return NULL;
    }
    ...
    for (int e = 0; e < ec; e++) {
        ...
        const lsm_gbuf_node_t *neighbor = lsm_gbuf_find_by_id(gbuf, id);
...
static int tokenize_call_neighbors(const lsm_gbuf_node_t *n, const lsm_gbuf_t *gbuf, bool outbound,
                                   char **tokens, int count, int max_tokens) {
    ...
    for (int e = 0; e < nn && e < MAX_CALLEES && count < max_tokens; e++) {
        count += lsm_sem_tokenize(names[e], tokens + count, max_tokens - count);
    }
    free((void *)names);
    return count;
}

static int tokenize_node(const lsm_gbuf_node_t *n, const lsm_gbuf_t *gbuf, char **tokens,
                         int max_tokens) {
    int count = 0;
    count += lsm_sem_tokenize(n->name, tokens + count, max_tokens - count);
    if (n->qualified_name && count < max_tokens) {
        count += lsm_sem_tokenize(n->qualified_name, tokens + count, max_tokens - count);
    }
    if (n->file_path && count < max_tokens) {
        count += lsm_sem_tokenize(n->file_path, tokens + count, max_tokens - count);
    }
...
static void build_api_vec(const lsm_gbuf_t *gbuf, int64_t node_id, lsm_sem_vec_t *out) {
    ...
    for (int i = 0; i < n && i < MAX_CALLEES; i++) {
        lsm_sem_vec_t callee_ri;
        lsm_sem_random_index(names[i], &callee_ri);
        lsm_sem_vec_add_scaled(out, &callee_ri, PSE_UNIT_POS);
    }
    free((void *)names);
    lsm_sem_normalize(out);
}

static void build_type_vec(const char *props_json, lsm_sem_vec_t *out) {
    memset(out, 0, sizeof(*out));
    if (!props_json) {
        return;
    }
    /* Extract param_types and return_type */
    char rt_buf[LSM_SZ_128];
    if (json_str_value(props_json, "return_type", rt_buf, sizeof(rt_buf))) {
        lsm_sem_vec_t ri;
        lsm_sem_random_index(rt_buf, &ri);
        lsm_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
    }
    char *ptypes[LSM_SZ_16];
    int pt_count = json_str_array(props_json, "param_types", ptypes, LSM_SZ_16);
    for (int i = 0; i < pt_count; i++) {
        lsm_sem_vec_t ri;
        lsm_sem_random_index(ptypes[i], &ri);
        lsm_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
        free(ptypes[i]);
    }
    lsm_sem_normalize(out);
...
static void build_deco_vec(const char *props_json, lsm_sem_vec_t *out) {
    memset(out, 0, sizeof(*out));
    if (!props_json) {
        return;
    }
    char *decos[LSM_SZ_16];
    int dc = json_str_array(props_json, "decorators", decos, LSM_SZ_16);
    for (int i = 0; i < dc; i++) {
        lsm_sem_vec_t ri;
        lsm_sem_random_index(decos[i], &ri);
        lsm_sem_vec_add_scaled(out, &ri, PSE_UNIT_POS);
        free(decos[i]);
    }
    lsm_sem_normalize(out);
...
static void decode_struct_profile(const char *props_json, float *out) {
    memset(out, 0, sizeof(float) * LSM_AST_PROFILE_DIMS);
    if (!props_json) {
        return;
    }
    ...
    if (json_str_value(props_json, "sp", sp_buf, sizeof(sp_buf))) {
        ...
        if (lsm_ast_profile_from_str(sp_buf, &profile)) {
            lsm_ast_profile_to_vector(&profile, out);
        }
    }
}

static void decode_minhash(const char *props_json, lsm_sem_func_t *func) {
    ...
    if (!end || (int)(end - hex) != LSM_MINHASH_HEX_LEN) {
        return;
    }
    char hex_buf[LSM_MINHASH_HEX_BUF];
    memcpy(hex_buf, hex, LSM_MINHASH_HEX_LEN);
    hex_buf[LSM_MINHASH_HEX_LEN] = '\0';
    lsm_minhash_t mh;
    if (lsm_minhash_from_hex(hex_buf, &mh)) {
...
typedef struct {
    const lsm_gbuf_node_t **node_ptrs; /* node pointer per function index */
    lsm_gbuf_t *gbuf;                  /* read-only during tokenization */
...
static void tokenize_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        const lsm_gbuf_node_t *n = tc->node_ptrs[f];
        ...
        char **dst = &tc->all_tokens[(ptrdiff_t)f * LSM_SEM_MAX_TOKENS];
        int count = tokenize_node(n, tc->gbuf, dst, LSM_SEM_MAX_TOKENS);
        count = inject_pattern_tokens(n, tc->gbuf, dst, count, LSM_SEM_MAX_TOKENS);
        if (tc->pools && tc->pools[worker_id]) {
            LSMHashTable *pool = tc->pools[worker_id];
            for (int t = 0; t < count; t++) {
                char *canon = lsm_ht_get(pool, dst[t]);
                if (canon) {
                    free(dst[t]);
                    dst[t] = canon;
                } else {
                    lsm_ht_set(pool, dst[t], dst[t]); /* key borrows the value */
                }
            }
        }
...
typedef struct {
    lsm_sem_func_t *funcs;
    char **all_tokens;
    int *token_counts;
    lsm_sem_corpus_t *corpus;
    uint8_t *qvecs; /* output: pre-quantized int8 vectors [func_count * LSM_SEM_DIM] */
    int func_count;
    _Atomic int next_idx;
} vec_build_ctx_t;
...
static void vec_build_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        char **tokens = &vc->all_tokens[(ptrdiff_t)f * LSM_SEM_MAX_TOKENS];
        ...
        int tfidf_len = 0;
        for (int t = 0; t < tc; t++) {
            float idf = lsm_sem_corpus_idf(vc->corpus, tokens[t]);
            if (idf > 0.0F) {
                indices[tfidf_len] = t;
        ...
        lsm_sem_vec_t ri_dense;
        memset(&ri_dense, 0, sizeof(lsm_sem_vec_t));
        for (int t = 0; t < tc; t++) {
            const lsm_sem_vec_t *ri = lsm_sem_corpus_ri_vec(vc->corpus, tokens[t]);
            if (ri) {
                float idf = lsm_sem_corpus_idf(vc->corpus, tokens[t]);
                lsm_sem_vec_add_scaled(&ri_dense, ri, idf);
            }
        }
        lsm_sem_normalize(&ri_dense);
        lsm_rsq_encode(ri_dense.v, &vc->funcs[f].ri_code);

        /* Int8 quantize into pre-allocated output array (parallel-safe) */
        uint8_t *qv = &vc->qvecs[(ptrdiff_t)f * LSM_SEM_DIM];
...
typedef float hyperplane_row_t[LSM_RSQ_DIM];
...
static void sig_build_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        float dec[LSM_RSQ_DIM];
        lsm_rsq_decode(&sc->funcs[f].ri_code, dec);
        uint64_t sig = 0;
...
static void score_try_emit(score_ctx_t *sc, int i, int j, int c, deferred_edge_buf_t *my_buf) {
    ...
    float score = lsm_sem_combined_score(&sc->funcs[i], &sc->funcs[j], &sc->cfg);
...
typedef struct {
    lsm_sem_func_t *funcs;
    const lsm_gbuf_node_t **node_ptrs;
...
static void collect_worker(int worker_id, void *ctx_ptr) {
    ...
    while (true) {
        ...
        for (int i = f; i < end; i++) {
            const lsm_gbuf_node_t *n = cc->node_ptrs[i];
            decode_minhash(n->properties_json, &cc->funcs[i]);
            decode_struct_profile(n->properties_json, cc->funcs[i].struct_profile);
            lsm_sem_vec_t tmp_vec;
            build_api_vec(cc->gbuf, n->id, &tmp_vec);
            lsm_rsq_encode(tmp_vec.v, &cc->funcs[i].api_code);
            build_type_vec(n->properties_json, &tmp_vec);
            lsm_rsq_encode(tmp_vec.v, &cc->funcs[i].type_code);
            build_deco_vec(n->properties_json, &tmp_vec);
            lsm_rsq_encode(tmp_vec.v, &cc->funcs[i].deco_code);
        }
    }
}

/* ── Phase helpers (keep lsm_pipeline_pass_semantic_edges complexity low) ── */
...
static int cmp_node_ptr_by_qn(const void *pa, const void *pb) {
    const lsm_gbuf_node_t *a = *(const lsm_gbuf_node_t *const *)pa;
    const lsm_gbuf_node_t *b = *(const lsm_gbuf_node_t *const *)pb;
...
static int phase1_scan_functions(lsm_gbuf_t *gbuf, lsm_sem_func_t **out_funcs,
    ...
    *out_nodes = NULL;
    ...
    for (int li = 0; labels[li]; li++) {
        ...
        int node_count = 0;
        if (lsm_gbuf_find_by_label(gbuf, labels[li], &nodes, &node_count) != 0) {
            continue;
        }
        for (int i = 0; i < node_count; i++) {
            ...
            if (func_count >= func_cap) {
                ...
                funcs = grown;
                const lsm_gbuf_node_t **np_grown =
                    realloc(node_ptrs, (size_t)new_cap * sizeof(lsm_gbuf_node_t *));
                if (!np_grown) {
                    break;
                }
                node_ptrs = np_grown;
            ...
            memset(&funcs[func_count], 0, sizeof(lsm_sem_func_t));
...
static int phase6b_merge_edges(lsm_gbuf_t *gbuf, deferred_edge_buf_t *worker_bufs, int worker_count,
    ...
    for (int e = 0; e < n; e++) {
        ...
        lsm_gbuf_insert_edge(gbuf, de->source_id, de->target_id, "SEMANTICALLY_RELATED", props);
        edge_counts[de->i]++;
        edge_counts[de->j]++;
        total_edges++;
    }
...
static void phase3c_export_token_vectors(lsm_gbuf_t *gbuf, lsm_sem_corpus_t *corpus) {
    int tv_count = lsm_sem_corpus_token_count(corpus);
    for (int t = 0; t < tv_count; t++) {
        const lsm_sem_vec_t *vec = NULL;
        float idf = 0.0F;
        const char *tok = lsm_sem_corpus_token_at(corpus, t, &vec, &idf);
        if (!tok || !vec || idf <= PSE_FLOW_WEIGHT) {
            continue;
        }
        uint8_t qvec[LSM_SEM_DIM];
        for (int d = 0; d < LSM_SEM_DIM; d++) {
        ...
        lsm_gbuf_store_token_vector(gbuf, tok, qvec, LSM_SEM_DIM, idf);
    }
    lsm_log_info("pass.semantic.token_vectors", "count", itoa_log(tv_count));
}

/* Phase 5a: generate NUM_HYPERPLANES × LSM_SEM_DIM deterministic random
...
static hyperplane_row_t *phase5a_build_hyperplanes(void) {
    ...
    for (int h = 0; h < NUM_HYPERPLANES; h++) {
        for (int d = 0; d < LSM_RSQ_DIM; d++) {
            uint64_t seed = XXH3_64bits_withSeed(&d, sizeof(d), (uint64_t)h * LSM_RSQ_DIM);
 ...
 * parallel.  Must be called after lsm_sem_ensure_ready() so the pretrained
 * token map is initialized. */
static void phase1b_decode_and_build(lsm_sem_func_t *funcs, const lsm_gbuf_node_t **node_ptrs,
                                     const lsm_gbuf_t *gbuf, int func_count, int worker_count) {
    if (func_count <= 0) {
        return;
    }
    collect_ctx_t cc = {
    ...
    lsm_parallel_for(worker_count, collect_worker, &cc, opts);
}

/* Phase 2: tokenize each function's metadata in parallel, filling
 * all_tokens[] and token_counts[].  Caller allocates the arrays. */
static void phase2_tokenize(const lsm_gbuf_node_t **node_ptrs, lsm_gbuf_t *gbuf, char **all_tokens,
    ...
    lsm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    lsm_parallel_for(worker_count, tokenize_worker, &tc, opts);
}
...
static void phase4_build_and_store_vectors(lsm_gbuf_t *gbuf, lsm_sem_func_t *funcs,
                                           char **all_tokens, int *token_counts,
                                           lsm_sem_corpus_t *corpus, int func_count,
                                           int worker_count) {
    uint8_t *qvecs = malloc((size_t)func_count * LSM_SEM_DIM);
    if (!qvecs) {
        return;
    }
    vec_build_ctx_t vc = {
    ...
    lsm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    lsm_parallel_for(worker_count, vec_build_worker, &vc, opts);
    for (int f = 0; f < func_count; f++) {
        lsm_gbuf_store_vector(gbuf, funcs[f].node_id, &qvecs[(ptrdiff_t)f * LSM_SEM_DIM],
                              LSM_SEM_DIM);
    }
    free(qvecs);
}
 ...
 * caller frees both. */
static void phase5_lsh_build(lsm_sem_func_t *funcs, int func_count, int worker_count,
    ...
    if (hyperplanes && signatures) {
        ...
        lsm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
        lsm_parallel_for(worker_count, sig_build_worker, &sc, opts);
    }
    free(hyperplanes);
...
static void phase6a_score_candidates(lsm_sem_func_t *funcs, uint64_t *signatures, int *edge_counts,
                                     sem_bucket_t **band_buckets, lsm_sem_config_t cfg,
    ...
    atomic_init(&sc.next_idx, 0);
    lsm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    lsm_parallel_for(worker_count, score_worker, &sc, opts);
}
 ...
 * the caller must lsm_sem_corpus_free() later. */
static lsm_sem_corpus_t *run_corpus_phase(lsm_gbuf_t *gbuf, char **all_tokens, int *token_counts,
                                          int func_count) {
    LSM_PROF_START(t_phase3a);
    lsm_sem_corpus_t *corpus = lsm_sem_corpus_new();
    lsm_sem_corpus_add_docs_batch(corpus, all_tokens, token_counts, func_count, LSM_SEM_MAX_TOKENS);
    LSM_PROF_END_N("semantic_edges", "3a_corpus_batch", t_phase3a, func_count);

    LSM_PROF_START(t_phase3b);
    lsm_sem_corpus_finalize(corpus);
    LSM_PROF_END_N("semantic_edges", "3b_corpus_finalize_seq", t_phase3b,
                   lsm_sem_corpus_token_count(corpus));

    LSM_PROF_START(t_phase3c);
    phase3c_export_token_vectors(gbuf, corpus);
    LSM_PROF_END_N("semantic_edges", "3c_token_vec_export_seq", t_phase3c,
                   lsm_sem_corpus_token_count(corpus));
    return corpus;
}
...
static int run_scoring_phase(lsm_gbuf_t *gbuf, lsm_sem_func_t *funcs, uint64_t *signatures,
                             sem_bucket_t **band_buckets, lsm_sem_config_t cfg, int func_count,
                             int worker_count) {
...
static void free_funcs_and_tokens(lsm_sem_func_t *funcs, int func_count, char **all_tokens,
                                  const int *token_counts, LSMHashTable **pools, int worker_count) {
    (void)token_counts;
    for (int f = 0; f < func_count; f++) {
    ...
    if (pools) {
        for (int w = 0; w < worker_count; w++) {
            if (pools[w]) {
                lsm_ht_foreach(pools[w], free_token_pool_entry, NULL);
                lsm_ht_free(pools[w]);
            }
        }
        free(pools);
...
int lsm_pipeline_pass_semantic_edges(lsm_pipeline_ctx_t *ctx) {
    ...
    lsm_log_info("pass.start", "pass", "semantic_edges");

    lsm_gbuf_t *gbuf = ctx->gbuf;
    lsm_sem_config_t cfg = lsm_sem_get_config();

    LSM_PROF_START(t_phase1a);
    lsm_sem_func_t *funcs = NULL;
    const lsm_gbuf_node_t **node_ptrs = NULL;
    int func_count = phase1_scan_functions(gbuf, &funcs, &node_ptrs);
    LSM_PROF_END_N("semantic_edges", "1a_scan_seq", t_phase1a, func_count);

    /* Phase 1b: Decode minhash + profile + build api/type/deco vectors (PARALLEL). */
    lsm_sem_ensure_ready();
    LSM_PROF_START(t_phase1b);
    phase1b_decode_and_build(funcs, node_ptrs, gbuf, func_count, lsm_default_worker_count(false));
    LSM_PROF_END_N("semantic_edges", "1b_decode_build_parallel", t_phase1b, func_count);
    lsm_log_info("pass.semantic.collected", "functions", itoa_log(func_count));

    if (func_count < PSE_MIN_FUNCS_FOR_PAIR) {
        free(funcs);
        free(node_ptrs);
        lsm_log_info("pass.done", "pass", "semantic_edges", "edges", "0");
        return 0;
    }

    /* Phase 2: Tokenize all nodes (PARALLEL) */
    int worker_count = lsm_default_worker_count(false);
    char **all_tokens = malloc((size_t)func_count * sizeof(char *) * LSM_SEM_MAX_TOKENS);
    int *token_counts = calloc((size_t)func_count, sizeof(int));

    LSM_PROF_START(t_phase2);
    LSMHashTable **token_pools = calloc((size_t)worker_count, sizeof(LSMHashTable *));
    if (token_pools) {
        for (int w = 0; w < worker_count; w++) {
            token_pools[w] = lsm_ht_create(LSM_SZ_1K);
        }
    }
    phase2_tokenize(node_ptrs, gbuf, all_tokens, token_counts, func_count, worker_count,
                    token_pools);
    LSM_PROF_END_N("semantic_edges", "2_tokenize_parallel", t_phase2, func_count);
    free(node_ptrs);

    /* Phase 3: Build corpus (batch add), finalize, export enriched token vectors. */
    lsm_sem_corpus_t *corpus = run_corpus_phase(gbuf, all_tokens, token_counts, func_count);

    /* Phase 4: Build per-function TF-IDF + RI vectors (PARALLEL) and store them. */
    LSM_PROF_START(t_phase4);
    phase4_build_and_store_vectors(gbuf, funcs, all_tokens, token_counts, corpus, func_count,
                                   worker_count);
    LSM_PROF_END_N("semantic_edges", "4_build_and_store_vec", t_phase4, func_count);

    lsm_log_info("pass.semantic.vectors_stored", "count", itoa_log(func_count));

    /* Phase 5: LSH hyperplanes → signatures → buckets. */
    LSM_PROF_START(t_phase5);
    ...
    LSM_PROF_END_N("semantic_edges", "5_lsh_build", t_phase5, func_count);

    lsm_log_info("pass.semantic.lsh_built", "functions", itoa_log(func_count), "bands",
    ...
    /* Phase 7: Cleanup */
    LSM_PROF_START(t_phase7);
    free_lsh_buckets(band_buckets);
    free(signatures);
    lsm_log_info("pass.done", "pass", "semantic_edges", "edges", itoa_log(total_edges));
    free_funcs_and_tokens(funcs, func_count, all_tokens, token_counts, token_pools, worker_count);
    free(token_counts);
    lsm_sem_corpus_free(corpus);
    LSM_PROF_END("semantic_edges", "7_cleanup", t_phase7);
