...
#include "foundation/platform.h" /* lsm_normalize_path_sep */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>
...
#ifdef _WIN32
#include "foundation/win_utf8.h" /* lsm_path_to_wide */
#include <windows.h>
...
static lsm_regex_t dockerfile_re;  /* ENV|ARG KEY=VALUE or KEY VALUE */
static lsm_regex_t yaml_kv_re;     /* key: "https://..." */
static lsm_regex_t yaml_setenv_re; /* --set-env-vars KEY=VALUE */
static lsm_regex_t terraform_re;   /* default|value = "https://..." */
static lsm_regex_t shell_re;       /* [export] KEY=https://... */
static lsm_regex_t envfile_re;     /* KEY=https://... */
static lsm_regex_t toml_re;        /* key = "https://..." */
static lsm_regex_t properties_re;  /* key=https://... */
...
static void compile_patterns(void) {
    ...
    lsm_regcomp(&dockerfile_re, "^(ENV|ARG)[[:space:]]+(" W "+)[= ](.*)", LSM_REG_EXTENDED);
    lsm_regcomp(&yaml_kv_re, "(" W "+):[[:space:]]*[\"']?(https?://" NW "+)", LSM_REG_EXTENDED);
    lsm_regcomp(&yaml_setenv_re, "--set-env-vars[[:space:]]+(" W "+)=([^ \t]+)", LSM_REG_EXTENDED);
    lsm_regcomp(&terraform_re, "(default|value)[[:space:]]*=[[:space:]]*\"(https?://[^\"]+)\"",
                LSM_REG_EXTENDED);
    lsm_regcomp(&shell_re, "(export[[:space:]]+)?(" W "+)=[\"']?(https?://" NW "+)",
                LSM_REG_EXTENDED);
    lsm_regcomp(&envfile_re, "^(" W "+)=(https?://[^ \t]+)", LSM_REG_EXTENDED);
    lsm_regcomp(&toml_re, "(" W "+)[[:space:]]*=[[:space:]]*\"(https?://[^\"]+)\"",
                LSM_REG_EXTENDED);
    lsm_regcomp(&properties_re, "(" W "+)[[:space:]]*=[[:space:]]*(https?://[^ \t]+)",
                LSM_REG_EXTENDED);
...
static int extract_kv_groups(const char *trimmed, const lsm_regmatch_t *m, int key_grp, int val_grp,
...
static int scan_dockerfile_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    lsm_regmatch_t m[ENV_REGEX_MAX];
    if (lsm_regexec(&dockerfile_re, line, ENV_GRP_4, m, 0) != 0) {
        return 0;
    }
...
static int scan_yaml_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    lsm_regmatch_t m[ENV_REGEX_MAX];
    if (lsm_regexec(&yaml_kv_re, line, ENV_GRP_3, m, 0) == 0 &&
        extract_kv_groups(line, m, ENV_GRP_1, ENV_GRP_2, key, ksz, val, vsz)) {
        return SKIP_ONE;
    }
    if (lsm_regexec(&yaml_setenv_re, line, ENV_GRP_3, m, 0) == 0 &&
        extract_kv_groups(line, m, ENV_GRP_1, ENV_GRP_2, key, ksz, val, vsz)) {
        return SKIP_ONE;
    }
    return 0;
...
static int scan_terraform_line(const char *line, char *key, size_t ksz, char *val, size_t vsz) {
    lsm_regmatch_t m[ENV_REGEX_MAX];
    if (lsm_regexec(&terraform_re, line, ENV_GRP_3, m, 0) != 0) {
        return 0;
    }
...
static int scan_regex_line(lsm_regex_t *re, const char *line, int kg, int vg, char *key, size_t ksz,
                           char *val, size_t vsz) {
    lsm_regmatch_t m[ENV_REGEX_MAX];
    if (lsm_regexec(re, line, ENV_GRP_5, m, 0) == 0 &&
...
static bool envscan_join_path(char *out, size_t out_sz, const char *dir, const char *name) {
    ...
    if (written <= 0 || (size_t)written >= out_sz) {
        out[0] = '\0';
        lsm_log_warn("envscan.path_too_long", "dir", dir, "name", name);
        return false;
    }
    return true;
}
 ...
 * Returns 0 on success, LSM_NOT_FOUND to skip the entry.
...
static int envscan_safe_stat(const char *abs_path, struct stat *st) {
    ...
    wchar_t *wide = lsm_path_to_wide(abs_path);
    if (!wide) {
 ...
 * scan because lsm_canonical_path costs a syscall per component. */
static bool envscan_within_root(const char *candidate, const char *canonical_root) {
    char resolved[LSM_SZ_4K];
    if (!lsm_canonical_path(candidate, resolved, sizeof(resolved))) {
        return false;
    }
#ifdef _WIN32
    lsm_normalize_path_sep(resolved);
#endif
 ...
 * allows it. Mode "r" is preserved from the previous lsm_fopen call so the
...
static FILE *envscan_open_file(const char *full_path) {
     ...
     * envscan_safe_stat. lsm_fopen is required for UTF-8 → _wfopen mapping. */
    return lsm_fopen(full_path, "r");
#else
     ...
     * lsm_fopen rule exists for Windows' wide-API mapping. */
...
static int scan_env_file(const char *full_path, const char *rel, file_type_t ft,
                         lsm_env_binding_t *out, int max_out) {
    ...
    struct stat fst;
    if (fstat(fileno(f), &fst) != 0 || fst.st_size > (long)LSM_SZ_1K * LSM_SZ_1K) {
        (void)fclose(f);
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), f) && count < max_out) {
        ...
        if (lsm_is_secret_binding(key, value) || lsm_is_secret_value(value)) {
            continue;
        }
...
static int process_env_entry(lsm_dirent_t *ent, const char *dir_path, const char *root_path,
                             size_t root_len, const char *canonical_root, lsm_env_binding_t *out,
                             int max_out, char path_stack[][LSM_SZ_512], int *stack_top,
                             char **excluded_dirs, int excluded_count) {
    char full_path[LSM_SZ_512];
    ...
    if (S_ISDIR(st.st_mode)) {
        if (is_ignored_dir(ent->name) ||
            lsm_pipeline_relpath_is_excluded(rel, excluded_dirs, excluded_count) ||
            *stack_top >= LSM_SZ_256) {
            return 0;
        }
        if (!envscan_within_root(full_path, canonical_root)) {
            lsm_log_warn("envscan.dir_outside_root", "path", full_path, "root", canonical_root);
            return 0;
        }
        ...
        if (written <= 0 || (size_t)written >= sizeof(path_stack[0])) {
            lsm_log_warn("envscan.path_too_long", "dir", dir_path, "name", ent->name);
            return 0;
        }
        (*stack_top)++;
        return 0;
...
int lsm_scan_project_env_urls_excluded(const char *root_path, lsm_env_binding_t *out, int max_out,
    ...
    int count = 0;
    char path_stack[LSM_SZ_256][LSM_SZ_512];
    ...
    if (written <= 0 || (size_t)written >= sizeof(path_stack[0])) {
        lsm_log_warn("envscan.root_too_long", "root", root_path);
        return 0;
    }
...
#ifdef _WIN32
    lsm_normalize_path_sep(canonical_root);
#endif

    while (stack_top > 0 && count < max_out) {
        stack_top--;
        char dir_path[LSM_SZ_512];
        ...
        lsm_dirent_t *ent;
        while ((ent = lsm_readdir(d)) && count < max_out) {
...
int lsm_scan_project_env_urls(const char *root_path, lsm_env_binding_t *out, int max_out) {
    return lsm_scan_project_env_urls_excluded(root_path, out, max_out, NULL, 0);
}
