 ...
 */
#ifndef LSM_PIPELINE_H
#define LSM_PIPELINE_H

#include <stdbool.h>
#include <stdint.h>
#include <stdatomic.h>

#include "discover/discover.h"    /* lsm_ignored_file_t (#963) */
#include "foundation/constants.h" /* LSM_SZ_512 */

/* Forward declarations */
typedef struct lsm_store lsm_store_t;
typedef struct lsm_gbuf lsm_gbuf_t;

/* ── Opaque handle ──────────────────────────────────────────────── */

typedef struct lsm_pipeline lsm_pipeline_t;
...
typedef enum {
    ...
    LSM_MODE_FULL = 0,     /* Full: everything including SIMILAR_TO + SEMANTICALLY_RELATED */
    LSM_MODE_MODERATE = 1, /* Moderate: fast discovery + SIMILAR_TO + SEMANTICALLY_RELATED */
    LSM_MODE_FAST = 2,     /* Fast: skip non-essential files, no similarity/semantic edges */
} lsm_index_mode_t;
#endif
...
lsm_pipeline_t *lsm_pipeline_new(const char *repo_path, const char *db_path, lsm_index_mode_t mode);

/* Enable persistent artifact export (.logan-spine/graph.db.zst).
 ...
 * existing database in place. Those codes (LSM_PIPELINE_ABORT_PRESERVE_DB and
 * LSM_PIPELINE_PERSIST_FAILED) are defined in pipeline_internal.h alongside the
 ...
 * owned by the pipeline. Valid until lsm_pipeline_free(). */
...
/* Get the index mode (LSM_MODE_FULL, LSM_MODE_MODERATE, LSM_MODE_FAST). */
 ...
 * valid until lsm_pipeline_free()); *count receives its length. Both are set
 ...
 * the pipeline (copied on record, freed in lsm_pipeline_free). A skip is the
 ...
 * lsm_pipeline_free()). out and count are set to NULL and 0 when p is NULL or
 ...
 * it is exactly lsm_pipeline_fqn_module(). Caller must free(). */
 ...
 * (e.g. "src/api/helpers") suitable for passing to lsm_pipeline_fqn_module,
...
typedef struct lsm_registry lsm_registry_t;
...
/* Per-file full-result cache for lsm_registry_resolve. The same
...
int lsm_parse_hunks(const char *output, lsm_changed_hunk_t *out, int max_out);

#endif /* LSM_PIPELINE_H */
