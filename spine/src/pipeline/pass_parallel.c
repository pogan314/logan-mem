 ...
 *
 * Each file is read and parsed ONCE (Phase 3A). The LSMFileResult is cached
 * and reused for resolution (Phase 4), eliminating 3x redundant I/O + parsing.
 *
 * Depends on: worker_pool, graph_buffer (shared IDs + merge), extraction (lsm.h)
 */
 ...
 * (source_reread fallback). Both caps are env-overridable — LSM_RETAIN_TOTAL_MB
 * / LSM_RETAIN_PER_FILE_MB raise or lower the auto-derived defaults directly
...
#include "pipeline/pass_lsp_cross.h" /* lsm_pxc_* helpers for fused cross-file LSP */
#include "pipeline/lsp_resolve.h"
#include "helpers.h" /* lsm_kind_in_set_free_cache — per-worker-thread cache teardown */
...
/* Parse a positive MB-valued retention env knob (LSM_RETAIN_*_MB) into bytes.
 * Follows the limits.c strtol convention: unset / unparseable / non-positive
 * → return 0 so the caller keeps its derived default. */
static size_t lsm_retain_env_bytes(const char *name) {
 ...
 * the ceiling. */
static size_t lsm_parallel_extract_default_total_cap(void) {
    size_t cap = lsm_mem_budget();
    if (cap > 0) {
        cap /= 8;
    } else {
...
static size_t lsm_parallel_extract_default_per_file_cap(size_t total_cap) {
 ...
 * explicit caller opts > LSM_RETAIN_*_MB env knobs > RAM-derived defaults.
...
static lsm_parallel_extract_opts_t lsm_parallel_extract_resolve_opts(
    const lsm_parallel_extract_opts_t *opts) {
    lsm_parallel_extract_opts_t resolved = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_total_budget_bytes = lsm_parallel_extract_default_total_cap(),
        .retain_per_file_max_bytes = 0,
    };

    size_t env_total = lsm_retain_env_bytes("LSM_RETAIN_TOTAL_MB");
    if (env_total > 0) {
        resolved.retain_total_budget_bytes = env_total;
    }

    resolved.retain_per_file_max_bytes =
        lsm_parallel_extract_default_per_file_cap(resolved.retain_total_budget_bytes);
    size_t env_per_file = lsm_retain_env_bytes("LSM_RETAIN_PER_FILE_MB");
    if (env_per_file > 0) {
...
static char *read_file(const char *path, int *out_len, long *out_size,
                       lsm_read_status_t *out_status) {
    if (out_size) {
        *out_size = 0;
    }
    if (out_status) {
        *out_status = LSM_READ_OK;
    }
    FILE *f = lsm_fopen(path, "rb");
    if (!f) {
        if (out_status) {
            *out_status = LSM_READ_OPEN_FAIL;
        }
        return NULL;
    }
    ...
    if (size <= 0) {
        (void)fclose(f);
        if (out_status) {
            *out_status = LSM_READ_EMPTY;
        }
        return NULL;
    }
    if (size > lsm_max_file_bytes()) { /* generous, env-configurable cap (B4) */
        (void)fclose(f);
        if (out_status) {
            *out_status = LSM_READ_OVERSIZED;
        }
        return NULL;
    }
 ...
 * lsm_file_error_t array in the existing sequential merge loop. */
typedef struct {
    lsm_file_error_t *items;
    int count;
    int cap;
} pp_err_list_t;
...
static void pp_err_add(pp_err_list_t *list, const char *path, const char *reason,
    ...
    if (list->count >= list->cap) {
        ...
        lsm_file_error_t *grown =
            (lsm_file_error_t *)realloc(list->items, (size_t)ncap * sizeof(*grown));
        if (!grown) {
...
static const char *itoa_log(int val) {
    static LSM_TLS char bufs[PP_RING][LSM_SZ_32];
...
static void build_def_props(char *buf, size_t bufsize, const LSMDefinition *def) {
    ...
    if (def->fingerprint && def->fingerprint_k > 0 &&
        pos + LSM_MINHASH_HEX_LEN + LSM_MINHASH_JSON_OVERHEAD < bufsize) {
        char fp_hex[LSM_MINHASH_HEX_BUF];
        lsm_minhash_to_hex((const lsm_minhash_t *)def->fingerprint, fp_hex, sizeof(fp_hex));
        append_json_string(buf, bufsize, &pos, "fp", fp_hex);
    }

    /* AST structural profile — append if present and buffer has room. */
    if (def->structural_profile && pos + LSM_AST_PROFILE_BUF < bufsize) {
    ...
    if (def->body_tokens && pos + LSM_SZ_512 < bufsize) {
...
static bool pp_module_is_dir(LSMLanguage lang) {
...
static const char *resolve_as_class(const lsm_registry_t *reg, const char *name,
                                    const char *module_qn, const char **imp_keys,
                                    const char **imp_vals, int imp_count) {
    lsm_resolution_t res =
        lsm_registry_resolve(reg, name, module_qn, imp_keys, imp_vals, imp_count);
    ...
    const char *label = lsm_registry_label_of(reg, res.qualified_name);
    if (!lsm_label_is_type_like(label)) {
        return NULL;
    }
...
#define LSM_CACHE_LINE LSM_SZ_128

typedef struct __attribute__((aligned(LSM_CACHE_LINE))) {
    lsm_gbuf_t *local_gbuf;
    int nodes_created;
    int errors;
    char _pad[LSM_CACHE_LINE - sizeof(lsm_gbuf_t *) - (PP_ESC_SPACE * sizeof(int))];
} extract_worker_state_t;

typedef struct {
    const lsm_file_info_t *files;
    ...
    lsm_pkg_entries_t *pkg_entries; /* per-worker manifest arrays (separate allocation) */
    ...
    const LSMMacroTable *macro_table;            /* ObjectScript $$$macros (NULL if none) */
    const LSMReturnTypeTable *return_type_table; /* ObjectScript return types (NULL if none) */

    /* Superlinearity probe — see profile.h. Ticked on each file claim below. */
    lsm_scale_probe_t scale;
} extract_ctx_t;
...
static void insert_def_into_gbuf(extract_worker_state_t *ws, const lsm_file_info_t *fi,
    ...
    if (def->route_path && def->route_path[0] != '\0') {
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", rm,
                 lsm_route_canon_path(def->route_path, cpath, sizeof(cpath)));
        char rprops[LSM_SZ_256];
        snprintf(rprops, sizeof(rprops), "{\"method\":\"%s\",\"source\":\"decorator\"}", rm);
        int64_t route_id =
            lsm_gbuf_upsert_node(ws->local_gbuf, "Route", def->route_path, route_qn,
                                 def->file_path ? def->file_path : fi->rel_path, 0, 0, rprops);
        char hprops[LSM_SZ_512];
        char esc_h[LSM_SZ_512];
        lsm_json_escape(esc_h, sizeof(esc_h), def->qualified_name);
        snprintf(hprops, sizeof(hprops), "{\"handler\":\"%s\"}", esc_h);
        lsm_gbuf_insert_edge(ws->local_gbuf, func_id, route_id, "HANDLES", hprops);
    }
}

static void log_extract_fail(int pos, uint64_t ms, const char *path) {
    if (pos < PP_LOG_THRESH) {
        lsm_log_warn("parallel.extract.file.fail", "pos", itoa_log(pos), "elapsed_ms",
...
static void log_extract_done(int pos, uint64_t ms, int defs, const char *path) {
    if (pos < PP_LOG_THRESH || ms > PP_TIMER_THRESH) {
        lsm_log_info("parallel.extract.file.done", "pos", itoa_log(pos), "elapsed_ms",
...
static void extract_worker(int worker_id, void *ctx_ptr) {
    ...
    /* Lazy gbuf creation */
    if (!ws->local_gbuf) {
        ws->local_gbuf = lsm_gbuf_new_shared_ids(ec->project_name, ec->repo_path, ec->shared_ids);
    }
    ...
    while (SKIP_ONE) {
        ...
        lsm_scale_tick(&ec->scale, sort_pos);
        ...
        if (lsm_mem_budget() > 0) {
            ...
            if (over && !futile) {
                ...
                int bp = 0;
                for (; bp < PP_BACKPRESSURE_MAX_SPINS && lsm_mem_over_budget() &&
                       !atomic_load_explicit(ec->cancelled, memory_order_relaxed);
                     bp++) {
                    struct timespec nap = {0, PP_BACKPRESSURE_NAP_NS};
                    lsm_nanosleep(&nap, NULL);
                }
                if (bp == PP_BACKPRESSURE_MAX_SPINS && lsm_mem_over_budget()) {
                    ...
                    if (atomic_exchange_explicit(&ec->bp_futile, 1, memory_order_relaxed) == 0) {
                        lsm_log_warn("mem.backpressure.futile", "action", "soft_overshoot");
                    }
                }
            } else if (!over && futile) {
        ...
        const lsm_file_info_t *fi = &ec->files[file_idx];
         ...
         * LSM_INDEX_QUARANTINE_FILE. Covers the parallel path; the supervisor's
         * single-threaded recovery run instead takes the sequential path
         * (pass_definitions.c), and lsm_extract_file's hard guard backstops both. */
        if (lsm_index_is_quarantined(fi->rel_path)) {
            const char *phase = lsm_index_quarantine_phase(fi->rel_path);
            if (!phase) {
                phase = "crash";
            }
            const char *reason =
        ...
        if (!source) {
            ...
            if (rst == LSM_READ_OVERSIZED) {
                ...
                if (atomic_fetch_add_explicit(&ec->oversized_warned, SKIP_ONE,
                    ...
                    lsm_log_warn("index.file_oversized", "path", fi->rel_path, "size_mb",
                                 itoa_log((int)(file_size / (LSM_SZ_1K * LSM_SZ_1K))), "cap_mb",
                                 itoa_log((int)(cap / (LSM_SZ_1K * LSM_SZ_1K))));
                }
            } else if (rst == LSM_READ_OPEN_FAIL || rst == LSM_READ_OOM) {
                pp_err_add(errs, fi->rel_path, "read failed", "read");
            }
            /* LSM_READ_EMPTY: benign 0-byte file — not reported. */
            continue;
        }
        ...
        if (sort_pos < PP_LOG_THRESH || /* first 2 rounds of workers = most interesting */
            lsm_log_crash_durable()) {
            lsm_log_info("parallel.extract.file.start", "pos", itoa_log(sort_pos), "size_kb",
                         itoa_log(source_len / LSM_SZ_1K), "path", fi->rel_path);
        }
        ...
        LSMFileResult *result =
            fi->language == LSM_LANG_OBJECTSCRIPT_EXPORT
                ? lsm_pipeline_extract_objectscript_export(source, source_len, ec->project_name,
                                                           fi->rel_path, ec->macro_table,
                                                           ec->return_type_table)
                : lsm_extract_file_ex(source, source_len, fi->language, ec->project_name,
                                      fi->rel_path, LSM_EXTRACT_BUDGET, NULL, NULL, ec->macro_table,
                                      ec->return_type_table);
        ...
        for (int d = 0; d < result->defs.count; d++) {
            LSMDefinition *def = &result->defs.items[d];
        ...
        lsm_free_tree(result);

        /* Detect and parse manifest files for package map */
        {
            const char *bn = strrchr(fi->rel_path, '/');
            lsm_pkgmap_try_parse(bn ? bn + SKIP_ONE : fi->rel_path, fi->rel_path, source,
        ...
        if (ec->retain_sources && source_len > 0) {
            ...
            if ((size_t)source_len > ec->retain_per_file_max_bytes) {
                ...
                if ((size_t)(prior + (int64_t)source_len) <= ec->retain_total_budget_bytes) {
                    char *copy = (char *)lsm_arena_alloc(&result->arena, (size_t)source_len + 1);
                    if (copy) {
            ...
            if (dropped &&
                ...
                lsm_log_warn("index.retain_capped", "path", fi->rel_path ? fi->rel_path : "?",
        ...
        if ((sort_pos + SKIP_ONE) % PP_LOG_INTERVAL == 0 || sort_pos + SKIP_ONE == ec->file_count) {
            lsm_log_info("parallel.extract.progress", "done", itoa_log(sort_pos + SKIP_ONE),
                         "total", itoa_log(ec->file_count));
        }

        /* Reclaim all slab + tier2 memory between files.
         *
         * After lsm_free_tree(result), all tree nodes are on free lists.
         ...
         *
         * get_thread_parser() in lsm_extract_file will create a fresh
    ...
    lsm_slab_destroy_thread();
    lsm_kind_in_set_free_cache(); /* free this worker thread's node-type bitset cache */
}

static void merge_pkg_entries(lsm_pipeline_ctx_t *ctx, lsm_pkg_entries_t *pkg_entries,
                              int worker_count) {
    if (!pkg_entries) {
        return;
    }
    ...
    lsm_pkgmap_scan_repo(ctx->repo_path, &pkg_entries[0], ctx->excluded_dirs, ctx->excluded_count);
    lsm_pipeline_set_pkgmap(lsm_pkgmap_build(pkg_entries, worker_count, ctx->project_name));
    for (int i = 0; i < worker_count; i++) {
        lsm_pkg_entries_free(&pkg_entries[i]);
    }
    free(pkg_entries);
}
...
static void log_extract_mem_stats(int worker_count) {
    if (lsm_mem_budget() > 0) {
        size_t mb = (size_t)LSM_SZ_1K * LSM_SZ_1K;
        lsm_log_info("parallel.extract.mem", "rss_mb", itoa_log((int)(lsm_mem_rss() / mb)),
                     "peak_mb", itoa_log((int)(lsm_mem_peak_rss() / mb)), "budget_mb",
                     itoa_log((int)(lsm_mem_budget() / mb)), "per_worker_mb",
                     itoa_log((int)(lsm_mem_worker_budget(worker_count) / mb)));
    }
}
...
int lsm_parallel_extract_ex(lsm_pipeline_ctx_t *ctx, const lsm_file_info_t *files, int file_count,
    ...
    lsm_parallel_extract_opts_t resolved_opts = lsm_parallel_extract_resolve_opts(opts);

    if (file_count == 0) {
        return 0;
    }

    lsm_log_info("parallel.extract.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    {
        size_t mb = (size_t)LSM_SZ_1K * LSM_SZ_1K;
        lsm_log_info("parallel.extract.retention", "retain_sources",
    ...
    if (lsm_mem_budget() > 0) {
        size_t worker_budget = lsm_mem_worker_budget(worker_count);
        lsm_log_info("parallel.mem.budget", "total_mb",
                     itoa_log((int)(lsm_mem_budget() / ((size_t)LSM_SZ_1K * LSM_SZ_1K))),
                     "per_worker_mb",
                     itoa_log((int)(worker_budget / ((size_t)LSM_SZ_1K * LSM_SZ_1K))));
    }

    /* Sub-phase: Ensure extraction library is initialized */
    LSM_PROF_START(t_init);
    lsm_init();
    ...
    lsm_destroy_thread_parser();
    lsm_slab_install();
    LSM_PROF_END("parallel_extract", "1_init_libs", t_init);

    /* Sub-phase: Sort files by descending size for tail-latency reduction */
    LSM_PROF_START(t_sort);
    ...
    LSM_PROF_END_N("parallel_extract", "2_sort_files", t_sort, file_count);

    /* Allocate per-worker state (cache-line aligned via posix_memalign) */
    extract_worker_state_t *workers = NULL;
    if (lsm_aligned_alloc((void **)&workers, LSM_CACHE_LINE,
    ...
    lsm_pkg_entries_t *pkg_entries = calloc((size_t)worker_count, sizeof(lsm_pkg_entries_t));
    if (!pkg_entries) {
        lsm_aligned_free(workers);
        free(sorted);
        return LSM_NOT_FOUND;
    }
    ...
    LSMMacroTable *pp_macro_table =
        lsm_build_macro_table_from_files(files, file_count, ctx->repo_path);
    ...
    LSM_PROF_START(t_dispatch);
    lsm_parallel_for_opts_t parallel_opts = {.max_workers = worker_count, .force_pthreads = false};
    lsm_scale_begin(&ec.scale, "parallel_extract", (long)file_count);
    lsm_parallel_for(worker_count, extract_worker, &ec, parallel_opts);
    lsm_scale_end(&ec.scale);
    LSM_PROF_END_N("parallel_extract", "3_dispatch_workers_parallel", t_dispatch, file_count);

    /* Sub-phase: Merge all local gbufs into main gbuf (SEQUENTIAL, gbuf not thread-safe) */
    LSM_PROF_START(t_merge);
    int total_nodes = 0;
    int total_errors = 0;
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_gbuf) {
            lsm_gbuf_merge(ctx->gbuf, workers[i].local_gbuf);
            total_nodes += workers[i].nodes_created;
            total_errors += workers[i].errors;
            lsm_gbuf_free(workers[i].local_gbuf);
        }
    }
    LSM_PROF_END_N("parallel_extract", "4_merge_gbufs_seq", t_merge, total_nodes);
    ...
    if (err_lists) {
        for (int i = 0; i < worker_count; i++) {
            for (int j = 0; j < err_lists[i].count; j++) {
                lsm_pipeline_add_file_error(ctx->pipeline, err_lists[i].items[j].path,
    ...
    lsm_aligned_free(workers);
    free(sorted);
    lsm_macro_table_free(pp_macro_table); /* ObjectScript macro table (NULL-safe) */
    ...
    lsm_log_info("parallel.extract.done", "nodes", itoa_log(total_nodes), "errors",
...
int lsm_parallel_extract(lsm_pipeline_ctx_t *ctx, const lsm_file_info_t *files, int file_count,
                         LSMFileResult **result_cache, _Atomic int64_t *shared_ids,
                         int worker_count) {
    return lsm_parallel_extract_ex(ctx, files, file_count, result_cache, shared_ids, worker_count,
                                   NULL);
}
...
static int register_and_link_def(lsm_pipeline_ctx_t *ctx, const LSMDefinition *def, const char *rel,
                                 int *reg_entries) {
    int edges = 0;
    ...
    if (strcmp(def->label, "Function") == 0 || strcmp(def->label, "Method") == 0 ||
        lsm_label_is_type_like(def->label) || strcmp(def->label, "Variable") == 0 ||
        strcmp(def->label, "Field") == 0) {
        lsm_registry_add(ctx->registry, def->name, def->qualified_name, def->label);
        (*reg_entries)++;
    }
    char *file_qn = lsm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
    const lsm_gbuf_node_t *file_node = lsm_gbuf_find_by_qn(ctx->gbuf, file_qn);
    const lsm_gbuf_node_t *def_node = lsm_gbuf_find_by_qn(ctx->gbuf, def->qualified_name);
    if (file_node && def_node) {
        lsm_gbuf_insert_edge(ctx->gbuf, file_node->id, def_node->id, "DEFINES", "{}");
        edges++;
    }
    free(file_qn);
    if (def->parent_class && strcmp(def->label, "Method") == 0) {
        const lsm_gbuf_node_t *parent = lsm_gbuf_find_by_qn(ctx->gbuf, def->parent_class);
        if (parent && def_node) {
            lsm_gbuf_insert_edge(ctx->gbuf, parent->id, def_node->id, "DEFINES_METHOD", "{}");
        }
    }
    return edges;
}
...
static int create_imports_edges(lsm_pipeline_ctx_t *ctx, const LSMFileResult *result,
                                const char *rel, LSMHashTable *namespace_map) {
    int count = 0;
    char *file_qn = lsm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
    const lsm_gbuf_node_t *source_node = lsm_gbuf_find_by_qn(ctx->gbuf, file_qn);
    if (!source_node) {
        free(file_qn);
        return 0;
    }
    for (int j = 0; j < result->imports.count; j++) {
        LSMImport *imp = &result->imports.items[j];
        if (!imp->module_path) {
            continue;
        }
        const lsm_gbuf_node_t *target =
            lsm_pipeline_resolve_import_node(ctx, rel, file_qn, imp, namespace_map);
        if (target && target->id != source_node->id) {
            char esc_ln[LSM_SZ_128];
            lsm_json_escape(esc_ln, sizeof(esc_ln), imp->local_name ? imp->local_name : "");
            char imp_props[LSM_SZ_256];
            snprintf(imp_props, sizeof(imp_props), "{\"local_name\":\"%s\"}", esc_ln);
            lsm_gbuf_insert_edge(ctx->gbuf, source_node->id, target->id, "IMPORTS", imp_props);
            count++;
        }
    }
    free(file_qn);
...
static const lsm_gbuf_node_t *find_channel_src(lsm_pipeline_ctx_t *ctx, const LSMChannel *ch,
                                               const char *rel) {
    const lsm_gbuf_node_t *node = NULL;
    if (ch->enclosing_func_qn && ch->enclosing_func_qn[0]) {
        node = lsm_gbuf_find_by_qn(ctx->gbuf, ch->enclosing_func_qn);
    }
    if (!node) {
        char *file_qn = lsm_pipeline_fqn_compute(ctx->project_name, rel, "__file__");
...
static void create_channel_edges(lsm_pipeline_ctx_t *ctx, const LSMFileResult *result,
                                 const char *rel) {
    for (int j = 0; j < result->channels.count; j++) {
        LSMChannel *ch = &result->channels.items[j];
        if (!ch->channel_name || !ch->channel_name[0]) {
            continue;
        }
        char channel_qn[LSM_SZ_512];
        snprintf(channel_qn, sizeof(channel_qn), "__channel__%s__%s",
                 ch->transport ? ch->transport : "unknown", ch->channel_name);
        char esc_cn[LSM_SZ_256];
        lsm_json_escape(esc_cn, sizeof(esc_cn), ch->channel_name);
        char channel_props[LSM_SZ_512];
        snprintf(channel_props, sizeof(channel_props), "{\"transport\":\"%s\",\"name\":\"%s\"}",
                 ch->transport ? ch->transport : "unknown", esc_cn);
        int64_t channel_id = lsm_gbuf_upsert_node(ctx->gbuf, "Channel", ch->channel_name,
                                                  channel_qn, "", 0, 0, channel_props);
        const lsm_gbuf_node_t *src_node = find_channel_src(ctx, ch, rel);
        if (src_node && channel_id > 0) {
            const char *edge_type = ch->direction == LSM_CHANNEL_EMIT ? "EMITS" : "LISTENS_ON";
            char edge_props[LSM_SZ_128];
            snprintf(edge_props, sizeof(edge_props), "{\"transport\":\"%s\"}",
                     ch->transport ? ch->transport : "unknown");
            lsm_gbuf_insert_edge(ctx->gbuf, src_node->id, channel_id, edge_type, edge_props);
        }
    }
}
...
int lsm_build_registry_from_cache(lsm_pipeline_ctx_t *ctx, const lsm_file_info_t *files,
    ...
    lsm_log_info("parallel.registry.start", "files", itoa_log(file_count));
    ...
    LSMHashTable *namespace_map =
        lsm_pipeline_namespace_map_build(ctx->project_name, result_cache, rels, file_count);
    free(rels);

    for (int i = 0; i < file_count; i++) {
        if (lsm_pipeline_check_cancel(ctx)) {
            lsm_pipeline_namespace_map_free(namespace_map);
            return LSM_NOT_FOUND;
        }

        LSMFileResult *result = result_cache[i];
        if (!result) {
            continue;
        }
        ...
        lsm_pipeline_create_env_configures_for_file(ctx, result, rel);
    }

    lsm_pipeline_namespace_map_free(namespace_map);

    lsm_log_info("parallel.registry.done", "entries", itoa_log(reg_entries), "defines",
...
typedef struct __attribute__((aligned(LSM_CACHE_LINE))) {
    ...
    int lsp_overrides;
    char _pad[LSM_CACHE_LINE - sizeof(lsm_gbuf_t *) - ((PP_RING + 1) * sizeof(int))];
...
typedef struct {
    ...
    LSMFileResult **result_cache;
    const lsm_gbuf_t *main_gbuf;    /* READ-ONLY during Phase 4 */
    const lsm_registry_t *registry; /* READ-ONLY during Phase 4 */
    _Atomic int64_t *shared_ids;
    _Atomic int *cancelled;
    _Atomic int next_file_idx;
     ...
     * match the existing lsm_run_X_lsp_cross callee signatures the
     ...
     * When non-NULL, the fused worker calls lsm_pxc_filter_defs_for_file
     ...
     * lsm_run_X_lsp_cross_with_registry — skip per-file build entirely.
     * Stored as LSMCrossLspRegistries* (typedef from pass_lsp_cross.h). */
    LSMCrossLspRegistries *cross_registries;
    ...
    _Atomic(LSMTypeRegistry *) rust_shared_reg;
    lsm_mutex_t rust_shared_mu;
    LSMArena rust_shared_arena;
     ...
     * lsm_parallel_for synchronization barrier at the end. */
...
#define LSM_ARG_JSON_GUARD LSM_SZ_32
...
static int format_call_arg(char *buf, size_t bufsize, const LSMCallArg *a, const char *expr) {
    char esc_k[LSM_SZ_128];
    char esc_e[LSM_SZ_128];
    char esc_v[LSM_SZ_128];
    lsm_json_escape(esc_e, sizeof(esc_e), expr);
    if (a->keyword && a->value) {
        lsm_json_escape(esc_k, sizeof(esc_k), a->keyword);
        lsm_json_escape(esc_v, sizeof(esc_v), a->value);
...
static size_t append_args_json(char *buf, size_t bufsize, size_t pos, const LSMCall *call) {
    ...
    pos += (size_t)n;
    for (int i = 0; i < call->arg_count && pos < bufsize - LSM_ARG_JSON_GUARD; i++) {
        const LSMCallArg *a = &call->args[i];
        size_t mark = pos; /* rollback point (before the separator) */
        if (i > 0 && pos < bufsize - SKIP_ONE) {
            buf[pos++] = ',';
        }
        char expr_buf[LSM_SZ_128];
...
static const char *find_route_path_in_args(const LSMCall *call, const char **out_handler) {
    *out_handler = NULL;
    ...
    for (int ai = 0; ai < call->arg_count && !found; ai++) {
        const LSMCallArg *ca = &call->args[ai];
...
static void finalize_and_emit(lsm_gbuf_t *gbuf, int64_t src_id, int64_t tgt_id,
                              const char *edge_type, char *props, int n, const LSMCall *call) {
    if (n > 0 && (size_t)n < LSM_SZ_2K - PP_ESC_SPACE) {
        size_t pos = append_args_json(props, LSM_SZ_2K, (size_t)n, call);
        if (call->start_line > 0 && strcmp(edge_type, "CALLS") == 0 &&
            pos < LSM_SZ_2K - PP_LINE_MARGIN) {
            int ln = snprintf(props + pos, LSM_SZ_2K - pos, ",\"line\":%d", call->start_line);
            if (ln > 0) {
                pos += (size_t)ln;
            }
        }
        if (pos < LSM_SZ_2K - SKIP_ONE) {
            props[pos] = '}';
            props[pos + SKIP_ONE] = '\0';
        }
    }
    lsm_gbuf_insert_edge(gbuf, src_id, tgt_id, edge_type, props);
}

/* Build Route node QN and properties for HTTP/async service edges. */
static int64_t build_service_route(lsm_gbuf_t *gbuf, const char *arg, const char *method,
                                   const char *broker, lsm_svc_kind_t svc) {
    char route_qn[LSM_ROUTE_QN_SIZE];
    const char *prefix;
    char cpath[LSM_SZ_256];
    ...
    if (svc == LSM_SVC_HTTP) {
        ...
        qpath = lsm_route_canon_path(arg, cpath, sizeof(cpath));
    } else {
    ...
    return lsm_gbuf_upsert_node(gbuf, "Route", arg, route_qn, "", 0, 0, route_props);
}

/* Emit HTTP_CALLS or ASYNC_CALLS edge via Route node. */
static void emit_http_async_service_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                                         const LSMCall *call, const lsm_resolution_t *res,
                                         lsm_svc_kind_t svc, const char *arg) {
    const char *edge_type = (svc == LSM_SVC_HTTP) ? "HTTP_CALLS" : "ASYNC_CALLS";
    const char *method =
        (svc == LSM_SVC_HTTP) ? lsm_service_pattern_http_method(call->callee_name) : NULL;
    const char *broker =
        (svc == LSM_SVC_ASYNC) ? lsm_service_pattern_broker(res->qualified_name) : NULL;

    int64_t route_id = build_service_route(gbuf, arg, method, broker, svc);

    char esc_c[LSM_SZ_256];
    char esc_a[LSM_SZ_256];
    lsm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    lsm_json_escape(esc_a, sizeof(esc_a), arg);
...
static void emit_config_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                             const lsm_gbuf_node_t *target, const LSMCall *call,
                             const lsm_resolution_t *res, const char *arg) {
    ...
    char esc_c[LSM_SZ_256];
    char esc_k[LSM_SZ_256];
    lsm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
    lsm_json_escape(esc_k, sizeof(esc_k), arg ? arg : "");
    char props[LSM_SZ_2K];
...
static void emit_normal_calls_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                                   const lsm_gbuf_node_t *target, const LSMCall *call,
                                   const lsm_resolution_t *res) {
...
static void emit_route_registration(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                                    const LSMCall *call, const char *route_path,
                                    const char *handler_ref, const char *module_qn,
                                    const lsm_registry_t *registry, const lsm_gbuf_t *main_gbuf,
                                    const char **ik, const char **iv, int ic) {
    const char *method = lsm_service_pattern_route_method(call->callee_name);
    char rqn[LSM_ROUTE_QN_SIZE];
    char cpath[LSM_SZ_256];
    ...
    int64_t rid = lsm_gbuf_upsert_node(gbuf, "Route", route_path, rqn, "", 0, 0, rp);
    char esc_cn[LSM_SZ_256]; /* sliced source text: escape quotes/newlines */
    char esc_rp[LSM_SZ_512];
    lsm_json_escape(esc_cn, sizeof(esc_cn), call->callee_name);
    lsm_json_escape(esc_rp, sizeof(esc_rp), route_path);
    char props[LSM_SZ_1K];
    snprintf(props, sizeof(props),
             "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"route_registration\"}", esc_cn,
             esc_rp);
    lsm_gbuf_insert_edge(gbuf, source->id, rid, "CALLS", props);
    if (handler_ref && handler_ref[0] != '\0') {
        lsm_resolution_t hres = lsm_registry_resolve(registry, handler_ref, module_qn, ik, iv, ic);
        if (hres.qualified_name && hres.qualified_name[0] != '\0') {
            const lsm_gbuf_node_t *h = lsm_gbuf_find_by_qn(main_gbuf, hres.qualified_name);
            if (h) {
                char hp[LSM_SZ_1K]; /* must exceed escaped value + wrapper or snprintf cuts the
                                       closing brace */
                char esc_h2[LSM_SZ_512];
                lsm_json_escape(esc_h2, sizeof(esc_h2), hres.qualified_name);
                snprintf(hp, sizeof(hp), "{\"handler\":\"%s\"}", esc_h2);
                lsm_gbuf_insert_edge(gbuf, h->id, rid, "HANDLES", hp);
            }
        }
    }
}
...
static void detect_url_in_args(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                               const LSMCall *call) {
    for (int ai = 0; ai < call->arg_count; ai++) {
        const LSMCallArg *ca = &call->args[ai];
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__ANY__%s",
                 lsm_route_canon_path(norm, cpath, sizeof(cpath)));
        int64_t route_id = lsm_gbuf_upsert_node(gbuf, "Route", norm, route_qn, "", 0, 0,
                                                "{\"source\":\"arg_url\"}");
        char esc_c[LSM_SZ_256];
        char esc_n[LSM_SZ_256];
        lsm_json_escape(esc_c, sizeof(esc_c), call->callee_name);
        lsm_json_escape(esc_n, sizeof(esc_n), norm);
        char eprops[LSM_SZ_512];
        snprintf(eprops, sizeof(eprops),
                 "{\"callee\":\"%s\",\"url_path\":\"%s\",\"via\":\"arg_url\"}", esc_c, esc_n);
        lsm_gbuf_insert_edge(gbuf, source->id, route_id, "HTTP_CALLS", eprops);
        break;
    }
}
...
bool extract_grpc_service_method(const char *callee, char *service, size_t srv_sz, char *method,
    ...
    const char *s = raw;
    if (strncmp(s, "pb.New", LSM_SZ_6) == 0) {
        s += LSM_SZ_6;
    } else if (strncmp(s, "pb.", LSM_SZ_3) == 0 || strncmp(s, "New", LSM_SZ_3) == 0) {
        s += LSM_SZ_3;
    }
...
static void emit_grpc_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source, const LSMCall *call,
                           const lsm_resolution_t *res) {
    char service[LSM_SZ_256];
    char method[LSM_SZ_256];
    ...
    char route_qn[LSM_SZ_512];
    snprintf(route_qn, sizeof(route_qn), "__grpc__%s/%s", service, method);

    char route_name[LSM_SZ_256];
    snprintf(route_name, sizeof(route_name), "%s/%s", service, method);

    int64_t route_id = lsm_gbuf_upsert_node(gbuf, "Route", route_name, route_qn, "", 0, 0,
                                            "{\"source\":\"grpc\"}");
    ...
    lsm_gbuf_insert_edge(gbuf, source->id, route_id, "GRPC_CALLS", props);
}

/* Emit GRAPHQL_CALLS edge. Extract operation from first string arg if available. */
static void emit_graphql_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source, const LSMCall *call,
                              const lsm_resolution_t *res) {
    ...
    if (strncmp(p, "query ", LSM_SZ_6) == 0) {
        p += LSM_SZ_6;
    } else if (strncmp(p, "mutation ", LSM_SZ_8) == 0) {
        p += LSM_SZ_8;
    }
    ...
    int64_t route_id =
        lsm_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"graphql\"}");
    ...
    lsm_gbuf_insert_edge(gbuf, source->id, route_id, "GRAPHQL_CALLS", props);
}

/* Emit TRPC_CALLS edge. Extract procedure path from callee chain. */
static void emit_trpc_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source, const LSMCall *call,
                           const lsm_resolution_t *res) {
    ...
    int64_t route_id =
        lsm_gbuf_upsert_node(gbuf, "Route", p, route_qn, "", 0, 0, "{\"source\":\"trpc\"}");
    ...
    snprintf(props, sizeof(props), "{\"callee\":\"%s\",\"procedure\":\"%s\",\"confidence\":%.2f}",
             esc_c, p, res->confidence);
    lsm_gbuf_insert_edge(gbuf, source->id, route_id, "TRPC_CALLS", props);
}
...
static void emit_service_edge(lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                              const lsm_gbuf_node_t *target, const LSMCall *call,
                              const lsm_resolution_t *res, const char *module_qn,
                              const lsm_registry_t *registry, const lsm_gbuf_t *main_gbuf,
                              const char **imp_keys, const char **imp_vals, int imp_count,
                              bool suppress_plain_calls) {
    lsm_svc_kind_t svc = lsm_service_pattern_match(res->qualified_name);
    ...
    if (svc == LSM_SVC_NONE && lsm_service_pattern_route_method(call->callee_name) != NULL) {
        svc = LSM_SVC_ROUTE_REG;
    }
    ...
    if (svc == LSM_SVC_NONE && res->qualified_name) {
    ...
    bool has_topic = (arg && arg[0] != '\0' && svc == LSM_SVC_ASYNC && strlen(arg) > PP_ESC_SPACE);

    if ((svc == LSM_SVC_HTTP || svc == LSM_SVC_ASYNC) && (has_url || has_topic)) {
        emit_http_async_service_edge(gbuf, source, call, res, svc, arg);
    } else if (svc == LSM_SVC_GRPC) {
        emit_grpc_edge(gbuf, source, call, res);
    } else if (svc == LSM_SVC_GRAPHQL) {
        emit_graphql_edge(gbuf, source, call, res);
    } else if (svc == LSM_SVC_TRPC) {
        emit_trpc_edge(gbuf, source, call, res);
    } else if (svc == LSM_SVC_CONFIG) {
...
static const lsm_gbuf_node_t *find_source_node(const lsm_gbuf_t *gbuf, const char *project,
                                               const char *rel, const char *enclosing_qn) {
    const lsm_gbuf_node_t *src = NULL;
    if (enclosing_qn) {
        src = lsm_gbuf_find_by_qn(gbuf, enclosing_qn);
        ...
        if (lsm_pipeline_node_is_dir_container(src)) {
            src = NULL;
        }
    }
    if (!src) {
        char *file_qn = lsm_pipeline_fqn_compute(project, rel, "__file__");
        src = lsm_gbuf_find_by_qn(gbuf, file_qn);
        free(file_qn);
    }
    return src;
}
...
static void try_field_type_hint(resolve_ctx_t *rc, lsm_resolution_t *res, const char *callee_name,
                                int64_t source_id) {
...
static LSMResolvedCall *const LSP_IDX_AMBIGUOUS = (LSMResolvedCall *)(uintptr_t)1;

static bool lsp_idx_keep_best(LSMHashTable *index, const char *key, LSMResolvedCall *candidate,
                              const lsm_gbuf_t *gbuf, const char *project_name,
                              bool allow_tail_match) {
    if (!index || !key || !candidate) {
        return false;
    }
    LSMResolvedCall *existing = (LSMResolvedCall *)lsm_ht_get(index, key);
    if (!existing) {
        char *owned_key = strdup(key);
        if (owned_key) {
            lsm_ht_set(index, owned_key, candidate);
            if (lsm_ht_get(index, key) == candidate) {
                return true;
            }
            free(owned_key);
        }
    ...
    if (!lsm_pipeline_invocation_targets_equal(existing->callee_qn, candidate->callee_qn, gbuf,
                                               project_name, allow_tail_match)) {
        const char *stored_key = lsm_ht_get_key(index, key);
        if (stored_key) {
            lsm_ht_set(index, stored_key, LSP_IDX_AMBIGUOUS);
        }
        return lsm_ht_get(index, key) != NULL;
    }
    if (candidate->confidence > existing->confidence) {
        const char *stored_key = lsm_ht_get_key(index, key);
        if (stored_key) {
            lsm_ht_set(index, stored_key, candidate);
        }
    }
    return lsm_ht_get(index, key) != NULL;
}
...
static bool lsp_idx_insert_leaf(LSMHashTable *index, LSMResolvedCall *candidate, const char *leaf,
                                bool exact_site, const lsm_gbuf_t *gbuf, const char *project_name,
                                bool allow_tail_match) {
...
static const LSMResolvedCall *lsp_idx_lookup(const LSMHashTable *index, const LSMCall *call,
    ...
    const char *leaf = lsm_pipeline_call_callee_leaf(call->callee_name);
    if (!leaf || !leaf[0]) {
        return NULL;
    }
    ...
    const LSMResolvedCall *candidate = (const LSMResolvedCall *)lsm_ht_get(index, key);
    free(key);
    ...
    if (!candidate || candidate->kind != LSM_RESOLVED_INVOCATION || !candidate->caller_qn ||
        strcmp(candidate->caller_qn, call->enclosing_func_qn) != 0 ||
        candidate->confidence < LSM_LSP_CONFIDENCE_FLOOR) {
        return NULL;
    }
    int site_rank = lsm_pipeline_invocation_site_rank(candidate, call);
    int expected_rank = exact_site ? 2 : 1;
    return site_rank == expected_rank &&
                   lsm_pipeline_invocation_leaf_matches(candidate, call, site_rank)
               ? candidate
               : NULL;
}

/* Resolve calls for one file and emit CALLS/HTTP_CALLS/ASYNC_CALLS edges. */
static void resolve_file_calls(resolve_ctx_t *rc, resolve_worker_state_t *ws, LSMFileResult *result,
    ...
    LSMHashTable *lsp_exact_idx = NULL;
    LSMHashTable *lsp_legacy_idx = NULL;
    ...
    if (result->calls.count > 0 && result->resolved_calls.count > 0) {
        ...
        lsp_exact_idx = lsm_ht_create(capacity);
        lsp_legacy_idx = lsm_ht_create(capacity);
        if (!lsp_exact_idx) {
        ...
        if (lsp_exact_idx || lsp_legacy_idx) {
            for (int i = 0; i < result->resolved_calls.count; i++) {
                LSMResolvedCall *rc_e = &result->resolved_calls.items[i];
                if (rc_e->kind != LSM_RESOLVED_INVOCATION || !rc_e->caller_qn || !rc_e->callee_qn ||
                    rc_e->confidence < LSM_LSP_CONFIDENCE_FLOOR) {
                    continue;
                }
                bool exact_site =
                    lsm_pipeline_source_site_present(rc_e->site_start_byte, rc_e->site_end_byte);
                bool legacy_site =
                    lsm_pipeline_source_site_legacy(rc_e->site_start_byte, rc_e->site_end_byte);
                if (!exact_site && !legacy_site) {
                    continue;
                }
                LSMHashTable *index = exact_site ? lsp_exact_idx : lsp_legacy_idx;
                bool inserted =
                    lsp_idx_insert_leaf(index, rc_e, lsm_lsp_bare_segment(rc_e->callee_qn),
                ...
                if (rc_e->reason && lsm_pipeline_invocation_reason_join_strategy(rc_e->strategy)) {
                    inserted = lsp_idx_insert_leaf(index, rc_e, lsm_lsp_bare_segment(rc_e->reason),
    ...
    for (int c = 0; c < result->calls.count; c++) {
        LSMCall *call = &result->calls.items[c];
        if (!call->callee_name) {
            continue;
        }
        uint64_t _rc_t0 = extract_now_ns();
        const lsm_gbuf_node_t *source_node =
         ...
         * Same helper + same LSM_LSP_CONFIDENCE_FLOOR as the sequential
         ...
         * fallbacks are JVM-only (see lsm_pipeline_lsp_allow_tail_match). */
        lsm_resolution_t res = {0};
        const LSMResolvedCall *lsp = NULL;
        bool exact_key_built = true;
        bool exact_key_ambiguous = false;
        bool legacy_key_built = true;
        _rc_t0 = extract_now_ns();
        if (lsm_pipeline_source_site_present(call->site_start_byte, call->site_end_byte)) {
        ...
        if (!lsp && !exact_index_authoritative) {
            ...
            lsp = lsm_pipeline_find_lsp_resolution_in_graph(
        ...
        atomic_fetch_add_explicit(&rc->time_ns_rc_lsp_lookup, extract_now_ns() - _rc_t0,
                                  memory_order_relaxed);
        _rc_t0 = extract_now_ns();
        const lsm_gbuf_node_t *lsp_target = NULL;
        if (lsp) {
            ...
            bool exact_external_target = call->requires_lsp_resolution &&
                                         lsm_pipeline_kotlin_external_target(lang, lsp->callee_qn);
            lsp_target = exact_external_target
                             ? lsm_pipeline_lsp_target_node_strict(rc->main_gbuf, rc->project_name,
                                                                   lsp->callee_qn, allow_tail)
                             : lsm_pipeline_lsp_target_node(rc->main_gbuf, rc->project_name,
                                                            lsp->callee_qn, allow_tail);
            if (lsp_target) {
        ...
        if ((!res.qualified_name || !res.qualified_name[0]) && !call->requires_lsp_resolution) {
            res = lsm_registry_resolve(rc->registry, call->callee_name, module_qn, imp_keys,
                                       imp_vals, imp_count);
        }
        ...
        if (lsm_perl_suppress_generic_match(lang == LSM_LANG_PERL, call->is_method,
        ...
        bool is_tsjs =
            lang == LSM_LANG_JAVASCRIPT || lang == LSM_LANG_TYPESCRIPT || lang == LSM_LANG_TSX;
        bool tsjs_drop_plain_call =
            lsm_tsjs_suppress_weak_method_match(is_tsjs, call->is_method, res.strategy);
        ...
        lsm_svc_kind_t csvc = lsm_service_pattern_match(call->callee_name);
        if (csvc == LSM_SVC_HTTP || csvc == LSM_SVC_ASYNC) {
            const char *cu = call->first_string_arg;
            bool chas_url = cu && cu[0] != '\0' &&
                            (cu[0] == '/' || strstr(cu, "://") != NULL ||
                             (csvc == LSM_SVC_ASYNC && strlen(cu) > PP_ESC_SPACE));
            if (chas_url) {
                lsm_resolution_t svc_res = {.qualified_name = call->callee_name,
        ...
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            if (lsm_service_pattern_route_method(call->callee_name) != NULL) {
                lsm_resolution_t fake_res = {.qualified_name = call->callee_name,
                                             .confidence = PP_HALF_CONF,
                                             .strategy = "callee_suffix"};
                emit_service_edge(ws->local_edge_buf, source_node, source_node, call, &fake_res,
                                  module_qn, rc->registry, rc->main_gbuf, imp_keys, imp_vals,
                                  imp_count, false);
            } else if (lsm_service_pattern_is_global_fetch(call->callee_name)) {
                 ...
                 * from res->qualified_name via lsm_service_pattern_match, which
        ...
        if (!target_node || source_node->id == target_node->id) {
            ...
            lsm_svc_kind_t psvc = lsm_service_pattern_match(res.qualified_name);
            if ((psvc == LSM_SVC_HTTP || psvc == LSM_SVC_ASYNC) && !target_node) {
                const char *u = call->first_string_arg;
                bool url_or_topic = u && u[0] != '\0' &&
                                    (u[0] == '/' || strstr(u, "://") != NULL ||
                                     (psvc == LSM_SVC_ASYNC && strlen(u) > PP_ESC_SPACE));
                if (url_or_topic) {
    ...
    if (lsp_exact_idx) {
        lsm_ht_foreach(lsp_exact_idx, lsp_idx_free_key, NULL);
        lsm_ht_free(lsp_exact_idx);
    }
    if (lsp_legacy_idx) {
        lsm_ht_foreach(lsp_legacy_idx, lsp_idx_free_key, NULL);
        lsm_ht_free(lsp_legacy_idx);
    }
}

/* Resolve usages for one file. */
static void resolve_file_usages(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                LSMFileResult *result, const char *rel, const char *module_qn,
                                const char **imp_keys, const char **imp_vals, int imp_count,
                                LSMLanguage lang) {
    ...
    for (int u = 0; u < result->usages.count; u++) {
        ...
        if (lsm_pipeline_usage_semantic_reference_candidate(usage)) {
            ...
            if (semantic_reference) {
                tgt = lsm_pipeline_lsp_target_node(rc->main_gbuf, rc->project_name,
        ...
        if (!tgt) {
            ...
            lsm_resolution_t res = lsm_registry_resolve(rc->registry, usage->ref_name, module_qn,
            ...
            tgt = lsm_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        ...
        char esc_ref[LSM_SZ_256]; /* sliced source text: escape quotes/newlines */
        ...
        lsm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, edge_type, uprops);
        ws->usages_resolved++;
    }
...
static void resolve_file_throws(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int t = 0; t < result->throws.count; t++) {
        ...
        lsm_resolution_t res = lsm_registry_resolve(rc->registry, thr->exception_name, module_qn,
                                                    imp_keys, imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        const lsm_gbuf_node_t *tgt = lsm_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        if (!tgt || src->id == tgt->id) {
            continue;
        }
        lsm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, edge_type, "{}");
    }
}

/* Resolve reads/writes for one file. */
static void resolve_file_rw(resolve_ctx_t *rc, resolve_worker_state_t *ws, LSMFileResult *result,
                            const char *rel, const char *module_qn, const char **imp_keys,
                            const char **imp_vals, int imp_count) {
    for (int r = 0; r < result->rw.count; r++) {
        ...
        lsm_resolution_t res = lsm_registry_resolve(rc->registry, rw->var_name, module_qn, imp_keys,
                                                    imp_vals, imp_count);
        if (!res.qualified_name || res.qualified_name[0] == '\0') {
            continue;
        }
        ...
        lsm_gbuf_insert_edge(ws->local_edge_buf, src->id, tgt->id, etype, "{}");
    }
}
...
static void resolve_def_inherits(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                 const LSMDefinition *def, const lsm_gbuf_node_t *node,
    ...
    for (int b = 0; def->base_classes[b]; b++) {
        ...
        const lsm_gbuf_node_t *bn = lsm_gbuf_find_by_qn(rc->main_gbuf, bqn);
        if (bn && node->id != bn->id) {
            ...
            lsm_gbuf_insert_edge(ws->local_edge_buf, node->id, bn->id,
                                 lsm_semantic_base_edge_type(bn), "{}");
...
static void resolve_def_decorators(resolve_ctx_t *rc, resolve_worker_state_t *ws,
    ...
    for (int dc = 0; def->decorators[dc]; dc++) {
        ...
        lsm_resolution_t res = lsm_registry_resolve(rc->registry, fn, mq, ik, iv, ic);
        if ((!res.qualified_name || res.qualified_name[0] == '\0') && !strchr(fn, '.')) {
            ...
            char with_suffix[LSM_SZ_256];
            int wn = snprintf(with_suffix, sizeof(with_suffix), "%sAttribute", fn);
            if (wn > 0 && (size_t)wn < sizeof(with_suffix)) {
                res = lsm_registry_resolve(rc->registry, with_suffix, mq, ik, iv, ic);
            }
        }
        const lsm_gbuf_node_t *dn = NULL;
        if (res.qualified_name && res.qualified_name[0] != '\0') {
            dn = lsm_gbuf_find_by_qn(rc->main_gbuf, res.qualified_name);
        }
        ...
        if (dn) {
            ...
            char syn_qn[LSM_SZ_512];
            snprintf(syn_qn, sizeof(syn_qn), "<decorator:%s>", fn);
            dn_id =
                lsm_gbuf_upsert_node(ws->local_edge_buf, "Decorator", fn, syn_qn, "", 0, 0, "{}");
        }
        if (dn_id != 0 && node->id != dn_id) {
            ...
            char esc_dec[LSM_SZ_256];
            lsm_json_escape(esc_dec, sizeof(esc_dec), def->decorators[dc]);
            char dp[LSM_SZ_512];
            snprintf(dp, sizeof(dp), "{\"decorator\":\"%s\"}", esc_dec);
            lsm_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "DECORATES", dp);
            ...
            lsm_gbuf_insert_edge(ws->local_edge_buf, node->id, dn_id, "USAGE", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static void resolve_file_semantic(resolve_ctx_t *rc, resolve_worker_state_t *ws,
                                  LSMFileResult *result, const char *module_qn,
                                  const char **imp_keys, const char **imp_vals, int imp_count) {
    for (int d = 0; d < result->defs.count; d++) {
        LSMDefinition *def = &result->defs.items[d];
        ...
        const lsm_gbuf_node_t *node = lsm_gbuf_find_by_qn(rc->main_gbuf, def->qualified_name);
        if (!node) {
            continue;
        }
    ...
    for (int t = 0; t < result->impl_traits.count; t++) {
        LSMImplTrait *it = &result->impl_traits.items[t];
        ...
        const lsm_gbuf_node_t *tn = lsm_gbuf_find_by_qn(rc->main_gbuf, tqn);
        const lsm_gbuf_node_t *sn = lsm_gbuf_find_by_qn(rc->main_gbuf, sqn);
        if (tn && sn && tn->id != sn->id) {
            lsm_gbuf_insert_edge(ws->local_edge_buf, sn->id, tn->id, "IMPLEMENTS", "{}");
            ws->semantic_resolved++;
        }
    }
}
...
static LSMTypeRegistry *pp_rust_shared_registry(resolve_ctx_t *rc) {
    LSMTypeRegistry *p = atomic_load_explicit(&rc->rust_shared_reg, memory_order_acquire);
    if (p)
        return p;
    if (!rc->all_defs || rc->def_count <= 0)
        return NULL;
    lsm_mutex_lock(&rc->rust_shared_mu);
    p = atomic_load_explicit(&rc->rust_shared_reg, memory_order_relaxed);
    if (!p) {
        lsm_arena_init(&rc->rust_shared_arena);
        rc->rust_shared_arena_live = true;
        p = lsm_rust_build_cross_registry(&rc->rust_shared_arena, rc->all_defs, rc->def_count);
        if (p) {
            char sb[96];
            snprintf(sb, sizeof(sb), "types=%d funcs=%d", p->type_count, p->func_count);
            lsm_log_info("cross_lsp.rust_registry", "scale", sb);
        }
        atomic_store_explicit(&rc->rust_shared_reg, p, memory_order_release);
    }
    lsm_mutex_unlock(&rc->rust_shared_mu);
    return p;
}
...
static LSMTypeRegistry *pp_rust_shared_registry_get(void *ctx) {
    return pp_rust_shared_registry((resolve_ctx_t *)ctx);
}

static int pp_call_reference_site_count(const LSMFileResult *result, LSMLanguage lang) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        if (lsm_pipeline_usage_semantic_reference_candidate(&result->usages.items[i])) {
            count++;
        }
    }
    return count;
...
static int pp_qualified_lsp_site_count(const LSMFileResult *result) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
 ...
 * syntax extractor cannot represent as a LSMCall yet: Python operators and
...
static bool pp_has_pending_lsp_site(const LSMFileResult *result) {
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const LSMResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != LSM_RESOLVED_INVOCATION &&
            resolved->kind != LSM_RESOLVED_CALL_REFERENCE) {
        ...
        if (resolved->reason || resolved->confidence < LSM_LSP_CONFIDENCE_FLOOR) {
            return true;
        }
    }
    return false;
...
static void resolve_worker(int worker_id, void *ctx_ptr) {
    ...
    if (!ws->local_edge_buf) {
        ws->local_edge_buf =
            lsm_gbuf_new_shared_ids(rc->project_name, rc->repo_path, rc->shared_ids);
    }
    ...
    while (SKIP_ONE) {
        ...
        lsm_scale_tick(&rc->scale, file_idx);
        ...
        LSMFileResult *result = rc->result_cache[file_idx];
        if (!result) {
        ...
        LSMLanguage lang = rc->files[file_idx].language;
        ...
        bool jvm_cross_lsp = (lang == LSM_LANG_JAVA || lang == LSM_LANG_KOTLIN);
        ...
        bool cross_lsp_eligible =
            (rc->all_defs && rc->def_count > 0 && lsm_pxc_has_cross_lsp(lang) &&
        ...
        lsm_pxc_build_import_map(rc->main_gbuf, rc->project_name, rel, lang, result, &imp_keys,
                                 &imp_vals, &imp_count);
         ...
         * through lsm_registry_resolve. Same callee_name appears in
        ...
        lsm_registry_reach_cache_begin(result->calls.count + result->usages.count + 64);
        ...
        lsm_registry_import_map_cache_begin(imp_keys, imp_vals, imp_count);

        /* THE BIG ONE: per-file cache of lsm_registry_resolve results.
        ...
        lsm_registry_resolve_cache_begin(result->calls.count + result->usages.count + 64);

        char *module_qn =
            lsm_pipeline_fqn_module_dir(rc->project_name, rel, pp_module_is_dir(lang));

        /* ── Cross-file LSP (FUSED) ─────────────────────────────
         * Runs BEFORE resolve_file_calls so its additions to
         * result->resolved_calls are picked up by
         * lsm_pipeline_find_lsp_resolution when calls become CALLS
        ...
        if (cross_lsp_eligible) {
            ...
            if ((!lsp_source || lsp_source_len <= 0) && rc->files[file_idx].path) {
                 ...
                 * by read_file's lsm_max_file_bytes cap), freed below. */
            ...
            if (lsp_source && lsp_source_len > 0) {
                ...
                lsm_index_mark_start(rel);
                lsm_pxc_dispatch_file(lang, result, lsp_source, lsp_source_len, rel, def_module,
                 ...
                 * lsm_extract_file on this thread will recreate the parser. */
                lsm_destroy_thread_parser();
                lsm_slab_reclaim();
                ...
                if (lsp_elapsed_ms > PP_TIMER_THRESH) {
                    lsm_log_info("parallel.resolve.lsp_cross.slow", "elapsed_ms",
                 ...
                 * unaffected. Deliberately NOT recorded as a lsm_file_error —
        ...
        lsm_registry_reach_cache_end();
        lsm_registry_import_map_cache_end();
        lsm_registry_resolve_cache_end();
    ...
    lsm_destroy_thread_parser();
    lsm_slab_destroy_thread();
    lsm_service_pattern_cache_end();
}

int lsm_parallel_resolve(lsm_pipeline_ctx_t *ctx, const lsm_file_info_t *files, int file_count,
                         LSMFileResult **result_cache, _Atomic int64_t *shared_ids,
                         int worker_count, LSMLSPDef *all_defs, int def_count,
                         char *const *def_modules, struct LSMModuleDefIndex *module_def_index,
                         void *cross_registries_v) {
    /* See header: typed as void* across the TU boundary; cast back here. */
    LSMCrossLspRegistries *cross_registries = (LSMCrossLspRegistries *)cross_registries_v;
    if (file_count == 0) {
        return 0;
    }

    lsm_log_info("parallel.resolve.start", "files", itoa_log(file_count), "workers",
                 itoa_log(worker_count));
    ...
    lsm_mutex_init(&rc.rust_shared_mu);
    rc.rust_shared_arena_live = false;

    /* Sub-phase: Dispatch resolve workers (per-file call/usage resolution, PARALLEL) */
    LSM_PROF_START(t_resolve_dispatch);
    lsm_parallel_for_opts_t opts = {.max_workers = worker_count, .force_pthreads = false};
    lsm_scale_begin(&rc.scale, "parallel_resolve", (long)file_count);
    lsm_parallel_for(worker_count, resolve_worker, &rc, opts);
    lsm_scale_end(&rc.scale);
    LSM_PROF_END_N("parallel_resolve", "1_dispatch_workers_parallel", t_resolve_dispatch,
                   file_count);
    ...
    if (rc.rust_shared_arena_live) {
        lsm_arena_destroy(&rc.rust_shared_arena);
        rc.rust_shared_arena_live = false;
    }
    lsm_mutex_destroy(&rc.rust_shared_mu);

    /* Sub-phase: Merge all local edge bufs into main gbuf (SEQUENTIAL) */
    LSM_PROF_START(t_resolve_merge);
    int total_calls = 0;
    int total_usages = 0;
    ...
    for (int i = 0; i < worker_count; i++) {
        if (workers[i].local_edge_buf) {
            lsm_gbuf_merge(ctx->gbuf, workers[i].local_edge_buf);
            ...
            lsm_gbuf_free(workers[i].local_edge_buf);
        }
    }
    LSM_PROF_END_N("parallel_resolve", "2_merge_edge_bufs_seq", t_resolve_merge,
                   total_calls + total_usages);

    lsm_aligned_free(workers);

    /* Go-style implicit interface satisfaction (needs full graph, serial) */
    int go_impl = lsm_pipeline_implements_go(ctx);
    ...
    total_lsp_overrides += lsm_pipeline_override_explicit(ctx);
    ...
    if (cross_files > 0) {
        char cf_buf[LSM_SZ_32];
        char nf_buf[LSM_SZ_32];
        char cu_buf[LSM_SZ_32];
        char pk_buf[LSM_SZ_32];
        ...
        snprintf(pk_buf, sizeof(pk_buf), "%llu",
                 (unsigned long long)(def_count > 0
                                          ...
                                          : 0ULL));
        lsm_log_info("parallel.resolve.cross_lsp_cost", "cross_lsp_ms", cu_buf, "files", nf_buf,
        ...
        lsm_pxc_filter_stats(&reg_defs, &reg_files, &flt_files, &flt_failed);
        if (reg_files > 0) {
            char rd_buf[LSM_SZ_32];
            char ff_buf[LSM_SZ_32];
            char fp_buf[LSM_SZ_32];
            ...
            lsm_log_info("parallel.resolve.perfile_registry", "defs_per_file", rd_buf, "defs_total",
                         itoa_log(def_count), "filtered_files", fp_buf, "filter_failed", ff_buf);
        }
    }

    lsm_log_info("parallel.resolve.done", "calls", itoa_log(total_calls), "usages",
    ...
    lsm_log_info("parallel.resolve.phase_summary", "total_loop_cpu_ms", loop_buf, "files_visited",
                 visits_buf);
    ...
    snprintf(cal_buf, sizeof(cal_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_calls, memory_order_relaxed) /
                                  1000000ULL));
    lsm_log_info("parallel.resolve.phase_ms_a", "import_map", imp_buf, "cross_lsp", xls_buf,
    ...
    snprintf(sem_buf, sizeof(sem_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_semantic, memory_order_relaxed) /
                                  1000000ULL));
    lsm_log_info("parallel.resolve.phase_ms_b", "resolve_usages", use_buf, "resolve_throws",
    ...
    snprintf(emt_buf, sizeof(emt_buf), "%llu",
             (unsigned long long)(atomic_load_explicit(&rc.time_ns_rc_emit, memory_order_relaxed) /
                                  1000000ULL));
    lsm_log_info("parallel.resolve.calls_breakdown", "find_source", src_buf, "lsp_lookup", lsp_buf,
                 "resolve", rsv_buf);
    lsm_log_info("parallel.resolve.calls_breakdown2", "field_hint", hnt_buf, "find_target", tgt_buf,
                 "emit_edge", emt_buf);
    ...
    char tl_buf[LSM_SZ_32];
    char tc_buf[LSM_SZ_32];
    char tp_buf[LSM_SZ_32];
    char fb_buf[LSM_SZ_32];
    ...
    snprintf(fb_buf, sizeof(fb_buf), "%llu",
             (unsigned long long)atomic_load_explicit(&g_lsp_linear_fallback_rows,
                                                      memory_order_relaxed));
    lsm_log_info("parallel.resolve.scan_cost", "tail_lookups", tl_buf, "tail_candidates", tc_buf,
