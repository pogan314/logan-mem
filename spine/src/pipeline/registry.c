...
const char *lsm_confidence_band(double score) {
...
typedef LSM_DYN_ARRAY(char *) qn_array_t;
 ...
 * contention. */
static LSM_TLS LSMHashTable *_reach_cache = NULL;
...
void lsm_registry_reach_cache_begin(int estimated_capacity) {
    if (_reach_cache) {
        /* Defensive: caller forgot to call _end. Clear and reuse. */
        lsm_ht_foreach(_reach_cache, reach_cache_free_key, NULL);
        lsm_ht_clear(_reach_cache);
        return;
    }
    if (estimated_capacity < 16)
        estimated_capacity = 16;
    _reach_cache = lsm_ht_create((uint32_t)estimated_capacity);
}

void lsm_registry_reach_cache_end(void) {
    if (!_reach_cache)
        return;
    lsm_ht_foreach(_reach_cache, reach_cache_free_key, NULL);
    lsm_ht_free(_reach_cache);
    _reach_cache = NULL;
}
...
static LSM_TLS LSMHashTable *_import_map_cache = NULL;

void lsm_registry_import_map_cache_begin(const char **keys, const char **vals, int count) {
    if (_import_map_cache) {
        lsm_ht_free(_import_map_cache);
        _import_map_cache = NULL;
    }
    if (!keys || !vals || count <= 0)
        return;
    _import_map_cache = lsm_ht_create((uint32_t)count * 2u + 8u);
    if (!_import_map_cache)
        return;
    for (int i = 0; i < count; i++) {
        if (keys[i] && vals[i]) {
            lsm_ht_set(_import_map_cache, keys[i], (void *)(uintptr_t)vals[i]);
        }
    }
}

void lsm_registry_import_map_cache_end(void) {
    if (!_import_map_cache)
        return;
...
/* ── Per-file full-result cache for lsm_registry_resolve ──────────
 *
 * THE big one: 98.7% of resolve_calls CPU on kubernetes lives inside
 * lsm_registry_resolve's strategy chain (suffix_match iterating
 ...
 * to the same QN every time. Cache the full lsm_resolution_t result
...
static LSM_TLS LSMHashTable *_resolve_cache = NULL;
...
void lsm_registry_resolve_cache_begin(int estimated_capacity) {
    if (_resolve_cache) {
        lsm_ht_foreach(_resolve_cache, resolve_cache_free_entry, NULL);
        lsm_ht_free(_resolve_cache);
        _resolve_cache = NULL;
    }
    if (estimated_capacity < 32)
        estimated_capacity = 32;
    _resolve_cache = lsm_ht_create((uint32_t)estimated_capacity);
}

void lsm_registry_resolve_cache_end(void) {
    if (!_resolve_cache)
        return;
    lsm_ht_foreach(_resolve_cache, resolve_cache_free_entry, NULL);
    lsm_ht_free(_resolve_cache);
...
static bool is_import_reachable(const char *candidate_qn, const char **import_vals,
                                int import_count) {
    if (_reach_cache) {
        void *cached = lsm_ht_get(_reach_cache, candidate_qn);
    ...
    if (_reach_cache) {
        ...
        if (kdup) {
            lsm_ht_set(_reach_cache, kdup, reachable ? REACH_CACHE_TRUE : REACH_CACHE_FALSE);
        }
    }
    return reachable;
}
...
static lsm_resolution_t empty_result(void) {
    lsm_resolution_t r = {0};
    return r;
}
...
bool lsm_perl_is_builtin(const char *name) {
    if (!name || !name[0]) {
        return false;
    }
 ...
 * lsm_resolution_t.strategy of a non-empty match;
...
bool lsm_perl_suppress_generic_match(bool is_perl, bool is_method, const char *callee_name,
                                     const char *strategy) {
    if (!is_perl) {
        return false;
    }
    if (!(is_method || lsm_perl_is_builtin(callee_name))) {
        return false;
    }
...
bool lsm_tsjs_suppress_weak_method_match(bool is_tsjs, bool is_method, const char *strategy) {
     ...
     * "fuzzy" is listed as defensive insurance only — lsm_registry_fuzzy_resolve
...
static bool js_ts_family(LSMLanguage lang) {
    return lang == LSM_LANG_JAVASCRIPT || lang == LSM_LANG_TYPESCRIPT || lang == LSM_LANG_TSX;
}
...
bool lsm_suppress_cross_language_suffix_match(LSMLanguage caller_lang, const char *target_file_path,
                                              const char *strategy) {
    ...
    if (caller_lang == LSM_LANG_COUNT || !target_file_path || !target_file_path[0]) {
        return false;
    }
    LSMLanguage target_lang = lsm_language_for_filename(path_basename(target_file_path));
    if (target_lang == LSM_LANG_COUNT) {
        return false;
    }
...
lsm_registry_t *lsm_registry_new(void) {
    lsm_registry_t *r = calloc(LSM_ALLOC_ONE, sizeof(lsm_registry_t));
    if (!r) {
        return NULL;
    }
    r->exact = lsm_ht_create(LSM_SZ_1K);
    r->by_name = lsm_ht_create(LSM_SZ_512);
    return r;
}
...
void lsm_registry_free(lsm_registry_t *r) {
    if (!r) {
        return;
    }
    /* by_name first: its items borrow exact's keys. */
    lsm_ht_foreach(r->by_name, free_qn_array, NULL);
    lsm_ht_free(r->by_name);
    lsm_ht_foreach(r->exact, free_label, NULL);
    lsm_ht_free(r->exact);
...
void lsm_registry_add(lsm_registry_t *r, const char *name, const char *qualified_name,
                      const char *label) {
    (void)name;
    ...
    /* Check for duplicate */
    if (lsm_ht_get(r->exact, qualified_name)) {
        return;
    }
    ...
    lsm_ht_set(r->exact, strdup(qualified_name), (void *)interned);
    const char *owned_qn = lsm_ht_get_key(r->exact, qualified_name);

    /* Index by simple name.
     * No array dedup needed: exact-map check above guarantees uniqueness. */
    const char *simple = simple_name(qualified_name);
    qn_array_t *arr = lsm_ht_get(r->by_name, simple);
    if (!arr) {
        arr = calloc(LSM_ALLOC_ONE, sizeof(qn_array_t));
        lsm_ht_set(r->by_name, strdup(simple), arr);
    }
    lsm_da_push(arr, (char *)owned_qn);
}

/* ── Lookup ──────────────────────────────────────────────────────── */

bool lsm_registry_exists(const lsm_registry_t *r, const char *qn) {
    if (!r || !qn) {
        return false;
    }
    return lsm_ht_get(r->exact, qn) != NULL;
}

const char *lsm_registry_label_of(const lsm_registry_t *r, const char *qn) {
    if (!r || !qn) {
        return NULL;
    }
    return lsm_ht_get(r->exact, qn);
}

int lsm_registry_find_by_name(const lsm_registry_t *r, const char *name, const char ***out,
                              int *count) {
    if (!r || !out || !count) {
        return LSM_NOT_FOUND;
    }
    qn_array_t *arr = lsm_ht_get(r->by_name, name);
    if (arr && arr->count > 0) {
...
int lsm_registry_size(const lsm_registry_t *r) {
    return r ? (int)lsm_ht_count(r->exact) : 0;
}
...
static lsm_resolution_t resolve_import_map(const lsm_registry_t *r, const char *prefix,
    ...
    if (_import_map_cache) {
        resolved = (const char *)lsm_ht_get(_import_map_cache, prefix);
    } else {
    ...
    if (!suffix || !suffix[0]) {
        const char *direct = lsm_ht_get_key(r->exact, resolved);
        if (direct) {
            return (lsm_resolution_t){direct, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
        }
    }
    char candidate[LSM_SZ_512];
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, suffix);
    } else {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, prefix);
    }
    /* Use lsm_ht_get_key to get the persistent heap-owned key string */
    const char *stored_key = lsm_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (lsm_resolution_t){stored_key, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
    }
     ...
     * candidates instead of lsm_ht_foreach over the WHOLE exact table — that
    ...
    if (suffix && suffix[0]) {
        char resolved_dot[LSM_SZ_512];
        char dot_suffix[LSM_SZ_256];
        snprintf(resolved_dot, sizeof(resolved_dot), "%s.", resolved);
        snprintf(dot_suffix, sizeof(dot_suffix), ".%s", suffix);
        qn_array_t *arr = lsm_ht_get(r->by_name, simple_name(suffix));
        if (arr) {
            ...
            for (int i = 0; i < arr->count; i++) {
                ...
                if (klen >= rd_len + ds_len && strncmp(qn, resolved_dot, rd_len) == 0 &&
                    ...
                    return (lsm_resolution_t){qn, "import_map_suffix", CONF_IMPORT_MAP_SUFFIX,
                                              REG_RESOLVED};
                }
            }
        }
...
static lsm_resolution_t resolve_same_module(const lsm_registry_t *r, const char *callee_name,
                                            const char *suffix, const char *module_qn) {
    char candidate[LSM_SZ_512];
    snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, callee_name);
    const char *stored_key = lsm_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (lsm_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
    }
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, suffix);
        stored_key = lsm_ht_get_key(r->exact, candidate);
        if (stored_key) {
            return (lsm_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
        }
    }
...
static lsm_resolution_t resolve_multi_with_imports(const qn_array_t *arr, const char *module_qn,
                                                   const char **import_vals, int import_count) {
    const char *filtered[LSM_SZ_256];
    int fcount = 0;
    for (int i = 0; i < arr->count && fcount < LSM_SZ_256; i++) {
    ...
    if (fcount == SKIP_ONE) {
        ...
        return (lsm_resolution_t){filtered[0], "suffix_match", conf, arr->count};
    }
    if (fcount > SKIP_ONE) {
        ...
        if (best) {
            ...
            return (lsm_resolution_t){best, "suffix_match", conf, fcount};
        }
    }
    ...
    if (best) {
        ...
        return (lsm_resolution_t){best, "suffix_match", conf, arr->count};
    }
    return empty_result();
}
...
static lsm_resolution_t resolve_name_lookup(const lsm_registry_t *r, const char *callee_name,
                                            const char *module_qn, const char **import_vals,
                                            int import_count) {
    const char *lookup = simple_name(callee_name);
    qn_array_t *arr = lsm_ht_get(r->by_name, lookup);
    if (!arr || arr->count == 0) {
        return empty_result();
    }
    ...
    if (arr->count > 1) {
        ...
        if (q) {
            return (lsm_resolution_t){q, "qualified_suffix", CONF_QUALIFIED_SUFFIX, REG_RESOLVED};
        }
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (lsm_resolution_t){arr->items[0], "unique_name", conf, REG_RESOLVED};
    }
...
lsm_resolution_t lsm_registry_resolve(const lsm_registry_t *r, const char *callee_name,
    ...
    if (_resolve_cache) {
        resolve_cache_entry_t *cached =
            (resolve_cache_entry_t *)lsm_ht_get(_resolve_cache, callee_name);
        if (cached) {
            return cached->res;
        }
    }
    ...
    if (_resolve_cache) {
        ...
        if (e) {
            ...
            if (kdup) {
                lsm_ht_set(_resolve_cache, kdup, e);
            } else {
                free(e);
            }
        }
...
lsm_fuzzy_result_t lsm_registry_fuzzy_resolve(const lsm_registry_t *r, const char *callee_name,
                                              const char *module_qn, const char **import_map_keys,
                                              const char **import_map_vals, int import_map_count) {
    (void)import_map_keys;
    lsm_fuzzy_result_t no_match = {{0}, false};
    if (!r || !callee_name) {
        return no_match;
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (lsm_fuzzy_result_t){{arr->items[0], "fuzzy", conf, REG_RESOLVED}, true};
    }
    ...
    if (have_imports) {
        fcount = filter_import_reachable((const char **)arr->items, arr->count, import_map_vals,
                                         import_map_count, filtered, LSM_SZ_256);
        fptr = filtered;
    }
...
int lsm_registry_find_ending_with(const lsm_registry_t *r, const char *suffix, const char ***out) {
    if (!r || !suffix || !out) {
        if (out) {
            *out = NULL;
        }
    ...
    lsm_ht_foreach(r->exact, few_scan, &ctx);
...
bool lsm_registry_is_import_reachable(const char *candidate_qn, const char **import_vals,
                                      int import_count) {
