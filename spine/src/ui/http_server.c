 ...
 *   POST /rpc         → JSON-RPC dispatch via own lsm_mcp_server_t
 ...
 * Has its own lsm_mcp_server_t with a separate SQLite connection (WAL reader).
 */
...
#include "foundation/subprocess.h" /* lsm_build_win_cmdline — shared MS-CRT arg quoting */
#include "foundation/win_utf8.h"   /* lsm_utf8_to_wide — CreateProcessW wide cmdline (#423/#20) */
...
#define MAX_BODY_SIZE LSM_HTTP_MAX_BODY
...
static void update_cors(const lsm_http_req_t *req, int port) {
...
static void handle_ui_config(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    const char *lang = NULL;
    char cache_dir[1024];
    snprintf(cache_dir, sizeof(cache_dir), "%s", lsm_resolve_cache_dir());
    lsm_config_t *cfg = lsm_config_open(cache_dir);
    if (cfg) {
        const char *pinned = lsm_config_get(cfg, LSM_CONFIG_UI_LANG, "auto");
    ...
    lsm_http_replyf(c, 200, g_cors_json, "{\"lang\":\"%s\",\"upstream_issues_url\":\"%s\"}",
                    lang_buf, "https://github.com/DeusData/logan-spine-mcp/issues/new");
}
...
struct lsm_http_server {
    lsm_httpd_t *listener;
    lsm_mcp_server_t *mcp;       /* own MCP server instance (read-only) */
    struct lsm_watcher *watcher; /* external watcher ref (not owned) */
    lsm_http_index_executor_fn index_executor;
    void *index_executor_context;
    lsm_http_project_mutation_begin_fn mutation_begin;
    lsm_http_project_mutation_end_fn mutation_end;
    ...
    int port;
    bool listener_ok;
    uint8_t readiness_secret[LSM_SHA256_DIGEST_LEN];
...
static bool serve_embedded(lsm_http_conn_t *c, const char *path) {
    const lsm_embedded_file_t *f = lsm_embedded_lookup(path);
    if (!f)
        return false;
    ...
    lsm_http_reply_buf(c, 200, hdrs, f->data, (size_t)f->size);
    return true;
}
 ...
 * libgit2 is initialized once at process start by lsm_alloc_init() (which also
...
static void handle_repo_info(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char project[256] = {0};
    if (!lsm_http_query_param(req->query, "project", project, (int)sizeof(project)) ||
        project[0] == '\0') {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project parameter\"}");
        return;
    }

    char db_path[1024];
    db_path_for_project(project, db_path, sizeof(db_path));
    if (db_path[0] == '\0' || !lsm_file_exists(db_path)) {
        lsm_http_replyf(c, 404, g_cors_json, "{\"error\":\"project not found\"}");
        return;
    }
    lsm_store_t *store = lsm_store_open_path_query(db_path);
    if (!store) {
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"cannot open store\"}");
        return;
    }
    ...
    if (lsm_store_get_project(store, project, &proj) == LSM_STORE_OK && proj.root_path) {
    ...
    char branch[256] = {0};
    if (root_path[0]) {
        lsm_git_context_t gctx;
        memset(&gctx, 0, sizeof(gctx));
        if (lsm_git_context_resolve(root_path, &gctx) == 0 && gctx.branch) {
    ...
    char *remote_safe = lsm_ui_git_strip_credentials(remote); /* never echo secrets */
    char *web_base = lsm_ui_git_web_base(remote);
    ...
    lsm_json_escape(esc_root, (int)sizeof(esc_root), root_path);
    lsm_json_escape(esc_branch, (int)sizeof(esc_branch), branch);
    lsm_json_escape(esc_remote, (int)sizeof(esc_remote), remote_safe ? remote_safe : "");
    lsm_json_escape(esc_web, (int)sizeof(esc_web), web_base ? web_base : "");
    lsm_json_escape(esc_blob, (int)sizeof(esc_blob), blob_base);

    lsm_http_replyf(c, 200, g_cors_json,
...
static lsm_mutex_t g_log_mutex;

enum { LSM_LOG_MUTEX_UNINIT = 0, LSM_LOG_MUTEX_INITING = 1, LSM_LOG_MUTEX_INITED = 2 };
static atomic_int g_log_mutex_init = LSM_LOG_MUTEX_UNINIT;

/* Safe for concurrent callers: only publishes INITED after lsm_mutex_init()
...
void lsm_ui_log_init(void) {
    ...
    if (state == LSM_LOG_MUTEX_INITED)
        return;

    state = LSM_LOG_MUTEX_UNINIT;
    if (atomic_compare_exchange_strong(&g_log_mutex_init, &state, LSM_LOG_MUTEX_INITING)) {
        lsm_mutex_init(&g_log_mutex);
        atomic_store(&g_log_mutex_init, LSM_LOG_MUTEX_INITED);
        return;
    }

    /* Another thread is initializing — spin until done */
    while (atomic_load(&g_log_mutex_init) != LSM_LOG_MUTEX_INITED) {
        lsm_usleep(1000); /* 1ms */
    }
}
...
static void handle_logs(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char lines_str[16] = {0};
    int max_lines = 100;
    if (lsm_http_query_param(req->query, "lines", lines_str, (int)sizeof(lines_str))) {
    ...
    if (!buf) {
        lsm_mutex_unlock(&g_log_mutex);
        lsm_http_replyf(c, 500, g_cors, "oom");
        return;
    }
    ...
    lsm_http_replyf(c, 200, g_cors_json, "%s", buf);
    free(buf);
}
...
/* GET /api/processes — list logan-spine-mcp processes via ps */
static void handle_processes(lsm_http_conn_t *c) {
    char buf[8192];
    int pos = 0;
...
static void append_roots_json(char *buf, size_t bufsz, int *pos) {
    ...
    for (int i = 0; i < 26; i++) {
         ...
         * offline volume) fails the same lsm_is_dir gate browse applies, and
...
static void handle_browse(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char path[1024] = {0};
    const char *home = lsm_get_home_dir();
    if (!lsm_http_query_param(req->query, "path", path, (int)sizeof(path)) || path[0] == '\0') {
     ...
     * "D:\projects\demo"). Normalize to forward slashes before the lsm_is_dir
     * gate, exactly as the MCP repo_path handler and lsm_project_name_from_path
     * already do — otherwise a real D:/ directory is rejected (#548). */
    lsm_normalize_path_sep(path);

    if (!lsm_is_dir(path)) {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"not a directory\"}");
        return;
    }

    DIR *dir = opendir(path);
    if (!dir) {
        lsm_http_replyf(c, 403, g_cors_json, "{\"error\":\"cannot open directory\"}");
        return;
    }
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        {
            char esc[512];
            lsm_json_escape(esc, (int)sizeof(esc), ent->d_name);
...

    {
        ...
        lsm_json_escape(esc_parent, (int)sizeof(esc_parent), parent);
...
static void handle_adr_get(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char name[256] = {0};
    if (!lsm_http_query_param(req->query, "project", name, (int)sizeof(name)) || name[0] == '\0') {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project\"}");
        return;
    }
    ...
    lsm_store_t *store = lsm_store_open_path_query(db_path);
    if (!store) {
        lsm_http_replyf(c, 200, g_cors_json, "{\"has_adr\":false}");
        return;
    }

    lsm_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    if (lsm_store_adr_get(store, name, &adr) == LSM_STORE_OK && adr.content) {
...
static void handle_adr_save(lsm_http_server_t *srv, lsm_http_conn_t *c, const lsm_http_req_t *req) {
    if (req->body_len == 0 || req->body_len > 16384) {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"invalid body\"}");
        return;
    }

    yyjson_doc *doc = yyjson_read(req->body, req->body_len, 0);
    if (!doc) {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"invalid json\"}");
        return;
    }
    ...
    if (!v_proj || !yyjson_is_str(v_proj) || !v_content || !yyjson_is_str(v_content)) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project or content\"}");
        return;
    }
    ...
    if (srv->mutation_begin && !srv->mutation_begin(srv->mutation_context, proj)) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 423, g_cors_json,
    ...
    int rc = lsm_store_adr_store(store, proj, content);
    lsm_store_close(store);
    if (mutation_held) {
        srv->mutation_end(srv->mutation_context, proj);
    }
    ...
    if (rc == LSM_STORE_OK) {
        lsm_http_replyf(c, 200, g_cors_json, "{\"saved\":true}");
    } else {
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"save failed\"}");
    }
}
...
#else
static bool resolve_self_executable(char *out, size_t outsz) {
    char *utf8 = lsm_module_path_utf8();
    if (!utf8) {
        return false;
    }
...
bool lsm_http_server_resolve_binary_path(const char *argv0, char *out, size_t outsz) {
    ...
    if (argv0 && argv0[0]) {
        ...
        wchar_t *wargv0 = lsm_utf8_to_wide(argv0);
...
void lsm_http_server_set_binary_path(const char *path) {
    if (path) {
        if (!lsm_http_server_resolve_binary_path(path, g_binary_path, sizeof(g_binary_path))) {
            g_binary_path[0] = '\0';
        }
    }
}
...
static void *index_thread_fn(void *arg) {
    index_job_t *job = arg;
    lsm_log_info("ui.index.start", "path", job->root_path);
    lsm_http_server_t *server = job->server;
    ...
    lsm_log_info("ui.index.done", "path", job->root_path, "rc", result == 0 ? "ok" : "err");
    atomic_store_explicit(&job->completed, 1, memory_order_release);
    return NULL;
}

/* POST /api/index — body: {"root_path": "/abs/path", "project_name": "..."} */
static void handle_index_start(lsm_http_server_t *server, lsm_http_conn_t *c,
                               const lsm_http_req_t *req) {
    if (!server || !server->index_executor) {
        lsm_http_replyf(c, 503, g_cors_json,
    ...
    if (!v_path || !yyjson_is_str(v_path)) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing root_path\"}");
        return;
    }
    ...
    if (!lsm_is_dir(rpath)) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"directory not found\"}");
        return;
    }
    ...
    if (!lsm_canonical_path(rpath, canonical_root, sizeof(canonical_root))) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"cannot resolve root_path\"}");
        return;
    }
    if (!lsm_workspace_root_allowed(canonical_root, lsm_workspace_home_dir(),
                                    lsm_workspace_cache_dir(), getenv("LSM_ALLOWED_ROOT"),
                                    boundary_err, sizeof(boundary_err))) {
        yyjson_doc_free(doc);
        char escaped[1024];
        lsm_json_escape(escaped, (int)sizeof(escaped), boundary_err);
        lsm_http_replyf(c, 403, g_cors_json, "{\"error\":\"%s\"}", escaped);
        return;
    }
    ...
    if (slot < 0) {
        yyjson_doc_free(doc);
        lsm_http_replyf(c, 429, g_cors_json, "{\"error\":\"all index slots busy\"}");
        return;
    }

    index_job_t *job = &server->index_jobs[slot];
    if (job->thread_started) {
        if (lsm_thread_join(&job->thread) != 0) {
            lsm_http_replyf(c, 503, g_cors_json, "{\"error\":\"index worker unavailable\"}");
            return;
        }
    ...
    if (lsm_thread_create(&job->thread, 0, index_thread_fn, job) != 0) {
        atomic_store(&job->status, 3);
        atomic_store_explicit(&job->completed, 1, memory_order_release);
        snprintf(job->error_msg, sizeof(job->error_msg), "thread creation failed");
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"thread creation failed\"}");
        return;
    }
    job->thread_started = true;

    lsm_http_replyf(c, 202, g_cors_json, "{\"status\":\"indexing\",\"slot\":%d,\"path\":\"%s\"}",
                    slot, job->root_path);
}

/* GET /api/index-status — returns status of all index jobs */
static void handle_index_status(lsm_http_server_t *server, lsm_http_conn_t *c) {
    char buf[2048] = "[";
    int pos = 1;
    for (int i = 0; i < MAX_INDEX_JOBS; i++) {
        int st = atomic_load(&server->index_jobs[i].status);
        ...
        char esc_path[2048];
        char esc_error[512];
        lsm_json_escape(esc_path, (int)sizeof(esc_path), server->index_jobs[i].root_path);
        lsm_json_escape(esc_error, (int)sizeof(esc_error),
...
static void unwatch_project(lsm_http_server_t *srv, const char *name) {
    if (srv && srv->watcher) {
...
static void handle_delete_project(lsm_http_server_t *srv, lsm_http_conn_t *c,
                                  const lsm_http_req_t *req) {
    char name[256] = {0};
    if (!lsm_http_query_param(req->query, "name", name, (int)sizeof(name)) || name[0] == '\0') {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing name\"}");
        return;
    }
    ...
    if (unlink(db_path) != 0) {
        ...
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"failed to delete\"}");
        return;
    }
    ...
    unwatch_project(srv, name);
    lsm_log_info("ui.project.deleted", "name", name);
    if (mutation_held) {
        srv->mutation_end(srv->mutation_context, name);
    }
    lsm_http_replyf(c, 200, g_cors_json, "{\"deleted\":true}");
}

/* GET /api/project-health?name=X — checks db integrity */
static void handle_project_health(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char name[256] = {0};
    if (!lsm_http_query_param(req->query, "name", name, (int)sizeof(name)) || name[0] == '\0') {
        lsm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing name\"}");
        return;
    ...
    if (!lsm_file_exists(db_path)) {
        lsm_http_replyf(c, 200, g_cors_json, "{\"status\":\"missing\"}");
        return;
    }

    lsm_store_t *store = lsm_store_open_path_query(db_path);
    if (!store) {
        lsm_http_replyf(c, 200, g_cors_json, "{\"status\":\"corrupt\",\"reason\":\"cannot open\"}");
        return;
    }

    int node_count = lsm_store_count_nodes(store, name);
    int edge_count = lsm_store_count_edges(store, name);
    lsm_store_close(store);

    int64_t size = lsm_file_size(db_path);
...
static int find_cross_repo_targets(lsm_store_t *store, const char *project, char **out,
                                   int max_out) {
...
static double layout_radius(const lsm_layout_result_t *r) {
    if (!r || r->node_count == 0)
        return 0.0;
    double max_r2 = 0.0;
...
static bool attach_missed_graph(yyjson_mut_doc *mdoc, yyjson_mut_val *mroot, lsm_store_t *store,
                                const char *project, double primary_radius) {
    char covproj[512];
    lsm_store_coverage_shadow_project(covproj, sizeof(covproj), project);
    lsm_layout_result_t *ml = lsm_layout_compute(store, covproj, LSM_LAYOUT_OVERVIEW, NULL, 0, 0);
    if (!ml) {
        return false;
    }
    if (ml->node_count == 0) {
    ...
    char *mjson = lsm_layout_to_json(ml);
    lsm_layout_free(ml);
    if (!mjson) {
        return false;
    }
...
static void handle_layout(lsm_http_conn_t *c, const lsm_http_req_t *req) {
    char project[256] = {0};
    char max_str[32] = {0};
    char graph_str[32] = {0};
    ...
    if (lsm_http_query_param(req->query, "max_nodes", max_str, (int)sizeof(max_str))) {
        int v = atoi(max_str);
        if (v > 0)
            max_nodes = v;
    }
    ...
    if (lsm_http_query_param(req->query, "graph", graph_str, (int)sizeof(graph_str))) {
        missed_graph = strcmp(graph_str, "missed") == 0;
    }
    char scoped_project[320];
    if (missed_graph) {
        lsm_store_coverage_shadow_project(scoped_project, sizeof(scoped_project), project);
    } else {
    ...
    lsm_layout_result_t *layout =
        lsm_layout_compute(store, scoped_project, LSM_LAYOUT_OVERVIEW, NULL, 0, max_nodes);
    ...
    if (!layout) {
        lsm_store_close(store);
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"layout computation failed\"}");
        return;
    }
    ...
    char *primary_json = lsm_layout_to_json(layout);
    lsm_layout_free(layout);
    if (!primary_json) {
        lsm_store_close(store);
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON serialization failed\"}");
        return;
    }
    ...
    if (linked_count == 0 && missed_graph) {
        lsm_store_close(store);
        lsm_http_replyf(c, 200, g_cors_json, "%s", primary_json);
        free(primary_json);
        return;
    }
    ...
    if (!pdoc) {
        lsm_store_close(store);
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON parse failed\"}");
        return;
    }
    ...
    for (int li = 0; li < linked_count; li++) {
        ...
        if (!lsm_file_exists(lp_path)) {
            free(linked[li]);
            continue;
        }

        lsm_store_t *lp_store = lsm_store_open_path_query(lp_path);
        if (!lp_store) {
            free(linked[li]);
            continue;
        }
        ...
        lsm_layout_result_t *lp_layout =
            lsm_layout_compute(lp_store, linked[li], LSM_LAYOUT_OVERVIEW, NULL, 0, max_nodes);

        if (!lp_layout) {
            lsm_store_close(lp_store);
            free(linked[li]);
            continue;
        }

        double sat_radius = layout_radius(lp_layout);
        char *lp_json = lsm_layout_to_json(lp_layout);
        lsm_layout_free(lp_layout);
        if (!lp_json) {
            lsm_store_close(lp_store);
            free(linked[li]);
            continue;
        ...
        struct sqlite3 *src_db = lsm_store_get_db(store);
        struct sqlite3 *lp_db = lsm_store_get_db(lp_store);
        if (src_db && lp_db) {
    ...
    if (final_json) {
        lsm_http_replyf(c, 200, g_cors_json, "%s", final_json);
        free(final_json);
    } else {
        lsm_http_replyf(c, 500, g_cors_json, "{\"error\":\"JSON write failed\"}");
    }
}
...
static void handle_rpc(lsm_http_conn_t *c, const lsm_http_req_t *req, lsm_mcp_server_t *mcp) {
    if (req->body_len == 0 || req->body_len > MAX_BODY_SIZE || !req->body) {
        lsm_http_replyf(c, 400, g_cors_json,
    ...
    if (!rpc_is_allowed_for_ui(req->body, req->body_len)) {
        lsm_http_replyf(c, 403, g_cors_json,
    ...
    char *response = lsm_mcp_server_handle(mcp, req->body);

    if (response) {
        lsm_http_replyf(c, 200, g_cors_json, "%s", response);
        free(response);
    } else {
        lsm_http_replyf(c, 204, g_cors, "%s", "");
    }
}
...
static bool readiness_hex_decode(const char *hex, uint8_t out[LSM_SHA256_DIGEST_LEN]) {
    if (!hex || strlen(hex) != LSM_SHA256_HEX_LEN) {
        return false;
    }
...
static void readiness_hex_encode(const uint8_t bytes[LSM_SHA256_DIGEST_LEN],
...
static void handle_ui_readiness(lsm_http_server_t *srv, lsm_http_conn_t *c,
                                const lsm_http_req_t *req) {
    if (!srv->readiness_secret_set) {
        lsm_http_replyf(c, 503, "Cache-Control: no-store\r\n", "%s", "readiness proof unavailable");
        return;
    }
    char challenge_hex[LSM_SHA256_HEX_LEN + 1U];
    uint8_t challenge[LSM_SHA256_DIGEST_LEN];
    static const char prefix[] = "challenge=";
    if (strncmp(req->query, prefix, sizeof(prefix) - 1U) != 0 ||
        strlen(req->query) != sizeof(prefix) - 1U + LSM_SHA256_HEX_LEN) {
        lsm_http_replyf(c, 400, "Cache-Control: no-store\r\n", "%s", "invalid challenge");
        return;
    }
    memcpy(challenge_hex, req->query + sizeof(prefix) - 1U, LSM_SHA256_HEX_LEN);
    challenge_hex[LSM_SHA256_HEX_LEN] = '\0';
    if (!readiness_hex_decode(challenge_hex, challenge)) {
        lsm_secure_zero(challenge, sizeof(challenge));
        lsm_http_replyf(c, 400, "Cache-Control: no-store\r\n", "%s", "invalid challenge");
        return;
    }
    uint8_t proof[LSM_SHA256_DIGEST_LEN];
    char proof_hex[LSM_SHA256_HEX_LEN + 1U];
    lsm_hmac_sha256(srv->readiness_secret, sizeof(srv->readiness_secret), challenge,
                    sizeof(challenge), proof);
    readiness_hex_encode(proof, proof_hex);
    lsm_http_replyf(c, 200,
    ...
    lsm_secure_zero(proof_hex, sizeof(proof_hex));
}

static bool request_passes_http_security(lsm_http_server_t *srv, lsm_http_conn_t *c,
                                         const lsm_http_req_t *req) {
    if (req->http_minor == 1 && req->host[0] == '\0') {
        lsm_http_replyf(c, 400, "", "%s", "{\"error\":\"Host header required\"}");
        return false;
    }
    if (req->host[0] != '\0' && !host_is_this_server(req->host, srv->port)) {
        lsm_http_replyf(c, 403, "", "%s", "{\"error\":\"forbidden host\"}");
        return false;
    }
    if (req->origin[0] != '\0' &&
        (req->host[0] == '\0' || !origin_is_same_server(req->origin, srv->port) ||
         !origin_matches_host(req->origin, req->host, srv->port))) {
        lsm_http_replyf(c, 403, "", "%s", "{\"error\":\"forbidden origin\"}");
        return false;
    }
    ...
    if (route_is_protected(req->path) && is_post && !content_type_is_json(req->content_type)) {
        lsm_http_replyf(c, 415, g_cors_json, "%s", "{\"error\":\"application/json required\"}");
        return false;
    }
    return true;
}

static void dispatch_request(lsm_http_server_t *srv, lsm_http_conn_t *c,
                             const lsm_http_req_t *req) {
    ...
    if (is_post && lsm_http_path_match(req->path, "/rpc")) {
        handle_rpc(c, req, srv->mcp);
        return;
    }

    /* GET /api/layout → 3D graph layout */
    if (is_get && lsm_http_path_match(req->path, "/api/layout*")) {
        handle_layout(c, req);
        return;
    }

    /* GET /api/repo-info → git remote / branch for GitHub deep-links */
    if (is_get && lsm_http_path_match(req->path, "/api/repo-info*")) {
        handle_repo_info(c, req);
        return;
    }

    /* POST /api/index → start background indexing */
    if (is_post && lsm_http_path_match(req->path, "/api/index")) {
        handle_index_start(srv, c, req);
        return;
    }

    /* GET /api/index-status → check indexing progress */
    if (is_get && lsm_http_path_match(req->path, "/api/index-status")) {
        handle_index_status(srv, c);
        return;
    }

    /* GET /api/ui-config → language and local UI preferences */
    if (is_get && lsm_http_path_match(req->path, "/api/ui-config")) {
        handle_ui_config(c, req);
        return;
    }

    /* DELETE /api/project → delete a project's .db file */
    if (is_delete && lsm_http_path_match(req->path, "/api/project*")) {
        handle_delete_project(srv, c, req);
        return;
    }

    /* GET /api/browse → directory browser for file picker */
    if (is_get && lsm_http_path_match(req->path, "/api/browse*")) {
        handle_browse(c, req);
        return;
    }

    /* GET /api/adr → get ADR for project */
    if (is_get && lsm_http_path_match(req->path, "/api/adr*")) {
        handle_adr_get(c, req);
        return;
    }

    /* POST /api/adr → save ADR for project */
    if (is_post && lsm_http_path_match(req->path, "/api/adr")) {
        handle_adr_save(srv, c, req);
        return;
    }

    /* GET /api/project-health → check db integrity */
    if (is_get && lsm_http_path_match(req->path, "/api/project-health*")) {
        handle_project_health(c, req);
        return;
    }

    /* GET /api/processes → list running logan-spine-mcp processes */
    if (is_get && lsm_http_path_match(req->path, "/api/processes")) {
        handle_processes(c);
        return;
    }

    /* GET /api/logs → recent log lines */
    if (is_get && lsm_http_path_match(req->path, "/api/logs*")) {
        handle_logs(c, req);
        return;
    }

    /* GET / → index.html (no-cache so browser always gets latest) */
    if (lsm_http_path_match(req->path, "/")) {
        const lsm_embedded_file_t *f = lsm_embedded_lookup("/index.html");
        if (f) {
            char html_hdrs[1024];
            snprintf(html_hdrs, sizeof(html_hdrs),
                     ...
                     "X-Content-Type-Options: nosniff\r\n" LSM_UI_CSP,
                     g_cors);
            lsm_http_reply_buf(c, 200, html_hdrs, f->data, (size_t)f->size);
            return;
        }
        lsm_http_replyf(c, 404, g_cors, "no frontend embedded");
        return;
    }
    ...
    lsm_http_replyf(c, 404, g_cors, "not found");
}
...
static char *http_read_only_index_rejected(void *context, const char *repo_path,
    ...
    (void)repo_path;
    (void)args_json;
    return lsm_mcp_text_result("UI RPC indexing is disabled; use the coordinated /api/index route",
                               true);
}

lsm_http_server_t *lsm_http_server_new(int port) {
    lsm_http_server_t *srv = calloc(1, sizeof(*srv));
    if (!srv)
        return NULL;
    ...
    srv->mcp = lsm_mcp_server_new(NULL);
    if (!srv->mcp) {
        lsm_log_error("ui.http.mcp_fail", "reason", "cannot create MCP instance");
        free(srv);
        return NULL;
    }
    lsm_mcp_server_set_background_tasks(srv->mcp, false);
    lsm_mcp_server_set_index_executor(srv->mcp, http_read_only_index_rejected, srv);

    /* Bind to localhost only (httpd refuses anything else by construction) */
    srv->listener = lsm_httpd_listen(port);
    if (!srv->listener) {
        char port_str[16];
        snprintf(port_str, sizeof(port_str), "%d", port);
        lsm_log_warn("ui.unavailable", "port", port_str, "reason", "in_use", "hint",
                     "use --port=N to override");
        lsm_mcp_server_free(srv->mcp);
        free(srv);
        return NULL;
    }

    srv->port = lsm_httpd_port(srv->listener);
    ...
    lsm_log_info("ui.serving", "url", url, "port", port_str);
    ...
    for (int i = 0; i < MAX_INDEX_JOBS; i++) {
        if (srv->index_jobs[i].thread_started) {
            if (!atomic_load_explicit(&srv->index_jobs[i].completed, memory_order_acquire))
                return false;
            if (lsm_thread_join(&srv->index_jobs[i].thread) != 0)
                return false;
            srv->index_jobs[i].thread_started = false;
        }
    }
    if (!lsm_httpd_close(srv->listener))
        return false;
    lsm_mcp_server_free(srv->mcp);
    lsm_secure_zero(srv->readiness_secret, sizeof(srv->readiness_secret));
    free(srv);
    return true;
}
...
void lsm_http_server_stop(lsm_http_server_t *srv) {
    if (srv) {
        ...
        lsm_httpd_interrupt(srv->listener);
    }
}
...
void lsm_http_server_run(lsm_http_server_t *srv) {
    ...
    while (!atomic_load(&srv->stop_flag)) {
        lsm_http_conn_t *conn = lsm_httpd_accept(srv->listener, 200);
        if (!conn)
            continue; /* timeout — re-check stop flag */

        uint64_t request_start_ms = lsm_now_ms();
        lsm_http_req_t req;
        int rc = lsm_httpd_read_request(conn, &req);
        if (rc == 0) {
            if (atomic_load(&srv->stop_flag)) {
                lsm_http_req_free(&req);
                lsm_httpd_conn_close(conn);
                break;
            }
            dispatch_request(srv, conn, &req);
            lsm_log_http_request("graph_ui", req.method, req.path, lsm_http_conn_status(conn),
                                 (int64_t)(lsm_now_ms() - request_start_ms), req.body_len,
                                 lsm_http_conn_response_bytes(conn));
            lsm_http_req_free(&req);
        } else if (rc > 0) {
            /* Parse/transport error with a known HTTP status (400/408/411/413/431).
             * No CORS reflection here — the request was never parsed. */
            lsm_http_replyf(conn, rc, "", "bad request");
            lsm_log_http_request("graph_ui", "", "", lsm_http_conn_status(conn),
                                 (int64_t)(lsm_now_ms() - request_start_ms), 0,
                                 lsm_http_conn_response_bytes(conn));
        }
        lsm_httpd_conn_close(conn);
    }
...
lsm_httpd_activity_t lsm_http_server_activity_for_test(lsm_http_server_t *srv) {
    return srv ? lsm_httpd_activity_for_test(srv->listener) : LSM_HTTPD_ACTIVITY_IDLE;
}
...
void lsm_http_server_set_recv_deadline_ms(lsm_http_server_t *srv, int ms) {
    if (srv && srv->listener_ok) {
        lsm_httpd_set_recv_deadline_ms(srv->listener, ms);
    }
}
...
void lsm_http_server_set_project_mutation_guard(lsm_http_server_t *srv,
    ...
    lsm_mcp_server_set_project_mutation_guard(srv->mcp, begin, end, begin ? context : NULL);
    lsm_mcp_server_set_project_mutation_try_guard(srv->mcp, begin);
}
