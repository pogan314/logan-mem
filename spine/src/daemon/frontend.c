...
enum {
    ...
    FRONTEND_MAINTENANCE_GRACE_MS =
        LSM_SUBPROCESS_MAX_CANCEL_GRACE_MS + LSM_SUBPROCESS_FORCE_SETTLE_MS + 1000,
...
struct lsm_daemon_maintenance_monitor {
    lsm_thread_t thread;
    lsm_version_cohort_manager_t *manager;
    lsm_daemon_maintenance_cancel_fn cancel;
    void *cancel_context;
...
static void *frontend_maintenance_monitor_worker(void *opaque) {
    lsm_daemon_maintenance_monitor_t *monitor = opaque;
    while (!atomic_load_explicit(&monitor->stopping, memory_order_acquire)) {
        lsm_version_cohort_maintenance_presence_t presence =
            lsm_version_cohort_maintenance_presence_terminal(monitor->manager);
        if (presence == LSM_VERSION_COHORT_MAINTENANCE_ABSENT) {
            lsm_usleep(FRONTEND_MAINTENANCE_POLL_MS * 1000U);
            continue;
        }
...
lsm_daemon_maintenance_monitor_t *lsm_daemon_maintenance_monitor_start(
    ...
    lsm_daemon_maintenance_monitor_t *monitor = calloc(1, sizeof(*monitor));
    if (!monitor) {
        return NULL;
    }
    ...
    if (written <= 0 || written >= (int)sizeof(monitor->participant) ||
        lsm_thread_create(&monitor->thread, 0, frontend_maintenance_monitor_worker, monitor) != 0) {
        free(monitor);
        return NULL;
    }
    return monitor;
...
bool lsm_daemon_maintenance_monitor_stop(lsm_daemon_maintenance_monitor_t **monitor_io) {
    ...
    lsm_daemon_maintenance_monitor_t *monitor = *monitor_io;
    atomic_store_explicit(&monitor->stopping, true, memory_order_release);
    if (lsm_thread_join(&monitor->thread) != 0) {
        return false;
    }
    free(monitor);
    *monitor_io = NULL;
...
static void frontend_exit_for_maintenance(frontend_state_t *state) {
    lsm_version_cohort_maintenance_presence_t presence =
        lsm_version_cohort_maintenance_presence_terminal(state->cohort_manager);
    if (presence == LSM_VERSION_COHORT_MAINTENANCE_ABSENT) {
        return;
    }
    ...
    _Exit(presence == LSM_VERSION_COHORT_MAINTENANCE_REQUESTED ? EXIT_SUCCESS : EXIT_FAILURE);
}
...
static bool frontend_pop_begin(frontend_state_t *state, frontend_item_t *item) {
    ...
    if (!state->stopping && state->count > 0) {
        ...
        bool token_ready = queued->cancelled || lsm_daemon_runtime_client_application_token_reserve(
...
static void frontend_end_request(frontend_state_t *state, bool failed) {
    ...
    state->active_request_token = LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
...
static bool frontend_parse_cancellation(const char *message, lsm_jsonrpc_request_t *request_out) {
    if (!message || !request_out) {
        return false;
    }
    memset(request_out, 0, sizeof(*request_out));
    if (lsm_jsonrpc_parse(message, request_out) != 0) {
        return false;
    }
    bool cancellation = !request_out->has_id && request_out->method &&
                        strcmp(request_out->method, "notifications/cancelled") == 0;
    if (!cancellation) {
        lsm_jsonrpc_request_free(request_out);
    }
    return cancellation;
}
...
bool lsm_daemon_frontend_is_cancellation_notification(const char *message) {
    ...
    if (cancellation) {
        lsm_jsonrpc_request_free(&request);
    }
    return cancellation;
}
...
bool lsm_daemon_frontend_cancellation_matches_request(const char *message, int64_t active_id,
    ...
    bool matches = cancellation &&
                   lsm_mcp_cancel_request_matches(request.params_raw, active_id, active_id_str);
    if (cancellation) {
        lsm_jsonrpc_request_free(&request);
    }
    return matches;
...
static frontend_cancellation_route_t frontend_route_cancellation(
    ...
    lsm_daemon_runtime_application_token_t *request_token_out) {
    if (request_token_out) {
        *request_token_out = LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    lsm_jsonrpc_request_t request = {0};
    ...
    if (state->in_request && state->active_has_id &&
        state->active_request_token != LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
        lsm_mcp_cancel_request_matches(request.params_raw, state->active_id,
                                       state->active_id_str)) {
        if (request_token_out) {
        ...
        for (size_t offset = 0; offset < state->count; offset++) {
            ...
            if (item->has_id &&
                lsm_mcp_cancel_request_matches(request.params_raw, item->id, item->id_str)) {
                item->cancelled = true;
...
static void *frontend_worker(void *opaque) {
    ...
    for (;;) {
        ...
        if (!frontend_pop_begin(state, &item)) {
            ...
            lsm_usleep(FRONTEND_IDLE_WAIT_US);
            continue;
        }
        ...
        if (item.cancelled) {
            ...
            lsm_daemon_runtime_application_status_t status =
                lsm_daemon_application_client_mcp_tagged(state->client, item.request_token,
                                                         item.message, &response, &response_length,
                                                         FRONTEND_REQUEST_TIMEOUT_MS);
            if (status == LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED) {
                failed = !frontend_write_cancelled_response(state->out, &item);
            } else {
                failed = status != LSM_DAEMON_RUNTIME_APPLICATION_OK;
            }
            if (status == LSM_DAEMON_RUNTIME_APPLICATION_OK && !failed && response &&
                response_length > 0) {
...
static bool frontend_enqueue(frontend_state_t *state, char *message, bool content_length_framed) {
    ...
    char *id_str = NULL;
    lsm_jsonrpc_request_t request = {0};
    if (lsm_jsonrpc_parse(message, &request) == 0) {
        has_id = request.has_id;
        id = request.id;
        if (request.id_str) {
            id_str = lsm_strdup(request.id_str);
        }
...
static bool frontend_stop_begin(frontend_state_t *state) {
    ...
    return lsm_daemon_runtime_client_close_begin(state->client);
}
...
static bool frontend_cancel_for_maintenance(void *opaque) {
    ...
    return lsm_daemon_runtime_client_application_cancel(state->client, request_token) ==
           LSM_DAEMON_RUNTIME_CANCEL_ACCEPTED;
}
...
int lsm_daemon_frontend_mcp_run(lsm_daemon_runtime_client_t *client,
    ...
    lsm_mutex_init(&state.mutex);
    atomic_init(&state.completed_items, 0);
    lsm_daemon_maintenance_monitor_t *maintenance_monitor = lsm_daemon_maintenance_monitor_start(
        cohort_manager, frontend_cancel_for_maintenance, &state, EXIT_SUCCESS, "MCP frontend");
    if (!maintenance_monitor) {
        lsm_mutex_destroy(&state.mutex);
        (void)lsm_daemon_runtime_client_close(client, FRONTEND_CLOSE_TIMEOUT_MS);
        return -1;
    }
    lsm_thread_t worker;
    if (lsm_thread_create(&worker, 0, frontend_worker, &state) != 0) {
        if (!lsm_daemon_maintenance_monitor_stop(&maintenance_monitor)) {
            _Exit(EXIT_FAILURE);
        }
        lsm_mutex_destroy(&state.mutex);
        (void)lsm_daemon_runtime_client_close(client, FRONTEND_CLOSE_TIMEOUT_MS);
    ...
    for (;;) {
        ...
        int read_status = lsm_mcp_read_message(in, &message, &content_length_framed);
        if (read_status <= 0) {
        ...
        lsm_daemon_runtime_application_token_t cancel_token =
            LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
        ...
        if (cancellation != FRONTEND_CANCELLATION_NONE) {
            ...
            if (cancellation == FRONTEND_CANCELLATION_ACTIVE) {
                lsm_daemon_runtime_cancel_result_t cancelled =
                    lsm_daemon_runtime_client_application_cancel(state.client, cancel_token);
                if (cancelled == LSM_DAEMON_RUNTIME_CANCEL_ERROR) {
                    result = -1;
    ...
    if (clean_eof && !close_begun) {
        ...
        uint64_t drain_deadline = lsm_now_ms() + FRONTEND_EOF_DRAIN_MS;
        while (!frontend_worker_is_done(&state) && lsm_now_ms() < drain_deadline) {
            lsm_usleep(FRONTEND_WAIT_US);
            uint64_t now_drained =
                atomic_load_explicit(&state.completed_items, memory_order_acquire);
            if (now_drained != drained) {
                drained = now_drained;
                drain_deadline = lsm_now_ms() + FRONTEND_EOF_DRAIN_MS;
            }
        }
    }
    if (!close_begun) {
    ...
    if (!frontend_worker_is_done(&state)) {
        ...
        if (lsm_thread_create(&watchdog_thread, 0, frontend_join_watchdog, &watchdog) != 0) {
            _Exit(EXIT_FAILURE);
        }
        watchdog_started = true;
    }
    if (lsm_thread_join(&worker) != 0) {
        /* Preserve every object the worker may still reference. */
        _Exit(EXIT_FAILURE);
    }
    if (watchdog_started) {
        atomic_store_explicit(&watchdog.complete, true, memory_order_release);
        if (lsm_thread_join(&watchdog_thread) != 0) {
            _Exit(EXIT_FAILURE);
        }
    }
    if (!lsm_daemon_maintenance_monitor_stop(&maintenance_monitor)) {
    ...
    if (close_begun) {
        (void)lsm_daemon_runtime_client_close_finish(state.client, FRONTEND_CLOSE_TIMEOUT_MS);
    } else {
        result = -1;
    }
