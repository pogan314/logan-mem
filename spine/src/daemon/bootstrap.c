...
lsm_daemon_process_role_t lsm_daemon_process_role(int argc, char *const argv[]) {
    if (argc <= 0 || !argv || !argv[0] || argv[0][0] == '\0') {
        return LSM_DAEMON_PROCESS_INVALID;
    }

    int daemon_arg = bootstrap_find_arg(argc, argv, LSM_DAEMON_INTERNAL_ARG);
    if (daemon_arg >= 0) {
        ...
        if (argc == 2 && daemon_arg == 1) {
            return LSM_DAEMON_PROCESS_DAEMON;
        }
        if (argc == 3 && daemon_arg == 1 && bootstrap_arg_is(argv[2], LSM_DAEMON_PERMANENT_ARG)) {
            return LSM_DAEMON_PROCESS_DAEMON;
        }
        return LSM_DAEMON_PROCESS_INVALID;
    }
    ...
    if (worker_arg >= 0) {
        return bootstrap_worker_argv_exact(argc, argv) ? LSM_DAEMON_PROCESS_WORKER
                                                       : LSM_DAEMON_PROCESS_INVALID;
    }
    ...
    for (int arg = 1; arg < argc; arg++) {
        if (bootstrap_arg_is(argv[arg], "cli")) {
            if (bootstrap_has_help_after(argc, argv, arg + 1)) {
                return LSM_DAEMON_PROCESS_STATELESS;
            }
            return LSM_DAEMON_PROCESS_LOCAL_CLI;
        }
        if (bootstrap_arg_is(argv[arg], "hook-augment")) {
            return LSM_DAEMON_PROCESS_HOOK_CLIENT;
        }
        if (bootstrap_arg_is(argv[arg], "config")) {
            return bootstrap_has_help_after(argc, argv, arg + 1) ? LSM_DAEMON_PROCESS_STATELESS
                                                                 : LSM_DAEMON_PROCESS_LOCAL_CLI;
        }
        /* Placed after the `cli` check on purpose: `lsm cli search "daemon
         * start"` is opaque tool input and must stay LOCAL_CLI. */
        if (bootstrap_arg_is(argv[arg], "daemon")) {
            return bootstrap_has_help_after(argc, argv, arg + 1) ? LSM_DAEMON_PROCESS_STATELESS
                                                                 : LSM_DAEMON_PROCESS_DAEMON_CTL;
        }
    ...
    return LSM_DAEMON_PROCESS_MCP_CLIENT;
}

bool lsm_daemon_process_role_requires_client(lsm_daemon_process_role_t role) {
    return role == LSM_DAEMON_PROCESS_MCP_CLIENT || role == LSM_DAEMON_PROCESS_HOOK_CLIENT;
}
 ...
 * LSM_TEST_DAEMON_RUNTIME_PARENT, compiled out unless LSM_ENABLE_TEST_SEAMS is
 * defined, so a test build started while the shipped build did not. LSM_CACHE_DIR
 * does not help either — it moves the cache, never the rendezvous.
 *
 * LSM_RUNTIME_DIR does NOT relax the check. The directory it names goes through
 ...
 * lsm_safe_getenv never truncates: a value too long for the buffer is reported
...
static const char *bootstrap_runtime_parent_override(char *buffer, size_t capacity) {
    const char *value = lsm_safe_getenv("LSM_RUNTIME_DIR", buffer, capacity, NULL);
    return value && value[0] != '\0' ? value : NULL;
}

lsm_daemon_ipc_endpoint_t *lsm_daemon_bootstrap_endpoint_new(const char *runtime_parent) {
    char key[LSM_DAEMON_KEY_SIZE];
    if (!lsm_daemon_rendezvous_key(key)) {
        return NULL;
    }
    ...
    return lsm_daemon_ipc_endpoint_new(key, parent);
}

bool lsm_daemon_bootstrap_launch_spec_init(const char *executable_path,
                                           lsm_daemon_bootstrap_launch_spec_t *spec_out) {
    ...
    spec_out->argv[1] = LSM_DAEMON_INTERNAL_ARG;
    spec_out->argc = 2U;
...
bool lsm_daemon_bootstrap_launch_spec_init_permanent(const char *executable_path,
                                                     lsm_daemon_bootstrap_launch_spec_t *spec_out) {
    if (!lsm_daemon_bootstrap_launch_spec_init(executable_path, spec_out)) {
        return false;
    }
    spec_out->argv[2] = LSM_DAEMON_PERMANENT_ARG;
    spec_out->argc = 3U;
    return true;
}
...
static _Noreturn void bootstrap_cleanup_fail_stop(const char *component) {
    (void)fprintf(stderr,
                  "logan-spine-mcp: coordination cleanup failed (%s); "
...
static void bootstrap_startup_lock_release_complete(const lsm_daemon_bootstrap_ops_t *ops,
                                                    lsm_daemon_bootstrap_lock_t *lock_io) {
...
static void bootstrap_result_reset(lsm_daemon_bootstrap_result_t *result,
                                   lsm_daemon_process_role_t role) {
    memset(result, 0, sizeof(*result));
    result->status = lsm_daemon_process_role_requires_client(role) ? LSM_DAEMON_BOOTSTRAP_FAILED
                                                                   : LSM_DAEMON_BOOTSTRAP_BYPASSED;
}

static lsm_daemon_bootstrap_status_t bootstrap_finish_probe(
    lsm_daemon_bootstrap_probe_status_t probe, lsm_daemon_runtime_client_t *client,
    const lsm_daemon_runtime_connect_result_t *connect_result,
    const lsm_daemon_bootstrap_ops_t *ops, lsm_daemon_bootstrap_result_t *result) {
    if (connect_result) {
        result->connect_result = *connect_result;
    }
    result->client = client;
    if (probe == LSM_DAEMON_BOOTSTRAP_PROBE_CONNECTED && client) {
        result->status = LSM_DAEMON_BOOTSTRAP_CONNECTED;
        return result->status;
    }
    result->client = NULL;
    if (probe == LSM_DAEMON_BOOTSTRAP_PROBE_CONFLICT) {
        result->status = LSM_DAEMON_BOOTSTRAP_CONFLICT;
        (void)snprintf(result->message, sizeof(result->message), "%s",
                       connect_result && connect_result->message[0]
                           ...
                           : "LSM could not start because a conflicting LSM process is active; "
                             "close all LSM sessions and commands, then retry. If a permanent "
                             "daemon from another build is running, `logan-spine-mcp daemon "
                             "stop` retires it");
    ...
    return LSM_DAEMON_BOOTSTRAP_FAILED;
}

static lsm_daemon_bootstrap_probe_status_t bootstrap_probe(
    const lsm_daemon_bootstrap_config_t *config, const lsm_daemon_bootstrap_ops_t *ops,
    lsm_daemon_runtime_client_t **client_out, lsm_daemon_runtime_connect_result_t *connect_result) {
...
static bool bootstrap_probe_is_finishable(lsm_daemon_bootstrap_probe_status_t probe) {
    return probe == LSM_DAEMON_BOOTSTRAP_PROBE_CONNECTED ||
           probe == LSM_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
}

static bool bootstrap_probe_is_waitable(lsm_daemon_bootstrap_probe_status_t probe) {
    return probe == LSM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE ||
           probe == LSM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
           probe == LSM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
}

static bool bootstrap_config_valid(const lsm_daemon_bootstrap_config_t *config,
...
lsm_daemon_bootstrap_status_t lsm_daemon_bootstrap_execute_with_ops(
    const lsm_daemon_bootstrap_config_t *config, const lsm_daemon_bootstrap_ops_t *ops,
    lsm_daemon_bootstrap_result_t *result_out) {
    if (!result_out) {
        return LSM_DAEMON_BOOTSTRAP_FAILED;
    }
    lsm_daemon_process_role_t role = config ? config->role : LSM_DAEMON_PROCESS_INVALID;
    bootstrap_result_reset(result_out, role);
    if (!lsm_daemon_process_role_requires_client(role)) {
        return role == LSM_DAEMON_PROCESS_INVALID ? LSM_DAEMON_BOOTSTRAP_FAILED
                                                  : LSM_DAEMON_BOOTSTRAP_BYPASSED;
    }
    ...
    lsm_daemon_bootstrap_probe_status_t probe =
    ...
    if (!bootstrap_probe_is_waitable(probe)) {
        probe = LSM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }

    lsm_daemon_bootstrap_lock_t startup_lock = NULL;
    bool lock_acquired = false;
    bool generation_observed = probe == LSM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
                               probe == LSM_DAEMON_BOOTSTRAP_PROBE_TERMINAL;
    while (lsm_now_ms() < deadline) {
        if (!bootstrap_probe_is_waitable(probe)) {
            break;
        }
        if (probe == LSM_DAEMON_BOOTSTRAP_PROBE_RESERVED ||
            probe == LSM_DAEMON_BOOTSTRAP_PROBE_TERMINAL) {
        ...
        if (bootstrap_probe_is_finishable(probe) || probe == LSM_DAEMON_BOOTSTRAP_PROBE_ERROR) {
            break;
        }
        if (probe != LSM_DAEMON_BOOTSTRAP_PROBE_UNAVAILABLE) {
            probe = LSM_DAEMON_BOOTSTRAP_PROBE_ERROR;
            break;
        }
        ...
        bool spec_ready =
            config->spawn_permanent
                ? lsm_daemon_bootstrap_launch_spec_init_permanent(config->executable_path, &spec)
                : lsm_daemon_bootstrap_launch_spec_init(config->executable_path, &spec);
    ...
    result_out->status = LSM_DAEMON_BOOTSTRAP_FAILED;
    if (generation_observed) {
        (void)snprintf(result_out->message, sizeof(result_out->message),
                       "LSM daemon is active or starting but could not accept this client "
                       "within %u ms",
                       config->startup_timeout_ms);
    } else {
        (void)snprintf(result_out->message, sizeof(result_out->message),
                       "LSM daemon could not start within %u ms", config->startup_timeout_ms);
    }
    if (ops->visible_diagnostic) {
        ops->visible_diagnostic(ops->context, result_out->message);
    }
...
lsm_daemon_bootstrap_probe_status_t lsm_daemon_bootstrap_classify_failed_connect(
    const lsm_daemon_runtime_connect_result_t *connect_result, int lifetime_status) {
    if (!connect_result) {
        return LSM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }
    if (connect_result->status == LSM_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        return LSM_DAEMON_BOOTSTRAP_PROBE_CONFLICT;
    }
    if (connect_result->status == LSM_DAEMON_RUNTIME_CONNECT_REJECTED) {
    ...
    return connect_result->status == LSM_DAEMON_RUNTIME_CONNECT_ERROR
...
static lsm_daemon_bootstrap_probe_status_t bootstrap_production_probe(
    void *context, const lsm_daemon_ipc_endpoint_t *endpoint,
    ...
    lsm_version_cohort_daemon_presence_t claim =
        lsm_version_cohort_daemon_claim_presence(production->cohort->manager);
    if (claim == LSM_VERSION_COHORT_DAEMON_ABSENT) {
    ...
    } else if (claim != LSM_VERSION_COHORT_DAEMON_COORDINATED) {
        return LSM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }

    *client_out = lsm_daemon_runtime_client_connect(endpoint, identity, timeout_ms, result_out);
    if (*client_out) {
    ...
    claim = lsm_version_cohort_daemon_claim_presence(production->cohort->manager);
    if (claim == LSM_VERSION_COHORT_DAEMON_COORDINATED) {
        return lsm_daemon_bootstrap_classify_failed_connect(result_out, 1);
    }
    if (claim != LSM_VERSION_COHORT_DAEMON_ABSENT) {
        return LSM_DAEMON_BOOTSTRAP_PROBE_ERROR;
    }
    int lifetime_status = lsm_daemon_ipc_lifetime_reservation_probe(endpoint);
    return lsm_daemon_bootstrap_classify_failed_connect(result_out, lifetime_status);
}

static lsm_version_cohort_status_t bootstrap_production_cohort_acquire(
    void *context, const lsm_daemon_ipc_endpoint_t *endpoint,
    ...
    *cohort_out = NULL;
    ...
    if (cohort) {
        cohort->manager = lsm_version_cohort_manager_new(endpoint);
    }
    if (!cohort || !cohort->manager) {
        free(cohort);
        return LSM_VERSION_COHORT_IO;
    }
    lsm_version_cohort_status_t status = lsm_version_cohort_acquire(
        cohort->manager, identity, deadline_ms, &cohort->lease, conflict_out);
    if (status == LSM_VERSION_COHORT_CONFLICT) {
        (void)lsm_version_cohort_log_conflict(conflict_out);
    }
    if (status == LSM_VERSION_COHORT_OK || cohort->lease) {
        *cohort_out = cohort;
        if (context) {
            bootstrap_production_context_t *production = context;
    ...
    lsm_private_file_lock_status_t cleanup = LSM_PRIVATE_FILE_LOCK_OK;
    while (cohort->manager) {
        cleanup = lsm_version_cohort_manager_free(&cohort->manager);
        if (!cohort->manager) {
            break;
        }
        if (lsm_now_ms() >= cleanup_deadline) {
            bootstrap_cleanup_fail_stop("cohort_manager_cleanup");
        }
        lsm_usleep(1000);
    }
    free(cohort);
    return cleanup == LSM_PRIVATE_FILE_LOCK_OK ? status : LSM_VERSION_COHORT_IO;
}

static void bootstrap_production_cohort_release(void *context,
                                                lsm_daemon_bootstrap_cohort_t opaque) {
    bootstrap_production_context_t *production = context;
    ...
    uint64_t cleanup_deadline = bootstrap_deadline_after(BOOTSTRAP_COORDINATION_CLEANUP_MS);
    while (cohort->lease) {
        (void)lsm_version_cohort_lease_release(&cohort->lease);
        if (!cohort->lease) {
            break;
        }
        if (lsm_now_ms() >= cleanup_deadline) {
    ...
    while (cohort->manager) {
        (void)lsm_version_cohort_manager_free(&cohort->manager);
        if (!cohort->manager) {
            break;
        }
        if (lsm_now_ms() >= cleanup_deadline) {
...
static int bootstrap_production_lock(void *context, const lsm_daemon_ipc_endpoint_t *endpoint,
                                     lsm_daemon_bootstrap_lock_t *lock_out) {
    (void)context;
    lsm_daemon_ipc_startup_lock_t *lock = NULL;
    int status = lsm_daemon_ipc_startup_lock_try_acquire(endpoint, &lock);
    *lock_out = lock;
    return status;
}

static bool bootstrap_production_unlock(void *context, lsm_daemon_bootstrap_lock_t *lock_io) {
    (void)context;
    if (!lock_io) {
        return false;
    }
    lsm_daemon_ipc_startup_lock_t *lock = *lock_io;
    bool released = lsm_daemon_ipc_startup_lock_release(&lock);
    *lock_io = lock;
    return released;
}

static bool bootstrap_production_handoff(void *context, lsm_daemon_bootstrap_lock_t lock) {
    (void)context;
    return lsm_daemon_ipc_startup_lock_prepare_handoff((lsm_daemon_ipc_startup_lock_t *)lock);
}

#ifdef _WIN32
static bool bootstrap_production_spawn(void *context,
                                       const lsm_daemon_bootstrap_launch_spec_t *spec) {
    bootstrap_production_context_t *production = context;
    if (production) {
    ...
    if (!lsm_build_win_cmdline(command_line, sizeof(command_line), spec->argv)) {
        if (production) {
            production->spawn_error = ERROR_INVALID_PARAMETER;
        }
        return false;
    }
    wchar_t *application = lsm_utf8_to_wide(spec->executable_path);
    wchar_t *command = lsm_utf8_to_wide(command_line);
...
static void bootstrap_daemon_grandchild(const lsm_daemon_bootstrap_launch_spec_t *spec) {
    (void)umask(077);
    sigset_t empty;
    (void)sigemptyset(&empty);
...
static void bootstrap_production_diagnostic(void *context, const char *message) {
    ...
    if (production && production->spawn_error != ERROR_SUCCESS) {
        (void)fprintf(stderr, "logan-spine-mcp: %s (daemon launch error %lu)\n",
    ...
    if (production && production->spawn_error != 0) {
        (void)fprintf(stderr, "logan-spine-mcp: %s (daemon launch: %s)\n",
                      message ? message : "daemon startup failed",
...
lsm_daemon_bootstrap_status_t lsm_daemon_bootstrap_execute(
    const lsm_daemon_bootstrap_config_t *config, lsm_daemon_bootstrap_result_t *result_out) {
    ...
    return lsm_daemon_bootstrap_execute_with_ops(config, &ops, result_out);
}
