/*
 * runtime.h — Mandatory per-account LSM daemon runtime.
 *
 ...
 */
#ifndef LSM_DAEMON_RUNTIME_H
#define LSM_DAEMON_RUNTIME_H
...
#define LSM_DAEMON_RUNTIME_WIRE_ABI 1U
...
#define LSM_DAEMON_RENDEZVOUS_ABI 1U
#define LSM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP 64U
#define LSM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP 65U
#define LSM_DAEMON_RENDEZVOUS_MESSAGE_CAP 512U
#define LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE 133U
#define LSM_DAEMON_RENDEZVOUS_RESPONSE_SIZE 798U
...
#define LSM_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE 137U
#define LSM_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE 24U
...
#define LSM_DAEMON_CONTROL_REQUEST_SIZE 65U
#define LSM_DAEMON_CONTROL_CLIENT_CAP 8U
#define LSM_DAEMON_STATUS_RESPONSE_SIZE 121U
#define LSM_DAEMON_STOP_RESPONSE_SIZE 40U
#define LSM_DAEMON_RUNTIME_PROJECT_KEY_MAX 4096U
#define LSM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX (LSM_DAEMON_MAX_FRAME_SIZE - 16U)
...
typedef enum {
    ...
    LSM_DAEMON_RUNTIME_OP_HEARTBEAT = 2,
    LSM_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE = 3,
    LSM_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE = 4,
    LSM_DAEMON_RUNTIME_OP_DISCONNECT = 5,
    LSM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST = 6,
    LSM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL = 7,
    ...
    LSM_DAEMON_RUNTIME_OP_CLOSE_INTENT = 9,
    ...
    LSM_DAEMON_RUNTIME_OP_STATUS = 10,
    ...
    LSM_DAEMON_RUNTIME_OP_STOP = 11,
} lsm_daemon_runtime_operation_t;

typedef enum {
    LSM_DAEMON_RUNTIME_ACTIVATION_INSTALL = 1,
    LSM_DAEMON_RUNTIME_ACTIVATION_UPDATE = 2,
    LSM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL = 3,
} lsm_daemon_runtime_activation_action_t;

typedef struct {
    bool accepted;
    uint64_t active_clients;
    uint64_t active_connections;
} lsm_daemon_runtime_activation_result_t;

typedef struct lsm_daemon_runtime_service lsm_daemon_runtime_service_t;
typedef struct lsm_daemon_runtime_client lsm_daemon_runtime_client_t;

typedef enum {
    LSM_DAEMON_RUNTIME_SERVICE_STARTING = 1,
    LSM_DAEMON_RUNTIME_SERVICE_RUNNING = 2,
    LSM_DAEMON_RUNTIME_SERVICE_STOPPING = 3,
    LSM_DAEMON_RUNTIME_SERVICE_EXITED = 4,
} lsm_daemon_runtime_service_state_t;
...
typedef enum {
    LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR = 0,
    LSM_DAEMON_RUNTIME_APPLICATION_OK = 1,
    LSM_DAEMON_RUNTIME_APPLICATION_BUSY = 2,
    LSM_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE = 3,
    LSM_DAEMON_RUNTIME_APPLICATION_REJECTED = 4,
    LSM_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR = 5,
    LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED = 6,
} lsm_daemon_runtime_application_status_t;

typedef uint64_t lsm_daemon_runtime_application_token_t;
#define LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID UINT64_C(0)

typedef enum {
    LSM_DAEMON_RUNTIME_CANCEL_ERROR = -1,
    LSM_DAEMON_RUNTIME_CANCEL_STALE = 0,
    LSM_DAEMON_RUNTIME_CANCEL_ACCEPTED = 1,
} lsm_daemon_runtime_cancel_result_t;

typedef void lsm_daemon_runtime_application_session_t;

typedef lsm_daemon_runtime_application_session_t *(*lsm_daemon_runtime_application_session_open_fn)(
...
typedef lsm_daemon_runtime_application_status_t (*lsm_daemon_runtime_application_request_fn)(
    void *context, lsm_daemon_runtime_application_session_t *session,
...
typedef void (*lsm_daemon_runtime_application_request_cancel_fn)(
    void *context, lsm_daemon_runtime_application_session_t *session,
...
typedef void (*lsm_daemon_runtime_application_session_cancel_fn)(
    void *context, lsm_daemon_runtime_application_session_t *session);
typedef void (*lsm_daemon_runtime_application_session_close_fn)(
    void *context, lsm_daemon_runtime_application_session_t *session);

typedef struct {
    void *context;
    lsm_daemon_runtime_application_session_open_fn session_open;
    lsm_daemon_runtime_application_request_fn request;
    lsm_daemon_runtime_application_request_cancel_fn request_cancel;
    lsm_daemon_runtime_application_session_cancel_fn session_cancel;
    lsm_daemon_runtime_application_session_close_fn session_close;
} lsm_daemon_runtime_application_callbacks_t;
...
typedef struct {
    ...
    bool permanent;
} lsm_daemon_runtime_service_config_t;

typedef enum {
    LSM_DAEMON_RUNTIME_CONNECT_ERROR = 0,
    LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED = 1,
    LSM_DAEMON_RUNTIME_CONNECT_CONFLICT = 2,
    LSM_DAEMON_RUNTIME_CONNECT_REJECTED = 3,
} lsm_daemon_runtime_connect_status_t;

typedef struct {
    lsm_daemon_runtime_connect_status_t status;
    lsm_daemon_hello_status_t hello_status;
...
} lsm_daemon_runtime_connect_result_t;
...
typedef struct {
    ...
    uint32_t client_pids[LSM_DAEMON_CONTROL_CLIENT_CAP];
} lsm_daemon_runtime_stop_result_t;
 ...
 * Client exchange timeouts must be finite, not LSM_DAEMON_IPC_WAIT_FOREVER. */
