/*
 * application.h — Daemon-owned LSM application sessions and thin-client wire.
 *
 ...
 */
#ifndef LSM_DAEMON_APPLICATION_H
#define LSM_DAEMON_APPLICATION_H
...
typedef struct lsm_daemon_application lsm_daemon_application_t;
typedef void *lsm_daemon_application_worker_t;
typedef void *lsm_daemon_application_update_worker_t;
...
typedef struct {
    ...
    lsm_index_worker_poll_t (*poll)(void *context, lsm_daemon_application_worker_t worker,
                                    const lsm_index_worker_result_t **result_out);
    bool (*cancel)(void *context, lsm_daemon_application_worker_t worker);
    const char *(*log_path)(void *context, lsm_daemon_application_worker_t worker);
    void (*destroy)(void *context, lsm_daemon_application_worker_t worker);
} lsm_daemon_application_worker_ops_t;

typedef enum {
    LSM_DAEMON_APPLICATION_UPDATE_POLL_ERROR = -1,
    LSM_DAEMON_APPLICATION_UPDATE_POLL_RUNNING = 0,
    LSM_DAEMON_APPLICATION_UPDATE_POLL_TERMINAL = 1,
} lsm_daemon_application_update_poll_t;
...
typedef struct {
    void *context;
    int (*start)(void *context, lsm_daemon_application_update_worker_t *worker_out);
    lsm_daemon_application_update_poll_t (*poll)(void *context,
                                                 lsm_daemon_application_update_worker_t worker,
                                                 const char **latest_version_out);
    bool (*cancel)(void *context, lsm_daemon_application_update_worker_t worker);
    void (*destroy)(void *context, lsm_daemon_application_update_worker_t worker);
} lsm_daemon_application_update_ops_t;

typedef struct {
    struct lsm_watcher *watcher;                           /* borrowed; daemon lifetime */
    struct lsm_config *config;                             /* borrowed; daemon lifetime */
    const lsm_daemon_application_worker_ops_t *worker_ops; /* NULL = production */
    const lsm_daemon_application_update_ops_t *update_ops; /* NULL = production */
     ...
     * The daemon host supplies its already-resolved LSM process budget. */
...
} lsm_daemon_application_config_t;

typedef enum {
    LSM_DAEMON_APPLICATION_REQUEST_SET_CONTEXT = 1,
    LSM_DAEMON_APPLICATION_REQUEST_MCP = 2,
    LSM_DAEMON_APPLICATION_REQUEST_TOOL = 3,
    LSM_DAEMON_APPLICATION_REQUEST_HOOK_AUGMENT = 4,
    LSM_DAEMON_APPLICATION_REQUEST_SET_UI_CONFIG = 5,
    LSM_DAEMON_APPLICATION_REQUEST_UI_READINESS_PROOF = 6,
} lsm_daemon_application_request_kind_t;

typedef enum {
    LSM_DAEMON_APPLICATION_UI_CONFIG_ENABLED = 1U << 0,
    LSM_DAEMON_APPLICATION_UI_CONFIG_PORT = 1U << 1,
} lsm_daemon_application_ui_config_mask_t;

lsm_daemon_application_t *lsm_daemon_application_new(const lsm_daemon_application_config_t *config);
...
lsm_daemon_runtime_application_status_t lsm_daemon_application_client_ui_readiness_proof(
    lsm_daemon_runtime_client_t *client, const uint8_t challenge[32], uint8_t proof_out[32],
    uint32_t timeout_ms);
...
#endif /* LSM_DAEMON_APPLICATION_H */
