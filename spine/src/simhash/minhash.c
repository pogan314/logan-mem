...
static void weighted_minhash_update(lsm_minhash_t *out, const char *trigram, int len, int w) {
    for (int k = 0; k < LSM_MINHASH_K; k++) {
...
static int hash_trigrams(const char **tokens, int token_count, lsm_minhash_t *out) {
    for (int k = 0; k < LSM_MINHASH_K; k++) {
...
bool lsm_minhash_compute(TSNode func_body, const char *source, int language, lsm_minhash_t *out) {
    ...
    if (token_count < LSM_MINHASH_MIN_NODES) {
        return false;
    }
...
double lsm_minhash_jaccard(const lsm_minhash_t *a, const lsm_minhash_t *b) {
    ...
    return (double)matching / (double)LSM_MINHASH_K;
}

/* ── Hex encoding/decoding ───────────────────────────────────────── */

void lsm_minhash_to_hex(const lsm_minhash_t *fp, char *buf, int bufsize) {
    if (!fp || !buf || bufsize < LSM_MINHASH_HEX_BUF) {
        if (buf && bufsize > 0) {
            buf[0] = '\0';
        }
        return;
...
bool lsm_minhash_from_hex(const char *hex, lsm_minhash_t *out) {
    ...
    if ((int)len != LSM_MINHASH_HEX_LEN) {
        return false;
    }
    for (int i = 0; i < LSM_MINHASH_K; i++) {
...
static uint32_t band_hash(const lsm_minhash_t *fp, int band) {
    int base = band * LSM_LSH_ROWS;
    /* Combine r=2 values into a single hash */
    uint32_t combined[LSM_LSH_ROWS];
    for (int r = 0; r < LSM_LSH_ROWS; r++) {
...
lsm_lsh_index_t *lsm_lsh_new(void) {
    lsm_lsh_index_t *idx = calloc(SKIP_ONE, sizeof(lsm_lsh_index_t));
    return idx;
}
...
void lsm_lsh_insert(lsm_lsh_index_t *idx, const lsm_lsh_entry_t *entry) {
    ...
    if (idx->entry_count >= idx->entry_cap) {
        ...
        lsm_lsh_entry_t *new_entries =
            realloc(idx->entries, (size_t)new_cap * sizeof(lsm_lsh_entry_t));
        if (!new_entries) {
            return;
        }
...
static bool result_push(lsm_lsh_index_t *idx, const lsm_lsh_entry_t *candidate) {
    if (idx->result_count >= idx->result_cap) {
        int new_cap =
            idx->result_cap < RESULT_INIT_CAP ? RESULT_INIT_CAP : idx->result_cap * GROW_FACTOR;
        const lsm_lsh_entry_t **new_buf =
            realloc(idx->result_buf, (size_t)new_cap * sizeof(const lsm_lsh_entry_t *));
        if (!new_buf) {
            return false;
        }
...
void lsm_lsh_query(const lsm_lsh_index_t *idx, const lsm_minhash_t *fp,
    ...
    *count = 0;
    ...
    lsm_lsh_index_t *mut_idx = (lsm_lsh_index_t *)idx;
    ...
    for (int b = 0; b < LSM_LSH_BANDS; b++) {
        ...
        for (int i = 0; i < bucket->count; i++) {
            const lsm_lsh_entry_t *candidate = &idx->entries[bucket->items[i]];
