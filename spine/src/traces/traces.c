...
const char *lsm_extract_service_name(const lsm_trace_resource_t *r) {
    if (!r) {
        return "";
    }
...
const char *lsm_extract_path_from_url(const char *url, char *buf, size_t buf_sz) {
    if (!url || !buf || buf_sz == 0) {
        if (buf) {
            buf[0] = '\0';
        }
...
int64_t lsm_parse_duration(const char *start_nano, const char *end_nano) {
    if (!start_nano || !end_nano) {
        return 0;
    }
    int64_t start = strtoll(start_nano, NULL, LSM_DECIMAL_BASE);
    int64_t end = strtoll(end_nano, NULL, LSM_DECIMAL_BASE);
    return (end > start) ? (end - start) : 0;
}

/* ── extractHTTPInfo ─────────────────────────────────────────────── */

bool lsm_extract_http_info(const lsm_trace_span_t *span, const char *service_name,
                           lsm_http_span_info_t *out) {
    if (!span || !out) {
        return false;
    }
    ...
    for (int i = 0; i < span->attr_count; i++) {
        ...
        if (strcmp(key, "http.method") == 0 || strcmp(key, "http.request.method") == 0) {
            ...
            const char *path = lsm_extract_path_from_url(val, url_buf, sizeof(url_buf));
            if (path[0] != '\0') {
                out->path = path;
                has_http = true;
            }
    ...
    out->duration_ns = lsm_parse_duration(span->start_time, span->end_time);
    return true;
}
...
int64_t lsm_calculate_p99(int64_t *values, int count) {
    if (!values || count <= 0) {
        return 0;
    }
