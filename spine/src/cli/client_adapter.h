 ...
 * (lsm_mcp_tool_count / lsm_mcp_tool_name), so it cannot drift: adding a tool to
 ...
 */
#ifndef LSM_CLI_CLIENT_ADAPTER_H
#define LSM_CLI_CLIENT_ADAPTER_H
...
#define LSM_ADAPTER_MARKER_START "// logan-spine-mcp:start"
#define LSM_ADAPTER_MARKER_END "// logan-spine-mcp:end"
 ...
 * path such as C:\Users\x\bin\lsm.exe must not corrupt the module or, worse,
...
char *lsm_client_adapter_pi(const char *binary_path);

/* Emit an OpenCode plugin that augments grep/glob results from the graph.
 *
 * OpenCode HAS MCP (see lsm_upsert_opencode_mcp), so this adds no tools — it
 ...
 *
 * Returns NULL on allocation failure or when binary_path is NULL/empty. */
char *lsm_client_adapter_opencode(const char *binary_path);
...
bool lsm_client_adapter_escape_js(const char *in, char *out, size_t out_sz);

#endif /* LSM_CLI_CLIENT_ADAPTER_H */
