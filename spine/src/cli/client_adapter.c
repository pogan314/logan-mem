...
static void sb_append(adapter_sb_t *sb, const char *s) {
    ...
    if (sb->len + add + 1 > sb->cap) {
        size_t want = sb->cap ? sb->cap : LSM_SZ_2K;
...
static void emit_header(adapter_sb_t *sb, const char *client) {
    /* No markers here: lsm_text_upsert_managed_block adds them and REJECTS
     * content that already carries them, so the body must stay marker-free. */
    sb_append(sb, "// Generated by logan-spine-mcp for ");
    sb_append(sb, client);
    sb_append(sb,
              ".\n"
...
char *lsm_client_adapter_pi(const char *binary_path) {
    ...
    char bin[LSM_SZ_1K];
    if (!lsm_client_adapter_escape_js(binary_path, bin, sizeof(bin))) {
        return NULL;
    }
    ...
    sb_append(
        &sb, "async function call(tool, args, signal) {\n"
             ...
             "      env: { ...process.env, LSM_LOG_LEVEL: 'error' },\n"
             "    });\n"
             "    let out = '';\n"
             ...
             "      }\n"
             "      resolve({ error: 'no JSON response from logan-spine-mcp' });\n"
             "    });\n"
             "  });\n"
             "}\n\n");
