 ...
 *
 * Port of Go cmd/logan-spine-mcp/ install/update/config logic.
 *
...
#define LSM_SKILL_COUNT 1

/* Skill name/content pair. */
typedef struct {
    const char *name;    /* e.g. "logan-spine" */
 ...
 * Format: { "mcpServers": { "logan-spine-mcp": { "command": binary_path } } }
 ...
 * Format: { "mcp": { "servers": { "logan-spine-mcp":
 ...
 * Format: { "servers": { "logan-spine-mcp": { "type": "stdio", "command": binary_path } } }
 * Returns 0 on success. */
 ...
 * Format: { "context_servers": { "logan-spine-mcp": { "command": path, "args": [] } } }
 * Returns 0 on success. */
...
bool lsm_cli_clients_apply_selection_for_testing(const char *spec, lsm_detected_agents_t *detected);
...
char *lsm_hook_augment_lifecycle_json_for_dialect(const char *input, const char *forced_event,
                                                  const char *dialect);
char *lsm_hook_augment_tool_json_for_testing(const char *input, const char *dialect,
                                             const char *context, char *path, size_t path_size);
bool lsm_hook_augment_invocation_supported_for_testing(const char *dialect,
...
/* Upsert a logan-spine-mcp instruction section in a markdown file.
 * Uses <!-- logan-spine-mcp:start --> / <!-- logan-spine-mcp:end --> markers.
...
/* Remove the logan-spine-mcp instruction section from a markdown file.
...
typedef void (*lsm_hook_json_prewrite_test_hook_t)(const char *settings_path, void *context);
...
/* ── Codex instructions (legacy, wraps lsm_get_agent_instructions) ── */
...
/* Extract a binary named "logan-spine-mcp*" from a tar.gz buffer.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
unsigned char *lsm_extract_binary_from_targz(const unsigned char *data, int data_len, int *out_len);

/* Extract the logan-spine-mcp binary from a zip archive in memory.
 * Returns malloc'd binary content and sets *out_len.
 * Returns NULL on error. Caller must free. */
...
/* List .db files in the cache directory (~/.cache/logan-spine-mcp/).
...
typedef struct lsm_config lsm_config_t;
...
#define LSM_CONFIG_AUTO_INDEX "auto_index"
#define LSM_CONFIG_AUTO_INDEX_LIMIT "auto_index_limit"
#define LSM_CONFIG_AUTO_WATCH "auto_watch"
#define LSM_CONFIG_UI_LANG "ui-lang"
...
#define LSM_CONFIG_UI_ENABLED "ui_enabled"
#define LSM_CONFIG_UI_PORT "ui_port"

/* ── Binary activation safety ─────────────────────────────────── */

typedef void *lsm_cli_activation_lock_t;
typedef int (*lsm_cli_activation_mutation_fn)(void *context);
 ...
 * quiescence of active modern LSM processes before returning an exclusive
...
typedef int (*lsm_cli_activation_reserve_fn)(void *context, lsm_cli_activation_lock_t *lease_out);
typedef void (*lsm_cli_activation_release_fn)(void *context, lsm_cli_activation_lock_t lease);
...
typedef struct {
    void *context;
    lsm_cli_activation_reserve_fn reserve_for_mutation;
    lsm_cli_activation_release_fn mutation_lease_release;
    void (*visible_diagnostic)(void *context, const char *message);
} lsm_cli_activation_ops_t;
...
typedef enum {
    LSM_HOOK_ADMISSION_DAEMON_ABSENT = 0, /* no daemon running: `daemon start` heals it */
    LSM_HOOK_ADMISSION_BUILD_CONFLICT     /* daemon runs another build: needs `daemon stop` */
} lsm_hook_admission_t;
 ...
 * no-op (#618). */
bool lsm_hook_path_is_abs(const char *path);
