 ...
 */
#ifndef LSM_SEMANTIC_H
#define LSM_SEMANTIC_H
 ...
 * but ~80% precision). */
#define LSM_SEM_EDGE_THRESHOLD 0.75

/* Maximum SEMANTICALLY_RELATED edges per node. */
enum { LSM_SEM_MAX_EDGES = 10 };

/* AST structural profile: 25 float features per function (control flow,
 * nesting, expression types, literals, data flow, Halstead). */
enum { LSM_SEM_AST_PROFILE_DIMS = 25 };

/* MinHash fingerprint length (must match simhash/minhash.h LSM_MINHASH_K). */
enum { LSM_SEM_MINHASH_K = 64 };
 ...
 * Call this BEFORE dispatching parallel work that invokes lsm_sem_random_index,
...
typedef struct {
    ...
    lsm_rsq_code_t ri_code;
    lsm_rsq_code_t api_code;
    lsm_rsq_code_t type_code;
    lsm_rsq_code_t deco_code;

    /* AST profile as float vector (decoded from "sp" property). */
    float struct_profile[LSM_SEM_AST_PROFILE_DIMS];

    /* MinHash fingerprint (decoded from "fp" property). */
    bool has_minhash;
    uint32_t minhash[LSM_SEM_MINHASH_K];
} lsm_sem_func_t;

/* ── Corpus-level data ───────────────────────────────────────────── */

/* Opaque corpus handle for IDF and Random Indexing state. */
typedef struct lsm_sem_corpus lsm_sem_corpus_t;
 ...
 * This replaces a loop of lsm_sem_corpus_add_doc() calls. */
