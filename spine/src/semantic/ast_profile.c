...
static void accumulate_control_flow(const char *kind, lsm_ast_profile_t *out, bool *in_return) {
...
static void accumulate_expressions(const char *kind, lsm_ast_profile_t *out) {
...
static void accumulate_halstead(const char *kind, uint32_t child_count, uint32_t *op_set,
                                uint32_t *operand_set, lsm_ast_profile_t *out) {
...
static void accumulate_data_flow(TSNode node, const char *kind, uint32_t child_count,
                                 ...
                                 bool in_return, bool in_condition, lsm_ast_profile_t *out) {
    if (!(child_count == 0 && is_identifier(kind) && source)) {
        return;
    }
    uint32_t start = ts_node_start_byte(node);
    uint32_t end = ts_node_end_byte(node);
    if (end <= start || (end - start) >= LSM_SZ_128) {
        return;
    }
    char ident_buf[LSM_SZ_128];
...
bool lsm_ast_profile_compute(TSNode func_body, const char *source, const char **param_names,
                             int param_count, lsm_ast_profile_t *out) {
...
void lsm_ast_profile_to_str(const lsm_ast_profile_t *p, char *buf, int bufsize) {
...
bool lsm_ast_profile_from_str(const char *str, lsm_ast_profile_t *out) {
    if (!str || !out) {
        return false;
    }
    memset(out, 0, sizeof(*out));
    /* Field layout must match lsm_ast_profile_to_str() exactly. */
...
void lsm_ast_profile_to_vector(const lsm_ast_profile_t *p, float *out) {
    if (!p || !out) {
        return;
    }
