...
static void check_pressure(size_t allocated) {
    ...
    if (over && !was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (allocated * LSM_PERCENT) / g_budget : 0);
        lsm_log_warn("mem.pressure.warn", "allocated_mb", alloc_mb, "budget_mb", budget_mb, "pct",
                     pct_str);
    } else if (!over && was) {
        ...
        snprintf(pct_str, sizeof(pct_str), "%zu",
                 g_budget > 0 ? (allocated * LSM_PERCENT) / g_budget : 0);
        lsm_log_info("mem.pressure.ok", "allocated_mb", alloc_mb, "budget_mb", budget_mb, "pct",
                     pct_str);
    }
}
...
void lsm_vmem_init(double ram_fraction) {
    ...
    lsm_log_info("vmem.init", "budget_mb", budget_mb, "total_ram_mb", ram_mb);
}
...
void *lsm_vmem_alloc(size_t size) {
    ...
    if (!ptr) {
        lsm_log_error("vmem.alloc.fail", "size_mb", size > MB_DIVISOR ? "large" : "small");
        return NULL;
    }
