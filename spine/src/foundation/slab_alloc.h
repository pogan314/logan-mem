 ...
 *   Matches tree-sitter SubtreeHeapData (LSM_SZ_64 bytes). Backed by
 ...
 *
 * Usage:
 *   lsm_slab_install();         // once, before any parsing
 *   ... parse files ...
 *   lsm_slab_destroy_thread();  // on thread exit — frees owned memory
 */
#ifndef LSM_SLAB_ALLOC_H
#define LSM_SLAB_ALLOC_H
 ...
 * Only safe after lsm_destroy_thread_parser() has been called. */
