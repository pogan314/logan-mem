...
static lsm_yaml_node_t *node_new(yaml_type_t type) {
    lsm_yaml_node_t *n = calloc(LSM_ALLOC_ONE, sizeof(*n));
    if (n) {
        n->type = type;
    }
    return n;
...
static bool node_add_child(lsm_yaml_node_t *parent, lsm_yaml_node_t *child) {
    if (!parent || !child) {
        return false;
    }
    if (parent->child_count >= parent->child_cap) {
        ...
        lsm_yaml_node_t **new_arr = realloc(parent->children, (size_t)new_cap * sizeof(*new_arr));
        if (!new_arr) {
            return false;
        }
...
void lsm_yaml_free(lsm_yaml_node_t *root) {
    if (!root) {
        return;
    }
    enum { YAML_FREE_STACK = 256 };
    lsm_yaml_node_t *stack[YAML_FREE_STACK];
    int top = 0;
    stack[top++] = root;
    while (top > 0) {
        lsm_yaml_node_t *node = stack[--top];
...
typedef struct {
    lsm_yaml_node_t *node;
    int indent;
} stack_entry_t;

/* Parse a list item "- value" and add to the correct parent. */
static void parse_list_item(const char *content, int content_len, lsm_yaml_node_t *parent) {
    ...
    lsm_yaml_node_t *list_parent = parent;
    if (parent->type == YAML_MAP && parent->child_count > 0) {
        lsm_yaml_node_t *last = parent->children[parent->child_count - SKIP_ONE];
        if (last->type == YAML_LIST) {
            list_parent = last;
        }
    }

    lsm_yaml_node_t *item = node_new(YAML_SCALAR);
    if (item) {
...
static void parse_key_line(const char *content, int content_len, int indent,
                           lsm_yaml_node_t *parent, stack_entry_t *stack, int *stack_depth,
    ...
    if (after_len > 0) {
        /* "key: value" — scalar */
        lsm_yaml_node_t *child = node_new(YAML_SCALAR);
        if (child) {
            child->key = key;
        ...
        lsm_yaml_node_t *child = node_new(is_list ? YAML_LIST : YAML_MAP);
        if (child) {
            child->key = key;
            if (!node_add_child(parent, child)) {
                lsm_yaml_free(child);
            } else if (*stack_depth < LSM_SZ_32) {
...
lsm_yaml_node_t *lsm_yaml_parse(const char *text, int len) {
    if (!text || len <= 0) {
        return node_new(YAML_MAP);
    }

    lsm_yaml_node_t *root = node_new(YAML_MAP);
    if (!root) {
        return NULL;
    }

    /* Stack for tracking parent context */
    stack_entry_t stack[LSM_SZ_32];
    int stack_depth = 0;
    ...
    while (p < end) {
        ...
        lsm_yaml_node_t *parent = stack[stack_depth - SKIP_ONE].node;
...
static const lsm_yaml_node_t *find_child(const lsm_yaml_node_t *node, const char *key) {
    if (!node || !key) {
        return NULL;
    }
...
static const lsm_yaml_node_t *navigate(const lsm_yaml_node_t *root, const char *path) {
    if (!root || !path) {
        return NULL;
    }

    const lsm_yaml_node_t *cur = root;
    char buf[LSM_SZ_256];
    const char *p = path;
...
const char *lsm_yaml_get_str(const lsm_yaml_node_t *root, const char *path) {
    const lsm_yaml_node_t *node = navigate(root, path);
...
double lsm_yaml_get_float(const lsm_yaml_node_t *root, const char *path, double default_val) {
    const char *str = lsm_yaml_get_str(root, path);
    if (!str) {
        return default_val;
    }
    char *endptr;
...
bool lsm_yaml_get_bool(const lsm_yaml_node_t *root, const char *path, bool default_val) {
    const char *str = lsm_yaml_get_str(root, path);
    if (!str) {
        return default_val;
    }
...
int lsm_yaml_get_str_list(const lsm_yaml_node_t *root, const char *path, const char **out,
                          int max_out) {
    const lsm_yaml_node_t *node = navigate(root, path);
...
bool lsm_yaml_has(const lsm_yaml_node_t *root, const char *path) {
