 ...
 *
 * Usage:
 *   LSM_DYN_ARRAY(int) nums = {0};    // zero-init
 *   lsm_da_push(&nums, 42);
 *   lsm_da_push(&nums, 99);
 ...
 *   - Uses realloc — NOT arena-compatible (use LSMDefArray etc. for arena)
 *   - Header-only: no .c file needed
 */
#ifndef LSM_DYN_ARRAY_H
#define LSM_DYN_ARRAY_H

#include <stdlib.h>
#include <string.h>

/* Declare a dynamic array type for a given element type. */
#define LSM_DYN_ARRAY(T) \
    struct {             \
        T *items;        \
        int count;       \
        int cap;         \
...
#define lsm_da_pop(da) ((da)->items[--(da)->count])

/* Get last element without removing. Undefined if empty. */
#define lsm_da_last(da) ((da)->items[(da)->count - 1])

/* Clear without freeing (reset count to 0). */
#define lsm_da_clear(da) ((da)->count = 0)

/* Free all memory. */
#define lsm_da_free(da)     \
    do {                    \
        free((da)->items);  \
        (da)->items = NULL; \
        (da)->count = 0;    \
...
#define lsm_da_insert(da, idx, item)                                           \
    do {                                                                       \
        lsm_da_push(da, item); /* ensure space */                              \
        if ((idx) < (da)->count - 1) {                                         \
