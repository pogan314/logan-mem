#ifndef LSM_FOUNDATION_WORKSPACE_H
#define LSM_FOUNDATION_WORKSPACE_H
 ...
 * of LSM_WS_ALLOW here means "this path is not obviously too broad", never
...
typedef enum {
    ...
    LSM_WS_DENY_TOO_SHALLOW,
    /* A filesystem root, a drive root, a UNC share root, or a path that is or
     * contains the lsm cache directory. Never overridable. */
    LSM_WS_DENY_ABSOLUTE,
 ...
 * human override for a LSM_WS_DENY_SENSITIVE path; absolute denials and shallow
 ...
 * LSM_ALLOWED_ROOT / session policy, treated as an additional grant, or NULL.
...
#endif /* LSM_FOUNDATION_WORKSPACE_H */

/*
 * ── Per-project request manifest (.lsmpathwhitelist) ─────────────────────────
 *
...
#define LSM_WS_MANIFEST_NAME ".lsmpathwhitelist"

enum { LSM_WS_MANIFEST_MAX_ENTRIES = 64 };

typedef struct {
    /* Requested roots, verbatim from the file (already control-char screened). */
    char entries[LSM_WS_MANIFEST_MAX_ENTRIES][1024];
    int count;
...
/* Read <project_root>/.lsmpathwhitelist. Returns false only on a malformed file
