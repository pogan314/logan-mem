...
typedef enum {
    LSM_READ_OK = 0,    /* file read successfully */
    LSM_READ_OPEN_FAIL, /* could not open (missing / permission) */
    LSM_READ_EMPTY,     /* zero/negative size — benign, nothing to index */
    LSM_READ_OVERSIZED, /* size exceeds lsm_max_file_bytes() */
    LSM_READ_OOM,       /* buffer allocation failed */
} lsm_read_status_t;
 ...
 * never silently dropped. Override with LSM_MAX_FILE_BYTES (a positive integer
 ...
 * LSM_CYPHER_MAX_DEPTH (a positive integer). Default 10. */
 ...
 * Override with LSM_MCP_MAX_DEPTH (a positive integer). Default 15. */
