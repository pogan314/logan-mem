...
/* Write through the descriptor lsm_mkstemp returned rather than reopening its
...
static const tool_def_t TOOLS[] = {
    {"index_repository", "Index repository",
     ...
     "deliberately NOT indexed (gitignore/.lsmignore/skip-lists) — by design, not failures.",
     ...
     "\"Write compressed artifact to .logan-spine/graph.db.zst for team sharing. "
    ...
    {"index_status", "Index status",
     ...
     "BY DESIGN (gitignore/.lsmignore/skip-lists): deliberate and deterministic, not failures; "
...
static bool mcp_tool_allowed(lsm_mcp_tool_profile_t profile, const char *name) {
...
static const char *mcp_tool_profile_name(lsm_mcp_tool_profile_t profile) {
    return profile == LSM_MCP_TOOL_PROFILE_SCOUT ? "scout" : "analysis";
}
...
int lsm_mcp_parse_tool_profile_args(int argc, const char *const argv[const],
    ...
    *profile_out = LSM_MCP_TOOL_PROFILE_ALL;
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "--tool-profile=analysis") == 0) {
            *profile_out = LSM_MCP_TOOL_PROFILE_ANALYSIS;
            continue;
        }
        if (strcmp(arg, "--tool-profile=scout") == 0) {
            *profile_out = LSM_MCP_TOOL_PROFILE_SCOUT;
            continue;
        }
...
bool lsm_mcp_tool_profile_allows_http(lsm_mcp_tool_profile_t profile) {
    return profile == LSM_MCP_TOOL_PROFILE_ALL;
}

static int mcp_allowed_tool_count(lsm_mcp_tool_profile_t profile) {
    int count = 0;
...
static char *lsm_mcp_tools_list_range(lsm_mcp_tool_profile_t profile, int offset, int limit,
...
char *lsm_mcp_tools_list(void) {
    return lsm_mcp_tools_list_range(LSM_MCP_TOOL_PROFILE_ALL, 0, TOOL_COUNT, false);
}
...
static char *lsm_mcp_tools_list_page(lsm_mcp_tool_profile_t profile, const char *params_json) {
    bool has_cursor = false;
    int offset = mcp_tools_cursor_offset(params_json, &has_cursor);
    if (!has_cursor) {
        return lsm_mcp_tools_list_range(profile, 0, TOOL_COUNT, false);
    }
    return lsm_mcp_tools_list_range(profile, offset, MCP_TOOLS_PAGE_SIZE, true);
}
...
static char *lsm_mcp_prompts_list(void) {
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
static char *lsm_mcp_prompt_get(const char *params_json, char **error_json) {
    *error_json = NULL;
...
static char *lsm_mcp_initialize_response_for_profile(const char *params_json,
                                                     lsm_mcp_tool_profile_t profile) {
    ...
    yyjson_mut_obj_add_str(doc, impl, "name", "logan-spine-mcp");
    yyjson_mut_obj_add_str(doc, impl, "version", lsm_cli_get_version());
...
char *lsm_mcp_initialize_response(const char *params_json) {
    return lsm_mcp_initialize_response_for_profile(params_json, LSM_MCP_TOOL_PROFILE_ALL);
}
...
static char *canonicalize_repo_path_if_exists(char *repo_path) {
    ...
    if (lsm_canonical_path(repo_path, real, sizeof(real))) {
...
static char *normalize_project_arg(char *project) {
    ...
    char *normalized = lsm_project_name_from_path(project);
    if (normalized) {
        free(project);
        return normalized;
    }
...
bool lsm_validate_project_name(const char *project);

/* #1025: agents naturally pass the repo FOLDER name ("logan-spine-mcp"),
...
static char *resolve_project_tail(char *project) {
    ...
    lsm_closedir(d);
    if (matches == 1) {
        lsm_log_info("mcp.project_tail_resolved", "passed", project, "resolved", match);
        free(project);
...
static char *get_project_arg(const char *args_json) {
    char *p = lsm_mcp_get_string_arg(args_json, "project");
    if (!p) {
        p = lsm_mcp_get_string_arg(args_json, "project_name");
    }
    if (!p) {
        p = lsm_mcp_get_string_arg(args_json, "project_id");
    }
    if (!p) {
        p = lsm_mcp_get_string_arg(args_json, "projectName");
    }
...
struct lsm_mcp_server {
    lsm_store_t *store;     /* currently open project store (or NULL) */
    ...
    char session_root[LSM_SZ_1K];     /* detected project root path */
    char session_project[LSM_SZ_256]; /* derived project name */
    ...
    struct lsm_watcher *watcher;      /* external watcher ref (not owned) */
    struct lsm_config *config;        /* external config ref (not owned) */
    lsm_mcp_index_executor_fn index_executor;
    void *index_executor_context;
    lsm_proc_log_cb index_log_callback;
    void *index_log_context;
    lsm_mcp_project_mutation_begin_fn mutation_begin;
    lsm_mcp_project_mutation_try_begin_fn mutation_try_begin;
    lsm_mcp_project_mutation_end_fn mutation_end;
    void *mutation_context;
    lsm_mcp_quarantine_test_hook_fn quarantine_test_hook;
    void *quarantine_test_context;
    lsm_mcp_command_test_hook_fn command_test_hook;
    void *command_test_context;
    size_t search_output_limit_override;
    lsm_thread_t autoindex_tid;
    ...
    lsm_mutex_t request_scope_mutex;
    unsigned int request_scope_depth;
    atomic_int pipeline_cancel_requested;
    lsm_pipeline_t *active_pipeline; /* non-NULL while index_repository runs */
...
lsm_mcp_server_t *lsm_mcp_server_new(const char *store_path) {
    lsm_mcp_server_t *srv = calloc(LSM_ALLOC_ONE, sizeof(*srv));
    if (!srv) {
        return NULL;
    }
    lsm_mutex_init(&srv->request_scope_mutex);
    ...
    if (store_path) {
        srv->store = lsm_store_open(store_path);
        srv->current_project = heap_strdup(store_path);
    } else {
        srv->store = lsm_store_open_memory();
    }
    srv->owns_store = true;
    srv->tool_profile = LSM_MCP_TOOL_PROFILE_ALL;
...
bool lsm_mcp_server_set_session_context(lsm_mcp_server_t *srv, const char *session_root,
    ...
    char *project = lsm_project_name_from_path(session_root);
...
void lsm_mcp_server_set_project_mutation_try_guard(
    lsm_mcp_server_t *srv, lsm_mcp_project_mutation_try_begin_fn try_begin) {
    if (srv && srv->mutation_begin) {
        srv->mutation_try_begin = try_begin;
    }
}

static bool mcp_project_mutation_begin(lsm_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin || srv->mutation_begin(srv->mutation_context, project);
}

static bool mcp_project_mutation_try_begin(lsm_mcp_server_t *srv, const char *project) {
    return !srv->mutation_begin ||
           (srv->mutation_try_begin && srv->mutation_try_begin(srv->mutation_context, project));
}

static void mcp_project_mutation_end(lsm_mcp_server_t *srv, const char *project) {
    if (srv->mutation_end) {
...
void lsm_mcp_server_free(lsm_mcp_server_t *srv) {
    ...
    if (srv->autoindex_active) {
        lsm_thread_join(&srv->autoindex_tid);
    }
    if (srv->owns_store && srv->store) {
        lsm_store_close(srv->store);
    }
    free(srv->current_project);
    free(srv->allowed_root);
    free(srv->active_request_id_str);
    lsm_mutex_destroy(&srv->request_scope_mutex);
    free(srv);
}
...
bool lsm_mcp_server_release_pristine_memory_store(lsm_mcp_server_t *srv) {
    const char *db_path = srv && srv->store ? lsm_store_db_path(srv->store) : NULL;
...
lsm_pipeline_t *lsm_mcp_server_active_pipeline(lsm_mcp_server_t *srv) {
...
static bool mcp_request_cancelled(const lsm_mcp_server_t *srv) {
...
static const char *project_db_path(const char *project, char *buf, size_t bufsz) {
    if (!lsm_validate_project_name(project)) {
        buf[0] = '\0';
        return buf;
    }
    char dir[LSM_SZ_1K];
 ...
 * handle is transferred to the caller (who must lsm_store_close it). On failure
...
static lsm_store_t *resolve_store_fallback_scan(const char *project);
...
static bool reserve_unique_corrupt_pending(const char *path, char *pending, size_t pending_size,
    ...
    for (unsigned int attempt = 0; attempt < 128; attempt++) {
        uint64_t token = lsm_now_ns() ^ ((uint64_t)(unsigned int)getpid() << 32) ^
        ...
        if (lsm_file_exists(backup)) {
            continue;
        }
#ifdef _WIN32
        wchar_t *wide = lsm_path_to_wide(pending);
...
static void discard_corrupt_pending(const char *pending) {
    ...
    (void)lsm_remove_db_sidecars(pending);
    (void)lsm_unlink(pending);
}

#ifndef _WIN32
static bool sync_parent_directory(const char *path) {
    char directory[LSM_SZ_2K];
...
#ifdef _WIN32
    wchar_t *wide_pending = lsm_path_to_wide(pending);
    wchar_t *wide_backup = lsm_path_to_wide(backup);
...
static bool quarantine_step_allowed(lsm_mcp_server_t *srv, const char *step) {
...
static bool quarantine_corrupt_store(lsm_mcp_server_t *srv, const char *project, const char *path,
                                     char *backup_out, size_t backup_out_size) {
    char backup[LSM_SZ_2K];
    char pending[LSM_SZ_2K];
    ...
    if (!path || !path[0]) {
        lsm_log_error("store.auto_clean_failed", "project", project, "path", "", "reason",
                      "empty store path");
        return false;
    }
    if (!reserve_unique_corrupt_pending(path, pending, sizeof(pending), backup, sizeof(backup))) {
        lsm_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
                      "cannot reserve unique backup");
        return false;
    }

    if (lsm_store_backup_path(path, pending) != LSM_STORE_OK ||
        lsm_store_prepare_path_for_replace(pending) != LSM_STORE_OK) {
    ...
    lsm_store_t *snapshot = lsm_store_open_path_query(pending);
    if (!snapshot) {
        discard_corrupt_pending(pending);
        lsm_log_error("store.auto_clean_failed", "project", project, "path", path, "reason",
    ...
    if (lsm_remove_db_sidecars(path) != 0) {
        lsm_log_error("store.auto_clean_sidecars", "project", project, "path", path, "reason",
...
static lsm_store_t *resolve_store_internal(lsm_mcp_server_t *srv, const char *project,
    ...
    srv->store = path[0] ? lsm_store_open_path_query(path) : NULL;
    if (srv->store) {
        /* Check DB integrity — back up (never silently delete) a corrupt DB */
        if (!lsm_store_check_integrity(srv->store)) {
            lsm_store_close(srv->store);
            srv->store = NULL;
            ...
            srv->store = lsm_store_open_path_query(path);
            lsm_integrity_verdict_t verdict = srv->store
                                                  ? lsm_store_check_integrity_verdict(srv->store)
                                                  : LSM_INTEGRITY_TRANSIENT;
            bool current_valid = (verdict == LSM_INTEGRITY_OK);
            if (verdict == LSM_INTEGRITY_TRANSIENT) {
            ...
            if (!current_valid) {
                lsm_store_close(srv->store);
                srv->store = NULL;
                char backup[LSM_SZ_2K] = {0};
                bool quarantined =
                    quarantine_corrupt_store(srv, project, path, backup, sizeof(backup));
                lsm_log_error("store.auto_clean", "project", project, "path", path, "action",
        ...
        lsm_project_t proj_verify = {0};
        if (lsm_store_get_project(srv->store, project, &proj_verify) == LSM_STORE_OK) {
            lsm_project_free_fields(&proj_verify);
            srv->owns_store = true;
            free(srv->current_project);
    ...
    lsm_store_t *scanned = resolve_store_fallback_scan(project);
    if (scanned) {
        srv->store = scanned;
        srv->owns_store = true;
        free(srv->current_project);
...
static lsm_store_t *resolve_store(lsm_mcp_server_t *srv, const char *project) {
...
static void free_node_contents(lsm_node_t *n);
...
static int collect_db_project_names(const char *dir_path, char *out, size_t out_sz) {
    int count = 0;
    int offset = 0;
    lsm_dir_t *d = lsm_opendir(dir_path);
    if (!d) {
        return 0;
    }
    lsm_dirent_t *entry;
...
static void add_git_context_json(yyjson_mut_doc *doc, yyjson_mut_val *obj, const char *root_path) {
    ...
    (void)lsm_git_context_resolve(root_path, &ctx);
...
static char *build_project_list_error(const char *reason) {
    char dir_path[LSM_SZ_1K];
    cache_dir(dir_path, sizeof(dir_path));

    char projects[LSM_SZ_4K] = "";
...
#define REQUIRE_STORE(store, project)                     \
    do {                                                  \
        if (!(store)) {                                   \
            ...
            char *_res = lsm_mcp_text_result(_err, true); \
            free(_err);                                   \
            free(project);                                \
            return _res;                                  \
        }                                                 \
...
static bool project_has_adr(lsm_store_t *store, const char *project, const char *root_path) {
    if (store && project) {
        lsm_adr_t adr;
        memset(&adr, 0, sizeof(adr));
        if (lsm_store_adr_get(store, project, &adr) == LSM_STORE_OK) {
    ...
    char adr_path[LSM_SZ_4K];
    snprintf(adr_path, sizeof(adr_path), "%s/.logan-spine/adr.md", root_path);
    struct stat adr_st;
 ...
 * "tmp-" (lsm_project_name_from_path: "/tmp/bench/..." → "tmp-bench-...";
...
static bool db_internal_project_name(const char *full_path, char *name_out, size_t name_sz,
    ...
    lsm_store_t *st = lsm_store_open_path_query(full_path);
    if (!st) {
        return false; /* nonexistent / unreadable */
    }
    lsm_project_t *projs = NULL;
    int n = 0;
    bool ok = false;
    if (lsm_store_list_projects(st, &projs, &n) == LSM_STORE_OK) {
    ...
    lsm_store_free_projects(projs, n);
    if (ok && out_store) {
...
static void build_project_json_entry(yyjson_mut_doc *doc, yyjson_mut_val *arr, const char *dir_path,
    ...
    char project_name[LSM_SZ_1K];
    lsm_store_t *pstore = NULL;
    ...
    int edges = 0;
    if (include_details) {
        nodes = lsm_store_count_nodes(pstore, project_name);
        edges = lsm_store_count_edges(pstore, project_name);
    }
    char root_path_buf[LSM_SZ_1K] = "";
    lsm_project_t proj = {0};
    if (lsm_store_get_project(pstore, project_name, &proj) == LSM_STORE_OK) {
        if (proj.root_path) {
            snprintf(root_path_buf, sizeof(root_path_buf), "%s", proj.root_path);
        }
        lsm_project_free_fields(&proj);
    }
    lsm_store_close(pstore);
    ...
    if (include_details && root_path_buf[0]) {
        lsm_git_context_t gctx = {0};
        (void)lsm_git_context_resolve(root_path_buf, &gctx);
        if (gctx.is_git && gctx.branch) {
            yyjson_mut_obj_add_strcpy(doc, p, "branch", gctx.branch);
        }
        lsm_git_context_free(&gctx);
    }
    if (include_details) {
...
static char *handle_list_projects(lsm_mcp_server_t *srv, const char *args) {
    (void)srv;
    int offset = lsm_mcp_get_int_arg(args, "offset", 0);
    int limit = lsm_mcp_get_int_arg(args, "limit", 50);
    bool include_details = lsm_mcp_get_bool_arg(args, "include_details");
    ...
    if (!d) {
        ...
        snprintf(msg, sizeof(msg),
                 ...
                 dir_path);
        yyjson_mut_doc_free(doc);
        return lsm_mcp_text_result(msg, true);
    }
    ...
    for (size_t i = start; i < end; i++) {
        ...
        char full_path[LSM_SZ_2K];
        snprintf(full_path, sizeof(full_path), "%s/%s", dir_path, name);
        int64_t size_bytes = lsm_file_size(full_path);
        if (size_bytes < 0) {
            continue;
        }
    ...
    char *result = lsm_mcp_text_result(json, false);
    free(json);
    return result;
}
 ...
 * resolve_store uses lsm_store_open_path_query (no SQLITE_OPEN_CREATE), so
...
static char *verify_project_indexed(lsm_store_t *store, const char *project) {
    lsm_project_t proj_check = {0};
    if (lsm_store_get_project(store, project, &proj_check) != LSM_STORE_OK) {
        char *err = build_project_list_error("project not indexed — run index_repository first");
        char *res = lsm_mcp_text_result(err, true);
        free(err);
        return res;
    }
    lsm_project_free_fields(&proj_check);
    return NULL;
}

static bool sg_field_blocked(const char *f); /* internal-only fields, defined with search_graph */

static char *handle_get_graph_schema(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    ...
    lsm_schema_info_t schema = {0};
    lsm_store_get_schema(store, project, &schema);
    ...
    /* Check ADR presence */
    lsm_project_t proj_info = {0};
    if (lsm_store_get_project(store, project, &proj_info) == 0 && proj_info.root_path) {
        ...
        lsm_project_free_fields(&proj_info);
    }
...
static void enrich_add_bfs(yyjson_mut_doc *doc, yyjson_mut_val *arr, lsm_traverse_result_t *tr) {
...
static yyjson_mut_val *enrich_connected(yyjson_mut_doc *doc, lsm_store_t *store, int64_t node_id,
    ...
    lsm_traverse_result_t tr_in = {0};
    lsm_store_bfs(store, node_id, "inbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_in);
    enrich_add_bfs(doc, conn, &tr_in);
    lsm_store_traverse_free(&tr_in);

    lsm_traverse_result_t tr_out = {0};
    lsm_store_bfs(store, node_id, "outbound", et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr_out);
    enrich_add_bfs(doc, conn, &tr_out);
    lsm_store_traverse_free(&tr_out);
...
static void enrich_connected_joined(lsm_store_t *store, int64_t node_id, const char *relationship,
    ...
    for (int d = 0; d < 2; d++) {
        ...
        lsm_store_bfs(store, node_id, dirs[d], et, SKIP_ONE, SKIP_ONE, MCP_DEFAULT_LIMIT, &tr);
...
static char *bm25_file_pattern_like(const char *file_pattern) {
    ...
    char *like = lsm_glob_to_like(file_pattern);
...
static char *bm25_search(lsm_store_t *store, const char *project, const char *query,
                         const char *file_pattern, int limit, int offset, bool toon) {
    sqlite3 *db = lsm_store_get_db(store);
    if (!db) {
        return NULL;
    }
    ...
    const char *sql =
        ...
        "               WHEN n.label IN (" LSM_SQL_TYPE_LIKE_LABELS ") THEN 5.0 "
    ...
    if (toon) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            lsm_tree_row_begin(&rows);
            lsm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_QN), true);
            lsm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_LABEL),
                              false);
            lsm_tree_cell_str(&rows, (const char *)sqlite3_column_text(stmt, BM25_COL_FILE), false);
            lsm_tree_cell_str(&rows, lines, false);
            lsm_tree_cell_real(&rows, sqlite3_column_double(stmt, BM25_COL_RANK), false);
            lsm_tree_row_end(&rows);
            emitted++;
        }
        ...
        lsm_sb_t sb;
        lsm_sb_init(&sb);
        lsm_tree_scalar_int(&sb, "total", total);
        lsm_tree_scalar_str(&sb, "search_mode", "bm25");
        static const char *const cols[] = {"qn", "label", "file", "lines", "rank"};
        lsm_tree_table_header(&sb, "results", emitted, cols, 5);
        char *rows_text = lsm_sb_finish(&rows);
        lsm_sb_append(&sb, rows_text ? rows_text : "");
        free(rows_text);
        lsm_tree_scalar_bool(&sb, "has_more", total > offset + emitted);
        return lsm_sb_finish(&sb);
    }
...
static void emit_semantic_results(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                  lsm_vector_result_t *vresults, int vcount) {
 ...
 * *out_vcount (caller frees via lsm_store_free_vector_results when vcount>0).
 * Returns true if semantic_query was provided as a non-array (type error —
 * caller should surface to the user). */
static bool run_semantic_query_core(const char *args, lsm_store_t *store, const char *project,
                                    int limit, lsm_vector_result_t **out_vresults, int *out_vcount,
                                    bool *out_present) {
    ...
    *out_vcount = 0;
    ...
    if (sq_val && !yyjson_is_arr(sq_val)) {
        ...
        lsm_vector_result_t *vresults = NULL;
        int vcount = 0;
        int sem_limit = limit > 0 ? limit : LSM_SZ_16;
        if (lsm_store_vector_search(store, project, keywords, ki, sem_limit, &vresults, &vcount) ==
                LSM_STORE_OK &&
            vcount > 0) {
...
static void sg_toon_property_cell(lsm_sb_t *sb, yyjson_val *v) {
    if (v && yyjson_is_str(v)) {
        lsm_tree_cell_str(sb, yyjson_get_str(v), false);
    } else if (v && yyjson_is_bool(v)) {
        lsm_tree_cell_bool(sb, yyjson_get_bool(v), false);
    } else if (v && yyjson_is_int(v)) {
        lsm_tree_cell_int(sb, yyjson_get_int(v), false);
    } else if (v && yyjson_is_real(v)) {
        lsm_tree_cell_real(sb, yyjson_get_real(v), false);
    } else if (v && !yyjson_is_null(v)) {
        char *json = yyjson_val_write(v, 0, NULL);
        lsm_tree_cell_str(sb, json ? json : "", false);
        free(json);
    } else {
        lsm_tree_cell_str(sb, "", false);
    }
}

/* Append one row's extra-field cells, pulled from the node's properties. */
static void sg_toon_extra_cells(lsm_sb_t *sb, const char *props_json, const char *const *fields,
                                int nfields) {
    yyjson_doc *pd =
...
static void emit_search_results_toon(lsm_sb_t *sb, const lsm_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, bool detail_ids) {
    lsm_tree_scalar_int(sb, "total", out->total);
    if (detail_ids) {
        ...
        lsm_tree_table_header(sb, "results", out->count, id_cols, 1);
        for (int i = 0; i < out->count; i++) {
            lsm_tree_row_begin(sb);
            lsm_tree_cell_str(sb, out->results[i].node.qualified_name, true);
            lsm_tree_row_end(sb);
        }
        lsm_tree_scalar_bool(sb, "has_more", out->total > offset + out->count);
        return;
    }
    ...
    lsm_tree_table_header(sb, "results", out->count, cols, ncols);
    for (int i = 0; i < out->count; i++) {
        const lsm_search_result_t *sr = &out->results[i];
        char lines[LSM_SZ_32];
        sg_lines_str(lines, sizeof(lines), sr->node.start_line, sr->node.end_line);
        lsm_tree_row_begin(sb);
        lsm_tree_cell_str(sb, sr->node.qualified_name, true);
        lsm_tree_cell_str(sb, sr->node.label, false);
        lsm_tree_cell_str(sb, sr->node.file_path, false);
        lsm_tree_cell_str(sb, lines, false);
        lsm_tree_cell_int(sb, sr->in_degree, false);
        lsm_tree_cell_int(sb, sr->out_degree, false);
...
static int sg_cmp_by_qn(const void *pa, const void *pb) {
    const lsm_search_result_t *a = (const lsm_search_result_t *)pa;
    const lsm_search_result_t *b = (const lsm_search_result_t *)pb;
    const char *qa = a->node.qualified_name ? a->node.qualified_name : "";
    const char *qb = b->node.qualified_name ? b->node.qualified_name : "";
    return strcmp(qa, qb);
}

static void emit_search_results_tree(lsm_sb_t *sb, lsm_search_output_t *out, int offset,
                                     const char *const *fields, int nfields, lsm_store_t *store,
                                     const char *relationship, bool include_connected) {
    char buf[LSM_SZ_512];
    char extra_cols[LSM_SZ_256] = "";
    for (int f = 0; f < nfields; f++) {
    ...
    lsm_sb_append(sb, buf);
    /* Sort by qn so same-prefix rows are adjacent (module clustering). */
    if (out->count > 1) {
        qsort(out->results, (size_t)out->count, sizeof(lsm_search_result_t), sg_cmp_by_qn);
    }
    char cur_group[LSM_SZ_1K] = "";
    for (int i = 0; i < out->count; i++) {
        const lsm_search_result_t *sr = &out->results[i];
...
static void emit_search_results_tree_json(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          lsm_search_output_t *out, int offset,
                                          const char *const *fields, int nfields,
                                          lsm_store_t *store, const char *relationship,
...
static void emit_semantic_results_toon(lsm_sb_t *sb, const lsm_vector_result_t *vresults,
                                       int vcount) {
    static const char *const cols[] = {"qn", "label", "file", "score"};
    lsm_tree_table_header(sb, "semantic", vcount, cols, 4);
    for (int v = 0; v < vcount; v++) {
        lsm_tree_row_begin(sb);
        lsm_tree_cell_str(sb, vresults[v].qualified_name, true);
        lsm_tree_cell_str(sb, vresults[v].label, false);
        lsm_tree_cell_str(sb, vresults[v].file_path, false);
        lsm_tree_cell_real(sb, vresults[v].score, false);
        lsm_tree_row_end(sb);
    }
}

static char *handle_search_graph(lsm_mcp_server_t *srv, const char *args) {
    ...
    lsm_mem_phase_mark("handler.args");
    char *project = get_project_arg(args);
    lsm_mem_phase_mark("handler.resolve_store");
    lsm_store_t *store = resolve_store(srv, project);
    lsm_mem_phase_mark("handler.body");
    REQUIRE_STORE(store, project);
    ...
    char *format_arg = lsm_mcp_get_string_arg(args, "format");
    ...
    char *query = lsm_mcp_get_string_arg(args, "query");
    if (query && query[0]) {
        int q_limit = lsm_mcp_get_int_arg(args, "limit", BM25_DEFAULT_LIMIT);
        int q_offset = lsm_mcp_get_int_arg(args, "offset", 0);
        char *q_file_pattern = lsm_mcp_get_string_arg(args, "file_pattern");
        char *bm25_json =
        ...
        if (bm25_json) {
            free(query);
            free(project);
            char *result = lsm_mcp_text_result(bm25_json, false);
            free(bm25_json);
            return result;
        }
    }
    ...
    char *label = lsm_mcp_get_string_arg(args, "label");
    char *name_pattern = lsm_mcp_get_string_arg(args, "name_pattern");
    char *qn_pattern = lsm_mcp_get_string_arg(args, "qn_pattern");
    char *file_pattern = lsm_mcp_get_string_arg(args, "file_pattern");
    char *relationship = lsm_mcp_get_string_arg(args, "relationship");
    bool exclude_entry_points = lsm_mcp_get_bool_arg(args, "exclude_entry_points");
    bool include_connected = lsm_mcp_get_bool_arg(args, "include_connected");
    int limit = lsm_mcp_get_int_arg(args, "limit", LSM_DEFAULT_SEARCH_LIMIT);
    int offset = lsm_mcp_get_int_arg(args, "offset", 0);
    int min_degree = lsm_mcp_get_int_arg(args, "min_degree", LSM_NOT_FOUND);
    int max_degree = lsm_mcp_get_int_arg(args, "max_degree", LSM_NOT_FOUND);
    ...
    if (relationship && !validate_edge_type(relationship)) {
        ...
        free(file_pattern);
        free(relationship);
        return lsm_mcp_text_result("relationship must be uppercase letters and underscores", true);
    }
    ...
    bool has_structural_filters = label || name_pattern || qn_pattern || file_pattern ||
                                  ...
                                  min_degree != LSM_NOT_FOUND || max_degree != LSM_NOT_FOUND;

    if (!legacy_json) {
        const char *fields[SG_MAX_EXTRA_FIELDS];
        yyjson_doc *fields_owner = NULL;
        char *sg_detail = lsm_mcp_get_string_arg(args, "detail");
        ...
        if (!sq_type_error) {
            ...
            lsm_sb_t sb;
            lsm_sb_init(&sb);
            lsm_search_output_t tout = {0};
            if (!semantic_only) {
                lsm_store_search(store, &params, &tout);
            ...
            free(file_pattern);
            free(relationship);
            char *text = lsm_sb_finish(&sb);
            char *result = lsm_mcp_text_result(text ? text : "out of memory", text == NULL);
            free(text);
            return result;
        }
    ...
    if (!semantic_only) {
        lsm_store_search(store, &params, &out);
    }
...
static char *handle_query_graph(lsm_mcp_server_t *srv, const char *args) {
    char *query = lsm_mcp_get_string_arg(args, "query");
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    int max_rows = lsm_mcp_get_int_arg(args, "max_rows", 0);
    ...
    char *graph_arg = lsm_mcp_get_string_arg(args, "graph");
    bool missed_graph = graph_arg && strcmp(graph_arg, "missed") == 0;
    free(graph_arg);

    if (!query) {
        free(project);
        return lsm_mcp_text_result("query is required", true);
    }
    if (missed_graph && !project) {
        free(query);
        return lsm_mcp_text_result("project is required when graph=\"missed\"", true);
    }
    if (!store) {
    ...
    char covproj[LSM_SZ_512];
    const char *cypher_project = project;
    if (missed_graph) {
        lsm_store_coverage_shadow_project(covproj, sizeof(covproj), project);
        cypher_project = covproj;
    }

    lsm_cypher_result_t result = {0};
    int rc = lsm_cypher_execute(store, query, cypher_project, max_rows, &result);

    if (rc < 0) {
        char *err_msg = result.error ? result.error : "query execution failed";
        char *resp = lsm_mcp_text_result(err_msg, true);
    ...
    char *qg_format = lsm_mcp_get_string_arg(args, "format");
    ...
    if (!qg_legacy_json) {
        lsm_sb_t sb;
        lsm_sb_init(&sb);
        lsm_tree_table_header(&sb, "rows", result.row_count, (const char *const *)result.columns,
                              result.col_count);
        for (int r = 0; r < result.row_count; r++) {
            lsm_tree_row_begin(&sb);
            for (int c = 0; c < result.col_count; c++) {
                lsm_tree_cell_str(&sb, result.rows[r][c], c == 0);
            }
            lsm_tree_row_end(&sb);
        }
        lsm_tree_scalar_int(&sb, "total", result.row_count);
        if (result.warning) {
            lsm_tree_scalar_str(&sb, "warning", result.warning);
        }
    ...
    char *res = lsm_mcp_text_result(json ? json : "out of memory", json == NULL);
    free(json);
    return res;
}
...
static void add_coverage_report(yyjson_mut_doc *doc, yyjson_mut_val *root, lsm_store_t *store,
                                const char *project) {
    lsm_coverage_row_t *rows = NULL;
    int count = 0;
    (void)lsm_store_coverage_get(store, project, &rows, &count);
    ...
    lsm_store_free_coverage(rows, count);
    ...
    if (ni_dir_n > 0 || ni_file_n > 0) {
        yyjson_mut_obj_add_str(doc, ni, "note",
                               ...
                               "gitignore/.lsmignore/skip-lists (see each file's reason). Not an "
...
bool lsm_path_within_root(const char *root_path, const char *abs_path); /* defined below */
...
#ifdef __APPLE__
    return ((int64_t)st->st_mtimespec.tv_sec * (int64_t)LSM_NSEC_PER_SEC) +
           (int64_t)st->st_mtimespec.tv_nsec;
#elif defined(_WIN32)
    return (int64_t)st->st_mtime * (int64_t)LSM_NSEC_PER_SEC;
#else
    return ((int64_t)st->st_mtim.tv_sec * (int64_t)LSM_NSEC_PER_SEC) + (int64_t)st->st_mtim.tv_nsec;
#endif
}

static const char *coverage_path_freshness(lsm_store_t *store, const char *project,
    ...
    *outside = false;
    ...
    if (!lsm_path_within_root(root_path, abs_path)) {
        *outside = true;
        return "outside_project";
    }

    lsm_file_hash_t hash = {0};
    int rc = lsm_store_get_file_hash(store, project, rel_path, &hash);
    if (rc == LSM_STORE_NOT_FOUND) {
        return "not_tracked";
    }
    if (rc != LSM_STORE_OK) {
        return "unavailable";
    ...
    lsm_store_clear_file_hash(&hash);
...
static void coverage_add_row_json(yyjson_mut_doc *doc, yyjson_mut_val *array,
                                  const lsm_coverage_row_t *row, const char *requested_path) {
    yyjson_mut_val *item = yyjson_mut_obj(doc);
...
static const char *coverage_status(const lsm_coverage_row_t *rows, int count,
...
static char *handle_check_index_coverage(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
    ...
    lsm_project_t proj = {0};
    bool have_project = lsm_store_get_project(store, project, &proj) == LSM_STORE_OK;
    lsm_coverage_meta_t meta = {0};
    bool have_meta = lsm_store_coverage_meta_get(store, project, &meta) == LSM_STORE_OK;
    ...
    if (paths) {
        yyjson_arr_foreach(paths, idx, max, value) {
            ...
            lsm_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = lsm_store_coverage_get_path(store, project, rel, &rows, &row_count);
            bool lookup_ok = cov_rc == LSM_STORE_OK || cov_rc == LSM_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
            ...
            lsm_store_free_coverage(rows, row_count);
            yyjson_mut_arr_add_val(path_results, item);
        }
    }
    yyjson_mut_obj_add_val(doc, root, "paths", path_results);

    int scope_limit = lsm_mcp_get_int_arg(args, "scope_limit", COVERAGE_SCOPE_DEFAULT_LIMIT);
    int scope_offset = lsm_mcp_get_int_arg(args, "scope_offset", 0);
    if (scope_limit < 1) {
        scope_limit = 1;
    ...
    if (scopes) {
        yyjson_arr_foreach(scopes, idx, max, value) {
            ...
            lsm_coverage_row_t *rows = NULL;
            int row_count = 0;
            int cov_rc = lsm_store_coverage_get_scope(store, project, scope, &rows, &row_count);
            bool lookup_ok = cov_rc == LSM_STORE_OK || cov_rc == LSM_STORE_NOT_FOUND;
            if (!lookup_ok) {
                row_count = 0;
                yyjson_mut_obj_add_str(doc, item, "coverage_lookup", "error");
...
static char *handle_index_status(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    REQUIRE_STORE(store, project);
     ...
     * stays lean. */
    bool verbose = lsm_mcp_get_bool_arg(args, "verbose");
    ...
    if (project) {
        int nodes = lsm_store_count_nodes(store, project);
        int edges = lsm_store_count_edges(store, project);
        yyjson_mut_obj_add_str(doc, root, "project", project);
        yyjson_mut_obj_add_int(doc, root, "nodes", nodes);
        yyjson_mut_obj_add_int(doc, root, "edges", edges);
        yyjson_mut_obj_add_str(doc, root, "status", nodes > 0 ? "ready" : "empty");
        lsm_project_t proj_info = {0};
        if (lsm_store_get_project(store, project, &proj_info) == LSM_STORE_OK) {
...
static char *handle_delete_project(lsm_mcp_server_t *srv, const char *args) {
    char *name = get_project_arg(args);
    if (!name) {
        return lsm_mcp_text_result("project is required", true);
    }
    if (!mcp_project_mutation_begin(srv, name)) {
        free(name);
        return lsm_mcp_text_result("project operation cancelled or blocked by an active index",
                                   true);
    }
    ...
    if (srv->watcher) {
        lsm_watcher_unwatch(srv->watcher, name);
    }

    lsm_mem_collect(); /* return freed pages to OS after closing database */
    ...
    char *result = lsm_mcp_text_result(json, is_error);
    free(json);
    return result;
}
 ...
 * lsm_store_arch_aspect_in_overview in store.c), or the aspect name itself. */
...
static bool aspect_wanted(yyjson_doc *aspects_doc, yyjson_val *aspects_arr, const char *name) {
    ...
    while ((val = yyjson_arr_iter_next(&iter)) != NULL) {
        ...
        if (strcmp(s, "overview") == 0 && lsm_store_arch_aspect_in_overview(name)) {
            return true;
        }
        if (strcmp(s, name) == 0) {
            return true;
...
static void append_cross_repo_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const lsm_schema_info_t *schema) {
 ...
 * LSM_STORE_OK, or LSM_STORE_ERR on failure (all outs zeroed). */
enum { ARCH_SCC_MAX_EDGES = 400000, ARCH_SCC_MAX_CYCLES = 100, ARCH_SCC_MEMBERS_SHOWN = 20 };

static int arch_compute_cycles(lsm_store_t *store, const char *project, int64_t ***out_members,
    ...
    *edges_truncated = false;
    ...
    int64_t *tgt = NULL;
    int ecount = 0;
    if (lsm_store_fetch_call_edges(store, project, ARCH_SCC_MAX_EDGES, &src, &tgt, &ecount,
                                   edges_truncated) != LSM_STORE_OK) {
    ...
    *scanned_edges = ecount;
    ...
    if (!scc_build(src, tgt, ecount, &g)) {
        free(src);
        free(tgt);
        return LSM_STORE_OK; /* no edges = no cycles, not an error */
    }
    free(src);
    free(tgt);
...
static void arch_node_qn(lsm_store_t *store, int64_t id, char *out, size_t outsz) {
    lsm_node_t n = {0};
    if (lsm_store_find_node_by_id(store, id, &n) == LSM_STORE_OK && n.qualified_name) {
...
static char *handle_get_architecture(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    ...
    REQUIRE_STORE(store, project);
    char *scope_path = lsm_mcp_get_string_arg(args, "path");
    ...
    /* Build a C string array from aspects for lsm_store_get_architecture.
    ...
    for (int i = 0; i < aspects_strs_count; i++) {
        if (!aspect_is_valid(aspects_strs[i])) {
            ...
            snprintf(msg, sizeof(msg), "Unknown aspect '%s'. Valid: %s.", aspects_strs[i],
                     valid_list);
            char *err = lsm_mcp_text_result(msg, true);
            free(project);
            free(scope_path);
            if (aspects_doc) {
    ...
    lsm_store_get_schema_counts_scoped(store, project, scope_path, &schema);

    lsm_architecture_info_t arch = {0};
    lsm_store_get_architecture(store, project, scope_path, aspects_strs, aspects_strs_count, &arch);

    int node_count = lsm_store_count_nodes_scoped(store, project, scope_path);
    int edge_count = lsm_store_count_edges_scoped(store, project, scope_path);
    char norm_path[LSM_SZ_512];
    bool path_scoped = lsm_store_normalize_arch_path(scope_path, norm_path, sizeof(norm_path));

    /* Response encoding: tree tables by default; format:"json" emits the
     * same model as structured JSON ({cols, rows} per section). */
    char *arch_format = lsm_mcp_get_string_arg(args, "format");
    ...
    if (!arch_legacy_json) {
        ...
        lsm_sb_init(&sb);
        if (project) {
            lsm_tree_scalar_str(&sb, "project", project);
        }
        if (default_summary) {
            lsm_tree_scalar_str(&sb, "aspects_hint",
        ...
        if (path_scoped) {
            lsm_tree_scalar_str(&sb, "path", norm_path);
            lsm_tree_scalar_int(&sb, "root_total_nodes", lsm_store_count_nodes(store, project));
            lsm_tree_scalar_int(&sb, "root_total_edges", lsm_store_count_edges(store, project));
            lsm_tree_scalar_int(&sb, "scoped_total_nodes", node_count);
            lsm_tree_scalar_int(&sb, "scoped_total_edges", edge_count);
        }
        lsm_tree_scalar_int(&sb, "total_nodes", node_count);
        lsm_tree_scalar_int(&sb, "total_edges", edge_count);

        if (aspect_wanted(aspects_doc, aspects_arr, "structure") && schema.node_label_count > 0) {
            static const char *const lcols[] = {"label", "count"};
            lsm_tree_table_header(&sb, "node_labels", schema.node_label_count, lcols, 2);
            for (int i = 0; i < schema.node_label_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, schema.node_labels[i].label, true);
                lsm_tree_cell_int(&sb, schema.node_labels[i].count, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "dependencies") && schema.edge_type_count > 0) {
            static const char *const tcols[] = {"type", "count"};
            lsm_tree_table_header(&sb, "edge_types", schema.edge_type_count, tcols, 2);
            for (int i = 0; i < schema.edge_type_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, schema.edge_types[i].type, true);
                lsm_tree_cell_int(&sb, schema.edge_types[i].count, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (aspect_wanted(aspects_doc, aspects_arr, "routes") && schema.rel_pattern_count > 0) {
            static const char *const pcols[] = {"pattern"};
            lsm_tree_table_header(&sb, "relationship_patterns", schema.rel_pattern_count, pcols, 1);
            for (int i = 0; i < schema.rel_pattern_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, schema.rel_patterns[i], true);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.language_count > 0) {
            static const char *const gcols[] = {"language", "files"};
            lsm_tree_table_header(&sb, "languages", arch.language_count, gcols, 2);
            for (int i = 0; i < arch.language_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.languages[i].language, true);
                lsm_tree_cell_int(&sb, arch.languages[i].file_count, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.package_count > 0) {
            static const char *const kcols[] = {"name", "nodes", "fan_in", "fan_out"};
            lsm_tree_table_header(&sb, "packages", arch.package_count, kcols, 4);
            for (int i = 0; i < arch.package_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.packages[i].name, true);
                lsm_tree_cell_int(&sb, arch.packages[i].node_count, false);
                lsm_tree_cell_int(&sb, arch.packages[i].fan_in, false);
                lsm_tree_cell_int(&sb, arch.packages[i].fan_out, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.entry_point_count > 0) {
            ...
            lsm_tree_table_header(&sb, "entry_points", arch.entry_point_count, ecols, 2);
            for (int i = 0; i < arch.entry_point_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.entry_points[i].qualified_name, true);
                lsm_tree_cell_str(&sb, arch.entry_points[i].file, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.route_count > 0) {
            static const char *const rcols[] = {"method", "path", "handler"};
            lsm_tree_table_header(&sb, "routes", arch.route_count, rcols, 3);
            for (int i = 0; i < arch.route_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.routes[i].method, true);
                lsm_tree_cell_str(&sb, arch.routes[i].path, false);
                lsm_tree_cell_str(&sb, arch.routes[i].handler, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.hotspot_count > 0) {
            static const char *const hcols[] = {"qn", "fan_in"};
            lsm_tree_table_header(&sb, "hotspots", arch.hotspot_count, hcols, 2);
            for (int i = 0; i < arch.hotspot_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.hotspots[i].qualified_name, true);
                lsm_tree_cell_int(&sb, arch.hotspots[i].fan_in, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.boundary_count > 0) {
            static const char *const bcols[] = {"from", "to", "calls"};
            lsm_tree_table_header(&sb, "boundaries", arch.boundary_count, bcols, 3);
            for (int i = 0; i < arch.boundary_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.boundaries[i].from, true);
                lsm_tree_cell_str(&sb, arch.boundaries[i].to, false);
                lsm_tree_cell_int(&sb, arch.boundaries[i].call_count, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.service_count > 0) {
            static const char *const scols[] = {"from", "to", "type", "count"};
            lsm_tree_table_header(&sb, "services", arch.service_count, scols, 4);
            for (int i = 0; i < arch.service_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.services[i].from, true);
                lsm_tree_cell_str(&sb, arch.services[i].to, false);
                lsm_tree_cell_str(&sb, arch.services[i].type, false);
                lsm_tree_cell_int(&sb, arch.services[i].count, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.layer_count > 0) {
            static const char *const ycols[] = {"name", "layer", "reason"};
            lsm_tree_table_header(&sb, "layers", arch.layer_count, ycols, 3);
            for (int i = 0; i < arch.layer_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.layers[i].name, true);
                lsm_tree_cell_str(&sb, arch.layers[i].layer, false);
                lsm_tree_cell_str(&sb, arch.layers[i].reason, false);
                lsm_tree_row_end(&sb);
            }
        }
        if (arch.cluster_count > 0) {
            ...
            lsm_tree_table_header(&sb, "clusters", arch.cluster_count, ccols, 7);
            for (int i = 0; i < arch.cluster_count; i++) {
                const lsm_cluster_info_t *c = &arch.clusters[i];
                char joined[LSM_SZ_1K];
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_int(&sb, c->id, true);
                lsm_tree_cell_str(&sb, c->label, false);
                lsm_tree_cell_int(&sb, c->members, false);
                lsm_tree_cell_real(&sb, c->cohesion, false);
                arch_join_list(joined, sizeof(joined), c->top_nodes, c->top_node_count);
                lsm_tree_cell_str(&sb, joined, false);
        ...
        if (arch.file_tree_count > 0) {
            ...
            lsm_tree_table_header(&sb, "file_tree", arch.file_tree_count, fcols, 3);
            for (int i = 0; i < arch.file_tree_count; i++) {
                lsm_tree_row_begin(&sb);
                lsm_tree_cell_str(&sb, arch.file_tree[i].path, true);
                lsm_tree_cell_str(&sb, arch.file_tree[i].type, false);
                lsm_tree_cell_int(&sb, arch.file_tree[i].children, false);
                lsm_tree_row_end(&sb);
            }
        }
        /* Cross-repo edge summary (mirrors append_cross_repo_summary). */
        {
            ...
            if (cross_total > 0) {
                lsm_tree_scalar_int(&sb, "cross_repo_links_total", cross_total);
            }
        }
        ...
        if (aspect_explicitly_named(aspects_arr, "cycles")) {
            ...
            if (arch_compute_cycles(store, project, &members, &sizes, &ncyc, &total_cyc, &scanned,
                                    &etrunc) == LSM_STORE_OK) {
                lsm_tree_scalar_int(&sb, "call_edges_scanned", scanned);
                lsm_tree_scalar_int(&sb, "cycles_total", total_cyc);
                if (etrunc) {
                    lsm_tree_scalar_bool(&sb, "cycles_partial", true);
                    lsm_tree_scalar_str(&sb, "cycles_hint",
                                        "call graph exceeded the scan budget; cycle list may be "
                                        "incomplete");
                }
                if (total_cyc > ncyc) {
                    char omit[LSM_SZ_128];
                    snprintf(omit, sizeof(omit), "cycles_omitted: %d  (showing the first %d)\n",
                             total_cyc - ncyc, ncyc);
                    lsm_sb_append(&sb, omit);
                }
                char hdr[LSM_SZ_128];
                snprintf(hdr, sizeof(hdr),
                         "cycles: %d  (rows: size members; circular CALLS dependencies)\n", ncyc);
                lsm_sb_append(&sb, hdr);
        ...
        lsm_store_architecture_free(&arch);
        lsm_store_schema_free(&schema);
        if (aspects_doc) {
            yyjson_doc_free(aspects_doc);
        }
    ...
    if (path_scoped) {
        ...
        int root_nodes = lsm_store_count_nodes(store, project);
        int root_edges = lsm_store_count_edges(store, project);
...
static const char *bfs_edge_args_for_hop(lsm_traverse_result_t *tr, int64_t hop_node_id,
                                         size_t *out_len) {
...
static bool bfs_edge_evidence_for_hop(lsm_traverse_result_t *tr, int64_t hop_node_id,
    ...
    for (int e = 0; e < tr->edge_count; e++) {
        ...
        memcpy(raw, open, len);
        raw[len] = '\0';
        const char *cls = lsm_mcp_edge_strategy_class(raw);
        if (!cls) {
            continue;
        }
        *class_out = cls;
...
static void bfs_to_toon_table(lsm_sb_t *sb, const char *key, lsm_traverse_result_t *tr,
    ...
    lsm_tree_table_header(sb, key, visible, cols, ncols);
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        lsm_tree_row_begin(sb);
        lsm_tree_cell_str(sb, tr->visited[i].node.qualified_name, true);
        lsm_tree_cell_int(sb, tr->visited[i].hop, false);
        if (risk_labels) {
            lsm_tree_cell_str(sb, lsm_risk_label(lsm_hop_to_risk(tr->visited[i].hop)), false);
        }
        if (include_tests) {
            lsm_tree_cell_bool(sb, test, false);
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
            if (ea && alen > 0 && alen < LSM_SZ_1K) {
                char abuf[LSM_SZ_1K];
                memcpy(abuf, ea, alen);
                abuf[alen] = '\0';
                lsm_tree_cell_str(sb, abuf, false);
            } else {
                lsm_tree_cell_str(sb, "", false);
            }
        }
...
static char *snippet_suggestions(const char *input, lsm_node_t *nodes, int count);
...
static long node_resolution_score(const lsm_node_t *n) {
...
static bool node_is_real_callable_def(const lsm_node_t *n) {
    if (!n->label) {
        return false;
    }
    if (strcmp(n->label, "Function") != 0 && strcmp(n->label, "Method") != 0) {
...
static int pick_resolved_node(const lsm_node_t *nodes, int count, bool *ambiguous) {
    *ambiguous = false;
    if (count <= 1) {
        return 0;
    }
...
static int node_hop_cmp_hop_id(const void *pa, const void *pb) {
    const lsm_node_hop_t *a = (const lsm_node_hop_t *)pa;
    const lsm_node_hop_t *b = (const lsm_node_hop_t *)pb;
    if (a->hop != b->hop) {
 ...
 * transfers into *out, freed by lsm_store_traverse_free. */
static void bfs_union_same_name(lsm_store_t *store, const lsm_node_t *nodes, int node_count,
                                const char *direction, const char **edge_types, int edge_type_count,
                                int depth, int limit, lsm_traverse_result_t *out) {
    memset(out, 0, sizeof(*out));
    int vcap = 0, ecap = 0;
    for (int k = 0; k < node_count; k++) {
        lsm_traverse_result_t tr = {0};
        lsm_store_bfs(store, nodes[k].id, direction, edge_types, edge_type_count, depth, limit,
                      &tr);
        for (int i = 0; i < tr.visited_count; i++) {
            bool dup = false;
            ...
            if (out->visited_count >= vcap) {
                vcap = vcap ? vcap * 2 : 8;
                out->visited = safe_realloc(out->visited, vcap * sizeof(lsm_node_hop_t));
            }
        ...
        for (int i = 0; i < tr.edge_count; i++) {
            ...
            if (out->edge_count >= ecap) {
                ecap = ecap ? ecap * 2 : 8;
                out->edges = safe_realloc(out->edges, ecap * sizeof(lsm_edge_info_t));
            }
            out->edges[out->edge_count++] = tr.edges[i];
            memset(&tr.edges[i], 0, sizeof(tr.edges[i])); /* ownership moved */
        }
        lsm_store_traverse_free(&tr); /* frees only the un-moved (root + dup) fields */
    }
    ...
    if (out->visited_count > 1) {
        qsort(out->visited, (size_t)out->visited_count, sizeof(lsm_node_hop_t),
              node_hop_cmp_hop_id);
    }
}
...
static int trace_watermark_index(const lsm_traverse_result_t *tr, int hop, int64_t node_id) {
    for (int i = 0; i < tr->visited_count; i++) {
...
static yyjson_mut_val *bfs_to_tree_json(yyjson_mut_doc *doc, lsm_traverse_result_t *tr,
                                        bool risk_labels, bool include_tests, bool data_flow,
    ...
    for (int i = 0; i < tr->visited_count; i++) {
        ...
        if (risk_labels) {
            yyjson_mut_arr_add_str(doc, row, lsm_risk_label(lsm_hop_to_risk(tr->visited[i].hop)));
        }
        if (data_flow) {
            size_t alen = 0;
            const char *ea = bfs_edge_args_for_hop(tr, tr->visited[i].node.id, &alen);
...
static void bfs_to_tree_table(lsm_sb_t *sb, const char *key, lsm_traverse_result_t *tr,
    ...
    if (tr->visited_count > 1) {
        qsort(tr->visited, (size_t)tr->visited_count, sizeof(lsm_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[LSM_SZ_1K] = "";
    for (int i = 0; i < tr->visited_count; i++) {
        if (!include_tests && is_test_file(tr->visited[i].node.file_path)) {
...
/* Clamp a client-supplied traversal depth to the MCP ceiling (lsm_mcp_max_depth),
 * WARN-logging when it does so — never a silent truncation (#887). An unclamped
 * `depth` would drive the shared lsm_store_bfs to an arbitrary hop count. */
static int clamp_mcp_depth(int depth, const char *tool) {
    int cap = lsm_mcp_max_depth();
    if (depth > cap) {
        char req_buf[16];
        char cap_buf[16];
        snprintf(req_buf, sizeof(req_buf), "%d", depth);
        snprintf(cap_buf, sizeof(cap_buf), "%d", cap);
        lsm_log_warn("mcp.depth_capped", "tool", tool, "requested", req_buf, "cap", cap_buf);
        return cap;
    }
    return depth;
}

static char *handle_trace_call_path(lsm_mcp_server_t *srv, const char *args) {
    char *func_name = lsm_mcp_get_string_arg(args, "function_name");
    char *project = get_project_arg(args);
    lsm_store_t *store = resolve_store(srv, project);
    char *direction = lsm_mcp_get_string_arg(args, "direction");
    char *mode = lsm_mcp_get_string_arg(args, "mode");
    char *param_name = lsm_mcp_get_string_arg(args, "parameter_name");
    int depth = lsm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH);
     ...
     * depth clamp, #887). */
    int trace_limit = lsm_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT);
    if (trace_limit < 1) {
        trace_limit = 1;
    }
    ...
    bool risk_labels = lsm_mcp_get_bool_arg(args, "risk_labels");
    bool include_tests = lsm_mcp_get_bool_arg(args, "include_tests");
    ...
    bool include_evidence = lsm_mcp_get_bool_arg(args, "include_evidence");
    ...
    if (!func_name) {
        ...
        free(mode);
        free(param_name);
        return lsm_mcp_text_result("function_name is required", true);
    }
    if (!store) {
        char *_err = build_project_list_error("project not found or not indexed");
        char *_res = lsm_mcp_text_result(_err, true);
    ...
    char generation[96];
    (void)lsm_store_generation(store, generation, sizeof(generation));
    char *cursor_arg = lsm_mcp_get_string_arg(args, "cursor");
    ...
    if (cursor_arg && cursor_arg[0]) {
        uint64_t qh =
            trace_params_hash(project, func_name, direction ? direction : "both", mode,
                              lsm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_DEPTH), include_tests,
                              lsm_mcp_get_int_arg(args, "limit", MCP_BFS_LIMIT));
        ...
        if (cerr) {
            ...
            free(mode);
            free(param_name);
            return lsm_mcp_text_result(cerr, true);
        }
        have_cursor = true;
    }
    free(cursor_arg);
    ...
    if (strcmp(direction, "inbound") != 0 && strcmp(direction, "outbound") != 0 &&
        ...
        free(mode);
        free(param_name);
        return lsm_mcp_text_result(errbuf, true);
    }
     ...
     * fields cleanly and lsm_store_free_nodes will free them. */
    lsm_node_t *nodes = NULL;
    int node_count = 0;
    lsm_store_find_nodes_by_name(store, project, func_name, &nodes, &node_count);

    if (node_count == 0) {
        lsm_node_t qn_node = {0};
        if (lsm_store_find_node_by_qn(store, project, func_name, &qn_node) == LSM_STORE_OK) {
            nodes = malloc(sizeof(lsm_node_t));
            if (nodes) {
                nodes[0] = qn_node;
                node_count = 1;
            } else {
    ...
    if (node_count == 0) {
        ...
        free(mode);
        free(param_name);
        lsm_store_free_nodes(nodes, 0);
        return lsm_mcp_text_result(hint, true);
    }
    ...
    char *trace_format = lsm_mcp_get_string_arg(args, "format");
    ...
    if (!trace_legacy_json) {
        lsm_sb_t sb;
        lsm_sb_init(&sb);
        lsm_tree_scalar_str(&sb, "function", func_name);
        lsm_tree_scalar_str(&sb, "direction", direction);
        if (mode) {
            lsm_tree_scalar_str(&sb, "mode", mode);
        }
        ...
        if (do_outbound) {
            lsm_tree_scalar_int(&sb, "callees_total", out_total);
            if (flat_trace) {
        ...
        if (do_inbound) {
            lsm_tree_scalar_int(&sb, "callers_total", in_total);
            if (flat_trace) {
        ...
        if (more_rows) {
            lsm_tree_scalar_bool(&sb, "truncated", true);
            if (next_tok[0]) {
                lsm_tree_scalar_str(&sb, "next", next_tok);
                lsm_tree_scalar_str(&sb, "hint",
...
static char *read_file_lines(const char *path, int start, int end) {
    FILE *fp = lsm_fopen(path, "r");
    if (!fp) {
        return NULL;
    }

    size_t cap = LSM_SZ_4K;
    char *buf = malloc(cap);
    size_t len = 0;
    buf[0] = '\0';
...
static char *project_root_from_store(lsm_store_t *store, const char *project) {
    if (!store || !project) {
        return NULL;
    }
    lsm_project_t proj = {0};
    if (lsm_store_get_project(store, project, &proj) != LSM_STORE_OK) {
        return NULL;
    }
...
static char *get_project_root(lsm_mcp_server_t *srv, const char *project) {
...
static char *handle_cross_repo_mode(lsm_mcp_server_t *srv, const char *repo_path,
                                    const char *name_override, const char *args) {
    if (name_override && name_override[0] && !lsm_validate_project_name(name_override)) {
        return lsm_mcp_text_result("invalid project name", true);
    }
    char *project = name_override && name_override[0] ? heap_strdup(name_override)
                                                      : lsm_project_name_from_path(repo_path);
    if (!project) {
        return lsm_mcp_text_result("cannot derive project name", true);
    }
    ...
    if (target_count > MCP_MAX_CROSS_REPO_TARGETS) {
        yyjson_doc_free(jdoc);
        free(project);
        return lsm_mcp_text_result("too many cross-repo target projects", true);
    }
    ...
    if (!targets || !lease_keys) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return lsm_mcp_text_result("failed to allocate cross-repo project leases", true);
    }
    size_t idx;
    size_t max;
    yyjson_val *val;
    ...
    yyjson_arr_foreach(tp_arr, idx, max, val) {
        ...
        if (!target || !target[0] || strlen(target) >= LSM_SZ_256 ||
            (strcmp(target, "*") != 0 && !lsm_validate_project_name(target))) {
            invalid_target = true;
            break;
        }
        targets[ti++] = target;
    ...
    if (invalid_target || ti != tp_count) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return lsm_mcp_text_result("target_projects must contain valid project names or '*'", true);
    }
    if (all_projects && tp_count != 1) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return lsm_mcp_text_result("target_projects wildcard '*' must be the only entry", true);
    }
    if (!all_projects) {
    ...
    if (held_count != lease_count || cancelled) {
        ...
        yyjson_doc_free(jdoc);
        free(project);
        return lsm_mcp_text_result("cross-repo operation cancelled or blocked by active indexing",
                                   true);
    }

    lsm_cross_repo_result_t result = lsm_cross_repo_match_cancellable(
    ...
    yyjson_mut_doc_free(doc);
    free(project);
    char *out = lsm_mcp_text_result(json, result.cancelled);
    free(json);
    return out;
}
...
static void try_artifact_bootstrap(const char *project_name, const char *repo_path) {
    char db_buf[LSM_SZ_1K];
    project_db_path(project_name, db_buf, sizeof(db_buf));
    if (lsm_file_size(db_buf) < 0 && lsm_artifact_exists(repo_path)) {
        lsm_log_info("index.artifact_bootstrap", "project", project_name);
        lsm_artifact_import(repo_path, db_buf);
    }
}
...
static void add_not_indexed_files_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                          lsm_pipeline_t *p) {
    lsm_ignored_file_t *ignored = NULL;
    int stored = 0;
    int total = 0;
    lsm_pipeline_get_ignored(p, &ignored, &stored, &total);
    ...
    yyjson_mut_obj_add_str(doc, ni, "note",
                           "Excluded by design (gitignore/.lsmignore/skip-lists); examples only — "
...
static bool is_parse_partial(const lsm_file_error_t *e) {
...
static void add_skipped_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                const lsm_file_error_t *errs, int count, const char *logfile) {
    int skips = 0;
    for (int i = 0; i < count; i++) {
...
static void add_parse_partial_summary(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                      const lsm_file_error_t *errs, int count) {
    int partials = 0;
    for (int i = 0; i < count; i++) {
        if (is_parse_partial(&errs[i])) {
            partials++;
...
static bool add_persisted_failure_summaries(yyjson_mut_doc *doc, yyjson_mut_val *root,
                                            lsm_store_t *store, const char *project,
                                            const char *logfile) {
    lsm_coverage_row_t *rows = NULL;
    int row_count = 0;
    if (lsm_store_coverage_get(store, project, &rows, &row_count) != LSM_STORE_OK) {
        return false;
    }
 ...
 *   $LSM_INDEX_LOG (override) else <cache_dir>/logs/<project>-<epoch>.log
 * Returns true and fills out_path on success. */
static bool write_skip_logfile(const char *project, const lsm_file_error_t *errs, int count,
                               char *out_path, size_t out_sz) {
    if (!errs || count <= 0) {
        return false;
    }
    char path[LSM_SZ_1K];
    const char *override = getenv("LSM_INDEX_LOG");
    if (override && override[0]) {
        snprintf(path, sizeof(path), "%s", override);
    } else {
        const char *cdir = lsm_resolve_cache_dir();
        if (!cdir) {
            return false;
        }
        char logdir[LSM_SZ_1K];
        snprintf(logdir, sizeof(logdir), "%s/logs", cdir);
        lsm_mkdir_p(logdir, 0755);
        snprintf(path, sizeof(path), "%s/%s-%lld.log", logdir, project ? project : "index",
                 (long long)time(NULL));
    }
    FILE *f = lsm_fopen(path, "wb");
    if (!f) {
        lsm_log_warn("index.logfile_open_fail", "path", path);
        return false;
    }
    int partials = 0;
    for (int i = 0; i < count; i++) {
    ...
    (void)fprintf(f, "# logan-spine-mcp index coverage report\n");
...
static bool build_index_success_response(lsm_mcp_server_t *srv, yyjson_mut_doc *doc,
                                         yyjson_mut_val *root, const char *project_name,
                                         const char *repo_path, bool persistence, lsm_pipeline_t *p,
                                         char **excluded_dirs, int excluded_count,
                                         const lsm_file_error_t *file_errors, int file_error_count,
                                         const char *logfile) {
    ...
    int exp_nodes = -1;
    int exp_edges = -1;
    lsm_pipeline_get_committed_counts(p, &exp_nodes, &exp_edges);

    const double ratio = lsm_dump_verify_min_ratio();
    const int min_floor = LSM_DUMP_VERIFY_MIN_FLOOR;

    lsm_store_t *store = resolve_store(srv, project_name);
    ...
    if (!store) {
        ...
        nodes = lsm_store_count_nodes(store, project_name);
        edges = lsm_store_count_edges(store, project_name);
        if (nodes < 0) {
            degraded = true;
            nodes = 0;
            edges = edges >= 0 ? edges : 0;
        } else if (lsm_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor)) {
            (void)lsm_store_checkpoint(store);
            int nodes2 = lsm_store_count_nodes(store, project_name);
            int edges2 = lsm_store_count_edges(store, project_name);
            if (nodes2 >= 0) {
                nodes = nodes2;
            }
            if (edges2 >= 0) {
            ...
            degraded = lsm_dump_verify_is_degraded(exp_nodes, nodes, ratio, min_floor);
        }
    }
    ...
    if (degraded) {
        if (!store) {
            ...
            lsm_log_warn("dump.verify", "reason", "store_missing", "expected_nodes",
            ...
            lsm_log_warn("dump.verify", "expected_nodes", exp_buf, "persisted_nodes", got_buf);
        }
    }
    ...
    bool has_artifact = lsm_artifact_exists(repo_path);
    ...
    if (persistence && has_artifact) {
        yyjson_mut_obj_add_str(doc, root, "artifact_hint",
                               "Persistent artifact written to .logan-spine/graph.db.zst. "
...
static char *build_worker_failure_response(const char *args, lsm_proc_outcome_t outcome) {
    char *repo_path = lsm_mcp_get_string_arg(args, "repo_path");
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
    yyjson_mut_val *root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
    yyjson_mut_obj_add_str(doc, root, "status", "error");
    yyjson_mut_obj_add_str(doc, root, "outcome", lsm_proc_outcome_str(outcome));
    const char *hint = NULL;
    if (outcome == LSM_PROC_SPAWN_FAILED) {
        hint = "Indexing worker could not be started. Supervision is mandatory in a LSM "
               "host, so no in-process fallback was attempted.";
    } else if (outcome == LSM_PROC_HANG) {
        hint = "Indexing worker timed out (a file made no progress). The worker was "
               "terminated and the server survived. Re-run to retry.";
    } else if (outcome == LSM_PROC_CRASH) {
    ...
    } else if (outcome == LSM_PROC_CLEAN) {
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *result = lsm_mcp_text_result(json, true);
    free(json);
    return result;
}

static char *build_worker_unsafe_terminal_response(const char *args, lsm_proc_outcome_t outcome,
    ...
    yyjson_mut_doc_free(doc);
    free(repo_path);
    char *response = lsm_mcp_text_result(json, true);
    free(json);
    return response;
}
 ...
 * to invalidate. */
static void invalidate_cached_store(lsm_mcp_server_t *srv) {
    if (!srv) {
        return;
    }
    if (srv->owns_store && srv->store) {
 ...
 * per line — see lsm_index_mark_start/done) into the crash/hang SUSPECT set:
    ...
    *out_n = 0;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (line[0] == 'S') {
            ...
            open_paths[open_n++] = lsm_strdup(rel);
        } else {
...
lsm_mcp_supervised_result_disposition_t lsm_mcp_supervised_result_disposition(
    ...
    if (spawn_result != 0 || !worker_result || worker_result->outcome == LSM_PROC_SPAWN_FAILED) {
        return LSM_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    if (worker_result->cancellation_requested || !worker_result->tree_quiesced ||
        worker_result->supervision_failed) {
        return LSM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL;
    }
    if (worker_result->outcome == LSM_PROC_CLEAN) {
        return worker_result->response ? LSM_MCP_SUPERVISED_RESULT_SUCCESS
                                       : LSM_MCP_SUPERVISED_RESULT_FALLBACK;
    }
    return LSM_MCP_SUPERVISED_RESULT_CONTAINED_FAILURE;
}
 ...
 * A physical LSM host never falls back to its in-process pipeline: an initial
 * start/protocol failure is returned as an explicit error response. */
static char *index_run_supervised(lsm_mcp_server_t *srv, const char *args) {
    invalidate_cached_store(srv);

    /* First attempt: normal parallel run. */
    lsm_index_worker_result_t wr;
    int rc = lsm_index_spawn_worker_with_log_cancel(
    ...
    lsm_mcp_supervised_result_disposition_t disposition =
        lsm_mcp_supervised_result_disposition(rc, &wr);

    if (disposition == LSM_MCP_SUPERVISED_RESULT_FALLBACK) {
        lsm_proc_outcome_t outcome = wr.outcome;
        lsm_index_worker_result_free(&wr);
        invalidate_cached_store(srv);
     ...
     * time. */
    lsm_proc_outcome_t last_outcome = wr.outcome;
    lsm_index_worker_result_free(&wr);

    char marker_path[LSM_SZ_1K];
    char quarantine_path[LSM_SZ_1K];
    ...
    FILE *qinit = lsm_fopen(quarantine_path, "wb");
    if (qinit) {
        (void)fclose(qinit);
    }

    int cap = 100;
    const char *cap_env = getenv("LSM_INDEX_MAX_RESTARTS");
    ...
    for (int i = 0; i < cap; i++) {
        lsm_index_worker_result_t wr2;
        int rc2 = lsm_index_spawn_worker_with_log_cancel(
        ...
        lsm_mcp_supervised_result_disposition_t recovery_disposition =
            lsm_mcp_supervised_result_disposition(rc2, &wr2);
        if (recovery_disposition == LSM_MCP_SUPERVISED_RESULT_FALLBACK) {
            last_outcome = wr2.outcome;
            lsm_index_worker_result_free(&wr2);
            break; /* spawn failed mid-recovery — give up */
        }
        if (recovery_disposition == LSM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wr2.outcome;
        ...
        if (recovery_disposition == LSM_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wr2.response; /* transfer ownership to caller */
            wr2.response = NULL;
            lsm_index_worker_result_free(&wr2);
            break; /* good files indexed; quarantined files reported as crash/hang */
        }
        if (wr2.outcome == LSM_PROC_CRASH || wr2.outcome == LSM_PROC_HANG) {
            last_outcome = wr2.outcome;
            lsm_index_worker_result_free(&wr2);
            ...
            const char *phase = (last_outcome == LSM_PROC_HANG) ? "hang" : "crash";
            int sus_n = 0;
            ...
            if (!suspects || sus_n == 0) {
                ...
                lsm_log_warn("index.supervisor.unattributable", "action", "give_up");
                break;
            }
            if (prev_suspects) {
                ...
                if (!supervisor_append_quarantine(quarantine_path, pick, phase)) {
                    lsm_log_warn("index.supervisor.quarantine_write_fail", "path", pick);
                    supervisor_free_suspects(suspects, sus_n);
                    break;
                }
                quarantined++;
                ...
                lsm_log_warn("index.file_quarantined", "path", pick, "outcome", phase, "attempt",
                             attempt_buf);
            }
    ...
    if (!resp && !unsafe_terminal && quarantined > 0) {
        lsm_index_worker_result_t wrp;
        int rcp = lsm_index_spawn_worker_with_log_cancel(
            args, /*single_thread=*/false, NULL, quarantine_path,
            srv ? srv->index_log_callback : NULL, srv ? srv->index_log_context : NULL,
            srv ? &srv->pipeline_cancel_requested : NULL, &wrp);
        lsm_mcp_supervised_result_disposition_t partial_disposition =
            lsm_mcp_supervised_result_disposition(rcp, &wrp);
        if (partial_disposition == LSM_MCP_SUPERVISED_RESULT_SUCCESS) {
            resp = wrp.response; /* transfer ownership to caller */
            wrp.response = NULL;
            char qn[MCP_FIELD_SIZE];
            snprintf(qn, sizeof(qn), "%d", quarantined);
            lsm_log_error("index.supervisor.partial", "quarantined", qn, "outcome",
                          lsm_proc_outcome_str(last_outcome));
        } else if (partial_disposition == LSM_MCP_SUPERVISED_RESULT_UNSAFE_TERMINAL) {
            last_outcome = wrp.outcome;
            unsafe_terminal = true;
            terminal_cancelled = wrp.cancellation_requested;
        }
        lsm_index_worker_result_free(&wrp);
    }
...
static char *index_run_supervised_path(lsm_mcp_server_t *srv, const char *root_path) {
    if (!root_path || !root_path[0]) {
        return NULL;
    }
    yyjson_mut_doc *doc = yyjson_mut_doc_new(NULL);
...
static bool resolve_session_repo_path(lsm_mcp_server_t *srv, char **repo_path) {
...
static char *resolved_repo_path_from_project_arg(const char *args) {
    ...
    char db_path[LSM_SZ_1K];
    project_db_path(project, db_path, sizeof(db_path));
    lsm_store_t *store = db_path[0] ? lsm_store_open_path_query(db_path) : NULL;
    char *root_path = NULL;
    if (store) {
        lsm_project_t proj = {0};
        if (lsm_store_get_project(store, project, &proj) == LSM_STORE_OK) {
...
static char *handle_index_repository(lsm_mcp_server_t *srv, const char *args) {
    char *repo_path = lsm_mcp_get_string_arg(args, "repo_path");
    char *mode_str = lsm_mcp_get_string_arg(args, "mode");
    char *name_override = lsm_mcp_get_string_arg(args, "name");
    lsm_normalize_path_sep(repo_path);
    ...
    if (!repo_path) {
        free(mode_str);
        free(name_override);
        return lsm_mcp_text_result("repo_path is required", true);
    }
    ...
    if (!resolve_session_repo_path(srv, &repo_path)) {
        ...
        free(name_override);
        free(repo_path);
        return lsm_mcp_text_result("failed to resolve repo_path", true);
    }
     ...
     * back to the process-wide LSM_ALLOWED_ROOT. The decision itself lives in one
    ...
    const char *allowed_root =
        srv->allowed_root_policy_set ? srv->allowed_root : getenv("LSM_ALLOWED_ROOT");
    ...
    char boundary_err[LSM_SZ_1K];
    if (repo_path && repo_path[0] &&
        !lsm_workspace_root_allowed(repo_path, lsm_workspace_home_dir(), lsm_workspace_cache_dir(),
                                    allowed_root, boundary_err, sizeof(boundary_err))) {
        free(mode_str);
        free(name_override);
        free(repo_path);
        return lsm_mcp_text_result(boundary_err, true);
    }
    ...
    char *mutation_project =
        lsm_project_name_from_path(name_override && name_override[0] ? name_override : repo_path);
    if (!mutation_project) {
        free(repo_path);
        free(mode_str);
        free(name_override);
        return lsm_mcp_text_result("could not resolve index project name", true);
    }
    ...
    if (lsm_index_supervisor_should_wrap()) {
        char *worker_args = index_args_with_repo_path(args, repo_path);
        if (!worker_args) {
            ...
            free(mode_str);
            free(name_override);
            return lsm_mcp_text_result("failed to prepare supervised index request", true);
        }
    ...
    if (!mcp_project_mutation_begin(srv, mutation_project)) {
        ...
        free(mode_str);
        free(name_override);
        return lsm_mcp_text_result("index operation blocked by another mutation for this project",
                                   true);
    }
    if (mcp_request_cancelled(srv)) {
        ...
        free(mode_str);
        free(name_override);
        return lsm_mcp_text_result("index operation cancelled for this request", true);
    }

    lsm_index_mode_t mode = LSM_MODE_FULL;
    if (mode_str && strcmp(mode_str, "fast") == 0) {
        mode = LSM_MODE_FAST;
    } else if (mode_str && strcmp(mode_str, "moderate") == 0) {
        mode = LSM_MODE_MODERATE;
    }
    free(mode_str);

    bool persistence = lsm_mcp_get_bool_arg(args, "persistence");

    lsm_pipeline_t *p = lsm_pipeline_new(repo_path, NULL, mode);
    if (!p) {
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
        free(repo_path);
        return lsm_mcp_text_result("failed to create pipeline", true);
    }
    if (name_override && name_override[0] && !lsm_pipeline_set_project_name(p, name_override)) {
        lsm_pipeline_free(p);
        mcp_project_mutation_end(srv, mutation_project);
        free(mutation_project);
        free(name_override);
    ...
    free(name_override);
    lsm_pipeline_set_persistence(p, persistence);

    char *project_name = heap_strdup(lsm_pipeline_project_name(p));
    ...
    lsm_pipeline_lock();
    lsm_pipeline_bind_cancel_flag(p, &srv->pipeline_cancel_requested);
     ...
     * JSON doc, so they need only outlive that call, not lsm_pipeline_free. */
    char **excluded_dirs = NULL;
    int excluded_count = 0;
    lsm_pipeline_get_excluded(p, &excluded_dirs, &excluded_count);
    ...
    lsm_file_error_t *file_errors = NULL;
    int file_error_count = 0;
    lsm_pipeline_get_file_errors(p, &file_errors, &file_error_count);

    lsm_mem_collect(); /* return mimalloc pages to OS after large indexing */
    ...
    if (lsm_index_worker_active()) {
        lsm_log_info("index.worker.fast_exit", "skip", "pipeline_free");
    } else {
        lsm_pipeline_free(p);
    }
    free(project_name);
    ...
    char *result = lsm_mcp_text_result(json, rc != 0);
    free(json);
    return result;
}
...
static void copy_node(const lsm_node_t *src, lsm_node_t *dst) {
    dst->id = src->id;
 ...
 * (>= LSM_SZ_4K bytes); returns true on success. Isolating the per-OS resolver
 * keeps lsm_path_within_root's control flow unconditional: the previous `#ifdef`
...
static bool resolve_canonical_path(const char *path, char *out, size_t out_sz) {
    /* lsm_canonical_path: realpath on POSIX; an opened handle plus
    ...
    if (!lsm_canonical_path(path, out, out_sz)) {
        return false;
    }
#ifdef _WIN32
...
#ifdef _WIN32
    wchar_t *wide_root = lsm_utf8_to_wide(root_path);
    wchar_t *wide_candidate = lsm_utf8_to_wide(candidate_path);
...
bool lsm_path_within_root(const char *root_path, const char *abs_path) {
    if (!root_path || !abs_path) {
        return false;
    }
    char real_root[LSM_SZ_4K];
    char real_file[LSM_SZ_4K];
...
static void add_snippet_coverage_note(yyjson_mut_doc *doc, yyjson_mut_val *root_obj,
                                      lsm_store_t *store, const lsm_node_t *node) {
    if (!node->file_path || !node->file_path[0] || !node->project) {
        return;
    }
    lsm_coverage_row_t *rows = NULL;
    int count = 0;
    if (lsm_store_coverage_get_path(store, node->project, node->file_path, &rows, &count) !=
        LSM_STORE_OK) {
        return;
    }
    for (int i = 0; i < count; i++) {
...
static char *build_snippet_response(lsm_mcp_server_t *srv, lsm_node_t *node,
                                    const char *match_method, bool include_neighbors,
                                    lsm_node_t *alternatives, int alt_count) {
    ...
    lsm_store_t *store = srv->store;
    int in_deg = 0;
    int out_deg = 0;
    lsm_store_node_degree(store, node->id, &in_deg, &out_deg);
    ...
    if (include_neighbors) {
        lsm_store_node_neighbor_names(store, node->id, MCP_DEFAULT_LIMIT, &nb_callers,
...
static char *handle_get_code_snippet(lsm_mcp_server_t *srv, const char *args) {
    char *qn = lsm_mcp_get_string_arg(args, "qualified_name");
    char *project = get_project_arg(args);
    bool include_neighbors = lsm_mcp_get_bool_arg(args, "include_neighbors");

    if (!qn) {
        free(project);
        return lsm_mcp_text_result("qualified_name is required", true);
    }
    ...
    lsm_node_t node = {0};
    int rc = lsm_store_find_node_by_qn(store, effective_project, qn, &node);
    if (rc == LSM_STORE_OK) {
    ...
    lsm_node_t *suffix_nodes = NULL;
    int suffix_count = 0;
    lsm_store_find_nodes_by_qn_suffix(store, effective_project, qn, &suffix_nodes, &suffix_count);

    if (suffix_count == SKIP_ONE) {
        copy_node(&suffix_nodes[0], &node);
        lsm_store_free_nodes(suffix_nodes, suffix_count);
...
typedef struct {
    char file[LSM_SZ_512];
    int line;
    char content[LSM_SZ_1K];
} grep_match_t;

/* Deduped result: one per containing graph node */
typedef struct {
    int64_t node_id; /* 0 = raw match (no containing node) */
    char node_name[LSM_SZ_256];
    char qualified_name[LSM_SZ_512];
    char label[LSM_SZ_64];
    char file[LSM_SZ_512];
    int start_line;
    int end_line;
    int in_degree;
 ...
 * defaults.) */
#define LSM_PS_UTF8_PRELUDE "[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; "
...
void lsm_search_code_build_grep_cmd(char *cmd, size_t cmd_sz, bool use_regex, bool scoped,
    ...
    if (scoped) {
        if (file_pattern) {
            if (lsm_search_code_file_pattern_can_prefilter(file_pattern)) {
                snprintf(
                    cmd, cmd_sz,
                    "powershell -Command \"" LSM_PS_UTF8_PRELUDE
...
static int aggregate_search_dirs(search_result_t *sr, int sr_count, char dir_names[][LSM_SZ_128],
    ...
    for (int di = 0; di < sr_count; di++) {
        ...
        if (found >= 0) {
            ...
            snprintf(dir_names[dir_n], LSM_SZ_128, "%s", top);
...
static yyjson_mut_val *build_dir_distribution(yyjson_mut_doc *doc, search_result_t *sr,
    ...
    char dir_names[LSM_SZ_64][LSM_SZ_128];
    int dir_counts[LSM_SZ_64];
    int dir_n = aggregate_search_dirs(sr, sr_count, dir_names, dir_counts, LSM_SZ_64);
    for (int d = 0; d < dir_n; d++) {
...
static char *assemble_search_output_toon(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    lsm_tree_table_header(&sb, "results", output_count, cols, 7);
    for (int ri = 0; ri < output_count; ri++) {
        ...
        lsm_tree_row_begin(&sb);
        lsm_tree_cell_str(&sb, r->qualified_name, true);
        lsm_tree_cell_str(&sb, r->label, false);
        lsm_tree_cell_str(&sb, r->file, false);
        lsm_tree_cell_str(&sb, lines, false);
        lsm_tree_cell_str(&sb, matches, false);
        lsm_tree_cell_int(&sb, r->in_degree, false);
        lsm_tree_cell_int(&sb, r->out_degree, false);
        lsm_tree_row_end(&sb);
    }
    ...
    if (raw_output > 0) {
        ...
        lsm_tree_table_header(&sb, "raw", raw_output, rcols, 3);
        for (int ri = 0; ri < raw_output; ri++) {
            lsm_tree_row_begin(&sb);
            lsm_tree_cell_str(&sb, raw[ri].file, true);
            lsm_tree_cell_int(&sb, raw[ri].line, false);
            lsm_tree_cell_str(&sb, raw[ri].content, false);
            lsm_tree_row_end(&sb);
        }
    }
    ...
    if (dir_n > 0) {
        ...
        lsm_tree_table_header(&sb, "dirs", dir_n, dcols, 2);
        for (int d = 0; d < dir_n; d++) {
            lsm_tree_row_begin(&sb);
            lsm_tree_cell_str(&sb, dir_names[d], true);
            lsm_tree_cell_int(&sb, dir_counts[d], false);
            lsm_tree_row_end(&sb);
        }
    }

    lsm_tree_scalar_int(&sb, "total_grep_matches", gm_count);
    lsm_tree_scalar_int(&sb, "total_results", sr_count);
    lsm_tree_scalar_int(&sb, "raw_match_count", raw_count);
    if (metrics->include_phase_timings) {
        lsm_tree_scalar_int(&sb, "scope_ms", (long long)metrics->scope_ms);
        lsm_tree_scalar_int(&sb, "scan_ms", (long long)metrics->scan_ms);
        lsm_tree_scalar_int(&sb, "enrich_ms", (long long)metrics->enrich_ms);
    }
    lsm_tree_scalar_int(&sb, "elapsed_ms", (long long)metrics->elapsed_ms);
    if (warn_literal_pipe) {
        lsm_tree_scalar_str(&sb, "warning",
    ...
    if (metrics->elapsed_ms >= SEARCH_SLOW_MS) {
        lsm_tree_scalar_str(&sb, "warning_slow",
...
static char *assemble_search_output(search_result_t *sr, int sr_count, grep_match_t *raw,
    ...
    if (metrics->elapsed_ms >= SEARCH_SLOW_MS) {
        ...
        lsm_log_warn("search.slow", "elapsed_ms", ems); /* visibility in logs */
    }
...
static grep_match_t *collect_grep_matches(FILE *fp, const char *root_path, size_t root_len,
                                          bool has_path_filter, lsm_regex_t *path_regex,
                                          int grep_limit, int *out_count) {
    int gm_cap = LSM_SZ_64;
    int gm_count = 0;
...
#ifdef _WIN32
        lsm_normalize_path_sep(line);
#endif
        const char *path = line;
        const char *file = strip_root_prefix(path, root_path, root_len);

        if (has_path_filter && lsm_regexec(path_regex, file, 0, NULL, 0) != LSM_REG_OK) {
            continue;
        }

        safe_grow(gm, gm_count, gm_cap, PAIR_LEN);
        snprintf(gm[gm_count].file, sizeof(gm[0].file), "%s", file);
        gm[gm_count].line = (int)strtol(sep1 + SKIP_ONE, NULL, LSM_DECIMAL_BASE);
...
static int find_tightest_node(lsm_node_t *nodes, int count, int line) {
    int best = LSM_NOT_FOUND;
...
static void add_to_search_results(search_result_t **sr, int *sr_count, int *sr_cap, lsm_node_t *n,
                                  int line) {
    for (int j = 0; j < *sr_count; j++) {
        if ((*sr)[j].node_id == n->id) {
            if ((*sr)[j].match_count < LSM_SZ_64) {
...
static void classify_grep_hit(grep_match_t *hit, lsm_node_t *file_nodes, int file_node_count,
    ...
    if (best >= 0) {
        ...
        if (*raw_count >= *raw_cap) {
            *raw_cap = (*raw_cap == 0) ? LSM_SZ_32 : *raw_cap * PAIR_LEN;
...
/* Free a file_nodes array returned from lsm_store_find_nodes_by_file. */
static void free_file_nodes(lsm_node_t *nodes, int count) {
    for (int j = 0; j < count; j++) {
...
static void classify_all_grep_hits(grep_match_t *gm, int gm_count, lsm_store_t *store,
    ...
    while (i < gm_count) {
        ...
        if (store) {
            lsm_store_find_nodes_by_file(store, project, cur_file, &file_nodes, &file_node_count);
        }
 ...
 *
 * `fl` is the caller's already-open binary stream on the descriptor lsm_mkstemp
...
static bool write_scoped_filelist(lsm_mcp_server_t *srv, const char *project, const char *root_path,
                                  FILE *fl, bool has_path_filter, lsm_regex_t *path_regex,
                                  int *out_written) {
    *out_written = 0;
    lsm_store_t *pre_store = resolve_store(srv, project);
    if (!pre_store) {
        return false;
    }
    char **indexed_files = NULL;
    int indexed_count = 0;
    if (lsm_store_list_files(pre_store, project, &indexed_files, &indexed_count) != LSM_STORE_OK ||
        indexed_count == 0) {
        return false;
    }
    bool ok = false;
...
#ifdef _WIN32
                lsm_normalize_path_sep(indexed_files[fi]);
#endif
                if (lsm_regexec(path_regex, indexed_files[fi], 0, NULL, 0) != LSM_REG_OK) {
                    continue;
                }
            }
 ...
 * "<tmp>/lsm_search_<pid>.pat" and its ".files" companion — opened with a plain
 ...
 * them. Now both live inside a directory created by lsm_mkdtemp (0700 on POSIX,
 * an explicit owner-only DACL on Windows) under an unguessable XXXXXX suffix,
 * and each file is created by lsm_mkstemp — O_CREAT|O_EXCL at mode 0600, so the
 * create fails rather than following anything already at the name. Every write
 * goes through the descriptor lsm_mkstemp returned; neither path is ever
 * reopened by name.
 *
 * Sizing: lsm_mkdtemp copies its expanded result back into `dir`, and its own
 * internal buffer is LSM_SZ_512, so `dir` must be at least that big to receive
 * it. The two file paths are `dir` plus a short basename. */
typedef struct {
    char dir[LSM_SZ_512];
    char pattern_path[LSM_SZ_1K];
    char filelist_path[LSM_SZ_1K];
...
static FILE *search_scratch_file(const char *dir, const char *basename, char *path_out,
    ...
    int descriptor = lsm_mkstemp(path_out);
    if (descriptor < 0) {
        path_out[0] = '\0';
        return NULL;
    }
...
static void search_scratch_close(search_scratch_t *scratch) {
    ...
    if (scratch->pattern_path[0] != '\0') {
        (void)lsm_unlink(scratch->pattern_path);
        scratch->pattern_path[0] = '\0';
    }
    if (scratch->filelist_path[0] != '\0') {
        (void)lsm_unlink(scratch->filelist_path);
        scratch->filelist_path[0] = '\0';
    }
    if (scratch->dir[0] != '\0') {
        (void)lsm_rmdir(scratch->dir);
        scratch->dir[0] = '\0';
    }
}
...
static bool search_scratch_open(search_scratch_t *scratch, const char *pattern) {
    ...
    int written =
        snprintf(scratch->dir, sizeof(scratch->dir), "%s/lsm-search-XXXXXX", lsm_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(scratch->dir) || !lsm_mkdtemp(scratch->dir)) {
        scratch->dir[0] = '\0';
        return false;
    }
...
static bool compile_path_filter(const char *filter, lsm_regex_t *re) {
    if (!filter || !filter[0]) {
        return false;
    }
    return lsm_regcomp(re, filter, LSM_REG_EXTENDED | LSM_REG_NOSUB) == LSM_REG_OK;
}

static int mcp_run_shell_command_cancellable_bounded(lsm_mcp_server_t *srv, const char *command,
                                                     char output_path[LSM_SZ_2K],
                                                     size_t output_limit,
                                                     bool *output_limit_exceeded,
                                                     lsm_proc_result_t *result_out);

#ifdef _WIN32
static char *search_code_scan_error(search_scratch_t *scratch, const char *output_path,
                                    bool has_path_filter, lsm_regex_t *path_regex, char *root_path,
                                    char *pattern, char *project, char *file_pattern,
                                    const char *message) {
    if (output_path && output_path[0]) {
        (void)lsm_unlink(output_path);
    }
    search_scratch_close(scratch);
    if (has_path_filter) {
        lsm_regfree(path_regex);
    }
    free(root_path);
    free(pattern);
    free(project);
...
static char *handle_search_code(lsm_mcp_server_t *srv, const char *args) {
    char *pattern = lsm_mcp_get_string_arg(args, "pattern");
    char *project = get_project_arg(args);
    char *file_pattern = lsm_mcp_get_string_arg(args, "file_pattern");
    char *path_filter = lsm_mcp_get_string_arg(args, "path_filter");
    char *mode_str = lsm_mcp_get_string_arg(args, "mode");
    int limit = lsm_mcp_get_int_arg(args, "limit", MCP_DEFAULT_LIMIT);
    ...
    int context_lines = lsm_mcp_get_int_arg(args, "context", 0);
    bool use_regex = lsm_mcp_get_bool_arg(args, "regex");
    uint64_t search_t0 = lsm_now_ms();
    search_metrics_t metrics = {0};
    metrics.include_phase_timings = lsm_mcp_get_bool_arg(args, "debug");
    ...
    if (!pattern) {
        free(project);
        free(file_pattern);
        return lsm_mcp_text_result("pattern is required", true);
    }
    ...
    if (!validate_search_args(root_path, file_pattern)) {
        ...
        free(project);
        free(file_pattern);
        return lsm_mcp_text_result("path or file_pattern contains invalid characters", true);
    }
    ...
    if (use_regex) {
        lsm_regex_t probe;
        if (lsm_regcomp(&probe, pattern, LSM_REG_EXTENDED | LSM_REG_NOSUB) != LSM_REG_OK) {
            if (has_path_filter) {
                lsm_regfree(&path_regex);
            }
            free(root_path);
    ...
    if (!search_scratch_open(&scratch, pattern)) {
        ...
        free(project);
        free(file_pattern);
        return lsm_mcp_text_result(errmsg, true);
    }
    ...
    uint64_t scope_t0 = metrics.include_phase_timings ? lsm_now_ms() : 0;
    ...
    scratch.filelist = NULL;
    if (metrics.include_phase_timings) {
        metrics.scope_ms = lsm_now_ms() - scope_t0;
    }

    /* Collect grep matches into array */
    int gm_count = 0;
    grep_match_t *gm = NULL;
    uint64_t scan_t0 = metrics.include_phase_timings ? lsm_now_ms() : 0;
    if (scoped && scoped_written == 0) {
        ...
        char cmd[LSM_SZ_4K];
        lsm_search_code_build_grep_cmd(cmd, sizeof(cmd), use_regex, scoped, file_pattern, tmpfile,
                                       filelist, root_path);

#ifdef _WIN32
        char output_path[LSM_SZ_2K] = {0};
        lsm_proc_result_t scan_result = {0};
        ...
        FILE *fp = lsm_fopen(output_path, "rb");
        if (!fp) {
            return search_code_scan_error(&scratch, output_path, has_path_filter, &path_regex,
                                          root_path, pattern, project, file_pattern,
        ...
        if (!fp) {
            ...
            free(project);
            free(file_pattern);
            return lsm_mcp_text_result("search failed", true);
        }
    ...
    if (metrics.include_phase_timings) {
        metrics.scan_ms = lsm_now_ms() - scan_t0;
    }
    ...
    uint64_t enrich_t0 = metrics.include_phase_timings ? lsm_now_ms() : 0;
    lsm_store_t *store = resolve_store(srv, project);

    int sr_cap = LSM_SZ_32;
    int sr_count = 0;
    search_result_t *sr = calloc(sr_cap, sizeof(search_result_t));

    int raw_cap = LSM_SZ_32;
    int raw_count = 0;
    ...
    if (store && sr_count > 0) {
        ...
        if (lsm_store_batch_count_degrees(store, ids, sr_count, "CALLS", in_degs, out_degs) ==
            LSM_STORE_OK) {
            for (int j = 0; j < sr_count; j++) {
    ...
    if (metrics.include_phase_timings) {
        metrics.enrich_ms = lsm_now_ms() - enrich_t0;
    }
    metrics.elapsed_ms = lsm_now_ms() - search_t0;
    ...
    char *sc_format = lsm_mcp_get_string_arg(args, "format");
    ...
    if (mode == 0 && !sc_legacy_json) {
        ...
        result = lsm_mcp_text_result(toon_text ? toon_text : "out of memory", toon_text == NULL);
        free(toon_text);
    } else {
        result =
...
static bool mcp_command_output_path(char out[LSM_SZ_2K]) {
    ...
    written = snprintf(out, LSM_SZ_2K, "%s/.mcp-command-XXXXXX", directory);
    if (written <= 0 || written >= LSM_SZ_2K) {
        out[0] = '\0';
        return false;
    }
 ...
 * COMSPEC. lsm_subprocess receives this as lpApplicationName and validates the
 * same absolute cmd.exe path before using the dedicated payload serializer. */
static bool mcp_resolve_windows_cmd(char out[LSM_SZ_4K]) {
    if (!out) {
        return false;
    }
    out[0] = '\0';
    ...
    char *candidate = lsm_wide_to_utf8(system_directory);
    if (!candidate) {
        return false;
    }
    bool resolved = lsm_canonical_path(candidate, out, LSM_SZ_4K) != 0;
    free(candidate);
    return resolved;
}
#endif
...
static int mcp_run_shell_command_cancellable_bounded(lsm_mcp_server_t *srv, const char *command,
    ...
    lsm_proc_poll_t state;
    bool limit_exceeded = false;
    for (;;) {
        if (mcp_request_cancelled(srv)) {
            (void)lsm_subprocess_request_cancel(process);
        }
        if (!limit_exceeded && output_limit > 0) {
            int64_t output_size = lsm_file_size(output_path);
            if (output_size > 0 && (uint64_t)output_size > output_limit) {
                limit_exceeded = true;
                (void)lsm_subprocess_request_cancel(process);
            }
        }
        state = lsm_subprocess_poll(process, result_out);
        if (state != LSM_PROC_POLL_RUNNING) {
            break;
        }
        lsm_usleep(10000);
    }
    bool contained = state == LSM_PROC_POLL_TERMINAL && result_out->tree_quiesced &&
                     !result_out->supervision_failed;
    lsm_subprocess_destroy(process);
    if (!limit_exceeded && output_limit > 0) {
        int64_t final_size = lsm_file_size(output_path);
...
static int mcp_run_shell_command_cancellable(lsm_mcp_server_t *srv, const char *command,
                                             char output_path[LSM_SZ_2K],
                                             lsm_proc_result_t *result_out) {
...
static void detect_collect_seeds(lsm_store_t *store, const char *project, const char *file,
                                 const lsm_changed_hunk_t *hunks, int hunk_count, int64_t **seeds,
                                 int *n, int *cap) {
    lsm_node_t *nodes = NULL;
    int ncount = 0;
    lsm_store_find_nodes_by_file(store, project, file, &nodes, &ncount);
    ...
    if (scope_to_hunks) {
        ...
        for (int i = 0; i < ncount && !any_overlap; i++) {
            any_overlap = detect_is_seedable_label(nodes[i].label) &&
                          lsm_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file);
        }
        scope_to_hunks = any_overlap;
    }
    for (int i = 0; i < ncount; i++) {
        if (detect_is_seedable_label(nodes[i].label)) {
            if (scope_to_hunks && !lsm_detect_node_in_hunks(&nodes[i], hunks, hunk_count, file)) {
                continue;
            }
            if (*n >= *cap) {
                *cap = *cap ? *cap * 2 : 16;
...
static int detect_module_rollup(const lsm_traverse_result_t *impact, char mods[][LSM_SZ_128],
    ...
    for (int i = 0; i < impact->visited_count; i++) {
        ...
        if (j == nmods) {
            if (nmods < DETECT_MODCAP) {
                snprintf(mods[nmods], LSM_SZ_128, "%s", m);
                mcnt[nmods] = 1;
                nmods++;
            } else {
                (*overflow)++;
...
static void detect_emit_impacted_tree(lsm_sb_t *sb, lsm_traverse_result_t *tr, int limit) {
    lsm_tree_scalar_int(sb, "impacted_total", tr->visited_count);
    ...
    lsm_sb_append(sb, hdr);
    if (shown > 1) {
        qsort(tr->visited, (size_t)shown, sizeof(lsm_node_hop_t), tree_hop_cmp_qn);
    }
    char cur_group[LSM_SZ_1K] = "";
    for (int i = 0; i < shown; i++) {
        const char *qn =
    ...
    if (shown < tr->visited_count) {
        ...
        lsm_sb_append(sb, more);
    }
}

static char *handle_detect_changes(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *base_branch = lsm_mcp_get_string_arg(args, "base_branch");
    char *since = lsm_mcp_get_string_arg(args, "since");
    char *scope = lsm_mcp_get_string_arg(args, "scope");
    int depth = lsm_mcp_get_int_arg(args, "depth", MCP_DEFAULT_BFS_DEPTH);
    ...
    if (!lsm_validate_shell_arg(base_branch) || base_branch[0] == '-' ||
        !validate_windows_cmd_interpolation_arg(base_branch)) {
        free(project);
        free(base_branch);
        free(scope);
        return lsm_mcp_text_result("base_branch contains invalid characters", true);
    }
    ...
    if (!validate_search_path_arg(root_path) ||
        ...
        free(base_branch);
        free(scope);
        return lsm_mcp_text_result("project path contains invalid characters", true);
    }
    ...
    char output_path[LSM_SZ_2K] = {0};
    lsm_proc_result_t git_result = {0};
    ...
    if (git_cancelled) {
        ...
        free(base_branch);
        free(scope);
        return lsm_mcp_text_result("detect_changes cancelled for this request", true);
    }
    if (git_run != 0) {
        (void)lsm_unlink(output_path);
        char errmsg[LSM_SZ_256];
    ...
    if (!fp) {
        ...
        free(base_branch);
        free(scope);
        return lsm_mcp_text_result("git diff failed: contained output could not be read", true);
    }
    ...
    char *fmt = lsm_mcp_get_string_arg(args, "format");
    ...
    int imp_limit = lsm_mcp_get_int_arg(args, "limit", MCP_DEFAULT_IMPACT_LIMIT);
    if (imp_limit < 1) {
        imp_limit = 1;
    }
    ...
    lsm_changed_hunk_t *hunks = NULL;
    int hunk_count = 0;
    if (want_symbols) {
        char hunk_cmd[LSM_SZ_2K];
#ifdef _WIN32
        ...
        snprintf(hunk_cmd, sizeof(hunk_cmd),
                 ...
                 root_path, base_branch, root_path);
#endif
        char hunk_output_path[LSM_SZ_2K] = {0};
        lsm_proc_result_t hunk_result = {0};
        int hunk_run =
        ...
        FILE *hfp = (!hunk_cancelled && hunk_run == 0) ? lsm_fopen(hunk_output_path, "rb") : NULL;
        if (hfp) {
            ...
            if (hsz > 0) {
                ...
                if (hbuf) {
                    ...
                    hbuf[hread] = '\0';
                    enum { HUNK_CAP = 4096 };
                    hunks = safe_realloc(NULL, (size_t)HUNK_CAP * sizeof(lsm_changed_hunk_t));
                    hunk_count = lsm_parse_hunks(hbuf, hunks, HUNK_CAP);
                    ...
                    if (hunk_count >= HUNK_CAP) {
                        lsm_log_info("detect_changes.hunks", "action", "scoping_disabled", "reason",
                                     "hunk_cap_reached");
                        free(hunks);
                        hunks = NULL;
                        hunk_count = 0;
        ...
        if (hunk_output_path[0]) {
            (void)lsm_unlink(hunk_output_path);
        }
    }
    ...
    char merge_base[64] = "";
    {
        ...
        snprintf(mbcmd, sizeof(mbcmd), "git -C '%s' merge-base '%s' HEAD 2>/dev/null", root_path,
                 base_branch);
#endif
        char mb_output_path[LSM_SZ_2K] = {0};
        lsm_proc_result_t mb_result = {0};
        ...
        FILE *mbfp =
            mb_run == 0 && mb_result.exit_code == 0 ? lsm_fopen(mb_output_path, "rb") : NULL;
        ...
        if (mb_output_path[0]) {
            (void)lsm_unlink(mb_output_path);
        }
    ...
    lsm_traverse_result_t impact = {0};
    bool truncated = false;
    if (want_symbols && seed_count > 0) {
        (void)lsm_store_bfs_multi(store, seeds, seed_count, direction, NULL, 0, depth,
    ...
    if (!legacy_json) {
        lsm_sb_t sb;
        lsm_sb_init(&sb);
        lsm_tree_scalar_str(&sb, "base", base_branch);
        if (merge_base[0]) {
            lsm_tree_scalar_str(&sb, "merge_base", merge_base);
        }
        lsm_tree_scalar_str(&sb, "direction", direction);
        if (is_error) {
            char hint_buf[LSM_SZ_256];
            snprintf(hint_buf, sizeof(hint_buf),
                     "git diff exited with status %d. Check that branch '%s' exists.", git_status,
                     base_branch);
            lsm_tree_scalar_str(&sb, "hint", hint_buf);
        }
    ...
    lsm_store_traverse_free(&impact);
    for (int i = 0; i < file_count; i++) {
        free(files[i]);
    }
    free(files);
    ...
    char *result = lsm_mcp_text_result(out_str, is_error);
    free(out_str);
    return result;
}
...
/* Read the legacy file-based ADR (<root>/.logan-spine/adr.md), used by
...
static lsm_store_t *open_adr_store_for_write(lsm_mcp_server_t *srv, lsm_store_t *resolved,
                                             lsm_store_t **owned_rw) {
    if (!srv || !resolved || !owned_rw) {
        return NULL;
    }
    const char *resolved_db_path = lsm_store_db_path(resolved);
    if (!resolved_db_path) {
        return resolved;
    }
    ...
    invalidate_cached_store(srv);
    *owned_rw = lsm_store_open_path(rw_path);
    free(rw_path);
    return *owned_rw;
}

static char *handle_manage_adr(lsm_mcp_server_t *srv, const char *args) {
    char *project = get_project_arg(args);
    char *mode_str = lsm_mcp_get_string_arg(args, "mode");
    char *content = lsm_mcp_get_string_arg(args, "content");
    ...
    if (write_request && project) {
        ...
        if (mcp_request_cancelled(srv)) {
            ...
            free(mode_str);
            free(content);
            return lsm_mcp_text_result("project operation cancelled for this request", true);
        }
    }
    ...
    lsm_store_t *resolved =
        resolve_store_internal(srv, project, mutation_held, !write_request, &recovery_status);
    if (!resolved) {
        char *res = NULL;
        if (recovery_status == STORE_RECOVERY_BUSY) {
            res = lsm_mcp_text_result("project is busy; retry after indexing", true);
        } else if (recovery_status == STORE_RECOVERY_TRY_GUARD_UNAVAILABLE) {
            res =
                lsm_mcp_text_result("project recovery requires a nonblocking mutation guard", true);
        } else {
            char *err = build_no_store_error(project);
            res = lsm_mcp_text_result(err, true);
            free(err);
        }
        if (mutation_held) {
            mcp_project_mutation_end(srv, project);
    ...
    lsm_store_t *store = resolved;
    lsm_store_t *owned_rw = NULL;
    if (write_request) {
        ...
        if (!store) {
            ...
            free(mode_str);
            free(content);
            return lsm_mcp_text_result("failed to open writable ADR store", true);
        }
    }

    /* One-time migration: older versions wrote ADRs to a file at
     * <root>/.logan-spine/adr.md. A read never waits for the project lease:
     * it returns the legacy content immediately and attempts migration only if
     * a nonblocking acquire succeeds. */
    lsm_adr_t adr;
    memset(&adr, 0, sizeof(adr));
    bool have_adr = (lsm_store_adr_get(store, project, &adr) == LSM_STORE_OK);
    char *legacy = NULL;
        ...
        if (legacy && mcp_project_mutation_try_begin(srv, project)) {
            if (!mcp_request_cancelled(srv)) {
                ...
                if (resolved) {
                    store = open_adr_store_for_write(srv, resolved, &owned_rw);
                    if (store) {
                        have_adr = (lsm_store_adr_get(store, project, &adr) == LSM_STORE_OK);
                        if (!have_adr &&
                            lsm_store_adr_store(store, project, legacy) == LSM_STORE_OK) {
                            have_adr = (lsm_store_adr_get(store, project, &adr) == LSM_STORE_OK);
                        }
                    }
                }
    ...
    if (write_request) {
        if (lsm_store_adr_store(store, project, content) == LSM_STORE_OK) {
...
static char *handle_ingest_traces(lsm_mcp_server_t *srv, const char *args) {
    (void)srv;
...
static char *dispatch_tool(lsm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
    if (!tool_name) {
        return lsm_mcp_text_result("missing tool name", true);
    }
 ...
 * process lifetime. */
static void release_request_store(lsm_mcp_server_t *srv) {
    if (!srv || !srv->owns_store || !srv->store || !lsm_store_db_path(srv->store)) {
        return;
    }
    lsm_store_close(srv->store);
    srv->store = NULL;
...
char *lsm_mcp_handle_tool(lsm_mcp_server_t *srv, const char *tool_name, const char *args_json) {
     ...
     * cannot hide between them (LSM_MEM_PHASES=1; see foundation/mem.h). The
    ...
    lsm_mem_phase_mark("request.scope_begin");
    bool request_scope = !srv || lsm_mcp_server_request_scope_begin(srv);
    if (!request_scope) {
        release_request_store(srv);
        lsm_mem_phase_mark("idle");
        return lsm_mcp_text_result("request cancellation scope unavailable", true);
    }
    lsm_mem_phase_mark("request.dispatch_tool");
    char *result = dispatch_tool(srv, tool_name, args_json);
    lsm_mem_phase_mark("request.scope_end");
    if (srv) {
        lsm_mcp_server_request_scope_end(srv);
    }
    lsm_mem_phase_mark("request.release_store");
    release_request_store(srv);
    lsm_mem_phase_mark("idle");
    ...
    lsm_mem_census_log("mcp.request");
    return result;
}
...
static void detect_session(lsm_mcp_server_t *srv) {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        ...
        if (strcmp(cwd, "/") != 0 && (home == NULL || strcmp(cwd, home) != 0)) {
            ...
            lsm_log_info("session.root.cwd", "path", cwd);
        }
    }

    /* Derive project name from path — must match lsm_project_name_from_path
    ...
    if (srv->session_root[0]) {
        char *pname = lsm_project_name_from_path(srv->session_root);
        if (pname) {
...
static bool auto_watch_enabled(lsm_mcp_server_t *srv) {
    if (!srv->config) {
        return true; /* default on */
    }
    return lsm_config_get_bool(srv->config, LSM_CONFIG_AUTO_WATCH, true);
}

/* Register the session project with the background watcher for ongoing
 * change detection — unless auto_watch is disabled. */
static void register_watcher_if_enabled(lsm_mcp_server_t *srv) {
    if (!srv->watcher || srv->session_project[0] == '\0' || srv->session_root[0] == '\0') {
        return;
    }
    if (!auto_watch_enabled(srv)) {
        lsm_log_info("watcher.register.skipped", "reason", "auto_watch_off", "project",
                     srv->session_project);
        return;
    }
    lsm_watcher_watch(srv->watcher, srv->session_project, srv->session_root);
}

/* Background auto-index thread function */
static void *autoindex_thread(void *arg) {
    lsm_mcp_server_t *srv = (lsm_mcp_server_t *)arg;

    lsm_log_info("autoindex.start", "project", srv->session_project, "path", srv->session_root);
    ...
    if (lsm_index_supervisor_should_wrap()) {
        ...
        if (resp) {
            free(resp);
            lsm_log_info("autoindex.done", "project", srv->session_project, "mode", "supervised");
        ...
        lsm_log_error("autoindex.supervision_failed", "project", srv->session_project, "action",
                      "fail_closed");
        return NULL;
    }

    lsm_pipeline_t *p = lsm_pipeline_new(srv->session_root, NULL, LSM_MODE_FULL);
    if (!p) {
        lsm_log_warn("autoindex.err", "msg", "pipeline_create_failed");
        return NULL;
    }
    ...
    lsm_pipeline_free(p);
    lsm_mem_collect(); /* return mimalloc pages to OS after indexing (in-process only) */

    if (rc == 0) {
        lsm_log_info("autoindex.done", "project", srv->session_project);
        register_watcher_if_enabled(srv);
    } else {
        lsm_log_warn("autoindex.err", "msg", "pipeline_run_failed");
    }
    return NULL;
}
...
bool lsm_mcp_auto_index_within_file_limit(const char *root_path, int file_limit,
    ...
    lsm_discover_opts_t options = {
        .mode = LSM_MODE_FULL,
        .ignore_file = NULL,
        .max_file_size = 0,
    };
    int count = -1;
    lsm_discover_status_t status = lsm_discover_count_bounded(
        root_path, &options, file_limit, lsm_now_ms() + AUTO_INDEX_COUNT_TIMEOUT_MS, &count);
    if (file_count_out) {
        *file_count_out = status == LSM_DISCOVER_LIMIT_EXCEEDED
                              ? (file_limit < INT_MAX ? file_limit + 1 : INT_MAX)
                              : count;
    }
    return status == LSM_DISCOVER_OK;
}

/* Start auto-indexing if configured and project not yet indexed. */
static void maybe_auto_index(lsm_mcp_server_t *srv) {
    ...
    const char *home = lsm_get_home_dir();
    if (home) {
        char db_check[LSM_SZ_1K];
        snprintf(db_check, sizeof(db_check), "%s/%s.db", lsm_resolve_cache_dir(),
                 srv->session_project);
        if (lsm_file_size(db_check) >= 0) {
            /* Already indexed → register watcher for change detection */
            lsm_log_info("autoindex.skip", "reason", "already_indexed", "project",
                         srv->session_project);
            register_watcher_if_enabled(srv);
            return;
        }
    ...
    int file_limit = LSM_MCP_DEFAULT_AUTO_INDEX_LIMIT;
    if (srv->config) {
        auto_index = lsm_config_get_bool(srv->config, LSM_CONFIG_AUTO_INDEX, false);
        file_limit = lsm_config_get_int(srv->config, LSM_CONFIG_AUTO_INDEX_LIMIT,
                                        LSM_MCP_DEFAULT_AUTO_INDEX_LIMIT);
    }

    if (!auto_index) {
        lsm_log_info("autoindex.skip", "reason", "disabled", "hint",
                     "run: logan-spine-mcp config set auto_index true");
        return;
    }

    /* Quick tracked-file count check to avoid OOM on massive repos. */
    int file_count = -1;
    if (!lsm_mcp_auto_index_within_file_limit(srv->session_root, file_limit, &file_count)) {
        char files[32];
        (void)snprintf(files, sizeof(files), "%d", file_count);
        lsm_log_warn("autoindex.skip", "reason",
                     file_count >= 0 ? "too_many_files" : "unsafe_or_unavailable_path", "files",
                     files, "limit", LSM_CONFIG_AUTO_INDEX_LIMIT);
        return;
    }

    /* Launch auto-index in background */
    if (lsm_thread_create(&srv->autoindex_tid, 0, autoindex_thread, srv) == 0) {
...
char *lsm_mcp_server_handle(lsm_mcp_server_t *srv, const char *line) {
    lsm_jsonrpc_request_t req = {0};
    if (lsm_jsonrpc_parse(line, &req) < 0) {
        return lsm_jsonrpc_format_error(0, JSONRPC_PARSE_ERROR, "Parse error");
    }

    /* Notifications (no id) → handle cancellation, then no response */
    if (!req.has_id) {
        if (req.method && strcmp(req.method, "notifications/cancelled") == 0) {
            if (lsm_mcp_cancel_request_matches(req.params_raw, srv->active_request_id,
                                               srv->active_request_id_str) &&
                lsm_mcp_server_cancel_active(srv)) {
                lsm_log_info("mcp.cancelled", "match", "true");
            }
        }
        lsm_jsonrpc_request_free(&req);
        return NULL;
    }

    if (!lsm_mcp_server_request_scope_begin(srv)) {
        int64_t request_id = req.id;
        lsm_jsonrpc_request_free(&req);
        return lsm_jsonrpc_format_error(request_id, JSONRPC_INTERNAL_ERROR,
                                        "Request cancellation scope unavailable");
    }

    struct timespec req_t0;
    lsm_clock_gettime(CLOCK_MONOTONIC, &req_t0);
    char *result_json = NULL;
    char *request_error_json = NULL;
    bool request_logged = false;

    if (strcmp(req.method, "initialize") == 0) {
        result_json = lsm_mcp_initialize_response_for_profile(req.params_raw, srv->tool_profile);
        detect_session(srv);
        if (srv->background_tasks && srv->tool_profile == LSM_MCP_TOOL_PROFILE_ALL) {
        ...
        result_json = lsm_mcp_prompts_list();
    } else if (strcmp(req.method, "prompts/get") == 0) {
        result_json = lsm_mcp_prompt_get(req.params_raw, &request_error_json);
    } else if (strcmp(req.method, "tools/list") == 0) {
        result_json = lsm_mcp_tools_list_page(srv->tool_profile, req.params_raw);
    } else if (strcmp(req.method, "tools/call") == 0) {
        char *tool_name = req.params_raw ? lsm_mcp_get_tool_name(req.params_raw) : NULL;
        char *tool_args =
            req.params_raw ? lsm_mcp_get_arguments(req.params_raw) : heap_strdup("{}");
        ...
        result_json = lsm_mcp_handle_tool(srv, tool_name, tool_args);
        srv->active_request_id = LSM_NOT_FOUND;
        free(srv->active_request_id_str);
        srv->active_request_id_str = NULL;
        struct timespec t1;
        lsm_clock_gettime(CLOCK_MONOTONIC, &t1);
        ...
        lsm_jsonrpc_response_t err_resp = {
            .id = req.id,
            .id_str = req.id_str,
            .error_json = err_obj,
        };
        char *err = lsm_jsonrpc_format_response(&err_resp);
        struct timespec t1;
        lsm_clock_gettime(CLOCK_MONOTONIC, &t1);
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        lsm_log_mcp_request(req.method, NULL, true, dur_us);
        lsm_mcp_server_request_scope_end(srv);
        lsm_jsonrpc_request_free(&req);
        return err;
    }
    ...
    if (!request_logged) {
        ...
        long long dur_us = ((long long)(t1.tv_sec - req_t0.tv_sec) * MCP_S_TO_US) +
                           ((long long)(t1.tv_nsec - req_t0.tv_nsec) / MCP_MS_TO_US);
        lsm_log_mcp_request(req.method, NULL, false, dur_us);
    }

    lsm_jsonrpc_response_t resp = {
        .id = req.id,
        .id_str = req.id_str,
        .result_json = result_json,
    };
    char *out = lsm_jsonrpc_format_response(&resp);
    free(result_json);
    lsm_mcp_server_request_scope_end(srv);
    lsm_jsonrpc_request_free(&req);
    return out;
...
static bool parse_content_length(const char *line, size_t *out) {
    ...
    errno = 0;
    char *end = NULL;
    unsigned long long parsed = strtoull(cursor, &end, LSM_DECIMAL_BASE);
...
static int poll_for_input_unix(lsm_mcp_server_t *srv, int fd, FILE *in) {
    ...
    if (saved_flags < 0) {
        ...
        if (pr == 0) {
            lsm_mcp_server_evict_idle(srv, STORE_IDLE_TIMEOUT_S);
            return 0;
        }
        return SKIP_ONE;
    }
...
int lsm_mcp_server_run(lsm_mcp_server_t *srv, FILE *in, FILE *out) {
    ...
    _setmode(lsm_fileno(in), _O_BINARY);
    _setmode(lsm_fileno(out), _O_BINARY);
#endif
    ...
    for (;;) {
        ...
        if (lsm_mcp_read_message(in, &message, &content_length_framed) <= 0) {
            break;
        }

        char *resp = lsm_mcp_server_handle(srv, message);
        free(message);
        if (resp) {
