#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_strdup/strndup/sprintf
#include "helpers.h"
#include "lang_specs.h"      // LSMLangSpec, LSMEmbeddedLangSpec, lsm_lang_spec, lsm_ts_language
...
static void parse_go_imports(LSMExtractCtx *ctx);
static void parse_python_imports(LSMExtractCtx *ctx);
static void parse_es_imports(LSMExtractCtx *ctx);
static void parse_java_imports(LSMExtractCtx *ctx);
static void parse_rust_imports(LSMExtractCtx *ctx);
static void parse_c_imports(LSMExtractCtx *ctx);
static void parse_ruby_imports(LSMExtractCtx *ctx);
static void parse_lua_imports(LSMExtractCtx *ctx);
static void parse_r_imports(LSMExtractCtx *ctx);
static void parse_kotlin_imports(LSMExtractCtx *ctx);
static void parse_dart_imports(LSMExtractCtx *ctx);
static void parse_haskell_imports(LSMExtractCtx *ctx);
static void parse_zig_imports(LSMExtractCtx *ctx);
static void parse_generic_imports(LSMExtractCtx *ctx, const char *node_type);
static void parse_wolfram_imports(LSMExtractCtx *ctx);
static void parse_php_imports(LSMExtractCtx *ctx);
static void parse_csharp_imports(LSMExtractCtx *ctx);
static void parse_spec_imports(LSMExtractCtx *ctx);
static void parse_hare_imports(LSMExtractCtx *ctx);
static void parse_pascal_imports(LSMExtractCtx *ctx);
static void parse_powershell_imports(LSMExtractCtx *ctx);
static void parse_lisp_imports(LSMExtractCtx *ctx);
static void parse_starlark_imports(LSMExtractCtx *ctx);
static void parse_tcl_imports(LSMExtractCtx *ctx);
static void parse_teal_imports(LSMExtractCtx *ctx);
static void parse_zsh_imports(LSMExtractCtx *ctx);
static void parse_css_imports(LSMExtractCtx *ctx);
static void parse_html_imports(LSMExtractCtx *ctx);
static void parse_cmake_imports(LSMExtractCtx *ctx);
static void parse_bitbake_imports(LSMExtractCtx *ctx);
static void parse_kconfig_imports(LSMExtractCtx *ctx);
static void parse_gn_imports(LSMExtractCtx *ctx);
static void parse_just_imports(LSMExtractCtx *ctx);
static void parse_nix_imports(LSMExtractCtx *ctx);
static void parse_jsonnet_imports(LSMExtractCtx *ctx);
static void parse_pkl_imports(LSMExtractCtx *ctx);
static void parse_nickel_imports(LSMExtractCtx *ctx);
static void parse_thrift_imports(LSMExtractCtx *ctx);
static void parse_capnp_imports(LSMExtractCtx *ctx);
static void parse_dlang_imports(LSMExtractCtx *ctx);
static void parse_tablegen_imports(LSMExtractCtx *ctx);
static void parse_crystal_imports(LSMExtractCtx *ctx);
static void parse_fsharp_imports(LSMExtractCtx *ctx);
static void parse_ada_imports(LSMExtractCtx *ctx);
static void parse_elm_imports(LSMExtractCtx *ctx);
static void parse_move_imports(LSMExtractCtx *ctx);
static void parse_smali_imports(LSMExtractCtx *ctx);
static void parse_tlaplus_imports(LSMExtractCtx *ctx);
static void parse_vhdl_imports(LSMExtractCtx *ctx);
static void parse_wit_imports(LSMExtractCtx *ctx);
static void parse_smithy_imports(LSMExtractCtx *ctx);
static void parse_hyprlang_imports(LSMExtractCtx *ctx);

// Helper: strip quotes from a string literal
static char *strip_quotes(LSMArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    size_t len = strlen(s);
    if (len >= LSM_QUOTE_PAIR && (s[0] == '"' || s[0] == '\'') && s[len - SKIP_ONE] == s[0]) {
        return lsm_arena_strndup(a, s + SKIP_ONE, len - PAIR_LEN);
    }
    return lsm_arena_strdup(a, s);
}
...
static const char *path_last(LSMArena *a, const char *path) {
    if (!path) {
        return NULL;
    }
    ...
    if (last_sep) {
        return lsm_arena_strdup(a, last_sep + SKIP_ONE);
    }
    return path;
}
...
static const char *python_import_root(LSMArena *a, const char *path) {
    if (!path) {
        return NULL;
    }
    const char *dot = strchr(path, '.');
    return dot ? lsm_arena_strndup(a, path, (size_t)(dot - path)) : path;
}
...
static void parse_go_import_spec(LSMExtractCtx *ctx, TSNode spec) {
    LSMArena *a = ctx->arena;
    TSNode path_node = ts_node_child_by_field_name(spec, TS_FIELD("path"));
    if (ts_node_is_null(path_node)) {
        return;
    }
    char *path = strip_quotes(a, lsm_node_text(a, path_node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
    ...
    const char *local_name =
        !ts_node_is_null(name_node) ? lsm_node_text(a, name_node, ctx->source) : path_last(a, path);

    LSMImport imp = {.local_name = local_name, .module_path = path};
    lsm_imports_push(&ctx->result->imports, a, imp);
}
...
static void emit_py_aliased_import(LSMExtractCtx *ctx, TSNode child, const char *mod_prefix) {
    LSMArena *a = ctx->arena;
    ...
    char *name = lsm_node_text(a, mod_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    const char *local = !ts_node_is_null(alias_node) ? lsm_node_text(a, alias_node, ctx->source)
                                                     : path_last(a, name);
    const char *full = mod_prefix ? lsm_arena_sprintf(a, "%s.%s", mod_prefix, name) : name;
    LSMImport imp = {.local_name = local, .module_path = full};
    lsm_imports_push(&ctx->result->imports, a, imp);
}

// Process a single Python import_statement node (import X, import X as Y).
static void process_py_import_stmt(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    ...
    if (ts_node_is_null(name_node)) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "dotted_name") == 0 || strcmp(ck, "identifier") == 0) {
                char *mod = lsm_node_text(a, child, ctx->source);
                if (mod && mod[0]) {
                    LSMImport imp = {.local_name = python_import_root(a, mod), .module_path = mod};
                    lsm_imports_push(&ctx->result->imports, a, imp);
                }
        ...
        char *mod = lsm_node_text(a, name_node, ctx->source);
        if (mod && mod[0]) {
            LSMImport imp = {.local_name = python_import_root(a, mod), .module_path = mod};
            lsm_imports_push(&ctx->result->imports, a, imp);
        }
...
static void emit_py_import_from_name(LSMExtractCtx *ctx, TSNode child, const char *mod_path) {
    LSMArena *a = ctx->arena;
    char *name = lsm_node_text(a, child, ctx->source);
    if (name && name[0]) {
        const char *full = mod_path ? lsm_arena_sprintf(a, "%s.%s", mod_path, name) : name;
        LSMImport imp = {.local_name = name, .module_path = full};
        lsm_imports_push(&ctx->result->imports, a, imp);
    }
}

// Process a single Python import_from_statement node (from X import Y [as Z]).
static void process_py_import_from(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    ...
    if (strcmp(ts_node_type(node), "future_import_statement") == 0) {
        LSMImport imp = {.local_name = lsm_arena_strdup(a, "__future__"),
                         .module_path = lsm_arena_strdup(a, "__future__")};
        lsm_imports_push(&ctx->result->imports, a, imp);
        return;
    }
    TSNode module_node = resolve_py_module_node(node);
    char *mod_path =
        ts_node_is_null(module_node) ? NULL : lsm_node_text(a, module_node, ctx->source);
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (strcmp(ck, "identifier") == 0 || strcmp(ck, "dotted_name") == 0) {
            ...
            if (mod_path && mod_path[0]) {
                LSMImport imp = {.local_name = path_last(a, mod_path), .module_path = mod_path};
                lsm_imports_push(&ctx->result->imports, a, imp);
                emitted = true;
            }
        }
...
static bool process_named_imports(LSMExtractCtx *ctx, TSNode sub, const char *path) {
    LSMArena *a = ctx->arena;
    bool found = false;
    ...
    for (uint32_t m = 0; m < nc2; m++) {
        ...
        if (!ts_node_is_null(orig)) {
            char *local_name = !ts_node_is_null(local) ? lsm_node_text(a, local, ctx->source)
                                                       : lsm_node_text(a, orig, ctx->source);
            LSMImport imp = {.local_name = local_name, .module_path = path};
            lsm_imports_push(&ctx->result->imports, a, imp);
            found = true;
        }
...
static bool process_import_clause(LSMExtractCtx *ctx, TSNode clause, const char *path) {
    LSMArena *a = ctx->arena;
    bool found = false;
    ...
    for (uint32_t k = 0; k < cc; k++) {
        ...
        if (strcmp(sk, "identifier") == 0) {
            char *name = lsm_node_text(a, sub, ctx->source);
            LSMImport imp = {.local_name = name, .module_path = path};
            lsm_imports_push(&ctx->result->imports, a, imp);
            found = true;
            ...
            if (!ts_node_is_null(as_name)) {
                char *name = lsm_node_text(a, as_name, ctx->source);
                LSMImport imp = {.local_name = name, .module_path = path};
                lsm_imports_push(&ctx->result->imports, a, imp);
                found = true;
            }
...
static bool process_es_import_statement(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    TSNode source_node = find_es_source_node(node);
    if (ts_node_is_null(source_node)) {
        return false;
    }
    char *path = strip_quotes(a, lsm_node_text(a, source_node, ctx->source));
    if (!path || !path[0]) {
        return false;
    }
    uint32_t nc = ts_node_child_count(node);
    ...
    if (!found) {
        LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
        lsm_imports_push(&ctx->result->imports, a, imp);
    }
    return true;
}
...
static bool process_commonjs_require(LSMExtractCtx *ctx, TSNode call) {
    LSMArena *a = ctx->arena;
    ...
    char *fn_name = lsm_node_text(a, fn, ctx->source);
    ...
    for (uint32_t i = 0; i < argc; i++) {
        ...
        if (strcmp(ak, "string") == 0 || strcmp(ak, "string_literal") == 0 ||
            ...
            path = strip_quotes(a, lsm_node_text(a, arg, ctx->source));
            break;
        }
    }
    if (!path || !path[0]) {
    ...
    if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "variable_declarator") == 0) {
        ...
        if (!ts_node_is_null(name_node) && strcmp(ts_node_type(name_node), "identifier") == 0) {
            local_name = lsm_node_text(a, name_node, ctx->source);
        }
    }
    if (!local_name) {
...
static void walk_es_imports(LSMExtractCtx *ctx, TSNode root) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, LSM_SZ_512);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(kind, "import_statement") == 0) {
            ...
            if (!ts_node_is_null(src)) {
                char *path = strip_quotes(ctx->arena, lsm_node_text(ctx->arena, src, ctx->source));
                if (path && path[0]) {
                    LSMImport imp = {.local_name = path_last(ctx->arena, path),
                                     .module_path = path};
                    lsm_imports_push(&ctx->result->imports, ctx->arena, imp);
                }
            }
    ...
    do {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ck, "scoped_identifier") == 0 || strcmp(ck, "identifier") == 0) {
                char *path = lsm_node_text(a, child, ctx->source);
                if (path && path[0]) {
                    LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
                    lsm_imports_push(&ctx->result->imports, a, imp);
                }
...
static void parse_rust_imports(LSMExtractCtx *ctx) {
    ...
    do {
        ...
        char *full = lsm_node_text(a, node, ctx->source);
        if (!full) {
            continue;
        }
        ...
        LSMImport imp = {.local_name = path_last(a, full), .module_path = full};
        lsm_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static char *strip_angle_brackets(LSMArena *a, char *path) {
    if (path && path[0] == '<') {
        size_t len = strlen(path);
        if (len > SKIP_ONE && path[len - SKIP_ONE] == '>') {
            return lsm_arena_strndup(a, path + SKIP_ONE, len - PAIR_LEN);
        }
    }
    return path;
}
...
static const char *ruby_require_method(LSMArena *a, TSNode node, const char *source) {
    ...
    char *name = lsm_node_text(a, method, source);
...
static char *extract_ruby_require_arg(LSMArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t j = 0; j < ac; j++) {
        ...
        const char *ck = ts_node_type(c);
        if (strcmp(ck, "string") == 0 || strcmp(ck, "string_literal") == 0) {
            return strip_quotes(a, lsm_node_text(a, c, source));
        }
    }
    return strip_quotes(a, lsm_node_text(a, args, source));
}
...
static void parse_ruby_imports(LSMExtractCtx *ctx) {
    ...
    do {
        ...
        LSMImport imp = {.local_name = path_last(a, arg_text), .module_path = arg_text};
        lsm_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static void parse_lua_imports(LSMExtractCtx *ctx) {
    ...
    do {
        ...
        char *text = lsm_node_text(a, node, ctx->source);
        if (!text) {
            continue;
        }
        ...
        char *mod = lsm_arena_strndup(a, start, (size_t)(end - start));
        LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
        lsm_imports_push(&ctx->result->imports, a, imp);
    } while (ts_tree_cursor_goto_next_sibling(&cursor));
    ts_tree_cursor_delete(&cursor);
}
...
static void r_push_import(LSMExtractCtx *ctx, const char *raw) {
    LSMArena *a = ctx->arena;
    if (!raw || !raw[0]) {
        return;
    }
...
static void r_collect_imports(LSMExtractCtx *ctx, TSNode node) { // NOLINT(misc-no-recursion)
    if (strcmp(ts_node_type(node), "call") == 0) {
        ...
        if (!ts_node_is_null(fn) && !ts_node_is_null(args)) {
            ...
            if (strcmp(ft, "namespace_operator") == 0) {
                ...
                char *lt =
                    ts_node_is_null(lhs) ? NULL : lsm_node_text(ctx->arena, lhs, ctx->source);
                char *rt =
                    ts_node_is_null(rhs) ? NULL : lsm_node_text(ctx->arena, rhs, ctx->source);
                if (lt && rt && strcmp(lt, "box") == 0 && strcmp(rt, "use") == 0) {
                    ...
                    for (uint32_t i = 0; i < na; i++) {
                        ...
                        if (!ts_node_is_null(val)) {
                            r_push_import(ctx, lsm_node_text(ctx->arena, val, ctx->source));
                        }
                    }
                }
                ...
                char *fname = lsm_node_text(ctx->arena, fn, ctx->source);
                if (fname &&
...
static bool try_generic_path_fields(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
...
static void generic_import_from_text(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    char *text = lsm_node_text(a, node, ctx->source);
    if (!text || !text[0]) {
        return;
    ...
    if (text && text[0]) {
        LSMImport imp = {.local_name = path_last(a, text), .module_path = text};
        lsm_imports_push(&ctx->result->imports, a, imp);
    }
}
...
static void extract_one_import_header(LSMExtractCtx *ctx, TSNode header) {
...
static void parse_dart_imports(LSMExtractCtx *ctx) {
    ...
    do {
        ...
        if (find_first_descendant_of(node, "string_literal", &uri)) {
            char *path = strip_quotes(a, lsm_node_text(a, uri, ctx->source));
            if (path && path[0]) {
                LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
                lsm_imports_push(&ctx->result->imports, a, imp);
            }
        ...
        if (strcmp(ts_node_type(node), "builtin_function") == 0) {
            ...
            if (bf && (strncmp(bf, "@import", sizeof("@import") - 1) == 0 ||
                ...
                if (find_first_descendant_of(node, "string", &str)) {
                    char *path = strip_quotes(a, lsm_node_text(a, str, ctx->source));
                    if (path && path[0]) {
                        LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
                        lsm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void process_wolfram_get_top(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "string") == 0 || strcmp(ck, "user_symbol") == 0) {
            char *text = lsm_node_text(a, child, ctx->source);
            if (text && text[0]) {
...
static void process_wolfram_needs(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    ...
    char *name = lsm_node_text(a, head, ctx->source);
    if (!name || strcmp(name, "Needs") != 0) {
        return;
    }
    TSNode arg = ts_node_named_child(node, SECOND_IDX);
    char *text = lsm_node_text(a, arg, ctx->source);
    if (text && text[0]) {
        char *path = strip_quotes(a, text);
        LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
        lsm_imports_push(&ctx->result->imports, a, imp);
...
static void walk_wolfram_imports(LSMExtractCtx *ctx, TSNode root) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, LSM_SZ_512);
    ts_nstack_push(&stack, ctx->arena, root);
...
static void emit_php_use_clause(LSMExtractCtx *ctx, TSNode clause, const char *group_prefix) {
    LSMArena *a = ctx->arena;
    ...
    char *path = lsm_node_text(a, path_node, ctx->source);
    if (!path || !path[0]) {
        return;
    }
    if (group_prefix && group_prefix[0]) {
        path = lsm_arena_sprintf(a, "%s\\%s", group_prefix, path);
    }
    ...
    const char *local =
        !ts_node_is_null(alias) ? lsm_node_text(a, alias, ctx->source) : path_last(a, path);
    LSMImport imp = {.local_name = local, .module_path = path};
    lsm_imports_push(&ctx->result->imports, a, imp);
}

static void emit_php_use_decl(LSMExtractCtx *ctx, TSNode decl) {
    LSMArena *a = ctx->arena;
    ...
    if (find_first_descendant_of(decl, "namespace_use_group", &group)) {
        ...
        for (uint32_t i = 0; i < dc; i++) {
            ...
            if (strcmp(ck, "qualified_name") == 0 || strcmp(ck, "namespace_name") == 0 ||
                strcmp(ck, "name") == 0) {
                prefix = lsm_node_text(a, c, ctx->source);
                break;
            }
        }
...
static void parse_csharp_imports(LSMExtractCtx *ctx) {
    ...
    do {
        ...
        char *path = found ? lsm_node_text(a, path_node, ctx->source) : NULL;
        if (!path || !path[0]) {
...
// `import_node_types` declared in the language's LSMLangSpec.  Each root-level
...
// it.  The host's LSMLangSpec.embedded_imports declares which content nodes
...
// resolved through lsm_ts_language(spec->embedded_language), the same hook
// that the main parser uses.  Adding another host language is a one-line
// declaration in lang_specs.c.

static void embedded_collect_content_nodes(TSNode root, const LSMEmbeddedLangSpec *spec,
...
static void parse_embedded_imports(LSMExtractCtx *ctx) {
    const LSMLangSpec *spec = lsm_lang_spec(ctx->language);
    if (!spec || !spec->embedded_imports) {
        return;
    }
    for (const LSMEmbeddedLangSpec *e = spec->embedded_imports; e->script_node_type != NULL; e++) {
        const TSLanguage *embedded_lang = lsm_ts_language(e->embedded_language);
        if (!embedded_lang) {
...
static void capture_namespace_decl(LSMExtractCtx *ctx) {
    LSMArena *a = ctx->arena;
    ...
    do {
        ...
        for (const char **nk = name_kinds; *nk; nk++) {
            ...
            if (find_first_descendant_of(node, *nk, &nn)) {
                char *ns = lsm_node_text(a, nn, ctx->source);
                if (ns && ns[0]) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            for (uint32_t j = 0; j < nc; j++) {
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "scoped_identifier") == 0) {
                    char *mod = lsm_node_text(a, c, ctx->source);
                    if (mod && mod[0]) {
                        LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        lsm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void parse_powershell_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            char *nm = ts_node_is_null(name) ? NULL : lsm_node_text(a, name, ctx->source);
            if (nm && strcmp(nm, "using") == 0) {
                ...
                TSNodeStack inner;
                ts_nstack_init(&inner, a, LSM_SZ_512);
                ...
                while (inner.count > 0) {
                    ...
                    if (strcmp(ts_node_type(c), "generic_token") == 0) {
                        char *t = lsm_node_text(a, c, ctx->source);
                ...
                if (last_tok && last_tok[0]) {
                    LSMImport imp = {.local_name = path_last(a, last_tok),
...
static void lisp_push_module(LSMExtractCtx *ctx, TSNode mod_node) {
    LSMArena *a = ctx->arena;
    ...
    if (strcmp(mk, "symbol") == 0 || strcmp(mk, "string") == 0 || strcmp(mk, "sym_lit") == 0 ||
        ...
        mod = lsm_node_text(a, mod_node, ctx->source);
        if (mod) {
            size_t len = strlen(mod);
            if (len >= 2 && mod[0] == '(' && mod[len - 1] == ')') {
                mod = lsm_arena_strndup(a, mod + 1, len - 2);
            }
        }
    }
    if (mod && mod[0]) {
...
static char *lisp_strip_kw(LSMArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    if (s[0] == ':') {
...
static void lisp_set_namespace(LSMExtractCtx *ctx, TSNode name_node) {
    LSMArena *a = ctx->arena;
    char *ns = lisp_strip_kw(a, lsm_node_text(a, name_node, ctx->source));
 ...
 * `(:require ...)` / `(:use ...)` list. */
static void lisp_push_clause_modules(LSMExtractCtx *ctx, TSNode clause) {
...
static char *lisp_head_text(LSMExtractCtx *ctx, TSNode list, TSNode *out_head) {
    ...
    return lsm_node_text(ctx->arena, head, ctx->source);
}

/* Process one s-expression list as a potential import/namespace form. */
static void lisp_process_list(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    (void)a;
    TSNode head = node;
        ...
        for (uint32_t j = 2; j < nc; j++) {
            ...
            if (chn && (strcmp(chn, ":use") == 0 || strstr(chn, "import-from"))) {
                ...
                if (strcmp(c0k, "kwd_lit") == 0) {
                    char *kw = lsm_node_text(ctx->arena, ch0, ctx->source);
...
static void parse_starlark_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "call") == 0) {
            TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            char *fname = ts_node_is_null(fn) ? NULL : lsm_node_text(a, fn, ctx->source);
                    ...
                    if (find_first_descendant_of(args, "string", &str)) {
                        ...
                        if (path && path[0] && (path[0] == '"' || path[0] == '\'')) {
                            ...
                            if (find_first_descendant_of(str, "string_content", &content)) {
                                path = lsm_node_text(a, content, ctx->source);
                            }
                        }
                        if (path && path[0]) {
                            LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
            ...
            if (nm && strcmp(nm, "source") == 0) {
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    for (uint32_t j = 0; j < nc; j++) {
                        ...
                        char *mod = strip_quotes(a, lsm_node_text(a, c, ctx->source));
                        if (mod && mod[0]) {
                            LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                            lsm_imports_push(&ctx->result->imports, a, imp);
                            break;
...
static void parse_teal_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "function_call") == 0) {
            ...
            char *cn = ts_node_is_null(callee) ? NULL : lsm_node_text(a, callee, ctx->source);
            if (cn && strcmp(cn, "require") == 0) {
                ...
                if (!ts_node_is_null(args) && find_first_descendant_of(args, "string", &str)) {
                    char *mod = strip_quotes(a, lsm_node_text(a, str, ctx->source));
                    if (mod && mod[0] && (mod[0] == '"' || mod[0] == '\'')) {
                        TSNode content = str;
                        if (find_first_descendant_of(str, "string_content", &content)) {
                            mod = lsm_node_text(a, content, ctx->source);
                        }
                    }
                    if (mod && mod[0]) {
                        LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "command") == 0) {
            ...
            if (nm && (strcmp(nm, "source") == 0 || strcmp(nm, ".") == 0)) {
                ...
                if (!ts_node_is_null(arg)) {
                    char *mod = strip_quotes(a, lsm_node_text(a, arg, ctx->source));
                    if (mod && mod[0]) {
                        LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        lsm_imports_push(&ctx->result->imports, a, imp);
                    }
...
static void css_push_import_from_stmt(LSMExtractCtx *ctx, TSNode stmt) {
    LSMArena *a = ctx->arena;
    TSNode sv = stmt;
    ...
    if (find_first_descendant_of(sv, "string_content", &content)) {
        ...
        path = strip_quotes(a, lsm_node_text(a, sv, ctx->source));
    }
    if (!path || !path[0]) {
        return;
    }
...
static void html_extract_tag_src(LSMExtractCtx *ctx, TSNode tag) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        char *an = lsm_node_text(a, name, ctx->source);
        ...
        if (find_first_descendant_of(attr, "attribute_value", &val)) {
            path = lsm_node_text(a, val, ctx->source);
        }
        if (path && path[0]) {
            LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
            lsm_imports_push(&ctx->result->imports, a, imp);
...
static void push_path_import(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    char *path = strip_quotes(a, lsm_node_text(a, node, ctx->source));
    if (!path || !path[0]) {
        return;
    }
...
static void push_string_descendant_import(LSMExtractCtx *ctx, TSNode node) {
    TSNode hit = node;
...
static void parse_cmake_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "normal_command") == 0) {
            ...
            char *cmd = lsm_node_text(a, id, ctx->source);
            if (cmd && (strcmp(cmd, "include") == 0 || strcmp(cmd, "add_subdirectory") == 0)) {
                TSNode args = lsm_find_child_by_kind(node, "argument_list");
                if (!ts_node_is_null(args)) {
                    uint32_t nc = ts_node_named_child_count(args);
                    for (uint32_t j = 0; j < nc; j++) {
...
static void parse_meson_imports(LSMExtractCtx *ctx) {
    LSMArena *a = ctx->arena;
    TSNodeStack stack;
    ts_nstack_init(&stack, a, LSM_SZ_512);
    ts_nstack_push(&stack, a, ctx->root);
    while (stack.count > 0) {
        ...
        if (strcmp(k, "normal_command") == 0) {
            ...
            if (nc >= 1) {
                ...
                char *fname = lsm_node_text(a, fn, ctx->source);
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_declaration") == 0) {
            ...
            if (find_first_descendant_of(node, "module_fqn", &fqn)) {
                char *mod = lsm_node_text(a, fqn, ctx->source);
                if (mod && mod[0]) {
                    LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    lsm_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "import_decl") == 0 || strcmp(k, "open_expression") == 0) {
            ...
            if (find_first_descendant_of(node, "long_identifier", &id) ||
                ...
                char *mod = lsm_node_text(a, id, ctx->source);
                if (mod && mod[0]) {
                    LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    lsm_imports_push(&ctx->result->imports, a, imp);
                }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "import_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "upper_case_qid", &name)) {
                char *mod = lsm_node_text(a, name, ctx->source);
                if (mod && mod[0]) {
...
static void parse_move_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_declaration") == 0) {
            ...
            char *mod = NULL;
            if (!ts_node_is_null(arg)) {
                mod = lsm_node_text(a, arg, ctx->source);
...
static char *smali_demangle_descriptor(LSMArena *a, const char *desc) {
    if (!desc || !desc[0]) {
        return NULL;
    }
    ...
    char *out = lsm_arena_strndup(a, start, len);
    if (out) {
        for (char *p = out; *p; p++) {
            if (*p == '/') {
                *p = '.';
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(k, "super_directive") == 0 || strcmp(k, "implements_directive") == 0) {
            ...
            if (find_first_descendant_of(node, "class_identifier", &cid)) {
                char *mod = smali_demangle_descriptor(a, lsm_node_text(a, cid, ctx->source));
                if (mod && mod[0]) {
                    LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    lsm_imports_push(&ctx->result->imports, a, imp);
                }
        ...
        if (strcmp(ts_node_type(node), "use_clause") == 0) {
            ...
            if (find_first_descendant_of(node, "selected_name", &sel)) {
                ...
                if (!ts_node_is_null(pkg)) {
                    mod = lsm_node_text(a, pkg, ctx->source);
                }
                if (mod && mod[0]) {
                    LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                    lsm_imports_push(&ctx->result->imports, a, imp);
        ...
        if (strcmp(ts_node_type(node), "use_item") == 0) {
            ...
            if (find_first_descendant_of(node, "use_path", &path)) {
                ...
                if (find_first_descendant_of(path, "id", &idn)) {
                    char *mod = lsm_node_text(a, idn, ctx->source);
                    if (mod && mod[0]) {
                        LSMImport imp = {.local_name = path_last(a, mod), .module_path = mod};
                        lsm_imports_push(&ctx->result->imports, a, imp);
                    }
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "use_statement") == 0) {
            ...
            if (find_first_descendant_of(node, "absolute_root_shape_id", &sid) ||
                ...
                char *mod = lsm_node_text(a, sid, ctx->source);
                if (mod && mod[0]) {
...
static void parse_hyprlang_imports(LSMExtractCtx *ctx) {
    ...
    while (stack.count > 0) {
        ...
        if (strcmp(ts_node_type(node), "source") == 0) {
            ...
            char *path = NULL;
            if (find_first_descendant_of(node, "string", &str)) {
                path = strip_quotes(a, lsm_node_text(a, str, ctx->source));
            } else {
                path = strip_quotes(a, lsm_node_text(a, node, ctx->source));
            }
            if (path && path[0]) {
                LSMImport imp = {.local_name = path_last(a, path), .module_path = path};
                lsm_imports_push(&ctx->result->imports, a, imp);
...
void lsm_extract_imports(LSMExtractCtx *ctx) {
    ...
    case LSM_LANG_CMAKE:
        ...
        break;
    case LSM_LANG_BITBAKE:
        parse_bitbake_imports(ctx);
        break;
    case LSM_LANG_KCONFIG:
    ...
    case LSM_LANG_DLANG:
        ...
        break;
    case LSM_LANG_TABLEGEN:
        parse_tablegen_imports(ctx);
        break;
    case LSM_LANG_CRYSTAL:
        parse_crystal_imports(ctx);
        break;
    case LSM_LANG_FSHARP:
    ...
    case LSM_LANG_WIT:
        ...
        break;
    case LSM_LANG_SMITHY:
        parse_smithy_imports(ctx);
        break;
    case LSM_LANG_HYPRLANG:
    ...
    default:
        ...
        // import_node_types declared in their LSMLangSpec generically.
