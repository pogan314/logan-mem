...
static const char *extract_new_expr_type(LSMArena *a, TSNode rhs, const char *source) {
    ...
    if (!ts_node_is_null(type_node)) {
        ...
        if (strcmp(tk, "type_identifier") == 0 || strcmp(tk, "identifier") == 0 ||
            ...
            return lsm_node_text(a, type_node, source);
        }
        if (strcmp(tk, "generic_type") == 0 && ts_node_child_count(type_node) > 0) {
            return lsm_node_text(a, ts_node_child(type_node, 0), source);
        }
        return lsm_node_text(a, type_node, source);
    }
...
static const char *extract_constructor_type(LSMArena *a, TSNode rhs, const char *source,
                                            LSMLanguage lang) {
    ...
    if (strcmp(kind, "call") == 0 || strcmp(kind, "call_expression") == 0) {
        ...
        if (!ts_node_is_null(func)) {
            char *fname = lsm_node_text(a, func, source);
    ...
    if (lang == LSM_LANG_RUST && strcmp(kind, "struct_expression") == 0) {
...
static void try_emit_type_assign(LSMExtractCtx *ctx, TSNode var_node, TSNode rhs_node,
                                 const char *func_qn) {
    char *var_name = lsm_node_text(ctx->arena, var_node, ctx->source);
    ...
    if (var_name && var_name[0] && type_name && type_name[0]) {
        ...
        lsm_typeassign_push(&ctx->result->type_assigns, ctx->arena, ta);
    }
}

// Process assignment-type nodes (left/right fields with identifier check).
static void process_assignment_type_assign(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
...
static void process_go_var_type_assign(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
...
static void process_declarator_type_assign(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
...
static void process_rust_let_type_assign(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
...
// let_declaration).
static void process_type_assign_node(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                     const char *func_qn) {
    ...
    if (strcmp(kind, "let_declaration") == 0 && ctx->language == LSM_LANG_RUST) {
        process_rust_let_type_assign(ctx, node, func_qn);
    }
}

// Walk AST for assignment patterns where RHS is a constructor call.
static void walk_type_assigns(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, 4096);
    ts_nstack_push(&stack, ctx->arena, root);
    while (stack.count > 0) {
        TSNode node = ts_nstack_pop(&stack);
        process_type_assign_node(ctx, node, spec, lsm_enclosing_func_qn_cached(ctx, node));
