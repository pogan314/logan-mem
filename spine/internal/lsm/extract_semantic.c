...
static bool is_throw_node(TSNode node, const LSMLangSpec *spec) {
    if (lsm_kind_in_set(node, spec->throw_node_types)) {
        return true;
    }
    if (spec->language == LSM_LANG_KOTLIN && strcmp(ts_node_type(node), "jump_expression") == 0) {
...
static char *resolve_exception_name(LSMArena *a, TSNode throw_node, const char *source) {
...
static void extract_throws_clause(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                  const char *func_qn) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *exc = lsm_node_text(ctx->arena, child, ctx->source);
            if (exc && exc[0]) {
                LSMThrow thr = {.exception_name = exc, .enclosing_func_qn = func_qn};
                lsm_throws_push(&ctx->result->throws, ctx->arena, thr);
            }
        }
    }
}
...
static void process_throw_node(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    if (is_throw_node(node, spec)) {
        ...
        if (exc_name && exc_name[0]) {
            ...
            thr.enclosing_func_qn = lsm_enclosing_func_qn_cached(ctx, node);
            lsm_throws_push(&ctx->result->throws, ctx->arena, thr);
        }
    }

    extract_throws_clause(ctx, node, spec, lsm_enclosing_func_qn_cached(ctx, node));
}

// Iterative throw walker
static void walk_throws(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    TSNodeStack stack;
...
static char *resolve_lhs_write_name(LSMExtractCtx *ctx, TSNode left) {
    ...
    if (strcmp(lk, "identifier") == 0 || strcmp(lk, "simple_identifier") == 0) {
        return lsm_node_text(ctx->arena, left, ctx->source);
    }
    ...
    if (strcmp(lk, "index_expression") == 0 || strcmp(lk, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(base)) {
            ...
            if (strcmp(bk, "identifier") == 0 || strcmp(bk, "simple_identifier") == 0) {
                return lsm_node_text(ctx->arena, base, ctx->source);
            }
        }
        return NULL;
    }
    ...
    if (strcmp(lk, "field_expression") == 0 || strcmp(lk, "member_access_expression") == 0 ||
        ...
        if (!ts_node_is_null(fld)) {
            return lsm_node_text(ctx->arena, fld, ctx->source);
        }
        return NULL;
    }
    return NULL;
...
static void try_emit_assignment_write(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
    ...
    if (name && name[0] && !lsm_is_keyword(name, ctx->language)) {
        ...
        lsm_rw_push(&ctx->result->rw, ctx->arena, rw);
    }
}

static void walk_readwrites(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, LSM_SZ_512);
    ts_nstack_push(&stack, ctx->arena, root);
    while (stack.count > 0) {
        TSNode node = ts_nstack_pop(&stack);
        if (lsm_kind_in_set(node, spec->assignment_node_types)) {
            try_emit_assignment_write(ctx, node, lsm_enclosing_func_qn_cached(ctx, node));
        }
        uint32_t count = ts_node_child_count(node);
        for (int i = (int)count - LAST_IDX; i >= 0; i--) {
            ts_nstack_push(&stack, ctx->arena, ts_node_child(node, (uint32_t)i));
...
void handle_throws(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec, WalkState *state) {
...
void handle_readwrites(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec, WalkState *state) {
    if (!spec->assignment_node_types || !spec->assignment_node_types[0]) {
        return;
    }
