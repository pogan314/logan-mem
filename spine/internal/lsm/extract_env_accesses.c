#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_strndup
#include "helpers.h"
...
static const char *unquote(LSMArena *a, const char *s) {
    if (!s || !s[0]) {
        return NULL;
    }
    // Trim whitespace
...
static const char *extract_env_key_from_call(LSMExtractCtx *ctx, TSNode node,
    ...
    char *callee = lsm_node_text(ctx->arena, func_node, ctx->source);
    if (!callee) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < ts_node_child_count(args); i++) {
        ...
        char *arg_text = lsm_node_text(ctx->arena, child, ctx->source);
...
static const char *extract_env_key_from_member(LSMExtractCtx *ctx, TSNode node,
                                               const LSMLangSpec *spec) {
    ...
    for (const char **pat = spec->env_access_member_patterns; *pat; pat++) {
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '.') {
            const char *key = text + plen + LSM_QUOTE_OFFSET;
        ...
        if (strncmp(text, *pat, plen) == 0 && text[plen] == '[') {
            const char *inner = text + plen + LSM_QUOTE_OFFSET;
            size_t ilen = strlen(inner);
            if (ilen > 0 && inner[ilen - LSM_QUOTE_OFFSET] == ']') {
                char *bracket_content =
                    lsm_arena_strndup(ctx->arena, inner, ilen - LSM_QUOTE_OFFSET);
...
static void walk_env_accesses(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    TSNodeStack stack;
    ...
    while (stack.count > 0) {
        ...
        if (lsm_kind_in_set(node, spec->call_node_types)) {
        ...
        if (env_key && env_key[0] && is_env_var_name(env_key)) {
            LSMEnvAccess ea;
            ea.env_key = env_key;
            ea.enclosing_func_qn = lsm_enclosing_func_qn_cached(ctx, node);
            lsm_envaccess_push(&ctx->result->env_accesses, ctx->arena, ea);
...
void handle_env_accesses(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
    ...
    if (has_funcs && spec->call_node_types && lsm_kind_in_set(node, spec->call_node_types)) {
        env_key = extract_env_key_from_call(ctx, node, spec);
