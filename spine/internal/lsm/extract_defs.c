#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_alloc/strdup/sprintf
#include "helpers.h"
#include "lang_specs.h"
#include "foundation/constants.h"
#include "foundation/platform.h" // safe_realloc (frees old on failure)
#include "foundation/log.h"      // lsm_log_warn
...
enum {
    ...
    DECLARATOR_DEPTH_LIMIT = LSM_DECLARATOR_DEPTH_LIMIT, // shared define in helpers.h
...
static char *extract_body_ident_tokens(LSMExtractCtx *ctx, TSNode body) {
    ...
    while (bt_top > 0 && bt_count < BT_MAX_IDENTS) {
        ...
        if (nc == 0) {
            ...
            if (strcmp(k, "identifier") == 0 || strcmp(k, "field_identifier") == 0 ||
                ...
                if (len > 0 && len < LSM_SZ_64 && s < (uint32_t)ctx->source_len) {
    ...
    if (bt_pos > 0) {
        bt_buf[bt_pos] = '\0';
        return lsm_arena_strdup(ctx->arena, bt_buf);
    }
    return NULL;
}
...
static void compute_fingerprint(LSMExtractCtx *ctx, LSMDefinition *def, TSNode func_node) {
    ...
    lsm_minhash_t result;
    if (!lsm_minhash_compute(body, ctx->source, (int)ctx->language, &result)) {
        return; /* Too short or empty — no fingerprint */
    }
    /* Arena-allocate the fingerprint array */
    uint32_t *fp = lsm_arena_alloc(ctx->arena, LSM_MINHASH_K * sizeof(uint32_t));
    if (!fp) {
        return;
    }
    memcpy(fp, result.values, LSM_MINHASH_K * sizeof(uint32_t));
    def->fingerprint = fp;
    def->fingerprint_k = LSM_MINHASH_K;

    /* AST structural profile (signals 8, 9, 11) — rides the same body node */
    lsm_ast_profile_t profile;
    int pc = 0;
    if (def->param_names) {
    ...
    if (lsm_ast_profile_compute(body, ctx->source, def->param_names, pc, &profile)) {
        profile.body_lines = (uint16_t)def->lines;
        char sp_buf[LSM_AST_PROFILE_BUF];
        lsm_ast_profile_to_str(&profile, sp_buf, sizeof(sp_buf));
        def->structural_profile = lsm_arena_strdup(ctx->arena, sp_buf);
    }
}
...
static void extract_func_def(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec);
static void extract_class_def(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec);
static void walk_defs(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec, int depth_unused);
static void extract_variables(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec);
static void extract_var_names(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec);
static void extract_class_variables(LSMExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                    const LSMLangSpec *spec);
static void extract_rust_impl(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec);
static void extract_class_methods(LSMExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                  const LSMLangSpec *spec);
static void extract_class_fields(LSMExtractCtx *ctx, TSNode class_node, const char *class_qn,
                                 const LSMLangSpec *spec);
static TSNode find_class_body(TSNode class_node, LSMLanguage lang);
static void extract_enum_members(LSMExtractCtx *ctx, TSNode node, const char *class_qn);
static void extract_elixir_call(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec);
...
static TSNode func_name_node(TSNode node) {
    ...
    if (ts_node_is_null(name)) {
        ...
        name = lsm_find_child_by_kind(node, "rpc_name");
    }
    return name;
}
...
static TSNode resolve_ocaml_func_name(TSNode node) {
    TSNode binding = lsm_find_child_by_kind(node, "let_binding");
...
static TSNode resolve_sql_func_name(TSNode node) {
    TSNode obj_ref = lsm_find_child_by_kind(node, "object_reference");
    if (!ts_node_is_null(obj_ref)) {
        TSNode id = lsm_find_child_by_kind(obj_ref, "identifier");
        if (!ts_node_is_null(id)) {
            return id;
        }
    }
    return lsm_find_child_by_kind(node, "identifier");
}

// Zig: resolve test_declaration name from string→string_content.
static TSNode resolve_zig_test_name(TSNode node) {
    TSNode str_node = lsm_find_child_by_kind(node, "string");
    if (!ts_node_is_null(str_node)) {
        TSNode content = lsm_find_child_by_kind(str_node, "string_content");
...
static TSNode resolve_vimscript_func_name(TSNode node) {
    TSNode decl = lsm_find_child_by_kind(node, "function_declaration");
...
static TSNode resolve_func_name_scripting(TSNode node, LSMLanguage lang, const char *kind) {
    if (lang == LSM_LANG_LUA && strcmp(kind, "function_definition") == 0) {
        return resolve_lua_func_name(node);
    }
    if (lang == LSM_LANG_OCAML && strcmp(kind, "value_definition") == 0) {
        return resolve_ocaml_func_name(node);
    }
    if (lang == LSM_LANG_SQL && strcmp(kind, "create_function") == 0) {
        return resolve_sql_func_name(node);
    }
    if (lang == LSM_LANG_ZIG && strcmp(kind, "test_declaration") == 0) {
        return resolve_zig_test_name(node);
    }
    if (lang == LSM_LANG_VIMSCRIPT && strcmp(kind, "function_definition") == 0) {
        return resolve_vimscript_func_name(node);
    }
    if (lang == LSM_LANG_JULIA && strcmp(kind, "function_definition") == 0) {
    ...
    if (lang == LSM_LANG_JULIA && strcmp(kind, "assignment") == 0) {
        if (ts_node_named_child_count(node) > 0) {
...
static TSNode resolve_lean_func_name(TSNode node, TSNode name) {
    ...
    if (!ts_node_is_null(decl_id)) {
        TSNode id = lsm_find_child_by_kind(decl_id, "ident");
        if (!ts_node_is_null(id)) {
            return id;
        }
    ...
    return lsm_find_child_by_kind(node, "ident");
}
...
static TSNode resolve_commonlisp_func_name(TSNode node) {
    ...
    TSNode header = lsm_find_child_by_kind(node, "defun_header");
    if (!ts_node_is_null(header)) {
        return lsm_find_child_by_kind(header, "sym_lit");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static TSNode resolve_elm_func_name(TSNode node) {
    ...
    if (ts_node_is_null(fdl)) {
        fdl = lsm_find_child_by_kind(node, "function_declaration_left");
    }
...
static TSNode resolve_func_name_fp(TSNode node, LSMLanguage lang, const char *kind, TSNode name) {
    if (lang == LSM_LANG_COMMONLISP && strcmp(kind, "defun") == 0) {
        return resolve_commonlisp_func_name(node);
    }

    if (lang == LSM_LANG_MAKEFILE && strcmp(kind, "rule") == 0) {
        return resolve_makefile_func_name(node);
    }

    if (lang == LSM_LANG_HASKELL && strcmp(kind, "function") == 0) {
        return resolve_haskell_func_name(node);
    }

    if (lang == LSM_LANG_ELM && strcmp(kind, "value_declaration") == 0) {
        return resolve_elm_func_name(node);
    }

    if (lang == LSM_LANG_MATLAB && strcmp(kind, "function_definition") == 0) {
        if (!ts_node_is_null(name)) {
            return name;
        }
        return lsm_find_child_by_kind(node, "identifier");
    ...
    if (lang == LSM_LANG_LEAN) {
        return resolve_lean_func_name(node, name);
    }

    if (lang == LSM_LANG_WOLFRAM &&
...
char *lsm_cpp_out_of_line_parent_class(LSMArena *a, TSNode node, const char *source) {
    ...
    char *text = lsm_node_text(a, scope, source);
...
// like port lists).
enum { LSM_DESCENDANT_MAX_DEPTH = 6 };
...
// unified/calls extractor shares this one resolver — see lsm_resolve_func_name.
TSNode lsm_resolve_func_name(TSNode node, LSMLanguage lang);
...
static TSNode find_cpp_template_inner_node(TSNode node, LSMLanguage lang) {
    if ((lang != LSM_LANG_CPP && lang != LSM_LANG_CUDA) ||
...
static TSNode resolve_func_name_c_family(TSNode *node_ptr, LSMLanguage lang, const char *kind) {
    if ((lang == LSM_LANG_CPP || lang == LSM_LANG_CUDA) &&
    ...
    if ((lang == LSM_LANG_C || lang == LSM_LANG_CPP || lang == LSM_LANG_CUDA ||
         lang == LSM_LANG_GLSL || lang == LSM_LANG_HLSL || lang == LSM_LANG_ISPC ||
         lang == LSM_LANG_SLANG || lang == LSM_LANG_OBJC) &&
        ...
        return lsm_resolve_c_declarator_name_node(*node_ptr);
    }
    TSNode null_node = {0};
    return null_node;
}
...
TSNode lsm_resolve_func_name(TSNode node, LSMLanguage lang) {
    ...
    for (int tmpl_depth = 0; tmpl_depth < MAX_TEMPLATE_DEPTH; tmpl_depth++) {
        ...
        if (lang == LSM_LANG_HASKELL && strcmp(kind, "signature") == 0) {
            TSNode null_node = {0};
            return null_node;
        }
        ...
        if (lang == LSM_LANG_OBJECTSCRIPT_ROUTINE &&
            (strcmp(kind, "tag") == 0 || strcmp(kind, "procedure") == 0)) {
            return strcmp(kind, "tag") == 0 ? node : lsm_find_child_by_kind(node, "tag");
        }
        // ObjectScript method/classmethod: name lives under method_definition ->
        // method_name -> first named child.
        if (lang == LSM_LANG_OBJECTSCRIPT_UDL &&
            (strcmp(kind, "method") == 0 || strcmp(kind, "classmethod") == 0)) {
            TSNode mdef = lsm_find_child_by_kind(node, "method_definition");
            if (!ts_node_is_null(mdef)) {
                TSNode mname = lsm_find_child_by_kind(mdef, "method_name");
        ...
        if (lang == LSM_LANG_OBJECTSCRIPT_UDL && strcmp(kind, "query") == 0) {
            return lsm_find_child_by_kind(node, "query_name");
        }

        TSNode name = func_name_node(node);

        if (lang == LSM_LANG_R && strcmp(kind, "function_definition") == 0) {
        ...
        if ((lang == LSM_LANG_SWIFT || lang == LSM_LANG_KOTLIN) &&
            strcmp(kind, "function_declaration") == 0) {
            TSNode si = lsm_find_child_by_kind(node, "simple_identifier");
        ...
        if (lang == LSM_LANG_POWERSHELL && strcmp(kind, "function_statement") == 0) {
            TSNode fn = lsm_find_child_by_kind(node, "function_name");
            if (!ts_node_is_null(fn)) {
                return fn;
            }
        }
        ...
        if ((lang == LSM_LANG_CAIRO || lang == LSM_LANG_DLANG || lang == LSM_LANG_ODIN ||
             lang == LSM_LANG_SQUIRREL) &&
            (strcmp(kind, "function_definition") == 0 || strcmp(kind, "function_signature") == 0 ||
             strcmp(kind, "function_declaration") == 0 ||
             strcmp(kind, "procedure_declaration") == 0)) {
            TSNode id = lsm_find_child_by_kind(node, "identifier");
            if (!ts_node_is_null(id)) {
                return id;
            }
        }
        ...
        if (lang == LSM_LANG_ADA &&
            (strcmp(kind, "subprogram_body") == 0 || strcmp(kind, "subprogram_declaration") == 0)) {
            TSNode spec = lsm_find_child_by_kind(node, "procedure_specification");
            if (ts_node_is_null(spec)) {
                spec = lsm_find_child_by_kind(node, "function_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == LSM_LANG_PASCAL && strcmp(kind, "defProc") == 0) {
        ...
        if (lang == LSM_LANG_JUST && strcmp(kind, "recipe") == 0) {
            TSNode hdr = lsm_find_child_by_kind(node, "recipe_header");
            if (!ts_node_is_null(hdr)) {
                TSNode nm = ts_node_child_by_field_name(hdr, TS_FIELD("name"));
                if (!ts_node_is_null(nm)) {
                    return nm;
        ...
        if (lang == LSM_LANG_RESCRIPT && strcmp(kind, "function") == 0) {
            TSNode parent = ts_node_parent(node);
        ...
        if (lang == LSM_LANG_NICKEL && strcmp(kind, "fun_expr") == 0) {
            TSNode parent = ts_node_parent(node);
         ...
         * segments are scope, supplied by lsm_nix_attrpath_scope. Taking the first
         * segment named `a.b.fn` "a", which collided with every other binding
         * whose path began `a`. */
        if (lang == LSM_LANG_NIX && strcmp(kind, "function_expression") == 0) {
            TSNode parent = ts_node_parent(node);
            if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "binding") == 0) {
                ...
                if (!ts_node_is_null(attrpath)) {
                    TSNode attr = lsm_nix_attrpath_last_attr(attrpath);
        ...
        if (lang == LSM_LANG_FORTRAN &&
            (strcmp(kind, "subroutine") == 0 || strcmp(kind, "function") == 0)) {
            TSNode stmt = lsm_find_child_by_kind(node, "subroutine_statement");
            if (ts_node_is_null(stmt)) {
                stmt = lsm_find_child_by_kind(node, "function_statement");
            }
            if (!ts_node_is_null(stmt)) {
        ...
        if (lang == LSM_LANG_FSHARP && strcmp(kind, "function_or_value_defn") == 0) {
            TSNode lhs = lsm_find_child_by_kind(node, "function_declaration_left");
            if (ts_node_is_null(lhs)) {
                lhs = lsm_find_child_by_kind(node, "value_declaration_left");
            }
            if (!ts_node_is_null(lhs)) {
                TSNode nm = lsm_find_child_by_kind(lhs, "identifier");
                if (ts_node_is_null(nm)) {
                    nm = lsm_find_child_by_kind(lhs, "long_identifier");
                }
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
        ...
        if (lang == LSM_LANG_GROOVY && strcmp(kind, "function_definition") == 0) {
            TSNode fn = ts_node_child_by_field_name(node, TS_FIELD("function"));
            if (ts_node_is_null(fn)) {
                fn = lsm_find_child_by_kind(node, "identifier");
            }
            if (!ts_node_is_null(fn)) {
                return fn;
            }
        ...
        if (lang == LSM_LANG_AGDA && strcmp(kind, "function") == 0) {
            TSNode lhs = lsm_find_child_by_kind(node, "lhs");
            if (!ts_node_is_null(lhs)) {
                TSNode fn = lsm_find_child_by_kind(lhs, "function_name");
                if (!ts_node_is_null(fn)) {
                    return fn;
                }
            }
        ...
        if (lang == LSM_LANG_COBOL && strcmp(kind, "program_definition") == 0) {
            TSNode iddiv = lsm_find_child_by_kind(node, "identification_division");
            if (!ts_node_is_null(iddiv)) {
                TSNode pname = lsm_find_child_by_kind(iddiv, "program_name");
        ...
        if (lang == LSM_LANG_SCSS &&
            (strcmp(kind, "function_statement") == 0 || strcmp(kind, "mixin_statement") == 0)) {
            TSNode nm = lsm_find_child_by_kind(node, "name");
            if (!ts_node_is_null(nm)) {
                return nm;
            }
        }
        ...
        if (lang == LSM_LANG_JSONNET && strcmp(kind, "bind") == 0) {
        ...
        if (lang == LSM_LANG_TYPST && strcmp(kind, "let") == 0) {
        ...
        if (lang == LSM_LANG_SQL && strcmp(kind, "create_function") == 0) {
            TSNode oref = lsm_find_child_by_kind(node, "object_reference");
        ...
        if (lang == LSM_LANG_ELM && strcmp(kind, "value_declaration") == 0) {
            ...
            if (ts_node_is_null(lhs)) {
                lhs = lsm_find_child_by_kind(node, "function_declaration_left");
            }
            if (!ts_node_is_null(lhs)) {
                TSNode nm = lsm_find_child_by_kind(lhs, "lower_case_identifier");
                if (!ts_node_is_null(nm)) {
                    return nm;
                }
            }
        ...
        if (lang == LSM_LANG_PINE && strcmp(kind, "function_declaration_statement") == 0) {
            TSNode nm = ts_node_child_by_field_name(node, TS_FIELD("function"));
            if (ts_node_is_null(nm)) {
        ...
        if (lang == LSM_LANG_SMALI && strcmp(kind, "method_definition") == 0) {
            TSNode sig = lsm_find_child_by_kind(node, "method_signature");
            if (!ts_node_is_null(sig)) {
                TSNode mid = lsm_find_child_by_kind(sig, "method_identifier");
        ...
        if ((lang == LSM_LANG_VERILOG || lang == LSM_LANG_SYSTEMVERILOG) &&
            (strcmp(kind, "function_declaration") == 0 || strcmp(kind, "task_declaration") == 0)) {
            TSNode si =
                find_first_descendant_by_kind(node, "simple_identifier", LSM_DESCENDANT_MAX_DEPTH);
            if (!ts_node_is_null(si)) {
                return si;
            }
        }
        ...
        if (lang == LSM_LANG_VHDL && (strcmp(kind, "subprogram_declaration") == 0 ||
                                      strcmp(kind, "subprogram_definition") == 0)) {
            TSNode spec = lsm_find_child_by_kind(node, "function_specification");
            if (ts_node_is_null(spec)) {
                spec = lsm_find_child_by_kind(node, "procedure_specification");
            }
            if (!ts_node_is_null(spec)) {
        ...
        if (lang == LSM_LANG_THRIFT || lang == LSM_LANG_SMITHY) {
            TSNode id = lsm_find_child_by_kind(node, "identifier");
            if (!ts_node_is_null(id)) {
                return id;
            }
        ...
        if (lang == LSM_LANG_CAPNP) {
            ...
            if (name_kind) {
                TSNode id = lsm_find_child_by_kind(node, name_kind);
                if (!ts_node_is_null(id)) {
                    return id;
                }
            }
        ...
        if (lang == LSM_LANG_CMAKE &&
            (strcmp(kind, "function_def") == 0 || strcmp(kind, "macro_def") == 0)) {
            const char *cmd_kind =
                strcmp(kind, "function_def") == 0 ? "function_command" : "macro_command";
            TSNode cmd = lsm_find_child_by_kind(node, cmd_kind);
            if (!ts_node_is_null(cmd)) {
                TSNode alist = lsm_find_child_by_kind(cmd, "argument_list");
                if (!ts_node_is_null(alist)) {
                    TSNode arg = lsm_find_child_by_kind(alist, "argument");
                    if (!ts_node_is_null(arg)) {
                        TSNode uq = lsm_find_child_by_kind(arg, "unquoted_argument");
         ...
         * child (no `name` field). */
        if (lang == LSM_LANG_PUPPET &&
            (strcmp(kind, "function_declaration") == 0 || strcmp(kind, "lambda") == 0)) {
            TSNode id = lsm_find_child_by_kind(node, "identifier");
            if (ts_node_is_null(id)) {
                id = lsm_find_child_by_kind(node, "class_identifier");
            }
            if (!ts_node_is_null(id)) {
                return id;
            }
        ...
        if (lang == LSM_LANG_ASSEMBLY && strcmp(kind, "label") == 0) {
            TSNode id = lsm_find_child_by_kind(node, "ident");
            if (!ts_node_is_null(id)) {
                return id;
            }
        }
         ...
         * field). */
        if (lang == LSM_LANG_BITBAKE && (strcmp(kind, "function_definition") == 0 ||
         ...
         * child. */
        if (lang == LSM_LANG_PKL &&
            (strcmp(kind, "classMethod") == 0 || strcmp(kind, "objectMethod") == 0)) {
            TSNode hdr = lsm_find_child_by_kind(node, "methodHeader");
            if (!ts_node_is_null(hdr)) {
                TSNode id = lsm_find_child_by_kind(hdr, "identifier");
                if (!ts_node_is_null(id)) {
                    return id;
                }
            }
...
static bool is_js_exported(TSNode node) {
    return lsm_has_ancestor_kind(node, "export_statement", EXPORT_ANCESTOR_DEPTH);
}
...
static char *extract_comment_text(LSMArena *a, TSNode node, const char *source) {
    char *text = lsm_node_text(a, node, source);
...
static const char *extract_go_type_docstring(LSMArena *a, TSNode node, const char *source) {
    const char *kind = ts_node_type(node);
...
static const char *extract_python_docstring(LSMArena *a, TSNode node, const char *source) {
...
static const char *extract_docstring(LSMArena *a, TSNode node, const char *source,
                                     LSMLanguage lang) {
    if (lang == LSM_LANG_GO) {
...
static const char *route_path_from_string_node(LSMArena *a, TSNode node, const char *source) {
    if (!is_route_string_kind(ts_node_type(node))) {
        return NULL;
    }
    char *path = lsm_node_text(a, node, source);
    if (!path) {
        return NULL;
    }
    int plen = (int)strlen(path);
    if (plen >= PAIR_CHARS && (path[0] == '"' || path[0] == '\'')) {
        path = lsm_arena_strndup(a, path + SKIP_CHAR, (size_t)(plen - PAIR_CHARS));
    }
    return (path && path[0] == '/') ? path : NULL;
}

static const char *find_route_path_literal(LSMArena *a, TSNode node, const char *source,
                                           int max_depth) {
...
static const char *extract_route_path_from_args(LSMArena *a, TSNode args, const char *source) {
    ...
    for (uint32_t ai = 0; ai < nc && ai < DECORATOR_SCAN_LIMIT; ai++) {
        ...
        const char *path = find_route_path_literal(a, arg, source, LSM_DESCENDANT_MAX_DEPTH);
        if (path) {
            return path;
        }
    }
...
static TSNode find_drf_kwarg_in_args(LSMArena *a, TSNode args, const char *kwarg_name,
                                     const char *source) {
    uint32_t nc = ts_node_named_child_count(args);
    for (uint32_t ai = 0; ai < nc; ai++) {
        ...
        if (ts_node_is_null(name_node))
            continue;
        char *name = lsm_node_text(a, name_node, source);
...
static bool try_drf_action_decorator(LSMArena *a, TSNode dchild, const char *source,
    ...
    char *fn_text = lsm_node_text(a, fn, source);
    ...
    if (!ts_node_is_null(methods_val) && strcmp(ts_node_type(methods_val), "list") == 0) {
        ...
        for (uint32_t mi = 0; mi < mc && !method; mi++) {
            ...
            if (strcmp(ts_node_type(item), "string") != 0)
                continue;
            char *text = lsm_node_text(a, item, source);
            if (!text)
                continue;
            int tlen = (int)strlen(text);
            if (tlen < PAIR_CHARS || (text[0] != '"' && text[0] != '\''))
                continue;
            char inner[LSM_SZ_16];
            ...
            method = lsm_arena_strdup(a, inner);
        }
    }
    if (!method) {
        method = "GET";
    ...
    if (!ts_node_is_null(url_path_val) && strcmp(ts_node_type(url_path_val), "string") == 0) {
        char *text = lsm_node_text(a, url_path_val, source);
        if (text) {
            int tlen = (int)strlen(text);
            if (tlen >= PAIR_CHARS && (text[0] == '"' || text[0] == '\'')) {
                segment = lsm_arena_strndup(a, text + SKIP_CHAR, (size_t)(tlen - PAIR_CHARS));
            }
        }
    }
    if (!segment) {
        ...
        if (!ts_node_is_null(name_node)) {
            segment = lsm_node_text(a, name_node, source);
        }
    }
    if (!segment) {
        return false;
    ...
    if (detail) {
        *out_path = lsm_arena_sprintf(a, "/{pk}/%s", segment);
    } else {
        *out_path = lsm_arena_sprintf(a, "/%s", segment);
    }
    *out_method = method;
    return true;
}
...
static bool try_route_from_decorator_call(LSMArena *a, TSNode dchild, const char *source,
...
static TSNode annotation_name_node(TSNode annotation) {
    ...
    TSNode ut = lsm_find_child_by_kind(annotation, "user_type");
    if (ts_node_is_null(ut)) {
        TSNode ci = lsm_find_child_by_kind(annotation, "constructor_invocation");
        if (!ts_node_is_null(ci)) {
            ut = lsm_find_child_by_kind(ci, "user_type");
        }
    }
    if (!ts_node_is_null(ut)) {
        TSNode ti = lsm_find_child_by_kind(ut, "type_identifier");
        if (ts_node_is_null(ti)) {
            ti = lsm_find_child_by_kind(ut, "simple_identifier");
        }
        return ti;
    }
    TSNode null_node = {0};
...
static TSNode annotation_args_node(TSNode annotation) {
    ...
    TSNode ci = lsm_find_child_by_kind(annotation, "constructor_invocation");
    if (!ts_node_is_null(ci)) {
        return lsm_find_child_by_kind(ci, "value_arguments");
    }
    return args;
}
...
static bool try_route_from_annotation(LSMArena *a, TSNode annotation, const char *source,
                                      const char **out_path, const char **out_method) {
...
static void scan_route_annotations(LSMArena *a, TSNode owner, const char *source,
                                   const LSMLangSpec *spec, const char **out_map_path,
    ...
    *out_jax_path = NULL;
    ...
    for (int w = 0; w < wn; w++) {
        ...
        for (uint32_t ci = 0; ci < cc; ci++) {
            ...
            if (!lsm_kind_in_set(child, spec->decorator_node_types)) {
                continue;
            }
...
static bool extract_route_from_annotations(LSMArena *a, TSNode func_node, const char *source,
                                           const LSMLangSpec *spec, const char **out_path,
                                           const char **out_method) {
...
static void extract_route_from_decorators(LSMArena *a, TSNode func_node, const char *source,
                                          const LSMLangSpec *spec, const char **out_path,
                                          const char **out_method) {
    *out_path = NULL;
    *out_method = NULL;
    ...
    while (!ts_node_is_null(prev)) {
        if (!lsm_kind_in_set(prev, spec->decorator_node_types)) {
            break;
        }
...
static const char *join_route_paths(LSMArena *a, const char *prefix, const char *path) {
    if (!path || !path[0]) {
        return prefix;
    }
    ...
    if (prefix_slash && path_slash) {
        return lsm_arena_sprintf(a, "%s%s", prefix, path + SKIP_CHAR);
    }
    if (!prefix_slash && !path_slash) {
        return lsm_arena_sprintf(a, "%s/%s", prefix, path);
    }
    return lsm_arena_sprintf(a, "%s%s", prefix, path);
}

static const char *spring_class_route_prefix(LSMArena *a, TSNode class_node, const char *source,
                                             const LSMLangSpec *spec) {
    const char *map_path = NULL;
    const char *method = NULL;
    const char *jax_path = NULL;
...
static int count_modifier_annotations(TSNode modifiers, const LSMLangSpec *spec) {
    int count = 0;
    uint32_t mc = ts_node_child_count(modifiers);
    for (uint32_t mi = 0; mi < mc; mi++) {
        TSNode mchild = ts_node_child(modifiers, mi);
        if (lsm_kind_in_set(mchild, spec->decorator_node_types)) {
            count++;
        }
    }
    return count;
...
static TSNode find_jvm_modifiers(TSNode node, LSMLanguage lang) {
    ...
    if (ts_node_is_null(w)) {
        w = lsm_find_child_by_kind(node, wrapper);
    }
    return w;
}
...
static int count_child_decorators(TSNode node, const LSMLangSpec *spec) {
    int count = 0;
...
static int collect_child_decorators(LSMArena *a, TSNode node, const char *source,
                                    const LSMLangSpec *spec, const char **result, int idx,
                                    int max) {
    uint32_t cc = ts_node_child_count(node);
    for (uint32_t ci = 0; ci < cc && idx < max; ci++) {
        TSNode child = ts_node_child(node, ci);
        if (lsm_kind_in_set(child, spec->decorator_node_types)) {
            result[idx++] = lsm_node_text(a, child, source);
        }
    }
    return idx;
}
...
static int collect_modifier_decorators(LSMArena *a, TSNode modifiers, const char *source,
                                       const LSMLangSpec *spec, const char **result, int idx,
                                       int max) {
    uint32_t mc = ts_node_child_count(modifiers);
    for (uint32_t mi = 0; mi < mc && idx < max; mi++) {
        TSNode mchild = ts_node_child(modifiers, mi);
        if (lsm_kind_in_set(mchild, spec->decorator_node_types)) {
            result[idx++] = lsm_node_text(a, mchild, source);
        }
    }
    return idx;
}
...
static const char **extract_decorators(LSMArena *a, TSNode node, const char *source,
                                       LSMLanguage lang, const LSMLangSpec *spec) {
    if (!spec->decorator_node_types || !spec->decorator_node_types[0]) {
        return NULL;
    }
    ...
    const char **result =
        (const char **)lsm_arena_alloc(a, sizeof(const char *) * (total + NULL_TERM));
    if (!result) {
        return NULL;
    }
    ...
    while (!ts_node_is_null(prev) && idx < count) {
        if (lsm_kind_in_set(prev, spec->decorator_node_types)) {
            result[idx++] = lsm_node_text(a, prev, source);
        } else if (ts_node_is_named(prev) && !is_comment_node(ts_node_type(prev))) {
            break;
        }
        prev = ts_node_prev_sibling(prev);
 ...
 * lsm's test detection is otherwise file-path-based (lsm_is_test_file:
...
static const char *rust_cfg_qualified_name(LSMArena *a, const char *base_qn,
    ...
    for (int i = 0; decorators[i]; i++) {
        ...
        buf[bi] = '\0';
        return lsm_arena_sprintf(a, "%s#%s", base_qn, buf);
    }
    return base_qn;
}

// Extract base class name text from a single base_class child node.
static char *extract_cpp_base_text(LSMArena *a, TSNode bc, const char *source) {
    ...
    if (strcmp(bk, "type_identifier") == 0 || strcmp(bk, "qualified_identifier") == 0 ||
        ...
        char *t = lsm_node_text(a, bc, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    if (strcmp(bk, "template_type") == 0) {
        ...
        if (!ts_node_is_null(tname)) {
            return lsm_node_text(a, tname, source);
        }
    }
    return NULL;
}
...
static const char **extract_cpp_base_classes(LSMArena *a, TSNode clause, const char *source) {
    ...
    if (base_count > 0) {
        const char **result =
            (const char **)lsm_arena_alloc(a, (base_count + NULL_TERM) * sizeof(const char *));
        if (result) {
...
static const char **make_single_base(LSMArena *a, const char *text) {
    if (!text || !text[0]) {
        return NULL;
    }
    const char **result = (const char **)lsm_arena_alloc(a, sizeof(const char *) * RT_PAIR_SIZE);
    if (result) {
        result[0] = text;
...
static const char **find_base_from_children(LSMArena *a, TSNode node, const char *source,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        for (const char **t = base_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                const char **r = make_single_base(a, lsm_node_text(a, child, source));
                if (r) {
                    return r;
                }
            }
...
static const char *extract_csharp_base_child_text(LSMArena *a, TSNode bc, const char *source) {
    const char *bk = ts_node_type(bc);
    char *text = NULL;
    if (strcmp(bk, "identifier") == 0 || strcmp(bk, "generic_name") == 0 ||
        strcmp(bk, "qualified_name") == 0) {
        text = lsm_node_text(a, bc, source);
    } else {
        TSNode inner = ts_node_named_child(bc, 0);
        if (!ts_node_is_null(inner)) {
            text = lsm_node_text(a, inner, source);
        }
    }
    if (text && text[0]) {
...
static const char **collect_csharp_bases(LSMArena *a, TSNode base_list, const char *source) {
    const char *bases[MAX_BASES];
    int base_count = 0;
...
static const char **extract_csharp_base_list(LSMArena *a, TSNode node, const char *source,
                                             uint32_t count) {
    for (uint32_t i = 0; i < count; i++) {
        TSNode child = ts_node_child(node, i);
...
static void push_base_text(LSMArena *a, TSNode n, const char *source, const char **out, int out_cap,
                           int *count) {
    if (*count >= out_cap) {
        return;
    }
    char *t = lsm_node_text(a, n, source);
    if (!t) {
        return;
    }
    char *angle = strchr(t, '<');
...
static int collect_ts_bases(LSMArena *a, TSNode clause, const char *source, const char **out,
...
static const char **extract_ts_bases(LSMArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    ...
    const char **result =
        (const char **)lsm_arena_alloc(a, (size_t)(count + NULL_TERM) * sizeof(const char *));
    if (!result) {
        return NULL;
    }
...
static const char **extract_php_bases(LSMArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
...
static const char **extract_kotlin_bases(LSMArena *a, TSNode node, const char *source) {
    const char *bases[MAX_BASES];
    int count = 0;
    uint32_t nc = ts_node_child_count(node);
...
static int collect_bases_from_field(LSMArena *a, TSNode field_node, const char *source,
    ...
    if (strcmp(fk, "type_identifier") == 0 || strcmp(fk, "generic_type") == 0 ||
        ...
        char *t = lsm_node_text(a, field_node, source);
        if (t) {
            char *angle = strchr(t, '<');
            if (angle) {
                *angle = '\0';
    ...
    for (uint32_t i = 0; i < nc && count < out_cap; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "generic_type") == 0 ||
            ...
            char *t = lsm_node_text(a, child, source);
            if (t) {
                char *angle = strchr(t, '<');
                if (angle) {
                    *angle = '\0';
            ...
            if (!ts_node_is_null(val)) {
                char *t = lsm_node_text(a, val, source);
                if (t && t[0]) {
                    out[count++] = t;
                }
            }
            ...
            for (uint32_t ti = 0; ti < tlnc && count < out_cap; ti++) {
                ...
                if (strcmp(tlk, "type_identifier") == 0 || strcmp(tlk, "generic_type") == 0 ||
                    ...
                    char *t = lsm_node_text(a, tl_child, source);
                    if (t) {
                        char *angle = strchr(t, '<');
                        if (angle) {
                            *angle = '\0';
...
static const char **extract_julia_base_classes(LSMArena *a, TSNode node, const char *source) {
    TSNode th = lsm_find_child_by_kind(node, "type_head");
    ...
    char *bname = lsm_node_text(a, base, source);
    if (!bname || !bname[0]) {
        return NULL;
    }
    const char **result = (const char **)lsm_arena_alloc(a, 2 * sizeof(const char *));
    if (!result) {
        return NULL;
    }
    result[0] = bname;
...
static const char **extract_base_classes(LSMArena *a, TSNode node, const char *source,
                                         LSMLanguage lang) {
    // ObjectScript: `Class X Extends (A, B)` — bases are class_name children of
    // the class_extends node.
    if (lang == LSM_LANG_OBJECTSCRIPT_UDL) {
        TSNode ext = lsm_find_child_by_kind(node, "class_extends");
        if (!ts_node_is_null(ext)) {
            ...
            for (uint32_t i = 0; i < nc && base_count < MAX_BASES_MINUS_1; i++) {
                ...
                if (strcmp(ts_node_type(ch), "class_name") == 0) {
                    char *base = lsm_node_text(a, ch, source);
                    if (base && base[0]) {
            ...
            if (base_count > 0) {
                const char **result =
                    (const char **)lsm_arena_alloc(a, (base_count + 1) * sizeof(const char *));
                if (result) {
    ...
    if (lang == LSM_LANG_TYPESCRIPT || lang == LSM_LANG_TSX) {
        const char **ts_result = extract_ts_bases(a, node, source);
        if (ts_result) {
            return ts_result;
        }
    }
    if (lang == LSM_LANG_PHP) {
    ...
    if (lang == LSM_LANG_SQUIRREL) {
        ...
        for (uint32_t i = 0; i < sc; i++) {
            ...
            if (seen_extends && strcmp(ck, "identifier") == 0) {
                char *base = lsm_node_text(a, child, source);
                if (base && base[0]) {
                    const char **result =
                        (const char **)lsm_arena_alloc(a, 2 * sizeof(const char *));
                    if (result) {
                        result[0] = base;
                        result[1] = NULL;
                        return result;
    ...
    if (lang == LSM_LANG_FSHARP) {
        TSNode inh =
            find_first_descendant_by_kind(node, "class_inherits_decl", LSM_DESCENDANT_MAX_DEPTH);
        if (!ts_node_is_null(inh)) {
            TSNode st = lsm_find_child_by_kind(inh, "simple_type");
            char *bn = ts_node_is_null(st) ? NULL : lsm_node_text(a, st, source);
            if (bn && bn[0]) {
                const char **result = (const char **)lsm_arena_alloc(a, 2 * sizeof(const char *));
                if (result) {
                    result[0] = bn;
    ...
    if (lang == LSM_LANG_DLANG) {
        ...
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = lsm_node_text(a, c, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
        if (pc > 0) {
            const char **result =
                (const char **)lsm_arena_alloc(a, (pc + NULL_TERM) * sizeof(const char *));
            if (result) {
                for (int i = 0; i < pc; i++) {
                    result[i] = pbases[i];
                }
    ...
    if (lang == LSM_LANG_POWERSHELL && strcmp(ts_node_type(node), "class_statement") == 0) {
        const char *pbases[MAX_BASES];
        int pc = 0;
    ...
    if (lang == LSM_LANG_PASCAL && strcmp(ts_node_type(node), "declClass") == 0) {
        const char *pbases[MAX_BASES];
        int pc = 0;
        uint32_t nc = ts_node_child_count(node);
        for (uint32_t i = 0; i < nc && pc < MAX_BASES_MINUS_1; i++) {
            ...
            char *bn = lsm_node_text(a, pn, source);
            if (bn && bn[0]) {
                pbases[pc++] = bn;
            }
        }
...
static char *clean_type_name(LSMArena *a, const char *raw) {
    if (!raw || !raw[0]) {
        return NULL;
    }
    const char *s = raw;
    ...
    char *result = lsm_arena_alloc(a, len + NULL_TERM);
    memcpy(result, s, len);
    result[len] = '\0';
    return result;
}
...
static char *resolve_param_name(LSMArena *a, TSNode param, const char *source) {
    ...
    if (strcmp(pk, "argument") == 0 || strcmp(pk, "tag_parameter") == 0) {
        ...
        return ts_node_is_null(name) ? NULL : lsm_node_text(a, name, source);
    }

    if (strcmp(pk, "parameter_declaration") == 0) {
        TSNode nm = ts_node_child_by_field_name(param, TS_FIELD("name"));
        if (!ts_node_is_null(nm)) {
            return lsm_node_text(a, nm, source);
        }
        return NULL;
    }
    if (strcmp(pk, "identifier") == 0) {
        return lsm_node_text(a, param, source);
    }
...
static const char **extract_param_names(LSMArena *a, TSNode params, const char *source,
                                        LSMLanguage lang) {
    (void)lang;
    if (ts_node_is_null(params)) {
        return NULL;
    ...
    const char **result =
        (const char **)lsm_arena_alloc(a, (count + NULL_TERM) * sizeof(const char *));
    for (int i = 0; i < count; i++) {
        result[i] = names[i];
    }
    result[count] = NULL;
...
static void add_cleaned_type(LSMArena *a, const char **types, int *count, char *type_text) {
...
static void extract_go_multi_return(LSMArena *a, TSNode rt_node, const char *source,
    ...
    for (uint32_t i = 0; i < nc && *count < MAX_RETURN_TYPES_MINUS_1; i++) {
        ...
        if (strcmp(ts_node_type(child), "parameter_declaration") == 0) {
            ...
            if (!ts_node_is_null(tn)) {
                add_cleaned_type(a, types, count, lsm_node_text(a, tn, source));
            }
        } else {
            add_cleaned_type(a, types, count, lsm_node_text(a, child, source));
        }
    }
}

// Build a NULL-terminated arena-allocated string array from a types buffer.
static const char **build_type_array(LSMArena *a, const char **types, int count) {
    if (count == 0) {
        return NULL;
    }
    const char **result =
...
static const char **extract_return_types(LSMArena *a, TSNode rt_node, const char *source,
                                         LSMLanguage lang) {
    (void)lang;
    ...
    if (strcmp(ts_node_type(rt_node), "parameter_list") == 0) {
        ...
        add_cleaned_type(a, types, &count, lsm_node_text(a, rt_node, source));
    }
...
static char *extract_ts_param_type(LSMArena *a, TSNode param, const char *source) {
    TSNode ta = lsm_find_child_by_kind(param, "type_annotation");
    ...
    for (uint32_t ti = 0; ti < tanc; ti++) {
        ...
        if (strcmp(tk, "type_identifier") == 0 || strcmp(tk, "generic_type") == 0 ||
            ...
            return lsm_node_text(a, tch, source);
        }
    }
    return NULL;
}
...
static char *resolve_jvm_param_type(LSMArena *a, TSNode param, const char *pk, const char *source,
                                    LSMLanguage lang) {
    ...
    if (lang == LSM_LANG_KOTLIN) {
        ...
        if (!ts_node_is_null(tn)) {
            return lsm_node_text(a, tn, source);
        }
        TSNode ut = lsm_find_child_by_kind(param, "user_type");
        return ts_node_is_null(ut) ? NULL : lsm_node_text(a, ut, source);
    }
    if (lang == LSM_LANG_SCALA || lang == LSM_LANG_DART) {
        TSNode tid = lsm_find_child_by_kind(param, "type_identifier");
        return ts_node_is_null(tid) ? NULL : lsm_node_text(a, tid, source);
    }
    if (lang == LSM_LANG_GROOVY) {
        TSNode tn = ts_node_child_by_field_name(param, TS_FIELD("type"));
        return ts_node_is_null(tn) ? NULL : lsm_node_text(a, tn, source);
    }
    if (lang == LSM_LANG_OCAML) {
        TSNode tp = lsm_find_child_by_kind(param, "typed_pattern");
...
static char *resolve_param_type_text(LSMArena *a, TSNode param, const char *source,
                                     LSMLanguage lang) {
    const char *pk = ts_node_type(param);

    if ((lang == LSM_LANG_OBJECTSCRIPT_UDL || lang == LSM_LANG_OBJECTSCRIPT_ROUTINE) &&
        (strcmp(pk, "argument") == 0 || strcmp(pk, "tag_parameter") == 0)) {
        TSNode return_type = lsm_find_child_by_kind(param, "return_type");
        TSNode typename = ts_node_is_null(return_type)
                              ? (TSNode){0}
                              : lsm_find_child_by_kind(return_type, "typename");
        return ts_node_is_null(typename) ? NULL : lsm_node_text(a, typename, source);
    }
    ...
    if (lang == LSM_LANG_KOTLIN || lang == LSM_LANG_SCALA || lang == LSM_LANG_DART ||
        lang == LSM_LANG_GROOVY || lang == LSM_LANG_OCAML) {
...
static void add_dedup_type(LSMArena *a, const char **types, int *count, char *type_text) {
    if (!type_text || !type_text[0]) {
        return;
    }
    char *cleaned = clean_type_name(a, type_text);
...
static const char **extract_param_types(LSMArena *a, TSNode params, const char *source,
                                        LSMLanguage lang) {
    if (ts_node_is_null(params)) {
        return NULL;
    }
...
static char *signature_receiver_name(LSMArena *a, TSNode param, const char *source,
    ...
    for (int depth = 0; depth < 4 && !ts_node_is_null(candidate); depth++) {
        if (ts_node_named_child_count(candidate) == 0) {
            return lsm_node_text(a, candidate, source);
        }
        if (!unwrap_wrappers) {
            return NULL;
        }
...
static bool is_signature_implicit_receiver(LSMArena *a, TSNode param, const char *source,
                                           LSMLanguage lang, bool method, bool first_parameter) {
    const char *pk = ts_node_type(param);
    ...
    if (method && first_parameter && lang == LSM_LANG_PYTHON) {
...
static bool is_signature_non_parameter(TSNode param, const char *source, LSMLanguage lang) {
    const char *pk = ts_node_type(param);
...
static int signature_param_multiplicity(TSNode param, LSMLanguage lang) {
    if (lang != LSM_LANG_GO) {
        return 1;
    }
    const char *pk = ts_node_type(param);
...
static char *resolve_signature_param_type_text(LSMArena *a, TSNode param, const char *source,
                                               LSMLanguage lang) {
    ...
    if (!ts_node_is_null(type_node)) {
        ...
        char *type_text = lsm_node_text(a, type_node, source);
        if (type_text && type_text[0]) {
            return type_text;
        }
    }

    TSNode annotation = lsm_find_child_by_kind(param, "type_annotation");
    if (!ts_node_is_null(annotation)) {
        uint32_t nc = ts_node_named_child_count(annotation);
        if (nc > 0) {
            char *type_text = lsm_node_text(a, ts_node_named_child(annotation, 0), source);
            if (type_text && type_text[0]) {
                return type_text;
            }
        }
    ...
    TSNode typed_pattern = lsm_find_child_by_kind(param, "typed_pattern");
...
static const char **extract_signature_param_types(LSMArena *a, TSNode params, const char *source,
                                                  LSMLanguage lang, bool method, int *out_count) {
    *out_count = 0;
    if (ts_node_is_null(params)) {
        return NULL;
    }
    ...
    const char **result =
        (const char **)lsm_arena_alloc(a, (size_t)(*out_count + NULL_TERM) * sizeof(const char *));
    if (!result) {
        *out_count = 0;
        return NULL;
    }
...
static TSNode unwrap_template_inner(TSNode node, LSMLanguage lang) {
...
static TSNode find_function_params(TSNode func_node, LSMLanguage lang) {
    ...
    if (ts_node_is_null(params) &&
        (lang == LSM_LANG_OBJECTSCRIPT_UDL || lang == LSM_LANG_OBJECTSCRIPT_ROUTINE)) {
        params = ts_node_child_by_field_name(func_node, TS_FIELD("parameter_list"));
    }
    if (ts_node_is_null(params) && lang == LSM_LANG_OBJECTSCRIPT_ROUTINE) {
        params = lsm_find_child_by_kind(func_node, "parameter_list");
    }
    if (ts_node_is_null(params) && lang == LSM_LANG_OBJECTSCRIPT_UDL) {
        TSNode method_definition = lsm_find_child_by_kind(func_node, "method_definition");
        if (!ts_node_is_null(method_definition)) {
            params = lsm_find_child_by_kind(method_definition, "arguments");
        }
    }
    if (ts_node_is_null(params) && lang == LSM_LANG_KOTLIN) {
        params = lsm_find_child_by_kind(func_node, "function_value_parameters");
    }
    if (ts_node_is_null(params) && (lang == LSM_LANG_C || lang == LSM_LANG_CPP ||
                                    lang == LSM_LANG_CUDA || lang == LSM_LANG_GLSL)) {
...
static void resolve_cpp_trailing_return(LSMArena *a, TSNode func_node, const char *source,
                                        LSMDefinition *def) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(ch), "trailing_return_type") == 0) {
            ...
            def->return_type = lsm_node_text(a, type_desc, source);
            if (def->return_type && def->return_type[0]) {
                const char **rt =
                    (const char **)lsm_arena_alloc(a, RT_PAIR_SIZE * sizeof(const char *));
                if (rt) {
...
static void set_def_complexity(LSMDefinition *def, TSNode body, const LSMLangSpec *spec) {
    lsm_complexity_t cx;
    lsm_compute_complexity(body, spec->branching_node_types, &cx);
...
static char *go_receiver_type_name(LSMArena *a, TSNode recv, const char *source) {
...
static char *resolve_cpp_test_macro_name(LSMArena *a, const char *macro, TSNode node,
                                         const char *source) {
    ...
    for (uint32_t i = 0; i < nc && count < 2; i++) {
        ...
        char *text = lsm_node_text(a, ts_node_is_null(type_node) ? child : type_node, source);
        if (text && text[0]) {
            args[count++] = text;
        }
    }
    ...
    if (count == 2) {
        return lsm_arena_sprintf(a, "%s_%s_%s", macro, args[0], args[1]);
    }
    if (count == 1) {
        return lsm_arena_sprintf(a, "%s_%s", macro, args[0]);
    }
    return NULL;
}
...
static void extract_func_def(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    ...
    TSNode name_node = lsm_resolve_func_name(node, ctx->language);
    if (ts_node_is_null(name_node)) {
        return;
    }

    char *name = lsm_func_name_node_text(a, name_node, ctx->source, ctx->language);
    if (!name || !name[0] || strcmp(name, "function") == 0) {
        return;
    }

    // Makefile special targets (.PHONY, .DEFAULT, .SUFFIXES, …) are directives,
    // not build-rule defs. Their leading '.' would also make lsm_fqn_compute
    // emit a "..PHONY" segment (a "double dot") and thus a malformed QN. Skip
    // any dot-prefixed Make target.
    if (ctx->language == LSM_LANG_MAKEFILE && name[0] == '.') {
        return;
    }
    ...
    bool is_gtest = false;
    if ((ctx->language == LSM_LANG_CPP || ctx->language == LSM_LANG_CUDA) &&
    ...
    if (ctx->language == LSM_LANG_NIX && lsm_nix_attr_is_interpolated(name_node)) {
        return;
    }
    ...
    if (ctx->language == LSM_LANG_NIX) {
        qn_name = lsm_nix_qn_name(a, node, ctx->source, name);
    }
    ...
    def.qualified_name =
        lsm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, qn_name, ctx->language);
    ...
    if (ctx->enclosing_class_qn &&
        (ctx->language == LSM_LANG_CPP || ctx->language == LSM_LANG_CUDA ||
         ctx->language == LSM_LANG_TYPESCRIPT || ctx->language == LSM_LANG_TSX ||
         ctx->language == LSM_LANG_NIX)) {
        def.qualified_name = lsm_arena_sprintf(a, "%s.%s", ctx->enclosing_class_qn, qn_name);
    }
    def.label = "Function";
    ...
    def.is_exported = lsm_is_exported(name, ctx->language);
    if (ctx->language == LSM_LANG_RUST &&
    ...
    if (!ts_node_is_null(params)) {
        def.signature = lsm_node_text(a, params, ctx->source);
    ...
    for (const char **f = rt_fields; *f; f++) {
        ...
        if (!ts_node_is_null(rt)) {
            def.return_type = lsm_node_text(a, rt, ctx->source);
    ...
    if (!ts_node_is_null(recv)) {
        def.receiver = lsm_node_text(a, recv, ctx->source);
        def.label = "Method";
         ...
         * is computed the same way (lsm_fqn_compute on the type name). */
        ...
        if (recv_type && recv_type[0]) {
            ...
            def.parent_class = lsm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path,
    ...
    if ((ctx->language == LSM_LANG_CPP || ctx->language == LSM_LANG_CUDA) &&
        ...
        char *scope_name = lsm_cpp_out_of_line_parent_class(a, node, ctx->source);
        if (scope_name && scope_name[0]) {
            const char *class_qn = lsm_fqn_compute(a, ctx->project, ctx->rel_path, scope_name);
            def.qualified_name = lsm_arena_sprintf(a, "%s.%s", class_qn, name);
            def.label = "Method";
    ...
    if (ctx->language == LSM_LANG_PONY && def.label && strcmp(def.label, "Function") == 0 &&
        spec->class_node_types) {
        for (TSNode cur = ts_node_parent(node); !ts_node_is_null(cur); cur = ts_node_parent(cur)) {
            if (lsm_kind_in_set(cur, spec->class_node_types)) {
                def.label = "Method";
                TSNode cn = lsm_find_child_by_kind(cur, "identifier");
                if (!ts_node_is_null(cn)) {
                    char *cname = lsm_node_text(a, cn, ctx->source);
                    if (cname && cname[0]) {
                        def.parent_class = lsm_fqn_compute(a, ctx->project, ctx->rel_path, cname);
                    }
                }
                break;
            }
    ...
    // A function/method defined in a file lsm treats as a test file is itself
    ...
    if (ctx->language == LSM_LANG_JAVASCRIPT || ctx->language == LSM_LANG_TYPESCRIPT ||
...
// well-formed QN segment. Markdown headings (e.g. "Logan Spine") legitimately
...
static const char *qn_safe_segment(LSMArena *a, const char *name) {
    if (!name) {
        return name;
    }
    bool has_ws = false;
    ...
    char *out = lsm_arena_strdup(a, name);
    if (!out) {
        return name;
    }
    char *w = out;
...
static void push_simple_class_def(LSMExtractCtx *ctx, TSNode node, char *name, const char *label) {
    LSMArena *a = ctx->arena;
    LSMDefinition def;
    memset(&def, 0, sizeof(def));
    def.name = name;
...
static char *find_toml_key_name(LSMArena *a, TSNode node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "bare_key") == 0 || strcmp(ck, "dotted_key") == 0 ||
            ...
            return lsm_node_text(a, child, source);
        }
    }
    return NULL;
}
...
static char *find_xml_element_name(LSMArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
        if (strcmp(ck, "start_tag") == 0 || strcmp(ck, "self_closing_tag") == 0 ||
            ...
            for (uint32_t j = 0; j < tnc; j++) {
                ...
                if (strcmp(tk, "tag_name") == 0 || strcmp(tk, "Name") == 0) {
                    return lsm_node_text(a, tag, source);
                }
            }
        }
    }
    // Fallback: try "Name" field directly for some XML grammars
    TSNode name_child = lsm_find_child_by_kind(node, "Name");
    if (!ts_node_is_null(name_child)) {
        return lsm_node_text(a, name_child, source);
    }
    return NULL;
}

// Extract text from an atx_heading node (# Title).
static char *extract_atx_heading_text(LSMArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
    ...
    char *full = lsm_node_text(a, node, source);
    if (full) {
        char *p = full;
        while (*p == '#') {
            p++;
...
static char *extract_markdown_heading_name(LSMArena *a, TSNode node, const char *kind,
                                           const char *source) {
    char *name = NULL;
    if (strcmp(kind, "atx_heading") == 0) {
        ...
        if (ts_node_child_count(node) > 0) {
            name = lsm_node_text(a, ts_node_child(node, 0), source);
        }
    }
...
static char *find_ini_section_name(LSMArena *a, TSNode node, const char *source) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        ...
        TSNode text = lsm_find_child_by_kind(child, "text");
        if (!ts_node_is_null(text)) {
            return lsm_node_text(a, text, source);
        }
        return lsm_node_text(a, child, source);
    }
    return NULL;
...
static char *find_hcl_block_name(LSMArena *a, TSNode node, const char *source) {
    TSNode id = lsm_find_child_by_kind(node, "identifier");
    if (ts_node_is_null(id)) {
        return NULL;
    }
    char *name = lsm_node_text(a, id, source);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        TSNode lit = lsm_find_child_by_kind(ch, "template_literal");
        if (ts_node_is_null(lit)) {
            continue;
        }
        char *label = lsm_node_text(a, lit, source);
        if (!label || !label[0]) {
            continue;
        }
        char *joined = lsm_arena_sprintf(a, "%s.%s", name, label);
        if (joined) {
            name = joined;
        }
    }
...
static bool extract_config_class_def(LSMExtractCtx *ctx, TSNode node, const char *kind) {
    LSMArena *a = ctx->arena;
    char *name = NULL;
    const char *label = "Class";

    if (ctx->language == LSM_LANG_TOML &&
        (strcmp(kind, "table") == 0 || strcmp(kind, "table_array_element") == 0)) {
        name = find_toml_key_name(a, node, ctx->source);
    } else if (ctx->language == LSM_LANG_INI && strcmp(kind, "section") == 0) {
        name = find_ini_section_name(a, node, ctx->source);
    } else if (ctx->language == LSM_LANG_XML && strcmp(kind, "element") == 0) {
        name = find_xml_element_name(a, node, ctx->source);
    } else if (ctx->language == LSM_LANG_MARKDOWN &&
        ...
        label = "Section";
    } else if (ctx->language == LSM_LANG_HCL && strcmp(kind, "block") == 0) {
...
static void extract_class_def(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_OBJC) {
        name_node = lsm_find_child_by_kind(node, "identifier");
    }
    // ObjectScript UDL: class name is a `class_name` child (no "name" field).
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_OBJECTSCRIPT_UDL) {
        name_node = lsm_find_child_by_kind(node, "class_name");
    }
    // Swift and newer tree-sitter-kotlin: class/object name is a type_identifier
    // child (no "name" field).
    if (ts_node_is_null(name_node) &&
        (ctx->language == LSM_LANG_SWIFT || ctx->language == LSM_LANG_KOTLIN)) {
        name_node = lsm_find_child_by_kind(node, "type_identifier");
    }
    // Protobuf: service_name / message_name / enum_name children
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_PROTOBUF) {
        name_node = lsm_find_child_by_kind(node, "service_name");
        if (ts_node_is_null(name_node)) {
            name_node = lsm_find_child_by_kind(node, "message_name");
        }
        if (ts_node_is_null(name_node)) {
            name_node = lsm_find_child_by_kind(node, "enum_name");
        }
    }
    ...
    if (ts_node_is_null(name_node) &&
        (ctx->language == LSM_LANG_THRIFT || ctx->language == LSM_LANG_SMITHY ||
         ctx->language == LSM_LANG_PONY || ctx->language == LSM_LANG_PKL)) {
        name_node = lsm_find_child_by_kind(node, "identifier");
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_FSHARP) {
        TSNode tn = find_first_descendant_by_kind(node, "type_name", LSM_DESCENDANT_MAX_DEPTH);
        if (!ts_node_is_null(tn)) {
            TSNode id = ts_node_child_by_field_name(tn, "type_name", 9);
            if (ts_node_is_null(id)) {
                id = lsm_find_child_by_kind(tn, "identifier");
            }
            if (!ts_node_is_null(id)) {
                name_node = id;
            }
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_DLANG) {
        name_node = lsm_find_child_by_kind(node, "identifier");
    }
    // PowerShell: class_statement / enum_statement have no `name` field; the
    // name is the FIRST `simple_name` child (`class Dog : Animal { ... }`).
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_POWERSHELL) {
        name_node = lsm_find_child_by_kind(node, "simple_name");
    }
    ...
    // (`TFoo = class ... end`).
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_PASCAL) {
        TSNode parent = ts_node_parent(node);
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_JULIA) {
        TSNode th = lsm_find_child_by_kind(node, "type_head");
        if (!ts_node_is_null(th)) {
            TSNode inner = ts_node_named_child_count(th) > 0 ? ts_node_named_child(th, 0) : th;
            if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "binary_expression") == 0 &&
                ...
                name_node = lsm_find_child_by_kind(th, "identifier");
            }
        }
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_CAPNP) {
        if (strcmp(kind, "enum") == 0) {
            name_node = lsm_find_child_by_kind(node, "enum_identifier");
        } else {
            name_node = lsm_find_child_by_kind(node, "type_identifier");
        }
    }
    // Agda (FIELD_COUNT 0): data > data_name, record > record_name (direct child).
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_AGDA) {
        if (strcmp(kind, "data") == 0) {
            name_node = lsm_find_child_by_kind(node, "data_name");
        } else if (strcmp(kind, "record") == 0) {
            name_node = lsm_find_child_by_kind(node, "record_name");
        }
    }
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_GRAPHQL) {
        name_node = lsm_find_child_by_kind(node, "name");
    }
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_PRISMA) {
        name_node = lsm_find_child_by_kind(node, "identifier");
    }
    // Puppet: class_definition / type_declaration name is a plain identifier or
    // class_identifier child (no `name` field).
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_PUPPET) {
        name_node = lsm_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(name_node)) {
            name_node = lsm_find_child_by_kind(node, "class_identifier");
        }
    }
    // Smali (no `name` field): class_definition > class_directive > class_identifier.
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_SMALI) {
        TSNode dir = lsm_find_child_by_kind(node, "class_directive");
        if (!ts_node_is_null(dir)) {
            name_node = lsm_find_child_by_kind(dir, "class_identifier");
        }
    }
    // VHDL: name lives on a declaration-keyword-named field whose value is an
    // `identifier`. Map node kind -> field name.
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_VHDL) {
    ...
    if (ts_node_is_null(name_node) &&
        (ctx->language == LSM_LANG_VERILOG || ctx->language == LSM_LANG_SYSTEMVERILOG)) {
        if (strcmp(kind, "type_declaration") == 0) {
            name_node = lsm_find_child_by_kind(node, "simple_identifier");
    ...
    if (ts_node_is_null(name_node)) {
        switch (ctx->language) {
        case LSM_LANG_SQUIRREL: // class_declaration > identifier
        case LSM_LANG_DLANG:    // class/struct/interface_declaration > identifier
        case LSM_LANG_HARE:     // type_declaration > identifier
        case LSM_LANG_ODIN:     // struct_declaration > identifier
        case LSM_LANG_BICEP:    // resource/module_declaration > identifier
            name_node = lsm_find_child_by_kind(node, "identifier");
            break;
        case LSM_LANG_POWERSHELL: // class_statement > simple_name
            name_node = lsm_find_child_by_kind(node, "simple_name");
            break;
        case LSM_LANG_SWAY: // impl_item: name is the implemented type (field `type`)
            if (strcmp(kind, "impl_item") == 0) {
                name_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
            }
            break;
        case LSM_LANG_GLEAM: // type_definition > type_name > type_identifier
            name_node =
                find_first_descendant_by_kind(node, "type_identifier", LSM_DESCENDANT_MAX_DEPTH);
            break;
        case LSM_LANG_RESCRIPT: { // type_declaration > type_binding(name=type_identifier)
            TSNode binding = lsm_find_child_by_kind(node, "type_binding");
            if (!ts_node_is_null(binding)) {
                name_node = ts_node_child_by_field_name(binding, TS_FIELD("name"));
                if (ts_node_is_null(name_node)) {
                    name_node = lsm_find_child_by_kind(binding, "type_identifier");
                }
            }
            break;
        }
        case LSM_LANG_FSHARP: { // type_definition > *_type_defn > type_name > identifier
            TSNode tn = find_first_descendant_by_kind(node, "type_name", LSM_DESCENDANT_MAX_DEPTH);
            if (!ts_node_is_null(tn)) {
                name_node = ts_node_child_by_field_name(tn, TS_FIELD("type_name"));
                if (ts_node_is_null(name_node)) {
                    name_node = lsm_find_child_by_kind(tn, "identifier");
                }
            }
            break;
        }
        case LSM_LANG_JULIA: { // struct/abstract_definition > type_head > identifier
            TSNode head = lsm_find_child_by_kind(node, "type_head");
            if (!ts_node_is_null(head)) {
                name_node =
                    find_first_descendant_by_kind(head, "identifier", LSM_DESCENDANT_MAX_DEPTH);
            }
            break;
        }
        case LSM_LANG_TCL: { // namespace > word_list > simple_word ("eval" then name)
            TSNode wl = lsm_find_child_by_kind(node, "word_list");
        ...
        case LSM_LANG_PASCAL: { // declClass is nested; name lives on the parent declType
            TSNode parent = ts_node_parent(node);
            if (!ts_node_is_null(parent) && strcmp(ts_node_type(parent), "declType") == 0) {
                name_node = ts_node_child_by_field_name(parent, TS_FIELD("name"));
            }
        ...
        case LSM_LANG_ZIG: { // `const Foo = struct {...}`: struct/enum/union_declaration
            ...
            if (!ts_node_is_null(parent) &&
                ...
                name_node = lsm_find_child_by_kind(parent, "identifier");
            }
            break;
        }
        default:
    ...
    char *name = lsm_node_text(a, name_node, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    ...
    const char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = lsm_arena_sprintf(a, "%s.%s", ctx->enclosing_class_qn, name);
    } else {
        class_qn = lsm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path, name, ctx->language);
    }
    ...
    if (ctx->language == LSM_LANG_SWAY || ctx->language == LSM_LANG_WGSL) {
    ...
    // through lsm_label_is_type_like(), so a struct still resolves as a type for
    // its methods, fields, inheritance and impls.
    if (ctx->language == LSM_LANG_RUST || ctx->language == LSM_LANG_SWIFT ||
        ctx->language == LSM_LANG_DLANG) {
        if (strcmp(kind, "struct_item") == 0 || strcmp(kind, "struct_declaration") == 0) {
            label = "Struct";
        }
    }
    ...
    if (ctx->language == LSM_LANG_SWIFT && strcmp(kind, "class_declaration") == 0) {
        TSNode dk = ts_node_child_by_field_name(node, TS_FIELD("declaration_kind"));
        if (!ts_node_is_null(dk)) {
            char *dk_text = lsm_node_text(a, dk, ctx->source);
    ...
    if (ctx->language == LSM_LANG_FSHARP && strcmp(label, "Type") == 0) {
        if (!ts_node_is_null(find_first_descendant_by_kind(node, "primary_constr_args",
                                                           LSM_DESCENDANT_MAX_DEPTH)) ||
    ...
    // its methods/fields/embedding resolve through lsm_label_is_type_like(), and
    // lsm_pipeline_implements_go() collects Struct nodes too).
    ...
    if (ctx->language == LSM_LANG_CSHARP) {
        ...
        if (!ts_node_is_null(primary_params)) {
            ...
            for (uint32_t k = 0; k < pcount; k++) {
                ...
                pdef.name = pname;
                pdef.qualified_name = lsm_arena_sprintf(a, "%s.%s", class_qn, pname);
                pdef.label = "Field";
                ...
                lsm_defs_push(&ctx->result->defs, a, pdef);
            }
        }
    }
}
...
static TSNode find_class_body(TSNode class_node, LSMLanguage lang) {
    ...
    if (lang == LSM_LANG_SMALI) {
        return class_node;
    }
    // GraphQL: object/interface fields live in a fields_definition child.
    if (lang == LSM_LANG_GRAPHQL) {
        TSNode b = lsm_find_child_by_kind(class_node, "fields_definition");
    ...
    if (lang == LSM_LANG_PRISMA) {
        TSNode b = lsm_find_child_by_kind(class_node, "statement_block");
        if (!ts_node_is_null(b)) {
            return b;
        }
    }
...
static TSNode find_class_member_body(TSNode class_node, LSMLanguage lang) {
    TSNode body = find_class_body(class_node, lang);
    if (ts_node_is_null(body) || lang != LSM_LANG_JAVA ||
        strcmp(ts_node_type(body), "enum_body") != 0) {
        return body;
    }

    TSNode declarations = lsm_find_child_by_kind(body, "enum_body_declarations");
...
static TSNode resolve_dart_method_name(TSNode child, const char *ck) {
    if (strcmp(ck, "method_signature") == 0) {
        TSNode func_sig = lsm_find_child_by_kind(child, "function_signature");
        if (!ts_node_is_null(func_sig)) {
            TSNode name_node = func_name_node(func_sig);
            if (!ts_node_is_null(name_node)) {
                return name_node;
            }
            return lsm_find_child_by_kind(func_sig, "identifier");
        }
    }
    if (strcmp(ck, "function_signature") == 0) {
        return lsm_find_child_by_kind(child, "identifier");
    }
    TSNode null_node = {0};
    return null_node;
}
...
static TSNode resolve_method_name(TSNode child, LSMLanguage lang) {
    ...
    if ((lang == LSM_LANG_C || lang == LSM_LANG_CPP || lang == LSM_LANG_CUDA ||
        ...
        return lsm_resolve_func_name(child, lang);
    }

    if (lang == LSM_LANG_GROOVY && strcmp(ck, "function_definition") == 0) {
    ...
    if (lang == LSM_LANG_DART) {
        return resolve_dart_method_name(child, ck);
    }

    if (lang == LSM_LANG_OBJC && strcmp(ck, "method_definition") == 0) {
        return lsm_find_child_by_kind(child, "identifier");
    }
    ...
    // (mirrors the free-function case in lsm_resolve_func_name).
    if (lang == LSM_LANG_PONY && (strcmp(ck, "method") == 0 || strcmp(ck, "constructor") == 0 ||
    ...
    if ((lang == LSM_LANG_SWIFT || lang == LSM_LANG_KOTLIN) &&
        ...
        return lsm_find_child_by_kind(child, "simple_identifier");
    }

    // Squirrel: function_declaration's name is a plain `identifier` child.
    if (lang == LSM_LANG_SQUIRREL && strcmp(ck, "function_declaration") == 0) {
        return lsm_find_child_by_kind(child, "identifier");
    }
    ...
    if (lang == LSM_LANG_OBJECTSCRIPT_UDL &&
        ...
        TSNode mdef = lsm_find_child_by_kind(child, "method_definition");
        if (!ts_node_is_null(mdef)) {
            TSNode mname = lsm_find_child_by_kind(mdef, "method_name");
            if (!ts_node_is_null(mname) && ts_node_named_child_count(mname) > 0) {
                return ts_node_named_child(mname, 0);
    ...
    if (lang == LSM_LANG_OBJECTSCRIPT_UDL && strcmp(ck, "query") == 0) {
        return lsm_find_child_by_kind(child, "query_name");
    }
...
static void push_method_def(LSMExtractCtx *ctx, TSNode child, TSNode class_node,
                            const char *class_qn, const LSMLangSpec *spec, TSNode name_node) {
    LSMArena *a = ctx->arena;
    ...
    const char *method_qn = lsm_arena_sprintf(a, "%s.%s", class_qn, name);
    ...
    if (!def.return_type && (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL ||
                             ctx->language == LSM_LANG_OBJECTSCRIPT_ROUTINE)) {
        TSNode mdef = lsm_find_child_by_kind(child, "method_definition");
        if (ts_node_is_null(mdef)) {
            mdef = child;
        }
        TSNode rt_node = lsm_find_child_by_kind(mdef, "return_type");
        if (!ts_node_is_null(rt_node)) {
            TSNode tname = lsm_find_child_by_kind(rt_node, "typename");
            if (!ts_node_is_null(tname)) {
                def.return_type = lsm_node_text(a, tname, ctx->source);
            }
        }
    }
    ...
    if (def.route_path && (ctx->language == LSM_LANG_JAVA || ctx->language == LSM_LANG_KOTLIN)) {
...
static void extract_objc_impl_methods(LSMExtractCtx *ctx, TSNode impl_node, const char *class_qn,
                                      const LSMLangSpec *spec) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (lsm_kind_in_set(inner, spec->function_node_types)) {
...
static void extract_class_methods(LSMExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (ctx->language == LSM_LANG_SQUIRREL &&
            strcmp(ts_node_type(child), "member_declaration") == 0) {
            TSNode inner = lsm_find_child_by_kind(child, "function_declaration");
            if (!ts_node_is_null(inner)) {
                child = inner;
            }
        }
        ...
        if (strcmp(ts_node_type(child), "decorated_definition") == 0) {
            ...
            if (ts_node_is_null(def) || !lsm_kind_in_set(def, spec->function_node_types)) {
                continue;
            }
            method_node = def;
        }
        ...
        if (strcmp(ts_node_type(child), "public_field_definition") == 0) {
            ...
            if (ts_node_is_null(value) || !lsm_kind_in_set(value, spec->function_node_types)) {
                continue;
            }
        ...
        if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL &&
        ...
        if (!lsm_kind_in_set(method_node, spec->function_node_types)) {
            continue;
        }
        ...
        if (ctx->language == LSM_LANG_KOTLIN &&
            ...
            if (ts_node_is_null(constructor_name)) {
                constructor_name = lsm_find_child_by_kind(class_node, "type_identifier");
            }
            if (!ts_node_is_null(constructor_name)) {
...
static void extract_rust_impl(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    ...
    char *type_name = lsm_node_text(a, type_node, ctx->source);
    ...
    const char *type_qn = lsm_fqn_compute(a, ctx->project, ctx->rel_path, type_name);
    ...
    if (!ts_node_is_null(trait_node)) {
        char *trait_name = lsm_node_text(a, trait_node, ctx->source);
        ...
        if (trait_name && trait_name[0]) {
            ...
            lsm_impltrait_push(&ctx->result->impl_traits, a, it);
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!lsm_kind_in_set(child, spec->function_node_types)) {
            continue;
        }
        ...
        const char *method_qn = lsm_arena_sprintf(a, "%s.%s", type_qn, name);
...
static void extract_elixir_func_def(LSMExtractCtx *ctx, TSNode node, const char *macro) {
    LSMArena *a = ctx->arena;
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = lsm_node_text(a, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = lsm_node_text(a, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = name;
    def.qualified_name = lsm_fqn_compute(a, ctx->project, ctx->rel_path, name);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static TSNode emit_elixir_module_class(LSMExtractCtx *ctx, TSNode cur) {
    LSMArena *a = ctx->arena;
    TSNode null_node = {0};
    ...
    def.is_exported = true;
    lsm_defs_push(&ctx->result->defs, a, def);
    return lsm_find_child_by_kind(cur, "do_block");
}
...
static void extract_elixir_call(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    (void)spec;
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, LSM_SZ_64);
    ...
    while (stack.count > 0) {
        ...
        char *macro = lsm_node_text(a, callee, ctx->source);
        if (!macro) {
            continue;
        }
...
static void push_var_def_qn(LSMExtractCtx *ctx, const char *name, const char *qn_name,
                            TSNode node) {
    ...
    def.qualified_name = lsm_fqn_compute_source_lang(a, ctx->project, ctx->rel_path,
...
static void push_var_def(LSMExtractCtx *ctx, const char *name, TSNode node) {
...
static const char *extract_c_declarator_name(LSMArena *a, TSNode decl, const char *source) {
    ...
    if (strcmp(dk, "identifier") == 0) {
        return lsm_node_text(a, declarator, source);
    }
    return NULL;
}

// Helper: extract name from Java/C# field_declaration (declarator > name)
static const char *extract_java_field_name(LSMArena *a, TSNode field, const char *source) {
    ...
    if (!ts_node_is_null(name)) {
        return lsm_node_text(a, name, source);
    }
    return NULL;
}
...
static void extract_csharp_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < nc; j++) {
            ...
            if (strcmp(ts_node_type(decl), "variable_declarator") == 0) {
                ...
                if (ts_node_is_null(id)) {
                    id = lsm_find_child_by_kind(decl, "identifier");
                }
                if (!ts_node_is_null(id)) {
                    push_var_def(ctx, lsm_node_text(a, id, ctx->source), decl);
                }
            }
        }
    }
...
static void extract_enum_members(LSMExtractCtx *ctx, TSNode node, const char *class_qn) {
    ...
    for (uint32_t mi = 0; mi < mc; mi++) {
        ...
        if (ts_node_is_null(mname)) {
            mname = lsm_find_child_by_kind(member, "identifier");
        }
        if (ts_node_is_null(mname)) {
            continue;
        }
        char *member_name = lsm_node_text(a, mname, ctx->source);
        ...
        mdef.qualified_name = lsm_arena_sprintf(a, "%s.%s", class_qn, member_name);
        mdef.label = "Variable";
        ...
        lsm_defs_push(&ctx->result->defs, a, mdef);
    }
}
...
static void extract_destructured_vars(LSMExtractCtx *ctx, TSNode pattern, TSNode decl,
                                      LSMArena *a) {
    ...
    for (uint32_t pi = 0; pi < pc; pi++) {
        ...
        char *id_text = lsm_node_text(a, ident, ctx->source);
...
static bool is_require_import_call(TSNode value, const char *source, LSMArena *a) {
    ...
    char *fn_name = lsm_node_text(a, fn, source);
...
static void extract_js_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        if (strcmp(ts_node_type(child), "variable_declarator") != 0) {
        ...
        if (!ts_node_is_null(vname)) {
            ...
            if (strcmp(nk, "object_pattern") == 0 || strcmp(nk, "array_pattern") == 0) {
                ...
                push_var_def(ctx, lsm_node_text(a, vname, ctx->source), child);
            }
        }
    }
}

static void extract_vars_mainstream(LSMExtractCtx *ctx, TSNode node, LSMArena *a,
                                    const char *kind) {
    (void)kind;
    switch (ctx->language) {
        ...
        if (strcmp(lt, "identifier") == 0) {
            ...
            for (uint32_t li = 0; li < ln; li++) {
                ...
                if (strcmp(ts_node_type(part), "identifier") == 0) {
                    push_var_def(ctx, lsm_node_text(a, part, ctx->source), node);
                }
            }
        }
        break;
    ...
    case LSM_LANG_RUST: {
        TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
        if (!ts_node_is_null(name_node)) {
            push_var_def(ctx, lsm_node_text(a, name_node, ctx->source), node);
        }
        break;
    }
    default:
...
static void extract_lua_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        ...
        TSNode expr_list = lsm_find_child_by_kind(child, "expression_list");
...
static void extract_perl_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    uint32_t n = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < n; i++) {
        TSNode child = ts_node_named_child(node, i);
        const char *ck = ts_node_type(child);
        if (is_perl_var_type(ck)) {
            push_var_def(ctx, strip_perl_sigil(lsm_node_text(a, child, ctx->source)), node);
            return;
        }
        ...
        push_var_def(ctx, strip_perl_sigil(lsm_node_text(a, left, ctx->source)), node);
        return;
    }
}

// R variable extraction: skip function-definitions, then extract left/lhs.
static void extract_r_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
...
static void extract_php_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a, const char *kind) {
    ...
    for (uint32_t j = 0; j < nc; j++) {
        ...
        if (strcmp(ts_node_type(inner), "assignment_expression") == 0) {
            ...
            if (!ts_node_is_null(left)) {
                char *name = lsm_node_text(a, left, ctx->source);
                if (name && name[0] == '$') {
                    name++;
                }
...
static void extract_vars_dynamic(LSMExtractCtx *ctx, TSNode node, LSMArena *a, const char *kind) {
    switch (ctx->language) {
    case LSM_LANG_PHP:
...
static TSNode resolve_kotlin_var_name(TSNode node) {
    ...
    TSNode var_decl = lsm_find_child_by_kind(node, "variable_declaration");
    if (!ts_node_is_null(var_decl)) {
        name_node = lsm_find_child_by_kind(var_decl, "simple_identifier");
        if (!ts_node_is_null(name_node)) {
            return name_node;
        }
        return lsm_find_child_by_kind(var_decl, "identifier");
    }
    TSNode null_node = {0};
    return null_node;
}

static void extract_vars_jvm(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    switch (ctx->language) {
    case LSM_LANG_SCALA: {
        TSNode pattern = ts_node_child_by_field_name(node, TS_FIELD("pattern"));
        if (!ts_node_is_null(pattern)) {
            push_var_def(ctx, lsm_node_text(a, pattern, ctx->source), node);
        } else {
            TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (!ts_node_is_null(name_node)) {
                push_var_def(ctx, lsm_node_text(a, name_node, ctx->source), node);
...
static void extract_ini_vars(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    uint32_t nc = ts_node_child_count(node);
    for (uint32_t i = 0; i < nc; i++) {
        TSNode child = ts_node_child(node, i);
        const char *ck = ts_node_type(child);
        if (strcmp(ck, "setting_name") == 0 || strcmp(ck, "name") == 0) {
            push_var_def(ctx, trim_whitespace(lsm_node_text(a, child, ctx->source)), node);
            return;
        }
    }
    if (nc > 0) {
        ...
        if (!found_name) {
            push_var_def(
                ctx, trim_whitespace(lsm_node_text(a, ts_node_child(node, 0), ctx->source)), node);
        }
    }
}

// Find first named child matching one of the given types and push as var def.
static void push_first_matching_child(LSMExtractCtx *ctx, TSNode node, LSMArena *a,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (const char **t = match_types; *t; t++) {
            if (strcmp(ck, *t) == 0) {
                push_var_def(ctx, lsm_node_text(a, child, ctx->source), node);
                return;
            }
        }
    }
...
static void extract_json_var(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    TSNode key_node = ts_node_child_by_field_name(node, TS_FIELD("key"));
    if (ts_node_is_null(key_node)) {
        return;
    }
    char *raw = lsm_node_text(a, key_node, ctx->source);
    if (raw) {
...
static void extract_scss_var(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    ...
    if (ts_node_is_null(prop)) {
        prop = lsm_find_child_by_kind(node, "property_name");
    }
    if (ts_node_is_null(prop)) {
        prop = lsm_find_child_by_kind(node, "variable_name");
    }
    if (!ts_node_is_null(prop)) {
        push_var_def(ctx, lsm_node_text(a, prop, ctx->source), node);
    }
}

static void extract_vars_config(LSMExtractCtx *ctx, TSNode node, LSMArena *a, const char *kind) {
    switch (ctx->language) {
    case LSM_LANG_YAML: {
        TSNode key = ts_node_child_by_field_name(node, TS_FIELD("key"));
        if (!ts_node_is_null(key)) {
            push_var_def(ctx, lsm_node_text(a, key, ctx->source), node);
        }
        break;
    }
    case LSM_LANG_TOML: {
...
static void extract_vars_nix(LSMExtractCtx *ctx, TSNode node, LSMArena *a) {
    ...
    if (lsm_nix_binding_is_attrset_scope(node)) {
        return; /* a scope, not a value */
    }
    TSNode attrpath = ts_node_child_by_field_name(node, TS_FIELD("attrpath"));
    TSNode leaf = lsm_nix_attrpath_last_attr(attrpath);
    if (ts_node_is_null(leaf) || lsm_nix_attr_is_interpolated(leaf)) {
        return;
    }
    char *name = lsm_node_text(a, leaf, ctx->source);
    if (!name || !name[0]) {
        return;
    }
    lsm_nix_strip_attr_quotes(name);
    const char *scope = lsm_nix_attrpath_scope(a, attrpath, ctx->source);
    const char *qn_name = scope ? lsm_arena_sprintf(a, "%s.%s", scope, name) : name;
    push_var_def_qn(ctx, name, qn_name, node);
}

/* Mint every direct binding of one Nix binding container. */
static void extract_nix_binding_set(LSMExtractCtx *ctx, TSNode set, const LSMLangSpec *spec) {
...
static void extract_nix_module_vars(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    ...
    if (strcmp(ts_node_type(cur), "let_expression") == 0) {
        extract_nix_binding_set(ctx, lsm_find_child_by_kind(cur, "binding_set"), spec);
        cur = ts_node_child_by_field_name(cur, TS_FIELD("body"));
        if (ts_node_is_null(cur)) {
            return;
        }
...
static void extract_var_names(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    ...
    case LSM_LANG_DOCKERFILE:
        if (strcmp(kind, "env_instruction") == 0) {
            ...
            for (uint32_t i = 0; i < ec; i++) {
                ...
                if (!ts_node_is_null(nm)) {
                    push_var_def(ctx, lsm_node_text(a, nm, ctx->source), pair);
                }
            }
        } else if (strcmp(kind, "arg_instruction") == 0) {
            TSNode nm = ts_node_child_by_field_name(node, TS_FIELD("name"));
            if (ts_node_is_null(nm)) {
                nm = lsm_find_child_by_kind(node, "unquoted_string");
            }
            if (!ts_node_is_null(nm)) {
                push_var_def(ctx, lsm_node_text(a, nm, ctx->source), node);
            }
        }
        return;
    ...
    case LSM_LANG_PROPERTIES:
        if (strcmp(kind, "property") == 0) {
            TSNode key = lsm_find_child_by_kind(node, "key");
            if (!ts_node_is_null(key)) {
                push_var_def(ctx, lsm_node_text(a, key, ctx->source), node);
            }
        }
    ...
    case LSM_LANG_GOMOD:
        if (strcmp(kind, "require_directive") == 0 || strcmp(kind, "replace_directive") == 0) {
            ...
            for (uint32_t i = 0; i < rc; i++) {
                ...
                TSNode mp = lsm_find_child_by_kind(req_spec, "module_path");
                if (!ts_node_is_null(mp)) {
                    push_var_def(ctx, lsm_node_text(a, mp, ctx->source), req_spec);
                }
            }
        }
        return;
...
static void walk_variables_iter(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, LSM_SZ_256);
...
static void extract_yaml_toplevel_keys(LSMExtractCtx *ctx, TSNode root) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!ts_node_is_null(key)) {
            push_var_def(ctx, lsm_node_text(a, key, ctx->source), pair);
        }
    }
}
...
static void extract_variables(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    ...
    if (ctx->language == LSM_LANG_YAML && is_helm_values_file(ctx->rel_path)) {
        extract_yaml_toplevel_keys(ctx, root);
        return;
    }

    // Config languages with nested structure: use recursive walk
    if (ctx->language == LSM_LANG_YAML || ctx->language == LSM_LANG_TOML ||
        ctx->language == LSM_LANG_INI || ctx->language == LSM_LANG_JSON) {
    ...
    if (!lsm_is_module_level_p(root, ctx->language)) {
        return;
    }
        ...
        do {
            ...
            if (strcmp(ck, "expression_statement") == 0 || strcmp(ck, "export_statement") == 0 ||
                ...
                for (uint32_t j = 0; j < nc; j++) {
                    ...
                    if (lsm_kind_in_set(inner, spec->variable_node_types)) {
...
static bool extract_schema_field(LSMExtractCtx *ctx, TSNode child, const char *class_qn) {
    LSMArena *a = ctx->arena;
    TSNode name_node = {0};
    TSNode type_node = {0};

    if (ctx->language == LSM_LANG_GRAPHQL) {
        name_node = ts_node_child_by_field_name(child, TS_FIELD("name"));
        if (ts_node_is_null(name_node)) {
            name_node = lsm_find_child_by_kind(child, "name");
        }
        type_node = ts_node_child_by_field_name(child, TS_FIELD("type"));
    } else if (ctx->language == LSM_LANG_PRISMA) {
        name_node = lsm_find_child_by_kind(child, "identifier");
        type_node = lsm_find_child_by_kind(child, "column_type");
    } else if (ctx->language == LSM_LANG_SMALI) {
        name_node = lsm_find_child_by_kind(child, "field_identifier");
        type_node = lsm_find_child_by_kind(child, "field_type");
    } else {
        return false;
    }
    ...
    def.parent_class = class_qn;
    if (!ts_node_is_null(type_node)) {
        def.return_type = lsm_node_text(a, type_node, ctx->source);
    }
    def.start_line = ts_node_start_point(child).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(child).row + TS_LINE_OFFSET;
    def.is_exported = lsm_is_exported(name, ctx->language);
...
static void extract_class_fields(LSMExtractCtx *ctx, TSNode class_node, const char *class_qn,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!lsm_kind_in_set(child, spec->field_node_types)) {
            continue;
        }
        ...
        if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL) {
            if (strcmp(ts_node_type(child), "property") == 0 ||
                ...
                TSNode pname = lsm_find_child_by_kind(child, "property_name");
                if (ts_node_is_null(pname)) {
                    pname = lsm_find_child_by_kind(child, "parameter_name");
                }
                if (!ts_node_is_null(pname) && ts_node_named_child_count(pname) > 0) {
                    TSNode ident = ts_node_named_child(pname, 0);
                    char *pn = lsm_node_text(a, ident, ctx->source);
                    if (pn && pn[0]) {
                        LSMDefinition pdef;
                        memset(&pdef, 0, sizeof(pdef));
                        pdef.name = pn;
                        pdef.qualified_name = lsm_arena_sprintf(a, "%s.%s", class_qn, pn);
                        pdef.label = "Variable";
                        pdef.file_path = ctx->rel_path;
                        pdef.parent_class = class_qn;
            ...
            if (name_child_kind) {
                TSNode nname = lsm_find_child_by_kind(child, name_child_kind);
                if (!ts_node_is_null(nname)) {
                    char *mn = lsm_node_text(a, nname, ctx->source);
                    if (mn && mn[0]) {
                        LSMDefinition mdef;
                        memset(&mdef, 0, sizeof(mdef));
                        mdef.name = mn;
                        mdef.qualified_name = lsm_arena_sprintf(a, "%s.%s", class_qn, mn);
                        ...
                        if (strcmp(member_label, "Storage") == 0) {
                            TSNode sbody = lsm_find_child_by_kind(child, "storage_body");
                            if (!ts_node_is_null(sbody)) {
                                char *xml = lsm_node_text(a, sbody, ctx->source);
                                if (xml) {
                                    ...
                                    if (!first) {
                                        mdef.docstring = lsm_arena_strdup(a, props);
                                    }
                                }
                            }
                        }
                        ...
                        if (strcmp(member_label, "Trigger") == 0) {
                            TSNode tbody = lsm_find_child_by_kind(child, "core_trigger");
                            if (ts_node_is_null(tbody)) {
                                tbody = lsm_find_child_by_kind(child, "external_trigger");
                            }
                            if (!ts_node_is_null(tbody)) {
                                mdef.body_tokens = extract_body_ident_tokens(ctx, tbody);
                                char *raw = lsm_node_text(a, tbody, ctx->source);
                                if (raw && raw[0]) {
                                    char esc[LSM_SZ_512];
                                    int ei = 0;
        ...
        char *type_text = lsm_node_text(a, type_node, ctx->source);
        if (!type_text || !type_text[0]) {
            continue;
        }
        ...
        const char *field_qn = lsm_arena_sprintf(a, "%s.%s", class_qn, name);
...
// Generous safety ceiling (frames), env-overridable via LSM_WALK_DEFS_MAX.
...
static int wd_stack_max(void) {
    const char *e = getenv("LSM_WALK_DEFS_MAX");
    if (e) {
        int v = atoi(e);
        if (v > 0) {
            return v;
    ...
    if (s->top >= s->cap) {
        ...
        if (ncap > wd_stack_max()) {
            if (!s->warned) {
                ...
                lsm_log_warn("extract.walk_defs_capped", "limit", lim, "path",
                             s->path ? s->path : "");
                s->warned = true;
            }
...
static void push_nested_class_nodes(TSNode body, const LSMLangSpec *spec, wd_stack_t *s,
                                    const char *enclosing_qn, LSMArena *arena) {
    ...
    while (nc_stack.count > 0) {
        ...
        for (int i = (int)nc - SKIP_CHAR; i >= 0; i--) {
            ...
            if (lsm_kind_in_set(child, spec->class_node_types)) {
...
static bool is_template_class_node(TSNode node, LSMLanguage lang) {
    if ((lang != LSM_LANG_CPP && lang != LSM_LANG_CUDA) ||
        strcmp(ts_node_type(node), "template_declaration") != 0) {
        return false;
    }
...
static bool is_namespace_scope_kind(LSMLanguage lang, const char *kind, TSNode node) {
    ...
    if (lsm_is_namespace_scope_kind(lang, kind)) {
        return true;
    }
    ...
    if (lang == LSM_LANG_NIX && strcmp(kind, "binding") == 0) {
        return lsm_nix_binding_is_attrset_scope(node);
    }
    return false;
}

static const char *compute_class_qn(LSMExtractCtx *ctx, TSNode node, const char *saved_enclosing) {
    ...
    if (ctx->language == LSM_LANG_NIX) {
        return lsm_nix_binding_scope_qn(ctx, node, saved_enclosing);
    }
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_OBJC) {
        name_node = lsm_find_child_by_kind(node, "identifier");
    }
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_SWIFT) {
        name_node = lsm_find_child_by_kind(node, "type_identifier");
    }
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_OBJECTSCRIPT_UDL) {
        name_node = lsm_find_child_by_kind(node, "class_name");
    ...
    if (!ts_node_is_null(name_node)) {
        char *cname = lsm_node_text(ctx->arena, name_node, ctx->source);
        if (cname && cname[0]) {
            if (saved_enclosing) {
                return lsm_arena_sprintf(ctx->arena, "%s.%s", saved_enclosing, cname);
            }
            /* Top-level: language-aware module so Java/Go don't double the
             * filename stem (matches extract_class_def above). */
            return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, cname,
                                               ctx->language);
        }
    }
...
static void extract_typescript_namespace_def(LSMExtractCtx *ctx, TSNode node,
                                             const char *saved_enclosing) {
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
    }
    char *name = lsm_node_text(ctx->arena, name_node, ctx->source);
    ...
    def.lines = (int)(def.end_line - def.start_line + TS_LINE_OFFSET);
    def.is_exported = true;
    lsm_defs_push(&ctx->result->defs, ctx->arena, def);
}

// Push nested class children from a class body container onto the walk stack.
static void push_class_body_children(TSNode node, const LSMLangSpec *spec, wd_stack_t *s,
                                     const char *new_enclosing, LSMArena *arena) {
    ...
    TSNode body = {0};
    if (spec->language != LSM_LANG_SMALI) {
...
static void extract_cfml_function_tag(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    char *name = NULL;
    uint32_t cc = ts_node_named_child_count(node);
    for (uint32_t i = 0; i < cc && !name; i++) {
        ...
        TSNode an = lsm_find_child_by_kind(ch, "cf_attribute_name");
        if (ts_node_is_null(an)) {
            continue;
        }
        char *aname = lsm_node_text(a, an, ctx->source);
        if (!ascii_ci_equals(aname, "name")) {
            continue;
        }
        TSNode val = lsm_find_child_by_kind(ch, "quoted_cf_attribute_value");
        if (ts_node_is_null(val)) {
            val = lsm_find_child_by_kind(ch, "cf_attribute_value");
        }
        if (ts_node_is_null(val)) {
            continue;
        }
        TSNode inner = lsm_find_child_by_kind(val, "attribute_value");
        name = lsm_node_text(a, ts_node_is_null(inner) ? val : inner, ctx->source);
    }
    if (!name || !name[0]) {
        return;
    }
...
static void extract_gotemplate_define(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    TSNode s = lsm_find_child_by_kind(node, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return;
    }
    char *raw = lsm_node_text(a, s, ctx->source);
    if (!raw) {
        return;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = lsm_arena_strndup(a, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return;
    }
    ...
    memset(&def, 0, sizeof(def));
    def.name = raw;
    def.qualified_name = lsm_fqn_compute(a, ctx->project, ctx->rel_path, raw);
    def.label = "Function";
    def.file_path = ctx->rel_path;
    def.start_line = ts_node_start_point(node).row + TS_LINE_OFFSET;
    def.end_line = ts_node_end_point(node).row + TS_LINE_OFFSET;
...
static void extract_janet_def(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    ...
    char *head_text = lsm_node_text(a, head, ctx->source);
...
static bool is_c_preprocessor_lang(LSMLanguage lang) {
    return lang == LSM_LANG_C || lang == LSM_LANG_CPP || lang == LSM_LANG_CUDA ||
           lang == LSM_LANG_GLSL || lang == LSM_LANG_OBJC || lang == LSM_LANG_ISPC;
}
...
static void extract_c_macro_def(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
...
static void extract_lisp_def(LSMExtractCtx *ctx, TSNode node) {
    LSMArena *a = ctx->arena;
    if (ts_node_named_child_count(node) < 2) {
        return;
    }
    char *head = lsm_node_text(a, ts_node_named_child(node, 0), ctx->source);
 ...
 */
static void recover_kotlin_error_classes(LSMExtractCtx *ctx, TSNode err_node) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        if (ctx->enclosing_class_qn) {
            ...
            class_qn = lsm_fqn_compute(a, ctx->project, ctx->rel_path, name);
        }
        ...
        def.is_exported = lsm_is_exported(name, ctx->language);
        if (bcount > 0) {
            const char **result = (const char **)lsm_arena_alloc(a, (size_t)(bcount + NULL_TERM) *
                                                                        sizeof(const char *));
            if (result) {
...
static void walk_defs(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec, int depth_unused) {
    ...
    while (s.top > 0) {
        ...
        if (ctx->language == LSM_LANG_KOTLIN && strcmp(kind, "ERROR") == 0) {
            recover_kotlin_error_classes(ctx, node);
        }

        if (ctx->language == LSM_LANG_ELIXIR && strcmp(kind, "call") == 0) {
        ...
        if (ctx->language == LSM_LANG_GOTEMPLATE && strcmp(kind, "define_action") == 0) {
        ...
        if ((ctx->language == LSM_LANG_CLOJURE || ctx->language == LSM_LANG_RACKET ||
             ctx->language == LSM_LANG_SCHEME) &&
        ...
        if (ctx->language == LSM_LANG_JANET && strcmp(kind, "par_tup_lit") == 0) {
        ...
        if (ctx->language == LSM_LANG_WIT &&
            (strcmp(kind, "export_item") == 0 || strcmp(kind, "import_item") == 0) &&
            ts_node_is_null(
                find_first_descendant_by_kind(node, "func_type", LSM_DESCENDANT_MAX_DEPTH))) {
            wd_push_children_reverse(&s, node, frame.enclosing_class_qn);
            continue;
        }

        if (lsm_kind_in_set(node, spec->function_node_types)) {
            if (!is_template_class_node(node, ctx->language)) {
                ...
                bool descend_into_func =
                    (ctx->language == LSM_LANG_WOLFRAM || ctx->language == LSM_LANG_TYPESCRIPT ||
                     ctx->language == LSM_LANG_JAVASCRIPT || ctx->language == LSM_LANG_TSX ||
                     ctx->language == LSM_LANG_ADA || ctx->language == LSM_LANG_NIX);
        ...
        if (ctx->language == LSM_LANG_RUST && strcmp(kind, "impl_item") == 0) {
        ...
        if (is_namespace_scope_kind(ctx->language, kind, node)) {
            ...
            if (ctx->language == LSM_LANG_TYPESCRIPT || ctx->language == LSM_LANG_TSX) {
        ...
        if (lsm_kind_in_set(node, spec->class_node_types)) {
...
void lsm_extract_definitions(LSMExtractCtx *ctx) {
    const LSMLangSpec *spec = lsm_lang_spec(ctx->language);
    if (!spec) {
        return;
    }
