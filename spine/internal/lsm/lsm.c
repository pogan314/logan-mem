#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_init/alloc/strdup/destroy
#include "helpers.h"
...
#include "foundation/compat_fs.h"  // lsm_fopen — crash-supervisor per-file marker write
#include "foundation/hash_table.h" // LSMHashTable — crash-supervisor quarantine set
...
#if defined(LSM_BIND_TS_ALLOCATOR) && LSM_BIND_TS_ALLOCATOR
...
/* Use compat.h's lsm_clock_gettime which accepts CLOCK_MONOTONIC (value
 * varies by platform: 1 on Linux/Windows, 6 on macOS). We pass the
 * platform value via the compat.h fallback. */
#if defined(CLOCK_MONOTONIC)
#define LSM_CLOCK_MONO CLOCK_MONOTONIC
#elif defined(__APPLE__)
#define LSM_CLOCK_MONO 6
#else
#define LSM_CLOCK_MONO 1
#endif

static uint64_t now_ns(void) {
    struct timespec ts;
    lsm_clock_gettime(LSM_CLOCK_MONO, &ts);
    return ((uint64_t)ts.tv_sec * NSEC_PER_SEC) + (uint64_t)ts.tv_nsec;
}

// lsm_get_profile returns accumulated parse/extract times and file count.
...
// lsm_reset_profile zeros the profiling counters.
...
#define GROW_ARRAY(arr, arena)                                                                   \
    do {                                                                                         \
        if ((arr)->count >= (arr)->cap) {                                                        \
            int new_cap = (arr)->cap == 0 ? LSM_SZ_32 : (arr)->cap * PAIR_LEN;                   \
            void *new_items = lsm_arena_alloc((arena), (size_t)new_cap * sizeof(*(arr)->items)); \
            if (!new_items)                                                                      \
                return;                                                                          \
...
static const char *lsm_string_read(void *payload, uint32_t byte, TSPoint point,
                                   uint32_t *bytes_read) {
    (void)point;
    LSMStringInput *self = (LSMStringInput *)payload;
    if (byte >= self->length) {
        *bytes_read = 0;
        return "";
    }
...
static bool lsm_timeout_cb(TSParseState *state) {
...
static LSM_TLS TSParser *tl_parser = NULL;
static LSM_TLS LSMLanguage tl_parser_lang = LSM_LANG_COUNT; // invalid sentinel

// Get or create a thread-local parser configured for the given language.
static TSParser *get_thread_parser(const TSLanguage *ts_lang, LSMLanguage lang) {
    if (!tl_parser) {
        tl_parser = ts_parser_new();
        if (!tl_parser) {
            return NULL;
        }
        tl_parser_lang = LSM_LANG_COUNT;
    }
 ...
 *
 * Guarded to the production build (LSM_BIND_TS_ALLOCATOR=1, which CFLAGS_PROD
 ...
 * (#581). */
static void *lsm_sqlite_malloc(int n) {
    void *block = mi_malloc((size_t)n);
    return block;
}
static void lsm_sqlite_free(void *p) {
    mi_free(p);
}
static void *lsm_sqlite_realloc(void *p, int n) {
    if (p) {}
    void *grown = mi_realloc(p, (size_t)n);
    return grown;
}
static int lsm_sqlite_size(void *p) {
    return (int)mi_usable_size(p);
}
static int lsm_sqlite_roundup(int n) {
...
static void *lsm_ts_malloc(size_t n) {
    void *block = mi_malloc(n);
    return block;
}
static void *lsm_ts_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    return block;
}
static void *lsm_ts_realloc(void *p, size_t n) {
    if (p) {}
    void *grown = mi_realloc(p, n);
    return grown;
}
static void lsm_ts_free(void *p) {
    mi_free(p);
}

static int lsm_sqlite_meminit(void *appdata) {
    (void)appdata;
    return SQLITE_OK;
}
static void lsm_sqlite_memshutdown(void *appdata) {
    (void)appdata;
}
#endif /* LSM_BIND_TS_ALLOCATOR */
...
void lsm_alloc_init(void) {
    ...
    /* tree-sitter runtime (was previously bound in lsm_init; consolidated here). */
    ts_set_allocator(lsm_ts_malloc, lsm_ts_calloc, lsm_ts_realloc, lsm_ts_free);
     ...
     * silently and the binding is ignored. lsm_alloc_init() runs as the very
     * first statement of main(), before lsm_mcp_server_new → lsm_store_open*. */
    static sqlite3_mem_methods lsm_sqlite_mem = {
        lsm_sqlite_malloc,      /* xMalloc */
        lsm_sqlite_free,        /* xFree */
        lsm_sqlite_realloc,     /* xRealloc */
        lsm_sqlite_size,        /* xSize */
        lsm_sqlite_roundup,     /* xRoundup */
        lsm_sqlite_meminit,     /* xInit */
        lsm_sqlite_memshutdown, /* xShutdown */
        NULL,                   /* pAppData */
    };
    int sqlite_rc = sqlite3_config(SQLITE_CONFIG_MALLOC, &lsm_sqlite_mem);
...
static int lsm_initialized = 0;

int lsm_init(void) {
    if (lsm_initialized) {
        return 0;
    }
    enum { LSM_INIT_DONE = 1 };
    lsm_initialized = LSM_INIT_DONE;
    /* Defense-in-depth allocator binds (idempotent). main() calls lsm_alloc_init
     * first; this covers non-main entry points (pipeline passes call lsm_init).
...
void lsm_reset_thread_parser(void) {
    ...
    // Must be called BEFORE lsm_slab_reset_thread() to avoid corrupting
...
// (Go: `s` in `func (s *Store) save()`, from LSMDefinition.receiver). Matching
...
static void lsm_index_mark(const char *rel_path, char event) {
    const char *mf = getenv("LSM_INDEX_MARKER_FILE");
    if (!mf || !mf[0] || !rel_path || !rel_path[0]) {
        return;
    }
    FILE *f = lsm_fopen(mf, "ab");
    if (f) {
        (void)fprintf(f, "%c %s\n", event, rel_path);
        (void)fclose(f);
    }
 ...
 * (lsm_extract_file): whether a pass re-extracts from disk on a cache miss
 ...
 * lsm_index_is_quarantined() so the crasher surfaces in the response skipped[].
...
static LSMHashTable *g_quarantine_set = NULL;
enum { LSM_QSET_UNINIT = 0, LSM_QSET_INITING = 1, LSM_QSET_INITED = 2 };
static atomic_int g_quarantine_state = LSM_QSET_UNINIT;

static void lsm_quarantine_load(void) {
    const char *qf = getenv("LSM_INDEX_QUARANTINE_FILE");
    if (!qf || !qf[0]) {
        return; /* normal path: empty set */
    }
    FILE *f = lsm_fopen(qf, "rb");
    if (!f) {
        return;
    }
    LSMHashTable *set = lsm_ht_create(16);
    if (!set) {
        (void)fclose(f);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
         ...
         * The value stores the phase so lsm_index_quarantine_phase() can report
         * "crash" vs "hang"; membership (lsm_index_is_quarantined) is value != NULL. */
        char *pval = lsm_strdup(phase);
        if (!pval) {
            continue;
        }
        if (lsm_ht_has(set, line)) {
             ...
             * value it displaces. */
            free(lsm_ht_set(set, line, (void *)pval));
        } else {
            char *key = lsm_strdup(line);
            if (key) {
                lsm_ht_set(set, key, (void *)pval);
            } else {
...
bool lsm_index_is_quarantined(const char *rel_path) {
    ...
    if (state != LSM_QSET_INITED) {
        /* First caller wins the CAS and loads; racers spin until INITED.
         * Same once-init pattern as lsm_ui_log_init (http_server.c). */
        state = LSM_QSET_UNINIT;
        if (atomic_compare_exchange_strong(&g_quarantine_state, &state, LSM_QSET_INITING)) {
            lsm_quarantine_load();
            atomic_store(&g_quarantine_state, LSM_QSET_INITED);
        } else {
            while (atomic_load(&g_quarantine_state) != LSM_QSET_INITED) {
                lsm_usleep(1000); /* 1ms */
            }
        }
    }
    return g_quarantine_set && lsm_ht_has(g_quarantine_set, rel_path);
}

const char *lsm_index_quarantine_phase(const char *rel_path) {
    /* lsm_index_is_quarantined drives the lazy once-load and returns true only
    ...
    if (!lsm_index_is_quarantined(rel_path)) {
        return NULL;
    }
    return (const char *)lsm_ht_get(g_quarantine_set, rel_path);
}
...
static void lsm_test_fault_inject(const char *rel_path) {
    if (!rel_path || !rel_path[0]) {
        return;
    }
    const char *crash_on = getenv("LSM_TEST_CRASH_ON");
    if (crash_on && crash_on[0] && strstr(rel_path, crash_on)) {
        abort(); /* SIGABRT → WIFSIGNALED → classified as a crash */
    }
    const char *hang_on = getenv("LSM_TEST_HANG_ON");
 ...
 *
 * lsm_source_nesting_exceeds scans the raw bytes for the maximum bracket-nesting
...
#define LSM_PERL_MAX_PARSE_NESTING 128

static bool lsm_source_nesting_exceeds(const char *source, int source_len, int cap) {
    int depth = 0;
 ...
 * Bounded by LSM_MAX_ERROR_REGIONS so pathological input can't blow up the
 * output. The ranges mark where constructs were dropped; they are a detection
 * aid, never a completeness proof. */
#define LSM_MAX_ERROR_REGIONS 64
typedef struct {
    uint32_t starts[LSM_MAX_ERROR_REGIONS];
    uint32_t ends[LSM_MAX_ERROR_REGIONS];
    int count;
} lsm_error_regions_t;

static void lsm_error_regions_push(lsm_error_regions_t *acc, TSNode n) {
    if (acc->count >= LSM_MAX_ERROR_REGIONS) {
        return;
    }
 ...
 * untouched. */
static bool lsm_is_eof_terminator_miss(TSNode n, int source_len) {
...
static void lsm_collect_error_regions(TSNode n, lsm_error_regions_t *acc, int source_len) {
    if (acc->count >= LSM_MAX_ERROR_REGIONS) {
        return;
    }
    uint32_t k = ts_node_child_count(n);
    for (uint32_t i = 0; i < k && acc->count < LSM_MAX_ERROR_REGIONS; i++) {
        TSNode c = ts_node_child(n, i);
        if (ts_node_is_missing(c) || strcmp(ts_node_type(c), "ERROR") == 0) {
            if (lsm_is_eof_terminator_miss(c, source_len)) {
                continue; /* absent final newline only — nothing was dropped */
            }
            lsm_error_regions_push(acc, c); /* top-most region; do not descend */
        } else if (ts_node_has_error(c)) {
            lsm_collect_error_regions(c, acc, source_len);
        }
    }
}
...
static bool lsm_region_is_recovered(uint32_t rs, uint32_t re, const LSMDefArray *defs) {
    ...
    for (int i = 0; i < defs->count && n < MAX_COVER_DEFS; i++) {
        const LSMDefinition *d = &defs->items[i];
...
static bool lsm_line_contains(const char *src, int src_len, uint32_t line, const char *name) {
...
static bool lsm_identifier_char(char c) {
...
static bool lsm_span_contains_callable_def(const char *src, int src_len, uint32_t start_line,
    ...
    for (int pos = span_start; pos + (int)name_len <= span_end; pos++) {
        if (strncmp(src + pos, name, name_len) != 0 ||
            (pos > 0 && lsm_identifier_char(src[pos - 1])) ||
            (pos + (int)name_len < src_len && lsm_identifier_char(src[pos + name_len]))) {
            continue;
        }
        int before = pos;
...
static bool lsm_remap_preprocessed_def(LSMDefinition *def, const LSMPreprocessedSource *pp) {
...
static void lsm_subtract_recovered_regions(lsm_error_regions_t *regs, const LSMDefArray *defs) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        if (!lsm_region_is_recovered(regs->starts[i], regs->ends[i], defs)) {
...
static bool lsm_span_is_macro_invocation(const char *src, int src_len, uint32_t start_line,
                                         uint32_t end_line, const LSMDefArray *defs) {
    ...
    for (int di = 0; di < defs->count; di++) {
        const LSMDefinition *d = &defs->items[di];
        ...
        for (int pos = span_start; pos + nlen <= span_end; pos++) {
            if (strncmp(src + pos, d->name, (size_t)nlen) != 0 ||
                (pos > 0 && lsm_identifier_char(src[pos - 1])) ||
                (pos + nlen < src_len && lsm_identifier_char(src[pos + nlen]))) {
                continue;
            }
            int open = pos + nlen;
            while (open < span_end && isspace((unsigned char)src[open])) {
...
static bool lsm_region_inside_callable(uint32_t rs, uint32_t re, const LSMDefArray *defs) {
...
static void lsm_subtract_macro_invocation_regions(lsm_error_regions_t *regs,
                                                  const LSMDefArray *defs, const char *src,
                                                  int src_len) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        bool benign =
            lsm_span_is_macro_invocation(src, src_len, regs->starts[i], regs->ends[i], defs) &&
            lsm_region_inside_callable(regs->starts[i], regs->ends[i], defs);
        if (!benign) {
            regs->starts[kept] = regs->starts[i];
            regs->ends[kept] = regs->ends[i];
            kept++;
...
static const char *lsm_error_ranges_str(LSMArena *a, const lsm_error_regions_t *regs) {
    if (regs->count <= 0) {
        return NULL;
    }
    enum { RANGE_MAX = 24 }; /* "4294967295-4294967295," */
    char *buf = (char *)lsm_arena_alloc(a, (size_t)regs->count * RANGE_MAX);
    if (!buf) {
        return NULL;
    }
    size_t off = 0;
...
LSMFileResult *lsm_extract_file(const char *source, int source_len, LSMLanguage language,
    ...
    LSMFileResult *r =
        lsm_extract_file_ex(source, source_len, language, project, rel_path, timeout_micros,
                            extra_defines, include_paths, NULL, NULL);
    return r;
}

LSMFileResult *lsm_extract_file_ex(const char *source, int source_len, LSMLanguage language,
                                   const char *project, const char *rel_path,
                                   int64_t timeout_micros, const char **extra_defines,
                                   const char **include_paths, const LSMMacroTable *macro_table,
                                   const LSMReturnTypeTable *return_type_table) {
    // Allocate result on heap (arena inside for all string data)
    enum { SINGLE = 1 };
    LSMFileResult *result = (LSMFileResult *)calloc(SINGLE, sizeof(LSMFileResult));
    if (!result) {
        return NULL;
    }

    lsm_arena_init(&result->arena);
    LSMArena *a = &result->arena;
    ...
    lsm_index_mark_start(rel_path);
#ifdef LSM_ENABLE_TEST_SEAMS
    lsm_test_fault_inject(rel_path);
#endif
    ...
    if (!spec) {
        ...
        result->error_msg = lsm_arena_strdup(a, "unsupported language");
        lsm_index_mark_done(rel_path);
        return result;
    }

    // Get tree-sitter language
    const TSLanguage *ts_lang = lsm_ts_language(language);
    if (!ts_lang) {
        result->has_error = true;
        result->error_msg = lsm_arena_strdup(a, "no tree-sitter grammar");
        lsm_index_mark_done(rel_path);
        return result;
    }
    ...
    // lsm_source_nesting_exceeds). Scoped to Perl: its ambiguous call grammar is
    // the only one that drives that recursion to the nesting depth.
    if (language == LSM_LANG_PERL &&
        lsm_source_nesting_exceeds(source, source_len, LSM_PERL_MAX_PARSE_NESTING)) {
        result->has_error = true;
        result->error_msg = lsm_arena_strdup(a, "perl source nesting too deep; skipped");
        return result;
    }
    ...
    if (!parser) {
        result->has_error = true;
        result->error_msg = lsm_arena_strdup(a, "parser alloc failed");
        lsm_index_mark_done(rel_path);
        return result;
    }
    ...
    LSMStringInput str_input = {source, (uint32_t)source_len};
    TSInput ts_input = {
        &str_input,
        lsm_string_read,
        TSInputEncodingUTF8,
    ...
    if (timeout_micros > 0) {
        ...
        opts.progress_callback = lsm_timeout_cb;
    }
    ...
    if (!tree) {
        result->has_error = true;
        result->error_msg =
            lsm_arena_strdup(a, timeout_micros > 0 ? "parse timeout" : "parse failed");
        lsm_index_mark_done(rel_path);
        return result;
    }
    ...
    result->module_qn = lsm_fqn_module_source_lang(a, project, rel_path, language);
    result->is_test_file = lsm_is_test_file(rel_path, language);
    ...
    lsm_extract_definitions(&ctx);
    lsm_extract_imports(&ctx);
    lsm_extract_unified(&ctx);

    // Channel detection (Socket.IO / EventEmitter) — JS/TS only.
    lsm_extract_channels(&ctx);

    // K8s / Kustomize semantic pass (additional structured extraction for YAML-based infra files).
    if (ctx.language == LSM_LANG_KUSTOMIZE || ctx.language == LSM_LANG_K8S) {
        lsm_extract_k8s(&ctx);
    }
    ...
    {
        if (language == LSM_LANG_GO) {
            lsm_run_go_lsp(a, result, source, source_len, root);
        }
        if (language == LSM_LANG_C || language == LSM_LANG_CPP || language == LSM_LANG_CUDA) {
            lsm_run_c_lsp(a, result, source, source_len, root, language != LSM_LANG_C,
                          LSM_SOURCE_ORIGIN_RAW);
        }
        if (language == LSM_LANG_PHP) {
            lsm_run_php_lsp(a, result, source, source_len, root);
        }
        if (language == LSM_LANG_PERL) {
            lsm_run_perl_lsp(a, result, source, source_len, root);
        }
        if (language == LSM_LANG_PYTHON) {
            lsm_run_py_lsp(a, result, source, source_len, root);
        }
        if (language == LSM_LANG_JAVASCRIPT || language == LSM_LANG_TYPESCRIPT ||
            language == LSM_LANG_TSX) {
            bool js_mode = (language == LSM_LANG_JAVASCRIPT);
            // jsx_mode: TSX always; .jsx in the JS bucket also enables it.
            bool jsx_mode = (language == LSM_LANG_TSX);
            if (language == LSM_LANG_JAVASCRIPT && rel_path) {
            ...
            bool dts_mode = false;
            if (language == LSM_LANG_TYPESCRIPT && rel_path) {
                size_t rl = strlen(rel_path);
                if (rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0)
                    dts_mode = true;
            }
            lsm_run_ts_lsp(a, result, source, source_len, root, js_mode, jsx_mode, dts_mode);
        }
        if (language == LSM_LANG_CSHARP) {
            lsm_run_cs_lsp(a, result, source, source_len, root);
        }
    }
    if (language == LSM_LANG_JAVA) {
        lsm_run_java_lsp(a, result, source, source_len, root);
    }
    if (language == LSM_LANG_KOTLIN) {
        lsm_run_kotlin_lsp(a, result, source, source_len, root);
    }
    if (language == LSM_LANG_RUST) {
        lsm_run_rust_lsp(a, result, source, source_len, root);
    }
    ...
    if (language == LSM_LANG_C || language == LSM_LANG_CPP || language == LSM_LANG_CUDA) {
        ...
        LSMPreprocessedSource *preprocessed = lsm_preprocess_with_map(
            source, source_len, rel_path, extra_defines, include_paths, language != LSM_LANG_C);
        if (preprocessed && preprocessed->source) {
            ...
            if (pp_parser) {
                ...
                LSMStringInput pp_input = {expanded, (uint32_t)expanded_len};
                TSInput pp_ts_input = {
                    &pp_input,
                    lsm_string_read,
                    TSInputEncodingUTF8,
                ...
                if (pp_tree) {
                    ...
                    for (int i = calls_before; i < result->calls.count; i++) {
                        result->calls.items[i].source_origin = LSM_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    for (int i = usages_before; i < result->usages.count; i++) {
                        result->usages.items[i].source_origin = LSM_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    ...
                    lsm_run_c_lsp(a, result, expanded, expanded_len, pp_root,
                                  language != LSM_LANG_C, LSM_SOURCE_ORIGIN_PREPROCESSED);
                    ...
                    if (ts_node_has_error(root)) {
                        lsm_error_regions_t raw_regs = {{0}, {0}, 0};
                        lsm_collect_error_regions(root, &raw_regs, source_len);
                        if (raw_regs.count > 0) {
                            int defs_before = result->defs.count;
                            lsm_extract_definitions(&pp_ctx);
                            int w = defs_before;
                            for (int i = defs_before; i < result->defs.count; i++) {
                                LSMDefinition *d = &result->defs.items[i];
                                bool adopt = false;
                                if (lsm_remap_preprocessed_def(d, preprocessed)) {
                                ...
                                if (adopt && (!d->name ||
                                              !lsm_line_contains(source, source_len, d->start_line,
                                                                 d->name) ||
                                              !lsm_span_contains_callable_def(
            ...
            lsm_preprocessed_source_free(preprocessed);
    ...
    // LSMFileResult share the same file). Range matching is used instead of
    ...
    for (int di = 0; di < def_count; di++) {
        LSMDefinition *d = &result->defs.items[di];
        int pc = 0;
        if (d->param_names) {
    ...
    for (int ci = 0; ci < orig_calls_count; ci++) {
        const LSMCall *c = &result->calls.items[ci];
        ...
        int best_span = -1;
        for (int di = 0; di < def_count; di++) {
            const LSMDefinition *d = &result->defs.items[di];
            if (!d->name || !d->label ||
        ...
        LSMDefinition *d = &result->defs.items[best];
    ...
    if (ts_node_has_error(root)) {
        lsm_error_regions_t regs = {{0}, {0}, 0};
        if (strcmp(ts_node_type(root), "ERROR") == 0) {
            lsm_error_regions_push(&regs, root); /* whole file unparseable */
        } else {
            lsm_collect_error_regions(root, &regs, source_len);
        }
        lsm_subtract_recovered_regions(&regs, &result->defs);
        /* #1071: don't flag a benign function-like-macro call (defined in-file)
         * that tree-sitter can't parse without the preprocessor. */
        lsm_subtract_macro_invocation_regions(&regs, &result->defs, source, source_len);
        if (regs.count > 0) {
            result->parse_incomplete = true;
            result->error_region_count = regs.count;
            result->error_ranges = lsm_error_ranges_str(a, &regs);
        }
    }
    ...
    // Retain tree for cross-file LSP reuse (caller frees via lsm_free_tree)
...
void lsm_free_result(LSMFileResult *result) {
    ...
    for (int i = 0; i < result->owned_result_count; i++) {
        lsm_free_result(result->owned_results[i]);
    }
