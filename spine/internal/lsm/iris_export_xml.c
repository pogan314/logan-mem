...
static void ub_init(UdlBuf *b, LSMArena *arena) {
    b->buf = (char *)lsm_arena_alloc(arena, BUF_CAP);
    b->pos = 0;
    b->cap = BUF_CAP;
    if (b->buf)
        b->buf[0] = '\0';
...
static char *transcode_class(LSMArena *arena, const char *cs, const char *ce) {
    UdlBuf b;
    ub_init(&b, arena);
    if (!b.buf)
        return NULL;
...
char **lsm_iris_export_to_udl(LSMArena *arena, const char *xml, int xml_len, int *class_count) {
    if (class_count)
        *class_count = 0;
    ...
    if (!count)
        return NULL;
    char **arr = (char **)lsm_arena_alloc(arena, (size_t)(count + 1) * sizeof(char *));
    if (!arr)
        return NULL;
