...
LSMPreprocessedSource *lsm_preprocess_with_map(const char *source, int source_len,
    ...
    try {
        ...
        LSMPreprocessedSource *pp = (LSMPreprocessedSource *)calloc(1, sizeof(*pp));
        if (!pp) {
            return NULL;
        }
        ...
        if (!pp->source || !pp->original_line_by_expanded_line || !pp->belongs_to_main_file) {
            lsm_preprocessed_source_free(pp);
            return NULL;
        }
...
char *lsm_preprocess(const char *source, int source_len, const char *filename,
    ...
    LSMPreprocessedSource *pp = lsm_preprocess_with_map(source, source_len, filename, extra_defines,
