...
// LSM patch: cap the recursive ambiguity-merge in stack_node_add_link.
...
// LSM's LSM_LSP_*_MAX_WALK_DEPTH walk caps; 512*~260 B ~= 130 KB, safe with
// wide headroom on a 1 MB stack, while far exceeding any realistic source
// nesting.
#define LSM_TS_STACK_MERGE_MAX_DEPTH 512
...
// LSM patch: depth-tracked inner worker. `depth` counts the recursive
// ambiguity-merge nesting; past LSM_TS_STACK_MERGE_MAX_DEPTH we stop merging
  ...
  for (int i = 0; i < self->link_count; i++) {
    ...
    if (stack__subtree_is_equivalent(existing_link->subtree, link.subtree)) {
      ...
      if (
        ...
        // LSM patch: bound the recursive merge to keep the C stack finite.
        if (depth < LSM_TS_STACK_MERGE_MAX_DEPTH) {
...
// LSM patch: public entry point — unchanged signature, seeds recursion depth.
