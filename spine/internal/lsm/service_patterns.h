 ...
 */
#ifndef LSM_SERVICE_PATTERNS_H
#define LSM_SERVICE_PATTERNS_H

#include <stdbool.h>

/* Edge type returned by pattern match. */
typedef enum {
    LSM_SVC_NONE = 0,      /* Not a service pattern — use normal CALLS */
    LSM_SVC_HTTP = 1,      /* Synchronous HTTP client call */
    LSM_SVC_ASYNC = 2,     /* Async dispatch (message broker, task queue) */
    LSM_SVC_CONFIG = 3,    /* Config/env accessor */
    LSM_SVC_ROUTE_REG = 4, /* Route registration (router.GET, app.get, Route::post) */
    LSM_SVC_GRPC = 5,      /* gRPC client call (protobuf stub invocation) */
    LSM_SVC_GRAPHQL = 6,   /* GraphQL client query/mutation */
    LSM_SVC_TRPC = 7,      /* tRPC client procedure call */
} lsm_svc_kind_t;
 ...
 * Returns the pattern kind, or LSM_SVC_NONE if no match.
...
/* Per-worker TLS cache for lsm_service_pattern_match results. The
