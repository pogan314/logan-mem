...
#include "arena.h" // lsm_arena_sprintf
#include "lsm.h"   // LSMExtractCtx
#include "helpers.h"
#include "lang_specs.h"      // LSMLangSpec, lsm_lang_spec, LSM_LANG_*
...
static bool ensure_scope_capacity(WalkState *state) {
    ...
    LSMWalkScope *grown =
        (LSMWalkScope *)lsm_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        return false;
    }
...
static const LSMLexicalScope *lexical_scope_by_id(const WalkState *state, uint32_t id) {
    return state && id > 0 && id <= (uint32_t)state->lexical_scope_count
               ? &state->lexical_scopes[id - 1U]
               : NULL;
}

static LSMLexicalScope *mutable_lexical_scope_by_id(WalkState *state, uint32_t id) {
    return state && id > 0 && id <= (uint32_t)state->lexical_scope_count
               ? &state->lexical_scopes[id - 1U]
               : NULL;
}
...
static bool ensure_lexical_scope_capacity(WalkState *state) {
    ...
    LSMLexicalScope *grown =
        (LSMLexicalScope *)lsm_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static uint32_t add_lexical_scope(WalkState *state, TSNode node, LSMLexicalScopeKind kind) {
    ...
    if (kind == LSM_LEXICAL_SCOPE_FUNCTION && state->language == LSM_LANG_PYTHON) {
        lookup_parent_id = python_function_lookup_parent(state, parent_id);
    }
    LSMLexicalScope *scope = &state->lexical_scopes[state->lexical_scope_count];
...
static bool push_scope(WalkState *state, uint8_t kind, uint32_t depth, const char *qn) {
    ...
    LSMWalkScope *f = &state->scopes[state->scope_top];
    f->kind = kind;
    f->depth = depth;
    f->qn = qn;
    f->lexical_scope_id = 0;
    f->invocation_kind = LSM_INVOCATION_NONE;
     ...
     * LSMWalkScope comment for the quadratic recompute this replaces. */
...
static bool push_lexical_scope(WalkState *state, uint8_t walk_kind, uint32_t depth, const char *qn,
                               TSNode node, LSMLexicalScopeKind lexical_kind) {
...
static bool push_existing_lexical_scope(WalkState *state, uint8_t walk_kind, uint32_t depth,
    ...
    LSMLexicalScope *scope = mutable_lexical_scope_by_id(state, lexical_scope_id);
...
static bool push_function_scope(WalkState *state, uint32_t depth, const char *function_qn,
    ...
    return existing_id ? push_existing_lexical_scope(state, SCOPE_FUNC, depth, function_qn,
                       ...
                       : push_lexical_scope(state, SCOPE_FUNC, depth, function_qn, node,
                                            LSM_LEXICAL_SCOPE_FUNCTION);
}

static void push_call_scope(WalkState *state, uint32_t depth,
                            const LSMInvocationDescriptor *invocation) {
    if (!invocation ||
        (invocation->kind != LSM_INVOCATION_CALLABLE_REFERENCE && !invocation->raw_call_emitted) ||
...
static void pop_expired_scopes(WalkState *state, uint32_t cur_depth) {
    while (state->scope_top > 0 && state->scopes[state->scope_top - SKIP_ONE].depth >= cur_depth) {
        const LSMWalkScope *f = &state->scopes[--state->scope_top];
...
static const char *compute_wolfram_func_qn(LSMExtractCtx *ctx, TSNode node) {
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(ts_node_type(lhs), "apply") == 0 && ts_node_named_child_count(lhs) > 0) {
            ...
            if (strcmp(ts_node_type(head), "user_symbol") == 0) {
                char *name = lsm_node_text(ctx->arena, head, ctx->source);
                if (name && name[0]) {
                    return lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
                }
            }
        }
    }
...
static const char *compute_lisp_func_qn(LSMExtractCtx *ctx, TSNode node) {
    if (ts_node_named_child_count(node) < 2) {
        return NULL;
    }
    char *head = lsm_node_text(ctx->arena, ts_node_named_child(node, 0), ctx->source);
...
static const char *compute_elixir_func_qn(LSMExtractCtx *ctx, TSNode node) {
    if (ts_node_child_count(node) == 0) {
        return NULL;
    }
    char *macro = lsm_node_text(ctx->arena, ts_node_child(node, 0), ctx->source);
    ...
    if (strcmp(fk, "call") == 0 && ts_node_child_count(first_arg) > 0) {
        name = lsm_node_text(ctx->arena, ts_node_child(first_arg, 0), ctx->source);
    } else if (strcmp(fk, "identifier") == 0) {
        name = lsm_node_text(ctx->arena, first_arg, ctx->source);
    }
    if (!name || !name[0]) {
        return NULL;
    }
...
static const char *compute_cfml_func_qn(LSMExtractCtx *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < cc && !name; i++) {
        ...
        char *aname = lsm_node_text(ctx->arena, an, ctx->source);
        ...
        name = lsm_node_text(ctx->arena, ts_node_is_null(inner) ? val : inner, ctx->source);
    }
    if (!name || !name[0]) {
        return NULL;
    }
...
static const char *compute_gotemplate_func_qn(LSMExtractCtx *ctx, TSNode node) {
    ...
    char *raw = lsm_node_text(ctx->arena, s, ctx->source);
    if (!raw) {
        return NULL;
    }
    size_t len = strlen(raw);
    if (len >= 2 && (raw[0] == '"' || raw[0] == '`')) {
        raw = lsm_arena_strndup(ctx->arena, raw + 1, len - 2); // strip surrounding quotes
    }
    if (!raw || !raw[0]) {
        return NULL;
    }
    return lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, raw);
}
...
static TSNode find_class_method_call(TSNode root, const char *end) {
    ...
    for (const char **c = containers; *c; c++) {
        TSNode inner = lsm_find_child_by_kind(root, *c);
        if (!ts_node_is_null(inner)) {
            TSNode hit = lsm_find_child_by_kind(inner, "class_method_call");
            if (!ts_node_is_null(hit)) {
                return hit;
            }
            TSNode inner2 = lsm_find_child_by_kind(inner, "expr_atom");
            if (!ts_node_is_null(inner2)) {
                hit = lsm_find_child_by_kind(inner2, "class_method_call");
                if (!ts_node_is_null(hit)) {
                    return hit;
                }
            }
    ...
    return lsm_find_child_by_kind(root, "class_method_call");
}
...
static void handle_objectscript_type_map(LSMExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    if (strcmp(nk, "command_set") == 0) {
        for (uint32_t i = 0; i < ts_node_named_child_count(node); i++) {
            ...
            TSNode method_name_node = lsm_find_child_by_kind(cm_call, "method_name");
            ...
            char *method_text = lsm_node_text(ctx->arena, mn_ident, ctx->source);
            if (!method_text) {
                continue;
            }

            TSNode class_ref = lsm_find_child_by_kind(cm_call, "class_ref");
            ...
            char *cls = lsm_node_text(ctx->arena, cname, ctx->source);
            if (!cls || !cls[0]) {
                continue;
            }
            ...
            TSNode var_node = lhs;
            TSNode inner = lsm_find_child_by_kind(lhs, "glvn");
            if (!ts_node_is_null(inner)) {
                var_node = inner;
            }
            inner = lsm_find_child_by_kind(var_node, "lvn");
            if (!ts_node_is_null(inner)) {
                var_node = inner;
            }
            char *var = lsm_node_text(ctx->arena, var_node, ctx->source);
            if (!var || !var[0]) {
                continue;
            }
    ...
    if (strcmp(nk, "property") == 0 || strcmp(nk, "relationship") == 0) {
        TSNode prop_name_node = lsm_find_child_by_kind(node, "property_name");
        if (ts_node_is_null(prop_name_node)) {
            prop_name_node = lsm_find_child_by_kind(node, "relationship_name");
        }
        TSNode ret_type = lsm_find_child_by_kind(node, "return_type");
        if (!ts_node_is_null(prop_name_node) && !ts_node_is_null(ret_type)) {
            TSNode tname = lsm_find_child_by_kind(ret_type, "typename");
            if (!ts_node_is_null(tname)) {
                char *pname = lsm_node_text(ctx->arena, prop_name_node, ctx->source);
                char *ptype = lsm_node_text(ctx->arena, tname, ctx->source);
                if (pname && pname[0] && ptype && ptype[0]) {
                    char *dot_name = lsm_arena_sprintf(ctx->arena, "..%s", pname);
...
static const char *objectscript_get_class_name(LSMExtractCtx *ctx, TSNode node) {
    for (uint32_t i = 0; i < ts_node_named_child_count(node); i++) {
        TSNode child = ts_node_named_child(node, i);
        if (strcmp(ts_node_type(child), "class_name") == 0) {
            char *name = lsm_node_text(ctx->arena, child, ctx->source);
            if (name && name[0]) {
                return lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
            }
        }
...
static const char *objectscript_get_method_qn(LSMExtractCtx *ctx, TSNode node,
                                              const char *enclosing_class_qn) {
    const char *nk = ts_node_type(node);
    if (strcmp(nk, "query") == 0) {
        TSNode query_name = lsm_find_child_by_kind(node, "query_name");
        if (ts_node_is_null(query_name)) {
            return NULL;
        }
        char *name = lsm_node_text(ctx->arena, query_name, ctx->source);
        if (!name || !name[0]) {
            return NULL;
        }
        return enclosing_class_qn ? lsm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, name)
                                  : lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
    }
            ...
            for (uint32_t j = 0; j < ts_node_named_child_count(child); j++) {
                ...
                if (strcmp(ts_node_type(mchild), "method_name") == 0) {
                    if (ts_node_named_child_count(mchild) > 0) {
                        ...
                        char *name = lsm_node_text(ctx->arena, ident, ctx->source);
                        if (name && name[0]) {
                            if (enclosing_class_qn) {
                                return lsm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn,
                                                         name);
                            }
                            return lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
                        }
...
// Compute function QN for scope tracking (mirrors lsm_enclosing_func_qn logic).
static const char *compute_func_qn(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                   WalkState *state) {
    (void)spec;
    if (ctx->language == LSM_LANG_WOLFRAM) {
        return compute_wolfram_func_qn(ctx, node);
    }
    if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL) {
        return objectscript_get_method_qn(ctx, node, state->enclosing_class_qn);
    }
    if (ctx->language == LSM_LANG_OBJECTSCRIPT_ROUTINE) {
        const char *kind = ts_node_type(node);
        TSNode tag = {0};
        if (strcmp(kind, "procedure") == 0) {
            tag = lsm_find_child_by_kind(node, "tag");
        ...
        if (!ts_node_is_null(tag)) {
            char *name = lsm_node_text(ctx->arena, tag, ctx->source);
            if (name && name[0]) {
                return lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, name);
            }
        }
     ...
     * resolver below. */
    if (ctx->language == LSM_LANG_CFML && strcmp(ts_node_type(node), "cf_function_tag") == 0) {
    ...
    if (ctx->language == LSM_LANG_GOTEMPLATE) {
    ...
    if (ctx->language == LSM_LANG_CLOJURE || ctx->language == LSM_LANG_SCHEME ||
        ctx->language == LSM_LANG_RACKET) {
     ...
     * child. Resolve the call-scope QN HERE (not via the shared lsm_resolve_func_name)
    ...
    if (ctx->language == LSM_LANG_OBJC && strcmp(ts_node_type(node), "method_definition") == 0) {
        TSNode id = lsm_find_child_by_kind(node, "identifier");
        if (!ts_node_is_null(id)) {
            char *mname = lsm_node_text(ctx->arena, id, ctx->source);
            if (mname && mname[0]) {
                if (state->enclosing_class_qn) {
                    return lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, mname);
                }
                return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, mname,
                                                   ctx->language);
            }
        }
    }
    ...
    if (ctx->language == LSM_LANG_DART && (strcmp(ts_node_type(node), "function_signature") == 0 ||
                                           strcmp(ts_node_type(node), "method_signature") == 0)) {
        TSNode sig = node;
        if (strcmp(ts_node_type(node), "method_signature") == 0) {
            TSNode fs = lsm_find_child_by_kind(node, "function_signature");
            if (!ts_node_is_null(fs)) {
                sig = fs;
            }
        }
        TSNode id = lsm_find_child_by_kind(sig, "identifier");
        if (!ts_node_is_null(id)) {
            char *nm = lsm_node_text(ctx->arena, id, ctx->source);
            if (nm && nm[0]) {
                if (state->enclosing_class_qn) {
                    return lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, nm);
                }
                return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, nm,
                                                   ctx->language);
            }
        }
    }
    ...
    if (ctx->language == LSM_LANG_AGDA && strcmp(ts_node_type(node), "function") == 0) {
        TSNode lhs = lsm_find_child_by_kind(node, "lhs");
        if (!ts_node_is_null(lhs)) {
            TSNode nm = lsm_find_child_by_kind(lhs, "function_name");
            ...
            if (!ts_node_is_null(nm)) {
                char *name = lsm_node_text(ctx->arena, nm, ctx->source);
                if (name && name[0]) {
                    return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path,
                                                       name, ctx->language);
                }
            }
        }
    ...
    char *name = lsm_func_name_node_text(ctx->arena, name_node, ctx->source, ctx->language);
    if (!name || !name[0]) {
        return NULL;
    }
    ...
    if ((ctx->language == LSM_LANG_CPP || ctx->language == LSM_LANG_CUDA) &&
        ...
        char *scope_name = lsm_cpp_out_of_line_parent_class(ctx->arena, node, ctx->source);
        if (scope_name && scope_name[0]) {
            const char *class_qn =
                lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, scope_name);
            return lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, name);
        }
    }
    ...
    if (ctx->language == LSM_LANG_NIX) {
        qn_name = lsm_nix_qn_name(ctx->arena, node, ctx->source, name);
    ...
    if (state->enclosing_class_qn) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, qn_name);
    }
    ...
    return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, qn_name,
                                       ctx->language);
}

// Compute class QN for scope tracking.
static const char *compute_class_qn(LSMExtractCtx *ctx, TSNode node, const WalkState *state) {
    if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL) {
     ...
     * nested binding. */
    if (ctx->language == LSM_LANG_NIX) {
        return lsm_nix_binding_scope_qn(ctx, node, state ? state->enclosing_class_qn : NULL);
    }
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    /* Newer tree-sitter-kotlin: class/object name is a type_identifier child. */
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_KOTLIN) {
    ...
    if (ts_node_is_null(name_node) && ctx->language == LSM_LANG_RUST &&
    ...
    if (state && state->enclosing_class_qn) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, name);
    }

    /* Java/Go: directory-based module (see compute_func_qn). */
    return lsm_fqn_compute_source_lang(ctx->arena, ctx->project, ctx->rel_path, name,
                                       ctx->language);
}
...
static void handle_string_constants(LSMExtractCtx *ctx, TSNode node, const WalkState *state) {
    ...
    if (strcmp(value_kind, "template_string") == 0) {
        flat_value = lsm_template_string_text(ctx->arena, value_node, ctx->source);
        if (!flat_value) {
            return;
        }
    ...
    char *name = lsm_node_text(ctx->arena, name_node, ctx->source);
    char *value =
        flat_value ? (char *)flat_value : lsm_node_text(ctx->arena, value_node, ctx->source);
    ...
    if (!flat_value && vlen >= LSM_QUOTE_PAIR && (value[0] == '"' || value[0] == '\'')) {
        value = lsm_arena_strndup(ctx->arena, value + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
        if (!value) {
            return;
        }
    }
    ...
    /* Add to constant map */
    LSMStringConstantMap *map = &ctx->string_constants;
    if (map->count < LSM_MAX_STRING_CONSTANTS) {
...
static void handle_string_refs(LSMExtractCtx *ctx, TSNode node, const WalkState *state) {
    const char *kind = ts_node_type(node);
    ...
    if (strcmp(kind, "template_string") == 0) {
        const char *flat = lsm_template_string_text(ctx->arena, node, ctx->source);
        if (!flat) {
            return;
        }
        int kind_val = lsm_classify_string(flat, (int)strlen(flat));
        if (kind_val < 0) {
            return;
        }
        LSMStringRef ref = {
            ...
            .kind = (LSMStringRefKind)kind_val,
        };
        lsm_stringref_push(&ctx->result->string_refs, ctx->arena, ref);
        return;
    }
    ...
    /* Classify */
    int kind_val = lsm_classify_string(content, len);
    if (kind_val < 0) {
        return;
    }

    /* Build null-terminated content string in arena */
    char *val = lsm_arena_strndup(ctx->arena, content, (size_t)len);
    if (!val) {
        return;
    }
...
static void emit_yaml_leaf_value(LSMExtractCtx *ctx, TSNode val, const char *path) {
    char *val_text = lsm_node_text(ctx->arena, val, ctx->source);
    ...
    if (vlen >= LSM_QUOTE_PAIR && (val_text[0] == '"' || val_text[0] == '\'')) {
    ...
    int kind_val = lsm_classify_string(content, vlen);
    if (kind_val < 0) {
        return;
    }

    char *stored = lsm_arena_strndup(ctx->arena, content, (size_t)vlen);
    if (!stored) {
        return;
    }
...
#define YAML_WALK_STACK_CAP LSM_SZ_256
...
static void walk_yaml_mapping(LSMExtractCtx *ctx, TSNode root, const char *root_prefix) {
    ...
    while (top > 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            char *key_text = lsm_node_text(ctx->arena, key, ctx->source);
            if (!key_text || !key_text[0]) {
                continue;
            }
            const char *path =
                prefix ? lsm_arena_sprintf(ctx->arena, "%s.%s", prefix, key_text) : key_text;
...
static char *strip_yaml_quotes(LSMArena *a, char *v) {
    if (!v || !v[0]) {
        return v;
    }
    int vlen = (int)strlen(v);
    if (vlen >= LSM_QUOTE_PAIR && (v[0] == '"' || v[0] == '\'')) {
        return lsm_arena_strndup(a, v + SKIP_ONE, (size_t)(vlen - PAIR_LEN));
    }
    return v;
}

// Scan a nested YAML block_mapping for target keys (push_endpoint, uri, etc.).
static void scan_nested_mapping_targets(LSMExtractCtx *ctx, TSNode val, const char **targets,
                                        int *n_targets) {
    uint32_t vnc = ts_node_named_child_count(val);
    for (uint32_t vi = 0; vi < vnc; vi++) {
        ...
        for (uint32_t mi = 0; mi < mnc; mi++) {
            ...
            char *mktext = lsm_node_text(ctx->arena, mk, ctx->source);
            if (mktext && is_target_key(mktext) && *n_targets < MAX_INFRA_BINDINGS) {
                char *mvtext =
                    strip_yaml_quotes(ctx->arena, lsm_node_text(ctx->arena, mv, ctx->source));
...
static void emit_infra_bindings(LSMExtractCtx *ctx, const char **sources, const char **source_keys,
    ...
    for (int si = 0; si < n_sources; si++) {
        for (int ti = 0; ti < n_targets; ti++) {
            ...
            lsm_infrabinding_push(&ctx->result->infra_bindings, ctx->arena, ib);
        }
    }
}

static void scan_mapping_for_bindings(LSMExtractCtx *ctx, TSNode mapping) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *k = lsm_node_text(ctx->arena, key, ctx->source);
        if (!k) {
            continue;
        }
        ...
        if (strcmp(vtype, "block_node") != 0 && strcmp(vtype, "block_mapping") != 0) {
            char *v = strip_yaml_quotes(ctx->arena, lsm_node_text(ctx->arena, val, ctx->source));
...
#define INFRA_SCAN_STACK_CAP LSM_SZ_512
static void scan_yaml_for_infra_bindings(LSMExtractCtx *ctx, TSNode root) {
...
static char *extract_hcl_string_val(LSMArena *a, TSNode val_node, const char *source) {
    ...
    for (int depth = 0; depth < HCL_MAX_UNWRAP; depth++) {
        ...
        if (strcmp(vk, "quoted_template") == 0 || strcmp(vk, "template_literal") == 0 ||
            ...
            char *val = lsm_node_text(a, val_node, source);
...
static TSNode hcl_block_body(TSNode block) {
    TSNode body = lsm_find_child_by_kind(block, "body");
    return ts_node_is_null(body) ? block : body;
}

// Scan a nested HCL block for target keys (push_endpoint, uri, etc.).
static void scan_hcl_nested_block_targets(LSMExtractCtx *ctx, TSNode block, const char **targets,
                                          int *n_targets) {
    ...
    for (uint32_t bi = 0; bi < bnc; bi++) {
        ...
        char *bk = lsm_node_text(ctx->arena, bkey, ctx->source);
...
static const char *hcl_scheduler_source(LSMExtractCtx *ctx, TSNode block, const char **broker_out) {
    ...
    for (uint32_t i = 0; i < cc; i++) {
        ...
        char *label = lsm_node_text(ctx->arena, lit, ctx->source);
        if (!label || !label[0]) {
            continue;
        }
        if (!first_label) {
...
static void scan_hcl_block_for_bindings(LSMExtractCtx *ctx, TSNode block) {
    const char *sources[MAX_INFRA_BINDINGS] = {NULL};
    const char *source_keys[MAX_INFRA_BINDINGS] = {NULL};
    int n_sources = 0;
    const char *targets[MAX_INFRA_BINDINGS] = {NULL};
...
static void handle_yaml_nested(LSMExtractCtx *ctx, TSNode node) {
    if (ctx->language != LSM_LANG_YAML) {
        return;
    }
    const char *kind = ts_node_type(node);
...
static void scan_infra_bindings(LSMExtractCtx *ctx, TSNode node) {
    if (ctx->language == LSM_LANG_YAML || ctx->language == LSM_LANG_JSON) {
        const char *nk = ts_node_type(node);
    ...
    } else if (ctx->language == LSM_LANG_HCL) {
...
static bool is_actual_import_boundary(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    bool direct_import = spec->import_node_types && lsm_kind_in_set(node, spec->import_node_types);
    bool from_import = spec->import_from_types && lsm_kind_in_set(node, spec->import_from_types);
    ...
    char *name = ts_node_is_null(head) ? NULL : lsm_node_text(ctx->arena, head, ctx->source);
    ...
    case LSM_LANG_TSX:
        if (strcmp(kind, "export_statement") == 0) {
            ...
            return !ts_node_is_null(lsm_find_child_by_kind(node, "export_clause")) &&
...
static TSNode objectscript_routine_statement_tag(TSNode statement) {
    TSNode tag_statement = lsm_find_child_by_kind(statement, "tag_statement");
    return ts_node_is_null(tag_statement) ? (TSNode){0}
                                          : lsm_find_child_by_kind(tag_statement, "tag");
}
...
static TSNode objectscript_routine_preceding_tag(TSNode node) {
    ...
    if (ts_node_is_null(parent) || strcmp(ts_node_type(parent), "source_file") != 0 ||
        !ts_node_is_null(lsm_find_child_by_kind(node, "tag_statement")) ||
        !ts_node_is_null(lsm_find_child_by_kind(node, "procedure"))) {
        return (TSNode){0};
    }
...
static bool push_pre_node_scope(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
    ...
    TSNode label = {0};
    if (ctx->language == LSM_LANG_NASM && strcmp(ts_node_type(node), "actual_instruction") == 0) {
        label = nasm_preceding_label(node);
    } else if (ctx->language == LSM_LANG_OBJECTSCRIPT_ROUTINE) {
        if (strcmp(ts_node_type(node), "tag_statement") == 0) {
            ...
            label = lsm_find_child_by_kind(node, "tag");
        } else {
...
static bool lexical_boundary_kind(TSNode node, LSMLexicalScopeKind *out_kind) {
    const char *kind = ts_node_type(node);
    ...
    if (strcmp(kind, "mod_item") == 0) {
        *out_kind = LSM_LEXICAL_SCOPE_MODULE;
        return true;
    }
    if (unified_kind_in_set(kind, comprehension_kinds)) {
        *out_kind = LSM_LEXICAL_SCOPE_COMPREHENSION;
        return true;
    }
    if (unified_kind_in_set(kind, anonymous_function_kinds)) {
        *out_kind = LSM_LEXICAL_SCOPE_FUNCTION;
        return true;
    }
    if (unified_kind_in_set(kind, block_kinds)) {
        *out_kind = LSM_LEXICAL_SCOPE_BLOCK;
        return true;
    }
    return false;
}
...
static void push_boundary_scopes(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                 WalkState *state, uint32_t depth,
                                 const LSMInvocationDescriptor *invocation) {
    if (spec->function_node_types && lsm_kind_in_set(node, spec->function_node_types)) {
        ...
        if (ctx->language == LSM_LANG_OCAML) {
            for (int i = 0; i < state->scope_top; i++) {
                if (state->scopes[i].kind == SCOPE_FUNC) {
                    skip_nested = true;
                    break;
        ...
        if (!skip_nested) {
            ...
            if (fqn && push_function_scope(state, depth, fqn, node)) {
                ...
                bool split_signature = (ctx->language == LSM_LANG_DART &&
                                        (strcmp(node_kind, "function_signature") == 0 ||
                                         strcmp(node_kind, "method_signature") == 0)) ||
                                       (ctx->language == LSM_LANG_LLVM_IR &&
                ...
                if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL ||
                    ...
                    TSNode args_node = lsm_find_child_by_kind(mdef, "arguments");
                        ...
                        for (uint32_t ai = 0; ai < ts_node_named_child_count(args_node); ai++) {
                            ...
                            for (uint32_t pi = 0; pi < ts_node_named_child_count(arg); pi++) {
                                ...
                                if (strcmp(pk, "method_arg") == 0) {
                                    ...
                                    type_node = lsm_find_child_by_kind(pchild, "typename");
                                }
                            }
                            if (!ts_node_is_null(param_name_node) && !ts_node_is_null(type_node)) {
                                TSNode lvn = lsm_find_child_by_kind(param_name_node, "expr_atom");
                                if (ts_node_is_null(lvn)) {
                                    lvn = param_name_node;
                                }
                                char *pname = lsm_node_text(ctx->arena, lvn, ctx->source);
                                char *ptype = lsm_node_text(ctx->arena, type_node, ctx->source);
                                if (pname && pname[0] && ptype && ptype[0]) {
    ...
    } else if (lsm_is_namespace_scope_kind(ctx->language, ts_node_type(node))) {
        const char *namespace_qn = compute_class_qn(ctx, node, state);
        if (namespace_qn) {
            push_lexical_scope(state, SCOPE_NAMESPACE, depth, namespace_qn, node,
                               LSM_LEXICAL_SCOPE_MODULE);
        }
    } else if (spec->class_node_types && lsm_kind_in_set(node, spec->class_node_types)) {
        const char *cqn = compute_class_qn(ctx, node, state);
        if (cqn) {
            push_lexical_scope(state, SCOPE_CLASS, depth, cqn, node, LSM_LEXICAL_SCOPE_CLASS);
    ...
    } else if (ctx->language == LSM_LANG_NIX && strcmp(ts_node_type(node), "binding") == 0 &&
               lsm_nix_binding_is_attrset_scope(node)) {
    ...
    } else if (ctx->language == LSM_LANG_RUST && strcmp(ts_node_type(node), "impl_item") == 0) {
        TSNode type_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
        if (!ts_node_is_null(type_node)) {
            char *type_name = lsm_node_text(ctx->arena, type_node, ctx->source);
            if (type_name && type_name[0]) {
                const char *tqn =
                    lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, type_name);
                push_lexical_scope(state, SCOPE_CLASS, depth, tqn, node, LSM_LEXICAL_SCOPE_CLASS);
            }
        }
    } else if (ctx->language == LSM_LANG_DART && strcmp(ts_node_type(node), "function_body") == 0) {
    ...
    } else if (ctx->language == LSM_LANG_LLVM_IR &&
    ...
    if (ts_node_is_named(node) && lsm_is_loop_node_type(ts_node_type(node))) {
        push_scope(state, SCOPE_LOOP, depth, NULL);
    } else if (spec->branching_node_types && lsm_kind_in_set(node, spec->branching_node_types)) {
...
void lsm_extract_unified(LSMExtractCtx *ctx) {
    ...
    state.root_lexical_scope_id = add_lexical_scope(&state, ctx->root, LSM_LEXICAL_SCOPE_MODULE);
    ...
    state.invocation_kind = LSM_INVOCATION_NONE;
    ...
    for (;;) {
        ...
        if (!trivia) {
            ...
            LSMInvocationDescriptor invocation = handle_calls(ctx, node, spec, &state);
    ...
    lsm_finalize_lexical_usages(ctx, &state);
