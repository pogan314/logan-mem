...
// Forward declaration
static void walk_usages(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec);
...
static bool is_reference_node(TSNode node, LSMLanguage lang) {
    ...
    if (lang == LSM_LANG_PYTHON && strcmp(kind, "attribute") == 0 &&
    ...
    if ((lang == LSM_LANG_PUPPET || lang == LSM_LANG_VIMSCRIPT) &&
        strcmp(kind, "identifier") == 0) {
        TSNode parent = ts_node_parent(node);
        if (!ts_node_is_null(parent) &&
            ((lang == LSM_LANG_PUPPET && strcmp(ts_node_type(parent), "variable") == 0) ||
             (lang == LSM_LANG_VIMSCRIPT && strcmp(ts_node_type(parent), "argument") == 0))) {
            return false;
        }
    }
    ...
    case LSM_LANG_LLVM_IR:
        return strcmp(kind, "local_var") == 0 || strcmp(kind, "global_var") == 0;
    case LSM_LANG_PUPPET:
    case LSM_LANG_POWERSHELL:
    ...
    case LSM_LANG_CLOJURE:
    case LSM_LANG_COMMONLISP:
        return strcmp(kind, "sym_lit") == 0;
    case LSM_LANG_VIMSCRIPT:
    ...
    case LSM_LANG_EMACSLISP:
    case LSM_LANG_SCHEME:
    case LSM_LANG_FENNEL:
    case LSM_LANG_RACKET:
    case LSM_LANG_LINKERSCRIPT:
        return strcmp(kind, "symbol") == 0;
    case LSM_LANG_MAKEFILE:
        return strcmp(kind, "variable_reference") == 0;
    case LSM_LANG_CMAKE:
        /* `${watched}` contains a named `variable` leaf spanning only `watched`. */
        return strcmp(kind, "variable") == 0;
    case LSM_LANG_WOLFRAM:
    ...
    case LSM_LANG_RESCRIPT:
        return strcmp(kind, "value_identifier") == 0;
    case LSM_LANG_PURESCRIPT:
        return strcmp(kind, "variable") == 0;
    case LSM_LANG_NICKEL:
        return strcmp(kind, "ident") == 0;
    case LSM_LANG_JSONNET:
        return strcmp(kind, "id") == 0;
    case LSM_LANG_CFML:
        return strcmp(kind, "property_identifier") == 0;
    case LSM_LANG_OBJECTSCRIPT_UDL:
    case LSM_LANG_OBJECTSCRIPT_ROUTINE:
...
static bool is_forward_sibling_callee(LSMLanguage language, TSNode node) {
    if (language == LSM_LANG_DART && strcmp(ts_node_type(node), "identifier") == 0) {
...
typedef enum {
    LSM_OCCURRENCE_STANDARD = 0,
    LSM_OCCURRENCE_LISP_DEF,
    LSM_OCCURRENCE_COMMONLISP_DEFUN,
    LSM_OCCURRENCE_FENNEL_FN,
    LSM_OCCURRENCE_ELIXIR_DEF,
    LSM_OCCURRENCE_JULIA_FUNCTION,
    LSM_OCCURRENCE_WOLFRAM_SET,
    LSM_OCCURRENCE_TYPST_LET,
    LSM_OCCURRENCE_AGDA_FUNCTION,
    LSM_OCCURRENCE_TLAPLUS_OPERATOR,
    LSM_OCCURRENCE_COBOL_MOVE,
    LSM_OCCURRENCE_HCL_ATTRIBUTE,
    LSM_OCCURRENCE_ELM_VALUE,
    LSM_OCCURRENCE_RESCRIPT_LET,
    LSM_OCCURRENCE_PURESCRIPT_LHS,
    LSM_OCCURRENCE_NICKEL_LET,
    LSM_OCCURRENCE_ERLANG_CLAUSE,
    LSM_OCCURRENCE_NIX_FUNCTION,
    LSM_OCCURRENCE_MATLAB_ARGUMENTS,
    LSM_OCCURRENCE_LEAN_BINDER,
    LSM_OCCURRENCE_PASCAL_PROC,
    LSM_OCCURRENCE_TEAL_FUNCTION,
    LSM_OCCURRENCE_VHDL_INTERFACE,
    LSM_OCCURRENCE_PINE_FUNCTION,
    LSM_OCCURRENCE_LLVM_FUNCTION,
} LSMOccurrencePolicy;

typedef struct {
    const char *const *whole_binding_nodes;
    const char *const *write_nodes;
    LSMOccurrencePolicy policy;
 ...
 * the positional LSMLangSpec initializer used by every language. */
static const LSMOccurrenceSpec occurrence_specs[LSM_LANG_COUNT] = {
    [LSM_LANG_SQL] = {sql_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_CLOJURE] = {NULL, NULL, LSM_OCCURRENCE_LISP_DEF, false},
    [LSM_LANG_SCHEME] = {NULL, NULL, LSM_OCCURRENCE_LISP_DEF, false},
    [LSM_LANG_RACKET] = {NULL, NULL, LSM_OCCURRENCE_LISP_DEF, false},
    [LSM_LANG_COMMONLISP] = {NULL, NULL, LSM_OCCURRENCE_COMMONLISP_DEFUN, false},
    [LSM_LANG_FENNEL] = {NULL, NULL, LSM_OCCURRENCE_FENNEL_FN, false},
    [LSM_LANG_ELIXIR] = {NULL, NULL, LSM_OCCURRENCE_ELIXIR_DEF, false},
    [LSM_LANG_JULIA] = {NULL, NULL, LSM_OCCURRENCE_JULIA_FUNCTION, false},
    [LSM_LANG_WOLFRAM] = {NULL, NULL, LSM_OCCURRENCE_WOLFRAM_SET, false},
    [LSM_LANG_TYPST] = {NULL, NULL, LSM_OCCURRENCE_TYPST_LET, false},
    [LSM_LANG_AGDA] = {NULL, NULL, LSM_OCCURRENCE_AGDA_FUNCTION, false},
    [LSM_LANG_TLAPLUS] = {NULL, NULL, LSM_OCCURRENCE_TLAPLUS_OPERATOR, false},
    [LSM_LANG_COBOL] = {NULL, NULL, LSM_OCCURRENCE_COBOL_MOVE, false},
    [LSM_LANG_HCL] = {NULL, NULL, LSM_OCCURRENCE_HCL_ATTRIBUTE, false},
    [LSM_LANG_ELM] = {NULL, NULL, LSM_OCCURRENCE_ELM_VALUE, false},
    [LSM_LANG_RESCRIPT] = {rescript_binding_nodes, NULL, LSM_OCCURRENCE_RESCRIPT_LET, false},
    [LSM_LANG_PURESCRIPT] = {purescript_binding_nodes, NULL, LSM_OCCURRENCE_PURESCRIPT_LHS, false},
    [LSM_LANG_NICKEL] = {nickel_binding_nodes, NULL, LSM_OCCURRENCE_NICKEL_LET, false},
    [LSM_LANG_JSONNET] = {jsonnet_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_HASKELL] = {haskell_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_ERLANG] = {NULL, NULL, LSM_OCCURRENCE_ERLANG_CLAUSE, false},
    [LSM_LANG_FSHARP] = {fsharp_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_NIX] = {NULL, NULL, LSM_OCCURRENCE_NIX_FUNCTION, false},
    [LSM_LANG_MATLAB] = {NULL, NULL, LSM_OCCURRENCE_MATLAB_ARGUMENTS, false},
    [LSM_LANG_LEAN] = {NULL, NULL, LSM_OCCURRENCE_LEAN_BINDER, false},
    [LSM_LANG_PASCAL] = {NULL, NULL, LSM_OCCURRENCE_PASCAL_PROC, false},
    [LSM_LANG_VERILOG] = {systemverilog_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_AWK] = {awk_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_CRYSTAL] = {crystal_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_TEAL] = {teal_binding_nodes, NULL, LSM_OCCURRENCE_TEAL_FUNCTION, false},
    [LSM_LANG_VHDL] = {NULL, NULL, LSM_OCCURRENCE_VHDL_INTERFACE, false},
    [LSM_LANG_SYSTEMVERILOG] = {systemverilog_binding_nodes, NULL, LSM_OCCURRENCE_STANDARD, false},
    [LSM_LANG_PINE] = {NULL, NULL, LSM_OCCURRENCE_PINE_FUNCTION, false},
    [LSM_LANG_PUPPET] = {NULL, NULL, LSM_OCCURRENCE_STANDARD, true},
    [LSM_LANG_LLVM_IR] = {llvm_binding_nodes, NULL, LSM_OCCURRENCE_LLVM_FUNCTION, false},
    [LSM_LANG_MESON] = {NULL, meson_write_nodes, LSM_OCCURRENCE_STANDARD, true},
    [LSM_LANG_GN] = {NULL, gn_write_nodes, LSM_OCCURRENCE_STANDARD, true},
    [LSM_LANG_LINKERSCRIPT] = {NULL, linkerscript_write_nodes, LSM_OCCURRENCE_STANDARD, true},
    [LSM_LANG_OBJECTSCRIPT_UDL] = {objectscript_binding_nodes, objectscript_write_nodes,
                                   LSM_OCCURRENCE_STANDARD, true},
    [LSM_LANG_OBJECTSCRIPT_ROUTINE] = {objectscript_binding_nodes, objectscript_write_nodes,
                                       LSM_OCCURRENCE_STANDARD, true},
};

static bool text_equals(LSMExtractCtx *ctx, TSNode node, const char *expected) {
    char *text = lsm_node_text(ctx->arena, node, ctx->source);
...
static bool is_lisp_def_binding(LSMExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
        ...
        char *head = lsm_node_text(ctx->arena, head_node, ctx->source);
        ...
        if (ctx->language == LSM_LANG_CLOJURE && ts_node_named_child_count(form) > 2 &&
...
static bool is_fennel_fn_binding(LSMExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool is_elixir_def_binding(LSMExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool fish_argument_marker(LSMExtractCtx *ctx, TSNode node) {
    char *text = lsm_node_text(ctx->arena, node, ctx->source);
    return text && (strcmp(text, "-a") == 0 || strcmp(text, "--argument") == 0 ||
                    strcmp(text, "--argument-names") == 0);
}

static bool is_fish_function_parameter(LSMExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    for (TSNode previous = ts_node_prev_named_sibling(node); !ts_node_is_null(previous);
        ...
        char *text = lsm_node_text(ctx->arena, previous, ctx->source);
        if (!text || text[0] != '-') {
            continue;
        }
...
static bool cfml_argument_tag_name_binding(LSMExtractCtx *ctx, TSNode node) {
    ...
    TSNode attribute_name = lsm_find_child_by_kind(attribute, "cf_attribute_name");
    char *name_text = lsm_node_text(ctx->arena, attribute_name, ctx->source);
    ...
    char *tag_text = lsm_node_text(ctx->arena, tag, ctx->source);
...
static bool is_exact_language_binding(LSMExtractCtx *ctx, TSNode node, WalkState *state) {
    const char *kind = ts_node_type(node);
    switch (ctx->language) {
    case LSM_LANG_OCAML: {
...
static bool is_policy_binding(LSMExtractCtx *ctx, TSNode node,
                              const LSMOccurrenceSpec *occurrence) {
    switch (occurrence->policy) {
    case LSM_OCCURRENCE_LISP_DEF:
        return is_lisp_def_binding(ctx, node);
    case LSM_OCCURRENCE_COMMONLISP_DEFUN:
        return is_commonlisp_defun_binding(node);
    case LSM_OCCURRENCE_FENNEL_FN:
        return is_fennel_fn_binding(ctx, node);
    case LSM_OCCURRENCE_ELIXIR_DEF:
        return is_elixir_def_binding(ctx, node);
    case LSM_OCCURRENCE_JULIA_FUNCTION:
        return is_first_named_part_of(node, "function_definition");
    case LSM_OCCURRENCE_WOLFRAM_SET:
        return is_wolfram_lhs(node);
    case LSM_OCCURRENCE_TYPST_LET:
        return is_first_named_part_of(node, "let");
    case LSM_OCCURRENCE_AGDA_FUNCTION:
        for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
             parent = ts_node_parent(parent)) {
    ...
    case LSM_OCCURRENCE_LLVM_FUNCTION:
        ...
        return false;
    case LSM_OCCURRENCE_STANDARD:
    default:
        return false;
    }
}

static bool elixir_binary_operator_binds(LSMExtractCtx *ctx, TSNode node) {
    if (ctx->language != LSM_LANG_ELIXIR || strcmp(ts_node_type(node), "binary_operator") != 0) {
        return false;
    }
...
static bool is_binding_occurrence(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                  WalkState *state) {
    const LSMOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    ...
    while (!ts_node_is_null(current)) {
        ...
        bool variable_container =
            spec->variable_node_types && lsm_kind_in_set(parent, spec->variable_node_types);
        if (ctx->language == LSM_LANG_ELIXIR && strcmp(kind, "binary_operator") == 0) {
        ...
        bool declared_container =
            ...
            (spec->function_node_types && lsm_kind_in_set(parent, spec->function_node_types)) ||
            (spec->class_node_types && lsm_kind_in_set(parent, spec->class_node_types)) ||
            (spec->field_node_types && lsm_kind_in_set(parent, spec->field_node_types)) ||
            variable_container;
...
static bool is_write_occurrence(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                WalkState *state) {
    const LSMOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    TSNode current = node;
    TSTreeCursor *cursor = reset_occurrence_cursor(state, node);
    while (!ts_node_is_null(current)) {
        ...
        bool assignment =
            (spec->assignment_node_types && lsm_kind_in_set(parent, spec->assignment_node_types)) ||
            kind_in_exact_set(ts_node_type(parent), occurrence->write_nodes);
        if (ctx->language == LSM_LANG_ELIXIR &&
        ...
        if (assignment) {
            ...
            if (ctx->language == LSM_LANG_LINKERSCRIPT &&
...
static bool language_may_stamp_exact_callable_value_candidate(LSMLanguage language) {
    switch (language) {
    case LSM_LANG_JAVASCRIPT:
    case LSM_LANG_TYPESCRIPT:
    case LSM_LANG_TSX:
...
static TSNode call_reference_candidate_site(LSMExtractCtx *ctx, TSNode node, const char *name,
                                            WalkState *state) {
    ...
    bool ts_family = ctx->language == LSM_LANG_JAVASCRIPT || ctx->language == LSM_LANG_TYPESCRIPT ||
    ...
    if (ctx->language == LSM_LANG_PYTHON && strcmp(kind, "identifier") == 0 &&
    ...
    if (ctx->language == LSM_LANG_GO && strcmp(kind, "field_identifier") == 0 &&
    ...
    if (ctx->language == LSM_LANG_RUST && strcmp(kind, "scoped_identifier") == 0) {
     ...
     * as before. */
    if (ctx->language == LSM_LANG_KOTLIN && !ts_node_is_null(parent) &&
    ...
    if (ctx->language == LSM_LANG_PYTHON && !ts_node_is_null(parent) &&
        strcmp(ts_node_type(parent), "assignment") == 0) {
...
static char *reference_name(LSMExtractCtx *ctx, TSNode node) {
    char *name = lsm_node_text(ctx->arena, node, ctx->source);
    if (!name || ctx->language != LSM_LANG_MAKEFILE ||
    ...
    return parenthesized ? lsm_arena_strndup(ctx->arena, name + 2, length - 3) : name;
}

static bool lexical_identifier_case_insensitive(LSMLanguage language) {
    switch (language) {
    case LSM_LANG_POWERSHELL:
    case LSM_LANG_FORTRAN:
    case LSM_LANG_ADA:
    case LSM_LANG_PASCAL:
    case LSM_LANG_COBOL:
    case LSM_LANG_VHDL:
        return true;
    default:
...
static const char *lexical_binding_key(LSMExtractCtx *ctx, WalkState *state, const char *name) {
    if (!ctx || !name) {
        return name;
    }
    ...
    if (ctx->language == LSM_LANG_TCL && name[0] == '$') {
        name++;
    }
    ...
    char *folded = (char *)lsm_arena_alloc(ctx->arena, n + 1U);
    if (!folded) {
        if (state) {
...
static void stamp_usage_site(LSMExtractCtx *ctx, LSMUsage *usage, TSNode node, const char *name,
                             WalkState *state) {
...
static const LSMLexicalScope *usage_lexical_scope(const WalkState *state, uint32_t id) {
...
static uint32_t usage_lexical_scope_id_for_node(LSMExtractCtx *ctx, const WalkState *state,
                                                TSNode node) {
    uint32_t scope_id = active_lexical_scope_id(state);
    if (!ctx || ctx->language != LSM_LANG_PYTHON || !python_default_value_reference(node)) {
        return scope_id;
    }
    uint32_t function_id = lexical_ancestor_of_kind(state, scope_id, true, false);
    const LSMLexicalScope *function_scope = usage_lexical_scope(state, function_id);
...
static uint32_t lexical_ancestor_of_kind(const WalkState *state, uint32_t start_id,
    ...
    while (id != 0 && remaining-- > 0) {
        const LSMLexicalScope *scope = usage_lexical_scope(state, id);
        if (!scope) {
            return 0;
        }
        bool function_scope = scope->kind == LSM_LEXICAL_SCOPE_FUNCTION ||
                              scope->kind == LSM_LEXICAL_SCOPE_COMPREHENSION;
        bool block_scope = scope->kind == LSM_LEXICAL_SCOPE_BLOCK;
...
static uint32_t python_nearest_namespace(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == LSM_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == LSM_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == LSM_LEXICAL_SCOPE_CLASS || scope->kind == LSM_LEXICAL_SCOPE_MODULE) {
            return id;
        }
        id = scope->parent_id;
    }
...
static bool ensure_lexical_binding_capacity(WalkState *state) {
    ...
    LSMLexicalBinding *grown =
        (LSMLexicalBinding *)lsm_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static TSNode declared_function_name_owner(TSNode node, const LSMLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->function_node_types && lsm_kind_in_set(parent, spec->function_node_types)) {
...
static TSNode declared_class_name_owner(TSNode node, const LSMLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->class_node_types && lsm_kind_in_set(parent, spec->class_node_types)) {
            return field_contains_node(parent, "name", node) ? parent : (TSNode){0};
        }
    }
    return (TSNode){0};
...
static bool ensure_python_directive_capacity(WalkState *state) {
    ...
    LSMPythonDirective *grown =
        (LSMPythonDirective *)lsm_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static void record_python_directive(WalkState *state, uint32_t function_scope_id, const char *name,
    ...
    LSMPythonDirective *directive = &state->python_directives[state->python_directive_count++];
    directive->function_scope_id = function_scope_id;
    directive->name = name;
    directive->kind = (uint8_t)kind;
}

static LSMPythonDirectiveKind python_directive_for(const WalkState *state,
                                                   uint32_t function_scope_id, const char *name) {
    for (int i = state ? state->python_directive_count - 1 : -1; i >= 0; i--) {
        const LSMPythonDirective *directive = &state->python_directives[i];
        if (directive->function_scope_id == function_scope_id && directive->name &&
            strcmp(directive->name, name) == 0) {
            return (LSMPythonDirectiveKind)directive->kind;
        }
    }
    return 0;
}
...
static uint32_t python_enclosing_function_namespace(const WalkState *state,
    ...
    const LSMLexicalScope *inner = usage_lexical_scope(state, inner_function_id);
...
static bool binding_is_parameter(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
    const LSMOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        const char *kind = ts_node_type(parent);
...
static bool import_kind_matches(TSNode node, const LSMLangSpec *spec) {
    return (spec->import_node_types && lsm_kind_in_set(node, spec->import_node_types)) ||
           (spec->import_from_types && lsm_kind_in_set(node, spec->import_from_types));
}

static TSNode nearest_import_ancestor(TSNode node, const LSMLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
 ...
 * name. */
static bool is_import_binding_occurrence(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec) {
...
static uint32_t lexical_import_scope(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == LSM_LEXICAL_SCOPE_MODULE || scope->kind == LSM_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == LSM_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == LSM_LEXICAL_SCOPE_BLOCK) {
            return id;
        }
        id = scope->parent_id;
    }
...
static void record_lexical_binding(LSMExtractCtx *ctx, WalkState *state, TSNode node,
                                   const LSMLangSpec *spec, const char *raw_name,
                                   bool import_binding) {
    if (!ctx || !state || !raw_name || !raw_name[0] || state->lexical_binding_tracking_failed) {
        return;
    }
    bool parameter = binding_is_parameter(ctx, node, spec);
    if (ctx->language == LSM_LANG_VIMSCRIPT && parameter && !strchr(raw_name, ':')) {
        raw_name = lsm_arena_sprintf(ctx->arena, "a:%s", raw_name);
        if (!raw_name) {
            state->lexical_binding_tracking_failed = true;
            return;
        }
    ...
    if (ctx->language == LSM_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"))) {
        uint32_t function_id = lexical_ancestor_of_kind(state, current_id, true, false);
        LSMPythonDirectiveKind directive = lexical_ancestor_kind(node, "global_statement")
                                               ? LSM_PYTHON_DIRECTIVE_GLOBAL
                                               : LSM_PYTHON_DIRECTIVE_NONLOCAL;
    ...
    if (!ts_node_is_null(function_declaration)) {
        ...
        if (ctx->language == LSM_LANG_PYTHON) {
            ...
            const LSMLexicalScope *owner = usage_lexical_scope(state, scope_id);
            if (!owner || (owner->kind != LSM_LEXICAL_SCOPE_FUNCTION &&
                           owner->kind != LSM_LEXICAL_SCOPE_COMPREHENSION)) {
                return;
            }
        } else {
        ...
        whole_scope = true;
    } else if (ctx->language == LSM_LANG_PYTHON && !ts_node_is_null(class_declaration)) {
        uint32_t class_id = python_nearest_namespace(state, current_id);
        const LSMLexicalScope *class_scope = usage_lexical_scope(state, class_id);
        ...
        whole_scope = true;
    } else if (ctx->language == LSM_LANG_PYTHON) {
        uint32_t namespace_id = python_nearest_namespace(state, current_id);
        const LSMLexicalScope *namespace_scope = usage_lexical_scope(state, namespace_id);
        uint32_t function_id =
            namespace_scope && (namespace_scope->kind == LSM_LEXICAL_SCOPE_FUNCTION ||
                                namespace_scope->kind == LSM_LEXICAL_SCOPE_COMPREHENSION)
                ? namespace_id
                : lexical_ancestor_of_kind(state, current_id, true, false);
        LSMPythonDirectiveKind directive = python_directive_for(state, function_id, name);
        if (directive == LSM_PYTHON_DIRECTIVE_GLOBAL) {
            scope_id = state->root_lexical_scope_id;
        } else if (directive == LSM_PYTHON_DIRECTIVE_NONLOCAL) {
        ...
        whole_scope = directive == 0 && (owner->kind == LSM_LEXICAL_SCOPE_FUNCTION ||
                                         owner->kind == LSM_LEXICAL_SCOPE_COMPREHENSION);
    } else if (ctx->language == LSM_LANG_POWERSHELL) {
        scope_id = lexical_ancestor_of_kind(state, current_id, true, false);
        whole_scope = true;
    } else if (ctx->language == LSM_LANG_JAVASCRIPT || ctx->language == LSM_LANG_TYPESCRIPT ||
               ctx->language == LSM_LANG_TSX) {
    ...
    } else if (ctx->language == LSM_LANG_RUST && import_binding) {
    ...
    const LSMLexicalScope *scope = usage_lexical_scope(state, scope_id);
    if (!scope || !ensure_lexical_binding_capacity(state)) {
        return;
    }
    LSMLexicalBinding *binding = &state->lexical_bindings[state->lexical_binding_count++];
...
static int compare_lexical_bindings(const void *left_ptr, const void *right_ptr) {
    const LSMLexicalBinding *left = (const LSMLexicalBinding *)left_ptr;
    const LSMLexicalBinding *right = (const LSMLexicalBinding *)right_ptr;
...
static int compare_binding_key(const LSMLexicalBinding *binding, uint32_t scope_id,
                               const char *name) {
...
static bool lexical_binding_active_at(const WalkState *state, uint32_t scope_id, const char *name,
    ...
    while (index < state->lexical_binding_count) {
        const LSMLexicalBinding *binding = &state->lexical_bindings[index++];
...
static const LSMLexicalScope *lexical_binding_scope_for_usage(const WalkState *state,
...
void lsm_finalize_lexical_usages(LSMExtractCtx *ctx, WalkState *state) {
    if (!ctx || !state) {
        return;
    }
    ...
    for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
        LSMUsage *usage = &ctx->result->usages.items[i];
        ...
        const LSMLexicalScope *binding_scope = lexical_binding_scope_for_usage(state, usage, name);
        if (!binding_scope) {
            continue;
        }
        usage->semantic_reference_blocked = true;
        usage->semantic_reference_local_shadow = binding_scope->kind != LSM_LEXICAL_SCOPE_MODULE;
    }
    ...
    if (state->lexical_binding_tracking_failed) {
        for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
            LSMUsage *usage = &ctx->result->usages.items[i];
            usage->semantic_reference_blocked = true;
            const LSMLexicalScope *scope = usage_lexical_scope(state, usage->lexical_scope_id);
            usage->semantic_reference_local_shadow =
                scope && scope->kind != LSM_LEXICAL_SCOPE_MODULE;
        }
    }
}

static char *perl_direct_coderef_name(LSMExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != LSM_LANG_PERL ||
    ...
    return end > start ? lsm_arena_strndup(ctx->arena, start, (size_t)(end - start)) : NULL;
}

static bool inside_direct_perl_coderef(LSMExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != LSM_LANG_PERL) {
        return false;
    }
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
...
static bool emit_direct_perl_coderef_usage(LSMExtractCtx *ctx, TSNode node,
    ...
    lsm_usages_push(&ctx->result->usages, ctx->arena, usage);
    return true;
}

// Try to emit a usage for a reference node. Returns early if the node should be skipped.
static void try_emit_usage(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                           bool inside_call, bool inside_import) {
    if (emit_direct_perl_coderef_usage(ctx, node, lsm_enclosing_func_qn_cached(ctx, node), 0)) {
        return;
    }
    ...
    if (name && name[0] && !lsm_is_keyword(name, ctx->language)) {
        LSMUsage usage = {0};
        usage.ref_name = name;
        usage.enclosing_func_qn = lsm_enclosing_func_qn_cached(ctx, node);
...
static void walk_usages(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
    ...
    int cap = 256;
    UsageFrame *frames = (UsageFrame *)lsm_arena_alloc(ctx->arena, (size_t)cap * sizeof(*frames));
    if (!frames) {
        return;
    }
    ...
    while (top > 0) {
        ...
        if (entering) {
            ...
            f->counts_call = lsm_kind_in_set(f->node, spec->call_node_types);
            f->counts_import =
                (has_imports && lsm_kind_in_set(f->node, spec->import_node_types)) ||
                (has_from_imports && lsm_kind_in_set(f->node, spec->import_from_types));
            if (f->counts_call) {
                call_depth++;
            }
            if (f->counts_import) {
        ...
        if (f->next_child < count) {
            ...
            if (top == cap) {
                int new_cap = cap * 2;
                UsageFrame *grown =
                    (UsageFrame *)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*grown));
                if (!grown) {
                    return;
                }
...
void handle_usages(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec, WalkState *state) {
    ...
    if (exact_callee) {
        if (reference_node && state->invocation_kind == LSM_INVOCATION_CALLABLE_REFERENCE) {
            char *name = reference_name(ctx, node);
            if (name && name[0] && !lsm_is_keyword(name, ctx->language)) {
                LSMUsage usage = {0};
                usage.ref_name = name;
    ...
    if (state->inside_import) {
        if (is_import_binding_occurrence(ctx, node, spec)) {
            ...
            if (binding_name && binding_name[0] && !lsm_is_keyword(binding_name, ctx->language)) {
    ...
    bool python_scope_directive =
        ctx->language == LSM_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"));
