 ...
 * no extra memory.
 */
#ifndef LSM_EXTRACT_NODE_STACK_H
#define LSM_EXTRACT_NODE_STACK_H
...
static inline void ts_nstack_init(TSNodeStack *s, LSMArena *arena, int initial_cap) {
    s->items = (TSNode *)lsm_arena_alloc(arena, (size_t)initial_cap * sizeof(TSNode));
    s->count = 0;
    s->cap = s->items ? initial_cap : 0;
}

/* Push a node onto the stack, growing 2x if needed. */
static inline void ts_nstack_push(TSNodeStack *s, LSMArena *arena, TSNode node) {
    if (s->count >= s->cap) {
        int new_cap = s->cap ? s->cap * 2 : 512;
        TSNode *new_items = (TSNode *)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(TSNode));
        if (!new_items)
 ...
 */
static inline void ts_nstack_push_children(TSNodeStack *s, LSMArena *arena, TSNode node) {
    int base = s->count;
...
#endif /* LSM_EXTRACT_NODE_STACK_H */
