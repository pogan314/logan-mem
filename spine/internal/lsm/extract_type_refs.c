#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_sprintf/strndup
#include "helpers.h"
...
static const char *clean_type_name(LSMArena *a, const char *name) {
    if (!name || !name[0]) {
        return name;
    }
    // Skip leading *, &, [], ?
    ...
    for (size_t i = 0; i < len; i++) {
        if (name[i] == '<' || name[i] == '[') {
            return lsm_arena_strndup(a, name, i);
        }
    }
    // Strip trailing ? (optionals)
    if (len > 0 && name[len - SKIP_ONE] == '?') {
        return lsm_arena_strndup(a, name, len - SKIP_ONE);
    }
    return name;
}

// Extract type name from a type annotation node.
static const char *extract_type_text(LSMArena *a, TSNode node, const char *source) {
    enum { MAX_UNWRAP = 8 };
    for (int depth = 0; depth < MAX_UNWRAP; depth++) {
        ...
        if (strcmp(kind, "type_identifier") == 0 || strcmp(kind, "identifier") == 0 ||
            ...
            return lsm_node_text(a, node, source);
        }
    ...
    return clean_type_name(a, lsm_node_text(a, node, source));
}

// Add a type reference for a function.
static void add_type_ref(LSMExtractCtx *ctx, const char *type_name, const char *func_qn) {
    ...
    lsm_typerefs_push(&ctx->result->type_refs, ctx->arena, tr);
}

// Extract parameter types from a parameters/formal_parameters node.
static void extract_param_type_refs(LSMExtractCtx *ctx, TSNode params, const char *func_qn) {
...
static void extract_return_type_refs(LSMExtractCtx *ctx, TSNode func_node, const char *func_qn) {
...
static void extract_type_field_ref(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
    TSNode type_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
    if (!ts_node_is_null(type_node)) {
...
static void extract_ts_body_type_refs(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                      const char *func_qn) {
    if (strcmp(kind, "as_expression") == 0 || strcmp(kind, "satisfies_expression") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ts_node_type(child), "type_identifier") == 0 ||
                ...
                add_type_ref(ctx, lsm_node_text(ctx->arena, child, ctx->source), func_qn);
            }
        }
...
static void extract_java_body_type_refs(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                        const char *func_qn) {
            ...
            if (strcmp(ts_node_type(child), "type_arguments") == 0) {
                ...
                for (uint32_t j = 0; j < nta; j++) {
                    ...
                    if (strcmp(ts_node_type(ta), "type_identifier") == 0) {
                        add_type_ref(ctx, lsm_node_text(ctx->arena, ta, ctx->source), func_qn);
                    }
                }
            }
        }
...
static void process_body_type_ref(LSMExtractCtx *ctx, TSNode node, const char *func_qn) {
    const char *kind = ts_node_type(node);
    ...
    case LSM_LANG_PYTHON:
        if (strcmp(kind, "assignment") == 0) {
            TSNode type_node = ts_node_child_by_field_name(node, TS_FIELD("type"));
            if (!ts_node_is_null(type_node)) {
                add_type_ref(ctx, lsm_node_text(ctx->arena, type_node, ctx->source), func_qn);
            }
        }
        break;
    case LSM_LANG_RUST:
...
static void walk_body_type_refs(LSMExtractCtx *ctx, TSNode root, const char *func_qn) {
    TSNodeStack stack;
...
static void process_func_type_refs(LSMExtractCtx *ctx, TSNode node) {
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
    }
    char *func_name = lsm_node_text(ctx->arena, name_node, ctx->source);
    if (!func_name || !func_name[0]) {
        return;
    }
    const char *func_qn = lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, func_name);
...
static void walk_type_refs(LSMExtractCtx *ctx, TSNode root, const LSMLangSpec *spec) {
...
static void extract_signature_type_refs(LSMExtractCtx *ctx, TSNode node, WalkState *state) {
    TSNode name_node = ts_node_child_by_field_name(node, TS_FIELD("name"));
    if (ts_node_is_null(name_node)) {
        return;
    }
    ...
    if (state->enclosing_class_qn) {
        func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, func_name);
    } else {
        func_qn = lsm_fqn_compute(ctx->arena, ctx->project, ctx->rel_path, func_name);
    }
...
void handle_type_refs(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec, WalkState *state) {
    if (!spec->function_node_types || !spec->function_node_types[0]) {
        return;
    }
