 ...
 * run for ~133 minutes). lsm_lsp_collect_children() enumerates the children in
 * a single O(n) cursor pass into an arena array; callers then index the array.
 */
#ifndef LSM_LSP_NODE_ITER_H
#define LSM_LSP_NODE_ITER_H
...
static inline TSNode* lsm_lsp_collect_children(LSMArena* arena, TSNode node, uint32_t* out_n) {
    uint32_t nc = ts_node_child_count(node);
    *out_n = 0;
    if (nc == 0) return NULL;
