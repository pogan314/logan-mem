...
long lsm_ts_full_registry_builds(void) {
    return atomic_load(&g_ts_full_reg_builds);
}
void lsm_ts_full_registry_builds_reset(void) {
 ...
 * enough that no real-world file ever hits it. LSM_TS_TYPE_BUDGET (absolute
...
static void ts_type_budget_reset(size_t source_len) {
    const char *e = getenv("LSM_TS_TYPE_BUDGET");
    if (e && e[0]) {
        long v = atol(e);
...
/* Expression-type memo (per file): node.id -> evaluated LSMType.
 *
...
static const LSMType *ts_memo_get(const TsEvalMemo *m, const void *id) {
    if (!m || !m->cap)
        return NULL;
...
static void ts_memo_put(TSLSPContext *ctx, const void *id, const LSMType *t) {
    TsEvalMemo *m = ctx->eval_memo;
    if (!m) {
        m = (TsEvalMemo *)lsm_arena_alloc(ctx->arena, sizeof(*m));
        if (!m)
            return;
        m->cap = 1024;
        m->count = 0;
        m->keys = (const void **)lsm_arena_alloc(ctx->arena, m->cap * sizeof(*m->keys));
        m->vals = (const LSMType **)lsm_arena_alloc(ctx->arena, m->cap * sizeof(*m->vals));
        if (!m->keys || !m->vals)
            return;
    ...
    if (m->count * 10 >= m->cap * 7) { /* grow at 70% load */
        TsEvalMemo bigger = {0};
        bigger.cap = m->cap * 2;
        bigger.keys = (const void **)lsm_arena_alloc(ctx->arena, bigger.cap * sizeof(*bigger.keys));
        bigger.vals =
            (const LSMType **)lsm_arena_alloc(ctx->arena, bigger.cap * sizeof(*bigger.vals));
 ...
 * the warned flag after a lsm_run_ts_lsp call. Budget units consumed are a
...
long lsm_ts_lsp_test_budget_remaining(void) {
    return g_ts_type_budget;
}
bool lsm_ts_lsp_test_budget_warned(void) {
...
static const LSMType *parse_ts_type_text(LSMArena *arena, const char *text, const char *module_qn);
...
static void process_function_body(TSLSPContext *ctx, TSNode body, const char *func_qn,
                                  const char *class_qn);
static const LSMType *type_of_identifier(TSLSPContext *ctx, const char *name);
static const LSMType *lookup_member_type(TSLSPContext *ctx, const LSMType *recv, const char *name);
static const LSMRegisteredFunc *lookup_method(TSLSPContext *ctx, const LSMType *recv,
...
static TSNode *collect_children(LSMArena *arena, TSNode node, uint32_t *out_n) {
    uint32_t nc = ts_node_child_count(node);
    *out_n = 0;
    if (nc == 0)
        return NULL;
    TSNode *kids = (TSNode *)lsm_arena_alloc(arena, (size_t)nc * sizeof(TSNode));
    if (!kids)
        return NULL;
    uint32_t kn = 0;
...
static const LSMType *simplify_type(TSLSPContext *ctx, const LSMType *t);
static const LSMType *unwrap_passthrough_template(const LSMType *t);
...
// Parse a textual TS type into a LSMType. Pragmatic v1: handle the common cases
...
// `lsm_node_text` on a `type_annotation` AST node), and trailing whitespace/`;`.
static const LSMType *parse_ts_type_text(LSMArena *arena, const char *text, const char *module_qn) {
    if (!text || !text[0])
    ...
    if (text[0] == '(') {
        ...
        if (after + 2 <= len && text[after] == '=' && text[after + 1] == '>') {
            ...
            if (params_len > 0) {
                ...
                for (size_t i = 0; i <= params_len && pc < 15; i++) {
                    ...
                    if (c == '(' || c == '<' || c == '[')
                        ...
                        if (i > start) {
                            char *part = lsm_arena_strndup(arena, params_text + start, i - start);
                            ...
                            if (colon) {
                                *colon = '\0';
                                ...
                                while (nl > 0 && (nm[nl - 1] == ' ' || nm[nl - 1] == '?'))
                                    nl--;
                                fn_param_names[pc] = lsm_arena_strndup(arena, nm, nl);
                                fn_param_types[pc] =
                                    parse_ts_type_text(arena, colon + 1, module_qn);
                            } else {
                                fn_param_names[pc] = lsm_arena_sprintf(arena, "_%d", pc);
            ...
            const LSMType *rets[2] = {ret, NULL};
            return lsm_type_func(arena, fn_param_names, fn_param_types, rets);
        }
    ...
    if (len == 4 && memcmp(text, "this", 4) == 0) {
        return lsm_type_type_param(arena, "this");
    }
    ...
    // keywords at depth 0, in order.
    {
        ...
        if (ext_pos != (size_t)-1 && q_pos != (size_t)-1 && colon_pos != (size_t)-1) {
            char *check_text = lsm_arena_strndup(arena, text, ext_pos);
            char *extends_text =
                lsm_arena_strndup(arena, text + ext_pos + 9, q_pos - (ext_pos + 9));
            char *true_text = lsm_arena_strndup(arena, text + q_pos + 3, colon_pos - (q_pos + 3));
            char *false_text =
                lsm_arena_strndup(arena, text + colon_pos + 3, len - (colon_pos + 3));
            const LSMType *check = parse_ts_type_text(arena, check_text, module_qn);
            const LSMType *extends = parse_ts_type_text(arena, extends_text, module_qn);
            const LSMType *true_branch = parse_ts_type_text(arena, true_text, module_qn);
            const LSMType *false_branch = parse_ts_type_text(arena, false_text, module_qn);
            return lsm_type_conditional(arena, check, extends, true_branch, false_branch);
        }
    }

    // `infer X` — emit LSM_TYPE_INFER. Only meaningful inside conditional `extends`
    ...
    if (len > 6 && memcmp(text, "infer ", 6) == 0) {
        ...
        char *name = lsm_arena_strndup(arena, rest, (size_t)(end - rest));
        return lsm_type_infer(arena, name);
    }

    // `keyof T` — emit LSM_TYPE_KEYOF for downstream evaluation.
    if (len > 6 && memcmp(text, "keyof ", 6) == 0) {
        const char *rest = text + 6;
        while (*rest == ' ' || *rest == '\t')
            rest++;
        char *rest_term = lsm_arena_strdup(arena, rest);
        const LSMType *operand = parse_ts_type_text(arena, rest_term, module_qn);
        return lsm_type_keyof(arena, operand);
    }

    // `typeof X` in type position — emit LSM_TYPE_TYPEOF_QUERY.
    if (len > 7 && memcmp(text, "typeof ", 7) == 0) {
        const char *rest = text + 7;
        while (*rest == ' ' || *rest == '\t')
            rest++;
        return lsm_type_typeof_query(arena, rest);
    }
    ...
    if (text[0] == '[' && len >= 2 && text[len - 1] == ']') {
        ...
        for (size_t i = 0; i <= inner_len && ec < 15; i++) {
            ...
            if (c == '<' || c == '[')
                ...
                char *part = lsm_arena_strndup(arena, inner_text + start, i - start);
                elems[ec++] = parse_ts_type_text(arena, part, module_qn);
                start = i + 1;
            }
        }
        elems[ec] = NULL;
        return lsm_type_tuple(arena, elems, ec);
    }

    // Top-level UNION ` | ` or INTERSECTION ` & ` — split at depth 0 (outside <>, []).
    {
        int depth = 0;
        const LSMType *members[16] = {0};
        int mc = 0;
        size_t start = 0;
        bool is_union = false;
        bool is_inter = false;
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '<' || c == '[')
                ...
                depth--;
                     ...
                     (text[i - 1] == ' ' || text[i - 1] == '\t')) {
                if (mc < 15) {
                    char *part = lsm_arena_strndup(arena, text + start, i - start);
        ...
        if (mc > 0 && (is_union || is_inter)) {
            ...
            // (UBSan: index 16 out of bounds for 'const LSMType *[16]').
            if (mc < 15) {
                char *part = lsm_arena_strndup(arena, text + start, len - start);
                members[mc++] = parse_ts_type_text(arena, part, module_qn);
            }
            members[mc] = NULL;
            return is_union ? lsm_type_union(arena, members, mc)
                            : lsm_type_intersection(arena, members, mc);
        }
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        ...
        if (len == bl && memcmp(text, builtins[i], bl) == 0) {
            return lsm_type_builtin(arena, builtins[i]);
        }
    }

    // Trailing `[]` → Array<elem>. Strip and recurse.
    if (len >= 2 && text[len - 1] == ']' && text[len - 2] == '[') {
        char *inner = lsm_arena_strndup(arena, text, len - 2);
        const LSMType *elem = parse_ts_type_text(arena, inner, module_qn);
        const LSMType *args[2] = {elem, NULL};
        return lsm_type_template(arena, "Array", args, 1);
    }
        ...
        if (open != (size_t)-1 && open > 0) {
            ...
            for (size_t i = 0; i <= args_len && arg_count < 15; i++) {
                ...
                if (c == '<')
                    ...
                    char *part = lsm_arena_strndup(arena, args_text + start, i - start);
            ...
            if (!is_stdlib && module_qn && strchr(base, '.') == NULL && base[0] >= 'A' &&
                base[0] <= 'Z') {
                qualified_base = lsm_arena_sprintf(arena, "%s.%s", module_qn, base);
            }
            return lsm_type_template(arena, qualified_base, args, arg_count);
        }
    }
    ...
    if (module_qn && strchr(text, '.') == NULL) {
        ...
        char *terminated = lsm_arena_strndup(arena, text, len);
        if (!terminated)
            return lsm_type_unknown();
        for (int i = 0; bare_stdlib_names[i]; i++) {
            if (strcmp(terminated, bare_stdlib_names[i]) == 0) {
                return lsm_type_named(arena, terminated);
            }
        }
        ...
        if ((terminated[0] >= 'A' && terminated[0] <= 'Z')) {
            const char *qn = lsm_arena_sprintf(arena, "%s.%s", module_qn, terminated);
            return lsm_type_named(arena, qn);
        }
        return lsm_type_named(arena, terminated);
    }
    return lsm_type_named(arena, lsm_arena_strndup(arena, text, len));
}

typedef struct {
    const char *module_qn;
} LSMTSLSPSignatureParserContext;

static const LSMType *lsm_ts_lsp_parse_signature_param_text(LSMArena *arena, const char *text,
                                                            void *parser_ctx) {
    const LSMTSLSPSignatureParserContext *ctx = (const LSMTSLSPSignatureParserContext *)parser_ctx;
    const char *module_qn = ctx && ctx->module_qn ? ctx->module_qn : "";
    return parse_ts_type_text(arena, text, module_qn);
}

// Return param-type LSMType array from a LSMDefinition's param_types text array.
static const LSMType **parse_param_types_array(LSMArena *arena, const char **texts,
                                               const char *module_qn) {
    if (!texts)
        return NULL;
    int count = 0;
    ...
    if (count == 0)
        return NULL;
    const LSMType **arr =
        (const LSMType **)lsm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const LSMType *));
    if (!arr)
        return NULL;
...
static int ts_is_assignable_inner(TSLSPContext *ctx, const LSMType *a, const LSMType *b, int depth,
                                  TSRelaterCacheSlot *cache);

static const LSMType *ts_relater_unwrap(const LSMType *t) {
    if (!t)
        return t;
    if (t->kind == LSM_TYPE_ALIAS)
        return lsm_type_resolve_alias(t);
    return t;
}
...
static bool ts_literal_promotes_to_builtin(const LSMType *lit, const LSMType *bi) {
    if (!lit || lit->kind != LSM_TYPE_TS_LITERAL || !lit->data.literal_ts.tag)
        return false;
    if (!bi || bi->kind != LSM_TYPE_BUILTIN || !bi->data.builtin.name)
        return false;
...
static bool ts_class_extends(TSLSPContext *ctx, const char *sub_qn, const char *super_qn,
    ...
    if (strcmp(sub_qn, super_qn) == 0)
        return true;
    const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, sub_qn);
...
static int ts_is_assignable_inner(TSLSPContext *ctx, const LSMType *a, const LSMType *b, int depth,
    ...
    if (b->kind == LSM_TYPE_BUILTIN && b->data.builtin.name) {
        if (strcmp(b->data.builtin.name, "any") == 0 ||
            strcmp(b->data.builtin.name, "unknown") == 0)
            return 1;
    }
    if (a->kind == LSM_TYPE_BUILTIN && a->data.builtin.name) {
    ...
    if (a->kind == LSM_TYPE_UNION && a->data.union_type.members) {
        result = 1;
    ...
    if (b->kind == LSM_TYPE_UNION && b->data.union_type.members) {
    ...
    if (a->kind == LSM_TYPE_INTERSECTION && a->data.union_type.members) {
        for (int i = 0; i < a->data.union_type.count; i++) {
            if (ts_is_assignable_inner(ctx, a->data.union_type.members[i], b, depth + 1, cache)) {
                result = 1;
                break;
    ...
    if (b->kind == LSM_TYPE_INTERSECTION && b->data.union_type.members) {
        result = 1;
        for (int i = 0; i < b->data.union_type.count; i++) {
            if (!ts_is_assignable_inner(ctx, a, b->data.union_type.members[i], depth + 1, cache)) {
                result = 0;
    ...
    case LSM_TYPE_FUNC: {
        // Return covariance.
        const LSMType *aret = (a->data.func.return_types && a->data.func.return_types[0])
                                  ? a->data.func.return_types[0]
                                  : NULL;
        const LSMType *bret = (b->data.func.return_types && b->data.func.return_types[0])
...
static bool ts_is_assignable(TSLSPContext *ctx, const LSMType *a, const LSMType *b) {
    if (!ctx || !a || !b)
        return false;
...
static const LSMType *subst_infer_bindings(TSLSPContext *ctx, const LSMType *t,
    ...
    if (t->kind == LSM_TYPE_TYPE_PARAM && t->data.type_param.name) {
        for (int i = 0; i < bind_count; i++) {
    ...
    if (t->kind == LSM_TYPE_NAMED && t->data.named.qualified_name) {
    ...
    if (t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_args &&
        t->data.template_type.arg_count > 0) {
        int ac = t->data.template_type.arg_count;
        const LSMType **new_args = (const LSMType **)lsm_arena_alloc(
            ctx->arena, (size_t)(ac + 1) * sizeof(const LSMType *));
        if (!new_args)
            return t;
        for (int i = 0; i < ac; i++) {
        ...
        new_args[ac] = NULL;
        return lsm_type_template(ctx->arena, t->data.template_type.template_name, new_args, ac);
    }
    if (t->kind == LSM_TYPE_TUPLE && t->data.tuple.count > 0) {
        int n = t->data.tuple.count;
        const LSMType **new_elems = (const LSMType **)lsm_arena_alloc(
            ctx->arena, (size_t)(n + 1) * sizeof(const LSMType *));
        if (!new_elems)
            return t;
        for (int i = 0; i < n; i++) {
            new_elems[i] =
        ...
        new_elems[n] = NULL;
        return lsm_type_tuple(ctx->arena, new_elems, n);
    }
    return t;
}

// Walk a type tree to detect any LSM_TYPE_INFER nodes.
static bool contains_infer(const LSMType *t, int depth) {
    if (!t || depth > 32)
        return false;
    if (t->kind == LSM_TYPE_INFER)
        return true;
    if (t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_args) {
...
// Evaluate a LSM_TYPE_CONDITIONAL `T extends U ? X : Y`. With distribution: when T is
// a UNION, evaluate per member and union the results. Uses the partial relater for
// the extends check.
static const LSMType *eval_conditional(TSLSPContext *ctx, const LSMType *t, int depth) {
    if (!t || t->kind != LSM_TYPE_CONDITIONAL || depth > 16)
        return t;
    const LSMType *check = t->data.conditional.check;
    const LSMType *extends = t->data.conditional.extends;
    const LSMType *tb = t->data.conditional.true_branch;
    const LSMType *fb = t->data.conditional.false_branch;
    if (!check || !extends)
        return t;

    // Distribution: when check is a UNION, evaluate per member and union results.
    if (check->kind == LSM_TYPE_UNION && check->data.union_type.members &&
        check->data.union_type.count > 0) {
        const LSMType **results = (const LSMType **)lsm_arena_alloc(
            ctx->arena, (size_t)(check->data.union_type.count + 1) * sizeof(const LSMType *));
        if (!results)
            return t;
        int n = 0;
        for (int i = 0; i < check->data.union_type.count; i++) {
            const LSMType *member = check->data.union_type.members[i];
            const LSMType *sub = lsm_type_conditional(ctx->arena, member, extends, tb, fb);
            results[n++] = eval_conditional(ctx, sub, depth + 1);
        }
        results[n] = NULL;
        if (n == 1)
            return results[0];
        return lsm_type_union(ctx->arena, results, n);
    }
...
// Resolve a LSMType against the registry's alias chain when it's a NAMED type that's a
// pure alias (`type Foo = Bar`). Also evaluates inline conditional types.
static const LSMType *simplify_type(TSLSPContext *ctx, const LSMType *t) {
    if (!t || !ctx || !ctx->registry)
        return t;
    if (t->kind == LSM_TYPE_CONDITIONAL) {
        const LSMType *evaluated = eval_conditional(ctx, t, 0);
    ...
    if (t->kind == LSM_TYPE_ALIAS)
        return lsm_type_resolve_alias(t);
    if (t->kind != LSM_TYPE_NAMED)
        return t;
    for (int i = 0; i < 16; i++) {
        if (!t || t->kind != LSM_TYPE_NAMED)
            return t;
        const LSMRegisteredType *rt =
            lsm_registry_lookup_type(ctx->registry, t->data.named.qualified_name);
        if (!rt || !rt->alias_of)
            return t;
        t = lsm_type_named(ctx->arena, rt->alias_of);
    }
    return t;
}

/* lsm_scope_lookup intentionally collapses an absent binding and an UNKNOWN
...
static const LSMScope *ts_scope_binding_owner(const LSMScope *scope, const char *name) {
    if (!name) {
        return NULL;
    }
    for (const LSMScope *s = scope; s; s = s->parent) {
        for (const LSMScopeChunk *chunk = s->chunks; chunk; chunk = chunk->next) {
...
static bool ts_scope_has_binding(const LSMScope *scope, const char *name) {
...
static const char *ts_import_symbol_qn(TSLSPContext *ctx, const char *module_qn, const char *name) {
    ...
    return lsm_arena_sprintf(ctx->arena, "%s.%s", module_qn, name);
}
...
static const LSMRegisteredFunc *ts_exact_same_module_function(TSLSPContext *ctx, const char *name) {
    if (!ctx || !ctx->registry || !ctx->module_qn || !name ||
        ts_scope_has_binding(ctx->current_scope, name)) {
        return NULL;
    }
    const char *full_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
    return full_qn ? lsm_registry_lookup_func(ctx->registry, full_qn) : NULL;
}
 ...
 * name is unavailable in LSMImport deliberately fail closed and remain USAGE. */
static const LSMRegisteredFunc *ts_exact_imported_function(TSLSPContext *ctx, const char *name) {
...
static void ts_block_shadowed_reference_usage(TSLSPContext *ctx, const char *name, TSNode site) {
    ...
    const LSMScope *binding_scope = ts_scope_binding_owner(ctx->current_scope, name);
    if (!binding_scope) {
        return;
    }
    ...
    for (int i = 0; i < ctx->usages->count; i++) {
        LSMUsage *usage = &ctx->usages->items[i];
...
static const LSMType *type_of_identifier(TSLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, full_qn);
        if (f && f->signature)
            return f->signature;
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, full_qn);
        if (rt)
            return lsm_type_named(ctx->arena, full_qn);

        // No specific match — fall back to NAMED(module_qn), which is correct for
        // namespace-style imports (`import * as foo from './foo'`).
        return lsm_type_named(ctx->arena, mqn);
    }

    // Module-local function or class.
    if (ctx->module_qn) {
        const char *candidate = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, candidate);
        if (rt)
            return lsm_type_named(ctx->arena, candidate);
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, candidate);
        if (f && f->signature)
            return f->signature;
    }
    ...
    {
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, name);
        if (rt)
            return lsm_type_named(ctx->arena, name);
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, name);
        if (f && f->signature)
            return f->signature;
    }
...
static const LSMType *lookup_member_type_inner(TSLSPContext *ctx, const LSMType *recv,
                                               const char *name);
...
static const LSMType *lookup_member_type(TSLSPContext *ctx, const LSMType *recv, const char *name) {
    if (!ctx || ctx->member_depth >= TS_LSP_MAX_MEMBER_DEPTH)
        return lsm_type_unknown();
    ctx->member_depth++;
    const LSMType *r = lookup_member_type_inner(ctx, recv, name);
    ctx->member_depth--;
    return r;
}
...
static const LSMType *lookup_member_type_inner(TSLSPContext *ctx, const LSMType *recv,
    ...
    if (!ctx || !recv || !name)
        return lsm_type_unknown();
    const LSMType *base = simplify_type(ctx, recv);
    if (!base)
        return lsm_type_unknown();
    ...
    if (base->kind == LSM_TYPE_BUILTIN) {
        const char *wrap = builtin_wrapper_class(base->data.builtin.name);
        if (wrap) {
            const LSMType *wrapped = lsm_type_named(ctx->arena, wrap);
            return lookup_member_type(ctx, wrapped, name);
        }
        return lsm_type_unknown();
    }

    if (base->kind == LSM_TYPE_TEMPLATE) {
        // Look up on the template name (e.g., "Array", "Promise"); generic params are
        // applied via lsm_type_substitute when we know the registered type's params.
        const LSMRegisteredType *rt =
            lsm_registry_lookup_type(ctx->registry, base->data.template_type.template_name);
        if (rt) {
            // Field?
            if (rt->field_names && rt->field_types) {
                for (int i = 0; rt->field_names[i]; i++) {
                    if (strcmp(rt->field_names[i], name) == 0) {
                        return lsm_type_substitute(ctx->arena, rt->field_types[i],
                                                   rt->type_param_names,
                ...
                for (int i = 0; rt->method_names[i]; i++) {
                    if (strcmp(rt->method_names[i], name) == 0) {
                        const LSMRegisteredFunc *f =
                            lsm_registry_lookup_func(ctx->registry, rt->method_qns[i]);
                        if (f && f->signature) {
                            return lsm_type_substitute(ctx->arena, f->signature,
                                                       rt->type_param_names,
                                                       base->data.template_type.template_args);
                        }
                    }
    ...
    if (base->kind == LSM_TYPE_OBJECT_LIT) {
    ...
    if (base->kind == LSM_TYPE_UNION) {
        ...
        if (base->data.union_type.members) {
            for (int i = 0; i < base->data.union_type.count; i++) {
                const LSMType *m = lookup_member_type(ctx, base->data.union_type.members[i], name);
                if (!lsm_type_is_unknown(m))
                    return m;
            }
        }
        return lsm_type_unknown();
    ...
    if (base->kind == LSM_TYPE_INTERSECTION) {
        if (base->data.union_type.members) {
            for (int i = 0; i < base->data.union_type.count; i++) {
                const LSMType *m = lookup_member_type(ctx, base->data.union_type.members[i], name);
                if (!lsm_type_is_unknown(m))
    ...
    if (base->kind != LSM_TYPE_NAMED)
        return lsm_type_unknown();

    const char *recv_qn = base->data.named.qualified_name;
    const LSMRegisteredType *rt = lsm_registry_resolve_alias(ctx->registry, recv_qn);
    if (!rt) {
        rt = lsm_registry_lookup_type(ctx->registry, recv_qn);
        if (!rt)
            return lsm_type_unknown();
    }
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const LSMType *parent = lsm_type_named(ctx->arena, rt->embedded_types[i]);
            const LSMType *m = lookup_member_type(ctx, parent, name);
            if (!lsm_type_is_unknown(m))
                return m;
        }
    }
...
static const LSMType *unwrap_passthrough_template(const LSMType *t) {
    ...
    if (strcmp(name, "ReturnType") == 0) {
        const LSMType *f = t->data.template_type.template_args[0];
        if (f && f->kind == LSM_TYPE_FUNC && f->data.func.return_types &&
...
static const LSMType *eval_keyof(TSLSPContext *ctx, const LSMType *operand) {
    if (!ctx || !operand)
        return lsm_type_unknown();
    const LSMType *base = simplify_type(ctx, operand);
    if (!base || base->kind != LSM_TYPE_NAMED)
        return lsm_type_unknown();
    const LSMRegisteredType *rt =
        lsm_registry_lookup_type(ctx->registry, base->data.named.qualified_name);
    if (!rt || !rt->field_names || !rt->field_names[0])
        return lsm_type_unknown();
    int fc = 0;
    while (rt->field_names[fc])
        fc++;
    const LSMType *members[64];
    int mc = 0;
    for (int i = 0; i < fc && mc < 63; i++) {
        members[mc++] = lsm_type_ts_literal(ctx->arena, "string", rt->field_names[i]);
    }
    members[mc] = NULL;
    if (mc == 0)
        return lsm_type_unknown();
    if (mc == 1)
        return members[0];
    return lsm_type_union(ctx->arena, members, mc);
}
...
static const LSMType *eval_indexed_access(TSLSPContext *ctx, const LSMType *obj,
                                          const LSMType *key) {
    if (!ctx || !obj || !key)
        return lsm_type_unknown();
    if (key->kind == LSM_TYPE_TS_LITERAL && key->data.literal_ts.value &&
...
static const LSMRegisteredFunc *lookup_method(TSLSPContext *ctx, const LSMType *recv,
    ...
    if (ctx->debug) {
        const char *base_qn = (base->kind == LSM_TYPE_NAMED) ? base->data.named.qualified_name
                              : (base->kind == LSM_TYPE_TEMPLATE)
        ...
        if (rt && rt->method_qns && rt->method_names) {
            for (int i = 0; rt->method_names[i]; i++) {
                if (strcmp(rt->method_names[i], method_name) == 0) {
                    return lsm_registry_lookup_func(ctx->registry, rt->method_qns[i]);
                }
            }
        }
    }

    if (base->kind == LSM_TYPE_UNION || base->kind == LSM_TYPE_INTERSECTION) {
        if (base->data.union_type.members) {
            for (int i = 0; i < base->data.union_type.count; i++) {
                const LSMRegisteredFunc *f =
    ...
    const LSMRegisteredFunc *f =
        lsm_registry_lookup_method_aliased(ctx->registry, recv_qn, method_name);
    if (f)
        return f;

    // Walk extends/implements via the registered type.
    const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, recv_qn);
    if (rt && rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const LSMType *parent = lsm_type_named(ctx->arena, rt->embedded_types[i]);
            const LSMRegisteredFunc *pf = lookup_method(ctx, parent, method_name);
            if (pf)
                return pf;
        }
    }
...
static const LSMRegisteredFunc *ts_lookup_method_for_call(TSLSPContext *ctx, const LSMType *recv,
                                                          const char *method_name, TSNode args) {
    const LSMRegisteredFunc *fallback = lookup_method(ctx, recv, method_name);
    if (!fallback || !fallback->receiver_type)
        return fallback;

    int arg_count = ts_node_is_null(args) ? 0 : (int)ts_node_named_child_count(args);
    const LSMRegisteredFunc *selected = NULL;
    if (arg_count > 0 && arg_count <= 16) {
        const LSMType *arg_types[16] = {0};
        for (int i = 0; i < arg_count; i++) {
            arg_types[i] = ts_eval_expr_type(ctx, ts_node_named_child(args, (uint32_t)i));
        }
        selected = lsm_registry_lookup_method_by_types(ctx->registry, fallback->receiver_type,
                                                       method_name, arg_types, arg_count);
    } else {
        selected = lsm_registry_lookup_method_by_args(ctx->registry, fallback->receiver_type,
...
static const LSMRegisteredFunc *ts_lookup_symbol_for_call(TSLSPContext *ctx, const char *module_qn,
    ...
    if (module_len > symbol_len + 1 && module_qn[module_len - symbol_len - 1] == '.' &&
        ...
        lookup_module = lsm_arena_strndup(ctx->arena, module_qn, module_len - symbol_len - 1);
    }
    ...
    if (arg_count > 0 && arg_count <= 16) {
        ...
        for (int i = 0; i < arg_count; i++)
            arg_types[i] = ts_eval_expr_type(ctx, ts_node_named_child(args, (uint32_t)i));
        selected = lsm_registry_lookup_symbol_by_types(ctx->registry, lookup_module, symbol_name,
                                                       arg_types, arg_count);
    }
    if (!selected) {
        selected = lsm_registry_lookup_symbol_by_args(ctx->registry, lookup_module, symbol_name,
                                                      arg_count);
    }
    return selected;
}
...
static const LSMRegisteredFunc *ts_lookup_namespace_call(TSLSPContext *ctx, const char *object_name,
                                                         const char *method_name, TSNode args,
                                                         bool *out_is_import) {
    if (out_is_import)
        *out_is_import = false;
    ...
    if (ctx && ctx->module_qn && object_name) {
        const char *local_module =
            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, object_name);
...
static const LSMType *ts_method_signature_for_receiver(TSLSPContext *ctx, const LSMType *recv,
                                                       const LSMRegisteredFunc *method) {
    if (!method || !method->signature)
        return NULL;
    const LSMType *base = simplify_type(ctx, recv);
    base = base ? unwrap_passthrough_template(base) : NULL;
    if (!base || base->kind != LSM_TYPE_TEMPLATE || !base->data.template_type.template_args) {
        return method->signature;
    }

    const LSMRegisteredType *registered =
        lsm_registry_lookup_type(ctx->registry, base->data.template_type.template_name);
    ...
    const LSMType *substituted =
        lsm_type_substitute(ctx->arena, method->signature, registered->type_param_names,
                            base->data.template_type.template_args);
...
static const LSMType *ts_signature_for_call(TSLSPContext *ctx, TSNode function, TSNode args) {
    ...
    if (node_kind_is(function, "member_expression")) {
        ...
        if (node_kind_is(object, "identifier")) {
            ...
            if (!ts_scope_has_binding(ctx->current_scope, object_name)) {
                ...
                const LSMRegisteredFunc *namespace_func =
        ...
        const LSMType *recv = ts_eval_expr_type(ctx, object);
        const LSMRegisteredFunc *method = ts_lookup_method_for_call(ctx, recv, method_name, args);
        const LSMType *signature = ts_method_signature_for_receiver(ctx, recv, method);
    ...
    if (node_kind_is(function, "identifier")) {
        ...
        if (name) {
            const LSMRegisteredFunc *registered = NULL;
            if (ctx->module_qn)
...
static const LSMType *return_type_of(LSMArena *arena, const LSMType *sig) {
    if (!sig || sig->kind != LSM_TYPE_FUNC)
        return lsm_type_unknown();
    ...
    // crashed in lsm_arena_alloc when the cross-resolve path evaluated a
    // multi-return signature (only reachable once the per-file LSP O(n²) was
    // fixed and the resolve phase actually ran).
    return lsm_type_tuple(arena, sig->data.func.return_types, count);
}

const LSMType *ts_eval_expr_type(TSLSPContext *ctx, TSNode node) {
    ...
    {
        const LSMType *memo = ts_memo_get(ctx->eval_memo, node.id);
        if (memo)
            return memo;
    }
    ...
    const LSMType *result = lsm_type_unknown();
    ...
    if (strcmp(kind, "identifier") == 0 || strcmp(kind, "shorthand_property_identifier") == 0) {
        ...
        if (ctx->enclosing_class_qn) {
            result = lsm_type_named(ctx->arena, ctx->enclosing_class_qn);
        }
    } else if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_class_qn) {
            const LSMRegisteredType *rt =
                lsm_registry_lookup_type(ctx->registry, ctx->enclosing_class_qn);
            if (rt && rt->embedded_types && rt->embedded_types[0]) {
                result = lsm_type_named(ctx->arena, rt->embedded_types[0]);
            }
        }
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(prop)) {
            const LSMType *recv = ts_eval_expr_type(ctx, obj);
        ...
        if (!ts_node_is_null(ctor)) {
            ...
            if (cname) {
                ...
                if (strchr(cname, '.') == NULL && ctx->module_qn) {
                    const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
                    result = lsm_type_named(ctx->arena, qn);
                } else {
                    result = lsm_type_named(ctx->arena, cname);
                }
            }
        }
        ...
        if (!ts_node_is_null(fn)) {
            ...
            if (fn_type && fn_type->kind == LSM_TYPE_FUNC) {
                ...
                if (result && result->kind == LSM_TYPE_TYPE_PARAM && result->data.type_param.name &&
                    ...
                    if (!ts_node_is_null(obj)) {
                        const LSMType *recv = ts_eval_expr_type(ctx, obj);
                        if (recv && !lsm_type_is_unknown(recv))
                            result = recv;
                    }
                }
                ...
                if (result && result->kind == LSM_TYPE_TYPE_PARAM &&
                    ...
                    if (!ts_node_is_null(args)) {
                        ...
                        for (uint32_t i = 0; i < pc && i < argc && ic < 7; i++) {
                            const LSMType *pt = fn_type->data.func.param_types[i];
                            if (!pt || pt->kind != LSM_TYPE_TYPE_PARAM)
                                continue;
                            TSNode arg = ts_node_named_child(args, i);
                            if (ts_node_is_null(arg))
                                continue;
                            const LSMType *at = ts_eval_expr_type(ctx, arg);
                            if (lsm_type_is_unknown(at))
                                continue;
                        ...
                        if (ic > 0) {
                            ...
                            inf_args[ic] = NULL;
                            const LSMType *sub =
                                lsm_type_substitute(ctx->arena, result, inf_names, inf_args);
                            if (sub && !lsm_type_is_unknown(sub))
                                result = sub;
                        }
                    }
                }
                ...
                if (ctx->debug) {
                    const char *res_qn = (result && result->kind == LSM_TYPE_NAMED)
        ...
        TSNode inner = ts_node_named_child(node, 0);
        const LSMType *t = ts_eval_expr_type(ctx, inner);
        // Promise<T> → T.
        if (t && t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_name &&
        ...
        names[count] = NULL;
        types[count] = NULL;
        result = lsm_type_object_lit(ctx->arena, names, types, NULL, NULL);
        ...
        const LSMType *a = ts_eval_expr_type(ctx, cons);
        const LSMType *b = ts_eval_expr_type(ctx, alt);
        if (lsm_type_is_unknown(a))
            result = b;
        else if (lsm_type_is_unknown(b))
            result = a;
        else if (a == b)
            result = a;
        else {
            const LSMType *members[3] = {a, b, NULL};
            result = lsm_type_union(ctx->arena, members, 2);
        }
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            const LSMType *l = ts_eval_expr_type(ctx, left);
            const LSMType *r = ts_eval_expr_type(ctx, right);
            if ((l && l->kind == LSM_TYPE_BUILTIN && l->data.builtin.name &&
                 strcmp(l->data.builtin.name, "string") == 0) ||
                (r && r->kind == LSM_TYPE_BUILTIN && r->data.builtin.name &&
...
static const LSMType *resolve_type_with_imports(TSLSPContext *ctx, const LSMType *t) {
    if (!t || !ctx)
        return t;
    ...
    if (t->kind == LSM_TYPE_NAMED && t->data.named.qualified_name) {
        ...
        if (!dot && ctx->module_qn) {
            const char *local_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
            const LSMRegisteredType *local_rt = lsm_registry_lookup_type(ctx->registry, local_qn);
            if (local_rt)
                return lsm_type_named(ctx->arena, local_qn);
        }
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            return lsm_type_named(ctx->arena, lsm_arena_sprintf(ctx->arena, "%s.%s", mqn, bare));
        }
        return t;
    }
    if (t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_args &&
    ...
    if (t->kind == LSM_TYPE_UNION || t->kind == LSM_TYPE_INTERSECTION) {
        if (!t->data.union_type.members || t->data.union_type.count == 0)
            return t;
        int mc = t->data.union_type.count;
        const LSMType **new_members = (const LSMType **)lsm_arena_alloc(
            ctx->arena, (size_t)(mc + 1) * sizeof(const LSMType *));
        if (!new_members)
            return t;
        for (int i = 0; i < mc; i++) {
        ...
        new_members[mc] = NULL;
        return t->kind == LSM_TYPE_UNION ? lsm_type_union(ctx->arena, new_members, mc)
                                         : lsm_type_intersection(ctx->arena, new_members, mc);
    }
    return t;
}

const LSMType *ts_parse_type_node(TSLSPContext *ctx, TSNode node) {
    if (!ctx || ts_node_is_null(node))
        return lsm_type_unknown();
    char *text = node_text(ctx, node);
    const LSMType *parsed = parse_ts_type_text(ctx->arena, text, ctx->module_qn);
...
static void bind_variable_declarator(TSLSPContext *ctx, TSNode decl) {
    ...
    const LSMType *inferred =
        ts_node_is_null(value) ? lsm_type_unknown() : ts_eval_expr_type(ctx, value);
    const LSMType *bound = (declared && !lsm_type_is_unknown(declared)) ? declared : inferred;

    if (node_kind_is(name, "identifier")) {
        char *nm = node_text(ctx, name);
        if (nm)
            lsm_scope_bind(ctx->current_scope, nm, bound);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(pk, "shorthand_property_identifier_pattern") == 0) {
                ...
                if (pnm)
                    lsm_scope_bind(ctx->current_scope, pnm, lookup_member_type(ctx, bound, pnm));
                ...
                if (!ts_node_is_null(key) && !ts_node_is_null(val) &&
                    ...
                    if (knm && vnm) {
                        lsm_scope_bind(ctx->current_scope, vnm,
...
static void bind_parameter(TSLSPContext *ctx, TSNode param) {
    ...
    const LSMType *t = lsm_type_unknown();
    if (!ts_node_is_null(tann)) {
            ...
            if (strcmp(tk, "object_type") == 0) {
                ...
                for (uint32_t oi = 0; oi < omc && pcount < 31; oi++) {
                    ...
                    if (strcmp(mk, "property_signature") == 0) {
                        ...
                        if (!pnm_text)
                            continue;
                        const LSMType *pt = lsm_type_unknown();
                ...
                p_names[pcount] = NULL;
                p_types[pcount] = NULL;
                t = lsm_type_object_lit(ctx->arena, p_names, p_types, NULL, NULL);
            } else {
    ...
    if (node_kind_is(pattern, "identifier")) {
        ...
        if (nm)
            lsm_scope_bind(ctx->current_scope, nm, t);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (node_kind_is(p, "shorthand_property_identifier_pattern")) {
                ...
                if (pnm)
                    lsm_scope_bind(ctx->current_scope, pnm, lookup_member_type(ctx, t, pnm));
                ...
                if (!ts_node_is_null(key) && !ts_node_is_null(val) &&
                    ...
                    char *vnm = node_text(ctx, val);
                    if (knm && vnm) {
                        lsm_scope_bind(ctx->current_scope, vnm, lookup_member_type(ctx, t, knm));
                    }
                }
            }
        }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (!pnm)
                continue;
            const LSMType *et = lsm_type_unknown();
            if (t && t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_args &&
...
void ts_process_statement(TSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "lexical_declaration") == 0 || strcmp(kind, "variable_declaration") == 0) {
        ...
        TSNode right = ts_node_child_by_field_name(node, "right", TS_LSP_FIELD_LEN("right"));
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            const LSMType *iter = ts_eval_expr_type(ctx, right);
            const LSMType *elem = lsm_type_unknown();
            if (iter && iter->kind == LSM_TYPE_TEMPLATE && iter->data.template_type.template_args &&
            ...
            if (node_kind_is(left, "lexical_declaration") ||
                ...
                if (node_kind_is(v, "variable_declarator")) {
                    ...
                    if (node_kind_is(nm, "identifier")) {
                        ...
                        if (s)
                            lsm_scope_bind(ctx->current_scope, s, elem);
                    }
                }
...
static void resolve_value_references_at(TSLSPContext *ctx, TSNode call_node) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (node_kind_is(arg, "member_expression")) {
            ...
            const LSMType *receiver = ts_eval_expr_type(ctx, object);
            const LSMType *base = receiver ? simplify_type(ctx, receiver) : NULL;
            base = base ? unwrap_passthrough_template(base) : NULL;
            if (!base || (base->kind != LSM_TYPE_NAMED && base->kind != LSM_TYPE_TEMPLATE)) {
                continue;
            }
            const LSMRegisteredFunc *target = lookup_method(ctx, receiver, method_name);
        ...
        const LSMRegisteredFunc *target = ts_exact_same_module_function(ctx, name);
...
static void resolve_call_at(TSLSPContext *ctx, TSNode call_node) {
    ...
    if (strcmp(fk, "member_expression") == 0) {
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(prop)) {
            ...
            if (mname) {
                    ...
                    if (on && !ts_scope_has_binding(ctx->current_scope, on)) {
                        ...
                        if (is_import) {
                            ts_emit_unresolved_call_at(
                                ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", on, mname),
                ...
                // Type-based dispatch.
                const LSMType *recv = ts_eval_expr_type(ctx, obj);
                const LSMRegisteredFunc *f = ts_lookup_method_for_call(ctx, recv, mname, args);
    ...
    if (strcmp(fk, "identifier") == 0) {
        ...
        if (ctx->module_qn) {
            const LSMRegisteredFunc *f = ts_lookup_symbol_for_call(ctx, ctx->module_qn, name, args);
            if (f) {
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            if (lname && mqn && strcmp(lname, name) == 0) {
                const LSMRegisteredFunc *f = ts_lookup_symbol_for_call(ctx, mqn, name, args);
                if (f) {
                ...
                const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", mqn, name);
...
static void process_callback_arrow(TSLSPContext *ctx, TSNode arrow, const LSMType *expected) {
    if (ts_node_is_null(arrow) || !expected || expected->kind != LSM_TYPE_FUNC)
        return;

    LSMScope *saved = ctx->current_scope;
    ctx->current_scope = lsm_scope_push(ctx->arena, saved);
    ...
    if (!ts_node_is_null(bare_param)) {
        ...
        if (strcmp(pk, "identifier") == 0) {
            ...
            const LSMType *pt =
                (expected->data.func.param_types && expected->data.func.param_types[0])
                    ...
                    : lsm_type_unknown();
            if (nm)
                lsm_scope_bind(ctx->current_scope, nm, pt);
        }
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (strcmp(pk, "identifier") == 0) {
                ...
                if (!ts_node_is_null(tann)) {
                    TSNode tch = (strcmp(ts_node_type(tann), "type_annotation") == 0)
                                     ...
                                     : tann;
                    if (!ts_node_is_null(tch)) {
                        const LSMType *annotated = ts_parse_type_node(ctx, tch);
                        if (!lsm_type_is_unknown(annotated)) {
            ...
            if (pname) {
                lsm_scope_bind(ctx->current_scope, pname,
                               expected_pt ? expected_pt : lsm_type_unknown());
            }
            idx++;
        }
    }
...
static bool extract_narrowing(TSLSPContext *ctx, TSNode condition, const char **out_var,
                              const LSMType **out_type, bool *out_inverted) {
    *out_var = NULL;
    *out_type = NULL;
    *out_inverted = false;
                ...
                if (opt2 && strcmp(opt2, "typeof") == 0 && !ts_node_is_null(arg) &&
                    ...
                    *out_var = node_text(ctx, arg);
                    ...
                    if (lit) {
                        ...
                        *out_type = lsm_type_builtin(ctx->arena, lit);
                    }
...
static const LSMType *narrow_discriminated_union(TSLSPContext *ctx, TSNode condition,
                                                 const char **out_var) {
    *out_var = NULL;
    if (ts_node_is_null(condition))
        return NULL;
    ...
    const LSMType *var_type = lsm_scope_lookup(ctx->current_scope, var_name);
    if (!var_type || var_type->kind != LSM_TYPE_UNION)
        return NULL;
    if (!var_type->data.union_type.members)
        return NULL;

    for (int i = 0; i < var_type->data.union_type.count; i++) {
        const LSMType *m = var_type->data.union_type.members[i];
        if (!m || m->kind != LSM_TYPE_NAMED)
            continue;
        const LSMRegisteredType *rt =
            lsm_registry_lookup_type(ctx->registry, m->data.named.qualified_name);
        if (!rt || !rt->field_names || !rt->field_types)
            continue;
        for (int j = 0; rt->field_names[j]; j++) {
            if (strcmp(rt->field_names[j], prop_name) != 0)
                continue;
            const LSMType *ft = rt->field_types[j];
            if (!ft)
                continue;
            ...
            if (ft->kind == LSM_TYPE_TS_LITERAL && ft->data.literal_ts.value &&
            ...
            if (ft->kind == LSM_TYPE_NAMED && ft->data.named.qualified_name) {
...
static void resolve_jsx_element(TSLSPContext *ctx, TSNode element_node) {
    ...
    if (ctx->module_qn) {
        const LSMRegisteredFunc *f =
            lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, tag_name);
        if (f) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (lname && mqn && strcmp(lname, tag_name) == 0) {
             ...
             * available. */
            const LSMRegisteredFunc *f = lsm_registry_lookup_symbol(ctx->registry, mqn, tag_name);
            if (f) {
        ...
        if (!ts_node_is_null(name)) {
            ...
            if (function_name) {
                const char *function_qn =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, function_name);
...
static void process_node(TSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        do {
            ...
            if (ts_node_is_null(args))
                break;
            const LSMType *fn_type = ts_signature_for_call(ctx, fn_node, args);
            if (!fn_type || fn_type->kind != LSM_TYPE_FUNC)
                break;
            ...
            // LSMType* → crash on expected->kind.
        ...
        if (!ts_node_is_null(value)) {
            ...
            if (!ts_node_is_null(v) && strcmp(ts_node_type(v), "member_expression") == 0) {
                ...
                if (!ts_node_is_null(obj) && !ts_node_is_null(prop) &&
                    ...
                    if (disc_var) {
                        const LSMType *t = lsm_scope_lookup(ctx->current_scope, disc_var);
                        if (t && t->kind == LSM_TYPE_UNION)
                            var_union = t;
                    }
                }
            }
        ...
        if (!ts_node_is_null(body)) {
            ...
            for (uint32_t i = 0; i < bnc; i++) {
                    ...
                    if (!ts_node_is_null(case_value) &&
                        ...
                        if (lit) {
                            ...
                            for (int mi = 0; mi < var_union->data.union_type.count; mi++) {
                                const LSMType *m = var_union->data.union_type.members[mi];
                                if (!m || m->kind != LSM_TYPE_NAMED)
                                    continue;
                                const LSMRegisteredType *rt = lsm_registry_lookup_type(
                                ...
                                for (int fj = 0; rt->field_names[fj]; fj++) {
                                    if (strcmp(rt->field_names[fj], disc_prop) != 0)
                                        continue;
                                    const LSMType *ft = rt->field_types[fj];
                                    if (!ft)
                                        continue;
                                    if (ft->kind == LSM_TYPE_TS_LITERAL &&
                                        ft->data.literal_ts.value &&
                                        strcmp(ft->data.literal_ts.value, lit) == 0) {
                                        narrowed = m;
                                        break;
                                    }
                                    if (ft->kind == LSM_TYPE_NAMED &&
                ...
                if (narrowed && disc_var) {
                    ctx->current_scope = lsm_scope_push(ctx->arena, saved);
                    lsm_scope_bind(ctx->current_scope, disc_var, narrowed);
                }
    ...
    if (strcmp(kind, "if_statement") == 0) {
        ...
        if (!ok) {
            const char *dv = NULL;
            const LSMType *dt = narrow_discriminated_union(ctx, condition, &dv);
            if (dv && dt) {
                nv = dv;
                nt = dt;
                inverted = false;
        ...
        if (!ts_node_is_null(consequence)) {
            if (ok && nv && nt && !inverted) {
                LSMScope *saved = ctx->current_scope;
                ctx->current_scope = lsm_scope_push(ctx->arena, saved);
                lsm_scope_bind(ctx->current_scope, nv, nt);
    ...
    if (strcmp(kind, "internal_module") == 0) {
        ...
        if (!ts_node_is_null(name) && saved_module_qn) {
            ...
            if (namespace_name && namespace_name[0]) {
                ctx->module_qn =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", saved_module_qn, namespace_name);
            }
        }
        TSNode body = ts_node_child_by_field_name(node, "body", TS_LSP_FIELD_LEN("body"));
        if (!ts_node_is_null(body)) {
    ...
    if (strcmp(kind, "class_declaration") == 0) {
        ...
        if (!ts_node_is_null(name) && ctx->module_qn) {
            ...
            if (cn)
                class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cn);
        }
        TSNode body = ts_node_child_by_field_name(node, "body", TS_LSP_FIELD_LEN("body"));
        if (!ts_node_is_null(body) && class_qn) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                const char *mqn = strcmp(mn, "constructor") == 0
                                      ? lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mn)
                                      : lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mn);
...
static void process_function_body(TSLSPContext *ctx, TSNode func_node, const char *func_qn,
    ...
    ctx->current_scope = lsm_scope_push(ctx->arena, saved_scope);
    ...
    if (class_qn) {
        ...
        lsm_scope_bind(ctx->current_scope, "this", lsm_type_named(ctx->arena, class_qn));
    }
    ...
    if (!ts_node_is_null(params)) {
        ...
        if (func_qn) {
            const LSMRegisteredFunc *rf = lsm_registry_lookup_func(ctx->registry, func_qn);
            if (rf && rf->signature && rf->signature->kind == LSM_TYPE_FUNC &&
                ...
                for (int i = 0; rf->signature->data.func.param_names[i] &&
                    ...
                    const LSMType *ptype = rf->signature->data.func.param_types[i];
                    if (lsm_type_is_unknown(ptype))
                        continue;
                    const LSMType *existing = lsm_scope_lookup(ctx->current_scope, pname);
                    if (existing && !lsm_type_is_unknown(existing))
                        continue;
                    lsm_scope_bind(ctx->current_scope, pname, ptype);
                }
            }
        }
    }
...
void ts_lsp_process_file(TSLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (strcmp(kind, "function_declaration") == 0) {
            ...
            if (!fn)
                continue;
            const char *fqn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fn);
...
void ts_lsp_init(TSLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
                 const LSMTypeRegistry *registry, const char *module_qn, bool js_mode,
                 bool jsx_mode, bool dts_mode, LSMResolvedCallArray *out) {
    if (!ctx)
        return;
    ...
    ctx->current_scope = arena ? lsm_scope_push(arena, NULL) : NULL;
...
void ts_lsp_add_import(TSLSPContext *ctx, const char *local_name, const char *module_qn) {
    ...
    const char **names =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_count * sizeof(const char *));
    const char **qns =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_count * sizeof(const char *));
    if (!names || !qns)
        return;
    ...
    names[ctx->import_count] = lsm_arena_strdup(ctx->arena, local_name);
    qns[ctx->import_count] = lsm_arena_strdup(ctx->arena, module_qn);
...
static const LSMType *tt_builtin(LSMArena *a, const char *n) {
    return lsm_type_builtin(a, n);
}
static const LSMType *tt_param(LSMArena *a, const char *n) {
    return lsm_type_type_param(a, n);
}

static void ts_reg_method(LSMTypeRegistry *reg, LSMArena *arena, const char *recv_qn,
                          const char *name, const LSMType *ret_type) {
    LSMRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.qualified_name = lsm_arena_sprintf(arena, "%s.%s", recv_qn, name);
    rf.short_name = lsm_arena_strdup(arena, name);
    rf.receiver_type = lsm_arena_strdup(arena, recv_qn);
    rf.min_params = -1;
    const LSMType *rets[2] = {ret_type ? ret_type : lsm_type_unknown(), NULL};
    rf.signature = lsm_type_func(arena, NULL, NULL, rets);
    lsm_registry_add_func(reg, rf);
}

static void reg_type_with_methods(LSMTypeRegistry *reg, LSMArena *arena, const char *qn,
                                  const char *short_name, const char **type_params,
                                  const char **method_names, const LSMType **method_returns) {
    LSMRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = lsm_arena_strdup(arena, qn);
    rt.short_name = lsm_arena_strdup(arena, short_name);
    ...
    if (method_names) {
        ...
        while (method_names[count])
            count++;
        const char **mqns =
            (const char **)lsm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const char *));
        const char **mnames_copy =
            (const char **)lsm_arena_alloc(arena, (size_t)(count + 1) * sizeof(const char *));
        if (mqns && mnames_copy) {
            for (int i = 0; i < count; i++) {
                mnames_copy[i] = lsm_arena_strdup(arena, method_names[i]);
                mqns[i] = lsm_arena_sprintf(arena, "%s.%s", qn, method_names[i]);
            }
...
void lsm_ts_stdlib_register(LSMTypeRegistry *reg, LSMArena *arena) {
    if (!reg || !arena)
        return;

    const LSMType *t_string = tt_builtin(arena, "string");
    const LSMType *t_number = tt_builtin(arena, "number");
    const LSMType *t_boolean = tt_builtin(arena, "boolean");
    const LSMType *t_void = tt_builtin(arena, "void");
    const LSMType *t_unknown = lsm_type_unknown();

    // Array<T>: minimal method surface for v1.
    {
        const LSMType *tparam_T = tt_param(arena, "T");
        const LSMType *tparam_U = tt_param(arena, "U");
        const char **tparams = (const char **)lsm_arena_alloc(arena, 2 * sizeof(const char *));
        tparams[0] = "T";
        tparams[1] = NULL;
        ...
        const LSMType *returns[24];
        const LSMType *t_T = tparam_T;
        const LSMType *t_arr_T_args[2] = {t_T, NULL};
        const LSMType *t_arr_T = lsm_type_template(arena, "Array", t_arr_T_args, 1);
        ...
        returns[11] =
            lsm_type_template(arena, "Array", (const LSMType *[]){tparam_U, NULL}, 1); // map<U>
        ...
        struct {
            const char *method;
            const LSMType *ret;
            const LSMType *cb_param0;
            const LSMType *cb_ret;
        } cb_specs[] = {
        ...
        for (int ci = 0; cb_specs[ci].method; ci++) {
            ...
            const LSMType *cb_param_types[2] = {cb_specs[ci].cb_param0, NULL};
            const LSMType *cb_returns[2] = {cb_specs[ci].cb_ret, NULL};
            const char *cb_param_names[2] = {"x", NULL};
            const LSMType *cb_func =
                lsm_type_func(arena, cb_param_names, cb_param_types, cb_returns);

            // Rebuild Array.<method>'s signature with this callback as param 0.
            const LSMType *outer_param_types[2] = {cb_func, NULL};
            const LSMType *outer_returns[2] = {cb_specs[ci].ret, NULL};
            const LSMType *new_sig =
                lsm_type_func(arena, p_names, outer_param_types, outer_returns);

            // Find Array.<method> in the registry and patch its signature.
            char *qn = lsm_arena_sprintf(arena, "Array.%s", cb_specs[ci].method);
    ...
    // Promise<T>
    {
        ...
        const LSMType *returns[4];
        const LSMType *t_promise_T_args[2] = {tparam_T, NULL};
        const LSMType *t_promise_T = lsm_type_template(arena, "Promise", t_promise_T_args, 1);
    ...
    // Map<K,V>
    {
        const char **tparams = (const char **)lsm_arena_alloc(arena, 3 * sizeof(const char *));
        tparams[0] = "K";
        tparams[1] = "V";
        tparams[2] = NULL;
        static const char *methods[] = {"get", "set", "has", "delete", "clear", "forEach", NULL};
        const LSMType *returns[7] = {
            tt_param(arena, "V"),
            lsm_type_template(arena, "Map",
                              (const LSMType *[]){tt_param(arena, "K"), tt_param(arena, "V"), NULL},
                              2),
            t_boolean,
            t_boolean,
            t_void,
    ...
    // Set<T>
    {
        ...
        const LSMType *returns[6] = {
            lsm_type_template(arena, "Set", (const LSMType *[]){tt_param(arena, "T"), NULL}, 1),
            t_boolean,
            t_boolean,
            t_void,
            t_void,
    ...
    // Iterator<T> / Iterable<T>
    {
        ...
        const LSMType *iter_returns[4] = {tparam_T, tparam_T, tparam_T, NULL};
        reg_type_with_methods(reg, arena, "Iterator", "Iterator", tparams, iter_methods,
                              iter_returns);

        const LSMType *iter_t =
            lsm_type_template(arena, "Iterator", (const LSMType *[]){tparam_T, NULL}, 1);
        static const char *iterable_methods[] = {NULL}; // [Symbol.iterator] not modeled by name
        const LSMType *iterable_returns[1] = {NULL};
        (void)iter_t;
    ...
    // AsyncIterator<T> / AsyncIterable<T> — methods return Promise<T>
    {
        ...
        tparams[0] = "T";
        tparams[1] = NULL;
        const LSMType *prom_T =
            lsm_type_template(arena, "Promise", (const LSMType *[]){tparam_T, NULL}, 1);
        static const char *methods[] = {"next", "return", "throw", NULL};
        const LSMType *returns[4] = {prom_T, prom_T, prom_T, NULL};
    ...
    // Generator<T> — extends Iterator<T> with yield-based methods.
    {
        ...
        tparams[1] = NULL;
        static const char *methods[] = {"next", "return", "throw", NULL};
        const LSMType *returns[4] = {tparam_T, tparam_T, tparam_T, NULL};
    ...
    // String prototype methods (most common).
    {
        ...
        while (methods[mc])
            mc++;
        const LSMType **returns =
            (const LSMType **)lsm_arena_alloc(arena, (size_t)(mc + 1) * sizeof(const LSMType *));
        for (int i = 0; i < mc; i++) {
            ...
            if (strcmp(m, "includes") == 0 || strcmp(m, "startsWith") == 0 ||
                ...
                returns[i] =
                    lsm_type_template(arena, "Array", (const LSMType *[]){t_string, NULL}, 1);
            } else {
                returns[i] = t_string;
            }
        }
    ...
    // console — used so frequently that resolving `console.log` directly is high value.
    {
        ...
        const LSMType *returns[6] = {t_void, t_void, t_void, t_void, t_void, NULL};
    ...
    // JSON namespace.
    {
        ...
        const LSMType *returns[3] = {t_unknown, t_string, NULL};
    ...
    // Object — constructor namespace methods that are commonly chained.
    {
        ...
        const LSMType *returns[9] = {
            lsm_type_template(arena, "Array", (const LSMType *[]){t_string, NULL}, 1),
            lsm_type_template(arena, "Array", (const LSMType *[]){t_unknown, NULL}, 1),
            lsm_type_template(arena, "Array", (const LSMType *[]){t_unknown, NULL}, 1),
            t_unknown,
            t_unknown,
            t_boolean,
    ...
    // Number wrapper class (so `(42).toString()` and similar resolve).
    {
        ...
        const LSMType *returns[6] = {t_string, t_string, t_string, t_string, t_number, NULL};
    ...
    // Boolean wrapper class.
    {
        ...
        const LSMType *returns[3] = {t_string, t_boolean, NULL};
    ...
    // RegExp wrapper class.
    {
        ...
        const LSMType *returns[5] = {t_boolean, t_unknown, t_string, t_unknown, NULL};
    ...
    // Error class — used heavily in `throw new Error(...)` and catch handlers.
    {
        ...
        const LSMType *returns[2] = {t_string, NULL};
    ...
    // Array.of(...items): Array<T>
    {
        ...
        const LSMType *t_T = tparam_T;
        const LSMType *prom_T =
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_T, NULL}, 1);
        const LSMType *arr_T = lsm_type_template(arena, "Array", (const LSMType *[]){t_T, NULL}, 1);
        const LSMType *prom_arr_T =
            lsm_type_template(arena, "Promise", (const LSMType *[]){arr_T, NULL}, 1);
    ...
    // be Phase 5 (generator).
    {
        ...
        const LSMType *et_returns[4] = {t_void, t_void, t_boolean, NULL};
        ...
        const LSMType *node_t = lsm_type_named(arena, "Node");
        const LSMType *node_returns[10] = {
        ...
        const LSMType *el_t = lsm_type_named(arena, "Element");
        const LSMType *nodelist_t =
            lsm_type_template(arena, "NodeList", (const LSMType *[]){el_t, NULL}, 1);
        const LSMType *el_returns[21] = {
        ...
        const LSMType *html_t = lsm_type_named(arena, "HTMLElement");
        const LSMType *html_returns[12] = {
            t_string, t_void,
            html_t,   lsm_type_template(arena, "NodeList", (const LSMType *[]){html_t, NULL}, 1),
            html_t,   t_void,
            t_void,   t_void,
            t_void,   t_void,
            t_void,   NULL,
        ...
        const LSMType *doc_returns[11] = {
            html_t,
            html_t,
            lsm_type_template(arena, "NodeList", (const LSMType *[]){html_t, NULL}, 1),
            lsm_type_template(arena, "HTMLCollection", (const LSMType *[]){html_t, NULL}, 1),
            lsm_type_template(arena, "HTMLCollection", (const LSMType *[]){html_t, NULL}, 1),
            html_t,
            node_t,
            node_t,
        ...
        const LSMType *event_returns[5] = {
            ...
            lsm_type_template(
                arena, "Array",
                (const LSMType *[]){
                    et_methods[0] ? lsm_type_named(arena, "EventTarget") : t_unknown, NULL},
                1),
            NULL,
        };
        ...
        const LSMType *resp_returns[7] = {
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_unknown, NULL}, 1),
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_string, NULL}, 1),
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_unknown, NULL}, 1),
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_unknown, NULL}, 1),
            lsm_type_template(arena, "Promise", (const LSMType *[]){t_unknown, NULL}, 1),
            lsm_type_named(arena, "Response"),
            NULL,
        };
        ...
        const LSMType *win_returns[13] = {
            t_void,
            t_void,
            t_void,
            t_boolean,
            t_string,
            lsm_type_template(arena, "Promise",
                              (const LSMType *[]){lsm_type_named(arena, "Response"), NULL}, 1),
            t_number,
            t_void,
            t_number,
            t_void,
...
static void register_file_defs(LSMArena *arena, LSMTypeRegistry *reg, LSMFileResult *result,
                               const char *module_qn) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0) {
            ...
            if (d->base_classes) {
                ...
                while (d->base_classes[bc])
                    bc++;
                if (bc > 0) {
                    const char **emb = (const char **)lsm_arena_alloc(
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (b) {
                                ...
                                char *base_name = lsm_arena_strndup(arena, b, bl);
                                if (strchr(base_name, '.') == NULL) {
                                    emb[j] =
                                        lsm_arena_sprintf(arena, "%s.%s", module_qn, base_name);
                                } else {
                                    emb[j] = base_name;
                                }
                            } else {
            ...
            const LSMType **rets = NULL;
            if (d->return_type && d->return_type[0]) {
                rets = (const LSMType **)lsm_arena_alloc(arena, 2 * sizeof(const LSMType *));
                if (rets) {
                ...
                while (d->return_types[n])
                    n++;
                if (n > 0) {
                    rets = (const LSMType **)lsm_arena_alloc(arena, (size_t)(n + 1) *
                                                                        sizeof(const LSMType *));
                    if (rets) {
                        for (int j = 0; j < n; j++) {
            ...
            const LSMType **params_t = NULL;
            const char **param_names = NULL;
            if (has_ordered_params) {
                LSMTSLSPSignatureParserContext param_parser_ctx = {module_qn};
                params_t = lsm_type_materialize_signature_params(
                    arena, d->signature_param_types, d->signature_param_count,
                    lsm_ts_lsp_parse_signature_param_text, &param_parser_ctx);
            } else {
                params_t = parse_param_types_array(arena, d->param_types, module_qn);
                param_names = d->param_names;
            }
            rf.signature = lsm_type_func(arena, param_names, params_t, rets);
            ...
            if (strcmp(d->label, "Method") == 0) {
                ...
                if (dot && dot != d->qualified_name) {
                    char *recv = lsm_arena_strndup(arena, d->qualified_name,
                                                   (size_t)(dot - d->qualified_name));
                    rf.receiver_type = recv;

                    // Ensure the receiver type is registered (auto-create if missing).
                    const LSMRegisteredType *existing = lsm_registry_lookup_type(reg, recv);
                    if (!existing) {
                        LSMRegisteredType auto_rt;
                        ...
                        lsm_registry_add_type(reg, auto_rt);
                    }
                }
            }
    ...
    for (int ti = 0; ti < reg->type_count; ti++) {
        LSMRegisteredType *rt = &reg->types[ti];
        ...
        int mcount = 0;
        for (int fi = 0; fi < reg->func_count; fi++) {
            const LSMRegisteredFunc *f = &reg->funcs[fi];
        ...
        const char **mnames =
            (const char **)lsm_arena_alloc(arena, (size_t)(mcount + 1) * sizeof(const char *));
        const char **mqns =
            (const char **)lsm_arena_alloc(arena, (size_t)(mcount + 1) * sizeof(const char *));
        if (!mnames || !mqns)
            continue;
...
static const LSMType *infer_return_type_from_body(TSLSPContext *ctx, TSNode body) {
    if (ts_node_is_null(body))
        return lsm_type_unknown();
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "return_statement") == 0) {
            ...
            if (ts_node_is_null(expr))
                continue;
            const LSMType *t = ts_eval_expr_type(ctx, expr);
            if (t && !lsm_type_is_unknown(t))
                return t;
            continue;
        }
...
// Rebuild a function's LSMRegisteredFunc signature from the AST directly. extract_defs
...
// in place.
static void rebuild_signatures_from_ast(TSLSPContext *ctx, TSNode root, LSMTypeRegistry *reg) {
    ...
    while (top > 0) {
        ...
        const char *fqn = NULL;
        if (is_func) {
            fqn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        } else {
            ...
            if (!cname)
                continue;
            fqn = lsm_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->module_qn, cname, fname);
        }
        ...
        const LSMType *ret = lsm_type_unknown();
        TSNode rt_node =
        ...
        const LSMType *rets[2] = {ret, NULL};
        const LSMType *new_sig = lsm_type_func(ctx->arena, p_names, p_types, rets);
        for (int fi = 0; fi < reg->func_count; fi++) {
            if (!reg->funcs[fi].qualified_name)
                continue;
            ...
            if (lsm_type_is_unknown(ret) && reg->funcs[fi].signature &&
                reg->funcs[fi].signature->kind == LSM_TYPE_FUNC &&
                reg->funcs[fi].signature->data.func.return_types &&
                reg->funcs[fi].signature->data.func.return_types[0] &&
                !lsm_type_is_unknown(reg->funcs[fi].signature->data.func.return_types[0])) {
                const LSMType *preserved[2] = {reg->funcs[fi].signature->data.func.return_types[0],
                                               NULL};
                new_sig = lsm_type_func(ctx->arena, p_names, p_types, preserved);
            }
            reg->funcs[fi].signature = new_sig;
            break;
        }
...
static void convert_signature_type_params(TSLSPContext *ctx, TSNode root, LSMTypeRegistry *reg) {
    if (ts_node_is_null(root) || !reg || !ctx->module_qn)
        return;
    ...
    while (top > 0) {
        ...
        for (uint32_t i = 0; i < tnc && tpc < 15; i++) {
            ...
            if (!nm)
                continue;
            names[tpc] = nm;
            args[tpc] = lsm_type_type_param(ctx->arena, nm);
            tpc++;
        }
        names[tpc] = NULL;
        args[tpc] = NULL;
        ...
        for (int fi = 0; fi < reg->func_count; fi++) {
            ...
            if (!old_sig)
                break;
            const LSMType *new_sig = lsm_type_substitute(ctx->arena, old_sig, names, args);
            if (new_sig)
                reg->funcs[fi].signature = new_sig;
            // Also store the type_param_names for later overload-by-types use.
            const char **stored = (const char **)lsm_arena_alloc(
...
// match, returns the type as a parsed LSMType. Used in `js_mode` to infer signatures
...
static void parse_jsdoc_block(TSLSPContext *ctx, const char *text, size_t len,
    ...
    *out_param_count = 0;
    ...
    while (p < end) {
        ...
        char *type_text = lsm_arena_strndup(ctx->arena, type_start, type_len);
        const LSMType *parsed = parse_ts_type_text(ctx->arena, type_text, ctx->module_qn);
        p++; // skip }
        ...
        if (*out_param_count < max_params) {
            char *nm = lsm_arena_strndup(ctx->arena, name_start, name_len);
...
static void apply_jsdoc_signatures(TSLSPContext *ctx, TSNode root, LSMTypeRegistry *reg) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (!fname)
            continue;
        const char *fqn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        ...
        for (int fi = 0; fi < reg->func_count; fi++) {
            ...
            const LSMType *old_sig = reg->funcs[fi].signature;
            const LSMType **rets = NULL;
            if (ret && !lsm_type_is_unknown(ret)) {
                rets = (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(const LSMType *));
                if (rets) {
                    rets[0] = ret;
                    rets[1] = NULL;
                }
            } else if (old_sig && old_sig->kind == LSM_TYPE_FUNC) {
                rets = old_sig->data.func.return_types;
            }
            const LSMType **params_arr = NULL;
            const char **param_names_arr = NULL;
            if (pcount > 0) {
                params_arr = (const LSMType **)lsm_arena_alloc(
                    ctx->arena, (size_t)(pcount + 1) * sizeof(const LSMType *));
                param_names_arr = (const char **)lsm_arena_alloc(
            ...
            reg->funcs[fi].signature = lsm_type_func(ctx->arena, param_names_arr, params_arr, rets);
            break;
        }
    }
}
...
static void infer_implicit_returns(TSLSPContext *ctx, TSNode root, LSMTypeRegistry *reg) {
    if (ts_node_is_null(root) || !reg || !ctx->module_qn)
        return;
    uint32_t kn = 0;
    TSNode *kids = collect_children(ctx->arena, root, &kn);
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (!fname)
            continue;
        const char *fqn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
        const LSMRegisteredFunc *existing = lsm_registry_lookup_func(reg, fqn);
        if (!existing)
            continue;
        // Only infer when the existing signature lacks an explicit return type.
        if (existing->signature && existing->signature->kind == LSM_TYPE_FUNC &&
            existing->signature->data.func.return_types &&
            existing->signature->data.func.return_types[0] &&
            !lsm_type_is_unknown(existing->signature->data.func.return_types[0])) {
            continue;
        }

        const LSMType *inferred = infer_return_type_from_body(ctx, body);
        if (lsm_type_is_unknown(inferred))
            continue;

        // Build a new FUNC signature carrying over params from the existing one (if any).
        const char **param_names = NULL;
        const LSMType **param_types = NULL;
        if (existing->signature && existing->signature->kind == LSM_TYPE_FUNC) {
            param_names = existing->signature->data.func.param_names;
            param_types = existing->signature->data.func.param_types;
        }
        const LSMType *rets[2] = {inferred, NULL};
        const LSMType *new_sig = lsm_type_func(ctx->arena, param_names, param_types, rets);
 ...
 * cannot consume the LSMLSPDef ordered carrier.  Build their FUNC type from the
 * AST without matching by QN (overloads intentionally share a QN). */
static const LSMType *interface_method_signature_from_ast(TSLSPContext *ctx, TSNode method,
                                                          int *out_min_params) {
    const LSMType *param_types[32] = {0};
    int param_count = 0;
    int min_params = 0;
    TSNode params =
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < count && param_count < 31; i++) {
            ...
            const LSMType *param_type = lsm_type_unknown();
            TSNode annotation =
    ...
    const LSMType *return_type = lsm_type_unknown();
    TSNode return_node =
    ...
    const LSMType *return_types[2] = {return_type, NULL};
    if (out_min_params)
        *out_min_params = min_params;
    return lsm_type_func(ctx->arena, NULL, param_count ? param_types : NULL, return_types);
}

// AST sweep: walk class/interface bodies to collect field names+types and refine
// embedded_types / type_param_names.
static void ast_sweep_shapes(TSLSPContext *ctx, TSNode root, LSMTypeRegistry *reg) {
    ...
    for (uint32_t i = 0; i < kn; i++) {
        ...
        if (strcmp(k, "type_alias_declaration") == 0) {
            ...
            if (!nm)
                continue;
            const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, nm);
            ...
            if (val_text && val_text[0] && strchr(val_text, ' ') == NULL &&
                ...
                if (strchr(val_text, '.') == NULL && val_text[0] >= 'A' && val_text[0] <= 'Z') {
                    alias_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, val_text);
                } else {
                    alias_qn = lsm_arena_strdup(ctx->arena, val_text);
                }
            }
            // Find or insert.
            bool found = false;
        ...
        if (!cname)
            continue;
        const char *class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);

        // Find the registered type and augment with fields.
        LSMRegisteredType *rt = NULL;
        for (int ti = 0; ti < reg->type_count; ti++) {
            if (reg->types[ti].qualified_name &&
        ...
        const LSMType *field_types[64] = {0};
        int field_count = 0;
        ...
        const LSMType *iface_method_sigs[64] = {0};
        ...
        for (uint32_t bi = 0; bi < bnc && field_count < 63; bi++) {
            ...
            if (strcmp(mk, "public_field_definition") == 0 ||
                ...
                if (!fnm)
                    continue;
                const LSMType *ft = lsm_type_unknown();
            ...
            if (is_interface && (strcmp(mk, "method_signature") == 0)) {
                ...
                int min_params = -1;
                const LSMType *sig = interface_method_signature_from_ast(ctx, m, &min_params);

                const char *mqn = lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mnm);
                iface_method_names[iface_method_count] = mnm;
                iface_method_qns[iface_method_count] = mqn;
                iface_method_sigs[iface_method_count] = sig;
                iface_method_count++;

                // Also register a LSMRegisteredFunc so lsm_registry_lookup_method picks it up.
                LSMRegisteredFunc rf;
                memset(&rf, 0, sizeof(rf));
        ...
        if (field_count > 0) {
            const char **fn_arr = (const char **)lsm_arena_alloc(
                ctx->arena, (size_t)(field_count + 1) * sizeof(const char *));
            const LSMType **ft_arr = (const LSMType **)lsm_arena_alloc(
                ctx->arena, (size_t)(field_count + 1) * sizeof(const LSMType *));
            if (fn_arr && ft_arr) {
        ...
        if (iface_method_count > 0) {
            const char **mn_arr = (const char **)lsm_arena_alloc(
                ctx->arena, (size_t)(iface_method_count + 1) * sizeof(const char *));
            const char **mq_arr = (const char **)lsm_arena_alloc(
                ctx->arena, (size_t)(iface_method_count + 1) * sizeof(const char *));
            if (mn_arr && mq_arr) {
            ...
            (void)iface_method_sigs; // sigs already attached via lsm_registry_add_func
        }
    }
}

void lsm_run_ts_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
    ...
    // Diagnostic / benchmarking knob: setting `LSM_LSP_DISABLED=1` skips the resolver.
    ...
    const char *disabled = getenv("LSM_LSP_DISABLED");
    if (disabled && disabled[0] && disabled[0] != '0')
        return;

    LSMTypeRegistry reg;
    lsm_registry_init(&reg, arena);
    lsm_ts_stdlib_register(&reg, arena);
...
/* Register a batch of LSMLSPDef[] into a registry. Mirrors the inline
 * loop in lsm_run_ts_lsp_cross — KEEP IN SYNC. Shared by the Tier 2
 * pre-built registry builder + per-file overlay builder. Def-driven
 * (return/field/embedded/method info from def strings). */
static void ts_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, LSMLSPDef *defs,
                                 int def_count) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0) {
            ...
            if (d->embedded_types && d->embedded_types[0]) {
                ...
                for (const char *s = d->embedded_types; *s; s++)
                    if (*s == '|')
                        n++;
                const char **arr =
                    (const char **)lsm_arena_alloc(arena, (size_t)(n + 1) * sizeof(const char *));
                if (arr) {
                    int idx = 0;
                    const char *start = d->embedded_types;
                    for (const char *s = d->embedded_types;; s++) {
                        if (*s == '|' || *s == '\0') {
                            arr[idx++] = lsm_arena_strndup(arena, start, (size_t)(s - start));
                            if (*s == '\0')
                                break;
                            start = s + 1;
                        }
            ...
            if (d->field_defs && d->field_defs[0]) {
                ...
                const char **fn_arr =
                    (const char **)lsm_arena_alloc(arena, (size_t)(n + 1) * sizeof(const char *));
                const LSMType **ft_arr = (const LSMType **)lsm_arena_alloc(
                    arena, (size_t)(n + 1) * sizeof(const LSMType *));
                if (fn_arr && ft_arr) {
                    int idx = 0;
                    const char *start = d->field_defs;
                    for (const char *s = d->field_defs;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *pair = lsm_arena_strndup(arena, start, (size_t)(s - start));
            ...
            if (d->method_names_str && d->method_names_str[0]) {
                ...
                if (mn_arr && mqn_arr) {
                    ...
                    for (const char *s = d->method_names_str;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *m = lsm_arena_strndup(arena, start, (size_t)(s - start));
                            mn_arr[idx] = m;
                            mqn_arr[idx] = lsm_arena_sprintf(arena, "%s.%s", d->qualified_name, m);
                            idx++;
                            if (*s == '\0')
                                break;
                            start = s + 1;
                    ...
                    mqn_arr[idx] = NULL;
                    if (reg->type_count > 0) {
                        LSMRegisteredType *rt_just = &reg->types[reg->type_count - 1];
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                for (const char *s = d->return_types; *s; s++)
                    if (*s == '|')
                        n++;
                rets = (const LSMType **)lsm_arena_alloc(arena,
                                                         (size_t)(n + 1) * sizeof(const LSMType *));
                if (rets) {
                    int idx = 0;
                    const char *start = d->return_types;
                    for (const char *s = d->return_types;; s++) {
                        if (*s == '|' || *s == '\0') {
                            char *part = lsm_arena_strndup(arena, start, (size_t)(s - start));
            ...
            LSMTSLSPSignatureParserContext param_parser_ctx = {d->def_module_qn};
            const LSMType **params = lsm_type_materialize_signature_params(
                arena, d->signature_param_types, d->signature_param_count,
                lsm_ts_lsp_parse_signature_param_text, &param_parser_ctx);
            rf.signature = lsm_type_func(arena, NULL, params, rets);
 ...
 * Per-file overlays (built by lsm_run_ts_lsp_cross_with_registry)
 * chain to this via the registry fallback pointer. */
LSMTypeRegistry *lsm_ts_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    ...
    for (int i = 0; i < def_count; i++) {
        LSMLSPDef *d = &defs[i];
        if (d->lang != LSM_LANG_JAVASCRIPT && d->lang != LSM_LANG_TYPESCRIPT &&
            d->lang != LSM_LANG_TSX) {
            continue;
        }
...
void lsm_run_ts_lsp_cross_with_registry(LSMArena *arena, const char *source, int source_len,
                                        const char *module_qn, bool js_mode, bool jsx_mode,
                                        bool dts_mode, LSMTypeRegistry *reg, LSMLSPDef *defs,
    ...
    LSMTypeRegistry overlay;
    lsm_registry_init(&overlay, arena);
...
void lsm_run_ts_lsp_cross(LSMArena *arena, const char *source, int source_len,
                          const char *module_qn, bool js_mode, bool jsx_mode, bool dts_mode,
                          LSMLSPDef *defs, int def_count, const char **import_names,
    ...
    // Build a faux LSMFileResult so register_file_defs's downstream paths work; we don't
    // need it here because cross-file defs already came in as LSMLSPDef.
...
void lsm_batch_ts_lsp_cross(LSMArena *arena, LSMBatchTSLSPFile *files, int file_count,
                            LSMResolvedCallArray *out) {
    if (!arena || !files || !out || file_count <= 0)
        return;
    // Project-scope merging happens implicitly: every file's LSMLSPDef set already
    ...
    for (int i = 0; i < file_count; i++) {
        LSMBatchTSLSPFile *f = &files[i];
        lsm_run_ts_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->js_mode, f->jsx_mode,
