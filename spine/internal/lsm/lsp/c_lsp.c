...
/* Safe kind accessor — returns LSM_TYPE_UNKNOWN for NULL types.
 * Prevents SEGV in c_eval_expr_type_inner on unusual C++ AST shapes. */
static inline LSMTypeKind safe_kind(const LSMType *t) {
    return t ? t->kind : LSM_TYPE_UNKNOWN;
}
...
static const LSMType *c_lookup_field_type(CLSPContext *ctx, const char *type_qn,
...
static const char *type_to_qn(const LSMType *t);
const LSMType *c_simplify_type(CLSPContext *ctx, const LSMType *t, bool unwrap_pointer);
const LSMType *c_eval_expr_type(CLSPContext *ctx, TSNode node);
...
void c_lsp_init(CLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
                const LSMTypeRegistry *registry, const char *module_qn, bool cpp_mode,
    ...
    ctx->source_origin = LSM_SOURCE_ORIGIN_RAW;
    ctx->current_scope = lsm_scope_push(arena, NULL);
...
void c_lsp_add_include(CLSPContext *ctx, const char *header_path, const char *ns_qn) {
    if (ctx->include_count % 32 == 0) {
        ...
        const char **new_paths =
            (const char **)lsm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
        const char **new_qns =
            (const char **)lsm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(const char *));
    ...
    ctx->include_paths[ctx->include_count] = lsm_arena_strdup(ctx->arena, header_path);
    ctx->include_ns_qns[ctx->include_count] = lsm_arena_strdup(ctx->arena, ns_qn);
    ctx->include_count++;
}
...
static void c_add_using_namespace(CLSPContext *ctx, const char *ns_qn) {
    if (ctx->using_ns_count >= ctx->using_ns_cap) {
        ...
        const char **new_arr =
            (const char **)lsm_arena_alloc(ctx->arena, new_cap * sizeof(const char *));
        if (!new_arr)
            return;
    ...
    ctx->using_namespaces[ctx->using_ns_count++] = lsm_arena_strdup(ctx->arena, ns_qn);
}
...
static void c_add_using_decl(CLSPContext *ctx, const char *short_name, const char *full_qn) {
    ...
    ctx->using_decl_names[ctx->using_decl_count] = lsm_arena_strdup(ctx->arena, short_name);
    ctx->using_decl_qns[ctx->using_decl_count] = lsm_arena_strdup(ctx->arena, full_qn);
...
static void c_add_ns_alias(CLSPContext *ctx, const char *alias, const char *full_qn) {
    ...
    ctx->ns_alias_names[ctx->ns_alias_count] = lsm_arena_strdup(ctx->arena, alias);
    ctx->ns_alias_qns[ctx->ns_alias_count] = lsm_arena_strdup(ctx->arena, full_qn);
    ctx->ns_alias_count++;
}
...
static void c_add_fp_target(CLSPContext *ctx, const char *var_name, const char *target_qn) {
    if (ctx->fp_count >= ctx->fp_cap) {
        ...
        const char **new_qns =
            (const char **)lsm_arena_alloc(ctx->arena, new_cap * sizeof(const char *));
        const LSMScope **new_scopes =
            (const LSMScope **)lsm_arena_alloc(ctx->arena, new_cap * sizeof(const LSMScope *));
        ...
        if (ctx->fp_var_names && ctx->fp_count > 0) {
            ...
            memcpy(new_scopes, ctx->fp_binding_scopes, ctx->fp_count * sizeof(const LSMScope *));
        }
    ...
    ctx->fp_var_names[ctx->fp_count] = lsm_arena_strdup(ctx->arena, var_name);
    ctx->fp_target_qns[ctx->fp_count] = target_qn ? lsm_arena_strdup(ctx->arena, target_qn) : NULL;
    ctx->fp_binding_scopes[ctx->fp_count] = ctx->current_scope;
    ctx->fp_count++;
}

static bool c_scope_declares_name(const LSMScope *scope, const char *var_name) {
    if (!scope || !var_name) {
        return false;
    }
    for (const LSMScopeChunk *chunk = scope->chunks; chunk; chunk = chunk->next) {
...
static int c_find_fp_binding_index(CLSPContext *ctx, const char *var_name) {
    ...
    for (int i = ctx->fp_count - 1; i >= 0; i--) {
        ...
        const LSMScope *binding_scope = ctx->fp_binding_scopes[i];
        for (const LSMScope *scope = ctx->current_scope; scope; scope = scope->parent) {
...
static void c_update_fp_target(CLSPContext *ctx, const char *var_name, const char *target_qn) {
    ...
    int index = c_find_fp_binding_index(ctx, var_name);
    if (index >= 0) {
        ctx->fp_target_qns[index] = target_qn ? lsm_arena_strdup(ctx->arena, target_qn) : NULL;
    }
}
    ...
    *out_has_cast = false;
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (len < 2 || text[0] != '"' || text[len - 1] != '"')
            continue;
        char *name = lsm_arena_strndup(ctx->arena, text + 1, len - 2);
        if (!name || !name[0])
            continue;
...
static void c_add_pending_template_call(CLSPContext *ctx, const char *func_qn,
    ...
    if (ctx->pending_tc_count >= ctx->pending_tc_cap) {
        ...
        void *new_arr = lsm_arena_alloc(ctx->arena, sz);
        if (!new_arr)
            return;
    ...
    const char *func_qn_copy = lsm_arena_strdup(ctx->arena, func_qn);
    const char *type_param_copy = lsm_arena_strdup(ctx->arena, type_param);
    const char *method_name_copy = lsm_arena_strdup(ctx->arena, method_name);
...
static void c_resolve_pending_template_calls(CLSPContext *ctx, const LSMRegisteredFunc *callee,
                                             const LSMType **call_arg_types, int call_arg_count) {
    ...
    if (callee->signature && callee->signature->kind == LSM_TYPE_FUNC &&
        ...
        for (int i = 0; i < limit; i++) {
            const LSMType *formal = callee->signature->data.func.param_types[i];
            if (!formal || !call_arg_types[i])
                continue;
            // Unwrap references/pointers
            while (formal &&
                   (formal->kind == LSM_TYPE_REFERENCE || formal->kind == LSM_TYPE_RVALUE_REF ||
                    formal->kind == LSM_TYPE_POINTER)) {
                formal = (formal->kind == LSM_TYPE_POINTER) ? formal->data.pointer.elem
                                                            : formal->data.reference.elem;
            }
            if (formal && formal->kind == LSM_TYPE_TYPE_PARAM) {
                for (int j = 0; j < tpn_count; j++) {
                    if (strcmp(tpn[j], formal->data.type_param.name) == 0) {
                        const LSMType *arg = call_arg_types[i];
    ...
    for (int i = 0; i < ctx->pending_tc_count; i++) {
        ...
        for (int j = 0; j < tpn_count; j++) {
            ...
            const LSMRegisteredFunc *m = c_lookup_member(ctx, concrete_qn, method);
            if (m) {
...
static const LSMType **c_extract_call_arg_types(CLSPContext *ctx, TSNode call_node,
                                                int *out_count) {
    ...
    const LSMType **types =
        (const LSMType **)lsm_arena_alloc(ctx->arena, (nc + 1) * sizeof(const LSMType *));
    if (!types) {
        *out_count = 0;
        return NULL;
    }
...
static void c_parse_template_params(CLSPContext *ctx, TSNode template_decl) {
    ...
    const char **names =
        (const char **)lsm_arena_alloc(ctx->arena, (nc + 1) * sizeof(const char *));
    const LSMType **defaults =
        (const LSMType **)lsm_arena_alloc(ctx->arena, (nc + 1) * sizeof(const LSMType *));
    if (!names || !defaults)
        return;
    ...
    for (uint32_t i = 0; i < nc && idx < 16; i++) {
        ...
        if (strcmp(pk, "type_parameter_declaration") == 0 ||
            ...
            if (!pname)
                pname = lsm_arena_sprintf(ctx->arena, "T%d", idx);
...
static const LSMType *c_resolve_template_param(CLSPContext *ctx, const char *param_name) {
...
static const char *c_build_qn(CLSPContext *ctx, const char *text) {
    ...
    size_t len = strlen(text);
    char *buf = (char *)lsm_arena_alloc(ctx->arena, len + 1);
    if (!buf)
        return text;
    size_t j = 0;
...
static const char *c_resolve_name(CLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const LSMRegisteredFunc *f =
            lsm_registry_lookup_method(ctx->registry, ctx->enclosing_class_qn, name);
        if (f)
    ...
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const LSMRegisteredFunc *f =
            lsm_registry_lookup_symbol(ctx->registry, ctx->using_namespaces[i], name);
        if (f)
            return f->qualified_name;
    }
    ...
    if (ctx->current_namespace) {
        ...
        while (ns && ns[0]) {
            const LSMRegisteredFunc *f = lsm_registry_lookup_symbol(ctx->registry, ns, name);
            if (f)
                return f->qualified_name;
            // Walk up: "a.b.c" -> "a.b" -> "a"
            ...
            if (!dot)
                break;
            char *parent = (char *)lsm_arena_alloc(ctx->arena, (size_t)(dot - ns) + 1);
            if (!parent)
                break;
    ...
    if (ctx->module_qn) {
        const LSMRegisteredFunc *f =
            lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
        if (f)
            return f->qualified_name;
    }
...
static const char *extract_namespace_from_qn(LSMArena *arena, const char *qn) {
    if (!qn)
        return NULL;
...
static const char *c_adl_resolve(CLSPContext *ctx, const char *name, TSNode call_node) {
    ...
    for (uint32_t i = 0; i < nc && ns_count < 8; i++) {
        ...
        const LSMType *arg_type = c_eval_expr_type(ctx, arg);
    ...
    for (int i = 0; i < ns_count; i++) {
        const LSMRegisteredFunc *f = lsm_registry_lookup_symbol(ctx->registry, namespaces[i], name);
        if (!f && ctx->module_qn) {
            const char *prefixed =
                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, namespaces[i]);
            f = lsm_registry_lookup_symbol(ctx->registry, prefixed, name);
        }
        if (f)
            return f->qualified_name;
    }
...
static const char *c_resolve_name_to_func_qn(CLSPContext *ctx, const char *name) {
    ...
    // Global scope
    const LSMRegisteredFunc *gf = lsm_registry_lookup_func(ctx->registry, name);
    if (gf)
...
static const char *c_exact_function_target_from_expr(CLSPContext *ctx, TSNode expr) {
    ...
    if (!name || lsm_scope_contains(ctx->current_scope, name)) {
        return NULL;
    }
    return c_resolve_name_to_func_qn(ctx, name);
}
...
static const LSMType *c_resolve_name_to_type(CLSPContext *ctx, const char *name) {
    if (!name)
    ...
    {
        const LSMType *scoped = lsm_scope_lookup(ctx->current_scope, name);
        if (scoped && scoped->kind == LSM_TYPE_ALIAS) {
            const LSMType *resolved = lsm_type_resolve_alias(scoped);
            if (resolved && !lsm_type_is_unknown(resolved))
                return resolved;
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_names) {
        for (int i = 0; i < ctx->template_param_count; i++) {
            if (ctx->template_param_names[i] && strcmp(ctx->template_param_names[i], name) == 0) {
                ...
                return lsm_type_type_param(ctx->arena, name);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->using_decl_count; i++) {
        if (strcmp(ctx->using_decl_names[i], name) == 0) {
            const LSMRegisteredType *rt =
                lsm_registry_lookup_type(ctx->registry, ctx->using_decl_qns[i]);
            if (rt)
                return lsm_type_named(ctx->arena, rt->qualified_name);
        }
    }

    // Enclosing class (nested types: Factory::Product)
    if (ctx->enclosing_class_qn) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return lsm_type_named(ctx->arena, qn);
    }

    // Current namespace
    if (ctx->current_namespace) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return lsm_type_named(ctx->arena, qn);
    }
    ...
    // Module scope
    if (ctx->module_qn) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return lsm_type_named(ctx->arena, qn);
    }
    ...
    // Using namespaces
    for (int i = 0; i < ctx->using_ns_count; i++) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->using_namespaces[i], name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return lsm_type_named(ctx->arena, qn);
    }
    ...
    if (ctx->cpp_mode) {
        const char *std_qn = lsm_arena_sprintf(ctx->arena, "std.%s", name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, std_qn);
        if (rt)
            return lsm_type_named(ctx->arena, std_qn);
    }

    // Unresolved — return named with module prefix
    if (ctx->module_qn) {
        return lsm_type_named(ctx->arena,
                              lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name));
    }
    return lsm_type_named(ctx->arena, name);
}

// ============================================================================
// c_parse_type_node: AST type node -> LSMType
// ============================================================================

const LSMType *c_parse_type_node(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return name ? lsm_type_builtin(ctx->arena, name) : lsm_type_unknown();
    }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (!ts_node_is_null(name_child) &&
            ...
            if (scope_text) {
                const char *scoped =
                    lsm_arena_sprintf(ctx->arena, "%s::%s", scope_text, tmpl_name_text);
            ...
            const LSMRegisteredType *trt = lsm_registry_lookup_type(ctx->registry, template_qn);
            if (!trt && ctx->module_qn) {
                const char *mod_qn =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, template_qn);
                trt = lsm_registry_lookup_type(ctx->registry, mod_qn);
                if (trt)
                    template_qn = mod_qn;
            }

            // Parse template arguments
            const LSMType *targs[16] = {
                NULL}; /* zero-fill: lsm_type_substitute requires NULL-terminated args
            ...
            if (targ_count > 0)
                return lsm_type_template(ctx->arena, template_qn, targs, targ_count);
            return lsm_type_named(ctx->arena, template_qn);
        }
            ...
            if (scope_text) {
                ...
                if (alias_qn) {
                    if (!ts_node_is_null(name_child)) {
                        ...
                        if (name)
                            text = (char *)lsm_arena_sprintf(ctx->arena, "%s::%s", alias_qn, name);
                    }
                }
            }
        }
    ...
    if (strcmp(kind, "template_type") == 0) {
        ...
        if (!trt && ctx->current_namespace) {
            ...
            // "std.shared_ptr")
            const char *ns_qn =
                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, template_qn);
            trt = lsm_registry_lookup_type(ctx->registry, ns_qn);
            if (trt)
                template_qn = ns_qn;
            // Also try module + namespace
            if (!trt && ctx->module_qn) {
                const char *full_qn = lsm_arena_sprintf(ctx->arena, "%s.%s.%s", ctx->module_qn,
                                                        ctx->current_namespace, template_qn);
                trt = lsm_registry_lookup_type(ctx->registry, full_qn);
                if (trt)
                    template_qn = full_qn;
            }
        }
        if (!trt) {
            // Try with std:: prefix
            const char *std_qn = lsm_arena_sprintf(ctx->arena, "std.%s", template_qn);
            trt = lsm_registry_lookup_type(ctx->registry, std_qn);
            if (trt)
                template_qn = std_qn;
        }
        ...
        // Parse template arguments
        const LSMType *targs[16] = {
            NULL}; /* zero-fill: lsm_type_substitute requires NULL-terminated args (uninitialized
    ...
    if (strcmp(kind, "pointer_declarator") == 0 || strcmp(kind, "pointer_type") == 0) {
        ...
        return lsm_type_pointer(ctx->arena, lsm_type_unknown());
    }
    ...
    if (strcmp(kind, "reference_declarator") == 0 ||
        ...
        if (ts_node_named_child_count(node) > 0) {
            return lsm_type_reference(ctx->arena,
                                      c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return lsm_type_reference(ctx->arena, lsm_type_unknown());
    }

    // auto keyword
    if (strcmp(kind, "auto") == 0 || strcmp(kind, "placeholder_type_specifier") == 0) {
        return lsm_type_unknown(); // will be resolved from initializer
    }
    ...
    if (strcmp(kind, "type_qualifier") == 0) {
        return lsm_type_unknown(); // qualifier itself has no type
    }

    // array_declarator: int[10]
    if (strcmp(kind, "array_declarator") == 0 || strcmp(kind, "array_type") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            return lsm_type_slice(ctx->arena, c_parse_type_node(ctx, ts_node_named_child(node, 0)));
        }
        return lsm_type_unknown();
    }
    ...
    if (strcmp(kind, "function_declarator") == 0 ||
        ...
        return lsm_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "dependent_type") == 0 || strcmp(kind, "dependent_name") == 0) {
        char *text = c_node_text(ctx, node);
        if (text)
            return lsm_type_named(ctx->arena, c_build_qn(ctx, text));
        return lsm_type_unknown();
    }
...
const LSMType *c_simplify_type(CLSPContext *ctx, const LSMType *t, bool unwrap_pointer) {
    for (int i = 0; i < 64 && t; i++) {
        ...
        if (t->kind == LSM_TYPE_TYPE_PARAM && ctx->in_template) {
            const LSMType *resolved = c_resolve_template_param(ctx, t->data.type_param.name);
            if (resolved) {
                t = resolved;
                continue;
            }
        ...
        if (t->kind == LSM_TYPE_TEMPLATE && is_smart_ptr(t->data.template_type.template_name)) {
        ...
        if (t->kind == LSM_TYPE_NAMED) {
            const LSMRegisteredFunc *op = lsm_registry_lookup_method(
                ctx->registry, t->data.named.qualified_name, "operator->");
            if (op && op->signature && op->signature->kind == LSM_TYPE_FUNC &&
        ...
        if (t->kind == LSM_TYPE_TEMPLATE) {
            const char *tqn = t->data.template_type.template_name;
            const LSMRegisteredFunc *op = c_lookup_member(ctx, tqn, "operator->");
            if (op && op->signature && op->signature->kind == LSM_TYPE_FUNC &&
                op->signature->data.func.return_types && op->signature->data.func.return_types[0]) {
                const LSMType *ret = op->signature->data.func.return_types[0];
                // Substitute template params using receiver's template args
                if (ret->kind == LSM_TYPE_TYPE_PARAM || ret->kind == LSM_TYPE_POINTER ||
                    ret->kind == LSM_TYPE_REFERENCE) {
                    const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, tqn);
                    if (!rt && ctx->module_qn) {
                        rt = lsm_registry_lookup_type(
                            ctx->registry,
                            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    }
                    if (rt && rt->type_param_names) {
                        ret = lsm_type_substitute(ctx->arena, ret, rt->type_param_names,
                                                  t->data.template_type.template_args);
                    } else {
                        const char *fb[] = {"T", "K", "V", NULL};
                        const LSMType *fa[3] = {NULL};
                        ...
                        ret = lsm_type_substitute(ctx->arena, ret, fb, fa);
                    }
                }
                t = ret;
                if (t->kind == LSM_TYPE_POINTER)
...
static const LSMType *c_eval_expr_type_inner(CLSPContext *ctx, TSNode node);
...
const LSMType *c_eval_expr_type(CLSPContext *ctx, TSNode node) {
    ...
    ctx->eval_depth++;
    const LSMType *result = c_eval_expr_type_inner(ctx, node);
    ctx->eval_depth--;
    return result ? result : lsm_type_unknown();
}
...
static const LSMType *c_eval_expr_type_inner(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (fqn) {
            const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, fqn);
            if (f && f->signature)
                return f->signature;
        }

        // Check if it's a type name (for constructor calls / casts)
        const LSMType *type = c_resolve_name_to_type(ctx, name);
        if (type && type->kind == LSM_TYPE_NAMED) {
    ...
    if (strcmp(kind, "this") == 0) {
        if (ctx->enclosing_class_qn) {
            return lsm_type_pointer(ctx->arena,
                                    lsm_type_named(ctx->arena, ctx->enclosing_class_qn));
        }
        return lsm_type_unknown();
    }
    ...
    if (strcmp(kind, "field_expression") == 0) {
        ...
        const LSMType *obj_type = c_eval_expr_type(ctx, arg_node);
        if (lsm_type_is_unknown(obj_type))
            return lsm_type_unknown();
        ...
        // Look up method
        const LSMRegisteredFunc *method = c_lookup_member(ctx, type_qn, field_name);
        if (method && method->signature)
            return method->signature;

        // Look up field
        const LSMType *field_type = c_lookup_field_type(ctx, type_qn, field_name, 0);
        if (field_type && !lsm_type_is_unknown(field_type)) {
            ...
            bool is_tparam = (field_type->kind == LSM_TYPE_TYPE_PARAM);
            ...
            if (!is_tparam && field_type->kind == LSM_TYPE_NAMED &&
                base->kind == LSM_TYPE_TEMPLATE) {
                const LSMRegisteredType *trt = lsm_registry_lookup_type(ctx->registry, type_qn);
                if (!trt && ctx->module_qn) {
                    trt = lsm_registry_lookup_type(
                        ctx->registry,
                        lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
                }
                if (trt && trt->type_param_names) {
                    ...
                    for (int tpi = 0; trt->type_param_names[tpi]; tpi++) {
                        if (strcmp(trt->type_param_names[tpi], short_fn) == 0) {
                            ...
                            field_type = lsm_type_type_param(ctx->arena, short_fn);
                            break;
                        }
                    }
                }
            }
            if (is_tparam && base->kind == LSM_TYPE_TEMPLATE &&
                ...
                if (rt && rt->type_param_names) {
                    field_type = lsm_type_substitute(ctx->arena, field_type, rt->type_param_names,
                                                     base->data.template_type.template_args);
                } else {
                    // Fallback: positional T, K, V
                    const char *params[] = {"T", "K", "V", "T1", "T2", NULL};
                    const LSMType *args[5] = {NULL};
                    ...
                    field_type = lsm_type_substitute(ctx->arena, field_type, params, args);
                }
            }
            return field_type;
        }
    ...
    if (strcmp(kind, "qualified_identifier") == 0 || strcmp(kind, "scoped_identifier") == 0) {
        ...
        if (!ts_node_is_null(qi_name_child) &&
            ...
            if (bare_name) {
                text = scope_text
                           ? (char *)lsm_arena_sprintf(ctx->arena, "%s::%s", scope_text, bare_name)
                           : bare_name;
            }
        }
        ...
        const LSMRegisteredFunc *f = NULL;
        if (ctx->module_qn) {
            f = lsm_registry_lookup_func(
                ctx->registry, lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
        }
        if (!f)
            f = lsm_registry_lookup_func(ctx->registry, qn);
        ...
        if (!f) {
            ...
            if (ns_dot && ctx->module_qn) {
                ...
                f = lsm_registry_lookup_func(
                    ctx->registry, lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare));
                if (!f)
                    f = lsm_registry_lookup_func(ctx->registry, bare);
            }
        }
        if (f && f->signature) {
            // If template args present, substitute into return type
            if (qi_has_tmpl_args && !ts_node_is_null(qi_tmpl_args) &&
                f->signature->kind == LSM_TYPE_FUNC && f->signature->data.func.return_types &&
                f->signature->data.func.return_types[0]) {
                const LSMType *base_ret = f->signature->data.func.return_types[0];
                const LSMType *targs[16] = {
                    NULL}; /* zero-fill: lsm_type_substitute requires NULL-terminated args
                              (uninitialized tail bound T to stack garbage -> corrupt type graph,
                              bitcoin serialize.h) */
                ...
                if (targ_count > 0) {
                    ...
                    if (tpn) {
                        base_ret = lsm_type_substitute(ctx->arena, base_ret, tpn, targs);
                    } else {
                        const char *fallback[] = {"T", "U", "V", "W", NULL};
                        base_ret = lsm_type_substitute(ctx->arena, base_ret, fallback, targs);
                    }
                    ...
                    if (base_ret->kind == LSM_TYPE_NAMED) {
                        const LSMType **final_targs = (const LSMType **)lsm_arena_alloc(
                            ctx->arena, (targ_count + 1) * sizeof(const LSMType *));
                        for (int i = 0; i < targ_count; i++)
                            final_targs[i] = targs[i];
                        final_targs[targ_count] = NULL;
                        base_ret =
                            lsm_type_template(ctx->arena, base_ret->data.named.qualified_name,
                                              final_targs, targ_count);
                    }
                }
                const LSMType **rets =
                    (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(const LSMType *));
                rets[0] = base_ret;
                rets[1] = NULL;
                return lsm_type_func_replace_returns(ctx->arena, f->signature, rets);
            }
            return f->signature;
        }
        ...
        if (rt)
            return lsm_type_named(ctx->arena, qn);
        if (!rt && ctx->module_qn) {
            const char *mod_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
            rt = lsm_registry_lookup_type(ctx->registry, mod_qn);
            if (rt)
                return lsm_type_named(ctx->arena, mod_qn);
        }
        ...
        if (dot) {
            ...
            char *enum_qn = (char *)lsm_arena_alloc(ctx->arena, prefix_len + 1);
            if (enum_qn) {
                memcpy(enum_qn, qn, prefix_len);
                enum_qn[prefix_len] = '\0';
                const LSMRegisteredType *enum_rt = lsm_registry_lookup_type(ctx->registry, enum_qn);
                if (enum_rt) {
                    return lsm_type_named(ctx->arena, enum_qn);
                }
                // Try with module prefix
                if (ctx->module_qn) {
                    const char *fqn_enum =
                        lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                    enum_rt = lsm_registry_lookup_type(ctx->registry, fqn_enum);
                    if (enum_rt)
                        return lsm_type_named(ctx->arena, fqn_enum);
                }
            }
            ...
            if (ctx->in_template && dot) {
                ...
                char *scope_name = (char *)lsm_arena_alloc(ctx->arena, prefix_len2 + 1);
                if (scope_name) {
                    memcpy(scope_name, qn, prefix_len2);
                    scope_name[prefix_len2] = '\0';
                    const LSMType *default_type = c_resolve_template_param(ctx, scope_name);
                    if (default_type) {
                        ...
                        if (default_qn) {
                            ...
                            const LSMRegisteredFunc *mf =
                                lsm_registry_lookup_method(ctx->registry, default_qn, member);
                            if (mf && mf->signature)
                                return mf->signature;
                            // Try as static function
                            const char *dep_qn =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", default_qn, member);
                            const LSMRegisteredFunc *sf =
                                lsm_registry_lookup_func(ctx->registry, dep_qn);
                            if (sf && sf->signature)
                                return sf->signature;
                        }
                    }
    ...
    if (strcmp(kind, "template_function") == 0) {
        ...
        if (strcmp(nk, "qualified_identifier") == 0 || strcmp(nk, "scoped_identifier") == 0) {
            ...
            if (!f && ctx->module_qn) {
                const char *mod_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
                f = lsm_registry_lookup_func(ctx->registry, mod_qn);
            }
        } else {
            const char *fqn = c_resolve_name(ctx, name);
            if (fqn)
                f = lsm_registry_lookup_func(ctx->registry, fqn);
        }
        if (!f || !f->signature)
            return lsm_type_unknown();

        // Get the base return type from the function signature
        const LSMType *base_ret = NULL;
        if (f->signature->kind == LSM_TYPE_FUNC && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
        ...
        if (targ_count > 0) {
            ...
            if (tpn) {
                ...
                base_ret = lsm_type_substitute(ctx->arena, base_ret, fallback_names, targs);
            }
        }

        // Wrap return type in FUNC signature
        const LSMType **rets = lsm_arena_alloc(ctx->arena, 2 * sizeof(LSMType *));
        rets[0] = base_ret;
        rets[1] = NULL;
        return lsm_type_func_replace_returns(ctx->arena, f->signature, rets);
    }
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        const LSMType *func_type = c_eval_expr_type(ctx, func_node);
        if (!func_type)
            return lsm_type_unknown();
        // FUNC type -> return its return type
        if (func_type->kind == LSM_TYPE_FUNC && func_type->data.func.return_types &&
            func_type->data.func.return_types[0]) {
            const LSMType *ret = func_type->data.func.return_types[0];
            ...
            bool needs_subst = (ret->kind == LSM_TYPE_TYPE_PARAM || ret->kind == LSM_TYPE_NAMED ||
                                ret->kind == LSM_TYPE_POINTER || ret->kind == LSM_TYPE_REFERENCE ||
                                ret->kind == LSM_TYPE_RVALUE_REF ||
                                ret->kind == LSM_TYPE_TEMPLATE || ret->kind == LSM_TYPE_SLICE);
            ...
            if (needs_subst) {
                ...
                if (strcmp(ts_node_type(func_node), "field_expression") == 0) {
                    ...
                    if (!ts_node_is_null(arg_node)) {
                        ...
                        if (obj_type && obj_type->kind == LSM_TYPE_TEMPLATE &&
                            ...
                            if (!tpn && recv_qn) {
                                const LSMRegisteredType *rrt =
                                    lsm_registry_lookup_type(ctx->registry, recv_qn);
                                if (!rrt && ctx->module_qn) {
                                    rrt = lsm_registry_lookup_type(
                                        ctx->registry, lsm_arena_sprintf(ctx->arena, "%s.%s",
                            ...
                            if (tpn) {
                                ...
                                ret =
                                    lsm_type_substitute(ctx->arena, ret, tpn,
                                                        obj_type->data.template_type.template_args);
                                substituted = true;
                            } else {
                                // Fallback: positional params T, K, V, T1, T2
                                const char *params[] = {"T", "K", "V", "T1", "T2", NULL};
                                const LSMType *pargs[5] = {NULL};
                                if (nargs > 0)
                                ...
                                ret = lsm_type_substitute(ctx->arena, ret, params, pargs);
                                substituted = true;
                            }
                        }
                    }
                ...
                if (!substituted) {
                    ...
                    const LSMRegisteredFunc *rf = NULL;
                    ...
                    if (strcmp(fn_type, "identifier") == 0) {
                        ...
                        if (fname) {
                            ...
                            if (fqn)
                                rf = lsm_registry_lookup_func(ctx->registry, fqn);
                        }
                        ...
                        if (fname) {
                            ...
                            rf = lsm_registry_lookup_func(ctx->registry, qn);
                        }
                    ...
                    if (rf && rf->type_param_names && rf->signature &&
                        rf->signature->kind == LSM_TYPE_FUNC &&
                        ...
                        if (tp_count > 0 && tp_count <= 8) {
                            ...
                            if (!ts_node_is_null(call_args)) {
                                ...
                                for (uint32_t ai = 0;
                                    ...
                                    if (ts_node_is_null(carg))
                                        continue;
                                    const LSMType *arg_t = c_eval_expr_type(ctx, carg);
                                    if (lsm_type_is_unknown(arg_t)) {
                                        pi++;
                                        continue;
                                    }
                                    // Unwrap ref/ptr from param type
                                    const LSMType *param_t =
                                        rf->signature->data.func.param_types[pi];
                                    while (param_t && (param_t->kind == LSM_TYPE_REFERENCE ||
                                                       param_t->kind == LSM_TYPE_RVALUE_REF ||
                                                       param_t->kind == LSM_TYPE_POINTER)) {
                                        if (param_t->kind == LSM_TYPE_POINTER)
                                            param_t = param_t->data.pointer.elem;
                                        else
                                            param_t = param_t->data.reference.elem;
                                    }
                                    // If param type is TYPE_PARAM, bind it
                                    if (param_t && param_t->kind == LSM_TYPE_TYPE_PARAM) {
                                        for (int ti = 0; ti < tp_count; ti++) {
                                            if (strcmp(rf->type_param_names[ti],
                                                ...
                                                const LSMType *unwrapped = arg_t;
                                                while (unwrapped &&
                                                       (unwrapped->kind == LSM_TYPE_REFERENCE ||
                                                        unwrapped->kind == LSM_TYPE_RVALUE_REF ||
                                                        unwrapped->kind == LSM_TYPE_POINTER)) {
                                                    if (unwrapped->kind == LSM_TYPE_POINTER)
                            ...
                            if (any_deduced) {
                                const LSMType *substituted_ret = lsm_type_substitute(
            ...
            if (!ret)
                return lsm_type_unknown();
            if (ret->kind == LSM_TYPE_REFERENCE || ret->kind == LSM_TYPE_RVALUE_REF)
                ret = ret->data.reference.elem;
            return ret;
        }

        // Constructor call: Type(args) — if func_node resolves to a named type
        if (func_type->kind == LSM_TYPE_NAMED) {
            return func_type;
        }
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            return lsm_type_pointer(ctx->arena, c_parse_type_node(ctx, type_node));
        }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "template_type") == 0 ||
                ...
                return lsm_type_pointer(ctx->arena, c_parse_type_node(ctx, child));
            }
        }
        return lsm_type_pointer(ctx->arena, lsm_type_unknown());
    }
    ...
    if (strcmp(kind, "subscript_expression") == 0) {
        ...
        if (ts_node_is_null(arg_node))
            return lsm_type_unknown();
        const LSMType *arr_type = c_eval_expr_type(ctx, arg_node);
        if (!arr_type)
            return lsm_type_unknown();

        // Array/slice: return element
        if (arr_type->kind == LSM_TYPE_SLICE)
            return arr_type->data.slice.elem;
        if (arr_type->kind == LSM_TYPE_POINTER)
        ...
        if (qn) {
            const LSMRegisteredFunc *op =
                lsm_registry_lookup_method(ctx->registry, qn, "operator[]");
            if (op && op->signature && op->signature->kind == LSM_TYPE_FUNC &&
                op->signature->data.func.return_types && op->signature->data.func.return_types[0]) {
                const LSMType *ret = op->signature->data.func.return_types[0];
                if (ret->kind == LSM_TYPE_REFERENCE)
                    ret = ret->data.reference.elem;

                // Substitute template params
                if (ret->kind == LSM_TYPE_TYPE_PARAM && arr_type->kind == LSM_TYPE_TEMPLATE) {
                    const char *params[] = {"T", "K", "V", NULL};
                    const LSMType *args[3] = {NULL};
                    ...
                    ret = lsm_type_substitute(ctx->arena, ret, params, args);
                }
                return ret;
            }
        }
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                ...
                if (!op)
                    continue;
                if (strcmp(op, "*") == 0) {
                    const LSMType *inner = c_eval_expr_type(ctx, operand);
                    inner = c_simplify_type(ctx, inner, false);
                    // TEMPLATE with operator*(): look up and substitute return type
                    if (inner && inner->kind == LSM_TYPE_TEMPLATE) {
                        const char *tqn = inner->data.template_type.template_name;
                        const LSMRegisteredFunc *opf = c_lookup_member(ctx, tqn, "operator*");
                        if (opf && opf->signature && opf->signature->kind == LSM_TYPE_FUNC &&
                            opf->signature->data.func.return_types &&
                            opf->signature->data.func.return_types[0]) {
                            const LSMType *ret = opf->signature->data.func.return_types[0];
                            const LSMRegisteredType *rt =
                                lsm_registry_lookup_type(ctx->registry, tqn);
                            if (!rt && ctx->module_qn) {
                                rt = lsm_registry_lookup_type(
                                    ctx->registry,
                                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, tqn));
                    ...
                    return lsm_type_deref(inner);
                }
                if (strcmp(op, "&") == 0) {
                    return lsm_type_pointer(ctx->arena, c_eval_expr_type(ctx, operand));
                }
    ...
    if (strcmp(kind, "pointer_expression") == 0) {
        ...
        for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
            ...
            if (!ts_node_is_named(child)) {
                char *op = c_node_text(ctx, child);
                if (op && strcmp(op, "*") == 0) {
                    const LSMType *inner = c_eval_expr_type(ctx, arg);
                    inner = c_simplify_type(ctx, inner, false);
                    if (inner && inner->kind == LSM_TYPE_TEMPLATE) {
                        const char *tqn = inner->data.template_type.template_name;
                        const LSMRegisteredFunc *opf = c_lookup_member(ctx, tqn, "operator*");
                ...
                if (op && strcmp(op, "&") == 0)
                    return lsm_type_pointer(ctx->arena, c_eval_expr_type(ctx, arg));
                break;
            }
        }
        return lsm_type_unknown();
    ...
    if (strcmp(kind, "sizeof_expression") == 0 || strcmp(kind, "alignof_expression") == 0) {
        return lsm_type_builtin(ctx->arena, "size_t");
    }

    // --- lambda_expression ---
    if (strcmp(kind, "lambda_expression") == 0) {
        const LSMType *ret_type = NULL;
        ...
        if (!ret_type || lsm_type_is_unknown(ret_type)) {
        ...
        if (ret_type && !lsm_type_is_unknown(ret_type)) {
            const LSMType **ret_types =
                (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(const LSMType *));
            if (ret_types) {
                ret_types[0] = ret_type;
                ret_types[1] = NULL;
                return lsm_type_func(ctx->arena, NULL, NULL, ret_types);
            }
        }
        return lsm_type_func(ctx->arena, NULL, NULL, NULL);
    }
    ...
    if (strcmp(kind, "co_await_expression") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            ...
            const LSMType *op_type = c_eval_expr_type(ctx, operand);
            ...
            if (op_type && op_type->kind == LSM_TYPE_TEMPLATE &&
            ...
            if (op_qn) {
                const LSMRegisteredFunc *ar = c_lookup_member(ctx, op_qn, "await_resume");
                if (ar && ar->signature && ar->signature->kind == LSM_TYPE_FUNC &&
    ...
    if (strcmp(kind, "fold_expression") == 0) {
        ...
        for (uint32_t fi = 0; fi < fnc; fi++) {
            ...
            if (!ts_node_is_null(child)) {
                const LSMType *ct = c_eval_expr_type(ctx, child);
                if (!lsm_type_is_unknown(ct))
                    return ct;
            }
        }
        return lsm_type_unknown();
        ...
        for (uint32_t gi = 1; gi < gnc; gi++) {
            ...
            if (!ts_node_is_null(assoc)) {
                ...
                if (anc > 0) {
                    ...
                    const LSMType *vt = c_eval_expr_type(ctx, val);
                    if (!lsm_type_is_unknown(vt))
                        return vt;
                }
            }
        }
        ...
        if (text) {
            ...
            for (const char *p = text; *p; p++) {
                if (*p == '.' || *p == 'f' || *p == 'F' || *p == 'e' || *p == 'E')
                    return lsm_type_builtin(ctx->arena, "double");
            }
        }
        return lsm_type_builtin(ctx->arena, "int");
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "concatenated_string") == 0 ||
        ...
        return lsm_type_pointer(ctx->arena, lsm_type_builtin(ctx->arena, "char"));
    }
    if (strcmp(kind, "char_literal") == 0) {
        return lsm_type_builtin(ctx->arena, "char");
    }
    ...
    if (strcmp(kind, "initializer_list") == 0) {
        return lsm_type_unknown(); // context-dependent
    }
...
// lsm_arena_sprintf). The buffer is only read by the immediate registry lookup — a
...
static const LSMRegisteredFunc *c_lookup_member_depth(CLSPContext *ctx, const char *type_qn,
                                                      const char *member_name, int depth) {
    if (!type_qn || !member_name)
        return NULL;
    if (depth > LSM_LSP_MAX_LOOKUP_DEPTH)
        return NULL;
    ...
    if (!neg_memo_hit && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        f = lsm_registry_lookup_method(ctx->registry, prefixed, member_name);
        if (f)
            return f;
    }
    ...
    {
        const LSMType *scoped = lsm_scope_lookup(ctx->current_scope, type_qn);
        if (scoped && scoped->kind == LSM_TYPE_ALIAS) {
            const LSMType *underlying = lsm_type_resolve_alias(scoped);
            if (underlying && !lsm_type_is_unknown(underlying)) {
    ...
    if (!rt && ctx->module_qn) {
        ...
        if (!prefixed)
            prefixed = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        rt = lsm_registry_lookup_type(ctx->registry, prefixed);
    }
    if (rt) {
        // Alias chain
        if (rt->alias_of) {
...
const LSMRegisteredFunc *c_lookup_member(CLSPContext *ctx, const char *type_qn,
...
static bool c_base_declares_member(CLSPContext *ctx, const char *type_qn, const char *member_name) {
    ...
    if (!rt && ctx->module_qn) {
        rt = lsm_registry_lookup_type(
            ctx->registry, lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn));
    }
...
static const LSMType *c_parse_declaration_type(CLSPContext *ctx, TSNode decl_node) {
...
void c_process_statement(CLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "declaration") == 0) {
        const LSMType *base_type = c_parse_declaration_type(ctx, node);
        bool has_auto = false;
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(ck, "init_declarator") == 0) {
                ...
                const LSMType *var_type = base_type;
                ...
                if (!ts_node_is_null(decl)) {
                    ...
                    if (strcmp(dk, "identifier") == 0) {
                        ...
                        if (!has_auto) {
                            for (int d = 0; d < ptr_depth; d++)
                                var_type = lsm_type_pointer(ctx->arena, var_type);
                        }
                        ...
                        if (ts_node_named_child_count(decl) > 0) {
                            ...
                            if (strcmp(ik, "identifier") == 0) {
                                ...
                                const LSMType *rhs_type = lsm_type_unknown();
                                ...
                                for (uint32_t bi = 0; bi < bnc; bi++) {
                                    ...
                                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                        ...
                                        if (!bname)
                                            continue;
                                        const LSMType *elem_type = lsm_type_unknown();
                                        if (rhs_type && rhs_type->kind == LSM_TYPE_TEMPLATE &&
                                        ...
                                        if (lsm_type_is_unknown(elem_type) && rhs_type &&
                                            rhs_type->kind == LSM_TYPE_SLICE)
                                            elem_type = rhs_type->data.slice.elem;
                                        if (lsm_type_is_unknown(elem_type) && rhs_type) {
                                            const char *rhs_qn = type_to_qn(rhs_type);
                                            const LSMRegisteredType *rt =
                                                rhs_qn ? lsm_registry_lookup_type(ctx->registry,
                                                                                  rhs_qn)
                                                       : NULL;
                                        ...
                                        lsm_scope_bind(ctx->current_scope, bname, elem_type);
                                        binding_idx++;
                                    }
                                }
                                continue;
                        ...
                        if (!has_auto)
                            var_type = lsm_type_reference(ctx->arena, var_type);
                        ...
                        var_type = lsm_type_slice(ctx->arena, var_type);
                        ...
                        for (uint32_t bi = 0; bi < bnc; bi++) {
                            ...
                            if (strcmp(ts_node_type(bn), "identifier") == 0) {
                                ...
                                if (lsm_type_is_unknown(elem_type) && rhs_type) {
                                    const char *rhs_qn = type_to_qn(rhs_type);
                                    const LSMRegisteredType *rt =
                                        rhs_qn ? lsm_registry_lookup_type(ctx->registry, rhs_qn)
                                               : NULL;
                                    if (!rt && rhs_qn && ctx->module_qn)
                                        rt = lsm_registry_lookup_type(
                                            ctx->registry,
                                            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                              rhs_qn));
                    ...
                    if (var_name && var_name[0] && strcmp(var_name, "_") != 0) {
                        lsm_scope_bind(ctx->current_scope, var_name, var_type);
                        ...
                        bool function_pointer_like =
                            var_type &&
                            (var_type->kind == LSM_TYPE_FUNC || var_type->kind == LSM_TYPE_POINTER);
                        ...
                        if (!fp_target && !ts_node_is_null(value)) {
                            ...
                            if (dll_func) {
                                bool is_fp_type = var_type && (var_type->kind == LSM_TYPE_FUNC ||
                                                               var_type->kind == LSM_TYPE_POINTER);
                                if (is_fp_type || has_cast) {
                                    fp_target =
                                        lsm_arena_sprintf(ctx->arena, "external.%s", dll_func);
                                }
                            }
                        }
                ...
                if (var_name && var_name[0]) {
                    lsm_scope_bind(ctx->current_scope, var_name, base_type);
                }
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (var_name) {
                        const LSMType *vt = base_type;
                        for (int d = 0; d < ptr_depth; d++)
                            vt = lsm_type_pointer(ctx->arena, vt);
                        lsm_scope_bind(ctx->current_scope, var_name, vt);
                    }
                }
                ...
                while (!ts_node_is_null(inner)) {
                    ...
                    if (strcmp(inner_kind, "identifier") == 0) {
                        if (saw_pointer) {
                            char *var_name = c_node_text(ctx, inner);
                            if (var_name && var_name[0]) {
                                const LSMType *var_type = lsm_type_pointer(ctx->arena, base_type);
                                lsm_scope_bind(ctx->current_scope, var_name, var_type);
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (var_name) {
                        lsm_scope_bind(ctx->current_scope, var_name,
                                       lsm_type_slice(ctx->arena, base_type));
                    }
                }
            }
        }
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (strcmp(ck, "identifier") == 0 || strcmp(ck, "qualified_identifier") == 0 ||
                    ...
                    if (enum_text) {
                        ...
                        const LSMRegisteredType *et =
                            lsm_registry_lookup_type(ctx->registry, enum_qn);
                        if (!et && ctx->module_qn) {
                            enum_qn =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, enum_qn);
                            et = lsm_registry_lookup_type(ctx->registry, enum_qn);
                        }
                        if (!et && ctx->current_namespace) {
                            enum_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                        enum_text);
                            et = lsm_registry_lookup_type(ctx->registry, enum_qn);
                        }
    ...
    if (strcmp(kind, "alias_declaration") == 0) {
        ...
        if (!ts_node_is_null(name_node) && !ts_node_is_null(type_node)) {
            ...
            const LSMType *target = c_parse_type_node(ctx, type_node);
            if (alias_name && target) {
                // Build alias QN
                const char *alias_qn =
                    ctx->current_namespace
                        ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, alias_name)
                        : lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                lsm_scope_bind(ctx->current_scope, alias_name,
                               lsm_type_alias(ctx->arena, alias_qn, target));
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "type_definition") == 0) {
        ...
        const LSMType *target = lsm_type_unknown();
        uint32_t nc = ts_node_named_child_count(node);
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (!found_type && strcmp(ck, "type_identifier") != 0 &&
                ...
                if (alias_name) {
                    const char *alias_qn =
                        ctx->current_namespace
                            ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace,
                                                alias_name)
                            : lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, alias_name);
                    lsm_scope_bind(ctx->current_scope, alias_name,
                                   lsm_type_alias(ctx->arena, alias_qn, target));
    ...
    if (strcmp(kind, "for_range_loop") == 0) {
        ...
        const LSMType *elem_type = lsm_type_unknown();
        if (!ts_node_is_null(right)) {
            const LSMType *container_type = c_eval_expr_type(ctx, right);
            // Deduce element type: container's begin()->operator* or element type
            if (container_type) {
                if (container_type->kind == LSM_TYPE_SLICE) {
                    elem_type = container_type->data.slice.elem;
                } else if (container_type->kind == LSM_TYPE_TEMPLATE &&
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (var_name)
                    lsm_scope_bind(ctx->current_scope, var_name, elem_type);
            } else if (strcmp(dk, "structured_binding_declarator") == 0) {
                ...
                for (uint32_t bi = 0; bi < bnc; bi++) {
                    ...
                    if (strcmp(ts_node_type(bn), "identifier") == 0) {
                        ...
                        if (!bname)
                            continue;
                        const LSMType *bt = lsm_type_unknown();
                        // TEMPLATE decomposition (e.g., pair<K,V>)
                        if (elem_type && elem_type->kind == LSM_TYPE_TEMPLATE &&
                        ...
                        // SLICE decomposition
                        if (lsm_type_is_unknown(bt) && elem_type &&
                            elem_type->kind == LSM_TYPE_SLICE) {
                        ...
                        if (lsm_type_is_unknown(bt) && elem_type) {
                            ...
                            const LSMRegisteredType *rt =
                                eq ? lsm_registry_lookup_type(ctx->registry, eq) : NULL;
                            if (!rt && eq && ctx->module_qn)
                                rt = lsm_registry_lookup_type(
                                    ctx->registry,
                                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, eq));
                            if (rt && rt->field_types && rt->field_types[binding_idx])
                                bt = rt->field_types[binding_idx];
                        }
                        lsm_scope_bind(ctx->current_scope, bname, bt);
                        binding_idx++;
                    }
                }
            }
    ...
    if (strcmp(kind, "parameter_declaration") == 0) {
        ...
        const LSMType *param_type = c_parse_type_node(ctx, type_node);
        ...
        if (!ts_node_is_null(decl)) {
            ...
            if (strcmp(dk, "identifier") == 0) {
                ...
                if (name)
                    lsm_scope_bind(ctx->current_scope, name, param_type);
            } else if (strcmp(dk, "pointer_declarator") == 0) {
                int ptr_depth = 0;
                TSNode inner = decl;
                while (!ts_node_is_null(inner) &&
                ...
                if (!ts_node_is_null(inner) && strcmp(ts_node_type(inner), "identifier") == 0) {
                    ...
                    if (name) {
                        const LSMType *vt = param_type;
                        for (int d = 0; d < ptr_depth; d++)
                            vt = lsm_type_pointer(ctx->arena, vt);
                        lsm_scope_bind(ctx->current_scope, name, vt);
                    }
                }
            } else if (strcmp(dk, "reference_declarator") == 0) {
                if (ts_node_named_child_count(decl) > 0) {
                    ...
                    if (strcmp(ts_node_type(inner), "identifier") == 0) {
                        char *name = c_node_text(ctx, inner);
                        if (name)
                            lsm_scope_bind(ctx->current_scope, name,
                                           lsm_type_reference(ctx->arena, param_type));
                    }
                }
            }
        }
...
static int c_operator_param_count(const LSMRegisteredFunc *candidate) {
    if (!candidate || !candidate->signature || candidate->signature->kind != LSM_TYPE_FUNC) {
        return -1;
    }
    int count = 0;
    const LSMType *const *params = candidate->signature->data.func.param_types;
...
static bool c_operator_signature_matches(CLSPContext *ctx, const LSMRegisteredFunc *candidate,
                                         const LSMType *const *actuals, int actual_count) {
    if (!candidate || candidate->receiver_type || !candidate->signature ||
        candidate->signature->kind != LSM_TYPE_FUNC || actual_count < 0 ||
        c_operator_param_count(candidate) != actual_count) {
        return false;
    }
    const LSMType *const *params = candidate->signature->data.func.param_types;
    for (int i = 0; i < actual_count; i++) {
        const LSMType *expected = c_simplify_type(ctx, params[i], false);
        const LSMType *actual = c_simplify_type(ctx, actuals[i], false);
...
static const LSMRegisteredFunc *c_lookup_free_operator(CLSPContext *ctx, const char *operator_name,
                                                       const LSMType *left_type,
                                                       const LSMType *right_type) {
    ...
    const LSMRegisteredFunc *match = NULL;
    LSMFreeFuncIter it;
    lsm_registry_free_funcs_by_short_name(ctx->registry, operator_name, &it);
    for (int index = lsm_free_func_iter_next(&it); index >= 0;
         index = lsm_free_func_iter_next(&it)) {
        if (index >= ctx->registry->func_count)
            continue;
        const LSMRegisteredFunc *candidate = &ctx->registry->funcs[index];
        ...
        const LSMType *actuals[2] = {left_type, right_type};
        bool signature_matches = c_operator_signature_matches(ctx, candidate, actuals, 2);
        if (ctx->debug) {
            const LSMType *const *params =
                candidate->signature && candidate->signature->kind == LSM_TYPE_FUNC
...
static const LSMRegisteredFunc *c_lookup_free_unary_operator(CLSPContext *ctx,
                                                             const char *operator_name,
                                                             const LSMType *operand_type) {
    ...
    const LSMRegisteredFunc *match = NULL;
    const LSMType *actuals[1] = {operand_type};
    LSMFreeFuncIter it;
    lsm_registry_free_funcs_by_short_name(ctx->registry, operator_name, &it);
    for (int index = lsm_free_func_iter_next(&it); index >= 0;
         index = lsm_free_func_iter_next(&it)) {
...
static const LSMRegisteredFunc *c_lookup_member_operator_by_arity(CLSPContext *ctx,
                                                                  const char *type_qn,
                                                                  const char *operator_name,
    ...
    const LSMRegisteredFunc *candidate = lsm_registry_lookup_method_by_args(
        ctx->registry, type_qn, operator_name, explicit_arg_count);
    if (candidate && c_operator_param_count(candidate) == explicit_arg_count) {
        return candidate;
    }
    if (ctx->module_qn) {
        const char *prefixed = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, type_qn);
        candidate = lsm_registry_lookup_method_by_args(ctx->registry, prefixed, operator_name,
                                                       explicit_arg_count);
        if (candidate && c_operator_param_count(candidate) == explicit_arg_count) {
            return candidate;
        }
...
static void c_emit_resolved_reference_at(CLSPContext *ctx, const char *callee_qn,
    ...
    resolved.kind = LSM_RESOLVED_CALL_REFERENCE;
    ...
    lsm_resolvedcall_push(ctx->resolved_calls, ctx->arena, resolved);
}
...
static void c_resolve_callable_argument_references(CLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (!target && lsm_scope_contains(ctx->current_scope, name)) {
            continue;
        }
        if (!target) {
 ...
 * lsm_type_substitute under hundreds of recursive c_resolve_calls_in_node
...
static void c_resolve_calls_in_node_inner(CLSPContext *ctx, TSNode node) {
        ...
        if (!ts_node_is_null(func_node)) {
            ...
            if (strcmp(fk, "field_expression") == 0) {
                ...
                if (!ts_node_is_null(arg_node) && !ts_node_is_null(field_node)) {
                    ...
                    if (field_name) {
                        ...
                        const LSMType *base = is_arrow ? c_simplify_type(ctx, obj_type, true)
                        ...
                        if (type_qn) {
                            ...
                            const LSMRegisteredFunc *method = lsm_registry_lookup_method_by_types(
                            ...
                            if (method) {
                                ...
                                if (method->receiver_type &&
                                    ...
                                    const LSMRegisteredFunc *override_m =
                                        lsm_registry_lookup_method(ctx->registry, type_qn,
                                                                   field_name);
                                    if (override_m) {
                                        method = override_m;
                                ...
                                if (is_arrow && obj_type->kind == LSM_TYPE_TEMPLATE &&
                        ...
                        if (base && base->kind == LSM_TYPE_TYPE_PARAM && ctx->enclosing_func_qn &&
                            ctx->in_template) {
                            int ac = 0;
                        ...
                        if (lsm_type_is_unknown(obj_type)) {
                            ...
                            c_emit_unresolved_call(
                                ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, field_name),
                                "method_not_found");
                        }
                    }
                }
                ...
                if (text) {
                    ...
                    if (angle) {
                        ...
                        if (!dot_after) {
                            ...
                            char *stripped = (char *)lsm_arena_alloc(ctx->arena, strip_len + 1);
                            ...
                            if (close && close < dot_after) {
                                ...
                                char *stripped = (char *)lsm_arena_alloc(
                    ...
                    int arg_count = 0;
                    const LSMType **arg_types = c_extract_call_arg_types(ctx, node, &arg_count);
                    // Module-prefixed first (shadows stdlib stubs), then bare QN
                    const LSMRegisteredFunc *f = NULL;
                    if (ctx->module_qn) {
                        f = lsm_registry_lookup_func(
                            ctx->registry,
                            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn));
                    }
                    if (!f)
                        f = lsm_registry_lookup_func(ctx->registry, qn);
                    if (f) {
                    ...
                    if (dot) {
                        ...
                        char *class_qn = (char *)lsm_arena_alloc(ctx->arena, prefix_len + 1);
                        memcpy(class_qn, qn, prefix_len);
                        class_qn[prefix_len] = '\0';
                        const LSMRegisteredFunc *m = lsm_registry_lookup_method_by_types(
                            ctx->registry, class_qn, dot + 1, arg_types, arg_count);
                        // Try with module prefix
                        if (!m && ctx->module_qn) {
                            const char *mod_class =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_qn);
                            m = lsm_registry_lookup_method_by_types(ctx->registry, mod_class,
                    ...
                    if (dot && ctx->module_qn) {
                        ...
                        const LSMRegisteredFunc *nf =
                            lsm_registry_lookup_func(ctx->registry, bare_name);
                        if (!nf) {
                            nf = lsm_registry_lookup_func(
                                ctx->registry,
                                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare_name));
                        }
                        if (nf) {
                    ...
                    if (name) {
                        ...
                        if (strcmp(nk, "qualified_identifier") == 0 ||
                            ...
                            if (!f) {
                                ...
                                if (dot) {
                                    size_t plen = dot - qn;
                                    char *cls = (char *)lsm_arena_alloc(ctx->arena, plen + 1);
                                    memcpy(cls, qn, plen);
                                    cls[plen] = '\0';
                                    const LSMRegisteredFunc *m =
                                        lsm_registry_lookup_method(ctx->registry, cls, dot + 1);
                                    if (!m && ctx->module_qn) {
                                        const char *mc = lsm_arena_sprintf(ctx->arena, "%s.%s",
                                                                           ctx->module_qn, cls);
                                        m = lsm_registry_lookup_method(ctx->registry, mc, dot + 1);
                                    }
                                    if (m)
                                        f = m;
                                }
                        ...
                        if (f) {
                            ...
                            if (ctx->pending_tc_count > 0 && f->type_param_names) {
                                int ac = 0;
                                const LSMType **at = c_extract_call_arg_types(ctx, node, &ac);
                                if (at)
            ...
            if (strcmp(fk, "identifier") == 0) {
                ...
                if (name && !is_c_builtin_func(name)) {
                    ...
                    const LSMType *var_type = lsm_scope_lookup(ctx->current_scope, name);
                    if (!lsm_type_is_unknown(var_type)) {
                        if (var_type->kind == LSM_TYPE_FUNC) {
                            // Function type variable without tracked target — can't resolve
                            goto recurse;
                        }
                        // Functor call: variable with operator()
                        const LSMType *base = c_simplify_type(ctx, var_type, false);
                        const char *type_qn = type_to_qn(base);
                        if (type_qn) {
                            const LSMRegisteredFunc *op =
                    ...
                    if (type && type->kind == LSM_TYPE_NAMED) {
                        ...
                        const LSMRegisteredType *rt =
                            lsm_registry_lookup_type(ctx->registry, type_qn_str);
                        if (rt && !rt->is_interface) {
                            // Constructor call
                            const char *ctor_qn =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn_str, name);
                    ...
                    if (fqn) {
                        ...
                        if (ctx->enclosing_class_qn) {
                            const LSMRegisteredFunc *m = lsm_registry_lookup_method(
                        ...
                        if (ctx->pending_tc_count > 0) {
                            const LSMRegisteredFunc *called =
                                lsm_registry_lookup_func(ctx->registry, fqn);
    ...
    if (strcmp(kind, "declaration") == 0 && ctx->cpp_mode) {
        ...
        uint32_t nc = ts_node_named_child_count(node);
        const LSMType *decl_type = lsm_type_unknown();
        bool has_type = false;
        for (uint32_t i = 0; i < nc; i++) {
            TSNode child = ts_node_named_child(node, i);
            const char *ck = ts_node_type(child);
                ...
                if (!ts_node_is_null(value)) {
                    ...
                    if (strcmp(vk, "argument_list") == 0 || strcmp(vk, "initializer_list") == 0) {
                        ...
                        if (type_qn) {
                            ...
                            const char *ctor_qn =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
    ...
    if (strcmp(kind, "new_expression") == 0) {
        ...
        if (!ts_node_is_null(type_node)) {
            const LSMType *t = c_parse_type_node(ctx, type_node);
            const char *type_qn = type_to_qn(t);
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *ctor_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, short_name);
                c_emit_resolved_call_at(ctx, ctor_qn, "lsp_constructor", 0.95f, node);
            }
        }
    }
    ...
    if (strcmp(kind, "delete_expression") == 0 && ctx->cpp_mode) {
        ...
        if (!ts_node_is_null(operand)) {
            const LSMType *ptr_type = c_eval_expr_type(ctx, operand);
            const LSMType *base = c_simplify_type(ctx, ptr_type, true);
            const char *type_qn = type_to_qn(base);
            if (type_qn) {
                const char *short_name = strrchr(type_qn, '.');
                short_name = short_name ? short_name + 1 : type_qn;
                const char *dtor_name = lsm_arena_sprintf(ctx->arena, "~%s", short_name);
                const LSMRegisteredFunc *dtor =
    ...
    if (ctx->cpp_mode && strcmp(kind, "binary_expression") == 0) {
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right)) {
            const LSMType *lhs_type = c_eval_expr_type(ctx, left);
            const LSMType *rhs_type = c_eval_expr_type(ctx, right);
            const LSMType *base = c_simplify_type(ctx, lhs_type, false);
            ...
            for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
                ...
                if (!ts_node_is_named(child)) {
                    char *op = c_node_text(ctx, child);
                    if (op) {
                        const char *op_name = lsm_arena_sprintf(ctx->arena, "operator%s", op);
                        const LSMRegisteredFunc *m = NULL;
                        if (base && base->kind != LSM_TYPE_BUILTIN && !lsm_type_is_unknown(base)) {
                            const char *type_qn = type_to_qn(base);
                            if (type_qn) {
    ...
    if (ctx->cpp_mode && strcmp(kind, "field_expression") == 0) {
        ...
        if (is_arrow && !ts_node_is_null(receiver) && !ts_node_is_null(member)) {
            const LSMType *receiver_type = c_eval_expr_type(ctx, receiver);
            const LSMType *base = c_simplify_type(ctx, receiver_type, false);
            if (base && base->kind != LSM_TYPE_BUILTIN && base->kind != LSM_TYPE_POINTER &&
                !lsm_type_is_unknown(base)) {
                const char *type_qn = type_to_qn(base);
    ...
    if (ctx->cpp_mode && strcmp(kind, "subscript_expression") == 0) {
        ...
        if (!ts_node_is_null(arg_node)) {
            const LSMType *arr_type = c_eval_expr_type(ctx, arg_node);
            const LSMType *base = c_simplify_type(ctx, arr_type, false);
            if (base && base->kind != LSM_TYPE_BUILTIN && !lsm_type_is_unknown(base) &&
                base->kind != LSM_TYPE_POINTER && base->kind != LSM_TYPE_SLICE) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    const LSMRegisteredFunc *m =
    ...
    if (ctx->cpp_mode &&
        ...
        if (!ts_node_is_null(operand)) {
            const LSMType *op_type = c_eval_expr_type(ctx, operand);
            const LSMType *base = c_simplify_type(ctx, op_type, false);
            if (base && base->kind != LSM_TYPE_BUILTIN && !lsm_type_is_unknown(base) &&
                base->kind != LSM_TYPE_POINTER) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    ...
                    if (op_name) {
                        ...
                        const LSMRegisteredFunc *m = c_lookup_member_operator_by_arity(
    ...
    if (ctx->cpp_mode && strcmp(kind, "declaration") == 0) {
        const LSMType *decl_type = c_parse_declaration_type(ctx, node);
        if (decl_type && decl_type->kind == LSM_TYPE_NAMED) {
                ...
                if (strcmp(ts_node_type(dchild), "init_declarator") == 0) {
                    ...
                    if (!ts_node_is_null(val)) {
                        ...
                        if (strcmp(vk, "argument_list") != 0 &&
                            ...
                            const LSMType *val_type = c_eval_expr_type(ctx, val);
                            const LSMType *val_base = c_simplify_type(ctx, val_type, false);
    ...
    if (ctx->cpp_mode &&
        ...
        if (!ts_node_is_null(cond)) {
            ...
            const LSMType *cond_type = c_eval_expr_type(ctx, cond);
            const LSMType *base = c_simplify_type(ctx, cond_type, false);
            if (base && base->kind != LSM_TYPE_BUILTIN && !lsm_type_is_unknown(base) &&
                base->kind != LSM_TYPE_POINTER) {
                const char *type_qn = type_to_qn(base);
                if (type_qn) {
                    const LSMRegisteredFunc *m = c_lookup_member(ctx, type_qn, "operator bool");
                    if (m) {
                ...
                for (uint32_t ci = 0; ci < cnc; ci++) {
                    ...
                    if (strcmp(ck, "lambda_capture_initializer") == 0) {
                        ...
                        if (!ts_node_is_null(name_node) && !ts_node_is_null(val_node)) {
                            ...
                            const LSMType *cap_type = c_eval_expr_type(ctx, val_node);
                            if (cap_name && !lsm_type_is_unknown(cap_type)) {
                                lsm_scope_bind(ctx->current_scope, cap_name, cap_type);
                            }
                        }
                    }
                }
...
static void c_process_function(CLSPContext *ctx, TSNode func_node) {
        ...
        if (strcmp(dk, "qualified_identifier") == 0 || strcmp(dk, "scoped_identifier") == 0) {
            ...
            if (!ts_node_is_null(scope_node)) {
                ...
                if (scope_text) {
                    ...
                    const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, scope_qn);
                    if (rt) {
                        ctx->enclosing_class_qn = scope_qn;
                    } else if (ctx->module_qn) {
                        const char *fqn =
                            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, scope_qn);
                        rt = lsm_registry_lookup_type(ctx->registry, fqn);
                        if (rt)
    ...
    // lsm_pipeline_find_lsp_resolution never joined them — every in-method call
    ...
    if (ctx->enclosing_class_qn && saved_class_qn == ctx->enclosing_class_qn &&
        !strchr(func_qn, '.')) {
        func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, func_qn);
        ...
        func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, dot + 1);
        ...
        if (scope) {
            func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", scope, func_qn);
        }
    }
    ...
    if (ctx->in_template && ctx->template_param_count > 0 && !ctx->registry_shared) {
        ...
        for (int ri = 0; ri < ((LSMTypeRegistry *)ctx->registry)->func_count; ri++) {
            LSMRegisteredFunc *rf = &((LSMTypeRegistry *)ctx->registry)->funcs[ri];
            if (strcmp(rf->qualified_name, func_qn) == 0 && !rf->type_param_names) {
                const char **tpn = (const char **)lsm_arena_alloc(
    ...
    if (ctx->enclosing_class_qn) {
        lsm_scope_bind(
            ...
            lsm_type_pointer(ctx->arena, lsm_type_named(ctx->arena, ctx->enclosing_class_qn)));
    }
...
static void c_process_body_child(CLSPContext *ctx, TSNode child) {
    ...
    if (strcmp(ck, "function_definition") == 0) {
                        ...
                        if (fname && fname[0]) {
                            ...
                            for (uint32_t ri = 0; ri < dnc; ri++) {
                                ...
                                if (strcmp(rk, "function_declarator") != 0 &&
                                    ...
                                    if (!lsm_type_is_unknown(ret_type))
                                        break;
                                }
                            }
                            if (ret_type && !lsm_type_is_unknown(ret_type)) {
                                if (is_ref_ret)
                                    ret_type = lsm_type_reference(ctx->arena, ret_type);
                                if (is_ptr_ret)
                                    ret_type = lsm_type_pointer(ctx->arena, ret_type);
                                const char *func_qn = c_build_qn(ctx, fname);
                                if (ctx->module_qn && !strchr(func_qn, '.'))
                                    func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
                                                                func_qn);
                                // Use namespace if available
                                if (ctx->current_namespace &&
                                    !strstr(func_qn, ctx->current_namespace))
                                    func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s",
                                ...
                                if (!ctx->registry_shared &&
                                    !lsm_registry_lookup_func(ctx->registry, func_qn)) {
                                    const LSMType **rets = (const LSMType **)lsm_arena_alloc(
                                        ctx->arena, 2 * sizeof(const LSMType *));
                                    rets[0] = ret_type;
                                    rets[1] = NULL;
                                    LSMRegisteredFunc rf = {0};
                                    rf.qualified_name = func_qn;
                                    rf.short_name = fname;
                                    rf.signature = lsm_type_func(ctx->arena, NULL, NULL, rets);
                                    ...
                                    lsm_registry_add_func((LSMTypeRegistry *)ctx->registry, rf);
                                }
                            }
                        }
                    }
        ...
        const LSMType **saved_tpd = ctx->template_param_defaults;
...
static void c_process_namespace(CLSPContext *ctx, TSNode ns_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (ns_name) {
            if (saved_ns) {
                ctx->current_namespace = lsm_arena_sprintf(ctx->arena, "%s.%s", saved_ns, ns_name);
            } else if (ctx->module_qn) {
                ctx->current_namespace =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ns_name);
            } else {
...
static void c_process_class(CLSPContext *ctx, TSNode class_node) {
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (class_name) {
            ...
            if (ctx->current_namespace) {
                class_qn =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace, class_name);
            } else if (ctx->module_qn) {
                class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, class_name);
            } else {
                class_qn = class_name;
            }
            ...
            if (ctx->in_template && ctx->template_param_names && ctx->template_param_count > 0) {
                ...
                for (int ri = 0; ri < ((LSMTypeRegistry *)ctx->registry)->type_count; ri++) {
                    if (strcmp(((LSMTypeRegistry *)ctx->registry)->types[ri].qualified_name,
                               class_qn) == 0) {
                        rt = &((LSMTypeRegistry *)ctx->registry)->types[ri];
                        break;
                    }
                }
                if (rt && !rt->type_param_names) {
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (ctx->enclosing_class_qn) {
            uint32_t bkn = 0;
            TSNode *bkids = lsm_lsp_collect_children(ctx->arena, body, &bkn);
            for (uint32_t i = 0; i < bkn; i++) {
                ...
                const LSMType *ret_type = c_parse_declaration_type(ctx, child);
                ...
                for (uint32_t di = 0; di < dnc; di++) {
                    ...
                    // function_declarator
                    TSNode cur = decl;
                    const LSMType *actual_ret = ret_type;
                    for (int depth = 0; depth < 5 && !ts_node_is_null(cur); depth++) {
                        ...
                        if (strcmp(dk, "function_declarator") == 0) {
                            ...
                            if (!method_name || !method_name[0])
                                break;
                            const char *method_qn = lsm_arena_sprintf(
                            ...
                            const LSMRegisteredFunc *existing = lsm_registry_lookup_method(
                            ...
                            if (!existing && ctx->module_qn) {
                                ...
                                if (strncmp(class_qn, ctx->module_qn, mod_len) == 0 &&
                                    class_qn[mod_len] == '.') {
                                    existing = lsm_registry_lookup_method(
                            ...
                            if (existing) {
                                ...
                                if (existing->signature &&
                                    existing->signature->kind == LSM_TYPE_FUNC &&
                                    ...
                                    int nk2 = actual_ret ? actual_ret->kind : LSM_TYPE_UNKNOWN;
                                    // Upgrade if existing is NAMED and new is more specific
                                    if (ek == LSM_TYPE_NAMED &&
                                        (nk2 == LSM_TYPE_TEMPLATE || nk2 == LSM_TYPE_TYPE_PARAM ||
                                         nk2 == LSM_TYPE_POINTER || nk2 == LSM_TYPE_REFERENCE))
                                        should_upgrade = true;
                                }
                                if (should_upgrade) {
                                    ...
                                    new_rets[1] = NULL;
                                    LSMRegisteredFunc *mut = (LSMRegisteredFunc *)existing;
                                    mut->signature = lsm_type_func_replace_returns(
                            ...
                            actual_ret = lsm_type_reference(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else if (strcmp(dk, "pointer_declarator") == 0) {
                            actual_ret = lsm_type_pointer(ctx->arena, actual_ret);
                            cur = ts_node_named_child_count(cur) > 0 ? ts_node_named_child(cur, 0)
                                                                     : (TSNode){0};
                        } else {
                            break;
...
// ============================================================================
// Parse C/C++ return type text into LSMType (for cross-file defs)
// ============================================================================

static const LSMType *c_parse_return_type_text(LSMArena *a, const char *text,
    ...
    size_t len = strlen(text);
    if (len > 0 && text[len - 1] == '*') {
        char *inner = lsm_arena_strndup(a, text, len - 1);
        // Trim trailing space
        size_t ilen = strlen(inner);
        while (ilen > 0 && inner[ilen - 1] == ' ')
            inner[--ilen] = '\0';
        return lsm_type_pointer(a, c_parse_return_type_text(a, inner, module_qn));
    }
    ...
    if (len > 0 && text[len - 1] == '&') {
        ...
        if (ilen > 0 && inner[ilen - 1] == '&') {
            ...
            while (ilen > 1 && inner[ilen - 2] == ' ')
                inner[--ilen - 1] = '\0';
            return lsm_type_rvalue_ref(a, c_parse_return_type_text(a, inner, module_qn));
        }
        return lsm_type_reference(a, c_parse_return_type_text(a, inner, module_qn));
    }

    // decltype(expr) — extract the type from the expression
    if (strncmp(text, "decltype(", 9) == 0 && len > 10 && text[len - 1] == ')') {
        char *inner = lsm_arena_strndup(a, text + 9, len - 10);
        size_t ilen = strlen(inner);
    ...
    if (is_c_builtin_type(text))
        return lsm_type_builtin(a, text);
    ...
    if (len == 1 &&
        ...
        return lsm_type_type_param(a, text);
    }
    ...
    // Template type: Name<Arg1, Arg2, ...>
    {
        ...
        if (angle && len > 0 && text[len - 1] == '>') {
            ...
            char *tmpl_name = lsm_arena_strndup(a, text, name_len);
            ...
            char *args_text = lsm_arena_strndup(a, args_start, args_len);

            // Split by comma at nesting depth 0
            const LSMType *arg_types[16];
            int arg_count = 0;
            int depth = 0;
            ...
            for (const char *p = args_text;; p++) {
                ...
                if ((*p == ',' && depth == 0) || *p == '\0') {
                    if (arg_count < 16) {
                        ...
                        char *arg = lsm_arena_strndup(a, arg_begin, alen);
                        // Trim whitespace
                        while (*arg == ' ')
                            arg++;
            ...
            if (!strchr(tmpl_name, '.') && module_qn && module_qn[0]) {
                qname = lsm_arena_sprintf(a, "%s.%s", module_qn, tmpl_name);
            }
            return lsm_type_template(a, qname, arg_types, arg_count);
        }
    }
    ...
    if (strstr(text, "::")) {
        // Replace :: with .
        char *qn = lsm_arena_strdup(a, text);
        for (char *p = qn; *p; p++) {
            if (p[0] == ':' && p[1] == ':') {
                p[0] = '.';
                memmove(p + 1, p + 2, strlen(p + 2) + 1);
 ...
 * before `count` and never leave an interior hole in the LSMType array. */
static const LSMType **c_parse_signature_param_types(LSMArena *arena, const char **type_texts,
...
static void c_rewrite_proved_destructor_candidates(LSMFileResult *result) {
    if (!result) {
        return;
    }
    for (int ci = 0; ci < result->calls.count; ci++) {
        LSMCall *call = &result->calls.items[ci];
        ...
        for (int ri = 0; ri < result->resolved_calls.count; ri++) {
            const LSMResolvedCall *resolved = &result->resolved_calls.items[ri];
...
void lsm_run_c_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
                   TSNode root, bool cpp_mode, LSMSourceOrigin source_origin) {

    LSMTypeRegistry reg;
    lsm_registry_init(&reg, arena);

    // Register stdlib
    lsm_c_stdlib_register(&reg, arena);
    if (cpp_mode)
        lsm_cpp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0)) {
            ...
            if (d->base_classes) {
                ...
                while (d->base_classes[bc_count])
                    bc_count++;
                if (bc_count > 0) {
                    const char **embedded = (const char **)lsm_arena_alloc(
        ...
        if (d->label && strcmp(d->label, "Field") == 0 && d->parent_class && d->return_type) {
            ...
            LSMRegisteredType *parent_rt = NULL;
            ...
            if (parent_rt) {
                ...
                const char **new_fnames =
                    (const char **)lsm_arena_alloc(arena, (existing + 2) * sizeof(const char *));
                const LSMType **new_ftypes = (const LSMType **)lsm_arena_alloc(
                    arena, (existing + 2) * sizeof(const LSMType *));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_type && d->return_type[0]) {
                ...
                while (d->return_types[count])
                    count++;
                if (count > 0) {
                    ret_types = (const LSMType **)lsm_arena_alloc(
                        arena, (count + 1) * sizeof(const LSMType *));
            ...
            const LSMType **param_types_arr = c_parse_signature_param_types(
                arena, d->signature_param_types, d->signature_param_count, module_qn);

            rf.signature = lsm_type_func(arena, NULL, param_types_arr, ret_types);

            // Method receiver
            if (strcmp(d->label, "Method") == 0 && d->parent_class) {
                rf.receiver_type = d->parent_class;
                // Auto-create type if needed
                if (!lsm_registry_lookup_type(&reg, d->parent_class)) {
...
/* Register one batch of LSMLSPDef[] into a registry. Shared by the
...
static void c_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, const char *module_qn,
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Type") == 0 ||
            ...
            if (d->embedded_types) {
                ...
                while (*src && embed_count < 31) {
                    ...
                    if (sep) {
                        embeds[embed_count++] = lsm_arena_strndup(arena, src, sep - src);
                        src = sep + 1;
                    } else {
                        embeds[embed_count++] = lsm_arena_strdup(arena, src);
                        break;
                    }
                }
                if (embed_count > 0) {
                    const char **arr = (const char **)lsm_arena_alloc(
            ...
            if (d->field_defs) {
                ...
                if (fcount > 0) {
                    const char **fnarr =
                        (const char **)lsm_arena_alloc(arena, (fcount + 1) * sizeof(const char *));
                    const LSMType **ftarr = (const LSMType **)lsm_arena_alloc(
                        arena, (fcount + 1) * sizeof(const LSMType *));
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            if (d->return_types) {
                ...
                if (rcount > 0) {
                    const LSMType **rarr = (const LSMType **)lsm_arena_alloc(
                        arena, (rcount + 1) * sizeof(const LSMType *));
                    for (int j = 0; j < rcount; j++)
                        rarr[j] = rets[j];
                    rarr[rcount] = NULL;
                    rf.signature = lsm_type_func(arena, NULL, NULL, rarr);
                }
            }
            if (!rf.signature)
                rf.signature = lsm_type_func(arena, NULL, NULL, NULL);

            const LSMType **param_types_arr = c_parse_signature_param_types(
                arena, d->signature_param_types, d->signature_param_count, def_module);
            const LSMType **return_types_arr =
                rf.signature ? rf.signature->data.func.return_types : NULL;
            rf.signature = lsm_type_func(arena, NULL, param_types_arr, return_types_arr);
...
LSMTypeRegistry *lsm_c_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    ...
    for (int i = 0; i < def_count; i++) {
        LSMLSPDef *d = &defs[i];
        if (d->lang != LSM_LANG_C && d->lang != LSM_LANG_CPP && d->lang != LSM_LANG_CUDA) {
            continue;
        }
...
void lsm_run_c_lsp_cross_with_registry(LSMArena *arena, const char *source, int source_len,
                                       const char *module_qn, bool cpp_mode, LSMTypeRegistry *reg,
...
void lsm_run_c_lsp_cross(LSMArena *arena, const char *source, int source_len, const char *module_qn,
                         bool cpp_mode, LSMLSPDef *defs, int def_count, const char **include_paths,
...
void lsm_batch_c_lsp_cross(LSMArena *arena, LSMBatchCLSPFile *files, int file_count,
                           LSMResolvedCallArray *out) {
    if (!files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        LSMBatchCLSPFile *file = &files[f];
        ...
        lsm_run_c_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items =
                (LSMResolvedCall *)lsm_arena_alloc(arena, file_out.count * sizeof(LSMResolvedCall));
