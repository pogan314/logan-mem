 ...
 * shape:
 *   1. Build a LSMTypeRegistry from file-local definitions + stdlib +
...
static const LSMType *php_substitute_template(LSMArena *arena, const LSMType *t,
                                              const char *const *param_names,
                                              const LSMType *const *args);
static const LSMType *eval_function_call_type(PHPLSPContext *ctx, TSNode call_node);
static const LSMType *eval_member_call_type(PHPLSPContext *ctx, TSNode call_node);
static const LSMType *eval_object_creation_type(PHPLSPContext *ctx, TSNode node);
...
static const LSMType *resolve_phpdoc_type(PHPLSPContext *ctx, const char *type_text);
...
static char *php_ns_to_dot(LSMArena *a, const char *ns) {
    if (!ns)
        return NULL;
    size_t n = strlen(ns);
    char *out = (char *)lsm_arena_alloc(a, n + 1);
    if (!out)
        return NULL;
...
void php_lsp_add_use(PHPLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **nl = (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nl));
        const char **nq = (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nq));
        int *nk = (int *)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(int));
        if (!nl || !nq || !nk)
            return;
    ...
    ctx->use_target_qns[ctx->use_count] = lsm_arena_strdup(ctx->arena, target_qn);
...
const char *php_resolve_class_name(PHPLSPContext *ctx, const char *name) {
    ...
    char *first;
    if (*sep) {
        first = lsm_arena_strndup(ctx->arena, name, (size_t)(sep - name));
    } else {
        first = lsm_arena_strdup(ctx->arena, name);
    }

    const char *use_target = find_use(ctx, first, LSM_PHP_USE_CLASS);
    if (use_target) {
        if (*sep) {
            /* use App\\Models as M;  M\\User -> App.Models.User */
            return lsm_arena_sprintf(ctx->arena, "%s%s", use_target,
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
...
static const LSMRegisteredType *lookup_type_with_project(PHPLSPContext *ctx, const char *qn) {
    if (!qn)
        return NULL;
    const LSMRegisteredType *t = lsm_registry_lookup_type(ctx->registry, qn);
    if (t)
        return t;

    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *combo = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
        t = lsm_registry_lookup_type(ctx->registry, combo);
        if (t)
            return t;
        ...
        while (last_dot) {
            char *prefix = lsm_arena_strndup(ctx->arena, m, (size_t)(last_dot - m));
            const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", prefix, qn);
            t = lsm_registry_lookup_type(ctx->registry, try_qn);
            if (t)
                return t;
    ...
    const LSMRegisteredType *best = NULL;
    int best_score = -1;
    for (int i = 0; ctx->registry && i < ctx->registry->type_count; i++) {
        const LSMRegisteredType *cand = &ctx->registry->types[i];
...
const LSMRegisteredFunc *php_lookup_method(PHPLSPContext *ctx, const char *class_qn,
    ...
    const LSMRegisteredFunc *f = lsm_registry_lookup_method(ctx->registry, class_qn, method_name);
    if (f)
        return f;

    /* If the QN we have isn't the registry's canonical key, resolve the
     * type to its registered identity and retry. */
    const LSMRegisteredType *t = lsm_registry_lookup_type(ctx->registry, class_qn);
    if (!t)
        t = lookup_type_with_project(ctx, class_qn);
    if (!t)
        return NULL;
    if (strcmp(t->qualified_name, class_qn) != 0) {
        f = lsm_registry_lookup_method(ctx->registry, t->qualified_name, method_name);
        if (f)
            return f;
    }
    ...
    while (frontier_count > 0 && visited_count < 32) {
        ...
        f = lsm_registry_lookup_method(ctx->registry, parent, method_name);
        if (f)
            return f;

        const LSMRegisteredType *next = lsm_registry_lookup_type(ctx->registry, parent);
        if (!next)
        ...
        if (strcmp(next->qualified_name, parent) != 0) {
            f = lsm_registry_lookup_method(ctx->registry, next->qualified_name, method_name);
            if (f)
                return f;
        }
        if (next->method_qns && next->method_names) {
            for (int i = 0; next->method_names[i]; i++) {
                if (strcmp(next->method_names[i], method_name) == 0) {
                    const LSMRegisteredFunc *cand =
                        lsm_registry_lookup_func(ctx->registry, next->method_qns[i]);
                    if (cand)
                        return cand;
                }
            }
...
const LSMType *php_parse_type_node(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return lsm_type_builtin(ctx->arena, t ? t : "mixed");
    }
    ...
    if (strcmp(kind, "union_type") == 0 || strcmp(kind, "intersection_type") == 0 ||
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (ts_node_is_null(c) || !ts_node_is_named(c))
                continue;
            const LSMType *t = php_parse_type_node(ctx, c);
            if (t && t->kind != LSM_TYPE_UNKNOWN) {
                if (t->kind == LSM_TYPE_BUILTIN && strcmp(t->data.builtin.name, "null") == 0) {
                    continue;
                }
                return t;
            }
    ...
    if (php_is_builtin_type_name(t))
        return lsm_type_builtin(ctx->arena, t);
    const char *resolved = php_resolve_class_name(ctx, t);
    if (!resolved)
        return lsm_type_unknown();
    return lsm_type_named(ctx->arena, resolved);
...
static char *phpdoc_clean(LSMArena *a, const char *raw) {
    if (!raw)
        return NULL;
    size_t n = strlen(raw);
    char *out = (char *)lsm_arena_alloc(a, n + 1);
...
static int parse_phpdoc_template_args(PHPLSPContext *ctx, const char *p, const LSMType ***out_args,
    ...
    while (*p && depth > 0) {
        ...
        if (arg_len > 0) {
            char *arg_text = lsm_arena_strndup(ctx->arena, arg_start, arg_len);
            const LSMType *arg_type = resolve_phpdoc_type(ctx, arg_text);
            if (count >= cap) {
                int new_cap = cap * 2;
                const LSMType **new_buf = (const LSMType **)lsm_arena_alloc(
...
static void php_add_phpstan_alias(PHPLSPContext *ctx, const char *name, const LSMType *type) {
    if (!ctx || !name || !type)
        return;
    ...
    if (ctx->phpstan_alias_count >= ctx->phpstan_alias_cap) {
        ...
        const char **nn =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const LSMType **nt =
            (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < ctx->phpstan_alias_count; i++) {
    ...
    ctx->phpstan_alias_names[ctx->phpstan_alias_count] = lsm_arena_strdup(ctx->arena, name);
    ctx->phpstan_alias_types[ctx->phpstan_alias_count] = type;
    ctx->phpstan_alias_count++;
}

static const LSMType *php_lookup_phpstan_alias(PHPLSPContext *ctx, const char *name) {
    if (!ctx || !name)
        return NULL;
    for (int i = 0; i < ctx->phpstan_alias_count; i++) {
        if (strcmp(ctx->phpstan_alias_names[i], name) == 0) {
...
static const LSMType *resolve_phpdoc_type(PHPLSPContext *ctx, const char *type_text) {
    ...
    if (end == 0)
        return lsm_type_unknown();
    char *first = lsm_arena_strndup(ctx->arena, type_text, end);
    if (!first)
        return lsm_type_unknown();
    ...
    if (has_array_shape)
        return lsm_type_builtin(ctx->arena, "array");

    /* @phpstan-type alias takes precedence over name resolution. */
    {
        const LSMType *aliased = php_lookup_phpstan_alias(ctx, first);
        if (aliased)
            return aliased;
    }
    ...
    if (php_is_builtin_type_name(first)) {
        if (has_generic) {
             ...
             * later. */
            const LSMType **args = NULL;
            const char *after = NULL;
            int n = parse_phpdoc_template_args(ctx, type_text + end + 1, &args, &after);
            return lsm_type_template(ctx->arena, first, args, n);
        }
        return lsm_type_builtin(ctx->arena, first);
    }
    ...
    if (has_generic) {
        ...
        const char *after = NULL;
        int n = parse_phpdoc_template_args(ctx, type_text + end + 1, &args, &after);
        return lsm_type_template(ctx->arena, resolved, args, n);
    }
    return lsm_type_named(ctx->arena, resolved);
}
...
static void parse_phpdoc_for_params(PHPLSPContext *ctx, const char *docstring, TSNode params) {
    ...
    while ((p = strstr(p, "@param")) != NULL) {
        ...
        if (p == type_start)
            continue;
        char *type_text = lsm_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
        while (*p == ' ' || *p == '\t')
            p++;
        if (*p != '$')
            continue;
        ...
        if (p == name_start)
            continue;
        char *vname = lsm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const LSMType *t = resolve_phpdoc_type(ctx, type_text);
        if (vname && t && t->kind != LSM_TYPE_UNKNOWN) {
            lsm_scope_bind(ctx->current_scope, vname, t);
        }
    }
}
...
const LSMType *php_eval_expr_type(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "variable_name") == 0) {
        ...
        if (t) {
            ...
            if (strcmp(name, "this") == 0) {
                ...
                const LSMType *bound = lsm_scope_lookup(ctx->current_scope, name);
                if (bound)
                    result = bound;
            }
        }
        ...
        result = lsm_type_named(ctx->arena, "Closure");
        ...
        if (!ts_node_is_null(lhs) && !ts_node_is_null(rhs)) {
            ...
            if (cls_text && member) {
                ...
                if (cls_qn) {
                    const LSMRegisteredType *t = lsm_registry_lookup_type(ctx->registry, cls_qn);
                    if (!t)
        ...
        if (!ts_node_is_null(obj)) {
            const LSMType *recv = php_eval_expr_type(ctx, obj);
            if (recv && recv->kind == LSM_TYPE_TEMPLATE) {
                const LSMType *const *args = recv->data.template_type.template_args;
                if (args) {
                    int n = 0;
                    while (args[n])
                        n++;
            ...
            } else if (recv && recv->kind == LSM_TYPE_NAMED) {
                /* offsetGet on a registered ArrayAccess-like class. */
                const LSMRegisteredFunc *og =
                    php_lookup_method(ctx, recv->data.named.qualified_name, "offsetGet");
                if (og && og->signature && og->signature->kind == LSM_TYPE_FUNC &&
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(k, "match_block") == 0) {
                ...
                for (uint32_t j = 0; j < bnc; j++) {
                    ...
                    if (ts_node_is_null(val))
                        continue;
                    const LSMType *t = php_eval_expr_type(ctx, val);
                    if (t && t->kind != LSM_TYPE_UNKNOWN) {
                        result = t;
                        goto match_done;
                    }
    ...
    match_done:;
        ...
        if (!ts_node_is_null(then_n)) {
            const LSMType *t = php_eval_expr_type(ctx, then_n);
            if (t && t->kind != LSM_TYPE_UNKNOWN) {
                result = t;
            }
        }
        if ((!result || result->kind == LSM_TYPE_UNKNOWN) && !ts_node_is_null(else_n)) {
            const LSMType *t = php_eval_expr_type(ctx, else_n);
            if (t && t->kind != LSM_TYPE_UNKNOWN)
                result = t;
        }
        ...
        result = lsm_type_builtin(ctx->arena, "float");
               ...
               strcmp(kind, "false") == 0) {
        result = lsm_type_builtin(ctx->arena, "bool");
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(field)) {
            ...
            if (recv && recv->kind == LSM_TYPE_NAMED) {
                ...
                const LSMRegisteredType *t =
                    lsm_registry_lookup_type(ctx->registry, recv->data.named.qualified_name);
                if (!t)
...
static const LSMType *eval_function_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    const char *target = find_use(ctx, name, LSM_PHP_USE_FUNCTION);
    const char *qn = NULL;
    if (target) {
        qn = target;
    } else if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
    } else {
        qn = lsm_arena_strdup(ctx->arena, name);
    }
    const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, qn);
    if (!f && ctx->module_qn) {
        f = lsm_registry_lookup_func(ctx->registry,
    ...
    if (!f && ctx->registry) {
        ...
        for (int i = 0; i < ctx->registry->func_count; i++) {
            const LSMRegisteredFunc *cand = &ctx->registry->funcs[i];
    ...
    const LSMType *sig = f->signature;
    if (sig->kind == LSM_TYPE_FUNC && sig->data.func.return_types &&
...
static const LSMType *eval_member_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    if (is_static) {
        ...
        const LSMType *recv_type =
            ts_node_is_null(recv_node) ? lsm_type_unknown() : php_eval_expr_type(ctx, recv_node);
        if (recv_type && recv_type->kind == LSM_TYPE_NAMED) {
            class_qn = recv_type->data.named.qualified_name;
        } else if (recv_type && recv_type->kind == LSM_TYPE_TEMPLATE) {
        ...
        if (recv_type && recv_type->kind == LSM_TYPE_TEMPLATE) {
            ...
            const LSMRegisteredFunc *f = php_lookup_method(ctx, class_qn, method_name);
            if (!f || !f->signature)
                return lsm_type_unknown();
            const LSMType *sig = f->signature;
            if (sig->kind == LSM_TYPE_FUNC && sig->data.func.return_types &&
...
static void emit_resolved_kind(PHPLSPContext *ctx, const char *callee_qn, const char *strategy,
                               float confidence, LSMResolvedKind kind) {
...
static void process_assignment(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(name, "this") == 0)
        return;
    const LSMType *rhs_type = php_eval_expr_type(ctx, rhs);
    if (rhs_type && rhs_type->kind != LSM_TYPE_UNKNOWN) {
        /* Don't override a more-specific (NAMED/TEMPLATE) binding with
         * a trivial null literal — preserves `@var T $x; $x = null;`. */
        if (rhs_type->kind == LSM_TYPE_BUILTIN && rhs_type->data.builtin.name &&
            strcmp(rhs_type->data.builtin.name, "null") == 0) {
            const LSMType *existing = lsm_scope_lookup(ctx->current_scope, name);
            if (existing &&
                (existing->kind == LSM_TYPE_NAMED || existing->kind == LSM_TYPE_TEMPLATE)) {
                return;
            }
        }
        lsm_scope_bind(ctx->current_scope, name, rhs_type);
    }
}
...
static void process_foreach(PHPLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(iterable)) {
        const LSMType *it_type = php_eval_expr_type(ctx, iterable);
        if (it_type && it_type->kind == LSM_TYPE_TEMPLATE) {
            const LSMType *const *args = it_type->data.template_type.template_args;
            if (args) {
                int n = 0;
                while (args[n])
                    n++;
        ...
        } else if (it_type && it_type->kind == LSM_TYPE_NAMED) {
            const LSMRegisteredFunc *cur =
                php_lookup_method(ctx, it_type->data.named.qualified_name, "current");
            if (cur && cur->signature && cur->signature->kind == LSM_TYPE_FUNC &&
                cur->signature->data.func.return_types &&
                cur->signature->data.func.return_types[0] &&
                cur->signature->data.func.return_types[0]->kind != LSM_TYPE_UNKNOWN) {
    ...
    for (int i = 0; i < loop_var_count; i++) {
        ...
        if (!t)
            continue;
        const char *name = (t[0] == '$') ? t + 1 : t;
        const LSMType *bind_type = (i == loop_var_count - 1) ? elem_type : lsm_type_unknown();
        lsm_scope_bind(ctx->current_scope, name, bind_type);
    }
}
...
static void process_catch(PHPLSPContext *ctx, TSNode node) {
    ...
    const LSMType *exc_type = lsm_type_unknown();
    ...
    lsm_scope_bind(ctx->current_scope, vname, exc_type);
}

/* ── call resolution ────────────────────────────────────────────── */

static void resolve_function_call(PHPLSPContext *ctx, TSNode call, LSMResolvedKind kind) {
    ...
    if (kind == LSM_RESOLVED_CALL_REFERENCE && strcmp(ts_node_type(fn), "variable_name") == 0) {
        const LSMType *receiver = php_eval_expr_type(ctx, fn);
        const char *class_qn = NULL;
        if (receiver && receiver->kind == LSM_TYPE_NAMED) {
            class_qn = receiver->data.named.qualified_name;
        } else if (receiver && receiver->kind == LSM_TYPE_TEMPLATE) {
            class_qn = receiver->data.template_type.template_name;
        }
        const LSMRegisteredFunc *invoke =
    ...
    const char *target = find_use(ctx, name, LSM_PHP_USE_FUNCTION);
    if (target) {
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, target);
        if (f) {
    ...
    if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        const char *ns_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, ns_qn);
        if (f) {
            emit_resolved_kind(ctx, f->qualified_name, "php_function_namespaced", 0.95f, kind);
            return;
        }
    ...
    const LSMRegisteredFunc *best = NULL;
    int best_score = -1;
...
static void resolve_member_call(PHPLSPContext *ctx, TSNode call, LSMResolvedKind kind) {
    TSNode obj = ts_node_child_by_field_name(call, "object", 6);
    TSNode name_node = ts_node_child_by_field_name(call, "name", 4);
    if (ts_node_is_null(name_node))
        return;
    if (kind == LSM_RESOLVED_CALL_REFERENCE &&
    ...
    if (class_has_magic_call(ctx, class_qn, false)) {
        emit_resolved_reason_kind(ctx, lsm_arena_sprintf(ctx->arena, "%s.__call", class_qn),
    ...
    emit_resolved_kind(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, method_name),
                       "php_method_typed_unindexed", 0.55f, kind);
}

static void resolve_static_call(PHPLSPContext *ctx, TSNode call, LSMResolvedKind kind) {
    ...
    const LSMRegisteredFunc *magic_static = php_lookup_method(ctx, class_qn, "__callStatic");
    if (magic_static && kind == LSM_RESOLVED_CALL_REFERENCE) {
...
typedef struct {
    ...
    const LSMType *type;  /* the narrowed type */
} php_narrowing_t;
...
static int parse_narrowing_one(PHPLSPContext *ctx, TSNode cond, php_narrowing_t *out) {
    ...
    if (strcmp(kind, "binary_expression") == 0) {
        ...
        if (opt && strcmp(opt, "instanceof") == 0) {
            ...
            if (!resolved)
                return false;
            out->var_name = lsm_arena_strdup(ctx->arena, vname);
            out->type = lsm_type_named(ctx->arena, resolved);
            return true;
        }
    ...
    if (strcmp(kind, "function_call_expression") == 0) {
        ...
        for (uint32_t i = 0; i < anc; i++) {
            ...
            const char *vname = (vt[0] == '$') ? vt + 1 : vt;
            out->var_name = lsm_arena_strdup(ctx->arena, vname);
            out->type = lsm_type_builtin(ctx->arena, builtin);
            return true;
        }
    }
    return false;
...
static void apply_assert_narrowing(PHPLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (parse_narrowing(ctx, predicate, &nw) && nw.var_name && nw.type) {
            lsm_scope_bind(ctx->current_scope, nw.var_name, nw.type);
        }
        return;
    }
}
...
static void walk_with_narrowings(PHPLSPContext *ctx, TSNode body, const php_narrowing_t *nws,
    ...
    for (int i = 0; i < nw_count; i++) {
        if (nws[i].var_name && nws[i].type) {
            lsm_scope_bind(ctx->current_scope, nws[i].var_name, nws[i].type);
        }
    }
    php_resolve_calls_in_node(ctx, body);
...
static void process_if_statement(PHPLSPContext *ctx, TSNode node) {
    ...
    if (body_is_terminator && has_neg_nw && neg_nw.var_name && neg_nw.type) {
        lsm_scope_bind(ctx->current_scope, neg_nw.var_name, neg_nw.type);
    }
}
...
static void php_resolve_calls_in_node_inner(PHPLSPContext *ctx, TSNode node) {
    ...
    LSMResolvedKind resolved_kind =
        is_callable_reference ? LSM_RESOLVED_CALL_REFERENCE : LSM_RESOLVED_INVOCATION;
    if (strcmp(kind, "function_call_expression") == 0) {
            ...
            if (mn && strcmp(mn, "bindTo") == 0 &&
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    for (uint32_t i = 0; i < anc; i++) {
                        ...
                        const LSMType *t = php_eval_expr_type(ctx, v);
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    if (!ts_node_is_null(closure_arg) && !ts_node_is_null(obj_arg)) {
                        ...
                        if (strcmp(ck, "anonymous_function") == 0 ||
                            ...
                            const LSMType *t = php_eval_expr_type(ctx, obj_arg);
                            if (t && t->kind == LSM_TYPE_NAMED) {
                                const char *saved = ctx->enclosing_class_qn;
                                ctx->enclosing_class_qn = t->data.named.qualified_name;
...
static void bind_typed_parameters(PHPLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const LSMType *ptype = lsm_type_unknown();
        if (!ts_node_is_null(tnode))
        ...
        if (strcmp(pk, "variadic_parameter") == 0) {
            ptype = lsm_type_builtin(ctx->arena, "array");
        }
        lsm_scope_bind(ctx->current_scope, name, ptype);
    }
}
            ...
            if (fname) {
                if (is_method && ctx->enclosing_class_qn) {
                    ctx->enclosing_func_qn =
                        lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, fname);
                } else if (ctx->module_qn) {
                    ctx->enclosing_func_qn =
                        lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
                } else {
                    ctx->enclosing_func_qn = lsm_arena_strdup(ctx->arena, fname);
                }
            }
        }
    }
    ...
    if (strcmp(kind, "anonymous_function") == 0) {
        ...
        for (uint32_t i = 0; i < fnc; i++) {
            ...
            for (uint32_t j = 0; j < unc; j++) {
                ...
                const LSMType *captured = lsm_scope_lookup(saved_scope, vname);
                if (captured) {
                    lsm_scope_bind(ctx->current_scope, vname, captured);
                }
            }
        }
    }
...
static void process_class_decl(PHPLSPContext *ctx, TSNode node) {
    ...
    if (ctx->module_qn) {
        ...
        ctx->enclosing_class_qn = lsm_arena_strdup(ctx->arena, cname);
    }
...
static void collect_use_declaration(PHPLSPContext *ctx, TSNode use_decl) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "function") == 0)
            kind = LSM_PHP_USE_FUNCTION;
        if (strcmp(k, "const") == 0)
            kind = LSM_PHP_USE_CONST;
    }
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_child(use_decl, i);
        if (ts_node_is_null(c) || !ts_node_is_named(c))
...
/* ── parse return-type text from LSMDefinition.return_type ──────── */

static const LSMType *parse_return_type_text(LSMArena *arena, const char *text,
    ...
    char *first;
    if (cut) {
        first = lsm_arena_strndup(arena, text, (size_t)(cut - text));
    } else {
        first = lsm_arena_strdup(arena, text);
    }
    if (!first)
        return lsm_type_unknown();
    /* trim. */
    ...
    *end = '\0';
    if (!*first || strcmp(first, "null") == 0)
        return lsm_type_unknown();
    if (php_is_builtin_type_name(first))
        return lsm_type_builtin(arena, first);

    /* Resolve via use map. */
    if (first[0] == '\\') {
        return lsm_type_named(arena, php_ns_to_dot(arena, first + 1));
    }
    /* Find first segment for use lookup. */
    const char *bs = first;
    while (*bs && *bs != '\\')
        bs++;
    char *first_seg = (*bs) ? lsm_arena_strndup(arena, first, (size_t)(bs - first))
                            : lsm_arena_strdup(arena, first);
    for (int i = 0; i < use_count; i++) {
        if (strcmp(use_local_names[i], first_seg) == 0) {
            const char *target = use_target_qns[i];
            if (*bs) {
                return lsm_type_named(
                    arena, lsm_arena_sprintf(arena, "%s%s", target, php_ns_to_dot(arena, bs)));
            }
            return lsm_type_named(arena, target);
        }
    }
    /* Fallback: current namespace prefix. */
    if (current_namespace_qn && *current_namespace_qn) {
        return lsm_type_named(arena, lsm_arena_sprintf(arena, "%s.%s", current_namespace_qn,
                                                       php_ns_to_dot(arena, first)));
    }
    if (module_qn) {
        return lsm_type_named(
            arena, lsm_arena_sprintf(arena, "%s.%s", module_qn, php_ns_to_dot(arena, first)));
    }
    return lsm_type_named(arena, php_ns_to_dot(arena, first));
}
...
static const LSMType *php_signature_parse_type(LSMArena *arena, const char *text,
                                               void *parser_ctx) {
 ...
 * typed properties on the corresponding LSMRegisteredType.field_names /
 * field_types. Handles:
 ...
 * returns the property's LSMType when known. */

typedef struct {
    const char *class_qn;
    const char **field_names; /* arena-backed, NULL-terminated */
    const LSMType **field_types;
    int count;
    int cap;
} php_class_fields_t;
...
static php_class_fields_t *fields_for(php_class_field_table_t *tab, const char *class_qn,
    ...
    if (tab->count >= tab->cap) {
        ...
        php_class_fields_t *next =
            (php_class_fields_t *)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*next));
        if (!next)
            return NULL;
        for (int i = 0; i < tab->count; i++)
            next[i] = tab->items[i];
    ...
    memset(f, 0, sizeof(*f));
    f->class_qn = lsm_arena_strdup(arena, class_qn);
    return f;
}

static void add_field(php_class_fields_t *f, LSMArena *arena, const char *name,
                      const LSMType *type) {
    if (!f || !name || !type)
        return;
    ...
    if (f->count >= f->cap) {
        ...
        const char **nn =
            (const char **)lsm_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const LSMType **nt =
            (const LSMType **)lsm_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_names[f->count] = lsm_arena_strdup(arena, name);
...
static const char *class_qn_for_node(PHPLSPContext *ctx, TSNode class_node) {
    ...
    if (!cname)
        return NULL;
    if (ctx->module_qn) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    }
    return lsm_arena_strdup(ctx->arena, cname);
}
...
static void extract_property_decl(PHPLSPContext *ctx, TSNode prop_decl, php_class_fields_t *out) {
    ...
    const LSMType *ptype = NULL;
    if (!ts_node_is_null(tnode))
        ptype = php_parse_type_node(ctx, tnode);
    if (!ptype || ptype->kind == LSM_TYPE_UNKNOWN) {
...
static void extract_ctor_assignments(PHPLSPContext *ctx, TSNode ctor_method,
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < nc && pc < 32; i++) {
            ...
            if (!vt)
                continue;
            names[pc] = lsm_arena_strdup(ctx->arena, (vt[0] == '$') ? vt + 1 : vt);
            types[pc] = ptype;
            pc++;
        }
    }
...
static const char **parse_phpdoc_template_params(PHPLSPContext *ctx, const char *docstring) {
    ...
    while ((p = strstr(p, "@template")) != NULL) {
        ...
        if (p == name_start)
            continue;
        if (n < 16) {
            names[n++] = lsm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        }
    }
    if (n == 0)
        return NULL;
    const char **out = (const char **)lsm_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(*out));
    if (!out)
        return NULL;
    for (int i = 0; i < n; i++)
        out[i] = names[i];
 ...
 * (param_names, args) mapping, returning a new arena-allocated LSMType. */
static const LSMType *php_substitute_template(LSMArena *arena, const LSMType *t,
                                              const char *const *param_names,
                                              const LSMType *const *args) {
    ...
    if (t->kind == LSM_TYPE_TEMPLATE) {
        ...
        const LSMType **new_args =
            (const LSMType **)lsm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*new_args));
        if (!new_args)
            return t;
        for (int i = 0; i < n; i++) {
...
static void apply_phpdoc_class_tags(PHPLSPContext *ctx, LSMTypeRegistry *reg, const char *class_qn,
        ...
        for (int pi = 0; prefixes[pi]; pi++) {
            ...
            while ((p = strstr(p, prefixes[pi])) != NULL) {
                ...
                if (p == name_start)
                    continue;
                char *alias_name =
                    lsm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
                while (*p == ' ' || *p == '\t' || *p == '=')
                    p++;
                ...
                if (p == type_start)
                    continue;
                char *type_text =
                    lsm_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
                ...
                if (!*type_text)
                    continue;
                const LSMType *aliased = resolve_phpdoc_type(ctx, type_text);
                if (aliased && aliased->kind != LSM_TYPE_UNKNOWN) {
     ...
     * inherited methods at least resolve (untyped). */
    {
        ...
        while ((p = strstr(p, "@extends")) != NULL) {
            ...
            if (p == parent_start)
                continue;
            char *parent_text =
                lsm_arena_strndup(ctx->arena, parent_start, (size_t)(p - parent_start));
            const LSMType *parent_type = resolve_phpdoc_type(ctx, parent_text);
            const char *parent_qn = NULL;
            if (parent_type && parent_type->kind == LSM_TYPE_NAMED) {
                parent_qn = parent_type->data.named.qualified_name;
            } else if (parent_type && parent_type->kind == LSM_TYPE_TEMPLATE) {
            ...
            for (int t = 0; t < reg->type_count; t++) {
                ...
                if (found)
                    break;
                const char **expanded = (const char **)lsm_arena_alloc(
    ...
    while ((p = strstr(p, "@property")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *fname = lsm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const LSMType *t = resolve_phpdoc_type(ctx, type_text);
        if (out_fields && fname && t && t->kind != LSM_TYPE_UNKNOWN) {
    ...
    while ((p = strstr(p, "@method")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *mname = lsm_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        ...
        rf.qualified_name = lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
        rf.short_name = mname;
        rf.receiver_type = class_qn;
        rf.min_params = -1;
        const LSMType *ret = resolve_phpdoc_type(ctx, type_text);
        const LSMType **rets = (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(*rets));
        if (rets) {
            rets[0] = ret;
            rets[1] = NULL;
        }
 ...
 *     LSMRegisteredFunc with receiver_type = class_qn
 ...
 */
static void flatten_trait_into_class(PHPLSPContext *ctx, LSMTypeRegistry *reg, const char *class_qn,
    ...
    const LSMRegisteredType *t = lsm_registry_lookup_type(reg, trait_qn);
    if (!t)
     ...
     * then re-matches the loop's filter while lsm_registry_add_func() keeps
     ...
     *
     * Snapshot the count before iterating: lsm_registry_add_func() below
    ...
    for (int i = 0; i < func_count_before; i++) {
        const LSMRegisteredFunc *src = &reg->funcs[i];
        ...
        LSMRegisteredFunc rf;
        memset(&rf, 0, sizeof(rf));
        rf.qualified_name = lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, new_short);
        rf.short_name = lsm_arena_strdup(ctx->arena, new_short);
        rf.receiver_type = class_qn;
        ...
        const LSMType *new_sig = src->signature;
        if (src->signature && src->signature->kind == LSM_TYPE_FUNC &&
            src->signature->data.func.return_types) {
            const LSMType *ret0 = src->signature->data.func.return_types[0];
            if (ret0 && ret0->kind == LSM_TYPE_NAMED && ret0->data.named.qualified_name &&
                strcmp(ret0->data.named.qualified_name, canonical_trait_qn) == 0) {
                const LSMType **rets =
                    (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(*rets));
                if (rets) {
                    rets[0] = lsm_type_named(ctx->arena, class_qn);
                    rets[1] = NULL;
                    new_sig = lsm_type_func_replace_returns(ctx->arena, src->signature, rets);
                }
            }
        }
...
static void process_trait_use(PHPLSPContext *ctx, LSMTypeRegistry *reg, const char *class_qn,
                              TSNode use_decl) {
...
static void process_class_for_fields(PHPLSPContext *ctx, LSMTypeRegistry *reg, TSNode class_node,
        ...
        if (strcmp(k, "property_declaration") == 0) {
            ...
            if (!ts_node_is_null(case_name)) {
                ...
                if (cn) {
                     ...
                     * handles below. */
                    add_field(f, ctx->arena, cn, lsm_type_named(ctx->arena, cqn));
                }
            }
            ...
            if (!ts_node_is_null(rt_node) && mn) {
                ...
                const LSMType *parsed = php_parse_type_node(ctx, rt_node);
                ctx->enclosing_class_qn = saved_class;
                ctx->enclosing_parent_qn = saved_parent;
                if (parsed && parsed->kind != LSM_TYPE_UNKNOWN) {
                    for (int fi = 0; fi < reg->func_count; fi++) {
                        LSMRegisteredFunc *rf = &reg->funcs[fi];
                        ...
                        if (rets) {
                            ...
                            rets[1] = NULL;
                            rf->signature =
                                lsm_type_func_replace_returns(ctx->arena, rf->signature, rets);
                        }
                        break;
                    }
                }
                ...
                if (p) {
                    ...
                    if (p > type_start) {
                        ...
                        if (strcmp(type_text, "$this") == 0 || strcmp(type_text, "static") == 0 ||
                            ...
                            ret_t = lsm_type_named(ctx->arena, cqn);
                        }
                            ...
                            for (int t = 0; t < reg->type_count; t++) {
                                ...
                                for (int j = 0; reg->types[t].type_param_names[j]; j++) {
                                    if (strcmp(reg->types[t].type_param_names[j], type_text) == 0) {
                                        ret_t = lsm_type_named(ctx->arena, type_text);
                                        break;
                                    }
                                }
                                break;
                        ...
                        if (ret_t && ret_t->kind != LSM_TYPE_UNKNOWN) {
                            ...
                            for (int fi = 0; fi < reg->func_count; fi++) {
                                ...
                                if (rets) {
                                    rets[0] = ret_t;
                                    rets[1] = NULL;
                                    rf->signature = lsm_type_func_replace_returns(
...
static void php_lsp_collect_class_fields(PHPLSPContext *ctx, LSMTypeRegistry *reg, TSNode root,
                                         php_class_field_table_t *tab) {
    if (ts_node_is_null(root))
        return;
...
void lsm_run_php_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
    ...
    lsm_php_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '\\' && !strchr(base, '.')) {
                                qualified = lsm_arena_sprintf(arena, "%s.%s", module_qn, base);
                            } else if (base[0] == '\\') {
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (d->signature_param_types || d->signature_param_count > 0) {
                param_types = lsm_type_materialize_signature_params(
                    ...
                    php_signature_parse_type, &parse_ctx);
                       ...
                       d->param_types) {
                /* Legacy LSMDefinition callers have no counted carrier. Keep
            ...
            rf.signature = lsm_type_func(arena, NULL, param_types, rets);
            lsm_registry_add_func(&reg, rf);
        }
    }
     ...
     * for $this->prop and $obj->prop sees the field types. */
    {
        ...
        /* Collect top-level children once (O(n)); ts_node_child(root,i) is O(i) → O(n²). */
        uint32_t pkn = 0;
        TSNode *pkids = lsm_lsp_collect_children(ctx.arena, root, &pkn);
        ...
        lsm_php_refine_lsp_registry(&ctx, &reg, root);
    ...
    if (ctx.debug) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            const LSMResolvedCall *rc = &result->resolved_calls.items[i];
...
static const char **php_split_pipe(LSMArena *arena, const char *text) {
    if (!text || !text[0])
        return NULL;
    int count = 1;
...
/* Build a registry from caller-supplied LSMLSPDef[] — covers both the source
...
void lsm_run_php_lsp_cross(LSMArena *arena, const char *source, int source_len,
    ...
    lsm_php_register_lsp_defs(arena, &idx_arena, &reg, defs, def_count);
    ...
    for (int i = 0; i < import_count; i++) {
        if (import_names && import_qns && import_names[i] && import_qns[i]) {
            php_lsp_add_use(&ctx, import_names[i], import_qns[i], LSM_PHP_USE_CLASS);
        }
    }

    /* Class field collection — same flow as lsm_run_php_lsp. Walks the AST
...
void lsm_batch_php_lsp_cross(LSMArena *arena, LSMBatchPHPLSPFile *files, int file_count,
                             LSMResolvedCallArray *out) {
    if (!arena || !files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        LSMBatchPHPLSPFile *file = &files[f];
        ...
        lsm_run_php_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
