...
static void py_register_func_decorators(LSMRegisteredFunc *func, const char **decorators) {
    if (!func)
        return;
...
static bool py_func_is_exact_callable_value(const LSMRegisteredFunc *func) {
    return func && func->qualified_name &&
           !(func->flags &
             (LSM_FUNC_FLAG_PROPERTY | LSM_FUNC_FLAG_OVERLOAD | LSM_FUNC_FLAG_AMBIGUOUS_BINDING)) &&
           (!func->decorator_qns || !func->decorator_qns[0]);
}

static int py_func_qn_pointer_cmp(const void *left, const void *right) {
    const LSMRegisteredFunc *a = *(LSMRegisteredFunc *const *)left;
    const LSMRegisteredFunc *b = *(LSMRegisteredFunc *const *)right;
 ...
 * behavior. */
static void py_mark_ambiguous_callable_bindings(LSMTypeRegistry *registry) {
    if (!registry || registry->func_count <= 1)
        return;
    size_t count = (size_t)registry->func_count;
    LSMRegisteredFunc **sorted = count <= SIZE_MAX / sizeof(*sorted)
                                     ? (LSMRegisteredFunc **)malloc(count * sizeof(*sorted))
                                     : NULL;
    if (!sorted) {
        /* Allocation failure must reduce precision, never fabricate it. */
        for (int i = 0; i < registry->func_count; i++)
            registry->funcs[i].flags |= LSM_FUNC_FLAG_AMBIGUOUS_BINDING;
        return;
    }
    for (int i = 0; i < registry->func_count; i++)
    ...
    for (size_t first = 0; first < count;) {
        ...
        if (qn && end - first > 1) {
            for (size_t i = first; i < end; i++)
                sorted[i]->flags |= LSM_FUNC_FLAG_AMBIGUOUS_BINDING;
        }
        first = end;
    }
    free(sorted);
...
static const LSMType *py_eval_expr_type(PyLSPContext *ctx, TSNode node);
static const LSMType *py_eval_expr_type_uncached(PyLSPContext *ctx, TSNode node);
...
static const LSMRegisteredFunc *py_lookup_attribute(PyLSPContext *ctx, const char *type_qn,
...
static const LSMType *py_resolve_annotation(PyLSPContext *ctx, const char *ann);
static const LSMType *py_iterable_element_type(PyLSPContext *ctx, const LSMType *iter_type);
static const LSMType *py_lookup_field(PyLSPContext *ctx, const char *type_qn,
                                      const char *field_name);
static void py_register_instance_field(PyLSPContext *ctx, const char *class_qn,
                                       const char *field_name, const LSMType *field_type);
static void py_bind_for_target(PyLSPContext *ctx, TSNode left, const LSMType *elem_type);
 ...
 * wrappers — never call lsm_scope_bind(ctx->current_scope, ...) or assign
...
static void py_scope_bind(PyLSPContext *ctx, const char *name, const LSMType *type) {
    ctx->type_cache_gen++;
...
static void py_scope_bind_callable(PyLSPContext *ctx, const char *name, const LSMType *type,
                                   const char *callable_qn) {
    ctx->type_cache_gen++;
    lsm_scope_bind_callable(ctx->current_scope, name, type, callable_qn);
    const char *bound = name ? lsm_scope_lookup_callable(ctx->current_scope, name) : NULL;
    if (name && (!lsm_scope_contains(ctx->current_scope, name) ||
...
static LSMScope *py_scope_push_checked(PyLSPContext *ctx) {
    if (!ctx)
        return NULL;
    LSMScope *parent = ctx->current_scope;
    LSMScope *child = lsm_scope_push(ctx->arena, parent);
...
static void py_scope_clear_callable(PyLSPContext *ctx, const char *name) {
    if (lsm_scope_update_callable(ctx->current_scope, name, NULL)) {
        ctx->type_cache_gen++;
    }
}

static void py_scope_restore(PyLSPContext *ctx, LSMScope *saved) {
    ctx->type_cache_gen++;
    ctx->current_scope = saved;
}

void py_lsp_init(PyLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
...
void py_lsp_add_import(PyLSPContext *ctx, const char *local_name, const char *module_qn) {
    ...
    const char **names =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const char **qns =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    unsigned char *kinds =
        (unsigned char *)lsm_arena_alloc(ctx->arena, (size_t)new_count * sizeof(unsigned char));
    if (!names || !qns || !kinds)
        return;
...
static char *py_import_node_text_dup(PyLSPContext *ctx, TSNode node) {
    ...
    if (end < start || (int)end > ctx->source_len)
        return NULL;
    return lsm_arena_strndup(ctx->arena, ctx->source + start, (size_t)(end - start));
}
...
static const char *py_canonical_from_import_qn(PyLSPContext *ctx, TSNode statement,
    ...
    const char *full_suffix =
        module_suffix && module_suffix[0]
            ? lsm_arena_sprintf(ctx->arena, "%s.%s", module_suffix, member)
            : member;
    if (!full_suffix)
        return NULL;
    ...
    return lsm_arena_sprintf(ctx->arena, "%s.%s", incoming_qn, member);
}

/* LSMImport intentionally stores only the local spelling and module path.
...
static void py_bind_dotted_prefixes(PyLSPContext *ctx, const char *qn) {
    ...
    for (;;) {
        ...
        char *prefix = (char *)lsm_arena_alloc(ctx->arena, prefix_len + 1);
        if (!prefix)
            return;
        ...
        if (lsm_type_is_unknown(lsm_scope_lookup(ctx->current_scope, bind_short))) {
            py_scope_bind(ctx, bind_short, lsm_type_module(ctx->arena, prefix));
        }
        p = dot + 1;
    }
}
...
static void py_bind_import_index(PyLSPContext *ctx, int index, bool synthetic_fallback) {
    ...
    if (direct_kind == PY_DIRECT_IMPORT_UNALIASED) {
        ...
        t = lsm_type_named(ctx->arena, qn);
            ...
            // LSMImport shape cannot distinguish the two, but NAMED(qn)
    ...
    const LSMRegisteredFunc *imported =
        from_style && ctx->registry ? lsm_registry_lookup_func(ctx->registry, qn) : NULL;
...
const LSMType *py_lsp_lookup_in_scope(const PyLSPContext *ctx, const char *name) {
    if (!ctx)
        return lsm_type_unknown();
    return lsm_scope_lookup(ctx->current_scope, name);
}
...
static char *py_node_text(PyLSPContext *ctx, TSNode node) {
    ...
    char *out = (char *)lsm_arena_alloc(ctx->arena, len + 1);
    if (!out)
        return NULL;
...
static void py_register_lambda(PyLSPContext *ctx, const char *name, TSNode lambda_node) {
    ...
    if (ctx->lambda_count >= ctx->lambda_cap) {
        ...
        LSMLambdaEntry *grown =
            (LSMLambdaEntry *)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(LSMLambdaEntry));
        if (!grown)
            return;
        if (ctx->lambdas && ctx->lambda_count > 0) {
            memcpy(grown, ctx->lambdas, (size_t)ctx->lambda_count * sizeof(LSMLambdaEntry));
        }
        ctx->lambdas = grown;
    ...
    ctx->lambdas[ctx->lambda_count].name = lsm_arena_strdup(ctx->arena, name);
...
static void py_register_dict_literal(PyLSPContext *ctx, const char *var, TSNode dict_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const LSMRegisteredFunc *f =
            lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, val_text);
        if (!f) {
            f = lsm_registry_lookup_symbol(ctx->registry, "builtins", val_text);
        }
        if (!f)
            continue;
        ...
        if (ctx->dict_literal_count >= ctx->dict_literal_cap) {
            ...
            LSMDictLiteralEntry *grown = (LSMDictLiteralEntry *)lsm_arena_alloc(
                ctx->arena, (size_t)new_cap * sizeof(LSMDictLiteralEntry));
            if (!grown)
                return;
            if (ctx->dict_literals && ctx->dict_literal_count > 0) {
                memcpy(grown, ctx->dict_literals,
                       (size_t)ctx->dict_literal_count * sizeof(LSMDictLiteralEntry));
            }
            ctx->dict_literals = grown;
            ctx->dict_literal_cap = new_cap;
        }
        ctx->dict_literals[ctx->dict_literal_count].var_name = lsm_arena_strdup(ctx->arena, var);
        ctx->dict_literals[ctx->dict_literal_count].literal_key =
            lsm_arena_strdup(ctx->arena, key_text);
        ctx->dict_literals[ctx->dict_literal_count].target_qn =
            lsm_arena_strdup(ctx->arena, f->qualified_name);
...
static const char *py_lookup_dict_dispatch(PyLSPContext *ctx, const char *var, const char *key) {
    ...
    for (int i = 0; i < ctx->dict_literal_count; i++) {
        const LSMDictLiteralEntry *e = &ctx->dict_literals[i];
...
static void py_emit_resolved_call_reason(PyLSPContext *ctx, const char *callee_qn,
    ...
    for (int i = start; i < n; i++) {
        LSMResolvedCall *rc = &ctx->resolved_calls->items[i];
        if (rc->kind == LSM_RESOLVED_INVOCATION && rc->site_start_byte == site_start &&
            ...
            if (confidence > rc->confidence) {
                ...
                rc->reason = reason ? lsm_arena_strdup(ctx->arena, reason) : NULL;
            }
            return;
        }
    }
    ...
    rc.callee_qn = lsm_arena_strdup(ctx->arena, callee_qn);
    rc.strategy = strategy;
    rc.confidence = confidence;
    rc.reason = reason ? lsm_arena_strdup(ctx->arena, reason) : NULL;
...
static void py_emit_resolved_reference(PyLSPContext *ctx, const char *callee_qn,
    ...
    for (int i = ctx->resolved_calls->count - 1; i >= 0; i--) {
        LSMResolvedCall *existing = &ctx->resolved_calls->items[i];
        if (existing->kind == LSM_RESOLVED_CALL_REFERENCE && existing->site_start_byte == start &&
    ...
    rc.reason = source_name && strcmp(source_name, leaf) != 0
                    ? lsm_arena_strdup(ctx->arena, source_name)
                    : NULL;
...
static void py_emit_unresolved_reference(PyLSPContext *ctx, const char *candidate_qn, TSNode site) {
    ...
    LSMResolvedCall rc = {0};
    rc.caller_qn = ctx->enclosing_func_qn;
    rc.callee_qn = lsm_arena_strdup(ctx->arena, candidate_qn);
...
static const char *py_exact_callable_target_ex(PyLSPContext *ctx, TSNode node,
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (lsm_scope_contains(ctx->current_scope, name)) {
            const char *bound = lsm_scope_lookup_callable(ctx->current_scope, name);
            if (bound && lexical_alias_out) {
                ...
                const LSMRegisteredFunc *module_symbol =
    ...
    if (strcmp(kind, "attribute") == 0) {
        ...
        const LSMType *receiver = py_eval_expr_type(ctx, object);
        if (!member || !receiver)
            return NULL;
        receiver = lsm_type_resolve_alias(receiver);
        if (receiver->kind == LSM_TYPE_MODULE) {
            const LSMRegisteredFunc *f =
                lsm_registry_lookup_symbol(ctx->registry, receiver->data.module.module_qn, member);
            return py_func_is_exact_callable_value(f) ? f->qualified_name : NULL;
        }
        if (receiver->kind == LSM_TYPE_NAMED) {
...
static void py_resolve_value_references_at(PyLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        const char *candidate = strcmp(kind, "identifier") == 0
                                    ...
                                    : NULL;
        if (candidate && lsm_scope_contains(ctx->current_scope, source_name)) {
            const LSMType *binding =
                lsm_type_resolve_alias(lsm_scope_lookup(ctx->current_scope, source_name));
            if (binding && binding->kind == LSM_TYPE_NAMED &&
...
static const LSMRegisteredFunc *py_lookup_attribute_depth(PyLSPContext *ctx, const char *type_qn,
...
static const LSMType *py_overlay_lookup_field(const PyLSPContext *ctx, const char *class_qn,
                                              const char *field_name) {
...
static void py_overlay_register_field(PyLSPContext *ctx, const char *class_qn,
    ...
    if (ctx->field_overlay_count >= ctx->field_overlay_cap) {
        ...
        void *na = lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ctx->field_overlay));
        if (!na) {
            py_disable_callable_value_proof(ctx);
            return;
        }
    ...
    const char *stored_class = lsm_arena_strdup(ctx->arena, class_qn);
    const char *stored_field = lsm_arena_strdup(ctx->arena, field_name);
 ...
 * or NULL if no match. */
static const LSMType *py_lookup_field_depth(PyLSPContext *ctx, const char *type_qn,
    ...
    {
        const LSMType *ov = py_overlay_lookup_field(ctx, type_qn, field_name);
        if (ov)
            return ov;
    }
    if (rt->alias_of) {
        const LSMType *a = py_lookup_field_depth(ctx, rt->alias_of, field_name, depth + 1);
        if (a)
            return a;
    }
    if (rt->embedded_types) {
...
static void py_register_instance_field(PyLSPContext *ctx, const char *class_qn,
    ...
    // Find the type entry. lsm_registry_lookup_type returns a const pointer;
    ...
    if (rt->field_names && rt->field_types) {
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], field_name) == 0) {
                ((const LSMType **)rt->field_types)[i] = field_type;
                return;
            }
        }
    }
    ...
    const char **new_names =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)(new_count + 1) * sizeof(const char *));
    const LSMType **new_types = (const LSMType **)lsm_arena_alloc(
        ctx->arena, (size_t)(new_count + 1) * sizeof(const LSMType *));
    ...
    new_names[existing] = lsm_arena_strdup(ctx->arena, field_name);
...
static const LSMType *py_literal_type(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "list_comprehension") == 0)
        return lsm_type_builtin(ctx->arena, "list");
    if (strcmp(k, "dictionary_comprehension") == 0)
        return lsm_type_builtin(ctx->arena, "dict");
    if (strcmp(k, "set_comprehension") == 0)
        return lsm_type_builtin(ctx->arena, "set");
    if (strcmp(k, "generator_expression") == 0)
        return lsm_type_builtin(ctx->arena, "generator");
    return NULL;
}
...
static const LSMType *py_substitute_self(PyLSPContext *ctx, const LSMType *t,
    ...
    if (t->kind == LSM_TYPE_NAMED && t->data.named.qualified_name) {
        ...
        if (strcmp(qn, "Self") == 0 || strcmp(qn, "typing.Self") == 0 ||
            ...
            return lsm_type_named(ctx->arena, receiver_qn);
        }
    }
    if (t->kind == LSM_TYPE_UNION) {
        int n = t->data.union_type.count;
        const LSMType **members = (const LSMType **)lsm_arena_alloc(
        ...
        return lsm_type_union(ctx->arena, members, n);
    }
    return t;
}
...
static const LSMType *py_func_return_type_recv(PyLSPContext *ctx, const char *func_qn,
                                               const char *receiver_qn) {
    if (!ctx || !func_qn)
        return lsm_type_unknown();
    const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, func_qn);
    if (!f || !f->signature)
        return lsm_type_unknown();
    if (f->signature->kind != LSM_TYPE_FUNC)
        return lsm_type_unknown();
    const LSMType **rets = f->signature->data.func.return_types;
    if (!rets || !rets[0])
        return lsm_type_unknown();
    const LSMType *base;
    if (!rets[1]) {
        ...
        while (rets[count])
            count++;
        base = lsm_type_tuple(ctx->arena, rets, count);
    }
    if (receiver_qn) {
...
static const LSMType *py_func_return_type(PyLSPContext *ctx, const char *func_qn) {
...
static void py_bind_for_target(PyLSPContext *ctx, TSNode left, const LSMType *elem_type) {
    ...
    if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
        ...
        const LSMType *const *elems = NULL;
        int count = 0;
        if (elem_type) {
            if (elem_type->kind == LSM_TYPE_TUPLE) {
                elems = elem_type->data.tuple.elems;
                count = elem_type->data.tuple.count;
            } else if (elem_type->kind == LSM_TYPE_TEMPLATE &&
        ...
        for (uint32_t i = 0; i < lc; i++) {
            ...
            if (strcmp(tk, "identifier") == 0) {
                ...
                if (!nm)
                    continue;
                const LSMType *bind_type;
                if (elems && (int)i < count && elems[i])
                    bind_type = elems[i];
                else
                    bind_type = lsm_type_unknown();
...
static const LSMType *py_iterable_element_type(PyLSPContext *ctx, const LSMType *iter_type) {
    if (!iter_type)
        return lsm_type_unknown();
    if (iter_type->kind == LSM_TYPE_TEMPLATE) {
        const char *name = iter_type->data.template_type.template_name;
        const LSMType **args = iter_type->data.template_type.template_args;
        ...
        if (strcmp(name, "ItemsView") == 0 || strcmp(name, "dict_items") == 0) {
            ...
            if (n >= 2) {
                const LSMType *elems[3] = {args[0], args[1], NULL};
                return lsm_type_tuple(ctx->arena, elems, 2);
            }
            return lsm_type_unknown();
        }
        ...
        if (strcmp(name, "tuple") == 0) {
            if (n == 1)
                return args[0];
            return lsm_type_union(ctx->arena, args, n);
        }
...
static const LSMType *py_eval_expr_type_uncached(PyLSPContext *ctx, TSNode node) {
    ...
    const LSMType *lit = py_literal_type(ctx, node);
    if (lit)
        return lit;
    ...
    if (strcmp(k, "tuple") == 0) {
        ...
        if (cn == 0)
            return lsm_type_builtin(ctx->arena, "tuple");
        const LSMType **elems = (const LSMType **)lsm_arena_alloc(
            ctx->arena, (size_t)(cn + 1) * sizeof(const LSMType *));
        if (!elems)
            return lsm_type_builtin(ctx->arena, "tuple");
        ...
        elems[cn] = NULL;
        return lsm_type_tuple(ctx->arena, elems, (int)cn);
    }
    if (strcmp(k, "list") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
            return lsm_type_builtin(ctx->arena, "list");
        const LSMType *first = py_eval_expr_type(ctx, ts_node_named_child(node, 0));
        if (!first || lsm_type_is_unknown(first))
            return lsm_type_builtin(ctx->arena, "list");
        return lsm_type_template(ctx->arena, "list", &first, 1);
    }
    if (strcmp(k, "set") == 0) {
        uint32_t cn = ts_node_named_child_count(node);
        if (cn == 0)
        ...
        if (!first || lsm_type_is_unknown(first))
            return lsm_type_builtin(ctx->arena, "set");
        return lsm_type_template(ctx->arena, "set", &first, 1);
    }
    if (strcmp(k, "dictionary") == 0) {
        ...
        for (uint32_t i = 0; i < cn; i++) {
            ...
            if (ts_node_is_null(key) || ts_node_is_null(value))
                continue;
            const LSMType *kt = py_eval_expr_type(ctx, key);
            const LSMType *vt = py_eval_expr_type(ctx, value);
            if (!kt || !vt)
                break;
            const LSMType *args[3] = {kt, vt, NULL};
            return lsm_type_template(ctx->arena, "dict", args, 2);
        }
        return lsm_type_builtin(ctx->arena, "dict");
    }
    ...
    if (strcmp(k, "identifier") == 0) {
        ...
        f = lsm_registry_lookup_symbol(ctx->registry, "builtins", name);
        if (f)
            return py_func_return_type(ctx, f->qualified_name);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(
            ctx->registry, lsm_arena_sprintf(ctx->arena, "builtins.%s", name));
        if (rt)
    ...
    if (strcmp(k, "attribute") == 0) {
        ...
        if (ts_node_is_null(obj) || ts_node_is_null(attr))
            return lsm_type_unknown();
        const LSMType *obj_type = py_eval_expr_type(ctx, obj);
        if (obj_type)
            obj_type = lsm_type_resolve_alias(obj_type);
        char *attr_name = py_node_text(ctx, attr);
        if (!attr_name || !obj_type)
            return lsm_type_unknown();

        if (obj_type->kind == LSM_TYPE_MODULE) {
            // module.attr — return type of the registered symbol if known.
            const char *mod = obj_type->data.module.module_qn;
            const LSMRegisteredFunc *f = lsm_registry_lookup_symbol(ctx->registry, mod, attr_name);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Could also be a class — return NAMED("mod.attr")
            const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", mod, attr_name);
            ...
            const char *prefix = lsm_arena_sprintf(ctx->arena, "%s.", qn);
            ...
            if (is_submodule)
                return lsm_type_module(ctx->arena, qn);
            return lsm_type_unknown();
        }
        if (obj_type->kind == LSM_TYPE_NAMED) {
            const LSMRegisteredFunc *f =
        ...
        if (obj_type->kind == LSM_TYPE_BUILTIN && obj_type->data.builtin.name) {
            // Builtins map to typeshed stdlib via "builtins.<typename>".
            const char *recv_qn =
                lsm_arena_sprintf(ctx->arena, "builtins.%s", obj_type->data.builtin.name);
            const LSMRegisteredFunc *f = py_lookup_attribute(ctx, recv_qn, attr_name);
            if (f)
                return py_func_return_type_recv(ctx, f->qualified_name, recv_qn);
        }
        if (obj_type->kind == LSM_TYPE_TEMPLATE && obj_type->data.template_type.template_name) {
            ...
            const char *recv_qn = lsm_arena_sprintf(ctx->arena, "builtins.%s", tname);
            const LSMRegisteredFunc *f = py_lookup_attribute(ctx, recv_qn, attr_name);
            if (!f) {
        ...
        if (obj_type->kind == LSM_TYPE_UNION) {
            int matches = 0;
            const LSMRegisteredFunc *hit = NULL;
            const char *hit_recv = NULL;
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                const LSMType *m = obj_type->data.union_type.members[i];
            ...
            int field_matches = 0;
            const LSMType *field_hit = NULL;
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                const LSMType *m = obj_type->data.union_type.members[i];
                if (!m || m->kind != LSM_TYPE_NAMED)
                    continue;
                const LSMType *fld = py_lookup_field(ctx, m->data.named.qualified_name, attr_name);
                if (fld) {
                    field_matches++;
                    field_hit = fld;
                }
    ...
    if (strcmp(k, "call") == 0) {
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (!ts_node_is_null(obj) && !ts_node_is_null(attr)) {
                ...
                if (mname && obj_type && obj_type->kind == LSM_TYPE_TEMPLATE) {
                    const char *tname = obj_type->data.template_type.template_name;
                    const LSMType **args = obj_type->data.template_type.template_args;
                    ...
                    if (is_dict_like && args && n >= 2) {
                        if (strcmp(mname, "items") == 0) {
                            const LSMType *pair[3] = {args[0], args[1], NULL};
                            return lsm_type_template(ctx->arena, "ItemsView", pair, 2);
                        }
                        if (strcmp(mname, "keys") == 0) {
                            const LSMType *k1[2] = {args[0], NULL};
                            return lsm_type_template(ctx->arena, "KeysView", k1, 1);
                        }
                        if (strcmp(mname, "values") == 0) {
                            const LSMType *v1[2] = {args[1], NULL};
                            return lsm_type_template(ctx->arena, "ValuesView", v1, 1);
                        }
                        if (strcmp(mname, "get") == 0) {
                            // dict.get(k) -> Optional[V]
                            return lsm_type_optional(ctx->arena, args[1]);
                        }
        ...
        if (strcmp(fk, "identifier") == 0) {
            ...
            if (strcmp(fname, "next") == 0) {
                ...
                if (!ts_node_is_null(args) && ts_node_named_child_count(args) >= 1) {
                    const LSMType *iter_type = py_eval_expr_type(ctx, ts_node_named_child(args, 0));
                    const LSMType *elem = py_iterable_element_type(ctx, iter_type);
            ...
            const LSMType *in_scope = lsm_scope_lookup(ctx->current_scope, fname);
            const char *callable_qn = lsm_scope_lookup_callable(ctx->current_scope, fname);
            if (callable_qn) {
                return py_func_return_type(ctx, callable_qn);
            }
            if (!lsm_type_is_unknown(in_scope)) {
                if (in_scope->kind == LSM_TYPE_NAMED) {
                    const LSMRegisteredType *rt = lsm_registry_lookup_type(
                ...
                if (in_scope->kind == LSM_TYPE_CALLABLE) {
                ...
                if (in_scope->kind == LSM_TYPE_TEMPLATE &&
            ...
            if (lsm_scope_contains(ctx->current_scope, fname))
                return lsm_type_unknown();
            // Module-local function call.
            const LSMRegisteredFunc *f =
                lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtins fallback: range / len / list / str / int / etc.
            f = lsm_registry_lookup_symbol(ctx->registry, "builtins", fname);
            if (f)
                return py_func_return_type(ctx, f->qualified_name);
            // Builtin class constructor: list() / dict() / set() / str() / ...
            const LSMRegisteredType *rt = lsm_registry_lookup_type(
                ctx->registry, lsm_arena_sprintf(ctx->arena, "builtins.%s", fname));
            if (rt)
                return lsm_type_builtin(ctx->arena, fname);
        }
        ...
        if (strcmp(fk, "call") == 0 || strcmp(fk, "parenthesized_expression") == 0 ||
            strcmp(fk, "lambda") == 0) {
            const LSMType *fn_type = py_eval_expr_type(ctx, fn);
            if (fn_type && fn_type->kind == LSM_TYPE_CALLABLE) {
                return fn_type->data.callable.return_type ? fn_type->data.callable.return_type
                                                          : lsm_type_unknown();
            }
            if (fn_type && fn_type->kind == LSM_TYPE_TEMPLATE &&
        ...
        if (strcmp(fk, "attribute") == 0) {
            ...
            if (obj_type->kind == LSM_TYPE_MODULE) {
                const char *mod = obj_type->data.module.module_qn;
                const LSMRegisteredFunc *f =
                    lsm_registry_lookup_symbol(ctx->registry, mod, attr_name);
                if (f)
                    return py_func_return_type(ctx, f->qualified_name);
            }
            if (obj_type->kind == LSM_TYPE_NAMED) {
    ...
    if (strcmp(k, "binary_operator") == 0) {
        ...
        const LSMType *lt = py_eval_expr_type(ctx, left);
        ...
        if (lt && lt->kind == LSM_TYPE_NAMED && !ts_node_is_null(op)) {
    ...
    if (strcmp(k, "conditional_expression") == 0) {
        ...
        if (nc >= 2) {
            const LSMType *a = py_eval_expr_type(ctx, ts_node_named_child(node, 0));
            const LSMType *b = py_eval_expr_type(ctx, ts_node_named_child(node, 2));
            const LSMType *members[2] = {a, b};
            return lsm_type_union(ctx->arena, members, 2);
        }
    }
    ...
    if (strcmp(k, "subscript") == 0) {
        ...
        if (ts_node_is_null(value))
            return lsm_type_unknown();
        const LSMType *container = py_eval_expr_type(ctx, value);
        ...
        if (container && container->kind == LSM_TYPE_NAMED) {
        ...
        if (!container || container->kind != LSM_TYPE_TEMPLATE) {
            return lsm_type_unknown();
        }
        const char *tname = container->data.template_type.template_name;
        const LSMType **args = container->data.template_type.template_args;
 ...
 * store NULL (the wrapper normalizes to lsm_type_unknown() first), so NULL
 * unambiguously means "miss". */
static const LSMType *py_type_cache_lookup(const PyLSPContext *ctx, const void *id) {
    ...
    for (int probed = 0; probed < ctx->type_cache_cap; probed++) {
        const LSMPyTypeCacheEntry *e = &ctx->type_cache[idx];
        if (!e->node_id)
...
static bool py_type_cache_grow(PyLSPContext *ctx) {
    ...
    LSMPyTypeCacheEntry *new_entries = (LSMPyTypeCacheEntry *)lsm_arena_alloc(
        ctx->arena, (size_t)new_cap * sizeof(LSMPyTypeCacheEntry));
    if (!new_entries)
        return false;
    memset(new_entries, 0, (size_t)new_cap * sizeof(LSMPyTypeCacheEntry));

    LSMPyTypeCacheEntry *old_entries = ctx->type_cache;
...
static void py_type_cache_insert(PyLSPContext *ctx, const void *id, const LSMType *result) {
    if (!id || !result)
        return;
    ...
    uint32_t idx = py_type_cache_hash(id) & mask;
    for (int probed = 0; probed < ctx->type_cache_cap; probed++) {
        LSMPyTypeCacheEntry *e = &ctx->type_cache[idx];
        if (!e->node_id) {
            e->node_id = id;
...
static const LSMType *py_eval_expr_type(PyLSPContext *ctx, TSNode node) {
    ...
    const LSMType *cached = py_type_cache_lookup(ctx, node.id);
    if (cached)
        return cached;
    ...
    ctx->eval_depth++;
    const LSMType *result = py_eval_expr_type_uncached(ctx, node);
    ctx->eval_depth--;
    if (!result)
        result = lsm_type_unknown();
...
static void py_process_statement(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "assignment") == 0) {
        ...
        const LSMType *rhs_type =
            ts_node_is_null(right) ? lsm_type_unknown() : py_eval_expr_type(ctx, right);
        ...
        if (strcmp(lk, "pattern_list") == 0 || strcmp(lk, "tuple_pattern") == 0 ||
            ...
            const LSMType *const *rhs_elems = NULL;
            int rhs_count = 0;
            if (rhs_type) {
                if (rhs_type->kind == LSM_TYPE_TUPLE) {
                    rhs_elems = rhs_type->data.tuple.elems;
                    rhs_count = rhs_type->data.tuple.count;
                } else if (rhs_type->kind == LSM_TYPE_TEMPLATE &&
            ...
            // is heterogeneous).
            const LSMType *elem_type = py_iterable_element_type(ctx, rhs_type);
            ...
            for (uint32_t i = 0; i < lc; i++) {
                ...
                const LSMType *bind_type;
                if (is_rest) {
                    bind_type = elem_type ? lsm_type_template(ctx->arena, "list", &elem_type, 1)
                                          : lsm_type_unknown();
                } else if (rhs_elems && (int)i < rhs_count && rhs_elems[i]) {
                    bind_type = rhs_elems[i];
                } else {
                    bind_type = elem_type ? elem_type : lsm_type_unknown();
                }
                py_scope_bind(ctx, nm, bind_type);
            }
            return;
    ...
    if (strcmp(k, "for_statement") == 0) {
        ...
        if (!ts_node_is_null(right)) {
            const LSMType *iter_type = py_eval_expr_type(ctx, right);
            ...
            if (lsm_type_is_unknown(elem_type) && iter_type && iter_type->kind == LSM_TYPE_NAMED) {
                const LSMRegisteredFunc *anext =
                    py_lookup_attribute(ctx, iter_type->data.named.qualified_name, "__anext__");
                if (anext) {
                    const LSMType *ret = py_func_return_type_recv(
    ...
    if (strcmp(k, "with_statement") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < cn; j++) {
                ...
                if (!name)
                    continue;
                const LSMType *cm_type =
                    ts_node_is_null(value) ? lsm_type_unknown() : py_eval_expr_type(ctx, value);
                const LSMType *bind_type = cm_type;
                if (cm_type && cm_type->kind == LSM_TYPE_NAMED) {
...
static void py_emit_call_for(PyLSPContext *ctx, TSNode call_node) {
    ...
    if (strcmp(fk, "identifier") == 0) {
        ...
        if (!ts_node_is_null(lambda_node)) {
            ...
            if (!ts_node_is_null(params) && !ts_node_is_null(args)) {
                ...
                for (uint32_t pi = 0; pi < pn && pi < an; pi++) {
                    ...
                    if (pname) {
                        const LSMType *arg_type = py_eval_expr_type(ctx, a);
            ...
            ctx->enclosing_func_qn = lsm_arena_sprintf(ctx->arena, "%s.<lambda>",
        ...
        if (lsm_scope_contains(ctx->current_scope, fname)) {
            const char *alias_target = lsm_scope_lookup_callable(ctx->current_scope, fname);
            if (alias_target) {
            ...
            const LSMType *in_scope = lsm_scope_lookup(ctx->current_scope, fname);
            if (!lsm_type_is_unknown(in_scope) && in_scope->kind == LSM_TYPE_NAMED) {
                ...
                if (imported_alias) {
                    py_emit_resolved_call_reason(ctx, qn, "lsp_import_alias", 0.85f, fname,
                                                 call_node);
                } else if (qn && lsm_registry_lookup_type(ctx->registry, qn)) {
    ...
    if (strcmp(fk, "attribute") == 0) {
        ...
        if (strcmp(ts_node_type(obj), "call") == 0) {
            ...
            if (!ts_node_is_null(super_fn) && strcmp(ts_node_type(super_fn), "identifier") == 0) {
                ...
                if (super_name && strcmp(super_name, "super") == 0 && ctx->enclosing_class_qn) {
                    const LSMRegisteredType *enclosing =
                    ...
                    if (enclosing && enclosing->embedded_types) {
                        for (int i = 0; enclosing->embedded_types[i]; i++) {
                            ...
                            if (strcmp(attr_name, "__init__") == 0) {
                                const LSMRegisteredFunc *fi = py_lookup_attribute(
                                    ctx, enclosing->embedded_types[i], attr_name);
                                const char *init_qn =
                                    fi ? fi->qualified_name
                                       : lsm_arena_sprintf(ctx->arena, "%s.__init__",
            ...
            if (mod && ctx->module_qn) {
                ...
                if (last_dot && last_dot > ctx->module_qn) {
                    ...
                    if (!(strncmp(mod, ctx->module_qn, root_len) == 0 && mod[root_len] == '.')) {
                        char *qual_mod = (char *)lsm_arena_alloc(ctx->arena, root_len + 1 +
                                                                                strlen(mod) + 1);
                        if (qual_mod) {
                            memcpy(qual_mod, ctx->module_qn, root_len);
                            qual_mod[root_len] = '.';
                            strcpy(qual_mod + root_len + 1, mod);
                            const LSMRegisteredFunc *qf =
                                lsm_registry_lookup_symbol(ctx->registry, qual_mod, attr_name);
                            if (qf) {
        ...
        if (obj_type->kind == LSM_TYPE_UNION) {
            ...
            for (int i = 0; i < obj_type->data.union_type.count; i++) {
                ...
                if (!m)
                    continue;
                if (m->kind == LSM_TYPE_BUILTIN && m->data.builtin.name &&
                    strcmp(m->data.builtin.name, "None") == 0)
                    continue;
                if (m->kind != LSM_TYPE_NAMED)
                    continue;
                const LSMRegisteredFunc *f =
                    py_lookup_attribute(ctx, m->data.named.qualified_name, attr_name);
                if (f) {
...
static const LSMType *py_strip_none(PyLSPContext *ctx, const LSMType *t) {
    if (!t || t->kind != LSM_TYPE_UNION)
        return t;
    int n = t->data.union_type.count;
    int retained = 0;
    const LSMType **kept =
        (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(const LSMType *));
    if (!kept)
        return t;
    for (int i = 0; i < n; i++) {
        const LSMType *m = t->data.union_type.members[i];
        if (m && m->kind == LSM_TYPE_BUILTIN && m->data.builtin.name &&
            strcmp(m->data.builtin.name, "None") == 0)
            continue;
        kept[retained++] = m;
    }
    ...
    if (retained == 1)
        return kept[0];
    return lsm_type_union(ctx->arena, kept, retained);
}
...
static void py_bind_walrus_in(PyLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "named_expression") == 0 || strcmp(k, "assignment_expression") == 0) {
        ...
        if (!ts_node_is_null(left) && !ts_node_is_null(right) &&
            ...
            char *name = py_node_text(ctx, left);
            if (name) {
                const LSMType *t = py_eval_expr_type(ctx, right);
...
static void py_walk_if_statement(PyLSPContext *ctx, TSNode if_node) {
    ...
    if (!ts_node_is_null(body)) {
        ...
        if (none_kind == 1 && none_var) {
            const LSMType *current = lsm_scope_lookup(ctx->current_scope, none_var);
            if (current && !lsm_type_is_unknown(current)) {
                const LSMType *narrowed = py_strip_none(ctx, current);
    ...
    if (!ts_node_is_null(body) && py_block_terminates(body)) {
        ...
        if (neg_none_kind == -1 && neg_none_var) {
            ...
            const LSMType *current = lsm_scope_lookup(ctx->current_scope, neg_none_var);
            if (current && !lsm_type_is_unknown(current)) {
                const LSMType *narrowed = py_strip_none(ctx, current);
...
static void py_emit_dunder_call(PyLSPContext *ctx, const LSMType *recv, const char *dunder,
                                TSNode site) {
    if (!recv || recv->kind != LSM_TYPE_NAMED || !dunder)
        return;
    const LSMRegisteredFunc *f = py_lookup_attribute(ctx, recv->data.named.qualified_name, dunder);
    if (f && f->qualified_name) {
         ...
         * `call` node, so the extractor produced no LSMCall for it and the
         * resolved_call above would never be matched into a CALLS edge. Inject
         * a synthetic LSMCall keyed on (enclosing_func_qn, short dunder name)
        ...
        if (ctx->syn_calls && ctx->arena && ctx->enclosing_func_qn) {
            ...
            for (int i = 0; i < ctx->syn_calls->count; i++) {
                const LSMCall *existing = &ctx->syn_calls->items[i];
            ...
            call.callee_name = lsm_arena_strdup(ctx->arena, dunder);
        ...
        const char *candidate =
            lsm_arena_sprintf(ctx->arena, "%s.%s", recv->data.named.qualified_name, dunder);
            ...
            for (uint32_t i = 0; i < bcc; i++) {
                ...
                if (subject_name && !ts_node_is_null(actual_pattern) &&
                    ...
                    if (ts_node_named_child_count(actual_pattern) > 0) {
                        ...
                        if (cls_text) {
                            const LSMType *narrowed = py_resolve_annotation(ctx, cls_text);
                    ...
                    for (uint32_t j = 1; j < ts_node_named_child_count(actual_pattern); j++) {
                        ...
                        if (strcmp(ts_node_type(sub), "capture_pattern") == 0) {
                            if (ts_node_named_child_count(sub) > 0) {
                                ...
                                if (nm)
                                    py_scope_bind(ctx, nm, lsm_type_unknown());
                            }
                        }
                    }
                }
                ...
                if (subject_name && !ts_node_is_null(actual_pattern) &&
                    ...
                    const LSMType *subj_t = lsm_scope_lookup(ctx->current_scope, subject_name);
                    const LSMType *elem_t = py_iterable_element_type(ctx, subj_t);
                    ...
                    for (uint32_t j = 0; j < pn; j++) {
                        ...
                        if (strcmp(ek, "splat_pattern") == 0 ||
                            ...
                            if (!ts_node_is_null(id)) {
                                ...
                                if (nm && elem_t) {
                                    const LSMType *lst =
                                        lsm_type_template(ctx->arena, "list", &elem_t, 1);
...
/* Parse a type-annotation TEXT into a LSMType without requiring a
...
static const LSMType *py_parse_type_text(LSMArena *arena, const char *ann);
static const LSMType *py_parse_type_text_qn(LSMArena *arena, const char *ann,
                                            const char *module_qn);

/* Trim ASCII whitespace from both ends of an arena-allocated copy. */
static char *py_trim_ws(LSMArena *arena, const char *start, size_t len) {
...
static const char **py_split_subscript_args(LSMArena *arena, const char *s, int *out_n) {
    if (!s) {
        *out_n = 0;
        return NULL;
    }
    int cap = 8;
    const char **out =
        (const char **)lsm_arena_alloc(arena, (size_t)(cap + 1) * sizeof(const char *));
    if (!out) {
        *out_n = 0;
        return NULL;
    }
    ...
    for (size_t i = 0; i <= len; i++) {
        ...
        if (c == '[' || c == '(' || c == '{')
            ...
            if (part && part[0]) {
                if (n >= cap) {
                    int new_cap = cap * 2;
                    const char **grown = (const char **)lsm_arena_alloc(
...
static const LSMType *py_parse_type_text_qn(LSMArena *arena, const char *ann,
    ...
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)lsm_arena_alloc(arena, len - 1);
        if (unquoted) {
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        ...
        char *base = (char *)lsm_arena_alloc(arena, base_len + 1);
        if (base) {
            ...
            char *args_text = (char *)lsm_arena_alloc(arena, inner_len + 1);
            if (args_text) {
                ...
                const LSMType **arg_types = NULL;
                if (arg_n > 0 && arg_strs) {
                    arg_types = (const LSMType **)lsm_arena_alloc(
                        arena, (size_t)(arg_n + 1) * sizeof(const LSMType *));
                    if (arg_types) {
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return lsm_type_optional(arena, arg_types[0]);
                    }
                }
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return lsm_type_union(arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return lsm_type_template(arena, btrim, arg_types, arg_n);
                }
...

    {
        ...
        if (out) {
            ...
            if (seen_pipe && onum >= 2) {
                const LSMType **members = (const LSMType **)lsm_arena_alloc(
                    arena, (size_t)(onum + 1) * sizeof(const LSMType *));
                if (members) {
                    for (int j = 0; j < onum; j++) {
                        members[j] = py_parse_type_text_qn(arena, out[j], module_qn);
                    }
                    return lsm_type_union(arena, members, onum);
                }
            }
        }
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return lsm_type_builtin(arena, ann);
        }
    }
    ...
    if (module_qn && !strchr(ann, '.')) {
        ...
        if (!is_typing) {
            const char *qn = lsm_arena_sprintf(arena, "%s.%s", module_qn, ann);
            return lsm_type_named(arena, qn);
        }
    }
    return lsm_type_named(arena, ann);
}
...
static const LSMType *py_signature_param_type_adapter(LSMArena *arena, const char *text,
                                                      void *parser_ctx) {
...
/* Resolve a type-annotation text into a LSMType. Tries: scope lookup
...
static const LSMType *py_resolve_annotation(PyLSPContext *ctx, const char *ann) {
    ...
    size_t len = strlen(ann);
    if (len >= 2 && (ann[0] == '"' || ann[0] == '\'') && ann[len - 1] == ann[0]) {
        char *unquoted = (char *)lsm_arena_alloc(ctx->arena, len - 1);
        if (unquoted) {
            memcpy(unquoted, ann + 1, len - 2);
            unquoted[len - 2] = '\0';
    ...
    if (lb && len > 0 && ann[len - 1] == ']') {
        size_t base_len = (size_t)(lb - ann);
        char *base = (char *)lsm_arena_alloc(ctx->arena, base_len + 1);
        if (base) {
            memcpy(base, ann, base_len);
            base[base_len] = '\0';
            ...
            size_t inner_start = (size_t)(lb - ann) + 1;
            size_t inner_len = len - inner_start - 1;
            char *args_text = (char *)lsm_arena_alloc(ctx->arena, inner_len + 1);
            if (args_text) {
                memcpy(args_text, ann + inner_start, inner_len);
                args_text[inner_len] = '\0';
                int arg_n = 0;
                const char **arg_strs = py_split_subscript_args(ctx->arena, args_text, &arg_n);
                // Recursively resolve each arg to a LSMType.
                const LSMType **arg_types = NULL;
                if (arg_n > 0 && arg_strs) {
                    arg_types = (const LSMType **)lsm_arena_alloc(
                        ctx->arena, (size_t)(arg_n + 1) * sizeof(const LSMType *));
                    if (arg_types) {
                        for (int i = 0; i < arg_n; i++) {
                ...
                if (strcmp(btrim, "Optional") == 0 || strcmp(btrim, "typing.Optional") == 0) {
                    if (arg_types && arg_n >= 1 && arg_types[0]) {
                        return lsm_type_optional(ctx->arena, arg_types[0]);
                    }
                }
                // Union[A, B, ...] -> UNION
                if (strcmp(btrim, "Union") == 0 || strcmp(btrim, "typing.Union") == 0) {
                    if (arg_types && arg_n > 0) {
                        return lsm_type_union(ctx->arena, arg_types, arg_n);
                    }
                }
                ...
                if (arg_types && arg_n > 0) {
                    return lsm_type_template(ctx->arena, btrim, arg_types, arg_n);
                }
    ...
    // X | Y top-level union (PEP 604).
    {
        ...
        for (size_t i = 0; i < len; i++) {
            ...
            if (c == '[' || c == '(' || c == '{')
                ...
                int cap = 4;
                const char **out = (const char **)lsm_arena_alloc(
                ...
                if (onum >= 2) {
                    const LSMType **members = (const LSMType **)lsm_arena_alloc(
                        ctx->arena, (size_t)(onum + 1) * sizeof(const LSMType *));
                    if (!members)
                        break;
                    for (int j = 0; j < onum; j++) {
                        members[j] = py_resolve_annotation(ctx, out[j]);
                    }
                    return lsm_type_union(ctx->arena, members, onum);
                }
                break;
            }
        }
    ...
    const LSMType *t = lsm_scope_lookup(ctx->current_scope, ann);
    if (!lsm_type_is_unknown(t))
        return t;
    if (ctx->module_qn) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, ann);
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, qn);
        if (rt)
            return lsm_type_named(ctx->arena, qn);
    }
    ...
    for (int i = 0; builtins[i]; i++) {
        if (strcmp(ann, builtins[i]) == 0) {
            return lsm_type_builtin(ctx->arena, ann);
        }
    }
    return lsm_type_named(ctx->arena, ann);
}
...
static void py_bind_parameters(PyLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (is_splat && t && !lsm_type_is_unknown(t)) {
            t = lsm_type_template(ctx->arena, "tuple", &t, 1);
        }
        if (is_kwargs && t && !lsm_type_is_unknown(t)) {
            const LSMType *str_t = lsm_type_builtin(ctx->arena, "str");
            const LSMType *args[3] = {str_t, t, NULL};
            t = lsm_type_template(ctx->arena, "dict", args, 2);
        }
        py_scope_bind(ctx, name, t);
    }
}
...
static void py_process_function(PyLSPContext *ctx, TSNode func_node, const char *container_qn) {
    ...
    ctx->enclosing_func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", base_qn, fname);
...
static void py_process_class(PyLSPContext *ctx, TSNode class_node) {
    ...
    ctx->enclosing_class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
            ...
            if (strcmp(ck, "expression_statement") == 0 && ts_node_named_child_count(c) > 0) {
                ...
                if (strcmp(ik, "assignment") == 0) {
                    ...
                    if (!ts_node_is_null(left) && !ts_node_is_null(ann) &&
                        ...
                        if (fname && atext) {
                            const LSMType *ft = py_resolve_annotation(ctx, atext);
...
static void py_bind_external_module_classes(PyLSPContext *ctx, TSNode root) {
    ...
    for (int i = 0; i < ctx->registry->type_count; i++) {
        const LSMRegisteredType *type = &ctx->registry->types[i];
        ...
        py_scope_bind(ctx, name, lsm_type_named(ctx->arena, qn));
    }
}
...
static void py_bind_module_class(PyLSPContext *ctx, TSNode class_node) {
    ...
    if (!qn) {
        py_disable_callable_value_proof(ctx);
        py_scope_bind(ctx, name, lsm_type_unknown());
        return;
    }
    const LSMRegisteredType *type = lsm_registry_lookup_type(ctx->registry, qn);
    py_scope_bind(ctx, name,
                  type ? lsm_type_named(ctx->arena, type->qualified_name) : lsm_type_unknown());
}
...
static void py_bind_module_function(PyLSPContext *ctx, TSNode func_node) {
    ...
    const LSMRegisteredFunc *func = lsm_registry_lookup_func(ctx->registry, qn);
    if (py_func_is_exact_callable_value(func)) {
        py_scope_bind_callable(ctx, name, lsm_type_unknown(), func->qualified_name);
    } else {
        py_scope_bind(ctx, name, lsm_type_unknown());
    }
}
...
static void py_invalidate_registry_module_functions(PyLSPContext *ctx,
    ...
    size_t prefix_len = strlen(ctx->module_qn);
    for (int i = 0; i < registry->func_count; i++) {
        const LSMRegisteredFunc *func = &registry->funcs[i];
        ...
        if (!suffix[0] || strchr(suffix, '.'))
            continue;
        py_scope_bind(ctx, func->short_name ? func->short_name : suffix, lsm_type_unknown());
    }
...
static void py_invalidate_module_bindings_for_wildcard(PyLSPContext *ctx) {
    if (!ctx || !ctx->current_scope)
        return;
    for (LSMScopeChunk *chunk = ctx->current_scope->chunks; chunk; chunk = chunk->next) {
...
static void py_invalidate_import_bindings(PyLSPContext *ctx, TSNode stmt) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (local) {
            py_scope_bind(ctx, local, lsm_type_unknown());
...
void py_lsp_process_file(PyLSPContext *ctx, TSNode root) {
    ...
    if (ctx->import_count > 0) {
        consumed_imports =
            (unsigned char *)lsm_arena_alloc(ctx->arena, (size_t)ctx->import_count);
        if (consumed_imports)
    ...
    const char *prev_func = ctx->enclosing_func_qn;
    ctx->enclosing_func_qn = lsm_arena_sprintf(ctx->arena, "%s.__module__", ctx->module_qn);
...
static bool py_register_def(LSMArena *arena, LSMTypeRegistry *reg, LSMDefinition *d,
                            const char *module_qn) {
                ...
                for (size_t k = lo; k <= hi; k++) {
                    ...
                    if (c == '[' || c == '(')
                        ...
                        if (eq >= name_end && name_end > s) {
                            ...
                            char *bname = (char *)lsm_arena_alloc(arena, blen + 1);
                            if (bname) {
                                ...
                                if (n < cap) {
                                    embedded[n++] =
                                        strchr(bname, '.')
                                            ? bname
                                            : lsm_arena_sprintf(arena, "%s.%s", module_qn, bname);
                                }
                            }
                        }
                        start = k + 1;
    ...
    if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
        ...
        // is the lsm_node_text of the whole annotation node, which is the
        ...
        if (prefer_full) {
            ...
            while (d->return_types[count])
                count++;
            ret_types = (const LSMType **)lsm_arena_alloc(arena, (size_t)(count + 1) *
        ...
        if (strcmp(d->label, "Method") == 0) {
            ...
            if (last_dot && last_dot != qn) {
                ...
                rf.receiver_type = lsm_arena_strndup(arena, qn, prefix_len);
            }
        }
...
/* ── lsm_run_py_lsp: single-file entry point ──────────────────── */

void lsm_run_py_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
                    TSNode root) {
    if (!arena || !result)
        return;
     ...
     * graph node. We run inside lsm_extract_file, before the pipeline mints
    ...
    lsm_python_stdlib_register(&reg, arena);
...
static const char **py_split_pipe(LSMArena *arena, const char *text) {
    if (!text || !text[0])
        return NULL;
    int count = 1;
    ...
    for (const char *p = text;; p++) {
        if (*p == '|' || *p == '\0') {
            ...
            char *s = (char *)lsm_arena_alloc(arena, n + 1);
            if (!s)
                return NULL;
            memcpy(s, start, n);
            s[n] = '\0';
...
/* Build a registry from LSMLSPDef[] supplied by the caller — covers both
 * the source file's own defs and cross-file referenced defs. */
static void py_register_lsp_defs(LSMArena *arena, LSMArena *idx_arena, LSMTypeRegistry *reg,
     ...
     * registry arena. */
    if (idx_arena) {
        lsm_registry_finalize_into(reg, idx_arena);
    }
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (ret_strs) {
                ...
                if (n > 0) {
                    ...
                    for (int j = 0; j < n; j++) {
                        ret_types[j] = lsm_type_named(arena, ret_strs[j]);
                    }
                    ret_types[n] = NULL;
                }
            }
...
void lsm_run_py_lsp_cross(LSMArena *arena, const char *source, int source_len,
...
LSMTypeRegistry *lsm_py_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
...
void lsm_run_py_lsp_cross_with_registry(LSMArena *arena, const char *source, int source_len,
                                        const char *module_qn, LSMTypeRegistry *reg,
                                        const char **import_names, const char **import_qns,
                                        int import_count, TSTree *cached_tree,
                                        LSMResolvedCallArray *out, LSMCallArray *synthetic_calls) {
...
void lsm_batch_py_lsp_cross(LSMArena *arena, LSMBatchPyLSPFile *files, int file_count,
                            LSMResolvedCallArray *out) {
    if (!arena || !files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        LSMBatchPyLSPFile *file = &files[f];
        ...
        lsm_run_py_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
        ...
        if (file_out.count > 0) {
            ...
            out[f].items = (LSMResolvedCall *)lsm_arena_alloc(arena, (size_t)file_out.count *
                                                                         sizeof(LSMResolvedCall));
            if (out[f].items) {
                for (int j = 0; j < file_out.count; j++) {
                    ...
                    dst->caller_qn =
                        src->caller_qn ? lsm_arena_strdup(arena, src->caller_qn) : NULL;
                    dst->callee_qn =
                        src->callee_qn ? lsm_arena_strdup(arena, src->callee_qn) : NULL;
...
#ifndef LSM_PYTHON_STDLIB_GENERATED
