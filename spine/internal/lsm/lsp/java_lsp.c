 ...
 *   - We populate a LSMTypeRegistry from this file's LSMDefinitions plus a
 ...
 *     compatibility (lsm_registry_lookup_method_by_types). Ambiguity ⇒
 ...
 *
 * Confidence policy (consumed by lsp_resolve.h with LSM_LSP_CONFIDENCE_FLOOR
 * = 0.6):
...
static const LSMType *eval_method_invocation(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_object_creation(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_field_access(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_array_access(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_cast(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_ternary(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_binary(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_unary(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_lambda(JavaLSPContext *ctx, TSNode node);
static const LSMType *eval_method_reference(JavaLSPContext *ctx, TSNode node);
static const LSMType *resolve_identifier_type(JavaLSPContext *ctx, const char *name);
static const LSMType *resolve_member_type(JavaLSPContext *ctx, const LSMType *recv_type,
                                          const char *member_name);
static void resolve_method_call(JavaLSPContext *ctx, TSNode call);
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, LSMTypeRegistry *reg,
                                                  LSMFileResult *result);
static const char *strip_generics(LSMArena *a, const char *type_text);
static const char *unwrap_array_text(LSMArena *a, const char *type_text, int *out_array_dim);
...
static const LSMType *box_primitive(LSMArena *a, const char *prim);
static int count_call_args(TSNode call_node);
static const LSMType *propagate_template(LSMArena *a, const char *recv_qn, const char *method_name,
                                         const LSMType *const *recv_targs, int recv_targ_count,
                                         const LSMType *return_t);
static bool method_implies_lambda_args(const char *recv_qn, const char *method_name,
                                       const LSMType *const *targs, int targ_count, int *out_arity,
                                       const LSMType **out_param0, const LSMType **out_param1);
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
                                     const LSMRegisteredFunc *outer_resolved, int arg_index,
                                     const LSMType *recv_type);
...
static const char *strip_generics(LSMArena *a, const char *type_text) {
    ...
    if (!lt)
        return type_text;
    return lsm_arena_strndup(a, type_text, (size_t)(lt - type_text));
}

/* Strip array suffix (`int[]` → `int`, `String[][]` → `String`, dim=2). */
static const char *unwrap_array_text(LSMArena *a, const char *type_text, int *out_dim) {
    if (out_dim)
        *out_dim = 0;
    if (!type_text)
        return NULL;
    ...
    if (dim == 0)
        return type_text;
    return lsm_arena_strndup(a, type_text, n);
}
...
static const LSMType *box_primitive(LSMArena *a, const char *prim) {
    ...
    for (size_t i = 0; i < sizeof(map) / sizeof(map[0]); i++) {
        if (strcmp(prim, map[i][0]) == 0)
            return lsm_type_named(a, map[i][1]);
    }
    return lsm_type_unknown();
}
...
void java_lsp_init(JavaLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
                   const LSMTypeRegistry *registry, const char *package_name, const char *module_qn,
...
void java_lsp_add_import(JavaLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_names =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_names));
        const char **new_qns =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_qns));
        int *new_kinds = (int *)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*new_kinds));
    ...
    ctx->import_target_qns[ctx->import_count] = lsm_arena_strdup(ctx->arena, target_qn);
...
static void push_enclosing_class(JavaLSPContext *ctx, const char *class_qn) {
    if (ctx->enclosing_class_depth >= ctx->enclosing_class_cap) {
        ...
        const char **arr =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*arr));
        if (!arr)
            return;
...
static const LSMType *parse_type_arguments(JavaLSPContext *ctx, TSNode targs_node);

const LSMType *java_parse_type_node(JavaLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node))
        return lsm_type_unknown();
    ...
    if (strcmp(kind, "scoped_type_identifier") == 0) {
        ...
        if (lsm_registry_lookup_type(ctx->registry, full)) {
            return lsm_type_named(ctx->arena, full);
        }
        /* Otherwise resolve the head segment and append the rest. */
        char *dot = strchr(full, '.');
        if (dot) {
            char *head = lsm_arena_strndup(ctx->arena, full, (size_t)(dot - full));
            const char *head_qn = java_resolve_type_name(ctx, head);
            if (head_qn) {
                return lsm_type_named(ctx->arena,
                                      lsm_arena_sprintf(ctx->arena, "%s%s", head_qn, dot));
            }
        }
        return lsm_type_named(ctx->arena, full);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        const LSMType *base = java_parse_type_node(ctx, raw);
        ...
        if (arg_count == 0) {
            arg_buf[arg_count++] = lsm_type_unknown();
        }
        const LSMType **args =
            (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(arg_count + 1) * sizeof(*args));
        for (int i = 0; i < arg_count; i++)
            args[i] = arg_buf[i];
        args[arg_count] = NULL;
        return lsm_type_template(ctx->arena, base_qn, args, arg_count);
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return lsm_type_slice(ctx->arena, java_parse_type_node(ctx, elem));
    }
...
const char *java_resolve_type_name(JavaLSPContext *ctx, const char *name) {
    ...
    for (int i = ctx->enclosing_class_depth - 1; i >= 0; i--) {
        ...
        if (n > 0 && (size_t)n < sizeof(buf)) {
            if (lsm_registry_lookup_type(ctx->registry, buf)) {
                return lsm_arena_strdup(ctx->arena, buf);
            }
        }
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != LSM_JAVA_IMPORT_TYPE)
            continue;
    ...
    /* 4. On-demand imports. */
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != LSM_JAVA_IMPORT_ON_DEMAND)
            continue;
        char buf[JAVA_LSP_BUF];
    ...
    if (lsm_registry_lookup_type(ctx->registry, name)) {
    ...
    if (ctx->registry && ctx->registry->types) {
        ...
        for (int i = 0; i < ctx->registry->type_count && matches < 2; i++) {
            const LSMRegisteredType *t = &ctx->registry->types[i];
...
const LSMType *java_eval_expr_type(JavaLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node))
        return lsm_type_unknown();
    ...
    if (strcmp(kind, "decimal_integer_literal") == 0 || strcmp(kind, "hex_integer_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'l' || txt[len - 1] == 'L')) {
                result = lsm_type_builtin(ctx->arena, "long");
                goto out;
            }
        }
        result = lsm_type_builtin(ctx->arena, "int");
        goto out;
    }
    if (strcmp(kind, "decimal_floating_point_literal") == 0 ||
        ...
        if (txt) {
            ...
            if (len > 0 && (txt[len - 1] == 'f' || txt[len - 1] == 'F')) {
                result = lsm_type_builtin(ctx->arena, "float");
                goto out;
            }
        }
        result = lsm_type_builtin(ctx->arena, "double");
        goto out;
    }
    ...
    if (strcmp(kind, "character_literal") == 0) {
        result = lsm_type_builtin(ctx->arena, "char");
        goto out;
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "string_fragment") == 0) {
        result = lsm_type_named(ctx->arena, "java.lang.String");
        goto out;
    }
    ...
    if (strcmp(kind, "super") == 0) {
        if (ctx->enclosing_super_qn) {
            result = lsm_type_named(ctx->arena, ctx->enclosing_super_qn);
        } else {
            result = lsm_type_named(ctx->arena, "java.lang.Object");
        }
        goto out;
    }
    ...
    if (strcmp(kind, "class_literal") == 0) {
        result = lsm_type_named(ctx->arena, "java.lang.Class");
        goto out;
    }
    ...
    if (strcmp(kind, "array_creation_expression") == 0) {
        ...
        const LSMType *elem = java_parse_type_node(ctx, type_node);
        result = lsm_type_slice(ctx->arena, elem);
        goto out;
    }
...
static const LSMType *resolve_identifier_type(JavaLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const LSMType *ft = java_lookup_field_type(ctx, ctx->enclosing_class_qn, name);
        if (ft && !lsm_type_is_unknown(ft))
            return ft;
    }
    /* Outer classes. */
    for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
        ...
        if (!outer)
            continue;
        const LSMType *ft = java_lookup_field_type(ctx, outer, name);
        if (ft && !lsm_type_is_unknown(ft))
            return ft;
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] != LSM_JAVA_IMPORT_STATIC)
            continue;
        if (strcmp(ctx->import_local_names[i], name) == 0) {
            ...
            if (last_dot) {
                char *cls = lsm_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
                const LSMType *ft = java_lookup_field_type(ctx, cls, name);
                if (ft && !lsm_type_is_unknown(ft))
                    return ft;
            }
            return lsm_type_unknown();
...
static const LSMType *eval_field_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname && strcmp(oname, "System") == 0) {
            if (strcmp(fname, "out") == 0 || strcmp(fname, "err") == 0) {
                return lsm_type_named(ctx->arena, "java.io.PrintStream");
            }
            if (strcmp(fname, "in") == 0) {
                return lsm_type_named(ctx->arena, "java.io.InputStream");
            }
        }
    }

    /* Special-case: `length` on array types is `int`. */
    const LSMType *recv = ts_node_is_null(obj) ? lsm_type_unknown() : java_eval_expr_type(ctx, obj);
    if (recv && recv->kind == LSM_TYPE_SLICE && strcmp(fname, "length") == 0) {
    ...
    if (!ts_node_is_null(obj) && strcmp(ts_node_type(obj), "this") == 0) {
        const LSMType *scope_t = lsm_scope_lookup(ctx->current_scope, fname);
        if (scope_t && !lsm_type_is_unknown(scope_t))
            return scope_t;
    }

    const LSMType *res = resolve_member_type(ctx, recv, fname);
    if (res && !lsm_type_is_unknown(res))
        return res;
...
const LSMType *java_lookup_field_type(JavaLSPContext *ctx, const char *class_qn,
                                      const char *field_name) {
    if (!class_qn || !field_name)
        return lsm_type_unknown();
    const char *cur = class_qn;
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const LSMType *resolve_member_type(JavaLSPContext *ctx, const LSMType *recv,
                                          const char *member_name) {
    if (!recv || !member_name)
        return lsm_type_unknown();
    ...
    if (recv->kind == LSM_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv->kind == LSM_TYPE_TEMPLATE)
...
const LSMRegisteredFunc *java_lookup_method(JavaLSPContext *ctx, const char *class_qn,
                                            const char *method_name, int arg_count) {
    if (!class_qn || !method_name)
        return NULL;
    const char *cur = class_qn;
    const LSMRegisteredFunc *fallback = NULL;
    for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur; hops++) {
        /* Try arg-count-aware lookup first. */
        const LSMRegisteredFunc *m =
            lsm_registry_lookup_method_by_args(ctx->registry, cur, method_name, arg_count);
        if (m)
            return m;
        /* Otherwise capture any name match as fallback. */
        if (!fallback) {
            fallback = lsm_registry_lookup_method(ctx->registry, cur, method_name);
        }
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, cur);
        if (!rt)
            break;
...
static const LSMType *java_return_type_of(const LSMRegisteredFunc *f) {
    if (!f || !f->signature)
        return lsm_type_unknown();
    if (f->signature->kind != LSM_TYPE_FUNC)
        return lsm_type_unknown();
    const LSMType *const *rets = f->signature->data.func.return_types;
    if (!rets || !rets[0])
        return lsm_type_unknown();
    return rets[0];
}
...
static const LSMType *substitute_generic_return(JavaLSPContext *ctx, const char *recv_qn,
                                                const char *method_name,
                                                const LSMType *const *arg_arr, int targ_count,
                                                const LSMType *fallback) {
    (void)ctx;
    ...
    if (fallback->kind != LSM_TYPE_NAMED)
        return fallback;
...
static const LSMType *eval_method_invocation(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = 0; i < ctx->import_count; i++) {
            ...
            if (!last_dot)
                continue;
            char *cls = lsm_arena_strndup(ctx->arena, target, (size_t)(last_dot - target));
            const LSMRegisteredFunc *f = java_lookup_method(ctx, cls, mname, arity);
            if (f)
                return java_return_type_of(f);
        }
        return lsm_type_unknown();
    ...
    if (strcmp(ts_node_type(obj), "super") == 0) {
        ...
        const LSMRegisteredFunc *f = java_lookup_method(ctx, super_qn, mname, arity);
        if (f)
            return java_return_type_of(f);
        return lsm_type_unknown();
    }
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        ...
        if (oname) {
            ...
            if (cls_qn) {
                ...
                if (lsm_type_is_unknown(lsm_scope_lookup(ctx->current_scope, oname))) {
                    const LSMRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                    if (f)
                        return java_return_type_of(f);
                }
            }
    ...
    /* Instance call. */
    const LSMType *recv = java_eval_expr_type(ctx, obj);
    const LSMType *base = recv;
    if (base && base->kind == LSM_TYPE_TEMPLATE) {
        const char *base_qn = base->data.template_type.template_name;
        if (base_qn) {
            const LSMRegisteredFunc *f = java_lookup_method(ctx, base_qn, mname, arity);
            if (f) {
                const LSMType *ret = java_return_type_of(f);
                /* Generic substitution + carrier propagation. */
                const LSMType *subst = substitute_generic_return(
...
static const LSMType *eval_array_access(JavaLSPContext *ctx, TSNode node) {
    ...
    if (ts_node_is_null(arr))
        return lsm_type_unknown();
    const LSMType *t = java_eval_expr_type(ctx, arr);
    if (t && t->kind == LSM_TYPE_SLICE)
...
static const LSMType *eval_ternary(JavaLSPContext *ctx, TSNode node) {
    ...
    const LSMType *t = java_eval_expr_type(ctx, then_n);
    if (t && !lsm_type_is_unknown(t))
        return t;
...
static bool is_string_concat(JavaLSPContext *ctx, TSNode node) {
    ...
    TSNode rhs = ts_node_child_by_field_name(node, "right", 5);
    const LSMType *l = java_eval_expr_type(ctx, lhs);
    const LSMType *r = java_eval_expr_type(ctx, rhs);
    if (l && l->kind == LSM_TYPE_NAMED &&
        strcmp(l->data.named.qualified_name, "java.lang.String") == 0)
        return true;
    if (r && r->kind == LSM_TYPE_NAMED &&
...
static const LSMType *eval_binary(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(op, "+") == 0 && is_string_concat(ctx, node)) {
        return lsm_type_named(ctx->arena, "java.lang.String");
    }
...
void java_process_statement(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "local_variable_declaration") == 0) {
        ...
        if (rn) {
            const LSMType *rt = lsm_type_unknown();
            bool is_var = false;
            ...
            if ((is_var || lsm_type_is_unknown(rt)) && !ts_node_is_null(rvalue)) {
                rt = java_eval_expr_type(ctx, rvalue);
            }
            lsm_scope_bind(ctx->current_scope, rn, rt ? rt : lsm_type_unknown());
        }
        ...
        if (!ts_node_is_null(formal)) {
            ...
            if (pn) {
                ...
                lsm_scope_bind(ctx->current_scope, pn, pt);
            }
        }
    }
...
static void process_local_var_decl(JavaLSPContext *ctx, TSNode node) {
    TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
    const LSMType *static_type =
        ts_node_is_null(type_node) ? lsm_type_unknown() : java_parse_type_node(ctx, type_node);
    bool is_var =
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!vname)
            continue;
        const LSMType *bind_type = static_type;
        if ((is_var || lsm_type_is_unknown(bind_type)) && !ts_node_is_null(value_node)) {
            bind_type = java_eval_expr_type(ctx, value_node);
        }
        lsm_scope_bind(ctx->current_scope, vname, bind_type ? bind_type : lsm_type_unknown());
    }
}
...
static void process_enhanced_for(JavaLSPContext *ctx, TSNode node) {
    ...
    const LSMType *t = (is_var || ts_node_is_null(type_node))
                           ? lsm_type_unknown()
                           : java_parse_type_node(ctx, type_node);

    /* When type is `var` or unparseable, infer from the iterable. */
    if (lsm_type_is_unknown(t) && !ts_node_is_null(value_node)) {
        const LSMType *iter_t = java_eval_expr_type(ctx, value_node);
        if (iter_t && iter_t->kind == LSM_TYPE_SLICE) {
            t = iter_t->data.slice.elem;
        } else if (iter_t && iter_t->kind == LSM_TYPE_TEMPLATE &&
    ...
    lsm_scope_bind(ctx->current_scope, vname, t ? t : lsm_type_unknown());
}
...
static void process_field_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (!fname)
            continue;
        lsm_scope_bind(ctx->current_scope, fname, static_type);
    }
}
...
static void process_method_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    if (!mname)
        return;
    char *method_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            if (!pn)
                continue;
            const LSMType *pt =
                ts_node_is_null(ptype) ? lsm_type_unknown() : java_parse_type_node(ctx, ptype);
            if (strcmp(pk, "spread_parameter") == 0 && pt) {
                pt = lsm_type_slice(ctx->arena, pt);
            }
            lsm_scope_bind(ctx->current_scope, pn, pt);
        }
    }
...
static void process_constructor_decl(JavaLSPContext *ctx, TSNode node, const char *class_qn,
    ...
    char *ctor_qn;
    if (cname) {
        ctor_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", class_qn, cname);
    } else {
        ctor_qn = lsm_arena_sprintf(ctx->arena, "%s.<init>", class_qn);
    }
...
static const char *class_super_qn(JavaLSPContext *ctx, TSNode class_node) {
    ...
    if (ts_node_is_null(tnode))
        return NULL;
    const LSMType *t = java_parse_type_node(ctx, tnode);
...
static void java_process_class_decl(JavaLSPContext *ctx, TSNode node) {
    ...
    char *class_qn;
    if (ctx->enclosing_class_qn) {
        class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = lsm_arena_strdup(ctx->arena, cname);
    }
...
void java_lsp_process_file(JavaLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(k, "package_declaration") == 0) {
            ...
            if (path) {
                if (is_on_demand) {
                    int kind = is_static ? LSM_JAVA_IMPORT_STATIC_OD : LSM_JAVA_IMPORT_ON_DEMAND;
                    ...
                    java_lsp_add_import(ctx, local, path, LSM_JAVA_IMPORT_STATIC);
                } else {
                    const char *last_dot = strrchr(path, '.');
                    const char *local = last_dot ? last_dot + 1 : path;
                    java_lsp_add_import(ctx, local, path, LSM_JAVA_IMPORT_TYPE);
                }
            }
        }
    }
...
static void java_emit_unresolved_kind(JavaLSPContext *ctx, const char *expr_text,
                                      const char *reason, LSMResolvedKind kind) {
...
static void java_emit_unresolved(JavaLSPContext *ctx, const char *expr_text, const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, LSM_RESOLVED_INVOCATION);
}

static void java_emit_unresolved_reference(JavaLSPContext *ctx, const char *expr_text,
                                           const char *reason) {
    java_emit_unresolved_kind(ctx, expr_text, reason, LSM_RESOLVED_CALL_REFERENCE);
}
...
static const char *java_find_sole_impl(JavaLSPContext *ctx, const char *iface_qn, const char *mname,
    ...
    for (int ti = 0; ti < ctx->registry->type_count && distinct < 2; ti++) {
         ...
         * name array. */
        bool has = lsm_registry_lookup_method(ctx->registry, cand->qualified_name, mname) != NULL;
        ...
        for (int hops = 0; hops < JAVA_LSP_MAX_INHERIT_HOPS && cur && !subtype; hops++) {
            const LSMRegisteredType *ct = lsm_registry_lookup_type(ctx->registry, cur);
...
static bool java_emit_interface_resolution(JavaLSPContext *ctx, const char *iface_qn,
    ...
    if (impl_count == 1 && sole_impl) {
        const LSMRegisteredFunc *cf = lsm_registry_lookup_method(ctx->registry, sole_impl, mname);
        const char *target =
            cf ? cf->qualified_name : lsm_arena_sprintf(ctx->arena, "%s.%s", sole_impl, mname);
        java_emit_resolved(ctx, target, "lsp_interface_resolve", 0.85f);
        return true;
    }
    if (impl_count >= 2) {
        java_emit_resolved(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", iface_qn, mname),
...
static void resolve_method_call(JavaLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(obj)) {
        ...
        for (int i = ctx->enclosing_class_depth - 2; i >= 0; i--) {
            ...
            if (!outer)
                continue;
            const LSMRegisteredFunc *f = java_lookup_method(ctx, outer, mname, arity);
            if (f) {
    ...
    if (strcmp(ts_node_type(obj), "identifier") == 0) {
        char *oname = java_node_text(ctx, obj);
        if (oname && lsm_type_is_unknown(lsm_scope_lookup(ctx->current_scope, oname))) {
            const char *cls_qn = java_resolve_type_name(ctx, oname);
            if (cls_qn) {
                const LSMRegisteredFunc *f = java_lookup_method(ctx, cls_qn, mname, arity);
                if (f) {
    ...
    if (recv_qn) {
        const LSMRegisteredFunc *f = java_lookup_method(ctx, recv_qn, mname, arity);
        if (f) {
            ...
            const LSMRegisteredType *rt0 = lsm_registry_lookup_type(ctx->registry, recv_qn);
        ...
        if (rt && rt->is_interface) {
            ...
            java_emit_resolved(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                               "lsp_interface_dispatch", 0.80f);
            return;
        }
        java_emit_unresolved(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", recv_qn, mname),
                             "no_method_match");
        return;
    }
 ...
 * `fi_qn` parameterized with `targs`. Returns lsm_type_unknown() when the
 * parametric mapping isn't modeled (e.g. specialized primitive variants). */
static const LSMType *sam_param_type(LSMArena *a, const char *fi_qn, int param_idx,
                                     const LSMType *const *targs, int targ_count) {
    if (!fi_qn)
        return lsm_type_unknown();
    ...
    for (int i = 0; single_t[i]; i++) {
        if (strcmp(single_t[i], fi_qn) == 0 && param_idx == 0 && targ_count >= 1) {
            return targs[0] ? targs[0] : lsm_type_unknown();
        }
    }
    ...
    if (strcmp(fi_qn, "java.util.function.BiFunction") == 0 ||
        ...
        if (param_idx <= 1 && targ_count > param_idx) {
            return targs[param_idx] ? targs[param_idx] : lsm_type_unknown();
        }
    }
 ...
 *
 * Used when the registry's LSMRegisteredFunc lacks param-type metadata
...
static const LSMType *propagate_template(LSMArena *a, const char *recv_qn, const char *method_name,
                                         const LSMType *const *recv_targs, int recv_targ_count,
                                         const LSMType *return_t) {
    if (!return_t || return_t->kind != LSM_TYPE_NAMED)
        return return_t;
    ...
    if (is_t0) {
        const LSMType **args = (const LSMType **)lsm_arena_alloc(a, 2 * sizeof(*args));
        if (!args)
            return return_t;
        args[0] = recv_targs[0];
        args[1] = NULL;
        return lsm_type_template(a, ret_qn, args, 1);
    }
    ...
    if (is_map_like(recv_qn) && recv_targ_count >= 2) {
        ...
        if (strcmp(method_name, "keySet") == 0) {
            args[0] = recv_targs[0];
            args[1] = NULL;
            return lsm_type_template(a, "java.util.Set", args, 1);
        }
        if (strcmp(method_name, "values") == 0) {
            args[0] = recv_targs[1];
            args[1] = NULL;
            return lsm_type_template(a, "java.util.Collection", args, 1);
        }
    }
    return return_t;
}

/* Given a method-invocation node and its resolved LSMRegisteredFunc, walk
...
static uint32_t bind_lambda_args(JavaLSPContext *ctx, TSNode call_node,
                                 const LSMRegisteredFunc *resolved, const LSMType *recv_type) {
    uint32_t handled_mask = 0;
    TSNode args_node = ts_node_child_by_field_name(call_node, "arguments", 9);
    if (ts_node_is_null(args_node))
        return handled_mask;
    const LSMType *const *param_types = NULL;
    if (resolved && resolved->signature && resolved->signature->kind == LSM_TYPE_FUNC) {
    ...
    const LSMType *const *recv_targs = NULL;
    int recv_targ_count = 0;
    const char *recv_qn = NULL;
    if (recv_type && recv_type->kind == LSM_TYPE_TEMPLATE) {
    ...
    } else if (recv_type && recv_type->kind == LSM_TYPE_NAMED) {
    ...
    for (uint32_t i = 0; i < n && i < 32; i++) {
        ...
        if (!expected && recv_qn && mname && recv_targ_count > 0) {
            ...
            int h_arity = 0;
            const LSMType *h_p0 = NULL;
            const LSMType *h_p1 = NULL;
                ...
                if (!ts_node_is_null(body_node)) {
                    ...
                    if (!ts_node_is_null(params_node)) {
                        ...
                        if (strcmp(pn_kind, "identifier") == 0) {
                            ...
                            if (pname)
                                lsm_scope_bind(ctx->current_scope, pname, h_p0);
                            ...
                            for (uint32_t pi = 0; pi < pc; pi++) {
                                ...
                                const char *pk = ts_node_type(p);
                                char *pname = NULL;
                                const LSMType *pt = (idx == 0) ? h_p0 : h_p1;
        ...
        const LSMType *const *fi_targs = NULL;
        int fi_targ_count = 0;
        if (expected->kind == LSM_TYPE_NAMED) {
            fi_qn = expected->data.named.qualified_name;
        } else if (expected->kind == LSM_TYPE_TEMPLATE) {
         ...
         * cases below. */
        const LSMType *resolved_fi_targs[8] = {0};
        ...
        if (!ts_node_is_null(params_node)) {
            ...
            if (strcmp(pn_kind, "identifier") == 0) {
                ...
                for (uint32_t pi = 0; pi < pc; pi++) {
                    ...
                    if (pname) {
                        lsm_scope_bind(ctx->current_scope, pname, pt ? pt : lsm_type_unknown());
                        idx++;
                    }
                }
            }
 ...
 *
 * Emits a LSMResolvedCall edge for the referenced method when we can pin
...
static void resolve_method_reference(JavaLSPContext *ctx, TSNode mref,
    ...
    if (outer_resolved && outer_resolved->signature &&
        outer_resolved->signature->kind == LSM_TYPE_FUNC &&
        ...
        const LSMType *const *pts = outer_resolved->signature->data.func.param_types;
        int ptc = 0;
        while (pts[ptc]) {
            ptc++;
        }
        const LSMType *expected = (arg_index >= 0 && arg_index < ptc) ? pts[arg_index] : NULL;
        if (expected) {
            const char *fi_qn = NULL;
            if (expected->kind == LSM_TYPE_NAMED)
                fi_qn = expected->data.named.qualified_name;
    ...
    if (lhs_is_type) {
        ...
        if (lhs_t && lhs_t->kind == LSM_TYPE_NAMED)
            type_qn = lhs_t->data.named.qualified_name;
        else if (lhs_t && lhs_t->kind == LSM_TYPE_TEMPLATE)
    ...
    if (strcmp(mname, "new") == 0) {
        ...
        const LSMRegisteredFunc *cf =
            lsm_registry_lookup_method(ctx->registry, type_qn, short_name);
    ...
    if (m) {
        ...
        java_emit_unresolved_reference_at(ctx,
                                          lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
...
static const LSMRegisteredFunc *lookup_method_for_call(JavaLSPContext *ctx, TSNode call,
                                                       const LSMType **out_recv_type) {
    if (out_recv_type)
        *out_recv_type = NULL;
    TSNode obj = ts_node_child_by_field_name(call, "object", 6);
    TSNode name_node = ts_node_child_by_field_name(call, "name", 4);
    ...
    if (recv && recv->kind == LSM_TYPE_NAMED)
        recv_qn = recv->data.named.qualified_name;
    else if (recv && recv->kind == LSM_TYPE_TEMPLATE)
        recv_qn = recv->data.template_type.template_name;
    if (recv_qn)
...
static void java_resolve_calls_in_node_inner(JavaLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "method_invocation") == 0) {
        ...
        const LSMType *recv_type = NULL;
        const LSMRegisteredFunc *outer = lookup_method_for_call(ctx, node, &recv_type);
        if (outer) {
        ...
        TSNode type_node = ts_node_child_by_field_name(node, "type", 4);
        if (!ts_node_is_null(type_node)) {
            const LSMType *t = java_parse_type_node(ctx, type_node);
            const char *qn = NULL;
            if (t && t->kind == LSM_TYPE_NAMED)
                qn = t->data.named.qualified_name;
            else if (t && t->kind == LSM_TYPE_TEMPLATE)
            ...
            if (qn) {
                ...
                const LSMRegisteredFunc *cf =
                    lsm_registry_lookup_method_by_args(ctx->registry, qn, short_name, arity);
                if (!cf)
                    cf = lsm_registry_lookup_method(ctx->registry, qn, short_name);
                if (cf) {
...
static const char **split_generic_args(LSMArena *a, const char *inside, int *out_count) {
    *out_count = 0;
    if (!inside || !inside[0])
        return NULL;
    const char *args[16];
    ...
    for (const char *p = inside; *p; p++) {
        if (*p == '<')
            ...
            if (count < 16 && seg_end > seg_start) {
                args[count++] = lsm_arena_strndup(a, seg_start, (size_t)(seg_end - seg_start));
            }
            seg_start = p + 1;
        }
    }
    ...
    if (count == 0)
        return NULL;
    const char **result = (const char **)lsm_arena_alloc(a, (size_t)(count + 1) * sizeof(*result));
...
/* Parse a type-text into a LSMType, with full inner-class qualification.
...
static const LSMType *parse_param_text_full(LSMArena *a, const char *text, const char *parent_class,
    ...
    if (no_arr) {
        ...
        if (lt) {
            ...
            if (gt && gt > lt + 1) {
                gen_args_inner = lsm_arena_strndup(a, lt + 1, (size_t)(gt - lt - 1));
            }
        }
    }

    const LSMType *base = NULL;
    if (no_gen && is_java_primitive(no_gen)) {
        base = lsm_type_builtin(a, no_gen);
    } else if (no_gen) {
        if (strchr(no_gen, '.')) {
            base = lsm_type_named(a, no_gen);
        } else {
            /* Try java.lang well-knowns. */
            for (int i = 0; JAVA_LANG_TYPES[i]; i++) {
                if (strcmp(JAVA_LANG_TYPES[i], no_gen) == 0) {
                    base = lsm_type_named(a, lsm_arena_sprintf(a, "java.lang.%s", no_gen));
                    break;
                }
            }
            ...
            if (!base && parent_class && reg) {
                ...
                while (cur) {
                    const char *cand = lsm_arena_sprintf(a, "%s.%s", cur, no_gen);
                    if (lsm_registry_lookup_type(reg, cand)) {
                        base = lsm_type_named(a, cand);
                        break;
                    }
                    const char *last_dot = strrchr(cur, '.');
                    if (!last_dot)
                        break;
                    cur = lsm_arena_strndup(a, cur, (size_t)(last_dot - cur));
                }
            }
            ...
            if (!base && reg) {
                for (int i = 0; JAVA_FALLBACK_PACKAGES[i]; i++) {
                    const char *cand =
                        lsm_arena_sprintf(a, "%s.%s", JAVA_FALLBACK_PACKAGES[i], no_gen);
                    if (lsm_registry_lookup_type(reg, cand)) {
                        base = lsm_type_named(a, cand);
                        break;
                    }
            ...
            if (!base) {
                if (module_qn && module_qn[0]) {
                    base = lsm_type_named(a, lsm_arena_sprintf(a, "%s.%s", module_qn, no_gen));
                } else {
                    base = lsm_type_named(a, no_gen);
                }
            }
    ...
    if (!base)
        base = lsm_type_unknown();
    /* If we extracted generic args and the base is NAMED, wrap as TEMPLATE
     * with each arg recursively parsed. */
    if (gen_args_inner && base && base->kind == LSM_TYPE_NAMED) {
        int gc = 0;
        const char **arg_strs = split_generic_args(a, gen_args_inner, &gc);
        if (arg_strs && gc > 0) {
            const LSMType **gargs =
                (const LSMType **)lsm_arena_alloc(a, (size_t)(gc + 1) * sizeof(*gargs));
            for (int i = 0; i < gc; i++) {
                gargs[i] = parse_param_text_full(a, arg_strs[i], parent_class, module_qn, reg);
            }
            gargs[gc] = NULL;
            base = lsm_type_template(a, base->data.named.qualified_name, gargs, gc);
        }
    }
    while (dim-- > 0)
        base = lsm_type_slice(a, base);
    return base;
}

/* Backwards-compatible wrapper for callers that don't know the parent_class
 * context. Used by lsm_run_java_lsp_cross. */
static const LSMType *parse_param_text(LSMArena *a, const char *text, const char *module_qn) {
...
static const LSMType *java_signature_parse_type(LSMArena *arena, const char *text,
                                                void *parser_ctx) {
...
static void register_local_func_or_type_from_file(JavaLSPContext *ctx, LSMTypeRegistry *reg,
     ...
     * registry entry in place via the type lookup (LSMRegisteredType *). */
    for (int i = 0; i < result->defs.count; i++) {
        LSMDefinition *d = &result->defs.items[i];
        if (!d->qualified_name || !d->name || !d->label)
            continue;
        ...
        const LSMRegisteredType *existing = lsm_registry_lookup_type(reg, d->qualified_name);
        if (!existing)
            continue;
        LSMRegisteredType rt = *existing; /* Will be re-added; lookup_type returns the
                                             registry's stored copy. We instead build a
                                             local copy and let lsm_registry_add_type's
        ...
        if (d->base_classes) {
            ...
            if (bc_count > 0) {
                const char **emb =
                    (const char **)lsm_arena_alloc(a, (size_t)(bc_count + 1) * sizeof(*emb));
                    ...
                    if (!strchr(no_gen, '.')) {
                        ...
                        for (int k = 0; JAVA_LANG_TYPES[k]; k++) {
                            if (strcmp(JAVA_LANG_TYPES[k], no_gen) == 0) {
                                resolved = lsm_arena_sprintf(a, "java.lang.%s", no_gen);
                                break;
                            }
                        }
                        ...
                        if (!parent_qn && d->qualified_name) {
                            ...
                            if (last_dot_qn) {
                                parent_qn =
                                    lsm_arena_strndup(a, d->qualified_name,
                        ...
                        if (!resolved && module_qn && module_qn[0]) {
                            resolved = lsm_arena_sprintf(a, "%s.%s", module_qn, no_gen);
                        }
                        emb[j] = resolved ? resolved : lsm_arena_strdup(a, no_gen);
                    } else {
                        emb[j] = lsm_arena_strdup(a, no_gen);
                    }
                }
                emb[bc_count] = NULL;
        ...
        if (!slot->embedded_types && !slot->is_interface) {
            const char **emb = (const char **)lsm_arena_alloc(a, 2 * sizeof(*emb));
            if (emb) {
     ...
     * since LSMDefinition doesn't carry a per-field type for Java. See
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->signature_param_types || d->signature_param_count > 0) {
            ...
            ptypes = lsm_type_materialize_signature_params(a, d->signature_param_types,
                                                           d->signature_param_count,
                                                           java_signature_parse_type, &parse_ctx);
        } else if (!d->signature_param_types && d->signature_param_count == 0) {
            /* Compatibility for callers that still construct LSMDefinition by
            ...
            if (d->param_types) {
                ...
                if (pc > 0) {
                    ptypes =
                        (const LSMType **)lsm_arena_alloc(a, (size_t)(pc + 1) * sizeof(*ptypes));
                    for (int j = 0; j < pc; j++) {
                        ptypes[j] =
        ...
        const LSMType **rtypes = NULL;
        if (d->return_type && d->return_type[0]) {
            rtypes = (const LSMType **)lsm_arena_alloc(a, 2 * sizeof(*rtypes));
        ...
        rf.signature = lsm_type_func(a, param_names, ptypes, rtypes);
        if (d->parent_class) {
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, LSMTypeRegistry *reg,
                                             TSNode class_node, const char *enclosing_class_qn);

static void patch_one_method(JavaLSPContext *ctx, LSMTypeRegistry *reg, TSNode method_node,
    ...
    const LSMType **ptypes = NULL;
    if (pn > 0) {
        ptypes = (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(pn + 1) * sizeof(*ptypes));
        if (!ptypes)
            return;
    }
    int idx = 0;
    ...
    LSMRegisteredFunc *slot = NULL;
    LSMRegisteredFunc *first_qn_match = NULL;
    for (int fi = 0; fi < reg->func_count; fi++) {
        LSMRegisteredFunc *candidate = &reg->funcs[fi];
        if (!candidate->qualified_name || strcmp(candidate->qualified_name, method_qn) != 0)
            continue;
        if (!first_qn_match)
            first_qn_match = candidate;
        if (!candidate->signature || candidate->signature->kind != LSM_TYPE_FUNC)
            continue;
        int candidate_arity = 0;
        const LSMType *const *candidate_params = candidate->signature->data.func.param_types;
        if (candidate_params) {
    ...
    if (!is_constructor) {
        ...
        if (!ts_node_is_null(rtype)) {
            rtypes = (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
    ...
    if (is_constructor) {
        rtypes = (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(*rtypes));
        rtypes[0] = lsm_type_named(ctx->arena, class_qn);
        rtypes[1] = NULL;
    }

    /* Rebuild the FUNC type with both param-types and return-types. */
    slot->signature = lsm_type_func(ctx->arena, NULL, ptypes, rtypes);
}
...
static void patch_method_signatures_from_ast(JavaLSPContext *ctx, LSMTypeRegistry *reg,
    ...
    if (enclosing_class_qn) {
        class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", enclosing_class_qn, cname);
    } else if (ctx->module_qn && ctx->module_qn[0]) {
        class_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    } else {
        class_qn = lsm_arena_strdup(ctx->arena, cname);
...
static void append_field_to_class(LSMTypeRegistry *reg, LSMArena *a, const char *class_qn,
                                  const char *field_name, const LSMType *ftype) {
    LSMRegisteredType *slot = NULL;
    for (int ti = 0; ti < reg->type_count; ti++) {
    ...
    const char **new_names =
        (const char **)lsm_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_names));
    const LSMType **new_types =
        (const LSMType **)lsm_arena_alloc(a, (size_t)(existing + 2) * sizeof(*new_types));
    ...
    new_names[existing] = lsm_arena_strdup(a, field_name);
    new_types[existing] = ftype ? ftype : lsm_type_unknown();
...
static void populate_class_fields_from_ast(JavaLSPContext *ctx, LSMTypeRegistry *reg,
                                           TSNode class_node, const char *enclosing_class_qn) {
    if (ts_node_is_null(class_node))
        return;
    const char *kind = ts_node_type(class_node);
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ck, "field_declaration") == 0) {
            ...
            const LSMType *ftype =
                ts_node_is_null(tn) ? lsm_type_unknown() : java_parse_type_node(ctx, tn);
...
/* ── Top-level entry: lsm_run_java_lsp ────────────────────────────── */

extern const TSLanguage *tree_sitter_java(void);

void lsm_run_java_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
                      TSNode root) {
    ...
    lsm_java_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->imports.count; i++) {
        ...
        if (!imp->local_name || !imp->module_path)
            continue;
        java_lsp_add_import(&ctx, imp->local_name, imp->module_path, LSM_JAVA_IMPORT_TYPE);
    }
...
void lsm_java_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, const LSMLSPDef *defs,
                                int def_count) {
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
            ...
            const LSMType **ptypes = lsm_type_materialize_signature_params(
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                const char *first;
                if (bar) {
                    first = lsm_arena_strndup(arena, no_pipe, (size_t)(bar - no_pipe));
                } else {
                    first = no_pipe;
                }
            ...
            const LSMType **rtypes = NULL;
            if (ret) {
                rtypes = (const LSMType **)lsm_arena_alloc(arena, 2 * sizeof(*rtypes));
                rtypes[0] = ret;
                rtypes[1] = NULL;
            }
            rf.signature = lsm_type_func(arena, NULL, ptypes, rtypes);
            lsm_registry_add_func(reg, rf);
        }
    }
}
 ...
 * internal/lsm layer free of src/pipeline includes. */
extern void lsm_pxc_count_perfile_defs(uint64_t defs);

LSMTypeRegistry *lsm_java_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    if (!arena) {
        return NULL;
    }
     ...
     * bare type names via lsm_registry_lookup_type. Before finalize that lookup
    ...
    LSMLSPDef *jvm = NULL;
    if (def_count > 0) {
        jvm = (LSMLSPDef *)lsm_arena_alloc(arena, (size_t)def_count * sizeof(*jvm));
        if (!jvm) {
            return NULL;
        }
    }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if ((defs[i].lang == LSM_LANG_JAVA || defs[i].lang == LSM_LANG_KOTLIN) && lb &&
    ...
    for (int i = 0; i < def_count; i++) {
        const char *lb = defs[i].label;
        if ((defs[i].lang == LSM_LANG_JAVA || defs[i].lang == LSM_LANG_KOTLIN) &&
            !(lb && (strcmp(lb, "Class") == 0 || strcmp(lb, "Interface") == 0 ||
                     strcmp(lb, "Enum") == 0 || strcmp(lb, "Type") == 0))) {
            jvm[total++] = defs[i];
        }
    }
    lsm_java_register_lsp_defs(arena, reg, jvm, type_count);
    lsm_registry_finalize(reg);
    lsm_java_register_lsp_defs(arena, reg, jvm + type_count, total - type_count);
    lsm_registry_finalize(reg);
...
void lsm_run_java_lsp_cross_with_registry(LSMArena *arena, LSMFileResult *result,
                                          const char *source, int source_len, const char *module_qn,
                                          LSMTypeRegistry *reg, const char **import_names,
    ...
    lsm_pxc_count_perfile_defs((uint64_t)result->defs.count);
    ...
    lsm_registry_finalize_into(&overlay, &idx_arena);
    ...
    for (int i = 0; i < import_count; i++) {
        ...
        java_lsp_add_import(&ctx, import_names[i], import_qns[i], LSM_JAVA_IMPORT_TYPE);
    }
...
void lsm_run_java_lsp_cross(LSMArena *arena, const char *source, int source_len,
    ...
    lsm_java_register_lsp_defs(arena, &reg, defs, def_count);
...
void lsm_batch_java_lsp_cross(LSMArena *arena, LSMBatchJavaLSPFile *files, int file_count,
                              LSMResolvedCallArray *out) {
    if (!files || file_count <= 0)
        return;
    for (int i = 0; i < file_count; i++) {
        lsm_run_java_lsp_cross(arena, files[i].source, files[i].source_len, files[i].module_qn,
