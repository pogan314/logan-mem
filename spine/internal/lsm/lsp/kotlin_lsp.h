 ...
 * scope, infers expression types, and emits LSMResolvedCall entries for
 ...
 * call-edge and method-dispatch attribution problems LSM cares about.
 *
 ...
 */
#ifndef LSM_LSP_KOTLIN_LSP_H
#define LSM_LSP_KOTLIN_LSP_H

#include "type_rep.h"
#include "scope.h"
#include "type_registry.h"
#include "../lsm.h"
#include "go_lsp.h" /* LSMLSPDef, LSMResolvedCallArray reused across languages */
...
typedef enum {
    LSM_KT_USE_UNKNOWN = 0,
    LSM_KT_USE_TYPE,     /* class / interface / object / typealias */
    LSM_KT_USE_FUNCTION, /* top-level fun, extension fun */
    LSM_KT_USE_PROPERTY, /* top-level val/var */
    LSM_KT_USE_WILDCARD, /* import a.b.* — local_name is a.b prefix */
} LSMKotlinUseKind;

typedef struct {
    const LSMScope *scope; /* exact lexical declaration frame, NULL for members */
    const char *owner_qn;  /* class/object QN for a member, NULL for lexical values */
    const char *name;
    const char *getter_qn;
    const char *setter_qn;
} LSMKotlinDelegateBinding;
...
typedef struct KotlinLSPContext {
    ...
    LSMKotlinDelegateBinding *delegate_bindings;
    int delegate_count;
    int delegate_cap;
     ...
     * QN, built from the project defs by lsm_run_kotlin_lsp_cross. Consulted
     ...
     * lsm_run_kotlin_lsp_cross; opaque pointer to avoid a foundation include
     * in this header. */
    const void *cross_type_short;
    /* Cross-file only (NULL per-file): receiver type QN -> KtCrossFieldList,
     * from lsm_kotlin_register_lsp_defs. Property-reference emission reads
 ...
 * pass the package prefix as target_qn and LSM_KT_USE_WILDCARD as kind. */
...
/* Evaluate a Kotlin expression's type. Returns lsm_type_unknown() on
 * failure — never NULL. */
 ...
 * Called from lsm_extract_file() after definitions and imports have been
 * extracted. */
 ...
 * Mirrors lsm_run_java_lsp_cross. `defs` carries the graph QNs of every
 ...
 * list map built during registration (opaque LSMHashTable; the caller frees
 * with lsm_ht_free). Pass NULL to discard it. The forward declaration lives at
...
void lsm_kotlin_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, const LSMLSPDef *defs,
                                  int def_count, struct LSMHashTable **out_field_map);
...
void lsm_kotlin_stdlib_register(LSMTypeRegistry *reg, LSMArena *arena);

/* Register Kotlin default-import targets — the prefixes auto-imported
 * by every Kotlin file. Used by the LSP context init. */
const char *const *lsm_kotlin_default_import_packages(int *count_out);

#endif /* LSM_LSP_KOTLIN_LSP_H */
