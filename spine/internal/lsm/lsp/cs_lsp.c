 ...
 * a LSMScope stack plus a CSLSPContext that tracks type/namespace/using
 * state that does not naturally live in the value scope.
 *
 * Resolution order for a bare identifier `X` inside member `M` in type
 * `T` in namespace `N`:
 *
 *    1. local variables / parameters in the current block          (LSMScope)
...
static const LSMType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call);
static const LSMType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node);
static const LSMType *cs_eval_object_creation_type(CSLSPContext *ctx, TSNode node);
static const LSMType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node);
static const LSMType *cs_substitute_type_params(LSMArena *arena, const LSMType *t,
                                                const char **param_names,
                                                const LSMType **param_args);
...
static void cs_register_type_decls(CSLSPContext *ctx, LSMTypeRegistry *reg, TSNode root);
static char *cs_node_text_cached(CSLSPContext *ctx, TSNode node);
static const LSMType *cs_unwrap_task(CSLSPContext *ctx, const LSMType *t);
static const LSMType *cs_unwrap_nullable(const LSMType *t);
...
static char *cs_normalize_name(LSMArena *a, const char *name) {
    if (!name)
        return NULL;
...
static char *cs_strip_generic_args(LSMArena *a, const char *name) {
    if (!name)
        return NULL;
    ...
    return lsm_arena_strndup(a, name, len);
}
...
void cs_lsp_init(CSLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
    ...
    cs_lsp_add_using(ctx, LSM_CS_USING_NAMESPACE, "", "System", false);
...
void cs_lsp_add_using(CSLSPContext *ctx, LSMCSUsingKind kind, const char *local_name,
    ...
    if (ctx->using_count >= ctx->using_cap) {
        ...
        LSMCSUsing *nu = (LSMCSUsing *)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nu));
        if (!nu)
            return;
        for (int i = 0; i < ctx->using_count; i++)
            nu[i] = ctx->usings[i];
    ...
    LSMCSUsing *slot = &ctx->usings[ctx->using_count++];
    slot->kind = kind;
    slot->local_name = local_name ? lsm_arena_strdup(ctx->arena, local_name) : "";
    slot->target_qn = lsm_arena_strdup(ctx->arena, target_qn);
...
static void cs_namespace_push(CSLSPContext *ctx, const char *ns_name) {
    ...
    ctx->namespace_stack[ctx->namespace_count++] = lsm_arena_strdup(ctx->arena, ns_name);
}
...
static const char *cs_namespace_qn(CSLSPContext *ctx) {
    ...
    char *out = (char *)lsm_arena_alloc(ctx->arena, total + 1);
    if (!out)
        return "";
    size_t w = 0;
    for (int i = 0; i < ctx->namespace_count; i++) {
...
static const LSMRegisteredType *cs_lookup_type_qn(CSLSPContext *ctx, const char *qn) {
    if (!ctx->registry || !qn)
        return NULL;
    return lsm_registry_lookup_type(ctx->registry, qn);
}
...
const char *cs_resolve_type_name(CSLSPContext *ctx, const char *raw) {
    ...
    for (int i = 0; i < ctx->type_param_count; i++) {
        if (strcmp(ctx->type_param_names[i], bare) == 0) {
            const LSMType *arg = ctx->type_param_args[i];
            if (arg && arg->kind == LSM_TYPE_NAMED)
    ...
    if (ctx->enclosing_class_qn) {
        const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, bare);
    ...
    if (ctx->namespace_count > 0) {
        ...
        for (;;) {
            if (!ns || !*ns)
                break;
            const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
            if (cs_lookup_type_qn(ctx, try_qn))
                return try_qn;
            const char *dot = strrchr(ns, '.');
            if (!dot)
                break;
            char *trim = lsm_arena_strndup(ctx->arena, ns, (size_t)(dot - ns));
            ns = trim;
        }
    }
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn))
            return try_qn;
    }
    ...
    for (int i = 0; i < ctx->using_count; i++) {
        const LSMCSUsing *u = &ctx->usings[i];
        if (u->kind != LSM_CS_USING_NAMESPACE)
            continue;
        const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", u->target_qn, bare);
        if (cs_lookup_type_qn(ctx, try_qn))
            return try_qn;
    }
        ...
        if (strncmp(bare, u->local_name, alias_len) == 0) {
            if (bare[alias_len] == '\0') {
                ...
                if (target_bare && cs_lookup_type_qn(ctx, target_bare))
                    return target_bare;
                return lsm_arena_strdup(ctx->arena, u->target_qn);
            }
            if (bare[alias_len] == '.') {
                /* `Alias.Sub` -> `Target.Sub` */
                char *target_bare = cs_strip_generic_args(ctx->arena, u->target_qn);
                const char *base = target_bare ? target_bare : u->target_qn;
                const char *try_qn = lsm_arena_sprintf(ctx->arena, "%s%s", base, bare + alias_len);
                if (cs_lookup_type_qn(ctx, try_qn))
                    return try_qn;
            }
        }
     ...
     * `namespace` declaration doesn't line up with the directory tree. */
    {
        ...
        if (the_short && *the_short) {
            ...
            LSMTypeShortIter ts_it;
            int i;
            if (ctx->registry) {
                lsm_registry_types_by_short_name(ctx->registry, the_short, &ts_it);
            }
            while (ctx->registry && (i = lsm_type_short_iter_next(&ts_it)) >= 0) {
...
const LSMRegisteredFunc *cs_lookup_method(CSLSPContext *ctx, const char *type_qn,
    ...
    const LSMRegisteredFunc *f = lsm_registry_lookup_method(ctx->registry, type_qn, method_name);
    if (f)
        return f;

    /* Walk inheritance chain (base + interfaces + transitive bases). */
    const LSMRegisteredType *t = cs_lookup_type_qn(ctx, type_qn);
    if (!t)
        return NULL;
    ...
    while (frontier_count > 0 && visited_count < CS_LSP_PARENT_WALK_MAX) {
        ...
        const LSMRegisteredType *next = cs_lookup_type_qn(ctx, parent);
        if (!next)
            continue;
    ...
    if (strcmp(type_qn, "System.Object") != 0) {
        f = lsm_registry_lookup_method(ctx->registry, "System.Object", method_name);
        if (f)
            return f;
    }
    return NULL;
 ...
 * dependent. */
static const LSMRegisteredFunc *cs_lookup_using_static_method(CSLSPContext *ctx,
                                                              const char *method_name,
                                                              bool *ambiguous) {
    if (ambiguous)
        *ambiguous = false;
    ...
    const LSMRegisteredFunc *only = NULL;
    for (int i = 0; i < ctx->using_count; i++) {
        const LSMCSUsing *u = &ctx->usings[i];
        if (u->kind != LSM_CS_USING_STATIC)
            continue;
...
static const LSMRegisteredFunc *cs_lookup_extension(CSLSPContext *ctx, const char *receiver_qn,
                                                    const char *method_name) {
     ...
     * order). */
    LSMFreeFuncIter ext_it;
    lsm_registry_free_funcs_by_short_name(ctx->registry, method_name, &ext_it);
    const LSMRegisteredFunc *best = NULL;
    int best_idx = -1;
    int i;
    while ((i = lsm_free_func_iter_next(&ext_it)) >= 0) {
        ...
        if (!cand->signature || cand->signature->kind != LSM_TYPE_FUNC)
            continue;
        const LSMType *sig = cand->signature;
        if (!sig->data.func.param_types)
            continue;
        const LSMType *first = sig->data.func.param_types[0];
        if (!first)
            continue;
        ...
        bool match = false;
        if (first->kind == LSM_TYPE_NAMED && first->data.named.qualified_name) {
            ...
            const LSMRegisteredType *rt = cs_lookup_type_qn(ctx, receiver_qn);
        ...
        } else if (first->kind == LSM_TYPE_TEMPLATE) {
...
const LSMType *cs_parse_type_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "predefined_type") == 0) {
        ...
        if (aliased)
            return lsm_type_named(ctx->arena, aliased);
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        if (ts_node_is_null(inner))
            inner = cs_first_named_child(node);
        const LSMType *elem = cs_parse_type_node(ctx, inner);
        return lsm_type_template(ctx->arena, "System.Array", (const LSMType *[]){elem, NULL}, 1);
    }
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        const LSMType **elems =
            (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*elems));
        if (!elems)
            return lsm_type_unknown();
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
    ...
    if (strcmp(kind, "generic_name") == 0) {
        ...
        const LSMType **args =
            (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*args));
        if (!args)
            return lsm_type_named(ctx->arena, resolved);
        int count = 0;
        for (uint32_t i = 0; i < nc; i++) {
        ...
        args[count] = NULL;
        return lsm_type_template(ctx->arena, resolved, args, count);
    }
    if (strcmp(kind, "qualified_name") == 0 || strcmp(kind, "identifier") == 0 ||
        ...
        if (pre)
            return lsm_type_named(ctx->arena, pre);
        return lsm_type_named(ctx->arena, resolved);
    }
    if (strcmp(kind, "ref_type") == 0) {
        ...
        if (ts_node_is_null(inner))
            inner = cs_first_named_child(node);
        const LSMType *t = cs_parse_type_node(ctx, inner);
        return lsm_type_reference(ctx->arena, t);
    }
...
const LSMType *cs_eval_expr_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (ctx->enclosing_base_qn) {
            result = lsm_type_named(ctx->arena, ctx->enclosing_base_qn);
        }
        ...
        result = lsm_type_named(ctx->arena, "System.String");
    } else if (strcmp(kind, "character_literal") == 0) {
        result = lsm_type_named(ctx->arena, "System.Char");
    } else if (strcmp(kind, "integer_literal") == 0) {
        result = lsm_type_named(ctx->arena, "System.Int32");
    } else if (strcmp(kind, "real_literal") == 0) {
        result = lsm_type_named(ctx->arena, "System.Double");
    } else if (strcmp(kind, "boolean_literal") == 0) {
        result = lsm_type_named(ctx->arena, "System.Boolean");
        ...
        if (!ts_node_is_null(inner)) {
            const LSMType *t = cs_eval_expr_type(ctx, inner);
        ...
        if (ts_node_is_null(b)) {
            ...
            for (uint32_t i = 0; i < nc; i++) {
                TSNode c = ts_node_child(node, i);
                if (!ts_node_is_null(c) && ts_node_is_named(c)) {
                    const LSMType *t = cs_eval_expr_type(ctx, c);
        ...
        elems[count] = NULL;
        result = lsm_type_tuple(ctx->arena, elems, count);
        ...
        result = lsm_type_named(ctx->arena, "System.Type");
        ...
        if (!ts_node_is_null(obj)) {
            const LSMType *recv = cs_eval_expr_type(ctx, obj);
        ...
        TSNode tnode = ts_node_child_by_field_name(node, "type", 4);
        if (!ts_node_is_null(tnode)) {
            const LSMType *elem = cs_parse_type_node(ctx, tnode);
            result =
                lsm_type_template(ctx->arena, "System.Array", (const LSMType *[]){elem, NULL}, 1);
        }
    } else if (strcmp(kind, "implicit_array_creation_expression") == 0) {
        result = lsm_type_template(ctx->arena, "System.Array",
                                   (const LSMType *[]){lsm_type_unknown(), NULL}, 1);
    }
...
static const LSMType *cs_eval_identifier_type(CSLSPContext *ctx, TSNode node) {
    ...
    /* Local / parameter scope. lsm_scope_lookup returns lsm_type_unknown()
    ...
    if (bound && bound->kind != LSM_TYPE_UNKNOWN)
        return bound;

    /* Implicit `this` member. Try to find a field on the enclosing class. */
    if (ctx->enclosing_class_qn) {
        const LSMRegisteredType *rt = cs_lookup_type_qn(ctx, ctx->enclosing_class_qn);
    ...
    if (resolved) {
        const LSMRegisteredType *rt = cs_lookup_type_qn(ctx, resolved);
        if (rt)
...
static const LSMType *cs_eval_invocation_type(CSLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        const LSMType *recv_type = lsm_type_unknown();
        if (!ts_node_is_null(recv))
        ...
        const LSMRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare_name);
        if (!f)
        ...
        if (f->signature->kind == LSM_TYPE_FUNC && f->signature->data.func.return_types &&
            ...
            const LSMType *ret = f->signature->data.func.return_types[0];
            ...
            if (recv_type && recv_type->kind == LSM_TYPE_TEMPLATE) {
                const LSMRegisteredType *rt = cs_lookup_type_qn(ctx, type_qn);
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (strcmp(bare, "nameof") == 0) {
            return lsm_type_named(ctx->arena, "System.String");
        }
        if (strcmp(bare, "typeof") == 0) {
            return lsm_type_named(ctx->arena, "System.Type");
        }

        /* Try enclosing class member. */
        if (ctx->enclosing_class_qn) {
            const LSMRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, bare);
        ...
        const char *ns = cs_namespace_qn(ctx);
        if (ns && *ns) {
            const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ns, bare);
        ...
        const LSMRegisteredFunc *f = cs_lookup_using_static_method(ctx, bare, &ambiguous);
        if (ambiguous)
            return lsm_type_unknown();
        if (f && f->signature && f->signature->data.func.return_types &&
            f->signature->data.func.return_types[0]) {
...
static const LSMType *cs_eval_member_access_type(CSLSPContext *ctx, TSNode node) {
    ...
    if (obj_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            ...
            const LSMRegisteredFunc *f = cs_lookup_method(ctx, type_qn, fname);
            if (f && f->signature && f->signature->kind == LSM_TYPE_FUNC &&
                f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
                return f->signature->data.func.return_types[0];
            }
    ...
    if (rt && rt->field_names) {
        for (int i = 0; rt->field_names[i]; i++) {
            if (strcmp(rt->field_names[i], fname) == 0) {
                if (rt->field_types && rt->field_types[i]) {
                    const LSMType *ft = rt->field_types[i];
                    if (recv->kind == LSM_TYPE_TEMPLATE && rt->type_param_names &&
    ...
    if (f && f->signature && f->signature->kind == LSM_TYPE_FUNC &&
        f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
        const LSMType *ret = f->signature->data.func.return_types[0];
        if (recv->kind == LSM_TYPE_TEMPLATE && rt && rt->type_param_names &&
            recv->data.template_type.template_args) {
            return cs_substitute_type_params(ctx->arena, ret, rt->type_param_names,
                                             recv->data.template_type.template_args);
        }
...
static const LSMType *cs_substitute_type_params(LSMArena *arena, const LSMType *t,
    ...
    if (t->kind == LSM_TYPE_TEMPLATE) {
        const LSMType *const *old_args = t->data.template_type.template_args;
        if (!old_args)
            return t;
        int n = 0;
        while (old_args[n])
...
static const char *cs_resolve_callable_name(CSLSPContext *ctx, const char *name) {
    ...
    if (ctx->enclosing_class_qn) {
        const LSMRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_class_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    if (ctx->enclosing_base_qn) {
        const LSMRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, name);
        if (f && f->qualified_name)
            return f->qualified_name;
    }

    bool using_static_ambiguous = false;
    const LSMRegisteredFunc *using_static =
    ...
    const char *ns = cs_namespace_qn(ctx);
    if (ns && ns[0]) {
        const char *qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ns, name);
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, qn);
        if (f && f->qualified_name)
            return f->qualified_name;
    }
    ...
    for (int i = 0; ctx->registry && i < ctx->registry->func_count; i++) {
        const LSMRegisteredFunc *candidate = &ctx->registry->funcs[i];
...
static const char *cs_resolve_callable_value(CSLSPContext *ctx, TSNode node,
    ...
    if (receiver_text) {
        ...
        if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
            const LSMRegisteredFunc *f = cs_lookup_method(ctx, type_qn, member_name);
            if (f && callable_leaf)
    ...
    const LSMType *receiver_type = cs_unwrap_nullable(cs_eval_expr_type(ctx, receiver));
    const char *type_qn = NULL;
    if (receiver_type && receiver_type->kind == LSM_TYPE_NAMED) {
        type_qn = receiver_type->data.named.qualified_name;
    } else if (receiver_type && receiver_type->kind == LSM_TYPE_TEMPLATE) {
...
static void cs_process_local_decl(CSLSPContext *ctx, TSNode node) {
    ...
    const LSMType *declared_type =
        ts_node_is_null(tnode) ? lsm_type_unknown() : cs_parse_type_node(ctx, tnode);
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (!rhs_t || rhs_t->kind == LSM_TYPE_UNKNOWN) {
            ...
            for (uint32_t j = 0; j < cc; j++) {
                ...
                if (strcmp(ck, "equals_value_clause") == 0) {
                    ...
                    if (!ts_node_is_null(rhs)) {
                        rhs_node = rhs;
                        const LSMType *t = cs_eval_expr_type(ctx, rhs);
                        if (t && t->kind != LSM_TYPE_UNKNOWN)
                            rhs_t = t;
                    }
                    break;
                ...
                if (seen_named >= 1) {
                    rhs_node = cn;
                    const LSMType *t = cs_eval_expr_type(ctx, cn);
                    if (t && t->kind != LSM_TYPE_UNKNOWN) {
                        rhs_t = t;
                        break;
                    }
        ...
        const LSMType *bind = is_var ? (rhs_t ? rhs_t : declared_type) : declared_type;
        if (!bind)
            bind = lsm_type_unknown();
        const char *callable_qn =
            ts_node_is_null(rhs_node) ? NULL : cs_resolve_callable_value(ctx, rhs_node, NULL);
        if (callable_qn) {
            lsm_scope_bind_callable(ctx->current_scope, vname, bind, callable_qn);
        } else {
            lsm_scope_bind(ctx->current_scope, vname, bind);
        }
    }
}
...
static void cs_process_foreach(CSLSPContext *ctx, TSNode node) {
    ...
    const LSMType *element_t = lsm_type_unknown();
    if (!ts_node_is_null(iter)) {
        const LSMType *iter_t = cs_eval_expr_type(ctx, iter);
        if (iter_t && iter_t->kind == LSM_TYPE_TEMPLATE) {
            const LSMType *const *args = iter_t->data.template_type.template_args;
            if (args && args[0])
                element_t = args[0];
        }
    }
    if (!ts_node_is_null(tnode)) {
        const LSMType *declared = cs_parse_type_node(ctx, tnode);
        char *tt = cs_node_text(ctx, tnode);
        bool is_var = (tt && (strcmp(tt, "var") == 0));
        if (!is_var && declared && declared->kind != LSM_TYPE_UNKNOWN)
            element_t = declared;
    }
    lsm_scope_bind(ctx->current_scope, vname, element_t);
}
...
static void cs_process_assignment(CSLSPContext *ctx, TSNode node) {
    ...
    if (callable_qn) {
        lsm_scope_update_callable(ctx->current_scope, vname, callable_qn);
    } else {
        ...
        lsm_scope_update_callable(ctx->current_scope, vname, NULL);
    }
}
...
static void cs_emit_resolved_kind_reason(CSLSPContext *ctx, const char *callee_qn,
                                         ...
                                         LSMResolvedKind kind, const char *reason) {
...
static void cs_emit_resolved(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
                             float confidence) {
    cs_emit_resolved_kind(ctx, callee_qn, strategy, confidence, LSM_RESOLVED_INVOCATION);
}
...
static void cs_emit_resolved_reason(CSLSPContext *ctx, const char *callee_qn, const char *strategy,
    ...
    cs_emit_resolved_kind_reason(ctx, callee_qn, strategy, confidence, LSM_RESOLVED_INVOCATION,
                                 reason);
}
...
static void cs_emit_resolved_kind_reason_at(CSLSPContext *ctx, const char *callee_qn,
                                            const char *strategy, float confidence,
                                            LSMResolvedKind kind, const char *reason, TSNode site) {
...
static void cs_retarget_callable_carrier(CSLSPContext *ctx, TSNode site, const char *source_name,
    ...
    for (int i = 0; i < ctx->call_carriers->count; i++) {
        LSMCall *call = &ctx->call_carriers->items[i];
...
static void cs_resolve_callable_arguments(CSLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        cs_emit_resolved_kind_reason_at(ctx, target, "lsp_callable_value_reference", 0.95f,
                                        LSM_RESOLVED_CALL_REFERENCE, source_name, callable_leaf);
    }
}
...
static void cs_resolve_invocation(CSLSPContext *ctx, TSNode call) {
    ...
    if (strcmp(fk, "member_access_expression") == 0 ||
        ...
        if (recv_text) {
            ...
            if (type_qn && cs_lookup_type_qn(ctx, type_qn)) {
                const LSMRegisteredFunc *f = cs_lookup_method(ctx, type_qn, bare);
                if (f) {
                    cs_emit_resolved(ctx, f->qualified_name, "cs_static_typed", 0.95f);
                    return;
                }
                /* Type known, method not in registry — synth a call. */
                cs_emit_resolved(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, bare),
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "generic_name") == 0) {
        ...
        if (lsm_scope_contains(ctx->current_scope, bare)) {
            const char *target = lsm_scope_lookup_callable(ctx->current_scope, bare);
            if (target) {
        ...
        if (ctx->enclosing_base_qn) {
            const LSMRegisteredFunc *f = cs_lookup_method(ctx, ctx->enclosing_base_qn, bare);
            if (f) {
...
static void cs_resolve_object_creation(CSLSPContext *ctx, TSNode call) {
    ...
    if (ts_node_is_null(tnode))
        return;
    const LSMType *t = cs_parse_type_node(ctx, tnode);
    const char *tqn = NULL;
    ...
    const LSMRegisteredFunc *f = cs_lookup_method(ctx, tqn, short_name);
    if (f) {
...
static void cs_resolve_calls_in_node(CSLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "block") == 0) {
        LSMScope *prev = ctx->current_scope;
        ctx->current_scope = lsm_scope_push(ctx->arena, prev);
...
static void cs_collect_type_params(CSLSPContext *ctx, TSNode node, const char ***out_names,
    ...
    *out_count = 0;
    ...
    int cap = 4;
    const char **arr = (const char **)lsm_arena_alloc(ctx->arena, (size_t)cap * sizeof(*arr));
    if (!arr)
        return;
    int n = 0;
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (n + 1 >= cap) {
            int new_cap = cap * 2;
            const char **ne =
                (const char **)lsm_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*ne));
            if (!ne)
                break;
            for (int j = 0; j < n; j++)
                ne[j] = arr[j];
...
static void cs_process_function_like(CSLSPContext *ctx, TSNode node) {
    ...
    const LSMType **saved_tp_args = ctx->type_param_args;
    ...
    if (short_name) {
        if (ctx->enclosing_class_qn) {
            ctx->enclosing_func_qn =
                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, short_name);
        } else if (ctx->module_qn) {
            ctx->enclosing_func_qn =
                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, short_name);
        } else {
            ctx->enclosing_func_qn = lsm_arena_strdup(ctx->arena, short_name);
        }
    }
    ...
    if (is_method) {
        ...
        if (tp_count > 0) {
            const LSMType **args = (const LSMType **)lsm_arena_alloc(
                ctx->arena, (size_t)(tp_count + 1) * sizeof(*args));
            if (args) {
                for (int i = 0; i < tp_count; i++) {
                    args[i] = lsm_type_type_param(ctx->arena, tp_names[i]);
                }
                args[tp_count] = NULL;
...
static void cs_process_type_decl(CSLSPContext *ctx, TSNode node) {
    ...
    if (tp_count > 0) {
        const LSMType **args =
            (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(tp_count + 1) * sizeof(*args));
        if (args) {
            for (int i = 0; i < tp_count; i++) {
                args[i] = lsm_type_type_param(ctx->arena, tp_names[i]);
            }
    ...
    /* Base list -> enclosing_base_qn + enclosing_iface_qns. */
    {
        ...
        if (!ts_node_is_null(bl)) {
            ...
            int icap = 4, icount = 0;
            const char **ifs =
                (const char **)lsm_arena_alloc(ctx->arena, (size_t)icap * sizeof(*ifs));
            for (uint32_t i = 0; i < nc; i++) {
                ...
                const LSMRegisteredType *rt = cs_lookup_type_qn(ctx, qn);
                if (rt && rt->is_interface) {
                    if (icount + 1 >= icap) {
                        int nc2 = icap * 2;
                        const char **ne =
                            (const char **)lsm_arena_alloc(ctx->arena, (size_t)nc2 * sizeof(*ne));
                        if (!ne)
                            continue;
                    ...
                    if (!ctx->enclosing_base_qn)
                        ...
                        if (icount + 1 >= icap) {
                            int nc2 = icap * 2;
                            const char **ne = (const char **)lsm_arena_alloc(
    ...
    if (!is_enum) {
        ...
        if (!ts_node_is_null(pctor)) {
            ...
            ctx->enclosing_func_qn =
                lsm_arena_sprintf(ctx->arena, "%s..ctor", ctx->enclosing_class_qn);
...
static void cs_collect_imports(CSLSPContext *ctx, TSNode root) {
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "using_directive") == 0) {
            ...
            if (target) {
                if (is_alias) {
                    cs_lsp_add_using(ctx, LSM_CS_USING_ALIAS, alias_name ? alias_name : "", target,
                                     is_global);
                } else if (is_static) {
                    cs_lsp_add_using(ctx, LSM_CS_USING_STATIC, "", target, is_global);
                } else {
                    cs_lsp_add_using(ctx, LSM_CS_USING_NAMESPACE, "", target, is_global);
                }
            }
        }
...
static void cs_parse_signature(LSMArena *arena, const char *signature, CSLSPContext *ctx,
                               const char ***out_names, const LSMType ***out_types) {
    *out_names = NULL;
    *out_types = NULL;
    if (!signature)
        return;
    ...
    int cap = 8;
    int count = 0;
    const char **names = (const char **)lsm_arena_alloc(arena, (size_t)cap * sizeof(*names));
    const LSMType **types = (const LSMType **)lsm_arena_alloc(arena, (size_t)cap * sizeof(*types));
    if (!names || !types)
        return;
    ...
    while (*p && *p != ')') {
        ...
        if (!last_space) {
            ...
            char *type_text = lsm_arena_strndup(arena, type_start, (size_t)(name_end - type_start));
            const LSMType *t = lsm_type_unknown();
            if (ctx)
                t = cs_resolve_type_name(ctx, type_text)
                        ? lsm_type_named(ctx->arena, cs_resolve_type_name(ctx, type_text))
                        : lsm_type_unknown();
            (void)t;
            if (count + 1 >= cap)
                break;
            names[count] = lsm_arena_sprintf(arena, "_arg%d", count);
            types[count] = t;
            count++;
        } else {
            char *type_text =
                lsm_arena_strndup(arena, type_start, (size_t)(last_space - type_start));
            char *pname =
                lsm_arena_strndup(arena, last_space + 1, (size_t)(name_end - last_space - 1));
            const LSMType *t = lsm_type_unknown();
            if (ctx) {
                const char *resolved = cs_resolve_type_name(ctx, type_text);
                if (resolved)
                    t = lsm_type_named(ctx->arena, resolved);
            }
            if (count + 1 >= cap)
                break;
            names[count] = pname;
...
static void cs_register_type_decls(CSLSPContext *ctx, LSMTypeRegistry *reg, TSNode root) {
    /* We rely on LSMFileResult.defs entries already being filled by the
...
static cs_fields_t *cs_fields_get(LSMArena *arena, cs_fields_table_t *tab, const char *class_qn) {
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_fields_t *ne = (cs_fields_t *)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne)
            return NULL;
        for (int i = 0; i < tab->count; i++)
            ne[i] = tab->items[i];
    ...
    slot->cap = 8;
    slot->field_names =
        (const char **)lsm_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_names));
    slot->field_types =
        (const LSMType **)lsm_arena_alloc(arena, (size_t)slot->cap * sizeof(*slot->field_types));
    return slot;
}

static void cs_fields_add_debug(LSMArena *arena, cs_fields_t *f, const char *name,
                                const LSMType *type, bool debug);

static void cs_fields_add(LSMArena *arena, cs_fields_t *f, const char *name, const LSMType *type) {
...
static void cs_fields_add_debug(LSMArena *arena, cs_fields_t *f, const char *name,
    ...
    if (f->count + 2 >= f->cap) {
        int new_cap = f->cap * 2;
        const char **nn = (const char **)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
        const LSMType **nt =
    ...
    f->field_types[f->count] = type ? type : lsm_type_unknown();
    f->count++;
    f->field_names[f->count] = NULL;
    f->field_types[f->count] = NULL;
}

static void cs_collect_class_fields(CSLSPContext *ctx, LSMTypeRegistry *reg, TSNode root,
    ...
    while (top > 0) {
        ...
        if (strcmp(k, "field_declaration") == 0 || strcmp(k, "property_declaration") == 0 ||
            ...
            if (cls_short) {
                const char *cls_qn = ctx->module_qn ? lsm_arena_sprintf(ctx->arena, "%s.%s",
                                                                        ctx->module_qn, cls_short)
                                                    : lsm_arena_strdup(ctx->arena, cls_short);
                ...
                if (strcmp(k, "field_declaration") == 0 ||
                    ...
                    if (!ts_node_is_null(vd)) {
                        ...
                        if (ts_node_is_null(tnode))
                            tnode = cs_first_named_child(vd);
                        const LSMType *t = ts_node_is_null(tnode) ? lsm_type_unknown()
        ...
        if (strcmp(k, "record_declaration") == 0 || strcmp(k, "record_struct_declaration") == 0 ||
            ...
            if (!ts_node_is_null(params) && !ts_node_is_null(nm)) {
                ...
                if (cls_short) {
                    const char *cls_qn =
                        ctx->module_qn
                            ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                            : lsm_arena_strdup(ctx->arena, cls_short);
                    cs_fields_t *f = cs_fields_get(ctx->arena, tab, cls_qn);
                    ...
                    for (uint32_t i = 0; i < pc; i++) {
                        ...
                        if (ts_node_is_null(tn) || ts_node_is_null(pn))
                            continue;
                        const LSMType *t = cs_parse_type_node(ctx, tn);
 ...
 *
 * The unified extractor leaves LSMDefinition.return_type NULL for C#
...
static void cs_method_rt_add(LSMArena *arena, cs_method_rt_table_t *tab, const char *qn,
                             const LSMType *rt) {
    if (!qn || !rt)
        return;
    for (int i = 0; i < tab->count; i++) {
    ...
    if (tab->count + 1 >= tab->cap) {
        ...
        cs_method_rt_entry_t *ne =
            (cs_method_rt_entry_t *)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*ne));
        if (!ne)
            return;
        for (int i = 0; i < tab->count; i++)
            ne[i] = tab->items[i];
...
static void cs_collect_method_return_types(CSLSPContext *ctx, TSNode root,
    ...
    while (top > 0) {
        ...
        if (is_method || is_property || is_indexer) {
            ...
            const char *cls_qn =
                ctx->module_qn ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cls_short)
                               : lsm_arena_strdup(ctx->arena, cls_short);
            TSNode tnode = ts_node_child_by_field_name(n, "type", 4);
            TSNode nm = ts_node_child_by_field_name(n, "name", 4);
            ...
            if (is_indexer) {
                ...
                if (!ts_node_is_null(tnode)) {
                    const LSMType *rt = cs_parse_type_node(ctx, tnode);
                    cs_method_rt_add(ctx->arena, tab,
                                     lsm_arena_sprintf(ctx->arena, "%s.%s", cls_qn, short_name),
                                     rt);
                }
...
void lsm_cs_refine_ast_return_types(CSLSPContext *ctx, LSMTypeRegistry *reg, TSNode root) {
    ...
    for (int i = 0; i < mtab.count; i++) {
        ...
        const LSMType *rt = mtab.items[i].rt;
        bool patched = false;
            ...
            if (strcmp(reg->funcs[j].qualified_name, qn) == 0) {
                ...
                if (rets) {
                    ...
                    reg->funcs[j].signature =
                        lsm_type_func_replace_returns(ctx->arena, reg->funcs[j].signature, rets);
                }
                patched = true;
                break;
            }
...
/* ── parse return type from LSMDefinition.return_type ──────────── */

static const LSMType *cs_parse_return_type_text(CSLSPContext *ctx, const char *text) {
    if (!text || !*text)
        return lsm_type_unknown();
    const char *p = text;
    while (*p == ' ' || *p == ':')
    ...
    if (n == 0)
        return lsm_type_unknown();
    char *trimmed = lsm_arena_strndup(ctx->arena, p, n);
...
static const LSMType *cs_signature_param_type_adapter(LSMArena *arena, const char *text,
                                                      void *parser_ctx) {
    (void)arena;
    CSLSPContext *ctx = (CSLSPContext *)parser_ctx;
    return ctx ? cs_parse_return_type_text(ctx, text) : lsm_type_unknown();
}

/* ── single-file entry: lsm_run_cs_lsp ──────────────────────────── */
...
void lsm_run_cs_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
    ...
    /* Phase A: stdlib. */
    lsm_csharp_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '.' && !strchr(base, '.') && module_qn) {
                                q = lsm_arena_sprintf(arena, "%s.%s", module_qn, base);
                            }
                            emb[j] = q;
                        }
                        emb[bc] = NULL;
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            const LSMType *rt = d->return_type ? lsm_type_unknown() /* will refine below with ctx */
                                               : lsm_type_unknown();
            const LSMType **rets =
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) != 0)
                continue;
            const LSMType **return_types = NULL;
            if (reg.funcs[j].signature && reg.funcs[j].signature->kind == LSM_TYPE_FUNC) {
                return_types = reg.funcs[j].signature->data.func.return_types;
            }
            reg.funcs[j].signature = lsm_type_func(arena, NULL, param_types, return_types);
            break;
        }
    }
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (strcmp(d->label, "Function") != 0 && strcmp(d->label, "Method") != 0)
            continue;
        const LSMType *rt = cs_parse_return_type_text(&ctx, d->return_type);
        ...
        for (int j = 0; j < reg.func_count; j++) {
            if (strcmp(reg.funcs[j].qualified_name, d->qualified_name) == 0) {
                ...
                if (rets) {
                    ...
                    rets[1] = NULL;
                    reg.funcs[j].signature =
                        lsm_type_func_replace_returns(arena, reg.funcs[j].signature, rets);
                }
                break;
            }
        }
     ...
     * leaves LSMDefinition.return_type NULL for C# methods, so we walk the
     * AST ourselves to recover declared return types. */
    lsm_cs_refine_ast_return_types(&ctx, &reg, root);
...
static void cs_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, LSMLSPDef *defs,
                                 int def_count) {
            ...
            if (d->embedded_types && *d->embedded_types) {
                ...
                for (const char *p = d->embedded_types; *p; p++)
                    if (*p == '|')
                        n++;
                const char **arr =
                    (const char **)lsm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*arr));
                if (arr) {
                    int idx = 0;
                    ...
                    for (const char *p = d->embedded_types;; p++) {
                        if (*p == '|' || *p == '\0') {
                            ...
                            if (len > 0) {
                                arr[idx++] = lsm_arena_strndup(arena, start, len);
                            }
                            if (*p == '\0')
                                break;
                            start = p + 1;
...
static bool cs_def_is_type(const LSMLSPDef *d) {
...
LSMTypeRegistry *lsm_cs_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    ...
    LSMLSPDef *cs = NULL;
    if (def_count > 0) {
        cs = (LSMLSPDef *)lsm_arena_alloc(arena, (size_t)def_count * sizeof(*cs));
        if (!cs)
            return NULL;
    }
    int total = 0;
    for (int i = 0; i < def_count; i++) {
        if (defs[i].lang == LSM_LANG_CSHARP && cs_def_is_type(&defs[i]))
            cs[total++] = defs[i];
    }
...
void lsm_run_cs_lsp_cross_with_registry(LSMArena *arena, const char *source, int source_len,
    ...
    for (int i = 0; i < using_count; i++) {
        if (using_targets[i]) {
            cs_lsp_add_using(&ctx, LSM_CS_USING_NAMESPACE, "", using_targets[i], false);
        }
    }
    cs_lsp_process_file(&ctx, root);
...
void lsm_batch_cs_lsp_cross(LSMArena *arena, LSMBatchCSLSPFile *files, int file_count,
    ...
    for (int i = 0; i < file_count; i++) {
        LSMBatchCSLSPFile *f = &files[i];
        lsm_run_cs_lsp_cross(arena, f->source, f->source_len, f->module_qn, f->defs, f->def_count,
