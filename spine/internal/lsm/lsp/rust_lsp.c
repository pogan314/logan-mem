...
static void rust_emit_unresolved_call(RustLSPContext *ctx, const char *expr_text,
                                      const char *reason);
static const LSMType *rust_lookup_field(RustLSPContext *ctx, const char *type_qn,
                                        const char *field_name, int depth);
static const LSMRegisteredFunc *rust_lookup_method_in_trait(RustLSPContext *ctx,
                                                            const char *trait_qn,
                                                            const char *method_name,
                                                            bool allow_abstract);
static char *rust_node_text(RustLSPContext *ctx, TSNode node);
static const char *convert_path_to_qn(LSMArena *arena, const char *path);
...
void rust_lsp_init(RustLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
    ...
    const char *dbg = getenv("LSM_LSP_DEBUG");
    ctx->debug = (dbg && dbg[0]);
}
...
void rust_lsp_add_use(RustLSPContext *ctx, const char *local_name, const char *module_path) {
    ...
    if (ctx->use_count % 32 == 0) {
        ...
        const char **nl =
            (const char **)lsm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char *));
        const char **np =
            (const char **)lsm_arena_alloc(ctx->arena, (new_cap + 1) * sizeof(char *));
        if (!nl || !np) {
            return;
    ...
    ctx->use_local_names[ctx->use_count] = lsm_arena_strdup(ctx->arena, local_name);
    ctx->use_module_paths[ctx->use_count] = lsm_arena_strdup(ctx->arena, module_path);
    ctx->use_count++;
}
...
void rust_lsp_add_glob(RustLSPContext *ctx, const char *module_qn) {
    ...
    ctx->glob_module_qns[ctx->glob_count++] = lsm_arena_strdup(ctx->arena, module_qn);
}
...
static const char *convert_path_to_qn(LSMArena *arena, const char *path) {
    ...
    char *out = (char *)lsm_arena_alloc(arena, len + 1);
    if (!out) {
        return path;
    }
    size_t j = 0;
...
typedef struct {
    LSMIdxMemo unique;
    LSMIdxMemo ambiguous;
} RustTraitNameIndex;

static void rust_trait_name_index_add(RustTraitNameIndex *index, LSMArena *arena,
                                      const LSMTypeRegistry *reg, const char *key, int type_index) {
    if (!index || !arena || !reg || !key || !key[0] || type_index < 0 ||
        type_index >= reg->type_count) {
        return;
    }
    int32_t previous = lsm_idxmemo_get(&index->unique, key);
    if (previous < 0) {
        lsm_idxmemo_put_if_absent(&index->unique, arena, key, type_index);
        return;
    }
    ...
    if (previous_qn && current_qn && strcmp(previous_qn, current_qn) != 0) {
        lsm_idxmemo_put_if_absent(&index->ambiguous, arena, key, 1);
    }
}

static void rust_trait_name_index_build(RustTraitNameIndex *index, LSMArena *arena,
    ...
    for (int i = 0; i < reg->type_count; i++) {
        const LSMRegisteredType *type = &reg->types[i];
...
static const char *rust_trait_name_index_lookup(const RustTraitNameIndex *index,
                                                const LSMTypeRegistry *reg, const char *key) {
    if (!index || !reg || !key || lsm_idxmemo_get(&index->ambiguous, key) >= 0) {
        return NULL;
    }
    int32_t type_index = lsm_idxmemo_get(&index->unique, key);
...
static const char *rust_canonical_impl_trait(LSMArena *arena, const LSMTypeRegistry *reg,
    ...
    if (def_module_qn && relative[0] && strncmp(relative, "crate.", 6) != 0 &&
        ...
        const char *local = lsm_arena_sprintf(arena, "%s.%s", def_module_qn, relative);
...
static void rust_registered_type_add_embedded(LSMArena *arena, LSMRegisteredType *type,
    ...
    const char **items =
        (const char **)lsm_arena_alloc(arena, (size_t)(count + 2) * sizeof(*items));
    if (!items) {
        return;
    }
...
static const char *rust_registered_relative_path(RustLSPContext *ctx, const char *normalized) {
    ...
    char *head = lsm_arena_strndup(ctx->arena, normalized, (size_t)(head_end - normalized));
    if (!head || lsm_idxmemo_get(&ctx->declared_modules, head) < 0) {
        return NULL;
    }

    const char *current = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, normalized);
    const char *parent_end = strrchr(ctx->module_qn, '.');
    const char *parent =
        parent_end ? lsm_arena_sprintf(ctx->arena, "%.*s.%s", (int)(parent_end - ctx->module_qn),
                                       ctx->module_qn, normalized)
                   : NULL;
    bool current_exists = current && lsm_registry_lookup_type(ctx->registry, current);
    bool parent_exists = parent && lsm_registry_lookup_type(ctx->registry, parent);
...
static const char *rust_resolve_path_expr(RustLSPContext *ctx, const char *path) {
    ...
    if (strncmp(path, "Self::", 6) == 0 && ctx->self_type_qn) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->self_type_qn,
    ...
    if (strncmp(path, "crate::", 7) == 0 && ctx->module_qn) {
        ...
        char *crate_buf = lsm_arena_strndup(ctx->arena, ctx->module_qn, crate_len);
        return lsm_arena_sprintf(ctx->arena, "%s.%s", crate_buf,
    ...
    if (strncmp(path, "super::", 7) == 0 && ctx->module_qn) {
        ...
        if (dot) {
            char *parent =
                lsm_arena_strndup(ctx->arena, ctx->module_qn, (size_t)(dot - ctx->module_qn));
            return lsm_arena_sprintf(ctx->arena, "%s.%s", parent,
                                     convert_path_to_qn(ctx->arena, path + 7));
        }
    }
    ...
    if (!sep) {
        ...
        if (ctx->module_qn) {
            return lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, path);
        }
        return path;
    }

    char *head = lsm_arena_strndup(ctx->arena, path, (size_t)(sep - path));
    ...
    if (full) {
        ...
        return lsm_arena_sprintf(ctx->arena, "%s.%s", full_dotted, tail_dotted);
    }

    /* Prelude head: `String::from` → `alloc.string.String.from`. */
    const char *prelude = rust_lookup_prelude(head);
    if (prelude) {
        return lsm_arena_sprintf(ctx->arena, "%s.%s", prelude,
    ...
    if (ctx->cargo_manifest) {
        const LSMCargoManifest *m = (const LSMCargoManifest *)ctx->cargo_manifest;
        const LSMCargoMember *mem = lsm_cargo_find_member(m, head);
        if (mem) {
            /* Workspace member: route to `<member_name>.<tail>` so the
             * pipeline's cross-crate resolution can match it. */
            return lsm_arena_sprintf(ctx->arena, "%s.%s", head,
                                     convert_path_to_qn(ctx->arena, tail));
        }
        if (lsm_cargo_is_known_dep(m, head)) {
...
static const char *resolve_path_to_type_qn(RustLSPContext *ctx, const char *path) {
    ...
    if (lsm_registry_lookup_type(ctx->registry, qn)) {
        return qn;
    }
...
const LSMType *rust_parse_type_node(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "type_identifier") == 0) {
        ...
        if (strcmp(name, "Self") == 0 && ctx->self_type_qn) {
            return lsm_type_named(ctx->arena, ctx->self_type_qn);
        }
    ...
    if (strcmp(kind, "reference_type") == 0) {
        ...
        const LSMType *elem = rust_parse_type_node(ctx, inner);
        return lsm_type_reference(ctx->arena, elem);
    }
    ...
    if (strcmp(kind, "pointer_type") == 0) {
        ...
        return lsm_type_pointer(ctx->arena, rust_parse_type_node(ctx, inner));
    }
    ...
    if (strcmp(kind, "array_type") == 0) {
        ...
        return lsm_type_slice(ctx->arena, rust_parse_type_node(ctx, elem));
    }
    ...
    if (strcmp(kind, "tuple_type") == 0) {
        ...
        if (count == 0) {
            return lsm_type_builtin(ctx->arena, "()");
        }
        if (count == 1) {
            return elems[0];
        }
        return lsm_type_tuple(ctx->arena, elems, count);
    }
    ...
    if (strcmp(kind, "generic_type") == 0) {
        ...
        if (targ_count > 0) {
            return lsm_type_template(ctx->arena, head_qn, targs, targ_count);
        }
        return lsm_type_named(ctx->arena, head_qn);
    }
 ...
 * into a LSMType. Used when we receive types as strings (return types of
 * extracted `LSMDefinition`s, cross-file `LSMRustLSPDef::return_types`,
 * stdlib seed entries).
 *
 ...
 * without a parser. This is the same trade-off `lsm_rust_parse_return_type_text`
 * makes for Go. */
static const LSMType *parse_type_text_with_params(LSMArena *arena, const char *text,
    ...
    if (text[0] == '&') {
        ...
        const LSMType *elem = parse_type_text_with_params(arena, p, module_qn, type_params);
        return lsm_type_reference(arena, elem);
    }
    ...
    if (text[0] == '[' && text[strlen(text) - 1] == ']') {
        ...
        char *inner = lsm_arena_strndup(arena, p, inner_len);
    ...
    if (text[0] == '(' && text[strlen(text) - 1] == ')') {
        ...
        if (count == 1)
            return elems[0];
        return lsm_type_tuple(arena, elems, count);
    }
    ...
    if (lt) {
        ...
        if (*gt == '>') {
            ...
            char *head = lsm_arena_strndup(arena, text, head_len);
            /* Recursive split of args by top-level commas. */
            const char *args = lt + 1;
            size_t args_len = (size_t)(gt - args);
            char *abuf = lsm_arena_strndup(arena, args, args_len);
            const LSMType *targs[16];
            int targ_count = 0;
            int depth = 0;
            char *start = abuf;
            ...
            if (is_rust_primitive(head)) {
                ...
                return lsm_type_builtin(arena, head);
            }
            /* Map a few well-known std type sugars. */
            return lsm_type_template(arena, head_qn, targs, targ_count);
        }
    }

    /* Bare identifier or path. */
    if (is_rust_primitive(text)) {
        return lsm_type_builtin(arena, text);
    }
    if (type_params) {
        for (int i = 0; type_params[i]; i++) {
            if (strcmp(text, type_params[i]) == 0) {
                return lsm_type_type_param(arena, text);
            }
        }
    }
    /* Self -> module-qualified placeholder caller will substitute. */
    if (strcmp(text, "Self") == 0) {
        return lsm_type_named(arena, "Self");
    }
    /* Has `::` → absolute path; treat dotted paths as already-qualified
     * QNs (cross-file callers pass module-qualified text directly). */
    if (strstr(text, "::")) {
        return lsm_type_named(arena, convert_path_to_qn(arena, text));
    }
    if (strchr(text, '.')) {
        return lsm_type_named(arena, text);
    }
    return lsm_type_named(arena, lsm_arena_sprintf(arena, "%s.%s", module_qn, text));
}

/* Public-ish helper used by the cross-file path. */
static const LSMType *rust_parse_return_type_text(LSMArena *arena, const char *text,
...
static const LSMType *rust_signature_param_type_adapter(LSMArena *arena, const char *text,
                                                        void *parser_ctx) {
 ...
 * types. */
static const LSMType *rust_substitute_type(LSMArena *arena, const LSMType *t, const char **params,
                                           const LSMType **args) {
    if (!t || !params || !args) {
        return t;
    }
    ...
    case LSM_TYPE_SLICE:
        return lsm_type_slice(arena, rust_substitute_type(arena, t->data.slice.elem, params, args));
    case LSM_TYPE_TEMPLATE: {
        const LSMType *new_args[16];
        int n = 0;
        for (; n < t->data.template_type.arg_count && n < 16; n++) {
            new_args[n] =
                rust_substitute_type(arena, t->data.template_type.template_args[n], params, args);
        }
        return lsm_type_template(arena, t->data.template_type.template_name, new_args, n);
    }
    case LSM_TYPE_TUPLE: {
        const LSMType *new_elems[16];
        int n = 0;
        for (; n < t->data.tuple.count && n < 16; n++) {
            new_elems[n] = rust_substitute_type(arena, t->data.tuple.elems[n], params, args);
        }
        return lsm_type_tuple(arena, new_elems, n);
    }
    default:
        return t;
    }
...
static void rust_unify_type(const LSMType *param_type, const LSMType *arg_type,
                            const char **type_param_names, const LSMType **inferred,
                            int param_count) {
    if (!param_type || !arg_type || lsm_type_is_unknown(arg_type)) {
        return;
    }
    ...
    if (param_type->kind == LSM_TYPE_TEMPLATE && arg_type->kind == LSM_TYPE_TEMPLATE) {
    ...
    if (param_type->kind == LSM_TYPE_TUPLE && arg_type->kind == LSM_TYPE_TUPLE) {
    ...
    if (arg_type->kind == LSM_TYPE_TYPE_PARAM) {
...
static const LSMType *rust_apply_subst(LSMArena *arena, const LSMType *t, const char **names,
                                       const LSMType **inferred, int count) {
    if (!t)
        return t;
    switch (t->kind) {
    case LSM_TYPE_TYPE_PARAM:
...
static const LSMType *rust_eval_literal_type(RustLSPContext *ctx, const char *kind, TSNode node) {
    if (strcmp(kind, "integer_literal") == 0) {
        ...
        if (text) {
            ...
            if (*p) {
                return lsm_type_builtin(ctx->arena, p);
            }
        }
        return lsm_type_builtin(ctx->arena, "i32");
    }
    if (strcmp(kind, "float_literal") == 0) {
        ...
        return lsm_type_builtin(ctx->arena, "f64");
    }
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "raw_string_literal") == 0) {
        /* &'static str — represented as &str */
        return lsm_type_reference(ctx->arena, lsm_type_builtin(ctx->arena, "str"));
    }
 ...
 * preference). */
static const LSMType *rust_eval_member_access(RustLSPContext *ctx, const LSMType *recv,
                                              const char *member);

const LSMType *rust_eval_expr_type(RustLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node)) {
        return lsm_type_unknown();
    }
    const char *kind = ts_node_type(node);
    ...
    if (strcmp(kind, "identifier") == 0) {
        ...
        if (full) {
            ...
            const LSMRegisteredFunc *uf = lsm_registry_lookup_func(ctx->registry, qn);
            if (uf && uf->signature) {
                return uf->signature;
            }
            const LSMRegisteredType *ut = lsm_registry_lookup_type(ctx->registry, qn);
            if (ut) {
                return lsm_type_named(ctx->arena, qn);
            }
        }
        /* Same-module type? */
        const char *type_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, name);
        if (lsm_registry_lookup_type(ctx->registry, type_qn)) {
            return lsm_type_named(ctx->arena, type_qn);
        }
        return lsm_type_unknown();
    }
    ...
    if (strcmp(kind, "self") == 0 || strcmp(kind, "self_parameter") == 0) {
        return lsm_scope_lookup(ctx->current_scope, "self");
    }
    ...
    if (strcmp(kind, "scoped_identifier") == 0) {
        ...
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, qn);
        if (f && f->signature) {
            return f->signature;
        }
        if (lsm_registry_lookup_type(ctx->registry, qn)) {
    ...
    if (strcmp(kind, "field_expression") == 0) {
        ...
        const LSMType *recv = rust_eval_expr_type(ctx, value);
        const char *fk = ts_node_type(field);
        if (strcmp(fk, "integer_literal") == 0) {
            /* Tuple field access. */
            if (recv) {
                const LSMType *base = recv;
                while (base && base->kind == LSM_TYPE_REFERENCE) {
                    base = base->data.reference.elem;
                }
                if (base && base->kind == LSM_TYPE_TUPLE) {
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "scoped_identifier") == 0) {
            ...
            if (path) {
                ...
                if (qn) {
                    ...
                    const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, qn);
                    if (f && f->signature && f->signature->kind == LSM_TYPE_FUNC) {
                        const LSMType *const *rt_arr = f->signature->data.func.return_types;
                        if (rt_arr && rt_arr[0]) {
                            const LSMType *ret = rt_arr[0];
                            ...
                            if (f->receiver_type && lsm_type_is_unknown(ret) && f->short_name &&
                                ...
                                if (is_wrapper && !ts_node_is_null(args_node)) {
                                    ...
                                    if (anc > 0) {
                                        const LSMType *arg_t = rust_eval_expr_type(
                                            ctx, ts_node_named_child(args_node, 0));
                                        if (arg_t && !lsm_type_is_unknown(arg_t)) {
                                            return lsm_type_template(ctx->arena, f->receiver_type,
                                                                     &arg_t, 1);
                                        }
                                    }
                                }
                                return lsm_type_named(ctx->arena, f->receiver_type);
                            }
                            /* Self -> receiver_type substitution. */
                            if (f->receiver_type && ret->kind == LSM_TYPE_NAMED &&
                     ...
                     * `lsm_registry_lookup_method`. */
                    const char *dot = strrchr(qn, '.');
                    if (dot) {
                        char *head = lsm_arena_strndup(ctx->arena, qn, (size_t)(dot - qn));
                        const char *short_name = dot + 1;
                        const LSMRegisteredFunc *m =
                            lsm_registry_lookup_method_aliased(ctx->registry, head, short_name);
                        if (!m && ctx->module_qn) {
                            const char *full_head =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                            m = lsm_registry_lookup_method_aliased(ctx->registry, full_head,
                                                                   short_name);
                            if (m)
                                head = (char *)full_head;
                        }
                        if (m && m->signature && m->signature->kind == LSM_TYPE_FUNC &&
                            m->signature->data.func.return_types &&
                            m->signature->data.func.return_types[0]) {
                            const LSMType *ret = m->signature->data.func.return_types[0];
                            /* Substitute Self / unknown returns with the
                             * receiver type so chained calls keep typing. */
                            if (ret->kind == LSM_TYPE_NAMED &&
                                strcmp(ret->data.named.qualified_name, "Self") == 0) {
                                return lsm_type_named(ctx->arena, head);
                            }
                            if (lsm_type_is_unknown(ret) &&
                                ...
                                if (strcmp(short_name, "new") == 0 &&
                                    ...
                                    if (anc > 0) {
                                        const LSMType *arg_t = rust_eval_expr_type(
                                            ctx, ts_node_named_child(args_node, 0));
                                        if (arg_t && !lsm_type_is_unknown(arg_t)) {
                                            return lsm_type_template(ctx->arena, head, &arg_t, 1);
                                        }
                                    }
                                }
                                return lsm_type_named(ctx->arena, head);
        ...
        if (strcmp(fk, "field_expression") == 0) {
            ...
            if (!ts_node_is_null(value) && !ts_node_is_null(field)) {
                ...
                if (mname && recv) {
                    ...
                    if (base && base->kind == LSM_TYPE_TEMPLATE) {
                        ...
                        if (strstr(tname, "Vec") || strstr(tname, "VecDeque") ||
                            ...
                            if (strcmp(mname, "iter") == 0 || strcmp(mname, "iter_mut") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    return lsm_type_template(
                                        ctx->arena, "core.iter.Iterator",
                                        &base->data.template_type.template_args[0], 1);
                                }
                                return lsm_type_template(ctx->arena, "core.iter.Iterator", NULL, 0);
                            }
                            ...
                            if (strcmp(mname, "len") == 0 || strcmp(mname, "capacity") == 0) {
                                return lsm_type_builtin(ctx->arena, "usize");
                            }
                            ...
                            if (strcmp(mname, "first") == 0 || strcmp(mname, "last") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    const LSMType *opt_args[1] = {
                                        base->data.template_type.template_args[0]};
                                    return lsm_type_template(ctx->arena, "core.option.Option",
                                                             opt_args, 1);
                                }
                            }
                        ...
                        if (strstr(tname, "Option")) {
                            ...
                            if (strcmp(mname, "as_ref") == 0) {
                                if (base->data.template_type.arg_count > 0) {
                                    const LSMType *arg0 = lsm_type_reference(
                        ...
                        if (strstr(tname, "Result")) {
                            ...
                            if (strcmp(mname, "ok") == 0 &&
                                ...
                                const LSMType *a0 = base->data.template_type.template_args[0];
                                return lsm_type_template(ctx->arena, "core.option.Option", &a0, 1);
                            }
                            if (strcmp(mname, "err") == 0 &&
                                base->data.template_type.arg_count > 1) {
                                const LSMType *a1 = base->data.template_type.template_args[1];
                                return lsm_type_template(ctx->arena, "core.option.Option", &a1, 1);
                            }
                        ...
                        if (strstr(tname, "Iterator")) {
                            ...
                            if (strcmp(mname, "collect") == 0) {
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    return lsm_type_template(ctx->arena, "alloc.vec.Vec",
                            ...
                            if (strcmp(mname, "next") == 0 || strcmp(mname, "last") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 0) {
                                    const LSMType *a0 = base->data.template_type.template_args[0];
                                    return lsm_type_template(ctx->arena, "core.option.Option", &a0,
                                                             1);
                                }
                            }
                            ...
                            if (strcmp(mname, "position") == 0) {
                                const LSMType *usize = lsm_type_builtin(ctx->arena, "usize");
                                return lsm_type_template(ctx->arena, "core.option.Option", &usize,
                                                         1);
                            }
                            if (strcmp(mname, "enumerate") == 0 &&
                                base->data.template_type.arg_count > 0) {
                                /* Iterator<(usize, T)>. */
                                const LSMType *pair[2] = {
                                    lsm_type_builtin(ctx->arena, "usize"),
                                    base->data.template_type.template_args[0]};
                                const LSMType *tup = lsm_type_tuple(ctx->arena, pair, 2);
                                return lsm_type_template(ctx->arena, "core.iter.Iterator", &tup, 1);
                            }
                        }
                        ...
                        if (strstr(tname, "HashMap") || strstr(tname, "BTreeMap")) {
                            ...
                            if (strcmp(mname, "get") == 0 || strcmp(mname, "get_mut") == 0 ||
                                ...
                                if (base->data.template_type.arg_count > 1) {
                                    const LSMType *v = base->data.template_type.template_args[1];
                                    return lsm_type_template(ctx->arena, "core.option.Option", &v,
                                                             1);
                                }
                            }
                            if (strcmp(mname, "iter") == 0 || strcmp(mname, "iter_mut") == 0) {
                                ...
                                if (base->data.template_type.arg_count > 1) {
                                    ...
                                    const LSMType *tup = lsm_type_tuple(ctx->arena, pair, 2);
                                    return lsm_type_template(ctx->arena, "core.iter.Iterator", &tup,
                                                             1);
                                }
                            }
                            if (strcmp(mname, "keys") == 0 &&
                                base->data.template_type.arg_count > 0) {
                                return lsm_type_template(ctx->arena, "core.iter.Iterator",
                                                         &base->data.template_type.template_args[0],
                                                         1);
                            }
                     ...
                     * across each link. */
                    const LSMType *t = rust_eval_member_access(ctx, recv, mname);
                    if (t && t->kind == LSM_TYPE_FUNC && t->data.func.return_types &&
                        t->data.func.return_types[0]) {
                        const LSMType *ret = t->data.func.return_types[0];
                        /* Self -> receiver. */
                        if (ret->kind == LSM_TYPE_NAMED &&
                            strcmp(ret->data.named.qualified_name, "Self") == 0) {
                            const LSMType *rb = recv;
                            while (rb && rb->kind == LSM_TYPE_REFERENCE)
                                rb = rb->data.reference.elem;
                            if (rb && rb->kind == LSM_TYPE_NAMED) {
                                return lsm_type_named(ctx->arena, rb->data.named.qualified_name);
                            }
                        }
                        return ret;
                    }
        ...
        const LSMType *func_type = rust_eval_expr_type(ctx, func_node);
        if (func_type && func_type->kind == LSM_TYPE_FUNC && func_type->data.func.return_types &&
        ...
        if (strcmp(name, "vec") == 0) {
            ...
            if (!ts_node_is_null(args)) {
                ...
                for (uint32_t i = 0; i < nc; i++) {
                    ...
                    const LSMType *elem = rust_eval_expr_type(ctx, c);
                    if (elem && !lsm_type_is_unknown(elem)) {
                        return lsm_type_template(ctx->arena, "alloc.vec.Vec", &elem, 1);
                    }
                }
            }
            return lsm_type_template(ctx->arena, "alloc.vec.Vec", NULL, 0);
        }
        if (is_string_macro(name)) {
            return lsm_type_named(ctx->arena, "alloc.string.String");
        }
    ...
    if (strcmp(kind, "reference_expression") == 0) {
        ...
        return lsm_type_reference(ctx->arena, rust_eval_expr_type(ctx, value));
    }
    ...
    if (strcmp(kind, "unary_expression") == 0) {
        ...
        const LSMType *inner =
            ts_node_is_null(operand) ? lsm_type_unknown() : rust_eval_expr_type(ctx, operand);
        if (op && strcmp(op, "*") == 0) {
            if (inner && inner->kind == LSM_TYPE_REFERENCE) {
                return inner->data.reference.elem;
            }
            if (inner && inner->kind == LSM_TYPE_POINTER) {
    ...
    if (strcmp(kind, "index_expression") == 0) {
        ...
        const LSMType *op_type = rust_eval_expr_type(ctx, value);
        const LSMType *base = op_type;
        while (base && base->kind == LSM_TYPE_REFERENCE)
            base = base->data.reference.elem;
        if (base && base->kind == LSM_TYPE_SLICE) {
    ...
    if (strcmp(kind, "try_expression") == 0 && ts_node_named_child_count(node) > 0) {
        const LSMType *inner = rust_eval_expr_type(ctx, ts_node_named_child(node, 0));
    ...
    if (strcmp(kind, "await_expression") == 0 && ts_node_named_child_count(node) > 0) {
        const LSMType *inner = rust_eval_expr_type(ctx, ts_node_named_child(node, 0));
        if (inner && inner->kind == LSM_TYPE_TEMPLATE && inner->data.template_type.arg_count > 0) {
            return inner->data.template_type.template_args[0];
        }
        return inner;
    }
    ...
    if (strcmp(kind, "array_expression") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            const LSMType *elem = rust_eval_expr_type(ctx, ts_node_named_child(node, 0));
            return lsm_type_slice(ctx->arena, elem);
        }
        return lsm_type_unknown();
    }
    ...
    if (strcmp(kind, "range_expression") == 0) {
        const LSMType *elem_type = NULL;
        uint32_t nc = ts_node_named_child_count(node);
        for (uint32_t i = 0; i < nc; i++) {
            TSNode c = ts_node_named_child(node, i);
            const LSMType *t = rust_eval_expr_type(ctx, c);
        ...
        if (elem_type) {
            return lsm_type_template(ctx->arena, "core.iter.Iterator", &elem_type, 1);
        }
        return lsm_type_template(ctx->arena, "core.iter.Iterator", NULL, 0);
    }
    ...
    if (strcmp(kind, "struct_expression") == 0) {
        ...
        if (!ts_node_is_null(name_node)) {
            ...
            if (path) {
                return lsm_type_named(ctx->arena, resolve_path_to_type_qn(ctx, path));
            }
        }
        return lsm_type_unknown();
    }
...
const LSMType *rust_eval_expr_typed(RustLSPContext *ctx, TSNode node, const LSMType *expected) {
    const LSMType *synth = rust_eval_expr_type(ctx, node);
    if (!expected)
        return synth;
    if (!synth || lsm_type_is_unknown(synth)) {
        return expected;
    }
    /* Template with same head and missing args -> use expected. */
    if (synth->kind == LSM_TYPE_TEMPLATE && expected->kind == LSM_TYPE_TEMPLATE &&
    ...
    if (synth->kind == LSM_TYPE_NAMED && expected->kind == LSM_TYPE_TEMPLATE &&
...
/* Member access type: returns the LSMType for `recv.field_or_method`.
...
static const LSMType *rust_eval_member_access(RustLSPContext *ctx, const LSMType *recv,
    ...
    const LSMType *base = recv;
    while (base && (base->kind == LSM_TYPE_REFERENCE || base->kind == LSM_TYPE_POINTER)) {
        base = (base->kind == LSM_TYPE_REFERENCE) ? base->data.reference.elem
    ...
    if (base->kind == LSM_TYPE_NAMED) {
        ...
        if (strcmp(nm, "str") == 0)
            ...
            type_qn = nm;
    } else if (base->kind == LSM_TYPE_SLICE) {
        type_qn = "core.slice";
    } else {
        return lsm_type_unknown();
    }
    ...
    const LSMRegisteredFunc *method = rust_lookup_method(ctx, type_qn, member);
    if (method && method->signature) {
        ...
        const LSMType *sig = method->signature;
        if (sig && sig->kind == LSM_TYPE_FUNC && sig->data.func.return_types &&
            sig->data.func.return_types[0]) {
            const LSMType *ret = sig->data.func.return_types[0];
            if (ret && ret->kind == LSM_TYPE_NAMED &&
                strcmp(ret->data.named.qualified_name, "Self") == 0) {
                return lsm_type_named(ctx->arena, type_qn);
            }
        }
    ...
    const LSMType *ft = rust_lookup_field(ctx, type_qn, member, 0);
    if (ft) {
...
static const LSMType *rust_lookup_field(RustLSPContext *ctx, const char *type_qn,
                                        const char *field_name, int depth) {
    if (!type_qn || !field_name || depth > LSM_LSP_MAX_LOOKUP_DEPTH)
        return NULL;
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const LSMType *f = rust_lookup_field(ctx, rt->embedded_types[i], field_name, depth + 1);
            if (f)
                return f;
        }
    }
...
static const LSMType *rust_deref_step(RustLSPContext *ctx, const LSMType *t) {
    if (!t)
        return NULL;
    /* Smart pointers: Box<T>, Rc<T>, Arc<T>, etc. The grammar gives us a
     * TEMPLATE; the inner type is the first template arg. */
    if (t->kind == LSM_TYPE_TEMPLATE && t->data.template_type.template_name) {
        ...
        if (rt && rt->embedded_types) {
            for (int i = 0; rt->embedded_types[i]; i++) {
                ...
                if (strncmp(e, prefix, sizeof(prefix) - 1) == 0) {
                    return lsm_type_named(ctx->arena, e + sizeof(prefix) - 1);
                }
            }
        }
    }
...
static const LSMRegisteredFunc *rust_registry_lookup_inherent_method(const LSMTypeRegistry *reg,
    ...
    int best_index = -1;
    LSMMethodIter it;
    lsm_registry_methods(reg, receiver_qn, method_name, &it);
    for (int index; (index = lsm_method_iter_next(&it)) >= 0;) {
        const LSMRegisteredFunc *candidate = &reg->funcs[index];
        if ((candidate->flags & LSM_FUNC_FLAG_RUST_TRAIT_IMPL) == 0 &&
...
static const LSMRegisteredFunc *rust_registry_lookup_inherent_method_aliased(
    const LSMTypeRegistry *reg, const char *receiver_qn, const char *method_name) {
    const LSMRegisteredFunc *method =
        rust_registry_lookup_inherent_method(reg, receiver_qn, method_name);
    if (method) {
        return method;
    }
    const LSMRegisteredType *type = lsm_registry_lookup_type(reg, receiver_qn);
    for (int depth = 0; depth < 16 && type && type->alias_of; depth++) {
        ...
        type = lsm_registry_lookup_type(reg, type->alias_of);
    }
    return NULL;
}
...
static const LSMRegisteredFunc *rust_registry_lookup_trait_impl_method(const LSMTypeRegistry *reg,
                                                                       const char *receiver_qn,
                                                                       const char *trait_qn,
                                                                       const char *method_name,
                                                                       bool *out_ambiguous) {
    ...
    for (int index; (index = lsm_method_iter_next(&it)) >= 0;) {
        const LSMRegisteredFunc *candidate = &reg->funcs[index];
        if ((candidate->flags & LSM_FUNC_FLAG_RUST_TRAIT_IMPL) == 0 || !candidate->impl_trait_qn) {
            continue;
        }
...
static const LSMRegisteredFunc *rust_lookup_method_depth(RustLSPContext *ctx, const char *type_qn,
...
const LSMRegisteredFunc *rust_lookup_method(RustLSPContext *ctx, const char *type_qn,
...
static const LSMRegisteredFunc *rust_lookup_method_in_trait(RustLSPContext *ctx,
    ...
    if (!ctx || !trait_qn || !method_name)
        return NULL;
    const LSMTypeRegistry *reg = ctx->registry;
    int best_default = -1;
    int best_any = -1;
    LSMMethodIter it;
    lsm_registry_methods(reg, trait_qn, method_name, &it);
    for (int index; (index = lsm_method_iter_next(&it)) >= 0;) {
        const LSMRegisteredFunc *candidate = &reg->funcs[index];
        if (candidate->flags & LSM_FUNC_FLAG_RUST_TRAIT_IMPL) {
            continue;
        }
        if (best_any < 0 || index < best_any) {
            best_any = index;
        }
        if ((candidate->flags & LSM_FUNC_FLAG_RUST_ABSTRACT) == 0 &&
...
static const LSMRegisteredFunc *rust_resolve_trait_method(RustLSPContext *ctx,
    ...
    const LSMRegisteredType *receiver_type = lsm_registry_lookup_type(reg, receiver_type_qn);
    if (receiver_type && receiver_type->is_interface) {
        const LSMRegisteredFunc *trait_method =
    ...
    uint64_t nm_key = 0;
    if (nm_active) {
        nm_key = lsm_negmemo_key(1, receiver_type_qn, method_name);
        if (lsm_negmemo_contains(&ctx->neg_memo, nm_key)) {
            return NULL;
        }
    }

    bool trait_ambiguous = false;
    const LSMRegisteredFunc *impl_method = rust_registry_lookup_trait_impl_method(
    ...
    const LSMRegisteredFunc *default_method = NULL;
    ...
    if (receiver_type && receiver_type->embedded_types) {
        for (int i = 0; receiver_type->embedded_types[i]; i++) {
            ...
            const LSMRegisteredType *trait_type = lsm_registry_lookup_type(reg, trait_qn);
            if (!trait_type || !trait_type->is_interface) {
                continue;
            }
            const LSMRegisteredFunc *candidate =
    ...
    if (nm_active) {
        lsm_negmemo_insert(&ctx->neg_memo, ctx->arena, nm_key);
    }
    return NULL;
}
...
static const LSMRegisteredFunc *rust_find_sole_trait_impl(RustLSPContext *ctx, const char *trait_qn,
    ...
    uint64_t nm_key = 0;
    if (nm_active) {
        nm_key = lsm_negmemo_key(2, trait_qn, method_name);
        if (lsm_negmemo_contains(&ctx->neg_memo, nm_key)) {
            return NULL;
        }
    }
    const LSMRegisteredFunc *first = NULL;
    const char *first_qn = NULL;
    int n = 0;
    const char *tdot = strrchr(trait_qn, '.');
    const char *tbare = tdot ? tdot + 1 : trait_qn;
    LSMTypeEmbedIter eit;
    lsm_registry_types_by_embedded_bare(reg, tbare, &eit);
    for (int ti; n < 2 && (ti = lsm_type_embed_iter_next(&eit)) >= 0;) {
        const LSMRegisteredType *t = &reg->types[ti];
        ...
        if (!impls)
            continue;
        const LSMRegisteredFunc *mf = rust_registry_lookup_trait_impl_method(
 ...
 * built-in arithmetic. */
static void rust_emit_operator_call(RustLSPContext *ctx, const LSMType *recv, const char *method,
                                    TSNode site) {
    if (!recv || !method)
        return;
    const LSMType *base = recv;
    ...
    if (!type_qn || is_rust_primitive(type_qn))
        return;
    int impl_count = 0;
    const LSMRegisteredFunc *m = rust_resolve_trait_method(ctx, type_qn, method, &impl_count, NULL);
    if (m && m->qualified_name) {
        ...
        rust_emit_resolved_call(ctx, m->qualified_name, "lsp_operator_trait",
                                LSM_RUST_CONF_OPERATOR);
        /* `a + b` is a binary_expression, never a syntactic call node, so the
         * extractor produced no LSMCall to pair with the resolved_call above.
...
static void rust_record_macro_rule(RustLSPContext *ctx, const char *macro_name, TSNode pattern,
    ...
    if (ctx->macro_rules_count % 16 == 0) {
        ...
        struct RustMacroRule **narr = (struct RustMacroRule **)lsm_arena_alloc(
    ...
    RustMacroRule *r = (RustMacroRule *)lsm_arena_alloc(ctx->arena, sizeof(*r));
    if (!r)
        return;
    memset(r, 0, sizeof(*r));
    r->macro_name = lsm_arena_strdup(ctx->arena, macro_name);
    ...
    if (!ts_node_is_null(pattern)) {
        char *pt = lsm_node_text(ctx->arena, pattern, ctx->source);
        if (pt) {
            const char *inner;
            int inner_len;
            rust_macro_strip_outer(pt, (int)strlen(pt), &inner, &inner_len);
            r->pattern_text = lsm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
            r->pattern_len = inner_len;
        }
    }

    char *tt = lsm_node_text(ctx->arena, transcriber, ctx->source);
    if (tt) {
        int len = (int)strlen(tt);
        const char *inner;
        int inner_len;
        rust_macro_strip_outer(tt, len, &inner, &inner_len);
        r->transcriber_text = lsm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
...
static char *macro_substitute(LSMArena *arena, const char *xs, int xs_len, const MacroEnv *env) {
    /* Estimate output size: each metavar expansion could be up to
     * ~256 bytes; bound the total at xs_len * 4 + 4KB. */
    int cap = xs_len * 4 + 4096;
    char *out = (char *)lsm_arena_alloc(arena, cap + 1);
    if (!out)
        return NULL;
    int op = 0;
    int xp = 0;
...
static void rust_expand_user_macro(RustLSPContext *ctx, const char *mname, TSNode invocation) {
    ...
    if (!ts_node_is_null(invocation)) {
        ...
        if (!ts_node_is_null(args_tt)) {
            char *at = lsm_node_text(ctx->arena, args_tt, ctx->source);
            if (at) {
    ...
    uint64_t mm_key = lsm_negmemo_key(3, mname, substituted);
    if (lsm_negmemo_contains(&ctx->macro_memo, mm_key)) {
        return;
    }
...
static void rust_resolve_macro_arg_exprs(RustLSPContext *ctx, TSNode invocation) {
    ...
    if (inner_len <= 0)
        return;
    char *arg_text = lsm_arena_strndup(ctx->arena, inner, (size_t)inner_len);
    if (!arg_text)
        return;
    ...
    if (ctx->macro_expand_depth > 0) {
        uint64_t am_key = lsm_negmemo_key(4, arg_text, NULL);
        if (lsm_negmemo_contains(&ctx->macro_memo, am_key)) {
            return;
        }
        lsm_negmemo_insert(&ctx->macro_memo, ctx->arena, am_key);
    }
    ...
    if (tree) {
        ...
        if (!ts_node_has_error(root)) {
            ...
            for (uint32_t i = 0; i < rnc; i++) {
                 ...
                 * no LSMCall to pair with. Inject matching synthetic calls. */
...
static void rust_bind_pattern(RustLSPContext *ctx, TSNode pattern, const LSMType *type) {
    if (ts_node_is_null(pattern))
        return;
    ...
    if (strcmp(kind, "tuple_pattern") == 0) {
        const LSMType *base = type;
        while (base && base->kind == LSM_TYPE_REFERENCE)
            base = base->data.reference.elem;
        uint32_t nc = ts_node_named_child_count(pattern);
        for (uint32_t i = 0; i < nc; i++) {
            const LSMType *elem_t = lsm_type_unknown();
            if (base && base->kind == LSM_TYPE_TUPLE && (int)i < base->data.tuple.count) {
    ...
    if (strcmp(kind, "tuple_struct_pattern") == 0) {
        ...
        while (base && base->kind == LSM_TYPE_REFERENCE)
            base = base->data.reference.elem;
        const LSMType *inner = lsm_type_unknown();
        if (base && base->kind == LSM_TYPE_TEMPLATE && base->data.template_type.arg_count > 0) {
    ...
    if (strcmp(kind, "struct_pattern") == 0) {
        ...
        if (base && base->kind == LSM_TYPE_NAMED)
            type_qn = base->data.named.qualified_name;
        else if (base && base->kind == LSM_TYPE_TEMPLATE)
            type_qn = base->data.template_type.template_name;
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(fk, "field_pattern") == 0) {
                ...
                if (!fname)
                    continue;
                const LSMType *ft =
                    type_qn ? rust_lookup_field(ctx, type_qn, fname, 0) : lsm_type_unknown();
                if (!ft)
                    ft = lsm_type_unknown();
                if (!ts_node_is_null(pat_node)) {
                    rust_bind_pattern(ctx, pat_node, ft);
                } else {
                    lsm_scope_bind(ctx->current_scope, fname, ft);
                }
                ...
                if (fname && strcmp(fname, "_") != 0) {
                    const LSMType *ft =
                        type_qn ? rust_lookup_field(ctx, type_qn, fname, 0) : lsm_type_unknown();
                    lsm_scope_bind(ctx->current_scope, fname, ft ? ft : lsm_type_unknown());
                }
            }
        }
        return;
...
void rust_process_statement(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "let_declaration") == 0) {
        ...
        if (!ts_node_is_null(pat) && strcmp(ts_node_type(pat), "identifier") == 0 &&
            ...
            if (strcmp(value_kind, "identifier") == 0) {
                ...
                if (value_name && lsm_scope_contains(ctx->current_scope, value_name)) {
                    callable_qn = lsm_scope_lookup_callable(ctx->current_scope, value_name);
                } else if (value_name) {
                    const char *resolved = rust_resolve_path_expr(ctx, value_name);
                    const LSMRegisteredFunc *function =
                        resolved ? lsm_registry_lookup_func(ctx->registry, resolved) : NULL;
                    if (!function && resolved && ctx->module_qn) {
                        function = lsm_registry_lookup_func(
                            ctx->registry,
                            lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, resolved));
                    }
            ...
            if (binding_name && callable_qn) {
                lsm_scope_bind_callable(ctx->current_scope, binding_name, let_type, callable_qn);
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "assignment_expression") == 0 ||
        ...
        if (!ts_node_is_null(left) && strcmp(ts_node_type(left), "identifier") == 0) {
            ...
            if (name && lsm_scope_contains(ctx->current_scope, name)) {
                ...
                lsm_scope_update_callable(ctx->current_scope, name, target);
            }
        }
        return;
    }
    ...
    if (strcmp(kind, "const_item") == 0 || strcmp(kind, "static_item") == 0) {
        ...
        TSNode tn = ts_node_child_by_field_name(node, "type", 4);
        const LSMType *type =
            ts_node_is_null(tn) ? lsm_type_unknown() : rust_parse_type_node(ctx, tn);
        if (!ts_node_is_null(name_node)) {
            char *name = rust_node_text(ctx, name_node);
            if (name)
                lsm_scope_bind(ctx->current_scope, name, type);
...
/* Inject a synthetic LSMCall into result->calls so the downstream pipeline
 * (lsm_pipeline_find_lsp_resolution) can pair it with the resolved_call and
...
static void rust_inject_syn_call(RustLSPContext *ctx, const char *callee_qn) {
    ...
    if (!short_name || !short_name[0])
        return;
    LSMCall call = {0};
    call.callee_name = lsm_arena_strdup(ctx->arena, short_name);
    ...
    lsm_calls_push(ctx->syn_calls, ctx->arena, call);
}
...
static void rust_ensure_known_macro_carrier(RustLSPContext *ctx, const char *macro_name,
    ...
    if (!ctx || !ctx->syn_calls || !macro_name || !callee_qn ||
        !lsm_is_keyword(macro_name, LSM_LANG_RUST) ||
    ...
    for (int i = 0; i < ctx->syn_calls->count; i++) {
        LSMCall *call = &ctx->syn_calls->items[i];
...
static const char *rust_resolve_callable_value(RustLSPContext *ctx, TSNode value,
    ...
    if (strcmp(kind, "identifier") == 0 && lsm_scope_contains(ctx->current_scope, path)) {
        return lsm_scope_lookup_callable(ctx->current_scope, path);
    }
    const char *resolved = rust_resolve_path_expr(ctx, path);
    if (!resolved) {
        return NULL;
    }
    const LSMRegisteredFunc *function = lsm_registry_lookup_func(ctx->registry, resolved);
    if (!function && ctx->module_qn) {
        function = lsm_registry_lookup_func(
            ctx->registry, lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, resolved));
    }
    ...
    const LSMRegisteredFunc *unique = NULL;
    int matches = 0;
    LSMFreeFuncIter iter;
    lsm_registry_free_funcs_by_short_name(ctx->registry, tail, &iter);
    for (int index; matches < 2 && (index = lsm_free_func_iter_next(&iter)) >= 0;) {
...
static void rust_resolve_callable_argument_references(RustLSPContext *ctx, TSNode arguments) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        LSMResolvedCall reference = {
            .caller_qn = ctx->enclosing_func_qn,
            .callee_qn = target,
            .strategy = "lsp_callable_value_reference",
            .confidence = LSM_RUST_CONF_DIRECT,
            .reason = source_name,
            .kind = LSM_RESOLVED_CALL_REFERENCE,
            .site_start_byte = start,
            .site_end_byte = end,
        };
        lsm_resolvedcall_push(ctx->resolved_calls, ctx->arena, reference);
    }
}
...
static void rust_resolve_call_expression_inner(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(fk, "field_expression") == 0) {
        ...
        if (base && base->kind == LSM_TYPE_NAMED)
            ...
            type_qn = base->data.template_type.template_name;
        else if (base && base->kind == LSM_TYPE_BUILTIN) {
            const char *nm = base->data.builtin.name;
            if (strcmp(nm, "str") == 0)
                type_qn = "core.str";
            else if (strcmp(nm, "String") == 0)
        ...
        } else if (base && base->kind == LSM_TYPE_SLICE) {
            /* `&[T]` / `[T]` method dispatch. */
            type_qn = "core.slice";
        }
        ...
        if (type_qn) {
            ...
            if (m) {
                ...
                float conf = LSM_RUST_CONF_METHOD;
                const LSMRegisteredType *method_owner =
                    m->receiver_type ? lsm_registry_lookup_type(ctx->registry, m->receiver_type)
                                     : NULL;
                if ((m->flags & LSM_FUNC_FLAG_RUST_TRAIT_IMPL) != 0 ||
                    (method_owner && method_owner->is_interface) || trait_dispatch) {
                    strategy = "lsp_trait_dispatch";
                    conf = (impl_count == 1) ? LSM_RUST_CONF_TRAIT_SOLE : LSM_RUST_CONF_TRAIT_AMB;
                }
            ...
            const LSMType *cur = base;
            for (int hop = 0; hop < 8; hop++) {
                const LSMType *next = rust_deref_step(ctx, cur);
                if (!next)
                    break;
                /* Unwrap reference layers introduced by deref. */
                while (next &&
                       (next->kind == LSM_TYPE_REFERENCE || next->kind == LSM_TYPE_POINTER)) {
                    next = (next->kind == LSM_TYPE_REFERENCE) ? next->data.reference.elem
                                                              : next->data.pointer.elem;
                }
                if (!next)
                    break;
                const char *next_qn = NULL;
                if (next->kind == LSM_TYPE_NAMED)
                    next_qn = next->data.named.qualified_name;
                else if (next->kind == LSM_TYPE_TEMPLATE)
                    next_qn = next->data.template_type.template_name;
                else if (next->kind == LSM_TYPE_BUILTIN) {
            ...
            if (is_prelude_trait_method(mname)) {
                rust_emit_resolved_call(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
                                        "lsp_prelude_trait", LSM_RUST_CONF_TRAIT_AMB);
                return;
            }
            rust_emit_unresolved_call(ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", type_qn, mname),
                                      "method_not_found");
            return;
        }
        ...
        rust_emit_unresolved_call(
            ctx, lsm_arena_sprintf(ctx->arena, "%s.%s", recv_text ? recv_text : "?", mname),
    ...
    if (strcmp(fk, "identifier") == 0 || strcmp(fk, "scoped_identifier") == 0 ||
        ...
        if (strcmp(ts_node_type(actual_func), "identifier") == 0) {
            const char *alias_target = lsm_scope_lookup_callable(ctx->current_scope, path);
            if (alias_target) {
                rust_emit_resolved_call_reason(ctx, alias_target, "lsp_callable_alias",
                                               LSM_RUST_CONF_DIRECT, path);
                return;
            }
            if (lsm_scope_contains(ctx->current_scope, path)) {
                return;
            }
        }
        ...
        if (lsm_registry_lookup_func(ctx->registry, qn)) {
            rust_emit_resolved_call(ctx, qn, "lsp_direct", LSM_RUST_CONF_DIRECT);
            return;
        }
        if (ctx->module_qn && strstr(qn, ".") == NULL) {
            const char *full = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
            if (lsm_registry_lookup_func(ctx->registry, full)) {
                rust_emit_resolved_call(ctx, full, "lsp_direct", LSM_RUST_CONF_DIRECT);
                return;
            }
        }
        ...
        if (dot) {
            ...
            const LSMRegisteredType *head_t = lsm_registry_lookup_type(ctx->registry, head);
            if (head_t && head_t->is_interface) {
                trait_qn = head;
            } else if (ctx->module_qn) {
                const char *fh = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                const LSMRegisteredType *ft = lsm_registry_lookup_type(ctx->registry, fh);
                if (ft && ft->is_interface)
                    trait_qn = fh;
            }
            if (trait_qn) {
                int tn = 0;
                const LSMRegisteredFunc *ti_m =
                    rust_find_sole_trait_impl(ctx, trait_qn, short_name, &tn);
                if (tn == 1) {
                    rust_emit_resolved_call(
                        ctx,
                        ti_m ? ti_m->qualified_name
                             : lsm_arena_sprintf(ctx->arena, "%s.%s", trait_qn, short_name),
                        "lsp_trait_ufcs", LSM_RUST_CONF_TRAIT_SOLE);
                }
                return;
            }
            const LSMRegisteredFunc *m =
            ...
            if (!m && ctx->module_qn) {
                ...
                const char *full_head =
                    lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, head);
                m = lsm_registry_lookup_method_aliased(ctx->registry, full_head, short_name);
            }
            if (m) {
        ...
        if (ctx->cargo_manifest && tail && *tail) {
            ...
            if (head_sep && head_sep > path) {
                char *head = lsm_arena_strndup(ctx->arena, path, (size_t)(head_sep - path));
                const LSMCargoManifest *m = (const LSMCargoManifest *)ctx->cargo_manifest;
                if (head && lsm_cargo_find_member(m, head)) {
                    /* `.crate_a.` — the member directory appears as a dotted
                     * QN segment for every def inside that crate. */
                    char *needle = lsm_arena_sprintf(ctx->arena, ".%s.", head);
                    const LSMRegisteredFunc *mem_unique = NULL;
                    int mem_matches = 0;
                    /* Iterate only free funcs whose short_name == tail via the index;
                     * the receiver/short_name/needle re-checks below are unchanged. */
                    LSMFreeFuncIter ffit;
                    lsm_registry_free_funcs_by_short_name(ctx->registry, tail, &ffit);
                    for (int i; mem_matches < 2 && (i = lsm_free_func_iter_next(&ffit)) >= 0;) {
                        const LSMRegisteredFunc *f = &ctx->registry->funcs[i];
        ...
        if (tail && *tail && ctx->module_qn) {
            ...
            LSMFreeFuncIter ffit;
            lsm_registry_free_funcs_by_short_name(ctx->registry, tail, &ffit);
            for (int i; matches < 2 && (i = lsm_free_func_iter_next(&ffit)) >= 0;) {
                const LSMRegisteredFunc *f = &ctx->registry->funcs[i];
                if (!f->short_name || !f->qualified_name)
                    continue;
                if (f->receiver_type)
...
static void rust_resolve_calls_in_node(RustLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "call_expression") == 0) {
        ...
        if (!ts_node_is_null(fn) && strcmp(ts_node_type(fn), "field_expression") == 0) {
            ...
            if (!ts_node_is_null(val) && !ts_node_is_null(fld)) {
                ...
                if (is_item_method) {
                    const LSMType *recv = rust_eval_expr_type(ctx, val);
                    const LSMType *base = recv;
                    while (base &&
                           (base->kind == LSM_TYPE_REFERENCE || base->kind == LSM_TYPE_POINTER)) {
                        base = (base->kind == LSM_TYPE_REFERENCE) ? base->data.reference.elem
                                                                  : base->data.pointer.elem;
                    }
                    if (base && base->kind == LSM_TYPE_TEMPLATE &&
            ...
            if (mname) {
                ...
                if (rust_has_visible_macro_definition(ctx, mname, node)) {
                    ...
                    if (strcmp(mname, "println") == 0 || strcmp(mname, "eprintln") == 0 ||
                        ...
                        path = lsm_arena_sprintf(ctx->arena, "std.macros.%s", mname);
                        ...
                        path = lsm_arena_sprintf(ctx->arena, "core.macros.%s", mname);
                    ...
                    if (path) {
                        rust_emit_resolved_call(ctx, path, "lsp_macro", LSM_RUST_CONF_MACRO_KNOWN);
    ...
    if (strcmp(kind, "if_let_expression") == 0 || strcmp(kind, "while_let_expression") == 0) {
        ...
        if (!ts_node_is_null(pat) && !ts_node_is_null(val)) {
            const LSMType *vt = rust_eval_expr_type(ctx, val);
        ...
        if (!ts_node_is_null(pat) && !ts_node_is_null(val)) {
            ...
            if (base && base->kind == LSM_TYPE_SLICE) {
                ...
                if ((strstr(nm, "HashMap") || strstr(nm, "BTreeMap")) &&
                    ...
                    const LSMType *pair[2] = {base->data.template_type.template_args[0],
                                              base->data.template_type.template_args[1]};
                    elem = lsm_type_tuple(ctx->arena, pair, 2);
                }
            }
    ...
    if (strcmp(kind, "match_arm") == 0) {
         ...
         * work using lsm_type_unknown() if we cannot derive it. */
    }
     ...
     * sibling closure. */
    if (strcmp(kind, "closure_expression") == 0) {
        const LSMType *hint = ctx->pending_closure_param_type;
        ...
        if (!ts_node_is_null(params)) {
            ...
            for (uint32_t i = 0; i < pc; i++) {
                 ...
                 * as the pattern. */
                TSNode pat = p;
                const LSMType *bound = (i == 0 && hint) ? hint : lsm_type_unknown();
...
static void rust_process_function(RustLSPContext *ctx, TSNode func_node, const char *parent_qn) {
    ...
    ctx->enclosing_func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", prefix, name);
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            if (strcmp(pk, "self_parameter") == 0) {
                if (ctx->self_type_qn) {
                    ...
                    const LSMType *self_t = lsm_type_named(ctx->arena, ctx->self_type_qn);
                    if (text && strchr(text, '&')) {
                        self_t = lsm_type_reference(ctx->arena, self_t);
                    }
                    lsm_scope_bind(ctx->current_scope, "self", self_t);
                }
                continue;
            }
...
static void rust_record_type_param_bound(RustLSPContext *ctx, const char *param_name,
    ...
    if (ctx->type_param_bound_count % 8 == 0) {
        ...
        void *narr = lsm_arena_alloc(ctx->arena, new_cap * sizeof(*ctx->type_param_bounds));
        if (!narr)
            return;
    ...
    ctx->type_param_bounds[ctx->type_param_bound_count].param_name =
        lsm_arena_strdup(ctx->arena, param_name);
    ctx->type_param_bounds[ctx->type_param_bound_count].trait_qn =
        lsm_arena_strdup(ctx->arena, trait_qn);
...
static void rust_collect_bounds_from_text(RustLSPContext *ctx, const char *text) {
    ...
    while (from < len) {
        ...
        char *pname =
            lsm_arena_strndup(ctx->arena, text + name_start, (size_t)(name_end - name_start));
        /* Look for `:`. */
        while (from < len && (text[from] == ' ' || text[from] == '\t'))
            from++;
        ...
        while (from < len) {
            ...
            if (c == '<' || c == '(' || c == '[')
                ...
                if (b_end > b_start) {
                    char *btext =
                        lsm_arena_strndup(ctx->arena, text + b_start, (size_t)(b_end - b_start));
...
void rust_lsp_process_file(RustLSPContext *ctx, TSNode root) {
     ...
     * already in this file. Only bare `mod foo;` declarations are. */
    {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            if (!name)
                continue;
            lsm_idxmemo_put_if_absent(&ctx->declared_modules, ctx->arena, name, 1);
...
static void rust_collect_uses(RustLSPContext *ctx, TSNode root) {
    ...
    stack_t *top = (stack_t *)lsm_arena_alloc(ctx->arena, sizeof(stack_t));
    top->node = root;
    top->prev = NULL;
    while (top) {
        TSNode n = top->node;
        ...
        if (strcmp(k, "use_declaration") == 0) {
            ...
            if (full) {
                ...
                if (flen >= 3 && strcmp(full + flen - 3, "::*") == 0) {
                    char *mod = lsm_arena_strndup(ctx->arena, full, flen - 3);
                    ...
                    if (lbr) {
                        ...
                        char *prefix = lsm_arena_strndup(ctx->arena, full, prefix_len);
                        char *body = lsm_arena_strdup(ctx->arena, lbr + 1);
                        ...
                        while (tok) {
                            ...
                            char *full_path =
                                (strcmp(tok, "self") == 0)
                                    ? lsm_arena_strdup(ctx->arena, prefix)
                                    : lsm_arena_sprintf(ctx->arena, "%s::%s", prefix, path_part);
        ...
        if (strcmp(k, "mod_item") == 0 || strcmp(k, "source_file") == 0 ||
            ...
            for (uint32_t i = 0; i < nc; i++) {
                ...
                if (ts_node_is_null(c))
                    continue;
                stack_t *nx = (stack_t *)lsm_arena_alloc(ctx->arena, sizeof(stack_t));
                nx->node = c;
                nx->prev = top;
                top = nx;
            }
...
void lsm_rust_build_local_registry(LSMArena *arena, LSMTypeRegistry *reg, LSMFileResult *result,
                                   const char *module_qn, TSNode root, const char *source) {

    lsm_registry_init(reg, arena);
    lsm_rust_stdlib_register(reg, arena);
     ...
     * leaf-first guesses. */
    LSMIdxMemo type_idx = {0};
    for (int i = 0; i < reg->type_count; i++) {
        if (reg->types[i].qualified_name) {
            lsm_idxmemo_put_if_absent(&type_idx, arena, reg->types[i].qualified_name, i);
        }
    }
    for (int i = 0; i < result->defs.count; i++) {
        LSMDefinition *d = &result->defs.items[i];
        if (!d->qualified_name || !d->name || !lsm_label_is_type_like(d->label)) {
            continue;
        }
        ...
        lsm_idxmemo_put_if_absent(&type_idx, arena, rt.qualified_name, reg->type_count - 1);
    }
    ...
    for (int i = 0; i < result->defs.count; i++) {
        ...
        if (d->label && (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0)) {
            ...
            rf.min_params = -1;
            if (d->is_abstract) {
                rf.flags |= LSM_FUNC_FLAG_RUST_ABSTRACT;
            }
            if (d->impl_trait) {
                rf.flags |= LSM_FUNC_FLAG_RUST_TRAIT_IMPL;
                rf.impl_trait_qn =
            ...
            if (d->signature_param_types || d->signature_param_count > 0) {
                ...
                param_types = lsm_type_materialize_signature_params(
                ...
                while (d->param_types[count])
                    count++;
                if (count > 0) {
                    param_types = (const LSMType **)lsm_arena_alloc(
                        arena, (count + 1) * sizeof(const LSMType *));
                    for (int j = 0; j < count; j++) {
                        param_types[j] =
            ...
            rf.signature = lsm_type_func(arena, param_names, param_types, ret_types);

            if (strcmp(d->label, "Method") == 0 && d->parent_class) {
                rf.receiver_type = d->parent_class;
                if (lsm_idxmemo_get(&type_idx, rf.receiver_type) < 0) {
                    LSMRegisteredType auto_t;
                    memset(&auto_t, 0, sizeof(auto_t));
                    auto_t.qualified_name = rf.receiver_type;
                    const char *dot = strrchr(rf.receiver_type, '.');
                    auto_t.short_name = dot ? lsm_arena_strdup(arena, dot + 1) : rf.receiver_type;
                    lsm_registry_add_type(reg, auto_t);
                    lsm_idxmemo_put_if_absent(&type_idx, arena, auto_t.qualified_name,
                                              reg->type_count - 1);
                }
            }
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(tk, "struct_item") == 0) {
                ...
                if (ts_node_is_null(name_node) || ts_node_is_null(body))
                    continue;
                char *tn = lsm_node_text(arena, name_node, source);
                if (!tn || !tn[0])
                    continue;
                const char *type_qn = lsm_arena_sprintf(arena, "%s.%s", module_qn, tn);
                ...
                if (strcmp(ts_node_type(body), "field_declaration_list") == 0) {
                    ...
                    for (uint32_t j = 0; j < fc && fld_count < 63; j++) {
                        ...
                        char *fname = lsm_node_text(arena, fn, source);
                        if (!fname)
                            continue;
                        ...
                        const LSMType *ft_t = rust_parse_type_node(&tmp, ft);
                    ...
                    if (fld_count > 0) {
                        for (int ti = 0; ti < reg->type_count; ti++) {
                            if (reg->types[ti].qualified_name &&
                                ...
                                const char **names = (const char **)lsm_arena_alloc(
                                    arena, (fld_count + 1) * sizeof(const char *));
                                const LSMType **types = (const LSMType **)lsm_arena_alloc(
                                    arena, (fld_count + 1) * sizeof(const LSMType *));
            ...
            if (strcmp(tk, "trait_item") == 0) {
                ...
                if (!tn || !tn[0])
                    continue;
                const char *trait_qn = lsm_arena_sprintf(arena, "%s.%s", module_qn, tn);
                        ...
                        if (!ts_node_is_null(body)) {
                            ...
                            for (uint32_t j = 0; j < bc && mc < 63; j++) {
                                ...
                                if (ts_node_is_null(mn))
                                    continue;
                                char *mname = lsm_node_text(arena, mn, source);
                                if (mname)
                                    methods[mc++] = mname;
                            }
                            if (mc > 0) {
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            if (ts_node_is_null(mn) || ts_node_is_null(rtn))
                continue;
            char *fname = lsm_node_text(arena, mn, source);
            if (!fname)
                continue;
            const LSMType *ret = rust_parse_type_node(&tmp, rtn);
            const char *fn_qn = lsm_arena_sprintf(arena, "%s.%s", module_qn, fname);
            for (int k = 0; k < reg->func_count; k++) {
                LSMRegisteredFunc *rf = &reg->funcs[k];
                ...
                if (strcmp(rf->qualified_name, fn_qn) != 0)
                    continue;
                const LSMType **ret_arr =
                    (const LSMType **)lsm_arena_alloc(arena, 2 * sizeof(const LSMType *));
                ret_arr[0] = ret;
                ret_arr[1] = NULL;
                rf->signature = lsm_type_func_replace_returns(arena, rf->signature, ret_arr);
                break;
            }
        }
    }
            ...
            for (int di = 0; d->decorators[di]; di++) {
                ...
                while (q < rparen) {
                    ...
                    for (int di2 = 0; di2 < derive_count; di2++) {
                        ...
                        const char **new_arr = (const char **)lsm_arena_alloc(
                        ...
                        for (int mi = 0; mi < 4 && di_entry->methods[mi].short_name; mi++) {
                            ...
                            rf.qualified_name = lsm_arena_sprintf(arena, "%s.%s", d->qualified_name,
                                                                  dm->short_name);
                            ...
                            const LSMType *ret_t = lsm_type_unknown();
                            if (dm->return_type) {
                                if (strcmp(dm->return_type, "bool") == 0) {
                                    ret_t = lsm_type_builtin(arena, "bool");
                                } else if (strcmp(dm->return_type, "()") == 0) {
                                    ret_t = lsm_type_builtin(arena, "()");
                                }
                            } else if (dm->is_static) {
                                /* `default()`, `parse()` return Self. */
                                ret_t = lsm_type_named(arena, d->qualified_name);
                            }
                            const LSMType **ra = (const LSMType **)lsm_arena_alloc(
                                arena, 2 * sizeof(const LSMType *));
                            ra[0] = ret_t;
                            ra[1] = NULL;
                            rf.signature = lsm_type_func(arena, NULL, NULL, ra);
                            lsm_registry_add_func(reg, rf);
                        }
                        break;
                    }
    ...
    if (!ts_node_is_null(root)) {
        ...
        for (uint32_t i = 0; i < rnc; i++) {
            ...
            if (ts_node_is_null(type_node) || ts_node_is_null(body))
                continue;
            char *type_name = lsm_node_text(arena, type_node, source);
            ...
            for (uint32_t j = 0; j < bnc; j++) {
                ...
                if (ret && ret->kind == LSM_TYPE_NAMED &&
                    strcmp(ret->data.named.qualified_name, "Self") == 0) {
                    ret = lsm_type_named(arena, type_qn);
                }
    ...
    for (int i = 0; i < result->impl_traits.count; i++) {
        LSMImplTrait *it = &result->impl_traits.items[i];
        if (!it->struct_name || !it->trait_name)
            continue;
        const char *recv_qn = it->struct_qn
                                  ? it->struct_qn
                                  : lsm_arena_sprintf(arena, "%s.%s", module_qn, it->struct_name);
        const char *trait_qn =
        ...
        LSMRegisteredType *rt = NULL;
        int32_t type_index = lsm_idxmemo_get(&type_idx, recv_qn);
        if (type_index >= 0) {
        ...
        if (!rt) {
            ...
            lsm_idxmemo_put_if_absent(&type_idx, arena, auto_t.qualified_name, reg->type_count - 1);
        }
        rust_registered_type_add_embedded(arena, rt, trait_qn);
    }
}

void lsm_run_rust_lsp_with_manifest(LSMArena *arena, LSMFileResult *result, const char *source,
                                    int source_len, TSNode root,
                                    const struct LSMCargoManifest *manifest) {
    if (!arena || !result || !source)
        return;
    const char *module_qn = result->module_qn ? result->module_qn : "rust";

    LSMTypeRegistry reg;
    lsm_rust_build_local_registry(arena, &reg, result, module_qn, root, source);
    ...
    /* Bridge any extracted LSMImports the unified extractor saw. */
...
void lsm_run_rust_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
                      TSNode root) {
    lsm_run_rust_lsp_with_manifest(arena, result, source, source_len, root, NULL);
}
 ...
 * per-file resolver (lsm_run_rust_lsp_cross_with_manifest) and the build-once shared
 * registry (lsm_rust_build_cross_registry) so both produce a byte-identical registry.
...
static void rust_populate_cross_registry(LSMTypeRegistry *reg, LSMArena *arena,
                                         LSMRustLSPDef *defs, int def_count,
                                         const char *module_qn) {
    lsm_registry_init(reg, arena);
    lsm_rust_stdlib_register(reg, arena);
    ...
    for (int ti = 0; ti < reg->type_count; ti++) {
        ...
        if (qn) {
            lsm_idxmemo_put_if_absent(&type_idx, arena, qn, ti);
        }
    }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        LSMRegisteredType rt;
        memset(&rt, 0, sizeof(rt));
        rt.qualified_name = lsm_arena_strdup(arena, d->qualified_name);
        rt.short_name = lsm_arena_strdup(arena, d->short_name);
        rt.is_interface =
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        int32_t type_index = lsm_idxmemo_get(&type_idx, d->receiver_type);
        if (type_index < 0) {
            LSMRegisteredType auto_t;
            memset(&auto_t, 0, sizeof(auto_t));
            auto_t.qualified_name = lsm_arena_strdup(arena, d->receiver_type);
            const char *dot = strrchr(d->receiver_type, '.');
            auto_t.short_name = dot ? lsm_arena_strdup(arena, dot + 1) : auto_t.qualified_name;
            lsm_registry_add_type(reg, auto_t);
            type_index = reg->type_count - 1;
            lsm_idxmemo_put_if_absent(&type_idx, arena, auto_t.qualified_name, type_index);
        }
    ...
    for (int i = 0; i < def_count; i++) {
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            LSMRegisteredFunc rf;
            memset(&rf, 0, sizeof(rf));
            rf.qualified_name = lsm_arena_strdup(arena, d->qualified_name);
            rf.short_name = lsm_arena_strdup(arena, d->short_name);
            rf.min_params = -1;
            if (d->is_abstract) {
                rf.flags |= LSM_FUNC_FLAG_RUST_ABSTRACT;
            }
            ...
            if (d->return_types && d->return_types[0]) {
                ...
                ret_types =
                    (const LSMType **)lsm_arena_alloc(arena, (count + 1) * sizeof(const LSMType *));
                int idx = 0;
                char *buf = lsm_arena_strdup(arena, d->return_types);
                char *start = buf;
                for (char *p = buf;; p++) {
                    if (*p == '|' || *p == '\0') {
                        char save = *p;
            ...
            if (strcmp(d->label, "Method") == 0 && d->receiver_type && d->receiver_type[0]) {
                rf.receiver_type = lsm_arena_strdup(arena, d->receiver_type);
                if (lsm_idxmemo_get(&type_idx, rf.receiver_type) < 0) {
                    LSMRegisteredType auto_t;
                    memset(&auto_t, 0, sizeof(auto_t));
                    auto_t.qualified_name = rf.receiver_type;
             ...
             * backwards-compatible direct LSMRustLSPDef callers. Production
             * extraction additionally emits an independent relation record. */
            if (rf.receiver_type && rf.impl_trait_qn) {
                LSMRegisteredType *rt = NULL;
                int32_t tix = lsm_idxmemo_get(&type_idx, rf.receiver_type);
                if (tix >= 0) {
                    rt = &reg->types[tix];
                }
                if (rt) {
...
static void rust_resolve_against_registry(LSMArena *arena, const char *source, int source_len,
                                          const char *module_qn, const LSMTypeRegistry *reg,
 ...
 * Converts the pipeline's LSMLSPDef into LSMRustLSPDef inline (same field copy as
...
LSMTypeRegistry *lsm_rust_build_cross_registry(LSMArena *arena, LSMLSPDef *defs, int def_count) {
    if (!arena)
        return NULL;
    LSMTypeRegistry *reg = (LSMTypeRegistry *)lsm_arena_alloc(arena, sizeof(*reg));
    if (!reg)
        return NULL;
    LSMRustLSPDef *rdefs = NULL;
    if (def_count > 0) {
        rdefs = (LSMRustLSPDef *)lsm_arena_alloc(arena, (size_t)def_count * sizeof(LSMRustLSPDef));
        if (!rdefs)
            return NULL;
        for (int i = 0; i < def_count; i++) {
 ...
 * per-file registry build; just parse + resolve. Mirrors lsm_run_c_lsp_cross_with_registry. */
void lsm_run_rust_lsp_cross_with_registry(LSMArena *arena, const char *source, int source_len,
                                          const char *module_qn, const LSMTypeRegistry *reg,
                                          const char **import_names, const char **import_qns,
...
void lsm_run_rust_lsp_cross_with_manifest(LSMArena *arena, const char *source, int source_len,
                                          const char *module_qn, LSMRustLSPDef *defs, int def_count,
                                          const char **import_names, const char **import_qns,
                                          int import_count, TSTree *cached_tree,
                                          const struct LSMCargoManifest *manifest,
                                          LSMResolvedCallArray *out,
...
void lsm_run_rust_lsp_cross(LSMArena *arena, const char *source, int source_len,
                            const char *module_qn, LSMRustLSPDef *defs, int def_count,
                            const char **import_names, const char **import_qns, int import_count,
                            TSTree *cached_tree, LSMResolvedCallArray *out) {
    lsm_run_rust_lsp_cross_with_manifest(arena, source, source_len, module_qn, defs, def_count,
                                         import_names, import_qns, import_count, cached_tree, NULL,
                                         out, NULL);
}

void lsm_batch_rust_lsp_cross(LSMArena *arena, LSMBatchRustLSPFile *files, int file_count,
                              LSMResolvedCallArray *out) {
    if (!files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        LSMBatchRustLSPFile *file = &files[f];
        ...
        lsm_run_rust_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
