#ifndef LSM_LSP_PY_LSP_H
#define LSM_LSP_PY_LSP_H
...
typedef struct {
    ...
    const LSMType *result; // full-fidelity result, never NULL in a live entry
} LSMPyTypeCacheEntry;
...
typedef struct {
    ...
    // Built from LSMFileResult.imports.
    ...
    // non-NULL, py_emit_dunder_call injects a matching synthetic LSMCall so the
    ...
    LSMPyTypeCacheEntry *type_cache; // open addressing, linear probe, arena-allocated
    ...
    struct {
        ...
        const LSMType *field_type;
    } *field_overlay;
    int field_overlay_count;
    int field_overlay_cap;

    // Debug mode (LSM_LSP_DEBUG env, shared across all language LSPs).
    bool debug;
} PyLSPContext;
...
void py_lsp_init(PyLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
                 const LSMTypeRegistry *registry, const char *module_qn, LSMResolvedCallArray *out);

// Add an import mapping (call once per LSMImport entry).
...
// Mirrors lsm_run_go_lsp / lsm_run_c_lsp.
