...
static void build_qn_index(LSMTypeRegistry *reg, LSMArena *idx_arena, bool for_funcs) {
    ...
    int *buckets = (int *)lsm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    LSMRegistryHashEntry *entries = (LSMRegistryHashEntry *)lsm_arena_alloc(
        idx_arena, (size_t)count * sizeof(LSMRegistryHashEntry));
    if (!buckets || !entries)
        return;
...
static void build_method_index(LSMTypeRegistry *reg, LSMArena *idx_arena) {
    if (reg->func_count == 0)
        return;
    ...
    int *buckets = (int *)lsm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    LSMRegistryHashEntry *entries = (LSMRegistryHashEntry *)lsm_arena_alloc(
        reg->arena, (size_t)reg->func_count * sizeof(LSMRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = 0; i < reg->func_count; i++) {
        const LSMRegisteredFunc *f = &reg->funcs[i];
...
static void build_type_embed_index(LSMTypeRegistry *reg, LSMArena *idx_arena) {
    int ecount = 0;
    ...
    int *buckets = (int *)lsm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    LSMRegistryHashEntry *entries = (LSMRegistryHashEntry *)lsm_arena_alloc(
        idx_arena, (size_t)ecount * sizeof(LSMRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
static void build_type_short_index(LSMTypeRegistry *reg, LSMArena *idx_arena) {
    int tcount = 0;
    for (int i = 0; i < reg->type_count; i++) {
    ...
    int *buckets = (int *)lsm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    LSMRegistryHashEntry *entries = (LSMRegistryHashEntry *)lsm_arena_alloc(
        idx_arena, (size_t)tcount * sizeof(LSMRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = reg->type_count - 1; i >= 0; i--) {
        const LSMRegisteredType *t = &reg->types[i];
        if (!t->short_name)
            continue;
...
static void build_ffunc_short_index(LSMTypeRegistry *reg, LSMArena *idx_arena) {
    int fcount = 0;
    for (int i = 0; i < reg->func_count; i++) {
        const LSMRegisteredFunc *f = &reg->funcs[i];
        if (!f->receiver_type && f->short_name)
    ...
    int *buckets = (int *)lsm_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    LSMRegistryHashEntry *entries = (LSMRegistryHashEntry *)lsm_arena_alloc(
        idx_arena, (size_t)fcount * sizeof(LSMRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
int lsm_type_embed_iter_next(LSMTypeEmbedIter *it) {
    const LSMTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const LSMRegistryHashEntry *e = &reg->type_embed_entries[it->chain_idx];
...
int lsm_free_func_iter_next(LSMFreeFuncIter *it) {
    const LSMTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const LSMRegistryHashEntry *e = &reg->ffunc_short_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
int lsm_type_short_iter_next(LSMTypeShortIter *it) {
    const LSMTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const LSMRegistryHashEntry *e = &reg->type_short_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
int lsm_method_iter_next(LSMMethodIter *it) {
    ...
    const LSMTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const LSMRegistryHashEntry *e = &reg->method_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash) {
        ...
        const LSMRegisteredFunc *f = &reg->funcs[p];
...
void lsm_registry_finalize(LSMTypeRegistry *reg) {
    if (!reg)
        return;
    lsm_registry_finalize_into(reg, reg->arena);
}

void lsm_registry_init(LSMTypeRegistry *reg, LSMArena *arena) {
    memset(reg, 0, sizeof(LSMTypeRegistry));
    reg->arena = arena;
}
...
void lsm_registry_add_func(LSMTypeRegistry *reg, LSMRegisteredFunc func) {
    ...
    if (reg->func_count >= reg->func_cap) {
        ...
        LSMRegisteredFunc *new_items = (LSMRegisteredFunc *)lsm_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(LSMRegisteredFunc));
        if (!new_items)
            return;
        if (reg->funcs && reg->func_count > 0) {
            memcpy(new_items, reg->funcs, (size_t)reg->func_count * sizeof(LSMRegisteredFunc));
        }
        reg->funcs = new_items;
...
void lsm_registry_add_type(LSMTypeRegistry *reg, LSMRegisteredType type) {
    ...
    if (reg->type_count >= reg->type_cap) {
        ...
        LSMRegisteredType *new_items = (LSMRegisteredType *)lsm_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(LSMRegisteredType));
        if (!new_items)
            return;
        if (reg->types && reg->type_count > 0) {
            memcpy(new_items, reg->types, (size_t)reg->type_count * sizeof(LSMRegisteredType));
        }
        reg->types = new_items;
...
static const LSMRegisteredFunc *lookup_method_self(const LSMTypeRegistry *reg,
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            if (reg->method_entries[idx].hash != h)
                continue;
            const LSMRegisteredFunc *f = &reg->funcs[reg->method_entries[idx].payload_index];
...
const LSMRegisteredFunc *lsm_registry_lookup_method(const LSMTypeRegistry *reg,
    ...
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    const LSMRegisteredFunc *r = lookup_method_self(reg, receiver_qn, method_name);
    if (!r && reg->fallback) {
        return lsm_registry_lookup_method(reg->fallback, receiver_qn, method_name);
    }
    return r;
}

static const LSMRegisteredType *lookup_type_self(const LSMTypeRegistry *reg,
...
const LSMRegisteredType *lsm_registry_lookup_type(const LSMTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const LSMRegisteredType *r = lookup_type_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return lsm_registry_lookup_type(reg->fallback, qualified_name);
    }
    return r;
}

static const LSMRegisteredFunc *lookup_func_self(const LSMTypeRegistry *reg,
                                                 const char *qualified_name) {
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        uint64_t h = fnv1a(qualified_name);
...
const LSMRegisteredFunc *lsm_registry_lookup_func(const LSMTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const LSMRegisteredFunc *r = lookup_func_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return lsm_registry_lookup_func(reg->fallback, qualified_name);
    }
    return r;
}
...
const LSMRegisteredType *lsm_registry_resolve_alias(const LSMTypeRegistry *reg,
    ...
    if (!reg || !type_qn)
        return NULL;
    const LSMRegisteredType *rt = lsm_registry_lookup_type(reg, type_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        const LSMRegisteredType *next = lsm_registry_lookup_type(reg, rt->alias_of);
        if (!next)
            return rt;
        rt = next;
    }
...
const LSMRegisteredFunc *lsm_registry_lookup_method_aliased(const LSMTypeRegistry *reg,
    ...
    // Direct lookup first
    const LSMRegisteredFunc *f = lsm_registry_lookup_method(reg, receiver_qn, method_name);
    if (f)
        return f;

    // Follow alias chain
    const LSMRegisteredType *rt = lsm_registry_lookup_type(reg, receiver_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        f = lsm_registry_lookup_method(reg, rt->alias_of, method_name);
        if (f)
            return f;
        rt = lsm_registry_lookup_type(reg, rt->alias_of);
    }
    return NULL;
}
...
const LSMRegisteredFunc *lsm_registry_lookup_symbol(const LSMTypeRegistry *reg,
    ...
    return lsm_registry_lookup_func(reg, buf);
}

// Count parameters in a FUNC signature.
static int count_func_params(const LSMRegisteredFunc *f) {
...
const LSMRegisteredFunc *lsm_registry_lookup_method_by_args(const LSMTypeRegistry *reg,
    ...
    const LSMRegisteredFunc *first_match = NULL;
    const LSMRegisteredFunc *range_match = NULL;
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            const LSMRegisteredFunc *f = &reg->funcs[pi];
            if (!f->receiver_type || !f->short_name || strcmp(f->receiver_type, receiver_qn) != 0 ||
                strcmp(f->short_name, method_name) != 0)
                continue;
        ...
        if (reg->fallback) {
            return lsm_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                      arg_count);
        }
        return NULL;
    }
    ...
    const LSMRegisteredFunc *res = range_match ? range_match : first_match;
    if (!res && reg->fallback) {
        return lsm_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                  arg_count);
    }
...
static const char *type_to_qn_simple(const LSMType *t) {
    if (!t)
        return NULL;
...
static int score_overload_match(const LSMRegisteredFunc *f, const LSMType **arg_types,
                                int arg_count) {
    int pc = count_func_params(f);
    int min_pc = (f->min_params >= 0) ? f->min_params : pc;
    if (arg_count < min_pc || arg_count > pc)
    ...
    for (int i = 0; i < arg_count; i++) {
        const LSMType *expected = f->signature->data.func.param_types[i];
        const LSMType *actual = arg_types[i];
        if (!expected || !actual || lsm_type_is_unknown(actual))
            continue; // neutral
...
const LSMRegisteredFunc *lsm_registry_lookup_method_by_types(const LSMTypeRegistry *reg,
    ...
    if (!arg_types)
        return lsm_registry_lookup_method_by_args(reg, receiver_qn, method_name, arg_count);
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return lsm_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                       arg_types, arg_count);
        }
        return NULL;
    }
    ...
    const LSMRegisteredFunc *res = best ? best : first_match;
    if (!res && reg->fallback) {
        return lsm_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                   arg_types, arg_count);
    }
...
const LSMRegisteredFunc *lsm_registry_lookup_symbol_by_types(const LSMTypeRegistry *reg,
    ...
    if (!reg || !package_qn || !name)
        return NULL;
    if (!arg_types)
        return lsm_registry_lookup_symbol_by_args(reg, package_qn, name, arg_count);
    ...
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return lsm_registry_lookup_symbol_by_types(reg->fallback, package_qn, name, arg_types,
                                                       arg_count);
        }
        return NULL;
    }
...
const LSMRegisteredFunc *lsm_registry_lookup_symbol_by_args(const LSMTypeRegistry *reg,
    ...
    // lsm_registry_lookup_symbol_by_types). O(overloads) not O(func_count).
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        int exact_pi = -1, range_pi = -1, first_pi = -1;
        uint64_t h = fnv1a(buf);
        int slot = (int)(h & (uint64_t)(reg->func_qn_bucket_count - 1));
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return lsm_registry_lookup_symbol_by_args(reg->fallback, package_qn, name, arg_count);
        }
        return NULL;
    }
...
const LSMRegisteredFunc *lsm_registry_lookup_callable(const LSMTypeRegistry *reg, LSMArena *arena,
    ...
    // Synthesise a LSMRegisteredFunc whose QN is "<type_qn>.__call".
    ...
    char *qn = (char *)lsm_arena_alloc(arena, total);
    if (!qn)
        return NULL;
    memcpy(qn, type_qn, qn_len);
    memcpy(qn + qn_len, suffix, sizeof(suffix));

    LSMRegisteredFunc *f = (LSMRegisteredFunc *)lsm_arena_alloc(arena, sizeof(LSMRegisteredFunc));
    if (!f)
        return NULL;
    memset(f, 0, sizeof(*f));
    f->qualified_name = qn;
...
const LSMType *lsm_registry_lookup_index_signature(const LSMTypeRegistry *reg, const char *type_qn,
    ...
    if (rt->index_key_type->kind == LSM_TYPE_BUILTIN && key_type->kind == LSM_TYPE_BUILTIN) {
