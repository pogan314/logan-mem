 ...
 *          method/function call expressions into LSMResolvedCall edges.
 *
 ...
 * at internal/lsm/vendored/grammars/perl/parser.c (ts_symbol_names and
 ...
 *
 * QN scheme (verified against helpers.c lsm_enclosing_func_qn): Perl has no
 ...
 * per-package LSMRegisteredType (keyed by the package name) carries
...
#define LSM_LSP_PERL_MAX_WALK_DEPTH 512
...
static const LSMType *perl_eval_function_call_type(PerlLSPContext *ctx, TSNode node);
static const LSMType *perl_eval_method_call_type(PerlLSPContext *ctx, TSNode node);
static const LSMType *perl_eval_new_type(PerlLSPContext *ctx, TSNode node);
...
static char *perl_pkg_to_dot(LSMArena *a, const char *pkg) {
    if (!pkg)
        return NULL;
    size_t n = strlen(pkg);
...
static char *perl_unquote(LSMArena *a, const char *s) {
    if (!s || !s[0])
        return NULL;
    size_t n = strlen(s);
    if ((s[0] == '\'' || s[0] == '"') && n >= 2 && s[n - 1] == s[0]) {
        return lsm_arena_strndup(a, s + 1, n - 2);
    }
    return NULL;
}
...
void perl_lsp_add_use(PerlLSPContext *ctx, const char *local_name, const char *target_qn) {
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **ln =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        const char **tq =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)newcap * sizeof(char *));
        if (!ln || !tq)
            return;
...
static void perl_add_isa(PerlLSPContext *ctx, const char *pkg, const char *parent) {
    ...
    ctx->isa_pkg_qns[ctx->isa_count] = lsm_arena_strdup(ctx->arena, pkg);
    ctx->isa_parent_qns[ctx->isa_count] = lsm_arena_strdup(ctx->arena, parent);
    ctx->isa_count++;
}
 ...
 * LSMRegisteredFunc or NULL. Bounded by LSM_LSP_MAX_LOOKUP_DEPTH * 2 visited.
 *
...
const LSMRegisteredFunc *perl_lookup_method(PerlLSPContext *ctx, const char *package_qn,
    ...
    enum { CAP = LSM_LSP_MAX_LOOKUP_DEPTH * 2 };
    ...
    while (frontier_count > 0 && visited_count < CAP) {
        ...
        const LSMRegisteredType *t = lsm_registry_lookup_type(ctx->registry, pkg);
        if (!t) {
            /* Even without a type entry, a stdlib receiver-keyed method may
             * exist (e.g. a curated CPAN class). */
            const LSMRegisteredFunc *direct =
                lsm_registry_lookup_method(ctx->registry, pkg, method_name);
            if (direct)
                return direct;
            continue;
        }
            ...
            for (int i = 0; t->method_names[i]; i++) {
                if (strcmp(t->method_names[i], method_name) == 0) {
                    const LSMRegisteredFunc *f =
                        lsm_registry_lookup_func(ctx->registry, t->method_qns[i]);
                    if (f)
                        return f;
                }
            }
...
static const LSMType *perl_eval_bless(PerlLSPContext *ctx, TSNode call_node) {
    ...
    if (!have_class) {
        ...
        if (pkg && pkg[0])
            return lsm_type_named(ctx->arena, pkg);
    ...
    if (perl_is_string_node(ack)) {
        ...
        if (inner && inner[0])
            return lsm_type_named(ctx->arena, perl_resolve_package_name(ctx, inner));
        ...
        if (bw && strcmp(bw, "__PACKAGE__") == 0) {
            ...
            return lsm_type_named(ctx->arena, perl_resolve_package_name(ctx, bw));
        }
    } else {
...
const LSMType *perl_eval_expr_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(k, "scalar") == 0 || strcmp(k, "scalar_variable") == 0) {
        ...
        if (txt) {
            ...
            const LSMType *t = lsm_scope_lookup(ctx->current_scope, bare);
            if (t)
                result = t;
        }
        ...
        result = perl_eval_method_call_type(ctx, node);
               ...
               strcmp(k, "ambiguous_function_call_expression") == 0) {
        const LSMType *blessed = perl_eval_bless(ctx, node);
        if (blessed && !lsm_type_is_unknown(blessed))
            result = blessed;
        else
...
static const LSMType *perl_eval_new_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (perl_is_bareword_node(ik)) {
        ...
        if (cls && cls[0])
            return lsm_type_named(ctx->arena, perl_resolve_package_name(ctx, cls));
...
static const LSMType *perl_eval_function_call_type(PerlLSPContext *ctx, TSNode node) {
    ...
    if (colons) {
        ...
        char *pkg = lsm_arena_strndup(ctx->arena, name, plen);
        const char *shortn = colons + 2;
        f = perl_lookup_method(ctx, pkg, shortn);
        if (!f)
            f = lsm_registry_lookup_symbol(ctx->registry, pkg, shortn);
    } else {
        /* Bare func() — Exporter import map, then file-local/global func. */
        const char *imp = perl_find_import(ctx, name);
        if (imp)
            f = lsm_registry_lookup_func(ctx->registry, imp);
        if (!f)
            f = lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
    }
...
static const LSMType *perl_eval_method_call_type(PerlLSPContext *ctx, TSNode node) {
    ...
    const LSMType *ctor = perl_eval_new_type(ctx, node);
    if (ctor)
        return ctor;
    ...
    if (!ts_node_is_null(inv)) {
        ...
        if (perl_is_bareword_node(ik)) {
            ...
            const LSMType *recv = perl_eval_expr_type(ctx, inv);
    ...
    const LSMRegisteredFunc *f = perl_lookup_method(ctx, class_qn, mname);
    if (f && f->signature && f->signature->kind == LSM_TYPE_FUNC &&
        f->signature->data.func.return_types && f->signature->data.func.return_types[0]) {
        return f->signature->data.func.return_types[0];
    }
...
static char *perl_exact_coderef_name(PerlLSPContext *ctx, TSNode refgen) {
    ...
    if (*p != '\0' || end == start)
        return NULL;
    return lsm_arena_strndup(ctx->arena, start, (size_t)(end - start));
}

static const LSMRegisteredFunc *perl_resolve_coderef_target(PerlLSPContext *ctx, char *name) {
    if (!ctx || !name || !name[0])
        return NULL;
    char *colons = NULL;
    for (char *p = strstr(name, "::"); p; p = strstr(p + 2, "::"))
        colons = p;
    if (colons) {
        char *pkg = lsm_arena_strndup(ctx->arena, name, (size_t)(colons - name));
        const char *short_name = colons + 2;
        const LSMRegisteredFunc *f = perl_lookup_method(ctx, pkg, short_name);
        return f ? f : lsm_registry_lookup_symbol(ctx->registry, pkg, short_name);
    }
    const char *import_qn = perl_find_import(ctx, name);
    if (import_qn) {
        const LSMRegisteredFunc *f = lsm_registry_lookup_func(ctx->registry, import_qn);
        if (f)
            return f;
    }
    return lsm_registry_lookup_symbol(ctx->registry, ctx->module_qn, name);
}

static void perl_resolve_one_coderef(PerlLSPContext *ctx, TSNode refgen) {
    char *name = perl_exact_coderef_name(ctx, refgen);
    const LSMRegisteredFunc *target = name ? perl_resolve_coderef_target(ctx, name) : NULL;
...
static void perl_emit_non_direct_coderef_usage(PerlLSPContext *ctx, TSNode refgen) {
    ...
    for (int i = 0; i < ctx->usages->count; i++) {
        const LSMUsage *existing = &ctx->usages->items[i];
        if (existing->kind == LSM_USAGE_VALUE && existing->ref_name &&
    ...
    lsm_usages_push(ctx->usages, ctx->arena, usage);
}
...
static void perl_resolve_method_call(PerlLSPContext *ctx, TSNode call) {
    ...
    if (strncmp(mname, "SUPER::", 7) == 0) {
        ...
        if (!parent_qn || !parent_qn[0])
            return;
        const LSMRegisteredFunc *sf = perl_lookup_method(ctx, parent_qn, super_method);
        if (sf)
...
static void perl_process_assignment(PerlLSPContext *ctx, TSNode assign) {
    ...
    const LSMType *rt = perl_eval_expr_type(ctx, right);
    if (rt && rt->kind == LSM_TYPE_NAMED)
        lsm_scope_bind(ctx->current_scope, bare, rt);
}
 ...
 * for the Java/C++ walkers). Past LSM_LSP_PERL_MAX_WALK_DEPTH the subtree is
 * skipped — graceful degradation, never a wrong edge. */
static void perl_resolve_calls_in_node(PerlLSPContext *ctx, TSNode node) {
    if (ctx->walk_depth >= LSM_LSP_PERL_MAX_WALK_DEPTH)
        return;
    ctx->walk_depth++;
...
static void perl_infer_self_type(PerlLSPContext *ctx, TSNode body) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const char *bare = perl_strip_sigil(vtxt);
        if (bare && bare[0])
            lsm_scope_bind(ctx->current_scope, bare, lsm_type_named(ctx->arena, pkg));
        free(kids);
...
static void process_subroutine(PerlLSPContext *ctx, TSNode node) {
    ...
    if (sname && sname[0]) {
        if (ctx->module_qn)
            ctx->enclosing_func_qn = lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname);
        else
            ctx->enclosing_func_qn = lsm_arena_strdup(ctx->arena, sname);
    }
...
static void process_package_decl(PerlLSPContext *ctx, TSNode node) {
    ...
    if (!pkg || !pkg[0])
        return;
    ctx->current_package_qn = lsm_arena_strdup(ctx->arena, pkg);
...
static void perl_collect_qw_imports(PerlLSPContext *ctx, TSNode container,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *target = lsm_arena_sprintf(ctx->arena, "%s.%s", module_dot, fn);
...
void perl_lsp_process_file(PerlLSPContext *ctx, TSNode root) {
     ...
     * (lsm_run_perl_lsp) has already run a pre-pass to build registry types. */
    ctx->current_package_qn = "";
...
/* Register a per-package LSMRegisteredType for every package that participates
 * in @ISA (as child or parent), then attach @ISA parents (embedded_types). */
static void perl_register_packages(PerlLSPContext *ctx, LSMTypeRegistry *reg) {
    for (int i = 0; i < ctx->isa_count; i++) {
        const char *names[2] = {ctx->isa_pkg_qns[i], ctx->isa_parent_qns[i]};
        for (int s = 0; s < 2; s++) {
            const char *pkg = names[s];
            if (!pkg || !pkg[0] || lsm_registry_lookup_type(reg, pkg))
                continue;
            LSMRegisteredType rt;
            memset(&rt, 0, sizeof(rt));
            rt.qualified_name = lsm_arena_strdup(ctx->arena, pkg);
    ...
    for (int t = 0; t < reg->type_count; t++) {
        LSMRegisteredType *rt = &reg->types[t];
        ...
        if (pc == 0)
            continue;
        const char **parents =
            (const char **)lsm_arena_alloc(ctx->arena, (size_t)(pc + 1) * sizeof(char *));
        if (!parents)
            continue;
        int w = 0;
        for (int i = 0; i < ctx->isa_count; i++) {
...
static void perl_type_set_methods(PerlLSPContext *ctx, LSMTypeRegistry *reg, const char *pkg,
    ...
    if (!rt) {
        LSMRegisteredType nt;
        memset(&nt, 0, sizeof(nt));
        nt.qualified_name = lsm_arena_strdup(ctx->arena, pkg);
        nt.short_name = nt.qualified_name;
        lsm_registry_add_type(reg, nt);
        if (reg->type_count == 0)
    ...
    const char **mn =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    const char **mq =
        (const char **)lsm_arena_alloc(ctx->arena, (size_t)(total + 1) * sizeof(char *));
    if (!mn || !mq)
        return;
...
static void perl_attach_methods(PerlLSPContext *ctx, LSMTypeRegistry *reg, TSNode root) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "package_statement") == 0) {
            ...
            if (!ts_node_is_null(name)) {
                ...
                if (p && p[0])
                    cur_pkg = lsm_arena_strdup(ctx->arena, p);
            }
            ...
            for (uint32_t bi = 0; bi < bn; bi++) {
                ...
                const char *bqn = ctx->module_qn
                                      ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, bsn)
                                      : lsm_arena_strdup(ctx->arena, bsn);
        ...
        const char *sub_qn = ctx->module_qn
                                 ? lsm_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, sname)
                                 : lsm_arena_strdup(ctx->arena, sname);
...
void lsm_run_perl_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
    ...
    lsm_perl_stdlib_register(&reg, arena);
    ...
    if (ctx.debug) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            LSMResolvedCall *r = &result->resolved_calls.items[i];
