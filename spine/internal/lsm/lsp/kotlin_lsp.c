 ...
 * tree-sitter Kotlin AST, populates a per-file LSMTypeRegistry, builds
 * lexical scopes, infers expression types, and emits LSMResolvedCall
...
#include "../helpers.h"
/* LSMHashTable — cross-registry field bucketing. Relative path: the LSP
 * amalgamation compiles with -Iinternal/lsm only, not -Isrc. */
#include "../../../src/foundation/hash_table.h"

/* Growable field list for one receiver type, bucketed per cross-registry call
 * (see lsm_kotlin_register_lsp_defs). qns carries each field's REAL def QN --
...
static void kt_register_class_members(KotlinLSPContext *ctx, const char *class_qn, TSNode body,
                                      bool is_object);
static const LSMType *kt_eval_call_expression_type(KotlinLSPContext *ctx, TSNode node);
static const LSMType *kt_eval_navigation_expression_type(KotlinLSPContext *ctx, TSNode node);
static const LSMType *kt_eval_navigation_expression_type_at(KotlinLSPContext *ctx, TSNode node,
                                                            TSNode site);
static const LSMType *kt_eval_user_type(KotlinLSPContext *ctx, TSNode node);
...
static const char *kt_type_qn_of(const LSMType *t);
static char *kt_join_dot(LSMArena *a, const char *prefix, const char *name);
...
static const char *kt_resolve_in_default_imports(KotlinLSPContext *ctx, const char *name,
                                                 LSMKotlinUseKind kind);
static const LSMType *kt_try_smart_cast(KotlinLSPContext *ctx, TSNode call_or_nav);
...
static char *kt_join_dot(LSMArena *a, const char *prefix, const char *name) {
    ...
    if (!prefix || !*prefix) {
        return lsm_arena_strdup(a, name);
    }
    size_t pl = strlen(prefix);
    size_t nl = strlen(name);
    char *out = (char *)lsm_arena_alloc(a, pl + 1 + nl + 1);
    if (!out) {
        return NULL;
    }
...
static const LSMType *kt_unwrap_nullable(const LSMType *t) {
...
static void kt_emit_resolved_kind(KotlinLSPContext *ctx, const char *callee_qn,
                                  const char *strategy, float confidence, LSMResolvedKind kind) {
    if (ctx->debug) {
    ...
    LSMResolvedCallArray *arr = ctx->resolved_calls;
    if (arr->count >= arr->cap) {
        int new_cap = arr->cap == 0 ? 16 : arr->cap * 2;
        LSMResolvedCall *new_items = (LSMResolvedCall *)lsm_arena_alloc(
            ctx->arena, (size_t)new_cap * sizeof(LSMResolvedCall));
        if (!new_items) {
            return;
        }
        if (arr->items && arr->count > 0) {
            memcpy(new_items, arr->items, (size_t)arr->count * sizeof(LSMResolvedCall));
        }
        arr->items = new_items;
        arr->cap = new_cap;
    }
    LSMResolvedCall *rc = &arr->items[arr->count];
    memset(rc, 0, sizeof(LSMResolvedCall));
...
static void kt_emit_resolved_reference_at(KotlinLSPContext *ctx, const char *callee_qn,
    ...
    int first = ctx->resolved_calls ? ctx->resolved_calls->count : -1;
    kt_emit_resolved_kind(ctx, callee_qn, strategy, confidence, LSM_RESOLVED_CALL_REFERENCE);
    kt_stamp_resolved_site(ctx, first, site);
    if (!ctx->resolved_calls || !reason || first < 0)
        return;
    for (int i = first; i < ctx->resolved_calls->count; i++) {
        ctx->resolved_calls->items[i].reason = lsm_arena_strdup(ctx->arena, reason);
    }
}
...
static void kt_emit_resolved_invocation_reason_at(KotlinLSPContext *ctx, const char *callee_qn,
                                                  ...
                                                  float confidence, TSNode site) {
    int first = ctx->resolved_calls ? ctx->resolved_calls->count : -1;
    kt_emit_resolved_kind(ctx, callee_qn, strategy, confidence, LSM_RESOLVED_INVOCATION);
    kt_stamp_resolved_site(ctx, first, site);
    if (!ctx->resolved_calls || !reason || first < 0)
        return;
    for (int i = first; i < ctx->resolved_calls->count; i++) {
...
static void kt_emit_resolved_at(KotlinLSPContext *ctx, const char *callee_qn, const char *strategy,
                                float confidence, TSNode site) {
    kt_emit_resolved_kind_at(ctx, callee_qn, strategy, confidence, LSM_RESOLVED_INVOCATION, site);
}

static const LSMScope *kt_scope_declaring_name(const LSMScope *scope, const char *name) {
    if (!name)
        return NULL;
    for (const LSMScope *current = scope; current; current = current->parent) {
        for (const LSMScopeChunk *chunk = current->chunks; chunk; chunk = chunk->next) {
...
static const LSMKotlinDelegateBinding *kt_find_delegate_binding(const KotlinLSPContext *ctx,
                                                                const LSMScope *scope,
                                                                const char *owner_qn,
                                                                const char *name) {
    if (!ctx || !name)
        return NULL;
    for (int i = ctx->delegate_count - 1; i >= 0; i--) {
        const LSMKotlinDelegateBinding *binding = &ctx->delegate_bindings[i];
...
static void kt_record_delegate_binding(KotlinLSPContext *ctx, const LSMScope *scope,
    ...
    for (int i = 0; i < ctx->delegate_count; i++) {
        LSMKotlinDelegateBinding *existing = &ctx->delegate_bindings[i];
    ...
    if (ctx->delegate_count >= ctx->delegate_cap) {
        ...
        LSMKotlinDelegateBinding *grown = (LSMKotlinDelegateBinding *)lsm_arena_alloc(
    ...
    LSMKotlinDelegateBinding *binding = &ctx->delegate_bindings[ctx->delegate_count++];
    memset(binding, 0, sizeof(*binding));
    binding->scope = scope;
    binding->owner_qn = owner_qn ? lsm_arena_strdup(ctx->arena, owner_qn) : NULL;
    binding->name = lsm_arena_strdup(ctx->arena, name);
    binding->getter_qn = getter_qn ? lsm_arena_strdup(ctx->arena, getter_qn) : NULL;
    binding->setter_qn = setter_qn ? lsm_arena_strdup(ctx->arena, setter_qn) : NULL;
}

static void kt_clear_lexical_delegate_binding(KotlinLSPContext *ctx, const LSMScope *scope,
                                              const char *name) {
    if (!ctx || !scope || !name)
        return;
    for (int i = ctx->delegate_count - 1; i >= 0; i--) {
        LSMKotlinDelegateBinding *binding = &ctx->delegate_bindings[i];
...
static void kt_inject_delegate_call(KotlinLSPContext *ctx, const char *callee_qn, TSNode site) {
    ...
    for (int i = 0; i < ctx->synthetic_calls->count; i++) {
        const LSMCall *existing = &ctx->synthetic_calls->items[i];
    ...
    lsm_calls_push(ctx->synthetic_calls, ctx->arena, call);
}
...
static void kt_retarget_callable_carrier(KotlinLSPContext *ctx, TSNode site,
    ...
    for (int i = 0; i < ctx->synthetic_calls->count; i++) {
        LSMCall *call = &ctx->synthetic_calls->items[i];
        ...
        if (!overlaps && call->start_line != line)
            continue;
        call->callee_name = lsm_arena_strdup(ctx->arena, target_name);
        call->requires_lsp_resolution = true;
    }
}
...
static void kt_emit_delegate_target(KotlinLSPContext *ctx, const char *callee_qn, TSNode site) {
    ...
    for (int i = 0; ctx->resolved_calls && i < ctx->resolved_calls->count; i++) {
        const LSMResolvedCall *existing = &ctx->resolved_calls->items[i];
        if (existing->kind == LSM_RESOLVED_INVOCATION && existing->caller_qn &&
    ...
    for (int depth = 0; depth < 4; depth++) {
        ...
        if (strcmp(kind, "assignment") == 0) {
            ...
            if (ctx && rhs_start > lhs_end && rhs_start <= (uint32_t)ctx->source_len) {
                ...
                if (lsm_memmem(between, length, "+=", 2) || lsm_memmem(between, length, "-=", 2) ||
                    lsm_memmem(between, length, "*=", 2) || lsm_memmem(between, length, "/=", 2) ||
                    lsm_memmem(between, length, "%=", 2)) {
...
static void kt_emit_delegate_access(KotlinLSPContext *ctx, const char *name, const char *owner_qn,
    ...
    if (!ctx || !name)
        return;
    const LSMKotlinDelegateBinding *binding = NULL;
    if (owner_qn) {
        binding = kt_find_delegate_binding(ctx, NULL, owner_qn, name);
    } else {
        const LSMScope *scope = kt_scope_declaring_name(ctx->current_scope, name);
        if (scope) {
...
static const LSMType **kt_signature_param_types_from_ast(KotlinLSPContext *ctx, TSNode func_node) {
    ...
    if (nc == 0)
        return NULL;
    const LSMType **types =
        (const LSMType **)lsm_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*types));
    if (!types)
        return NULL;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        types[count++] = ts_node_is_null(type_node) ? lsm_type_unknown()
...
static const LSMType *kt_build_func_sig_with_return(KotlinLSPContext *ctx,
                                                    const LSMType **param_types,
                                                    const LSMType *return_type) {
    const LSMType **returns = NULL;
    if (return_type && !lsm_type_is_unknown(return_type)) {
        returns = (const LSMType **)lsm_arena_alloc(ctx->arena, 2 * sizeof(const LSMType *));
        if (!returns)
            return NULL;
        returns[0] = return_type;
        returns[1] = NULL;
    }
    return lsm_type_func(ctx->arena, NULL, param_types, returns);
}

/* ── public API ───────────────────────────────────────────────────── */

void kotlin_lsp_init(KotlinLSPContext *ctx, LSMArena *arena, const char *source, int source_len,
                     const LSMTypeRegistry *registry, const char *package_qn, const char *module_qn,
                     const char *project_name, const char *rel_path, LSMResolvedCallArray *out) {
    memset(ctx, 0, sizeof(KotlinLSPContext));
    ctx->arena = arena;
    ctx->source = source;
    ctx->source_len = source_len;
    ctx->registry = registry;
    ctx->package_qn = package_qn ? lsm_arena_strdup(arena, package_qn) : "";
    ctx->module_qn = module_qn ? lsm_arena_strdup(arena, module_qn) : "";
    ctx->project_name = project_name ? lsm_arena_strdup(arena, project_name) : "";
    ctx->rel_path = rel_path ? lsm_arena_strdup(arena, rel_path) : "";
    ctx->resolved_calls = out;
    ctx->import_cap = KT_IMPORT_INITIAL_CAP;
    ctx->import_locals =
        (const char **)lsm_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_targets =
        (const char **)lsm_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_kinds = (LSMKotlinUseKind *)lsm_arena_alloc(arena, sizeof(LSMKotlinUseKind) *
                                                                       (size_t)ctx->import_cap);
    ctx->import_count = 0;
    ctx->debug = (getenv("LSM_LSP_DEBUG") != NULL);
    ...
    if (rel_path && *rel_path) {
        ...
        char *fn = (char *)lsm_arena_alloc(arena, name_len + 3);
        if (fn) {
...
void kotlin_lsp_add_import(KotlinLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_locals =
            (const char **)lsm_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        const char **new_targets =
            (const char **)lsm_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        LSMKotlinUseKind *new_kinds = (LSMKotlinUseKind *)lsm_arena_alloc(
            ctx->arena, sizeof(LSMKotlinUseKind) * (size_t)new_cap);
        ...
        memcpy(new_kinds, ctx->import_kinds, sizeof(LSMKotlinUseKind) * (size_t)ctx->import_count);
    ...
    ctx->import_locals[ctx->import_count] =
        local_name ? lsm_arena_strdup(ctx->arena, local_name) : NULL;
    ctx->import_targets[ctx->import_count] = lsm_arena_strdup(ctx->arena, target_qn);
...
const char *kotlin_resolve_class_name(KotlinLSPContext *ctx, const char *name) {
    ...
    if (ctx->registry && lsm_registry_lookup_type(ctx->registry, name)) {
        return lsm_arena_strdup(ctx->arena, name);
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == LSM_KT_USE_TYPE ||
             ctx->import_kinds[i] == LSM_KT_USE_UNKNOWN)) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] == LSM_KT_USE_WILDCARD && ctx->import_targets[i]) {
    ...
    int n = 0;
    const char *const *defs = lsm_kotlin_default_import_packages(&n);
    for (int i = 0; i < n; i++) {
    ...
    if (ctx->cross_type_short) {
        const char *qn =
            (const char *)lsm_ht_get((const LSMHashTable *)ctx->cross_type_short, name);
        if (qn && qn[0]) {
            return qn;
        }
    }
...
const char *kotlin_resolve_function_name(KotlinLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        const char *local = ctx->import_locals[i];
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == LSM_KT_USE_FUNCTION ||
             ctx->import_kinds[i] == LSM_KT_USE_UNKNOWN)) {
            const char *target = ctx->import_targets[i];
            const LSMRegisteredFunc *direct =
                ctx->registry && target ? lsm_registry_lookup_func(ctx->registry, target) : NULL;
            ...
            if (ctx->registry && target) {
                ...
                const LSMRegisteredFunc *from_module =
                    member ? lsm_registry_lookup_func(ctx->registry, member) : NULL;
    ...
    if (ctx->package_qn && *ctx->package_qn) {
        char *cand = kt_join_dot(ctx->arena, ctx->package_qn, name);
        if (ctx->registry && lsm_registry_lookup_func(ctx->registry, cand)) {
            return cand;
        }
    }

    /* Bare-name lookup for files without a package declaration. */
    if (ctx->registry && lsm_registry_lookup_func(ctx->registry, name)) {
        return lsm_arena_strdup(ctx->arena, name);
    }
    ...
    /* Default imports */
    const char *via_default = kt_resolve_in_default_imports(ctx, name, LSM_KT_USE_FUNCTION);
    if (via_default) {
        return via_default;
    }
    ...
    if (ctx->registry && ctx->registry->funcs) {
        ...
        if (matches == 1 && only_qn) {
            return lsm_arena_strdup(ctx->arena, only_qn);
        }
    }
...
static const char *kt_resolve_in_default_imports(KotlinLSPContext *ctx, const char *name,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (kind == LSM_KT_USE_FUNCTION || kind == LSM_KT_USE_UNKNOWN) {
            if (lsm_registry_lookup_func(ctx->registry, cand)) {
                return cand;
            }
        }
        if (kind == LSM_KT_USE_TYPE || kind == LSM_KT_USE_UNKNOWN) {
            if (lsm_registry_lookup_type(ctx->registry, cand)) {
                return cand;
            }
        }
    }
...
/* Synthesize a LSMRegisteredFunc on the fly for stdlib types where we
...
static const LSMRegisteredFunc *kt_synth_method(KotlinLSPContext *ctx, const char *class_qn,
                                                const char *method_name) {
    LSMRegisteredFunc *rf =
        (LSMRegisteredFunc *)lsm_arena_alloc(ctx->arena, sizeof(LSMRegisteredFunc));
    if (!rf) {
        return NULL;
    }
    memset(rf, 0, sizeof(LSMRegisteredFunc));
    rf->qualified_name = kt_join_dot(ctx->arena, class_qn, method_name);
    rf->receiver_type = lsm_arena_strdup(ctx->arena, class_qn);
    rf->short_name = lsm_arena_strdup(ctx->arena, method_name);
    rf->min_params = 0;
    return rf;
}
...
static const LSMType *kt_stdlib_method_return_type(KotlinLSPContext *ctx, const char *class_qn,
                                                   const char *method) {
    if (!class_qn || !method) {
        return lsm_type_unknown();
    }
    if (kt_method_returns_self(class_qn, method)) {
        return lsm_type_named(ctx->arena, class_qn);
    }
    if (strcmp(method, "toString") == 0 || strcmp(method, "toRegex") == 0 ||
        ...
        return lsm_type_named(ctx->arena, "kotlin.String");
    }
    if (strcmp(method, "size") == 0 || strcmp(method, "length") == 0 ||
        ...
        return lsm_type_named(ctx->arena, "kotlin.Int");
    }
    if (strcmp(method, "isEmpty") == 0 || strcmp(method, "isNotEmpty") == 0 ||
        ...
        return lsm_type_named(ctx->arena, "kotlin.Boolean");
    }
...
const LSMRegisteredFunc *kotlin_lookup_method(KotlinLSPContext *ctx, const char *class_qn,
                                              const char *method_name) {
    ...
    while (qhead < qtail) {
         ...
         * via lsm_registry_lookup_method_aliased). */
        const LSMRegisteredFunc *rf =
            lsm_registry_lookup_method_aliased(ctx->registry, cur_qn, method_name);
        if (rf) {
            return rf;
        }

        /* method_names fallback */
        const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, cur_qn);
...
static const char *kt_materialized_constructor_qn(KotlinLSPContext *ctx, const char *class_qn) {
    ...
    const LSMRegisteredFunc *named =
        lsm_registry_lookup_method(ctx->registry, class_qn, class_name);
    if (named && named->qualified_name) {
        return named->qualified_name;
    }
    const LSMRegisteredFunc *init = lsm_registry_lookup_method(ctx->registry, class_qn, "<init>");
    return init ? kt_join_dot(ctx->arena, class_qn, class_name) : NULL;
}

const LSMType *kotlin_lookup_property_type(KotlinLSPContext *ctx, const char *class_qn,
                                           const char *prop_name) {
    if (!ctx || !class_qn || !prop_name || !ctx->registry) {
        return lsm_type_unknown();
    }
    const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, class_qn);
    if (!rt) {
        return lsm_type_unknown();
    }
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const LSMType *t = kotlin_lookup_property_type(ctx, rt->embedded_types[i], prop_name);
...
static const char *kt_parse_package_header(KotlinLSPContext *ctx, TSNode header) {
    ...
    char *out = (char *)lsm_arena_alloc(ctx->arena, strlen(txt) + 1);
    if (!out) {
        return "";
    }
    int oi = 0;
...
static void kt_parse_import_directive(KotlinLSPContext *ctx, TSNode imp) {
    ...
    /* Compress whitespace */
    char *cleaned = (char *)lsm_arena_alloc(ctx->arena, strlen(qid_text) + 1);
    int oi = 0;
    ...
    if (full_text) {
        ...
        if (as_kw) {
            ...
            char *al = (char *)lsm_arena_alloc(ctx->arena, 128);
            if (al) {
                int ai = 0;
    ...
    if (wildcard) {
        kotlin_lsp_add_import(ctx, NULL, cleaned, LSM_KT_USE_WILDCARD);
        return;
    }

    /* Default: kind = UNKNOWN — we'll match it both ways at lookup time. */
    const char *local = alias ? alias : kt_short(cleaned);
    kotlin_lsp_add_import(ctx, local, cleaned, LSM_KT_USE_UNKNOWN);
}
...
static void kt_collect_top_level_decls(KotlinLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(kind, "class_declaration") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                const LSMType *underlying = kotlin_parse_type_node(ctx, rhs);
                if (underlying && underlying->kind == LSM_TYPE_NAMED &&
                    underlying->data.named.qualified_name) {
                    rt.alias_of = underlying->data.named.qualified_name;
                }
            }
            lsm_registry_add_type((LSMTypeRegistry *)ctx->registry, rt);
        }
    }
}
...
static void kt_process_class_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(pc)) {
        ...
        TSNode pcontainer = ts_node_is_null(params) ? pc : params;
        {
            ...
            for (uint32_t pi = 0; pi < pnc && field_count < 63; pi++) {
                ...
                const LSMType *ft = ts_node_is_null(type_node)
                                        ? lsm_type_unknown()
                                        : kotlin_parse_type_node(ctx, type_node);
    ...
    if (field_count > 0) {
        const char **fn = (const char **)lsm_arena_alloc(ctx->arena, sizeof(const char *) *
                                                                         (size_t)(field_count + 1));
        const LSMType **ft = (const LSMType **)lsm_arena_alloc(
            ctx->arena, sizeof(const LSMType *) * (size_t)(field_count + 1));
        if (fn && ft) {
...
static void kt_register_class_members(KotlinLSPContext *ctx, const char *class_qn, TSNode body,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(kind, "function_declaration") == 0) {
            ...
            rf.min_params = 0;
            const LSMType **param_types = kt_signature_param_types_from_ast(ctx, c);
            const LSMType *return_type = NULL;
             ...
             * site and propagate it as the lambda's `this`. */
            {
                ...
                if (!ts_node_is_null(params)) {
                    ...
                    for (uint32_t pi = 0; pi < pnc; pi++) {
                        ...
                        const char **dq =
                            (const char **)lsm_arena_alloc(ctx->arena, 2 * sizeof(const char *));
                        if (dq) {
                            char *tag = (char *)lsm_arena_alloc(
...
static void kt_process_function_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    const LSMType **param_types = kt_signature_param_types_from_ast(ctx, node);
    const LSMType *return_type = NULL;
    /* Capture return type for chain tracking. */
...
static void kt_process_property_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, pname), pt);
}

/* ── type parsing ─────────────────────────────────────────────────── */

const LSMType *kotlin_parse_type_node(KotlinLSPContext *ctx, TSNode node) {
    if (ts_node_is_null(node)) {
        return lsm_type_unknown();
    }
    const char *kind = ts_node_type(node);
    ...
    if (resolved) {
        return lsm_type_named(ctx->arena, resolved);
    }
    return lsm_type_named(ctx->arena, txt);
}
...
static const LSMType *kt_eval_user_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    /* Strip whitespace */
    char *clean = (char *)lsm_arena_alloc(ctx->arena, strlen(txt) + 1);
    int oi = 0;
    for (int i = 0; txt[i]; i++) {
        if (txt[i] != ' ' && txt[i] != '\t' && txt[i] != '\n') {
            clean[oi++] = txt[i];
    ...
    if (!resolved) {
        return lsm_type_named(ctx->arena, clean);
    }
    return lsm_type_named(ctx->arena, resolved);
}
...
static const LSMType *kt_eval_literal_type(KotlinLSPContext *ctx, TSNode node) {
    const char *kind = ts_node_type(node);
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "multiline_string_literal") == 0) {
        return lsm_type_named(ctx->arena, "kotlin.String");
    }
    if (strcmp(kind, "character_literal") == 0) {
        return lsm_type_named(ctx->arena, "kotlin.Char");
    }
    if (strcmp(kind, "number_literal") == 0) {
        ...
        if (tail == 'l') {
            return lsm_type_named(ctx->arena, "kotlin.Long");
        }
        if (tail == 'f') {
            return lsm_type_named(ctx->arena, "kotlin.Float");
        }
        if (strchr(txt, '.') || strchr(txt, 'e') || strchr(txt, 'E')) {
            return lsm_type_named(ctx->arena, "kotlin.Double");
        }
        return lsm_type_named(ctx->arena, "kotlin.Int");
    }
    ...
    if (strcmp(kind, "null_literal") == 0 ||
        ...
        return lsm_type_named(ctx->arena, "kotlin.Nothing");
    }
    return lsm_type_unknown();
}
...
static const char *kt_exact_callable_reference_target(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (count == 2) {
        ...
        if (lsm_scope_contains(ctx->current_scope, receiver_name)) {
            const LSMType *receiver_type = lsm_scope_lookup(ctx->current_scope, receiver_name);
            receiver_qn = kt_type_qn_of(receiver_type);
        } else {
            const char *candidate = kotlin_resolve_class_name(ctx, receiver_name);
            if (candidate && ctx->registry && lsm_registry_lookup_type(ctx->registry, candidate)) {
                receiver_qn = candidate;
            }
        }
        if (!receiver_qn)
            return NULL;
        const LSMRegisteredFunc *method = kotlin_lookup_method(ctx, receiver_qn, member);
        return method ? method->qualified_name : NULL;
    }

    if (lsm_scope_contains(ctx->current_scope, member)) {
        return lsm_scope_lookup_callable(ctx->current_scope, member);
    }

    const char *class_qn = kotlin_resolve_class_name(ctx, member);
    if (class_qn && ctx->registry && lsm_registry_lookup_type(ctx->registry, class_qn)) {
        return kt_materialized_constructor_qn(ctx, class_qn);
    }
    return kotlin_resolve_function_name(ctx, member);
}

const LSMType *kotlin_eval_expr_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0 || strcmp(kind, "simple_identifier") == 0) {
        ...
        if (strcmp(name, "true") == 0 || strcmp(name, "false") == 0) {
            result = lsm_type_named(ctx->arena, "kotlin.Boolean");
            goto out;
        }
        if (strcmp(name, "null") == 0) {
            result = lsm_type_named(ctx->arena, "kotlin.Nothing");
            goto out;
        }
        ...
        if (lsm_scope_contains(ctx->current_scope, name)) {
            const char *callable_qn = lsm_scope_lookup_callable(ctx->current_scope, name);
        ...
        if (cls_qn) {
            const LSMRegisteredType *rt = lsm_registry_lookup_type(ctx->registry, cls_qn);
            if (rt) {
                result = lsm_type_named(ctx->arena, cls_qn);
                goto out;
            }
        }
    ...
    if (strcmp(kind, "this_expression") == 0) {
        result = ctx->this_type ? ctx->this_type : lsm_type_unknown();
        goto out;
    }
    if (strcmp(kind, "super_expression") == 0) {
        result = ctx->super_type ? ctx->super_type : lsm_type_unknown();
        goto out;
    }
    if (strcmp(kind, "call_expression") == 0) {
    ...
    if (strcmp(kind, "binary_expression") == 0 || strcmp(kind, "additive_expression") == 0 ||
        ...
        const LSMType *lhs_t = kotlin_eval_expr_type(ctx, lhs);
        ...
        if (op_method && lhs_t && !lsm_type_is_unknown(lhs_t)) {
            const char *lhs_qn = kt_type_qn_of(lhs_t);
            if (lhs_qn) {
                const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, lhs_qn, op_method);
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                    if (rf->signature && rf->signature->kind == LSM_TYPE_FUNC &&
    ...
    if (strcmp(kind, "unary_expression") == 0 || strcmp(kind, "prefix_expression") == 0 ||
        ...
        if (nc > 0) {
            ...
            const LSMType *t = kotlin_eval_expr_type(ctx, operand);
            const char *op_method = kt_unary_operator_method(node, kind);
            if (op_method && t && !lsm_type_is_unknown(t)) {
                const char *qn = kt_type_qn_of(t);
                if (qn) {
                    const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, qn, op_method);
                    if (rf && rf->qualified_name) {
    ...
    if (strcmp(kind, "in_expression") == 0 || strcmp(kind, "check_expression") == 0) {
        ...
        if (is_membership && nc >= 2) {
            ...
            const LSMType *ct = kotlin_eval_expr_type(ctx, container);
            const char *cqn = kt_type_qn_of(ct);
            if (cqn) {
                const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, cqn, "contains");
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                }
    ...
    if (strcmp(kind, "index_expression") == 0 || strcmp(kind, "indexing_expression") == 0) {
        ...
        if (nc >= 1) {
            ...
            const LSMType *rt = kotlin_eval_expr_type(ctx, recv);
            const char *rqn = kt_type_qn_of(rt);
            if (rqn) {
                const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, rqn, "get");
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                    if (rf->signature && rf->signature->kind == LSM_TYPE_FUNC &&
                ...
                if (nc >= 2) {
                    ...
                    if (lhs_text) {
                        ...
                        if (candidate && ctx->registry &&
                            lsm_registry_lookup_type(ctx->registry, candidate)) {
                            recv_qn = candidate;
                        }
                    }
                    if (!recv_qn) {
                        ...
                        const LSMType *t =
                            lhs_text ? lsm_scope_lookup(ctx->current_scope, lhs_text) : NULL;
                    ...
                    if (recv_qn) {
                        if (strcmp(member, "class") == 0) {
                            ...
                            result = lsm_type_named(ctx->arena, "kotlin.reflect.KClass");
                            goto out;
                        }
                        const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, recv_qn, member);
                        if (rf && rf->qualified_name) {
                            kt_emit_resolved_kind_at(ctx, rf->qualified_name, "lsp_kt_callable_ref",
                                                     KT_CONF_METHOD, LSM_RESOLVED_CALL_REFERENCE,
                                                     last);
                        } else if (!lsm_type_is_unknown(
                            ...
                            const char *prop_qn =
                                lsm_arena_sprintf(ctx->arena, "%s.%s", recv_qn, member);
                    ...
                    if (lsm_scope_contains(ctx->current_scope, member)) {
                        const LSMType *lexical = lsm_scope_lookup(ctx->current_scope, member);
                        const char *lexical_qn =
                            lsm_scope_lookup_callable(ctx->current_scope, member);
                        if (lexical_qn) {
                            kt_emit_resolved_kind_at(ctx, lexical_qn, "lsp_kt_callable_alias_ref",
                                                     KT_CONF_TOP_LEVEL, LSM_RESOLVED_CALL_REFERENCE,
                                                     last);
                        }
                    ...
                    const char *class_qn = kotlin_resolve_class_name(ctx, member);
                    const LSMRegisteredType *class_type =
                        class_qn && ctx->registry
                            ? lsm_registry_lookup_type(ctx->registry, class_qn)
                            : NULL;
                    if (class_type) {
                        ...
                        if (constructor_qn) {
                            kt_emit_resolved_kind_at(ctx, constructor_qn, "lsp_kt_constructor_ref",
                                                     KT_CONF_CONSTRUCTOR,
                                                     LSM_RESOLVED_CALL_REFERENCE, last);
                        }
                        result = lsm_type_unknown();
                        goto out;
                    }
    ...
    if (strcmp(kind, "object_literal") == 0) {
        ...
        result = lsm_type_named(ctx->arena, "kotlin.Any");
        goto out;
    }
    if (strcmp(kind, "collection_literal") == 0) {
        /* [a, b, c] — typically a List */
        result = lsm_type_named(ctx->arena, "kotlin.collections.List");
        goto out;
    }
    if (strcmp(kind, "range_expression") == 0) {
        result = lsm_type_named(ctx->arena, "kotlin.ranges.IntRange");
        goto out;
    }
...
static const LSMType *kt_eval_constructor_or_func_call(KotlinLSPContext *ctx, TSNode call_node,
    ...
    if (lsm_scope_contains(ctx->current_scope, callee_text)) {
        const char *target = lsm_scope_lookup_callable(ctx->current_scope, callee_text);
        const LSMType *bound = lsm_scope_lookup(ctx->current_scope, callee_text);
        if (target) {
            ...
            const LSMRegisteredFunc *rf =
                ctx->registry ? lsm_registry_lookup_func(ctx->registry, target) : NULL;
        ...
        if (bound && bound->kind == LSM_TYPE_CALLABLE) {
            return bound->data.callable.return_type ? bound->data.callable.return_type
                                                    : lsm_type_unknown();
        }
        if (bound && bound->kind == LSM_TYPE_FUNC && bound->data.func.return_types &&
    ...
    if (ctx->this_type && !lsm_type_is_unknown(ctx->this_type)) {
        const char *this_qn = kt_type_qn_of(ctx->this_type);
        if (this_qn) {
            const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, this_qn, callee_text);
            if (rf && rf->qualified_name) {
    ...
    if (cls_qn && ctx->registry) {
        ...
        if (rt) {
            ...
            return lsm_type_named(ctx->arena, cls_qn);
        }
    }
    /* 3. Top-level fun */
    ...
    if (fn_qn) {
        ...
        if (ctx->registry) {
            const LSMRegisteredFunc *rf = lsm_registry_lookup_func(ctx->registry, fn_qn);
            if (rf && rf->signature && rf->signature->kind == LSM_TYPE_FUNC) {
        ...
        for (int i = 0; stdlib_returns[i].qn; i++) {
            if (strcmp(stdlib_returns[i].qn, fn_qn) == 0) {
                return lsm_type_named(ctx->arena, stdlib_returns[i].ret);
            }
        }
    }
    (void)call_node;
...
static const LSMType *kt_eval_call_expression_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "navigation_expression") == 0) {
        ...
        const LSMType *t = kt_eval_navigation_expression_type_at(ctx, callee, node);
        return t;
    }
...
static const LSMType *kt_eval_navigation_expression_type_at(KotlinLSPContext *ctx, TSNode node,
    ...
    const LSMType *recv_type = kotlin_eval_expr_type(ctx, receiver_node);
    ...
    if (recv_qn && ctx->registry) {
        ...
        const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, recv_qn, member_text);
        if (rf && rf->qualified_name) {
            ...
            if (is_member) {
                ...
                const LSMRegisteredType *recv_rt =
            ...
            const LSMType *guess = kt_stdlib_method_return_type(ctx, recv_qn, member_text);
            if (!lsm_type_is_unknown(guess)) {
                return guess;
            }
            return lsm_type_unknown();
        }
        ...
        const LSMType *pt = kotlin_lookup_property_type(ctx, recv_qn, member_text);
        if (!lsm_type_is_unknown(pt)) {
            ...
            if (ts_node_eq(site, node)) {
                ...
                if (ctx->cross_field_map) {
                    const KtCrossFieldList *fl = (const KtCrossFieldList *)lsm_ht_get(
                        (const LSMHashTable *)ctx->cross_field_map, recv_qn);
                ...
                kt_emit_resolved_kind_at(ctx, prop_qn, "lsp_kt_property_ref", KT_CONF_METHOD,
                                         LSM_RESOLVED_CALL_REFERENCE, selector);
            }
            return pt;
        }
        ...
        rf = lsm_registry_lookup_method(ctx->registry, recv_qn, member_text);
        if (rf && rf->qualified_name) {
    ...
    if (ctx->it_type && kt_node_is(receiver_node, "identifier")) {
        ...
        if (recv_text && strcmp(recv_text, "it") == 0) {
            ...
            if (it_qn) {
                const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, it_qn, member_text);
                if (rf && rf->qualified_name) {
...
static void kt_register_property_delegate(KotlinLSPContext *ctx, TSNode prop,
    ...
    const LSMType *delegate_type = kotlin_eval_expr_type(ctx, inner);
    const char *delegate_qn = kt_type_qn_of(delegate_type);
    if (!delegate_qn)
        return;
    const LSMRegisteredFunc *getter = kotlin_lookup_method(ctx, delegate_qn, "getValue");
    const LSMRegisteredFunc *setter = NULL;
    ...
    const LSMScope *scope = owner ? NULL : ctx->current_scope;
...
static void kt_bind_property_to_scope(KotlinLSPContext *ctx, TSNode prop) {
        ...
        if (init_t && !lsm_type_is_unknown(init_t)) {
            ...
            for (uint32_t i = 0; i < mnc; i++) {
                ...
                if (iqn) {
                    ...
                    const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, iqn, comp);
                    if (rf && rf->qualified_name) {
                ...
                if (!ts_node_is_null(vid)) {
                    ...
                    if (vname) {
                        lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, vname),
                                       lsm_type_unknown());
                    }
                }
            }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (ts_node_is_null(type_node)) {
            const LSMType *candidate = kotlin_eval_expr_type(ctx, child);
            if (!lsm_type_is_unknown(candidate)) {
                t = candidate;
                break;
            }
        }
    ...
    if (callable_qn) {
        lsm_scope_bind_callable(ctx->current_scope, lsm_arena_strdup(ctx->arena, name), t,
                                callable_qn);
    } else {
        lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, name), t);
    }
}
...
static void kt_bind_assignment_to_scope(KotlinLSPContext *ctx, TSNode assignment) {
    ...
    if (callable_qn) {
        lsm_scope_update_callable(ctx->current_scope, name, callable_qn);
    } else {
...
static void kt_apply_smart_cast(KotlinLSPContext *ctx, TSNode condition_expr, bool then_branch) {
    ...
    if (!is_check && kt_node_is(condition_expr, "check_expression")) {
        ...
        if (ctext &&
            (lsm_memmem(ctext, strlen(ctext), " is ", 4) != NULL || strstr(ctext, "!is") != NULL)) {
            is_check = true;
        }
    }
    if (!is_check) {
    ...
    const LSMType *t = kotlin_parse_type_node(ctx, rhs);
    if (!lsm_type_is_unknown(t)) {
        lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, name), t);
    }
}
...
static void kt_process_when_expression(KotlinLSPContext *ctx, TSNode node) {
    ...
    const LSMType *subject_type = NULL;
        ...
        for (uint32_t e = 0; e < en; e++) {
            ...
            if (kt_node_is(ec, "_when_condition") || kt_node_is(ec, "when_condition") ||
                ...
                if (!ts_node_is_null(tt) && subject_name) {
                    const LSMType *narrow = kotlin_parse_type_node(ctx, tt);
                    if (!lsm_type_is_unknown(narrow)) {
                        lsm_scope_bind(ctx->current_scope,
                                       lsm_arena_strdup(ctx->arena, subject_name), narrow);
                    }
                }
            }
        }
...
static void kt_process_for_statement(KotlinLSPContext *ctx, TSNode node) {
    ...
    const LSMType *iter_t = kotlin_eval_expr_type(ctx, iter);
    if (iter_t && !lsm_type_is_unknown(iter_t)) {
        const char *iqn = kt_type_qn_of(iter_t);
        if (iqn) {
            const char *protocol[] = {"iterator", "hasNext", "next", NULL};
            for (int i = 0; protocol[i]; i++) {
                const LSMRegisteredFunc *rf = kotlin_lookup_method(ctx, iqn, protocol[i]);
                if (rf && rf->qualified_name) {
    ...
    if (!ts_node_is_null(loop_var)) {
        ...
        if (!ts_node_is_null(id)) {
            char *name = kt_node_text(ctx, id);
            if (name) {
                lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, name),
                               lsm_type_unknown());
            }
        }
    }
...
static void kt_process_lambda(KotlinLSPContext *ctx, TSNode lambda, const LSMType *receiver_type) {
    ...
    ctx->current_scope = lsm_scope_push(ctx->arena, ctx->current_scope);
    const LSMType *prev_it = ctx->it_type;
    ...
    const LSMType *prev_this = ctx->this_type;
    if (receiver_type && !lsm_type_is_unknown(receiver_type)) {
        ...
        for (uint32_t i = 0; i < pnc; i++) {
            ...
            if (!ts_node_is_null(id)) {
                ...
                if (name) {
                    ...
                    const LSMType *t =
                        ts_node_is_null(tn) ? lsm_type_unknown() : kotlin_parse_type_node(ctx, tn);
                    lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, name), t);
                }
            }
        }
...
static void kt_process_statement(KotlinLSPContext *ctx, TSNode stmt) {
    ...
    if (strcmp(kind, "function_declaration") == 0) {
        ...
        {
            LSMRegisteredFunc lrf = {0};
            lrf.qualified_name = new_qn;
            lrf.short_name = fname;
            lrf.min_params = 0;
            lsm_registry_add_func((LSMTypeRegistry *)ctx->registry, lrf);
            ...
            lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, fname),
                           lsm_type_callable(ctx->arena, NULL, 0, lsm_type_unknown()));
            ...
            if (ctx->package_qn && *ctx->package_qn) {
                LSMRegisteredFunc alias = lrf;
                alias.qualified_name = kt_join_dot(ctx->arena, ctx->package_qn, fname);
                lsm_registry_add_func((LSMTypeRegistry *)ctx->registry, alias);
            } else {
                /* No package — register at root. */
                LSMRegisteredFunc alias = lrf;
                alias.qualified_name = lsm_arena_strdup(ctx->arena, fname);
                lsm_registry_add_func((LSMTypeRegistry *)ctx->registry, alias);
            }
        }
    ...
    if (strcmp(kind, "callable_reference") == 0) {
        ...
        if (ctx->resolved_calls) {
            ...
            for (int i = 0; i < ctx->resolved_calls->count; i++) {
                const LSMResolvedCall *resolved = &ctx->resolved_calls->items[i];
...
static void kt_bind_function_params(KotlinLSPContext *ctx, TSNode func_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const LSMType *t = ts_node_is_null(type_node) ? lsm_type_unknown()
                                                      : kotlin_parse_type_node(ctx, type_node);
        lsm_scope_bind(ctx->current_scope, lsm_arena_strdup(ctx->arena, pname), t);
    }

    /* For extension fun and method: bind `this` */
    if (ctx->this_type) {
        lsm_scope_bind(ctx->current_scope, "this", ctx->this_type);
    }
}
...
static void kt_process_function_body(KotlinLSPContext *ctx, TSNode func_node, const char *func_qn,
    ...
    const char *prev_class = ctx->enclosing_class_qn;
    const LSMType *prev_this = ctx->this_type;
    const LSMType *prev_super = ctx->super_type;
    const char *prev_super_qn = ctx->enclosing_super_qn;

    ctx->enclosing_func_qn = func_qn;
    const LSMRegisteredType *receiver_rt = NULL;
    if (receiver_class_qn) {
        ctx->this_type = lsm_type_named(ctx->arena, receiver_class_qn);
        receiver_rt = lsm_registry_lookup_type(ctx->registry, receiver_class_qn);
        if (receiver_rt && receiver_rt->embedded_types && receiver_rt->embedded_types[0]) {
            ...
            ctx->super_type = lsm_type_named(ctx->arena, receiver_rt->embedded_types[0]);
        }
    }
    ...
    if (receiver_rt && receiver_rt->field_names && receiver_rt->field_types) {
        for (int i = 0; receiver_rt->field_names[i]; i++) {
            ...
            lsm_scope_bind(ctx->current_scope,
                           lsm_arena_strdup(ctx->arena, receiver_rt->field_names[i]),
                           receiver_rt->field_types[i]);
            const LSMKotlinDelegateBinding *member_delegate =
...
/* Debug AST dumper — only active when LSM_LSP_KOTLIN_AST=1 in env. */
...
/* ── public entry point used by lsm.c dispatcher ──────────────────── */
 ...
 * patched source. */
static char *kt_repair_bodyless_interface_methods(LSMArena *arena, const char *src, int src_len,
                                                  int *out_len) {
    if (!src || src_len <= 0) {
        return NULL;
    }
    ...
    char *out = (char *)lsm_arena_alloc(arena, (size_t)new_len + 1);
    if (!out) {
        return NULL;
    }
    int oi = 0;
...
void lsm_run_kotlin_lsp(LSMArena *arena, LSMFileResult *result, const char *source, int source_len,
                        TSNode root) {
    ...
    bool debug = (getenv("LSM_LSP_DEBUG") != NULL);
    ...
    LSMTypeRegistry registry;
    lsm_registry_init(&registry, arena);

    /* Curated stdlib */
    lsm_kotlin_stdlib_register(&registry, arena);
    ...
    if (first_dot) {
        ...
        char *pn = (char *)lsm_arena_alloc(arena, pl + 1);
        if (pn) {
...
static const LSMType *kt_signature_parse_type(LSMArena *arena, const char *text, void *parser_ctx) {
    const KotlinSignatureParseContext *ctx = (const KotlinSignatureParseContext *)parser_ctx;
    const LSMLSPDef *def = ctx ? ctx->def : NULL;
    if (!text || !text[0])
        return lsm_type_unknown();

    char *clean = lsm_arena_strdup(arena, text);
    if (!clean)
        return lsm_type_unknown();
    ...
    return lsm_type_named(arena, qualified);
}

static const LSMType *kt_cross_return_type(LSMArena *arena, const LSMLSPDef *d) {
    if (!arena || !d || !d->return_types || !d->return_types[0]) {
        return NULL;
    }
    const char *text = d->return_types;
    const char *bar = strchr(text, '|');
    const char *first = bar ? lsm_arena_strndup(arena, text, (size_t)(bar - text)) : text;
    if (!first || !first[0]) {
        return NULL;
    }
    if (!strchr(first, '.')) {
    ...
    return lsm_type_named(arena, first);
}

static const LSMType *kt_cross_func_sig_with_return(LSMArena *arena, const LSMLSPDef *d) {
    const LSMType *ret = kt_cross_return_type(arena, d);
    ...
    if (ret && !lsm_type_is_unknown(ret)) {
        rets = (const LSMType **)lsm_arena_alloc(arena, 2 * sizeof(*rets));
        if (!rets)
            return NULL;
        rets[0] = ret;
        rets[1] = NULL;
    }
    return lsm_type_func(arena, NULL, param_types, rets);
}

static void kt_register_cross_def(LSMTypeRegistry *reg, LSMArena *arena, const LSMLSPDef *d,
                                  const LSMHashTable *field_map) {
    ...
    if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
        ...
        if (field_map) {
            const KtCrossFieldList *fl =
                (const KtCrossFieldList *)lsm_ht_get((LSMHashTable *)field_map,
                                                     d->qualified_name);
            if (fl && fl->count > 0) {
        ...
        if (d->embedded_types && d->embedded_types[0]) {
            ...
            const char **emb =
                (const char **)lsm_arena_alloc(arena, (size_t)(n + 1) * sizeof(*emb));
            int idx = 0;
            ...
            while (*start) {
                ...
                if (end > start) {
                    emb[idx++] = lsm_arena_strndup(arena, start, (size_t)(end - start));
                }
                if (!*end) {
                    break;
                }
...
void lsm_kotlin_register_lsp_defs(LSMArena *arena, LSMTypeRegistry *reg, const LSMLSPDef *defs,
                                  int def_count, LSMHashTable **out_field_map) {
     ...
     * quadratic pattern. */
    LSMHashTable *field_map = lsm_ht_create(64);
    if (field_map) {
        for (int i = 0; i < def_count; i++) {
            ...
            KtCrossFieldList *fl = (KtCrossFieldList *)lsm_ht_get(field_map, d->receiver_type);
            if (!fl) {
                fl = (KtCrossFieldList *)lsm_arena_alloc(arena, sizeof(*fl));
                if (!fl) {
                    continue;
                }
                memset(fl, 0, sizeof(*fl));
                lsm_ht_set(field_map, d->receiver_type, fl);
            }
            if (fl->count + 1 >= fl->cap) {
                int new_cap = fl->cap ? fl->cap * 2 : 4;
                const char **nn =
                    (const char **)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
                const LSMType **nt =
                    (const LSMType **)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*nt));
                const char **nq =
                    (const char **)lsm_arena_alloc(arena, (size_t)new_cap * sizeof(*nq));
                if (!nn || !nt || !nq) {
                    continue;
                }
                if (fl->count > 0) {
            ...
            const LSMType *ft = kt_cross_return_type(arena, d);
            ...
            fl->types[fl->count] = ft ? ft : lsm_type_named(arena, "kotlin.Any");
    ...
    if (out_field_map) {
        ...
        *out_field_map = field_map;
        ...
        lsm_ht_free(field_map);
    }
}

void lsm_run_kotlin_lsp_cross(LSMArena *arena, const char *source, int source_len,
    ...
    lsm_kotlin_stdlib_register(&reg, arena);
    ...
    LSMHashTable *cross_field_map = NULL;
    lsm_kotlin_register_lsp_defs(arena, &reg, defs, def_count, &cross_field_map);
    ...
    LSMHashTable *type_short = lsm_ht_create(64);
    if (type_short) {
        for (int i = 0; i < def_count; i++) {
            const LSMLSPDef *d = &defs[i];
            if (!d->qualified_name || !d->short_name || !d->label) {
            ...
            const char *prev = (const char *)lsm_ht_get(type_short, d->short_name);
            if (!prev) {
                lsm_ht_set(type_short, d->short_name, (void *)d->qualified_name);
            } else if (strcmp(prev, d->qualified_name) != 0) {
                lsm_ht_set(type_short, d->short_name, (void *)"");
            }
        }
    }
    ...
    for (int i = 0; i < import_count; i++) {
        ...
        kotlin_lsp_add_import(&ctx, import_names[i], import_qns[i], LSM_KT_USE_UNKNOWN);
    }

    kotlin_lsp_process_file(&ctx, root);
    lsm_arena_destroy(&idx_arena);
    lsm_ht_free(type_short);
    lsm_ht_free(cross_field_map);
