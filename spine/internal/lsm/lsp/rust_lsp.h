 ...
 *
 * Architecture mirrors go_lsp / py_lsp: build a `LSMTypeRegistry` from the
 ...
 *
 * Public entry points:
 *   - `lsm_run_rust_lsp` — single-file resolution invoked from
 *     `lsm_extract_file()`.
 *   - `lsm_run_rust_lsp_cross` — cross-file resolution given a list of
 *     `LSMLSPDef`s gathered by the pipeline.
 *   - `lsm_batch_rust_lsp_cross` — batch wrapper that processes several
 *     files in one CGo call (per-file arenas + result copy).
 */
#ifndef LSM_LSP_RUST_LSP_H
#define LSM_LSP_RUST_LSP_H
...
#include "../lsm.h"
#include "go_lsp.h" // for LSMLSPDef (pipeline def), used by the Tier-2 builder
 ...
 * scoring >= LSM_LSP_CONFIDENCE_FLOOR (0.6). Numbers here mirror the Go
 * LSP so the call-edge mix from a polyglot project stays comparable. */
#define LSM_RUST_CONF_DIRECT 0.95f      /* path::to::func or alias hit  */
#define LSM_RUST_CONF_METHOD 0.95f      /* inherent method dispatch     */
#define LSM_RUST_CONF_TRAIT_SOLE 0.92f  /* trait method, single impl    */
#define LSM_RUST_CONF_TRAIT_AMB 0.85f   /* trait method, many impls     */
#define LSM_RUST_CONF_UFCS 0.93f        /* T::method() / Self::new()    */
#define LSM_RUST_CONF_PROMOTED 0.90f    /* through Deref / blanket impl */
#define LSM_RUST_CONF_MACRO_KNOWN 0.85f /* known std macro mapped to fn */
#define LSM_RUST_CONF_OPERATOR 0.88f    /* a+b → T::add (operator trait) */

/* Rust-flavoured LSP context: one per file, lifetime tied to a single
 * `lsm_extract_file()` invocation (or the cross-file caller's arena). */
typedef struct {
    LSMArena *arena;
    const char *source;
    int source_len;
     ...
     * to every emitted LSMResolvedCall as `caller_qn`. */
    ...
    const LSMType *pending_closure_param_type;
    ...
    const struct LSMCargoManifest *cargo_manifest;
     ...
     * into a CALLS edge when a *syntactic* LSMCall with the same
    ...
    /* LSM_LSP_DEBUG=1 in env enables verbose stderr trace. */
    bool debug;
 ...
 * `lsm_type_unknown()` for anything we cannot type. */
 ...
 * reference_type, primitive_type, …) into a LSMType. */
...
/* Populate the same per-file registry used by lsm_run_rust_lsp, including
 ...
 * `LSMLSPDef` in `go_lsp.h` for compatibility with the pipeline plumbing
...
/* Same as `lsm_run_rust_lsp_cross`, plus an optional parsed Cargo manifest
