 ...
 * lsm_pipeline_lsp_target_node() resolves the callee_qn to a graph node
 ...
 * during the per-file Kotlin LSP run (lsm_run_kotlin_lsp, which executes inside
 * lsm_extract_file, BEFORE the parallel pipeline mints def nodes from
 ...
 *
 * Node minting upserts by QN (lsm_gbuf_upsert_node), so injecting the same
 ...
 */
static void kt_builtins_inject_defs(LSMFileResult *result, LSMArena *arena) {
    if (!result || !arena) {
        return;
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        def.start_line = 1;
        def.end_line = 1;
        lsm_defs_push(&result->defs, arena, def);
    }
}
