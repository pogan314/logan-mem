...
static int parse_key(LSMArena* a, const char* s, int len, int from,
    const char** out) {
    int start = from;
    if (from < len && s[from] == '"') {
        from++;
        ...
        *out = lsm_arena_strndup(a, s + start, (size_t)(from - start));
...
static int parse_string(LSMArena* a, const char* s, int len, int from,
    const char** out) {
...
static int parse_section(LSMArena* a, const char* s, int len, int from,
    const char** out) {
 ...
 * matter for us. */
static int parse_dep_entry(LSMArena* a, const char* s, int len, int from,
    ...
    if (key && out->dep_count < LSM_CARGO_MAX_DEPS) {
...
static int parse_package_kv(LSMArena* a, const char* s, int len, int from,
    LSMCargoManifest* out) {
    from = skip_ws_and_comment(s, len, from);
    if (from >= len || s[from] == '[') return from;
    const char* key = NULL;
...
static int parse_workspace_kv(LSMArena* a, const char* s, int len, int from,
    LSMCargoManifest* out) {
    from = skip_ws_and_comment(s, len, from);
    if (from >= len || s[from] == '[') return from;
    ...
    if (key && strcmp(key, "members") == 0 && from < len && s[from] == '[') {
        ...
        while (from < len && s[from] != ']') {
            ...
            if (from < len && (s[from] == '"' || s[from] == '\'')) {
                ...
                if (mem && out->member_count < LSM_CARGO_MAX_MEMBERS) {
