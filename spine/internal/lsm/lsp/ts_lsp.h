#ifndef LSM_LSP_TS_LSP_H
#define LSM_LSP_TS_LSP_H

#include "type_rep.h"
#include "scope.h"
#include "type_registry.h"
#include "../lsm.h"
#include "go_lsp.h" // for LSMLSPDef, LSMResolvedCallArray (shared cross-language)

// TSLSPContext holds state for TypeScript / JavaScript / JSX / TSX expression type
// evaluation within a single file. One context per lsm_run_ts_lsp invocation.
//
...
typedef struct {
    ...
    const LSMType **type_param_constraints;
...
// for the calling thread, valid after a lsm_run_ts_lsp on the same thread.
...
// Evaluate the type of a TS/JS expression node. Returns lsm_type_unknown() on miss.
const LSMType *ts_eval_expr_type(TSLSPContext *ctx, TSNode node);

// Parse a TS/JS type-position AST node into a LSMType.
...
// Cross-file LSP. Caller passes in cross-file definitions (LSMLSPDef list) and the
