...
static const char* join_path(LSMArena* arena, yyjson_val* path_arr,
    const char* crate_qn) {
    ...
    return lsm_arena_strndup(arena, buf, (size_t)off);
}

/* Convert a rustdoc Type JSON value into a LSMType. Best-effort —
 ...
 */
static const LSMType* parse_rustdoc_type(yyjson_val* t, LSMArena* arena,
    const char* crate_qn) {
    if (!t || !yyjson_is_obj(t)) return lsm_type_unknown();

    yyjson_val* prim = yyjson_obj_get(t, "primitive");
    if (prim && yyjson_is_str(prim)) {
        return lsm_type_builtin(arena, yyjson_get_str(prim));
    }
    ...
    if (resolved && yyjson_is_obj(resolved)) {
        ...
        if (name && yyjson_is_str(name)) {
            qn = lsm_arena_strdup(arena, yyjson_get_str(name));
        }
        if (!qn) return lsm_type_unknown();
        ...
        if (args && yyjson_is_obj(args)) {
            ...
            if (angle && yyjson_is_obj(angle)) {
                ...
                if (arr && yyjson_is_arr(arr) && yyjson_arr_size(arr) > 0) {
                    ...
                    if (ti > 0) {
                        return lsm_type_template(arena, qn, targs, ti);
                    }
                }
            }
        }
    ...
    if (borrowed && yyjson_is_obj(borrowed)) {
        ...
        return lsm_type_reference(arena,
    ...
    if (tuple && yyjson_is_arr(tuple)) {
        ...
        if (n == 0) return lsm_type_builtin(arena, "()");
        const LSMType* elems[8];
        int ei = 0;
        yyjson_val* item;
        yyjson_arr_iter it;
        ...
        return lsm_type_tuple(arena, elems, ei);
    }
...
static bool register_function(LSMTypeRegistry* reg, LSMArena* arena,
     ...
     * (implicit unit). */
    const LSMType* ret = lsm_type_builtin(arena, "()");
    ...
    const LSMType** ra = (const LSMType**)lsm_arena_alloc(arena,
...
int lsm_rust_rustdoc_ingest(LSMTypeRegistry* reg, LSMArena* arena,
    ...
    while ((key = yyjson_obj_iter_next(&it))) {
        ...
        if (kind && (strcmp(kind, "struct") == 0 ||
            ...
            if (register_function(reg, arena, lsm_arena_strdup(arena, short_name),
                                  qn, NULL, fn, crate_qn)) {
                count++;
            }
        }
