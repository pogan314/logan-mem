 ...
 * (reg->read_only, set at finalize; lsm_registry_add_* hard-return then) and
 ...
 *
 * Per-language wiring is a few lines: add a LSMNegMemo to the language ctx,
 * key each cascade entry with lsm_negmemo_key (site tag + query strings),
 ...
 */
#ifndef LSM_LSP_NEG_MEMO_H
#define LSM_LSP_NEG_MEMO_H
...
enum {
    LSM_NEGMEMO_INIT_CAP = 1024,
    LSM_NEGMEMO_GROW = 2,
    LSM_NEGMEMO_LOAD_NUM = 7, /* grow at 70% load */
    LSM_NEGMEMO_LOAD_DEN = 10,
};
...
static inline uint64_t lsm_negmemo_key(uint8_t site, const char *a, const char *b) {
...
static inline bool lsm_negmemo_contains(const LSMNegMemo *m, uint64_t key) {
...
static inline void lsm_negmemo_insert_raw(uint64_t *slots, int cap, uint64_t key) {
...
static inline void lsm_negmemo_insert(LSMNegMemo *m, LSMArena *arena, uint64_t key) {
    if (!arena) {
        return; /* no arena — memo silently disabled */
    }
    if (!m->slots) {
        m->slots = lsm_arena_alloc(arena, sizeof(uint64_t) * LSM_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(uint64_t) * LSM_NEGMEMO_INIT_CAP);
        m->cap = LSM_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * LSM_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * LSM_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * LSM_NEGMEMO_GROW;
        uint64_t *ns = lsm_arena_alloc(arena, sizeof(uint64_t) * (size_t)new_cap);
        if (!ns) {
        ...
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i]) {
                lsm_negmemo_insert_raw(ns, new_cap, m->slots[i]);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    }
    if (!lsm_negmemo_contains(m, key)) {
        lsm_negmemo_insert_raw(m->slots, m->cap, key);
        m->count++;
    }
}

/* ── LSMIdxMemo: exact-match string-key → int index map ─────────────────────
...
static inline int32_t lsm_idxmemo_get(const LSMIdxMemo *m, const char *key) {
    if (!m->slots || !key || m->count == 0) {
        return -1;
    }
    uint64_t h = lsm_negmemo_key(0, key, NULL);
    uint64_t mask = (uint64_t)(m->cap - 1);
...
static inline void lsm_idxmemo_put_raw(struct lsm_idxmemo_slot *slots, int cap, uint64_t h,
...
static inline void lsm_idxmemo_put_if_absent(LSMIdxMemo *m, LSMArena *arena, const char *key,
                                             int32_t val) {
    if (!arena || !key) {
        return;
    }
    if (!m->slots) {
        m->slots = lsm_arena_alloc(arena,
                                   sizeof(struct lsm_idxmemo_slot) * LSM_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(struct lsm_idxmemo_slot) * LSM_NEGMEMO_INIT_CAP);
        m->cap = LSM_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * LSM_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * LSM_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * LSM_NEGMEMO_GROW;
        struct lsm_idxmemo_slot *ns =
            lsm_arena_alloc(arena, sizeof(struct lsm_idxmemo_slot) * (size_t)new_cap);
        if (!ns) {
            return;
        }
        memset(ns, 0, sizeof(struct lsm_idxmemo_slot) * (size_t)new_cap);
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i].h) {
                lsm_idxmemo_put_raw(ns, new_cap, m->slots[i].h, m->slots[i].key, m->slots[i].val);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    ...
    uint64_t h = lsm_negmemo_key(0, key, NULL);
    if (lsm_idxmemo_get(m, key) < 0) {
        lsm_idxmemo_put_raw(m->slots, m->cap, h, key, val);
        m->count++;
    }
}

#endif /* LSM_LSP_NEG_MEMO_H */
