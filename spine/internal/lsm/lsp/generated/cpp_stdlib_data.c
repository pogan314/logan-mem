...
static void reg_method(LSMTypeRegistry* reg, LSMArena* arena,
    const char* recv_qn, const char* method_name, const LSMType* ret_type) {
    ...
    char* qn = (char*)lsm_arena_alloc(arena, rlen + 1 + mlen + 1);
    if (!qn) return;
    ...
    rf.signature = lsm_type_func(arena, NULL, NULL,
        ret_type ? (const LSMType*[]){ret_type, NULL} : NULL);
    lsm_registry_add_func(reg, rf);
}

// Helper: register a type
static void reg_type(LSMTypeRegistry* reg, const char* qn, const char* short_name) {
...
static void reg_type_with_fields(LSMTypeRegistry* reg, LSMArena* arena,
    const char* qn, const char* short_name,
    const char** fnames, const LSMType** ftypes) {
    LSMRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = qn;
    rt.short_name = short_name;
...
static void reg_func(LSMTypeRegistry* reg, LSMArena* arena,
    const char* qn, const char* short_name, const LSMType* ret_type) {
    LSMRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.min_params = -1;
    rf.qualified_name = qn;
...
void lsm_cpp_stdlib_register(LSMTypeRegistry* reg, LSMArena* arena) {
    const LSMType* t_void = lsm_type_builtin(arena, "void");
    const LSMType* t_void_ptr = lsm_type_pointer(arena, t_void);
    const LSMType* t_bool = lsm_type_builtin(arena, "bool");
    const LSMType* t_int = lsm_type_builtin(arena, "int");
    const LSMType* t_size_t = lsm_type_builtin(arena, "size_t");
    const LSMType* t_long = lsm_type_builtin(arena, "long");
    const LSMType* t_char = lsm_type_builtin(arena, "char");
    const LSMType* t_char_ptr = lsm_type_pointer(arena, lsm_type_builtin(arena, "const char"));
    ...
    const LSMType* t_string = lsm_type_named(arena, string_qn);
    const LSMType* t_string_ref = lsm_type_reference(arena, t_string);
    ...
    reg_method(reg, arena, string_qn, "front", lsm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "back", lsm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "at", lsm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "operator[]", lsm_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "begin", lsm_type_named(arena, "std.string.iterator"));
    reg_method(reg, arena, string_qn, "end", lsm_type_named(arena, "std.string.iterator"));
    ...
    reg_method(reg, arena, sv_qn, "substr", lsm_type_named(arena, sv_qn));
    ...
    const LSMType* t_T = lsm_type_type_param(arena, "T");
    const LSMType* t_T_ref = lsm_type_reference(arena, t_T);
    const LSMType* t_T_ptr = lsm_type_pointer(arena, t_T);
    ...
    reg_method(reg, arena, vec_qn, "begin", lsm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "end", lsm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "erase", lsm_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "insert", lsm_type_named(arena, "std.vector.iterator"));
    ...
    const LSMType* t_V = lsm_type_type_param(arena, "V");
    const LSMType* t_V_ref = lsm_type_reference(arena, t_V);

    reg_method(reg, arena, map_qn, "operator[]", t_V_ref);
    reg_method(reg, arena, map_qn, "at", t_V_ref);
    reg_method(reg, arena, map_qn, "find", lsm_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "insert", lsm_type_named(arena, "std.pair"));
    reg_method(reg, arena, map_qn, "emplace", lsm_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, map_qn, "begin", lsm_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "end", lsm_type_named(arena, "std.map.iterator"));
    ...
    reg_method(reg, arena, umap_qn, "find", lsm_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "insert", lsm_type_named(arena, "std.pair"));
    reg_method(reg, arena, umap_qn, "emplace", lsm_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, umap_qn, "begin", lsm_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "end", lsm_type_named(arena, "std.unordered_map.iterator"));
    ...
    reg_method(reg, arena, set_qn, "insert", lsm_type_named(arena, "std.pair"));
    reg_method(reg, arena, set_qn, "find", lsm_type_named(arena, "std.set.iterator"));
    ...
    reg_method(reg, arena, set_qn, "begin", lsm_type_named(arena, "std.set.iterator"));
    reg_method(reg, arena, set_qn, "end", lsm_type_named(arena, "std.set.iterator"));

    const char* uset_qn = "std.unordered_set";
    reg_type(reg, uset_qn, "unordered_set");
    reg_method(reg, arena, uset_qn, "insert", lsm_type_named(arena, "std.pair"));
    reg_method(reg, arena, uset_qn, "find", lsm_type_named(arena, "std.unordered_set.iterator"));
    ...
    reg_method(reg, arena, wptr_qn, "lock", lsm_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique",
        lsm_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared",
        lsm_type_named(arena, sptr_qn));
    ...
    const char* pair_qn = "std.pair";
    {
        ...
        const LSMType** pair_field_types = (const LSMType**)lsm_arena_alloc(arena,
            3 * sizeof(const LSMType*));
        pair_field_types[0] = lsm_type_type_param(arena, "T1");
        pair_field_types[1] = lsm_type_type_param(arena, "T2");
    ...
    reg_func(reg, arena, "std.get", "get", lsm_type_unknown());
    ...
    reg_method(reg, arena, "std.function", "operator()", lsm_type_unknown());
    ...
    const LSMType* t_ostream_ref = lsm_type_reference(arena, lsm_type_named(arena, os_qn));
    ...
    const LSMType* t_istream_ref = lsm_type_reference(arena, lsm_type_named(arena, is_qn));
    ...
    reg_func(reg, arena, "std.cout", "cout", lsm_type_named(arena, os_qn));
    reg_func(reg, arena, "std.cin", "cin", lsm_type_named(arena, is_qn));
    reg_func(reg, arena, "std.cerr", "cerr", lsm_type_named(arena, os_qn));
    reg_func(reg, arena, "std.clog", "clog", lsm_type_named(arena, os_qn));
    ...
    const LSMType* t_path = lsm_type_named(arena, fspath_qn);
    ...
    reg_method(reg, arena, fspath_qn, "operator/=", lsm_type_reference(arena, t_path));
    ...
    reg_method(reg, arena, thread_qn, "get_id", lsm_type_named(arena, "std.thread.id"));
    ...
    reg_func(reg, arena, "std.chrono.system_clock.now", "now",
        lsm_type_named(arena, "std.chrono.time_point"));
    ...
    reg_func(reg, arena, "std.find", "find", lsm_type_unknown());
    reg_func(reg, arena, "std.find_if", "find_if", lsm_type_unknown());
    reg_func(reg, arena, "std.for_each", "for_each", lsm_type_unknown());
    reg_func(reg, arena, "std.transform", "transform", lsm_type_unknown());
    reg_func(reg, arena, "std.copy", "copy", lsm_type_unknown());
    reg_func(reg, arena, "std.move", "move", lsm_type_unknown());
    reg_func(reg, arena, "std.swap", "swap", t_void);
    reg_func(reg, arena, "std.min", "min", lsm_type_unknown());
    reg_func(reg, arena, "std.max", "max", lsm_type_unknown());
    reg_func(reg, arena, "std.accumulate", "accumulate", lsm_type_unknown());
    reg_func(reg, arena, "std.count", "count", t_size_t);
    reg_func(reg, arena, "std.count_if", "count_if", t_size_t);
    reg_func(reg, arena, "std.remove", "remove", lsm_type_unknown());
    reg_func(reg, arena, "std.remove_if", "remove_if", lsm_type_unknown());
    reg_func(reg, arena, "std.reverse", "reverse", t_void);
    reg_func(reg, arena, "std.unique", "unique", lsm_type_unknown());
    reg_func(reg, arena, "std.lower_bound", "lower_bound", lsm_type_unknown());
    reg_func(reg, arena, "std.upper_bound", "upper_bound", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.stof", "stof", lsm_type_builtin(arena, "float"));
    reg_func(reg, arena, "std.stod", "stod", lsm_type_builtin(arena, "double"));
    ...
    reg_method(reg, arena, "std.any", "type", lsm_type_unknown()); // type_info
    reg_method(reg, arena, "std.any", "reset", t_void);
    reg_func(reg, arena, "std.any_cast", "any_cast", t_T);
    reg_func(reg, arena, "std.make_any", "make_any", lsm_type_named(arena, "std.any"));
    ...
    reg_method(reg, arena, "std.smatch", "prefix", lsm_type_unknown());
    reg_method(reg, arena, "std.smatch", "suffix", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.partial_sum", "partial_sum", lsm_type_unknown());
    reg_func(reg, arena, "std.adjacent_difference", "adjacent_difference", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.distance", "distance", lsm_type_builtin(arena, "ptrdiff_t"));
    reg_func(reg, arena, "std.next", "next", lsm_type_unknown()); // iterator
    reg_func(reg, arena, "std.prev", "prev", lsm_type_unknown());
    reg_func(reg, arena, "std.begin", "begin", lsm_type_unknown());
    reg_func(reg, arena, "std.end", "end", lsm_type_unknown());
    reg_func(reg, arena, "std.rbegin", "rbegin", lsm_type_unknown());
    reg_func(reg, arena, "std.rend", "rend", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.min_element", "min_element", lsm_type_unknown());
    reg_func(reg, arena, "std.max_element", "max_element", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.partition", "partition", lsm_type_unknown());
    reg_func(reg, arena, "std.stable_partition", "stable_partition", lsm_type_unknown());
    ...
    reg_func(reg, arena, "std.merge", "merge", lsm_type_unknown());
    reg_func(reg, arena, "std.includes", "includes", t_bool);
    reg_func(reg, arena, "std.set_union", "set_union", lsm_type_unknown());
    reg_func(reg, arena, "std.set_intersection", "set_intersection", lsm_type_unknown());
    reg_func(reg, arena, "std.set_difference", "set_difference", lsm_type_unknown());
    reg_func(reg, arena, "std.generate", "generate", t_void);
    reg_func(reg, arena, "std.generate_n", "generate_n", lsm_type_unknown());
    ...
    reg_method(reg, arena, "std.bitset", "set", lsm_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "reset", lsm_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "flip", lsm_type_named(arena, "std.bitset"));
    ...
    reg_method(reg, arena, "std.bitset", "to_ulong", lsm_type_builtin(arena, "unsigned long"));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size", lsm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all", lsm_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.current_path", "current_path",
        lsm_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "path", lsm_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "file_size", lsm_type_builtin(arena, "uintmax_t"));

    // =========================================================================
    // std.chrono — additional
    // =========================================================================
    reg_type(reg, "std.chrono.time_point", "time_point");
    reg_method(reg, arena, "std.chrono.time_point", "time_since_epoch", lsm_type_unknown());
    reg_method(reg, arena, "std.chrono.system_clock", "now",
        lsm_type_named(arena, "std.chrono.time_point"));
    reg_method(reg, arena, "std.chrono.steady_clock", "now",
        lsm_type_named(arena, "std.chrono.time_point"));
    reg_func(reg, arena, "std.chrono.duration_cast", "duration_cast", lsm_type_unknown());
    ...
    reg_method(reg, arena, bfspath_qn, "parent_path", lsm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "filename", lsm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "extension", lsm_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "stem", lsm_type_named(arena, bfspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data", lsm_type_pointer(arena, lsm_type_builtin(arena, "char")));
    ...
    reg_method(reg, arena, absv_qn, "substr", lsm_type_named(arena, absv_qn));

    const char* abstat_qn = "absl.Status";
    reg_type(reg, abstat_qn, "Status");
    reg_method(reg, arena, abstat_qn, "ok", t_bool);
    reg_method(reg, arena, abstat_qn, "code", lsm_type_unknown());
    reg_method(reg, arena, abstat_qn, "message", lsm_type_named(arena, absv_qn));
    reg_method(reg, arena, abstat_qn, "ToString", t_string);
    reg_func(reg, arena, "absl.OkStatus", "OkStatus", lsm_type_named(arena, abstat_qn));

    const char* absor_qn = "absl.StatusOr";
    reg_type(reg, absor_qn, "StatusOr");
    reg_method(reg, arena, absor_qn, "ok", t_bool);
    reg_method(reg, arena, absor_qn, "status", lsm_type_named(arena, abstat_qn));
    ...
    reg_method(reg, arena, abfhm_qn, "find", lsm_type_unknown());
    ...
    reg_method(reg, arena, abfhm_qn, "insert", lsm_type_unknown());
    ...
    reg_method(reg, arena, abfhs_qn, "insert", lsm_type_unknown());
    ...
    reg_method(reg, arena, abspan_qn, "subspan", lsm_type_named(arena, abspan_qn));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit",
        lsm_type_named(arena, "std.vector"));
    ...
    reg_method(reg, arena, grpc_status_qn, "error_code", lsm_type_unknown());
    reg_method(reg, arena, grpc_status_qn, "error_message", t_string);
    reg_func(reg, arena, "grpc.Status.OK", "OK", lsm_type_named(arena, grpc_status_qn));

    const char* grpc_sb_qn = "grpc.ServerBuilder";
    reg_type(reg, grpc_sb_qn, "ServerBuilder");
    reg_method(reg, arena, grpc_sb_qn, "AddListeningPort", lsm_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "RegisterService", lsm_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "BuildAndStart", lsm_type_unknown());

    const char* grpc_chan_qn = "grpc.Channel";
    reg_type(reg, grpc_chan_qn, "Channel");
    reg_func(reg, arena, "grpc.CreateChannel", "CreateChannel",
        lsm_type_named(arena, grpc_chan_qn));
    ...
    reg_func(reg, arena, "spdlog.get", "get",
        lsm_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent", lsm_type_pointer(arena, lsm_type_named(arena, qobj_qn)));
    reg_method(reg, arena, qobj_qn, "children", lsm_type_unknown());
    reg_method(reg, arena, qobj_qn, "objectName", lsm_type_named(arena, "QString"));
    ...
    reg_method(reg, arena, qstr_qn, "toUtf8", lsm_type_unknown());
    reg_method(reg, arena, qstr_qn, "toLatin1", lsm_type_unknown());
    ...
    reg_method(reg, arena, qstr_qn, "trimmed", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "simplified", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toLower", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toUpper", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "arg", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "split", lsm_type_unknown());
    reg_method(reg, arena, qstr_qn, "replace", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "mid", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "left", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "right", lsm_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toInt", t_int);
    reg_method(reg, arena, qstr_qn, "toDouble", lsm_type_builtin(arena, "double"));
    reg_func(reg, arena, "QString.number", "number", lsm_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromStdString", "fromStdString", lsm_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromUtf8", "fromUtf8", lsm_type_named(arena, qstr_qn));
