 ...
 *
 * Return types are left UNKNOWN (lsm_type_unknown) for v1: real signature
...
#define REG_FUNC(module_dot_, name_, ret_type_)                                                 \
    do {                                                                                        \
        memset(&rf, 0, sizeof(rf));                                                             \
        rf.min_params = -1;                                                                     \
        rf.qualified_name = lsm_arena_sprintf(arena, "%s.%s", (module_dot_), (name_));          \
        rf.short_name = (name_);                                                                \
        {                                                                                       \
            const LSMType **rets = (const LSMType **)lsm_arena_alloc(arena, 2 * sizeof(*rets)); \
            rets[0] = (ret_type_);                                                              \
...
void lsm_perl_stdlib_register(LSMTypeRegistry *reg, LSMArena *arena) {
    ...
    REG_BUILTIN("sprintf", lsm_type_builtin(arena, "string"));
    REG_BUILTIN("open", MIXED);
    REG_BUILTIN("close", MIXED);
    REG_BUILTIN("push", lsm_type_builtin(arena, "int"));
    REG_BUILTIN("pop", MIXED);
    REG_BUILTIN("shift", MIXED);
    REG_BUILTIN("unshift", lsm_type_builtin(arena, "int"));
    REG_BUILTIN("map", MIXED);
    REG_BUILTIN("grep", MIXED);
    REG_BUILTIN("sort", MIXED);
    REG_BUILTIN("join", lsm_type_builtin(arena, "string"));
    REG_BUILTIN("split", MIXED);
    REG_BUILTIN("length", lsm_type_builtin(arena, "int"));
    REG_BUILTIN("substr", lsm_type_builtin(arena, "string"));
    REG_BUILTIN("chomp", MIXED);
    REG_BUILTIN("chop", MIXED);
    REG_BUILTIN("die", MIXED);
    REG_BUILTIN("warn", MIXED);
    REG_BUILTIN("ref", lsm_type_builtin(arena, "string"));
    REG_BUILTIN("bless", MIXED);
    REG_BUILTIN("defined", lsm_type_builtin(arena, "bool"));
    REG_BUILTIN("exists", lsm_type_builtin(arena, "bool"));
    ...
    REG_FUNC("Scalar.Util", "reftype", lsm_type_builtin(arena, "string"));
    ...
    REG_FUNC("POSIX", "strftime", lsm_type_builtin(arena, "string"));
    REG_FUNC("POSIX", "INT_MAX", lsm_type_builtin(arena, "int"));

    /* ── Storable ───────────────────────────────────────────────────
     * Source: RESEARCH.md L367. Module QN "Storable". */
    REG_FUNC("Storable", "dclone", MIXED);
    REG_FUNC("Storable", "freeze", lsm_type_builtin(arena, "string"));
    ...
    REG_FUNC("Data.Dumper", "Dumper", lsm_type_builtin(arena, "string"));
}
