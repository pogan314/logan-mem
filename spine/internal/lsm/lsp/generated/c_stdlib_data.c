...
#define REG_FUNC(qn, short, ret_type) do { \
    ...
    rf.short_name = (short); \
    rf.signature = lsm_type_func(arena, NULL, NULL, (const LSMType*[]){(ret_type), NULL}); \
...
void lsm_c_stdlib_register(LSMTypeRegistry* reg, LSMArena* arena) {
    ...
    const LSMType* t_double = lsm_type_builtin(arena, "double");
    const LSMType* t_void = lsm_type_builtin(arena, "void");
    const LSMType* t_char_ptr = lsm_type_pointer(arena, lsm_type_builtin(arena, "char"));
    const LSMType* t_void_ptr = lsm_type_pointer(arena, t_void);

    // FILE type
    REG_TYPE("FILE", "FILE");
    const LSMType* t_file_ptr = lsm_type_pointer(arena, lsm_type_named(arena, "FILE"));
    ...
    REG_FUNC("ftell", "ftell", lsm_type_builtin(arena, "long"));
    ...
    REG_FUNC("atol", "atol", lsm_type_builtin(arena, "long"));
    REG_FUNC("strtol", "strtol", lsm_type_builtin(arena, "long"));
    ...
    REG_FUNC("time", "time", lsm_type_builtin(arena, "time_t"));
    REG_FUNC("clock", "clock", lsm_type_builtin(arena, "clock_t"));
