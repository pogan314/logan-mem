...
//
// For LSM_LANG_KUSTOMIZE: walks top-level block_mapping_pair nodes whose key
// matches "resources", "bases", "patches", "components", or
// "patchesStrategicMerge", then emits one LSMImport per block_sequence item.
//
// For LSM_LANG_K8S: finds apiVersion, kind, and metadata.name scalars in the
// first document's block_mapping and emits one LSMDefinition with label
...
static const char *get_scalar_text(LSMArena *a, TSNode node, const char *source) {
    enum { MAX_UNWRAP = 4 };
    for (int depth = 0; depth < MAX_UNWRAP; depth++) {
        ...
        if (strcmp(type, "double_quote_scalar") == 0 || strcmp(type, "single_quote_scalar") == 0) {
            const char *raw = lsm_node_text(a, node, source);
            if (!raw) {
                return NULL;
            }
            size_t len = strlen(raw);
            if (len >= PAIR_LEN) {
                return lsm_arena_strndup(a, raw + SKIP_ONE, len - PAIR_LEN);
            }
            return raw;
        }
        break;
...
// Walk a block_sequence node and emit one LSMImport per block_sequence_item
// scalar child, using key_name as the local_name.
static void emit_kustomize_sequence(LSMExtractCtx *ctx, TSNode seq_node, const char *key_name) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        for (uint32_t j = 0; j < ic; j++) {
            ...
            LSMImport imp = {
                .local_name = lsm_arena_strdup(a, key_name),
                .module_path = lsm_arena_strdup(a, scalar),
            };
...
static void process_kustomize_pair(LSMExtractCtx *ctx, TSNode pair) {
...
static void emit_kustomize_kind_def(LSMExtractCtx *ctx, TSNode mapping) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t pi = 0; pi < pair_n; pi++) {
        ...
        LSMDefinition def = {0};
        def.name = lsm_arena_strdup(a, kind);
        def.qualified_name = lsm_arena_sprintf(a, "%s.%s", ctx->module_qn, kind);
        def.label = lsm_arena_strdup(a, "Resource");
...
static void extract_kustomize(LSMExtractCtx *ctx) {
...
static void extract_metadata_name(LSMArena *a, TSNode meta_mapping, const char *source,
...
static void extract_k8s_scalars(LSMExtractCtx *ctx, TSNode mapping, char *kind_buf, size_t kind_sz,
                                char *meta_name_buf, size_t meta_sz) {
    LSMArena *a = ctx->arena;
    kind_buf[0] = '\0';
    meta_name_buf[0] = '\0';
...
static void extract_k8s_manifest(LSMExtractCtx *ctx) {
    LSMArena *a = ctx->arena;
    ...
    for (uint32_t si = 0; si < root_n; si++) {
        ...
        LSMDefinition def = {0};
        def.name = lsm_arena_strdup(a, def_name);
        def.qualified_name = lsm_arena_sprintf(a, "%s.%s", ctx->module_qn, def_name);
...
void lsm_extract_k8s(LSMExtractCtx *ctx) {
    if (ctx->language == LSM_LANG_KUSTOMIZE) {
        extract_kustomize(ctx);
    } else if (ctx->language == LSM_LANG_K8S) {
