#ifndef LSM_ARENA_H
#define LSM_ARENA_H

#include <stddef.h>

// LSMArena is a simple bump allocator that allocates from fixed-size blocks.
// All memory is freed at once via lsm_arena_destroy(). Individual frees are not
// supported — this is by design for per-file extraction where all data has the
// same lifetime.
#define LSM_ARENA_MAX_BLOCKS 256
#define LSM_ARENA_DEFAULT_BLOCK_SIZE (64 * 1024) // 64KB initial

typedef struct {
    char *blocks[LSM_ARENA_MAX_BLOCKS];
    size_t block_sizes[LSM_ARENA_MAX_BLOCKS]; // per-block sizes (for stats)
    int nblocks;
    size_t block_size;
    size_t used;        // bytes used in current block
    size_t total_alloc; // cumulative bytes allocated (for stats)
} LSMArena;

// Initialize an arena with the default block size.
void lsm_arena_init(LSMArena *a);

// Allocate n bytes from the arena. Returns NULL on OOM or block exhaustion.
// All returned pointers are 8-byte aligned.
void *lsm_arena_alloc(LSMArena *a, size_t n);

// Duplicate a string into arena memory. Returns arena-owned copy.
char *lsm_arena_strdup(LSMArena *a, const char *s);

// Duplicate a string of known length into arena memory. NUL-terminates.
char *lsm_arena_strndup(LSMArena *a, const char *s, size_t len);

// sprintf into arena memory. Returns arena-owned string.
char *lsm_arena_sprintf(LSMArena *a, const char *fmt, ...) __attribute__((format(printf, 2, 3)));

// Free all blocks. Arena is invalid after this call.
void lsm_arena_destroy(LSMArena *a);

#endif // LSM_ARENA_H
