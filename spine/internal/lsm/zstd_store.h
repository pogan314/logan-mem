#ifndef LSM_ZSTD_STORE_H
#define LSM_ZSTD_STORE_H
...
int64_t lsm_zstd_compress(const char *src, size_t srcLen, char *dst, size_t dstCap, int level);
...
int64_t lsm_zstd_decompress(const char *src, size_t srcLen, char *dst, size_t dstCap);
...
size_t lsm_zstd_frame_content_size(const char *src, size_t srcLen);

// Maximum compressed size bound for given input size.
size_t lsm_zstd_compress_bound(size_t inputSize);

#endif // LSM_ZSTD_STORE_H
