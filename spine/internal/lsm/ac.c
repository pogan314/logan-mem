...
#define LSM_AC_MAX_BITMASK 64

#define LSM_AC_BYTE_RANGE 256
#define LSM_AC_NO_STATE (-1)
#define LSM_AC_ROOT_STATES 1
#define LSM_AC_ALLOC_ONE 1
#define LSM_AC_PATTERN_BIT(p) (1ULL << (p))
#define LSM_AC_CLEAR_LOW_BIT(b) ((b) & ((b) - 1ULL))
...
struct LSMAutomaton {
    ...
    uint8_t alpha_map[LSM_AC_BYTE_RANGE]; // byte → mapped index (identity if alpha_size==256)
...
static int ac_build_trie(LSMAutomaton *ac, const char **patterns, const int *lengths, int count) {
    int alpha_size = ac->alpha_size;
    int num_states = LSM_AC_ROOT_STATES; // state 0 = root
    ...
    for (int p = 0; p < count; p++) {
        ...
        for (int j = 0; j < lengths[p]; j++) {
            ...
            if (ac->go_table[idx] == LSM_AC_NO_STATE) {
                ac->go_table[idx] = num_states++;
            }
            state = ac->go_table[idx];
        }
        if (p < LSM_AC_MAX_BITMASK) {
            ac->output[state] |= LSM_AC_PATTERN_BIT(p);
        }
    ...
    for (int c = 0; c < alpha_size; c++) {
        if (ac->go_table[c] == LSM_AC_NO_STATE) {
            ac->go_table[c] = 0;
        }
    }
    return num_states;
...
static void ac_build_failure(LSMAutomaton *ac, int num_states) {
    int alpha_size = ac->alpha_size;
    ...
    while (!queue_empty(&q)) {
        ...
        for (int c = 0; c < alpha_size; c++) {
            ...
            if (s != LSM_AC_NO_STATE) {
                fail[s] = ac->go_table[(fail[r] * alpha_size) + c];
                ac->output[s] |= ac->output[fail[s]];
                if (ac->output_next[s] == LSM_AC_NO_STATE &&
                    ac->output_list[fail[s]] != LSM_AC_NO_STATE) {
...
static void ac_shrink_tables(LSMAutomaton *ac, int num_states, int max_states) {
...
// lsm_ac_build constructs an Aho-Corasick automaton from a set of patterns.
//
// Parameters:
...
//
// Returns a heap-allocated automaton. Caller must call lsm_ac_free().
LSMAutomaton *lsm_ac_build(const char **patterns, const int *lengths, int count,
    ...
    if (alpha_size <= 0) {
        alpha_size = LSM_AC_BYTE_RANGE;
    }

    int max_states = LSM_AC_ROOT_STATES;
    for (int i = 0; i < count; i++) {
        max_states += lengths[i];
    }

    LSMAutomaton *ac = (LSMAutomaton *)calloc(LSM_AC_ALLOC_ONE, sizeof(LSMAutomaton));
    ac->alpha_size = alpha_size;
    ac->num_patterns = count;

    if (alpha_map) {
        memcpy(ac->alpha_map, alpha_map, LSM_AC_BYTE_RANGE);
    } else {
        for (int i = 0; i < LSM_AC_BYTE_RANGE; i++) {
            ac->alpha_map[i] = (uint8_t)i;
        }
    }

    ac->go_table = (int *)malloc((size_t)max_states * alpha_size * sizeof(int));
    memset(ac->go_table, LSM_AC_NO_STATE, (size_t)max_states * alpha_size * sizeof(int));
    ...
    for (int i = 0; i < max_states; i++) {
        ac->output_list[i] = LSM_AC_NO_STATE;
        ac->output_next[i] = LSM_AC_NO_STATE;
    }
...
// lsm_ac_free releases all memory for an automaton.
...
// lsm_ac_scan_bitmask scans text through the automaton and returns a bitmask
...
// Each goroutine gets its own OS thread (via CGo), so LSM_TLS is safe.
static LSM_TLS char *tls_decomp_buf = NULL;
static LSM_TLS int tls_decomp_cap = 0;
...
// lsm_ac_scan_lz4_bitmask decompresses LZ4 data into a thread-local buffer
...
uint64_t lsm_ac_scan_lz4_bitmask(const LSMAutomaton *ac, const char *compressed, int compressed_len,
    ...
    int decompressed = lsm_lz4_decompress(compressed, compressed_len, buf, original_len);
    if (decompressed < 0) {
        return 0;
    }

    return lsm_ac_scan_bitmask(ac, buf, decompressed);
}

// ─── Batch LZ4 + AC scan ───────────────────────────────────────────────────

// LSMLz4Entry and LSMLz4Match defined in ac.h.

// lsm_ac_scan_lz4_batch decompresses and scans multiple files in one call.
...
int lsm_ac_scan_lz4_batch(const LSMAutomaton *ac, const LSMLz4Entry *entries, int num_entries,
    ...
    for (int i = 0; i < num_entries && total < max_matches; i++) {
        ...
        int decompressed = lsm_lz4_decompress(entries[i].data, entries[i].compressed_len, buf,
...
// LSMMatchResult defined in ac.h.

// lsm_ac_scan_batch scans multiple NUL-separated names through the automaton.
        ...
        for (int i = 0; i < text_len; i++) {
            ...
            while (s > 0 && total < max_matches) {
                ...
                while (bits && total < max_matches) {
                    ...
                    total++;
                    seen |= LSM_AC_PATTERN_BIT(pid);
                    bits = LSM_AC_CLEAR_LOW_BIT(bits);
                }

                // Follow output_next for patterns beyond bitmask range.
                int next_state = ac->output_next[s];
                if (next_state == LSM_AC_NO_STATE || next_state == s) {
                    break;
                }
                s = next_state;
            }
...
// lsm_ac_table_bytes returns the approximate memory used by the goto table.
