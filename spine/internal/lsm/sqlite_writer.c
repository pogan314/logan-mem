...
#include <process.h>
#include <windows.h>
#define lsm_writer_getpid _getpid
#else
#include <unistd.h>
#define lsm_writer_getpid getpid
#endif

#define LSM_PAGE_SIZE 65536
...
#define LSM_PENDING_BYTE (0x40000000u)
#define LSM_PENDING_BYTE_PAGE ((LSM_PENDING_BYTE / LSM_PAGE_SIZE) + 1)

/* Skip the pending byte page if allocation lands on it. */
static inline uint32_t lsm_skip_pending_byte(uint32_t pgno) {
    return pgno == LSM_PENDING_BYTE_PAGE ? pgno + SKIP_ONE : pgno;
}
...
static bool dynbuf_ensure(DynBuf *b, int needed) {
    ...
    if (!p) {
        (void)fprintf(stderr, "lsm_write_db: dynbuf realloc failed size=%d\n", newcap);
        return false;
    }
    b->data = p;
    b->cap = newcap;
...
static void pb_init(PageBuilder *pb, FILE *fp, uint32_t start_page, bool is_index) {
    ...
    pb->cell_count = 0;
    pb->content_offset = LSM_PAGE_SIZE;
    ...
    memset(pb->page, 0, LSM_PAGE_SIZE);
    pb->leaves = NULL;
    pb->leaf_count = 0;
    pb->leaf_cap = 0;
}
...
static void pb_flush_leaf(PageBuilder *pb) {
    ...
    pb->next_page = lsm_skip_pending_byte(pb->next_page);
    uint32_t page_num = pb->next_page;
    long offset = (long)(page_num - SKIP_ONE) * LSM_PAGE_SIZE;
    (void)fseek(pb->fp, offset, SEEK_SET);
    (void)fwrite(pb->page, SKIP_ONE, LSM_PAGE_SIZE, pb->fp);
...
static int write_interior_page(PageBuilder *pb, uint8_t *page, int cell_count, int content_offset,
    ...
    (void)fseek(pb->fp, (long)(pnum - SKIP_ONE) * LSM_PAGE_SIZE, SEEK_SET);
    (void)fwrite(page, SKIP_ONE, LSM_PAGE_SIZE, pb->fp);
...
static uint32_t pb_build_interior(PageBuilder *pb, bool is_index) {
    ...
    while (child_count > SKIP_ONE && children) {
        ...
        while (i < child_count) {
            uint8_t page[LSM_PAGE_SIZE];
            memset(page, 0, LSM_PAGE_SIZE);
            int cell_count = 0;
            int content_offset = LSM_PAGE_SIZE;
...
// end_line, properties)
static uint8_t *build_node_record(const LSMDumpNode *n, int *out_len) {
    RecordBuilder r;
    rec_init(&r);
...
static uint8_t *build_edge_record(const LSMDumpEdge *e, int *out_len) {
    RecordBuilder r;
    rec_init(&r);
...
static uint8_t *build_vector_record(const LSMDumpVector *v, int *out_len) {
    RecordBuilder r;
    rec_init(&r);
...
static uint8_t *build_token_vec_record(const LSMDumpTokenVec *tv, int *out_len) {
    RecordBuilder r;
    rec_init(&r);
...
// Each overflow page: 4-byte next-page pointer + up to (LSM_PAGE_SIZE-4) bytes of data.
static uint32_t write_overflow_pages(FILE *fp, uint32_t *next_page, const uint8_t *data,
                                     int data_len) {
    int per_page = LSM_PAGE_SIZE - BTREE_PTR_SIZE;
    ...
    while (offset < data_len) {
        ...
        long page_offset = (long)(pnum - SKIP_ONE) * LSM_PAGE_SIZE;
        prev_next_ptr_offset = page_offset;
        (void)fseek(fp, page_offset, SEEK_SET);
        (void)fwrite(page, SKIP_ONE, LSM_PAGE_SIZE, fp);
...
static uint8_t *overflowize_index_cell(FILE *fp, uint32_t *next_page, uint8_t *cell,
    ...
    int64_t per_ovfl = (int64_t)LSM_PAGE_SIZE - BTREE_PTR_SIZE;
...
static void pb_add_table_cell_with_flush(PageBuilder *pb, int64_t rowid, const uint8_t *payload,
    ...
    if (payload_len > TABLE_OVERFLOW_MAX_LOCAL) {
        ...
        int ovfl_page_data = LSM_PAGE_SIZE - BTREE_PTR_SIZE;
...
static uint32_t write_table_btree(FILE *fp, uint32_t *next_page, const uint8_t **records,
    ...
    if (count == 0) {
        ...
        *next_page = lsm_skip_pending_byte(*next_page);
        uint32_t pnum = (*next_page)++;
        uint8_t page[LSM_PAGE_SIZE];
        memset(page, 0, LSM_PAGE_SIZE);
        ...
        put_u16(page + hdr + HDR_CONTENT_OFF, (uint16_t)LSM_PAGE_SIZE); // content at end of page
        page[hdr + HDR_FRAGBYTES_OFF] = 0;                              // 0 fragmented bytes
        (void)fseek(fp, (long)(pnum - SKIP_ONE) * LSM_PAGE_SIZE, SEEK_SET);
        (void)fwrite(page, SKIP_ONE, LSM_PAGE_SIZE, fp);
        return pnum;
    }
...
static uint32_t write_empty_index_leaf(FILE *fp, uint32_t *next_page) {
    *next_page = lsm_skip_pending_byte(*next_page);
    ...
    put_u16(page + HDR_CONTENT_OFF, (uint16_t)LSM_PAGE_SIZE);
    page[HDR_FRAGBYTES_OFF] = 0;
    (void)fseek(fp, (long)(pnum - SKIP_ONE) * LSM_PAGE_SIZE, SEEK_SET);
    (void)fwrite(page, SKIP_ONE, LSM_PAGE_SIZE, fp);
    return pnum;
...
static uint32_t write_index_btree(FILE *fp, uint32_t *next_page, uint8_t **cells, int *cell_lens,
    ...
    for (int i = 0; i < count; i++) {
        if (!pb_cell_fits(&pb, cell_lens[i])) {
            ...
            if (!pb_cell_fits(&pb, cell_lens[i])) {
                (void)fprintf(stderr, "lsm_write_db: index cell oversized, skipped len=%d idx=%d\n",
                              cell_lens[i], i);
                continue;
            }
        }
...
static const LSMDumpNode *g_sort_nodes;
static const LSMDumpEdge *g_sort_edges;
...
static int *make_sorted_perm(int n, int (*cmp)(const void *, const void *)) {
    ...
    if (!perm) {
        (void)fprintf(stderr, "lsm_write_db: perm malloc failed n=%d size=%zu\n", n,
                      (size_t)n * sizeof(int));
        return NULL;
    }
    for (int i = 0; i < n; i++) {
...
typedef uint8_t *(*edge_cell_fn)(const LSMDumpEdge *e, int *out_len);

static uint8_t *ecell_source(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_int_text_rowid(e->source_id, e->type, e->id, out_len);
}
static uint8_t *ecell_target(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_int_text_rowid(e->target_id, e->type, e->id, out_len);
}
static uint8_t *ecell_type(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_2text_rowid(e->project, e->type, e->id, out_len);
}
static uint8_t *ecell_proj_target_type(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_text_int_text_rowid(e->project, e->target_id, e->type, e->id, out_len);
}
static uint8_t *ecell_proj_source_type(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_text_int_text_rowid(e->project, e->source_id, e->type, e->id, out_len);
}
static uint8_t *ecell_src_tgt_type(const LSMDumpEdge *e, int *out_len) {
    return build_index_entry_unique_2int_2text_rowid(e->source_id, e->target_id, e->type,
                                                     safe_str(e->local_name), e->id, out_len);
}
static uint8_t *ecell_url_path(const LSMDumpEdge *e, int *out_len) {
...
static uint32_t build_edge_index_sorted(FILE *fp, uint32_t *next_page, LSMDumpEdge *edges,
...
typedef const char *(*node_col_fn)(const LSMDumpNode *n);
static const char *ncol_label(const LSMDumpNode *n) {
    return n->label;
}
static const char *ncol_name(const LSMDumpNode *n) {
    return n->name;
}
static const char *ncol_file(const LSMDumpNode *n) {
    return n->file_path ? n->file_path : "";
}
static const char *ncol_qn(const LSMDumpNode *n) {
    return n->qualified_name;
}

/* Build a 2-text node index from a pre-sorted permutation. Returns root page or 0. */
static uint32_t build_node_index_sorted(FILE *fp, uint32_t *next_page, LSMDumpNode *nodes,
...
/* Write context passed to sub-phases of lsm_write_db. */
typedef struct {
    FILE *fp;
    uint32_t next_page;
    char final_path[LSM_SZ_4K];
    char temp_path[LSM_SZ_4K];
    const char *project;
    ...
    LSMDumpEdge *edges;
    int edge_count;
    LSMDumpVector *vectors;
    int vector_count;
    LSMDumpTokenVec *token_vecs;
    int token_vec_count;
} write_db_ctx_t;

static int make_writer_temp_path(const char *path, const void *token, char *out, size_t out_size) {
    int n = snprintf(out, out_size, "%s.tmp.%ld.%p", path, (long)lsm_writer_getpid(), token);
...
static int publish_writer_output(write_db_ctx_t *w) {
    ...
    if (lsm_rename_replace(w->temp_path, w->final_path) != 0) {
        (void)lsm_unlink(w->temp_path);
        return ERR_WRITE_FAILED;
    }
    ...
    lsm_remove_db_sidecars(w->final_path);
    return 0;
}
 ...
 * PageBuilder in lsm_writer_append_nodes, so no node adapter is needed here). */
static uint8_t *adapt_build_edge(const void *items, int i, int *out_len) {
    return build_edge_record(&((const LSMDumpEdge *)items)[i], out_len);
}
static int64_t adapt_edge_id(const void *items, int i) {
    return ((const LSMDumpEdge *)items)[i].id;
}
static uint8_t *adapt_build_vector(const void *items, int i, int *out_len) {
    return build_vector_record(&((const LSMDumpVector *)items)[i], out_len);
}
static int64_t adapt_vector_id(const void *items, int i) {
    return ((const LSMDumpVector *)items)[i].node_id;
}
static uint8_t *adapt_build_token_vec(const void *items, int i, int *out_len) {
    return build_token_vec_record(&((const LSMDumpTokenVec *)items)[i], out_len);
}
static int64_t adapt_token_vec_id(const void *items, int i) {
    return ((const LSMDumpTokenVec *)items)[i].id;
}
...
static int write_master_page1(FILE *fp, MasterEntry *master, int master_count, uint32_t next_page) {
    ...
    uint8_t page1[LSM_PAGE_SIZE];
    memset(page1, 0, LSM_PAGE_SIZE);
    int hdr = SQLITE_HEADER_SIZE;
    page1[hdr] = BTREE_LEAF_TABLE;
    int content_off = LSM_PAGE_SIZE;
    ...
    (void)fwrite(page1, SKIP_ONE, LSM_PAGE_SIZE, fp);
...
static void pad_file_to_page_boundary(FILE *fp, uint32_t next_page) {
    ...
    long expected_size = (long)(next_page - SKIP_ONE) * LSM_PAGE_SIZE;
...
static int build_node_indexes(FILE *fp, uint32_t *next_page, LSMDumpNode *nodes, int node_count,
...
static int build_edge_indexes(FILE *fp, uint32_t *next_page, LSMDumpEdge *edges, int edge_count,
...
static void parallel_sort_indexes(SortJob *nsorts, int n_node, SortJob *esorts, int n_edge) {
    lsm_thread_t st[TOTAL_SORT_THREADS];
    int nt = 0;
    for (int i = 0; i < n_node; i++) {
        if (nsorts[i].count > 0) {
            lsm_thread_create(&st[nt++], 0, sort_worker, &nsorts[i]);
        }
    }
    for (int i = 0; i < n_edge; i++) {
        if (esorts[i].count > 0) {
            lsm_thread_create(&st[nt++], 0, sort_worker, &esorts[i]);
        }
    }
    for (int i = 0; i < nt; i++) {
        lsm_thread_join(&st[i]);
    }
}
...
static int write_db_after_nodes(write_db_ctx_t *w, uint32_t nodes_root) {
    ...
    LSM_PROF_END_N("write_db", "1_data_tables", t_data, node_count + edge_count);

    // Phase 2: Metadata tables (projects, file_hashes, summaries, sqlite_sequence)
    LSM_PROF_START(t_meta);
    ...
    LSM_PROF_END("write_db", "2_metadata_tables", t_meta);
    ...
    LSM_PROF_END_N("write_db", "3_parallel_sort_indexes", t_sort, node_count + edge_count);

    /* Phase 4-5: Build node + edge index B-trees */
    LSM_PROF_START(t_node_idx);
    ...
    LSM_PROF_END_N("write_db", "4_node_indexes_seq", t_node_idx, node_count * NODE_SORT_THREADS);
    if (nrc != 0) {
        return discard_writer_output(w, nrc);
    }

    LSM_PROF_START(t_edge_idx);
    ...
    LSM_PROF_END_N("write_db", "5_edge_indexes_seq", t_edge_idx, edge_count * EDGE_SORT_THREADS);
    if (erc != 0) {
...
lsm_db_writer_t *lsm_writer_open(const char *path) {
    lsm_db_writer_t *w = (lsm_db_writer_t *)calloc(LSM_ALLOC_ONE, sizeof(*w));
    if (!w) {
        return NULL;
    }
    ...
    FILE *fp = lsm_fopen(w->wc.temp_path, "wb");
    if (!fp) {
        (void)lsm_unlink(w->wc.temp_path);
        free(w);
        return NULL;
    }
    w->wc.fp = fp;
...
int lsm_write_db(const char *path, const char *project, const char *root_path,
    ...
    (void)lsm_writer_append_nodes(w, nodes,
                                  node_count); /* error recorded in w, handled by finalize */
    return lsm_writer_finalize(w, project, root_path, indexed_at, nodes, node_count, edges,
