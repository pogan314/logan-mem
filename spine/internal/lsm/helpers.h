#ifndef LSM_HELPERS_H
#define LSM_HELPERS_H
...
// lsm_is_keyword: it only covers builtins with a target node, so un-filtering
// them cannot produce a node-less / Module-sourced edge. The Python set MUST
// stay in sync with kPyBuiltinNodes in internal/lsm/lsp/py_builtins.c.
bool lsm_is_resolvable_builtin(const char *name, LSMLanguage lang);

// Classify a string literal as URL, config, or neither.
// Returns LSM_STRREF_URL (0), LSM_STRREF_CONFIG (1), or -1 for neither.
...
#define LSM_DECLARATOR_DEPTH_LIMIT 8
...
// Free the calling thread's lsm_kind_in_set bitset cache (call at thread/process
...
// lsm_fqn_module returns (no behavior change).
...
// language this is exactly lsm_fqn_compute (no behavior change).
