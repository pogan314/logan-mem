...
void lsm_macro_table_add(LSMMacroTable *t, LSMArena *arena, const char *name, int param_count,
    ...
    if (t->count >= LSM_MACRO_TABLE_CAP || !name)
        return;
    ...
    LSMMacroEntry *e = &t->entries[t->count++];
    e->name = lsm_arena_strdup(arena, name);
    e->param_count = param_count;
    e->expansion = expansion ? lsm_arena_strdup(arena, expansion) : NULL;
    e->resolved_callee = resolved_callee ? lsm_arena_strdup(arena, resolved_callee) : NULL;
    for (int p = 0; p < LSM_MACRO_MAX_PARAMS; p++) {
        e->param_names[p] =
            (param_names && p < param_count) ? lsm_arena_strdup(arena, param_names[p]) : NULL;
    }
}
...
void lsm_parse_inc_file(LSMMacroTable *t, LSMArena *arena, const char *content) {
    ...
    while (*line) {
        ...
        if (strncmp(p, "#define", 7) == 0 && (p[7] == ' ' || p[7] == '\t')) {
            ...
            int param_count = -1;
            char param_names_buf[LSM_MACRO_MAX_PARAMS][64];
            const char *param_name_ptrs[LSM_MACRO_MAX_PARAMS] = {NULL};
            ...
            if (*p == '(') {
                ...
                while (*p && *p != ')' && p < end) {
                    ...
                    while (plen > 0 && (pn_start[plen - 1] == ' ' || pn_start[plen - 1] == '\t'))
                        plen--;
                    if (plen > 0 && param_count < LSM_MACRO_MAX_PARAMS) {
            ...
            char *expansion = NULL;
            if (explen > 0) {
                expansion = lsm_arena_strndup(arena, p, explen);
            }

            lsm_macro_table_add(t, arena, name, param_count,
...
char *lsm_macro_expand(LSMArena *arena, const LSMMacroEntry *entry, const char **args,
    ...
    while (*p && out < (int)sizeof(buf) - 1) {
        if (*p == '%') {
            bool matched = false;
            for (int i = 0; i < entry->param_count && i < LSM_MACRO_MAX_PARAMS; i++) {
    ...
    buf[out] = '\0';
    return lsm_arena_strdup(arena, buf);
}
...
char *lsm_macro_extract_callee(LSMArena *arena, const char *expansion) {
    ...
    if (p) {
        ...
        if (clen <= 0 || mlen <= 0)
            return NULL;
        return lsm_arena_sprintf(arena, "%.*s.%.*s", clen, p, mlen, method_start);
    }
    ...
    if (p && p[2] != '$') {
        ...
        if (*tag_end == '^') {
            ...
            if (tlen > 0 && rlen > 0) {
                return lsm_arena_sprintf(arena, "%.*s^%.*s", tlen, p, rlen, rtn);
            }
        }
    }
