...
static const char *unquote_string(LSMArena *a, const char *s) {
    if (!s) {
        return NULL;
    }
    size_t len = strlen(s);
    if (len < LSM_QUOTE_PAIR) {
        return NULL;
    }
    char first = s[0];
    char last = s[len - LSM_QUOTE_OFFSET];
    if ((first == '"' && last == '"') || (first == '\'' && last == '\'') ||
        (first == '`' && last == '`')) {
        return lsm_arena_strndup(a, s + LSM_QUOTE_OFFSET, len - LSM_QUOTE_PAIR);
    }
    return NULL;
}
...
static const char *literal_from_arg(LSMExtractCtx *ctx, TSNode arg) {
...
static const char *literal_from_first_child(LSMExtractCtx *ctx, TSNode node) {
...
static void scan_string_consts_js(LSMExtractCtx *ctx, chan_const_table_t *tbl) {
    TSNodeStack stack;
        ...
        if (strcmp(kind, "variable_declarator") == 0) {
            ...
            if (!ts_node_is_null(name_node) && !ts_node_is_null(value_node)) {
                ...
                if (strcmp(nk, "identifier") == 0 &&
                    ...
                    char *name_text = lsm_node_text(ctx->arena, name_node, ctx->source);
                    char *value_text = lsm_node_text(ctx->arena, value_node, ctx->source);
...
static void scan_string_consts_python(LSMExtractCtx *ctx, chan_const_table_t *tbl) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
    ...
    while (stack.count > 0 && tbl->count < CHAN_CONST_CAP) {
        ...
        if (strcmp(kind, "assignment") == 0) {
            ...
            if (!ts_node_is_null(left) && !ts_node_is_null(right) &&
                ...
                char *name = lsm_node_text(ctx->arena, left, ctx->source);
...
static const char *enclosing_function_qn(LSMExtractCtx *ctx, TSNode node) {
...
static const char *extract_channel_name(LSMExtractCtx *ctx, TSNode args,
    ...
    if (!channel_name && consts) {
        ...
        if (strcmp(kind, "identifier") == 0) {
            char *ident = lsm_node_text(ctx->arena, first, ctx->source);
...
static void push_channel(LSMExtractCtx *ctx, const char *channel_name, const char *transport,
                         LSMChannelDirection direction, TSNode call) {
    LSMChannel ch = {
    ...
    lsm_channels_push(&ctx->result->channels, ctx->arena, ch);
}
...
static const char *js_classify_receiver(LSMExtractCtx *ctx, TSNode object_node) {
    char *text = lsm_node_text(ctx->arena, object_node, ctx->source);
    if (!text) {
        return NULL;
    }
...
static void js_process_call(LSMExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = lsm_node_text(ctx->arena, property, ctx->source);
    ...
    if (strcmp(transport, "kafka") == 0) {
        if (js_is_kafka_send(method)) {
            direction = LSM_CHANNEL_EMIT;
        } else if (js_is_kafka_listen(method)) {
            direction = LSM_CHANNEL_LISTEN;
        } else {
            return;
        }
...
static void extract_channels_js(LSMExtractCtx *ctx) {
...
static const char *py_classify_receiver(LSMExtractCtx *ctx, TSNode object_node) {
    char *text = lsm_node_text(ctx->arena, object_node, ctx->source);
    if (!text) {
        return NULL;
    }
...
static const struct {
    ...
    {"kafka", "send", LSM_CHANNEL_EMIT},
    {"kafka", "produce", LSM_CHANNEL_EMIT},
    {"kafka", "subscribe", LSM_CHANNEL_LISTEN},
    {"kafka", "poll", LSM_CHANNEL_LISTEN},
    {"django_channels", "send", LSM_CHANNEL_EMIT},
    {"django_channels", "group_send", LSM_CHANNEL_EMIT},
    {"django_channels", "receive", LSM_CHANNEL_LISTEN},
    {"django_channels", "group_add", LSM_CHANNEL_LISTEN},
    {"websocket", "send", LSM_CHANNEL_EMIT},
    {"websocket", "send_text", LSM_CHANNEL_EMIT},
    {"websocket", "send_json", LSM_CHANNEL_EMIT},
    {"websocket", "send_bytes", LSM_CHANNEL_EMIT},
    {"websocket", "receive", LSM_CHANNEL_LISTEN},
    {"websocket", "receive_text", LSM_CHANNEL_LISTEN},
    {"websocket", "receive_json", LSM_CHANNEL_LISTEN},
    {"websocket", "receive_bytes", LSM_CHANNEL_LISTEN},
    {NULL, "emit", LSM_CHANNEL_EMIT},
    {NULL, "send", LSM_CHANNEL_EMIT},
    {NULL, "on", LSM_CHANNEL_LISTEN},
...
static void py_process_call(LSMExtractCtx *ctx, TSNode call, const chan_const_table_t *consts) {
    ...
    char *method = lsm_node_text(ctx->arena, attr, ctx->source);
    ...
    LSMChannelDirection direction = (LSMChannelDirection)dir;
...
static void py_process_decorator(LSMExtractCtx *ctx, TSNode decorator,
                                 const chan_const_table_t *consts) {
    ...
    if (strcmp(ek, "call") == 0) {
        ...
        push_channel(ctx, channel_name, transport, LSM_CHANNEL_LISTEN, decorator);
    }
}

static void extract_channels_python(LSMExtractCtx *ctx) {
    chan_const_table_t consts = {0};
...
static void go_process_call(LSMExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *method = lsm_node_text(ctx->arena, field, ctx->source);
    if (!method) {
        return;
    }
    ...
    char *recv = lsm_node_text(ctx->arena, operand, ctx->source);
    if (!recv) {
        return;
    }
...
static void extract_channels_go(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
...
static void java_process_call(LSMExtractCtx *ctx, TSNode call) {
    ...
    char *method = lsm_node_text(ctx->arena, func, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "convertAndSend") == 0 || strcmp(method, "convertAndSendToUser") == 0) {
        ...
        if (channel_name) {
            push_channel(ctx, channel_name, "spring_websocket", LSM_CHANNEL_EMIT, call);
        }
        return;
    }
    ...
    if (strcmp(method, "sendText") == 0 || strcmp(method, "sendObject") == 0 ||
        ...
        const char *func_name = enclosing_function_qn(ctx, call);
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", LSM_CHANNEL_EMIT, call);
        return;
    }
...
static void java_process_annotation(LSMExtractCtx *ctx, TSNode annotation) {
    ...
    if (strcmp(name, "OnMessage") == 0 || strcmp(name, "OnOpen") == 0 ||
        ...
        const char *channel_name = func_name ? func_name : "(websocket)";
        push_channel(ctx, channel_name, "websocket", LSM_CHANNEL_LISTEN, annotation);
        ...
        if (!ts_node_is_null(args)) {
            ...
            if (path) {
                push_channel(ctx, path, "spring_websocket", LSM_CHANNEL_LISTEN, annotation);
                return;
            }
        }
        push_channel(ctx, "(spring_ws)", "spring_websocket", LSM_CHANNEL_LISTEN, annotation);
    } else if (strcmp(name, "ServerEndpoint") == 0) {
        TSNode args = ts_node_child_by_field_name(annotation, TS_FIELD("arguments"));
        if (!ts_node_is_null(args)) {
            const char *path = extract_channel_name(ctx, args, NULL);
            if (path) {
                push_channel(ctx, path, "websocket", LSM_CHANNEL_LISTEN, annotation);
                return;
            }
        }
    }
...
static void extract_channels_java(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
...
static void csharp_process_call(LSMExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *method = lsm_node_text(ctx->arena, name_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "SendAsync") == 0 || strcmp(method, "SendCoreAsync") == 0) {
        char *full = lsm_node_text(ctx->arena, func, ctx->source);
        if (full && (strstr(full, "Clients") || strstr(full, "clients"))) {
            TSNode args = ts_node_child_by_field_name(call, TS_FIELD("arguments"));
            if (!ts_node_is_null(args)) {
                const char *channel_name = extract_channel_name(ctx, args, NULL);
                if (channel_name) {
                    push_channel(ctx, channel_name, "signalr", LSM_CHANNEL_EMIT, call);
                }
            }
        }
        return;
    ...
    if (strcmp(method, "On") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "signalr", LSM_CHANNEL_LISTEN, call);
            }
        }
    }
}

static void extract_channels_csharp(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
...
static void ruby_process_call(LSMExtractCtx *ctx, TSNode call) {
    ...
    char *method = lsm_node_text(ctx->arena, method_node, ctx->source);
    if (!method) {
        return;
    }
    ...
    if (strcmp(method, "broadcast") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", LSM_CHANNEL_EMIT, call);
            }
        }
        return;
    }
    ...
    if (strcmp(method, "stream_from") == 0 || strcmp(method, "stream_for") == 0) {
        ...
        if (!ts_node_is_null(args)) {
            const char *channel_name = extract_channel_name(ctx, args, NULL);
            if (channel_name) {
                push_channel(ctx, channel_name, "actioncable", LSM_CHANNEL_LISTEN, call);
            }
        }
    }
}

static void extract_channels_ruby(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
...
static const char *elixir_nth_arg_literal(LSMExtractCtx *ctx, TSNode args, uint32_t index) {
...
static void elixir_emit_second_arg(LSMExtractCtx *ctx, TSNode call, TSNode args,
                                   const char *transport, LSMChannelDirection direction) {
    if (ts_node_is_null(args)) {
        return;
    }
...
static void elixir_process_call(LSMExtractCtx *ctx, TSNode call) {
    TSNode target = ts_node_child_by_field_name(call, TS_FIELD("target"));
    if (ts_node_is_null(target)) {
        return;
    }
    char *target_text = lsm_node_text(ctx->arena, target, ctx->source);
    if (!target_text) {
        return;
    }
    ...
    if (strstr(target_text, "PubSub.broadcast") || strstr(target_text, "PubSub.local_broadcast")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", LSM_CHANNEL_EMIT);
        return;
    }
    if (strstr(target_text, "PubSub.subscribe")) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_pubsub", LSM_CHANNEL_LISTEN);
        return;
    }
    if (strcmp(target_text, "push") == 0 || strcmp(target_text, "broadcast") == 0 ||
        strcmp(target_text, "broadcast!") == 0) {
        elixir_emit_second_arg(ctx, call, args, "phoenix_channel", LSM_CHANNEL_EMIT);
    }
}

/* Detect handle_in("event", payload, socket) — Phoenix Channel listener */
static void elixir_process_function_def(LSMExtractCtx *ctx, TSNode func_def) {
    ...
    if (val) {
        push_channel(ctx, val, "phoenix_channel", LSM_CHANNEL_LISTEN, func_def);
    }
}

static void extract_channels_elixir(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
...
static void rust_process_call(LSMExtractCtx *ctx, TSNode call) {
    TSNode func = ts_node_child_by_field_name(call, TS_FIELD("function"));
    if (ts_node_is_null(func)) {
        return;
    }
    ...
    char *recv = lsm_node_text(ctx->arena, value, ctx->source);
    if (!recv) {
        return;
    }
    const char *tail = recv;
...
static void extract_channels_rust(LSMExtractCtx *ctx) {
    TSNodeStack stack;
    ts_nstack_init(&stack, ctx->arena, CHAN_STACK_CAP);
    ts_nstack_push(&stack, ctx->arena, ctx->root);
