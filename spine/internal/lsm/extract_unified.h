#ifndef LSM_EXTRACT_UNIFIED_H
#define LSM_EXTRACT_UNIFIED_H
...
typedef enum {
    LSM_INVOCATION_NONE = 0,
    LSM_INVOCATION_PRIMARY,
    LSM_INVOCATION_CALLABLE_REFERENCE,
} LSMInvocationKind;

typedef struct {
    LSMInvocationKind kind;
    TSNode site;
    TSNode callee_expr;
    TSNode callee_leaf;
    const char *callee_name;
    bool raw_call_emitted;
} LSMInvocationDescriptor;
...
typedef struct {
    ...
    uint8_t kind;
    LSMInvocationKind invocation_kind;
    TSNode callee_expr;
    TSNode callee_leaf;
    ...
    LSMInvocationKind prev_invocation_kind;
...
typedef enum {
    LSM_LEXICAL_SCOPE_MODULE = 0,
    LSM_LEXICAL_SCOPE_CLASS,
    LSM_LEXICAL_SCOPE_FUNCTION,
    LSM_LEXICAL_SCOPE_BLOCK,
    LSM_LEXICAL_SCOPE_COMPREHENSION,
} LSMLexicalScopeKind;
...
typedef enum {
    LSM_PYTHON_DIRECTIVE_GLOBAL = 1,
    LSM_PYTHON_DIRECTIVE_NONLOCAL,
} LSMPythonDirectiveKind;
...
typedef struct {
    ...
    LSMInvocationKind invocation_kind;  // exact active invocation/reference role
    ...
    LSMArena *arena;
    LSMWalkScope *scopes;
    LSMWalkScope inline_scopes[MAX_SCOPES];
    int scope_capacity;
    int scope_top;

    LSMLexicalScope *lexical_scopes;
    LSMLexicalScope inline_lexical_scopes[INLINE_LEXICAL_SCOPES];
    ...
    LSMLexicalBinding *lexical_bindings;
    LSMLexicalBinding inline_lexical_bindings[INLINE_LEXICAL_BINDINGS];
    int lexical_binding_capacity;
    int lexical_binding_count;
    int usage_start_index;
    bool lexical_binding_tracking_failed;
    LSMPythonDirective *python_directives;
    LSMPythonDirective inline_python_directives[INLINE_PYTHON_DIRECTIVES];
...
void handle_env_accesses(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                         WalkState *state);
void handle_type_assigns(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                         WalkState *state);
