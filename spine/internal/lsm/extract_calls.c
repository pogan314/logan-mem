#include "lsm.h"
#include "arena.h" // LSMArena, lsm_arena_sprintf
#include "helpers.h"
...
#include "service_patterns.h" // lsm_service_pattern_route_method (#952)
...
enum { MAX_STRING_ARG_LEN = LSM_SZ_512 };
...
static const char *lookup_string_constant(const LSMExtractCtx *ctx, const char *name) {
    if (!name || !name[0]) {
        return NULL;
    }
    const LSMStringConstantMap *map = &ctx->string_constants;
...
static const char *strip_quotes(LSMArena *a, const char *text) {
    if (!text || !text[0]) {
        return NULL;
    }
    int len = (int)strlen(text);
    if (len >= LSM_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        return lsm_arena_strndup(a, text + LSM_QUOTE_OFFSET, (size_t)(len - LSM_QUOTE_PAIR));
    }
    return text;
}
...
// rather than in LSMLangSpec to avoid -Wmissing-field-initializers across 155
// language rows.
...
static char *extract_callee_name(LSMArena *a, TSNode node, const char *source, LSMLanguage lang);
static char *gotemplate_callee(LSMArena *a, TSNode node, const char *source);
static const char *strip_and_validate_string_arg(LSMArena *a, char *text);
 ...
 * Without this, lsm_node_text returns full text including args/parens.
 * Iteratively walks the chain: selector → operand(call) → function(selector) → ... */
static char *resolve_chained_selector(LSMArena *a, TSNode sel, const char *source) {
    ...
    if (ts_node_is_null(operand) || ts_node_is_null(field) ||
        ...
        return lsm_node_text(a, sel, source);
    }
    ...
    char *method = lsm_node_text(a, field, source);
    ...
    for (int depth = 0; depth < MAX_CHAIN_DEPTH; depth++) {
        ...
        char *base = lsm_node_text(a, fn, source);
        if (base && method) {
            return lsm_arena_sprintf(a, "%s.%s", base, method);
        }
        return method;
    }
...
static char *extract_constructor_callee(LSMArena *a, TSNode node, const char *source,
                                        const char *nk) {
    ...
    for (const char **f = type_fields; *f; f++) {
        ...
        if (!ts_node_is_null(tn)) {
            ...
            char *t = strip_generic_args(lsm_node_text(a, tn, source));
            if (t && t[0]) {
                return t;
            }
        }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ck, "type_identifier") == 0 || strcmp(ck, "identifier") == 0 ||
            ...
            char *t = strip_generic_args(lsm_node_text(a, child, source));
            if (t && t[0]) {
                return t;
            }
        }
...
static char *extract_callee_from_fields(LSMArena *a, TSNode node, const char *source) {
    // Try "function" field
    ...
    if (!ts_node_is_null(func_node)) {
        ...
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "simple_identifier") == 0 ||
            ...
            return lsm_node_text(a, func_node, source);
        }
        ...
        if (strcmp(fk, "extract_operator") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                char *rt = lsm_node_text(a, rhs, source);
                if (!ts_node_is_null(lhs)) {
                    char *lt = lsm_node_text(a, lhs, source);
                    if (lt && lt[0] && rt && rt[0]) {
                        return lsm_arena_sprintf(a, "%s.%s", lt, rt);
                    }
                }
                return rt;
            }
    ...
    if (!ts_node_is_null(name_node)) {
        ...
        if (!ts_node_is_null(obj) && name) {
            char *obj_text = lsm_node_text(a, obj, source);
            if (obj_text && obj_text[0]) {
                return lsm_arena_sprintf(a, "%s.%s", obj_text, name);
            }
        }
        return name;
    }
    ...
    if (!ts_node_is_null(method_node)) {
        char *method = lsm_node_text(a, method_node, source);
        TSNode recv = ts_node_child_by_field_name(node, TS_FIELD("receiver"));
        if (!ts_node_is_null(recv) && method) {
            char *recv_text = lsm_node_text(a, recv, source);
            if (recv_text && recv_text[0]) {
                return lsm_arena_sprintf(a, "%s.%s", recv_text, method);
            }
        }
        return method;
    }
...
static char *extract_fp_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    while (fp_is_apply_kind(nk) && ts_node_child_count(node) > 0) {
        ...
        if (strcmp(ck, "identifier") == 0 || strcmp(ck, "variable") == 0 ||
            ...
            return lsm_node_text(a, callee, source);
        }
    ...
    if (strcmp(nk, "infix") == 0 || strcmp(nk, "infix_expression") == 0) {
        ...
        if (!ts_node_is_null(op)) {
            return lsm_node_text(a, op, source);
        }
        enum { INFIX_MIN_CHILDREN = 3, INFIX_OP_IDX = 1 };
        if (ts_node_child_count(node) >= INFIX_MIN_CHILDREN) {
            return lsm_node_text(a, ts_node_child(node, INFIX_OP_IDX), source);
        }
    }
    return NULL;
}
...
static char *extract_wolfram_callee(LSMArena *a, TSNode node, const char *source) {
    ...
    if (ts_node_named_child_count(node) > 0) {
        ...
        if (strcmp(hk, "user_symbol") == 0 || strcmp(hk, "builtin_symbol") == 0) {
            return lsm_node_text(a, head, source);
        }
    }
    return NULL;
}
...
static char *extract_swift_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_scripting_callee(LSMArena *a, TSNode node, const char *source,
                                      LSMLanguage lang, const char *nk) {
    if (lang == LSM_LANG_ELIXIR && strcmp(nk, "binary_operator") == 0) {
        ...
        char *operator_name = lsm_node_text(a, op, source);
    ...
    if (lang == LSM_LANG_ELIXIR && strcmp(nk, "call") == 0 && ts_node_child_count(node) > 0) {
        TSNode first = ts_node_child(node, 0);
        const char *fk = ts_node_type(first);
        if (strcmp(fk, "identifier") == 0 || strcmp(fk, "dot") == 0) {
            return lsm_node_text(a, first, source);
        }
        return NULL;
    }
    if (lang == LSM_LANG_PERL && ts_node_child_count(node) > 0) {
        ...
        char *pn = lsm_node_text(a, name_node, source);
    ...
    if (lang == LSM_LANG_PHP) {
        ...
        char *pn = ts_node_is_null(func_node) ? NULL : lsm_node_text(a, func_node, source);
        return pn;
    }
    if (lang == LSM_LANG_KOTLIN && ts_node_child_count(node) > 0) {
        return lsm_node_text(a, ts_node_child(node, 0), source);
    }
    if (lang == LSM_LANG_MATLAB && strcmp(nk, "command") == 0 && ts_node_child_count(node) > 0) {
        return lsm_node_text(a, ts_node_child(node, 0), source);
    }
    return NULL;
}
...
static char *extract_objc_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "message_expression") != 0) {
        return NULL;
    }
    TSNode selector = ts_node_child_by_field_name(node, TS_FIELD("selector"));
    return ts_node_is_null(selector) ? NULL : lsm_node_text(a, selector, source);
}

// Erlang: extract callee from call node's first child.
static char *extract_erlang_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static bool lisp_definition_head(LSMLanguage lang, TSNode head, const char *source) {
    ...
    return call_node_text_in(head, source,
                             lang == LSM_LANG_COMMONLISP ? common_lisp_heads
                                                         : clojure_scheme_racket_heads);
}

static bool lisp_list_is_definition_role(LSMLanguage lang, TSNode node, const char *source) {
    ...
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
        ...
        if (lang == LSM_LANG_COMMONLISP &&
            (strcmp(parent_kind, "defun_header") == 0 || strcmp(parent_kind, "lambda_list") == 0)) {
            return true;
        }
        if (lang == LSM_LANG_EMACSLISP && (strcmp(parent_kind, "function_definition") == 0 ||
...
static bool call_node_is_definition_container(LSMLanguage lang, TSNode node, const char *source) {
    const char *kind = ts_node_type(node);
    if ((lang == LSM_LANG_CLOJURE || lang == LSM_LANG_SCHEME || lang == LSM_LANG_RACKET ||
         lang == LSM_LANG_COMMONLISP || lang == LSM_LANG_EMACSLISP) &&
    ...
    if (lang == LSM_LANG_TYPST && strcmp(kind, "call") == 0) {
        return typst_call_is_let_pattern(node);
    }
    if (lang == LSM_LANG_AGDA && strcmp(kind, "expr") == 0) {
        return agda_expr_is_definition_role(node);
    }
    return lang == LSM_LANG_ELIXIR && strcmp(kind, "call") == 0 &&
...
static char *extract_lisp_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_fsharp_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_css_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    TSNode fn = lsm_find_child_by_kind(node, "function_name");
    return ts_node_is_null(fn) ? NULL : lsm_node_text(a, fn, source);
}
...
static char *extract_linkerscript_callee(LSMArena *a, TSNode node, const char *source,
                                         const char *nk) {
    if (strcmp(nk, "call_expression") != 0) {
        return NULL;
    }
    ...
    return lsm_node_text(a, function, source);
}

// PowerShell: a `command` node's callee is its `command_name` child.
static char *extract_powershell_callee(LSMArena *a, TSNode node, const char *source,
                                       const char *nk) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (strcmp(ts_node_type(c), "command_name") == 0) {
            return lsm_node_text(a, c, source);
        }
    }
    return NULL;
}
...
static char *extract_ada_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_solidity_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_groovy_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_wgsl_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_dart_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "selector") == 0) {
        ...
        if (!ts_node_is_null(prev) && strcmp(ts_node_type(prev), "identifier") == 0) {
            return lsm_node_text(a, prev, source);
        }
        return NULL;
    }
...
static char *extract_scss_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        TSNode id = lsm_find_child_by_kind(node, "identifier");
        return ts_node_is_null(id) ? NULL : lsm_node_text(a, id, source);
    }
    ...
    if (strcmp(nk, "call_expression") == 0) {
        TSNode fn = lsm_find_child_by_kind(node, "function_name");
        if (!ts_node_is_null(fn)) {
            return lsm_node_text(a, fn, source);
        }
    }
    return NULL;
}
...
static char *extract_sql_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    return ts_node_is_null(nm) ? NULL : lsm_node_text(a, nm, source);
}
...
static char *extract_cobol_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call_statement") != 0) {
        return NULL;
    }
    TSNode x = ts_node_child_by_field_name(node, TS_FIELD("x"));
    if (ts_node_is_null(x)) {
        x = lsm_find_child_by_kind(node, "string");
    }
    if (ts_node_is_null(x)) {
        return NULL;
    }
    char *text = lsm_node_text(a, x, source);
...
static char *extract_elm_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    TSNode ve = strcmp(ts_node_type(target), "value_expr") == 0
                    ? target
                    : lsm_find_child_by_kind(target, "value_expr");
    if (ts_node_is_null(ve)) {
        return NULL;
    }
    TSNode qid = ts_node_child_by_field_name(ve, TS_FIELD("name"));
    if (ts_node_is_null(qid)) {
        qid = lsm_find_child_by_kind(ve, "value_qid");
    }
    if (ts_node_is_null(qid)) {
        return NULL;
    }
    TSNode id = lsm_find_child_by_kind(qid, "lower_case_identifier");
    if (ts_node_is_null(id)) {
        // module-qualified call: emit the whole qualified id text
        return lsm_node_text(a, qid, source);
    }
    return lsm_node_text(a, id, source);
}
...
static char *extract_jsonnet_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "functioncall") != 0) {
        return NULL;
    }
    TSNode id = lsm_find_child_by_kind(node, "id");
    return ts_node_is_null(id) ? NULL : lsm_node_text(a, id, source);
}
...
// nickel_call_types.)
static char *extract_nickel_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    for (int depth = 0; depth < NICKEL_APPLY_DEPTH && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(ck, "ident") == 0) {
            return lsm_node_text(a, cur, source);
        }
...
static char *extract_typst_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "call") != 0) {
        return NULL;
    }
    TSNode item = ts_node_child_by_field_name(node, TS_FIELD("item"));
    return ts_node_is_null(item) ? NULL : lsm_node_text(a, item, source);
}

// Meson: a builtin invocation (`executable(...)`, `dependency(...)`) is a
// `normal_command` whose `command` field is the called identifier.
static char *extract_meson_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "normal_command") != 0) {
        return NULL;
    }
    TSNode cmd = ts_node_child_by_field_name(node, TS_FIELD("command"));
    return ts_node_is_null(cmd) ? NULL : lsm_node_text(a, cmd, source);
}
...
static char *first_leaf_identifier(LSMArena *a, TSNode node, const char *source) {
    TSNode cur = node;
    for (int depth = 0; depth < 8 && !ts_node_is_null(cur); depth++) {
        ...
        if (strcmp(k, "simple_identifier") == 0 || strcmp(k, "identifier") == 0 ||
            ...
            char *t = lsm_node_text(a, cur, source);
            return (t && t[0]) ? t : NULL;
        }
...
static char *extract_hdl_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_vhdl_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (strcmp(pk, "library_function") == 0 || strcmp(pk, "identifier") == 0 ||
        ...
        char *t = lsm_node_text(a, prev, source);
        return (t && t[0]) ? t : NULL;
    }
    return NULL;
}
...
static char *extract_nasm_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    char *m = lsm_node_text(a, mnem, source);
...
static char *extract_llvm_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    ...
    if (!t) {
        t = lsm_node_text(a, callee, source);
    }
    if (t && (t[0] == '@' || t[0] == '%')) {
        return t + 1;
    }
...
static char *extract_func_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_nix_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_agda_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "expr") != 0) {
        return NULL;
    }
    TSNode qid = agda_head_qid(agda_application_head_atom(node));
    return ts_node_is_null(qid) ? NULL : lsm_node_text(a, qid, source);
}
...
static char *extract_make_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "shell_function") == 0) {
        return lsm_arena_strndup(a, "shell", 5);
    }
...
static char *extract_just_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
...
static char *extract_puppet_callee(LSMArena *a, TSNode node, const char *source, const char *nk) {
    if (strcmp(nk, "include_statement") == 0) {
        return lsm_arena_strndup(a, "include", 7);
    }
    if (strcmp(nk, "function_call") == 0) {
        if (ts_node_named_child_count(node) > 0) {
            TSNode head = ts_node_named_child(node, 0);
...
static char *extract_callee_lang_specific(LSMArena *a, TSNode node, const char *source,
                                          LSMLanguage lang) {
    const char *nk = ts_node_type(node);

    if (lang == LSM_LANG_FORTRAN && strcmp(nk, "subroutine_call") == 0) {
        TSNode subroutine = ts_node_child_by_field_name(node, TS_FIELD("subroutine"));
        if (!ts_node_is_null(subroutine)) {
            return lsm_node_text(a, subroutine, source);
        }
    }
    ...
    if (lang == LSM_LANG_PYTHON && strcmp(nk, "call") == 0) {
    ...
    if (lang == LSM_LANG_ERLANG) {
        return extract_erlang_callee(a, node, source, nk);
    }
    if (lang == LSM_LANG_HASKELL || lang == LSM_LANG_OCAML || lang == LSM_LANG_PURESCRIPT ||
        lang == LSM_LANG_SCALA) {
        return extract_fp_callee(a, node, source, nk);
    }
    if (lang == LSM_LANG_WOLFRAM && strcmp(nk, "apply") == 0) {
        return extract_wolfram_callee(a, node, source);
    }
    if (lang == LSM_LANG_SWIFT) {
    ...
    if (lang == LSM_LANG_OBJECTSCRIPT_UDL || lang == LSM_LANG_OBJECTSCRIPT_ROUTINE) {
        ...
        if (strcmp(nk, "class_method_call") == 0) {
            TSNode class_ref = lsm_find_child_by_kind(node, "class_ref");
            TSNode method_name = lsm_find_child_by_kind(node, "method_name");
            if (!ts_node_is_null(class_ref) && !ts_node_is_null(method_name)) {
                TSNode cname = lsm_find_child_by_kind(class_ref, "class_name");
                if (ts_node_is_null(cname)) {
                    return NULL;
                }
                char *cls = lsm_node_text(a, cname, source);
                if (!cls || !cls[0]) {
                    return NULL;
                }
                ...
                char *meth = lsm_node_text(a, mname_ident, source);
                if (!meth || !meth[0]) {
                    return cls;
                }
                return lsm_arena_sprintf(a, "%s.%s", cls, meth);
            }
            return NULL;
        }
        ...
        if (strcmp(nk, "extrinsic_function") == 0 || strcmp(nk, "routine_tag_call") == 0) {
            TSNode line_ref = lsm_find_child_by_kind(node, "line_ref");
            if (!ts_node_is_null(line_ref)) {
                return lsm_node_text(a, line_ref, source);
            }
            return NULL;
        }
        // $$$Macro(...) -> raw "$$$Name" callee (expanded later in handle_calls)
        if (strcmp(nk, "macro") == 0) {
            char *raw = lsm_node_text(a, node, source);
            ...
            return lsm_arena_sprintf(a, "$$$%s", name_start);
        }
        return NULL;
    }
...
static const char *php_chain_prefix_arg(LSMArena *a, TSNode call_node, const char *source) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(ts_node_type(inner), "string") == 0 ||
            ...
            char *txt = lsm_node_text(a, inner, source);
            if (txt && txt[0]) {
...
static const char *php_group_prefix_for_call(LSMArena *a, TSNode node, const char *source) {
    ...
    for (int depth = 0; depth < PHP_GROUP_WALK_MAX && !ts_node_is_null(cur); depth++) {
        if (strcmp(ts_node_type(cur), "anonymous_function") == 0 ||
            ...
            if (!ts_node_is_null(p) && (strcmp(ts_node_type(p), "member_call_expression") == 0 ||
                ...
                char *gtxt = ts_node_is_null(gname) ? NULL : lsm_node_text(a, gname, source);
                if (gtxt && strcmp(gtxt, "group") == 0) {
                    ...
                    for (int hops = 0; hops < PHP_GROUP_WALK_MAX && !ts_node_is_null(recv);
                        ...
                        if (strcmp(rk, "member_call_expression") == 0 ||
                            ...
                            char *rtxt =
                                ts_node_is_null(rname) ? NULL : lsm_node_text(a, rname, source);
    ...
    buf[pos] = '\0';
    return pos ? lsm_arena_strndup(a, buf, pos) : NULL;
}

static bool is_nested_verilog_call_wrapper(LSMLanguage lang, TSNode node) {
    if (lang != LSM_LANG_VERILOG || strcmp(ts_node_type(node), "subroutine_call") != 0) {
        return false;
    }
...
static char *extract_callee_name(LSMArena *a, TSNode node, const char *source, LSMLanguage lang) {
    ...
    if (lang == LSM_LANG_LEAN && strcmp(ts_node_type(node), "apply") == 0) {
    ...
    if (lang == LSM_LANG_RUBY) {
        ...
        if (!ts_node_is_null(m) && !ts_node_is_null(recv) &&
            ...
            char *mt = lsm_node_text(a, m, source);
            if (mt && strcmp(mt, "new") == 0) {
                char *rt = lsm_node_text(a, recv, source);
                if (rt && rt[0]) {
                    return rt;
                }
            }
     ...
     * not recognized. */
    if (lang == LSM_LANG_PHP && strcmp(ts_node_type(node), "scoped_call_expression") == 0) {
        ...
        if (!ts_node_is_null(scope) && !ts_node_is_null(mname)) {
            char *sc = lsm_node_text(a, scope, source);
            char *mn = lsm_node_text(a, mname, source);
            if (sc && mn && strcmp(sc, "Route") == 0) {
                char *qual = lsm_arena_sprintf(a, "%s::%s", sc, mn);
                if (qual && lsm_service_pattern_route_method(qual) != NULL) {
                    return qual;
                }
            }
        }
...
static const char *strip_and_validate_string_arg(LSMArena *a, char *text) {
    ...
    int len = (int)strlen(text);
    if (len >= LSM_QUOTE_PAIR && (text[0] == '"' || text[0] == '\'')) {
        text = lsm_arena_strndup(a, text + LSM_QUOTE_OFFSET, (size_t)(len - LSM_QUOTE_PAIR));
        len -= LSM_QUOTE_PAIR;
    }
...
static char *gotemplate_string_child(LSMArena *a, TSNode parent, const char *source) {
    TSNode s = lsm_find_child_by_kind(parent, "interpreted_string_literal");
    if (ts_node_is_null(s)) {
        return NULL;
    }
    char *text = lsm_node_text(a, s, source);
...
static char *gotemplate_callee(LSMArena *a, TSNode node, const char *source) {
    ...
    if (strcmp(k, "function_call") == 0) {
        TSNode fn = lsm_find_child_by_kind(node, "identifier");
        if (ts_node_is_null(fn)) {
            return NULL;
        }
        char *fname = lsm_node_text(a, fn, source);
        ...
        if (ts_node_is_null(args)) {
            args = lsm_find_child_by_kind(node, "argument_list");
        }
        if (ts_node_is_null(args)) {
            return NULL;
        }
...
static const char *extract_nth_string_arg(LSMExtractCtx *ctx, TSNode args, uint32_t n) {
    uint32_t nc = ts_node_named_child_count(args);
    uint32_t found = 0;
    for (uint32_t ai = 0; ai < nc && ai < MAX_POSITIONAL_SCAN + n; ai++) {
        ...
        if (is_string_like(ak)) {
            if (found == n) {
                char *text = lsm_node_text(ctx->arena, arg, ctx->source);
...
static void process_keyword_arg(LSMExtractCtx *ctx, TSNode arg_node, LSMCallArg *ca) {
    ...
    if (!ts_node_is_null(key_n)) {
        ca->keyword = lsm_node_text(ctx->arena, key_n, ctx->source);
    }
    if (!ts_node_is_null(val_n)) {
        ca->expr = lsm_node_text(ctx->arena, val_n, ctx->source);
...
static void extract_call_args(LSMExtractCtx *ctx, TSNode args, LSMCall *call) {
    uint32_t argc = ts_node_named_child_count(args);
    int positional_idx = 0;
    for (uint32_t ai = 0; ai < argc && call->arg_count < LSM_MAX_CALL_ARGS; ai++) {
        TSNode arg_node = ts_node_named_child(args, ai);
        const char *ak = ts_node_type(arg_node);
        LSMCallArg *ca = &call->args[call->arg_count];
        ...
        if (strcmp(ak, "keyword_argument") == 0 || strcmp(ak, "pair") == 0) {
            ...
            ca->expr = lsm_node_text(ctx->arena, arg_node, ctx->source);
            ca->index = positional_idx++;
...
static const char *extract_string_value(LSMExtractCtx *ctx, TSNode val_node) {
    const char *vk = ts_node_type(val_node);
    if (strcmp(vk, "template_string") == 0) {
        return lsm_template_string_text(ctx->arena, val_node, ctx->source);
    }
    if (is_string_like(vk)) {
        char *text = lsm_node_text(ctx->arena, val_node, ctx->source);
        if (text && text[0]) {
            return strip_quotes(ctx->arena, text);
        }
    } else if (strcmp(vk, "identifier") == 0) {
        char *const_name = lsm_node_text(ctx->arena, val_node, ctx->source);
        if (const_name) {
...
static const char *extract_composite_queue_field(LSMExtractCtx *ctx, TSNode node) {
    ...
    if (ts_node_is_null(body)) {
        body = lsm_find_child_by_kind(node, "literal_value");
    }
    if (ts_node_is_null(body)) {
        return NULL;
    }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        char *key = lsm_node_text(ctx->arena, key_n, ctx->source);
        ...
        char *raw = lsm_node_text(ctx->arena, val_n, ctx->source);
        if (raw && raw[0]) {
            return raw;
        }
    }
...
static const char *extract_keyword_url(LSMExtractCtx *ctx, TSNode arg) {
    ...
    char *key = lsm_node_text(ctx->arena, key_node, ctx->source);
...
static const char *extract_binary_concat_suffix(LSMExtractCtx *ctx, TSNode node) {
    TSNode op_node = ts_node_child_by_field_name(node, TS_FIELD("operator"));
    if (!ts_node_is_null(op_node)) {
        char *op = lsm_node_text(ctx->arena, op_node, ctx->source);
        if (!op || strcmp(op, "+") != 0) {
            return NULL;
        }
    }
    ...
    return strip_and_validate_string_arg(ctx->arena, lsm_node_text(ctx->arena, rhs, ctx->source));
}

// Try to extract URL/topic from a positional argument (string or constant).
static const char *extract_positional_url(LSMExtractCtx *ctx, TSNode arg, const char *ak) {
    ...
    if (strcmp(ak, "template_string") == 0) {
        const char *flat = lsm_template_string_text(ctx->arena, arg, ctx->source);
        if (flat) {
    ...
    if (strcmp(ak, "identifier") == 0) {
        char *const_name = lsm_node_text(ctx->arena, arg, ctx->source);
        if (const_name) {
            return lookup_string_constant(ctx, const_name);
        }
    }
...
static const char *extract_url_or_topic_arg(LSMExtractCtx *ctx, TSNode args) {
    uint32_t nc = ts_node_named_child_count(args);
...
static const char *normalize_string_handler(LSMArena *a, const char *raw) {
    ...
    if (at && at[1]) {
        return lsm_arena_strdup(a, at + 1);
    }
    return unq;
}

static const char *extract_handler_arg(LSMExtractCtx *ctx, TSNode args) {
    uint32_t nc = ts_node_named_child_count(args);
    for (uint32_t ai = HANDLER_START_IDX; ai < nc && ai < MAX_HANDLER_SCAN; ai++) {
        ...
        if (strcmp(ak2, "identifier") == 0 || strcmp(ak2, "member_expression") == 0 ||
            ...
            return lsm_node_text(ctx->arena, arg2, ctx->source);
        }
        if (is_string_like(ak2)) {
            const char *h =
                normalize_string_handler(ctx->arena, lsm_node_text(ctx->arena, arg2, ctx->source));
            if (h && h[0]) {
                return h;
            }
        }
...
static void extract_jsx_component_ref(LSMExtractCtx *ctx, TSNode node, const char *kind,
    ...
    if (name && name[0] >= 'A' && name[0] <= 'Z') {
        ...
        lsm_calls_push(&ctx->result->calls, ctx->arena, call);
    }
}
...
static bool kotlin_operator_call_matches(const LSMCall *existing, const char *callee_name,
...
static void push_kotlin_operator_call(LSMExtractCtx *ctx, TSNode node, const char *callee_name,
                                      const char *enclosing_func_qn) {
    ...
    if (ctx->result->calls.count > 0) {
        const LSMCall *existing = &ctx->result->calls.items[ctx->result->calls.count - SKIP_ONE];
...
static void extract_kotlin_operator_call(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                         const char *enclosing_func_qn) {
...
static void kt_push_implicit_call(LSMExtractCtx *ctx, TSNode node, const char *callee,
                                  const char *enclosing_func_qn) {
    LSMCall call = {0};
...
static void push_cpp_semantic_call_candidate(LSMExtractCtx *ctx, TSNode node,
...
static const char *cpp_operator_token(LSMExtractCtx *ctx, TSNode node) {
    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
        TSNode child = ts_node_child(node, i);
        if (!ts_node_is_named(child)) {
            const char *token = lsm_node_text(ctx->arena, child, ctx->source);
            if (token && token[0]) {
                return token;
            }
        }
...
static void extract_cpp_operator_call(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                      const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "unary_expression") == 0 || strcmp(kind, "pointer_expression") == 0 ||
        ...
        if (!ts_node_is_null(operand) &&
            ...
            push_cpp_semantic_call_candidate(
                ctx, node, lsm_arena_sprintf(ctx->arena, "operator%s", token), enclosing_func_qn);
        }
        return;
    }
...
static void extract_cpp_implicit_calls(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                       const char *enclosing_func_qn) {
    ...
    if (strcmp(kind, "delete_expression") == 0) {
        ...
        if (!ts_node_is_null(type) && !ts_node_is_null(decl) &&
            ...
            if (!ts_node_is_null(value) && strcmp(ts_node_type(value), "identifier") == 0) {
                char *tn = lsm_node_text(ctx->arena, type, ctx->source);
                if (tn) {
...
static void extract_kotlin_desugared_calls(LSMExtractCtx *ctx, TSNode node, const char *kind,
                                           const char *enclosing_func_qn) {
    if (strcmp(kind, "property_declaration") == 0) {
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            for (uint32_t j = 0; j < vc; j++) {
                ...
                comp++;
                kt_push_implicit_call(ctx, node, lsm_arena_sprintf(ctx->arena, "component%u", comp),
                                      enclosing_func_qn);
            }
            break;
        }
...
static char *resolve_objectscript_instance_call(LSMArena *a, TSNode node, const char *source,
    ...
    TSNode method_name_node = lsm_find_child_by_kind(oref, "method_name");
    ...
    char *method = lsm_node_text(a, mn_ident, source);
    if (!method || !method[0]) {
        return NULL;
    }
    ...
    if (nk_first && strcmp(nk_first, "relative_dot_property") == 0) {
        TSNode prop_name = lsm_find_child_by_kind(receiver, "member_name");
        if (!ts_node_is_null(prop_name)) {
            char *pname = lsm_node_text(a, prop_name, source);
            if (pname && pname[0]) {
                var_text = lsm_arena_sprintf(a, "..%s", pname);
            }
        }
        if (!var_text) {
            var_text = lsm_node_text(a, receiver, source);
        }
    } else {
        var_text = lsm_node_text(a, receiver, source);
    }
    ...
    for (int i = 0; i < type_map->count; i++) {
        if (strcasecmp(type_map->entries[i].var_name, var_text) == 0) {
            return lsm_arena_sprintf(a, "%s.%s", type_map->entries[i].class_name, method);
        }
    }
    return NULL;
}

static bool is_objectscript_language(LSMLanguage language) {
    return language == LSM_LANG_OBJECTSCRIPT_UDL || language == LSM_LANG_OBJECTSCRIPT_ROUTINE;
}
...
static const char *resolve_objectscript_callee(LSMExtractCtx *ctx, TSNode node, WalkState *state,
                                               const char *callee) {
    ...
    if ((!callee || !callee[0]) && strcmp(kind, "relative_dot_method") == 0 &&
        ...
        TSNode oref = lsm_find_child_by_kind(node, "oref_method");
        TSNode method_name =
            ts_node_is_null(oref) ? (TSNode){0} : lsm_find_child_by_kind(oref, "method_name");
        TSNode identifier =
            !ts_node_is_null(method_name) && ts_node_named_child_count(method_name) > 0
                ? ts_node_named_child(method_name, 0)
                : (TSNode){0};
        if (!ts_node_is_null(identifier)) {
            char *method = lsm_node_text(ctx->arena, identifier, ctx->source);
            if (method && method[0]) {
                callee = lsm_arena_sprintf(ctx->arena, "%s.%s", state->enclosing_class_qn, method);
            }
        }
    }

    if (callee && strncmp(callee, "$$$", 3) == 0 && ctx->macro_table) {
        const LSMMacroEntry *entry = lsm_macro_table_find(ctx->macro_table, callee + 3);
        if (entry) {
            if (entry->resolved_callee) {
                callee = lsm_arena_strdup(ctx->arena, entry->resolved_callee);
            } else if (entry->expansion) {
                callee = lsm_macro_extract_callee(ctx->arena, entry->expansion);
            } else {
                callee = NULL;
            }
        }
...
static TSNode objectscript_call_args(TSNode node) {
    TSNode oref = lsm_find_child_by_kind(node, "oref_method");
    if (!ts_node_is_null(oref)) {
        TSNode args = lsm_find_child_by_kind(oref, "method_args");
        if (!ts_node_is_null(args)) {
            return args;
        }
    }

    TSNode args = lsm_find_child_by_kind(node, "method_args");
    if (!ts_node_is_null(args)) {
        return args;
    }
    ...
    TSNode macro_function = lsm_find_child_by_kind(node, "macro_function");
    return ts_node_is_null(macro_function) ? (TSNode){0}
                                           : lsm_find_child_by_kind(macro_function, "method_args");
}
...
static bool is_callable_reference_value(LSMLanguage language, TSNode node) {
    const char *kind = ts_node_type(node);
    if (language == LSM_LANG_JAVA && strcmp(kind, "method_reference") == 0) {
        return true;
    }
    if (language == LSM_LANG_KOTLIN && strcmp(kind, "callable_reference") == 0) {
    ...
    return language == LSM_LANG_PHP && php_is_first_class_callable(node);
}
...
static bool is_dynamic_callee_expr(const LSMExtractCtx *ctx, TSNode expr) {
...
static TSNode terminal_callee_leaf(const LSMExtractCtx *ctx, TSNode expr) {
...
static TSNode objectscript_callee_expr(TSNode node) {
    ...
    if (strcmp(kind, "class_method_call") == 0) {
        return lsm_find_child_by_kind(node, "method_name");
    }
    if (strcmp(kind, "method_call") == 0 || strcmp(kind, "relative_dot_method") == 0) {
        TSNode oref = lsm_find_child_by_kind(node, "oref_method");
        return ts_node_is_null(oref) ? (TSNode){0} : lsm_find_child_by_kind(oref, "method_name");
    }
    if (strcmp(kind, "extrinsic_function") == 0 || strcmp(kind, "routine_tag_call") == 0) {
        return lsm_find_child_by_kind(node, "line_ref");
    }
    return (TSNode){0};
}
...
static TSNode language_specific_callee_expr(LSMLanguage language, TSNode node) {
    const char *kind = ts_node_type(node);
    ...
    if (language == LSM_LANG_SCALA && strcmp(kind, "infix_expression") == 0) {
    ...
    if (language == LSM_LANG_VHDL && strcmp(kind, "parenthesis_group") == 0) {
        TSNode previous = ts_node_prev_named_sibling(node);
    ...
    if (language == LSM_LANG_CSS && strcmp(kind, "call_expression") == 0) {
        return lsm_find_child_by_kind(node, "function_name");
    }

    if (language == LSM_LANG_NASM) {
    ...
    if (language == LSM_LANG_LLVM_IR) {
        return ts_node_child_by_field_name(node, TS_FIELD("callee"));
    }

    if (language == LSM_LANG_AGDA && strcmp(kind, "expr") == 0) {
        return agda_application_head_atom(node);
    }

    if (language == LSM_LANG_ADA &&
    ...
    if (language == LSM_LANG_JUST && strcmp(kind, "dependency") == 0) {
        return (TSNode){0};
    }
    ...
    if (language == LSM_LANG_PUPPET && strcmp(kind, "include_statement") == 0) {
        return (TSNode){0};
    }
...
static TSNode language_specific_callee_leaf(LSMLanguage language, TSNode expr,
                                            const LSMExtractCtx *ctx) {
    if (ts_node_is_null(expr)) {
        return (TSNode){0};
    }
    ...
    if (language == LSM_LANG_CSS && strcmp(ts_node_type(expr), "function_name") == 0) {
        return expr;
    }
    if (language == LSM_LANG_NASM) {
    ...
    if (language == LSM_LANG_AGDA) {
        return agda_head_qid(expr);
    }
    if (language == LSM_LANG_ELM) {
    ...
    if (language == LSM_LANG_PURESCRIPT) {
...
static LSMPrimaryCalleeSelection select_primary_callee(LSMExtractCtx *ctx, TSNode node,
                                                       WalkState *state) {
    LSMPrimaryCalleeSelection selection = {0};
    ...
    bool scala_named_infix =
        ctx->language == LSM_LANG_SCALA && strcmp(site_kind, "infix_expression") == 0;
...
static bool primary_callee_name_is_allowed(const LSMExtractCtx *ctx,
                                           const LSMPrimaryCalleeSelection *callee) {
    if (!callee->name || !callee->name[0]) {
        return false;
    }
    if (!lsm_is_keyword(callee->name, ctx->language) ||
        lsm_is_resolvable_builtin(callee->name, ctx->language)) {
        return true;
    }
    ...
    return ctx->language == LSM_LANG_CSS && !ts_node_is_null(callee->expr) &&
...
static LSMInvocationDescriptor describe_callable_reference(LSMExtractCtx *ctx, TSNode node) {
    LSMInvocationDescriptor descriptor = {0};
    descriptor.kind = LSM_INVOCATION_CALLABLE_REFERENCE;
    ...
    uint32_t count = ts_node_named_child_count(node);
    if (ctx->language == LSM_LANG_JAVA && count > 0) {
    ...
    } else if (ctx->language == LSM_LANG_KOTLIN && count > 0) {
        descriptor.callee_expr = ts_node_named_child(node, count - 1);
    } else if (ctx->language == LSM_LANG_PHP) {
    ...
    if (!ts_node_is_null(name_node)) {
        descriptor.callee_name = lsm_node_text(ctx->arena, name_node, ctx->source);
    }
    return descriptor;
}

static LSMInvocationDescriptor describe_emitted_primary_call(
    TSNode node, const LSMPrimaryCalleeSelection *callee) {
    LSMInvocationDescriptor descriptor = {0};
    descriptor.kind = LSM_INVOCATION_PRIMARY;
    descriptor.site = node;
...
LSMInvocationDescriptor handle_calls(LSMExtractCtx *ctx, TSNode node, const LSMLangSpec *spec,
                                     WalkState *state) {
    LSMInvocationDescriptor invocation = {0};
    ...
    if (!callable_reference && spec->call_node_types && spec->call_node_types[0] &&
        lsm_kind_in_set(node, spec->call_node_types)) {
        LSMPrimaryCalleeSelection callee = select_primary_callee(ctx, node, state);
        ...
        if (primary_callee_name_is_allowed(ctx, &callee)) {
            ...
            if (!ts_node_is_null(args)) {
                 ...
                 * only see the flat LSMCall, so the enclosing chain can only
                 * be read here where the AST still exists. */
                if (ctx->language == LSM_LANG_PHP && call.first_string_arg &&
                    call.first_string_arg[0] == '/' && call.callee_name &&
                    lsm_service_pattern_route_method(call.callee_name) != NULL) {
                    ...
                    if (gp && gp[0]) {
                        ...
                        call.first_string_arg =
                            rel[0] ? lsm_arena_sprintf(ctx->arena, "%s/%s", gp, rel)
                                   : lsm_arena_strndup(ctx->arena, gp, strlen(gp));
                    }
                }
                ...
                if (ctx->language == LSM_LANG_OBJECTSCRIPT_UDL ||
                    ...
                    for (uint32_t ai = 0;
                         ai < ts_node_named_child_count(args) && call.arg_count < LSM_MAX_CALL_ARGS;
                         ai++) {
                        ...
                        ca->expr = lsm_node_text(ctx->arena, achild, ctx->source);
            ...
            const char **dispatch_suffixes = lsm_string_dispatch_suffixes(ctx->language);
            if (dispatch_suffixes && !ts_node_is_null(args)) {
                ...
                for (const char **nm = dispatch_suffixes; *nm; nm++) {
                    ...
                    if (len >= nlen && strcmp(cn + len - nlen, *nm) == 0) {
                        ...
                        if (cls && mth) {
                            LSMCall xcall = {0};
                            xcall.callee_name = lsm_arena_sprintf(ctx->arena, "%s.%s", cls, mth);
                            ...
                            lsm_calls_push(&ctx->result->calls, ctx->arena, xcall);
                        }
                        break;
                    }
                }
    ...
    if (ctx->language == LSM_LANG_TSX || ctx->language == LSM_LANG_JAVASCRIPT) {
