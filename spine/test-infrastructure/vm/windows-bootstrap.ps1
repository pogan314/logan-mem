...
#   mkdir /tmp/lsmiso
#   cp windows-bootstrap.ps1 /tmp/lsmiso/
#   cp ~/.claude/lsm-vm/id_ed25519.pub /tmp/lsmiso/host-key.pub
#   printf '@echo off\r\npowershell -NoProfile -ExecutionPolicy Bypass -File "%%~dp0windows-bootstrap.ps1" -SshPublicKeyPath "%%~dp0host-key.pub"\r\n' > /tmp/lsmiso/RUN-ME.bat
#   hdiutil makehybrid -iso -joliet -default-volume-name LSMSETUP -o ~/lsm-setup.iso /tmp/lsmiso
...
New-NetFirewallRule -Name lsm-sshd -DisplayName "OpenSSH (lsm test VM)" `
...
Write-Host "On the host, write ~/.claude/lsm-vm/config with:"
Write-Host "  LSM_VM_HOST=$ip"
Write-Host ("  LSM_VM_USER=" + $env:USERNAME)
Write-Host "  LSM_VM_HOST_KEY_SHA256=<copy the SHA256:... value above>"
