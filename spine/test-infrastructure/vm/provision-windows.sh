...
#   2. Host config in ~/.claude/lsm-vm/config (KEY=VALUE), values printed by
#      windows-bootstrap.ps1 at the end of its run:
#        LSM_VM_HOST=<vm ip>
#        LSM_VM_USER=<vm user>
#        LSM_VM_HOST_KEY_SHA256=<SHA256:... host-key fingerprint>
#      Key at ~/.claude/lsm-vm/id_ed25519 (see README.md to generate your own;
...
REPO_URL="${LSM_VM_REPO:-https://github.com/DeusData/logan-spine-mcp.git}"
...
trap lsm_vm_cleanup_known_hosts EXIT
...
vm_powershell() { lsm_vm_run_powershell "$1" "${SSH[@]}"; }
...
step "4/6 repo clone/update -> /c/lsm @ ${BRANCH}"
if vm "clangarm64" "test -d /c/lsm/.git && echo REPO_PRESENT" | grep -q REPO_PRESENT; then
    vm "clangarm64" "cd /c/lsm && git fetch origin ${BRANCH} && git reset --hard FETCH_HEAD && git clean -fdx && git log --oneline -1"
else
    vm "clangarm64" "git clone --branch ${BRANCH} --single-branch --depth 200 ${REPO_URL} /c/lsm && cd /c/lsm && git log --oneline -1"
fi

step "5/6 build: native ARM64 binary + test-runner (no ASan on arm64)"
vm "clangarm64" "cd /c/lsm && make -j${JOBS} -f Makefile.lsm CC=clang CXX=clang++ SANITIZE= lsm build/c/test-runner > /tmp/provision-build.log 2>&1 && echo BUILD_OK || (echo BUILD_FAIL; tail -15 /tmp/provision-build.log; exit 1)"

step "6/6 smoke: binary + test-runner start"
vm "clangarm64" "cd /c/lsm && ./build/c/logan-spine-mcp.exe --version && ./build/c/test-runner --list-suites | head -3"
