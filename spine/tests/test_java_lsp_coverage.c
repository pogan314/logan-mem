...
static LSMFileResult *cov_extract(const char *source) {
    return lsm_extract_file(source, (int)strlen(source), LSM_LANG_JAVA, "test", "Main.java", 0,
                            NULL, NULL);
}

static int cov_find(const LSMFileResult *r, const char *callerSub, const char *calleeSub) {
...
static int cov_require(const LSMFileResult *r, const char *callerSub, const char *calleeSub) {
...
static int cov_count_strict(const LSMFileResult *r) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
...
static int cov_no_crash(LSMFileResult *r) { return r != NULL; }
...
TEST(cov_lit_long) {
    LSMFileResult *r = cov_extract("public class Main {\n"
...
TEST(cov_typename_via_static_import) {
    LSMFileResult *r = cov_extract("import static java.lang.Math.PI;\n"
...
TEST(cov_typename_generic_type) {
    LSMFileResult *r = cov_extract("import java.util.List;\n"
...
TEST(cov_typeast_void_method) {
    LSMFileResult *r = cov_extract("public class Main { public void f() {} }");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(cov_cross_no_imports) {
    ...
    LSMResolvedCallArray out;
    memset(&out, 0, sizeof(out));
    lsm_run_java_lsp_cross(&arena, src, (int)strlen(src), "test.A", NULL, 0, NULL, NULL, 0, NULL, &out);
...
TEST(cov_cross_batch) {
    ...
    lsm_batch_java_lsp_cross(&arena, files, 2, outs);
    lsm_arena_destroy(&arena);
    PASS();
}
TEST(cov_cross_inheritance) {
    ...
    defs[1].return_types = "java.lang.String";
    lsm_run_java_lsp_cross(&arena, src, (int)strlen(src), "test.B", defs, 2, NULL, NULL, 0, NULL, &out);
    int found = 0;
    for (int i = 0; i < out.count; i++) {
        if (out.items[i].confidence < 0.5f) continue;
...
TEST(cov_edge_huge_class_no_crash) {
    ...
    LSMFileResult *r = cov_extract(buf);
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
