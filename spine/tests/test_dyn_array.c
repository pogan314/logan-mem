...
TEST(da_struct_type) {
    ...
    LSM_DYN_ARRAY(Point) pts = {0};
    lsm_da_push(&pts, ((Point){.x = 1, .y = 2.5f}));
    lsm_da_push(&pts, ((Point){.x = 3, .y = 4.5f}));
...
TEST(da_string_ptrs) {
    LSM_DYN_ARRAY(const char *) strs = {0};
    lsm_da_push(&strs, "hello");
    lsm_da_push(&strs, "world");
...
TEST(da_insert) {
    ...
    lsm_da_push(&arr, 10);
    lsm_da_push(&arr, 30);
    lsm_da_insert(&arr, 1, 20); /* insert 20 at index 1 */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    ASSERT_EQ(arr.items[2], 30);
...
TEST(da_push_ptr_struct) {
    ...
    LSM_DYN_ARRAY(Record) arr = {0};

    Record *r = lsm_da_push_ptr(&arr);
    r->id = 42;
    r->value = 3.14;
    r->tag = "first";

    r = lsm_da_push_ptr(&arr);
    r->id = 99;
    r->value = 2.72;
    r->tag = "second";
...
TEST(da_insert_at_beginning) {
    ...
    lsm_da_push(&arr, 30);
    lsm_da_push(&arr, 40);
    lsm_da_insert(&arr, 0, 10); /* insert at front */
...
TEST(da_insert_at_end) {
    ...
    lsm_da_push(&arr, 10);
    lsm_da_push(&arr, 20);
    lsm_da_insert(&arr, 2, 30); /* insert at count (append) */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    ASSERT_EQ(arr.items[2], 30);
...
TEST(da_remove_first) {
    ...
    lsm_da_push(&arr, 20);
    lsm_da_push(&arr, 30);
    lsm_da_remove(&arr, 0); /* remove first */
    ASSERT_EQ(arr.count, 2);
...
TEST(da_remove_last) {
    ...
    lsm_da_push(&arr, 20);
    lsm_da_push(&arr, 30);
    lsm_da_remove(&arr, 2); /* remove last element */
    ASSERT_EQ(arr.count, 2);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 20);
    lsm_da_free(&arr);
...
TEST(da_remove_sequence) {
    LSM_DYN_ARRAY(int) arr = {0};
    for (int i = 0; i < 5; i++)
        lsm_da_push(&arr, i * 10); /* 0, 10, 20, 30, 40 */

    lsm_da_remove(&arr, 2); /* remove 20 -> 0,10,30,40 */
    ASSERT_EQ(arr.count, 4);
    ASSERT_EQ(arr.items[2], 30);

    lsm_da_remove(&arr, 0); /* remove 0 -> 10,30,40 */
    ASSERT_EQ(arr.count, 3);
    ASSERT_EQ(arr.items[0], 10);

    lsm_da_remove(&arr, 2); /* remove 40 -> 10,30 */
    ASSERT_EQ(arr.count, 2);
    ASSERT_EQ(arr.items[0], 10);
    ASSERT_EQ(arr.items[1], 30);
    lsm_da_free(&arr);
...
TEST(da_large_array_10k) {
    LSM_DYN_ARRAY(int64_t) arr = {0};
    for (int i = 0; i < 10000; i++)
        lsm_da_push(&arr, (int64_t)i * 1000);
...
TEST(da_nested_struct_with_ptrs) {
    ...
    LSM_DYN_ARRAY(Entry) arr = {0};

    lsm_da_push(&arr, ((Entry){.name = "alpha", .scores = {10, 20, 30}, .category = "A"}));
    lsm_da_push(&arr, ((Entry){.name = "beta", .scores = {40, 50, 60}, .category = "B"}));
    lsm_da_push(&arr, ((Entry){.name = "gamma", .scores = {70, 80, 90}, .category = "A"}));
...
TEST(da_push_pop_lifo) {
    ...
    ASSERT_EQ(lsm_da_pop(&arr), 5);
    ASSERT_EQ(lsm_da_pop(&arr), 4);
    ASSERT_EQ(lsm_da_pop(&arr), 3);
    ASSERT_EQ(lsm_da_pop(&arr), 2);
    ASSERT_EQ(lsm_da_pop(&arr), 1);
    ASSERT_EQ(arr.count, 0);
...
TEST(da_mixed_operations) {
    ...
    /* last should be 30 */
    ASSERT_EQ(lsm_da_last(&arr), 30);

    /* insert at end -> 15,20,30,50 */
    lsm_da_insert(&arr, 3, 50);
    ASSERT_EQ(arr.count, 4);
