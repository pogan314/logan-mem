...
TEST(yaml_parse_null_input) {
    lsm_yaml_node_t *root = lsm_yaml_parse(NULL, 0);
    ASSERT_NOT_NULL(root); /* returns empty map on NULL */
    ASSERT_FALSE(lsm_yaml_has(root, "anything"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_parse_empty_string) {
    ...
    ASSERT_FALSE(lsm_yaml_has(root, "key"));
    lsm_yaml_free(root);
    PASS();
}

TEST(yaml_parse_negative_len) {
    lsm_yaml_node_t *root = lsm_yaml_parse("key: val", -1);
    ASSERT_NOT_NULL(root);
...
TEST(yaml_single_kv) {
    ...
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "name"), "hello");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_multiple_kv) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "name"), "myproject");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "version"), "1.2.3");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "author"), "someone");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_nested_map_2_levels) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "database.host"), "localhost");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "database.port"), "5432");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_nested_map_3_levels) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "level1.level2.level3"), "deep_value");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_nested_siblings) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "server.host"), "0.0.0.0");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "server.port"), "8080");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "database.host"), "db.local");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "database.port"), "3306");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_string_list) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[8];
    int count = lsm_yaml_get_str_list(root, "fruits", items, 8);
    ASSERT_EQ(count, 3);
...
TEST(yaml_list_max_out_limit) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[2];
    int count = lsm_yaml_get_str_list(root, "items", items, 2);
    ASSERT_EQ(count, 2);
...
TEST(yaml_comment_only) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FALSE(lsm_yaml_has(root, "#"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_comment_between_keys) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "a"), "1");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "b"), "2");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_mixed_maps_and_lists) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "project.name"), "myapp");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "project.version"), "2.0");
    const char *tags[4];
    int count = lsm_yaml_get_str_list(root, "project.tags", tags, 4);
    ASSERT_EQ(count, 2);
...
TEST(yaml_url_value) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "url"), "https://example.com:8080/path");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_multiple_colons) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "time"), "12:30:45");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_empty_value_becomes_map) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_has(root, "parent"));
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "parent.child"), "val");
    /* parent itself is a map, not a scalar */
    ASSERT_NULL(lsm_yaml_get_str(root, "parent"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_missing) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_NULL(lsm_yaml_get_str(root, "nonexistent"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_nested) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "a.b.c"), "found");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_on_map_node) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_NULL(lsm_yaml_get_str(root, "group"));
    lsm_yaml_free(root);
    PASS();
}

TEST(yaml_get_str_null_root) {
    ASSERT_NULL(lsm_yaml_get_str(NULL, "key"));
    PASS();
}
...
TEST(yaml_get_str_null_path) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_NULL(lsm_yaml_get_str(root, NULL));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_valid) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "confidence", -1.0), 0.85, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_integer) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "count", -1.0), 42.0, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_negative) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "offset", 0.0), -3.14, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_invalid_string) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "val", 99.0), 99.0, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_missing_key) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "missing", 77.7), 77.7, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_float_zero) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "val", -1.0), 0.0, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_yes_no) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "feature_a", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "feature_b", true));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_on_off) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "logging", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "debug", true));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_1_0) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "flag_on", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "flag_off", true));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_case_insensitive) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "a", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "b", true));
    ASSERT_TRUE(lsm_yaml_get_bool(root, "c", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "d", true));
    ASSERT_TRUE(lsm_yaml_get_bool(root, "e", false));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "f", true));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_missing_returns_default) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "missing", true));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "missing", false));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_bool_unrecognized_returns_default) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_get_bool(root, "val", true));
    ASSERT_FALSE(lsm_yaml_get_bool(root, "val", false));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_list_non_list_path) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[4];
    int count = lsm_yaml_get_str_list(root, "scalar", items, 4);
    ASSERT_EQ(count, 0);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_list_missing_path) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[4];
    int count = lsm_yaml_get_str_list(root, "nonexistent", items, 4);
    ASSERT_EQ(count, 0);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_get_str_list_null_root) {
    const char *items[4];
    int count = lsm_yaml_get_str_list(NULL, "key", items, 4);
    ASSERT_EQ(count, 0);
    PASS();
}
...
TEST(yaml_has_existing) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_TRUE(lsm_yaml_has(root, "key"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_has_missing) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_FALSE(lsm_yaml_has(root, "nope"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_has_nested) {
    ...
    ASSERT_FALSE(lsm_yaml_has(root, "a.b.c"));
    lsm_yaml_free(root);
    PASS();
}

TEST(yaml_has_null_root) {
    ASSERT_FALSE(lsm_yaml_has(NULL, "key"));
    PASS();
}
...
TEST(yaml_long_value) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, n);
    ASSERT_NOT_NULL(root);
    const char *val = lsm_yaml_get_str(root, "longkey");
    ASSERT_NOT_NULL(val);
...
TEST(yaml_special_chars_in_value) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "pattern"), "[a-z]+(foo|bar)*");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_equals_in_value) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "query"), "SELECT * FROM t WHERE x=1");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_deeply_nested) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "l1.l2.l3.l4.l5"), "bottom");
    /* Intermediate nodes exist but are not scalars */
    ASSERT_TRUE(lsm_yaml_has(root, "l1.l2.l3.l4"));
    ASSERT_NULL(lsm_yaml_get_str(root, "l1.l2.l3.l4"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_missing_intermediate) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_NULL(lsm_yaml_get_str(root, "x.y.z"));
    ASSERT_FALSE(lsm_yaml_has(root, "x.y.z"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_tab_not_indentation) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "key1"), "val1");
...
TEST(yaml_quoted_string_with_hash) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    const char *val = lsm_yaml_get_str(root, "color");
    ASSERT_NOT_NULL(val);
...
TEST(yaml_single_quoted_with_hash) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    const char *val = lsm_yaml_get_str(root, "regex");
    ASSERT_NOT_NULL(val);
...
TEST(yaml_whitespace_after_colon) {
    ...
    ASSERT_TRUE(lsm_yaml_has(root, "bare"));
    /* Not a scalar, so get_str returns NULL */
    ASSERT_NULL(lsm_yaml_get_str(root, "bare"));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_numeric_string_value) {
    ...
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "port"), "8080");
    /* But can be retrieved as float */
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "port", -1.0), 8080.0, 0.001);
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_line_without_colon_skipped) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "valid"), "yes");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "also_valid"), "sure");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_hash_no_preceding_space) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "channel"), "#general");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_list_after_comments) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[4];
    int count = lsm_yaml_get_str_list(root, "paths", items, 4);
    ASSERT_EQ(count, 2);
...
TEST(yaml_path_segment_overflow) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_NULL(lsm_yaml_get_str(root, long_path));
    ASSERT_FALSE(lsm_yaml_has(root, long_path));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_empty_path) {
    ...
    ASSERT_NULL(lsm_yaml_get_str(root, ""));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_path_trailing_dot) {
    ...
    ASSERT_NULL(lsm_yaml_get_str(root, "a."));
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_duplicate_keys) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "key"), "first");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_indentation_dedent) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "outer.inner"), "deep");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "top"), "level");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_smoke_cgrconfig) {
    ...
    /* Top-level scalar */
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "mode"), "fast");

    /* Nested bool */
    ASSERT_TRUE(lsm_yaml_get_bool(root, "http_linker.enabled", false));

    /* Nested float */
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "http_linker.min_confidence", 0.0), 0.7, 0.001);

    /* Nested list */
    const char *paths[8];
    int count = lsm_yaml_get_str_list(root, "http_linker.exclude_paths", paths, 8);
    ASSERT_EQ(count, 3);
    ...
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "pipeline.workers"), "4");
    ASSERT_FALSE(lsm_yaml_get_bool(root, "pipeline.verbose", true));

    /* Non-existent */
    ASSERT_FALSE(lsm_yaml_has(root, "pipeline.timeout"));
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "pipeline.timeout", 30.0), 30.0, 0.001);
...
TEST(yaml_smoke_multi_query) {
    ...
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "app.name"), "testapp");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "app.db.host"), "pghost");

    /* Bools */
    ASSERT_TRUE(lsm_yaml_get_bool(root, "app.debug", false));
    ASSERT_TRUE(lsm_yaml_get_bool(root, "app.db.ssl", false));

    /* Float */
    ASSERT_FLOAT_EQ(lsm_yaml_get_float(root, "app.port", 0.0), 9090.0, 0.001);

    /* List */
    const char *feats[4];
    int count = lsm_yaml_get_str_list(root, "app.features", feats, 4);
    ASSERT_EQ(count, 2);
    ASSERT_STR_EQ(feats[0], "auth");
    ASSERT_STR_EQ(feats[1], "logging");

    /* has() checks */
    ASSERT_TRUE(lsm_yaml_has(root, "app"));
    ASSERT_TRUE(lsm_yaml_has(root, "app.features"));
    ASSERT_TRUE(lsm_yaml_has(root, "app.db"));
    ASSERT_FALSE(lsm_yaml_has(root, "app.cache"));
...
TEST(yaml_partial_len) {
    ...
    const char *yaml = "key: value\n";
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, 7);
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "key"), "va");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_top_level_list_items) {
    ...
    ASSERT_NOT_NULL(root);
    const char *items[4];
    int count = lsm_yaml_get_str_list(root, "colors", items, 4);
    ASSERT_EQ(count, 3);
...
TEST(yaml_inconsistent_indentation) {
    ...
    lsm_yaml_node_t *root = lsm_yaml_parse(yaml, (int)strlen(yaml));
    ASSERT_NOT_NULL(root);
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "a.b.c"), "deep");
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "a.d"), "shallow");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_value_whitespace_trimmed) {
    ...
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "key"), "spaced");
    lsm_yaml_free(root);
    PASS();
}
...
TEST(yaml_indented_top_level) {
    ...
    ASSERT_STR_EQ(lsm_yaml_get_str(root, "indented_key"), "val");
    lsm_yaml_free(root);
    PASS();
}
