...
typedef struct {
    ...
    lsm_version_cohort_quiesce_result_t quiesce_result;
    lsm_version_cohort_lease_t *lease;
} version_cohort_mutation_wait_t;

static lsm_daemon_build_identity_t version_cohort_identity(const char *version, const char *build) {
...
static bool version_cohort_fixture_start(version_cohort_fixture_t *fixture, const char *tag) {
    ...
    int written = snprintf(fixture->parent, sizeof(fixture->parent),
                           "%s/lsm-version-cohort-%s-XXXXXX", lsm_tmpdir(), tag);
...
static void version_cohort_release(lsm_version_cohort_lease_t **lease) {
    while (lease && *lease && lsm_version_cohort_lease_release(lease) != LSM_PRIVATE_FILE_LOCK_OK) {
        lsm_usleep(1000);
    }
}

static void version_cohort_manager_close(lsm_version_cohort_manager_t **manager) {
    while (manager && *manager &&
           lsm_version_cohort_manager_free(manager) != LSM_PRIVATE_FILE_LOCK_OK) {
        lsm_usleep(1000);
    }
}
...
static void version_cohort_mutation_wait_init(version_cohort_mutation_wait_t *wait,
    ...
    wait->status = LSM_VERSION_COHORT_IO;
    wait->quiesce_result = LSM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    atomic_init(&wait->callback_count, 0);
    atomic_init(&wait->callback_seen, false);
    atomic_init(&wait->finished, false);
}

static lsm_version_cohort_quiesce_result_t version_cohort_test_request_quiesce(void *context) {
...
static void *version_cohort_mutation_wait_thread(void *context) {
    version_cohort_mutation_wait_t *wait = context;
    wait->status = lsm_version_cohort_reserve_for_mutation(
...
static bool version_cohort_wait_for_atomic(atomic_bool *value, uint64_t deadline_ms) {
    while (!atomic_load_explicit(value, memory_order_acquire) && lsm_now_ms() < deadline_ms) {
        lsm_usleep(1000);
    }
...
TEST(version_cohort_shares_exact_build_rejects_conflict_and_turns_over) {
    ...
    lsm_version_cohort_manager_t *first = lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *second = lsm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(first);
    ASSERT_NOT_NULL(second);

    lsm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    lsm_daemon_build_identity_t build_b = version_cohort_identity("2.5.0", VERSION_COHORT_BUILD_B);
    lsm_version_cohort_lease_t *a_first = NULL;
    lsm_version_cohort_lease_t *a_second = NULL;
    lsm_version_cohort_lease_t *b_lease = NULL;
    lsm_daemon_conflict_t conflict;

    ASSERT_EQ(lsm_version_cohort_acquire(first, &build_a, UINT64_MAX, &a_first, &conflict),
              LSM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(a_first);
    ASSERT_EQ(lsm_version_cohort_acquire(second, &build_a, UINT64_MAX, &a_second, &conflict),
              LSM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(a_second);
    ASSERT_EQ(lsm_version_cohort_acquire(second, &build_b, lsm_now_ms(), &b_lease, &conflict),
              LSM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(b_lease);
    ASSERT_EQ(conflict.status, LSM_DAEMON_HELLO_VERSION_CONFLICT);
    ...
    ASSERT_EQ(lsm_version_cohort_acquire(second, &build_b, UINT64_MAX, &b_lease, &conflict),
              LSM_VERSION_COHORT_OK);
...
TEST(version_cohort_rejects_same_hash_with_different_abi) {
    ...
    ASSERT_NOT_NULL(first);
    ASSERT_NOT_NULL(second);
    lsm_daemon_build_identity_t active = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    lsm_daemon_build_identity_t requested = active;
    requested.feature_abi++;
    lsm_version_cohort_lease_t *active_lease = NULL;
    lsm_version_cohort_lease_t *requested_lease = NULL;
    lsm_daemon_conflict_t conflict;

    ASSERT_EQ(lsm_version_cohort_acquire(first, &active, UINT64_MAX, &active_lease, &conflict),
              LSM_VERSION_COHORT_OK);
    ASSERT_EQ(
        lsm_version_cohort_acquire(second, &requested, lsm_now_ms(), &requested_lease, &conflict),
        LSM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(requested_lease);
    ASSERT_EQ(conflict.status, LSM_DAEMON_HELLO_FEATURE_ABI_CONFLICT);
...
TEST(version_cohort_rejects_missing_cache_fingerprint) {
    ...
    lsm_version_cohort_manager_t *manager = lsm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(manager);
    lsm_daemon_build_identity_t identity = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    identity.cache_fingerprint = NULL;
    lsm_version_cohort_lease_t *lease = NULL;
    lsm_daemon_conflict_t conflict;

    ASSERT_EQ(lsm_version_cohort_acquire(manager, &identity, lsm_now_ms(), &lease, &conflict),
              LSM_VERSION_COHORT_UNSAFE);
    ASSERT_NULL(lease);
...
TEST(version_cohort_rejects_exact_build_with_different_cache_root) {
    ...
    ASSERT_EQ(
        ...
        LSM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(requested_lease);
    ASSERT_EQ(conflict.status, LSM_DAEMON_HELLO_CACHE_CONFLICT);
    ...
    ASSERT_TRUE(lsm_daemon_conflict_format(&conflict, message, sizeof(message)));
...
TEST(version_cohort_exclusive_activation_blocks_and_is_blocked_by_participants) {
    ...
    lsm_version_cohort_manager_t *participant_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *activation_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ASSERT_NOT_NULL(activation_manager);
    lsm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    lsm_version_cohort_lease_t *participant = NULL;
    lsm_version_cohort_lease_t *activation = NULL;
    lsm_daemon_conflict_t conflict;

    ASSERT_EQ(lsm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              LSM_VERSION_COHORT_OK);
    ASSERT_EQ(lsm_version_cohort_reserve_exclusive(activation_manager, lsm_now_ms(), &activation),
              LSM_VERSION_COHORT_BUSY);
    ASSERT_NULL(activation);
    version_cohort_release(&participant);

    ASSERT_EQ(lsm_version_cohort_reserve_exclusive(activation_manager, UINT64_MAX, &activation),
              LSM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(activation);
    ASSERT_EQ(lsm_version_cohort_acquire(participant_manager, &build_a, lsm_now_ms(), &participant,
                                         &conflict),
              LSM_VERSION_COHORT_BUSY);
...
TEST(version_cohort_mutation_intent_fails_new_admission_and_spans_lease) {
    ...
    lsm_version_cohort_manager_t *participant_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *mutation_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *contender_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ...
    lsm_daemon_conflict_t conflict;
    lsm_version_cohort_lease_t *participant = NULL;
    lsm_version_cohort_lease_t *contender = NULL;
    ASSERT_EQ(lsm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              LSM_VERSION_COHORT_OK);
    lsm_version_cohort_maintenance_presence_t before =
        lsm_version_cohort_maintenance_presence(contender_manager);

    version_cohort_mutation_wait_t wait;
    version_cohort_mutation_wait_init(&wait, mutation_manager, lsm_now_ms() + 5000U);
    lsm_thread_t thread;
    bool started = lsm_thread_create(&thread, 0, version_cohort_mutation_wait_thread, &wait) == 0;
    bool callback_seen =
        started && version_cohort_wait_for_atomic(&wait.callback_seen, lsm_now_ms() + 2000U);
    lsm_version_cohort_maintenance_presence_t during =
        callback_seen ? lsm_version_cohort_maintenance_presence(contender_manager)
                      : LSM_VERSION_COHORT_MAINTENANCE_IO;
    lsm_version_cohort_status_t racing_status =
        callback_seen ? lsm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX,
                                                   &contender, &conflict)
    ...
    version_cohort_release(&participant);
    bool finished = started && version_cohort_wait_for_atomic(&wait.finished, lsm_now_ms() + 5500U);
    bool joined = started && lsm_thread_join(&thread) == 0;
    lsm_version_cohort_maintenance_presence_t retained =
        finished && wait.lease ? lsm_version_cohort_maintenance_presence(contender_manager)
                               : LSM_VERSION_COHORT_MAINTENANCE_IO;
    lsm_version_cohort_status_t retained_admission_status =
        finished && wait.lease ? lsm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX,
                                                            &contender, &conflict)
                               : LSM_VERSION_COHORT_IO;
    bool retained_admission_absent = contender == NULL;
    version_cohort_release(&contender);
    version_cohort_release(&wait.lease);
    lsm_version_cohort_maintenance_presence_t after =
        lsm_version_cohort_maintenance_presence(contender_manager);
    lsm_version_cohort_status_t post_status =
        lsm_version_cohort_acquire(contender_manager, &build_a, UINT64_MAX, &contender, &conflict);
    ...
    ASSERT_EQ(before, LSM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_TRUE(started);
    ASSERT_TRUE(callback_seen);
    ASSERT_EQ(during, LSM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(racing_status, LSM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(racing_lease_absent);
    ASSERT_TRUE(still_draining);
    ASSERT_TRUE(finished);
    ASSERT_TRUE(joined);
    ASSERT_EQ(wait.status, LSM_VERSION_COHORT_OK);
    ASSERT_EQ(wait.quiesce_result, LSM_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_EQ(atomic_load_explicit(&wait.callback_count, memory_order_relaxed), 1);
    ASSERT_EQ(retained, LSM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(retained_admission_status, LSM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(retained_admission_absent);
    ASSERT_EQ(after, LSM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_EQ(post_status, LSM_VERSION_COHORT_OK);
    PASS();
}
...
TEST(version_cohort_mutation_waits_for_every_lifetime_participant) {
    ...
    lsm_version_cohort_manager_t *first_manager = lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *second_manager = lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *mutation_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    ...
    lsm_daemon_build_identity_t build_a = version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
    lsm_daemon_conflict_t conflict;
    lsm_version_cohort_lease_t *first = NULL;
    lsm_version_cohort_lease_t *second = NULL;
    ASSERT_EQ(lsm_version_cohort_acquire(first_manager, &build_a, UINT64_MAX, &first, &conflict),
              LSM_VERSION_COHORT_OK);
    ASSERT_EQ(lsm_version_cohort_acquire(second_manager, &build_a, UINT64_MAX, &second, &conflict),
              LSM_VERSION_COHORT_OK);
...
TEST(version_cohort_mutation_timeout_releases_all_guards) {
    ...
    lsm_version_cohort_manager_t *mutation_manager =
        lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_manager_t *probe_manager = lsm_version_cohort_manager_new(fixture.endpoint);
    ASSERT_NOT_NULL(participant_manager);
    ASSERT_NOT_NULL(mutation_manager);
    ...
    lsm_version_cohort_lease_t *probe = NULL;
    ASSERT_EQ(lsm_version_cohort_acquire(participant_manager, &build_a, UINT64_MAX, &participant,
                                         &conflict),
              LSM_VERSION_COHORT_OK);

    version_cohort_mutation_wait_t callback;
    version_cohort_mutation_wait_init(&callback, mutation_manager, lsm_now_ms() + 25U);
    lsm_version_cohort_quiesce_result_t quiesce_result = LSM_VERSION_COHORT_QUIESCE_NOT_NEEDED;
    lsm_version_cohort_status_t timeout_status = lsm_version_cohort_reserve_for_mutation(
    ...
    lsm_version_cohort_maintenance_presence_t after_timeout =
        lsm_version_cohort_maintenance_presence(probe_manager);
    lsm_version_cohort_status_t admission_status =
        lsm_version_cohort_acquire(probe_manager, &build_a, UINT64_MAX, &probe, &conflict);

    version_cohort_release(&probe);
    version_cohort_release(&participant);
    lsm_version_cohort_quiesce_result_t invalid_result = LSM_VERSION_COHORT_QUIESCE_REQUESTED;
    lsm_version_cohort_status_t unbounded_status = lsm_version_cohort_reserve_for_mutation(
    ...
    lsm_version_cohort_quiesce_result_t retry_result = LSM_VERSION_COHORT_QUIESCE_REQUESTED;
    lsm_version_cohort_status_t retry_status = lsm_version_cohort_reserve_for_mutation(
        mutation_manager, lsm_now_ms() + 250U, version_cohort_test_request_quiesce, &callback,
        &retry_result, &mutation);
    lsm_version_cohort_maintenance_presence_t during_retry =
        mutation ? lsm_version_cohort_maintenance_presence(probe_manager)
                 : LSM_VERSION_COHORT_MAINTENANCE_IO;
    ...
    lsm_version_cohort_maintenance_presence_t after_retry =
        lsm_version_cohort_maintenance_presence(probe_manager);
    ...
    ASSERT_EQ(timeout_status, LSM_VERSION_COHORT_BUSY);
    ASSERT_TRUE(no_mutation_authority);
    ASSERT_EQ(quiesce_result, LSM_VERSION_COHORT_QUIESCE_REQUESTED);
    ASSERT_EQ(atomic_load_explicit(&callback.callback_count, memory_order_relaxed), 1);
    ASSERT_EQ(after_timeout, LSM_VERSION_COHORT_MAINTENANCE_ABSENT);
    ASSERT_EQ(admission_status, LSM_VERSION_COHORT_OK);
    ASSERT_EQ(unbounded_status, LSM_VERSION_COHORT_UNSAFE);
    ASSERT_EQ(invalid_result, LSM_VERSION_COHORT_QUIESCE_NOT_NEEDED);
    ASSERT_TRUE(unbounded_lease_absent);
    ASSERT_EQ(retry_status, LSM_VERSION_COHORT_OK);
    ASSERT_EQ(retry_result, LSM_VERSION_COHORT_QUIESCE_NOT_NEEDED);
    ASSERT_EQ(during_retry, LSM_VERSION_COHORT_MAINTENANCE_REQUESTED);
    ASSERT_EQ(callback_count_after_retry, 1);
    ASSERT_EQ(after_retry, LSM_VERSION_COHORT_MAINTENANCE_ABSENT);
    PASS();
}
...
TEST(version_cohort_does_not_repurpose_daemon_startup_lock_for_lifetime) {
    ...
    ASSERT_EQ(lsm_daemon_ipc_startup_lock_try_acquire(fixture.endpoint, &startup), 1);
    ...
    lsm_daemon_conflict_t conflict;
    ASSERT_NOT_NULL(manager);
    ASSERT_EQ(lsm_version_cohort_acquire(manager, &build_a, UINT64_MAX, &lease, &conflict),
              LSM_VERSION_COHORT_OK);
...
TEST(version_cohort_distinguishes_coordinated_daemon_without_connecting) {
    ...
    lsm_version_cohort_manager_t *manager = lsm_version_cohort_manager_new(fixture.endpoint);
    lsm_version_cohort_daemon_claim_t *claim = NULL;
    lsm_daemon_ipc_lifetime_reservation_t *lifetime = NULL;
    lsm_daemon_ipc_startup_lock_t *startup = NULL;
    ASSERT_NOT_NULL(manager);

    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_ABSENT);
    ...
    ASSERT_NOT_NULL(startup);
    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_UNCOORDINATED);
    lsm_daemon_ipc_startup_lock_release(&startup);
    startup = NULL;
    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_ABSENT);
    ...
    ASSERT_EQ(lsm_daemon_ipc_lifetime_reservation_try_acquire(fixture.endpoint, &lifetime), 1);
    ASSERT_NOT_NULL(lifetime);
    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_UNCOORDINATED);

    ASSERT_EQ(lsm_version_cohort_daemon_claim_acquire(manager, &claim), LSM_VERSION_COHORT_OK);
    ASSERT_NOT_NULL(claim);
    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_COORDINATED);
    ...
    ASSERT_EQ(lsm_version_cohort_daemon_claim_release(&claim), LSM_PRIVATE_FILE_LOCK_OK);
    ASSERT_NULL(claim);
    ASSERT_EQ(lsm_version_cohort_daemon_presence(manager, fixture.endpoint),
              LSM_VERSION_COHORT_DAEMON_ABSENT);
...
TEST(version_cohort_transition_presence_is_authoritative_and_marker_checked) {
    ...
    ASSERT_EQ(lsm_daemon_ipc_local_transition_try_acquire(fixture.endpoint, &transition), 1);
    ASSERT_NOT_NULL(transition);
    ASSERT_EQ(
        lsm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        LSM_VERSION_COHORT_DAEMON_UNSAFE);
    ASSERT_EQ(lsm_daemon_ipc_local_transition_seal_legacy(transition), 1);
    ASSERT_EQ(
        lsm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        LSM_VERSION_COHORT_DAEMON_ABSENT);
    ASSERT_TRUE(lsm_daemon_ipc_local_transition_release(&transition));
...
TEST(version_cohort_presence_recovers_current_posix_listener_crash) {
    ...
    if (child == 0) {
        ...
        lsm_daemon_ipc_listener_t *listener = lsm_daemon_ipc_listen(fixture.endpoint);
    ...
    ASSERT_EQ(lsm_daemon_ipc_lifetime_reservation_probe(fixture.endpoint), 0);
    ASSERT_EQ(lsm_daemon_ipc_endpoint_probe(fixture.endpoint, 1), 1);
    ...
    ASSERT_EQ(
        lsm_version_cohort_daemon_presence_under_transition(manager, fixture.endpoint, transition),
        LSM_VERSION_COHORT_DAEMON_ABSENT);
    ASSERT_EQ(lsm_daemon_ipc_endpoint_probe(fixture.endpoint, 1), 0);
...
TEST(version_cohort_crash_releases_process_lifetime_lease) {
    ...
    lsm_subprocess_t *child = NULL;
    int spawn_status = launch_ready ? lsm_subprocess_spawn(&options, &child) : -1;
    bool ready = false;
    bool terminal = false;
    lsm_proc_result_t process_result = {0};
    uint64_t ready_deadline = lsm_now_ms() + 5000U;
    while (child && lsm_now_ms() < ready_deadline) {
        lsm_proc_poll_t poll = lsm_subprocess_poll(child, &process_result);
        if (poll == LSM_PROC_POLL_TERMINAL) {
            terminal = true;
            break;
        }
        if (poll == LSM_PROC_POLL_ERROR) {
            break;
        }
        FILE *marker = lsm_fopen(ready_path, "rb");
        if (marker) {
    ...
    lsm_version_cohort_manager_t *manager =
        ready ? lsm_version_cohort_manager_new(fixture.endpoint) : NULL;
    lsm_daemon_build_identity_t build_b = version_cohort_identity("2.5.0", VERSION_COHORT_BUILD_B);
    lsm_version_cohort_lease_t *lease = NULL;
    lsm_daemon_conflict_t conflict;
    lsm_version_cohort_status_t conflict_status =
        manager ? lsm_version_cohort_acquire(manager, &build_b, lsm_now_ms(), &lease, &conflict)
                : LSM_VERSION_COHORT_IO;
    bool conflict_lease_absent = lease == NULL;
    version_cohort_release(&lease);
    bool cancel_requested = child && !terminal && lsm_subprocess_request_cancel(child);
    uint64_t terminal_deadline = lsm_now_ms() + 5000U;
    while (child && !terminal && lsm_now_ms() < terminal_deadline) {
        lsm_proc_poll_t poll = lsm_subprocess_poll(child, &process_result);
        if (poll == LSM_PROC_POLL_TERMINAL) {
            terminal = true;
            break;
    ...
    lsm_daemon_conflict_t turnover_conflict;
    lsm_version_cohort_status_t turnover_status =
        manager && terminal
            ? lsm_version_cohort_acquire(manager, &build_b, UINT64_MAX, &lease, &turnover_conflict)
            : LSM_VERSION_COHORT_IO;

    version_cohort_release(&lease);
    version_cohort_manager_close(&manager);
    if (child && terminal) {
        lsm_subprocess_destroy(child);
        child = NULL;
    }
    (void)lsm_unlink(ready_path);
    version_cohort_fixture_finish(&fixture);

    ASSERT_TRUE(launch_ready);
    ASSERT_EQ(spawn_status, 0);
    ASSERT_TRUE(ready);
    ASSERT_EQ(conflict_status, LSM_VERSION_COHORT_CONFLICT);
    ...
    ASSERT_EQ(process_result.outcome, LSM_PROC_KILLED);
    ASSERT_EQ(turnover_status, LSM_VERSION_COHORT_OK);
    PASS();
#else
    version_cohort_fixture_t fixture;
    ...
    if (child == 0) {
        ...
        lsm_version_cohort_manager_t *manager = lsm_version_cohort_manager_new(fixture.endpoint);
        lsm_daemon_build_identity_t build_a =
            version_cohort_identity("2.4.0", VERSION_COHORT_BUILD_A);
        lsm_version_cohort_lease_t *lease = NULL;
        lsm_daemon_conflict_t conflict;
        bool acquired = manager && lsm_version_cohort_acquire(manager, &build_a, UINT64_MAX, &lease,
    ...
    lsm_daemon_conflict_t conflict;
    ASSERT_NOT_NULL(manager);
    ASSERT_EQ(lsm_version_cohort_acquire(manager, &build_b, lsm_now_ms(), &lease, &conflict),
              LSM_VERSION_COHORT_CONFLICT);
    ASSERT_NULL(lease);
    ASSERT_EQ(conflict.status, LSM_DAEMON_HELLO_VERSION_CONFLICT);
    ...
    ASSERT_EQ(lsm_version_cohort_acquire(manager, &build_b, UINT64_MAX, &lease, &conflict),
              LSM_VERSION_COHORT_OK);
    version_cohort_release(&lease);
    version_cohort_manager_close(&manager);
    version_cohort_fixture_finish(&fixture);
