...
TEST(arena_init_default) {
    ...
    ASSERT_EQ(a.block_size, LSM_ARENA_DEFAULT_BLOCK_SIZE);
    ASSERT_EQ(a.used, 0);
...
TEST(arena_alloc_null_arena) {
    void *p = lsm_arena_alloc(NULL, 16);
    ASSERT_NULL(p);
    PASS();
}
...
TEST(arena_alloc_alignment) {
    ...
    void *p1 = lsm_arena_alloc(&a, 1);
    void *p2 = lsm_arena_alloc(&a, 1);
    ASSERT_NOT_NULL(p1);
    ASSERT_NOT_NULL(p2);
...
TEST(arena_alloc_grows_blocks) {
    LSMArena a;
    lsm_arena_init_sized(&a, 64); /* tiny block to force growth */
    /* Allocate more than one block's worth */
    void *p1 = lsm_arena_alloc(&a, 48);
    ASSERT_NOT_NULL(p1);
    ASSERT_EQ(a.nblocks, 1);
    void *p2 = lsm_arena_alloc(&a, 48);
    ASSERT_NOT_NULL(p2);
...
TEST(arena_calloc) {
    LSMArena a;
    lsm_arena_init(&a);
    unsigned char *p = (unsigned char *)lsm_arena_calloc(&a, 64);
    ASSERT_NOT_NULL(p);
...
TEST(arena_strdup) {
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_strdup(&a, "hello world");
    ASSERT_NOT_NULL(s);
...
TEST(arena_strdup_null) {
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_strdup(&a, NULL);
    ASSERT_NULL(s);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(arena_strndup) {
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_strndup(&a, "hello world", 5);
    ASSERT_NOT_NULL(s);
...
TEST(arena_sprintf) {
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_sprintf(&a, "%s.%s.%s", "project", "path", "name");
    ASSERT_NOT_NULL(s);
...
TEST(arena_sprintf_int) {
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_sprintf(&a, "count=%d", 42);
    ASSERT_NOT_NULL(s);
...
TEST(arena_total) {
    LSMArena a;
    lsm_arena_init(&a);
    ASSERT_EQ(lsm_arena_total(&a), 0);
    lsm_arena_alloc(&a, 100);
    ASSERT_GTE(lsm_arena_total(&a), 100); /* at least 100 (may be aligned) */
    lsm_arena_alloc(&a, 200);
    ASSERT_GTE(lsm_arena_total(&a), 300);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(arena_multiple_resets) {
    ...
    void *p = lsm_arena_alloc(&a, 60000);
    ASSERT_NOT_NULL(p);
    lsm_arena_alloc(&a, 60000);
    ASSERT_GTE(a.nblocks, 2);
    lsm_arena_reset(&a);
    ASSERT_EQ(a.nblocks, 1);
    ASSERT_EQ(a.used, 0);
...
TEST(arena_reset_single_block) {
    ...
    LSMArena a;
    lsm_arena_init_sized(&a, 1024);
    lsm_arena_alloc(&a, 16);
    ASSERT_EQ(a.nblocks, 1);
    lsm_arena_reset(&a);
    ASSERT_EQ(a.nblocks, 1);
    ASSERT_EQ(a.used, 0);
    /* Still usable */
    void *p = lsm_arena_alloc(&a, 8);
    ASSERT_NOT_NULL(p);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(arena_strndup_len_exceeds_string) {
    ...
    char *s = lsm_arena_strndup(&a, src, 3);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "abc");
    /* Now test with len matching exactly */
    s = lsm_arena_strndup(&a, src, 2);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "ab");
    lsm_arena_destroy(&a);
    PASS();
...
TEST(arena_many_large_allocs_block_growth) {
    ...
    LSMArena a;
    lsm_arena_init_sized(&a, 64); /* tiny blocks */
    for (int i = 0; i < 50; i++) {
        void *p = lsm_arena_alloc(&a, 128); /* each > block_size initially */
        ASSERT_NOT_NULL(p);
    }
    /* Should have grown many blocks */
    ASSERT_GT(a.nblocks, 1);
    ASSERT_GTE(lsm_arena_total(&a), 50 * 128);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(arena_total_through_reset) {
    ...
    lsm_arena_init_sized(&a, 1024);
    lsm_arena_alloc(&a, 100);
    ASSERT_GTE(lsm_arena_total(&a), 100);
    size_t before_reset = lsm_arena_total(&a);
    ...
    lsm_arena_alloc(&a, 50);
    ASSERT_GTE(lsm_arena_total(&a), 50);
    /* But should be less than before_reset (fresh accumulation) */
    ASSERT_LTE(lsm_arena_total(&a), before_reset);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(arena_reset_block_size_invariant) {
    ...
    lsm_arena_alloc(&a, 100);
    lsm_arena_alloc(&a, 100); /* triggers grow */
    ASSERT_GTE(a.nblocks, 2);
...
TEST(arena_strndup_zero_len) {
    ...
    LSMArena a;
    lsm_arena_init(&a);
    char *s = lsm_arena_strndup(&a, "hello", 0);
