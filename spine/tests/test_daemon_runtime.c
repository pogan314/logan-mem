...
#define RUNTIME_TEST_BLOCKING_GIT_MARKER_ENV "LSM_TEST_RUNTIME_BLOCKING_GIT_PID_FILE"

_Static_assert(LSM_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN == 8 &&
...
#endif
static char runtime_self_build[LSM_DAEMON_BUILD_FINGERPRINT_SIZE];
...
static lsm_daemon_build_identity_t runtime_test_identity(const char *version, const char *build) {
    lsm_daemon_build_identity_t identity = {
...
static const char *runtime_test_self_build(void) {
    if (runtime_self_build[0] == '\0') {
        (void)lsm_daemon_runtime_process_build_fingerprint(runtime_test_process_id(),
                                                           runtime_self_build);
    }
...
static bool runtime_test_is_fingerprint(const char *value) {
    if (!value || strlen(value) != LSM_DAEMON_BUILD_FINGERPRINT_SIZE - 1) {
        return false;
    }
    for (size_t i = 0; i < LSM_DAEMON_BUILD_FINGERPRINT_SIZE - 1; i++) {
        char ch = value[i];
...
static bool runtime_test_windows_copy_self(const char *destination) {
    ...
    wchar_t *destination_wide = lsm_path_to_wide(destination);
...
static bool runtime_test_windows_posix_replace(const char *source, const char *destination) {
    wchar_t *source_wide = lsm_utf8_to_wide(source);
    wchar_t *destination_wide = lsm_utf8_to_wide(destination);
...
static bool runtime_test_paths_refer_to_same_file(const char *left, const char *right) {
    ...
    if (!left || !right || !lsm_canonical_path(left, left_canonical, sizeof(left_canonical)) ||
        !lsm_canonical_path(right, right_canonical, sizeof(right_canonical))) {
        return false;
    }
#ifdef _WIN32
...
static bool runtime_test_process_image_matches(uint64_t process_id, const char *expected_image) {
    ...
    char *utf8 = queried ? lsm_wide_to_utf8(wide) : NULL;
    if (utf8) {
...
static bool runtime_test_mac_ad_hoc_sign(const char *path) {
    ...
    if (child == 0) {
        execl("/usr/bin/codesign", "codesign", "--force", "--sign", "-", "--timestamp=none",
              "--identifier", "org.deusdata.lsm.foreign-test", path, (char *)NULL);
        _exit(127);
    }
    int status = 0;
    pid_t waited;
...
static lsm_daemon_runtime_application_session_t *runtime_application_session_open(
...
static lsm_daemon_runtime_application_status_t runtime_application_request(
...
static void *runtime_application_client_request_thread(void *opaque) {
    ...
    call->status = call->tagged
                       ? lsm_daemon_runtime_client_application_request_tagged(
                       ...
                       : lsm_daemon_runtime_client_application_request(
...
static void *runtime_real_application_detect_changes_thread(void *opaque) {
    ...
    call->status =
        lsm_daemon_application_client_tool(call->client, "detect_changes", call->arguments,
                                           &call->response, &call->response_length, timeout_ms);
    atomic_store_explicit(&call->completed, true, memory_order_release);
    return NULL;
}

static bool runtime_real_application_ingest_probe(lsm_daemon_runtime_client_t *client) {
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    lsm_daemon_runtime_application_status_t status =
        lsm_daemon_application_client_tool(client, "ingest_traces", "{\"traces\":[]}", &response,
                                           &response_length, RUNTIME_TEST_TIMEOUT_MS);
    bool usable = status == LSM_DAEMON_RUNTIME_APPLICATION_OK && response && response_length > 0 &&
...
static void runtime_test_restore_environment(const char *name, const char *saved_value,
                                             bool was_set) {
    if (was_set) {
        (void)lsm_setenv(name, saved_value, 1);
    } else {
...
static void *runtime_application_client_connect_thread(void *opaque) {
    ...
    call->client = lsm_daemon_runtime_client_connect(call->endpoint, &call->identity,
...
static lsm_daemon_runtime_application_callbacks_t runtime_application_callbacks(
    runtime_application_context_t *context) {
...
static bool runtime_test_wait_atomic_bool(atomic_bool *value, uint32_t timeout_ms) {
    uint64_t deadline = lsm_now_ms() + (uint64_t)timeout_ms;
...
static bool runtime_test_raw_activation_exchange(
    const lsm_daemon_ipc_endpoint_t *endpoint, const uint8_t *request, uint32_t request_length,
    ...
    lsm_daemon_ipc_connection_t *connection =
        lsm_daemon_ipc_connect(endpoint, RUNTIME_TEST_TIMEOUT_MS);
    ...
    uint8_t *response = NULL;
    int received =
        sent ? lsm_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &response)
             : 0;
...
static bool runtime_test_activation_request_encode(
    ...
    return lsm_daemon_runtime_hello_request_encode(out + RUNTIME_TEST_ACTIVATION_IDENTITY_OFFSET,
                                                   identity);
}
...
static lsm_daemon_ipc_connection_t *runtime_test_raw_client_connect(
    const lsm_daemon_ipc_endpoint_t *endpoint, const lsm_daemon_build_identity_t *identity) {
    uint8_t hello[LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    if (!lsm_daemon_runtime_hello_request_encode(hello, identity)) {
        return NULL;
    }
    lsm_daemon_ipc_connection_t *connection =
        lsm_daemon_ipc_connect(endpoint, RUNTIME_TEST_TIMEOUT_MS);
    if (!connection ||
        !lsm_daemon_ipc_send_frame(connection, LSM_DAEMON_FRAME_REQUEST,
                                   LSM_DAEMON_RUNTIME_OP_HELLO, hello, (uint32_t)sizeof(hello))) {
        lsm_daemon_ipc_connection_close(connection);
        return NULL;
    }
    lsm_daemon_frame_t frame = {0};
    ...
    int received =
        lsm_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
    bool accepted = received == 1 && frame.type == LSM_DAEMON_FRAME_RESPONSE &&
                    frame.flags == LSM_DAEMON_RUNTIME_OP_HELLO && frame.length > 0;
    free(payload);
    if (!accepted) {
        lsm_daemon_ipc_connection_close(connection);
        return NULL;
...
static bool runtime_test_raw_application_send_token(lsm_daemon_ipc_connection_t *connection,
    ...
    bool sent = lsm_daemon_ipc_send_frame(connection, LSM_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_send(lsm_daemon_ipc_connection_t *connection,
...
static bool runtime_test_raw_application_cancel(lsm_daemon_ipc_connection_t *connection,
    ...
    runtime_test_put_u64(wire, request_token);
    return lsm_daemon_ipc_send_frame(connection, LSM_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_receive_status_token(
    lsm_daemon_ipc_connection_t *connection, uint64_t expected_token,
    lsm_daemon_runtime_application_status_t expected_status) {
    lsm_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received =
        lsm_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
    bool valid = received == 1 && frame.type == LSM_DAEMON_FRAME_RESPONSE &&
                 frame.flags == LSM_DAEMON_RUNTIME_OP_APPLICATION_REQUEST && frame.length >= 16 &&
...
static bool runtime_test_fixture_start_configured(
    runtime_test_fixture_t *fixture, const char *tag, const lsm_daemon_build_identity_t *identity,
    ...
    if (!lsm_daemon_rendezvous_key(fixture->key)) {
        return runtime_test_fixture_start_failed(tag, "rendezvous-key",
                                                 runtime_test_last_os_error());
    }
    fixture->endpoint = lsm_daemon_ipc_endpoint_new(fixture->key, fixture->parent);
    if (!fixture->endpoint) {
    ...
    if (!runtime_test_copy_path(fixture->runtime_dir,
                                lsm_daemon_ipc_endpoint_runtime_dir(fixture->endpoint))) {
    ...
    fixture->service = lsm_daemon_runtime_service_start(&config);
    if (!fixture->service) {
        return runtime_test_fixture_start_failed(tag, "service-start",
                                                 runtime_test_last_os_error());
    }
    lsm_daemon_runtime_service_state_t state = lsm_daemon_runtime_service_state(fixture->service);
    if (state != LSM_DAEMON_RUNTIME_SERVICE_RUNNING) {
...
TEST(daemon_runtime_convenience_service_owns_participant_guard) {
    ...
    int active = started ? lsm_daemon_ipc_legacy_generation_probe(fixture.endpoint) : -1;
    bool running_free_refused = started && !lsm_daemon_runtime_service_free(fixture.service);
    bool stopped =
        started && lsm_daemon_runtime_service_stop(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    bool freed = stopped && lsm_daemon_runtime_service_free(fixture.service);
    if (freed) {
        fixture.service = NULL;
    }
    int released = freed ? lsm_daemon_ipc_legacy_generation_probe(fixture.endpoint) : -1;
...
static bool runtime_test_fixture_start_application(runtime_test_fixture_t *fixture, const char *tag,
                                                   const lsm_daemon_build_identity_t *identity,
                                                   runtime_application_context_t *context) {
    lsm_daemon_runtime_application_callbacks_t application = runtime_application_callbacks(context);
...
static void runtime_test_fixture_finish(runtime_test_fixture_t *fixture) {
    if (fixture->service) {
        lsm_daemon_runtime_service_state_t state =
            lsm_daemon_runtime_service_state(fixture->service);
        bool stopped =
            state == LSM_DAEMON_RUNTIME_SERVICE_EXITED ||
            lsm_daemon_runtime_service_stop(fixture->service, RUNTIME_TEST_CLEANUP_TIMEOUT_MS);
        bool freed = false;
        for (size_t attempt = 0; stopped && !freed && attempt < RUNTIME_TEST_CLEANUP_FREE_ATTEMPTS;
             attempt++) {
            freed = lsm_daemon_runtime_service_free(fixture->service);
            if (!freed) {
                lsm_usleep(1000);
            }
        }
    ...
    (void)lsm_unlink(fixture->rotated_log_path);
    (void)lsm_unlink(fixture->log_path);
    (void)lsm_unlink(fixture->lock_log_path);
...
TEST(daemon_host_early_coordination_failure_is_durable) {
    ...
    int log_written =
        parent_created ? snprintf(log_path, sizeof(log_path), "%s/logs/lsm-daemon.log", cache) : -1;
    bool environment_ready = cache_written > 0 && cache_written < (int)sizeof(cache) &&
                             log_written > 0 && log_written < (int)sizeof(log_path) &&
                             lsm_mkdir_p(cache, 0700) && lsm_setenv("LSM_CACHE_DIR", cache, 1) == 0;
    lsm_daemon_ipc_endpoint_t *endpoint =
        environment_ready ? lsm_daemon_ipc_endpoint_new("0123456789abcdef", parent) : NULL;
    lsm_version_cohort_manager_t *active_manager =
        endpoint ? lsm_version_cohort_manager_new(endpoint) : NULL;
    lsm_version_cohort_lease_t *active_lease = NULL;
    lsm_version_cohort_daemon_claim_t *active_claim = NULL;
    lsm_daemon_conflict_t active_conflict;
    lsm_daemon_build_identity_t active_identity =
        runtime_test_identity("active-host", runtime_test_self_build());
    active_identity.cache_fingerprint = RUNTIME_BUILD_B;
    lsm_version_cohort_status_t active_status =
        active_manager ? lsm_version_cohort_acquire(active_manager, &active_identity, UINT64_MAX,
                                                    &active_lease, &active_conflict)
                       : LSM_VERSION_COHORT_IO;
    lsm_version_cohort_status_t claim_status =
        active_status == LSM_VERSION_COHORT_OK
            ? lsm_version_cohort_daemon_claim_acquire(active_manager, &active_claim)
            : LSM_VERSION_COHORT_IO;
    atomic_int stop_requested = ATOMIC_VAR_INIT(0);
    lsm_daemon_host_config_t config = {
        .endpoint = endpoint,
        .identity = active_identity,
        .executable_path = "/host-early-log-test",
        .stop_requested = &stop_requested,
    };
    int run_result = claim_status == LSM_VERSION_COHORT_OK ? lsm_daemon_host_run(&config) : 0;
    ...
    while (active_claim &&
           lsm_version_cohort_daemon_claim_release(&active_claim) != LSM_PRIVATE_FILE_LOCK_OK) {
        lsm_usleep(1000);
    }
    while (active_lease &&
           lsm_version_cohort_lease_release(&active_lease) != LSM_PRIVATE_FILE_LOCK_OK) {
        lsm_usleep(1000);
    }
    while (active_manager &&
           lsm_version_cohort_manager_free(&active_manager) != LSM_PRIVATE_FILE_LOCK_OK) {
        lsm_usleep(1000);
    }
    lsm_daemon_ipc_endpoint_free(endpoint);
    runtime_test_restore_environment("LSM_CACHE_DIR", saved_cache, had_cache);
    free(saved_cache);
    ...
    ASSERT_EQ(active_status, LSM_VERSION_COHORT_OK);
    ASSERT_EQ(claim_status, LSM_VERSION_COHORT_OK);
...
TEST(daemon_host_refuses_unopenable_runtime_config_database) {
    ...
    bool blocker_created = cache_written > 0 && cache_written < (int)sizeof(cache) &&
                           ...
                           lsm_mkdir_p(cache, 0700) && lsm_mkdir_p(config_blocker, 0700);
    bool environment_ready = blocker_created && lsm_setenv("LSM_CACHE_DIR", cache, 1) == 0;
    lsm_daemon_ipc_endpoint_t *endpoint =
        environment_ready ? lsm_daemon_ipc_endpoint_new("0123456789abcdef", parent) : NULL;
    bool endpoint_created = endpoint != NULL;
    bool prepared = endpoint && lsm_daemon_host_state_prepare_for_test(endpoint);
...
TEST(daemon_host_http_reconcile_rate_limits_and_retries_transient_failures) {
    ...
    lsm_daemon_host_http_reconcile_test_result_t result = {0};
    bool driven = lsm_daemon_host_http_reconcile_sequence_for_test(
...
TEST(daemon_host_http_reconcile_retains_busy_server_until_free_succeeds) {
    lsm_daemon_host_http_free_refusal_test_result_t result = {0};
    bool driven = lsm_daemon_host_http_reconcile_free_refusal_for_test(&result);
...
static int runtime_test_failed_host_child(const char *parent, const char *key) {
    ...
    int log_written = snprintf(log_path, sizeof(log_path), "%s/logs/lsm-daemon.log", cache);
    if (cache_written <= 0 || cache_written >= (int)sizeof(cache) || log_written <= 0 ||
        log_written >= (int)sizeof(log_path) || lsm_setenv("LSM_CACHE_DIR", cache, 1) != 0) {
        return 40;
    }

    lsm_daemon_ipc_endpoint_t *endpoint = lsm_daemon_ipc_endpoint_new(key, parent);
    atomic_int stop_requested = ATOMIC_VAR_INIT(0);
    lsm_daemon_build_identity_t mismatched =
    ...
    int run_result = endpoint ? lsm_daemon_host_run(&config) : 0;
...
TEST(daemon_host_failed_listener_reservation_starts_no_background_work) {
    char parent[RUNTIME_TEST_PATH_CAP];
    int written = snprintf(parent, sizeof(parent), "%s/lsm-host-order-XXXXXX", lsm_tmpdir());
    bool parent_created =
        written > 0 && written < (int)sizeof(parent) && lsm_mkdtemp(parent) != NULL;
    char key[LSM_DAEMON_KEY_SIZE] = {0};
    lsm_daemon_ipc_endpoint_t *endpoint = parent_created && lsm_daemon_rendezvous_key(key)
                                              ? lsm_daemon_ipc_endpoint_new(key, parent)
                                              : NULL;
    lsm_daemon_ipc_startup_lock_t *startup = NULL;
    bool setup = endpoint && lsm_daemon_ipc_startup_lock_try_acquire(endpoint, &startup) == 1 &&
    ...
    bool startup_released = lsm_daemon_ipc_startup_lock_release(&startup);
    lsm_daemon_ipc_endpoint_free(endpoint);
...
static _Noreturn void runtime_test_forced_host_shutdown_child(const char *parent) {
    ...
    (void)alarm(2);
    lsm_daemon_host_force_terminate_for_test("noncooperative_callback");
}
...
static _Noreturn void runtime_test_persistent_cleanup_failure_child(const char *parent) {
    ...
    lsm_daemon_host_cleanup_release_until_complete_for_test(
...
TEST(daemon_host_persistent_cleanup_release_failure_is_process_bounded) {
    ...
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written =
        snprintf(parent, sizeof(parent), "%s/lsm-host-cleanup-XXXXXX", lsm_tmpdir());
    bool parent_created =
        parent_written > 0 && parent_written < (int)sizeof(parent) && lsm_mkdtemp(parent) != NULL;
    int log_written = parent_created ? snprintf(log_path, sizeof(log_path),
                                                "%s/cache/logs/lsm-daemon.log", parent)
                                     : -1;
    ...
    uint64_t deadline = lsm_now_ms() + RUNTIME_TEST_TIMEOUT_MS;
    while (child > 0 && lsm_now_ms() < deadline) {
...
TEST(daemon_host_forced_shutdown_is_logged_flushed_and_process_bounded) {
    char parent[RUNTIME_TEST_PATH_CAP];
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written = snprintf(parent, sizeof(parent), "%s/lsm-host-force-XXXXXX", lsm_tmpdir());
    bool parent_created =
        parent_written > 0 && parent_written < (int)sizeof(parent) && lsm_mkdtemp(parent) != NULL;
    int log_written = parent_created ? snprintf(log_path, sizeof(log_path),
                                                "%s/cache/logs/lsm-daemon.log", parent)
...
TEST(daemon_runtime_exact_hello_issues_connection_bound_identity) {
    ...
    lsm_daemon_runtime_connect_result_t result = {0};
    lsm_daemon_runtime_client_t *client = NULL;
    bool accepted = false;
    ...
    if (started) {
        ...
        client = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    if (client) {
        uint64_t expected_pid = runtime_test_process_id();
        accepted = result.status == LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
                   result.hello_status == LSM_DAEMON_HELLO_COMPATIBLE &&
                   result.client_id != LSM_DAEMON_CLIENT_ID_INVALID &&
                   result.client_id == lsm_daemon_runtime_client_id(client);
        identity_anchored = result.authenticated_process_id == expected_pid &&
                            lsm_daemon_runtime_client_process_id(client) == expected_pid &&
                            lsm_daemon_runtime_service_client_process_id(
                                fixture.service, result.client_id) == expected_pid;
        closed = lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }

    if (client) {
        (void)lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_image_rejection_reaches_client_issue1383) {
    ...
    lsm_daemon_runtime_force_peer_image_mismatch_for_testing(true);
    if (started) {
        client = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    lsm_daemon_runtime_force_peer_image_mismatch_for_testing(false);
 ...
 * check — it is a peer we could not look at. Every `npx logan-spine-mcp`
...
TEST(daemon_runtime_unverifiable_image_is_admitted_issue1539) {
    ...
    lsm_daemon_runtime_force_peer_image_unverified_for_testing(true);
    if (started) {
        client = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    lsm_daemon_runtime_force_peer_image_unverified_for_testing(false);

    bool admitted = client != NULL && result.status == LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    if (client) {
        admitted = lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS) && admitted;
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_unexpected_frame_payload_is_freed_once) {
    ...
    lsm_daemon_runtime_connect_result_t owner_result = {0};
    lsm_daemon_runtime_client_t *owner = NULL;
    lsm_daemon_ipc_connection_t *raw = NULL;
    ...
    if (started) {
        owner = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
    ...
    if (raw) {
        unexpected_sent = lsm_daemon_ipc_send_frame(
            raw, LSM_DAEMON_FRAME_RESPONSE, LSM_DAEMON_RUNTIME_OP_HEARTBEAT, unexpected_payload,
            (uint32_t)sizeof(unexpected_payload));
    }
    if (unexpected_sent) {
        bad_peer_released = lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                        RUNTIME_TEST_TIMEOUT_MS);
    }
    if (bad_peer_released) {
        lsm_daemon_frame_t frame = {0};
        uint8_t *payload = NULL;
        int received = lsm_daemon_ipc_receive_frame(raw, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
        bad_peer_closed = received != 1;
        free(payload);
        owner_survived = lsm_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    lsm_daemon_ipc_connection_close(raw);
    raw = NULL;
    if (owner) {
        owner_closed = lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    if (started) {
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    if (owner) {
        (void)lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    lsm_daemon_ipc_connection_close(raw);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_activation_rejects_forged_and_malformed_without_stop) {
    lsm_daemon_build_identity_t identity =
        runtime_test_identity("2.4.0", runtime_test_self_build());
    lsm_daemon_build_identity_t forged = runtime_test_identity("9.9.9", RUNTIME_BUILD_B);
    runtime_test_fixture_t fixture;
    ...
    bool encoded = runtime_test_activation_request_encode(
        request, LSM_DAEMON_RUNTIME_ACTIVATION_UPDATE, &identity);
    lsm_daemon_runtime_activation_result_t short_result = {0};
    lsm_daemon_runtime_activation_result_t action_result = {0};
    lsm_daemon_runtime_activation_result_t abi_result = {0};
    lsm_daemon_runtime_activation_result_t forged_result = {0};
    ...
    if (owner && encoded) {
        ...
        runtime_test_put_u32(request, (uint32_t)LSM_DAEMON_RUNTIME_ACTIVATION_UPDATE);
        ...
        abi_rejected = runtime_test_raw_activation_exchange(fixture.endpoint, request,
                                                            ...
                                                            &abi_result) &&
                       !abi_result.accepted;
        forged_rejected = lsm_daemon_runtime_request_activation_shutdown(
                              fixture.endpoint, &forged, LSM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL,
                              RUNTIME_TEST_TIMEOUT_MS, &forged_result) &&
                          !forged_result.accepted;
        unchanged = lsm_daemon_runtime_service_state(fixture.service) ==
                        LSM_DAEMON_RUNTIME_SERVICE_RUNNING &&
                    lsm_daemon_runtime_service_active_clients(fixture.service) == 1 &&
                    lsm_daemon_runtime_service_wait_for_connections(fixture.service, 1,
                                                                    RUNTIME_TEST_TIMEOUT_MS);
        heartbeat = lsm_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        closed = lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    if (started) {
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    ASSERT_TRUE(started);
    ASSERT_EQ(owner_result.status, LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_TRUE(encoded);
...
TEST(daemon_runtime_activation_ack_snapshots_then_interrupts_all_clients) {
    ...
    lsm_daemon_runtime_connect_result_t first_result = {0};
    lsm_daemon_runtime_connect_result_t second_result = {0};
    lsm_daemon_runtime_client_t *first = NULL;
    lsm_daemon_runtime_client_t *second = NULL;
    lsm_daemon_runtime_activation_result_t activation = {0};
    ...
    lsm_log_set_sink(runtime_test_activation_shutdown_sink);

    if (started) {
        first = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &first_result);
        second = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &second_result);
    }
    if (first && second) {
        requested = lsm_daemon_runtime_request_activation_shutdown(
            fixture.endpoint, &identity, LSM_DAEMON_RUNTIME_ACTIVATION_UPDATE,
            RUNTIME_TEST_TIMEOUT_MS, &activation);
        first_interrupted = !lsm_daemon_runtime_client_heartbeat(first, RUNTIME_TEST_TIMEOUT_MS);
        second_interrupted = !lsm_daemon_runtime_client_heartbeat(second, RUNTIME_TEST_TIMEOUT_MS);
    }
    lsm_log_set_sink(NULL);
    if (first) {
        (void)lsm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
    }
    if (second) {
        (void)lsm_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
    }
    if (started) {
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    ASSERT_TRUE(started);
    ASSERT_EQ(first_result.status, LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_EQ(second_result.status, LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED);
...
TEST(daemon_runtime_activation_accepts_authenticated_different_build) {
    ...
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/lsm-runtime-activation-XXXXXX", lsm_tmpdir());
    bool directory_created = directory_written > 0 && directory_written < (int)sizeof(directory) &&
                             lsm_mkdtemp(directory) != NULL;
...
#endif
    char foreign_build[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool fingerprinted =
        runnable && lsm_daemon_build_fingerprint_file(foreign_image, foreign_build);
    bool differs = fingerprinted && strcmp(foreign_build, active_identity.build_fingerprint) != 0;
    lsm_daemon_build_identity_t foreign_identity = runtime_test_identity("9.9.9", foreign_build);
    ...
    lsm_daemon_runtime_client_t *owner = NULL;
    if (started) {
        owner = lsm_daemon_runtime_client_connect(fixture.endpoint, &active_identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    int foreign_exit = -1;
    bool foreign_ran =
        ...
        runtime_test_run_activation_image(foreign_image, &fixture, &foreign_identity,
                                          LSM_DAEMON_RUNTIME_ACTIVATION_INSTALL, &foreign_exit);
    bool owner_interrupted = owner && foreign_ran &&
                             !lsm_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    if (owner) {
        (void)lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    bool exited =
        started && lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    runtime_test_fixture_finish(&fixture);
    if (image_written > 0 && image_written < (int)sizeof(foreign_image)) {
        (void)lsm_unlink(foreign_image);
    }
    if (directory_created) {
        (void)lsm_rmdir(directory);
    }
...
TEST(daemon_runtime_rendezvous_layout_is_frozen_and_detailed_abi_independent) {
    lsm_daemon_build_identity_t first = runtime_test_identity("2.4.0", runtime_test_self_build());
    lsm_daemon_build_identity_t different_detail = first;
    different_detail.protocol_abi = 0;
    different_detail.store_abi = 0;
    different_detail.feature_abi = 0;

    uint8_t first_wire[LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    uint8_t second_wire[LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    bool first_encoded = lsm_daemon_runtime_hello_request_encode(first_wire, &first);
    bool second_encoded = lsm_daemon_runtime_hello_request_encode(second_wire, &different_detail);

    ASSERT_TRUE(first_encoded);
    ASSERT_TRUE(second_encoded);
    ASSERT_EQ(LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE, RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
    ASSERT_EQ(LSM_DAEMON_RENDEZVOUS_RESPONSE_SIZE, RUNTIME_TEST_RENDEZVOUS_RESPONSE_SIZE);
    ASSERT_EQ(LSM_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, LSM_DAEMON_VERSION_TEXT_SIZE);
    ASSERT_EQ(LSM_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, LSM_DAEMON_BUILD_FINGERPRINT_SIZE);
    ASSERT_EQ(LSM_DAEMON_RENDEZVOUS_MESSAGE_CAP, LSM_DAEMON_CONFLICT_MESSAGE_SIZE);
...
TEST(daemon_runtime_future_generation_gets_stable_explicit_conflict) {
    ...
    lsm_daemon_build_identity_t active = runtime_test_identity("2.4.0", active_build);
    lsm_daemon_build_identity_t future =
    ...
    lsm_daemon_runtime_connect_result_t owner_result = {0};
    lsm_daemon_runtime_client_t *owner = NULL;
    lsm_daemon_ipc_connection_t *future_connection = NULL;
    uint8_t future_wire[LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    bool encoded = false;
    bool sent = false;
    bool explicit_conflict = false;
    bool logged = false;
    bool exited = false;
    lsm_daemon_frame_t response_frame = {0};
    uint8_t *response_payload = NULL;
    char log[RUNTIME_TEST_LOG_CAP] = {0};
    char expected_message[LSM_DAEMON_CONFLICT_MESSAGE_SIZE] = {0};
    ...
    if (started) {
        owner = lsm_daemon_runtime_client_connect(fixture.endpoint, &active,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    if (owner) {
        encoded = lsm_daemon_runtime_hello_request_encode(future_wire, &future);
        future_connection = lsm_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (future_connection && encoded) {
        ...
        sent = lsm_daemon_ipc_send_frame(future_connection, LSM_DAEMON_FRAME_REQUEST,
                                         LSM_DAEMON_RUNTIME_OP_HELLO, future_wire,
                                         RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
        int received = lsm_daemon_ipc_receive_frame(future_connection, RUNTIME_TEST_TIMEOUT_MS,
                                                    &response_frame, &response_payload);
        explicit_conflict =
            expected_message_valid && received == 1 && response_payload &&
            response_frame.type == LSM_DAEMON_FRAME_RESPONSE &&
            response_frame.flags == LSM_DAEMON_RUNTIME_OP_HELLO &&
            response_frame.length == RUNTIME_TEST_RENDEZVOUS_RESPONSE_SIZE &&
            runtime_test_get_u32(response_payload) == LSM_DAEMON_RUNTIME_CONNECT_CONFLICT &&
            runtime_test_get_u32(response_payload + 4) == LSM_DAEMON_HELLO_VERSION_CONFLICT &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_ACTIVE_VERSION_OFFSET,
                LSM_DAEMON_VERSION_TEXT_SIZE, active.semantic_version) &&
            runtime_test_fixed_string_equals(response_payload +
                                                 RUNTIME_TEST_RENDEZVOUS_ACTIVE_BUILD_OFFSET,
                                             LSM_DAEMON_BUILD_FINGERPRINT_SIZE, active_build) &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_REQUESTED_VERSION_OFFSET,
                LSM_DAEMON_VERSION_TEXT_SIZE, future.semantic_version) &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_REQUESTED_BUILD_OFFSET,
                LSM_DAEMON_BUILD_FINGERPRINT_SIZE, future.build_fingerprint) &&
            runtime_test_fixed_string_equals(response_payload +
                                                 RUNTIME_TEST_RENDEZVOUS_MESSAGE_OFFSET,
                                             LSM_DAEMON_CONFLICT_MESSAGE_SIZE, expected_message);
    }
    lsm_daemon_ipc_connection_close(future_connection);
...
TEST(daemon_runtime_matching_clients_share_one_service_endpoint) {
    ...
    lsm_daemon_ipc_endpoint_t *same_endpoint = NULL;
    lsm_daemon_runtime_client_t *first = NULL;
    lsm_daemon_runtime_client_t *second = NULL;
    lsm_daemon_runtime_connect_result_t first_result = {0};
    lsm_daemon_runtime_connect_result_t second_result = {0};
    ...
    char key[LSM_DAEMON_KEY_SIZE];
    if (started && lsm_daemon_rendezvous_key(key)) {
        same_endpoint = lsm_daemon_ipc_endpoint_new(key, fixture.parent);
    }
    if (same_endpoint) {
        same_address = strcmp(lsm_daemon_ipc_endpoint_address(fixture.endpoint),
                              lsm_daemon_ipc_endpoint_address(same_endpoint)) == 0;
        first = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &first_result);
        second = lsm_daemon_runtime_client_connect(same_endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &second_result);
    }
    if (first && second) {
        both_registered = first_result.client_id != second_result.client_id &&
                          lsm_daemon_runtime_service_active_clients(fixture.service) == 2;
        (void)lsm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
        one_remains = lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                  RUNTIME_TEST_TIMEOUT_MS);
        (void)lsm_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    lsm_daemon_ipc_endpoint_free(same_endpoint);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_same_version_different_build_is_visible_and_logged) {
    const char *active_build = runtime_test_self_build();
    lsm_daemon_build_identity_t active = runtime_test_identity("2.4.0", active_build);
    lsm_daemon_build_identity_t rebuilt = runtime_test_identity("2.4.0", RUNTIME_BUILD_B);
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start(&fixture, "build-conflict", &active);
    lsm_daemon_runtime_connect_result_t owner_result = {0};
    lsm_daemon_runtime_connect_result_t conflict_result = {0};
    lsm_daemon_runtime_client_t *owner = NULL;
    lsm_daemon_runtime_client_t *rejected = NULL;
    bool explicit_conflict = false;
    bool logged = false;
    ...
    if (owner) {
        rejected = lsm_daemon_runtime_client_connect(fixture.endpoint, &rebuilt,
                                                     RUNTIME_TEST_TIMEOUT_MS, &conflict_result);
        explicit_conflict = rejected == NULL &&
                            conflict_result.status == LSM_DAEMON_RUNTIME_CONNECT_CONFLICT &&
                            conflict_result.hello_status == LSM_DAEMON_HELLO_BUILD_CONFLICT &&
        ...
        owner_unchanged = lsm_daemon_runtime_service_active_clients(fixture.service) == 1;
        logged = runtime_test_read_log(fixture.log_path, log) &&
    ...
    if (rejected) {
        (void)lsm_daemon_runtime_client_close(rejected, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        (void)lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_conflict_log_failure_uses_operation_log_fallback) {
    ...
    bool obstruction_created = started && lsm_mkdir_p(fixture.log_path, 0700);
    lsm_daemon_runtime_connect_result_t owner_result = {0};
    lsm_daemon_runtime_connect_result_t conflict_result = {0};
    lsm_daemon_runtime_client_t *owner = NULL;
    lsm_daemon_runtime_client_t *rejected = NULL;
    LSMLogLevel prior_level = lsm_log_get_level();
    bool explicit_conflict = false;
    bool exited = false;

    atomic_store_explicit(&runtime_conflict_log_fallback_seen, false, memory_order_release);
    if (obstruction_created) {
        lsm_log_set_level(LSM_LOG_ERROR);
        lsm_log_set_sink(runtime_test_conflict_log_fallback_sink);
        owner = lsm_daemon_runtime_client_connect(fixture.endpoint, &active,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    if (owner) {
    ...
    if (obstruction_created) {
        (void)lsm_rmdir(fixture.log_path);
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_disconnect_releases_only_connection_subscriptions) {
    ...
    lsm_daemon_runtime_connect_result_t first_result = {0};
    lsm_daemon_runtime_connect_result_t second_result = {0};
    lsm_daemon_subscription_id_t first_subscription = LSM_DAEMON_SUBSCRIPTION_ID_INVALID;
    lsm_daemon_subscription_id_t second_subscription = LSM_DAEMON_SUBSCRIPTION_ID_INVALID;
    ...
    if (first && second) {
        lsm_daemon_subscription_result_t first_status = lsm_daemon_runtime_client_job_subscribe(
            first, project, &first_subscription, RUNTIME_TEST_TIMEOUT_MS);
        lsm_daemon_subscription_result_t second_status = lsm_daemon_runtime_client_job_subscribe(
            second, project, &second_subscription, RUNTIME_TEST_TIMEOUT_MS);
        subscribed = first_status == LSM_DAEMON_SUBSCRIPTION_STARTED &&
                     second_status == LSM_DAEMON_SUBSCRIPTION_JOINED &&
                     first_subscription != LSM_DAEMON_SUBSCRIPTION_ID_INVALID &&
                     second_subscription != LSM_DAEMON_SUBSCRIPTION_ID_INVALID &&
                     first_subscription != second_subscription &&
                     lsm_daemon_runtime_service_job_subscribers(fixture.service, project) == 2;

        (void)lsm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
        second_survived =
            lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                        RUNTIME_TEST_TIMEOUT_MS) &&
            lsm_daemon_runtime_service_job_subscribers(fixture.service, project) == 1 &&
            lsm_daemon_runtime_client_job_unsubscribe(second, second_subscription,
                                                      RUNTIME_TEST_TIMEOUT_MS) &&
            lsm_daemon_runtime_service_job_subscribers(fixture.service, project) == 0;
        reaped = lsm_daemon_runtime_service_job_reaped(fixture.service, project);
        (void)lsm_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_final_disconnect_automatically_exits_within_bound) {
    ...
    if (client) {
        (void)lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        lsm_daemon_runtime_service_state_t after_close =
            lsm_daemon_runtime_service_state(fixture.service);
        terminal_transition = after_close == LSM_DAEMON_RUNTIME_SERVICE_STOPPING ||
                              after_close == LSM_DAEMON_RUNTIME_SERVICE_EXITED;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_authenticated_idle_connection_outlives_lease_interval) {
    ...
    if (client) {
        ...
        (void)lsm_nanosleep(&beyond_lease, NULL);
        remained_connected = lsm_daemon_runtime_service_active_clients(fixture.service) == 1 &&
                             lsm_daemon_runtime_client_heartbeat(client, RUNTIME_TEST_TIMEOUT_MS);
        (void)lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_connection_cap_covers_slow_hello_and_stopping_is_terminal) {
    ...
    lsm_daemon_ipc_connection_t *slow_hello = NULL;
    lsm_daemon_runtime_client_t *accepted = NULL;
    lsm_daemon_runtime_client_t *overflow = NULL;
    lsm_daemon_runtime_client_t *resurrection = NULL;
    lsm_daemon_runtime_connect_result_t accepted_result = {0};
    lsm_daemon_runtime_connect_result_t overflow_result = {0};
    lsm_daemon_runtime_connect_result_t resurrection_result = {0};
    ...
    if (started) {
        slow_hello = lsm_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (slow_hello) {
        slow_slot_counted = lsm_daemon_runtime_service_wait_for_connections(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
        accepted = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                     RUNTIME_TEST_TIMEOUT_MS, &accepted_result);
    }
    if (accepted) {
        overflow = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                     RUNTIME_TEST_TIMEOUT_MS, &overflow_result);
        capacity_rejected = overflow == NULL &&
                            overflow_result.status == LSM_DAEMON_RUNTIME_CONNECT_REJECTED &&
                            strstr(overflow_result.message, "capacity") != NULL &&
                            lsm_daemon_runtime_service_active_connections(fixture.service) == 2 &&
                            lsm_daemon_runtime_service_active_clients(fixture.service) == 1;

        lsm_daemon_ipc_connection_close(slow_hello);
        slow_hello = NULL;
        slow_slot_released = lsm_daemon_runtime_service_wait_for_connections(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
        (void)lsm_daemon_runtime_client_close(accepted, RUNTIME_TEST_TIMEOUT_MS);
        accepted = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);

        resurrection = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity, 100,
                                                         &resurrection_result);
        lsm_daemon_runtime_service_state_t terminal_state =
            lsm_daemon_runtime_service_state(fixture.service);
        no_resurrection = resurrection == NULL &&
                          (resurrection_result.status == LSM_DAEMON_RUNTIME_CONNECT_ERROR ||
                           resurrection_result.status == LSM_DAEMON_RUNTIME_CONNECT_REJECTED) &&
                          lsm_daemon_runtime_service_active_clients(fixture.service) == 0 &&
                          (terminal_state == LSM_DAEMON_RUNTIME_SERVICE_STOPPING ||
                           terminal_state == LSM_DAEMON_RUNTIME_SERVICE_EXITED);
    }

    if (resurrection) {
        (void)lsm_daemon_runtime_client_close(resurrection, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (overflow) {
        (void)lsm_daemon_runtime_client_close(overflow, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (accepted) {
        (void)lsm_daemon_runtime_client_close(accepted, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_rejects_forged_identity_extension) {
    ...
    lsm_daemon_ipc_connection_t *raw = NULL;
    uint8_t forged[LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE + sizeof(uint64_t) * 2] = {0};
    ...
    if (started) {
        encoded = lsm_daemon_runtime_hello_request_encode(forged, &identity);
        memcpy(forged + LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE, &forged_client_id,
               sizeof(forged_client_id));
        memcpy(forged + LSM_DAEMON_RENDEZVOUS_REQUEST_SIZE + sizeof(forged_client_id),
               &forged_process_id, sizeof(forged_process_id));
        raw = lsm_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    connected = raw != NULL;
    if (raw && encoded) {
         ...
         * attacker-controlled bytes are read (lsm_daemon_ipc_receive_frame_bounded).
        ...
        sent = lsm_daemon_ipc_send_frame(raw, LSM_DAEMON_FRAME_REQUEST, LSM_DAEMON_RUNTIME_OP_HELLO,
                                         forged, (uint32_t)sizeof(forged));
        int received = sent ? lsm_daemon_ipc_receive_frame(raw, RUNTIME_TEST_TIMEOUT_MS,
                                                           &response_frame, &response_payload)
                            : 0;
        ...
        rejected = received != 1 && lsm_daemon_runtime_service_wait_for_clients(
    ...
    lsm_daemon_runtime_connect_result_t valid_result = {0};
    lsm_daemon_runtime_client_t *valid = NULL;
    bool valid_after_rejection = false;
    bool exited = false;
    if (started) {
        valid = lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &valid_result);
    }
    if (valid) {
        valid_after_rejection =
            valid_result.status == LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
            valid_result.authenticated_process_id == runtime_test_process_id() &&
            valid_result.client_id != forged_client_id;
        (void)lsm_daemon_runtime_client_close(valid, RUNTIME_TEST_TIMEOUT_MS);
        valid = NULL;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_application_response_roundtrip_is_byte_exact) {
    ...
    if (client) {
        status = lsm_daemon_runtime_client_application_request(
            client, request, (uint32_t)sizeof(request), &response, &response_length,
            RUNTIME_TEST_TIMEOUT_MS);
        exact = status == LSM_DAEMON_RUNTIME_APPLICATION_OK && response_length == sizeof(request) &&
...
TEST(daemon_runtime_final_disconnect_rejects_blocked_provisional_session) {
    ...
    if (owner) {
        ...
        connect_thread_create_rc = lsm_thread_create(
    ...
    if (provisional_started) {
        owner_closed = lsm_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
        shutdown_won = lsm_daemon_runtime_service_state(fixture.service) ==
                       LSM_DAEMON_RUNTIME_SERVICE_STOPPING;
    }
    ...
    if (connect_thread_started) {
        connect_thread_join_rc = lsm_thread_join(&connect_thread);
    ...
    if (contender.client) {
        (void)lsm_daemon_runtime_client_close(contender.client, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_request_cancel_is_exact_and_session_remains_usable) {
    ...
    lsm_daemon_runtime_cancel_result_t wrong_cancel = LSM_DAEMON_RUNTIME_CANCEL_ERROR;
    int cancels_after_wrong = -1;
    lsm_daemon_runtime_cancel_result_t exact_cancel = LSM_DAEMON_RUNTIME_CANCEL_ERROR;
    bool cancel_delivered = false;
    lsm_daemon_runtime_cancel_result_t late_duplicate_cancel = LSM_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (client) {
        token_reserved =
            lsm_daemon_runtime_client_application_token_reserve(client, &request_token);
    }
    if (token_reserved) {
        call.client = client;
        call.request_token = request_token;
        request_thread_create_rc = lsm_thread_create(
    ...
    if (callback_started) {
        wrong_cancel = lsm_daemon_runtime_client_application_cancel(client, request_token + 1U);
        cancels_after_wrong = atomic_load_explicit(&context.request_cancels, memory_order_acquire);
        exact_cancel = lsm_daemon_runtime_client_application_cancel(client, request_token);
        cancel_delivered =
            exact_cancel == LSM_DAEMON_RUNTIME_CANCEL_ACCEPTED &&
    ...
    if (request_thread_started && !cancel_delivered && client) {
        close_begun = lsm_daemon_runtime_client_close_begin(client);
    }
    if (request_thread_started && (cancel_delivered || close_begun)) {
        request_thread_join_attempted = true;
        request_thread_join_rc = lsm_thread_join(&request_thread);
    ...
    if (client) {
        closed = close_begun
                     ? lsm_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                     : lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
    }
    if (started) {
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    bool cancelled_response = call.status == LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
    ...
    ASSERT_EQ(wrong_cancel, LSM_DAEMON_RUNTIME_CANCEL_STALE);
    ASSERT_EQ(cancels_after_wrong, 0);
    ASSERT_EQ(exact_cancel, LSM_DAEMON_RUNTIME_CANCEL_ACCEPTED);
    ASSERT_TRUE(cancel_delivered);
    ASSERT_TRUE(request_thread_joined);
    ASSERT_TRUE(cancelled_response);
    ASSERT_EQ(late_duplicate_cancel, LSM_DAEMON_RUNTIME_CANCEL_STALE);
...
TEST(daemon_runtime_presend_request_cancel_is_sticky_and_nonterminal) {
    ...
    lsm_daemon_runtime_application_token_t request_token =
        LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    bool token_reserved = false;
    lsm_daemon_runtime_cancel_result_t cancel = LSM_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (token_reserved) {
        cancel = lsm_daemon_runtime_client_application_cancel(client, request_token);
        cancelled_status = lsm_daemon_runtime_client_application_request_tagged(
    ...
    if (cancelled_status == LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED) {
        next_status = lsm_daemon_runtime_client_application_request(
            client, next_request, (uint32_t)sizeof(next_request), &next_response,
            &next_response_length, RUNTIME_TEST_TIMEOUT_MS);
        next_exact = next_status == LSM_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    ASSERT_TRUE(started);
    ASSERT_TRUE(token_reserved);
    ASSERT_EQ(cancel, LSM_DAEMON_RUNTIME_CANCEL_ACCEPTED);
...
TEST(daemon_runtime_allows_only_one_unstarted_application_token) {
    ...
    lsm_daemon_runtime_connect_result_t result = {0};
    lsm_daemon_runtime_client_t *client = NULL;
    lsm_daemon_runtime_application_token_t first_token =
        LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    lsm_daemon_runtime_application_token_t rejected_token = UINT64_MAX;
    lsm_daemon_runtime_application_token_t second_token =
        LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    ...
    if (client) {
        first_reserved = lsm_daemon_runtime_client_application_token_reserve(client, &first_token);
        duplicate_reservation_rejected =
            !lsm_daemon_runtime_client_application_token_reserve(client, &rejected_token) &&
            rejected_token == LSM_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    if (first_reserved && duplicate_reservation_rejected) {
        ...
        lsm_daemon_runtime_application_status_t status =
            lsm_daemon_runtime_client_application_request_tagged(
                ...
                &response_length, RUNTIME_TEST_TIMEOUT_MS);
        first_exact = status == LSM_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (first_exact) {
        second_reserved =
            lsm_daemon_runtime_client_application_token_reserve(client, &second_token);
    }
    if (second_reserved) {
        uint8_t *response = NULL;
        uint32_t response_length = 0;
        lsm_daemon_runtime_application_status_t status =
            lsm_daemon_runtime_client_application_request_tagged(
                ...
                &response_length, RUNTIME_TEST_TIMEOUT_MS);
        second_exact = status == LSM_DAEMON_RUNTIME_APPLICATION_OK &&
...
TEST(daemon_runtime_consumes_busy_application_token_before_response) {
    ...
    if (first_started) {
        ...
        busy_received = busy_sent && runtime_test_raw_application_receive_status_token(
                                         raw, BUSY_TOKEN, LSM_DAEMON_RUNTIME_APPLICATION_BUSY);
    }
    if (busy_received) {
        ...
        cancellation_received =
            cancel_sent && runtime_test_raw_application_receive_status_token(
                               raw, FIRST_TOKEN, LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED);
    }
...
TEST(daemon_runtime_close_begin_retains_storage_and_rejects_late_exchange) {
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    lsm_daemon_runtime_application_status_t status = LSM_DAEMON_RUNTIME_APPLICATION_OK;
    bool close_begun = false;
    ...
    if (client) {
        close_begun = lsm_daemon_runtime_client_close_begin(client);
        duplicate_begin_rejected = close_begun && !lsm_daemon_runtime_client_close_begin(client);
    }
    if (close_begun) {
    ...
    if (client) {
        if (close_begun) {
            (void)lsm_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS);
        } else {
            (void)lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        }
    }
    ...
    ASSERT_EQ(status, LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR);
...
TEST(daemon_runtime_disconnect_cancels_blocked_application_before_exit) {
    ...
    if (client) {
        close_acknowledged =
            close_begun ? lsm_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                        : lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
    }
    if (started) {
    ...
    request_interrupted = (call.status == LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
                           call.status == LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED) &&
                          call.response == NULL && call.response_length == 0;
    free(call.response);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_disconnect_cancels_blocked_non_index_child_and_preserves_other_session) {
    ...
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    char *saved_path = old_path ? lsm_strdup(old_path) : NULL;
    char *saved_marker = old_marker ? lsm_strdup(old_marker) : NULL;
    bool snapshots_ok =
...
#else
    (void)snprintf(work, sizeof(work), "%s/lsm-runtime-non-index-XXXXXX", lsm_tmpdir());
    bool work_ready = snapshots_ok && lsm_mkdtemp(work) != NULL;
#endif
    ...
    bool paths_ready = work_ready && root_written > 0 && root_written < (int)sizeof(root) &&
                       ...
                       marker_written < (int)sizeof(marker) && lsm_mkdir_p(root, 0700) &&
                       lsm_mkdir_p(cache, 0700) && lsm_mkdir_p(bin, 0700) &&
                       runtime_test_copy_self_image(fake_git);

    char path_bin[RUNTIME_TEST_PATH_CAP] = {0};
#ifdef _WIN32
    bool path_bin_ready = paths_ready && lsm_canonical_path(bin, path_bin, sizeof(path_bin));
    if (path_bin_ready) {
    ...
    char *project = paths_ready ? lsm_project_name_from_path(root) : NULL;
    char db_path[RUNTIME_TEST_PATH_CAP] = {0};
    int db_written = project ? snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project) : -1;
    lsm_store_t *seed =
        db_written > 0 && db_written < (int)sizeof(db_path) ? lsm_store_open_path(db_path) : NULL;
    bool seeded = seed && lsm_store_upsert_project(seed, project, root) == LSM_STORE_OK;
    ...
    bool environment_ready = test_path && lsm_setenv("LSM_CACHE_DIR", cache, 1) == 0 &&
                             lsm_setenv("PATH", test_path, 1) == 0 &&
                             lsm_setenv(RUNTIME_TEST_BLOCKING_GIT_MARKER_ENV, marker, 1) == 0;

    lsm_daemon_application_t *application =
        environment_ready ? lsm_daemon_application_new(NULL) : NULL;
    lsm_daemon_runtime_application_callbacks_t callbacks =
    ...
    lsm_daemon_runtime_connect_result_t first_result = {0};
    lsm_daemon_runtime_connect_result_t second_result = {0};
    lsm_daemon_runtime_client_t *first =
        started ? lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                    RUNTIME_TEST_TIMEOUT_MS, &first_result)
                : NULL;
    lsm_daemon_runtime_client_t *second =
        first ? lsm_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &second_result)
              : NULL;
    bool contexts_set =
        first && second &&
        lsm_daemon_application_client_set_context(first, root, root, LSM_MCP_TOOL_PROFILE_ALL, NULL,
                                                  NULL, RUNTIME_TEST_TIMEOUT_MS) ==
            LSM_DAEMON_RUNTIME_APPLICATION_OK &&
        lsm_daemon_application_client_set_context(second, root, root, LSM_MCP_TOOL_PROFILE_ALL,
                                                  NULL, NULL, RUNTIME_TEST_TIMEOUT_MS) ==
            LSM_DAEMON_RUNTIME_APPLICATION_OK;
    bool second_usable_before =
        contexts_set && runtime_real_application_ingest_probe(second) &&
        lsm_daemon_runtime_client_heartbeat(second, RUNTIME_TEST_TIMEOUT_MS);
    ...
    int request_thread_create_rc =
        second_usable_before && arguments_written > 0 &&
            ...
            ? lsm_thread_create(&request_thread, 128U * 1024U,
    ...
    bool first_close_begun = child_identity_exact && lsm_daemon_runtime_client_close_begin(first);
    ...
    bool only_second_admitted =
        first_close_begun &&
        lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
    size_t clients_after_admission_wait =
        started ? lsm_daemon_runtime_service_active_clients(fixture.service) : 0;
    size_t connections_after_admission_wait =
        started ? lsm_daemon_runtime_service_active_connections(fixture.service) : 0;
    ...
    int request_thread_join_rc = request_completed ? lsm_thread_join(&request_thread) : -1;
    if (request_thread_join_rc == 0) {
        request_thread_started = false;
    }
    bool first_transport_interrupted =
        request_completed && call.status == LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR &&
        call.response == NULL && call.response_length == 0;
    bool first_close_finished = false;
    if (first && !request_thread_started) {
        first_close_finished =
            first_close_begun
                ? lsm_daemon_runtime_client_close_finish(first, RUNTIME_TEST_TIMEOUT_MS)
                : lsm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
    }
    ...
    if (second) {
        second_closed = lsm_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
    }
    bool exited =
        started && lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    if (first) {
        if (!first_close_begun) {
            first_close_begun = lsm_daemon_runtime_client_close_begin(first);
        }
        if (request_thread_started) {
        ...
        if (first_close_begun) {
            (void)lsm_daemon_runtime_client_close_finish(first, RUNTIME_TEST_TIMEOUT_MS);
        } else {
            (void)lsm_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        }
        first = NULL;
    ...
    bool application_stopped =
        application && lsm_daemon_application_shutdown(application, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_noncooperative_callback_does_not_detach_or_unbound_stop) {
     ...
     * scheduler starvation (the LSM_LOCAL_CI_CPUS=4 fidelity pass, CI runners)
    ...
    if (callback_started) {
        close_begun = lsm_daemon_runtime_client_close_begin(client);
        uint64_t stop_started_ms = lsm_now_ms();
        stop_returned = lsm_daemon_runtime_service_stop(fixture.service, STOP_BOUND_MS);
        stop_elapsed_ms = lsm_now_ms() - stop_started_ms;
        exited_before_release =
            lsm_daemon_runtime_service_state(fixture.service) == LSM_DAEMON_RUNTIME_SERVICE_EXITED;
    }
    ...
    if (started) {
        exited_after_release =
            lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    free(call.response);
    runtime_test_fixture_finish(&fixture);

    ASSERT_TRUE(started);
    ASSERT_EQ(result.status, LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_EQ(request_thread_create_rc, 0);
    ASSERT_TRUE(callback_started);
    ASSERT_TRUE(close_begun);
...
TEST(daemon_runtime_application_busy_cap_and_malformed_are_isolated) {
    ...
    lsm_daemon_runtime_application_status_t oversize_status = LSM_DAEMON_RUNTIME_APPLICATION_OK;
    uint8_t *valid_response = NULL;
    uint32_t valid_response_length = 0;
    lsm_daemon_runtime_application_status_t valid_status =
        LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    bool owner_survived = false;
    bool exited = false;
    ...
    if (callback_started) {
        ...
        busy_rejected = busy_sent && runtime_test_raw_application_receive_status(
                                         raw, LSM_DAEMON_RUNTIME_APPLICATION_BUSY);
    }
    if (busy_rejected) {
    ...
    if (owner && bad_peer_released) {
        oversize_status = lsm_daemon_runtime_client_application_request(
            owner, &sentinel, LSM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX + 1U, &oversize_response,
            &oversize_response_length, RUNTIME_TEST_TIMEOUT_MS);
        valid_status = lsm_daemon_runtime_client_application_request(
            owner, valid_request, (uint32_t)sizeof(valid_request), &valid_response,
            &valid_response_length, RUNTIME_TEST_TIMEOUT_MS);
        owner_survived = oversize_status == LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR &&
                         oversize_response == NULL && oversize_response_length == 0 &&
                         valid_status == LSM_DAEMON_RUNTIME_APPLICATION_OK &&
...
TEST(daemon_runtime_malformed_and_zero_cancel_close_only_offending_connections) {
    ...
    if (raw) {
        malformed_sent = lsm_daemon_ipc_send_frame(
            raw, LSM_DAEMON_FRAME_REQUEST, LSM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
            malformed_cancel, (uint32_t)sizeof(malformed_cancel));
    }
    if (malformed_sent) {
        malformed_released = lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                         RUNTIME_TEST_TIMEOUT_MS);
    }
    ...
    if (raw) {
        zero_sent = lsm_daemon_ipc_send_frame(
            raw, LSM_DAEMON_FRAME_REQUEST, LSM_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
            zero_token_cancel, (uint32_t)sizeof(zero_token_cancel));
    }
    if (zero_sent) {
        zero_released = lsm_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                    RUNTIME_TEST_TIMEOUT_MS);
    }
    if (zero_released) {
        lsm_daemon_frame_t frame = {0};
    ...
    if (owner && zero_closed) {
        valid_status = lsm_daemon_runtime_client_application_request(
            owner, valid_request, (uint32_t)sizeof(valid_request), &valid_response,
            &valid_response_length, RUNTIME_TEST_TIMEOUT_MS);
        owner_survived = valid_status == LSM_DAEMON_RUNTIME_APPLICATION_OK &&
                         valid_response_length == sizeof(valid_request) && valid_response &&
                         memcmp(valid_response, valid_request, sizeof(valid_request)) == 0 &&
                         lsm_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_kernel_process_fingerprint_is_stable_and_fail_closed) {
    char first[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char repeated[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char invalid[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool first_ok = lsm_daemon_runtime_process_build_fingerprint(runtime_test_process_id(), first);
    bool repeated_ok =
        lsm_daemon_runtime_process_build_fingerprint(runtime_test_process_id(), repeated);
    bool invalid_rejected = !lsm_daemon_runtime_process_build_fingerprint(UINT64_MAX, invalid);
...
TEST(daemon_runtime_process_fingerprint_supports_extended_length_image) {
    ...
    int root_written =
        snprintf(root, sizeof(root), "%s/lsm-runtime-long-image-XXXXXX", lsm_tmpdir());
    bool root_created =
        root_written > 0 && root_written < (int)sizeof(root) && lsm_mkdtemp(root) != NULL;
    ...
    bool directory_created = path_built && lsm_mkdir_p(directory, 0700);
    ...
    int event_written =
        snprintf(event_name, sizeof(event_name), "Local\\lsm-runtime-long-image-%lu-%llu",
    ...
    char observed[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool fingerprinted = queried_long_image && lsm_daemon_runtime_process_build_fingerprint(
    ...
    bool image_removed = !copied || lsm_unlink(image_path) == 0;
    ...
    if (root_created) {
        ...
        for (;;) {
            tree_removed = lsm_rmdir(cleanup) == 0 && tree_removed;
...
TEST(daemon_runtime_mac_fast_path_rejects_foreign_main_image_mapping_active) {
    ...
    char foreign_image[RUNTIME_TEST_PATH_CAP] = {0};
    char foreign_fingerprint[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/lsm-runtime-mapped-image-XXXXXX", lsm_tmpdir());
    bool directory_created = directory_written > 0 && directory_written < (int)sizeof(directory) &&
                             lsm_mkdtemp(directory) != NULL;
    ...
    bool foreign_fingerprinted =
        foreign_signed && lsm_daemon_build_fingerprint_file(foreign_image, foreign_fingerprint);
...
TEST(daemon_runtime_copied_image_fallback_accepts_identical_and_rejects_changed) {
    ...
    char identical_fingerprint[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool identical_bytes =
        identical_copied &&
        lsm_daemon_build_fingerprint_file(identical_path, identical_fingerprint) &&
    ...
    bool identical_exited =
        ...
        lsm_daemon_runtime_service_wait_exited(identical_fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    (void)lsm_unlink(identical_path);
    ...
    char changed_fingerprint[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool changed_bytes = changed_copied &&
                         lsm_daemon_build_fingerprint_file(changed_path, changed_fingerprint) &&
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    char event_name[128] = {0};
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/lsm-runtime-image-XXXXXX", lsm_tmpdir());
    ...
    int replacement_written = setup ? snprintf(replacement_path, sizeof(replacement_path),
                                    ...
                                    : -1;
    int event_written =
        snprintf(event_name, sizeof(event_name), "Local\\lsm-runtime-image-%lu-%llu",
                 (unsigned long)GetCurrentProcessId(), (unsigned long long)GetTickCount64());
    ...
    FILE *replacement_file = setup ? lsm_fopen(replacement_path, "wb") : NULL;
    bool replacement_written_ok =
        replacement_file && fputs("lsm-windows-replacement-image", replacement_file) >= 0;
    if (replacement_file) {
        replacement_written_ok = fclose(replacement_file) == 0 && replacement_written_ok;
    }
    setup = setup && replacement_written_ok;

    char original[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char replacement[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char observed[LSM_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    setup = setup && lsm_daemon_build_fingerprint_file(image_path, original) &&
            lsm_daemon_build_fingerprint_file(replacement_path, replacement) &&
    ...
    bool fingerprinted = ready && lsm_daemon_runtime_process_build_fingerprint(
                                      (uint64_t)process.dwProcessId, observed);
    ...
    (void)lsm_unlink(replacement_path);
    (void)lsm_unlink(image_path);
    (void)lsm_rmdir(directory);
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    setup = setup && child > 0 && rename(replacement_path, image_path) == 0 &&
            lsm_daemon_build_fingerprint_file(image_path, replacement) &&
            strcmp(original, replacement) != 0;
    bool fingerprinted =
        setup && lsm_daemon_runtime_process_build_fingerprint((uint64_t)child, observed);
...
TEST(daemon_runtime_close_begin_releases_admission_with_inflight_request) {
    ...
    admission_released_at_begin = close_begun && lsm_daemon_runtime_service_wait_for_clients(
                                                     fixture.service, 0, RUNTIME_TEST_TIMEOUT_MS);
    if (request_thread_started) {
        request_thread_joined = lsm_thread_join(&request_thread) == 0;
        request_thread_started = false;
    }
    request_ended_without_payload =
        request_thread_joined &&
        (call.status == LSM_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
         call.status == LSM_DAEMON_RUNTIME_APPLICATION_CANCELLED) &&
        call.response == NULL && call.response_length == 0;
    if (client) {
        (void)(close_begun ? lsm_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                           : lsm_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS));
        client = NULL;
    }
    if (started) {
...
TEST(daemon_runtime_permanent_service_survives_last_disconnect_until_stop) {
    ...
    if (client) {
        ...
        survived =
            !lsm_daemon_runtime_service_wait_exited(fixture.service, 300U) &&
            lsm_daemon_runtime_service_state(fixture.service) == LSM_DAEMON_RUNTIME_SERVICE_RUNNING;
        lsm_daemon_runtime_status_t status = {0};
        status_ok = lsm_daemon_runtime_request_status(fixture.endpoint, &identity,
                                                      RUNTIME_TEST_TIMEOUT_MS, &status) &&
                    status.permanent && !status.stopping && status.committed_clients == 0;
        lsm_daemon_runtime_stop_result_t stop_result = {0};
        stop_ok = lsm_daemon_runtime_request_stop(fixture.endpoint, &identity,
...
TEST(daemon_runtime_stop_refuses_while_committed_clients_exist) {
    ...
    if (client) {
        lsm_daemon_runtime_stop_result_t busy_result = {0};
        refused = lsm_daemon_runtime_request_stop(fixture.endpoint, &identity,
        ...
        accepted_after_close =
            lsm_daemon_runtime_service_wait_for_clients(fixture.service, 0,
                                                        RUNTIME_TEST_TIMEOUT_MS) &&
            lsm_daemon_runtime_request_stop(fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS,
                                            &stop_result) &&
            stop_result.accepted && !stop_result.busy;
        exited = lsm_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
