/*
 * test_git_context.c — Tests for lsm_git_context_resolve(), focusing on
...
TEST(canonical_root_repo_root) {
...
#else
    char *tmp = th_mktempdir("lsm_gitctx");
    ...
    int rc = lsm_git_context_resolve(tmp, &ctx);
    if (rc != 0 || !ctx.is_git) {
        lsm_git_context_free(&ctx);
        th_rmtree(tmp);
        FAIL("lsm_git_context_resolve failed or not a git repo");
    }
...
TEST(canonical_root_subdir) {
    ...
    lsm_git_context_t ctx = {0};
    int rc = lsm_git_context_resolve(subdir, &ctx);
    if (rc != 0 || !ctx.is_git) {
        lsm_git_context_free(&ctx);
        th_rmtree(tmp);
        FAIL("lsm_git_context_resolve on subdir failed or not a git repo");
    }
...
TEST(canonical_root_linked_worktree) {
    ...
    lsm_git_context_t ctx = {0};
    int rc = lsm_git_context_resolve(wt_tmp, &ctx);
    if (rc != 0 || !ctx.is_git) {
        lsm_git_context_free(&ctx);
        git_run(main_tmp, "worktree prune");
        th_rmtree(main_tmp);
        th_rmtree(wt_tmp);
        FAIL("lsm_git_context_resolve on linked worktree failed");
    }
