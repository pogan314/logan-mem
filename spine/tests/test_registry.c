...
TEST(fqn_simple) {
    char *qn = lsm_pipeline_fqn_compute("myproj", "cmd/server/main.go", "HandleRequest");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_no_name) {
    char *qn = lsm_pipeline_fqn_compute("myproj", "pkg/service.go", NULL);
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_python_init) {
    ...
    char *qn = lsm_pipeline_fqn_compute("myproj", "pkg/__init__.py", "Foo");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_js_index) {
    ...
    char *qn = lsm_pipeline_fqn_compute("myproj", "src/index.ts", "App");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_module) {
    char *qn = lsm_pipeline_fqn_module("myproj", "cmd/server/main.go");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_folder) {
    char *qn = lsm_pipeline_fqn_folder("myproj", "cmd/server");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_root_file) {
    char *qn = lsm_pipeline_fqn_compute("proj", "main.go", "main");
    ASSERT_NOT_NULL(qn);
...
TEST(fqn_init_module_distinct_from_folder) {
    ...
    char *mod_qn = lsm_pipeline_fqn_module("proj", "pkg/__init__.py");
    char *folder_qn = lsm_pipeline_fqn_folder("proj", "pkg");
...
TEST(fqn_init_nested_module_distinct) {
    ...
    char *mod_qn =
        lsm_pipeline_fqn_module("proj", "docker-images/cloud-runs/bq-sync-api/__init__.py");
    char *folder_qn = lsm_pipeline_fqn_folder("proj", "docker-images/cloud-runs/bq-sync-api");
    ASSERT_NOT_NULL(mod_qn);
    ASSERT_NOT_NULL(folder_qn);
    ASSERT_STR_NEQ(mod_qn, folder_qn);
    free(mod_qn);
...
TEST(fqn_index_ts_module_distinct_from_folder) {
    ...
    char *mod_qn = lsm_pipeline_fqn_module("proj", "src/components/index.ts");
    char *folder_qn = lsm_pipeline_fqn_folder("proj", "src/components");
    ASSERT_NOT_NULL(mod_qn);
    ASSERT_NOT_NULL(folder_qn);
    ASSERT_STR_NEQ(mod_qn, folder_qn);
    free(mod_qn);
...
TEST(fqn_init_symbols_get_clean_package_qn) {
    ...
    char *sym_qn = lsm_pipeline_fqn_compute("proj", "pkg/__init__.py", "Foo");
...
TEST(fqn_index_symbols_get_clean_qn) {
    ...
    char *sym_qn = lsm_pipeline_fqn_compute("proj", "src/index.ts", "App");
    ASSERT_NOT_NULL(sym_qn);
...
TEST(fqn_init_file_node_distinct) {
    ...
    char *file_qn = lsm_pipeline_fqn_compute("proj", "pkg/__init__.py", "__file__");
    char *folder_qn = lsm_pipeline_fqn_folder("proj", "pkg");
...
TEST(fqn_regular_module_unchanged) {
    ...
    char *qn = lsm_pipeline_fqn_module("proj", "pkg/utils.py");
    ASSERT_NOT_NULL(qn);
...
TEST(project_name_from_path) {
    char *name = lsm_project_name_from_path("/Users/dev/project");
...
TEST(project_name_from_root) {
    char *name = lsm_project_name_from_path("/");
    ASSERT_NOT_NULL(name);
...
TEST(registry_create_free) {
    lsm_registry_t *r = lsm_registry_new();
    ASSERT_NOT_NULL(r);
    ASSERT_EQ(lsm_registry_size(r), 0);
    lsm_registry_free(r);
    PASS();
}

TEST(registry_free_null) {
    lsm_registry_free(NULL); /* should not crash */
    PASS();
}

TEST(registry_add_and_exists) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "main", "proj.cmd.main", "Function");
    ASSERT_EQ(lsm_registry_size(r), 1);
    ASSERT_TRUE(lsm_registry_exists(r, "proj.cmd.main"));
    ASSERT_FALSE(lsm_registry_exists(r, "proj.cmd.other"));
    lsm_registry_free(r);
    PASS();
}
...
TEST(registry_label_of) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Foo", "proj.pkg.Foo", "Class");
    lsm_registry_add(r, "bar", "proj.pkg.bar", "Function");

    ASSERT_STR_EQ(lsm_registry_label_of(r, "proj.pkg.Foo"), "Class");
    ASSERT_STR_EQ(lsm_registry_label_of(r, "proj.pkg.bar"), "Function");
    ASSERT_NULL(lsm_registry_label_of(r, "proj.pkg.baz"));
...
TEST(registry_find_by_name) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "main", "proj.cmd.main", "Function");
    lsm_registry_add(r, "main", "proj.srv.main", "Function");
    lsm_registry_add(r, "helper", "proj.util.helper", "Function");

    const char **out = NULL;
    int count = 0;
    lsm_registry_find_by_name(r, "main", &out, &count);
    ASSERT_EQ(count, 2);

    lsm_registry_find_by_name(r, "helper", &out, &count);
    ASSERT_EQ(count, 1);

    lsm_registry_find_by_name(r, "nonexistent", &out, &count);
    ASSERT_EQ(count, 0);
...
TEST(registry_no_duplicates) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "main", "proj.cmd.main", "Function");
    lsm_registry_add(r, "main", "proj.cmd.main", "Function"); /* duplicate */
    ASSERT_EQ(lsm_registry_size(r), 1);
...
TEST(resolve_same_module) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "helper", "proj.pkg.service.helper", "Function");

    /* Call "helper" from the same module → should resolve */
    lsm_resolution_t res = lsm_registry_resolve(r, "helper", "proj.pkg.service", NULL, NULL, 0);
...
TEST(resolve_qualified_disambiguates_same_name) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "save", "proj.lib.App.Alpha.save", "Function");
    lsm_registry_add(r, "save", "proj.lib.App.Beta.save", "Function");
    lsm_registry_add(r, "save", "proj.lib.App.Gamma.save", "Function");

    /* Each fully-qualified call routes to its own package. */
    lsm_resolution_t a =
        lsm_registry_resolve(r, "App::Alpha::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(a.qualified_name, "proj.lib.App.Alpha.save");
    ASSERT_STR_EQ(a.strategy, "qualified_suffix");

    lsm_resolution_t b =
        lsm_registry_resolve(r, "App::Beta::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(b.qualified_name, "proj.lib.App.Beta.save");

    lsm_resolution_t g =
        lsm_registry_resolve(r, "App::Gamma::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ASSERT_STR_EQ(g.qualified_name, "proj.lib.App.Gamma.save");

    /* The dotted callee form (Go/Python/C#) disambiguates identically. */
    lsm_resolution_t dotted =
        lsm_registry_resolve(r, "App.Beta.save", "proj.lib.App.Caller", NULL, NULL, 0);
    ...
    lsm_resolution_t nomatch =
        lsm_registry_resolve(r, "Other::Pkg::save", "proj.lib.App.Caller", NULL, NULL, 0);
    ...
    lsm_resolution_t bare =
        lsm_registry_resolve(r, "save", "proj.lib.App.Caller", NULL, NULL, 0);
...
TEST(resolve_qualified_ambiguous_tail_falls_through) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "run", "proj.svcA.Foo.Bar.run", "Function");
    lsm_registry_add(r, "run", "proj.svcB.Foo.Bar.run", "Function");

    /* "Foo::Bar::run" tail matches BOTH candidates → not unique → fall through. */
    lsm_resolution_t res =
        lsm_registry_resolve(r, "Foo::Bar::run", "proj.svcA.Caller", NULL, NULL, 0);
...
TEST(resolve_import_map) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Process", "proj.pkg.worker.Process", "Function");
    ...
    lsm_resolution_t res =
        lsm_registry_resolve(r, "worker.Process", "proj.cmd.main", keys, vals, 1);
...
TEST(resolve_import_map_bare_function) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "requireAdmin", "proj.lib.authorization.requireAdmin", "Function");
    /* Same name in another module — without the fix this is what gets picked. */
    lsm_registry_add(r, "requireAdmin", "proj.lib.users.requireAdmin", "Function");

    const char *keys[] = {"requireAdmin"};
    const char *vals[] = {"proj.lib.authorization"};

    lsm_resolution_t res =
        lsm_registry_resolve(r, "requireAdmin", "proj.actions.settings", keys, vals, 1);
...
TEST(resolve_import_map_bare_alias) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "scan_bash", "proj.security_scan.scan_bash", "Function");
    ...
    lsm_resolution_t res =
        lsm_registry_resolve(r, "_scan_bash", "proj.hooks.pre_tool", keys, vals, 1);
...
TEST(resolve_import_map_aliased_from_import) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "execute", "proj.services.satori_bridge.gate.execute", "Function");
    /* Alias ghost must not win if somehow registered. */
    lsm_registry_add(r, "bridge_execute", "proj.services.satori_bridge.gate.bridge_execute",
                     "Function");
    ...
    lsm_resolution_t res =
        lsm_registry_resolve(r, "bridge_execute", "proj.services.yui_core.router", keys, vals, 1);
...
TEST(resolve_unique_name) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "UniqueFunc", "proj.deep.path.UniqueFunc", "Function");

    /* Call "UniqueFunc" — only one candidate project-wide */
    lsm_resolution_t res =
        lsm_registry_resolve(r, "UniqueFunc", "proj.other.module", NULL, NULL, 0);
...
TEST(resolve_unresolved) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "foo", "proj.pkg.foo", "Function");

    /* Call "nonexistent" — not in registry */
    lsm_resolution_t res = lsm_registry_resolve(r, "nonexistent", "proj.other", NULL, NULL, 0);
...
TEST(resolve_many_nodes) {
    ...
    for (int i = 0; i < 500; i++) {
        ...
        lsm_registry_add(r, name, qn, "Function");
    }
    ASSERT_EQ(lsm_registry_size(r), 500);

    /* Resolve one */
    lsm_resolution_t res = lsm_registry_resolve(r, "func_250", "proj.pkg", NULL, NULL, 0);
...
TEST(resolve_suffix_match) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Process", "proj.svcA.Process", "Function");
    lsm_registry_add(r, "Process", "proj.svcB.Process", "Function");

    /* Caller in svcA — should prefer svcA via import distance */
    lsm_resolution_t res = lsm_registry_resolve(r, "Process", "proj.svcA.caller", NULL, NULL, 0);
...
TEST(resolve_caps_unresolvably_ambiguous_names) {
    ...
    for (int i = 0; i < 300; i++) {
        ...
        lsm_registry_add(r, "flags", qn, "Variable");
    }
    lsm_resolution_t res = lsm_registry_resolve(r, "flags", "proj.other.caller", NULL, NULL, 0);
    ASSERT_TRUE(res.qualified_name == NULL || res.qualified_name[0] == '\0');

    /* Same-module resolution still wins regardless of candidate count. */
    res = lsm_registry_resolve(r, "flags", "proj.mod7", NULL, NULL, 0);
...
TEST(resolve_import_map_suffix) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Foo", "proj.other.sub.Foo", "Function");
    ...
    lsm_resolution_t res = lsm_registry_resolve(r, "other.Foo", "proj.pkg", keys, vals, 1);
...
TEST(resolve_is_import_reachable) {
    ...
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Helper", "proj.shared.utils.Helper", "Function");

    /* With import covering the module → full confidence */
    const char *keys1[] = {"utils"};
    const char *vals1[] = {"proj.shared.utils"};
    lsm_resolution_t res = lsm_registry_resolve(r, "Helper", "proj.caller", keys1, vals1, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
    ...
    res = lsm_registry_resolve(r, "Helper", "proj.caller", keys2, vals2, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
...
TEST(resolve_import_reachable_prefix) {
    ...
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Process", "proj.handler.sub.Process", "Function");

    const char *keys[] = {"handler"};
    const char *vals[] = {"proj.handler"};
    lsm_resolution_t res = lsm_registry_resolve(r, "Process", "proj.caller", keys, vals, 1);
    ASSERT_STR_EQ(res.strategy, "unique_name");
...
TEST(negative_import_rejects_unimported) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Process", "proj.billing.Process", "Function");
    lsm_registry_add(r, "Process", "proj.handler.Process", "Function");
...
TEST(fuzzy_resolve_single_candidate) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "CreateOrder", "svcA.handlers.CreateOrder", "Function");
    lsm_registry_add(r, "ValidateOrder", "svcB.validators.ValidateOrder", "Function");

    /* FuzzyResolve should find by simple name even with unknown prefix */
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknownPkg.CreateOrder", "svcC.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_nonexistent) {
    ...
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "NonExistent", "svcC.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(fuzzy_resolve_multiple_best_by_distance) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Process", "svcA.handlers.Process", "Function");
    lsm_registry_add(r, "Process", "svcB.handlers.Process", "Function");

    /* Caller in svcA — should prefer svcA */
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknown.Process", "svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_STR_EQ(fr.result.qualified_name, "svcA.handlers.Process");

    /* Caller in svcB — should prefer svcB */
    fr = lsm_registry_fuzzy_resolve(r, "unknown.Process", "svcB.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_deep_name_extraction) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "DoWork", "myproject.utils.DoWork", "Function");

    /* Deeply qualified callee — should extract "DoWork" */
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "some.deep.module.DoWork", "myproject.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_empty_registry) {
    ...
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "SomeFunc", "myproject.caller", NULL, NULL, 0);
    ASSERT_FALSE(fr.ok);
...
TEST(fuzzy_resolve_confidence_single) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Handler", "proj.svc.Handler", "Function");

    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknownPkg.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_resolve_confidence_distance) {
    ...
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknownPkg.Process", "proj.svcA.other", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
...
TEST(fuzzy_penalty_unreachable_import) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "Handler", "proj.billing.Handler", "Function");

    /* Import for different module → confidence halved */
    const char *keys[] = {"other"};
    const char *vals[] = {"proj.other"};
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknown.Handler", "proj.caller", keys, vals, 1);
    ASSERT_TRUE(fr.ok);
    /* 0.40 * 0.5 = 0.20 */
...
TEST(fuzzy_no_import_map_passthrough) {
    ...
    lsm_fuzzy_result_t fr =
        lsm_registry_fuzzy_resolve(r, "unknown.Handler", "proj.caller", NULL, NULL, 0);
    ASSERT_TRUE(fr.ok);
    ASSERT_TRUE(fr.result.confidence >= 0.35 && fr.result.confidence <= 0.45);
...
TEST(perl_builtin_set_recognizes_core_builtins) {
    ...
    ASSERT_TRUE(lsm_perl_is_builtin("push"));
    ASSERT_TRUE(lsm_perl_is_builtin("shift"));
    ASSERT_TRUE(lsm_perl_is_builtin("keys"));
    ASSERT_TRUE(lsm_perl_is_builtin("sprintf"));
    ASSERT_TRUE(lsm_perl_is_builtin("abs"));   /* first element */
    ASSERT_TRUE(lsm_perl_is_builtin("write")); /* last element */
    ASSERT_TRUE(lsm_perl_is_builtin("wantarray"));
    PASS();
}

TEST(perl_builtin_set_rejects_project_subs) {
    /* Genuine project sub names and edge inputs must NOT be flagged. */
    ASSERT_FALSE(lsm_perl_is_builtin("helper"));
    ASSERT_FALSE(lsm_perl_is_builtin("process_request"));
    ASSERT_FALSE(lsm_perl_is_builtin("Push")); /* case-sensitive */
    ASSERT_FALSE(lsm_perl_is_builtin(""));
    ASSERT_FALSE(lsm_perl_is_builtin(NULL));
    PASS();
}
...
TEST(perl_suppress_drops_weak_builtin_and_method_matches) {
    ...
    ASSERT_TRUE(lsm_perl_suppress_generic_match(true, false, "push", "suffix_match"));
    ASSERT_TRUE(lsm_perl_suppress_generic_match(true, false, "keys", "unique_name"));
    ASSERT_TRUE(lsm_perl_suppress_generic_match(true, true, "commit", "suffix_match"));
    ASSERT_TRUE(lsm_perl_suppress_generic_match(true, true, "log", "unique_name"));
    PASS();
}
...
TEST(perl_suppress_keeps_high_confidence_and_genuine_calls) {
    ...
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, false, "log", "same_module"));
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, false, "open", "import_map"));
    ...
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, true, "Foo::Bar::m", "import_map_suffix"));
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, true, "commit", "same_module"));
    /* A genuine non-builtin function call is never suppressed (edge survives). */
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, false, "helper", "suffix_match"));
    /* Non-Perl languages are never affected. */
    ASSERT_FALSE(lsm_perl_suppress_generic_match(false, false, "push", "suffix_match"));
    ASSERT_FALSE(lsm_perl_suppress_generic_match(false, true, "commit", "suffix_match"));
    /* No match (NULL/empty strategy) → nothing to suppress. */
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, false, "push", NULL));
    ASSERT_FALSE(lsm_perl_suppress_generic_match(true, true, "commit", ""));
    PASS();
}
...
TEST(cross_language_suffix_match_drops_py_vs_js) {
    ...
    ASSERT_TRUE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, "web/src/pages/Editor.js",
                                                         "suffix_match"));
    ASSERT_TRUE(lsm_suppress_cross_language_suffix_match(LSM_LANG_JAVASCRIPT, "store.py",
                                                         "suffix_match"));
    ASSERT_TRUE(lsm_suppress_cross_language_suffix_match(LSM_LANG_BASH, "cli/main.py",
                                                         "suffix_match"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, "store.py",
                                                          "suffix_match"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, "web/src/pages/Editor.js",
                                                          "unique_name"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, "web/src/pages/Editor.js",
                                                          "same_module"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, "web/src/pages/Editor.js",
    ...
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_JAVASCRIPT, "lib/util.ts",
                                                          "suffix_match"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_TYPESCRIPT, "ui/Panel.tsx",
                                                          "suffix_match"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_PYTHON, NULL, "suffix_match"));
    ASSERT_FALSE(lsm_suppress_cross_language_suffix_match(LSM_LANG_COUNT, "store.py",
                                                          "suffix_match"));
    PASS();
}
...
TEST(tsjs_suppress_drops_weak_method_matches) {
     ...
     * covered defensively (lsm_registry_fuzzy_resolve is not wired into the
     * resolvers today) so a future wiring cannot silently reintroduce it. */
    ASSERT_TRUE(lsm_tsjs_suppress_weak_method_match(true, true, "suffix_match"));
    ASSERT_TRUE(lsm_tsjs_suppress_weak_method_match(true, true, "unique_name"));
    ASSERT_TRUE(lsm_tsjs_suppress_weak_method_match(true, true, "field_type_hint"));
    ASSERT_TRUE(lsm_tsjs_suppress_weak_method_match(true, true, "fuzzy"));
    PASS();
}
...
TEST(tsjs_suppress_keeps_high_confidence_and_non_methods) {
    ...
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "same_module"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "import_map"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "import_map_suffix"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "qualified_suffix"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "callee_suffix"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "service_pattern"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "lsp_ts_method"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "lsp_cross"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, "lsp_ts_local"));
    /* A bare call (is_method=false) is a free-function call → never suppressed. */
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, false, "unique_name"));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, false, "suffix_match"));
    /* Non-TS/JS languages are never affected. */
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(false, true, "suffix_match"));
    /* No match (NULL/empty strategy) → nothing to suppress. */
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, NULL));
    ASSERT_FALSE(lsm_tsjs_suppress_weak_method_match(true, true, ""));
    PASS();
}
...
TEST(resolve_import_map_alias_with_suffix_hits_method) {
    lsm_registry_t *r = lsm_registry_new();
    lsm_registry_add(r, "user_logged_in", "proj.auth.signals.user_logged_in", "Variable");
    lsm_registry_add(r, "send", "proj.auth.signals.user_logged_in.send", "Method");
    const char *keys[] = {"user_logged_in"};
    const char *vals[] = {"proj.auth.signals.user_logged_in"};
    lsm_resolution_t res =
        lsm_registry_resolve(r, "user_logged_in.send", "proj.auth.views", keys, vals, 1);
