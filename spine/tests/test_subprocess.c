 ...
 *
 * Two layers:
 *   1. lsm_proc_classify() — pure, exercised on EVERY platform (so the Windows
 *      NTSTATUS crash-code mapping is guarded on Linux/macOS CI too, not just an
 *      untested Windows branch).
 *   2. lsm_subprocess_run() — real spawn/reap, exercised on POSIX via /bin/sh
...
TEST(subprocess_classify_clean) {
    ASSERT_EQ(lsm_proc_classify(true, 0, 0, false), LSM_PROC_CLEAN);
    PASS();
}

TEST(subprocess_classify_exit_nonzero) {
    ASSERT_EQ(lsm_proc_classify(true, 3, 0, false), LSM_PROC_EXIT_NONZERO);
    ASSERT_EQ(lsm_proc_classify(true, 127, 0, false), LSM_PROC_EXIT_NONZERO);
    PASS();
}
...
TEST(subprocess_classify_windows_crash_codes) {
    ASSERT_EQ(lsm_proc_classify(true, (int)0xC0000005u, 0, false), LSM_PROC_CRASH);
    ASSERT_EQ(lsm_proc_classify(true, (int)0xC00000FDu, 0, false), LSM_PROC_CRASH);
    ASSERT_EQ(lsm_proc_classify(true, (int)0xC000001Du, 0, false), LSM_PROC_CRASH);
    ASSERT_EQ(lsm_proc_classify(true, (int)0xC000013Au, 0, false), LSM_PROC_KILLED);
    PASS();
}

TEST(subprocess_classify_posix_fault_signal_is_crash) {
#ifndef _WIN32
    ASSERT_EQ(lsm_proc_classify(false, -1, SIGSEGV, false), LSM_PROC_CRASH);
    ASSERT_EQ(lsm_proc_classify(false, -1, SIGABRT, false), LSM_PROC_CRASH);
    ASSERT_EQ(lsm_proc_classify(false, -1, SIGBUS, false), LSM_PROC_CRASH);
#endif
    PASS();
}
...
#ifndef _WIN32
    ASSERT_EQ(lsm_proc_classify(false, -1, SIGTERM, false), LSM_PROC_KILLED);
    ASSERT_EQ(lsm_proc_classify(false, -1, SIGKILL, false), LSM_PROC_KILLED);
#endif
    PASS();
}
...
TEST(subprocess_classify_timeout_dominates) {
    ASSERT_EQ(lsm_proc_classify(false, -1, 9 /*SIGKILL*/, true), LSM_PROC_HANG);
    ASSERT_EQ(lsm_proc_classify(true, 0, 0, true), LSM_PROC_HANG);
    PASS();
}

TEST(subprocess_outcome_str) {
    ASSERT_STR_EQ(lsm_proc_outcome_str(LSM_PROC_CLEAN), "clean");
    ASSERT_STR_EQ(lsm_proc_outcome_str(LSM_PROC_CRASH), "crash");
    ASSERT_STR_EQ(lsm_proc_outcome_str(LSM_PROC_HANG), "hang");
    ASSERT_STR_EQ(lsm_proc_outcome_str(LSM_PROC_EXIT_NONZERO), "exit_nonzero");
    ASSERT_STR_EQ(lsm_proc_outcome_str(LSM_PROC_KILLED), "killed");
    PASS();
}
...
#ifndef _WIN32
static lsm_proc_result_t run_sh(const char *script, int quiet_timeout_ms) {
    const char *argv[] = {"/bin/sh", "-c", script, NULL};
    lsm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.log_file = NULL;
    opts.quiet_timeout_ms = quiet_timeout_ms;
    lsm_proc_result_t r;
    lsm_subprocess_run(&opts, &r);
    return r;
}
#endif
...
#else
    lsm_proc_result_t r = run_sh("exit 0", 0);
    ASSERT_EQ(r.outcome, LSM_PROC_CLEAN);
...
TEST(subprocess_run_exit_nonzero) {
    ...
    SKIP_PLATFORM("POSIX /bin/sh spawn");
#else
    lsm_proc_result_t r = run_sh("exit 7", 0);
    ASSERT_EQ(r.outcome, LSM_PROC_EXIT_NONZERO);
...
TEST(subprocess_run_resolves_literal_binary_name_from_path) {
    ...
    lsm_proc_opts_t opts = {
        .bin = "sh",
        .argv = argv,
    };
    lsm_proc_result_t result;
    int rc = lsm_subprocess_run(&opts, &result);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(result.outcome, LSM_PROC_CLEAN);
...
#else
    lsm_proc_result_t r = run_sh("kill -SEGV $$", 0);
    ASSERT_EQ(r.outcome, LSM_PROC_CRASH);
...
TEST(subprocess_run_hang_is_hang) {
    ...
    SKIP_PLATFORM("POSIX /bin/sh spawn");
#else
    lsm_proc_result_t r = run_sh("sleep 30", 300 /* ms quiet-timeout */);
    ASSERT_EQ(r.outcome, LSM_PROC_HANG);
    PASS();
#endif
}
...
TEST(subprocess_retries_transient_spawn_refusal) {
    ...
    lsm_subprocess_force_spawn_eagain_for_testing(3);
    lsm_proc_result_t r = run_sh("exit 0", 0);
    ASSERT_EQ(lsm_subprocess_pending_spawn_eagain_for_testing(), 0);
    ASSERT_EQ(r.outcome, LSM_PROC_CLEAN);
    ASSERT_EQ(r.exit_code, 0);
    PASS();
#endif
...
TEST(subprocess_gives_up_after_the_retry_budget) {
    ...
    lsm_subprocess_force_spawn_eagain_for_testing(50);
    lsm_proc_result_t r = run_sh("exit 0", 0);
    bool refused = r.outcome == LSM_PROC_SPAWN_FAILED;
    lsm_subprocess_force_spawn_eagain_for_testing(0); /* never leak into later tests */
    ASSERT_TRUE(refused);
    PASS();
#endif
}
...
TEST(subprocess_run_spawn_failure) {
    ...
    const char *argv[] = {"/nonexistent/lsm-bogus-binary", NULL};
    lsm_proc_opts_t opts = {0};
    opts.bin = "/nonexistent/lsm-bogus-binary";
    opts.argv = argv;
    lsm_proc_result_t r;
    int rc = lsm_subprocess_run(&opts, &r);
...
TEST(subprocess_run_null_bin_rejected) {
    ...
    int rc = lsm_subprocess_run(&opts, &r);
    ASSERT_EQ(rc, -1);
    ASSERT_EQ(r.outcome, LSM_PROC_SPAWN_FAILED);
    PASS();
}

/* ── Layer 3: nonblocking handle + whole-tree cancellation (POSIX) ──────────
 *
 * The daemon cannot block its coordinator thread in lsm_subprocess_run(). It
...
static bool poll_until_terminal(lsm_subprocess_t *process, int timeout_ms, lsm_proc_result_t *out) {
    ...
    do {
        ...
        if (state == LSM_PROC_POLL_ERROR) {
            return false;
        }
        subprocess_test_pause();
    } while (lsm_now_ms() < deadline);
    return lsm_subprocess_poll(process, out) == LSM_PROC_POLL_TERMINAL;
}

static bool make_tree_pid_path(char path[64]) {
    strcpy(path, "/tmp/lsm-subprocess-tree-XXXXXX");
...
static bool wait_for_tree_pids(const char *path, lsm_subprocess_t *process, pid_t *parent_pid,
    ...
    do {
        ...
        lsm_proc_result_t ignored;
        if (lsm_subprocess_poll(process, &ignored) != LSM_PROC_POLL_RUNNING) {
            return false;
        }
        subprocess_test_pause();
    } while (lsm_now_ms() < deadline);
...
static int spawn_ignoring_tree(const char *pid_path, int quiet_timeout_ms, int cancel_grace_ms,
    ...
    const char *script = "trap '' TERM; "
                         "/bin/sh -c 'trap \"\" TERM; while :; do sleep 1; done' lsm-grandchild & "
                         "grandchild=$!; echo \"$$ $grandchild\" > \"$1\"; wait";
    const char *argv[] = {"/bin/sh", "-c", script, "lsm-parent", pid_path, NULL};
    lsm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.quiet_timeout_ms = quiet_timeout_ms;
    opts.cancel_grace_ms = cancel_grace_ms;
    return lsm_subprocess_spawn(&opts, out);
}
...
static void ordered_log_callback(const char *line, void *opaque) {
    ...
    if (index == 399) {
        ...
        capture->late_cancel_accepted = lsm_subprocess_request_cancel(capture->process);
    }
}
...
TEST(subprocess_spawn_returns_while_child_is_running) {
    ...
    uint64_t before_spawn = lsm_now_ms();
    int spawn_rc = lsm_subprocess_spawn(&opts, &process);
    uint64_t spawn_elapsed = lsm_now_ms() - before_spawn;
    ASSERT_EQ(spawn_rc, 0);
    ASSERT_NOT_NULL(process);

    lsm_proc_result_t result;
    uint64_t before_poll = lsm_now_ms();
    lsm_proc_poll_t first_poll = lsm_subprocess_poll(process, &result);
    uint64_t poll_elapsed = lsm_now_ms() - before_poll;
    bool cancel_accepted = lsm_subprocess_request_cancel(process);
     ...
     * LSM_LOCAL_CI_CPUS=4 fidelity pass, CI runners) can never make them
     * test-significant. */
    ASSERT_EQ(first_poll, LSM_PROC_POLL_RUNNING);
...
TEST(subprocess_natural_completion_is_cached_across_polls) {
    ...
    opts.cancel_grace_ms = 100;
    lsm_subprocess_t *process = NULL;
    ASSERT_EQ(lsm_subprocess_spawn(&opts, &process), 0);
    ASSERT_NOT_NULL(process);

    lsm_proc_result_t first;
    ASSERT_TRUE(poll_until_terminal(process, 2000, &first));
    lsm_proc_result_t second;
    lsm_proc_result_t third;
    lsm_proc_poll_t second_poll = lsm_subprocess_poll(process, &second);
    lsm_proc_poll_t third_poll = lsm_subprocess_poll(process, &third);
    lsm_subprocess_destroy(process);

    ASSERT_EQ(second_poll, LSM_PROC_POLL_TERMINAL);
    ASSERT_EQ(third_poll, LSM_PROC_POLL_TERMINAL);
    ASSERT_EQ(first.outcome, LSM_PROC_EXIT_NONZERO);
...
TEST(subprocess_cancel_is_idempotent_and_kills_ignoring_tree) {
    ...
    bool first_cancel = ready && lsm_subprocess_request_cancel(process);
    bool second_cancel = ready && lsm_subprocess_request_cancel(process);
    lsm_proc_result_t result;
    ...
    if (!terminal) {
        ...
        lsm_proc_result_t cleanup_result;
    ...
    ASSERT_TRUE(terminal);
    ASSERT_EQ(result.outcome, LSM_PROC_KILLED);
    ASSERT_TRUE(result.cancellation_requested);
...
TEST(subprocess_quiet_timeout_kills_ignoring_tree) {
    ...
    ASSERT_TRUE(ready);
    ASSERT_TRUE(terminal);
    ASSERT_EQ(result.outcome, LSM_PROC_HANG);
...
TEST(subprocess_cancel_grace_is_hard_capped) {
    ...
    pid_t grandchild_pid = -1;
    bool ready = wait_for_tree_pids(pid_path, process, &parent_pid, &grandchild_pid, 1000);
    bool cancel_accepted = ready && lsm_subprocess_request_cancel(process);
...
TEST(subprocess_poll_log_delivery_is_bounded_and_terminal_is_lossless) {
...
#else
    char log_path[] = "/tmp/lsm-subprocess-log-budget-XXXXXX";
    int log_fd = lsm_mkstemp(log_path);
    ...
    bool backlog_ready = wait_for_log_marker(log_path, "line-399", lsm_now_ms() + 2000U);
    lsm_proc_result_t result = {0};
    bool terminal = false;
    int max_poll_delivery = 0;
    uint64_t deadline = lsm_now_ms() + 5000U;
    while (backlog_ready && lsm_now_ms() < deadline) {
        int before = capture.count;
        lsm_proc_poll_t state = lsm_subprocess_poll(process, &result);
...
TEST(subprocess_final_log_drain_error_is_terminal_and_preserves_classification) {
#ifdef _WIN32
    SKIP_PLATFORM("POSIX log-rename final-drain probe; UTF-8 Windows open uses lsm_fopen");
#else
    char log_path[] = "/tmp/lsm-subprocess-log-drain-XXXXXX";
    int log_fd = lsm_mkstemp(log_path);
    ASSERT_TRUE(log_fd >= 0);
    (void)close(log_fd);
    ...
    bool backlog_ready = wait_for_log_marker(log_path, "line-129", lsm_now_ms() + 2000U);
    lsm_proc_result_t result = {0};
    lsm_proc_poll_t first =
        backlog_ready ? lsm_subprocess_poll(process, &result) : LSM_PROC_POLL_ERROR;
    int first_delivery = capture.count;
    bool moved = first == LSM_PROC_POLL_RUNNING && rename(log_path, saved_path) == 0;
    bool terminal = moved && poll_until_terminal(process, 2000, &result);
    int callbacks_at_terminal = capture.count;
    lsm_proc_result_t cached = {0};
    lsm_proc_poll_t cached_state =
        terminal ? lsm_subprocess_poll(process, &cached) : LSM_PROC_POLL_ERROR;
    ...
    ASSERT_TRUE(backlog_ready);
    ASSERT_EQ(first, LSM_PROC_POLL_RUNNING);
    ASSERT_EQ(first_delivery, 64);
    ASSERT_TRUE(moved);
    ASSERT_TRUE(terminal);
    ASSERT_TRUE(capture.ordered);
    ASSERT_EQ(callbacks_at_terminal, 64);
    ASSERT_EQ(cached_state, LSM_PROC_POLL_TERMINAL);
...
TEST(subprocess_posix_child_closes_unrelated_descriptors) {
...
#else
    char sentinel_path[] = "/tmp/lsm-subprocess-sentinel-XXXXXX";
    int sentinel = lsm_mkstemp(sentinel_path);
    ...
    const char *argv[] = {"/bin/sh", "-c", script, "lsm-fd-probe", fd_text, NULL};
    lsm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    lsm_proc_result_t result;
    int run_rc = lsm_subprocess_run(&opts, &result);
    (void)close(sentinel);
...
TEST(subprocess_root_exit_drains_surviving_descendant) {
    ...
    const char *argv[] = {"/bin/sh", "-c", script, "lsm-root-exit", pid_path, NULL};
    lsm_proc_opts_t opts = {0};
    opts.bin = "/bin/sh";
    opts.argv = argv;
    opts.cancel_grace_ms = 100;
    ...
    pid_t descendant = -1;
    uint64_t deadline = lsm_now_ms() + 1000;
    while (descendant <= 1 && lsm_now_ms() < deadline) {
 ...
 * supervisor misreported as a per-file crash. We guard lsm_build_win_cmdline by
...
static bool cmdline_roundtrips(const char *const *argv) {
    char cmd[4096];
    if (!lsm_build_win_cmdline(cmd, sizeof(cmd), argv)) {
        return false;
    }
    char parsed[16][256];
...
TEST(win_cmdline_index_worker_json) {
    ...
    char cmd[4096];
    ASSERT(lsm_build_win_cmdline(cmd, sizeof(cmd), argv));
...
TEST(win_cmdline_overflow_rejected) {
    ...
    char tiny[8];
    ASSERT_FALSE(lsm_build_win_cmdline(tiny, sizeof(tiny), argv));
    PASS();
}
...
TEST(win_cmdline_overflow_leaves_empty_string) {
    ...
    ASSERT_FALSE(lsm_build_win_cmdline(buf, sizeof(buf), argv)); /* overflows cap */
...
TEST(win_cmd_payload_is_verbatim_and_capacity_checked) {
    ...
    ASSERT(lsm_build_win_cmd_payload(result, strlen(expected) + 1, cmd, payload));
    ASSERT_STR_EQ(result, expected);
    ASSERT_NULL(strstr(result, "\\\"C:\\Users"));

    memset(result, 'x', sizeof(result));
    ASSERT_FALSE(lsm_build_win_cmd_payload(result, strlen(expected), cmd, payload));
...
TEST(win_cmd_payload_rejects_short_relative_and_non_cmd_paths) {
    ...
    char result[256];
    ASSERT_FALSE(lsm_build_win_cmd_payload(result, sizeof(result), "", payload));
    ASSERT_FALSE(lsm_build_win_cmd_payload(result, sizeof(result), "C", payload));
    ASSERT_FALSE(lsm_build_win_cmd_payload(result, sizeof(result), "C:", payload));
    ASSERT_FALSE(lsm_build_win_cmd_payload(result, sizeof(result), "cmd.exe", payload));
    ASSERT_FALSE(
        lsm_build_win_cmd_payload(result, sizeof(result), "C:\\tools\\other.exe", payload));
    ASSERT_FALSE(
        lsm_build_win_cmd_payload(result, sizeof(result), "C:\\tools\\cmd.exe\\child", payload));
    ASSERT(
        lsm_build_win_cmd_payload(result, sizeof(result), "c:/Windows/System32/CMD.EXE", payload));
    PASS();
}
