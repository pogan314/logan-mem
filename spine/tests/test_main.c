...
#include "foundation/compat.h"     /* lsm_setenv — #845 supervisor kill switch */
#include "foundation/compat_fs.h"  /* lsm_fopen — worker response file */
#include "foundation/constants.h"  /* LSM_SZ_4K — forced stderr buffer */
#include "foundation/log.h"        /* crash-durable worker log probe */
#include "foundation/mem.h"        /* lsm_mem_init — worker budget */
#include "foundation/platform.h"   /* lsm_file_exists — blocking-git marker */
...
#include "mcp/index_supervisor.h"  /* lsm_index_set_worker_role */
#include "mcp/mcp.h"               /* lsm_mcp_handle_tool — act as a real worker */
#include <sqlite3.h>
#include <errno.h>
#include <stdbool.h>
#include <stdint.h>
...
#define TF_BLOCKING_GIT_MARKER_ENV "LSM_TEST_RUNTIME_BLOCKING_GIT_PID_FILE"
...
#endif
    FILE *marker = lsm_fopen(marker_path, "wbx");
    if (!marker) {
        ...
        bool marker_exists = lsm_file_exists(marker_path);
...
#else
        return lsm_file_exists(marker_path) ? 0 : 30;
#endif
    }
#ifdef _WIN32
 ...
 * does not isolate them: lsm_codex_config_dir() and its siblings return the
...
static bool tf_setup_cache_sentinel(void) {
    snprintf(tf_home_sentinel, sizeof(tf_home_sentinel), "/tmp/lsm-test-home-XXXXXX");
    if (!lsm_mkdtemp(tf_home_sentinel)) {
        return false;
    }
    /* Legacy integration fixtures derive DB paths from HOME, while production
     * cache_dir() prefers LSM_CACHE_DIR. A private HOME plus no inherited cache
     * override keeps both conventions pointed at the same isolated tree. */
    lsm_setenv("HOME", tf_home_sentinel, 1);
    lsm_unsetenv("LSM_CACHE_DIR");
    for (size_t i = 0U; i < sizeof(tf_client_home_overrides) / sizeof(tf_client_home_overrides[0]);
         i++) {
        lsm_unsetenv(tf_client_home_overrides[i]);
    }
...
static void tf_index_worker_probe(const char *args_json, const char *response_out) {
    ...
    if (strstr(args_json, "\"clean\"")) {
        FILE *response = response_out ? lsm_fopen(response_out, "wb") : NULL;
        if (response) {
    ...
    if (strstr(args_json, "\"buffered-kill\"")) {
         ...
         * is terminated. */
        lsm_log_info("index.worker.buffered_kill_probe", "phase", "before_kill");
#ifdef _WIN32
    ...
    if (strstr(args_json, "\"oversize\"")) {
        ...
        if (response) {
            written =
                fseek(response, (long)LSM_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX, SEEK_SET) == 0 &&
...
#endif
        const char *marker = getenv("LSM_INDEX_MARKER_FILE");
        FILE *ready = marker ? lsm_fopen(marker, "wb") : NULL;
        if (ready) {
            (void)fprintf(
                ready, "single=%s\nmarker=%s\nquarantine=%s\nbudget=%zu\ndescendant=%ld\n",
                getenv("LSM_INDEX_SINGLE_THREAD") ? getenv("LSM_INDEX_SINGLE_THREAD") : "", marker,
                getenv("LSM_INDEX_QUARANTINE_FILE") ? getenv("LSM_INDEX_QUARANTINE_FILE") : "",
                lsm_mem_budget(), descendant);
            (void)fclose(ready);
        }
 ...
 * exact build-bound worker grammar produced by lsm_index_worker_start(), act
...
static int tf_maybe_run_index_worker(int argc, char **argv) {
    ...
    lsm_index_worker_argv_status_t status =
        lsm_index_worker_parse_process_argv(argc, argv, &invocation);
    if (status == LSM_INDEX_WORKER_ARGV_NOT_WORKER) {
        return -1;
    }
    if (status != LSM_INDEX_WORKER_ARGV_VALID) {
        (void)fprintf(stderr, "LSM test index worker could not start: %s\n",
                      lsm_index_worker_argv_status_message(status));
        return 1;
    }
     ...
     * the code under test. */
    static char tf_worker_forced_buffer[LSM_SZ_4K];
    ...
    lsm_mem_init_with_cap(0.5, invocation.memory_budget_bytes);
    tf_index_worker_probe(invocation.args_json, invocation.response_out);
    lsm_mcp_server_t *srv = lsm_mcp_server_new(NULL);
    if (!srv) {
        return 1;
    }
    char *result = lsm_mcp_handle_tool(srv, "index_repository", invocation.args_json);
    if (result) {
 ...
 * lsm_popen — the same cmd.exe-grandchild path git takes — passing the numeric
 * value of an inheritable listening-socket handle. If lsm_popen correctly
...
static int tf_maybe_run_daemon_ipc_lock_probe(int argc, char **argv) {
    ...
    if (strcmp(argv[2], "startup") == 0) {
        ...
        result = lsm_daemon_ipc_startup_lock_try_acquire(endpoint, &lock);
        if (!lsm_daemon_ipc_startup_lock_release(&lock)) {
            result = -1;
        }
    } else if (strcmp(argv[2], "lifetime") == 0) {
        lsm_daemon_ipc_lifetime_reservation_t *reservation = NULL;
        result = lsm_daemon_ipc_lifetime_reservation_try_acquire(endpoint, &reservation);
...
static int tf_maybe_run_version_cohort_crash_holder(int argc, char **argv) {
    ...
    lsm_version_cohort_status_t status =
        manager ? lsm_version_cohort_acquire(manager, &identity, UINT64_MAX, &lease, &conflict)
                : LSM_VERSION_COHORT_IO;
    FILE *ready = status == LSM_VERSION_COHORT_OK ? lsm_fopen(argv[4], "wb") : NULL;
...
static int tf_maybe_run_runtime_hello_client(int argc, char **argv) {
    ...
    lsm_daemon_runtime_client_t *client =
        endpoint ? lsm_daemon_runtime_client_connect(endpoint, &identity,
                                                     TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS, &result)
                 : NULL;
    bool accepted = client && result.status == LSM_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
                    result.hello_status == LSM_DAEMON_HELLO_COMPATIBLE;
    bool closed =
        !client || lsm_daemon_runtime_client_close(client, TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS);
    lsm_daemon_ipc_endpoint_free(endpoint);
    if (!accepted) {
        return 26;
    }
...
static int tf_maybe_run_runtime_activation_client(int argc, char **argv) {
    ...
    bool action_valid = action_end != argv[6] && *action_end == '\0' &&
                        action_value >= (unsigned long)LSM_DAEMON_RUNTIME_ACTIVATION_INSTALL &&
                        action_value <= (unsigned long)LSM_DAEMON_RUNTIME_ACTIVATION_UNINSTALL;
    lsm_daemon_ipc_endpoint_t *endpoint =
        action_valid ? lsm_daemon_ipc_endpoint_new(argv[3], argv[2]) : NULL;
    lsm_daemon_build_identity_t identity = {
        .semantic_version = argv[4],
        .build_fingerprint = argv[5],
    };
    lsm_daemon_runtime_activation_result_t result;
    memset(&result, 0, sizeof(result));
    bool exchanged =
        endpoint && lsm_daemon_runtime_request_activation_shutdown(
                        endpoint, &identity, (lsm_daemon_runtime_activation_action_t)action_value,
                        TF_RUNTIME_IMAGE_EXCHANGE_TIMEOUT_MS, &result);
    lsm_daemon_ipc_endpoint_free(endpoint);
...
/* LSM_SKIP_PERF=1 excludes throughput/scale/perf-metric suites from the run.
 * The fidelity pass (LSM_LOCAL_CI_CPUS=4) and CI PRs set it: those suites
 ...
 * suites run at full power in the perf/release legs (LSM_SKIP_PERF unset). The
...
extern void lsm_kind_in_set_free_cache(void);
...
int main(int argc, char **argv) {
     ...
     * only under LSM_CLI_ENABLE_TEST_API — never in a production binary. */
    if (!getenv("LSM_TEST_BUILD_FINGERPRINT")) {
        (void)lsm_setenv("LSM_TEST_BUILD_FINGERPRINT",
    ...
    if (argc == 2 && strcmp(argv[1], "--version") == 0) {
        (void)puts("logan-spine-mcp test-runner");
        return 0;
    }
    ...
    (void)lsm_index_supervisor_capture_build_fingerprint();
    ...
    /* #845 belt-and-suspenders: this binary EMBEDS lsm_mcp_handle_tool. The
    ...
    const char *skip_perf_env = getenv("LSM_SKIP_PERF");
