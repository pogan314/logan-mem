...
TEST(intern_create_free) {
    LSMInternPool *pool = lsm_intern_create();
    ASSERT_NOT_NULL(pool);
    ASSERT_EQ(lsm_intern_count(pool), 0);
    lsm_intern_free(pool);
    PASS();
}

TEST(intern_basic) {
    LSMInternPool *pool = lsm_intern_create();
    const char *s1 = lsm_intern(pool, "hello");
    ASSERT_NOT_NULL(s1);
    ASSERT_STR_EQ(s1, "hello");
    ASSERT_EQ(lsm_intern_count(pool), 1);
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_dedup) {
    LSMInternPool *pool = lsm_intern_create();
    const char *s1 = lsm_intern(pool, "hello");
    const char *s2 = lsm_intern(pool, "hello");
    /* Must return same pointer */
    ASSERT_EQ((uintptr_t)s1, (uintptr_t)s2);
    ASSERT_EQ(lsm_intern_count(pool), 1); /* still 1 */
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_different_strings) {
    LSMInternPool *pool = lsm_intern_create();
    const char *s1 = lsm_intern(pool, "hello");
    const char *s2 = lsm_intern(pool, "world");
    ASSERT_NEQ((uintptr_t)s1, (uintptr_t)s2);
    ASSERT_STR_EQ(s1, "hello");
    ASSERT_STR_EQ(s2, "world");
    ASSERT_EQ(lsm_intern_count(pool), 2);
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_n_with_length) {
    LSMInternPool *pool = lsm_intern_create();
    const char *s1 = lsm_intern_n(pool, "hello world", 5);
    ASSERT_STR_EQ(s1, "hello");
...
TEST(intern_many_strings) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        const char *s = lsm_intern(pool, buf);
        ASSERT_NOT_NULL(s);
    }
    ASSERT_EQ(lsm_intern_count(pool), 1000);
    ...
    ASSERT_EQ(lsm_intern_count(pool), 1000); /* still 1000 */
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_bytes) {
    LSMInternPool *pool = lsm_intern_create();
    lsm_intern(pool, "abc");   /* 3 bytes + NUL */
    lsm_intern(pool, "defgh"); /* 5 bytes + NUL */
    /* bytes should be at least 8 (3+5 for the content) */
    ASSERT_GTE(lsm_intern_bytes(pool), 8);
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_null_returns_null) {
    LSMInternPool *pool = lsm_intern_create();
    const char *s = lsm_intern(pool, NULL);
    ASSERT_NULL(s);
    ASSERT_EQ(lsm_intern_count(pool), 0);
    lsm_intern_free(pool);
    PASS();
...
TEST(intern_n_zero_len) {
    ...
    LSMInternPool *pool = lsm_intern_create();
    const char *s = lsm_intern_n(pool, "anything", 0);
    ASSERT_NOT_NULL(s);
    ASSERT_STR_EQ(s, "");
    ASSERT_EQ(lsm_intern_count(pool), 1);
    /* Should dedup with intern("") */
    const char *s2 = lsm_intern(pool, "");
...
TEST(intern_same_prefix_different_lengths) {
    ...
    const char *s3 = lsm_intern_n(pool, src, 3); /* "abc" */
    const char *s5 = lsm_intern_n(pool, src, 5); /* "abcde" */
    const char *s8 = lsm_intern_n(pool, src, 8); /* "abcdefgh" */
    ...
    ASSERT_EQ(lsm_intern_count(pool), 3);
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_pointer_stability) {
    ...
    LSMInternPool *pool = lsm_intern_create();
    const char *original = lsm_intern(pool, "sentinel_value");
    ...
    ASSERT_EQ(lsm_intern_count(pool), 10001);
    ...
    const char *again = lsm_intern(pool, "sentinel_value");
...
TEST(intern_two_pools_independent) {
    ...
    LSMInternPool *p1 = lsm_intern_create();
    LSMInternPool *p2 = lsm_intern_create();
    const char *s1 = lsm_intern(p1, "shared");
    const char *s2 = lsm_intern(p2, "shared");
...
TEST(intern_free_empty_pool) {
    ...
    LSMInternPool *pool = lsm_intern_create();
    ASSERT_EQ(lsm_intern_count(pool), 0);
    ASSERT_EQ(lsm_intern_bytes(pool), 0);
    lsm_intern_free(pool);
    PASS();
}
...
TEST(intern_bytes_tracking) {
    ...
    lsm_intern(pool, "abc");   /* 3 bytes */
    ASSERT_EQ(lsm_intern_bytes(pool), 3);

    lsm_intern(pool, "defgh"); /* 5 bytes */
    ASSERT_EQ(lsm_intern_bytes(pool), 8);

    /* Duplicate should not add bytes */
    lsm_intern(pool, "abc");
    ASSERT_EQ(lsm_intern_bytes(pool), 8);

    lsm_intern(pool, "");      /* 0 bytes */
    ASSERT_EQ(lsm_intern_bytes(pool), 8);

    lsm_intern(pool, "x");     /* 1 byte */
    ASSERT_EQ(lsm_intern_bytes(pool), 9);
...
TEST(intern_free_null_pool) {
    ...
    lsm_intern_free(NULL);
    PASS();
}

TEST(intern_count_null_pool) {
    ASSERT_EQ(lsm_intern_count(NULL), 0);
    PASS();
}
