...
static LSMFileResult *extract_with(const char *source, LSMLanguage lang, const char *fname) {
    return lsm_extract_file(source, (int)strlen(source), lang, "test", fname, 0, NULL, NULL);
}

static LSMFileResult *extract_ts(const char *source) {
    return extract_with(source, LSM_LANG_TYPESCRIPT, "main.ts");
}

static LSMFileResult *extract_tsx(const char *source) {
    return extract_with(source, LSM_LANG_TSX, "main.tsx");
}

static LSMFileResult *extract_js(const char *source) {
    return extract_with(source, LSM_LANG_JAVASCRIPT, "main.js");
}

static LSMFileResult *extract_dts(const char *source) {
    return extract_with(source, LSM_LANG_TYPESCRIPT, "main.d.ts");
}
 ...
 * emitted first. */
static bool ts_same_leaf_render_occurrences_are_exact(const LSMFileResult *r, const char *source) {
    ...
    for (int i = 0; i < r->resolved_calls.count; i++) {
        ...
        if (resolved->kind != LSM_RESOLVED_INVOCATION || resolved->confidence <= 0 ||
    ...
    const LSMResolvedCall *alpha_join =
        alpha_call ? lsm_pipeline_find_lsp_resolution(&r->resolved_calls, alpha_call, false) : NULL;
    const LSMResolvedCall *beta_join =
        beta_call ? lsm_pipeline_find_lsp_resolution(&r->resolved_calls, beta_call, false) : NULL;
...
TEST(tslsp_smoke_minimal_function) {
    LSMFileResult *r = extract_ts("function greet(name: string): string {\n"
                                  "    return 'hello ' + name;\n"
                                  "}\n");
    ASSERT_NOT_NULL(r);
    ASSERT(r->resolved_calls.count >= 0);
...
TEST(tslsp_smoke_jsx_minimal) {
    LSMFileResult *r = extract_tsx("function App(): JSX.Element {\n"
...
TEST(tslsp_smoke_js_minimal) {
    LSMFileResult *r = extract_js("function add(a, b) {\n"
                                  "    return a + b;\n"
                                  "}\n"
                                  "add(1, 2);\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_smoke_dts_minimal) {
    LSMFileResult *r = extract_dts("export interface User {\n"
                                   "    name: string;\n"
                                   "    age: number;\n"
                                   "}\n"
...
TEST(tslsp_param_simple) {
    LSMFileResult *r = extract_ts("class Database { query(sql: string): string { return ''; } }\n"
...
TEST(tslsp_param_multi) {
    LSMFileResult *r = extract_ts("class Logger { info(msg: string): void {} }\n"
...
TEST(tslsp_param_arrow) {
    LSMFileResult *r = extract_ts("class Conn { close(): void {} }\n"
...
TEST(tslsp_param_optional) {
    LSMFileResult *r = extract_ts("class Foo { ping(): void {} }\n"
...
TEST(tslsp_param_with_default) {
    LSMFileResult *r = extract_ts("class Cache { put(k: string): void {} }\n"
...
TEST(tslsp_param_destructured) {
    LSMFileResult *r = extract_ts("interface Box { tag: string; }\n"
...
TEST(tslsp_return_simple) {
    LSMFileResult *r = extract_ts("class File { read(): number { return 0; } }\n"
...
TEST(tslsp_return_chain) {
    LSMFileResult *r = extract_ts("class Result { describe(): string { return ''; } }\n"
...
TEST(tslsp_return_inferred_local) {
    LSMFileResult *r = extract_ts("class Conn { ping(): void {} }\n"
...
TEST(tslsp_return_async_promise) {
    LSMFileResult *r = extract_ts("class User { hello(): void {} }\n"
...
TEST(tslsp_return_method_no_rttype) {
    LSMFileResult *r = extract_ts("class Service { handle(): string { return ''; } }\n"
...
TEST(tslsp_chain_fluent) {
    LSMFileResult *r = extract_ts("class Query {\n"
...
TEST(tslsp_destructure_array) {
    LSMFileResult *r = extract_ts("class A { run(): void {} }\n"
...
TEST(tslsp_destructure_param) {
    LSMFileResult *r = extract_ts("interface Pkg { tool: { name(): string } }\n"
...
TEST(tslsp_object_property_method) {
    LSMFileResult *r = extract_ts("class Greeter { say(msg: string): void {} }\n"
                                  "function go() {\n"
...
TEST(tslsp_object_nested_property) {
    LSMFileResult *r = extract_ts("class Tool { use(): number { return 0; } }\n"
                                  "function go() {\n"
...
TEST(tslsp_alias_simple) {
    LSMFileResult *r = extract_ts("class Inner { ping(): void {} }\n"
                                  "type Wrapper = Inner;\n"
...
TEST(tslsp_class_method_dispatch) {
    LSMFileResult *r = extract_ts("class Foo { bar(): void {} }\n"
...
TEST(tslsp_class_this_dispatch) {
    LSMFileResult *r = extract_ts("class Counter {\n"
                                  "    inc(): void {}\n"
                                  "    bump(): void { this.inc(); }\n"
                                  "}\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_class_inheritance_method) {
    LSMFileResult *r = extract_ts("class Animal { breathe(): void {} }\n"
...
TEST(tslsp_class_super_call) {
    LSMFileResult *r = extract_ts("class Base { run(): void {} }\n"
...
TEST(tslsp_iface_method) {
    LSMFileResult *r = extract_ts("interface Closer { close(): void; }\n"
...
TEST(tslsp_iface_field_chain) {
    LSMFileResult *r = extract_ts("interface Tool { run(): void; }\n"
...
TEST(tslsp_stdlib_array_push) {
    LSMFileResult *r = extract_ts("function go() { const a: number[] = []; a.push(1); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_array_map) {
    LSMFileResult *r = extract_ts("function go(xs: number[]) { xs.map(x => x + 1); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_console_log) {
    LSMFileResult *r = extract_ts("function go() { console.log('hi'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_map_get) {
    LSMFileResult *r = extract_ts("function go(m: Map<string, number>) { m.get('k'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_set_has) {
    LSMFileResult *r = extract_ts("function go(s: Set<string>) { s.has('k'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_json_stringify) {
    LSMFileResult *r = extract_ts("function go(x: object) { JSON.stringify(x); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_object_keys) {
    LSMFileResult *r = extract_ts("function go(x: object) { Object.keys(x); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_optional_chain_method) {
    LSMFileResult *r = extract_ts("class Foo { run(): void {} }\n"
...
TEST(tslsp_literal_const_assertion) {
    LSMFileResult *r = extract_ts("function go() { const k = 'foo' as const; }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_module_named_import_call) {
    LSMFileResult *r = extract_ts("import { console as logger } from 'console';\n"
...
TEST(tslsp_diag_unknown_call_emits_unresolved) {
    LSMFileResult *r = extract_ts("function go() { mystery(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_nocrash_unclosed_string) {
    LSMFileResult *r = extract_ts("function go() { const s = 'unclosed");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_self_referential_alias) {
    LSMFileResult *r = extract_ts("type T = T;\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_with_block_legacy) {
    LSMFileResult *r = extract_js("function go(o) { with (o) { foo(); } }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_eval_value) {
    LSMFileResult *r = extract_js("function go() { eval('1+1'); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_using_decl) {
    LSMFileResult *r = extract_ts("function go() { using x = { [Symbol.dispose]: () => {} }; }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_megasource) {
    ...
    memset(buf, ';', 50000);
    buf[50000] = '\0';
    LSMFileResult *r = extract_ts(buf);
    free(buf);
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_generic_identity_inference) {
    LSMFileResult *r = extract_ts("function identity<T>(x: T): T { return x; }\n"
                                  "class Box { use(): void {} }\n"
...
TEST(tslsp_generic_promise_resolve) {
    LSMFileResult *r = extract_ts("function go() { Promise.resolve(42).then(x => x); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_generic_array_from) {
    LSMFileResult *r = extract_ts("function go(s: string) { Array.from(s); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_generic_array_isArray) {
    LSMFileResult *r = extract_ts("function go(x: unknown) { Array.isArray(x); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_await_promise_all) {
    LSMFileResult *r = extract_ts("async function go() { await Promise.all([1, 2, 3]); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_narrow_typeof_check) {
    LSMFileResult *r = extract_ts("function go(x: string | number) {\n"
...
TEST(tslsp_narrow_instanceof) {
    LSMFileResult *r = extract_ts("class A { ping(): void {} }\n"
                                  "class B { pong(): void {} }\n"
                                  "function go(x: A | B) {\n"
...
TEST(tslsp_literal_string_union) {
    LSMFileResult *r = extract_ts("type Mode = 'on' | 'off';\n"
...
TEST(tslsp_union_common_method) {
    LSMFileResult *r = extract_ts("interface A { run(): void; }\n"
...
TEST(tslsp_union_many_members_no_overflow) {
    ...
    LSMFileResult *r = extract_ts("function go(x: 'a' | 'b' | 'c' | 'd' | 'e' | 'f' | 'g' | 'h' "
                                  "| 'i' | 'j' | 'k' | 'l' | 'm' | 'n' | 'o' | 'p' | 'q' | 'r' "
                                  "| 's' | 't') { return x; }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
...
TEST(tslsp_class_static_method) {
    LSMFileResult *r = extract_ts("class Util { static helper(): void {} }\n"
...
TEST(tslsp_class_method_then_field) {
    LSMFileResult *r = extract_ts("class Inner { fire(): void {} }\n"
...
TEST(tslsp_class_chained_field_method) {
    LSMFileResult *r = extract_ts("class Leaf { ping(): void {} }\n"
...
TEST(tslsp_stdlib_array_filter) {
    LSMFileResult *r = extract_ts("function go(xs: number[]) { xs.filter(x => x > 0); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_array_find) {
    LSMFileResult *r = extract_ts("function go(xs: number[]) { xs.find(x => x > 0); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_array_reduce) {
    LSMFileResult *r = extract_ts("function go(xs: number[]) { xs.reduce((a, b) => a + b, 0); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_string_split) {
    LSMFileResult *r = extract_ts("function go(s: string) { s.split(',').map(p => p.trim()); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_set_add) {
    LSMFileResult *r = extract_ts("function go(s: Set<string>) { s.add('x').size; }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_nocrash_circular_extends) {
    LSMFileResult *r = extract_ts("class A extends B {}\n"
                                  "class B extends A {}\n"
                                  "function go(a: A) { a.x; }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
...
TEST(tslsp_nocrash_recursive_type) {
    LSMFileResult *r = extract_ts("interface List { next?: List; value: number; }\n"
...
TEST(tslsp_nocrash_unicode_identifier) {
    LSMFileResult *r = extract_ts("class \xc3\x84pf { ping(): void {} }\n"
...
TEST(tslsp_nocrash_decorator) {
    LSMFileResult *r = extract_ts("function dec(target: any) { return target; }\n"
                                  "@dec\n"
                                  "class Foo { run(): void {} }\n"
                                  "function go(f: Foo) { f.run(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_nocrash_private_field) {
    LSMFileResult *r = extract_ts("class Foo { #x = 1; bar(): number { return this.#x; } }\n"
                                  "function go(f: Foo) { f.bar(); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_nocrash_getter_setter) {
    LSMFileResult *r = extract_ts("class Foo { _x = 0; get x(): number { return this._x; } set "
...
TEST(tslsp_nocrash_abstract_class) {
    LSMFileResult *r = extract_ts("abstract class Base { abstract step(): void; }\n"
...
TEST(tslsp_jsx_import_requires_registered_component) {
    LSMFileResult *r = extract_tsx("import { MissingCard } from '@app/missing-card';\n"
...
TEST(tslsp_jsx_component_resolutions_join_exact_tag_sites) {
    ...
    LSMFileResult *r = extract_tsx(source);
    ASSERT_NOT_NULL(r);
    ...
    const LSMCall *self_call = NULL;
    const LSMCall *open_call = NULL;
    int carrier_count = 0;
    for (int i = 0; i < r->calls.count; i++) {
        const LSMCall *call = &r->calls.items[i];
    ...
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const LSMResolvedCall *resolved = &r->resolved_calls.items[i];
        if (resolved->kind == LSM_RESOLVED_INVOCATION && resolved->caller_qn &&
    ...
    const LSMResolvedCall *self_join =
        self_call ? lsm_pipeline_find_lsp_resolution(&r->resolved_calls, self_call, false) : NULL;
    const LSMResolvedCall *open_join =
        open_call ? lsm_pipeline_find_lsp_resolution(&r->resolved_calls, open_call, false) : NULL;
...
TEST(tslsp_jsx_local_tag_shadow_blocks_module_component) {
    ...
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const LSMResolvedCall *resolved = &r->resolved_calls.items[i];
        if (resolved->kind == LSM_RESOLVED_INVOCATION && resolved->confidence >= 0.6f &&
...
TEST(tslsp_jsx_intrinsic_skipped) {
    LSMFileResult *r = extract_tsx("function go(): JSX.Element { return <div>plain</div>; }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_tsx_typed_props_method_call) {
    LSMFileResult *r = extract_tsx("class Service { fetch(): string { return ''; } }\n"
...
TEST(tslsp_tuple_destructure_method) {
    LSMFileResult *r = extract_ts("class Box { run(): void {} }\n"
...
TEST(tslsp_stdlib_number_toString) {
    LSMFileResult *r = extract_ts("function go(n: number) { n.toString(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_number_toFixed) {
    LSMFileResult *r = extract_ts("function go(n: number) { n.toFixed(2); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_jsdoc_param_method_call) {
    LSMFileResult *r = extract_js("class Database { query(sql) { return ''; } }\n"
                                  "/** @param {Database} db */\n"
...
TEST(tslsp_jsdoc_returns_propagation) {
    LSMFileResult *r = extract_js("class Conn { ping() {} }\n"
                                  "/** @returns {Conn} */\n"
                                  "function makeConn() { return new Conn(); }\n"
                                  "function go() { const c = makeConn(); c.ping(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_jsdoc_multi_param) {
    LSMFileResult *r = extract_js("class Logger { info(s) {} }\n"
                                  "class Cache { put(k) {} }\n"
                                  "/**\n"
                                  " * @param {Logger} log\n"
                                  " * @param {Cache} cache\n"
...
TEST(tslsp_jsdoc_array_generic) {
    LSMFileResult *r = extract_js("/** @param {Array<number>} xs */\n"
...
TEST(tslsp_jsdoc_promise_returns) {
    LSMFileResult *r = extract_js("/** @returns {Promise<string>} */\n"
...
TEST(tslsp_real_observer_pattern) {
    LSMFileResult *r = extract_ts("interface Observer { update(value: number): void; }\n"
                                  "class Subject {\n"
...
TEST(tslsp_readonly_field_method) {
    LSMFileResult *r = extract_ts("class Tool { fire(): void {} }\n"
...
TEST(tslsp_callback_returning_method_call) {
    LSMFileResult *r = extract_ts("class Box { run(): string { return ''; } }\n"
...
TEST(tslsp_this_in_arrow_method) {
    LSMFileResult *r = extract_ts("class Foo {\n"
                                  "    a: number = 1;\n"
                                  "    handler = () => { this.bar(); };\n"
                                  "    bar(): void {}\n"
                                  "}\n");
...
TEST(tslsp_nocrash_enum_basic) {
    LSMFileResult *r = extract_ts("enum Color { Red, Green, Blue }\n"
...
TEST(tslsp_nocrash_namespace) {
    LSMFileResult *r = extract_ts("namespace MyNS { export function inner(): void {} }\n"
...
TEST(tslsp_imported_namespace_shadows_local_namespace) {
    LSMFileResult *r = extract_ts("import * as MyNS from './external';\n"
...
TEST(tslsp_nested_namespace_preserves_exact_callers) {
    LSMFileResult *r = extract_ts("namespace App {\n"
...
TEST(tslsp_nocrash_async_generator) {
    LSMFileResult *r = extract_ts("async function* go() { yield 1; yield 2; }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_template_literal_expr) {
    LSMFileResult *r = extract_ts("function go(s: string) { console.log(`hello ${s.length}`); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_jsdoc_param_after_text) {
    LSMFileResult *r = extract_js("class Box { run() {} }\n"
                                  "/**\n"
                                  " * Helpful description.\n"
...
TEST(tslsp_jsdoc_typedef_used_via_param) {
    ...
    LSMFileResult *r = extract_js("/** @typedef {{ name: string }} User */\n"
                                  "/** @param {User} u */\n"
...
TEST(tslsp_obj_literal_method_property) {
    LSMFileResult *r = extract_ts("class Logger { info(): void {} }\n"
                                  "function go() {\n"
...
TEST(tslsp_ctx_array_forEach_callback) {
    LSMFileResult *r = extract_ts("class Job { execute(): void {} }\n"
...
TEST(tslsp_ctx_array_block_body_callback) {
    LSMFileResult *r = extract_ts("class Foo { ping(): void {} bar(): string { return ''; } }\n"
                                  "function go(xs: Foo[]) {\n"
...
TEST(tslsp_narrow_discriminated_kind) {
    LSMFileResult *r = extract_ts("interface Circle { kind: 'circle'; r(): number; }\n"
...
TEST(tslsp_stdlib_array_concat) {
    LSMFileResult *r = extract_ts("function go(a: number[], b: number[]) { a.concat(b); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_array_slice) {
    LSMFileResult *r = extract_ts("function go(a: string[]) { a.slice(0, 3); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_string_repeat) {
    LSMFileResult *r = extract_ts("function go(s: string) { s.repeat(3); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_promise_catch) {
    LSMFileResult *r = extract_ts("function go(p: Promise<string>) { p.catch(e => e); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, ".go", "catch"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_stdlib_set_size_and_clear) {
    LSMFileResult *r = extract_ts("function go(s: Set<string>) { s.clear(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_class_implements_interface) {
    LSMFileResult *r = extract_ts("interface Runnable { run(): void; }\n"
...
TEST(tslsp_class_multi_inheritance_chain) {
    LSMFileResult *r = extract_ts("class A { fromA(): void {} }\n"
...
TEST(tslsp_type_predicate_guard) {
    LSMFileResult *r = extract_ts("class A { aOnly(): void {} }\n"
...
TEST(tslsp_nocrash_symbol_iterator) {
    LSMFileResult *r = extract_ts("class Iter { [Symbol.iterator]() { return { next() { return { "
                                  "value: 1, done: false }; } }; } }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_nocrash_index_signature) {
    LSMFileResult *r = extract_ts("interface Bag { [k: string]: number; }\n"
...
/* ── Cross-file resolution (uses lsm_run_ts_lsp_cross directly) ─────────────── */
...
TEST(tslsp_crossfile_method_dispatch) {
    ...
    lsm_run_ts_lsp_cross(&arena, source, (int)strlen(source), "test.main",
...
TEST(tslsp_scale_many_defs_no_crash_issue344) {
    ...
    LSMLSPDef *defs = (LSMLSPDef *)lsm_arena_alloc(&arena, (size_t)SCALE_N * sizeof(LSMLSPDef));
    ASSERT_NOT_NULL(defs);
    memset(defs, 0, (size_t)SCALE_N * sizeof(LSMLSPDef));
    for (int i = 0; i < SCALE_N; i++) {
        defs[i].qualified_name = lsm_arena_sprintf(&arena, "test.mod%d.fn%d", i, i);
        defs[i].short_name = lsm_arena_sprintf(&arena, "fn%d", i);
        defs[i].label = "Function";
        defs[i].def_module_qn = lsm_arena_sprintf(&arena, "test.mod%d", i);
...
TEST(tslsp_crossfile_function_call) {
    ...
    lsm_run_ts_lsp_cross(&arena, source, (int)strlen(source), "test.main", false, false, false,
...
TEST(tslsp_stdlib_date_toISOString) {
    LSMFileResult *r = extract_ts("function go(d: Date) { d.toISOString(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_date_getTime) {
    LSMFileResult *r = extract_ts("function go(d: Date) { d.getTime(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_regexp_test) {
    LSMFileResult *r = extract_ts("function go(re: RegExp) { re.test('x'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_regexp_exec) {
    LSMFileResult *r = extract_ts("function go(re: RegExp) { re.exec('x'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_error_toString) {
    LSMFileResult *r = extract_ts("function go(e: Error) { e.toString(); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, ".go", "toString"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_stdlib_math_floor) {
    LSMFileResult *r = extract_ts("function go(n: number) { Math.floor(n); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_math_max) {
    LSMFileResult *r = extract_ts("function go() { Math.max(1, 2, 3); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_stdlib_math_random) {
    LSMFileResult *r = extract_ts("function go() { Math.random(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_react_useEffect_pattern) {
    LSMFileResult *r = extract_tsx("function useEffect(cb: () => void, deps?: unknown[]): void {}\n"
                                   "function App(): JSX.Element {\n"
...
TEST(tslsp_react_useCallback_pattern) {
    LSMFileResult *r = extract_tsx("function useCallback<T extends (...args: any[]) => any>(cb: T, "
...
TEST(tslsp_nocrash_iife) {
    LSMFileResult *r = extract_ts("(function() { console.log('hi'); })();\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_chained_optional) {
    LSMFileResult *r = extract_ts("interface A { b?: { c?: { d?: () => void } } }\n"
...
TEST(tslsp_narrow_switch_three_way) {
    LSMFileResult *r = extract_ts("interface A { kind: 'a'; aOnly(): void; }\n"
...
TEST(tslsp_overload_simple) {
    LSMFileResult *r = extract_ts("interface Transform {\n"
...
TEST(tslsp_static_factory_returns_instance) {
    LSMFileResult *r = extract_ts("class Conn {\n"
...
TEST(tslsp_generator_with_yield) {
    LSMFileResult *r = extract_ts("function* gen(): Generator<number> { yield 1; yield 2; }\n"
...
TEST(tslsp_object_destructure_param) {
    LSMFileResult *r = extract_ts("interface Config { port: number; host: string; }\n"
...
TEST(tslsp_default_export_class) {
    LSMFileResult *r = extract_ts("export default class Foo { run(): void {} }\n"
                                  "function go(f: Foo) { f.run(); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, ".go", "run"), 0);
    lsm_free_result(r);
...
TEST(tslsp_export_named_class) {
    LSMFileResult *r = extract_ts("export class Bar { fire(): void {} }\n"
...
TEST(tslsp_class_with_async_method) {
    LSMFileResult *r = extract_ts("class Service {\n"
...
TEST(tslsp_method_on_returned_promise_chain) {
    LSMFileResult *r = extract_ts("class API {\n"
...
TEST(tslsp_nocrash_template_literal_type) {
    LSMFileResult *r = extract_ts("type Greeting = `hello ${string}`;\n"
...
TEST(tslsp_nocrash_intersect_with_func) {
    LSMFileResult *r = extract_ts("type Cb = (() => void) & { extra: number };\n"
...
TEST(tslsp_nocrash_class_expression) {
    LSMFileResult *r = extract_ts("const C = class { run(): void {} };\n"
...
TEST(tslsp_partial_method_passthrough) {
    LSMFileResult *r = extract_ts("interface User { greet(): void; name: string; }\n"
...
TEST(tslsp_readonly_method) {
    LSMFileResult *r = extract_ts("interface Conn { ping(): void; }\n"
...
TEST(tslsp_required_field_method) {
    LSMFileResult *r = extract_ts("interface Box { tool: { fire(): void; }; }\n"
...
TEST(tslsp_nonnullable_method) {
    LSMFileResult *r = extract_ts("interface Foo { run(): void; }\n"
...
TEST(tslsp_array_of_class_map) {
    LSMFileResult *r = extract_ts("class User { name(): string { return ''; } }\n"
...
TEST(tslsp_throwing_error_method) {
    LSMFileResult *r = extract_ts("function go() {\n"
...
TEST(tslsp_nocrash_module_declaration) {
    LSMFileResult *r = extract_ts("declare module 'foo' { export function bar(): string; }\n"
...
TEST(tslsp_nocrash_typeof_in_type_position) {
    LSMFileResult *r = extract_ts("const x = { a: 1 };\n"
                                  "type X = typeof x;\n"
...
static void count_calls(const LSMFileResult *r, int *resolved, int *unresolved, int *total) {
...
TEST(tslsp_baseline_vs_lsp_simple) {
    ...
    LSMFileResult *base = extract_ts(source);
    int br = 0, bu = 0, bt = 0;
    count_calls(base, &br, &bu, &bt);
    lsm_free_result(base);

    lsm_unsetenv("LSM_LSP_DISABLED");
    LSMFileResult *lsp = extract_ts(source);
    int lr = 0, lu = 0, lt = 0;
...
TEST(tslsp_dom_document_getElementById) {
    LSMFileResult *r = extract_ts("function go(d: Document) { d.getElementById('x'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_dom_element_querySelector) {
    LSMFileResult *r = extract_ts("function go(e: Element) { e.querySelector('.foo'); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_dom_html_click) {
    LSMFileResult *r = extract_ts("function go(e: HTMLElement) { e.click(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_dom_event_preventDefault) {
    LSMFileResult *r = extract_ts("function go(e: Event) { e.preventDefault(); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_real_zustand_store) {
    LSMFileResult *r = extract_ts("interface Store { count: number; inc(): void; reset(): void; }\n"
...
TEST(tslsp_real_router_chain) {
    LSMFileResult *r = extract_ts("interface Router {\n"
...
TEST(tslsp_real_odm_model) {
    LSMFileResult *r = extract_ts("interface Model<T> {\n"
...
TEST(tslsp_class_factory_with_extends) {
    LSMFileResult *r = extract_ts("class Base { common(): void {} }\n"
...
TEST(tslsp_method_returning_self_chained) {
    LSMFileResult *r = extract_ts("class Chainable {\n"
                                  "    a(): this { return this; }\n"
                                  "    b(): this { return this; }\n"
...
TEST(tslsp_nocrash_top_level_await) {
    LSMFileResult *r = extract_ts("async function fetch1(): Promise<number> { return 1; }\n"
...
TEST(tslsp_overload_by_arg_types_class) {
    LSMFileResult *r = extract_ts("class Codec {\n"
...
TEST(tslsp_overload_function_by_arg_types) {
    LSMFileResult *r = extract_ts("function process(x: string): string { return x; }\n"
...
TEST(tslsp_real_react_useReducer) {
    LSMFileResult *r = extract_tsx("interface Action { type: string; }\n"
                                   "interface State { count: number; }\n"
...
TEST(tslsp_real_axios_client) {
    LSMFileResult *r = extract_ts("interface AxiosClient {\n"
...
TEST(tslsp_real_class_with_protected_methods) {
    LSMFileResult *r = extract_ts("class Base {\n"
...
TEST(tslsp_real_jest_mock) {
    LSMFileResult *r = extract_ts("interface MockFn<T> {\n"
...
TEST(tslsp_real_state_machine) {
    LSMFileResult *r = extract_ts("interface Service {\n"
...
TEST(tslsp_nocrash_optional_call) {
    LSMFileResult *r = extract_ts("function go(fn?: () => string) { fn?.(); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(tslsp_nocrash_intersection_class) {
    LSMFileResult *r = extract_ts("interface Loggable { log(): void; }\n"
...
TEST(tslsp_nocrash_branded_type) {
    LSMFileResult *r = extract_ts("type UserId = string & { readonly __brand: 'UserId' };\n"
...
TEST(tslsp_nocrash_tuple_with_rest) {
    LSMFileResult *r = extract_ts("type T = [string, ...number[]];\n"
                                  "function go(x: T) { console.log(x); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_nocrash_named_tuple) {
    LSMFileResult *r = extract_ts("type T = [first: string, second: number];\n"
                                  "function go(x: T) { console.log(x); }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_typeof_in_type_position) {
    LSMFileResult *r = extract_ts("const config = { port: 8080, host: 'localhost' };\n"
...
TEST(tslsp_hash_registry_basic) {
    ...
    ASSERT_NOT_NULL(lsm_registry_lookup_type(&reg, "test.A"));
    ASSERT_NOT_NULL(lsm_registry_lookup_func(&reg, "test.A.foo"));
    ASSERT_NOT_NULL(lsm_registry_lookup_method(&reg, "test.A", "foo"));
    ...
    ASSERT(lsm_registry_lookup_type(&reg, "test.B") == NULL);
    ASSERT(lsm_registry_lookup_method(&reg, "test.A", "bar") == NULL);

    lsm_arena_destroy(&arena);
    PASS();
}

/* Entries added AFTER lsm_registry_finalize must remain findable. The hashed
...
TEST(tslsp_hash_registry_post_finalize_adds) {
    ...
    const LSMType *zero_param_signature = lsm_type_func(&arena, NULL, NULL, NULL);
    LSMRegisteredFunc seed_method;
    ...
    lsm_registry_add_func(&reg, seed_method);

    LSMRegisteredFunc seed_free;
    ...
    f2.min_params = 0;
    lsm_registry_add_func(&reg, f2);

    LSMRegisteredFunc late_free;
    ...
    lsm_registry_add_func(&reg, late_free);

    ASSERT_NOT_NULL(lsm_registry_lookup_type(&reg, "test.A"));
    ASSERT_NOT_NULL(lsm_registry_lookup_type(&reg, "test.LateStub"));
    ASSERT_NOT_NULL(lsm_registry_lookup_func(&reg, "test.LateStub.run"));
    const LSMType *no_arg_types[1] = {NULL};
    bool method_args = lsm_registry_lookup_method_by_args(&reg, "test.LateStub", "run", 0) != NULL;
    bool method_types =
        lsm_registry_lookup_method_by_types(&reg, "test.LateStub", "run", no_arg_types, 0) != NULL;
    bool symbol_args = lsm_registry_lookup_symbol_by_args(&reg, "test", "lateFree", 0) != NULL;
    bool symbol_types =
        lsm_registry_lookup_symbol_by_types(&reg, "test", "lateFree", no_arg_types, 0) != NULL;
    ...
    ASSERT_NOT_NULL(lsm_registry_lookup_type(&reg, "test.LateStub"));
    ASSERT_NOT_NULL(lsm_registry_lookup_func(&reg, "test.LateStub.run"));
    ASSERT_NOT_NULL(lsm_registry_lookup_method_by_args(&reg, "test.LateStub", "run", 0));
    ASSERT_NOT_NULL(lsm_registry_lookup_symbol_by_args(&reg, "test", "lateFree", 0));
...
TEST(tslsp_hash_registry_stress_10k) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        LSMRegisteredType t;
        memset(&t, 0, sizeof(t));
        t.qualified_name = lsm_arena_strdup(&arena, buf);
        t.short_name = "X";
        lsm_registry_add_type(&reg, t);
        ...
        LSMRegisteredFunc f;
        memset(&f, 0, sizeof(f));
        f.qualified_name = lsm_arena_strdup(&arena, buf);
        f.short_name = "go";
        f.min_params = -1;
        char recv[64];
        snprintf(recv, sizeof(recv), "p.T%d", i);
        f.receiver_type = lsm_arena_strdup(&arena, recv);
        lsm_registry_add_func(&reg, f);
    }
    lsm_registry_finalize(&reg);
    ...
    for (int i = 0; i < N; i += 100) {
        snprintf(buf, sizeof(buf), "p.T%d", i);
        if (lsm_registry_lookup_type(&reg, buf))
            hits++;
        snprintf(buf, sizeof(buf), "p.T%d", i);
        if (lsm_registry_lookup_method(&reg, buf, "go"))
            hits++;
    }
...
TEST(tslsp_async_iter_for_await_of) {
    LSMFileResult *r = extract_ts("class Item { run(): void {} }\n"
...
TEST(tslsp_iter_iterator_next) {
    LSMFileResult *r = extract_ts("class Item { go(): void {} }\n"
...
TEST(tslsp_async_iterator_next_promise) {
    LSMFileResult *r = extract_ts("function go(it: AsyncIterator<string>) { it.next(); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, ".go", "next"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(tslsp_generator_next) {
    LSMFileResult *r = extract_ts("function* gen(): Generator<number> { yield 1; }\n"
...
TEST(tslsp_method_override) {
    LSMFileResult *r = extract_ts("class Base { run(): string { return 'base'; } }\n"
...
TEST(tslsp_type_only_import_safe) {
    LSMFileResult *r = extract_ts("import type { Foo } from './foo';\n"
...
TEST(tslsp_satisfies_method_chain) {
    LSMFileResult *r = extract_ts("interface Cfg { port: number; host: string; }\n"
                                  "function go() {\n"
...
TEST(tslsp_in_operator_narrow_smoke) {
    LSMFileResult *r = extract_ts("interface A { kind: 'a'; aField: number; }\n"
...
TEST(tslsp_optional_property_chain) {
    LSMFileResult *r = extract_ts("interface User { profile?: { name(): string } }\n"
...
TEST(tslsp_void_returning_method) {
    LSMFileResult *r = extract_ts("class Logger { log(msg: string): void {} }\n"
...
TEST(tslsp_nested_class_resolution) {
    LSMFileResult *r = extract_ts("class Outer {\n"
...
TEST(tslsp_eval_memo_bounds_spread_bomb_work) {
    ...
    LSMFileResult *r = extract_js(src);
    ASSERT_NOT_NULL(r);
    ...
    ASSERT_FALSE(lsm_ts_lsp_test_budget_warned());
    long budget0 = 1000000 + (long)strlen(src) * 64;
    long consumed = budget0 - lsm_ts_lsp_test_budget_remaining();
...
TEST(tslsp_js_ordinary_same_leaf_calls_join_by_exact_site) {
    static const char source[] = "class Alpha { render() {} }\n"
                                 ...
                                 "    b.render();\n"
                                 "}\n";
    LSMFileResult *r = extract_js(source);
    ASSERT_NOT_NULL(r);
...
TEST(tslsp_ts_ordinary_same_leaf_calls_join_by_exact_site) {
    static const char source[] = "class Alpha { render(): void {} }\n"
                                 ...
                                 "    b.render();\n"
                                 "}\n";
    LSMFileResult *r = extract_ts(source);
    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error || r->parse_incomplete);
    const bool exact = ts_same_leaf_render_occurrences_are_exact(r, source);
    lsm_free_result(r);
...
SUITE(ts_lsp) {
    ...
    /* LSP vs baseline comparison (requires LSM_LSP_DISABLED knob in resolver) */
