 ...
 *
 * Exercises lsm_ac_build(), lsm_ac_scan_bitmask(), lsm_ac_scan_batch()
...
TEST(ac_build_single) {
    ...
    int lengths[] = {5};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 1, NULL, 256);
    ASSERT_NOT_NULL(ac);
    ASSERT_EQ(lsm_ac_num_patterns(ac), 1);
    ASSERT_GT(lsm_ac_num_states(ac), 0);
    lsm_ac_free(ac);
    PASS();
}
...
TEST(ac_build_multiple) {
    ...
    int lengths[] = {2, 3, 3, 4};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 4, NULL, 256);
    ASSERT_NOT_NULL(ac);
    ASSERT_EQ(lsm_ac_num_patterns(ac), 4);
    lsm_ac_free(ac);
    PASS();
}
...
TEST(ac_scan_single_match) {
    ...
    uint64_t result = lsm_ac_scan_bitmask(ac, "say hello world", 15);
...
TEST(ac_scan_no_match) {
    ...
    uint64_t result = lsm_ac_scan_bitmask(ac, "hello world", 11);
    ASSERT_EQ(result, 0);
    lsm_ac_free(ac);
    PASS();
}
...
TEST(ac_scan_multiple_matches) {
    ...
    uint64_t result = lsm_ac_scan_bitmask(ac, "ushers", 6);
...
TEST(ac_scan_overlapping) {
    ...
    int lengths[] = {2, 3, 4};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 3, NULL, 256);

    uint64_t result = lsm_ac_scan_bitmask(ac, "xabcdy", 6);
...
TEST(ac_scan_empty_text) {
    ...
    uint64_t result = lsm_ac_scan_bitmask(ac, "", 0);
    ASSERT_EQ(result, 0);
    lsm_ac_free(ac);
    PASS();
}
...
TEST(ac_custom_alphabet) {
    ...
    const char *patterns[] = {"hello"};
    int lengths[] = {5};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 1, alpha_map, alpha_size);
    ASSERT_NOT_NULL(ac);
    ...
    int full_bytes = lsm_ac_num_states(ac) * 256 * 4;
    int compact_bytes = lsm_ac_table_bytes(ac);
...
TEST(ac_batch_scan) {
    ...
    LSMMatchResult matches[16];
    int n = lsm_ac_scan_batch(ac, names_buf, offsets, lens, 4, matches, 16);
...
TEST(ac_table_bytes) {
    ...
    int bytes = lsm_ac_table_bytes(ac);
    ASSERT_GT(bytes, 0);
    /* states * 256 * sizeof(int) */
    ASSERT_EQ(bytes, lsm_ac_num_states(ac) * 256 * 4);
...
TEST(ac_null_input) {
    LSMAutomaton *ac = lsm_ac_build(NULL, NULL, 0, NULL, 256);
    ASSERT_NULL(ac);
    lsm_ac_free(NULL); /* should not crash */
    PASS();
}
...
TEST(ac_scan_string) {
    ...
    int lengths[] = {3, 3};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 2, NULL, 256);
    ASSERT_NOT_NULL(ac);
    uint64_t mask = lsm_ac_scan_bitmask(ac, "foobar", 6);
...
TEST(ac_free_double_call) {
    ...
    /* ac is now freed — calling free again should not crash.
       Note: in C the pointer is dangling, but lsm_ac_free checks for NULL
...
extern int lsm_lz4_compress_hc(const char *src, int srcLen, char *dst, int dstCap);
extern int lsm_lz4_bound(int inputSize);
...
extern uint64_t lsm_ac_scan_lz4_bitmask(const LSMAutomaton *ac, const char *compressed,
                                        int compressed_len, int original_len);
extern int lsm_ac_scan_lz4_batch(const LSMAutomaton *ac, const LSMLz4Entry *entries,
                                 int num_entries, LSMLz4Match *out_matches, int max_matches);
...
TEST(ac_scan_lz4_bitmask) {
    ...
    int bound = lsm_lz4_bound(src_len);
    char *compressed = malloc(bound);
    ASSERT_NOT_NULL(compressed);
    int comp_len = lsm_lz4_compress_hc(source, src_len, compressed, bound);
    ASSERT_GT(comp_len, 0);

    uint64_t mask = lsm_ac_scan_lz4_bitmask(ac, compressed, comp_len, src_len);
    ASSERT_EQ(mask, 1ULL); /* http.Get matched */

    /* No match */
    const char *no_http = "package main\nfunc main() { println(42) }\n";
    int nh_len = (int)strlen(no_http);
    int bound2 = lsm_lz4_bound(nh_len);
    char *comp2 = malloc(bound2);
    int comp2_len = lsm_lz4_compress_hc(no_http, nh_len, comp2, bound2);
    mask = lsm_ac_scan_lz4_bitmask(ac, comp2, comp2_len, nh_len);
    ASSERT_EQ(mask, 0ULL);
...
TEST(ac_scan_lz4_batch) {
    ...
    LSMLz4Entry entries[3];
    char *comp_bufs[3];
    for (int i = 0; i < 3; i++) {
        int slen = (int)strlen(files[i]);
        int bound = lsm_lz4_bound(slen);
        comp_bufs[i] = malloc(bound);
        int clen = lsm_lz4_compress_hc(files[i], slen, comp_bufs[i], bound);
        entries[i].data = comp_bufs[i];
        entries[i].compressed_len = clen;
        entries[i].original_len = slen;
    }

    LSMLz4Match matches[8];
    int n = lsm_ac_scan_lz4_batch(ac, entries, 3, matches, 8);
...
TEST(ac_scan_lz4_batch_empty) {
    ...
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 1, NULL, 256);
    ASSERT_NOT_NULL(ac);
    LSMLz4Match matches[1];
    int n = lsm_ac_scan_lz4_batch(ac, NULL, 0, matches, 1);
    ASSERT_EQ(n, 0);
    lsm_ac_free(ac);
    PASS();
}
...
TEST(ac_large_pattern_set) {
    ...
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, count, NULL, 256);
    ASSERT_NOT_NULL(ac);
    ASSERT_GT(lsm_ac_num_states(ac), 0);
    ...
    uint64_t mask = lsm_ac_scan_bitmask(ac, source, (int)strlen(source));
    ...
    /* Non-matching source */
    mask = lsm_ac_scan_bitmask(ac, "func main() { fmt.Println(42) }", 31);
    ASSERT_EQ(mask, 0ULL);
...
TEST(ac_http_patterns) {
    ...
    for (int i = 0; i < 7; i++) {
        uint64_t mask = lsm_ac_scan_bitmask(ac, tests[i].input, (int)strlen(tests[i].input));
...
TEST(ac_compact_alphabet_extended) {
    ...
    int lengths[] = {12, 7, 4};
    LSMAutomaton *ac = lsm_ac_build(patterns, lengths, 3, alpha_map, alpha_size);
    ASSERT_NOT_NULL(ac);

    /* Exact match */
    uint64_t mask = lsm_ac_scan_bitmask(ac, "database_url", 12);
    ASSERT(mask & 1ULL);

    /* Substring match */
    mask = lsm_ac_scan_bitmask(ac, "my_database_url_setting", 23);
    ASSERT(mask & 1ULL);

    /* Verify table size = states * alpha_size * sizeof(int) */
    int table_bytes = lsm_ac_table_bytes(ac);
    ASSERT_EQ(table_bytes, lsm_ac_num_states(ac) * alpha_size * 4);
...
TEST(ac_batch_scan_detailed) {
    ...
    LSMMatchResult matches[16];
    int n = lsm_ac_scan_batch(ac, names_buf, offsets, lens, 5, matches, 16);
