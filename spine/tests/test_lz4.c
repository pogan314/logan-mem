 ...
 *
 * Ports from internal/lsm/lz4_test.go:
...
extern int lsm_lz4_decompress(const char *src, int srcLen, char *dst, int originalLen);
...
static int lz4_roundtrip(const char *data, int len) {
    ...
    int clen = lsm_lz4_compress_hc(data, len, cbuf, bound);
    if (clen <= 0) {
        free(cbuf);
        return -1;
    }
    ...
    int dlen = lsm_lz4_decompress(cbuf, clen, dbuf, len);
...
TEST(lz4_compression_ratio) {
    ...
    int bound = lsm_lz4_bound(total);
    char *cbuf = malloc(bound);
    ASSERT_NOT_NULL(cbuf);

    int clen = lsm_lz4_compress_hc(data, total, cbuf, bound);
    ASSERT_GT(clen, 0);
...
TEST(lz4_decompress_wrong_len) {
    ...
    int clen = lsm_lz4_compress_hc(src, src_len, cbuf, bound);
    ASSERT_GT(clen, 0);
    ...
    int dlen = lsm_lz4_decompress(cbuf, clen, dbuf, wrong_len);
...
TEST(lz4_bound_positive) {
    ...
    ASSERT_GT(lsm_lz4_bound(1), 0);
    ASSERT_GT(lsm_lz4_bound(100), 0);
    ASSERT_GT(lsm_lz4_bound(1000000), 0);
    /* Bound should be >= input size (compressed can't be smaller than worst case) */
    ASSERT_GTE(lsm_lz4_bound(100), 100);
    PASS();
}
