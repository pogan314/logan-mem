...
extern int lsm_zstd_compress(const char *src, int srcLen, char *dst, int dstCap, int level);
extern int lsm_zstd_decompress(const char *src, int srcLen, char *dst, int dstCap);
extern size_t lsm_zstd_compress_bound(int inputSize);

TEST(zstd_roundtrip) {
    const char *data = "Hello, zstd compression roundtrip test!";
    int len = (int)strlen(data);

    size_t bound = lsm_zstd_compress_bound(len);
    ASSERT_GT((int)bound, 0);

    char *cbuf = malloc(bound);
    ASSERT_NOT_NULL(cbuf);

    int clen = lsm_zstd_compress(data, len, cbuf, (int)bound, 3);
    ASSERT_GT(clen, 0);

    char *dbuf = malloc(len);
    ASSERT_NOT_NULL(dbuf);

    int dlen = lsm_zstd_decompress(cbuf, clen, dbuf, len);
    ASSERT_EQ(dlen, len);
...
TEST(zstd_roundtrip_large) {
    ...
    int clen = lsm_zstd_compress(data, len, cbuf, (int)bound, 9);
    ASSERT_GT(clen, 0);
...
TEST(zstd_compress_levels) {
    ...
    int clen3 = lsm_zstd_compress(data, len, cbuf, (int)bound, 3);
    ASSERT_GT(clen3, 0);

    int clen9 = lsm_zstd_compress(data, len, cbuf, (int)bound, 9);
    ASSERT_GT(clen9, 0);
...
TEST(zstd_decompress_too_small_output) {
    ...
    char small[4];
    int dlen = lsm_zstd_decompress(cbuf, clen, small, 4);
    ASSERT_EQ(dlen, 0);
...
TEST(zstd_bound_positive) {
    ASSERT_GT((int)lsm_zstd_compress_bound(1), 0);
    ASSERT_GT((int)lsm_zstd_compress_bound(100), 0);
    ASSERT_GT((int)lsm_zstd_compress_bound(1000000), 0);
    PASS();
}
