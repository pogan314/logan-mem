...
TEST(vmem_budget_tracking) {
    size_t before = lsm_vmem_allocated();
    size_t sz = 256 * 1024;
    void *p = lsm_vmem_alloc(sz);
    if (!p) {
        PASS();
    }
    size_t after = lsm_vmem_allocated();
    /* allocated should have increased (may be rounded to page) */
    ASSERT_GT(after, before);

    lsm_vmem_free(p, sz);
    size_t freed = lsm_vmem_allocated();
...
TEST(vmem_peak_tracks) {
    size_t sz = 512 * 1024;
    void *p1 = lsm_vmem_alloc(sz);
    if (!p1) {
        PASS();
    }
    size_t peak1 = lsm_vmem_peak();
    ASSERT_GT(peak1, 0);

    void *p2 = lsm_vmem_alloc(sz);
    if (!p2) {
        lsm_vmem_free(p1, sz);
        PASS();
    }
    size_t peak2 = lsm_vmem_peak();
...
TEST(vmem_worker_budget) {
    ...
    if (budget == 0) {
        ...
        ASSERT_EQ(lsm_vmem_worker_budget(4), 0);
        PASS();
    }
    /* Budget divides correctly */
    size_t wb4 = lsm_vmem_worker_budget(4);
    size_t wb8 = lsm_vmem_worker_budget(8);
    ASSERT_EQ(wb4, budget / 4);
    ASSERT_EQ(wb8, budget / 8);
    /* Edge case: 0 workers */
    ASSERT_EQ(lsm_vmem_worker_budget(0), 0);
    PASS();
}
...
TEST(vmem_worker_budget_negative_workers) {
    ...
    size_t wb = lsm_vmem_worker_budget(-3);
    ASSERT_EQ(wb, budget);
    PASS();
}
...
TEST(vmem_over_budget_when_nothing_allocated) {
    ...
    bool over = lsm_vmem_over_budget();
    ASSERT_FALSE(over);
    PASS();
}
...
TEST(vmem_allocated_tracks_alloc_free_cycle) {
    lsm_vmem_init(0.5);
    size_t base = lsm_vmem_allocated();

    size_t sz = 128 * 1024; /* 128 KB */
    void *p = lsm_vmem_alloc(sz);
    ASSERT_NOT_NULL(p);

    size_t after_alloc = lsm_vmem_allocated();
    ASSERT_GT(after_alloc, base);

    lsm_vmem_free(p, sz);

    size_t after_free = lsm_vmem_allocated();
...
TEST(vmem_multiple_alloc_tracks_cumulative) {
    ...
    void *p1 = lsm_vmem_alloc(sz);
    ASSERT_NOT_NULL(p1);
    size_t after_one = lsm_vmem_allocated();

    void *p2 = lsm_vmem_alloc(sz);
    ASSERT_NOT_NULL(p2);
    size_t after_two = lsm_vmem_allocated();
...
TEST(vmem_peak_never_decreases) {
    ...
    void *p = lsm_vmem_alloc(sz);
    ASSERT_NOT_NULL(p);
    size_t peak_with_alloc = lsm_vmem_peak();

    lsm_vmem_free(p, sz);
    size_t peak_after_free = lsm_vmem_peak();
...
TEST(vmem_worker_budget_one_worker) {
    ...
    size_t wb = lsm_vmem_worker_budget(1);
    ASSERT_EQ(wb, budget);
    PASS();
}
...
TEST(vmem_worker_budget_many_workers) {
    ...
    size_t wb = lsm_vmem_worker_budget(1000);
    ASSERT_GT(wb, 0);
...
TEST(arena_vmem_alloc_and_destroy) {
    ...
    /* Allocate some data */
    char *s = lsm_arena_strdup(&a, "hello vmem integration");
    ASSERT_NOT_NULL(s);
...
TEST(tier2_alloc_and_free_128) {
    ...
    void *p = lsm_slab_test_malloc(100);
    ASSERT_NOT_NULL(p);
    ...
    void *p2 = lsm_slab_test_malloc(100);
    ASSERT_NOT_NULL(p2);
...
TEST(tier2_free_list_reuse) {
    ...
    void *addrs[10];
    for (int i = 0; i < 10; i++) {
        addrs[i] = lsm_slab_test_malloc(200);
        ASSERT_NOT_NULL(addrs[i]);
    }
    for (int i = 0; i < 10; i++) {
        lsm_slab_test_free(addrs[i]);
    }
...
TEST(tier2_oversized_dedicated) {
    ...
    size_t before_alloc = lsm_vmem_allocated();

    void *big = lsm_slab_test_malloc(8192);
    ASSERT_NOT_NULL(big);
...
TEST(tier2_realloc_same_class) {
    ...
    void *p2 = lsm_slab_test_realloc(p, 110);
    ASSERT_NOT_NULL(p2);
    ...
    void *p3 = lsm_slab_test_realloc(p2, 70);
    ASSERT_NOT_NULL(p3);
    ASSERT_EQ(p2, p3); /* same pointer */

    lsm_slab_test_free(p3);
    lsm_slab_destroy_thread();
    PASS();
}
...
TEST(tier2_realloc_grows_class) {
    ...
    void *p2 = lsm_slab_test_realloc(p, 300);
    ASSERT_NOT_NULL(p2);
...
TEST(tier2_realloc_slab_to_tier2) {
    ...
    void *p = lsm_slab_test_malloc(32); /* Tier 1 slab */
    ASSERT_NOT_NULL(p);
    memset(p, 0x42, 32);

    /* Promote to Tier 2 */
    void *p2 = lsm_slab_test_realloc(p, 200);
    ASSERT_NOT_NULL(p2);
...
TEST(tier2_calloc_zeroed) {
    ...
    lsm_slab_test_free(p);
    void *p2 = lsm_slab_test_calloc(1, 200);
    ASSERT_NOT_NULL(p2);
...
static int setup_vmem_test_repo(void) {
    snprintf(g_vmem_tmpdir, sizeof(g_vmem_tmpdir), "/tmp/lsm_vmem_XXXXXX");
...
TEST(vmem_parallel_extract_with_slab) {
    ...
    int file_count = 0;
    if (lsm_discover(g_vmem_tmpdir, &opts, &files, &file_count) != 0) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new("vmem-test", g_vmem_tmpdir);
