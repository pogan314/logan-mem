...
TEST(path_join_basic) {
    setup();
    ASSERT_STR_EQ(lsm_path_join(&a, "src", "main.c"), "src/main.c");
    teardown();
    PASS();
}
...
TEST(path_join_empty_base) {
    setup();
    ASSERT_STR_EQ(lsm_path_join(&a, "", "main.c"), "main.c");
    teardown();
    PASS();
}
...
TEST(path_join_empty_name) {
    setup();
    ASSERT_STR_EQ(lsm_path_join(&a, "src", ""), "src");
    teardown();
    PASS();
}
...
TEST(path_join_n) {
    ...
    ASSERT_STR_EQ(lsm_path_join_n(&a, parts, 4), "a/b/c/d.txt");
    teardown();
    PASS();
}

TEST(path_ext) {
    ASSERT_STR_EQ(lsm_path_ext("foo.go"), "go");
    ASSERT_STR_EQ(lsm_path_ext("foo.tar.gz"), "gz");
    ASSERT_STR_EQ(lsm_path_ext("Makefile"), "");
    ASSERT_STR_EQ(lsm_path_ext(".gitignore"), "gitignore");
    PASS();
}

TEST(path_base) {
    ASSERT_STR_EQ(lsm_path_base("src/main.c"), "main.c");
    ASSERT_STR_EQ(lsm_path_base("main.c"), "main.c");
    ASSERT_STR_EQ(lsm_path_base("/a/b/c"), "c");
    PASS();
}

TEST(path_dir) {
    setup();
    ASSERT_STR_EQ(lsm_path_dir(&a, "src/main.c"), "src");
    ASSERT_STR_EQ(lsm_path_dir(&a, "main.c"), ".");
    ASSERT_STR_EQ(lsm_path_dir(&a, "/a/b/c"), "/a/b");
    teardown();
    PASS();
}

TEST(str_starts_with) {
    ASSERT_TRUE(lsm_str_starts_with("hello world", "hello"));
    ASSERT_FALSE(lsm_str_starts_with("hello", "hello world"));
    ASSERT_TRUE(lsm_str_starts_with("hello", ""));
    ASSERT_TRUE(lsm_str_starts_with("hello", "hello"));
    PASS();
}

TEST(str_ends_with) {
    ASSERT_TRUE(lsm_str_ends_with("hello world", "world"));
    ASSERT_FALSE(lsm_str_ends_with("hello", "hello world"));
    ASSERT_TRUE(lsm_str_ends_with("hello", ""));
    ASSERT_TRUE(lsm_str_ends_with("hello", "hello"));
    PASS();
}

TEST(str_contains) {
    ASSERT_TRUE(lsm_str_contains("hello world", "lo wo"));
    ASSERT_FALSE(lsm_str_contains("hello", "xyz"));
    ASSERT_TRUE(lsm_str_contains("hello", ""));
    PASS();
}

TEST(str_tolower) {
    setup();
    ASSERT_STR_EQ(lsm_str_tolower(&a, "Hello World"), "hello world");
    ASSERT_STR_EQ(lsm_str_tolower(&a, "already"), "already");
    ASSERT_STR_EQ(lsm_str_tolower(&a, ""), "");
    teardown();
    PASS();
}
...
TEST(str_replace_char) {
    setup();
    ASSERT_STR_EQ(lsm_str_replace_char(&a, "a/b/c", '/', '.'), "a.b.c");
    ASSERT_STR_EQ(lsm_str_replace_char(&a, "no-change", '/', '.'), "no-change");
    teardown();
    PASS();
}
...
TEST(str_strip_ext) {
    setup();
    ASSERT_STR_EQ(lsm_str_strip_ext(&a, "foo.go"), "foo");
    ASSERT_STR_EQ(lsm_str_strip_ext(&a, "foo.tar.gz"), "foo.tar");
    ASSERT_STR_EQ(lsm_str_strip_ext(&a, "Makefile"), "Makefile");
    teardown();
    PASS();
}
...
TEST(str_split) {
    setup();
    int count = 0;
    char **parts = lsm_str_split(&a, "a/b/c/d", '/', &count);
    ASSERT_EQ(count, 4);
...
TEST(str_split_empty) {
    setup();
    int count = 0;
    char **parts = lsm_str_split(&a, "", '/', &count);
    ASSERT_EQ(count, 1);
...
TEST(str_split_no_delim) {
    setup();
    int count = 0;
    char **parts = lsm_str_split(&a, "hello", '/', &count);
    ASSERT_EQ(count, 1);
...
TEST(path_join_null_base) {
    setup();
    ASSERT_NULL(lsm_path_join(&a, NULL, "main.c"));
    teardown();
    PASS();
}
...
TEST(path_join_null_name) {
    setup();
    ASSERT_NULL(lsm_path_join(&a, "src", NULL));
    teardown();
    PASS();
}
...
TEST(path_join_both_null) {
    setup();
    ASSERT_NULL(lsm_path_join(&a, NULL, NULL));
    teardown();
    PASS();
}
...
TEST(path_dir_null) {
    setup();
    ASSERT_STR_EQ(lsm_path_dir(&a, NULL), ".");
    teardown();
    PASS();
}

TEST(str_starts_with_null_s) {
    ASSERT_FALSE(lsm_str_starts_with(NULL, "hello"));
    PASS();
}

TEST(str_starts_with_null_prefix) {
    ASSERT_FALSE(lsm_str_starts_with("hello", NULL));
    PASS();
}

TEST(str_ends_with_null) {
    ASSERT_FALSE(lsm_str_ends_with(NULL, "world"));
    ASSERT_FALSE(lsm_str_ends_with("world", NULL));
    PASS();
}

TEST(str_contains_null) {
    ASSERT_FALSE(lsm_str_contains(NULL, "sub"));
    ASSERT_FALSE(lsm_str_contains("hello", NULL));
    PASS();
}

TEST(str_tolower_null) {
    setup();
    ASSERT_NULL(lsm_str_tolower(&a, NULL));
    teardown();
    PASS();
}
...
TEST(str_replace_char_null) {
    setup();
    ASSERT_NULL(lsm_str_replace_char(&a, NULL, 'a', 'b'));
    teardown();
    PASS();
}
...
TEST(str_strip_ext_null) {
    setup();
    ASSERT_NULL(lsm_str_strip_ext(&a, NULL));
    teardown();
    PASS();
}
...
TEST(str_split_null_string) {
    setup();
    int count = 0;
    ASSERT_NULL(lsm_str_split(&a, NULL, '/', &count));
    teardown();
    PASS();
}
...
TEST(str_split_null_out_count) {
    setup();
    ASSERT_NULL(lsm_str_split(&a, "a/b", '/', NULL));
    teardown();
    PASS();
}
...
TEST(path_join_long_paths) {
    ...
    long_name[511] = '\0';
    char *result = lsm_path_join(&a, long_base, long_name);
    ASSERT_NOT_NULL(result);
    ASSERT_TRUE(lsm_str_starts_with(result, "aaa"));
    ASSERT_TRUE(lsm_str_ends_with(result, "bbb"));
    ASSERT_TRUE(lsm_str_contains(result, "/"));
...
TEST(path_join_n_zero) {
    setup();
    ASSERT_STR_EQ(lsm_path_join_n(&a, NULL, 0), "");
    teardown();
    PASS();
}
...
TEST(path_join_n_null_parts) {
    setup();
    ASSERT_STR_EQ(lsm_path_join_n(&a, NULL, 3), "");
    teardown();
    PASS();
}
...
TEST(path_ext_null) {
    ASSERT_STR_EQ(lsm_path_ext(NULL), "");
    PASS();
}

TEST(path_ext_dot_directory) {
    ASSERT_STR_EQ(lsm_path_ext("dir.name/file"), "");
    PASS();
}
...
TEST(path_ext_multiple_dots) {
    ASSERT_STR_EQ(lsm_path_ext("file.test.spec.ts"), "ts");
    PASS();
}

TEST(path_ext_trailing_dot) {
    ASSERT_STR_EQ(lsm_path_ext("file."), "");
    PASS();
}
...
TEST(path_base_null) {
    ASSERT_STR_EQ(lsm_path_base(NULL), "");
    PASS();
}

TEST(path_base_empty) {
    ASSERT_STR_EQ(lsm_path_base(""), "");
    PASS();
}

TEST(path_base_just_filename) {
    ASSERT_STR_EQ(lsm_path_base("hello.txt"), "hello.txt");
    PASS();
}

TEST(path_base_trailing_slash) {
    ASSERT_STR_EQ(lsm_path_base("dir/"), "");
    PASS();
}
...
TEST(validate_shell_arg_safe) {
    ASSERT_TRUE(lsm_validate_shell_arg("hello-world_123"));
    PASS();
}

TEST(validate_shell_arg_single_quote) {
    ASSERT_FALSE(lsm_validate_shell_arg("it's bad"));
    PASS();
}

TEST(validate_shell_arg_semicolon) {
    ASSERT_FALSE(lsm_validate_shell_arg("cmd; rm -rf"));
    PASS();
}

TEST(validate_shell_arg_pipe) {
    ASSERT_FALSE(lsm_validate_shell_arg("cmd | evil"));
    PASS();
}

TEST(validate_shell_arg_ampersand) {
    ASSERT_FALSE(lsm_validate_shell_arg("cmd & bg"));
    PASS();
}

TEST(validate_shell_arg_dollar) {
    ASSERT_FALSE(lsm_validate_shell_arg("$HOME"));
    PASS();
}
...
TEST(validate_shell_arg_newline) {
    ASSERT_FALSE(lsm_validate_shell_arg("line1\nline2"));
    PASS();
}

TEST(validate_shell_arg_carriage_return) {
    ASSERT_FALSE(lsm_validate_shell_arg("line1\rline2"));
    PASS();
}
...
TEST(validate_shell_arg_backslash) {
    ...
    ASSERT_TRUE(lsm_validate_shell_arg("path\\to\\file"));
#else
    ASSERT_FALSE(lsm_validate_shell_arg("path\\to\\file"));
#endif
    PASS();
}
...
TEST(validate_shell_arg_spaces) {
    ASSERT_TRUE(lsm_validate_shell_arg("hello world with spaces"));
    PASS();
}
...
TEST(json_escape_control_chars) {
    ...
    const char *input = "A\x01"
                        "B\n";
    int len = lsm_json_escape(buf, sizeof(buf), input);
...
TEST(snprintf_append_basic) {
    char buf[64];
    int off = 0;
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, "hello");
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, " world");
...
TEST(snprintf_append_overflow) {
    char buf[8];
    int off = 0;
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, "hello world this is way too long");
...
TEST(snprintf_append_multiple_sequential) {
    char buf[32];
    int off = 0;
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, "a=%d", 1);
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, " b=%d", 2);
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, " c=%d", 3);
    LSM_SNPRINTF_APPEND(buf, sizeof(buf), off, " d=%d", 4);
