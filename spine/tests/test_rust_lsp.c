 ...
 *   - Cross-file: lsm_run_rust_lsp_cross resolves project + stdlib defs
 *
...
static LSMFileResult *extract_rust(const char *source) {
    return lsm_extract_file(source, (int)strlen(source), LSM_LANG_RUST,
                            "test", "src/main.rs", 0, NULL, NULL);
}

static LSMTypeRegistry *rustlsp_return_shared_registry(void *ctx) {
    return (LSMTypeRegistry *)ctx;
}

static int find_resolved(const LSMFileResult *r, const char *callerSub,
...
static int require_resolved(const LSMFileResult *r, const char *callerSub,
                            const char *calleeSub) {
...
static int count_resolved(const LSMFileResult *r, const char *callerSub,
                          const char *calleeSub) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const LSMResolvedCall *rc = &r->resolved_calls.items[i];
...
static bool rustlsp_definition_has_decorator(const LSMFileResult *result,
...
static int rustlsp_count_proc_macro_pseudo_calls(const LSMFileResult *result) {
    int count = 0;
    if (!result)
        return 0;
...
static int find_confident(const LSMResolvedCallArray *arr, const char *callerSub,
                          const char *calleeSub) {
...
static int count_confident_strategy(const LSMResolvedCallArray *arr, const char *strategy) {
    int count = 0;
...
TEST(rustlsp_crossfile_method_dispatch) {
    ...
    lsm_run_rust_lsp_cross(&a, caller, (int)strlen(caller),
                           "test.caller",
                           defs, 2,
                           imp_names, imp_qns, 1,
                           NULL, &out);
...
/* F4 Tier-2: the build-once shared registry path (lsm_rust_build_cross_registry +
 * lsm_run_rust_lsp_cross_with_registry) must resolve IDENTICALLY to the per-file
 * lsm_run_rust_lsp_cross — same fixture as rustlsp_crossfile_method_dispatch. Also
...
TEST(rustlsp_shared_registry_resolves_like_per_file) {
    ...
    LSMTypeRegistry *reg = lsm_rust_build_cross_registry(&a, defs, 2);
    ASSERT_NOT_NULL(reg);
    ASSERT_TRUE(reg->read_only);

    LSMResolvedCallArray out;
    memset(&out, 0, sizeof(out));
    lsm_run_rust_lsp_cross_with_registry(&a, caller, (int)strlen(caller), "test.caller", reg,
...
static LSMTypeRegistry *rustlsp_default_trait_registry(LSMArena *arena, bool add_ambiguous_type) {
    LSMLSPDef defs[5];
    memset(defs, 0, sizeof(defs));
    ...
    defs[3].lang = LSM_LANG_RUST;
    defs[4].qualified_name = "test.main.contract.S";
    defs[4].short_name = "S";
    defs[4].label = "Struct";
    defs[4].def_module_qn = "test.main.contract";
    defs[4].lang = LSM_LANG_RUST;
    return lsm_rust_build_cross_registry(arena, defs, add_ambiguous_type ? 5 : 4);
}
...
TEST(rustlsp_relative_type_requires_declared_module) {
    ...
    LSMTypeRegistry *registry = rustlsp_default_trait_registry(&arena, false);
    ASSERT_NOT_NULL(registry);
    LSMResolvedCallArray out = {0};
    lsm_run_rust_lsp_cross_with_registry(&arena, caller, (int)strlen(caller), "test.main", registry,
...
TEST(rustlsp_relative_type_ambiguous_graph_paths_fail_closed) {
    ...
    LSMArena arena;
    lsm_arena_init(&arena);
    LSMTypeRegistry *registry = rustlsp_default_trait_registry(&arena, true);
    ASSERT_NOT_NULL(registry);
    LSMResolvedCallArray out = {0};
    lsm_run_rust_lsp_cross_with_registry(&arena, caller, (int)strlen(caller), "test.main", registry,
                                         NULL, NULL, 0, NULL, NULL, &out, NULL);
 ...
 * pass_calls iterates LSMCall entries, so the cross resolver must also return a
 * matching synthetic carrier to the owning LSMFileResult. */
TEST(rustlsp_shared_registry_macro_hidden_call_has_carrier) {
    ...
    LSMTypeRegistry *reg = lsm_rust_build_cross_registry(&result.arena, defs, 1);
    ASSERT_NOT_NULL(reg);

    lsm_run_rust_lsp_cross_with_registry(&result.arena, caller, (int)strlen(caller), "test.main",
    ...
    const LSMResolvedCall *joined =
        carrier ? lsm_pipeline_find_lsp_resolution(&result.resolved_calls, carrier, false) : NULL;
    bool joined_target =
    ...
    /* RED: the corresponding exact LSMCall carrier is currently never returned. */
...
TEST(rustlsp_shared_dispatch_merges_existing_exact_occurrence) {
    ...
    LSMFileResult *result = extract_rust(source);
    ...
    LSMArena shared_arena;
    lsm_arena_init(&shared_arena);
    LSMTypeRegistry *shared = lsm_rust_build_cross_registry(&shared_arena, defs, 2);
    ASSERT_NOT_NULL(shared);
    const char *imp_names[] = {"lib"};
    const char *imp_qns[] = {"test::lib"};

    lsm_pxc_dispatch_file(LSM_LANG_RUST, result, source, (int)strlen(source), "src/main.rs",
    ...
    const LSMCall *render_carrier = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const LSMResolvedCall *rc = &result->resolved_calls.items[i];
    ...
    const LSMResolvedCall *joined =
        render_carrier
            ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, render_carrier, false)
            : NULL;
    ...
    lsm_arena_destroy(&shared_arena);
 ...
 * strings after lsm_pxc_run_one returns, making ownership bugs visible under
...
TEST(rustlsp_scratch_cross_macro_carrier_survives_copy) {
    ...
    const char *imp_names[] = {"lib"};
    const char *imp_qns[] = {"test::lib"};
    lsm_pxc_run_one(LSM_LANG_RUST, &result, caller, (int)strlen(caller), "test.main", defs, 1,
                    imp_names, imp_qns, 1);
...
TEST(rustlsp_macro_same_leaf_carriers_are_occurrence_exact) {
    ...
    LSMTypeRegistry *reg = lsm_rust_build_cross_registry(&result.arena, defs, 2);
    ASSERT_NOT_NULL(reg);
    lsm_run_rust_lsp_cross_with_registry(&result.arena, caller, (int)strlen(caller), "test.main",
    ...
    const LSMResolvedCall *a_hit =
        a_call ? lsm_pipeline_find_lsp_resolution(&result.resolved_calls, a_call, false) : NULL;
    const LSMResolvedCall *b_hit =
        b_call ? lsm_pipeline_find_lsp_resolution(&result.resolved_calls, b_call, false) : NULL;
...
TEST(rustlsp_operator_carriers_are_occurrence_exact) {
    ...
    const LSMResolvedCall *alpha_hit =
        alpha_call ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, alpha_call, false)
                   : NULL;
    const LSMResolvedCall *beta_hit =
        beta_call ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, beta_call, false)
                  : NULL;
...
TEST(rustlsp_known_std_macro_carriers_join_exact_sites) {
    ...
    const LSMResolvedCall *first_join =
        first_call ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, first_call, false)
                   : NULL;
    const LSMResolvedCall *second_join =
        second_call ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, second_call, false)
                    : NULL;
...
TEST(rustlsp_local_macro_shadows_known_std_macro) {
    LSMFileResult *result = extract_rust("macro_rules! println { () => { local_target() } }\n"
                                         "fn local_target() {}\n"
                                         "fn run() { println!(); }\n");
    ASSERT_NOT_NULL(result);
...
TEST(rustlsp_later_macro_does_not_shadow_earlier_std_macro) {
    LSMFileResult *result = extract_rust("fn run() { println!(); }\n"
...
TEST(rustlsp_nested_macro_scope_does_not_leak) {
    LSMFileResult *result = extract_rust("mod nested {\n"
 ...
 * record even though tree-sitter produced two distinct LSMCall sites. */
TEST(rustlsp_ordinary_same_leaf_calls_join_by_exact_site) {
    LSMFileResult *result = extract_rust("struct Alpha; impl Alpha { fn render(&self) {} }\n"
    ...
    const LSMResolvedCall *alpha_hit =
        calls[0] ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, calls[0], false)
                 : NULL;
    const LSMResolvedCall *beta_hit =
        calls[1] ? lsm_pipeline_find_lsp_resolution(&result->resolved_calls, calls[1], false)
                 : NULL;
...
TEST(rustlsp_crossfile_free_function) {
    ...
    lsm_run_rust_lsp_cross(&a, caller, (int)strlen(caller), "test.main",
...
TEST(rustlsp_handles_empty_file) {
    LSMFileResult *r = extract_rust("// nothing here\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_macro_println_no_args) {
    LSMFileResult *r = extract_rust("fn run() { println!(); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "println"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(rustlsp_cov_opt_is_some) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> bool { o.is_some() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.is_some"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_unwrap) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> i32 { o.unwrap() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.unwrap"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_unwrap_or) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> i32 { o.unwrap_or(0) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.unwrap_or"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_unwrap_or_default) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> i32 { o.unwrap_or_default() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.unwrap_or_default"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_map) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Option<i32> { o.map(|x| x + 1) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.map"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_and_then) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Option<i32> { o.and_then(|x| Some(x)) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.and_then"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_or) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Option<i32> { o.or(Some(0)) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.or"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_ok_or) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Result<i32, ()> { o.ok_or(()) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.ok_or"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_take) {
    LSMFileResult *r = extract_rust("fn run(mut o: Option<i32>) -> Option<i32> { o.take() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.take"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_filter) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Option<i32> { o.filter(|x| *x > 0) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.filter"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_iter) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) { o.iter(); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Option.iter"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_opt_as_ref) {
    LSMFileResult *r = extract_rust("fn run(o: Option<i32>) -> Option<&i32> { o.as_ref() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_res_is_ok) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> bool { r.is_ok() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.is_ok"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_is_err) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> bool { r.is_err() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.is_err"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_ok) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> Option<i32> { r.ok() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.ok"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_err) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, String>) -> Option<String> { r.err() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.err"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_unwrap) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> i32 { r.unwrap() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.unwrap"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_expect) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> i32 { r.expect(\"x\") }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.expect"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_unwrap_or) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> i32 { r.unwrap_or(0) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.unwrap_or"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_unwrap_or_else) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, String>) -> i32 { r.unwrap_or_else(|_| 0) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.unwrap_or_else"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_map) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> Result<i32, ()> { r.map(|x| x + 1) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.map"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_map_err) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, String>) -> Result<i32, usize> { r.map_err(|e| e.len()) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.map_err"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_and_then) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> Result<i32, ()> { r.and_then(|x| Ok(x)) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Result.and_then"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_res_or_else) {
    LSMFileResult *r = extract_rust("fn run(r: Result<i32, ()>) -> Result<i32, ()> { r.or_else(|_| Ok(0)) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_vec_push) {
    LSMFileResult *r = extract_rust("fn run() { let mut v = Vec::<i32>::new(); v.push(1); }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Vec.push"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_vec_pop) {
    LSMFileResult *r = extract_rust("fn run(mut v: Vec<i32>) -> Option<i32> { v.pop() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Vec.pop"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_vec_len) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> usize { v.len() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Vec.len"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_vec_is_empty) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> bool { v.is_empty() }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Vec.is_empty"), 0);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_vec_first_last) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) {\n    let _f = v.first();\n    let _l = v.last();\n}\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "Vec.first"), 0);
...
TEST(rustlsp_cov_iter_count) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> usize { v.iter().count() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_sum) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> i32 { v.iter().sum() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_product) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> i32 { v.iter().product() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_max) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> Option<i32> { v.iter().max().copied() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_min) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> Option<i32> { v.iter().min().copied() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_any) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> bool { v.iter().any(|x| *x > 0) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_all) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> bool { v.iter().all(|x| *x > 0) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_find) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> Option<&i32> { v.iter().find(|x| **x > 0) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_position) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> Option<usize> { v.iter().position(|x| *x > 0) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_for_each) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) { v.iter().for_each(|x| { let _ = x; }); }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_fold) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> i32 { v.iter().fold(0, |acc, x| acc + x) }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_chain) {
    LSMFileResult *r = extract_rust("fn run(a: Vec<i32>, b: Vec<i32>) -> usize { a.iter().chain(b.iter()).count() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_zip) {
    LSMFileResult *r = extract_rust("fn run(a: Vec<i32>, b: Vec<i32>) { for (_, _) in a.iter().zip(b.iter()) {} }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_take_skip) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> usize { v.iter().take(5).skip(2).count() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_iter_rev) {
    LSMFileResult *r = extract_rust("fn run(v: Vec<i32>) -> usize { v.iter().rev().count() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_cov_xf_two_methods) {
    ...
    LSMResolvedCallArray out; memset(&out, 0, sizeof(out));
    lsm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 3, imp_n, imp_q, 1, NULL, &out);
    ASSERT_GTE(find_confident(&out, "run", "Thing.alpha"), 0);
    ASSERT_GTE(find_confident(&out, "run", "Thing.beta"), 0);
    lsm_arena_destroy(&a); PASS();
}
...
TEST(rustlsp_cov_xf_free_function_chain) {
    ...
    LSMResolvedCallArray out; memset(&out, 0, sizeof(out));
    lsm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 1, imp_n, imp_q, 1, NULL, &out);
...
TEST(rustlsp_cov_xf_batch) {
    ...
    LSMResolvedCallArray out[2]; memset(out, 0, sizeof(out));
    lsm_batch_rust_lsp_cross(&a, files, 2, out);
    lsm_arena_destroy(&a); PASS();
}
TEST(rustlsp_cov_xf_empty_defs) {
    ...
    LSMArena a; lsm_arena_init(&a);
    LSMResolvedCallArray out; memset(&out, 0, sizeof(out));
    lsm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", NULL, 0, NULL, NULL, 0, NULL, &out);
    lsm_arena_destroy(&a); PASS();
}
TEST(rustlsp_cov_xf_nested_modules) {
    ...
    LSMResolvedCallArray out; memset(&out, 0, sizeof(out));
    lsm_run_rust_lsp_cross(&a, src, (int)strlen(src), "p.caller", defs, 1, NULL, NULL, 0, NULL, &out);
    lsm_arena_destroy(&a); PASS();
}
...
TEST(rustlsp_cov_robust_only_imports) {
    LSMFileResult *r = extract_rust("use std::collections::HashMap;\nuse std::sync::Arc;\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_robust_only_struct) {
    LSMFileResult *r = extract_rust("struct A;\nstruct B { x: i32 }\nstruct C(i32);\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_robust_only_enum) {
    LSMFileResult *r = extract_rust("enum E { A, B(i32), C { x: i32 } }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r); PASS();
}
TEST(rustlsp_cov_robust_only_trait) {
    LSMFileResult *r = extract_rust("trait T { fn f(&self); fn g(&self) {} }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r); PASS();
}
...
TEST(rustlsp_cov_async_fn_decl) {
    LSMFileResult *r = extract_rust("async fn run() -> i32 { 42 }\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r); PASS();
}
...
TEST(rustlsp_cov_edge_many_function_def) {
    ...
    LSMFileResult *r = extract_rust(src);
    ASSERT_NOT_NULL(r);
    int hits = 0;
    for (int i = 0; i < 20; i++) {
        char needle[16];
...
TEST(rustlsp_partial_cargo_parses_simple) {
    ...
    const char *toml =
        ...
        "local = { path = \"../local\" }\n";
    LSMCargoManifest m;
    lsm_cargo_parse(&a, toml, (int)strlen(toml), &m);
    ...
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "serde"));
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "tokio"));
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "local"));
    ASSERT_EQ(false, lsm_cargo_is_known_dep(&m, "unknown_crate"));
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_partial_cargo_parses_workspace) {
    ...
    const LSMCargoMember *a_mem = lsm_cargo_find_member(&m, "a");
    ASSERT_NOT_NULL(a_mem);
    ASSERT_EQ(0, strcmp(a_mem->member_path, "crates/a"));
    const LSMCargoMember *shared = lsm_cargo_find_member(&m, "shared");
    ASSERT_NOT_NULL(shared);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_partial_cargo_handles_comments_and_quirks) {
    ...
    ASSERT_GTE(m.dep_count, 3);
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "regex"));
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "log"));
    ASSERT_EQ(true, lsm_cargo_is_known_dep(&m, "anyhow"));
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_extra_cargo_wires_workspace_member) {
    ...
    /* Wire up a LSMFileResult by hand. */
    LSMFileResult* r = lsm_extract_file(src, (int)strlen(src),
        LSM_LANG_RUST, "test", "src/main.rs", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);

    /* Manually re-run with manifest. */
    LSMArena scratch; lsm_arena_init(&scratch);
    /* Discard prior resolved calls. */
    memset(&r->resolved_calls, 0, sizeof(r->resolved_calls));
    lsm_run_rust_lsp_with_manifest(&scratch, r, src, (int)strlen(src),
...
TEST(rustlsp_proc_macro_match_requires_exact_attribute_path) {
    LSMFileResult *r = extract_rust("#[not_tokio::main_wrapper]\nasync fn main() {}\n");
    ASSERT_NOT_NULL(r);
...
TEST(rustlsp_extra_rustdoc_ingest_basic_function) {
    ...
    const char* json =
        ...
        "  }\n"
        "}\n";
    int n = lsm_rust_rustdoc_ingest(&reg, &a, json, (int)strlen(json), "my_crate");
    ASSERT_GTE(n, 1);
    /* hello should be findable. */
    const LSMRegisteredFunc* f = lsm_registry_lookup_func(&reg, "my_crate.hello");
    ASSERT_NOT_NULL(f);
    lsm_arena_destroy(&a);
    PASS();
}
...
TEST(rustlsp_extra_rustdoc_ingest_struct_and_trait) {
    ...
    int n = lsm_rust_rustdoc_ingest(&reg, &a, json, (int)strlen(json), "my_crate");
    ASSERT_GTE(n, 2);
    const LSMRegisteredType* foo = lsm_registry_lookup_type(&reg, "my_crate.Foo");
    ASSERT_NOT_NULL(foo);
    const LSMRegisteredType* bar = lsm_registry_lookup_type(&reg, "my_crate.Bar");
    ASSERT_NOT_NULL(bar);
...
TEST(rustlsp_extra_rustdoc_handles_malformed) {
    ...
    int n = lsm_rust_rustdoc_ingest(&reg, &a, "{}", 2, "x");
    ASSERT_EQ(0, n);
    /* Truly broken — also should not crash. */
    int n2 = lsm_rust_rustdoc_ingest(&reg, &a, "not json", 8, "x");
    ASSERT_EQ(0, n2);
    lsm_arena_destroy(&a);
    PASS();
}
