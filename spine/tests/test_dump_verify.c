...
TEST(dump_verify_no_baseline) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(-1, 500, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_sparse_at_floor) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(50, 10, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    ASSERT_FALSE(lsm_dump_verify_is_degraded(12, 5, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_shortfall_below_ratio) {
    ASSERT_TRUE(lsm_dump_verify_is_degraded(1000, 400, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_just_above_ratio) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(1000, 500, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_just_below_ratio) {
    ASSERT_TRUE(lsm_dump_verify_is_degraded(1000, 499, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_zero_persisted) {
    ASSERT_TRUE(lsm_dump_verify_is_degraded(1000, 0, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_growth) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(500, 750, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_count_error) {
    ASSERT_TRUE(lsm_dump_verify_is_degraded(1000, -1, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_ratio_zero_disables) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(1000, 10, 0.0, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_loosened_ratio) {
    ASSERT_FALSE(lsm_dump_verify_is_degraded(1000, 600, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_tightened_ratio) {
    ASSERT_TRUE(lsm_dump_verify_is_degraded(1000, 900, 0.95, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}

TEST(dump_verify_edges_shrank_nodes_ok) {
    /* Edges are not gated; this documents nodes-only semantics for integrators. */
    ASSERT_FALSE(lsm_dump_verify_is_degraded(200, 200, 0.5, LSM_DUMP_VERIFY_MIN_FLOOR));
    PASS();
}
