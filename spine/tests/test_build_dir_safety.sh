...
for path in build/c build/linux-arm64 build/linux-amd64 build/win-cross; do
    lsm_require_safe_build_dir "$path"
done
...
for path in \
    ...
    build/nested/c \
    'build\..\outside'; do
    if lsm_require_safe_build_dir "$path" >/dev/null 2>&1; then
...
symlink_fixture="$(mktemp -d "${TMPDIR:-/tmp}/lsm-build-dir-symlink.XXXXXX")"
...
if make_real_dir_link "$symlink_fixture/outside" "$symlink_fixture/repo/build"; then
    if (
        ...
        lsm_require_safe_build_dir build/c "$symlink_fixture/repo"
    ); then
...
if make_real_dir_link "$symlink_fixture/final-target" "$symlink_fixture/safe-repo/build/c"; then
    if ! lsm_remove_build_dir "$symlink_fixture/safe-repo" build/c; then
...
lsm_remove_build_dir "$symlink_fixture/safe-repo" build/ordinary
...
if make -s -f "$ROOT/Makefile.lsm" clean-c BUILD_DIR=build/nested/c >/dev/null 2>&1; then
