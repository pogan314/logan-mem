...
TEST(usage_semantic_reference_candidate_trusts_marked_producer) {
    ...
    ASSERT_TRUE(lsm_pipeline_usage_semantic_reference_candidate(&usage));

    usage.may_be_call_reference = false;
    ASSERT_FALSE(lsm_pipeline_usage_semantic_reference_candidate(&usage));
...
static int setup_parallel_repo(void) {
    snprintf(g_par_tmpdir, sizeof(g_par_tmpdir), "/tmp/lsm_par_XXXXXX");
    if (!lsm_mkdtemp(g_par_tmpdir))
        return -1;
...
static void seed_test_file_nodes(lsm_gbuf_t *gbuf, const char *project,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (file_qn) {
            lsm_gbuf_upsert_node(gbuf, "File", basename, file_qn, rel, 0, 0, "{}");
        }
        free(file_qn);
    }
}
...
static void harness_ctx_free_tables(lsm_pipeline_ctx_t *ctx) {
...
static lsm_gbuf_t *run_sequential(const char *project, const char *repo_path,
                                  lsm_file_info_t *files, int file_count) {
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, repo_path);
    ...
    lsm_init();
    lsm_pipeline_pass_definitions(&ctx, files, file_count);
    lsm_pipeline_pass_calls(&ctx, files, file_count);
    lsm_pipeline_pass_usages(&ctx, files, file_count);
    lsm_pipeline_pass_semantic(&ctx, files, file_count);
...
typedef void (*parallel_result_mutator_fn)(LSMFileResult **result_cache, int file_count, void *ud);
...
static lsm_gbuf_t *run_sequential_with_lsp_cross_and_mutator(
    const char *project, const char *repo_path, lsm_file_info_t *files, int file_count,
    parallel_result_mutator_fn mutator, void *mutator_ud, bool seed_structure) {
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, repo_path);
    lsm_registry_t *reg = lsm_registry_new();
    LSMFileResult **cache = (LSMFileResult **)calloc((size_t)file_count, sizeof(LSMFileResult *));
    if (!gbuf || !reg || !cache) {
        lsm_gbuf_free(gbuf);
        lsm_registry_free(reg);
        free(cache);
    ...
    lsm_init();
    lsm_pipeline_pass_definitions(&ctx, files, file_count);
    lsm_pipeline_pass_lsp_cross(&ctx, files, file_count, cache);
    if (mutator) {
...
static lsm_gbuf_t *run_sequential_with_lsp_cross(const char *project, const char *repo_path,
                                                 lsm_file_info_t *files, int file_count) {
...
static lsm_gbuf_t *run_parallel_with_extract_opts_and_mutator(
    const char *project, const char *repo_path, lsm_file_info_t *files, int file_count,
    int worker_count, const lsm_parallel_extract_opts_t *extract_opts,
    parallel_result_mutator_fn mutator, void *mutator_ud, bool seed_structure) {
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, repo_path);
    lsm_registry_t *reg = lsm_registry_new();
    atomic_int cancelled;
    ...
    int64_t gbuf_next = lsm_gbuf_next_id(gbuf);
    atomic_init(&shared_ids, gbuf_next);

    LSMFileResult **result_cache = calloc((size_t)file_count, sizeof(LSMFileResult *));

    lsm_init();
    if (extract_opts) {
        lsm_parallel_extract_ex(&ctx, files, file_count, result_cache, &shared_ids, worker_count,
                                extract_opts);
    } else {
        lsm_parallel_extract(&ctx, files, file_count, result_cache, &shared_ids, worker_count);
    }
    lsm_gbuf_set_next_id(gbuf, atomic_load(&shared_ids));

    if (mutator) {
        mutator(result_cache, file_count, mutator_ud);
    }

    lsm_build_registry_from_cache(&ctx, files, file_count, result_cache);
    int64_t registry_next = lsm_gbuf_next_id(gbuf);
     ...
     * lsm_parallel_resolve where the fused resolve_worker invokes
     * lsm_pxc_run_one(_ts) per file BEFORE materializing CALLS edges. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    int def_count = 0;
    LSMLSPDef *all_defs =
        def_modules ? lsm_pxc_collect_all_defs(result_cache, files, file_count, ctx.project_name,
    ...
    lsm_parallel_resolve(&ctx, files, file_count, result_cache, &shared_ids, worker_count, all_defs,
    ...
    for (int i = 0; i < file_count; i++)
        if (result_cache[i])
            lsm_free_result(result_cache[i]);
    free(result_cache);
...
static lsm_gbuf_t *run_parallel_with_extract_opts(const char *project, const char *repo_path,
                                                  lsm_file_info_t *files, int file_count,
                                                  int worker_count,
                                                  const lsm_parallel_extract_opts_t *extract_opts) {
...
static lsm_gbuf_t *run_parallel(const char *project, const char *repo_path, lsm_file_info_t *files,
...
static lsm_gbuf_t *g_seq_gbuf = NULL;
static lsm_gbuf_t *g_par_gbuf = NULL;
...
static int ensure_parity_setup(void) {
    ...
    /* Discover files */
    lsm_discover_opts_t opts = {.mode = LSM_MODE_FULL};
    lsm_file_info_t *files = NULL;
    int file_count = 0;
    if (lsm_discover(g_par_tmpdir, &opts, &files, &file_count) != 0)
        return -1;
...
static void parity_teardown(void) {
    if (g_seq_gbuf) {
        lsm_gbuf_free(g_seq_gbuf);
        g_seq_gbuf = NULL;
    }
    if (g_par_gbuf) {
        lsm_gbuf_free(g_par_gbuf);
        g_par_gbuf = NULL;
    }
...
TEST(parallel_node_count) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = lsm_gbuf_node_count(g_seq_gbuf);
    int par = lsm_gbuf_node_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
static int assert_edge_type_parity(const char *type) {
    if (ensure_parity_setup() != 0)
        return -1;
    int seq = lsm_gbuf_edge_count_by_type(g_seq_gbuf, type);
    int par = lsm_gbuf_edge_count_by_type(g_par_gbuf, type);
    if (seq != par) {
...
TEST(parallel_semantic_fixture_expected_counts) {
    ...
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_seq_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_par_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_seq_gbuf, "INHERITS"), 1);
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_par_gbuf, "INHERITS"), 1);
    /* Circle.area overrides Shape.area; Base.area overrides Circle.area. */
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_seq_gbuf, "OVERRIDE"), 2);
    ASSERT_EQ(lsm_gbuf_edge_count_by_type(g_par_gbuf, "OVERRIDE"), 2);
    PASS();
}
...
TEST(parallel_total_edges) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = lsm_gbuf_edge_count(g_seq_gbuf);
    int par = lsm_gbuf_edge_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
TEST(parallel_empty_files) {
    lsm_gbuf_t *gbuf = lsm_gbuf_new("empty-proj", "/tmp");
    lsm_registry_t *reg = lsm_registry_new();
    atomic_int cancelled;
    atomic_init(&cancelled, 0);
    ...
    LSMFileResult **cache = NULL;
    int rc = lsm_parallel_extract(&ctx, NULL, 0, cache, &shared_ids, 2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(lsm_gbuf_node_count(gbuf), 0);
 ...
 * position exceed the fixed LSM_SZ_2K `props` stack buffer in
...
TEST(parallel_args_json_no_overflow) {
    char dir[256];
    snprintf(dir, sizeof(dir), "/tmp/lsm_argov_XXXXXX");
    ASSERT_TRUE(lsm_mkdtemp(dir) != NULL);
    ...
    lsm_file_info_t *files = NULL;
    int file_count = 0;
    ASSERT_EQ(lsm_discover(dir, &opts, &files, &file_count), 0);
    ASSERT_GT(file_count, 0);

    lsm_gbuf_t *gbuf = run_parallel("argov-test", dir, files, file_count, 4);
    ASSERT_TRUE(gbuf != NULL);
    ASSERT_GT(lsm_gbuf_edge_count(gbuf), 0);
...
TEST(gbuf_shared_ids_unique) {
    ...
    lsm_gbuf_t *ga = lsm_gbuf_new_shared_ids("proj", "/", &shared);
    lsm_gbuf_t *gb = lsm_gbuf_new_shared_ids("proj", "/", &shared);

    int64_t id1 = lsm_gbuf_upsert_node(ga, "Function", "foo", "proj.foo", "a.go", 1, 5, "{}");
    int64_t id2 = lsm_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "b.go", 1, 3, "{}");
    ASSERT_GT(id1, 0);
    ASSERT_GT(id2, 0);
    ASSERT_NEQ(id1, id2);
...
TEST(gbuf_merge_nodes) {
    _Atomic int64_t shared = 1;
    lsm_gbuf_t *dst = lsm_gbuf_new_shared_ids("proj", "/", &shared);
    lsm_gbuf_t *src = lsm_gbuf_new_shared_ids("proj", "/", &shared);

    lsm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    lsm_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    lsm_gbuf_upsert_node(src, "Function", "c", "proj.c", "b.go", 1, 5, "{}");
    lsm_gbuf_upsert_node(src, "Function", "d", "proj.d", "b.go", 6, 10, "{}");

    ASSERT_EQ(lsm_gbuf_node_count(dst), 2);
    lsm_gbuf_merge(dst, src);
    ASSERT_EQ(lsm_gbuf_node_count(dst), 4);

    ASSERT_NOT_NULL(lsm_gbuf_find_by_qn(dst, "proj.c"));
    ASSERT_NOT_NULL(lsm_gbuf_find_by_qn(dst, "proj.d"));
    /* dst originals still there */
    ASSERT_NOT_NULL(lsm_gbuf_find_by_qn(dst, "proj.a"));
    ASSERT_NOT_NULL(lsm_gbuf_find_by_qn(dst, "proj.b"));
...
TEST(gbuf_merge_edges) {
    ...
    int64_t a = lsm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int64_t b = lsm_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    /* Put an edge in src that references dst nodes (by ID) */
    lsm_gbuf_insert_edge(src, a, b, "CALLS", "{}");

    lsm_gbuf_merge(dst, src);
    ASSERT_GT(lsm_gbuf_edge_count(dst), 0);

    const lsm_gbuf_edge_t **edges = NULL;
    int count = 0;
    lsm_gbuf_find_edges_by_source_type(dst, a, "CALLS", &edges, &count);
    ASSERT_EQ(count, 1);
...
TEST(gbuf_merge_empty_src) {
    ...
    lsm_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int before = lsm_gbuf_node_count(dst);
    lsm_gbuf_merge(dst, src);
    ASSERT_EQ(lsm_gbuf_node_count(dst), before);
...
TEST(gbuf_merge_src_free_safe) {
    ...
    lsm_gbuf_upsert_node(src, "Function", "x", "proj.x", "x.go", 1, 5, "{}");
    lsm_gbuf_merge(dst, src);
    lsm_gbuf_free(src); /* must not crash */

    /* dst node still accessible */
    ASSERT_NOT_NULL(lsm_gbuf_find_by_qn(dst, "proj.x"));
    lsm_gbuf_free(dst);
    PASS();
}

TEST(gbuf_next_id_accessors) {
    lsm_gbuf_t *gb = lsm_gbuf_new("proj", "/");
    ASSERT_EQ(lsm_gbuf_next_id(gb), 1);

    lsm_gbuf_upsert_node(gb, "Function", "foo", "proj.foo", "f.go", 1, 5, "{}");
    ASSERT_GT(lsm_gbuf_next_id(gb), 1);

    lsm_gbuf_set_next_id(gb, 100);
    int64_t id = lsm_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "f.go", 6, 10, "{}");
    ASSERT_GTE(id, 100);
 ...
 * pass_parallel.c (parallel) on lsm_pipeline_find_lsp_resolution +
 * LSM_LSP_CONFIDENCE_FLOOR (lsp_resolve.h). Before the unification, the
...
static void count_lsp_call_edges(const lsm_gbuf_edge_t *edge, void *ud) {
...
static const lsm_gbuf_node_t *find_unique_callable_node_by_tail(const lsm_gbuf_t *gbuf,
                                                                const char *tail) {
    ...
    int count = 0;
    if (lsm_gbuf_find_by_name(gbuf, method, &nodes, &count) != 0) {
        return NULL;
    }
    const lsm_gbuf_node_t *match = NULL;
    for (int i = 0; i < count; i++) {
        const lsm_gbuf_node_t *node = nodes[i];
...
static const lsm_gbuf_node_t *find_unique_node_by_name_label_qn_suffix(const lsm_gbuf_t *gbuf,
                                                                       const char *name,
                                                                       const char *label,
    ...
    const lsm_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (lsm_gbuf_find_by_name(gbuf, name, &nodes, &count) != 0) {
        return NULL;
    }
    const lsm_gbuf_node_t *match = NULL;
    for (int i = 0; i < count; i++) {
...
static int count_edges_between_nodes(const lsm_gbuf_t *gbuf, const lsm_gbuf_node_t *source,
                                     const lsm_gbuf_node_t *target, const char *edge_type) {
    if (!gbuf || !source || !target || !edge_type) {
        return -1;
    }
    const lsm_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (lsm_gbuf_find_edges_by_source_type(gbuf, source->id, edge_type, &edges, &count) != 0) {
        return -1;
    }
    int matches = 0;
    for (int i = 0; i < count; i++) {
...
static bool has_edge_from_callable_to_node(const lsm_gbuf_t *gbuf, const char *source_tail,
                                           const lsm_gbuf_node_t *target, const char *edge_type) {
    const lsm_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
...
static const lsm_gbuf_edge_t *find_calls_edge_by_tails(const lsm_gbuf_t *gbuf,
                                                       const char *source_tail,
                                                       const char *target_tail) {
    const lsm_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
    const lsm_gbuf_node_t *target = find_unique_callable_node_by_tail(gbuf, target_tail);
    if (!source || !target) {
        return NULL;
    }
    ...
    const lsm_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (lsm_gbuf_find_edges_by_source_type(gbuf, source->id, "CALLS", &edges, &count) != 0) {
        return NULL;
    }
    for (int i = 0; i < count; i++) {
        if (edges[i] && edges[i]->target_id == target->id) {
...
static bool has_edge_from_callable_to_qn(const lsm_gbuf_t *gbuf, const char *source_tail,
                                         const char *target_qn, const char *edge_type) {
    const lsm_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
    const lsm_gbuf_node_t *target = lsm_gbuf_find_by_qn(gbuf, target_qn);
    if (!source || !target)
        return false;
    const lsm_gbuf_edge_t **edges = NULL;
    int count = 0;
...
static bool callable_has_call_target_fragment(const lsm_gbuf_t *gbuf, const char *source_tail,
    ...
    for (int i = 0; i < count; i++) {
        const lsm_gbuf_node_t *target =
            edges[i] ? lsm_gbuf_find_by_id(gbuf, edges[i]->target_id) : NULL;
...
static int count_calls_edges_to_tail(const lsm_gbuf_t *gbuf, const char *target_tail) {
    const lsm_gbuf_node_t *target = find_unique_callable_node_by_tail(gbuf, target_tail);
    if (!target)
        return -1;
    const lsm_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (lsm_gbuf_find_edges_by_target_type(gbuf, target->id, "CALLS", &edges, &count) != 0)
        return -1;
    return count;
}
...
static void inject_higher_confidence_legacy_render(LSMFileResult **result_cache, int file_count,
                                                   void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        LSMFileResult *result = result_cache ? result_cache[file] : NULL;
        if (!result) {
            continue;
        }
        const LSMResolvedCall *exact = NULL;
        const char *beta_qn = NULL;
        for (int i = 0; i < result->resolved_calls.count; i++) {
            const LSMResolvedCall *candidate = &result->resolved_calls.items[i];
            if (candidate->kind == LSM_RESOLVED_INVOCATION && candidate->caller_qn &&
        ...
        lsm_resolvedcall_push(&result->resolved_calls, &result->arena, legacy);
        state->injected = true;
        return;
    }
}
...
TEST(parallel_lsp_index_exact_site_beats_legacy_record_in_graph) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_lsp_site_XXXXXX");
    ...
    lsm_file_info_t files[1] = {0};
    files[0].path = path;
    files[0].rel_path = (char *)"app.py";
    files[0].language = LSM_LANG_PYTHON;
    lsp_legacy_injection_t injection = {0};
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_par_lsp_site", tmpdir, files, 1, 1, NULL, inject_higher_confidence_legacy_render,
        &injection, false);
    ASSERT_NOT_NULL(gbuf);
    ASSERT_TRUE(injection.injected);

    const lsm_gbuf_edge_t *exact = find_calls_edge_by_tails(gbuf, "Caller.run", "Alpha.render");
    const lsm_gbuf_edge_t *wrong = find_calls_edge_by_tails(gbuf, "Caller.run", "Beta.render");
    if (!exact || wrong) {
...
static void inject_distinct_exact_render_target(LSMFileResult **result_cache, int file_count,
                                                void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        LSMCall *carrier = NULL;
        const LSMResolvedCall *exact = NULL;
        const char *beta_qn = NULL;
        for (int i = 0; i < result->calls.count; i++) {
            LSMCall *candidate = &result->calls.items[i];
            if (candidate->callee_name && candidate->enclosing_func_qn &&
                strstr(candidate->enclosing_func_qn, "Caller.run") &&
                strcmp(lsm_pipeline_call_callee_leaf(candidate->callee_name), "render") == 0 &&
                lsm_pipeline_source_site_present(candidate->site_start_byte,
...
TEST(parallel_lsp_index_distinct_exact_targets_fail_closed_in_graph) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_lsp_ambiguous_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    lsm_file_info_t file_info = {0};
    file_info.path = path;
    file_info.rel_path = (char *)"app.py";
    file_info.language = LSM_LANG_PYTHON;
    lsp_exact_ambiguity_injection_t injection = {0};
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_par_lsp_ambiguous", tmpdir, &file_info, 1, 1, NULL,
    ...
    const lsm_gbuf_edge_t *alpha = find_calls_edge_by_tails(gbuf, "Caller.run", "Alpha.render");
    const lsm_gbuf_edge_t *beta = find_calls_edge_by_tails(gbuf, "Caller.run", "Beta.render");
    if (alpha || beta) {
...
static void inject_legacy_after_exact_render_ambiguity(LSMFileResult **result_cache, int file_count,
                                                       void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        probe->shared_matcher_failed_closed =
            lsm_pipeline_find_lsp_resolution(&result->resolved_calls, carrier, false) == NULL;
        return;
    }
}
...
TEST(parallel_lsp_index_exact_ambiguity_does_not_fall_through_to_legacy) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_lsp_ambiguous_legacy_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_par_lsp_ambiguous_legacy", tmpdir, &file_info, 1, 1, NULL,
    ...
    const lsm_gbuf_edge_t *legacy_edge =
...
static void inject_long_caller_exact_and_legacy(LSMFileResult **result_cache, int file_count,
                                                void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            LSMResolvedCall *candidate = &result->resolved_calls.items[i];
            if (candidate->kind == LSM_RESOLVED_INVOCATION && candidate->callee_qn &&
                strstr(candidate->callee_qn, "Alpha.render") &&
                candidate->site_end_byte > candidate->site_start_byte) {
                exact = candidate;
                break;
        ...
        const LSMResolvedCall *authoritative =
...
TEST(parallel_lsp_long_exact_key_never_yields_legacy_target) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_lsp_long_key_XXXXXX");
    if (!lsm_mkdtemp(tmpdir))
        FAIL("mkdtemp failed");
    char path[512];
    snprintf(path, sizeof(path), "%s/app.py", tmpdir);
    ...
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_long_key", tmpdir, files, 1, 1, NULL, inject_long_caller_exact_and_legacy, &probe,
        false);
    ASSERT_NOT_NULL(gbuf);
    int alpha_edges = count_calls_edges_to_tail(gbuf, "Alpha.render");
    int beta_edges = count_calls_edges_to_tail(gbuf, "Beta.render");
...
static void inspect_synthetic_add_occurrences(LSMFileResult **result_cache, int file_count,
                                              void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        LSMFileResult *result = result_cache[file];
        if (!result) {
            continue;
        }
        for (int i = 0; i < result->calls.count; i++) {
            ...
            const LSMResolvedCall *resolved =
                lsm_pipeline_find_lsp_resolution(&result->resolved_calls, call, false);
            if (resolved &&
                lsm_pipeline_source_site_eq(call->site_start_byte, call->site_end_byte,
...
TEST(parallel_lsp_exact_index_handles_repeated_synthetic_occurrences_without_linear_scan) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_lsp_perf_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    lsm_pp_lsp_linear_fallback_rows_reset();
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_par_lsp_perf", tmpdir, files, 1, 1, NULL, inspect_synthetic_add_occurrences, &probe,
        false);
    ASSERT_NOT_NULL(gbuf);
    ...
    uint64_t fallback_rows = lsm_pp_lsp_linear_fallback_rows();
    if (fallback_rows != 0) {
...
static void inject_call_reference_scale_rows(LSMFileResult **result_cache, int file_count,
                                             void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        int inserted = 0;
        for (int i = 0; i < result->defs.count; i++) {
            const LSMDefinition *target = &result->defs.items[i];
            ...
            lsm_usages_push(&result->usages, &result->arena, usage);
            ...
            lsm_resolvedcall_push(&result->resolved_calls, &result->arena, resolved);
            inserted++;
        }
...
TEST(parallel_call_reference_lookup_rows_grow_linearly) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_ref_scale_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    lsm_pipeline_lsp_reference_lookup_test_reset();
    lsm_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "lsm_par_ref_scale", tmpdir, &file_info, 1, 1, NULL, inject_call_reference_scale_rows,
        &probe, false);
    uint64_t rows_examined = lsm_pipeline_lsp_reference_lookup_test_rows_examined();

    const lsm_gbuf_node_t *caller =
        gbuf ? find_unique_callable_node_by_tail(gbuf, "app.scale_caller") : NULL;
    const lsm_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    int lookup_rc = caller ? lsm_gbuf_find_edges_by_source_type(gbuf, caller->id, "CALL_REFERENCE",
                                                                &edges, &edge_count)
                           : -1;
...
TEST(parallel_tsx_local_component_shadow_has_no_false_call) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_tsx_shadow_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    files[0].language = LSM_LANG_TSX;
    lsm_gbuf_t *sequential = run_sequential("lsm_tsx_shadow", tmpdir, files, 1);
    lsm_gbuf_t *parallel = run_parallel("lsm_tsx_shadow", tmpdir, files, 1, 1);
    ...
    lsm_gbuf_free(sequential);
    lsm_gbuf_free(parallel);
    unlink(path);
    rmdir(tmpdir);
...
static void inject_exact_noncallable_reference(LSMFileResult **result_cache, int file_count,
                                               void *ud) {
...
TEST(parallel_exact_semantic_noncallable_target_stays_usage) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_exact_noncallable_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    lsm_file_info_t file = {0};
    file.path = path;
    file.rel_path = (char *)"app.ts";
    file.language = LSM_LANG_TYPESCRIPT;
    exact_noncallable_reference_probe_t sequential_probe = {0};
    exact_noncallable_reference_probe_t parallel_probe = {0};
    lsm_gbuf_t *sequential = run_sequential_with_lsp_cross_and_mutator(
        "lsm_exact_noncallable", tmpdir, &file, 1, inject_exact_noncallable_reference,
        &sequential_probe, false);
    lsm_gbuf_t *parallel = run_parallel_with_extract_opts_and_mutator(
        "lsm_exact_noncallable", tmpdir, &file, 1, 1, NULL, inject_exact_noncallable_reference,
        &parallel_probe, false);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const lsm_gbuf_node_t *sequential_target = find_unique_node_by_name_label_qn_suffix(
        sequential, "semanticValue", "Variable", "app.semanticValue");
    const lsm_gbuf_node_t *parallel_target = find_unique_node_by_name_label_qn_suffix(
...
TEST(parallel_typescript_module_value_usage_respects_lexical_shadows) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_ts_usage_scope_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    lsm_file_info_t files[2] = {0};
    files[0].path = settings_path;
    files[0].rel_path = (char *)"settings.ts";
    files[0].language = LSM_LANG_TYPESCRIPT;
    files[1].path = shadows_path;
    files[1].rel_path = (char *)"shadows.ts";
    files[1].language = LSM_LANG_TYPESCRIPT;
    lsm_gbuf_t *sequential = run_sequential("lsm_ts_usage_scope", tmpdir, files, 2);
    lsm_gbuf_t *parallel = run_parallel("lsm_ts_usage_scope", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const lsm_gbuf_node_t *sequential_config = find_unique_node_by_name_label_qn_suffix(
        sequential, "config", "Variable", "settings.config");
    const lsm_gbuf_node_t *parallel_config =
...
static ts_exact_parity_observation_t run_ts_exact_parity_fixture(LSMLanguage main_language,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_ts_exact_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        return observation;
    }
    ...
    for (int route = 0; route < 2; route++) {
        lsm_gbuf_t *graph = graphs[route];
        const lsm_gbuf_node_t *choose =
            find_unique_node_by_name_label_qn_suffix(graph, "choose", "Function", "main.choose");
        const lsm_gbuf_node_t *selected = find_unique_node_by_name_label_qn_suffix(
            graph, "Config", "Interface", "a.types.Config");
        const lsm_gbuf_node_t *decoy = find_unique_node_by_name_label_qn_suffix(
            graph, "Config", "Interface", "b.types.Config");
        const lsm_gbuf_node_t *local_class =
            find_unique_node_by_name_label_qn_suffix(graph, "Config", "Class", "main.App.Config");
        const lsm_gbuf_node_t *normalise = find_unique_node_by_name_label_qn_suffix(
            graph, "normalise", "Function", "main.App.Utils.normalise");
        const lsm_gbuf_node_t *clamp = find_unique_node_by_name_label_qn_suffix(
            graph, "clamp", "Function", "main.App.Utils.clamp");
        const lsm_gbuf_node_t *constructor = find_unique_node_by_name_label_qn_suffix(
    ...
    lsm_gbuf_free(graphs[0]);
    lsm_gbuf_free(graphs[1]);
    th_rmtree(tmpdir);
    return observation;
}
...
TEST(parallel_typescript_import_namespace_exact_parity) {
    ts_exact_parity_observation_t observation =
        run_ts_exact_parity_fixture(LSM_LANG_TYPESCRIPT, "ts", "lsm_ts_exact_parity");
...
TEST(parallel_tsx_import_namespace_exact_parity) {
    ts_exact_parity_observation_t observation =
        run_ts_exact_parity_fixture(LSM_LANG_TSX, "tsx", "lsm_tsx_exact_parity");
...
static kotlin_implicit_probe_t probe_kotlin_implicit_site(const LSMFileResult *result,
    ...
    for (int j = 0; j < result->resolved_calls.count; j++) {
        const LSMResolvedCall *resolved = &result->resolved_calls.items[j];
        ...
        if (resolved->kind != LSM_RESOLVED_INVOCATION || !resolved_caller || !resolved->callee_qn ||
        ...
        for (int i = 0; i < result->calls.count; i++) {
            ...
            if (lsm_pipeline_source_site_eq(call->site_start_byte, call->site_end_byte,
                                            resolved->site_start_byte, resolved->site_end_byte)) {
...
TEST(parallel_kotlin_external_protocol_does_not_use_project_class_method_tail) {
    ...
    const char *project = "lsm_kt_target_policy";

    LSMFileResult *raw = lsm_extract_file(main_source, (int)strlen(main_source), LSM_LANG_KOTLIN,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_kt_target_policy_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    files[0].language = LSM_LANG_KOTLIN;
    files[1].path = main_path;
    files[1].rel_path = (char *)"app/Main.kt";
    files[1].language = LSM_LANG_KOTLIN;
    lsm_gbuf_t *sequential = run_sequential_with_lsp_cross(project, tmpdir, files, 2);
    lsm_gbuf_t *parallel = run_parallel(project, tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const lsm_gbuf_node_t *sequential_iterator = find_unique_node_by_name_label_qn_suffix(
        sequential, "iterator", "Method", "Collisions.IntArray.iterator");
    const lsm_gbuf_node_t *parallel_iterator = find_unique_node_by_name_label_qn_suffix(
        parallel, "iterator", "Method", "Collisions.IntArray.iterator");
    const lsm_gbuf_node_t *sequential_component = find_unique_node_by_name_label_qn_suffix(
        sequential, "component1", "Method", "Collisions.Pair.component1");
    const lsm_gbuf_node_t *parallel_component = find_unique_node_by_name_label_qn_suffix(
 ...
 * LSMCall carrier so both graph paths can materialize CALLS without a textual
...
TEST(parallel_kotlin_nonbinary_operator_carriers_reach_graph) {
    ...
    static const char source[] = "package app\n"
                                 ...
                                 "fun isBag(value: Any): Boolean = value is Bag\n"
                                 "fun assign(slots: Slots) { slots[0] = 1 }\n";
    const char *project = "lsm_kt_nonbinary_operators";

    LSMFileResult *raw = lsm_extract_file(source, (int)strlen(source), LSM_LANG_KOTLIN, project,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_kt_nonbinary_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
...
TEST(parallel_rust_known_macro_does_not_fallback_to_local_function) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_rust_macro_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    LSMFileResult *extracted = lsm_extract_file(source, (int)strlen(source), LSM_LANG_RUST,
                                                "lsm_rust_macro", "main.rs", 0, NULL, NULL);
    ...
    for (int i = 0; i < extracted->resolved_calls.count; i++) {
        const LSMResolvedCall *resolved = &extracted->resolved_calls.items[i];
    ...
    for (int i = 0; i < extracted->calls.count; i++) {
        const LSMCall *call = &extracted->calls.items[i];
    ...
    lsm_free_result(extracted);

    lsm_file_info_t files[1] = {0};
    files[0].path = path;
    files[0].rel_path = (char *)"main.rs";
    files[0].language = LSM_LANG_RUST;
    lsm_gbuf_t *sequential = run_sequential("lsm_rust_macro", tmpdir, files, 1);
    lsm_gbuf_t *parallel = run_parallel("lsm_rust_macro", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);
...
TEST(parallel_rust_proc_macros_are_decorates_and_usage_only) {
    static const char source[] = "#[tokio::main]\nasync fn main() {}\n"
                                 "#[tracing::instrument]\nfn work() {}\n";
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_rust_proc_macro_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    files[0].rel_path = (char *)"main.rs";
    files[0].language = LSM_LANG_RUST;
    lsm_gbuf_t *sequential = run_sequential("lsm_rust_proc_macro", tmpdir, files, 1);
    lsm_gbuf_t *parallel = run_parallel("lsm_rust_proc_macro", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);
...
static int assert_c_family_preprocessed_collision_graph(LSMLanguage language, const char *extension,
                                                        const char *project, const char *source,
                                                        const char *target_tail) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_c_origin_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char rel_path[64];
    ...
    lsm_gbuf_t *sequential = run_sequential(project, tmpdir, files, 1);
    lsm_gbuf_t *parallel = run_parallel(project, tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const lsm_gbuf_edge_t *sequential_edge =
        find_calls_edge_by_tails(sequential, "main.occurrence_probe", target_tail);
    const lsm_gbuf_edge_t *parallel_edge =
...
TEST(parallel_c_preprocessed_coordinate_collision_preserves_hidden_target) {
    static const char source[] =
        ...
        "}\n";
    return assert_c_family_preprocessed_collision_graph(LSM_LANG_C, "c", "lsm_c_origin", source,
                                                        "main.bravo_target");
}

static int assert_cpp_like_preprocessed_collision_graph(LSMLanguage language, const char *extension,
                                                        const char *project) {
    static const char source[] =
...
TEST(parallel_cpp_preprocessed_coordinate_collision_preserves_hidden_target) {
    return assert_cpp_like_preprocessed_collision_graph(LSM_LANG_CPP, "cpp", "lsm_cpp_origin");
}

TEST(parallel_cuda_preprocessed_coordinate_collision_preserves_hidden_target) {
    return assert_cpp_like_preprocessed_collision_graph(LSM_LANG_CUDA, "cu", "lsm_cuda_origin");
}

TEST(parallel_java_kotlin_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_jvm_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    lsm_mkdir_p(jdir, 0755);
    FILE *jf = lsm_fopen(jpath, "w");
    if (!jf) {
    ...
    lsm_mkdir_p(kdir, 0755);
    FILE *kf = lsm_fopen(kpath, "w");
    if (!kf) {
        unlink(jpath);
        rmdir(tmpdir);
    ...
    files[0].language = LSM_LANG_JAVA;
    files[1].path = kpath;
    files[1].rel_path = (char *)"src/main/kotlin/com/example/KotlinService.kt";
    files[1].language = LSM_LANG_KOTLIN;

    lsm_gbuf_t *gbuf = run_parallel("com", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(gbuf);

    const lsm_gbuf_edge_t *java_to_kotlin =
        find_calls_edge_by_tails(gbuf, "JavaCaller.call", "KotlinService.ping");
    const lsm_gbuf_edge_t *kotlin_to_java =
 ...
 * lsm_pipeline_lsp_allow_tail_match must keep the fallbacks OFF there.
 *
...
TEST(parallel_lsp_tail_match_fallbacks_gated_to_jvm) {
    ...
    ASSERT_TRUE(lsm_pipeline_lsp_allow_tail_match(LSM_LANG_JAVA));
    ASSERT_TRUE(lsm_pipeline_lsp_allow_tail_match(LSM_LANG_KOTLIN));
    ASSERT_TRUE(!lsm_pipeline_lsp_allow_tail_match(LSM_LANG_PYTHON));
    ASSERT_TRUE(!lsm_pipeline_lsp_allow_tail_match(LSM_LANG_GO));
    ASSERT_TRUE(!lsm_pipeline_lsp_allow_tail_match(LSM_LANG_TYPESCRIPT));
    ASSERT_TRUE(!lsm_pipeline_lsp_allow_tail_match(LSM_LANG_CPP));
    ...
    ASSERT_TRUE(lsm_pipeline_find_lsp_resolution(&arr, &call, false) == NULL);
    ASSERT_TRUE(lsm_pipeline_find_lsp_resolution(&arr, &call, true) == &rc_item);
    ...
    LSMResolvedCall top_level_rows[2] = {0};
    ...
    top_level_rows[0].source_origin = LSM_SOURCE_ORIGIN_RAW;
    LSMResolvedCallArray top_level_arr = {0};
    top_level_arr.items = top_level_rows;
    top_level_arr.count = 1;
    top_level_arr.cap = 2;

    LSMCall top_level_call = {0};
    ...
    top_level_call.source_origin = LSM_SOURCE_ORIGIN_RAW;
    top_level_call.requires_lsp_resolution = true;
    ASSERT_TRUE(lsm_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, false) == NULL);
    ASSERT_TRUE(lsm_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, true) ==
                &top_level_rows[0]);

    top_level_call.enclosing_func_qn = "proj.app.Main.differentCaller";
    ASSERT_TRUE(lsm_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, true) == NULL);
    top_level_call.enclosing_func_qn = "proj.app.Main.operatorCaller";
    ...
    lsm_gbuf_t *tgbuf = lsm_gbuf_new("proj", "/tmp");
    ASSERT_NOT_NULL(tgbuf);
    int64_t nid = lsm_gbuf_upsert_node(tgbuf, "Method", "run", "proj.zeta.Helper.run",
                                       "zeta/helper.py", 1, 3, NULL);
    ASSERT_TRUE(nid != 0);
    ASSERT_TRUE(lsm_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", false) == NULL);
    const lsm_gbuf_node_t *jvm_hit =
        lsm_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", true);
    ...
    ASSERT_TRUE(lsm_pipeline_lsp_target_node_strict(tgbuf, "proj", "com.other.Helper.run", true) ==
                NULL);
    lsm_gbuf_free(tgbuf);
    PASS();
}
...
TEST(parallel_python_lsp_override_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_pylsp_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    lsm_gbuf_t *gbuf = run_parallel("lsm_par_pylsp", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(gbuf);

    lsp_edge_count_ctx_t c = {0};
    lsm_gbuf_foreach_edge(gbuf, count_lsp_call_edges, &c);
     ...
     * CALLS edge. This proves the unified lsm_pipeline_find_lsp_resolution
 ...
 * Before lsm_pipeline_lsp_target_node added the project-prefix fallback,
 * the LSP match succeeded (lsp_overrides counter incremented) but the
 * downstream lsm_gbuf_find_by_qn lookup missed silently, dropping the
...
TEST(parallel_python_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_pylsp_xf_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    files[1].language = LSM_LANG_PYTHON;

    lsm_gbuf_t *gbuf = run_parallel("lsm_par_pylsp_xf", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(gbuf);
     ...
     * lsm_pipeline_lsp_target_node this assertion would fail because the
...
TEST(parallel_cross_file_reread_preserves_unretained_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_par_xf_reread_XXXXXX");
    if (!lsm_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    const lsm_parallel_extract_opts_t no_retain = {
        .retain_sources = false,
        .retain_sources_set = true,
    };
    const lsm_parallel_extract_opts_t over_cap = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_per_file_max_bytes = 1, /* 1 byte → every file dropped by the size cap */
    };
    const lsm_parallel_extract_opts_t *scenarios[3] = {NULL, &no_retain, &over_cap};

    for (int s = 0; s < 3; s++) {
        lsm_gbuf_t *gbuf = run_parallel_with_extract_opts("com", tmpdir, files, 2, 2, scenarios[s]);
        ASSERT_NOT_NULL(gbuf);
...
/* ── Shared "::" normalization in lsm_pipeline_find_lsp_resolution (QA F3) ─
 *
 ...
 * against lsm_pipeline_find_lsp_resolution: (1) a qualified static call still
...
static LSMResolvedCall make_rc(const char *caller, const char *callee, float conf) {
...
static LSMCall make_call(const char *enclosing, const char *callee_name) {
    LSMCall c;
    memset(&c, 0, sizeof(c));
...
TEST(lsp_bare_segment_skips_preprocessor_spacing) {
    ...
    ASSERT_STR_EQ(lsm_lsp_bare_segment("bravo . render"), "render");
    ASSERT_STR_EQ(lsm_lsp_bare_segment("ptr -> run"), "run");
    PASS();
}
...
TEST(lsp_resolve_qualified_static_call_normalizes_colons) {
    ...
    LSMResolvedCallArray arr = {items, 1, 1};
    LSMCall call = make_call("proj.mod.caller", "Pkg::sub");
    const LSMResolvedCall *hit = lsm_pipeline_find_lsp_resolution(&arr, &call, false);
    ASSERT(hit != NULL);
    ...
    LSMCall bare = make_call("proj.mod.caller", "sub");
    const LSMResolvedCall *bare_hit = lsm_pipeline_find_lsp_resolution(&arr, &bare, false);
...
TEST(lsp_resolve_distinct_exact_caller_targets_fail_closed) {
    ...
    LSMResolvedCallArray arr = {items, 4, 4};
    LSMCall call = make_call("proj.mod.caller", "B::foo");
    const LSMResolvedCall *hit = lsm_pipeline_find_lsp_resolution(&arr, &call, false);
    ASSERT_NULL(hit);

    /* The cross-caller high-confidence entry only matches its own caller. */
    LSMCall other = make_call("proj.mod.other", "D::foo");
    const LSMResolvedCall *other_hit = lsm_pipeline_find_lsp_resolution(&arr, &other, false);
    ...
    LSMCall absent = make_call("proj.mod.absent", "foo");
    ASSERT(lsm_pipeline_find_lsp_resolution(&arr, &absent, false) == NULL);
    PASS();
}

TEST(lsp_resolve_duplicate_exact_caller_rows_same_target_are_not_ambiguous) {
    LSMCall call = make_call("proj.mod.caller", "receiver.render");
    call.site_start_byte = 75;
    call.site_end_byte = 91;
    LSMResolvedCall low = make_rc("proj.mod.caller", "proj.Alpha.render", 0.75f);
    low.site_start_byte = call.site_start_byte;
    low.site_end_byte = call.site_end_byte;
    LSMResolvedCall high = low;
    high.confidence = 0.95f;
    LSMResolvedCall items[] = {low, high};
    LSMResolvedCallArray arr = {items, 2, 2};

    ASSERT(lsm_pipeline_find_lsp_resolution(&arr, &call, false) == &items[1]);
    PASS();
}
...
TEST(lsp_resolve_distinct_exact_caller_site_targets_are_ambiguous) {
    ...
    LSMResolvedCall alpha = make_rc("proj.mod.caller", "proj.Alpha.render", 0.75f);
    alpha.site_start_byte = call.site_start_byte;
    alpha.site_end_byte = call.site_end_byte;
    LSMResolvedCall beta = make_rc("proj.mod.caller", "proj.Beta.render", 0.99f);
    beta.site_start_byte = call.site_start_byte;
    beta.site_end_byte = call.site_end_byte;
    LSMResolvedCall items[] = {alpha, beta};
    LSMResolvedCallArray arr = {items, 2, 2};

    ASSERT_NULL(lsm_pipeline_find_lsp_resolution(&arr, &call, false));
    PASS();
}
...
TEST(lsp_resolve_project_prefixed_duplicate_is_not_ambiguous) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = lsm_gbuf_upsert_node(gbuf, "Function", "handler",
                                             "proj.mod.Target.handler", "target.py", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    LSMCall call = make_call("proj.mod.Caller.run", "handler");
    call.site_start_byte = 110;
    call.site_end_byte = 119;
    LSMResolvedCall raw = make_rc("proj.mod.Caller.run", "mod.Target.handler", 0.75f);
    raw.site_start_byte = call.site_start_byte;
    raw.site_end_byte = call.site_end_byte;
    LSMResolvedCall prefixed =
    ...
    LSMResolvedCall items[] = {raw, prefixed};
    LSMResolvedCallArray arr = {items, 2, 2};

    const LSMResolvedCall *hit =
        lsm_pipeline_find_lsp_resolution_in_graph(&arr, &call, false, gbuf, project);
    lsm_gbuf_free(gbuf);
    ASSERT(hit == &items[1]);
    PASS();
}
...
TEST(lsp_resolve_project_prefix_spellings_stay_ambiguous_when_both_nodes_exist) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t raw_id = lsm_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                          "raw.py", 1, 1, "{}");
    int64_t prefixed_id = lsm_gbuf_upsert_node(gbuf, "Function", "handler",
...
TEST(lsp_resolve_jvm_package_alias_of_materialized_target_is_not_ambiguous) {
    ...
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id =
        lsm_gbuf_upsert_node(gbuf, "Method", "twice", "proj.Util.twice", "Util.java", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    LSMCall call = make_call("proj.Client.run", "twice");
    call.site_start_byte = 120;
    call.site_end_byte = 128;
    LSMResolvedCall local = make_rc("proj.Client.run", "proj.Util.twice", 0.92f);
    local.site_start_byte = call.site_start_byte;
    local.site_end_byte = call.site_end_byte;
    LSMResolvedCall package = make_rc("proj.Client.run", "demo.Util.twice", 0.92f);
    package.site_start_byte = call.site_start_byte;
    package.site_end_byte = call.site_end_byte;
    LSMResolvedCall items[] = {local, package};
    LSMResolvedCallArray arr = {items, 2, 2};

    const LSMResolvedCall *hit =
        lsm_pipeline_find_lsp_resolution_in_graph(&arr, &call, true, gbuf, project);
    lsm_gbuf_free(gbuf);
    ASSERT_NOT_NULL(hit);
    PASS();
}
...
TEST(lsp_resolve_jvm_two_tail_only_packages_stay_ambiguous) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = lsm_gbuf_upsert_node(gbuf, "Method", "run", "proj.generated.Service.run",
                                             "Service.java", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    LSMCall call = make_call("proj.Client.call", "run");
    call.site_start_byte = 130;
    call.site_end_byte = 138;
    LSMResolvedCall first = make_rc("proj.Client.call", "pkg1.Service.run", 0.80f);
    first.site_start_byte = call.site_start_byte;
    first.site_end_byte = call.site_end_byte;
    LSMResolvedCall second = make_rc("proj.Client.call", "pkg2.Service.run", 0.90f);
    second.site_start_byte = call.site_start_byte;
    second.site_end_byte = call.site_end_byte;
    LSMResolvedCall items[] = {first, second};
    LSMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_exact_site_beats_higher_confidence_legacy_record) {
    ...
    LSMResolvedCall exact = make_rc("proj.mod.caller", "proj.Alpha.render", 0.80f);
     ...
     * carrier occurrence. */
    LSMResolvedCall legacy = make_rc("proj.mod.caller", "proj.Beta.render", 0.95f);
    LSMResolvedCall items[] = {exact, legacy};
    LSMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_exact_site_beats_equal_confidence_legacy_record) {
    ...
    LSMResolvedCall legacy = make_rc("proj.mod.caller", "proj.Beta.render", 0.90f);
    LSMResolvedCall exact = make_rc("proj.mod.caller", "proj.Alpha.render", 0.90f);
    exact.site_start_byte = call.site_start_byte;
    exact.site_end_byte = call.site_end_byte;
    LSMResolvedCall items[] = {legacy, exact};
    LSMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_malformed_span_is_not_legacy_compatible) {
    ...
    LSMResolvedCall malformed = make_rc("proj.mod.caller", "proj.Wrong.render", 0.99f);
    malformed.site_start_byte = 317;
    malformed.site_end_byte = 300;
    LSMResolvedCall legacy = make_rc("proj.mod.caller", "proj.Legacy.render", 0.75f);
    LSMResolvedCall items[] = {malformed, legacy};
    LSMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_below_floor_exact_does_not_block_legacy) {
    ...
    LSMResolvedCall below_floor = make_rc("proj.mod.caller", "proj.Exact.render", 0.55f);
    below_floor.site_start_byte = call.site_start_byte;
    below_floor.site_end_byte = call.site_end_byte;
    LSMResolvedCall legacy = make_rc("proj.mod.caller", "proj.Legacy.render", 0.70f);
    LSMResolvedCall items[] = {below_floor, legacy};
    LSMResolvedCallArray arr = {items, 2, 2};
...
static LSMCall make_tail_call(uint32_t start, uint32_t end) {
    LSMCall call = make_call("proj.raw.Service.handle", "helper.run");
    call.site_start_byte = start;
    call.site_end_byte = end;
    return call;
}

static LSMResolvedCall make_tail_rc(const char *callee, float confidence, uint32_t start,
                                    uint32_t end) {
    LSMResolvedCall resolved = make_rc("com.example.generated.Service.handle", callee, confidence);
...
TEST(lsp_tail_exact_site_beats_legacy_regardless_of_order) {
    LSMCall call = make_tail_call(500, 512);
    LSMResolvedCall exact = make_tail_rc("com.example.Alpha.run", 0.80f, 500, 512);
    LSMResolvedCall legacy = make_tail_rc("com.example.Beta.run", 0.99f, 0, 0);
    LSMResolvedCall exact_first[] = {exact, legacy};
    LSMResolvedCall legacy_first[] = {legacy, exact};
    LSMResolvedCallArray first = {exact_first, 2, 2};
    LSMResolvedCallArray second = {legacy_first, 2, 2};

    ASSERT(lsm_pipeline_find_lsp_resolution(&first, &call, true) == &exact_first[0]);
    ASSERT(lsm_pipeline_find_lsp_resolution(&second, &call, true) == &legacy_first[1]);
    PASS();
}

TEST(lsp_tail_duplicate_exact_rows_same_target_are_not_ambiguous) {
    LSMCall call = make_tail_call(600, 612);
    LSMResolvedCall low = make_tail_rc("com.example.Helper.run", 0.75f, 600, 612);
    LSMResolvedCall high = make_tail_rc("com.example.Helper.run", 0.90f, 600, 612);
    LSMResolvedCall items[] = {low, high};
    LSMResolvedCallArray arr = {items, 2, 2};

    ASSERT(lsm_pipeline_find_lsp_resolution(&arr, &call, true) == &items[1]);
    PASS();
}

TEST(lsp_tail_distinct_exact_targets_are_ambiguous) {
    LSMCall call = make_tail_call(700, 712);
    LSMResolvedCall alpha = make_tail_rc("com.example.Alpha.run", 0.75f, 700, 712);
    LSMResolvedCall beta = make_tail_rc("com.example.Beta.run", 0.99f, 700, 712);
    LSMResolvedCall items[] = {alpha, beta};
    LSMResolvedCallArray arr = {items, 2, 2};

    ASSERT_NULL(lsm_pipeline_find_lsp_resolution(&arr, &call, true));
    PASS();
}

TEST(lsp_tail_duplicate_legacy_rows_same_target_are_not_ambiguous) {
    LSMCall call = make_tail_call(800, 812);
    LSMResolvedCall low = make_tail_rc("com.example.Helper.run", 0.75f, 0, 0);
    LSMResolvedCall high = make_tail_rc("com.example.Helper.run", 0.90f, 0, 0);
    LSMResolvedCall items[] = {low, high};
    LSMResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_tail_distinct_legacy_targets_are_ambiguous) {
    LSMCall call = make_tail_call(900, 912);
    LSMResolvedCall alpha = make_tail_rc("com.example.Alpha.run", 0.75f, 0, 0);
    LSMResolvedCall beta = make_tail_rc("com.example.Beta.run", 0.99f, 0, 0);
    LSMResolvedCall items[] = {alpha, beta};
    LSMResolvedCallArray arr = {items, 2, 2};
...
static LSMUsage make_reference_usage(const char *caller, const char *name, uint32_t start,
                                     uint32_t end) {
    LSMUsage usage = {0};
...
static LSMResolvedCall make_reference_rc(const char *caller, const char *callee, float confidence,
                                         uint32_t start, uint32_t end) {
    LSMResolvedCall resolved = make_rc(caller, callee, confidence);
    resolved.kind = LSM_RESOLVED_CALL_REFERENCE;
    resolved.site_start_byte = start;
    resolved.site_end_byte = end;
    return resolved;
...
TEST(lsp_reference_duplicate_exact_rows_same_target_are_not_ambiguous) {
    LSMUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1000, 1007);
    LSMResolvedCall low =
        make_reference_rc("proj.mod.Caller.run", "proj.Target.handler", 0.75f, 1000, 1007);
    LSMResolvedCall high =
        make_reference_rc("proj.mod.Caller.run", "proj.Target.handler", 0.90f, 1000, 1007);
    LSMResolvedCall items[] = {low, high};
    LSMResolvedCallArray arr = {items, 2, 2};

    ASSERT(lsm_pipeline_find_lsp_reference(&arr, &usage, false) == &items[1]);
    PASS();
}
...
TEST(lsp_reference_project_prefixed_duplicate_is_not_ambiguous) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = lsm_gbuf_upsert_node(gbuf, "Function", "handler", "proj.mod.Target.handler",
                                             "target.go", 1, 1, "{}");
    ASSERT_GT(target_id, 0);
    const lsm_gbuf_node_t *raw_target =
        lsm_pipeline_lsp_target_node(gbuf, project, "mod.Target.handler", false);
    const lsm_gbuf_node_t *prefixed_target =
        lsm_pipeline_lsp_target_node(gbuf, project, "proj.mod.Target.handler", false);
    ASSERT_NOT_NULL(raw_target);
    ASSERT_NOT_NULL(prefixed_target);
    ASSERT_EQ(raw_target->id, prefixed_target->id);

    LSMUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1050, 1057);
    LSMResolvedCall raw =
    ...
    const LSMResolvedCall *hit =
        lsm_pipeline_find_lsp_reference_in_graph(&arr, &usage, false, gbuf, project);
    lsm_gbuf_free(gbuf);
    ASSERT(hit == &items[1]);
    PASS();
}
...
TEST(lsp_reference_project_prefix_spellings_stay_ambiguous_when_both_nodes_exist) {
    ...
    int64_t raw_id = lsm_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                          "raw.go", 1, 1, "{}");
    int64_t prefixed_id = lsm_gbuf_upsert_node(
        gbuf, "Function", "handler", "proj.mod.Target.handler", "prefixed.go", 1, 1, "{}");
    ASSERT_GT(raw_id, 0);
    ASSERT_GT(prefixed_id, 0);
    ASSERT(raw_id != prefixed_id);

    LSMUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1070, 1077);
    LSMResolvedCall raw =
...
TEST(lsp_reference_jvm_distinct_packages_do_not_collapse_via_tail_node) {
    ...
    lsm_gbuf_t *gbuf = lsm_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t materialized_id = lsm_gbuf_upsert_node(
        gbuf, "Method", "handler", "proj.generated.Service.handler", "Service.kt", 1, 1, "{}");
    ASSERT_GT(materialized_id, 0);

    LSMUsage usage = make_reference_usage("proj.consumer.Caller.run", "handler", 1080, 1087);
    LSMResolvedCall pkg1 =
        make_reference_rc("proj.consumer.Caller.run", "pkg1.Service.handler", 0.75f, 1080, 1087);
    LSMResolvedCall pkg2 =
        make_reference_rc("proj.consumer.Caller.run", "pkg2.Service.handler", 0.90f, 1080, 1087);
    LSMResolvedCall pkg1_first[] = {pkg1, pkg2};
    LSMResolvedCall pkg2_first[] = {pkg2, pkg1};
    LSMResolvedCallArray first = {pkg1_first, 2, 2};
    LSMResolvedCallArray second = {pkg2_first, 2, 2};

    const lsm_gbuf_node_t *pkg1_tail =
        lsm_pipeline_lsp_target_node(gbuf, project, pkg1.callee_qn, true);
    const lsm_gbuf_node_t *pkg2_tail =
        lsm_pipeline_lsp_target_node(gbuf, project, pkg2.callee_qn, true);
    bool same_tail_node = pkg1_tail && pkg2_tail && pkg1_tail->id == materialized_id &&
                          pkg2_tail->id == materialized_id;
    const LSMResolvedCall *first_hit =
        lsm_pipeline_find_lsp_reference_in_graph(&first, &usage, true, gbuf, project);
    const LSMResolvedCall *second_hit =
        lsm_pipeline_find_lsp_reference_in_graph(&second, &usage, true, gbuf, project);
    lsm_gbuf_free(gbuf);
...
TEST(lsp_reference_distinct_exact_targets_are_order_invariant) {
    LSMUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1100, 1107);
    LSMResolvedCall alpha =
        make_reference_rc("proj.mod.Caller.run", "proj.Alpha.handler", 0.90f, 1100, 1107);
    LSMResolvedCall beta =
        make_reference_rc("proj.mod.Caller.run", "proj.Beta.handler", 0.90f, 1100, 1107);
    LSMResolvedCall alpha_first[] = {alpha, beta};
    LSMResolvedCall beta_first[] = {beta, alpha};
    LSMResolvedCallArray first = {alpha_first, 2, 2};
    LSMResolvedCallArray second = {beta_first, 2, 2};

    const LSMResolvedCall *first_hit = lsm_pipeline_find_lsp_reference(&first, &usage, false);
    const LSMResolvedCall *second_hit = lsm_pipeline_find_lsp_reference(&second, &usage, false);
...
TEST(lsp_reference_duplicate_tail_rows_same_target_are_not_ambiguous) {
    LSMUsage usage = make_reference_usage("proj.raw.Service.handle", "handler", 1200, 1207);
    LSMResolvedCall low =
    ...
    ASSERT(lsm_pipeline_find_lsp_reference(&arr, &usage, true) == &items[1]);
    PASS();
}

TEST(lsp_reference_distinct_tail_targets_are_ambiguous) {
    LSMUsage usage = make_reference_usage("proj.raw.Service.handle", "handler", 1300, 1307);
    LSMResolvedCall alpha =
    ...
    ASSERT_NULL(lsm_pipeline_find_lsp_reference(&arr, &usage, true));
    PASS();
}
...
TEST(lsp_target_node_supports_long_prefixed_qualified_name) {
    ...
    ASSERT_NOT_NULL(gbuf);
    int64_t id =
        lsm_gbuf_upsert_node(gbuf, "Function", "target", qualified, "target.c", 1, 1, "{}");
    ASSERT_GT(id, 0);
    const lsm_gbuf_node_t *found = lsm_pipeline_lsp_target_node(gbuf, project, callee, false);
    if (!found) {
