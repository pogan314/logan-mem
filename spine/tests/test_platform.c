...
#include "../src/foundation/compat.h" /* lsm_setenv / lsm_unsetenv (Windows-portable) */
...
/* Worker staging files land under LSM_CACHE_DIR, which users may place at
...
TEST(platform_file_apis_survive_max_path_overflow) {
    char base[LSM_SZ_512];
    int written = snprintf(base, sizeof(base), "/tmp/lsm-longpath-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(lsm_mkdtemp(base));
    ...
    char file_path[LSM_SZ_1K];
    written = snprintf(file_path, sizeof(file_path), "%s/store.db", deep);
    ASSERT_TRUE(written > 0 && written < (int)sizeof(file_path));
    FILE *file = lsm_fopen(file_path, "wb");
    ASSERT_NOT_NULL(file);
    ASSERT_GTE(fputs("deep\n", file), 0);
    ASSERT_EQ(fclose(file), 0);
    ASSERT_TRUE(lsm_file_exists(file_path));
    ASSERT_TRUE(lsm_is_dir(deep));
    ASSERT_EQ(lsm_unlink(file_path), 0);
    char cursor[LSM_SZ_1K];
    (void)snprintf(cursor, sizeof(cursor), "%s", deep);
    for (int index = 0; index < LONG_SEGMENTS; index++) {
        ASSERT_EQ(lsm_rmdir(cursor), 0);
...
TEST(platform_mkstemp_and_mkdtemp_survive_non_ascii_directory) {
    char base[LSM_SZ_256];
    int written = snprintf(base, sizeof(base), "/tmp/lsm-utf8-̩̬-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(lsm_mkdtemp(base));

    char file_template[LSM_SZ_512];
    ...
    int descriptor = lsm_mkstemp(file_template);
...
typedef struct {
    ...
    int created;
    bool ok;
    char paths[PLATFORM_MKDTEMP_ITERATIONS][LSM_SZ_512];
    ...
    for (int index = 0; index < PLATFORM_MKDTEMP_ITERATIONS; index++) {
        int written =
            snprintf(worker->paths[index], sizeof(worker->paths[index]),
                     "/tmp/lsm-mkdtemp-concurrent-%d-%d-XXXXXX", worker->worker_index, index);
        if (written <= 0 || written >= (int)sizeof(worker->paths[index]) ||
            !lsm_mkdtemp(worker->paths[index])) {
            worker->ok = false;
            break;
        }
        worker->created++;
...
/* MCP daemon sessions can enter lsm_mkdtemp simultaneously. Every request must
 * retain its own expanded path and create a distinct directory. */
TEST(platform_mkdtemp_is_thread_safe) {
    lsm_thread_t threads[PLATFORM_MKDTEMP_THREADS];
    ...
    for (; started < PLATFORM_MKDTEMP_THREADS; started++) {
        ...
        if (lsm_thread_create(&threads[started], 0, platform_mkdtemp_concurrent_worker,
                              &workers[started]) != 0) {
            break;
        }
    }
    ...
    for (int index = 0; index < started; index++) {
        ...
        for (int path_index = 0; path_index < workers[index].created; path_index++) {
            all_ok = all_ok && lsm_is_dir(workers[index].paths[path_index]);
            (void)lsm_rmdir(workers[index].paths[path_index]);
        }
    }
    ASSERT_TRUE(all_ok);
    PASS();
...
TEST(platform_counter_scaling_avoids_intermediate_overflow) {
    ...
    ASSERT(lsm_platform_scale_counter_ns(counter, frequency) == UINT64_C(2000000000000));
    ASSERT(lsm_platform_scale_counter_ns(UINT64_MAX, UINT64_MAX) == UINT64_C(1000000000));
    ASSERT(lsm_platform_scale_counter_ns(UINT64_MAX, 1) == UINT64_MAX);
    PASS();
}
...
TEST(platform_counter_scaling_preserves_monotonic_deadlines) {
    ...
    const uint64_t now_ns = lsm_platform_scale_counter_ns(counter, frequency);
    const uint64_t next_ns = lsm_platform_scale_counter_ns(counter + 1, frequency);
    const uint64_t deadline_ns = lsm_platform_scale_counter_ns(counter + 5 * frequency, frequency);
...
static void *platform_time_first_call(void *opaque) {
    ...
    worker->value = lsm_now_ns();
    return NULL;
}

TEST(platform_now_ns_concurrent_first_call) {
    lsm_thread_t threads[PLATFORM_TIME_THREADS];
    ...
    for (; created < PLATFORM_TIME_THREADS; created++) {
        ...
        if (lsm_thread_create(&threads[created], 0, platform_time_first_call, &workers[created]) !=
            0) {
            break;
        }
    }
...
TEST(platform_now_ns) {
    uint64_t t1 = lsm_now_ns();
    ASSERT_GT(t1, 0);
    /* Busy-wait a tiny bit */
    for (volatile int i = 0; i < 100000; i++) {}
    uint64_t t2 = lsm_now_ns();
    ASSERT_GT(t2, t1);
    PASS();
}

TEST(platform_now_ms) {
    uint64_t t1 = lsm_now_ms();
    ASSERT_GT(t1, 0);
    PASS();
}
...
TEST(platform_file_exists) {
    ...
    ASSERT_TRUE(lsm_file_exists("tests/test_platform.c"));
    ASSERT_FALSE(lsm_file_exists("nonexistent_file_xyz.txt"));
    PASS();
}

TEST(platform_is_dir) {
    ASSERT_TRUE(lsm_is_dir("tests"));
    ASSERT_FALSE(lsm_is_dir("tests/test_platform.c"));
    ASSERT_FALSE(lsm_is_dir("nonexistent_dir"));
    PASS();
}

TEST(platform_file_size) {
    int64_t sz = lsm_file_size("tests/test_platform.c");
    ASSERT_GT(sz, 0);
    ASSERT_EQ(lsm_file_size("nonexistent_file_xyz.txt"), -1);
    PASS();
}
...
TEST(platform_mmap) {
    ...
    size_t sz = 0;
    void *data = lsm_mmap_read("tests/test_platform.c", &sz);
...
TEST(platform_mmap_nonexistent) {
    size_t sz = 0;
    void *data = lsm_mmap_read("nonexistent_xyz.txt", &sz);
    ASSERT_NULL(data);
    PASS();
}
...
static void *platform_capture_path_buffers(void *opaque) {
    ...
    result->home = lsm_get_home_dir();
    result->cache = lsm_resolve_cache_dir();
...
TEST(platform_path_helpers_use_per_thread_storage) {
    ...
    ASSERT_EQ(lsm_setenv("HOME", "/tmp/lsm-platform-thread-home", 1), 0);
    ASSERT_EQ(lsm_setenv("LSM_CACHE_DIR", "/tmp/lsm-platform-thread-cache", 1), 0);
    ...
    bool started0 =
        lsm_thread_create(&threads[0], 0, platform_capture_path_buffers, &results[0]) == 0;
    bool started1 =
        lsm_thread_create(&threads[1], 0, platform_capture_path_buffers, &results[1]) == 0;
    ...
    if (saved_home_copy) {
        (void)lsm_setenv("HOME", saved_home_copy, 1);
    } else {
        (void)lsm_unsetenv("HOME");
    }
    if (saved_cache_copy) {
        (void)lsm_setenv("LSM_CACHE_DIR", saved_cache_copy, 1);
    } else {
...
TEST(platform_cache_dir_rejects_truncated_override) {
    ...
    value[length] = '\0';
    ASSERT_EQ(lsm_setenv("LSM_CACHE_DIR", value, 1), 0);

    const char *resolved = lsm_resolve_cache_dir();

    if (saved_copy) {
        (void)lsm_setenv("LSM_CACHE_DIR", saved_copy, 1);
    } else {
        (void)lsm_unsetenv("LSM_CACHE_DIR");
    }
    free(saved_copy);
...
#ifdef _WIN32
/* lsm_safe_getenv reads Windows' wide environment as UTF-8. Its matching
...
TEST(platform_setenv_preserves_utf8_in_wide_environment) {
    static const char utf8[] = "C:/lsm-cache-\xce\x94-\xe6\x97\xa5\xe6\x9c\xac";
    static const wchar_t wide[] = L"C:/lsm-cache-\u0394-\u65e5\u672c";
    ASSERT_EQ(lsm_setenv("LSM_CACHE_DIR", utf8, 1), 0);
    wchar_t observed_wide[128];
    ASSERT_EQ(GetEnvironmentVariableW(L"LSM_CACHE_DIR", observed_wide, 128), (DWORD)(wcslen(wide)));
    ASSERT_EQ(wcscmp(observed_wide, wide), 0);
    char observed_utf8[128];
    ASSERT_NOT_NULL(lsm_safe_getenv("LSM_CACHE_DIR", observed_utf8, sizeof(observed_utf8), NULL));
 ...
 * particular, an explicitly empty LSM_CACHE_DIR means "use the default"; it
...
TEST(platform_windows_empty_environment_is_read_and_unset_idempotently) {
    ASSERT_EQ(lsm_setenv("LSM_CACHE_DIR", "", 1), 0);
    char observed[9] = "sentinel";
    ASSERT_NOT_NULL(lsm_safe_getenv("LSM_CACHE_DIR", observed, sizeof(observed), "fallback"));
    ASSERT_STR_EQ(observed, "");
    ASSERT_EQ(lsm_unsetenv("LSM_CACHE_DIR"), 0);
    ASSERT_EQ(lsm_unsetenv("LSM_CACHE_DIR"), 0);
    PASS();
}
#endif
...
/*
 * LSM_WORKERS env override for lsm_default_worker_count.
 *
 * Containers running lsm on a host with more CPUs than the cgroup's
...
TEST(platform_default_workers_env_invalid) {
    ...
    int baseline = lsm_default_worker_count(true);
    ASSERT_GT(baseline, 0);

    lsm_setenv("LSM_WORKERS", "0", 1);
    ASSERT_EQ(lsm_default_worker_count(true), baseline);

    lsm_setenv("LSM_WORKERS", "-1", 1);
    ASSERT_EQ(lsm_default_worker_count(true), baseline);

    lsm_setenv("LSM_WORKERS", "9999", 1);
    ASSERT_EQ(lsm_default_worker_count(true), baseline);

    lsm_setenv("LSM_WORKERS", "not-a-number", 1);
    ASSERT_EQ(lsm_default_worker_count(true), baseline);
...
TEST(platform_default_workers_env_unset) {
    /* When LSM_WORKERS is unset the result matches today's behaviour
     * (info.total_cores for initial=true, perf_cores-1 for false). */
    lsm_unsetenv("LSM_WORKERS");
    lsm_system_info_t info = lsm_system_info();
    ASSERT_EQ(lsm_default_worker_count(true), info.total_cores);
    PASS();
}
...
static int cgroup_test_setup(char *root, size_t root_sz) {
    strncpy(root, "/tmp/lsm_cgroup_test_XXXXXX", root_sz);
...
TEST(cgroup_v2_cpu_quota) {
    ...
    ASSERT_EQ(lsm_detect_cgroup_cpus(root), 2);
...
TEST(cgroup_v2_cpu_unlimited) {
    ...
    ASSERT_EQ(lsm_detect_cgroup_cpus(root), -1);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem) {
    ...
    ASSERT_EQ(lsm_detect_cgroup_mem(root), (size_t)2147483648UL);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem_unlimited) {
    ...
    ASSERT_EQ(lsm_detect_cgroup_mem(root), (size_t)0);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v1_mem) {
    ...
    ASSERT_EQ(lsm_detect_cgroup_mem(root), (size_t)1073741824UL);
    cgroup_test_teardown(root);
    PASS();
}
