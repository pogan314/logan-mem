...
static bool cross_repo_fixture_begin(cross_repo_fixture_t *fixture) {
    ...
    snprintf(fixture->cache, sizeof(fixture->cache), "/tmp/lsm-cross-hardening-XXXXXX");
    return lsm_mkdtemp(fixture->cache) != NULL &&
           lsm_setenv("LSM_CACHE_DIR", fixture->cache, 1) == 0;
}

static void cross_repo_fixture_end(cross_repo_fixture_t *fixture) {
    if (fixture->saved_cache) {
        (void)lsm_setenv("LSM_CACHE_DIR", fixture->saved_cache, 1);
    } else {
...
static bool cross_repo_create_project(const cross_repo_fixture_t *fixture, const char *project) {
    ...
    bool ok = lsm_store_upsert_project(store, project, fixture->cache) == LSM_STORE_OK;
...
static bool cross_repo_seed_http_pair(const cross_repo_fixture_t *fixture,
    ...
    bool ok = lsm_store_upsert_project(source, source_project, fixture->cache) == LSM_STORE_OK &&
              lsm_store_upsert_project(target, target_project, fixture->cache) == LSM_STORE_OK;
    char caller_qn[256];
...
static bool cross_repo_exec(const cross_repo_fixture_t *fixture, const char *project,
    ...
    char *error = NULL;
    int rc = sqlite3_exec(lsm_store_get_db(store), sql, NULL, NULL, &error);
    sqlite3_free(error);
    lsm_store_close(store);
    return rc == SQLITE_OK;
}
...
static int cross_repo_count_edges(const cross_repo_fixture_t *fixture, const char *project,
    ...
    int count = lsm_store_count_edges_by_type(store, project, edge_type);
    lsm_store_close(store);
    return count;
}
...
TEST(cross_repo_null_target_fails_without_dereference) {
    ...
    lsm_cross_repo_result_t result = lsm_cross_repo_match("null-target-source", targets, 1);
...
TEST(cross_repo_wildcard_keeps_projects_containing_internal_tokens) {
    ...
    const char *targets[] = {"*"};
    lsm_cross_repo_result_t result = lsm_cross_repo_match("wildcard-source", targets, 1);
    cross_repo_fixture_end(&fixture);
...
static bool cross_repo_seed_bounded_scan(const cross_repo_fixture_t *fixture,
    ...
    int64_t caller_id = ok ? lsm_store_upsert_node(source, &caller) : 0;
    ok = ok && caller_id > 0 &&
         sqlite3_exec(lsm_store_get_db(source), "BEGIN IMMEDIATE", NULL, NULL, NULL) == SQLITE_OK;
    for (int i = 0; ok && i < TEST_SCAN_ROWS; i++) {
        ...
        lsm_node_t local_route = {.project = source_project,
                                  ...
                                  .qualified_name = qn,
                                  .file_path = "client.c"};
        int64_t route_id = lsm_store_upsert_node(source, &local_route);
        lsm_edge_t edge = {
        ...
        ok = route_id > 0 && lsm_store_insert_edge(source, &edge) > 0;
    }
    if (ok) {
        ok = sqlite3_exec(lsm_store_get_db(source), "COMMIT", NULL, NULL, NULL) == SQLITE_OK;
    } else {
        (void)sqlite3_exec(lsm_store_get_db(source), "ROLLBACK", NULL, NULL, NULL);
    }
...
TEST(cross_repo_scan_bound_counts_examined_rows_not_matches) {
    ...
    lsm_cross_repo_result_t result = lsm_cross_repo_match("bounded-source", &target, 1);
    cross_repo_fixture_end(&fixture);
...
TEST(cross_repo_propagates_delete_failure) {
    ...
    lsm_cross_repo_result_t initial = lsm_cross_repo_match("delete-source", &target, 1);
    ...
    lsm_cross_repo_result_t failed = {0};
    if (trigger_created) {
        failed = lsm_cross_repo_match("delete-source", &target, 1);
    }
    cross_repo_fixture_end(&fixture);
...
TEST(cross_repo_failed_bidirectional_insert_is_not_counted) {
    ...
    lsm_cross_repo_result_t result = lsm_cross_repo_match("insert-source", &target, 1);
    cross_repo_fixture_end(&fixture);
...
TEST(cross_repo_cancel_mid_run_keeps_completed_target_and_stops_before_later_target) {
    ...
    lsm_cross_repo_set_after_insert_hook_for_tests(cross_repo_cancel_after_target_write, &hook);
    lsm_cross_repo_result_t result =
        lsm_cross_repo_match_cancellable("cancel-source", targets, 3, &cancelled);
    lsm_cross_repo_set_after_insert_hook_for_tests(NULL, NULL);
...
TEST(cross_repo_pre_cancel_preserves_existing_cross_edges) {
    ...
    lsm_cross_repo_result_t initial = lsm_cross_repo_match("pre-cancel-source", &target, 1);
    int before = cross_repo_count_edges(&fixture, "pre-cancel-source", "CROSS_HTTP_CALLS");
    atomic_int cancelled;
    atomic_init(&cancelled, 1);
    lsm_cross_repo_result_t result =
        lsm_cross_repo_match_cancellable("pre-cancel-source", &target, 1, &cancelled);
...
static bool cross_repo_add_missed_shadow(const cross_repo_fixture_t *fixture, const char *project) {
    ...
    bool ok = lsm_store_upsert_project(store, shadow, fixture->cache) == LSM_STORE_OK;
    lsm_store_close(store);
    return ok;
}
...
TEST(cross_repo_accepts_project_with_missed_shadow_row_issue1609) {
    ...
    lsm_cross_repo_result_t result = lsm_cross_repo_match("shadow-source", &target, 1);
    cross_repo_fixture_end(&fixture);
