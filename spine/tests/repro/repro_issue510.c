 ...
 *   lsm_discover_ex() loads the root .gitignore ONLY when a .git directory is
 ...
 *   When lsm_discover() is called on a directory that is NOT a git repo root
 ...
 *   lsm_discover() returns the excluded file as a normal discovered file
 ...
 *   and writes secret.py + keep.py.  After lsm_discover(), the loop below
 ...
 *   Additionally, lsm_discover_ex() should attempt to load a root .gitignore
 ...
 *   lsm_gitignore_matches(gi, "secret.py", false) == true.
 ...
 *   After lsm_discover(), "secret.py" must NOT appear in the file list.
 *
...
TEST(repro_issue510_nested_gitignore_honored) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/lsm_repro510_XXXXXX", lsm_tmpdir());
    ...
    lsm_gitignore_t *gi = lsm_gitignore_parse("secret.py\n");
    ASSERT_NOT_NULL(gi);
    /* If this assertion fails, the bug is in the gitignore matcher, not
     * in discovery — a different bug, not #510. */
    ASSERT_TRUE(lsm_gitignore_matches(gi, "secret.py", false));
