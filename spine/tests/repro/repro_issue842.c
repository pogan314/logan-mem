 ...
 *
 * Both called lsm_gbuf_find_by_qn(enclosing_func_qn) directly, with no guard
...
static int collect_edge_sources(const RFile *files, int nfiles, const char *node_name,
    ...
    int ncand = 0;
    int rc = lsm_store_find_nodes_by_name(store, lp.project, node_name, &candidates, &ncand);
    ...
    int nedges = 0;
    rc = lsm_store_find_edges_by_target_type(store, target_id, edge_type, &edges, &nedges);
    if (rc != LSM_STORE_OK) {
        rh_cleanup(&lp, store);
        return -1;
    }
 ...
 * direct lsm_gbuf_find_by_qn(enclosing_func_qn) with no dir-container guard,
 ...
 * lsm_gbuf_find_by_qn(enclosing_func_qn) with no dir-container guard and no
