 ...
 *
 * The public LSMDefinition.param_types array is a legacy type-usage projection:
 * it is not positional.  Cross-LSP registration instead receives the internal
 * LSMLSPDef.signature_param_types + signature_param_count carrier.  These tests
...
typedef LSMTypeRegistry *(*CrossRegistryBuilder)(LSMArena *, LSMLSPDef *, int);
...
static TSTree *parse_lsp_fixture(LSMLanguage language, const char *source) {
    const TSLanguage *grammar = lsm_ts_language(language);
...
static LSMTypeRegistry *alloc_test_registry(LSMArena *arena) {
    LSMTypeRegistry *registry = (LSMTypeRegistry *)lsm_arena_alloc(arena, sizeof(*registry));
    if (!registry)
        return NULL;
...
static LSMTypeRegistry *build_java_cross_registry_for_contract(LSMArena *arena, LSMLSPDef *defs,
                                                               int def_count) {
    LSMTypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        lsm_java_register_lsp_defs(arena, registry, defs, def_count);
    return registry;
}

static LSMTypeRegistry *build_kotlin_cross_registry_for_contract(LSMArena *arena, LSMLSPDef *defs,
                                                                 int def_count) {
    LSMTypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        lsm_kotlin_register_lsp_defs(arena, registry, defs, def_count, NULL);
    return registry;
}

static LSMTypeRegistry *build_php_cross_registry_for_contract(LSMArena *arena, LSMLSPDef *defs,
                                                              int def_count) {
    LSMTypeRegistry *registry = alloc_test_registry(arena);
    if (registry)
        lsm_php_register_lsp_defs(arena, NULL, registry, defs, def_count);
    return registry;
}

static int func_param_count(const LSMType *signature) {
    if (!signature || signature->kind != LSM_TYPE_FUNC || !signature->data.func.param_types)
        return 0;
    int count = 0;
...
static const char *type_atom_name(const LSMType *type) {
    if (!type)
        return NULL;
    switch (type->kind) {
...
static int param_matches(const LSMType *actual, const char *expected) {
    if (strcmp(expected, "?") == 0)
        return lsm_type_is_unknown(actual);
    if (!actual || lsm_type_is_unknown(actual))
        return 0;
...
static int check_signature(const LSMTypeRegistry *registry, const char *qualified_name,
                           const char *case_name, const char *const *expected, int expected_count) {
    const LSMRegisteredFunc *func = lsm_registry_lookup_func(registry, qualified_name);
    if (!func) {
        fprintf(stderr, "  [ordered-signature] case=%s missing function=%s\n", case_name,
                qualified_name);
        return 1;
    }
    if (!func->signature || func->signature->kind != LSM_TYPE_FUNC) {
    ...
    for (int i = 0; i < expected_count; i++) {
        const LSMType *actual = func->signature->data.func.param_types[i];
        if (!param_matches(actual, expected[i])) {
            ...
            fprintf(stderr,
                    ...
                    actual_name ? actual_name
                                : (lsm_type_is_unknown(actual) ? "?" : "<structured>"),
...
static int check_single_return(const LSMTypeRegistry *registry, const char *qualified_name,
                               const char *case_name, const char *expected) {
    const LSMRegisteredFunc *func = lsm_registry_lookup_func(registry, qualified_name);
    if (!func || !func->signature || func->signature->kind != LSM_TYPE_FUNC ||
    ...
    const LSMType *actual = func->signature->data.func.return_types[0];
    if (!param_matches(actual, expected)) {
        ...
        fprintf(stderr,
                ...
                actual_name ? actual_name : (lsm_type_is_unknown(actual) ? "?" : "<structured>"),
                (int)actual->kind);
        return 1;
    }
    return 0;
...
static int check_nullable_signature(const LSMTypeRegistry *registry, const char *qualified_name,
    ...
    if (lsm_type_is_unknown(func->signature->data.func.param_types[0])) {
        fprintf(stderr,
...
static void init_function_def(LSMLSPDef *def, LSMArena *arena,
                              const OrderedSignatureLanguage *language, const char *case_name,
                              const char **param_types, int param_count) {
    memset(def, 0, sizeof(*def));
    def->qualified_name = lsm_arena_sprintf(arena, "lsm.ordered.%s.%s", language->tag, case_name);
    def->short_name = case_name;
    def->label = "Function";
    def->def_module_qn = lsm_arena_sprintf(arena, "lsm.ordered.%s", language->tag);
...
static int run_typed_language_contract(const OrderedSignatureLanguage *language) {
    ...
    LSMTypeRegistry *registry = language->build(&arena, defs, 5);
    if (!registry) {
...
static int run_javascript_unknown_contract(const OrderedSignatureLanguage *language) {
    ...
    LSMTypeRegistry *registry = language->build(&arena, defs, 2);
    if (!registry) {
        fprintf(stderr, "  [ordered-signature] lang=%s registry build failed\n", language->tag);
        lsm_arena_destroy(&arena);
        return 1;
...
static const OrderedSignatureLanguage C_LANGUAGE = {
    "c", LSM_LANG_C, lsm_c_build_cross_registry, "int", "string", "Thing *", "void"};
static const OrderedSignatureLanguage CPP_LANGUAGE = {
    "cpp", LSM_LANG_CPP, lsm_c_build_cross_registry, "int", "string", "std::optional<Thing>",
    "void"};
static const OrderedSignatureLanguage CUDA_LANGUAGE = {
    "cuda", LSM_LANG_CUDA, lsm_c_build_cross_registry, "int", "string", "Thing *", "void"};
static const OrderedSignatureLanguage GO_LANGUAGE = {
    "go", LSM_LANG_GO, lsm_go_build_cross_registry, "int", "string", "*Thing", "void"};
static const OrderedSignatureLanguage JS_LANGUAGE = {
    "javascript", LSM_LANG_JAVASCRIPT, lsm_ts_build_cross_registry, NULL, NULL, NULL, "void"};
static const OrderedSignatureLanguage TS_LANGUAGE = {"typescript",
                                                     LSM_LANG_TYPESCRIPT,
                                                     lsm_ts_build_cross_registry,
                                                     "number",
                                                     "string",
                                                     "Thing | null",
                                                     "void"};
static const OrderedSignatureLanguage TSX_LANGUAGE = {
    "tsx", LSM_LANG_TSX, lsm_ts_build_cross_registry, "number", "string", "Thing | null", "void"};
static const OrderedSignatureLanguage PYTHON_LANGUAGE = {
    "python", LSM_LANG_PYTHON, lsm_py_build_cross_registry, "int", "str", "Thing | None", "None"};
static const OrderedSignatureLanguage CSHARP_LANGUAGE = {
    "csharp", LSM_LANG_CSHARP, lsm_cs_build_cross_registry, "int", "string", "Thing?", "void"};
static const OrderedSignatureLanguage RUST_LANGUAGE = {
    "rust", LSM_LANG_RUST, lsm_rust_build_cross_registry, "i32", "String", "Option<Thing>", "()"};
static const OrderedSignatureLanguage JAVA_LANGUAGE = {
    "java", LSM_LANG_JAVA, build_java_cross_registry_for_contract,
    "int",  "String",      "java.util.List<?>",
    "void"};
static const OrderedSignatureLanguage KOTLIN_LANGUAGE = {
    "kotlin", LSM_LANG_KOTLIN, build_kotlin_cross_registry_for_contract, "Int", "String",
    "Thing?", "Unit"};
static const OrderedSignatureLanguage PHP_LANGUAGE = {
    "php", LSM_LANG_PHP, build_php_cross_registry_for_contract, "int", "string", "?Thing", "void"};

static void add_fixture_type(LSMTypeRegistry *registry, const char *qualified_name,
                             const char *short_name) {
    LSMRegisteredType type = {0};
    type.qualified_name = qualified_name;
    type.short_name = short_name;
    lsm_registry_add_type(registry, type);
}

static void add_php_seeded_method(LSMArena *arena, LSMTypeRegistry *registry,
    ...
    const LSMType *param_types[] = {lsm_type_builtin(arena, "int"), lsm_type_named(arena, thing_qn),
                                    lsm_type_builtin(arena, "int"), NULL};
    const LSMType *return_types[] = {return_type, NULL};
    LSMRegisteredFunc function = {0};
    ...
    function.signature = lsm_type_func(arena, param_names, param_types, return_types);
    lsm_registry_add_func(registry, function);
}

static void add_csharp_seeded_method(LSMArena *arena, LSMTypeRegistry *registry,
                                     const char *qualified_name, const char *short_name,
                                     const char *receiver_type, const char *thing_qn) {
    const LSMType *param_types[] = {lsm_type_builtin(arena, "int"), lsm_type_named(arena, thing_qn),
                                    lsm_type_builtin(arena, "int"), NULL};
    const LSMType *return_types[] = {lsm_type_unknown(), NULL};
    LSMRegisteredFunc function = {0};
    function.qualified_name = qualified_name;
    function.short_name = short_name;
    function.receiver_type = receiver_type;
    function.min_params = 3;
    function.signature = lsm_type_func(arena, NULL, param_types, return_types);
    lsm_registry_add_func(registry, function);
}
...
static const LSMType *parse_signature_probe_type(LSMArena *arena, const char *text,
                                                 void *parser_ctx) {
...
TEST(repro_lsp_signature_param_materializer_contract) {
    ...
    const LSMType **ordered = lsm_type_materialize_signature_params(
    ...
    ASSERT_EQ(ordered[0]->kind, LSM_TYPE_BUILTIN);
    ASSERT_STR_EQ(ordered[0]->data.builtin.name, "int");
    ASSERT_TRUE(lsm_type_is_unknown(ordered[1]));
    ASSERT_EQ(ordered[2]->kind, LSM_TYPE_BUILTIN);
    ASSERT_STR_EQ(ordered[2]->data.builtin.name, "string");
    ASSERT_NULL(ordered[3]);

    memset(&probe, 0, sizeof(probe));
    const char *duplicate_texts[] = {"int", "int"};
    const LSMType **duplicates = lsm_type_materialize_signature_params(
    ...
    const LSMType **trailing = lsm_type_materialize_signature_params(
    ...
    ASSERT_TRUE(lsm_type_is_unknown(trailing[2]));
    ASSERT_NULL(trailing[3]);

    memset(&probe, 0, sizeof(probe));
    const char *sentinel_texts[] = {"?", "?Foo", "Foo?", "parser-null", "", NULL};
    const LSMType **sentinels = lsm_type_materialize_signature_params(
    ...
    ASSERT_TRUE(lsm_type_is_unknown(sentinels[0]));
    ASSERT_EQ(sentinels[1]->kind, LSM_TYPE_BUILTIN);
    ASSERT_STR_EQ(sentinels[1]->data.builtin.name, "?Foo");
    ASSERT_EQ(sentinels[2]->kind, LSM_TYPE_BUILTIN);
    ASSERT_STR_EQ(sentinels[2]->data.builtin.name, "Foo?");
    ASSERT_TRUE(lsm_type_is_unknown(sentinels[3]));
    ASSERT_TRUE(lsm_type_is_unknown(sentinels[4]));
    ASSERT_TRUE(lsm_type_is_unknown(sentinels[5]));
    ASSERT_NULL(sentinels[6]);

    memset(&probe, 0, sizeof(probe));
    const LSMType **missing_texts =
        lsm_type_materialize_signature_params(&arena, NULL, 3, parse_signature_probe_type, &probe);
    ASSERT_NOT_NULL(missing_texts);
    ASSERT_EQ(probe.call_count, 0);
    ASSERT_TRUE(lsm_type_is_unknown(missing_texts[0]));
    ASSERT_TRUE(lsm_type_is_unknown(missing_texts[1]));
    ASSERT_TRUE(lsm_type_is_unknown(missing_texts[2]));
    ASSERT_NULL(missing_texts[3]);
    ASSERT_NULL(lsm_type_materialize_signature_params(&arena, ordered_texts, 0,
...
TEST(repro_lsp_func_replace_returns_contract) {
    ...
    const LSMType *integer = lsm_type_builtin(&arena, "int");
    const LSMType *string = lsm_type_builtin(&arena, "string");
    const LSMType *old_return = lsm_type_builtin(&arena, "old-return");
    const LSMType *new_return = lsm_type_builtin(&arena, "new-return");
    const char *param_names[] = {"first", "middle", "last", NULL};
    const LSMType *param_types[] = {integer, lsm_type_unknown(), string, NULL};
    const LSMType *old_returns[] = {old_return, NULL};
    const LSMType *old_signature = lsm_type_func(&arena, param_names, param_types, old_returns);
    ASSERT_NOT_NULL(old_signature);
    ASSERT_EQ(old_signature->kind, LSM_TYPE_FUNC);

    const LSMType *new_returns[] = {new_return, NULL};
    const LSMType *replaced = lsm_type_func_replace_returns(&arena, old_signature, new_returns);
    ASSERT_NOT_NULL(replaced);
    ASSERT_EQ(replaced->kind, LSM_TYPE_FUNC);
    ...
    ASSERT_TRUE(lsm_type_is_unknown(replaced->data.func.param_types[1]));
    ...
    ASSERT_TRUE(lsm_type_is_unknown(old_signature->data.func.param_types[1]));
...
static int run_c_explicit_template_value_signature_contract(const char *source,
    ...
    TSTree *tree = parse_lsp_fixture(LSM_LANG_CPP, source);
    ...
    const LSMType *param_types[] = {lsm_type_builtin(&arena, "long"),
                                    lsm_type_builtin(&arena, "bool"), NULL};
    const LSMType *return_types[] = {lsm_type_type_param(&arena, "T"), NULL};
    const char *type_params[] = {"T", NULL};
    LSMRegisteredFunc registered = {0};
    registered.qualified_name = registered_qn;
    registered.short_name = "materialize";
    registered.signature = lsm_type_func(&arena, param_names, param_types, return_types);
    registered.type_param_names = type_params;
    registered.min_params = 2;
    lsm_registry_add_func(&registry, registered);

    LSMResolvedCallArray out = {0};
    CLSPContext ctx;
    c_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry, NULL,
               /*cpp_mode=*/true, &out);
    const LSMType *value_type = c_eval_expr_type(&ctx, function);

    ASSERT_NOT_NULL(value_type);
    ASSERT_EQ(value_type->kind, LSM_TYPE_FUNC);
    ASSERT_NOT_NULL(value_type->data.func.return_types);
    ASSERT_NOT_NULL(value_type->data.func.return_types[0]);
    ASSERT_EQ(value_type->data.func.return_types[0]->kind, LSM_TYPE_BUILTIN);
    ...
    ASSERT_EQ(value_type->data.func.param_types[0]->kind, LSM_TYPE_BUILTIN);
    ASSERT_STR_EQ(value_type->data.func.param_types[0]->data.builtin.name, "long");
    ASSERT_EQ(value_type->data.func.param_types[1]->kind, LSM_TYPE_BUILTIN);
...
TEST(repro_lsp_ordered_signatures_csharp_cross_return) {
    ...
    LSMTypeRegistry *registry = lsm_cs_build_cross_registry(&arena, &def, 1);
...
TEST(repro_lsp_ordered_signatures_csharp_ast_return_refinement) {
    const char *module_qn = "lsm.ordered.csharp.local";
    const char *subject_qn = "lsm.ordered.csharp.local.Subject";
    const char *thing_qn = "lsm.ordered.csharp.local.Thing";
    const char *result_qn = "lsm.ordered.csharp.local.Result";
    const char *method_qn = "lsm.ordered.csharp.local.Subject.method";
    const char *source =
        "class Thing {}\n"
        "class Result {}\n"
        "class Subject {\n"
        ...
        "}\n";
    TSTree *tree = parse_lsp_fixture(LSM_LANG_CSHARP, source);
    ASSERT_NOT_NULL(tree);
    ...
    lsm_cs_refine_ast_return_types(&ctx, &registry, root);
...
TEST(repro_lsp_ordered_signatures_rust_ast_return_refinement) {
    const char *module_qn = "lsm.ordered.rust.local";
    const char *thing_qn = "lsm.ordered.rust.local.Thing";
    const char *result_qn = "lsm.ordered.rust.local.Result";
    const char *subject_qn = "lsm.ordered.rust.local.Subject";
    const char *top_qn = "lsm.ordered.rust.local.top";
    const char *method_qn = "lsm.ordered.rust.local.Subject.method";
    const char *source =
        "struct Thing;\n"
        "struct Result;\n"
        "struct Subject;\n"
        ...
        "}\n";
    TSTree *tree = parse_lsp_fixture(LSM_LANG_RUST, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));

    const char *params[] = {"i32", "Thing", "i32", NULL};
    LSMDefinition defs[5] = {0};
    defs[0].name = "Thing";
    ...
    lsm_arena_init(&arena);
    LSMTypeRegistry registry;
    lsm_rust_build_local_registry(&arena, &registry, &result, module_qn, root, source);
...
TEST(repro_lsp_ordered_signatures_kotlin_local_registry) {
    ...
    TSTree *tree = parse_lsp_fixture(LSM_LANG_KOTLIN, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));
    ...
    kotlin_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry,
                    "lsm.ordered.kotlin.local", "lsm.ordered.kotlin.local", "lsm", "Local.kt",
                    &out);
    ...
    const char *expected[] = {"Int", "Thing", "Int"};
    int failures = check_signature(&registry, "lsm.ordered.kotlin.local.Holder.method",
                                   "kotlin-local-method", expected, 3);
    failures += check_single_return(&registry, "lsm.ordered.kotlin.local.Holder.method",
                                    "kotlin-local-method", "Result");
    failures +=
        check_signature(&registry, "lsm.ordered.kotlin.local.top", "kotlin-local-top", expected, 3);
    failures += check_single_return(&registry, "lsm.ordered.kotlin.local.top", "kotlin-local-top",
                                    "Result");
...
TEST(repro_lsp_ordered_signatures_php_return_refinement) {
    const char *module_qn = "lsm.ordered.php.local";
    const char *subject_qn = "lsm.ordered.php.local.Subject";
    const char *thing_qn = "lsm.ordered.php.local.Thing";
    const char *result_qn = "lsm.ordered.php.local.Result";
    const char *source =
        "<?php\n"
        "class Thing {}\n"
        "class Result {}\n"
        ...
        "  }\n"
        "}\n";
    TSTree *tree = parse_lsp_fixture(LSM_LANG_PHP, source);
    ASSERT_NOT_NULL(tree);
    TSNode root = ts_tree_root_node(tree);
    ASSERT_FALSE(ts_node_has_error(root));
    ...
    add_php_seeded_method(&arena, &registry, "lsm.ordered.php.local.Subject.declared", "declared",
                          subject_qn, thing_qn, lsm_type_unknown());
    add_php_seeded_method(&arena, &registry, "lsm.ordered.php.local.Subject.documented",
                          "documented", subject_qn, thing_qn, lsm_type_unknown());

    LSMResolvedCallArray out = {0};
    PHPLSPContext ctx;
    php_lsp_init(&ctx, &arena, source, (int)strlen(source), &registry, module_qn, &out);
    lsm_php_refine_lsp_registry(&ctx, &registry, root);

    const char *expected[] = {"int", "Thing", "int"};
    int failures = check_signature(&registry, "lsm.ordered.php.local.Subject.declared",
                                   "php-declared-return-refinement", expected, 3);
    failures += check_single_return(&registry, "lsm.ordered.php.local.Subject.declared",
                                    "php-declared-return-refinement", "Result");
    failures += check_signature(&registry, "lsm.ordered.php.local.Subject.documented",
                                "phpdoc-return-refinement", expected, 3);
    failures += check_single_return(&registry, "lsm.ordered.php.local.Subject.documented",
...
TEST(repro_lsp_ordered_signatures_php_trait_self_refinement) {
    const char *module_qn = "lsm.ordered.php.trait";
    const char *thing_qn = "lsm.ordered.php.trait.Thing";
    const char *trait_qn = "lsm.ordered.php.trait.Fluent";
    const char *consumer_qn = "lsm.ordered.php.trait.Consumer";
    const char *source =
        "<?php\n"
        "class Thing {}\n"
        "trait Fluent {\n"
    ...
    add_php_seeded_method(&arena, &registry, "lsm.ordered.php.trait.Fluent.tap", "tap", trait_qn,
                          thing_qn, lsm_type_named(&arena, trait_qn));
    ...
    const char *expected[] = {"int", "Thing", "int"};
    int failures = check_signature(&registry, "lsm.ordered.php.trait.Consumer.tap",
                                   "php-trait-self-refinement", expected, 3);
    failures += check_single_return(&registry, "lsm.ordered.php.trait.Consumer.tap",
