 ...
 *
 * Issue: #557 -- "lsm v0.8.1 silently deletes project DBs on 'corrupt'
 ...
 *
 *   The sequence is:
 *     1. resolve_store() opens the project DB with lsm_store_open_path_query().
 *     2. It calls lsm_store_check_integrity() (src/store/store.c:664).
 ...
 *     3. On false, resolve_store() calls lsm_unlink(path) at mcp.c:803,
 *        then lsm_unlink(wal_path) and lsm_unlink(shm_path) -- with NO rename,
 ...
 *   After lsm_store_check_integrity() returns false and resolve_store()
 ...
 *   lsm_unlink(path) at mcp.c:803 destroys the DB file.  After resolve_store()
 ...
 *   On buggy code lsm_unlink() runs with no rename, so both conditions are
 ...
 *
 *   1. Set LSM_CACHE_DIR to a temp directory so the DB lands in a controlled
 *      location and does not pollute the real cache.
 *   2. Create the DB via lsm_store_open_path() (creates schema + tables).
 ...
 *      lsm_store_check_integrity() because '8' is not '/', 'A'-'Z', or 'a'-'z'.
 *   4. Close the store, verify the DB file exists (precondition).
 *   5. Call lsm_mcp_handle_tool(srv, "search_graph", ...) with the project
 ...
 *      and executes the destroying lsm_unlink() at mcp.c:803.
 ...
 *   dependent on kill timing or WAL state.  lsm_store_check_integrity() is
 ...
 *   Replace lsm_unlink(path) with a rename to a timestamped .corrupt path,
...
/* Project name used throughout: must pass lsm_validate_project_name().
 * Kept short and slug-safe so it is valid on every platform. */
#define REPRO557_PROJECT "lsm-repro557-test"
...
static int unique_corrupt_main_exists(const char *cache_dir) {
    lsm_dir_t *dir = lsm_opendir(cache_dir);
    if (!dir) {
        return 0;
    }
...
static int is_backup_artifact_name(const char *name) {
    char corrupt_prefix[LSM_DIRENT_NAME_MAX];
    char bak_prefix[LSM_DIRENT_NAME_MAX];
...
TEST(repro_issue557_corrupt_db_not_silently_deleted) {
    /* ── Step 1: redirect LSM_CACHE_DIR to a temp dir ─────────────────
     *
     * lsm_resolve_cache_dir() checks the LSM_CACHE_DIR env var first.
     ...
     *
     * The static buffer in lsm_resolve_cache_dir() is updated on the
     * next call because it re-reads LSM_CACHE_DIR each time.  We must
     * also call lsm_mkdir on the directory before opening the store.
     */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/lsm_repro557_XXXXXX");
    if (!lsm_mkdtemp(tmp_cache)) {
        /* mkdtemp failed -- cannot run the test */
        ASSERT_NOT_NULL(NULL); /* marks setup failure clearly */
    }

    /* Set the env var so all subsequent lsm_resolve_cache_dir() calls
    ...
    /* ── Step 3: create the DB via lsm_store_open_path() ──────────────
     *
     * lsm_store_open_path() calls store_open_internal() with
     ...
     * SQL in lsm_store_check_integrity():
     *
     ...
     * the query returns the row and lsm_store_check_integrity() returns
     * false -- which is the exact trigger for the destroying path.
     *
     * lsm_store_upsert_project() is the store's own public API for
     * writing project rows (used by the pipeline on every full index).
     */
    int rc = lsm_store_upsert_project(setup_store, REPRO557_PROJECT, "826");
    ASSERT_EQ(rc, LSM_STORE_OK); /* precondition: row must be written */
    ...
    /* ── Step 6: drive resolve_store() via lsm_mcp_handle_tool ────────
     *
     * search_graph is the lightest query tool that reaches resolve_store().
     * The tool handler calls resolve_store(srv, project) which:
     *   1. Calls lsm_store_open_path_query(path) -- opens read-write/no-create.
     *      The DB was created in step 3 so SQLITE_OPEN_READWRITE succeeds.
     *   2. Calls lsm_store_check_integrity() -- returns false (root_path="826").
     *   3. Closes the store and calls lsm_unlink(path) at mcp.c:803.
     *      Then lsm_unlink(wal_path) and lsm_unlink(shm_path).
     ...
     *   lsm_unlink(path) at mcp.c:803 removes the file.
     ...
     *
     * On buggy code: both are 0 because lsm_unlink() ran with no backup.
