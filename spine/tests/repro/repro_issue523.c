 ...
 *   `lsm_registry_resolve` resolves the callee name to a candidate QN
 *   containing "requests", but `lsm_gbuf_find_by_qn(ctx->gbuf, res.qualified_name)`
 ...
 *   pass_cross_repo.c finds nothing to iterate over, and `lsm_cross_repo_match`
 ...
 *   Subsequently, `lsm_cross_repo_match` must produce at least one
 ...
 *   lsm_cross_repo_match returns http_edges == 0.  The assertion below is RED.
 *
 ...
 *   lsm_registry_resolve("requests.get", …) returns a candidate QN that
 *   contains "requests" (service-pattern match → LSM_SVC_HTTP), BUT
 *   lsm_gbuf_find_by_qn returns NULL for that QN because no `requests` node
 ...
 *   via lsm_route_canon_path).  A HANDLES edge links the Route to `get_order`.
 */
 ...
 *   3. Run lsm_cross_repo_match(client_project, [server_project], 1).
...
TEST(repro_issue523_crossrepo_http_calls_edge) {
    ...
    RProj client;
    lsm_store_t *client_store = rh_index_files(&client, client_files, N_CLIENT_FILES);
    ...
    lsm_store_close(client_store);
    client_store = NULL; /* re-opened inside lsm_cross_repo_match via cache dir */

    /* ── Index server service ─────────────────────────────────── */
    RProj server;
    lsm_store_t *server_store = rh_index_files(&server, server_files, N_SERVER_FILES);
    ...
    lsm_store_close(server_store);
    server_store = NULL; /* re-opened bidirectionally inside lsm_cross_repo_match */

    /* ── Cross-repo match ─────────────────────────────────────── */
    /*
     * lsm_cross_repo_match opens both project DBs from the cache directory
     * (the same $HOME/.cache/logan-spine-mcp/<project>.db paths that
    ...
    lsm_cross_repo_result_t result = lsm_cross_repo_match(client.project, &server_project, 1);
     ...
     *   lsm_gbuf_find_by_qn returns NULL for the `requests` QN (not indexed).
     ...
     *   lsm_cross_repo_match returns http_edges = 0.
     *
     ...
     * (i.e., skip the lsm_gbuf_find_by_qn guard for LSM_SVC_HTTP / LSM_SVC_ASYNC
 ...
 *      from the call's first string arg. lsm_route_canon_path never strips
 ...
 *      → reverse-direction run in lsm_cross_repo_match()
 ...
 * pipeline. Closes both stores again — lsm_cross_repo_match re-opens the DBs
...
static bool cr536_setup(RProj *client, const char *client_py, RProj *server,
    ...
    lsm_store_t *cs = rh_index(client, "client/orders.py", client_py);
    if (!cs) {
        return false;
    }
    ...
    lsm_store_t *ss = rh_index(server, "server/app.py", server_py);
    if (!ss) {
        return false;
    }
 ...
 * because lsm_cross_repo_match wrote through its own store handles). */
static int cr536_count_cross(RProj *proj) {
    lsm_store_t *st = lsm_store_open_path(proj->dbpath);
    if (!st) {
        return -1;
    }
...
TEST(repro_issue523_scheme_stripped_url_match) {
    ...
    lsm_cross_repo_result_t r = lsm_cross_repo_match(client.project, &tgt, 1);
...
TEST(repro_issue523_reverse_direction_match) {
    ...
    lsm_cross_repo_result_t r = lsm_cross_repo_match(server.project, &tgt, 1);
...
TEST(repro_issue523_idempotent_cross_edges) {
    ...
    lsm_cross_repo_result_t r1 = lsm_cross_repo_match(client.project, &tgt_srv, 1);
    ...
    lsm_cross_repo_result_t r2 = lsm_cross_repo_match(server.project, &tgt_cli, 1);
    ...
    lsm_cross_repo_result_t r3 = lsm_cross_repo_match(client.project, &tgt_srv, 1);
