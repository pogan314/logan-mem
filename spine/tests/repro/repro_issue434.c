 ...
 *
 *     if (repo_path && lsm_artifact_exists(repo_path)) {
 *         lsm_artifact_export(db_path, repo_path, project, LSM_ARTIFACT_FAST);
 *     }
 *
 ...
 *   but lsm_pipeline_run_incremental() calls the local dump_and_persist()
 *   which only checks lsm_artifact_exists(), not the pipeline flag.
 *
 ...
 *   prior artifact MUST create .logan-spine/graph.db.zst after the run.
 *   lsm_artifact_exists(repo_path) MUST return true after the first
 ...
 *   written. lsm_artifact_exists() returns false. Only a SECOND call (when
 ...
 *   repo (no prior artifact). We then assert lsm_artifact_exists() returns
 *   true. On buggy code dump_and_persist() skips the export because
 *   lsm_artifact_exists() was false at the time of the check, so the
 *   assertion fires RED.
 *
 ...
 *   call lsm_artifact_export() when persistence=true, regardless of whether
 *   an artifact already exists. The existing auto-update branch should be
 *   merged with a new persistence-flag branch so that:
 *     if (repo_path && (persistence || lsm_artifact_exists(repo_path))) {
 *         lsm_artifact_export(...);
 *     }
...
TEST(repro_issue434_persistence_honored_on_first_create) {
     ...
     * lsm_mcp_get_bool_arg -> lsm_pipeline_set_persistence -> the pipeline. */
    RProj lp;
    memset(&lp, 0, sizeof(lp));

    /* Create a fresh temp directory for the fixture repo */
    snprintf(lp.tmpdir, sizeof(lp.tmpdir), "/tmp/lsm_repro434_XXXXXX");
    ...
    ASSERT_FALSE(lsm_artifact_exists(lp.tmpdir));
    ...
    if (!lp.srv) {
        th_rmtree(lp.tmpdir);
        FAIL("lsm_mcp_server_new failed");
    }
     ...
     * the artifact MUST exist in .logan-spine/graph.db.zst.
     *
     * On buggy code (pipeline_incremental.c dump_and_persist only checks
     * lsm_artifact_exists() not p->persistence) the artifact is NOT written
     * on the first run, so lsm_artifact_exists() returns false here and this
    ...
    char art_dir[600];
    snprintf(art_dir, sizeof(art_dir), "%s/.logan-spine", lp.tmpdir);
    th_rmtree(art_dir);
    th_rmtree(lp.tmpdir);
    ...
    if (proj) {
        ...
        if (!home) home = "/tmp";
        char dbpath[600];
        snprintf(dbpath, sizeof(dbpath), "%s/.cache/logan-spine-mcp/%s.db",
                 home, proj);
        unlink(dbpath);
        free(proj);
    }
