 ...
 *   lsm_install_vscode_mcp() itself takes a single config_path argument and
 ...
 *   Concretely: after lsm_build_install_plan_json() (the dry-run oracle for
 ...
 *   lsm_build_install_plan_json() runs the real install logic in dry-run mode.
 ...
 *   the plan) for each discovered profile path, using lsm_install_vscode_mcp.
 */
 ...
 * same on both platforms.  We use the portable lsm_app_config_dir() path on
 * non-Apple builds and the Library path on Apple builds so the detection in
 * lsm_detect_agents() actually fires, which is required for the plan to
 ...
 *
 * After lsm_build_install_plan_json(tmpdir, BIN) the plan JSON must contain:
...
TEST(repro_issue431_vscode_profile_inherits_mcp_json) {
    ...
    char tmpdir[512];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_repro431_XXXXXX");
    if (!lsm_mkdtemp(tmpdir))
        FAIL("lsm_mkdtemp failed");

    /* Create the VSCode User dir so lsm_detect_agents() marks vscode=true.
...
#else
    /* Linux: detection uses lsm_app_config_dir() which is XDG-derived.
     * lsm_detect_agents() resolves that internally; we emulate it with
    ...
    if (!agents.vscode) {
         ...
         * lsm_detect_agents() cannot see the fixture home on some platform — in
    ...
    char *plan_json =
        lsm_build_install_plan_json(tmpdir, "/usr/local/bin/logan-spine-mcp");
