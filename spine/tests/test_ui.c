...
TEST(config_load_defaults) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_test_config_XXXXXX");
...
TEST(config_save_and_reload) {
    ...
    /* Save */
    lsm_ui_config_t cfg = {.ui_enabled = true, .ui_port = 8080};
    ASSERT_TRUE(lsm_ui_config_save(&cfg));

    /* Reload */
    lsm_ui_config_t loaded;
    lsm_ui_config_load(&loaded);
...
TEST(config_save_atomically_replaces_a_complete_generation) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/lsm_test_config_atomic_XXXXXX");
    char *td = lsm_mkdtemp(tmpdir);
    ASSERT_NOT_NULL(td);

    char *old_cache = getenv("LSM_CACHE_DIR") ? strdup(getenv("LSM_CACHE_DIR")) : NULL;
    ASSERT_EQ(lsm_setenv("LSM_CACHE_DIR", td, 1), 0);

    lsm_ui_config_t old_generation = {
        .ui_enabled = false,
        .ui_port = 11111,
    };
    ASSERT_TRUE(lsm_ui_config_save(&old_generation));
...
#else
    FILE *old_handle = lsm_fopen(path, "rb");
    ASSERT_NOT_NULL(old_handle);
#endif

    lsm_ui_config_t new_generation = {
        .ui_enabled = true,
        .ui_port = 22222,
    };
    /* Capture everything, clean up, and only then assert: an assert firing
     * before the env restore leaks LSM_CACHE_DIR into every later test in the
     * process, turning one regression into a cascade. */
    bool saved = lsm_ui_config_save(&new_generation);
...
TEST(config_overwrite) {
    ...
    lsm_ui_config_t cfg1 = {.ui_enabled = true, .ui_port = 9749};
    ASSERT_TRUE(lsm_ui_config_save(&cfg1));

    /* Overwrite with ui_enabled=false */
    lsm_ui_config_t cfg2 = {.ui_enabled = false, .ui_port = 9749};
    ASSERT_TRUE(lsm_ui_config_save(&cfg2));
...
TEST(config_corrupt_file) {
    ...
    char dir[1024];
    snprintf(dir, sizeof(dir), "%s/.cache/logan-spine-mcp", td);
...
TEST(embedded_lookup_not_found) {
    ...
    const lsm_embedded_file_t *f = lsm_embedded_lookup("/nonexistent");
    ASSERT_NULL(f);
    PASS();
}
...
TEST(embedded_stub_count) {
    ...
    ASSERT_EQ(LSM_EMBEDDED_FILE_COUNT, 0);
    PASS();
}
...
TEST(layout_empty_graph) {
    ...
    lsm_layout_result_t *r =
        lsm_layout_compute(store, "test-project", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);
...
TEST(layout_single_node) {
    ...
    lsm_store_upsert_project(store, "test", "/tmp/test");
    lsm_node_t node = {
        .project = "test",
        .label = "Function",
        .name = "main",
    ...
    int64_t id = lsm_store_upsert_node(store, &node);
    ASSERT_GT(id, 0);

    lsm_layout_result_t *r = lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);
...
TEST(layout_two_connected) {
    ...
    lsm_node_t n2 = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 5};
    int64_t id1 = lsm_store_upsert_node(store, &n1);
    int64_t id2 = lsm_store_upsert_node(store, &n2);

    lsm_edge_t edge = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    lsm_store_insert_edge(store, &edge);
...
TEST(layout_respects_max_nodes) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        lsm_node_t n = {.project = "test",
                        ...
                        .start_line = i,
                        .end_line = i + 1};
        lsm_store_upsert_node(store, &n);
    }

    /* max_nodes=5 should return at most 5 */
    lsm_layout_result_t *r = lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 5);
    ASSERT_NOT_NULL(r);
...
TEST(layout_clamps_render_cap_from_env) {
    ...
    const char *old_raw = getenv("LSM_UI_MAX_RENDER_NODES");
    char *old_cap = old_raw ? strdup(old_raw) : NULL;
    lsm_setenv("LSM_UI_MAX_RENDER_NODES", "25", 1);
    ...
    lsm_layout_result_t *r = lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 50000);
    ASSERT_NOT_NULL(r);
    ...
    lsm_store_close(store);
    if (old_cap) {
        lsm_setenv("LSM_UI_MAX_RENDER_NODES", old_cap, 1);
        free(old_cap);
    } else {
        lsm_unsetenv("LSM_UI_MAX_RENDER_NODES");
    }
    PASS();
}
...
TEST(layout_honors_budget_above_default) {
    ...
    lsm_layout_result_t *r =
        lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, BUDGET_NODES);
    ASSERT_NOT_NULL(r);
...
TEST(layout_deterministic) {
    ...
    lsm_node_t n2 = {.project = "test",
                     ...
                     .start_line = 1,
                     .end_line = 5};
    lsm_store_upsert_node(store, &n1);
    lsm_store_upsert_node(store, &n2);

    /* Run twice, check positions match */
    lsm_layout_result_t *r1 = lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    lsm_layout_result_t *r2 = lsm_layout_compute(store, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r1);
    ASSERT_NOT_NULL(r2);
...
TEST(layout_to_json) {
    ...
    char *json = lsm_layout_to_json(r);
...
TEST(layout_null_inputs) {
    ...
    lsm_layout_result_t *r = lsm_layout_compute(NULL, "test", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NULL(r);

    /* NULL project → NULL result */
    lsm_store_t *store = lsm_store_open_memory();
    r = lsm_layout_compute(store, NULL, LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NULL(r);

    /* lsm_layout_free(NULL) should not crash */
    lsm_layout_free(NULL);

    /* lsm_layout_to_json(NULL) should return NULL */
    char *json = lsm_layout_to_json(NULL);
    ASSERT_NULL(json);
...
static const lsm_layout_node_t *find_layout_node(const lsm_layout_result_t *r, const char *name) {
...
TEST(layout_dead_code_classification) {
    lsm_store_t *store = lsm_store_open_memory();
    ASSERT_NOT_NULL(store);
    ASSERT_EQ(lsm_store_upsert_project(store, "dc", "/tmp/dc"), LSM_STORE_OK);

    /* Candidates (Function, non-test path unless noted). */
    lsm_node_t dead = {.project = "dc",
                       .label = "Function",
                       .name = "deadfn",
    ...
    int64_t id_dead = lsm_store_upsert_node(store, &dead);
    lsm_store_upsert_node(store, &entry);
    lsm_store_upsert_node(store, &tst);
    lsm_store_upsert_node(store, &tstpath);
    lsm_store_upsert_node(store, &exp);
    int64_t id_single = lsm_store_upsert_node(store, &single);
    int64_t id_norm = lsm_store_upsert_node(store, &norm);
    int64_t id_caller = lsm_store_upsert_node(store, &caller);
    lsm_store_upsert_node(store, &cls);
    ASSERT_GT(id_dead, 0);
    ...
    lsm_edge_t e3 = {.project = "dc", .source_id = id_dead, .target_id = id_norm, .type = "CALLS"};
    lsm_store_insert_edge(store, &e1);
    lsm_store_insert_edge(store, &e2);
    lsm_store_insert_edge(store, &e3);

    lsm_layout_result_t *r = lsm_layout_compute(store, "dc", LSM_LAYOUT_OVERVIEW, NULL, 0, 100);
    ASSERT_NOT_NULL(r);

    const lsm_layout_node_t *ln;
...
static void layout_octree_guard_child(void) {
    ...
    if (!store)
        _exit(2);
    if (lsm_store_upsert_project(store, "test", "/tmp/test") != LSM_STORE_OK)
        _exit(2);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        lsm_node_t n = {.project = "test",
                        ...
                        .start_line = i + 1,
                        .end_line = i + 2};
        if (lsm_store_upsert_node(store, &n) <= 0)
            _exit(2);
    }
