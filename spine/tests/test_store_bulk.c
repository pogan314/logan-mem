 ...
 *
 * Verifies that lsm_store_begin_bulk / lsm_store_end_bulk never switch away
 ...
 * independent of any state held inside the lsm_store_t under test. */
...
static void make_temp_path(char *buf, size_t n) {
    snprintf(buf, n, "%s/cmm_bulk_test_%d.db", lsm_tmpdir(), (int)getpid());
}
...
TEST(bulk_crash_recovery) {
    ...
    lsm_store_t *s = lsm_store_open_path(db_path);
    ASSERT_NOT_NULL(s);
    int rc = lsm_store_upsert_project(s, "baseline", "/tmp/baseline");
    ASSERT_EQ(rc, LSM_STORE_OK);
    lsm_store_close(s);
    ...
    pid_t pid = fork();
    if (pid == 0) {
        lsm_store_t *cs = lsm_store_open_path(db_path);
        if (!cs)
            _exit(1);
        lsm_store_begin_bulk(cs);
        lsm_store_begin(cs); /* explicit open transaction */
        lsm_store_upsert_project(cs, "crashed", "/tmp/crashed");
    ...
    lsm_store_t *recovered = lsm_store_open_path(db_path);
    ASSERT_NOT_NULL(recovered); /* NULL would indicate corruption */

    /* Baseline commit must survive. */
    lsm_project_t p = {0};
    rc = lsm_store_get_project(recovered, "baseline", &p);
    ASSERT_EQ(rc, LSM_STORE_OK);
    ...
    lsm_project_t p2 = {0};
    int rc2 = lsm_store_get_project(recovered, "crashed", &p2);
    ASSERT_NEQ(rc2, LSM_STORE_OK); /* row must be absent */

    lsm_store_close(recovered);
    cleanup_db(db_path);
    PASS();
}
#endif /* _WIN32 */
