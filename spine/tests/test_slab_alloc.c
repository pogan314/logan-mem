...
TEST(slab_malloc_free_large) {
    void *p = lsm_slab_test_malloc(128);
    ASSERT_NOT_NULL(p);
    memset(p, 0xBB, 128);
    lsm_slab_test_free(p);
    PASS();
...
TEST(slab_calloc_zeros) {
    void *p = lsm_slab_test_calloc(10, 8);
    ASSERT_NOT_NULL(p);
    unsigned char *b = p;
    for (int i = 0; i < 80; i++) {
        ASSERT_EQ(b[i], 0);
...
TEST(slab_calloc_overflows) {
    void *p = lsm_slab_test_calloc(SIZE_MAX, 2);
    ASSERT_NULL(p);
    PASS();
}
...
TEST(slab_realloc_null_is_malloc) {
    void *p = lsm_slab_test_realloc(NULL, 64);
    ASSERT_NOT_NULL(p);
    lsm_slab_test_free(p);
    PASS();
}
...
TEST(slab_realloc_shrink_to_zero) {
    void *p = lsm_slab_test_malloc(32);
    ASSERT_NOT_NULL(p);
    void *q = lsm_slab_test_realloc(p, 0); /* frees and returns NULL */
    ASSERT_NULL(q);
    PASS();
}

TEST(slab_realloc_small_keep_same) {
    void *p = lsm_slab_test_malloc(16);
    ASSERT_NOT_NULL(p);
    memset(p, 0xCC, 16);
    /* Realloc within slab size: same pointer, content preserved */
    void *q = lsm_slab_test_realloc(p, 32);
    ASSERT_EQ(p, q);
    unsigned char *b = q;
    ASSERT_EQ(b[0], 0xCC);
    lsm_slab_test_free(q);
    PASS();
}
...
TEST(slab_realloc_promote_to_heap) {
    ...
    void *q = lsm_slab_test_realloc(p, 128);
    ASSERT_NOT_NULL(q);
    unsigned char *b = q;
    ASSERT_EQ(b[0], 0xDD);
    ASSERT_EQ(b[31], 0xDD);
...
TEST(slab_many_small_allocs) {
    void *ptrs[64];
    for (int i = 0; i < 64; i++) {
        ptrs[i] = lsm_slab_test_malloc(SLAB_CHUNK_SIZE);
    ...
    void *again = lsm_slab_test_malloc(SLAB_CHUNK_SIZE);
    ASSERT_NOT_NULL(again);
    lsm_slab_test_free(again);
    PASS();
}
...
TEST(slab_distinct_allocations) {
    void *a = lsm_slab_test_malloc(SLAB_CHUNK_SIZE);
    void *b = lsm_slab_test_malloc(SLAB_CHUNK_SIZE);
    void *c = lsm_slab_test_malloc(SLAB_CHUNK_SIZE);
    ASSERT_NOT_NULL(a);
    ASSERT_NOT_NULL(b);
    ASSERT_NOT_NULL(c);
    ASSERT_NEQ(a, b);
    ASSERT_NEQ(b, c);
    ASSERT_NEQ(a, c);
    lsm_slab_test_free(a);
    lsm_slab_test_free(b);
    lsm_slab_test_free(c);
    PASS();
}
...
TEST(slab_large_alloc_free) {
    void *p = lsm_slab_test_malloc(1024);
    ASSERT_NOT_NULL(p);
    lsm_slab_test_free(p);
    PASS();
}
...
TEST(slab_large_calloc_zero) {
    void *p = lsm_slab_test_calloc(64, 16);
    ASSERT_NOT_NULL(p);
    unsigned char *b = p;
    ASSERT_EQ(b[0], 0);
    ASSERT_EQ(b[1023], 0);
...
TEST(slab_reset_thread) {
    ...
    ASSERT_NOT_NULL(p);
    lsm_slab_test_free(p);
    lsm_slab_reset_thread();
    /* After reset, new allocs should still work */
    void *q = lsm_slab_test_malloc(32);
    ASSERT_NOT_NULL(q);
    lsm_slab_test_free(q);
    PASS();
}
