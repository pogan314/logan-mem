...
static LSMFileResult *extract_kotlin(const char *source) {
    return lsm_extract_file(source, (int)strlen(source), LSM_LANG_KOTLIN, "test", "Main.kt", 0,
                            NULL, NULL);
}

static LSMFileResult *extract_kotlin_path(const char *source, const char *rel_path) {
    return lsm_extract_file(source, (int)strlen(source), LSM_LANG_KOTLIN, "test", rel_path, 0,
                            NULL, NULL);
}
...
static int count_exact_resolved_site(const LSMFileResult *r, const char *callerSub,
                                     const char *calleeSub, uint32_t start, uint32_t end) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const LSMResolvedCall *rc = &r->resolved_calls.items[i];
        if (rc->kind == LSM_RESOLVED_INVOCATION && rc->caller_qn &&
...
static int count_exact_synthetic_call(const LSMFileResult *r, const char *callerSub,
...
static int count_synthetic_call(const LSMFileResult *r, const char *callee) {
    int n = 0;
    for (int i = 0; i < r->calls.count; i++) {
        const LSMCall *call = &r->calls.items[i];
...
TEST(ktlsp_package_set) {
    LSMFileResult *r = extract_kotlin("package com.example.foo\n\n"
...
TEST(ktlsp_explicit_import_function) {
    LSMFileResult *r = extract_kotlin("package com.example\n\n"
...
TEST(ktlsp_top_level_call_default_imports) {
    LSMFileResult *r = extract_kotlin("fun main() { println(\"hi\") }\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_top_level_listof) {
    LSMFileResult *r = extract_kotlin("fun build() { val xs = listOf(1, 2, 3) }\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "build", "listOf"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(ktlsp_top_level_setof_mapof) {
    LSMFileResult *r = extract_kotlin("fun build() {\n"
                                      "    val s = setOf(1, 2, 3)\n"
...
TEST(ktlsp_intra_file_top_level) {
    LSMFileResult *r = extract_kotlin("package myapp\n\n"
...
TEST(ktlsp_class_constructor) {
    LSMFileResult *r = extract_kotlin("class Greeter(val name: String) {\n"
...
TEST(ktlsp_data_class) {
    LSMFileResult *r = extract_kotlin("data class User(val id: Int, val name: String)\n"
...
TEST(ktlsp_secondary_constructor) {
    LSMFileResult *r = extract_kotlin("class Box {\n"
                                      "    constructor() {}\n"
                                      "    constructor(x: Int) {}\n"
                                      "    fun open() {}\n"
                                      "}\n"
...
TEST(ktlsp_method_on_known_type) {
    LSMFileResult *r = extract_kotlin("class Service {\n"
...
TEST(ktlsp_method_chain) {
    LSMFileResult *r = extract_kotlin("class Builder {\n"
...
TEST(ktlsp_string_method_dispatch) {
    LSMFileResult *r = extract_kotlin("fun shout(s: String): String = s.uppercase()\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_list_method_dispatch) {
    LSMFileResult *r = extract_kotlin("fun first_n(xs: List<Int>): List<Int> = xs.take(3)\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_companion_object_call) {
    LSMFileResult *r = extract_kotlin("class Logger {\n"
                                      "    companion object {\n"
...
TEST(ktlsp_object_singleton) {
    LSMFileResult *r = extract_kotlin("object Constants {\n"
...
TEST(ktlsp_extension_method_on_list) {
    LSMFileResult *r = extract_kotlin("fun List<Int>.sumPlus(n: Int): Int = this.sum() + n\n"
...
TEST(ktlsp_super_call) {
    LSMFileResult *r = extract_kotlin("open class Animal {\n"
...
TEST(ktlsp_interface_dispatch) {
    LSMFileResult *r = extract_kotlin("interface Closer { fun close() }\n"
...
TEST(ktlsp_smartcast_is) {
    LSMFileResult *r = extract_kotlin("class Animal { fun speak() {} }\n"
                                      "fun handle(x: Any) {\n"
                                      "    if (x is Animal) {\n"
                                      "        x.speak()\n"
                                      "    }\n"
...
TEST(ktlsp_when_smartcast) {
    LSMFileResult *r = extract_kotlin("interface Shape { fun area(): Double }\n"
...
TEST(ktlsp_safe_call) {
    LSMFileResult *r = extract_kotlin("class User { fun getName(): String = \"\" }\n"
...
TEST(ktlsp_lambda_it_string) {
    LSMFileResult *r = extract_kotlin("fun upper(s: String): String = s.let { it.uppercase() }\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_lambda_it_object) {
    LSMFileResult *r = extract_kotlin("class User { fun login() {} }\n"
...
TEST(ktlsp_scope_apply) {
    LSMFileResult *r = extract_kotlin("class Builder { fun setName(n: String) {} }\n"
...
TEST(ktlsp_typealias) {
    LSMFileResult *r = extract_kotlin("typealias UserId = Int\n"
                                      "fun id(): UserId = 42\n"
...
TEST(ktlsp_enum_class) {
    LSMFileResult *r = extract_kotlin("enum class Color { RED, GREEN, BLUE }\n"
...
TEST(ktlsp_sealed_when) {
    LSMFileResult *r = extract_kotlin("sealed class Event\n"
...
TEST(ktlsp_generic_call) {
    LSMFileResult *r = extract_kotlin("class Box<T>(val value: T) {\n"
                                      "    fun unwrap(): T = value\n"
                                      "}\n"
...
TEST(ktlsp_stdlib_println) {
    LSMFileResult *r = extract_kotlin("fun main() {\n"
                                      "    println(\"a\")\n"
                                      "    print(\"b\")\n"
                                      "    error(\"c\")\n"
                                      "}\n");
...
TEST(ktlsp_stdlib_require_check) {
    LSMFileResult *r = extract_kotlin("fun validate(x: Int) {\n"
                                      "    require(x > 0)\n"
                                      "    check(x < 100)\n"
                                      "    requireNotNull(x)\n"
                                      "}\n");
...
TEST(ktlsp_stdlib_string_chain) {
    LSMFileResult *r = extract_kotlin("fun proc(s: String): String =\n"
...
TEST(ktlsp_constructor_param_property) {
    LSMFileResult *r = extract_kotlin("class Repo(val name: String) {\n"
...
TEST(ktlsp_local_function) {
    LSMFileResult *r = extract_kotlin("fun outer() {\n"
...
TEST(ktlsp_factory_pattern) {
    LSMFileResult *r = extract_kotlin("class Database private constructor() {\n"
                                      "    fun query() {}\n"
                                      "    companion object {\n"
...
TEST(ktlsp_empty_file) {
    LSMFileResult *r = extract_kotlin("");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_unknown_external_unresolved) {
    ...
    LSMFileResult *r = extract_kotlin("fun work(x: SomeUnknownType): UnknownReturn {\n"
                                      "    return x.doMagic()\n"
                                      "}\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    LSMFileResult *r = extract_kotlin(source);
    ASSERT_NOT_NULL(r);
    ...
    const LSMResolvedCall *alpha_joined =
        lsm_pipeline_find_lsp_resolution(&r->resolved_calls, alpha_call, false);
    const LSMResolvedCall *beta_joined =
...
TEST(ktlsp_no_duplicate_emission) {
    ...
    LSMFileResult *r = extract_kotlin("fun main() { println(\"a\"); println(\"b\") }\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_constructor_high_confidence) {
    LSMFileResult *r = extract_kotlin("class Foo\n"
                                      "fun make(): Foo = Foo()\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_method_high_confidence) {
    LSMFileResult *r = extract_kotlin("class Service { fun run() {} }\n"
...
TEST(ktlsp_real_world_builder) {
    LSMFileResult *r = extract_kotlin("class StringBuilder {\n"
...
TEST(ktlsp_real_world_dsl) {
    LSMFileResult *r = extract_kotlin("class Html {\n"
...
TEST(ktlsp_malformed_recovery) {
    ...
    LSMFileResult *r = extract_kotlin("fun broken( {\n}\n");
    ASSERT_NOT_NULL(r);
    lsm_free_result(r);
    PASS();
}
...
TEST(ktlsp_unicode_identifier) {
    LSMFileResult *r = extract_kotlin("class Café {\n"
...
TEST(ktlsp_operator_plus) {
    ...
    LSMFileResult *r = extract_kotlin("class Money(val cents: Int) {\n"
...
TEST(ktlsp_operator_compareTo) {
    LSMFileResult *r = extract_kotlin("class Version(val n: Int) {\n"
...
TEST(ktlsp_operator_in_contains) {
    LSMFileResult *r = extract_kotlin("class Bag(val items: List<String>) {\n"
...
TEST(ktlsp_operator_get) {
    LSMFileResult *r = extract_kotlin("class Cache {\n"
...
TEST(ktlsp_operator_unary) {
    LSMFileResult *r = extract_kotlin("class Vec(val x: Int) {\n"
...
TEST(ktlsp_operator_int_plus) {
    ...
    LSMFileResult *r = extract_kotlin("fun add(a: Int, b: Int): Int = a + b\n");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "add", "plus"), 0);
    lsm_free_result(r);
    PASS();
...
TEST(ktlsp_operator_dedup_comparison_growth_is_linear) {
    ...
    lsm_kotlin_operator_dedup_test_reset();
    LSMFileResult *r = extract_kotlin(source);
    ASSERT_NOT_NULL(r);
    ASSERT_EQ(count_synthetic_call(r, "plus"), OPERATOR_SITE_COUNT);

    uint64_t comparisons = lsm_kotlin_operator_dedup_test_comparisons();
...
TEST(ktlsp_for_iterator_protocol) {
    ...
    LSMFileResult *r = extract_kotlin("fun walk(xs: List<Int>) {\n"
...
TEST(ktlsp_for_range_iterator) {
    LSMFileResult *r = extract_kotlin("fun count() {\n"
...
TEST(ktlsp_destructuring_pair) {
    LSMFileResult *r = extract_kotlin("fun unpack(p: Pair<String, Int>): String {\n"
                                      "    val (a, b) = p\n"
                                      "    return a\n"
                                      "}\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_destructuring_data_class) {
    LSMFileResult *r = extract_kotlin("data class Point(val x: Int, val y: Int)\n"
...
TEST(ktlsp_string_interpolation) {
    ...
    LSMFileResult *r = extract_kotlin("class Item { fun label(): String = \"x\" }\n"
...
TEST(ktlsp_range_rangeTo) {
    ...
    LSMFileResult *r = extract_kotlin("fun mkRange(): IntRange = 1..10\n");
    ASSERT_NOT_NULL(r);
...
TEST(ktlsp_callable_ref_top_level) {
    ...
    LSMFileResult *r = extract_kotlin("fun helper(x: Int): Int = x + 1\n"
                                      "fun main() {\n"
                                      "    val ref = ::helper\n"
                                      "}\n");
    ASSERT_NOT_NULL(r);
