...
TEST(traces_extract_service_name) {
    lsm_trace_attr_t attrs[] = {
        {.key = "service.name", .string_value = "order-service"},
    };
    lsm_trace_resource_t r = {.attributes = attrs, .attr_count = 1};

    const char *name = lsm_extract_service_name(&r);
...
TEST(traces_extract_service_name_null) {
    const char *name = lsm_extract_service_name(NULL);
    ASSERT_STR_EQ(name, "");
    PASS();
}
...
TEST(traces_extract_http_info) {
    ...
    lsm_http_span_info_t info;
    bool ok = lsm_extract_http_info(&span, "svc", &info);
    ASSERT(ok);
...
TEST(traces_extract_service_name_empty_attrs) {
    lsm_trace_resource_t r = {.attributes = NULL, .attr_count = 0};
    const char *name = lsm_extract_service_name(&r);
    ASSERT_STR_EQ(name, "");
    PASS();
}
...
TEST(traces_extract_service_name_multiple_attrs) {
    ...
    lsm_trace_resource_t r = {.attributes = attrs, .attr_count = 3};
    const char *name = lsm_extract_service_name(&r);
...
TEST(traces_extract_path_from_url) {
    ...
    lsm_extract_path_from_url("https://example.com/api/orders", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "/api/orders");

    lsm_extract_path_from_url("http://localhost:8080/health?check=true", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "/health");

    lsm_extract_path_from_url("not-a-url", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "");

    lsm_extract_path_from_url("https://example.com/", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "/");
...
TEST(traces_extract_path_null_url) {
    ...
    const char *result = lsm_extract_path_from_url(NULL, buf, sizeof(buf));
...
TEST(traces_extract_path_empty_url) {
    char buf[256];
    lsm_extract_path_from_url("", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "");
    PASS();
}
...
TEST(traces_extract_path_with_port) {
    char buf[256];
    lsm_extract_path_from_url("https://api.example.com:8443/v2/users", buf, sizeof(buf));
...
TEST(traces_extract_path_with_query_and_fragment) {
    ...
    lsm_extract_path_from_url("https://example.com/search?q=test&page=2", buf, sizeof(buf));
...
TEST(traces_extract_path_relative_path) {
    ...
    char buf[256];
    lsm_extract_path_from_url("/api/orders", buf, sizeof(buf));
    ASSERT_STR_EQ(buf, "");
    PASS();
}
...
TEST(traces_calculate_p99) {
    ...
    int64_t p99 = lsm_calculate_p99(values, 10);
    ASSERT_EQ(p99, 100);
    PASS();
}
...
TEST(traces_calculate_p99_single) {
    int64_t values[] = {42};
    int64_t p99 = lsm_calculate_p99(values, 1);
    ASSERT_EQ(p99, 42);
    PASS();
}

TEST(traces_calculate_p99_empty) {
    int64_t p99 = lsm_calculate_p99(NULL, 0);
    ASSERT_EQ(p99, 0);
    PASS();
}
...
TEST(traces_parse_duration) {
    int64_t d = lsm_parse_duration("1000000000", "1050000000");
    ASSERT_EQ(d, 50000000);
    PASS();
}

TEST(traces_parse_duration_zero) {
    int64_t d = lsm_parse_duration("1000", "500");
...
TEST(traces_parse_duration_null_start) {
    int64_t d = lsm_parse_duration(NULL, "1000");
    ASSERT_EQ(d, 0);
    PASS();
}

TEST(traces_parse_duration_null_end) {
    int64_t d = lsm_parse_duration("1000", NULL);
    ASSERT_EQ(d, 0);
    PASS();
}

TEST(traces_parse_duration_both_null) {
    int64_t d = lsm_parse_duration(NULL, NULL);
    ASSERT_EQ(d, 0);
    PASS();
}
...
TEST(traces_parse_duration_equal) {
    ...
    int64_t d = lsm_parse_duration("5000000000", "5000000000");
    ASSERT_EQ(d, 0);
    PASS();
}
...
TEST(traces_parse_duration_large_values) {
    ...
    int64_t d = lsm_parse_duration("1700000000000000000", "1700000000050000000");
...
TEST(traces_calculate_p99_100_values) {
    ...
    int64_t p99 = lsm_calculate_p99(values, 100);
...
TEST(traces_calculate_p99_two_values) {
    ...
    int64_t p99 = lsm_calculate_p99(values, 2);
...
TEST(traces_calculate_p99_all_same) {
    ...
    int64_t p99 = lsm_calculate_p99(values, 5);
    ASSERT_EQ(p99, 42);
    PASS();
}
