...
static lsm_store_t *setup_arch_test_store(void) {
    ...
    lsm_store_upsert_node(s, &pkg1);
    lsm_store_upsert_node(s, &pkg2);
    lsm_store_upsert_node(s, &pkg3);

    /* Functions with 4-segment QNs */
    lsm_node_t fn_main = {.project = "test",
                          .label = "Function",
                          .name = "main",
                          .qualified_name = "test.cmd.server.main",
                          .file_path = "cmd/server/main.go",
                          .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_main = lsm_store_upsert_node(s, &fn_main);

    lsm_node_t fn_handle = {.project = "test",
                            .label = "Function",
                            .name = "HandleRequest",
                            .qualified_name = "test.internal.handler.HandleRequest",
                            .file_path = "internal/handler/handler.go",
                            .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_handle = lsm_store_upsert_node(s, &fn_handle);

    lsm_node_t fn_process = {.project = "test",
                             .label = "Function",
                             .name = "ProcessOrder",
                             .qualified_name = "test.internal.service.ProcessOrder",
                             .file_path = "internal/service/service.go"};
    int64_t id_process = lsm_store_upsert_node(s, &fn_process);

    lsm_node_t fn_validate = {.project = "test",
                              .label = "Function",
                              .name = "ValidateOrder",
                              .qualified_name = "test.internal.service.ValidateOrder",
                              .file_path = "internal/service/service.go"};
    int64_t id_validate = lsm_store_upsert_node(s, &fn_validate);

    lsm_node_t fn_helper = {.project = "test",
                            .label = "Function",
                            .name = "formatDate",
                            .qualified_name = "test.internal.service.formatDate",
                            .file_path = "internal/service/service.go"};
    int64_t id_helper = lsm_store_upsert_node(s, &fn_helper);

    /* Test function (should be excluded) */
    lsm_node_t fn_test = {.project = "test",
                          .label = "Function",
                          ...
                          .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_test = lsm_store_upsert_node(s, &fn_test);
...
TEST(arch_get_all) {
    lsm_store_t *s = setup_arch_test_store();
    lsm_architecture_info_t info;
    ASSERT_EQ(lsm_store_get_architecture(s, "test", NULL, NULL, 0, &info), LSM_STORE_OK);
    ...
    lsm_store_architecture_free(&info);
    lsm_store_close(s);
    PASS();
}
...
TEST(arch_entry_points_exclude_tests) {
    ...
    ASSERT_EQ(lsm_store_get_architecture(s, "test", NULL, aspects, 1, &info), LSM_STORE_OK);
...
TEST(arch_specific_aspects) {
    ...
    ASSERT_EQ(lsm_store_get_architecture(s, "test", NULL, aspects, 2, &info), LSM_STORE_OK);
...
TEST(arch_path_scoping) {
    lsm_store_t *s = lsm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(lsm_store_upsert_project(s, "pscope", "/tmp/pscope"), LSM_STORE_OK);

    lsm_node_t f1 = {.project = "pscope",
                     .label = "File",
                     .name = "a.go",
                     .qualified_name = "pscope.apps.foo.a.go",
                     .file_path = "apps/foo/a.go"};
    lsm_node_t f2 = {.project = "pscope",
                     .label = "File",
                     .name = "b.go",
                     .qualified_name = "pscope.other.b.go",
                     .file_path = "other/b.go"};
    lsm_store_upsert_node(s, &f1);
    lsm_store_upsert_node(s, &f2);

    lsm_node_t fn_foo = {.project = "pscope",
                         .label = "Function",
                         .name = "Foo",
                         .qualified_name = "pscope.apps.foo.Foo",
                         .file_path = "apps/foo/a.go"};
    lsm_node_t fn_other = {.project = "pscope",
                           .label = "Function",
                           .name = "Bar",
                           .qualified_name = "pscope.other.Bar",
                           .file_path = "other/b.go"};
    lsm_store_upsert_node(s, &fn_foo);
    lsm_store_upsert_node(s, &fn_other);

    const char *aspects[] = {"languages", "packages"};
    lsm_architecture_info_t whole;
    memset(&whole, 0, sizeof(whole));
    ASSERT_EQ(lsm_store_get_architecture(s, "pscope", NULL, aspects, 2, &whole), LSM_STORE_OK);

    lsm_architecture_info_t scoped;
    memset(&scoped, 0, sizeof(scoped));
    ASSERT_EQ(lsm_store_get_architecture(s, "pscope", "apps/foo", aspects, 2, &scoped),
              LSM_STORE_OK);
    ...
    ASSERT_TRUE(lsm_store_count_nodes(s, "pscope") > lsm_store_count_nodes_scoped(s, "pscope", "apps/foo"));

    lsm_architecture_info_t scoped_slash;
    memset(&scoped_slash, 0, sizeof(scoped_slash));
    ASSERT_EQ(lsm_store_get_architecture(s, "pscope", "apps/foo/", aspects, 2, &scoped_slash),
              LSM_STORE_OK);
    int slash_go = 0;
    ...
    lsm_store_architecture_free(&scoped_slash);
    lsm_store_architecture_free(&whole);
    lsm_store_architecture_free(&scoped);
    lsm_store_close(s);
    PASS();
}
...
TEST(arch_empty_project) {
    lsm_store_t *s = lsm_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(lsm_store_upsert_project(s, "empty", "/tmp/empty"), LSM_STORE_OK);

    lsm_architecture_info_t info;
    const char *aspects[] = {"all"};
    ASSERT_EQ(lsm_store_get_architecture(s, "empty", NULL, aspects, 1, &info), LSM_STORE_OK);
...
static double timed_boundaries_ms(int n_nodes, int n_edges, int n_pkgs) {
    ...
    lsm_store_upsert_project(s, "perf", "/tmp/perf");
    ...
    int rc = lsm_store_get_architecture(s, "perf", NULL, aspects, 1, &info);
...
TEST(adr_store_and_retrieve) {
    ...
    ASSERT_EQ(lsm_store_adr_store(s, "test", content), LSM_STORE_OK);

    lsm_adr_t adr;
    ASSERT_EQ(lsm_store_adr_get(s, "test", &adr), LSM_STORE_OK);
...
TEST(adr_upsert) {
    ...
    ASSERT_EQ(lsm_store_adr_store(s, "test", "v1"), LSM_STORE_OK);
    ASSERT_EQ(lsm_store_adr_store(s, "test", "v2"), LSM_STORE_OK);
...
TEST(adr_delete) {
    ...
    ASSERT_EQ(lsm_store_adr_store(s, "test", "## PURPOSE\nTest"), LSM_STORE_OK);
    ASSERT_EQ(lsm_store_adr_delete(s, "test"), LSM_STORE_OK);

    lsm_adr_t adr;
    ASSERT_TRUE(lsm_store_adr_get(s, "test", &adr) != LSM_STORE_OK);
...
TEST(adr_delete_not_found) {
    ...
    ASSERT_TRUE(lsm_store_adr_delete(s, "nonexistent") != LSM_STORE_OK);
...
TEST(adr_parse_sections_basic) {
    lsm_adr_sections_t sec = lsm_adr_parse_sections("## PURPOSE\nFoo\n\n## STACK\nBar");
    ...
    lsm_adr_sections_free(&sec);
    PASS();
}

TEST(adr_parse_sections_all_six) {
    lsm_adr_sections_t sec =
        lsm_adr_parse_sections("## PURPOSE\nA\n\n## STACK\nB\n\n## ARCHITECTURE\nC\n\n## "
                               "PATTERNS\nD\n\n## TRADEOFFS\nE\n\n## PHILOSOPHY\nF");
...
TEST(adr_parse_sections_empty) {
    lsm_adr_sections_t sec = lsm_adr_parse_sections("");
...
TEST(adr_parse_sections_preamble) {
    lsm_adr_sections_t sec = lsm_adr_parse_sections("preamble\n## PURPOSE\nFoo");
...
TEST(adr_render_all_sections) {
    lsm_adr_sections_t sec = {.count = 6};
...
TEST(adr_render_non_canonical) {
    lsm_adr_sections_t sec = {.count = 3};
...
TEST(adr_render_empty) {
    lsm_adr_sections_t sec = {.count = 0};
    char *rendered = lsm_adr_render(&sec);
...
TEST(adr_parse_render_roundtrip) {
    const char *original =
        ...
        "## TRADEOFFS\n- Speed over features\n\n## PHILOSOPHY\n- Keep it simple";
    lsm_adr_sections_t sec = lsm_adr_parse_sections(original);
    char *rendered = lsm_adr_render(&sec);
...
TEST(adr_update_sections) {
    ...
    lsm_adr_t updated;
    ASSERT_EQ(lsm_store_adr_update_sections(s, "test", keys, values, 1, &updated), LSM_STORE_OK);

    /* Verify all sections preserved */
    lsm_adr_sections_t sec = lsm_adr_parse_sections(updated.content);
...
TEST(adr_update_overflow) {
    ...
    /* Create huge content */
    char *huge = malloc(LSM_ADR_MAX_LENGTH + 2);
    memset(huge, 'x', LSM_ADR_MAX_LENGTH + 1);
    huge[LSM_ADR_MAX_LENGTH + 1] = '\0';

    const char *keys[] = {"STACK"};
    const char *values[] = {huge};
    lsm_adr_t out;
    ASSERT_TRUE(lsm_store_adr_update_sections(s, "test", keys, values, 1, &out) != LSM_STORE_OK);
...
TEST(adr_validate_all_sections) {
    const char *content = "## PURPOSE\nA\n\n## STACK\nB\n\n## ARCHITECTURE\nC\n\n## "
                          "PATTERNS\nD\n\n## TRADEOFFS\nE\n\n## PHILOSOPHY\nF";
    char errbuf[256];
    ASSERT_EQ(lsm_adr_validate_content(content, errbuf, sizeof(errbuf)), LSM_STORE_OK);
    PASS();
}
...
TEST(adr_validate_missing_sections) {
    const char *content = "## PURPOSE\nA\n\n## STACK\nB";
    char errbuf[512];
    ASSERT_TRUE(lsm_adr_validate_content(content, errbuf, sizeof(errbuf)) != LSM_STORE_OK);
...
TEST(adr_validate_empty) {
    char errbuf[256];
    ASSERT_TRUE(lsm_adr_validate_content("", errbuf, sizeof(errbuf)) != LSM_STORE_OK);
    PASS();
}
...
TEST(adr_validate_keys_valid) {
    ...
    char errbuf[256];
    ASSERT_EQ(lsm_adr_validate_section_keys(keys, 2, errbuf, sizeof(errbuf)), LSM_STORE_OK);
    PASS();
}
...
TEST(adr_validate_keys_invalid) {
    ...
    char errbuf[256];
    ASSERT_TRUE(lsm_adr_validate_section_keys(keys, 3, errbuf, sizeof(errbuf)) != LSM_STORE_OK);
...
TEST(adr_validate_keys_empty) {
    char errbuf[256];
    ASSERT_EQ(lsm_adr_validate_section_keys(NULL, 0, errbuf, sizeof(errbuf)), LSM_STORE_OK);
    PASS();
}
...
TEST(louvain_basic) {
    ...
    lsm_louvain_edge_t edges[] = {{1, 2}, {2, 3}, {1, 3}, {4, 5}};
    lsm_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(lsm_louvain(nodes, 5, edges, 4, &result, &count), LSM_STORE_OK);
    ASSERT_EQ(count, 5);
...
TEST(louvain_empty) {
    lsm_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(lsm_louvain(NULL, 0, NULL, 0, &result, &count), LSM_STORE_OK);
    ASSERT_EQ(count, 0);
    free(result);
    PASS();
}
...
TEST(louvain_single_node) {
    ...
    lsm_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(lsm_louvain(nodes, 1, NULL, 0, &result, &count), LSM_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(louvain_converges) {
    ...
    for (int i = 1; i <= 10; i++) {
        for (int j = i + 1; j <= 10; j++) {
            edges[nedges++] = (lsm_louvain_edge_t){i, j};
        }
    }
    ...
    /* Bridge */
    edges[nedges++] = (lsm_louvain_edge_t){5, 15};

    lsm_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(lsm_louvain(nodes, 20, edges, nedges, &result, &count), LSM_STORE_OK);
    ASSERT_EQ(count, 20);
...
static int leiden_count_communities(const lsm_louvain_result_t *r, int n) {
...
static bool leiden_all_communities_connected(const lsm_louvain_result_t *r, int n,
                                             const lsm_louvain_edge_t *edges, int ne) {
...
TEST(leiden_multilevel_collapses_noise) {
    ...
    lsm_louvain_edge_t edges[CL * (SZ * (SZ - 1) / 2) + CL];
    int ne = 0;
    for (int c = 0; c < CL; c++) {
        int base = c * SZ + 1;
        for (int i = 0; i < SZ; i++) {
            for (int j = i + 1; j < SZ; j++) {
                edges[ne++] = (lsm_louvain_edge_t){base + i, base + j};
            }
        }
    }
    for (int c = 0; c + 1 < CL; c++) {
        edges[ne++] = (lsm_louvain_edge_t){c * SZ + 1, (c + 1) * SZ + 1};
    }

    lsm_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(lsm_louvain(nodes, N, edges, ne, &result, &count), LSM_STORE_OK);
    ASSERT_EQ(count, N);
...
TEST(leiden_resolution_controls_granularity) {
    ...
    lsm_louvain_edge_t edges[N - 1];
    int ne = 0;
    for (int i = 0; i + 1 < N; i++) {
        edges[ne++] = (lsm_louvain_edge_t){i + 1, i + 2};
    }

    lsm_louvain_result_t *lo = NULL;
    lsm_louvain_result_t *hi = NULL;
    int lc = 0;
    int hc = 0;
    ASSERT_EQ(lsm_leiden(nodes, N, edges, ne, 0.1, &lo, &lc), LSM_STORE_OK);
    ASSERT_EQ(lsm_leiden(nodes, N, edges, ne, 5.0, &hi, &hc), LSM_STORE_OK);
    ASSERT_EQ(lc, N);
    ASSERT_EQ(hc, N);
...
TEST(arch_clusters_basic) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        lsm_node_t node = {.project = "test",
                           ...
                           .qualified_name = qn,
                           .file_path = "f.go"};
        id[i] = lsm_store_upsert_node(s, &node);
    }
    for (int g = 0; g < 2; g++) {
        for (int a = 0; a < 4; a++) {
            for (int b = a + 1; b < 4; b++) {
    ...
    lsm_store_insert_edge(s, &bridge);
...
TEST(qn_to_package) {
    ...
    ASSERT_STR_EQ(lsm_qn_to_package("project.internal.store.search.Search"), "store");
    ASSERT_STR_EQ(lsm_qn_to_package("project.src.utils.helper.foo"), "utils");
    ASSERT_STR_EQ(lsm_qn_to_package("project.src.components.Button.render"), "components");
    ASSERT_STR_EQ(lsm_qn_to_package("project.cmd.server.main"), "server");
    /* 3 segments: falls back to segment[1] */
    ASSERT_STR_EQ(lsm_qn_to_package("project.main.foo"), "main");
    ASSERT_STR_EQ(lsm_qn_to_package("project.cmd"), "cmd");
    /* Edge cases */
    ASSERT_STR_EQ(lsm_qn_to_package("standalone"), "");
    ASSERT_STR_EQ(lsm_qn_to_package(""), "");
    PASS();
}

TEST(qn_to_top_package) {
    ASSERT_STR_EQ(lsm_qn_to_top_package("project.internal.store.search.Search"), "internal");
    ASSERT_STR_EQ(lsm_qn_to_top_package("project.src.components.Button"), "src");
    ASSERT_STR_EQ(lsm_qn_to_top_package("project.cmd"), "cmd");
    ASSERT_STR_EQ(lsm_qn_to_top_package("standalone"), "");
    PASS();
}

TEST(is_test_file_path) {
    ASSERT_TRUE(!lsm_is_test_file_path("internal/handler/handler.go"));
    ASSERT_TRUE(lsm_is_test_file_path("src/__tests__/handler.test.ts"));
    ASSERT_TRUE(lsm_is_test_file_path("src/test/java/com/example/Test.java"));
    ASSERT_TRUE(lsm_is_test_file_path("tests/test_handler.py"));
    ASSERT_TRUE(lsm_is_test_file_path("testdata/fixture.json"));
    ASSERT_TRUE(!lsm_is_test_file_path(""));
    PASS();
}
...
TEST(find_architecture_docs) {
    ...
    char **docs = NULL;
    int count = 0;
    ASSERT_EQ(lsm_store_find_architecture_docs(s, "test", &docs, &count), LSM_STORE_OK);
    ASSERT_EQ(count, 2);
...
TEST(search_case_insensitive_default) {
    ...
    ASSERT_EQ(lsm_store_search(s, &params, &out), LSM_STORE_OK);
