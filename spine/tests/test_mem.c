...
#define LSM_ASAN_ACTIVE 1
#else
#define LSM_ASAN_ACTIVE 0
#endif
...
TEST(mem_collect_reclaims) {
    ...
    size_t rss_before_free = lsm_mem_rss();

    free(p);
    lsm_mem_collect();

    size_t rss_after_collect = lsm_mem_rss();
...
TEST(mem_budget_check) {
    ...
    /* NOTE: lsm_mem_init only takes effect once, so we test with whatever
    ...
    size_t wb4 = lsm_mem_worker_budget(4);
    ASSERT_EQ(wb4, budget / 4);

    /* Edge case: 0 workers defaults to 1 */
    size_t wb0 = lsm_mem_worker_budget(0);
...
TEST(mem_worker_budget_zero_workers) {
    ...
    size_t wb = lsm_mem_worker_budget(0);
    ASSERT_EQ(wb, budget);
    PASS();
}
...
TEST(mem_worker_budget_negative_workers) {
    ...
    size_t wb = lsm_mem_worker_budget(-5);
    ASSERT_EQ(wb, budget);
    PASS();
}
...
TEST(mem_worker_budget_one_worker) {
    ...
    size_t wb = lsm_mem_worker_budget(1);
    ASSERT_EQ(wb, budget);
    PASS();
}
...
TEST(mem_worker_budget_many_workers) {
    ...
    size_t wb = lsm_mem_worker_budget(1000);
    ASSERT_GT(wb, 0);
    /* Must be budget / 1000 */
    ASSERT_EQ(wb, lsm_mem_budget() / 1000);
    PASS();
}
...
TEST(mem_ram_fraction_16gb_tier) {
    ...
    ASSERT_EQ(lsm_mem_ram_fraction_for_total(ram_16gb), 0.25);
    ASSERT_EQ(lsm_mem_ram_fraction_for_total(ram_16gb - 1), 0.25);
    PASS();
}
...
TEST(mem_ram_fraction_32gb_tier) {
    ...
    ASSERT_EQ(lsm_mem_ram_fraction_for_total(ram_17gb), 0.35);
    ASSERT_EQ(lsm_mem_ram_fraction_for_total(ram_32gb), 0.35);
    PASS();
}

TEST(mem_ram_fraction_large_host) {
    size_t ram_64gb = 64ULL * 1024 * 1024 * 1024;
    ASSERT_EQ(lsm_mem_ram_fraction_for_total(ram_64gb), 0.5);
    PASS();
}
...
TEST(mem_peak_rss_gte_rss) {
     ...
     * statm-vs-ru_maxrss source mismatch: lsm_mem_rss() reads the live
     ...
     * invariant. lsm_mem_peak_rss() now reconciles the two sources. Touch a
...
/* Reproduce-first guard for the Linux lsm_mem_rss() undercount (distilled
 * from #776's 132460f5).
 *
 ...
 * (stats.c:555). The UNFIXED lsm_mem_rss() returns that counter whenever it is
 ...
 * On UNFIXED Linux, lsm_mem_rss() then returns the ~few-MB committed counter
 ...
 * include the mapped+touched region, so lsm_mem_rss() is accurate there both
...
TEST(mem_rss_reflects_external_resident_memory) {
...
#ifdef _WIN32
    /* On Windows lsm_mem_rss() reads WorkingSetSize (GetProcessMemoryInfo),
     ...
     * warm buffer. This still guards the real regression — lsm_mem_rss()
...
/* ── Init edge cases ──────────────────────────────────────────── */
/* NOTE: lsm_mem_init uses atomic CAS — only the very first call in the
...
TEST(mem_init_zero_fraction) {
    ...
    size_t budget_before = lsm_mem_budget();
    lsm_mem_init(0.0);
    size_t budget_after = lsm_mem_budget();
...
TEST(mem_init_second_call_noop) {
    size_t budget_before = lsm_mem_budget();
    lsm_mem_init(0.9); /* different fraction — but it's a no-op */
    size_t budget_after = lsm_mem_budget();
    ASSERT_EQ(budget_before, budget_after);
    PASS();
}

/* ── LSM_MEM_BUDGET_MB budget override (pure resolver) ────────────
 * lsm_mem_init is one-shot per process, so the override logic lives in the
 * pure lsm_mem_resolve_budget() helper which we can exercise directly. */

#define LSM_TEST_MB ((size_t)1024 * 1024)

TEST(resolve_budget_no_override_uses_fraction) {
    /* No env override → ram_fraction × total_ram, source=ram_fraction. */
    size_t total = 8192 * LSM_TEST_MB;
    lsm_mem_budget_t r = lsm_mem_resolve_budget(total, 0.5, NULL);
    ASSERT_EQ(r.budget, 4096 * LSM_TEST_MB);
    ASSERT_STR_EQ(r.source, "ram_fraction");
    ASSERT_FALSE(r.clamped);
    ASSERT_FALSE(r.invalid);
    ASSERT_EQ(lsm_mem_resolve_budget(total, 0.25, "").budget, 2048 * LSM_TEST_MB);
    PASS();
}
...
TEST(resolve_budget_invalid_fraction_defaults) {
    ...
    size_t total = 8192 * LSM_TEST_MB;
    ASSERT_EQ(lsm_mem_resolve_budget(total, 0.0, NULL).budget, 4096 * LSM_TEST_MB);
    ASSERT_EQ(lsm_mem_resolve_budget(total, -1.0, NULL).budget, 4096 * LSM_TEST_MB);
    ASSERT_EQ(lsm_mem_resolve_budget(total, 1.5, NULL).budget, 4096 * LSM_TEST_MB);
    PASS();
}
...
TEST(resolve_budget_override_wins) {
    ...
    size_t total = 8192 * LSM_TEST_MB;
    lsm_mem_budget_t below = lsm_mem_resolve_budget(total, 0.5, "2048");
    ASSERT_EQ(below.budget, 2048 * LSM_TEST_MB);
    ASSERT_STR_EQ(below.source, "LSM_MEM_BUDGET_MB");
    ASSERT_FALSE(below.clamped);
    ASSERT_FALSE(below.invalid);
    /* Override above the fraction default is also honored (up to total_ram). */
    ASSERT_EQ(lsm_mem_resolve_budget(total, 0.5, "6144").budget, 6144 * LSM_TEST_MB);
    PASS();
}

TEST(resolve_budget_override_clamped_to_total) {
    /* Override larger than physical/cgroup RAM clamps to total_ram. */
    size_t total = 1024 * LSM_TEST_MB;
    lsm_mem_budget_t r = lsm_mem_resolve_budget(total, 0.5, "100000");
    ASSERT_EQ(r.budget, total);
    ASSERT_TRUE(r.clamped);
    ASSERT_STR_EQ(r.source, "LSM_MEM_BUDGET_MB");
    PASS();
}
...
TEST(resolve_budget_override_when_total_unknown) {
    ...
    lsm_mem_budget_t r = lsm_mem_resolve_budget(0, 0.5, "512");
    ASSERT_EQ(r.budget, 512 * LSM_TEST_MB);
    ASSERT_FALSE(r.clamped);
    ASSERT_FALSE(r.invalid);
    PASS();
}
...
TEST(resolve_budget_worker_cap_preserves_lower_user_override) {
    size_t total = 8192 * LSM_TEST_MB;
    size_t worker_cap = 16 * LSM_TEST_MB;
    lsm_mem_budget_t lower = lsm_mem_resolve_budget_capped(total, 0.5, "8", worker_cap);
    ASSERT_EQ(lower.budget, 8 * LSM_TEST_MB);
    ASSERT_STR_EQ(lower.source, "LSM_MEM_BUDGET_MB");
    ASSERT_FALSE(lower.hard_capped);

    lsm_mem_budget_t capped = lsm_mem_resolve_budget_capped(total, 0.5, "64", worker_cap);
...
TEST(resolve_budget_invalid_override_falls_back) {
    ...
    size_t total = 8192 * LSM_TEST_MB;
    size_t fraction_budget = 4096 * LSM_TEST_MB;
    const char *bad[] = {
        "abc", "0", "-512", "512MB", "512x", "0x400", "99999999999999999999999999",
    };
    for (size_t i = 0; i < sizeof(bad) / sizeof(bad[0]); i++) {
        lsm_mem_budget_t r = lsm_mem_resolve_budget(total, 0.5, bad[i]);
 ...
 * so a naive clamp misses it), pinning lsm_mem_over_budget() permanently true.
 * The MiB-space clamp must instead clamp to total_ram. */
TEST(resolve_budget_override_overflow_clamps_to_total) {
    size_t total = 2048 * LSM_TEST_MB;
    /* 2^44 MiB: (size_t)2^44 * (2^20 bytes/MiB) == 2^64 == 0 on wrap. */
    lsm_mem_budget_t r = lsm_mem_resolve_budget(total, 0.5, "17592186044416");
    ASSERT_EQ(r.budget, total);
    ASSERT_TRUE(r.clamped);
    ASSERT_FALSE(r.invalid);
    PASS();
...
TEST(resolve_budget_override_overflow_total_unknown_caps) {
    ...
    lsm_mem_budget_t r = lsm_mem_resolve_budget(0, 0.5, "99999999999999999");
...
TEST(arena_alloc_and_destroy) {
    ...
    ASSERT_EQ(a.block_sizes[0], LSM_ARENA_DEFAULT_BLOCK_SIZE);

    char *s = lsm_arena_strdup(&a, "hello mem integration");
    ASSERT_NOT_NULL(s);
...
TEST(arena_grow_tracks_sizes) {
    LSMArena a;
    lsm_arena_init_sized(&a, 64);
    ASSERT_EQ(a.block_sizes[0], 64);

    lsm_arena_alloc(&a, 48);
    lsm_arena_alloc(&a, 48); /* triggers grow */
...
TEST(arena_reset_frees_blocks) {
    LSMArena a;
    lsm_arena_init_sized(&a, 128);

    lsm_arena_alloc(&a, 100);
    lsm_arena_alloc(&a, 100);
    ASSERT_GTE(a.nblocks, 2);
...
TEST(slab_tier1_malloc_backed) {
    ...
    void *p = lsm_slab_test_malloc(32);
    ASSERT_NOT_NULL(p);
    memset(p, 0x42, 32);
    ASSERT_EQ(((unsigned char *)p)[0], 0x42);
    ASSERT_EQ(((unsigned char *)p)[31], 0x42);

    lsm_slab_test_free(p);

    /* Re-alloc should reuse from free list */
    void *p2 = lsm_slab_test_malloc(32);
    ASSERT_NOT_NULL(p2);
    memset(p2, 0x43, 32);
    lsm_slab_test_free(p2);
...
TEST(slab_heap_alloc_and_free) {
    ...
    void *p = lsm_slab_test_malloc(200);
    ASSERT_NOT_NULL(p);
    memset(p, 0xAA, 200);
    ...
    void *ptrs[6];
    for (int i = 0; i < 6; i++) {
        ptrs[i] = lsm_slab_test_malloc(test_sizes[i]);
...
TEST(slab_reclaim_returns_memory) {
    ...
    for (int i = 0; i < 2048; i++) {
        ptrs[i] = lsm_slab_test_malloc(32);
        ASSERT_NOT_NULL(ptrs[i]);
    }
...
TEST(slab_realloc_slab_to_heap) {
    ...
    void *p = lsm_slab_test_malloc(32); /* slab */
    ASSERT_NOT_NULL(p);
    memset(p, 0x42, 32);

    void *p2 = lsm_slab_test_realloc(p, 200); /* heap */
    ASSERT_NOT_NULL(p2);
...
TEST(slab_calloc_zeroed) {
    ...
    void *p = lsm_slab_test_calloc(1, 200);
    ASSERT_NOT_NULL(p);
    unsigned char *bytes = (unsigned char *)p;
    int nonzero = 0;
    for (int i = 0; i < 200; i++) {
...
TEST(slab_mixed_alloc_free_stress) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        ptrs[i] = lsm_slab_test_malloc(sizes[i]);
        ASSERT_NOT_NULL(ptrs[i]);
...
TEST(slab_destroy_thread_with_live_chunk_no_uaf) {
    ...
    void *p = lsm_slab_test_malloc(48); /* ≤64B → slab chunk */
    ASSERT_NOT_NULL(p);
    memset(p, 0x7E, 48);
...
TEST(slab_cross_thread_free_is_safe) {
    ...
    lsm_thread_t t;
    ASSERT_EQ(lsm_thread_create(&t, 0, slab_cross_thread_free_worker, &ctx), 0);
    ASSERT_EQ(lsm_thread_join(&t), 0);
...
static int setup_mem_test_repo(void) {
    snprintf(g_mem_tmpdir, sizeof(g_mem_tmpdir), "/tmp/lsm_mem_XXXXXX");
    if (!lsm_mkdtemp(g_mem_tmpdir)) {
        return -1;
    }
...
static size_t count_retained_source_bytes(LSMFileResult **result_cache, int file_count,
                                          int *retained_count) {
...
TEST(parallel_extract_without_source_retention) {
    ...
    int file_count = 0;
    if (lsm_discover(g_mem_tmpdir, &opts, &files, &file_count) != 0) {
        teardown_mem_test_repo();
        FAIL("discover failed");
    }

    lsm_gbuf_t *gbuf = lsm_gbuf_new("mem-test", g_mem_tmpdir);
    ...
    ASSERT_GT(lsm_gbuf_node_count(gbuf), 0);
...
TEST(parallel_extract_with_slab) {
    ...
    LSMFileResult **result_cache = calloc(file_count, sizeof(LSMFileResult *));
    ASSERT_NOT_NULL(result_cache);

    int rc = lsm_parallel_extract(&ctx, files, file_count, result_cache, &shared_ids, 2);
    ASSERT_EQ(rc, 0);
...
TEST(mem_map_attributes_a_known_allocation) {
    ...
    lsm_mem_map_t before;
    lsm_mem_map_t after;
    ASSERT_TRUE(lsm_mem_map_collect(&before));
    ...
    ASSERT_TRUE(lsm_mem_map_collect(&after));
    ...
    for (int i = 0; i < LSM_MEM_MAP_BUCKETS; i++) {
        size_t limit = lsm_mem_map_bucket_limit(i);
...
SUITE(mem) {
    ...
    /* LSM_MEM_BUDGET_MB budget override */
