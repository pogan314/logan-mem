...
TEST(sem_tokenize_camel) {
    char *tokens[32];
    int n = lsm_sem_tokenize("parseUserInput", tokens, 32);
    ASSERT_GTE(n, 3);
...
TEST(sem_tokenize_snake) {
    char *tokens[32];
    int n = lsm_sem_tokenize("handle_http_request", tokens, 32);
    ASSERT_GTE(n, 3);
...
TEST(sem_tokenize_dot) {
    char *tokens[32];
    int n = lsm_sem_tokenize("net.http.client", tokens, 32);
    ASSERT_GTE(n, 3);
...
TEST(sem_tokenize_null) {
    int n = lsm_sem_tokenize(NULL, NULL, 0);
    ASSERT_EQ(n, 0);
    PASS();
}
...
TEST(sem_tokenize_max_out) {
    char *tokens[3];
    int n = lsm_sem_tokenize("a_b_c_d_e_f_g", tokens, 3);
    ASSERT_EQ(n, 3);
    for (int i = 0; i < n; i++)
        free(tokens[i]);
    PASS();
...
TEST(sem_tokenize_abbrev_expansion) {
    char *tokens[32];
    int n = lsm_sem_tokenize("getCtxErrMsg", tokens, 32);
...
static void fill_vec(lsm_sem_vec_t *v, float val) {
...
TEST(sem_cosine_null) {
    ASSERT_FLOAT_EQ(lsm_sem_cosine(NULL, NULL), 0.0f, 0.001f);
    PASS();
}
...
TEST(sem_normalize_unit) {
    ...
    v.v[0] = 1.0f;
    lsm_sem_normalize(&v);
    ASSERT_FLOAT_EQ(lsm_sem_cosine(&v, &v), 1.0f, 0.001f);
    PASS();
}
...
TEST(sem_normalize_null) {
    lsm_sem_normalize(NULL); /* should not crash */
    PASS();
}
...
TEST(sem_vec_add_scaled_basic) {
    ...
    lsm_sem_vec_t src;
    fill_vec(&src, 1.0f);
    lsm_sem_vec_add_scaled(&dst, &src, 0.5f);
    ASSERT_FLOAT_EQ(dst.v[0], 0.5f, 0.001f);
    ASSERT_FLOAT_EQ(dst.v[LSM_SEM_DIM - 1], 0.5f, 0.001f);
    PASS();
}
...
TEST(sem_vec_add_scaled_null) {
    lsm_sem_vec_t v;
    fill_vec(&v, 1.0f);
    lsm_sem_vec_add_scaled(NULL, &v, 1.0f); /* should not crash */
    lsm_sem_vec_add_scaled(&v, NULL, 1.0f); /* should not crash */
    PASS();
}
...
TEST(sem_random_index_deterministic) {
    lsm_sem_vec_t a, b;
    lsm_sem_random_index("hello", &a);
    lsm_sem_random_index("hello", &b);
    ASSERT_FLOAT_EQ(lsm_sem_cosine(&a, &b), 1.0f, 0.001f);
    PASS();
}

TEST(sem_random_index_different_tokens) {
    lsm_sem_vec_t a, b;
    lsm_sem_random_index("function", &a);
    lsm_sem_random_index("variable", &b);
...
TEST(sem_proximity_same_file) {
    float p = lsm_sem_proximity("src/main.c", "src/main.c");
    ASSERT_FLOAT_EQ(p, 1.1f, 0.01f); /* LSM_SEM_UNIT_POS + LSM_SEM_PROX_MAX_BOOST */
    PASS();
}

TEST(sem_proximity_same_dir) {
    /* Files sharing 1 of 2 directory components: ratio = 0.5 → 1.0 + 0.5*0.10 = 1.05 */
    float p = lsm_sem_proximity("src/core/a.c", "src/io/b.c");
    ASSERT_TRUE(p > 1.0f && p < 1.10f);
    PASS();
}

TEST(sem_proximity_different_paths) {
    float p = lsm_sem_proximity("src/foo/a.c", "tests/bar/b.c");
    ASSERT_FLOAT_EQ(p, 1.0f, 0.01f);
    PASS();
}

TEST(sem_proximity_null) {
    ASSERT_FLOAT_EQ(lsm_sem_proximity(NULL, "foo.c"), 1.0f, 0.01f);
    ASSERT_FLOAT_EQ(lsm_sem_proximity("foo.c", NULL), 1.0f, 0.01f);
    PASS();
}
...
TEST(sem_diffuse_single_neighbor) {
    ...
    v.v[0] = 0.5f;
    v.v[1] = 0.5f;
    lsm_sem_normalize(&v); /* unit-length input */
    lsm_sem_vec_t nb;
    memset(&nb, 0, sizeof(nb));
    nb.v[0] = 1.0f;
    lsm_sem_normalize(&nb);
    lsm_sem_diffuse(&v, &nb, 1, 0.3f);
...
TEST(sem_corpus_new_free) {
    lsm_sem_corpus_t *c = lsm_sem_corpus_new();
    ASSERT_NOT_NULL(c);
    ASSERT_EQ(lsm_sem_corpus_doc_count(c), 0);
    ASSERT_EQ(lsm_sem_corpus_token_count(c), 0);
    lsm_sem_corpus_free(c);
    PASS();
}
...
TEST(sem_corpus_add_one_doc) {
    ...
    lsm_sem_corpus_add_doc(c, tokens, 3);
    ASSERT_EQ(lsm_sem_corpus_doc_count(c), 1);
    ASSERT_TRUE(lsm_sem_corpus_token_count(c) > 0);
    lsm_sem_corpus_free(c);
    PASS();
}
...
TEST(sem_corpus_idf) {
    ...
    lsm_sem_corpus_add_doc(c, doc1, 3);
    lsm_sem_corpus_add_doc(c, doc2, 3);
    /* IDF for "a" (appears in 2 docs): log(2/2) = log(1) = 0 */
    float idf_a = lsm_sem_corpus_idf(c, "a");
    ASSERT_TRUE(idf_a < 0.01f);
    /* IDF for "b" (appears in 1 doc): log(2/1) > 0 */
    float idf_b = lsm_sem_corpus_idf(c, "b");
...
TEST(sem_corpus_add_null_doc) {
    lsm_sem_corpus_t *c = lsm_sem_corpus_new();
    ASSERT_NOT_NULL(c);
    lsm_sem_corpus_add_doc(c, NULL, 0);
    lsm_sem_corpus_add_doc(c, NULL, -1);
    ASSERT_EQ(lsm_sem_corpus_doc_count(c), 0);
    lsm_sem_corpus_free(c);
    PASS();
}
...
TEST(sem_corpus_free_null) {
    lsm_sem_corpus_free(NULL); /* should not crash */
    PASS();
}
...
static void *rotsq_concurrent_first_encode(void *opaque) {
    ...
    float vec[LSM_RSQ_IN_DIM];
    for (int i = 0; i < LSM_RSQ_IN_DIM; i++) {
        vec[i] = ctx->value + (float)i / (float)LSM_RSQ_IN_DIM;
    }
    atomic_fetch_add_explicit(ctx->ready, 1, memory_order_release);
    while (atomic_load_explicit(ctx->start, memory_order_acquire) == 0) {
        lsm_usleep(1000);
    }
    lsm_rsq_encode(vec, &ctx->code);
    return NULL;
}
...
TEST(sem_rotsq_concurrent_first_encode) {
    ...
    bool started0 = lsm_thread_create(&threads[0], 0, rotsq_concurrent_first_encode, &ctx[0]) == 0;
    bool started1 = lsm_thread_create(&threads[1], 0, rotsq_concurrent_first_encode, &ctx[1]) == 0;
...
TEST(sem_rotsq_ip_error_bounds) {
    enum { N = 64 };
    static float vecs[N][LSM_RSQ_IN_DIM];
    static lsm_rsq_code_t codes[N];
    ...
    for (int i = 0; i < N; i++) {
        for (int j = i; j < N; j++) {
            ...
            double est = (double)lsm_rsq_ip(&codes[i], &codes[j]);
    ...
    double self_est = (double)lsm_rsq_ip(&codes[0], &codes[0]);
