...
static bool index_supervisor_test_poll_terminal(lsm_index_worker_handle_t *handle,
                                                uint32_t timeout_ms,
                                                const lsm_index_worker_result_t **result_out) {
    uint64_t deadline = lsm_now_ms() + timeout_ms;
    do {
        lsm_index_worker_poll_t state = lsm_index_worker_poll(handle, result_out);
    ...
    return lsm_index_worker_poll(handle, result_out) == LSM_INDEX_WORKER_POLL_TERMINAL;
}

static bool index_supervisor_test_wait_file(lsm_index_worker_handle_t *handle, const char *path,
                                            char *out, size_t out_size) {
    uint64_t deadline = lsm_now_ms() + INDEX_SUPERVISOR_TEST_READY_MS;
    do {
        ...
        const lsm_index_worker_result_t *unexpected = NULL;
        if (lsm_index_worker_poll(handle, &unexpected) != LSM_INDEX_WORKER_POLL_RUNNING) {
            return false;
        }
        index_supervisor_test_pause();
    } while (lsm_now_ms() < deadline);
...
static void index_supervisor_test_dump(const char *label, const char *path) {
    ...
    FILE *file = path && path[0] ? lsm_fopen(path, "rb") : NULL;
    if (!file) {
...
static void index_supervisor_test_cleanup_handle(lsm_index_worker_handle_t *handle) {
    if (!handle) {
        return;
    }
    (void)lsm_index_worker_request_cancel(handle);
    const lsm_index_worker_result_t *result = NULL;
...
static void index_supervisor_test_capture_log(const char *line, void *context) {
    ...
    if (capture->render_progress) {
        lsm_progress_sink_fn(line);
    }
}

TEST(index_supervisor_worker_argv_requires_exact_build_bound_grammar) {
    const char *captured = lsm_index_supervisor_build_fingerprint();
    ASSERT_NOT_NULL(captured);
    ASSERT_EQ(strlen(captured), LSM_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH);

    char mismatched[LSM_INDEX_WORKER_BUILD_FINGERPRINT_SIZE];
    ...
    char *valid[] = {
        ...
        LSM_INDEX_WORKER_MARKER_ARG,
        "/tmp/m",
        LSM_INDEX_WORKER_QUARANTINE_ARG,
        "/tmp/q",
        NULL,
    };
    lsm_index_worker_invocation_t invocation;
    ASSERT_EQ(lsm_index_worker_parse_process_argv(16, valid, &invocation),
              LSM_INDEX_WORKER_ARGV_VALID);
    ASSERT_EQ(lsm_daemon_process_role(16, valid), LSM_DAEMON_PROCESS_WORKER);
    ...
    ASSERT_EQ(lsm_index_worker_parse_process_argv(7, missing_build, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_daemon_process_role(7, missing_build), LSM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(9, wrong_build, &invocation),
              LSM_INDEX_WORKER_ARGV_BUILD_MISMATCH);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(13, reordered, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(10, trailing, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(11, zero_budget, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_daemon_process_role(11, zero_budget), LSM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(11, overflow_budget, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_daemon_process_role(11, overflow_budget), LSM_DAEMON_PROCESS_INVALID);
    ASSERT_EQ(lsm_index_worker_parse_process_argv(5, user_value, &invocation),
              LSM_INDEX_WORKER_ARGV_INVALID);
    ASSERT_EQ(lsm_daemon_process_role(5, user_value), LSM_DAEMON_PROCESS_INVALID);
    PASS();
}
...
TEST(index_supervisor_async_jobs_are_isolated_cancellable_and_terminal_cached) {
    ...
    ASSERT_EQ(lsm_daemon_process_role(14, worker_argv), LSM_DAEMON_PROCESS_WORKER);

    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-async-XXXXXX", lsm_tmpdir());
    ...
    const char *old_single = getenv("LSM_INDEX_SINGLE_THREAD");
    const char *old_marker = getenv("LSM_INDEX_MARKER_FILE");
    const char *old_quarantine = getenv("LSM_INDEX_QUARANTINE_FILE");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    char *saved_single = old_single ? lsm_strdup(old_single) : NULL;
    char *saved_marker = old_marker ? lsm_strdup(old_marker) : NULL;
    char *saved_quarantine = old_quarantine ? lsm_strdup(old_quarantine) : NULL;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);
    (void)lsm_setenv("LSM_INDEX_SINGLE_THREAD", "parent-single", 1);
    ...
    uint64_t start_before = lsm_now_ms();
    int first_rc =
        lsm_index_worker_start_with_log(args, 4096, true, marker_a, quarantine_a,
                                        index_supervisor_test_capture_log, &first_capture, &first);
    int second_rc = lsm_index_worker_start_with_log(args, 8192, true, marker_b, quarantine_b,
                                                    index_supervisor_test_capture_log,
                                                    &second_capture, &second);
    uint64_t start_elapsed = lsm_now_ms() - start_before;
    ...
    if (first) {
        (void)snprintf(response_a, sizeof(response_a), "%s", lsm_index_worker_response_path(first));
        (void)snprintf(log_a, sizeof(log_a), "%s", lsm_index_worker_log_path(first));
    }
    if (second) {
        (void)snprintf(response_b, sizeof(response_b), "%s",
                       lsm_index_worker_response_path(second));
        (void)snprintf(log_b, sizeof(log_b), "%s", lsm_index_worker_log_path(second));
    }
    ...
    bool parent_unchanged = strcmp(getenv("LSM_INDEX_SINGLE_THREAD"), "parent-single") == 0 &&
                            strcmp(getenv("LSM_INDEX_MARKER_FILE"), "parent-marker") == 0 &&
                            strcmp(getenv("LSM_INDEX_QUARANTINE_FILE"), "parent-quarantine") == 0;
    ...
    const lsm_index_worker_result_t *premature = (const lsm_index_worker_result_t *)1;
    uint64_t poll_before = lsm_now_ms();
    lsm_index_worker_poll_t running =
        first ? lsm_index_worker_poll(first, &premature) : LSM_INDEX_WORKER_POLL_ERROR;
    uint64_t poll_elapsed = lsm_now_ms() - poll_before;
    uint64_t cancel_before = lsm_now_ms();
    bool first_cancel = first && lsm_index_worker_request_cancel(first);
    bool second_cancel = second && lsm_index_worker_request_cancel(second);
    uint64_t cancel_elapsed = lsm_now_ms() - cancel_before;

    const lsm_index_worker_result_t *first_result = NULL;
    const lsm_index_worker_result_t *second_result = NULL;
    ...
    bool cached_result = first_terminal &&
                         lsm_index_worker_poll(first, &cached) == LSM_INDEX_WORKER_POLL_TERMINAL &&
                         cached == first_result;
    ...
    bool terminal_cancel_rejected = first && !lsm_index_worker_request_cancel(first);
    bool response_cleaned = lsm_file_size(response_a) < 0 && lsm_file_size(response_b) < 0;
    bool failure_logs_kept = lsm_file_size(log_a) >= 0 && lsm_file_size(log_b) >= 0;

    if (first_terminal) {
        lsm_index_worker_destroy(first);
    } else {
        index_supervisor_test_cleanup_handle(first);
    }
    if (second_terminal) {
        lsm_index_worker_destroy(second);
    } else {
    ...
    (void)lsm_unlink(log_a);
    (void)lsm_unlink(log_b);
    (void)lsm_unlink(marker_a);
    (void)lsm_unlink(marker_b);
    index_supervisor_test_restore_env("LSM_CACHE_DIR", saved_cache);
    index_supervisor_test_restore_env("LSM_INDEX_SINGLE_THREAD", saved_single);
    index_supervisor_test_restore_env("LSM_INDEX_MARKER_FILE", saved_marker);
    index_supervisor_test_restore_env("LSM_INDEX_QUARANTINE_FILE", saved_quarantine);
    ...
    ASSERT_EQ(running, LSM_INDEX_WORKER_POLL_RUNNING);
...
TEST(index_supervisor_sync_wrapper_forwards_cancel_and_drains_tree) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-sync-cancel-XXXXXX", lsm_tmpdir());
    ASSERT_NOT_NULL(lsm_mkdtemp(cache));
    const char *old_cache = getenv("LSM_CACHE_DIR");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);
    ...
    lsm_index_worker_result_free(&result);
    index_supervisor_test_restore_env("LSM_CACHE_DIR", saved_cache);
    (void)th_rmtree(cache);
...
static bool index_supervisor_test_run_probe(const char *mode, bool profiling,
    ...
    (void)snprintf(log_path, sizeof(log_path), "%s", lsm_index_worker_log_path(handle));
    (void)snprintf(response_path, sizeof(response_path), "%s",
                   lsm_index_worker_response_path(handle));
    const lsm_index_worker_result_t *result = NULL;
    bool terminal =
    ...
    if (terminal && result) {
        ...
        *log_exists_out = lsm_file_size(log_path) >= 0;
        *response_path_exists_out = lsm_file_size(response_path) >= 0;
        lsm_index_worker_destroy(handle);
    } else {
    ...
    if (lsm_file_size(log_path) >= 0) {
        (void)lsm_unlink(log_path);
    }
    return terminal;
}
...
TEST(index_supervisor_terminal_log_lifecycle_matches_outcome_and_profiling) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-logs-XXXXXX", lsm_tmpdir());
    ASSERT_NOT_NULL(lsm_mkdtemp(cache));
    const char *old_cache = getenv("LSM_CACHE_DIR");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    bool saved_profile = lsm_profile_active;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);

    lsm_proc_outcome_t clean_outcome = LSM_PROC_SPAWN_FAILED;
    lsm_proc_outcome_t profile_outcome = LSM_PROC_SPAWN_FAILED;
    lsm_proc_outcome_t crash_outcome = LSM_PROC_SPAWN_FAILED;
...
#ifdef _WIN32
    bool crash_classified_failure = crash_outcome != LSM_PROC_CLEAN;
#else
    bool crash_classified_failure = crash_outcome == LSM_PROC_CRASH;
#endif

    lsm_profile_active = saved_profile;
    index_supervisor_test_restore_env("LSM_CACHE_DIR", saved_cache);
    (void)th_rmtree(cache);

    ASSERT_TRUE(clean_terminal);
    ASSERT_EQ(clean_outcome, LSM_PROC_CLEAN);
    ASSERT_TRUE(clean_response);
    ASSERT_FALSE(clean_log);
    ASSERT_FALSE(clean_response_file);
    ASSERT_TRUE(profile_terminal);
    ASSERT_EQ(profile_outcome, LSM_PROC_CLEAN);
...
TEST(index_supervisor_drains_terminal_backlog_into_request_progress_callback) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-relay-XXXXXX", lsm_tmpdir());
    ASSERT_NOT_NULL(lsm_mkdtemp(cache));
    const char *old_cache = getenv("LSM_CACHE_DIR");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);
    ...
    lsm_proc_outcome_t outcome = result ? result->outcome : LSM_PROC_SPAWN_FAILED;
    int callbacks_at_terminal = capture.delivered;
    const lsm_index_worker_result_t *cached = NULL;
    bool terminal_cached =
        terminal && lsm_index_worker_poll(handle, &cached) == LSM_INDEX_WORKER_POLL_TERMINAL &&
    ...
    ASSERT_EQ(outcome, LSM_PROC_CLEAN);
...
TEST(index_supervisor_oversized_response_is_contained_and_log_is_retained) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-oversize-XXXXXX", lsm_tmpdir());
    ASSERT_NOT_NULL(lsm_mkdtemp(cache));
    const char *old_cache = getenv("LSM_CACHE_DIR");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);
    ...
    bool rejected = terminal && result && result->response_rejected && !result->response &&
                    result->outcome == LSM_PROC_EXIT_NONZERO && result->tree_quiesced &&
                    !result->supervision_failed;
    bool log_retained = log_path[0] && lsm_file_size(log_path) >= 0;
    bool response_removed = response_path[0] && lsm_file_size(response_path) < 0;
    if (terminal) {
        lsm_index_worker_destroy(handle);
    } else {
...
TEST(index_supervisor_killed_worker_log_is_never_empty_and_names_the_run) {
    char cache[INDEX_SUPERVISOR_TEST_PATH_CAP];
    (void)snprintf(cache, sizeof(cache), "%s/lsm-index-logheader-XXXXXX", lsm_tmpdir());
    ASSERT_NOT_NULL(lsm_mkdtemp(cache));
    const char *old_cache = getenv("LSM_CACHE_DIR");
    char *saved_cache = old_cache ? lsm_strdup(old_cache) : NULL;
    (void)lsm_setenv("LSM_CACHE_DIR", cache, 1);
    ...
    long log_size = log_path[0] ? lsm_file_size(log_path) : -1;
    char log_text[8192] = {0};
    FILE *log = log_path[0] ? lsm_fopen(log_path, "rb") : NULL;
    if (log) {
    ...
    bool has_header =
        strstr(log_text, "\"event\":\"" LSM_INDEX_WORKER_LOG_START_EVENT "\"") != NULL;
    bool names_repo =
    ...
    ASSERT_EQ(start_rc, 0);
    ASSERT_TRUE(terminal);
    ASSERT_TRUE(outcome != LSM_PROC_CLEAN); /* killed: the supervisor keeps the log */
