    ...
    if (( windows )); then
        ...
        if [[ -n "${LSM_CI_TEMP_ROOT:-}" ]]; then
            parent="$(cygpath -u "$LSM_CI_TEMP_ROOT")" ||
                { _cbm_test_runtime_error "cannot convert LSM_CI_TEMP_ROOT"; return 1; }
            [[ -d "$parent" && ! -L "$parent" ]] ||
                { _cbm_test_runtime_error "LSM_CI_TEMP_ROOT is not a real directory"; return 1; }
...
_cbm_test_runtime_daemon() {
    LSM_RUNTIME_DIR="$_CBM_TEST_RUNTIME_PRODUCT_RUNTIME" \
        LSM_CACHE_DIR="$_CBM_TEST_RUNTIME_PRODUCT_CACHE" "$1" daemon "$2"
}
