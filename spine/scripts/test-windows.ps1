<#
.SYNOPSIS
    Run the native-Windows product-surface test suite for logan-spine-mcp.
...
.DESCRIPTION
    ...
    Two categories of test:
      ...
      GUARDS      - regression guards for Windows bugs already fixed on main.
                    ...
                    regressed and fails this runner.
                      ...
                      * test_non_ascii_cache_dump.py guards #996 (writer lsm_fopen)
    ...
    under the mandatory coordination daemon LSM_INDEX_SUPERVISOR=0 is a
...
.PARAMETER Target
    Makefile.lsm target used when building: 'lsm-with-ui' (default; needed for the
    drive-picker guard's embedded HTTP UI) or 'lsm' (no UI - the drive guard then
...
.EXAMPLE
    pwsh -File scripts/test-windows.ps1 -GuardsOnly -Binary build\c\logan-spine-mcp.exe
#>
[CmdletBinding()]
param(
    [string]$Binary,
    [ValidateSet("lsm-with-ui", "lsm")]
    [string]$Target = "lsm-with-ui",
    [switch]$GuardsOnly,
    [string]$Make = "make"
)
...
function Resolve-Binary {
    ...
    $built = Join-Path $repoRoot "build\c\logan-spine-mcp.exe"
    if (Test-Path $built) { return $built }
    Write-Host "Building $Target via Makefile.lsm ..." -ForegroundColor Cyan
    & $Make "-j" "-f" "Makefile.lsm" $Target "SANITIZE=" "TMP=$tmp" "TEMP=$tmp" "TMPDIR=$tmp" | Out-Host
...
try {
    ...
    $guardRoot = Join-Path $userProfile ("lsm-windows-guards-root-" + [guid]::NewGuid().ToString("N"))
    ...
    $guardBundle = Join-Path $guardRoot ("lsm-windows-guards-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $guardBundle | Out-Null
    $guardBin = Join-Path $guardBundle "logan-spine-mcp.exe"
...
# UI (build target lsm-with-ui); against a non-UI binary it reports a precondition
...
if ($guardSkips.Count -gt 0) {
    ...
    Write-Host "needs a UI build (-Target lsm-with-ui, the default)." -ForegroundColor Yellow
}
