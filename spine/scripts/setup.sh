...
# logan-spine-mcp setup script (macOS + Linux)
...
SOURCE_DIR="$HOME/.local/share/logan-spine-mcp"
...
download_binary() {
    ...
    local asset="logan-spine-mcp-${platform}.tar.gz"
...
build_from_source() {
    ...
    (cd "$SOURCE_DIR" && scripts/build.sh && cp build/c/logan-spine-mcp "${INSTALL_DIR}/${BINARY_NAME}")
...
configure_claude() {
    ...
    printf "%s" "${BOLD}Configure Claude Code to use logan-spine-mcp? [y/N] ${RESET}"
    read -r answer
    if [[ ! "$answer" =~ ^[Yy]$ ]]; then
        echo ""
...
JSONEOF
    ...
    if command -v jq &>/dev/null; then
        ...
        if [ -f "$settings_file" ]; then
            local tmp
            tmp=$(mktemp)
            jq --argjson entry "$mcp_entry" '.mcpServers["logan-spine-mcp"] = $entry' "$settings_file" > "$tmp"
            mv "$tmp" "$settings_file"
        else
            echo "{}" | jq --argjson entry "$mcp_entry" '.mcpServers["logan-spine-mcp"] = $entry' > "$settings_file"
        fi
...
if os.path.exists(path):
    with open(path) as f:
        data = json.load(f)
data.setdefault('mcpServers', {})['logan-spine-mcp'] = json.loads('$mcp_entry')
...
echo ""
echo "${BOLD}logan-spine-mcp installer${RESET}"
echo ""
...
info "  rm -rf ~/.cache/logan-spine-mcp/  # graph database"
