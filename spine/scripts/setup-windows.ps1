# logan-spine-mcp setup script (Windows)
...
$BinaryName = "logan-spine-mcp"
$InstallDir = Join-Path $env:LOCALAPPDATA "logan-spine-mcp"
...
function Configure-ClaudeCode($McpConfig) {
    Write-Host ""
    $answer = Read-Host "Configure Claude Code to use logan-spine-mcp? [y/N]"
    ...
    if ($answer -match '^[Yy]$') {
        ...
        $settings["mcpServers"]["logan-spine-mcp"] = $McpConfig
        ...
        Write-Host ""
        $snippet = @{ mcpServers = @{ "logan-spine-mcp" = $McpConfig } }
...
Write-Host ""
Write-Host "logan-spine-mcp installer (Windows)" -ForegroundColor White
Write-Host ""
...
if ($FromSource) {
    ...
    # Clone or update
    Write-Host ""
    $sourceDir = "/home/$wslUser/.local/share/logan-spine-mcp"
    try {
    ...
    Invoke-WSL "mkdir -p /home/$wslUser/.local/bin && cd $sourceDir && scripts/build.sh && cp build/c/logan-spine-mcp $wslBinaryPath"
    ...
    Write-Host "    wsl.exe -- rm -rf ~/.cache/logan-spine-mcp/"
    ...
    $release = Invoke-RestMethod -Uri $releaseUrl -Headers @{ "User-Agent" = "logan-spine-mcp-setup" }
    ...
    $asset = "logan-spine-mcp-windows-amd64.zip"
    ...
    Write-Host "    Remove-Item -Recurse -Force `"$env:LOCALAPPDATA\logan-spine-mcp`"  # graph database"
}
