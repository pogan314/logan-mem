...
| `.claude-plugin/marketplace.json` | Makes this repository a Claude Code plugin marketplace named `logan-mem`, listing `./plugins/logan-spine`. The plugin key is `logan-spine@logan-mem`, formed from that plugin name and this marketplace name. Only the local-directory route (`claude plugin marketplace add <path>`) has been exercised; the design intends one file to serve a `pogan314/logan-mem` add too, but that has not been tried. **One thing is now known about that untried route without trying it:** a `directory` source is local, but a GitHub source is *external*, and per code.claude.com/docs/en/settings-reference#enabledplugins an externally-sourced plugin that only a project's `.claude/settings.json` enables "doesn't install it for other people" — Claude Code reports it as not installed until each user runs `claude plugin install`. So over GitHub the committed `enabledPlugins` entry stops being sufficient on its own, and the per-repository step becomes mandatory rather than a convenience. | Code |
