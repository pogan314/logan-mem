...
- **Installation has two halves, and they are different scopes.** Per machine: `plugins/logan-spine/scripts/install.sh` builds the engine, places the binary at `~/.local/bin/logan-spine-mcp`, puts that directory on `PATH`, sets `auto_index`, and registers this repository as the `logan-mem` marketplace. It enables the plugin nowhere. Per repository: `claude plugin install logan-spine@logan-mem --scope project`, which writes `enabledPlugins` into that repository's `.claude/settings.json`; a repository without that entry gets nothing. Per repository again, and unchanged from 01: the index at `~/.cache/logan-spine-mcp/<project>.db`, built on the first session there because `auto_index` is true, skipped above 50,000 tracked files. Still ask the owner before installing on a machine that does not have it.
...
- **Editing a file under `plugins/logan-spine/` changes nothing in a running session.** The plugin Claude Code runs is a cached copy at `~/.claude/plugins/cache/logan-mem/logan-spine/<version>/`, keyed by the manifest version, so an edit is invisible until you bump `version` in `plugins/logan-spine/.claude-plugin/plugin.json`, then `claude plugin marketplace update logan-mem`, then `claude plugin update logan-spine@logan-mem --scope project`, then `/reload-plugins`. **The `--scope project` is required, not optional:** `claude plugin update` defaults to `--scope user`, and run bare against this project-scope install it exits 1 with `Plugin "logan-spine" is not installed at scope user` — measured 2026-08-24. Skip the version bump and the update has nothing new to resolve to, so you go on testing the stale copy. The update does not rename the cache directory either; it creates a new one and leaves the old beside it, so read `~/.claude/plugins/installed_plugins.json` to see which copy is actually in use. `claude --plugin-dir plugins/logan-spine` loads the tree in place for one session and is the faster loop while iterating; use the four-step loop when you want to measure what a normal install produces.
- **The marketplace is registered against whichever checkout held `.claude-plugin/marketplace.json` when `install.sh` ran, and right now that is the worktree.** `install.sh` prefers the main checkout but falls back to the current one and says so, because while this work sat on a branch the main checkout had no marketplace file. Verified 2026-08-24: both `~/.claude/plugins/known_marketplaces.json` and `~/.claude/settings.json` name `.../logan-mem/.worktrees/plugin-packaging`. A worktree is deleted at merge, and a marketplace pinned to a path that no longer exists stops resolving — **after merging this branch, re-run `claude plugin marketplace add` (or `install.sh`) from the main checkout.** `add` is idempotent and re-points an existing name at a new path.
