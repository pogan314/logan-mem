...
- **Running `spine/scripts/test.sh --suites cli` kills any live logan-spine MCP session on this machine.** The suite exercises upstream's real install/uninstall activation path, which quiesces active sessions — it prints "Stopping active LSM sessions and operations" 29 times in one run. Verified 2026-08-22 by probe: a running MCP server survived a binary swap and died during the cli suite. Nothing is broken when this happens; the daemon logs `reason=last_committed_client_disconnected` and a new Claude Code session reconnects. Expect it, and do not run the cli suite mid-session if you are relying on the spine's tools in that session.
