...
| 7 | `plugins/logan-spine/tests/run.sh` passes | `plugins/logan-spine/tests/run.sh` in the worktree, at manifest version 1.0.0, 2026-08-24 09:15 CDT | ~~199 `ok` lines, zero failures, `exit=0`~~ **Superseded**, re-measured post-merge on the main checkout, manifest version 1.2.0, 2026-08-24 12:29 CDT: 231 `ok` lines, zero `FAIL` lines, zero `skip` lines, `exit=0` | Shared runtime, after cutover. Uses stubs and a fixture `HOME`; never touches the real `$HOME` | **Verified.** The suite grew by 32 checks between 1.0.0 and 1.2.0 from the follow-up fixes (`467e6fc`, `0fae388`, `de2700d`, `bed3cf7`), each of which added its own coverage. |
