...
- **This condition can recur, and the trigger is known.** Rebuilding or reinstalling `logan-spine-mcp` while other sessions still hold live connections to a daemon of the older build strands every new session until the last of those old clients disconnects. `~/.cache/logan-spine-mcp/logs/daemon-conflicts.ndjson` and `lsm-daemon.log` identify it in seconds. See `docs/superpowers/02/follow-ups.md` item 1, which this makes more urgent: `install.sh` performs exactly that rebuild-and-replace.
