...
1. A project-scope `enabledPlugins` entry alone does **not** load the plugin. The marketplace must first be registered under `$HOME` with `claude plugin marketplace add`; after that, the committed entry is sufficient on its own. **Resolved 2026-08-24 from the official docs (code.claude.com/docs/en/settings-reference#extraknownmarketplaces), and the question was a false dichotomy.** The two files are not alternatives: `extraKnownMarketplaces` is a *declaration* — "Claude Code registers each marketplace it doesn't already know" — and `~/.claude/plugins/known_marketplaces.json` is the registry that declaration produces. So nothing depends on one *instead of* the other; the settings key is an input to the registry, and `marketplace add` simply writes both ends at once. Two operational consequences do follow, and neither was recorded before:

  - **`extraKnownMarketplaces` in a repository's own `.claude/settings.json` is trust-gated.** Claude Code honours it "only after you accept the workspace trust dialog for that folder; in a folder you haven't trusted, including a `-p` run there, it ignores them without a message." This repository's committed `.claude/settings.json` carries only `enabledPlugins`, so this does not bite here — but it is the reason a repository cannot bootstrap its own marketplace silently, and it is why the per-machine half of the install exists.
  - **An externally-sourced plugin enabled only by project settings does not install for anyone else.** "Enabling a plugin from an external source such as a GitHub repository or npm package in a project's `.claude/settings.json` doesn't install it for other people. On every path that loads plugins, Claude Code reports the plugin as not installed until each user installs it themselves." Our marketplace is a `directory` source pointing at a local checkout, so this does not affect the exercised route. It directly affects the **unexercised** one: if this marketplace is ever added as `pogan314/logan-mem` over GitHub, the committed `enabledPlugins` entry stops being sufficient on its own and every user needs `claude plugin install`.
...
| Every test measures a stale cached copy of the plugin | The development loop is specified, every editing task bumps `version`, and success criterion 9 asserts the installed copy's version segment matches the manifest. **Measured 2026-08-24, this risk did not materialise and could not have:** a `directory`-source marketplace loads the plugin from the repository tree, so no test ever read the cache. The cache copies exist but nothing resolves to them. |
...
| 7 | `plugins/logan-spine/tests/run.sh` passes | `plugins/logan-spine/tests/run.sh` in the worktree, at manifest version 1.0.0, 2026-08-24 09:15 CDT | ~~199 `ok` lines, zero failures, `exit=0`~~ **Superseded**, re-measured post-merge on the main checkout, manifest version 1.2.0, 2026-08-24 12:29 CDT: 231 `ok` lines, zero `FAIL` lines, zero `skip` lines, `exit=0`; re-measured again at manifest version 1.3.0, 2026-08-24 14:0x CDT, after the visualizer work: **265 `ok` lines, zero `FAIL`, `exit=0`** | Shared runtime, after cutover. Uses stubs and a fixture `HOME`; never touches the real `$HOME` | **Verified.** The suite grew by 32 checks between 1.0.0 and 1.2.0 from the follow-up fixes (`467e6fc`, `0fae388`, `de2700d`, `bed3cf7`), and by a further 34 between 1.2.0 and 1.3.0 from the visualizer work and the defects it exposed — the octal permission guard, the arg parser, and the skill's `project` contract — each of which added its own coverage. |
...
### Every hook, measured in a live session — 2026-08-24

All five handlers were exercised end to end after the cutover, on the shared runtime, with a warm daemon.

| Hook | Event | Result |
|---|---|---|
| `session-reminder.sh` | SessionStart | **Fires.** `Hook SessionStart ("${CLAUDE_PLUGIN_ROOT}"/hooks/session-reminder.sh) provided additionalContext (733 chars)` |
| `subagent-reminder.sh` | SubagentStart | **Fires.** Same shape, 733 chars, and the three agents receive Tier 1, Tier 2 and Tier 3 respectively |
| `docstring-check.sh` | PostToolUse `Edit\|Write` | **Fires.** `Hook PostToolUse:Edit (PostToolUse) error: logan-spine: add docstrings before moving on:` here, and nothing at all in a control repository |
| `code-discovery-gate.sh` | PostToolUse `Read` | **Fires, and is correctly silent on a clean file.** The engine returns nothing when coverage is `no_recorded_issue`, `outside_project` or `invalid_path` (`spine/src/cli/hook_augment.c`), which is the documented "nothing to add" contract |
| `code-discovery-gate.sh` | PreToolUse `Grep\|Glob` | **Works when invoked; cannot be exercised live on this machine.** Given a Grep payload it returns `[logan-spine] … 1 graph symbol(s) match "lsm_bin"`. But `Grep` and `Glob` are absent from this environment's tool registry entirely — `ToolSearch "select:Grep,Glob"` returns "No matching deferred tools found", and **no** `PreToolUse` hook from **any** plugin fires in these sessions. That is an environment fact, not a plugin defect, and the matcher is kept because it works wherever those tools exist |

**A warm daemon is required for any of the hook-augment hooks to speak, and the installer now provides one.** A hook connects to a daemon but never spawns one, giving up after 250 ms, while a session's own MCP server takes about 6.3 s to start — so before this fix the first SessionStart of every fresh session lost that race and the hooks looked broken. `install.sh` step 5 now runs `daemon start`, which creates a permanent daemon that outlives sessions.
...
- ~~**A `claude -p` session in this worktree does not receive the plugin's MCP tools, cause unknown.**~~ **Resolved 2026-08-24, and the cause IS established.** It was the engine's own build-admission guard, not anything in the plugin.

  The mechanism, from source: `spine/src/daemon/version_cohort.h:1` describes "crash-safe exact-build admission across LSM processes", and `spine/src/daemon/service.c:195-232` is the comparison — semantic version, then build fingerprint — with the exact `snprintf` that produced the observed text. The engine refuses to start a new process while a daemon of a **different build** is resident.

  The evidence, measured: `~/.cache/logan-spine-mcp/logs/daemon-conflicts.ndjson` holds **63** `daemon.version_conflict` records, every one `active_version 0.10.8-logan.4` build `bb71bffe…` against `requested_version 0.10.8-logan.4-46-gdc37c42` build `fc9a1cc5…`, spanning 19:25:54 to 21:20:39 CDT on 2026-08-23. Claude Code's own per-server log at `~/.cache/claude-cli-nodejs/…/mcp-logs-plugin-logan-spine-spine/2026-08-24T03-00-34-527Z.jsonl` records the rejection reaching the client: `Server stderr: logan-spine-mcp: LSM could not start because a conflicting LSM process is active …` then `Connection failed after 2256ms (CONNECTION_CLOSED)`.

  **Two claims made earlier in this document and in the task reports were wrong, and are corrected here.**

  1. "Not even a failed start was logged." A failed start **was** logged, in Claude Code's persistent per-server log. It was absent only from the session `--debug-file` that was inspected at the time.
  2. "The debug log never mentions `spine-launch`." That was never evidence of anything. The string `spine-launch` appears **zero** times in successful runs too — Claude Code logs stdio servers by their configured key, never by the resolved command path, and no other stdio server's command appears either. The real tell would have been the absence of the `MCP server "plugin:logan-spine:spine": Starting connection` line.

  It was also **not a timeout**. The rejection came in 2256 ms; Claude Code's 30000 ms budget, configurable with `MCP_TIMEOUT` (checked against the official docs at code.claude.com/docs/en/mcp-quickstart), never expired.

  Current state, measured: 8 of 8 fresh `claude -p` sessions connected and returned the real 10-project list, including 3 launched simultaneously, each connecting in about 8 s and completing `list_projects` in 20-23 ms. No daemon is resident between sessions any more — the log now shows each one starting and exiting on `last_committed_client_disconnected`.

- **This condition can recur, and the trigger is known.** Rebuilding or reinstalling `logan-spine-mcp` while other sessions still hold live connections to a daemon of the older build strands every new session until the last of those old clients disconnects. `~/.cache/logan-spine-mcp/logs/daemon-conflicts.ndjson` and `lsm-daemon.log` identify it in seconds. **No longer reachable through `install.sh`, as of commit `0fae388`:** it no longer swaps the file itself but delegates to the engine's own `install --force --skip-config`, which asks the resident daemon to drain, takes the version-cohort mutation lock, stages out of line and publishes atomically (`plugins/logan-spine/scripts/install.sh` lines 27-33; `docs/superpowers/02/follow-ups.md` item 1, now FIXED). The hazard still exists for anyone who replaces the binary by hand — a `cp` or `mv` over `~/.local/bin/logan-spine-mcp` while another session holds a connection reproduces it in 30 seconds — so publish through the engine, never by copying.
...
- ~~**No AGENT has been observed reaching the graph on the shared runtime.**~~ **Closed 2026-08-24.** All three agents were dispatched on the shared runtime and each called the prefixed graph tools successfully, at Tier 1, Tier 2 and Tier 3 respectively. See row 5.
...
- ~~**This worktree has no index in the real cache.**~~ **Superseded 2026-08-24.** It has one: `~/.cache/logan-spine-mcp/home-ubuntu-projects-org-logan-mem-.worktrees-plugin-packaging.db`, 160 MB, built 09:37, listed by `list_projects` as its own project with its own `root_path`. A worktree always gets its own independent index, and this is structural rather than incidental: `lsm_project_name_from_path` at `spine/src/pipeline/fqn.c:416-517` derives the project key from the canonicalized absolute path with non-alphanumerics mapped to dashes, and never calls into git — so a worktree's differing path unavoidably yields a different database. The observation was true when written, before the current binary was installed and `auto_index` took effect.
- ~~**The control-repository half of the docstring criterion has not been re-measured since the version-01 nudge was removed.**~~ **Closed 2026-08-24, re-measured after cutover.** In a fresh `mktemp -d` + `git init` control repository with no `.claude` directory, a session deleted a docstring line with the `Edit` tool and **no** nudge appeared — the version-01 global hook that used to fire there is gone. In this repository the same edit produced `Hook PostToolUse:Edit (PostToolUse) error: logan-spine: add docstrings before moving on:` in the session's debug log. Both halves now hold with only the plugin's hook installed, and the attribution no longer depends on telling two identical messages apart by script path.

  A note for anyone re-running this: the nudge is a `PostToolUse` hook, so its text reaches the transcript rather than the process's stdout. A `claude -p` run whose final answer is captured will not show it. Read the session's `--debug-file` and grep for `Hook PostToolUse:Edit`. A first attempt at this measurement wrongly read a clean stdout as the hook failing to fire.
