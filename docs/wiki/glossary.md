...
sources: [live Claude Code docs via claude-code-docs MCP (code.claude.com/docs/en/glossary, /en/hooks.mdx, /en/skills.mdx, /en/plugins-reference.mdx) verified 2026-08-21, general knowledge of established database/IR/CS terms, CLAUDE.md for this repo's own usage of "memory" and "spine", spine/LOGAN-CHANGES.md and docs/superpowers/01/specs/2026-08-21-spine-v1-design.md for the version-01 spine terms added 2026-08-22, docs/superpowers/02/specs/2026-08-23-plugin-packaging-design.md and a direct read of spine/src/daemon/version_cohort.c and spine/src/cli/hook_augment.c for the version-02 plugin/marketplace/daemon/evidence-tier terms added 2026-08-24]
---
...
| Daemon (in this repo) | A `logan-spine-mcp` process started with `daemon start` that keeps running after the session that started it ends, so a later session's MCP server connects to a warm process instead of paying the ~6.3s startup cost. `plugins/logan-spine/scripts/install.sh` starts one per machine; without it, a hook's 250ms connect budget loses the race on a session's first `SessionStart` and the hooks go quiet. |
| Docstring coverage | What fraction of a codebase's files and symbols have a docstring. In this repo, `plugins/logan-spine/scripts/docstring-coverage.sh` computes it by running the spine's `docstrings` subcommand over every tracked file. |
...
| Evidence tier | One of three fixed levels of graph-verification effort this repo's plugin routes a subagent to: Tier 1 quick (`scout`), Tier 2 verification (`verify`, the default), Tier 3 full graph verification (`auditor`). The engine's `ha_active_tier` (`spine/src/cli/hook_augment.c`) picks the tier by exact-matching `SubagentStart`'s `agent_type` against the bare or `logan-spine-`-prefixed name, but a plugin agent's real `agent_type` is colon-scoped (`logan-spine:scout`) and matches neither — a hook-side prefix strip is required to route `scout`/`auditor` correctly; anything unmatched defaults to Tier 2 (`docs/superpowers/02/specs/2026-08-23-plugin-packaging-design.md`). |
...
| Index (in this repo) | The per-project graph database `logan-spine-mcp` builds and queries, stored at `~/.cache/logan-spine-mcp/<project>.db`. Built on a repository's first session because `auto_index` is on, skipped above 50,000 tracked files; the daemon's file watcher keeps a watched project's index refreshed afterward. |
...
| Marketplace | A `.claude-plugin/marketplace.json` file at a repo root that lists one or more plugins and where to fetch each one from. A user registers a marketplace once (`claude plugin marketplace add <path\|repo\|url>`), then installs a named plugin from it (`claude plugin install <plugin>@<marketplace>`). This repo is itself a marketplace named `logan-mem`, listing the one plugin `plugins/logan-spine/`. |
...
| Plugin tool-name prefix | The naming pattern Claude Code gives every tool from a plugin-bundled MCP server: `mcp__plugin_<plugin-name>_<server-key>__<tool-name>`. This repo's spine tools are `mcp__plugin_logan-spine_spine__<tool>`, never `mcp__logan-spine-mcp__<tool>` — a hook matcher, agent tool allowlist, or permission rule written against the bare server key never fires. |
...
| Version cohort | The `logan-spine-mcp` daemon's mechanism for coordinating which build of the binary currently holds the daemon and its cache, using a short transition lock plus a process-lifetime shared/exclusive barrier, so an old daemon is drained rather than collided with when a new build starts (`spine/src/daemon/version_cohort.c`). `install.sh` relies on this when it publishes a new binary via `install --force --skip-config`. |
